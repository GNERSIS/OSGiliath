...
class MessageBox
{
    protected:
        ...
        osgWidget::Frame*
        createButtonOk( const std::string& theme,
                        const std::string& text,
                        const std::string& font,
                        int                fontSize );
        osgWidget::Label*
                                       createLabel( const std::string&      string,
                                                    const std::string&      font,
                                                    int                     size,
    ...
    public:
        ...
        bool
        create( const std::string& themeMessage,
...
struct AlphaSetterVisitor : public osg::DualModeVisitor
{
        float _alpha;

        AlphaSetterVisitor( float alpha = 1.0 ) :
        ...
        apply( osg::MatrixTransform& node )
        {
            ...
            if( win )
            {
                ...
                for( osgWidget::Window::Iterator it = win->begin(); it != win->end();
                     it++ )
                {
                    // osgWidget::warn() << "   I am operating on Widget: " <<
                    // it->get()->getName() << std::endl;
...
struct ColorSetterVisitor : public osg::DualModeVisitor
{
...
struct EventOK : public osgWidget::Callback,
                 osg::NodeCallback
{
        ...
        EventOK( osgWidget::Frame* frame ) :
            osgWidget::Callback( osgWidget::EVENT_ALL ),
            _frame( frame )
        {
        ...
        operator()( osgWidget::Event& ev )
        {
            if( ev.type == osgWidget::EVENT_MOUSE_ENTER )
            {
                ...
                //_frame->setMatrix(osg::scale(2, 2, 1) * _frame->getMatrix());
                _frame->setScale(
                    1.1F
                );                   // osg::scale(2, 2, 1) * _frame->getMatrix());
                _frame->update();    // osg::scale(2, 2, 1) * _frame->getMatrix());
                    ...
                    osg::NodeVisitor* nv )
        {
            if( nv->getVisitorType() == osg::NodeVisitor::UPDATE_VISITOR )
            {
                ...
                if( _frame.valid() )
                {
                    ...
                    osgWidget::Color c =
                        _defaultColor + ( ( _overColor - _defaultColor ) * value );
...
osgWidget::Label*
MessageBox::createLabel( const std::string&      string,
                         const std::string&      font,
                         int                     size,
                         const osgWidget::Color& color )
{
...
osgWidget::Frame*
MessageBox::createButtonOk( const std::string& theme,
                            const std::string& text,
                            const std::string& font,
                            int                fontSize )
{
    osg::ref_ptr<osgWidget::Frame> frame =
        osgWidget::Frame::createSimpleFrameFromTheme( "ButtonOK",
    ...
    osgWidget::Label* label =
        createLabel( text, font, fontSize, osgWidget::Color( 0, 0, 0, 1 ) );
...
bool
MessageBox::create( const std::string& themeMessage,
                    const std::string& themeButton,
                    const std::string& titleText,
                    const std::string& messageText,
                    const std::string& buttonText,
                    ...
                    int                fontSize )
{
    ...
    osgWidget::Label* labelText =
        createLabel( messageText, font, fontSize, osgWidget::Color( 0, 0, 0, 1 ) );
    osgWidget::Label* labelTitle =
        createLabel( titleText, font, fontSize + 5, osgWidget::Color( 0.4, 0, 0, 1 ) );
      ...
      char** argv )
{
    ...
    message.getWindow()->setPosition(
        osgWidget::Point( std::round( ox ),
                          std::round( oy ),
                          message.getWindow()->getPosition()[2] )
    );
    // frame->resizeAdd(30, 30);

    // AlphaSetterVisitor alpha(.8f);
    // frame->accept(alpha);
    return osgWidget::createExample( viewer,
                                     wm );    // osgDB::readNodeFile("duck.glb"));
}
...
{
    float _alpha;
    AlphaSetterVisitor( float alpha = 1.0):osg::DualModeVisitor(TRAVERSE_ALL_CHILDREN) { _alpha = alpha;}
...
{
    osgWidget::Color _color;
    ColorSetterVisitor( const osgWidget::Color& color):osg::DualModeVisitor(TRAVERSE_ALL_CHILDREN) { _color = color;}
