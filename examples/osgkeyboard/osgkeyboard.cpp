 ...
 * osgkeyboard example application
 */
...
void
KeyboardModel::keyChange( int key,
                          int virtualKey,
                          int value )
{
    osg::notify( osg::INFO ) << "key value change, code=" << std::hex << key
...
osg::Switch*
KeyboardModel::addKey( osg::vec3&         pos,
                       int                key,
                       const std::string& text,
                       float              width,
                       float              height )
...
KeyboardModel::createKeyboard()
{
    ...
    osg::Geode* geodeInput  = new osg::Geode;
    {
        ...
        _inputText->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
class KeyboardEventHandler : public osgGA::GUIEventHandler
{
    ...
    #define PRINT( mask )                                                   \
        osg::notify( osg::NOTICE )                                          \
            << #mask << " =" << ( ea.getModKeyMask() & mask ) << std::endl;
            ...
            switch( ea.getEventType() )
            {
                case( osgGA::GUIEventAdapter::KEYDOWN ) :
                    {
                        _keyboardModel->keyChange( ea.getKey(),
                                                   ea.getUnmodifiedKey(),
                                                   1 );
                        return true;
                    }
