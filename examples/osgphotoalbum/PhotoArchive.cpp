...
bool
PhotoArchive::readPhotoIndex( const std::string& filename )
{
...
void
PhotoArchive::getImageFileNameList( FileNameList& filenameList )
{
...
PhotoArchive::readImage( const std::string& filename,
                         ...
                         float&             original_t )
{
         ...
         ++itr )
    {
        if( filename == itr->filename )
        {
            ...
            if( target_s <=
                photoHeader.thumbnail_s &&
                target_t <=
                photoHeader.thumbnail_t &&
                photoHeader.thumbnail_position != 0 )
            {
                osgDB::ifstream in( _archiveFileName.c_str(),
                ...
                image->setImage( photoHeader.thumbnail_s,
                                 photoHeader.thumbnail_t,
                                 1,
                                 imageHeader.pixelFormat,
                                 imageHeader.pixelFormat,
                                 imageHeader.type,
                                 data,
                ...
                photoHeader.fullsize_position != 0 )
            {
                ...
                osg::ref_ptr<osg::Image> image = new osg::Image;
                image->setImage( photoHeader.fullsize_s,
                                 photoHeader.fullsize_t,
                                 1,
                                 imageHeader.pixelFormat,
                                 imageHeader.pixelFormat,
...
void
PhotoArchive::buildArchive( const std::string&  filename,
                            ...
                            bool /*compressed*/ )
{
    ...
    for( FileNameList::const_iterator fitr = imageList.begin(); fitr != imageList.end();
         ++fitr )
    {
        PhotoHeader header;
    ...
    std::cout << "Building photo archive containing " << photoIndex.size() << " pictures"
              << std::endl;
    ...
    for( PhotoIndexList::iterator pitr = photoIndex.begin(); pitr != photoIndex.end();
         ++pitr, ++photoCount )
    {
        PhotoHeader& photoHeader = *pitr;

        std::cout << "Processing image " << photoCount << " of " << photoIndex.size()
                  << " filename=" << photoHeader.filename << std::endl;
...

        {
            ...
            unsigned int newTotalSize =
                image->computeRowWidthInBytes( thumbnailSize,
            ...
            if( !newData )
            {
                ...
                osg::notify( osg::FATAL )
                    << "Error scaleImage() did not succeed : out of memory."
            ...
            GLint status         = osg::gluScaleImage( &psm,
                                                       image->getPixelFormat(),
                                                       image->s(),
                                                       image->t(),
                                                       image->getDataType(),
            ...
            if( status != 0 )
            {
                delete[] newData;
                osg::notify( osg::WARN )
                    << "Error scaleImage() did not succeed : errorString = "
                    << osg::gluErrorString( ( GLenum )status ) << std::endl;
                return;
            }
        ...
        {
            std::cout << "    creating fullsize image...";
            std::cout.flush();

            photoHeader.fullsize_s = std::min( ( unsigned int )image->s(), maximumSize );
            photoHeader.fullsize_t = std::min( ( unsigned int )image->t(), maximumSize );
            photoHeader.fullsize_position = ( unsigned int )out.tellp();

            // first need to rescale image to the require thumbnail size
            unsigned int newTotalSize =
                image->computeRowWidthInBytes( photoHeader.fullsize_s,
                                               image->getPixelFormat(),
                                               image->getDataType(),
                                               image->getPacking() ) *
                photoHeader.fullsize_t;
