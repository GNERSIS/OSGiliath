...
ImageReaderWriter::DataReference::DataReference( const std::string& fileName,
                                                 unsigned int       res,
                                                 float              width,
                                                 float              height,
                                                 bool               backPage ) :
...
std::string
ImageReaderWriter::local_insertReference( const std::string& fileName,
                                          unsigned int       res,
                                          float              width,
                                          float              height,
                                          bool               backPage )
{
    ...
    _dataReferences[myReference] =
        DataReference( fileName, res, width, height, backPage );
    return myReference;
}

osg::ref_ptr<osg::Image>
ImageReaderWriter::readImage_Archive( DataReference& dr,
                                      float&         s,
                                      float&         t )
{
         ...
         ++itr )
    {
        osg::ref_ptr<osg::Image> image =
            ( *itr )->readImage( dr._fileName, dr._resolutionX, dr._resolutionY, s, t );
        if( image )
        {
            return image;
        }
...
osg::ref_ptr<osg::Image>
ImageReaderWriter::readImage_DynamicSampling( DataReference& dr,
                                              float&         s,
                                              float&         t )
{
    // record previous options.
    osg::ref_ptr<osgDB::ReaderWriter::Options> previousOptions =
...
ImageReaderWriter::local_readNode( const std::string& fileName,
                                   const Options* )
{
    ...
    if( image )
    {
        ...
        float maxWidth    = osg::length( dr._maximumWidth );
        float maxHeight   = osg::length( dr._maximumHeight );
        ...
        osg::vec3 halfHeightVector( dr._maximumHeight *
                                    ( photoHeight * 0.5F / maxHeight ) );
