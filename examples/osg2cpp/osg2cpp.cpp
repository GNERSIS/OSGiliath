...
void
searchAndReplace( std::string&       str,
                  const std::string& spat,
                  const std::string& rpat )
{
             ...
             const std::string& variableName )
{
    ...
    if( !fout )
    {
        std::cout << "Error: could not open file `" << cppFileName << "` for writing."
                  << std::endl;
    }
    ...
    std::string variableString =
        std::string( "char " ) + variableName + std::string( "[] = " );
    ...
    else
    {
        ...
        fout << variableString << "\""
             << shaderSource.substr( startOfLine, endOfLine - startOfLine ) << "\\n\""
             << std::endl;
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        arguments.getApplicationName() +
        " is a utility for converting glsl shader files into char arrays that can be "
