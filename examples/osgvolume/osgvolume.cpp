...
osg::Image*
createTexture3D( osg::ImageList& imageList,
...
struct ScaleOperator
{
        ...
        ScaleOperator( float scale ) :
            _scale( scale )
        {
        }

        ScaleOperator( const ScaleOperator& so ) :
            _scale( so._scale )
        {
        }

        ScaleOperator&
        operator=( const ScaleOperator& so )
        {
            _scale = so._scale;
            return *this;
        }
...
void
clampToNearestValidPowerOfTwo( int& sizeX,
                               int& sizeY,
                               int& sizeZ,
                               int  s_maximumTextureSize,
                               int  t_maximumTextureSize,
         ...
         const std::string& raw_filename )
{
    ...
    switch( numberOfComponents )
    {
        ...
        default :
            osg::notify( osg::NOTICE )
                << "Error: numberOfComponents=" << numberOfComponents
                << " not supported, only 1,2,3 or 4 are supported." << std::endl;
            return 0;
    }
    ...
    switch( numberBytesPerComponent )
    {
        ...
        default :
            osg::notify( osg::NOTICE )
                << "Error: numberBytesPerComponent=" << numberBytesPerComponent
                << " not supported, only 1,2 or 4 are supported." << std::endl;
            return 0;
    }
    ...
    int sizeT                = sizeY;
    int sizeR                = sizeZ;
    clampToNearestValidPowerOfTwo( sizeS,
                                   sizeT,
                                   sizeR,
                                   s_maximumTextureSize,
                                   t_maximumTextureSize,
    ...
    bool endianSwap = ( osg::getCpuByteOrder() == osg::BigEndian ) ? ( endian != "big" )
                                                                   : ( endian == "big" );
...
class TestSupportOperation : public osg::GraphicsOperation
{
    public:
        ...
        operator()( osg::GraphicsContext* /*gc*/ )
        {
            ...
            osg::notify( osg::NOTICE )
                << "Max texture size=" << maximumTextureSize << std::endl;
        }
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--shader",
        "Use OpenGL Shading Language. (default)"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--multi-pass",
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--model",
        "load 3D model and insert into the scene along with the volume."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--hull",
        "load 3D hull that defines the extents of the region to volume render."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--no-shader",
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--gpu-tf",
        "Aply the transfer function on the GPU. (default)"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--cpu-tf",
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--mip",
        "Use Maximum Intensity Projection (MIP) filtering."
    );
    arguments.getApplicationUsage()->addCommandLineOption( "--isosurface",
                                                           "Use Iso surface render." );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--light",
        "Use normals computed on the GPU to render a lit volume."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-n",
        "Use normals computed on the GPU to render a lit volume."
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--sequence-length <num>",
        "Set the length of time that a sequence of images with run for."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--sd <num>",
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--lod",
        "Enable techniques to reduce the level of detail when moving."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--bg r g b a",
        "Set the window background color(r,g,b,a) with each component the 0 to 1.0 range"
    );
    // arguments.getApplicationUsage()->addCommandLineOption("--raw <sizeX> <sizeY>
    // <sizeZ> <numberBytesPerComponent> <numberOfComponents> <endian> <filename>","read
    // a raw image data");
...

    {
        ...
        flightManipulator->setYawControlMode(
            osgGA::FlightManipulator::NO_AUTOMATIC_YAW
        );
    ...
    {
        transferFunction =
            osgDB::readRefFile<osg::TransferFunction1D>( tranferFunctionFile );
    }
...

    {
        ...
        {
            OSG_NOTICE << "Warning: -s option no longer supported." << std::endl;
            invalidOption = true;
        }
        ...
        {
            OSG_NOTICE << "Warning: --clip option no longer supported." << std::endl;
            invalidOption = true;
        }
    ...
    while( arguments.read( "--modulate-alpha-by-colour",
                           colourModulate.x,
                           colourModulate.y,
                           colourModulate.z,
                           colourModulate.w ) )
    ...
    while( arguments.read( "--vh", vh_filename ) )
    {
        ...
        {
            images.push_back(
                readRaw( xdim, ydim, zdim, 1, 1, "little", raw_filename )
            );
        }
        ...
        if( !transfer_filename.empty() )
        {
            ...
            if( fin )
            {
                ...
                while( fin && value <= 1.0 )
                {
                    ...
                    if( fin )
                    {
                        colorMap[value] = osg::vec4( red / 255.0F,
                                                     green / 255.0F,
                                                     blue / 255.0F,
                                                     alpha / 255.0F );
                        std::cout << "value = " << value << " (" << red << ", " << green
                                  << ", " << blue << ", " << alpha << ")";
                ...
                if( colorMap.empty() )
                {
                    std::cout << "Error: No values read from transfer function file: "
                              << transfer_filename << std::endl;
                    return 0;
                }
    ...
    while( arguments.read( "--raw",
                           ...
                           sizeY,
                           sizeZ,
                           numberBytesPerComponent,
                           numberOfComponents,
                           endian,
                           raw_filename ) )
    {
        images.push_back( readRaw( sizeX,
                                   sizeY,
                                   sizeZ,
                                   numberBytesPerComponent,
                                   numberOfComponents,
    ...
    if( images_pos >= 0 )
    {
        ...
        for( ; pos < arguments.argc() && !arguments.isOption( pos ); ++pos )
        {
            ...
            if( arg.find( '*' ) != std::string::npos )
            {
                ...
                for( unsigned int i = 0; i < contents.size(); ++i )
                {
                    osg::ref_ptr<osg::Image> image =
                        osgDB::readRefImageFile( contents[i] );

                    if( image )
                    {
                        OSG_NOTICE
                            << "Read osg::Image FileName::" << image->getFileName()
                            << ", pixelFormat=0x" << std::hex << image->getPixelFormat()
                            << std::dec << ", s=" << image->s() << ", t=" << image->t()
                            << ", r=" << image->r() << std::endl;
            ...
            else
            {
                ...
                osg::ref_ptr<osg::Image> image =
                    osgDB::readRefImageFile( arguments[pos] );

                if( image )
                {
                    OSG_NOTICE << "Read osg::Image FileName::" << image->getFileName()
    ...
    for( int pos = 1; pos < arguments.argc(); ++pos )
    {
        if( !arguments.isOption( pos ) )
        {
            ...
            else
            {
                ...
                {
                    osg::ref_ptr<osg::Image> image =
                        osgDB::readRefImageFile( filename + ".dicom" );
                    if( image )
                    {
                        images.push_back( image );
                    }
                ...
                {
                    osg::notify( osg::NOTICE )
                        << "Error: could not find file: " << filename << std::endl;
                    return 1;
                }
            }
        }
    ...
    if( images.empty() )
    {
        std::cout << "No model loaded, please specify a volumetric image file on the "
                     "command line."
                  << std::endl;
        return 1;
    }
    ...
    for( ; sizeItr != images.end(); ++sizeItr )
    {
        if( ( *sizeItr )->s() !=
            ...
            ( *sizeItr )->r() != image_r )
        {
            std::cout << "Images in sequence are not of the same dimensions."
                      << std::endl;
            return 1;
        }
    }
    ...
    if( !matrix )
    {
        ...
        matrix = new osg::RefMatrix( xSize,
                                     0.0,
                                     0.0,
                                     0.0,
                                     0.0,
    ...
    if( xMultiplier != 1.0 || yMultiplier != 1.0 || zMultiplier != 1.0 )
    {
        osg::postMultScale(
            *matrix,
            osg::dvec3( fabs( xMultiplier ), fabs( yMultiplier ), fabs( zMultiplier ) )
        );
    }
    ...
    if( colourSpaceOperation != osg::NO_COLOR_SPACE_OPERATION )
    {
        for( Images::iterator itr = images.begin(); itr != images.end(); ++itr )
        {
            ( *itr ) = osg::colorSpaceConversion( colourSpaceOperation,
                                                  itr->get(),
                                                  colourModulate );
        }
    }
    ...
    if( !gpuTransferFunction && transferFunction.valid() )
    {
        ...
        {
            *itr =
                osgVolume::applyTransferFunction( itr->get(), transferFunction.get() );
        }
    }
    ...
    {
        osg::notify( osg::NOTICE )
            << "Single image " << images.size() << " volumes." << std::endl;
        image_3d = images.front();
    }
    else
    {
        osg::notify( osg::NOTICE )
            << "Creating sequence of " << images.size() << " volumes." << std::endl;
    ...
    osg::ref_ptr<osgVolume::ImageLayer> layer =
        new osgVolume::ImageLayer( image_3d.get() );
    ...
    {
        layer->setLocator(
            new osgVolume::Locator( osg::translate( xMultiplier < 0.0 ? -1.0 : 0.0,
                                                    yMultiplier < 0.0 ? -1.0 : 0.0,
                                                    zMultiplier < 0.0 ? -1.0 : 0.0 ) *
                                    osg::scale( xMultiplier < 0.0 ? -1.0 : 1.0,
                                                yMultiplier < 0.0 ? -1.0 : 1.0,
                                                zMultiplier < 0.0 ? -1.0 : 1.0 ) *
                                    ( *matrix ) )
        );
        ;
    }
    ...
    if( useShader )
    {
        ...
        osgVolume::SampleRatioWhenMovingProperty* srwm =
            sampleRatioWhenMoving != 0.0
                ? new osgVolume::SampleRatioWhenMovingProperty( sampleRatioWhenMoving )
                : 0;

        osgVolume::TransparencyProperty* tp = new osgVolume::TransparencyProperty( 1.0 );
        osgVolume::TransferFunctionProperty* tfp =
            transferFunction.valid()
                ? new osgVolume::TransferFunctionProperty( transferFunction.get() )
                : 0;
    ...
    if( !outputFile.empty() )
    {
        ...
        if( ext == "osg" || ext == "osgt" || ext == "osgx" )
        {
            if( image_3d.valid() )
            {
                ...
                osg::ref_ptr<osgDB::Options> options =
                    new osgDB::Options( "ddsNoAutoFlipWrite" );
                ;
                osgDB::writeImageFile( *image_3d,
                                       image_3d->getFileName(),
                                       options.get() );
            }
        ...
        {
            std::cout << "Extension not support for file output, not file written."
                      << std::endl;
        }
    ...
    if( volume.valid() )
    {
        ...
        if( useManipulator )
        {
            ...
            dragger->setMatrix( tile->getLocator()->getTransform() *
                                osg::translate( 0.5, 0.5, 0.5 ) );
        ...
        // if we want to do multi-pass volume rendering we need decorate the whole scene
        // with a VolumeScene node.
        if( useMultipass )
        {
