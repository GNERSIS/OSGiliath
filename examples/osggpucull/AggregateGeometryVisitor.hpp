...
// AggregateGeometryVisitor uses ConvertTrianglesOperator to create and fill
// osg::Geometry with data matching users needs
struct ConvertTrianglesOperator : public osg::Referenced
{
        ...
        virtual void
        initGeometry( osg::Geometry* outputGeometry ) = 0;

        virtual bool
        pushNode( osg::Node* /*node*/ )
        {
            return false;
        }
        ...
        popNode() = 0;
        virtual void
        setGeometryData( const osg::dmat4& matrix,
                         osg::Geometry*    inputGeometry,
                         osg::Geometry*    outputGeometry,
                         float             typeID,
                         float             lodNumber ) = 0;
        virtual void
        operator()( unsigned int i1,
                    unsigned int i2,
                    unsigned int i3 ) = 0;
};
...
// aggregated geometry with standard set of vertex attributes : vertices, normals, color
// and texCoord0. texCoord1 holds additional information about vertex ( typeID,
// lodNumber, boneIndex )
struct ConvertTrianglesOperatorClassic : public ConvertTrianglesOperator
{
        ...
        virtual void
        initGeometry( osg::Geometry* outputGeometry )
        {
            osg::Vec3Array* vertices = new osg::Vec3Array;
            outputGeometry->setVertexArray( vertices );
            osg::Vec4Array* colors = new osg::Vec4Array;
            outputGeometry->setColorArray( colors, osg::Array::BIND_PER_VERTEX );
            osg::Vec3Array* normals = new osg::Vec3Array;
            outputGeometry->setNormalArray( normals, osg::Array::BIND_PER_VERTEX );
            osg::Vec2Array* texCoords0 = new osg::Vec2Array;
            outputGeometry->setTexCoordArray( 0, texCoords0 );
            osg::Vec3Array* texCoords1 = new osg::Vec3Array;
            outputGeometry->setTexCoordArray( 1, texCoords1 );
                         ...
                         float             lodNumber )
        {
            ...
            _inputVertices =
                dynamic_cast<osg::Vec3Array*>( inputGeometry->getVertexArray() );
            _inputColors =
                dynamic_cast<osg::Vec4Array*>( inputGeometry->getColorArray() );
            _inputNormals =
                dynamic_cast<osg::Vec3Array*>( inputGeometry->getNormalArray() );
            _inputTexCoord0 = inputGeometry->getTexCoordArray( 0 );

            _outputVertices =
                dynamic_cast<osg::Vec3Array*>( outputGeometry->getVertexArray() );
            _outputColors =
                dynamic_cast<osg::Vec4Array*>( outputGeometry->getColorArray() );
            _outputNormals =
                dynamic_cast<osg::Vec3Array*>( outputGeometry->getNormalArray() );
            _outputTexCoord0 =
                dynamic_cast<osg::Vec2Array*>( outputGeometry->getTexCoordArray( 0 ) );
            _outputTexCoord1 =
                dynamic_cast<osg::Vec3Array*>( outputGeometry->getTexCoordArray( 1 ) );
        ...
        operator()( unsigned int i1,
                    ...
                    unsigned int i3 )
        {
            unsigned int ic1 = i1, ic2 = i2, ic3 = i3, in1 = i1, in2 = i2, in3 = i3,
                         it01 = i1, it02 = i2, it03 = i3;
            ...
            _outputVertices->push_back( osg::vec3( _inputVertices->at( i1 ) *
                                                   _matrix ) );
            _outputVertices->push_back( osg::vec3( _inputVertices->at( i2 ) *
                                                   _matrix ) );
            _outputVertices->push_back( osg::vec3( _inputVertices->at( i3 ) *
                                                   _matrix ) );
            ...
            {
                _outputNormals->push_back( osg::transform3x3( _inputNormals->at( in1 ),
                                                              _matrix ) );
                _outputNormals->push_back( osg::transform3x3( _inputNormals->at( in2 ),
                                                              _matrix ) );
                _outputNormals->push_back( osg::transform3x3( _inputNormals->at( in3 ),
                                                              _matrix ) );
            }
            else
            {
            ...
            for( unsigned int i = 0; i < 3; ++i )
            {
                _outputTexCoord1->push_back(
                    osg::vec3( _typeID, _lodNumber, _boneIndices.back() )
                );
            }
        }

        void
        registerBoneByName( const std::string& boneName,
                            int                boneIndex )
        {
...
class AggregateGeometryVisitor : public osg::DualModeVisitor
{
    public:
        ...
        // osg::TriangleIndexFunctor uses its template parameter as a base class, so we
        // must use an adapter pattern to hack it
        struct ConvertTrianglesBridge
        {
                inline void
                setConverter( ConvertTrianglesOperator* cto )
                {
                    _converter = cto;
                }
                ...
                setGeometryData( const osg::dmat4& matrix,
                                 ...
                                 float             lodNumber )
                {
                    _converter->setGeometryData( matrix,
                                                 inputGeometry,
                                                 outputGeometry,
                                                 typeID,
                                                 lodNumber );
        ...
        // struct returning information about added object ( first vertex, vertex count,
        // primitiveset index ) used later to create indirect command texture buffers
        struct AddObjectResult
        {
                AddObjectResult( unsigned int f,
                                 unsigned int c,
                                 unsigned int i ) :
                    first( f ),
                    count( c ),
