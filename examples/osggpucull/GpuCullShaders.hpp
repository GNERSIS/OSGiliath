 ...
 * GpuCullShaders example application
 */
...
char SHADER_STATIC_CULL_VERTEX[] =
    ...
    "uniform int indirectCommandSize; // = sizeof(DrawArraysIndirectCommand) / "
    ...
    "    vec4 bbMin;\n"
    "    vec4 bbMax;\n"
    "    ivec4 indirectTargetParams;    // x=targetID, y=indexInTarget, "
    "z=offsetInTarget, w=lodMaxQuantity\n"
    "    vec4 distances;               // x=minDistance, y=minFadeDistance, "
    "z=maxFadeDistance, w=maxDistance\n"
    "};\n"
    "\n"
    ...
    "{\n"
    "    vec4 box[8];\n"
    "    box[0] = matrix * vec4( bbMax.x, bbMax.y, bbMax.z, 1.0);\n"
    "    box[1] = matrix * vec4( bbMin.x, bbMax.y, bbMax.z, 1.0);\n"
    "    box[2] = matrix * vec4( bbMax.x, bbMin.y, bbMax.z, 1.0);\n"
    "    box[3] = matrix * vec4( bbMin.x, bbMin.y, bbMax.z, 1.0);\n"
    "    box[4] = matrix * vec4( bbMax.x, bbMax.y, bbMin.z, 1.0);\n"
    "    box[5] = matrix * vec4( bbMin.x, bbMax.y, bbMin.z, 1.0);\n"
    "    box[6] = matrix * vec4( bbMax.x, bbMin.y, bbMin.z, 1.0);\n"
    "    box[7] = matrix * vec4( bbMin.x, bbMin.y, bbMin.z, 1.0);\n"
    "\n"
    "    int outOfBound[6] = int[6]( 0, 0, 0, 0, 0, 0 );\n"
    "    for (int i=0; i<8; i++)\n"
    "    {\n"
    "        outOfBound[0] += int( box[i].x >  box[i].w );\n"
    "        outOfBound[1] += int( box[i].x < -box[i].w );\n"
    "        outOfBound[2] += int( box[i].y >  box[i].w );\n"
    "        outOfBound[3] += int( box[i].y < -box[i].w );\n"
    "        outOfBound[4] += int( box[i].z >  box[i].w );\n"
    "        outOfBound[5] += int( box[i].z < -box[i].w );\n"
    "    }\n"
    "    return (outOfBound[0] < 8 ) && ( outOfBound[1] < 8 ) && ( outOfBound[2] < 8 ) "
    "&& ( outOfBound[3] < 8 ) && ( outOfBound[4] < 8 ) && ( outOfBound[5] < 8 );\n"
    "}\n"
    "\n"
    "void main(void) \n"
    "{\n"
    ...
    "    if( boundingBoxInViewFrustum( mvpoMatrix, "
    "instanceTypes[instanceTypeIndex].bbMin.xyz, "
    "instanceTypes[instanceTypeIndex].bbMax.xyz ) )\n"
    "    {\n"
    "        fColor = vec4(1.0,0.0,0.0,1.0);\n"
    "        float distanceToObject = distance(osg_ViewMatrixInverse[3].xyz / "
    "osg_ViewMatrixInverse[3].w, instanceMatrix[3].xyz / instanceMatrix[3].w );\n"
    "        for(int i=0;i<instanceTypes[instanceTypeIndex].params.x;++i)\n"
    "        {\n"
    "            if( boundingBoxInViewFrustum( mvpoMatrix, "
    "instanceTypes[instanceTypeIndex].lods[i].bbMin.xyz, "
    "instanceTypes[instanceTypeIndex].lods[i].bbMax.xyz ) &&\n"
    "                ( distanceToObject >= "
    "instanceTypes[instanceTypeIndex].lods[i].distances.x ) && ( distanceToObject < "
    "instanceTypes[instanceTypeIndex].lods[i].distances.w ) )\n"
    "            {\n"
    "                fColor = vec4(1.0,1.0,0.0,1.0);\n"
    "                float fadeAlpha  = 4.0 * ( clamp( "
    "(distanceToObject-instanceTypes[instanceTypeIndex].lods[i].distances.x)/( "
    "instanceTypes[instanceTypeIndex].lods[i].distances.y - "
    "instanceTypes[instanceTypeIndex].lods[i].distances.x), 0.0, 1.0 ) \n"
    "                    +  clamp( (distanceToObject- "
    "instanceTypes[instanceTypeIndex].lods[i].distances.z)/( "
    "instanceTypes[instanceTypeIndex].lods[i].distances.w - "
    "instanceTypes[instanceTypeIndex].lods[i].distances.z), 0.0, 1.0 ) );\n"
    "                int sampleMask = ( 0xF0 >> int(fadeAlpha) ) & 0xF;\n"
    "                int indirectCommandIndex   = "
    "instanceTypes[instanceTypeIndex].lods[i].indirectTargetParams.x;\n"
    "                int indirectCommandAddress = "
    "instanceTypes[instanceTypeIndex].lods[i].indirectTargetParams.y;\n"
    "                int objectIndex            = imageAtomicAdd( getIndirectCommand( "
    "indirectCommandIndex ), indirectCommandAddress*indirectCommandSize+1, 1 );\n"
    "                int indirectTargetAddress  = "
    "6*(instanceTypes[instanceTypeIndex].lods[i].indirectTargetParams.z + "
    "objectIndex);\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress+0, M0 );\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress+1, M1 );\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress+2, M2 );\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress+3, M3 );\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress+4, ExtraParams );\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress+5, vec4(IdParams.x,IdParams.y,float(sampleMask),0.0) );\n"
    "            }\n"
    "        }\n"
    "    }\n"
    "}\n";
...
char SHADER_STATIC_DRAW_0_VERTEX[] =
    ...
    "   int indirectTargetAddress  = "
    "6*(instanceTypes[instanceTypeIndex].lods[instanceLodNumber].indirectTargetParams.z "
    "+ gl_InstanceID);\n"
...
char SHADER_STATIC_DRAW_0_FRAGMENT[] =
    "#version 460 core\n"
    "\n"
    "uniform vec4 lightPosition;   // eye-space direction (w=0)\n"
    ...
    "void main()\n"
    "{\n"
    "// simple fragment shader that calculates ambient + diffuse lighting\n"
    "    vec3 lightDir = normalize(vec3(lightPosition));\n"
    "    vec3 normal   = normalize(ecNormal);\n"
    "    float NdotL   = max(dot(normal, lightDir), 0.0);\n"
    "    FragColor =  NdotL * color * lightDiffuse +  color * lightAmbient;\n"
...
char SHADER_STATIC_DRAW_1_VERTEX[] =
    ...
    "   sMask = int(idParams.z);\n"
    "\n"
    "   // simple tree waving in the wind. Amplitude, frequency and offset are coded in "
    "extraParams\n"
    "   vec4 mPos = instanceMatrix * vec4(VertexPosition.xyz,1.0);\n"
    "   float wavingAmplitute = VertexPosition.z * extraParams.y;\n"
    "   mPos.xy += windDirection * wavingAmplitute * sin( extraParams.z*osg_FrameTime + "
    "extraParams.w );\n"
    "\n"
    "   mat4 mvMatrix = gl_ModelViewMatrix * instanceMatrix;\n"
    "   \n"
...
char SHADER_DYNAMIC_CULL_VERTEX[] =
    ...
    "uniform int dynamicInstancesDataSize; // = sizeof(DynamicInstance) / "
    "sizeof(osg::vec4)\n"
    "\n"
    "layout(R32I) coherent uniform iimageBuffer indirectCommand0;\n"
    "uniform int indirectCommandSize; // = sizeof(DrawArraysIndirectCommand) / "
    ...
    "};\n"
    "\n"
    "layout(location = 0) in vec3 VertexPosition; // xyz - used only for debugging "
    "purposes\n"
    "layout(location = 10) in vec4 InstanceID; // typeid, id - points to the data in "
    "dynamicInstancesData buffer\n"
    "\n"
    "bool boundingBoxInViewFrustum( in mat4 matrix, in vec3 bbMin, in vec3 bbMax )\n"
    "{\n"
    "    vec4 box[8];\n"
    ...
    "indirectCommandIndex ), indirectCommandAddress*indirectCommandSize+1, 1 );\n"
    "                int indirectTargetAddress  = "
    "instanceTypes[instanceTypeIndex].lods[i].indirectTargetParams.z + objectIndex;\n"
    "                ivec4 indirectCommandData  = ivec4( instanceTypeIndex, "
    "instanceIndex, sampleMask, 0 );\n"
    "                imageStore( getIndirectTarget(indirectCommandIndex), "
    "indirectTargetAddress, indirectCommandData );\n"
    "            }\n"
    "        }\n"
    "    }\n"
    "}\n";
...
char SHADER_DYNAMIC_DRAW_0_VERTEX[] =
    ...
    "   int indirectTargetAddress  = "
    "instanceTypes[instanceTypeIndex].lods[instanceLodNumber].indirectTargetParams.z + "
    "gl_InstanceID;\n"
    "\n"
    "   // if we know instance type, lod number and gl_InstanceID, we can reconstruct "
    "all instance data\n"
    ...
    "       texelFetch(dynamicInstancesData, instanceAddress + 3) );\n"
    "\n"
    "   vec4 instanceExtraParams = texelFetch(dynamicInstancesData, instanceAddress + "
    "4);\n"
