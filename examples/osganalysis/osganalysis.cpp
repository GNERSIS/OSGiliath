 ...
 * osganalysis example application
 */
...
class StripStateVisitor : public osg::DualModeVisitor
{
    public:

        StripStateVisitor( bool useStateSets,
...
class OptimizeImageVisitor : public osg::DualModeVisitor
{
    public:

        OptimizeImageVisitor( osgDB::ImageProcessor* imageProcessor,
                              bool                   compressImages,
        ...
        processStateSet( osg::StateSet* stateset )
        {
            ...
            for( unsigned int ti = 0; ti < stateset->getNumTextureAttributeLists();
                 ++ti )
            {
                osg::StateAttribute* sa =
                    stateset->getTextureAttribute( ti,
        ...
        proccessImage( osg::Image* image )
        {
            ...
            {
                OSG_NOTICE << "Will be using ImageProcessor to process image "
                           << image->getFileName() << std::endl;
            }
            else
            {
                OSG_NOTICE << "No ImageProcessor to process image "
                           << image->getFileName() << std::endl;
            }
...
class MemoryVisitor : public osg::DualModeVisitor
{
    public:
        ...
        reallocate()
        {
                 ...
                 ++itr )
            {
                ...
                osg::Geometry* newGeometry =
                    osg::clone( geometry, osg::CopyOp( osg::CopyOp::DEEP_COPY_ALL ) );
                 ...
                 ++itr, ++geom_itr )
            {
                ...
                for( Geodes::iterator gitr = geodes.begin(); gitr != geodes.end();
                     ++gitr )
                {
        ...
        osg::Array*
        cloneArray( ArrayVector& arrayVector,
                    osg::Array*  array )
        {
            if( !array )
            {
        ...
        osg::PrimitiveSet*
        clonePrimitiveSet( PrimitiveSetVector& psVector,
                           osg::PrimitiveSet*  ps )
        {
            if( !ps )
            {
            ...
            osg::PrimitiveSet* newPS =
                static_cast<osg::PrimitiveSet*>( ps->cloneType() );
        ...
        reallocate2()
        {
            ...
            {
                osg::Geometry*              geometry = itr->first;
                osg::ref_ptr<osg::Geometry> newGeometry =
                    osg::clone( geometry, osg::CopyOp::SHALLOW_COPY );
                newGeometries.push_back( newGeometry.get() );

                newGeometry->setVertexArray( cloneArray( arrayVector,
                                                         geometry->getVertexArray() ) );
                newGeometry->setNormalArray( cloneArray( arrayVector,
                                                         geometry->getNormalArray() ) );
                newGeometry->setColorArray( cloneArray( arrayVector,
                                                        geometry->getColorArray() ) );
                newGeometry->setSecondaryColorArray(
                    cloneArray( arrayVector, geometry->getSecondaryColorArray() )
                );
                newGeometry->setFogCoordArray(
                    cloneArray( arrayVector, geometry->getFogCoordArray() )
                );
                for( unsigned int i = 0; i < geometry->getNumTexCoordArrays(); ++i )
                {
                    newGeometry->setTexCoordArray(
                        i,
                        cloneArray( arrayVector, geometry->getTexCoordArray( i ) )
                    );
                }
                for( unsigned int i = 0; i < geometry->getNumVertexAttribArrays(); ++i )
                {
                    newGeometry->setVertexAttribArray(
                        i,
                        cloneArray( arrayVector, geometry->getVertexAttribArray( i ) )
                    );
                }

                for( unsigned int i = 0; i < geometry->getNumPrimitiveSets(); ++i )
                {
                    newGeometry->setPrimitiveSet(
                        i,
                        clonePrimitiveSet( primitiveSetVector,
                                           geometry->getPrimitiveSet( i ) )
                    );
                }
            }
...
class SceneGraphProcessor : public osg::Referenced
{
    public:
        ...
        SceneGraphProcessor( osg::ArgumentParser& arguments )
        {
            ...
            while( arguments.read( "--remove-duplicate-vertices" ) ||
                   arguments.read( "--rdv" ) )
            {
                removeDuplicateVertices = true;
            }
            while( arguments.read( "--optimize-vertex-cache" ) ||
                   arguments.read( "--ovc" ) )
            {
                optimizeVertexCache = true;
            }
            while( arguments.read( "--optimize-vertex-order" ) ||
                   arguments.read( "--ovo" ) )
            {
            ...
            {
                modifyTextureSettings = true;
                buildImageMipmaps     = true;
            }
        ...
        virtual osg::Node*
        process( osg::Node* node )
        {
            if( !node )
            {
                OSG_NOTICE << "SceneGraphProcessor::process(Node*) : error cannot "
                              "process NULL Node."
                           << std::endl;
                return 0;
            }

            OSG_NOTICE << "SceneGraphProcessor::process(" << node
                       << ") : " << node->getName() << std::endl;

            if( simplificatioRatio < 1.0 )
            {
                OSG_NOTICE << "Running simplifier with simplification ratio="
                           << simplificatioRatio << std::endl;
                float               maxError = 4.0F;
      ...
      public osgUtil::IncrementalCompileOperation::CompileCompletedCallback
{
    public:
        ...
        DatabasePagingOperation( const std::string&   filename,
            ...
            osg::Referenced( true ),
            Operation( "DatabasePaging Operation",
                       false ),
            _filename( filename ),
        ...
        operator()( osg::Object* /*object*/ )
        {
            osg::notify( osg::NOTICE )
                << "LoadAndCompileOperation " << _filename << std::endl;
            ...
            osg::notify( osg::NOTICE )
                << "done LoadAndCompileOperation " << _filename << std::endl;
        }
    ...
    public:
                ...
                osgGA::GUIActionAdapter& /*aa*/ )
        {
            if( ea.getEventType() == osgGA::GUIEventAdapter::KEYUP )
            {
                if( ea.getKey() == 'r' )
                {
                    osg::getOrCreateContextData( 0 )->reportStats(
                        osg::notify( osg::NOTICE )
                    );
                }
            }
...
struct ReportStatsAnimationCompletedCallback
    : public osgGA::AnimationPathManipulator::AnimationCompletedCallback
{
        virtual void
        completed( const osgGA::AnimationPathManipulator* )
        {
      ...
      char** argv )
{
    ...
    // set up camera manipulators
    {
        ...
        while( arguments.read( "-p", pathfile ) )
        {
            ...
            if( apm || !apm->valid() )
            {
                ...
                apm->setAnimationCompletedCallback(
                    new ReportStatsAnimationCompletedCallback()
                );
    ...
    //
    osg::ref_ptr<osgUtil::IncrementalCompileOperation> incrementalCompile =
    ...
    //
    osg::ref_ptr<SceneGraphProcessor> sceneGraphProcessor =
        new SceneGraphProcessor( arguments );
    ...
    std::string outputFilename = outputPostfix.empty()
                                   ? std::string()
                                   : osgDB::getStrippedName( filename ) + outputPostfix;

    databasePagingOperation    = new DatabasePagingOperation( filename,
                                                              outputFilename,
    ...
    while( !viewer.done() )
    {
            ...
            ( currentTime - timeOfLastMerge ) > timeBetweenMerges )
        {
            ...
            outputFilename = outputPostfix.empty()
                               ? std::string()
                               : osgDB::getStrippedName( filename ) + outputPostfix;

            databasePagingOperation =
                new DatabasePagingOperation( filename,
                                             outputFilename,
                                             sceneGraphProcessor.get(),
                                             incrementalCompile.get() );
        ...
        if( databasePagingOperation.get() &&
            databasePagingOperation->_modelReadyToMerge )
        {
