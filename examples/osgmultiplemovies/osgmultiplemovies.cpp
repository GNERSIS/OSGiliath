 ...
 * osgmultiplemovies example application
 */
#include <iostream>
...
class ImageStreamStateCallback : public osg::DrawableUpdateCallback
{
    public:
        ...
        ImageStreamStateCallback( osgText::Text*    text,
                                  osg::ImageStream* is ) :
        ...
        void
        setImageStream( osg::ImageStream* is )
        {
            _imageStream = is;
        }
                ...
                osg::Drawable* )
        {
            if( _text.valid() && _imageStream.valid() )
            {
                ...
                {
                    double dt =
                        nv->getFrameStamp()->getReferenceTime() - _lastDataTimeStamp;

                    _fps               = 0.9 * _fps + 0.1 * ( 1 / dt );
                    _fps               = std::round( 10 * _fps ) / 10.0;
                ...
                ss << _imageStream->s() << "x" << _imageStream->t() << " | " << _fps
                   << "fps";
                ss << " | len: " << std::round( _imageStream->getLength() * 10 ) / 10.0;
                ss << " | cur: " << std::round( _imageStream->getCurrentTime() * 10 ) /
                    10.0;
...
bool
MovieEventHandler::handle( const osgGA::GUIEventAdapter& ea,
                           osgGA::GUIActionAdapter&      aa,
                           osg::Object*,
                           osg::NodeVisitor* /*nv*/ )
{
        ...
        case( osgGA::GUIEventAdapter::RELEASE ) :
            {
                ...
                bool                                           foundIntersection =
                    view == 0 ? false : view->computeIntersections( ea, intersections );
                    ...
                    if( geometry )
                    {
                        osg::Texture* tex =
                            geometry->getStateSet()
                                ? dynamic_cast<osg::Texture*>(
                                      geometry->getStateSet()->getTextureAttribute(
                                          0,
                        ...
                        {
                            osg::ImageStream* is =
                                dynamic_cast<osg::ImageStream*>( tex->getImage( 0 ) );
                            if( is )
                            {
                                setColor( _currentGeometry.get(),
        ...
        case( osgGA::GUIEventAdapter::KEYDOWN ) :
            {
                ...
                if( ea.getKey() == 'p' )
                {
                    osg::ImageStream::StreamStatus playToggle =
                        _currentImageStream->getStatus();
                ...
                else if( ea.getKey() == '>' )
                {
                    ...
                    _currentImageStream->seek( _currentImageStream->getCurrentTime() +
                                               1.0 );
                ...
                {
                    if( _currentImageStream->getLoopingMode() ==
                        osg::ImageStream::LOOPING )
                    {
                        std::cout << _currentImageStream.get() << " Toggle Looping Off"
                                  << std::endl;
                        _currentImageStream->setLoopingMode(
                            osg::ImageStream::NO_LOOPING
                        );
                    }
                    else
                    {
                        std::cout << _currentImageStream.get() << " Toggle Looping On"
                                  << std::endl;
                ...
                else if( ea.getKey() == '+' )
                {
                    ...
                    std::cout << _currentImageStream.get() << " Increase speed rate "
                              << _currentImageStream->getTimeMultiplier() << std::endl;
                ...
                else if( ea.getKey() == '-' )
                {
                    ...
                    tm        -= 0.1;
                    _currentImageStream->setTimeMultiplier( tm );
                    std::cout << _currentImageStream.get() << " Decrease speed rate "
                              << _currentImageStream->getTimeMultiplier() << std::endl;
                ...
                {
                    std::cout << _currentImageStream.get() << " Frame rate  "
                              << _currentImageStream->getFrameRate() << std::endl;
...
void
MovieEventHandler::getUsage( osg::ApplicationUsage& usage ) const
{
...
static osgDB::DirectoryContents
getSuitableFiles( osg::ArgumentParser& arguments )
{
    ...
    for( int i = 1; i < arguments.argc(); ++i )
    {
        ...
        if( osgDB::fileType( arguments[i] ) == osgDB::DIRECTORY )
        {
            ...
            for( osgDB::DirectoryContents::iterator itr = dc.begin(); itr != dc.end();
                 ++itr )
            {
...
class MyDimensionsChangedCallback : public osg::Image::DimensionsChangedCallback
{
    public:
        ...
        MyDimensionsChangedCallback( osg::Texture*  tex,
                                     osg::Geometry* geo ) :
            ...
            if( img && _tex.valid() && _geo.valid() )
            {
                ...
                */
                ...
                osg::Vec2Array* tex_coords =
                    dynamic_cast<osg::Vec2Array*>( _geo->getTexCoordArray( 0 ) );
                if( tex_coords )
                {
...
static osg::Node*
readImageStream( const std::string& file_name,
                 osg::vec3&         p,
                 ...
                 osgDB::Options*    options )
{
    ...
    if( !tex )
    {
        osg::ref_ptr<osg::ImageStream> img_stream =
            dynamic_cast<osg::ImageStream*>( obj.get() );
    ...
    if( tex )
    {
        ...
        osg::ref_ptr<osg::ImageStream> img =
            dynamic_cast<osg::ImageStream*>( tex->getImage( 0 ) );
        if( img )
        {
        ...
        if( img->getOrigin() == osg::Image::TOP_LEFT )
        {
            geo = osg::createTexturedQuadGeometry( p,
                                                   osg::vec3( w, 0, 0 ),
                                                   osg::vec3( 0, 0, desired_height ),
                                                   0,
                                                   tex_t,
                                                   tex_s,
                                                   0 );
        ...
        img->addDimensionsChangedCallback( new MyDimensionsChangedCallback( tex.get(),
                                                                            geo ) );
...
class ReplaceTextureVisitor : public osg::DualModeVisitor
{
    public:
        ...
        apply( osg::Geode& geode )
        {
            ...
            for( unsigned int i = 0; i < geode.getNumDrawables(); ++i )
            {
                ...
                ImageStreamStateCallback* cb = dynamic_cast<ImageStreamStateCallback*>(
                    drawable->getUpdateCallback()
                );
                if( cb )
                {
                    cb->setImageStream(
                        dynamic_cast<osg::ImageStream*>( _tex->getImage( 0 ) )
                    );
                }
            }
        ...
        {
            if( ss && ss->getTextureAttribute( 0, osg::StateAttribute::Type::TEXTURE ) )
            {
...
class SlideShowEventHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        SlideShowEventHandler( osg::Node*                      node,
                               const osgDB::DirectoryContents& files,
    ...
    private:
        ...
        loadSlide( int new_ndx )
        {
            ...
            osg::ref_ptr<osg::Object> obj =
                osgDB::readRefObjectFile( _files[_currentFile], _options.get() );
            osg::ref_ptr<osg::Texture> tex = dynamic_cast<osg::Texture*>( obj.get() );
            if( !tex )
            {
                ...
                if( !stream )
                {
                    stream = osgDB::readRefFile<osg::ImageStream>( _files[_currentFile],
                                                                   _options.get() );
                }
      ...
      char** argv )
{
    ...
    */
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--disableCoreVideo",
        "disable CoreVideo (QTKit+AVFoundation plugin)"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--disableMultiThreadedFrameDispatching",
        "disable frame dispatching via multiple threads (QTKit+AVFoundation plugin)"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--maxVideos [numVideos]",
        "max videos to open from a folder"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--slideShow",
        "present movies in a slide-show"
    );
    ...
    osg::ref_ptr<osgDB::ReaderWriter::Options> options =
        new osgDB::ReaderWriter::Options( options_str );

    if( slide_show )
    {
        osg::Node* node =
            readImageStream( files[0], pos, desired_height, options.get() );
        group->addChild( node );
    ...
    else
    {
        ...
        unsigned int num_files_per_row = std::max(
            std::round( sqrt( static_cast<double>(
                std::min( max_videos, static_cast<unsigned int>( files.size() ) )
            ) ) ),
            1.0
        );
        ...
        for( osgDB::DirectoryContents::iterator i = files.begin();
             ( i != files.end() ) && ( num_videos < max_videos );
             ++i )
        {
