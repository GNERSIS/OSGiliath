...
textInfo( osgText::Text* text )
{
    ...
    {
        osg::vec2 ul;
        text->getCoord( 0 + ( i * 4 ), ul );    // upperLeft
        osg::vec2 ll;
        text->getCoord( 1 + ( i * 4 ), ll );    // lowerLeft
        osg::vec2 lr;
        text->getCoord( 2 + ( i * 4 ), lr );    // lowerRight
        osg::vec2 ur;
        text->getCoord( 3 + ( i * 4 ), ur );    // upperRight

        osg::notify( osg::NOTICE ) << "'" << static_cast<char>( s[i] ) << "':"
                                   << " width(" << lr.x -
            ll.x << ")"
                                   << " height(" << ul.y -
            ll.y << ")" << std::endl
...
osg::Camera*
createOrthoCamera( double width,
                   double height )
{
    osg::Camera* camera = new osg::Camera();

    osg::dmat4   m      = osg::ortho2D( 0.0, width, 0.0, height );
...
struct TextSettings
{
        ...
        void
        readFilterMode( const std::string&        value,
                        osg::Texture::FilterMode& filterMode )
        {
            ...
            {
                filterMode = osg::Texture::LINEAR_MIPMAP_LINEAR;
            }
        }

        void
        read( osg::ArgumentParser& arguments )
        {
            ...
            if( arguments.read( "--SIGNED_DISTANCE_FIELD" ) )
            {
                shaderTechnique = osgText::SIGNED_DISTANCE_FIELD;
            }
            if( arguments.read( "--ALL_FEATURES" ) )
            {
                shaderTechnique = osgText::ALL_FEATURES;
            }
            ...
            std::string value;
            if( arguments.read( "--min", value ) )
            {
                readFilterMode( value, minFilter );
            }
            if( arguments.read( "--mag", value ) )
            {
                readFilterMode( value, magFilter );
            }
            ...
            if( arguments.read( "--shadow" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_BOTTOM_RIGHT;
            }
            if( arguments.read( "--shadow-br" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_BOTTOM_RIGHT;
            }
            if( arguments.read( "--shadow-cr" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_CENTER_RIGHT;
            }
            if( arguments.read( "--shadow-tr" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_TOP_RIGHT;
            }
            if( arguments.read( "--shadow-bc" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_BOTTOM_CENTER;
            }
            if( arguments.read( "--shadow-tc" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_TOP_CENTER;
            }
            if( arguments.read( "--shadow-bl" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_BOTTOM_LEFT;
            }
            if( arguments.read( "--shadow-cl" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_CENTER_LEFT;
            }
            if( arguments.read( "--shadow-tl" ) )
            {
                backdropType = osgText::Text::DROP_SHADOW_TOP_LEFT;
            }

            float offset;
            if( arguments.read( "--offset", offset ) )
            {
                backdropOffset.set( offset, offset );
            }

            if( arguments.read( "--text-color",
                                textColor.r,
                                textColor.g,
                                textColor.b,
                                textColor.a ) )
            ...
            if( arguments.read( "--constant-size" ) )
            {
                scaleFontSizeToFontResolution = false;
            }
            if( arguments.read( "--scale-size" ) )
            {
                scaleFontSizeToFontResolution = true;
            }
        }

        void
        setText( osgText::Text& text )
        {
            ...
            {
                font = osgText::readRefFontFile( fontFilename );
            }
...
class KeyHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        toggleDefine( osg::StateSet*     stateset,
      ...
      char** argv )
{
    ...
    if( program->getNumShaders() > 0 )
    {
        ...
        root->getOrCreateStateSet()->setAttribute( program.get(),
    ...
    for( Sizes::const_iterator i = settings.sizes.begin(); i != settings.sizes.end();
         i++ )
    {
        std::stringstream ss;
    ...
    // Headless capture: early check before viewer construction
    {
        ...
        if( args.read( "--headless", headlessOutput ) )
        {
            auto node = osgDB::readRefNodeFiles( args );
            if( !node )
            {
        ...
        if( viewer.getCamera()->getGraphicsContext() )
        {
            ...
            // left half
            {
                ...
                viewer.addSlave( camera.get(),
                                 osg::translate( 1.0, 0.0, 0.0 ),
                                 osg::scale( 2.0, 1.0, 1.0 ) );
            }

            {
                ...
                camera->setGraphicsContext( gc.get() );
                camera->setViewport( new osg::Viewport( traits->width / 2 + 2,
                                                        0,
                                                        traits->width / 2,
                                                        traits->height ) );
                viewer.addSlave( camera.get(),
