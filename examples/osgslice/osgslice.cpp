...
class SliceProcessor
{
    public:
        ...
        SliceProcessor() :
            ...
            _sliceDelta( 0.1 ),
            _nearPlane( 0.0 ),
            _farPlane( MIN_NEARFAROFFSET )
        {
            //
        }

        SliceProcessor( const double& objectRadius,
                        unsigned int  numberSlices ) :
            _sliceNumber( numberSlices )
        {
            ...
            _image->allocateImage( _sliceNumber,
                                   _sliceNumber,
                                   _sliceNumber,
                                   GL_RGBA,
                                   GL_UNSIGNED_BYTE );
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption( "-o <filename>",
                                                           "Object to be loaded" );
    ...
    if( arguments.read( "-h" ) || arguments.read( "--help" ) )
    {
        ...
        std::cout << "\t--slices <unsigned int> sets number of slices through the object"
                  << std::endl;
    ...
    const osg::sphere& bs = loadedModel->getBound();
    ...
    viewMatrix = osg::lookAt( bs.center - osg::vec3( 0.0, 2.0F * bs.radius, 0.0 ),
                              bs.center,
    ...
    {
        // set up the frame stamp for current frame to record the current time and frame
        // number so that animtion code can advance correctly
        osg::ref_ptr<osg::FrameStamp> frameStamp = new osg::FrameStamp;
        frameStamp->setReferenceTime(
            osg::Timer::instance()->delta_s( start_tick, osg::Timer::instance()->tick() )
        );
        frameStamp->setFrameNumber( frameNum++ );

        // pass frame stamp to the SceneView so that the update, cull and draw traversals
