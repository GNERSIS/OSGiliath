// info : osgSSBO example,testing ShaderStorageBufferObjects ,Markus Hein,  2014,
// osg-3.2.1 required hardware and driver must support GL >=  GL 4.3 or GL ES 3.1 (GL ES
// not tested, would be nice if someone will test it on a small device)
...
#include <osg/core/StateAttributeCallback.hpp>
...
#define PRERENDER_WIDTH                 1'920
#define PRERENDER_HEIGHT                1'080
...
class ShaderStorageBufferCallback : public osg::StateAttributeCallback
{
    public:
        ...
        operator()( osg::StateAttribute* /*attr*/,
                    osg::NodeVisitor* /*nv*/ )
        {
            // if you need to process the data in your app-code , better leaving it on
            // GPU and processing there, uploading per frame will make it slow
#if 0
...
// do not forget to set OSG_FILE_PATH to default OSG-Data and make sure the new shaders
// are copied there under"shaders"
class ComputeNode : public osg::PositionAttitudeTransform
{
    ...
    public:
        ...
        osg::ref_ptr<osg::Shader>
            _computeShader;     // compute and write position data in SSBO

        osg::ref_ptr<osg::Shader>
            _vertexShader;      // reading position data from SSBO (OBS!: make sure
                                // glMemoryBuffer() is syncing this)
        osg::ref_ptr<osg::Shader>
            _geometryShader;    // building a quad looking to the camera
        osg::ref_ptr<osg::Shader>
            _fragmentShader;    // use false-colors etc. for making your data visible
        ...
        addHelperGeometry();
        void
        addDataMonitor( osg::vec3    placement,
                        osg::vec3    relativePlacement,
                        float        scale,
                        Channel      channel,
                        BufferOffset shaderBufferOffset,
...
class ComputeNodeUpdateCallback : public osg::NodeCallback
{
    public:
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( _timer.delta_s( _prevShaderUpdateTime, currTime ) >
                1.0 )    // one  second interval for shader-changed-do-reload check
            {
                ...
                if( _computeNode->_computeShader.valid() )
                {
                    ...
                    reloadedshader = osgDB::readRefShaderFile(
                        osg::Shader::COMPUTE,
                        _computeNode->_computeShaderSourcePath
                    );

                    reloadedstring = reloadedshader->getShaderSource();
                    if( !osgDB::equalCaseInsensitive( runningSource.c_str(),
                                                      reloadedstring.c_str() ) )
                    {
                        _computeNode->_computeProgram->removeShader(
                            _computeNode->_computeShader.get()
                        );
                        _computeNode->_computeShader = reloadedshader.get();
                        _computeNode->_computeProgram->addShader(
                            _computeNode->_computeShader.get()
                        );
                    }
                }
                ...
                if( _computeNode->_vertexShader.valid() )
                {
                    ...
                    reloadedshader = osgDB::readRefShaderFile(
                        osg::Shader::VERTEX,
                        _computeNode->_vertexShaderSourcePath
                    );
                    ...
                    if( !osgDB::equalCaseInsensitive( runningSource.c_str(),
                                                      reloadedstring.c_str() ) )
                    {
                        _computeNode->_computationResultsRenderProgram->removeShader(
                            _computeNode->_vertexShader.get()
                        );
                        _computeNode->_vertexShader = reloadedshader.get();
                        _computeNode->_computationResultsRenderProgram->addShader(
                            _computeNode->_vertexShader.get()
                        );
                    }
                }
                ...
                if( _computeNode->_geometryShader.valid() )
                {
                    ...
                    reloadedshader = osgDB::readRefShaderFile(
                        osg::Shader::GEOMETRY,
                        _computeNode->_geometryShaderSourcePath
                    );
                    ...
                    {
                        _computeNode->_computationResultsRenderProgram->removeShader(
                            _computeNode->_geometryShader.get()
                        );
                ...
                if( _computeNode->_fragmentShader.valid() )
                {
                    ...
                    reloadedshader = osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        _computeNode->_fragmentShaderSourcePath
                    );
                    ...
                    {
                        _computeNode->_computationResultsRenderProgram->removeShader(
                            _computeNode->_fragmentShader.get()
                        );
...
// set  OSG_FILE_PATH for loading duck.glb
void
ComputeNode::addHelperGeometry()
{
    _helperNode = osgDB::readRefNodeFile( "duck.glb" );
    // Note: _helperNode is added to the scene root (not ComputeNode)
    // so it uses the global shader pipeline instead of compute shaders.
    ...
    // osg::BlendFunc::ONE_MINUS_SRC_ALPHA); stateset->setAttributeAndModes(blend,
    // osg::StateAttribute::ON); pat->addChild(sphereGeode); addChild(pat);
}

void
ComputeNode::addDataMonitor( osg::vec3    placement,
                             osg::vec3    relativePlacement,
                             float        scale,
                             Channel      colorchannel,
                             BufferOffset shaderStorageBufferOffset,
                             std::string  labelCaption,
                             float        minDataRange,
                             float        maxDataRange )
{
    osg::PositionAttitudeTransform* pat = new osg::PositionAttitudeTransform;
    pat->setPosition( osg::dvec3( relativePlacement ) );
    addChild( pat );
    osg::Geometry* geom;
    ...
    if( NUM_ELEMENTS_X >= NUM_ELEMENTS_Y )
    {
        ...
        geom        = osg::createTexturedQuadGeometry(
            placement,
            osg::vec3( 1.0F * scale, 0.0F, 0.0F ),
            osg::vec3( 0.0F, 0.0F, ratio * 1.0F * scale )
        );
    }
    else
    {
        ...
        geom        = osg::createTexturedQuadGeometry(
            placement,
            osg::vec3( ratio * 1.0F * scale, 0.0F, 0.0F ),
            osg::vec3( 0.0F, 0.0F, 1.0F * scale )
        );
    }

    geom->setVertexAttribArray( 1,
    ...
    static const char* vertexShaderSrcChannelMonitor = {
        ...
        "in vec2 tex_coords;\n"
        "void main() {\n"
        "texCoordFromVertexShader.xy = tex_coords.xy;  gl_Position = ( "
        "osg_ProjectionMatrix * osg_ModelViewMatrix * "
        "vec4(vertexpos.x,vertexpos.y,vertexpos.z,1) ); \n"
        "}\n"
    };

    std::stringstream fragmentshaderstringstreamChannelMonitor;
    fragmentshaderstringstreamChannelMonitor << "#version 460 core\n";
    fragmentshaderstringstreamChannelMonitor << "out vec4 fragColor;\n";
    ...
    fragmentshaderstringstreamChannelMonitor << "{\n";
    fragmentshaderstringstreamChannelMonitor
        << "ivec2 storePos = ivec2(numRows*texCoordFromVertexShader.x, "
           "numCols*texCoordFromVertexShader.y);  particle particleData = p["
        << shaderStorageBufferOffset *
        NUM_ELEMENTS_X *
        NUM_ELEMENTS_Y
        << " + (storePos.x*numRows + storePos.y)]; ";

    // fragmentshaderstringstreamChannelMonitor << " memoryBarrierBuffer(); \n";
    fragmentshaderstringstreamChannelMonitor
        << " float dataRangeMultiplier = 1.0 / abs(dataRangeMax - dataRangeMin); \n";
    ...
    switch( colorchannel )
    {
        case RED_CHANNEL :
            {
                fragmentshaderstringstreamChannelMonitor
                    << "   vec4 color; color.x =  "
                       "0.5+dataRangeMultiplier*particleData.x; color.y =0.0; color.z = "
                       "0.0; color.w = 1.0; fragColor = color;\n";
        ...
        case GREEN_CHANNEL :
            {
                fragmentshaderstringstreamChannelMonitor
                    << "   vec4 color; color.x = 0.0; color.y = "
                       "0.5+dataRangeMultiplier*particleData.y; color.z = 0.0; color.w "
                       "= 1.0; fragColor = color;\n";
                break;
            }
        case BLUE_CHANNEL :
            {
                fragmentshaderstringstreamChannelMonitor
                    << "   vec4 color; color.x = 0.0; color.y = 0.0; color.z = "
                       "0.5+dataRangeMultiplier*particleData.z;  color.w = 0.0 ; "
                       "fragColor = color;\n";
                break;
            }
        case ALPHA_CHANNEL :
            {
                fragmentshaderstringstreamChannelMonitor
                    << "   vec4 color; color.x = "
                       "0.5+dataRangeMultiplier*particleData.w; color.y = "
                       "0.5+dataRangeMultiplier*particleData.w; color.z = "
                       "0.5+dataRangeMultiplier*particleData.w; color.w = "
                       "0.5+0.5*particleData.w; fragColor = color;\n";
                break;
            }
        ...
        case RGB_CHANNEL :
            {
                fragmentshaderstringstreamChannelMonitor
                    << "   vec4 color; color.x = "
                       "0.5+dataRangeMultiplier*particleData.x; color.y = "
                       "0.5+dataRangeMultiplier*particleData.y; color.z = "
                       "0.5+dataRangeMultiplier*particleData.z; color.w = 1.0; "
                       "fragColor = color;\n";
                break;
            }
        ...
        case RGBA_CHANNEL :
            {
                fragmentshaderstringstreamChannelMonitor
                    << "   vec4 color; color.x = "
                       "0.5+dataRangeMultiplier*particleData.x; color.y = "
                       "0.5+dataRangeMultiplier*particleData.y; color.z = "
                       "0.5+dataRangeMultiplier*particleData.z; color.w = "
                       "0.5+0.5*particleData.w; fragColor = color;\n";
                break;
            }
    }
    ...
    program->addShader( new osg::Shader( osg::Shader::VERTEX,
                                         vertexShaderSrcChannelMonitor ) );
    program->addShader(
        new osg::Shader( osg::Shader::FRAGMENT,
                         fragmentshaderstringstreamChannelMonitor.str().c_str() )
    );
...
void
ComputeNode::addComputationResultsRenderTree()
{

    _computationResultsRenderProgram = new osg::Program;

    _vertexShader =
        osgDB::readRefShaderFile( osg::Shader::VERTEX, _vertexShaderSourcePath );
    ...
    _fragmentShader =
        osgDB::readRefShaderFile( osg::Shader::FRAGMENT, _fragmentShaderSourcePath );
    ...
    _computationResultsRenderGroup->setDataVariance(
        osg::Object::DataVariance::DYNAMIC
    );
    _computationResultsRenderStateSet =
        _computationResultsRenderGroup->getOrCreateStateSet();
    _computationResultsRenderStateSet->setRenderingHint(
        osg::StateSet::TRANSPARENT_BIN
    );

    int texture_unit = 0;
    _computationResultsRenderStateSet->setAttributeAndModes(
        _computationResultsRenderProgram.get(),
        osg::StateAttribute::ON
    );
    _computationResultsRenderStateSet->addUniform( new osg::Uniform( "particleTexture",
                                                                     texture_unit ) );
    _computationResultsRenderStateSet->addUniform(
        new osg::Uniform( "numRows", ( int )NUM_ELEMENTS_X )
    );
    _computationResultsRenderStateSet->addUniform(
        new osg::Uniform( "numCols", ( int )NUM_ELEMENTS_Y )
    );
    ...
    osg::Image*     particleImage = createSpotLightImage( osg::vec4( 1, 0, 0, 1 ),
                                                          osg::vec4( 0.5, 0, 0, 0.0 ),
                                                          32,
                                                          0.7 );
    if( particleImage )
    {
    ...
    _computationResultsRenderStateSet
        ->setTextureAttributeAndModes( texture_unit, tex, osg::StateAttribute::ON );
    ...
    _computationResultsRenderStateSet->setAttributeAndModes( blend,
                                                             osg::StateAttribute::ON );
    ...
    _computationResultsRenderStateSet->setAttributeAndModes( depth,
    ...
    particleGeometry->addPrimitiveSet(
        new osg::DrawArrays( GL_POINTS, 0, numVertices )
    );
    particleGeometry->setTexCoordArray( 0, tcoords );
    // this glMemoryBarrier thing... not sure if we could better do instanced drawing?
    // all the data is in Shader Storage Buffer..
    particleGeometry->setVertexAttribArray( 1,
                                            particleGeometry->getTexCoordArray( 0 ),
                                            osg::Array::BIND_PER_VERTEX );
...
ComputeNode::initComputingSetup()
{
    ...
    _DispatchCompute->setComputeGroups( ( NUM_ELEMENTS_X / WORK_GROUP_SIZE ) <= 1
                                            ? 1
                                            : ( NUM_ELEMENTS_X / WORK_GROUP_SIZE ),
                                        ( NUM_ELEMENTS_Y / WORK_GROUP_SIZE ) <= 1
                                            ? 1
                                            : ( NUM_ELEMENTS_Y / WORK_GROUP_SIZE ),
                                        1 );
    _computeShader =
        osgDB::readRefShaderFile( osg::Shader::COMPUTE, _computeShaderSourcePath );
    ...
    statesetComputation->addUniform( new osg::Uniform( "numCols",
                                                       ( int )NUM_ELEMENTS_X ) );
    statesetComputation->addUniform( new osg::Uniform( "numRows",
                                                       ( int )NUM_ELEMENTS_Y ) );
    // GL_LIGHTING removed for Core Profile compatibility
    ...
    const unsigned blockSize =
        numParticles * __numChannels * __numDataValuesPerChannel * sizeof( GLfloat );

    // init all the particle data array
    int idx = 0;
    ( void )idx;
    _data = new GLfloat
        [NUM_ELEMENTS_X * NUM_ELEMENTS_Y * __numChannels * __numDataValuesPerChannel];
    _dataArray = new FloatArray;

    // init the data array  somehow, this way all is stored in one BufferObject. maybe
    // better using multiple buffers instead? not sure what is faster and better for
    // threading
            ...
            for( int j = 0; j < NUM_ELEMENTS_Y; ++j )
            {
                ...
                for( int k = 0; k < __numChannels; ++k )
                {
                    switch( k )
                    {
                        ...
                        case( RED_CHANNEL ) :
                            {
                                if( ( d == POSITION_NOW_OFFSET ) ||
                                    ( d == POSITION_OLD_OFFSET ) ||
                                    ( d == POSITION_INIT_OFFSET ) )    // position
                                {
                                    *_data = random( 0.25, 0.75 );
                                }
                                if( ( d == VELOCITY_NOW_OFFSET ) ||
                                    ( d == VELOCITY_OLD_OFFSET ) ||
                                    ( d == VELOCITY_INIT_OFFSET ) )    // velocity
                                {
                                ...
                                if( d == PROPERTIES_OFFSET )    // property particle mass
                                                                // (compute shader is
                                                                // computing sphere mass
                                                                // from radius instead)
                                {
                        ...
                        case( GREEN_CHANNEL ) :
                            {
                                ...
                                if( d ==
                                    PROPERTIES_OFFSET )    // property particle radius
                                {
                        ...
                        case( BLUE_CHANNEL ) :
                            {
                                ...
                                if( d == PROPERTIES_OFFSET )    // place for some other
                                                                // property
                                {
    ...
    addDataMonitor( osg::vec3( 0, -1, 0 ),
                    osg::vec3( SUB_PLACEMENT_OFFSET_HORIZONTAL * 0,
                               -SUB_PLACEMENT_OFFSET_VERTICAL * -2.0,
                               SUB_PLACEMENT_OFFSET_HORIZONTAL * 0 ),
                    1.0,
                    RGB_CHANNEL,
                    POSITION_NOW_OFFSET,
                    "X,Y,Z - PositionNow",
...
osg::Node*
createPrerenderSubgraph( osg::Node*       subgraph,
                         const osg::vec4& clearColour )
{
...

    {
        ...
        prerenderCamera->setProjectionMatrix( osg::dmat4() );
        prerenderCamera->setViewMatrix( osg::dmat4() );
        prerenderCamera->setViewport( 0, 0, tex_width, tex_height );
        prerenderCamera->setRenderOrder( osg::Camera::PRE_RENDER );
        prerenderCamera->setRenderTargetImplementation(
            osg::Camera::FRAME_BUFFER_OBJECT
        );
        prerenderCamera->attach( osg::Camera::COLOR_BUFFER0,
                                 texture,
                                 0,
                                 0,
                                 false,
                                 PRERENDER_ANTIALIASINGMULTISAMPLES,
                                 PRERENDER_ANTIALIASINGMULTISAMPLES );
...

    {
        ...
        for( i = 0; i < noSteps; ++i )
        {
            ...
            for( j = 0; j < noSteps; ++j )
            {
                ...
                texcoords->push_back( osg::vec2(
                    ( sin( texcoord.x * osg::PI - osg::PI * 0.5 ) + 1.0F ) * 0.5F,
        ...
        nestedRenderCamera->setViewMatrix( osg::dmat4() );
        nestedRenderCamera->setProjectionMatrixAsOrtho2D( 0, 1'280, 0, 1'024 );
      ...
      char** argv )
{
    ...
    );    // we can play with threading models later

    osg::ref_ptr<ComputeNode> computeNode = new ComputeNode();
    computeNode->setPosition( osg::dvec3( 0, 0, 0 ) );
    computeNode->setUpdateCallback(
        new ComputeNodeUpdateCallback( computeNode.get() )
    );    // on-the-fly reloading the shaders if shader source on disk is changed
    computeNode->initComputingSetup();

    scene->addChild( computeNode.get() );
    scene->addChild( computeNode->_computationResultsRenderGroup.get() );

    // Add helper geometry (axes) to scene root so it uses the global
    // shader pipeline instead of inheriting compute shaders from ComputeNode
    if( computeNode->_helperNode.valid() )
    {
        scene->addChild( computeNode->_helperNode.get() );
    }

#ifdef PRERENDER_HIGH_QUALITY_ANTIALIASING
    viewer.setSceneData( createPrerenderSubgraph( scene.get(),
                                                  osg::vec4( 0.3, 0.4, 0.6, 1 ) ) );
#else
