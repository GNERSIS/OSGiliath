 ...
 * osggeometry example application
 */
#include <iostream>
...
// the osg::Geometry class can represent.  This demo uses the OpenGL red book diagram of
// different OpenGL Primitives as a template for all the equivalent OpenSceneGraph
// Primitives.  The OpenSceneGraph wraps OpenGL very thinly and therefore uses all the
// same enum and naming conventions. The coordinate data is also wrapped around OpenGL's
// vertex arrays and draw arrays/elements calls.  Familiarity with OpenGL will help you
// understand the osg::Geometry class which encapsulate all this, or if you haven't
// learned OpenGL yet, learning osg::Geometry will help you understand how OpenGL works!

// The osg::Geometry class "is a" subclass of osg::Drawable base class, so is an object
// that provides a draw method for drawing objects in the scene.  osg::Geometry contains
// all the vertex, normal color and texture coordinate arrays required to specify the
// coordinates of your objects, and the primitives join these coordinates together as the
// points, lines or surfaces that you will see rendered on your screen.
//
// This demo is split into two functions, the createScene() function which creates the
// scene graph with the various primitives in it, and the main() which sets up a basic
// viewer window and adds to the it the scene generated by createScene().
...
struct NormalPrint
{
                    ...
                    bool ) const
        {
            osg::vec3 normal = ( v2 - v1 ) ^ ( v3 - v2 );
            normal           = osg::normalize( normal );
            std::cout << "\tquad(" << v1 << ") (" << v2 << ") (" << v3 << ") (" << v4
                      << ") " << ")" << std::endl;
        }
};

// decompose Drawable primitives into triangles, print out these triangles and computed
// normals.
void
printPrimitives( const std::string& name,
...
{
        void
        operator()( unsigned int p1 ) const
        {
        ...
        operator()( unsigned int p1,
                    unsigned int p2,
                    unsigned int p3,
                    unsigned int p4 ) const
        {
            std::cout << "\tquad(" << p1 << ", " << p2 << ", " << p3 << ", " << p4 << ")"
                      << std::endl;
        }
};
...
// normals.
void
printPrimitiveIndices( const std::string& name,
                       osg::Drawable&     drawable )
{
    std::cout << name << std::endl;
...
createScene()
{
    ...
    // following are separate blocks for creating POINTS, LINES, LINE_STRIP, LINE_LOOP,
    // TRIANGLE_FAN (was POLYGON), TRIANGLES (was QUADS), TRIANGLE_STRIP (was
    // QUAD_STRIP), TRIANGLES, TRIANGLE_STRIP and TRIANGLE_FAN primitives. Legacy types
    // (GL_POLYGON, GL_QUADS, GL_QUAD_STRIP) have been converted to Core Profile
    // equivalents.
    ...
    // create POINTS
    {
        ...
        // Like all the *Array variants (see include/osg/Array) , Vec3Array is derived
        // from both osg::Array and std::vector<>.  osg::Array's are reference counted
        // and hence shareable, which std::vector<> provides all the convenience,
        // flexibility and robustness of the most popular of all STL containers.
    ...
    // create LINE_STRIP
    {
        ...
        // since we know up front,
        linesGeom->addPrimitiveSet(
            new osg::DrawArrays( osg::PrimitiveSet::LINE_STRIP, 0, 5 )
        );
    ...
    // create LINE_LOOP
    {
        ...
        // since we know up front,
        linesGeom->addPrimitiveSet(
            new osg::DrawArrays( osg::PrimitiveSet::LINE_LOOP, 0, numCoords )
        );
    ...
    // According to the OpenGL diagram vertices should be specified in a clockwise
    // direction. In reality you need to specify coords for polygons in a anticlockwise
    // direction for their front face to be pointing towards you; get this wrong and you
    // could find back face culling removing the wrong faces of your models.  The OpenGL
    // diagram is just plain wrong, but it's a nice diagram so we'll keep it for now!
    ...
    // create POLYGON
    {
        ...
        polyGeom->addPrimitiveSet(
            new osg::DrawArrays( osg::PrimitiveSet::TRIANGLE_FAN, 0, numCoords )
        );

        printPrimitiveIndices( "TRIANGLE_FAN (was POLYGON) indices", *polyGeom );
        printPrimitives( "TRIANGLE_FAN (was POLYGON) vertices", *polyGeom );
    ...
    // create QUADS
    {
        ...
        // Convert QUADS to TRIANGLES using an index buffer
        // Quad 0: vertices (0,1,2,3) -> triangles (0,1,3) and (1,2,3)
        // Quad 1: vertices (4,5,6,7) -> triangles (4,5,7) and (5,6,7)
        {
            osg::DrawElementsUShort* indices =
                new osg::DrawElementsUShort( GL_TRIANGLES );
            for( int q = 0; q < numCoords / 4; ++q )
            {
        ...
        printPrimitiveIndices( "TRIANGLES (was QUADS) indices", *polyGeom );
        printPrimitives( "TRIANGLES (was QUADS) vertices", *polyGeom );
    ...
    // create QUAD_STRIP
    {
        ...
        // note, first coord at top, second at bottom, reverse to that buggy OpenGL
        // image..
        osg::vec3      myCoords[] = {
        ...
        // This time we simply use primitive, and hardwire the number of coords to use
        // since we know up front,
        // QUAD_STRIP not available in Core Profile; TRIANGLE_STRIP has identical vertex
        // layout
        polyGeom->addPrimitiveSet(
            new osg::DrawArrays( osg::PrimitiveSet::TRIANGLE_STRIP, 0, numCoords )
        );

        printPrimitiveIndices( "TRIANGLE_STRIP (was QUAD_STRIP) indices", *polyGeom );
        printPrimitives( "TRIANGLE_STRIP (was QUAD_STRIP) vertices", *polyGeom );
...
// define a node callback to animate a transform as a cycle along the y axis, between 0
// and 2.0.
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( nv && transform && nv->getFrameStamp() )
            {
                ...
                transform->setMatrix( osg::dmat4(
                    osg::translate( 0.0F, 1.0F + cosf( time * _angular_velocity ), 0.0F )
                ) );
            }
...
createBackground()
{
    ...
    osg::vec2 myTexCoords[] =
        { osg::vec2( 0, 1 ), osg::vec2( 0, 0 ), osg::vec2( 1, 0 ), osg::vec2( 1, 1 ) };
    ...
    // There are three variants of the DrawElements osg::Primitive, UByteDrawElements
    // which contains unsigned char indices, UShortDrawElements which contains unsigned
    // short indices, and UIntDrawElements which contains ... unsigned int indices. The
    // first parameter to DrawElements is
    polyGeom->addPrimitiveSet(
