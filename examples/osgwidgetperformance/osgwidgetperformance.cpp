...
setupArguments( osg::ArgumentParser& args )
{
    args.getApplicationUsage()->setDescription(
        ...
        " is a performance testing application for osgWidget."
    );

    args.getApplicationUsage()->setCommandLineUsage( args.getApplicationName() +
                                                     " [options] widgets" );

    args.getApplicationUsage()->addCommandLineOption( "--width <int>",
                                                      "The WindowManager width." );

    args.getApplicationUsage()->addCommandLineOption( "--height <int>",
...
void
readSize( osg::ArgumentParser& args,
          const char*          opt,
          unsigned int&        val )
{
    std::string size;
...
int
doApp( osgViewer::Viewer& viewer,
       osg::Node*         node,
       unsigned int       width,
       unsigned int       height )
{
    osgWidget::WindowManager* wm =
        new osgWidget::WindowManager( &viewer, width, height, 0X12 );
      ...
      char** argv )
{
    ...
    {
        return doError( "Please specify one of --single-window or --multi-window." );
    }
        ...
        */
        ...
        for( unsigned int c = 0; c < cols; c++ )
        {
            for( unsigned int r = 0; r < rows; r++ )
            {
                ...
                osgWidget::Widget* widget = new osgWidget::Widget( "", wSize, wSize );
