 ...
 * osgvertexattributes example application
 */
...
class ConvertToVertexAttibArrays : public osg::DualModeVisitor
{
    public:
        ...
        void
        bindAttribute( osg::Program&         program,
                       const AttributeAlias& alias )
        {
            program.addBindAttribLocation( alias.second, alias.first );
        }

        void
        replaceAndBindAttrib( osg::Program&         program,
                              std::string&          source,
        ...
        void
        replaceBuiltInUniform( std::string&       source,
                               const std::string& originalStr,
        ...
        void
        convertVertexShader( osg::Program& program,
                             osg::Shader&  shader )
        {
        ...
        void
        convertFragmentShader( osg::Program& /*program*/,
                               osg::Shader& /*shader*/ )
        {
        }
               ...
               osg::Shader&  shader )
        {
            ...
            osg::notify( osg::NOTICE ) << "Shader " << shader.getTypename()
                                       << " ----before-----------" << std::endl;
        ...
        apply( osg::StateSet& stateset )
        {
            ...
            if( program )
            {
                osg::notify( osg::NOTICE )
                    << "   Found Program " << program << std::endl;
        ...
        apply( osg::Geometry& geom )
        {
            ...
            {
                osg::notify( osg::NOTICE )
                    << "Warning: Ignoring " << maxNumTexCoords -
                    8
                    << " texture coordinate arrays, only 8 are currently supported in "
                       "vertex attribute conversion code."
                    << std::endl;
                maxNumTexCoords = 8;
            }
            for( unsigned int i = 0; i < maxNumTexCoords; ++i )
            {
                ...
                {
                    osg::notify( osg::NOTICE )
                        << "Found empty TexCoordArray(" << i << ")" << std::endl;
                }
            }
        }

        void
        setVertexAttrib( osg::Geometry&        geom,
                         const AttributeAlias& alias,
                         osg::Array*           array,
                         bool                  normalize,
                         osg::Array::Binding   binding = osg::Array::BIND_UNDEFINED )
        {
            ...
            osg::notify( osg::NOTICE )
                << "   vertex attrib(" << name << ", index=" << index
                << ", normalize=" << normalize << " binding=" << binding << ")"
                << std::endl;
        }
      ...
      char* argv[] )
{
    ...
    else
    {
        ...
        {
            osg::notify( osg::NOTICE )
                << "No model loaded, please specify a model filename." << std::endl;
            return 1;
        }
        ...
        {
            // find any conventional vertex, colour, normal and tex coords arrays and
            // convert to vertex attributes
