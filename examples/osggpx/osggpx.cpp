...
class TrackSegment : public osg::Inherit<osg::Object, TrackSegment>
{
    public:
        ...
        TrackSegment( const TrackSegment& ts,
        ...
        void
        addTrackPoint( const TrackPoint& trackPoint )
        {
...
class Track : public osg::Inherit<osg::Object, Track>
{
    public:
...
double
convertTime( const std::string& timestr )
{
...
Track*
readTrack( const std::string& filename )
{
         ...
         ++itr )
    {
                 ...
                 ++sitr )
            {
                if( ( *sitr )->name == "rtept" )
                {
                    ...
                    if( trkpt->properties.count( latitude ) != 0 )
                    {
                        valid = true;
                        point.latitude =
                            osg::asciiToDouble( trkpt->properties[latitude].c_str() );
                    }
                    if( trkpt->properties.count( longitude ) != 0 )
                    {
                        valid = true;
                        point.longitude =
                            osg::asciiToDouble( trkpt->properties[longitude].c_str() );
                    }

                    for( osgDB::XmlNode::Children::iterator pitr =
                             trkpt->children.begin();
                         ...
                         ++pitr )
                    {
                        ...
                        {
                            point.elevation =
                                osg::asciiToDouble( ( *pitr )->contents.c_str() );
                        }
        ...
        else if( ( *itr )->name == "trk" )
        {
                 ...
                 ++citr )
            {
                ...
                {
                    osg::ref_ptr<TrackSegment> trackSegment = new TrackSegment;
                    for( osgDB::XmlNode::Children::iterator sitr =
                             ( *citr )->children.begin();
                         ...
                         ++sitr )
                    {
                        if( ( *sitr )->name == "trkpt" || ( *sitr )->name == "rtept" )
                        {
                            ...
                            {
                                valid          = true;
                                point.latitude = osg::asciiToDouble(
                                    trkpt->properties[latitude].c_str()
                                );
                            }
                            if( trkpt->properties.count( longitude ) != 0 )
                            {
                                valid           = true;
                                point.longitude = osg::asciiToDouble(
                                 ...
                                 ++pitr )
                            {
                                if( ( *pitr )->name == "ele" )
                                {
                                    point.elevation = osg::asciiToDouble(
                                        ( *pitr )->contents.c_str()
                                    );
                                }
                                else if( ( *pitr )->name == "time" )
...
TrackSegment*
computeSmoothedTrackSegment( TrackSegment* ts )
{
    if( !ts )
    {
        return 0;
    ...
    if( orig_points.size() > 2 )
    {
        ...
        {
            new_points[i].latitude  = ( orig_points[i - 1].latitude +
                                        orig_points[i].latitude +
                                        orig_points[i + 1].latitude ) /
                                      3.0;
            new_points[i].longitude = ( orig_points[i - 1].longitude +
                                        orig_points[i].longitude +
                                        orig_points[i + 1].longitude ) /
                                      3.0;
            new_points[i].elevation = ( orig_points[i - 1].elevation +
                                        orig_points[i].elevation +
                                        orig_points[i + 1].elevation ) /
                                      3.0;
            new_points[i].time      = ( orig_points[i - 1].time +
                                        orig_points[i].time +
                                        orig_points[i + 1].time ) /
                                      3.0;
        }
    ...
    {
        // we have two or less points and can't do smoothing, so will just return
...
TrackSegment*
computeAveragedSpeedTrackSegment( TrackSegment* ts )
{
    if( !ts )
    {
        return 0;
    ...
    if( orig_points.size() > 2 )
    {
        ...
        for( unsigned int i = 1; i < orig_points.size() - 1; ++i )
        {
            osg::dvec3 point_a, point_b;
            em->convertLatLongHeightToXYZ( osg::radians( orig_points[i].latitude ),
                                           osg::radians( orig_points[i].longitude ),
                                           orig_points[i].elevation,
                                           point_a.x,
                                           point_a.y,
                                           point_a.z );
            em->convertLatLongHeightToXYZ( osg::radians( orig_points[i + 1].latitude ),
                                           osg::radians( orig_points[i + 1].longitude ),
                                           orig_points[i + 1].elevation,
                                           point_b.x,
                                           point_b.y,
                                           point_b.z );
            total_distance += osg::length( point_b - point_a );
        }

        double total_time =
            orig_points[orig_points.size() - 1].time - orig_points[0].time;
        ...
        for( unsigned int i = 0; i < orig_points.size() - 1; ++i )
        {
            ...
            accumulated_distance   += osg::length( point_b - point_a );
...
Track*
computeAveragedSpeedTrack( Track* track )
{
...
Track*
computeSmoothedTrack( Track* track )
{
    osg::ref_ptr<Track> new_track = new Track;
                  ...
                  const osg::vec4& colour )
{
         ...
         ++itr )
    {
        ...
        {
            osg::dvec3 point;
            em->convertLatLongHeightToXYZ( osg::radians( points[i].latitude ),
                                           osg::radians( points[i].longitude ),
                                           points[i].elevation,
                                           point.x,
                                           point.y,
                                           point.z );
        ...
        geometry->addPrimitiveSet(
            new osg::DrawArrays( GL_LINE_STRIP, 0, points.size() )
        );
      ...
      char** argc )
{
    ...
    for( Tracks::iterator titr = tracks.begin(); titr != tracks.end(); ++titr )
    {
             ...
             ++itr )
        {
            ...
            if( points.size() > 1 )
            {
                ...
                osg::dvec3                          previousPos;
                double                              previousElevation = pitr->elevation;
                em->convertLatLongHeightToXYZ( osg::radians( pitr->latitude ),
                                               osg::radians( pitr->longitude ),
                                               0.0,
                                               previousPos.x,
                                               previousPos.y,
                                               previousPos.z );
                ...
                for( ; pitr != ts->getTrackPoints().end(); ++pitr )
                {
                    osg::dvec3 newPos;
                    ...
                    double distance  = osg::length( newPos - previousPos );
                    ...
                    osg::notify( osg::NOTICE )
                        << "     distance=" << distance << ", "
                        << newElevation - previousElevation << std::endl;
        ...
        osg::notify( osg::NOTICE )
            ...
            << totalDistance * metersToMiles << " miles" << std::endl;
        osg::notify( osg::NOTICE ) << "totalAscent = " << totalAscent << "m, "
                                   << totalAscent * metersToFeet << "ft" << std::endl;
        osg::notify( osg::NOTICE ) << "totalDescent = " << totalDescent << "m, "
                                   << totalDescent * metersToFeet << "ft" << std::endl;
    }
    ...
    if( !outputFilename.empty() )
    {
        ...
        fout << "<?xml version=\"1.0\" encoding=\"utf-8\"?><gpx version=\"1.0\" "
                "creator=\"osggpx\" "
        ...
        for( Tracks::iterator titr = tracks.begin(); titr != tracks.end(); ++titr )
        {
                 ...
                 ++itr )
            {
                ...
                for( TrackSegment::TrackPoints::iterator pitr =
                         ts->getTrackPoints().begin();
                     pitr != ts->getTrackPoints().end();
                     ++pitr )
                {
                    fout << "<trkpt lat=\"" << pitr->latitude << "\" lon=\""
                         << pitr->longitude << "\">" << std::endl;
