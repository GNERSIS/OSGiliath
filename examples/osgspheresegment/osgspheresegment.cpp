 ...
 * osgspheresegment example application
 */
#include <iostream>
                    ...
                    osg::NodeVisitor* nv )
        {
            if( !root_ || !terrain_ || !ss_ || !intersectionGroup_ )
            {
                osg::notify( osg::NOTICE )
                    << "IntersectionUpdateCallback not set up correctly." << std::endl;
                return;
            }
            ...
            if( frameCount_ > 200 )
            {
                ...
                osg::dmat4      terrainLocalToWorld;
                osg::MatrixList terrain_worldMatrices =
                    terrain_->getWorldMatrices( root_.get() );
                if( terrain_worldMatrices.empty() )
                {
                    terrainLocalToWorld = osg::dmat4();
                }
                ...
                {
                    osg::notify( osg::NOTICE )
                        << "IntersectionUpdateCallback: warning cannot interestect with "
                           "multiple terrain instances, just uses first one."
                        << std::endl;
                    terrainLocalToWorld = terrain_worldMatrices.front();
                }

                // sphere segment is easier as this callback is attached to the node, so
                // the node visitor has the unique path to it already.
                osg::dmat4 ssWorldToLocal =
                    osg::computeWorldToLocal( nv->getNodePath() );

                // now we can compute the terrain to ss transform
                osg::dmat4 possie = terrainLocalToWorld * ssWorldToLocal;

                osgSim::SphereSegment::LineList lines =
                    ss_->computeIntersection( possie, terrain_.get() );
                if( !lines.empty() )
                {
                    if( intersectionGroup_.valid() )
                    {
                        // now we need to place the intersections which are in the
                        // SphereSegmenet's coordinate frame into to the final position.
                        ...
                        for( osgSim::SphereSegment::LineList::iterator itr =
                                 lines.begin();
                             itr != lines.end();
                             ++itr )
                        {
...
class RotateUpdateCallback : public osg::NodeCallback
{
    public:
                    ...
                    osg::NodeVisitor* /*nv*/ )
        {
            ...
            if( ss )
            {
                ss->setArea( osg::vec3( cos( i / ( 2 * osg::PI ) ),
                                        sin( i / ( 2 * osg::PI ) ),
                                        0 ),
                             osg::PI_2,
                             osg::PI_2 );
...
createMovingModel( const osg::vec3& center,
                   float            radius,
                   osg::Geode*      terrainGeode,
                   osg::Group*      root,
                   bool             createMovingRadar = false )
{
    ...
    {
        // The IntersectionUpdateCallback has to have a safe place to put all its
        // generated geometry into, and this group can't be in the parental chain of the
        // callback otherwise we will end up invalidating traversal iterators.
        ...
        osgSim::SphereSegment* ss =
            new osgSim::SphereSegment( osg::vec3( 0.0F, 0.0F, 0.0F ),
                                       700.0F,    // radius
                                       osg::radians( 135.0F ),
                                       osg::radians( 240.0F ),
                                       osg::radians( -60.0F ),
...
osg::Group*
createOverlay( const osg::vec3& center,
               float            radius )
{
    ...
    // create a grid of lines.
    {
        ...
        osg::vec3      delta_row =
            osg::vec3( 0.0F, 2.0F * radius / float( num_rows - 1 ), 0.0F );

        osg::vec3 top    = center + osg::vec3( -radius, radius, 0.0F );
        osg::vec3 bottom = center + osg::vec3( -radius, -radius, 0.0F );
        osg::vec3 delta_column =
            osg::vec3( 2.0F * radius / float( num_rows - 1 ), 0.0F, 0.0F );
        ...
        geom->addPrimitiveSet(
            new osg::DrawArrays( GL_LINES, 0, vertices->getNumElements() )
        );
...
class AdjustSphereSegmentCallback : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        scaleSphereSegment( osgSim::SphereSegment* ss,
                            float                  scale )
        {
            if( !ss )
            {
                ...
                case( osgGA::GUIEventAdapter::KEYDOWN ) :
                    {
                        ...
                        {
                            scaleSphereSegment(
                                dynamic_cast<osgSim::SphereSegment*>( object ),
                                1.1F
                            );
                            return true;
                        }
             ...
             osgSim::OverlayNode::OverlayTechnique technique )
{
    ...
    osg::ref_ptr<osgSim::SphereSegment> ss = 0;
    {
        ...
        osg::dmat4 terrainToSS;

        switch( testCase )
        {
            case( 0 ) :
                ss = new osgSim::SphereSegment(
                    computeTerrainIntersection( terrainGeode.get(),
                                                550.0F,
                                                780.0F ),    // center
                    510.0F,                                  // radius
                    osg::radians( 135.0F ),
            ...
            case( 4 ) :
                {
                    ss = new osgSim::SphereSegment( osg::vec3( 0.0F, 0.0F, 0.0F ),
                                                    700.0F,    // radius
                                                    osg::radians( 135.0F ),
                                                    osg::radians( 240.0F ),
                                                    osg::radians( -60.0F ),
                    ...
                    mt->setMatrix( osg::dmat4( -0.851781,
                                               0.156428,
                                               -0.5,
                                               0,
                                               -0.180627,
                    ...
                    terrainToSS = osg::inverse( mt->getMatrix() );
        ...
        if( ss.valid() )
        {
            ...
            {
                ss->getParent( 0 )->addChild(
                    ss->computeIntersectionSubgraph( terrainToSS, terrainGeode.get() )
                );
            }
    ...
    if( useOverlay )
    {
        ...
        const osg::sphere&   bs          = terrainGeode->getBound();
    ...
    {
        osg::vec3 position =
            computeTerrainIntersection( terrainGeode.get(), 100.0F, 100.0F );
    ...
    {
        osg::vec3 position =
            computeTerrainIntersection( terrainGeode.get(), 200.0F, 100.0F );
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        ...
        " is the example which demonstrates use of particle systems."
    );
