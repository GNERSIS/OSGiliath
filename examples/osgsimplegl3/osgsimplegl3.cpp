...
void
configureShaders( osg::StateSet* stateSet )
{
    ...
    const std::string fragmentSource = "#version 460 core \n"
                                       " \n"
                                       "in vec4 color; \n"
                                       "out vec4 fragData; \n"
                                       " \n"
      ...
      char** argv )
{
    ...
    if( root == NULL )
    {
        osg::notify( osg::FATAL )
            << "Unable to load model from command line." << std::endl;
        return ( 1 );
    }
    ...
    {
        osg::notify( osg::FATAL )
            << "Unable to create OpenGL v" << version << " context." << std::endl;
        return ( 1 );
    }
    ...
    cam->setProjectionMatrix(
        osg::perspective( 30., ( double )width / ( double )height, 1., 100. )
    );
    ...
    // for non GL3/GL4 and non GLES2 platforms we need enable the osg_ uniforms that the
    // shaders will use, you don't need thse two lines on GL3/GL4 and GLES2 specific
...
/* This version of OSG targets OpenGL 4.6 Core Profile exclusively.
   No legacy fixed-function pipeline configuration is required. */
