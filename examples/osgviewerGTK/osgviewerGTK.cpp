...
class ExampleOSGGTKDrawingArea : public OSGGTKDrawingArea
{
        ...
        void
        _menuAdd( const std::string& title )
        {
    ...
    protected:
                          ...
                          unsigned int button )
        {
            ...
            {
                gtk_menu_popup( GTK_MENU( _menu ), 0, 0, 0, 0, button, 0 );
            }
...
// Our main() function! FINALLY! Most of this code is GTK stuff, so it's mostly
// boilerplate. If we wanted to get real jiggy with it we could use Glade and cut down
// about 20 lines of code or so.
int
main( int    argc,
      char** argv )
{
    ...
    if( da.createWidget( 640, 480 ) )
    {
        ...
        {
            gtk_box_pack_start( GTK_BOX( vbox2 ), buttons[i], false, false, 0 );

            g_signal_connect( G_OBJECT( buttons[i] ),
                              "clicked",
