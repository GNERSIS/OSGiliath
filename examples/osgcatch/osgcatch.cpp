...
class Character : public osg::Referenced
{
    public:
        ...
        void
        setCharacter( const std::string& filename,
                      const std::string& name,
                      const osg::vec3&   orgin,
                      const osg::vec3&   width,
                      const osg::vec3&   catchPos,
                      float              positionRatio );

        void
        setLives( const std::string& filename,
                  const osg::vec3&   orgin,
                  const osg::vec3&   delta,
                  unsigned int       numLives );

        void
        setCatches( const std::string& filename,
                    const osg::vec3&   orgin,
                    const osg::vec3&   delta,
                    unsigned int       numLives );
        ...
        osg::vec3
        getCurrentCenterOfBasket() const
        {
            return osg::vec3( _character->getPosition() + osg::dvec3( _centerBasket ) );
        }

        float
        getCurrentRadiusOfBasket() const
        {
        ...
        getLowerLeft() const
        {
            return osg::vec3( _character->getPosition() );
        }
...
void
Character::setCharacter( const std::string& filename,
                         const std::string& name,
                         const osg::vec3&   origin,
                         const osg::vec3&   cwidth,
                         const osg::vec3&   catchPos,
                         float              positionRatio )
{
    _origin                                 = origin;
    ...
    float                    _characterSize = osg::length( _width ) * 0.2F;
    ...
    if( image )
    {
        ...
        osg::vec3 width(
            _characterSize * ( ( float )image->s() ) / ( float )( image->t() ),
            0.0F,
            0.0
        );
        ...
        _radiusBasket = osg::length( width ) * catchPos.z;
    }
}

void
Character::setLives( const std::string& filename,
                     const osg::vec3&   origin,
                     const osg::vec3&   delta,
                     unsigned int       numLives )
{
    float characterSize            = osg::length( delta );
    ...
    if( image )
    {
        ...
        for( unsigned int i = 0; i < numLives; ++i )
        {
            ...
            osg::vec3 width(
                characterSize * ( ( float )image->s() ) / ( float )( image->t() ),
                0.0F,
                0.0
            );
...
void
Character::setCatches( const std::string& filename,
                       const osg::vec3&   origin,
                       const osg::vec3&   delta,
                       unsigned int       numCatches )
{
    float characterSize            = osg::length( delta );
    ...
    if( image )
    {
        ...
        for( unsigned int i = 0; i < numCatches; ++i )
        {
            ...
            osg::vec3 height(
                0.0F,
                0.0F,
                characterSize * ( ( float )image->t() ) / ( float )( image->s() )
            );
...
void
Character::moveTo( float positionRatio )
{
    ...
    _positionRatio = positionRatio;
    _character->setPosition( osg::dvec3( _origin + _width * positionRatio ) );
}
...
class CatchableObject : public osg::Referenced
{
    public:
        ...
        void
        setObject( const std::string& filename,
                   const std::string& name,
        ...
        bool
        anyInside( const osg::vec3& lower_left,
                   const osg::vec3& top_right );

        bool
        centerInside( const osg::vec3& center,
                      float            radius );
        ...
        static void
        setUpCatchablesMap( const FileList& fileList );
...
void
CatchableObject::setUpCatchablesMap( const FileList& fileList )
{
    for( FileList::const_iterator itr = fileList.begin(); itr != fileList.end(); ++itr )
    {
        ...
        if( image )
        {
            ...
            osg::vec3      width( ( float )( image->s() ) / ( float )( image->t() ),
                                  0.0F,
                                  0.0 );
...
void
CatchableObject::setObject( const std::string& filename,
                            const std::string& name,
                            const osg::vec3&   center,
                            float              characterSize,
                            const osg::vec3&   velocity )
{
    ...
    {
        osg::PositionAttitudeTransform* scaleTransform =
            new osg::PositionAttitudeTransform;
        scaleTransform->setScale(
            osg::dvec3( characterSize, characterSize, characterSize )
        );
        scaleTransform->addChild( s_objectMap[filename].get() );

        _object = new osg::PositionAttitudeTransform;
        _object->setName( name );
        _object->setPosition( osg::dvec3( center ) );
        _object->addChild( scaleTransform );
    }
    else
    {
        osg::notify( osg::NOTICE )
            << "CatchableObject::setObject(" << filename
            << ") not able to create catchable object." << std::endl;
    }
}

void
CatchableObject::update( double dt )
{
    if( _stopped )
    {
        return;
    ...
    // compute force due to gravity + boyancy of displacing the fluid that the particle
    // is emersed in.
    ...
    force -=
        relative_wind *
        Area *
        ( _viscosityCoefficient + _densityCoefficeint * osg::length( relative_wind ) );
...
bool
CatchableObject::anyInside( const osg::vec3& lower_left,
                            const osg::vec3& upper_right )
{
...
bool
CatchableObject::centerInside( const osg::vec3& center,
                               float            radius )
{
    osg::vec3 delta( osg::vec3( _object->getPosition() ) - center );
    return ( osg::length( delta ) < radius );
}
...
CatchableObject::explode()
{
    ...
    osgParticle::ExplosionEffect* explosion =
        new osgParticle::ExplosionEffect( position, _radius );
    osgParticle::ExplosionDebrisEffect* explosionDebri =
        new osgParticle::ExplosionDebrisEffect( position, _radius );
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
class GameEventHandler : public osg::Inherit<osgGA::GUIEventHandler, GameEventHandler>
{
    public:
        ...
        GameEventHandler( const GameEventHandler& rhs,
        ...
        compileGLObjects( osg::State& state )
        {
            ...
            for( ObjectMap::iterator itr = s_objectMap.begin(); itr != s_objectMap.end();
                 ++itr )
            {
                ...
                case PLAYER_GIRL :
                    {
                        ...
                        _players[_numberOfPlayers].setCharacter(
                            player_one,
                            "girl",
                            _originBaseLine + osg::vec3( 0.0F, -1.0F, 0.0F ),
                            _widthBaseLine,
                            catchPos,
                            0.5F
                        );
                        _players[_numberOfPlayers].setLives(
                            player_one,
                            livesPosition,
                            osg::vec3( 0.0F, 0.0F, 100.0F ),
                            3
                        );
                        _players[_numberOfPlayers].setCatches(
                            "Catch/broach.png",
                            catchesPosition,
                            osg::vec3( 0.0F, 0.0F, 100.0F ),
                            10
                ...
                case PLAYER_BOY :
                    {
                        ...
                        _players[_numberOfPlayers].setCharacter(
                            player_two,
                            "boy",
                            _originBaseLine + osg::vec3( 0.0F, -2.0F, 0.0F ),
                            _widthBaseLine,
                            catchPos,
                            0.5F
                        );
        ...
        updateScoreWithLevel()
        {
            ...
            double       timeForLevel =
                osg::Timer::instance()->delta_s( _levelStartTick, newTick );

            // a ten second level gets you 10 points,
            // a twenty second levels gets you 5 points.
            _currentScore +=
                static_cast<unsigned int>( 10000.0F / ( timeForLevel * timeForLevel ) );
...
GameEventHandler::GameEventHandler()
{
    ...
    _characterSize            = osg::length( _width ) * 0.2F;
    ...
    setFOVY( osg::radians( 60.0 ) );
}

bool
GameEventHandler::handle( const osgGA::GUIEventAdapter& ea,
                          osgGA::GUIActionAdapter& )
{
            ...
            case( osgGA::GUIEventAdapter::FRAME ) :
                {
                         ...
                         ++itr )
                    {
                        ...
                        for( unsigned int i = 0; i < _numberOfPlayers; ++i )
                        {
                            bool inBasket = ( ( *itr )->centerInside(
                                _players[i].getCurrentCenterOfBasket(),
                                _players[i].getCurrentRadiusOfBasket()
                            ) );

                            if( ( *itr )->dangerous() )
                            {
                                if( ( *itr )->anyInside( _players[i].getLowerLeft(),
                                                         _players[i].getUpperRight() ) ||
                                    inBasket )
                                {
                                    // player has hit or caught a dangerous object, must
                                    // loose a life.
                        ...
                        if( !( *itr )->anyInside( _origin,
                                                  _origin + _width + _height ) ||
                        ...
                        else if( ( *itr )->anyInside( _origin, _origin + _width ) &&
                                 !( *itr )->stopped() )
                        {
                            // hit base line
                            ( *itr )->explode();
...
void
GameEventHandler::getUsage( osg::ApplicationUsage& ) const
{
}

osg::dmat4
GameEventHandler::getCameraPosition()
{
    osg::dmat4 cameraPosition;
    osg::vec3  center   = _origin + ( _width + _height ) * 0.5F;

    float      distance = osg::length( _height ) / ( 2.0F * tanf( _fovy * 0.5F ) );

    cameraPosition =
        osg::lookAt( osg::dvec3( center - osg::vec3( 0.0F, distance, 0.0F ) ),
                     osg::dvec3( center ),
...
GameEventHandler::createScene()
{
    ...
    _dummyCatchable->setObject( "Catch/a.png",
                                "a",
                                osg::vec3( 0.0F, 0.0, 0.0F ),
                                1.0F,
                                osg::vec3( 0.0F, 0.0, 0.0F ) );
    ...
    // set up welcome subgraph
    {
        ...
        osg::vec3 textPosition =
            _origin + _width * 0.5F + _height * 0.8F - osg::vec3( 0.0F, 0.1F, 0.0F );
        {
            osgText::Text* text = new osgText::Text;
            text->setText( "osgcatch is a childrens catching game\nMove your character "
                           "using the mouse to\ncatch falling objects in your net\nbut "
                           "avoid burning objects - they kill!!" );
            text->setFont( "fonts/dirtydoz.ttf" );
            text->setPosition( textPosition );
            text->setCharacterSize( osg::length( _width ) * 0.025F );
...

        {
            ...
            osgText::Text* text  = new osgText::Text;
            text->setText(
                "Move mouse left and right to move character\nCatch ten objects to "
...

        {
            textPosition        -= _height * 0.25F;
            osgText::Text* text  = new osgText::Text;
            text->setText( "Game concept and artwork - Caitlin Osfield, aged "
    ...
    // set up you've lost subgraph
    {
        ...
        // set up the text
        osg::vec3 textPosition =
            _origin + _width * 0.5F + _height * 0.75F - osg::vec3( 0.0F, 0.1F, 0.0F );
        {
            osgText::Text* text = new osgText::Text;
            text->setText( "Game Over\nYou lost all three lives" );
            text->setFont( "fonts/dirtydoz.ttf" );
            text->setPosition( textPosition );
            text->setCharacterSize( osg::length( _width ) * 0.04F );
            text->setColor( osg::vec4( 0.0F, 0.2F, 0.2F, 1.0F ) );
            text->setAlignment( osgText::Text::CENTER_CENTER );
            text->setAxisAlignment( osgText::Text::XZ_PLANE );
...

        {
            ...
            osgText::Text* text  = new osgText::Text;
            text->setText(
                "Press any key to have another game.\nPress Escape to exit game."
            );
            text->setFont( "fonts/dirtydoz.ttf" );
            text->setPosition( textPosition );
            text->setCharacterSize( osg::length( _width ) * 0.025F );
    ...
    // set up game subgraph.
    {
        ...
        // set up the text
        osg::vec3 textPosition =
            _origin + _width * 0.05F + _height * 0.95F - osg::vec3( 0.0F, 0.1F, 0.0F );
        {
            osgText::Text* text = new osgText::Text;
            text->setText( "Score : 0" );
            text->setFont( "fonts/dirtydoz.ttf" );
            ...
            _levelText->setCharacterSize( osg::length( _width ) * 0.04F );
            _levelText->setColor( osg::vec4( 0.0F, 0.2F, 0.2F, 1.0F ) );
            _levelText->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
void
GameEventHandler::createNewCatchable()
{
    ...
    unsigned int catachableIndex = ( unsigned int )( ( float )_benignCatachables.size() *
                                                     ( float )rand() /
                                                     ( float )RAND_MAX );
    ...
    float              angle =
        osg::PI * 0.25F + 0.5F * osg::PI * ( ( float )rand() / ( float )RAND_MAX );
    float            speed           = 200.0F * ( ( float )rand() / ( float )RAND_MAX );

    CatchableObject* catchableObject = new CatchableObject;
    osg::vec3        position =
        _origin + _height + _width * ratio + osg::vec3( 0.0F, -0.7F, 0.0F );
...
class CompileStateCallback : public osg::Operation
{
    public:
        ...
        CompileStateCallback( GameEventHandler* eh ) :
            osg::Referenced( true ),
            osg::Operation( "CompileStateCallback",
                            false ),
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setCommandLineUsage(
        arguments.getApplicationName() +
        " [options] image_file_left_eye image_file_right_eye"
    );
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "-x <float>",
        "Horizontal offset of left and right images."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-y <float>",
        "Vertical offset of left and right images."
    );
    ...
    // for some reason osgcatch is hanging on exit inside the new TextureObject clean up
    // code when the it's run as multi-threaded view, switching to SingleThreaded cures
    // this.
    ...
    // todo for osgViewer - implement warp pointer that can be done relative to different
    // coordinate frames viewer.requestWarpPointer(0.5f,0.5f);
