      ...
      char** argv )
{
    ...
    if( !scene && arguments.read( "--relative-camera-scene" ) )
    {
        ...
        camera->setViewMatrix( osg::translate( -2.0, 0.0, 0.0 ) );

        osg::MatrixTransform* xform =
            new osg::MatrixTransform( osg::translate( 1.0, 1.0, 1.0 ) );
    ...
    if( !scene )
    {
        scene = osgDB::readRefNodeFile( "lantern.glb" );
    }
    ...
    // Headless capture: before any viewer construction
    {
        std::string headlessOutput;
        if( arguments.read( "--headless", headlessOutput ) )
        {
            return osg::headlessCapture( group.get(), headlessOutput, 640, 480 ) ? 0 : 1;
        }
    }
