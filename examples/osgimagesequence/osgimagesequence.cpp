 ...
 * osgimagesequence example application
 */
#include <iostream>
...
//
// A simple demo demonstrating how to set on an animated texture using an
// osg::ImageSequence
//
...
createState( osg::ArgumentParser& arguments )
{
    ...
    if( !files.empty() )
    {
        for( osgDB::DirectoryContents::iterator itr = files.begin(); itr != files.end();
             ++itr )
        {
...
class MovieEventHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        setTrackMouse( bool tm )
        {
            ...
            std::cout << "tracking mouse: " << ( _trackMouse ? "ON" : "OFF" )
                      << std::endl;
        ...
        case( osgGA::GUIEventAdapter::FRAME ) :
            {
                ...
                ImageStreamPlayBackSpeedList::iterator fps_itr =
                    _imageStreamPlayBackSpeedList.begin();
                for( ImageStreamList::iterator itr = _imageStreamList.begin();
                     itr != _imageStreamList.end();
                     ++itr, ++fps_itr )
                {
                    if( ( ( *itr )->getStatus() == osg::ImageStream::PLAYING ) &&
                        ( ( *itr )->data() != ( *fps_itr ).lastData ) )
                    {
                        ImageStreamPlaybackSpeedData& data( *fps_itr );
                        double                        dt =
                            ( data.timeStamp > 0 ) ? t - data.timeStamp : 1 / 60.0;
        ...
        case( osgGA::GUIEventAdapter::MOVE ) :
            {
                if( _trackMouse )
                {
                    for( ImageStreamList::iterator itr = _imageStreamList.begin();
                         ...
                         ++itr )
                    {
                        double dt = ( *itr )->getLength() *
                                    ( ( 1.0 + ea.getXnormalized() ) / 2.0 );
                        ( *itr )->seek( dt );
                        std::cout << "seeking to " << dt
                                  << " length: " << ( *itr )->getLength() << std::endl;
                    }
                }
                return false;
            }
...
{
    usage.addKeyboardMouseBinding( "i",
                                   "toggle interactive mode, scrub via mouse-move" );
