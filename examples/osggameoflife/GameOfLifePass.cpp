...
ProcessPass::createTexturedQuad()
{
...
void
ProcessPass::setShader( std::string filename )
{
    ...
    if( !fshader )
    {
        osg::notify( osg::NOTICE )
            << "Could not file shader file: " << filename << std::endl;
        return;
    }
...
GameOfLifePass::getOutputTexture()
{
...
void
GameOfLifePass::createOutputTextures()
{
    for( int i = 0; i < 2; i++ )
    {
        ...
        _InOutTextureLife[i]->setFilter( osg::Texture2D::MIN_FILTER,
                                         osg::Texture2D::NEAREST );
        _InOutTextureLife[i]->setFilter( osg::Texture2D::MAG_FILTER,
                                         osg::Texture2D::NEAREST );
    }
}
