...
bool
windowClicked( osgWidget::Event& ev )
{
...
bool
windowScrolled( osgWidget::Event& ev )
{
...
struct Object
{
        ...
        windowClicked( osgWidget::Event& ev )
        {
            std::cout << "Object::windowClicked " << ev.getWindow()->getName()
                      << std::endl;
      ...
      char** argv )
{
    ...
    // expected. Note that you create a WindowManager with a NodeMask--it is very
    // important that this be unique for picking to work properly. This also makes it
    // possible to have multiple WindowManagers each operating on their own, unique set
    // of Window objects. The final bool argument is a group of flags that introduce
    // optional functionality for the WindowManager. In our case we include the flags
    // USE_PYTHON and USE_LUA, to demonstrate (and test) their usage. Finally, we pass
    // the temporary WM_NO_BETA_WARN argument, which prevents creating the orange warning
    // window. :) It will be shown in other examples...
    ...
    // An actual osgWidget::Window is pure virtual, so we've got to use the
    // osgWidget::Box implementation for now. At a later time, support for Tables and
    // other kinds of advanced layout Window types will be added.
    ...
    /*
    box->addCallback(new osgWidget::Callback(&windowClicked, osgWidget::EVENT_MOUSE_PUSH,
    &data)); box->addCallback(new osgWidget::Callback(&windowScrolled,
    osgWidget::EVENT_MOUSE_SCROLL)); box->addCallback(osgWidget::Callback(
    ...
    */
    ...
    osgWidget::Widget* widget1 =
        new osgWidget::NotifyWidget( "widget1", 300.0F, 100.0F );
    ...
    osg::dmat4 s = osg::scale(1.0f, -1.0f, 1.0f);
