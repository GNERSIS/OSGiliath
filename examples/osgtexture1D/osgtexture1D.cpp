 ...
 * osgtexture1D example application
 */
#include <iostream>
...
// Creates a stateset which contains a 1D texture which is populated by contour banded
// color, and allows tex gen to override the S texture coordinate
osg::StateSet*
create1DTextureStateToDecorate( osg::Node* /*loadedModel*/ )
{
    osg::Image* image    = new osg::Image;

    int         noPixels = 1'024;

    // allocate the image data, noPixels x 1 x 1 with 4 rgba floats - equivalent to a
    // vec4!
    ...
    osg::Uniform* texUniform =
        new osg::Uniform( osg::Uniform::SAMPLER_1D, "contourTex" );
    texUniform->set( 0 );
...
class AnimateTexGenCallback : public osg::NodeCallback
{
    public:
        ...
        AnimateTexGenCallback( osg::Uniform*     planeUniform,
                               osg::Uniform*     modeUniform,
                               const osg::Plane& plane ) :
            _planeUniform( planeUniform ),
            _modeUniform( modeUniform ),
            _plane( plane )
        {
        }
        ...
        animate( double time )
        {
            ...
            _planeUniform->set(
                osg::vec4( _plane[0], _plane[1], _plane[2], _plane[3] )
            );
        }
      ...
      char** argv )
{
    ...
    if( !loadedModel )
    {
        osg::notify( osg::NOTICE )
            << arguments.getApplicationUsage()->getCommandLineUsage() << std::endl;
        return 0;
    }

    osg::ref_ptr<osg::StateSet> stateset =
        create1DTextureStateToDecorate( loadedModel.get() );
    if( !stateset )
    {
    ...
    osg::Uniform*      planeUniform =
        new osg::Uniform( "texGenPlaneS",
    ...
    texgenGroup->setUpdateCallback(
        new AnimateTexGenCallback( planeUniform, modeUniform, sPlane )
    );
