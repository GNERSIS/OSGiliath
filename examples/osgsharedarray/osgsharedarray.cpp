 ...
 * osgsharedarray example application
 */
...
class MyArray : public osg::Array
{
    public:
        ...
        MyArray() :
            osg::Array( osg::Array::Vec3ArrayType,
                        3,
                        GL_FLOAT ),
            _numElements( 0 ),
            _ptr( NULL )
         ...
         */
        virtual void
        accept( osg::ConstArrayVisitor& cav ) const
        {
            cav.apply( *this );
        }
        ...
        virtual void
        accept( unsigned int            index,
                osg::ConstValueVisitor& cvv ) const
        {
...
createGeometry()
{
    ...
    geom->setVertexArray( new MyArray( numVertices,
                                       const_cast<osg::vec3*>( &myVertices[0] ) ) );

    // add normals
    unsigned int numNormals = sizeof( myNormals ) / sizeof( myNormals[0] );
    geom->setNormalArray( new osg::Vec3Array( numNormals,
                                              const_cast<osg::vec3*>( &myNormals[0] ) ),
                          osg::Array::BIND_PER_VERTEX );

    // add colors
    unsigned int    numColors = sizeof( myColors ) / sizeof( myColors[0] );
    osg::Vec4Array* normal_array =
        new osg::Vec4Array( numColors, const_cast<osg::vec4*>( &myColors[0] ) );
    geom->setColorArray( normal_array, osg::Array::BIND_PER_VERTEX );

    // add PrimitiveSet - decompose quads to triangles for Core Profile
