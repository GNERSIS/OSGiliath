 ...
 * osgkeyboardmouse example application
 */
...
class CreateModelToSaveVisitor : public osg::DualModeVisitor
{
    public:
...
class SelectedNodesVisitor : public osg::DualModeVisitor
{
    public:
        ...
        createSelectedNodeSubgraph()
        {
            ...
            // note code doesn't yet handle selected nodes being nested within
            // transforms.
...
class PickHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        setPrimitiveMask( unsigned int primitiveMask )
        {
                ...
                case( osgGA::GUIEventAdapter::KEYUP ) :
                    {
                        ...
                        else if( ea.getKey() == 'o' )
                        {
                            ...
                            // osg::notify(osg::NOTICE)<<"Saved model to file
                            // 'saved_model.osgt'"<<std::endl;
                            // osgDB::writeNodeFile(*(viewer->getSceneData()),
                            // "saved_model.osgt");
                        }
                        else if( ea.getKey() == 's' )
                        {
                            ...
                            osg::ref_ptr<osg::Node> selected =
                                snv.createSelectedNodeSubgraph();

                            if( selected )
                            {
                                // .osgt format removed; debug save disabled
                                // osg::notify(osg::NOTICE)<<"Saved selected to file
                                // 'saved_selected.osgt'"<<std::endl;
                                // osgDB::writeNodeFile(*(viewer->getSceneData()),
                                // "saved_selected.osgt");
                            }
                        }
                        else if( ea.getKey() == 'p' )
                        {
                            ...
                            if( _usePolytopeIntersector )
                            {
                                osg::notify( osg::NOTICE )
                                    << "Using PolytopeIntersector" << std::endl;
                            }
                            else
                            {
                                osg::notify( osg::NOTICE )
                                    << "Using LineSegmentIntersector" << std::endl;
                            }
                        }
                        else if( ea.getKey() == 'c' )
                        {
                            ...
                            {
                                osg::notify( osg::NOTICE )
                                    << "Using window coordinates for picking"
                                    << std::endl;
                            }
                            else
                            {
                                osg::notify( osg::NOTICE )
                                    << "Using projection coordinates for picking"
                                    << std::endl;
                            }
                        }
                        else if( ea.getKey() == 'a' )
        ...
        void
        fullWindowIntersectionTest( osgViewer::Viewer* viewer )
        {
            ...
            OSG_NOTICE << "Intersection traversal took " << elapsedTime.elapsedTime_m()
                       << "ms for " << intersectors->getIntersectors().size()
                       << " intersectors" << std::endl;
        }

        void
        pick( const osgGA::GUIEventAdapter& ea,
              osgViewer::Viewer*            viewer )
        {
            ...
            if( _usePolytopeIntersector )
            {
                ...
                if( _useWindowCoordinates )
                {
                    ...
                    osg::Viewport* viewport = viewer->getCamera()->getViewport();
                    double         mx =
                        viewport->x() + ( int )( ( double )viewport->width() *
                                                 ( ea.getXnormalized() * 0.5 + 0.5 ) );
                    double my =
                        viewport->y() + ( int )( ( double )viewport->height() *
                ...
                OSG_NOTICE << "PoltyopeIntersector traversal took "
                           << elapsedTime.elapsedTime_m() << "ms" << std::endl;
                ...
                if( picker->containsIntersections() )
                {
                    ...
                    osg::notify( osg::NOTICE )
                        << "Picked " << intersection.localIntersectionPoint << std::endl
                        ...
                        << intersection.numIntersectionPoints << std::endl;
                    ...
                    if( node )
                    {
                        std::cout << "  Hits " << node->className() << " nodePath size "
                                  << nodePath.size() << std::endl;
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    osg::Viewport* viewport = viewer->getCamera()->getViewport();
                    float          mx =
                        viewport->x() + ( int )( ( float )viewport->width() *
                                                 ( ea.getXnormalized() * 0.5F + 0.5F ) );
                    float my =
                        viewport->y() + ( int )( ( float )viewport->height() *
                                                 ( ea.getYnormalized() * 0.5F + 0.5F ) );
                    picker = new osgUtil::LineSegmentIntersector(
                        osgUtil::Intersector::WINDOW,
                        mx,
                        my
                ...
                OSG_NOTICE << "LineSegmentIntersector traversal took "
                           << elapsedTime.elapsedTime_m() << "ms" << std::endl;
        ...
        void
        saveSelectedModel( osg::Node* scene )
        {
            if( !scene )
            {
                return;
            ...
            {
                // .osgt format removed; debug save disabled
                // std::cout<<"Writing selected components to
                // 'selected_model.osgt'"<<std::endl; osgDB::writeNodeFile(*cmtsv._group,
                // "selected_model.osgt");
            }
        }
...
class ConvertPrimitives : public osg::DualModeVisitor
{
    public:
      ...
      char** argv )
{
    ...
    while( arguments.read( "--kdtree" ) )
    {
        useKdTree = true;
    }

    osg::ref_ptr<PickHandler> pickhandler = new PickHandler;
    while( arguments.read( "--double" ) )
    {
        pickhandler->setPrecisionHint( osgUtil::Intersector::USE_DOUBLE_CALCULATIONS );
    }
    while( arguments.read( "--float" ) )
    {
        pickhandler->setPrecisionHint( osgUtil::Intersector::USE_FLOAT_CALCULATIONS );
    }

    unsigned int mask = osgUtil::PolytopeIntersector::ALL_PRIMITIVES;
    while( arguments.read( "--prim-mask", mask ) || arguments.read( "--pm", mask ) )
    {
        pickhandler->setPrimitiveMask( mask );
    }
    ...
    {
        ConvertPrimitives cp( osg::PrimitiveSet::POINTS );
        loadedModel->accept( cp );
    }
    while( arguments.read( "--lines" ) )
    {
        ConvertPrimitives cp( osg::PrimitiveSet::LINES );
        loadedModel->accept( cp );
    }
    ...
    // main loop (note, window toolkits which take control over the main loop will
    // require a window redraw callback containing the code below.)
    while( !viewer.done() )
    {
        viewer.frame();
    }
