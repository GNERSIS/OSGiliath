 ...
 * osguserstats example application
 */
#include <iostream>
...
void
initUserStats( osgViewer::StatsHandler* statsHandler )
{
    // This line displays the frame number. It's not averaged, just displayed as is.
    statsHandler->addUserStatsLine( "Frame",
                                    osg::vec4( 0.7, 0.7, 0.7, 1 ),
                                    osg::vec4( 0.7, 0.7, 0.7, 0.5 ),
                                    frameNumberName,
                                    1.0,
                                    false,
                                    false,
    ...
    // This line displays the frame time (from beginning of event to end of draw). No
    // bars.
    statsHandler->addUserStatsLine( "MS/frame",
                                    osg::vec4( 1, 0, 1, 1 ),
                                    osg::vec4( 1, 0, 1, 0.5 ),
                                    frameTimeName,
                                    1000.0,
    ...
    statsHandler->addUserStatsLine( "Custom",
                                    osg::vec4( 1, 1, 1, 1 ),
                                    osg::vec4( 1, 1, 1, 0.5 ),
                                    customTimeName + " time taken",
                                    1000.0,
                                    true,
                                    false,
                                    customTimeName + " begin",
                                    customTimeName + " end",
                                    0.016 );

    // This line displays the time taken by a function below ( doSomethingAndTimeIt() )
    statsHandler->addUserStatsLine( "Sleep1",
                                    osg::vec4( 1, 0, 0, 1 ),
                                    osg::vec4( 1, 0, 0, 0.5 ),
                                    operation1TimeName + " time taken",
                                    1000.0,
                                    true,
                                    false,
                                    operation1TimeName + " begin",
                                    operation1TimeName + " end",
                                    0.016 );

    // This line displays the time taken by a function below ( doSomethingAndTimeIt() )
    statsHandler->addUserStatsLine( "Sleep2",
                                    osg::vec4( 1, 0.5, 0.5, 1 ),
                                    osg::vec4( 1, 0.5, 0.5, 0.5 ),
                                    operation2TimeName + " time taken",
                                    1000.0,
                                    true,
                                    false,
                                    operation2TimeName + " begin",
                                    operation2TimeName + " end",
                                    0.016 );

    // This line displays the time taken by a function below ( doSomethingAndTimeIt() )
    statsHandler->addUserStatsLine( "Thread",
                                    osg::vec4( 0, 0.5, 0, 1 ),
                                    osg::vec4( 0, 0.5, 0, 0.5 ),
                                    otherThreadTimeName + " time taken",
                                    1000.0,
                                    true,
                                    false,
                                    otherThreadTimeName + " begin",
                                    otherThreadTimeName + " end",
                                    0.016 );
}

void
updateUserStats( osgViewer::Viewer& viewer )
{
    // Test the custom stats line by just adding up the update and cull
    // times for the viewer main camera for the previous frame.
    if( viewer.getViewerStats()->collectStats( "update" ) &&
        viewer.getCamera()->getStats()->collectStats( "rendering" ) )
    {
        ...
        viewer.getViewerStats()->getAveragedAttribute( "Update traversal time taken",
                                                       update );
        viewer.getCamera()->getStats()->getAveragedAttribute(
        ...
        viewer.getViewerStats()->getAttribute( framenumber - 2,
                                               "Event traversal begin time",
                                               eventBegin );
        viewer.getViewerStats()->getAttribute( framenumber - 2,
                                               "Update traversal begin time",
                                               updateBegin );
        viewer.getCamera()->getStats()->getAttribute( framenumber - 2,
        ...
        // This line displays the frame number. It's not averaged, just displayed as is.
        viewer.getViewerStats()->setAttribute( framenumber,
                                               frameNumberName,
                                               framenumber );

        // This line displays the frame time (from beginning of event to end of draw). No
        // bars.
        viewer.getViewerStats()->setAttribute( framenumber - 1,
                                               frameTimeName,
                                               drawEnd - eventBegin );
...
void
startTiming( osgViewer::Viewer& viewer,
...
/// recorded times will all be consistent using the viewer's time.
void
endTiming( osgViewer::Viewer& viewer,
           const std::string& name )
{
    osg::Timer_t tick  = osg::Timer::instance()->tick();
    double currentTime = osg::Timer::instance()->delta_s( viewer.getStartTick(), tick );
...
void
doSomethingAndTimeIt( osgViewer::Viewer& viewer,
                      const std::string& name,
                      double             milliseconds )
{
    ...
    while( osg::Timer::instance()->delta_m( startTick, osg::Timer::instance()->tick() ) <
           milliseconds )
    {
...
/// called.
class UselessThread : public osg::Thread
{
    public:

        UselessThread( osgViewer::Viewer& viewer,
                       double             timeToRun ) :
            _viewer( viewer ),
        ...
        run()
        {
            while( !_done )
            {
                if( _process )
                {
                    ...
                    // Do nothing for the specified number of  milliseconds, just so we
                    // can see it in the stats.
                    osg::Timer_t startTick = osg::Timer::instance()->tick();
                    while( osg::Timer::instance()->delta_m(
                               startTick,
