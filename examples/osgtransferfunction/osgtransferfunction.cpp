 ...
 * osgtransferfunction example application
 */
...
class Histogram
{
    public:
        ...
        void
        analyse( const osg::Image* image,
                 double            interval = 0.0 );

        void
        insertZeroBoundaryValues( float xMin = FLT_MAX,
                                  float xMax = -FLT_MAX );
...
void
Histogram::analyse( const osg::Image* image,
                    double /*interval*/ )
{
...
void
Histogram::insertZeroBoundaryValues( float xMin,
                                     float xMax )
{
    if( _valueMap.empty() )
    {
...
osg::Node*
Histogram::createGraphicalRepresentation()
{
    if( _valueMap.empty() )
    {
        return 0;
    ...
    // find yMax
    for( ValueMap::iterator itr = _valueMap.begin(); itr != _valueMap.end(); ++itr )
    {
        float y = itr->second;
        if( y > yMax )
        {
...

    {
        ...
        osg::ref_ptr<osg::DrawElementsUShort> background_primitives =
            new osg::DrawElementsUShort( GL_TRIANGLE_STRIP );
        osg::ref_ptr<osg::DrawElementsUShort> historgram_primitives =
            new osg::DrawElementsUShort( GL_TRIANGLE_STRIP );
        osg::ref_ptr<osg::DrawElementsUShort> outline_primitives =
    ...
    // transform->setMatrix(osg::scale(xScale/(maxX-minY), yScale/(yMax), 1.0f));

    transform->setMatrix( osg::rotate( osg::radians( 90.0 ),
...
readTransferFunctionFile( const std::string& filename,
                          float              colorScale = 1.0F )
{
    std::string foundFile = osgDB::findDataFile( filename );
    if( foundFile.empty() )
    {
        std::cout << "Error: could not find transfer function file : " << filename
                  << std::endl;
        return 0;
    }
    ...
    while( fin )
    {
        ...
        if( fin )
        {
            std::cout << "value = " << value << " (" << red << ", " << green << ", "
                      << blue << ", " << alpha << ")" << std::endl;
            colorMap[value] = osg::vec4( red * colorScale,
                                         green * colorScale,
                                         blue * colorScale,
                                         alpha * colorScale );
        }
    ...
    if( colorMap.empty() )
    {
        std::cout << "Error: No values read from transfer function file: " << filename
                  << std::endl;
        return 0;
    }
...
#endif
        virtual void
        apply( osgVolume::TransferFunctionProperty& tfp )
        {
...
class InsertTransferFunctionPropertyVisitor : public osgVolume::PropertyVisitor
{
    public:
        ...
        virtual void
        apply( osgVolume::SwitchProperty& sp )
        {
            OSG_NOTICE << "Found SwitchProperty" << std::endl;
        ...
        virtual void
        apply( osgVolume::CompositeProperty& cp )
        {
            OSG_NOTICE << "Found CompositeProperty, inserting transfer function"
                       << std::endl;
            if( _tfp.valid() )
            {
...
class FindVolumeTiles : public osg::DualModeVisitor
{
    public:
...
class MyScriptCallback : public osg::CallbackObject
{
    public:
        ...
        MyScriptCallback( osg::ScriptEngine* se,
                          osg::Script*       script,
             ...
             osg::Parameters& outputParameters ) const
        {
            ...
            return _scriptEngine->run( _script.get(),
                                       getName(),
                                       inputParameters,
                                       outputParameters );
        }
...
class MyClass : public osg::Inherit<osg::Object, MyClass>
{
    public:

        OSG_REGISTER_TYPE( local,
                           MyClass )
      ...
      char** argv )
{
    ...
    if( model.valid() )
    {
        osg::ref_ptr<osgVolume::VolumeTile> volumeTile =
            dynamic_cast<osgVolume::VolumeTile*>( model.get() );
        if( volumeTile.valid() )
        {
    ...
    if( imageLayer )
    {
        ...
        if( property )
        {
            ...
            if( ftfpv._tfp.valid() )
            {
                if( tf.valid() )
                {
                    OSG_NOTICE << "Need to replace volumes transfer function"
                               << std::endl;
                ...
                else
                {
                    ...
                    tf = dynamic_cast<osg::TransferFunction1D*>(
                        ftfpv._tfp->getTransferFunction()
                    );
                }
            }
            else if( tf.valid() )
            {
                ...
                OSG_NOTICE << "Need to assign transfer function to CompositeProperty"
                           << std::endl;
        ...
        else if( tf.valid() )
        {
            ...
            imageLayer->setProperty(
                new osgVolume::TransferFunctionProperty( tf.get() )
            );
        }
    }
    ...
    else
    {
        ...
        for( Nodes::iterator itr = nodes.begin(); itr != nodes.end(); ++itr )
        {
            ...
            pat->setPivotPoint( osg::dvec3( bb.center ) );
            pat->setScale( osg::dvec3( scale, scale, scale ) );
            position.x += 1.1;
