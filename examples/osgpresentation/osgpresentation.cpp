      ...
      char** argv )
{
    ...
    osg::ref_ptr<osgPresentation::Presentation> presentation =
    ...
    model->setProperty( "filename", std::string( "milk_truck.glb" ) );
    ...
    movie->setProperty(
        "filename",
        std::string( "/home/robert/Data/Movie/big_buck_bunny_1080p_stereo.ogg" )
    );
    movie->setProperty( "scale", 0.75 );

    volume->setProperty( "filename",
                         std::string( "/home/robert/Data/MaleVisibleHumanHead" ) );
    ...
    pi.getWhiteList()["osgPresentation::Presentation"]["filename"] =
    ...
    OSG_NOTICE<<std::endl;
    // presentation->setDataVariance(osg::Object::DataVariance::DYNAMIC);
    ...
    presentation->setMatrix(osg::translate(osg::dvec3(1.0,2.0,3.0)));

//    if (pi.setProperty(presentation.get(), "dmat4", osg::scale(1.0,2.0,2.0)))
    if (pi.setProperty(presentation.get(), "dmat4", osg::scale(2.0,2.0,2.0)))
    {
        OSG_NOTICE<<"setProperty(..,dmat4) succeeded."<<std::endl;
    }
    else
    {
        OSG_NOTICE<<"setProperty(..,dmat4) failed."<<std::endl;
    }

    osg::dmat4 matrix;
    if (pi.getProperty(presentation.get(), "dmat4", matrix))
    {
        OSG_NOTICE<<"getProperty(presentation.get(), ...) succeeded, dmat4 = "<<matrix<<std::endl;
    }
    else
    {
    ...
    OSG_NOTICE<<"Type(osg::dmat4) = "<<osgDB::getTypeEnum<osg::dmat4>()<<", "<<osgDB::getTypeString<osg::dmat4>()<<std::endl;
    OSG_NOTICE<<"Type(osg::uivec2) = "<<osgDB::getTypeEnum<osg::uivec2>()<<", "<<osgDB::getTypeString<osg::uivec2>()<<std::endl;
    ...
    osg::dmat4 mymatrix = osg::translate(-1,2,3);
    pi.setProperty(presentation.get(), "mymatrix", mymatrix);

    osg::dmat4 copyofmatrix;
    if (pi.getProperty(presentation.get(), "mymatrix", copyofmatrix))
    {
        OSG_NOTICE<<"mymatrix = "<<copyofmatrix<<std::endl;
    }

    if (pi.getProperty(presentation.get(), "dmat4", copyofmatrix))
    {
        OSG_NOTICE<<"dmat4 = "<<copyofmatrix<<std::endl;
    }
    ...
    else
    {
        ...
        #define PRINT_TYPE( O, PN )                                            \
            {                                                                  \
                ...
                {                                                              \
                    OSG_NOTICE << #PN << " : type " << type << ", "            \
                               << pi.getTypeName( type ) << std::endl;         \
                }                                                              \
                else                                                           \
                {                                                              \
    ...
    PRINT_TYPE(presentation.get(), dmat4)
    ...
    osg::vec3 pos( 1.5, 3.0, 4.5 );
    presentation->setProperty( "position", pos );

    osg::vec2 texcoord( 0.5F, 0.20F );
    ...
    if( luaScriptEngine.valid() )
    {
        presentation->getOrCreateUserDataContainer()->addUserObject(
            luaScriptEngine.get()
        );
        std::string str;
        while( arguments.read( "--lua", str ) )
        {
            ...
            if( script.valid() )
            {
                presentation->addUpdateCallback(
                    new osg::ScriptNodeCallback( script.get(), "update" )
                );
            }
        }
        ...
        if( arguments.read( "--test", str ) )
        {
            ...
            if( script.valid() )
            {
                ...
                inputParameters.push_back(
                    new osg::StringValueObject( "string", "my very first string input" )
                );
                inputParameters.push_back( new osg::DoubleValueObject( "double",
                                                                       1.234 ) );
                inputParameters.push_back( new osg::MatrixfValueObject( "matrix",
                                                                        osg::mat4() ) );

                osg::ref_ptr<osg::MatrixdValueObject> svo =
                    new osg::MatrixdValueObject( "return", osg::dmat4() );
                outputParameters.push_back( svo.get() );

                if( luaScriptEngine->run( script.get(),
                                          "test",
                                          inputParameters,
                                          outputParameters ) )
                {
                    OSG_NOTICE
                        << "Successfully ran script : return value = " << svo->getValue()
                        << std::endl;
                }
                else
                {
    ...
    if( pi.getSupportedProperties( obj.get(), properties, true ) )
    {
        ...
        for( osgDB::ClassInterface::PropertyMap::iterator itr = properties.begin();
             ...
             ++itr )
        {
            OSG_NOTICE << "   Property " << itr->first << ", "
                       << pi.getTypeName( itr->second ) << std::endl;
        }
    }
    else
    {
