...
bool
DirectInputRegistry::initKeyboard( HWND handle )
{
    if( !_inputDevice )
    {
        return false;
...
bool
DirectInputRegistry::initMouse( HWND handle )
{
    if( !_inputDevice )
    {
        return false;
...
bool
DirectInputRegistry::initJoystick( HWND handle )
{
    if( !_inputDevice )
    {
        return false;
    ...
    HRESULT hr = _inputDevice->EnumDevices( DI8DEVCLASS_GAMECTRL,
...
void
DirectInputRegistry::updateState( osgGA::EventQueue* eventQueue )
{
    HRESULT hr;
...
bool
DirectInputRegistry::initImplementation( HWND                 handle,
...
void
DirectInputRegistry::pollDevice( LPDIRECTINPUTDEVICE8 device )
{
...
void
DirectInputRegistry::releaseDevice( LPDIRECTINPUTDEVICE8 device )
{
    if( device )
    {
        device->Unacquire();
...
BOOL CALLBACK
DirectInputRegistry::EnumJoysticksCallback( const DIDEVICEINSTANCE* didInstance,
                                            VOID* )
{
    HRESULT        hr;
    ...
    if( device )
    {
        hr = device->CreateDevice( didInstance->guidInstance,
                                   &( DirectInputRegistry::instance()->getJoyStick() ),
                                   NULL );
        if( SUCCEEDED( hr ) )
        {
            return DIENUM_STOP;
