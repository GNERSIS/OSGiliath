 ...
 * osgterrain example application
 */
#include <iostream>
...
class TerrainHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        toggleDefine( const std::string& defineName,
                      int                expectedDefault = osg::StateAttribute::ON )
        {
            osg::StateSet::DefineList& defineList =
                _terrain->getOrCreateStateSet()->getDefineList();
            osg::StateSet::DefineList::iterator itr = defineList.find( defineName );
            if( itr == defineList.end() )
            {
                defineList[defineName].second =
                    ( expectedDefault |
                      osg::StateAttribute::OVERRIDE );    // assume the defines start
                                                          // off.
    ...
    protected:
        ...
        void
        assignTextureWeightToSingleTextureUnit( unsigned int unit )
        {
            if( !_mtc )
            {
                return;
...
class CleanTechniqueReadFileCallback : public osgDB::ReadFileCallback
{
    ...
    public:
        ...
        class CleanTechniqueVisitor : public osg::DualModeVisitor
        {
            public:
                ...
                apply( osg::Node& node )
                {
                    ...
                    if( tile )
                    {
                        ...
                        {
                            // OSG_NOTICE<<"Resetting TerrainTechnhique
                            // "<<tile->getTerrainTechnique()->className()<<" to
                            // 0"<<std::endl;
                  ...
                  const osgDB::Options* options )
        {
            osgDB::ReaderWriter::ReadResult rr =
                ReadFileCallback::readNode( filename, options );
            if( rr.validNode() )
            {
      ...
      char** argv )
{
    ...
    if( useDisplacementMappingTechnique )
    {
        osgDB::Registry::instance()->setReadFileCallback(
            new CleanTechniqueReadFileCallback()
        );
    }
    ...
    if( !rootnode )
    {
        osg::notify( osg::NOTICE ) << "Warning: no valid data loaded, please specify a "
    ...
    if( !terrain )
    {
        ...
        // if CoordinateSystemNode is present copy it's contents into the Terrain, and
        // discard it.
    ...
    if( useDisplacementMappingTechnique )
    {
        terrain->setTerrainTechniquePrototype(
            new osgTerrain::DisplacementMappingTechnique()
        );
    }

    // register our custom handler for adjust Terrain settings
    viewer.addEventHandler( new TerrainHandler(
        terrain.get(),
        findTopMostNodeOfType<osgFX::MultiTextureControl>( rootnode.get() )
    ) );
    ...
    // viewer.setSceneData() so that the osgViewer::Scene object is constructed with it's
    // DatabasePager.
    if( setDatabaseThreadAffinity )
    {
        for( unsigned int i = 0; i < viewer.getDatabasePager()->getNumDatabaseThreads();
             ++i )
        {
            ...
            OSG_NOTICE << "Settings affinity of DatabaseThread=" << thread
                       << " isRunning()=" << thread->isRunning() << " cpuNum=" << cpuNum
                       << std::endl;
        }
    }
    ...
    // terrain->getOrCreateStateSet()->setDefine("LIGHTING"); // ,
    // osg::StateAttribute::OFF|osg::StateAttribute::OVERRIDE);
    // terrain->getOrCreateStateSet()->setDefine("COMPUTE_DIAGONALS"); // ,
    // osg::StateAttribute::OFF|osg::StateAttribute::OVERRIDE);
