 ...
 * osgcluster example application
 */
#ifdef USE_MEM_CHECK
    #include <mcheck.h>
#endif
...
const unsigned int SWAP_BYTES_COMPARE = 0X12'34'56'78;
...
class CameraPacket
{
    public:
        ...
        void
        setPacket( const osg::dmat4&      matrix,
        ...
        void
        getModelView( osg::dmat4& matrix,
                      float       angle_offset = 0.0F )
        {

            matrix =
                _matrix * osg::rotate( osg::radians( angle_offset ), 0.0F, 1.0F, 0.0F );
        }

        void
        readEventQueue( osgViewer::Viewer& viewer );

        void
        writeEventQueue( osgViewer::Viewer& viewer );

        void
        setMasterKilled( const bool flag )
        {
            _masterKilled = flag;
        }

        bool
        getMasterKilled() const
        {
...
class DataConverter
{
    public:
        ...
        void
        write( const osg::FrameStamp& fs )
        {
            osg::notify( osg::NOTICE ) << "writeFramestamp = " << fs.getFrameNumber()
                                       << " " << fs.getReferenceTime() << std::endl;
        ...
        void
        read( osg::FrameStamp& fs )
        {
            fs.setFrameNumber( readUInt() );
            fs.setReferenceTime( readDouble() );
            fs.setSimulationTime( readDouble() );

            osg::notify( osg::NOTICE ) << "readFramestamp = " << fs.getFrameNumber()
                                       << " " << fs.getReferenceTime() << std::endl;
        }

        void
        write( const osg::dmat4& matrix )
        {
        ...
        void
        write( const osgGA::GUIEventAdapter& event )
        {
        ...
        void
        read( osgGA::GUIEventAdapter& event )
        {
        ...
        void
        write( CameraPacket& cameraPacket )
        {
        ...
        void
        read( CameraPacket& cameraPacket )
        {
...
void
CameraPacket::readEventQueue( osgViewer::Viewer& viewer )
{
    _events.clear();
...
void
CameraPacket::writeEventQueue( osgViewer::Viewer& viewer )
{
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        arguments.getApplicationName() + " is the example which demonstrates how to "
                                         "approach implementation of clustering."
    );
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "-m",
        "Set viewer to MASTER mode, sending view via packets."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-s",
        "Set viewer to SLAVE mode, receiving view via packets."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-n <int>",
        "Socket number to transmit packets"
    );
    arguments.getApplicationUsage()->addCommandLineOption( "-f <float>",
                                                           "Field of view of camera" );
    arguments.getApplicationUsage()->addCommandLineOption( "-o <float>",
                                                           "Offset angle of camera" );
    ...
    std::string ifrName;
    if( arguments.read( "--ifr-name", ifrName ) )
    {
        bc.setIFRName( ifrName );
    }
    ...
    if( camera_fov > 0.0F )
    {
        ...
        double original_fov =
            atan( tan( osg::radians( fovy ) * 0.5 ) * aspectRatio ) * 2.0;
        std::cout << "setting lens perspective : original " << original_fov << "  "
                  << fovy << std::endl;

        fovy = atan( tan( osg::radians( camera_fov ) * 0.5 ) / aspectRatio ) * 2.0;
        ...
        viewer.getCamera()->getProjectionMatrixAsPerspective( fovy,
                                                              ...
                                                              zNear,
                                                              zFar );
        original_fov = atan( tan( osg::radians( fovy ) * 0.5 ) * aspectRatio ) * 2.0;
        std::cout << "setting lens perspective : new " << original_fov << "  " << fovy
                  << std::endl;
    }
    ...
    if( readWriteFrame )
    {
        ...
        {
            OSG_NOTICE << "Setting up RGBA osg::Image, width = " << width
                       << ", height = " << height << std::endl;
            image->allocateImage( width, height, 1, GL_RGBA, GL_UNSIGNED_BYTE );
        }
        else
        {
            OSG_NOTICE << "Setting up RGB osg::Image, width = " << width
                       << ", height = " << height << std::endl;
            image->allocateImage( width, height, 1, GL_RGB, GL_UNSIGNED_BYTE );
        }

        // if (image->getImageStepInBytes()>messageSize) messageSize =
        // image->getImageStepInBytes();
    }
    ...
    while( !viewer.done() && !masterKilled )
    {
            ...
            case( SLAVE ) :
                {
                    ...
                    if( readsize )
                    {
                        std::cout << std::endl << "readsize = " << readsize << std::endl;
                    }
                    ...
                    if( image )
                    {
                        ...
                        if( readsize )
                        {
                            std::cout << "image readsize = " << readsize << std::endl;
                        }
                    }
                }
                break;
        ...
        osg::notify( osg::INFO )
            << "Time to do cluster sync "
            << osg::Timer::instance()->delta_m( startTick, endTick ) << std::endl;
    ...
    if( viewerMode == MASTER )
    {
        ...
        cp->setPacket( osg::dmat4(), viewer.getFrameStamp() );
