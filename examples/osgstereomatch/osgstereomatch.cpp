 ...
 * osgstereomatch example application
 */
#include "StereoMultipass.hpp"
#include "StereoPass.hpp"
             ...
             bool         single_pass )
{
    ...
    // each geom will contain a quad (two triangles)
    ...
    for( int i = 0; i < 4; i++ )
    {
        ...
        vcoords->push_back(
            osg::vec3( 0 + ( xoff * width ), 0, 0 + ( zoff * height ) )
        );
        vcoords->push_back(
            osg::vec3( width + ( xoff * width ), 0, 0 + ( zoff * height ) )
        );
        vcoords->push_back(
            osg::vec3( width + ( xoff * width ), 0, height + ( zoff * height ) )
        );
        vcoords->push_back(
            osg::vec3( 0 + ( xoff * width ), 0, height + ( zoff * height ) )
        );
    ...
    geomss[0]->setTextureAttributeAndModes( 0,
                                            texture[0].get(),
                                            osg::StateAttribute::ON );
    geomss[1]->setTextureAttributeAndModes( 0,
                                            texture[1].get(),
                                            osg::StateAttribute::ON );
    ...
    if( single_pass )
    {
        StereoPass* stereopass = new StereoPass( texture[0].get(),
                                                 texture[1].get(),
                                                 width,
                                                 height,
                                                 min_disp,
    ...
    {
        StereoMultipass* stereomp = new StereoMultipass( texture[0].get(),
                                                         texture[1].get(),
                                                         width,
                                                         height,
                                                         min_disp,
      ...
      char* argv[] )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        ...
        " is the example which demonstrates a stereo matching algorithm. It uses "
        "multiple render targets and multiple passes with texture ping-pong."
    );
    arguments.getApplicationUsage()->setCommandLineUsage(
        arguments.getApplicationName() +
        " [options] --left left_image --right right_image --min min_disparity --max "
        "max_disparity --window window_size"
    );
    arguments.getApplicationUsage()->addCommandLineOption( "-h or --help",
                                                           "Display this information" );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--left",
        "The left image of the stereo pair to load."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--right",
        "The right image of the stereo pair to load."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--min",
        "The minimum disparity to start matching pixels."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--max",
        "The maximum disparity to stop matching pixels."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--window",
        "The window size used to match areas around pixels."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--single",
        "Use a single pass instead on multiple passes."
    );
    ...
    osg::Node*               scene   = createScene( leftIm.get(),
                                                    rightIm.get(),
                                                    minDisparity,
                                                    maxDisparity,
                                                    windowSize,
