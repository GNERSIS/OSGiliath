                ...
                osgGA::GUIActionAdapter&      aa )
        {
            ...
            else if( ea.getEventType() == osgGA::GUIEventAdapter::KEYUP )
            {
                if( ea.getKey() == osgGA::GUIEventAdapter::KEY_Up )
                {
                    ...
                    OSG_NOTICE << "m_Text3D->getCharacterHeight() = "
                               << m_Text3D->getCharacterHeight() << std::endl;
                }
                else if( ea.getKey() == osgGA::GUIEventAdapter::KEY_Down )
                {
                    m_Text3D->setCharacterDepth( m_Text3D->getCharacterDepth() + 0.1 );
                    OSG_NOTICE << "m_Text3D->getCharacterDepth() = "
                               << m_Text3D->getCharacterDepth() << std::endl;
                }
                else if( ea.getKey() == osgGA::GUIEventAdapter::KEY_Left )
                {
                    ...
                    if( counter % 3 == 0 )
                    {
                        m_Text3D->setText( "Press arrow keys.",
                    ...
                    {
                        m_Text3D->setText( "setText really works?",
                                           osgText::String::ENCODING_UTF8 );
                    }
                    else if( counter % 3 == 3 )
                    {
                        m_Text3D->setText( "setText works, really!",
                                           osgText::String::ENCODING_UTF8 );
                    }
                    ...
                    OSG_NOTICE
                        << "m_Text3D->getText().size() = " << m_Text3D->getText().size()
                        << std::endl;
                }
                else if( ea.getKey() == osgGA::GUIEventAdapter::KEY_Right )
                {
                    ...
                    OSG_NOTICE
                        << "m_Text3D->getLineSpacing() = " << m_Text3D->getLineSpacing()
                        << std::endl;
                }
            }
              ...
              const osgGA::GUIEventAdapter& ea )
        {
            ...
            if( view->computeIntersections( ea, intersections ) )
            {
                ...
                for( osgUtil::LineSegmentIntersector::Intersections::iterator itr =
                     ...
                     ++itr )
                {
                    OSG_NOTICE << "    intersection " << itr->localIntersectionPoint
                               << std::endl;
                }
            }
            else
      ...
      char** argv )
{
    ...
    {
        bevel = new osgText::Bevel;
        bevel->roundedBevel2( r );
    }
    ...
    if( bevel.valid() )
    {
        while( arguments.read( "--smooth-concave-Junctions" ) ||
               arguments.read( "--scj" ) )
        {
    ...
    else if( !arguments.read( "--no-3d" ) )
    {
        ...
        // Does not help
        text3D->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        if( arguments.read( "--size-quad" ) )
        {
            geode->addDrawable( osg::createTexturedQuadGeometry(
                osg::vec3( 0.0F, characterSize * thickness, 0.0F ),
                osg::vec3( characterSize, 0.0, 0.0 ),
                osg::vec3( 0.0F, 0.0, characterSize ),
                0.0,
                0.0,
                1.0,
                1.0
        ...
        {
            group->addChild( osgDB::readRefNodeFile( "duck.glb" ) );
        }
        ...
        if( arguments.read( "--character-size-mode", mode ) )
        {
            if( mode == "screen_coords" )
            {
                ...
                text3D->setCharacterSize( 1'080 / 4 );
            }
        }
