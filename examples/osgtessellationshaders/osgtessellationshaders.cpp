...
static const char* tessEvalSource = { "#version 460 core\n"
                                      ...
                                      "    gl_Position = osg_ProjectionMatrix * "
                                      "osg_ModelViewMatrix * vec4(tePosition, 1);\n"
                                      "}\n" };
...
CreateIcosahedron( osg::Program* /*program*/ )
{
    ...
    geometry->addPrimitiveSet(
        new osg::DrawElementsUInt( osg::PrimitiveSet::PATCHES, IndexCount, Faces )
    );

    // Expand the bounding box, otherwise the geometry is clipped in front when
    // tessellating.
    osg::box bbox( osg::vec3( -1.0F, -1.9F, -1.0F ), osg::vec3( 1.0F, 1.0F, 1.0F ) );
      ...
      char* argv[] )
{
    ...
    state->addUniform( new osg::Uniform( "AmbientMaterial",
                                         osg::vec3( 0.04F, 0.04F, 0.04F ) ) );
    state->addUniform( new osg::Uniform( "DiffuseMaterial",
                                         osg::vec3( 0.0F, 0.75F, 0.75F ) ) );
    state->addUniform( new osg::Uniform( "LightPosition",
                                         osg::vec3( 0.25F, 0.25F, 1.0F ) ) );
