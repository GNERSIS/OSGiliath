 ...
 * osgdistortion example application
 */
#include <iostream>
...
struct CommandLineOptions
{
        ...
        read( osg::ArgumentParser& arguments )
        {
            ...
            while( arguments.read( "--texture-width", tex_width ) ||
                   arguments.read( "--tx", tex_width ) )
            {
            }
            while( arguments.read( "--texture-height", tex_height ) ||
                   arguments.read( "--ty", tex_height ) )
            {
            }
...
class WindowResizedHandler : public osgGA::GUIEventHandler
{
    public:
                ...
                osg::NodeVisitor* nv )
        {
            ...
            {
                OSG_NOTICE << "Window resized event context=" << ea.getGraphicsContext()
                           << " frameNumber = " << fs->getFrameNumber() << std::endl;
                ...
                // reset the Camera viewport and associated Texture's to make sure it
                // tracks the new window size.
...
osg::Node*
createDistortionSubgraph( CommandLineOptions& options,
                          osg::Node*          subgraph,
...
void
setDomeFaces( osgViewer::Viewer&  viewer,
...
osg::Geometry*
createDomeDistortionMesh( const osg::vec3&    origin,
                          const osg::vec3&    widthVector,
                          const osg::vec3&    heightVector,
                          CommandLineOptions& options )
{
    osg::dvec3 center( 0.0, 0.0, 0.0 );
    osg::dvec3 eye( 0.0, 0.0, 0.0 );

    bool       centerProjection = false;

    osg::dvec3 projector        = eye - osg::dvec3( 0.0, 0.0, options.distance );
    ...
    osg::dvec3      screenCenter =
        osg::dvec3( origin + widthVector * 0.5F + heightVector * 0.5F );
    ...
    else
    {
        for( i = 0; i < noSteps; ++i )
        {
            ...
            for( j = 0; j < noSteps; ++j )
            {
                ...
                double e = options.distance *
                           cos( phi ) +
                           sqrt( options.sphere_radius * options.sphere_radius - f * f );
                double l = e * cos( phi );
                double h = e * sin( phi );
...
void
setDomeCorrection( osgViewer::Viewer&  viewer,
                   CommandLineOptions& options )
{
    osg::ref_ptr<osg::GraphicsContext::Traits> traits = new osg::GraphicsContext::Traits;
    traits->x                                         = 1'000;
    ...
    // distortion correction set up.
    {
        osg::Geode* geode = new osg::Geode();
        geode->addDrawable(
            createDomeDistortionMesh( osg::vec3( 0.0F, 0.0F, 0.0F ),
                                      osg::vec3( options.width, 0.0F, 0.0F ),
                                      osg::vec3( 0.0F, options.height, 0.0F ),
                                      options )
        );
        ...
        viewer.addSlave( camera.get(), osg::dmat4(), osg::dmat4(), false );
    }
      ...
      char** argv )
{
    ...
    {
        osg::Node* distortionNode =
            createDistortionSubgraph( options,
                                      loadedModel.get(),
