...
CaptureSettings::CaptureSettings( const CaptureSettings& cs,
...
void
CaptureSettings::setOutputFileName( const std::string& filename )
{
...
const std::string&
CaptureSettings::getOutputFileName() const
{
    return _outputFileName;
}

std::string
CaptureSettings::getOutputFileName( unsigned int frameNumber ) const
{
    std::stringstream str;
    str << _outputDirectoryName << _outputBaseFileName << "_" << frameNumber
        << _outputExtension;
    return str.str();
}

std::string
CaptureSettings::getOutputFileName( unsigned int cameraNum,
                                    unsigned int frameNumber ) const
{
    std::stringstream str;
    str << _outputDirectoryName << _outputBaseFileName << "_" << cameraNum << "_"
        << frameNumber << _outputExtension;
    return str.str();
}

bool
CaptureSettings::valid() const
{
    return _numberOfFrames >
           0 &&
           !_outputBaseFileName.empty() &&
           !_outputExtension.empty() &&
           !_inputFileName.empty();
}
...
//
static bool
checkEventHandlers( const gsc::CaptureSettings& cs )
{
    return !cs.getEventHandlers().empty();
}

static bool
readEventHandlers( osgDB::InputStream&   is,
                   gsc::CaptureSettings& cs )
{
    unsigned int size = 0;
...
static bool
writeEventHandlers( osgDB::OutputStream&        os,
                    const gsc::CaptureSettings& cs )
{
...
static bool
checkProperties( const gsc::CaptureSettings& cs )
{
    return !cs.getProperties().empty();
}

static bool
readProperties( osgDB::InputStream&   is,
                gsc::CaptureSettings& cs )
{
    unsigned int size = 0;
    is >> size >> is.BEGIN_BRACKET;
...
static bool
writeProperties( osgDB::OutputStream&        os,
                 const gsc::CaptureSettings& cs )
{
                         ...
                         "osg::Object gsc::CaptureSettings" )
{
    ...
    ADD_UINT_SERIALIZER( Width, 1'024 );
