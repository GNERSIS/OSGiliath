 ...
 * osgplanets example application
 */
...
osg::Image*
createBillboardImage( const osg::vec4& centerColour,
                      unsigned int     size,
                      float            power )
{
...
class SolarSystem
{
    ...
    public:
        ...
        osg::MatrixTransform*
        createTranslationAndTilt( double translation,
                                  double tilt );
        osg::MatrixTransform*
        createRotation( double orbit,
                        double speed );
        ...
        createPlanet( double             radius,
                      const std::string& name,
                      const osg::vec4&   color,
                      const std::string& textureName1,
                      const std::string& textureName2 );
        osg::Group*
        createSunLight();

        void
        rotateSpeedCorrection()
        {
            ...
            std::cout << "rotateSpeed corrected by factor " << _rotateSpeedFactor
                      << std::endl;
        }
...
osg::MatrixTransform*
SolarSystem::createRotation( double orbit,
                             double speed )
{
    osg::vec3           center( 0.0, 0.0, 0.0 );
    float               animationLength = 10.0F;
    osg::AnimationPath* animationPath =
        createAnimationPath( center, orbit, animationLength );

    osg::MatrixTransform* rotation = new osg::MatrixTransform;
    rotation->setUpdateCallback(
        new osg::AnimationPathCallback( animationPath, 0.0F, speed )
    );
...
osg::MatrixTransform*
SolarSystem::createTranslationAndTilt( double /*translation*/,
                                       double tilt )
{
    osg::MatrixTransform* moonPositioned = new osg::MatrixTransform;
    moonPositioned->setMatrix( osg::rotate( osg::radians( tilt ), 0.0, 0.0, 1.0 ) *
                               osg::scale( 1.0, 1.0, 1.0 ) *
                               osg::translate( osg::vec3( 0.0, _RorbitMoon, 0.0 ) ) );

    return moonPositioned;
}    // end SolarSystem::createTranslationAndTilt

osg::Geode*
SolarSystem::createSpace( const std::string& name,
                          const std::string& textureName )
{
    osg::Sphere* spaceSphere =
    ...
    if( !textureName.empty() )
    {
        ...
        if( image )
        {
            sSpaceSphere->getOrCreateStateSet()->setTextureAttributeAndModes(
                0,
...
osg::Geode*
SolarSystem::createPlanet( double             radius,
                           const std::string& name,
                           const osg::vec4&   color,
                           const std::string& textureName )
{
...

    {
        ...
        for( j = 0; j < numY; ++j, elevation += delta_elevation, ty += delta_ty )
        {
            double azim = 0.0;
            float  tx   = 0.0;
            for( unsigned int i = 0; i < numX;
                 ++i, ++vert, azim += delta_azim, tx += delta_tx )
            {
                osg::vec3 direction( cos( azim ) * cos( elevation ),
                                     sin( azim ) * cos( elevation ),
                                     sin( elevation ) );
                ( *coords )[vert]  = direction * radius;
                ( *normals )[vert] = direction;
    ...
    if( !textureName.empty() )
    {
        ...
        if( image )
        {
            ...
            geodePlanet->getOrCreateStateSet()
                ->setTextureAttributeAndModes( 0, tex2d, osg::StateAttribute::ON );
            geodePlanet->getOrCreateStateSet()->addUniform(
                new osg::Uniform( "hasBaseTexture", true )
            );
        }
    }
                           ...
                           const std::string& textureName2 )
{
    ...
    if( !textureName2.empty() )
    {
        ...
        if( image )
        {
            ...
            stateset->addUniform( new osg::Uniform( "hasNightTexture", true ) );
        }
    }
...
SolarSystem::printParameters()
{
    ...
    std::cout << "rotateSpeedEarthAndMoon(" << _rotateSpeedEarthAndMoon << ")"
              << std::endl;
...
static osg::Program*
createPlanetShaderProgram()
{
    osg::Program* program = new osg::Program;
    program->setName( "PlanetShader" );
    ...
    program->addShader( new osg::Shader(
        ...
        "    vColor = osg_Color;\n"
        "    vTexCoord0 = osg_MultiTexCoord0.xy;\n"
        "    vTexCoord1 = osg_MultiTexCoord1.xy;\n"
    ...
    program->addShader( new osg::Shader(
        ...
        "uniform bool hasBaseTexture = false;\n"
        "uniform bool hasNightTexture = false;\n"
        "uniform bool isEmissive = false;\n"
        "out vec4 fragColor;\n"
        "void main() {\n"
        "    vec3 lightDir = normalize(-vFragPos);\n"    // Light is at origin (sun)
        "    vec3 norm = normalize(vNormal);\n"
        "    float diff = max(dot(norm, lightDir), 0.0);\n"
        "    vec4 texColor = vColor;\n"
        "    if (hasBaseTexture) {\n"
        "        texColor *= texture(baseTexture, vTexCoord0);\n"
        "    }\n"
        "    if (isEmissive) {\n"
        "        fragColor = texColor;\n"
        "    } else if (hasNightTexture) {\n"
        "        vec4 nightColor = texture(nightTexture, vTexCoord1);\n"
        "        fragColor = vec4(mix(nightColor.rgb, texColor.rgb * diff, diff) + "
        "texColor.rgb * 0.05, texColor.a);\n"
        "    } else {\n"
        "        float ambient = 0.1;\n"
        "        fragColor = vec4(texColor.rgb * (diff + ambient), texColor.a);\n"
        "    }\n"
        "}\n"
    ) );
...
{
    osg::Program* program = new osg::Program;
    program->setName( "BillboardShader" );
      ...
      char** argv )
{
    ...
    while( arguments.read( "--rotateSpeedEarthAndMoon",
                           solarSystem._rotateSpeedEarthAndMoon ) )
    {
    }
    ...
    // Attach planet shader to root
    osg::Program* planetShader = createPlanetShaderProgram();
    root->getOrCreateStateSet()->setAttributeAndModes( planetShader,
                                                       osg::StateAttribute::ON );
    root->getOrCreateStateSet()->addUniform( new osg::Uniform( "baseTexture", 0 ) );
    root->getOrCreateStateSet()->addUniform( new osg::Uniform( "nightTexture", 1 ) );
    root->getOrCreateStateSet()->addUniform( new osg::Uniform( "hasBaseTexture",
                                                               false ) );
    root->getOrCreateStateSet()->addUniform( new osg::Uniform( "hasNightTexture",
                                                               false ) );
    root->getOrCreateStateSet()->addUniform( new osg::Uniform( "isEmissive", false ) );
    ...
    // create the sun
    osg::Node* solarSun = solarSystem.createPlanet( solarSystem._radiusSun,
                                                    "Sun",
                                                    osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                    solarSystem._mapSun );
    osg::StateSet* sunStateSet = solarSun->getOrCreateStateSet();
    sunStateSet->addUniform( new osg::Uniform( "isEmissive", true ) );
    sunStateSet->addUniform( new osg::Uniform( "hasBaseTexture", true ) );

    osg::Billboard* sunBillboard = new osg::Billboard();
    sunBillboard->setMode( osg::Billboard::POINT_ROT_EYE );
    sunBillboard->addDrawable(
        createSquare( osg::vec3( -150.0F, 0.0F, -150.0F ),
                      osg::vec3( 300.0F, 0.0F, 0.0F ),
                      osg::vec3( 0.0F, 0.0F, 300.0F ),
                      createBillboardImage( osg::vec4( 1.0, 1.0, 0, 1.0F ), 64, 1.0 ) ),
        osg::vec3( 0.0F, 0.0F, 0.0F )
    );

    // Billboard uses its own shader for simple texture+color pass-through
    osg::Program* billboardShader = createBillboardShaderProgram();
    sunBillboard->getOrCreateStateSet()->setAttributeAndModes( billboardShader,
                                                               osg::StateAttribute::ON );
    sunBillboard->getOrCreateStateSet()->addUniform( new osg::Uniform( "baseTexture",
                                                                       0 ) );
    sunBillboard->getOrCreateStateSet()->addUniform( new osg::Uniform( "hasBaseTexture",
                                                                       true ) );
    ...
    */
    ...
    osg::Node* earth = solarSystem.createPlanet( solarSystem._radiusEarth,
                                                 "Earth",
                                                 osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                 solarSystem._mapEarth,
                                                 solarSystem._mapEarthNight );
    osg::Node* moon  = solarSystem.createPlanet( solarSystem._radiusMoon,
                                                 "Moon",
                                                 osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                 solarSystem._mapMoon );

    // create transformations for the earthMoonGroup
    osg::MatrixTransform* aroundSunRotationEarthMoonGroup =
        solarSystem.createRotation( solarSystem._RorbitEarth,
                                    solarSystem._rotateSpeedEarthAndMoon );
    // osg::MatrixTransform* earthMoonGroupPosition =
    // solarSystem.createTranslationAndTilt( solarSystem._RorbitEarth,
    // solarSystem._tiltEarth );
    osg::MatrixTransform* earthMoonGroupPosition =
        solarSystem.createTranslationAndTilt( solarSystem._RorbitEarth, 0.0 );

    // Group with earth and moon under it
    osg::Group*           earthMoonGroup = new osg::Group;

    // transformation to rotate the earth around itself
    osg::MatrixTransform* earthAroundItselfRotation =
        solarSystem.createRotation( 0.0, solarSystem._rotateSpeedEarth );

    // transformations for the moon
    osg::MatrixTransform* moonAroundEarthRotation =
        solarSystem.createRotation( solarSystem._RorbitMoon,
                                    solarSystem._rotateSpeedMoon );
    osg::MatrixTransform* moonTranslation =
        solarSystem.createTranslationAndTilt( solarSystem._RorbitMoon, 0.0 );
    ...
    *********************************************
    */
    osg::Node* mercury = solarSystem.createPlanet( solarSystem._radiusMercury,
                                                   "Mercury",
                                                   osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                   solarSystem._mapMercury,
                                                   "" );

    osg::MatrixTransform* aroundSunRotationMercury =
        solarSystem.createRotation( solarSystem._RorbitMercury,
                                    solarSystem._rotateSpeedMercury );
    osg::MatrixTransform* mercuryPosition =
        solarSystem.createTranslationAndTilt( solarSystem._RorbitMercury, 0.0F );
    ...
    *********************************************
    */
    osg::Node* venus = solarSystem.createPlanet( solarSystem._radiusVenus,
                                                 "Venus",
                                                 osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                 solarSystem._mapVenus,
                                                 "" );

    osg::MatrixTransform* aroundSunRotationVenus =
        solarSystem.createRotation( solarSystem._RorbitVenus,
                                    solarSystem._rotateSpeedVenus );
    osg::MatrixTransform* venusPosition =
        solarSystem.createTranslationAndTilt( solarSystem._RorbitVenus, 0.0F );
    ...
    *********************************************
    */
    osg::Node* mars = solarSystem.createPlanet( solarSystem._radiusMars,
                                                "Mars",
                                                osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                solarSystem._mapMars,
                                                "" );

    osg::MatrixTransform* aroundSunRotationMars =
        solarSystem.createRotation( solarSystem._RorbitMars,
                                    solarSystem._rotateSpeedMars );
    osg::MatrixTransform* marsPosition =
        solarSystem.createTranslationAndTilt( solarSystem._RorbitMars, 0.0F );
    ...
    *********************************************
    */
    osg::Node* jupiter = solarSystem.createPlanet( solarSystem._radiusJupiter,
                                                   "Jupiter",
                                                   osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ),
                                                   solarSystem._mapJupiter,
                                                   "" );

    osg::MatrixTransform* aroundSunRotationJupiter =
        solarSystem.createRotation( solarSystem._RorbitJupiter,
                                    solarSystem._rotateSpeedJupiter );
    osg::MatrixTransform* jupiterPosition =
        solarSystem.createTranslationAndTilt( solarSystem._RorbitJupiter, 0.0F );
