 ...
 * osgviewerFLTK example application
 */
#include <FL/Fl.H>
...
AdapterWidget::AdapterWidget( int         x,
                              int         y,
                              int         w,
                              int         h,
                              const char* label ) :
...
int
AdapterWidget::handle( int event )
{
    switch( event )
    {
        case FL_PUSH :
            _gw->getEventQueue()->mouseButtonPress( Fl::event_x(),
                                                    Fl::event_y(),
                                                    Fl::event_button() );
            return 1;
        case FL_MOVE :
        case FL_DRAG :
            ...
            return 1;
        case FL_RELEASE :
            _gw->getEventQueue()->mouseButtonRelease( Fl::event_x(),
                                                      Fl::event_y(),
                                                      Fl::event_button() );
            return 1;
        case FL_KEYDOWN :
...
class ViewerFLTK : public osgViewer::Viewer,
...
class CompositeViewerFLTK : public osgViewer::CompositeViewer,
                            public AdapterWidget
{
    public:

        CompositeViewerFLTK( int         x,
                             int         y,
                             int         w,
                             int         h,
                             const char* label = 0 ) :
      ...
      char** argv )
{
    ...
    // Headless capture: early check before viewer construction
    {
        ...
        {
            return osg::headlessCapture( loadedModel.get(), headlessOutput, 640, 480 )
                     ? 0
                     : 1;
        }
    }
    ...
    if( arguments.read( "--CompositeViewer" ) )
    {
...

        {
            ...
            view1->getCamera()->setProjectionMatrixAsPerspective(
                30.0F,
                static_cast<double>( width ) / static_cast<double>( height / 2 ),
                1.0,
                1000.0
            );
            view1->getCamera()->setViewport(
                new osg::Viewport( 0, 0, width, height / 2 )
            );
...

        {
            ...
            view2->getCamera()->setViewport(
                new osg::Viewport( 0, height / 2, width, height / 2 )
            );
