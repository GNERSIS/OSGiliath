...
struct Benchmark
{
        ...
        void
        calibrate( unsigned int numLoops = 100'000 )
        {
...
{
        virtual void
        apply( InlineMethod& m );
        virtual void
        apply( Method& m );
        virtual void
        apply( VirtualMethod& m );
        virtual void
        apply( VirtualMethod2& m );
...
void
VirtualMethod2::method()
{
    OPERATION
}

void
Visitor::apply( Method& m )
{
    m.method();
}

void
Visitor::apply( VirtualMethod& m )
{
    m.method();
}

void
Visitor::apply( InlineMethod& m )
{
    m.method();
}

void
Visitor::apply( VirtualMethod2& m )
{
    m.method();
}
...
class CustomNodeVisitor : public osg::DualModeVisitor
{
    public:
...
runPerformanceTests()
{
    ...
    unsigned int iterations = 10'000'000;
    ...
    RUN(
        benchmark,
        {
            osg::MatrixTransform* mtl = dynamic_cast<osg::MatrixTransform*>( m );
            if( mtl )
            {
                cnv.apply( *mtl );
            }
    ...
    RUN( benchmark, { m->accept( cnv ); }, 10'000 )
}
