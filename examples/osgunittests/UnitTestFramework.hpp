 ...
 * UnitTestFramework example application
 */
...
namespace osgUtx
{
    ...
    */
    class TestContext
    {
        public:
            ...
            void
            setTraceLevel( TraceLevel tl );
            TraceLevel
            getTraceLevel() const;

            std::ostream&
            tout( TraceLevel tl = Full ) const;

        private:

            TestContext( const TestContext& );
            TestContext
            operator=( const TestContext& );
            ...
            class TraceStream
            {
                ...
                public:
                    ...
                    TraceStream( std::ostream& o  = osg::notify( osg::NOTICE ),
                                 TraceLevel    tl = Results );
                    ~TraceStream();
    ...
    */
    class TestCase : public Test
    {
        public:
            ...
            virtual void
            run( const Context& ) = 0;    // Subclass OSGUTX_EXPORT Responsibility
    ...
    */
    ...
    class TestCase_ : public TestCase
    {
        ...
        public:
            ...
            TestCase_( const std::string& sName,
    ...
    class TestGraph
    {
        ...
        public:
            ...
            */
            ...
            /**
                A utility function for accessing an arbitrary suite by pathname, relative
               to the suite 'tsuite' (defaults to root if null), and with the option of
               creating the \em TestSuite designated by \em path, if it does not already
               exist.

                This method may return 0 if the suite either cannot be found (and
               createIfNecssary is 0), or the first component of \em path is not the same
               as the name of the TestSuite \em tsuite.

                This was written to aid the auto-registration of tests at specific points
               in the test tree, where the tests' AutoRegistrationAgents may be
               distributed across several files, and cannot be guaranteed to run in a
               given order. E.g. You cannot register a test "root.osg.MyTest" unless you
               know that the suite "root.osg" already exists.


                @param path                    The name of the TestSuite to return.
                @param tsuite                The suite to 'start from'. Path is relative
               to this suite (defaults to root suite).
                @param createIfNecessary    Optionally create the TestSuite(s) denoted by
               path if they do not exist.
            */
            TestSuite*
            suite( const std::string& path,
                   TestSuite*         tsuite            = 0,
        ...
        private:
            ...
            /**
                ...
                is passed in as components in a list, represented by the iterator
               parameters.
            */
            TestSuite*
            suite( std::list<std::string>::iterator it,
    ...
    */
    class TestQualifier : public TestVisitor
    {
        ...
        public:
            ...
            virtual bool
            visitEnter( TestSuite* pSuite );

            // Leaving a composite: Pop its name from the Path
            virtual bool
            visitLeave( TestSuite* pSuite );
    ...
    */
    class TestRecord
    {
        public:
            ...
            stop();
            void
            log( const TestFailureX& e );
            void
            log( const TestErrorX& e );
            void
    ...
    */
    class TestReport
    {
        public:
            ...
            TestRecord&
            createRecord( const std::string& s )
            {
    ...
    */
    class TestRunner : public TestQualifier
    {
        public:
            ...
            */
            void
            specify( const std::string& sQualifiedName );
        ...
        protected:
            ...
            void
            perform( TestCase* pTest );

        private:

            TestRunner&
            operator=( const TestRunner& )
            {
                return *this;
            }
    ...
    */
    ...
    #define OSGUTX_AUTOREGISTER_TESTSUITE( tsuite )                                   \
        static osgUtx::TestSuiteAutoRegistrationAgent tsuite##_autoRegistrationObj__( \
            tsuite##_TestSuite()                                                      \
        );
...
namespace osgUtx
{
    ...
    */
    ...
    {
            TestSuiteAutoRegistrationAgent( TestSuite*  tsuite,
                                            const char* path = 0 )
            {
                if( !path )
                {
                ...
                TestSuite* regSuite =
                    osgUtx::TestGraph::instance().suite( path, 0, true );

                if( !regSuite )
                {
                    osg::notify( osg::WARN )
                        << "Warning, unable to register test suite named \""
                        << tsuite->name() << "\" at " << path
                        << ", falling back to root suite." << std::endl;
    ...
    */
    #define OSGUTX_TEST_F( expr )                                            \
        ...
        {                                                                    \
            std::stringstream ss;                                            \
            ss << #expr << " failure: " << __FILE__ << ", line " << __LINE__ \
               << std::ends;                                                 \
    ...
    */
    #define OSGUTX_TEST_E( expr )                                          \
        ...
        {                                                                  \
            std::stringstream ss;                                          \
            ss << #expr << " error: " << __FILE__ << ", line " << __LINE__ \
               << std::ends;                                               \
