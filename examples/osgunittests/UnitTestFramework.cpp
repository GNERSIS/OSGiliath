...
namespace osgUtx
{
    ...
    void
    TestContext::setTraceLevel( TraceLevel tl )
    {
        _tout.setTraceLevel( tl );
    }

    TestContext::TraceLevel
    TestContext::getTraceLevel() const
    {
        return _tout.getTraceLevel();
    }

    std::ostream&
    TestContext::tout( TraceLevel tl ) const
    {
    ...
    TestContext::TraceStream::TraceStream( std::ostream& o,
                                           TraceLevel    tl ) :
        _traceLevel( tl ),
        _outputStreamPtr( &o ),
    ...
    void
    TestContext::TraceStream::setTraceLevel( TraceLevel tl )
    {
        _traceLevel = tl;
    }

    TestContext::TraceLevel
    TestContext::TraceStream::getTraceLevel() const
    {
        return _traceLevel;
    }

    std::ostream&
    TestContext::TraceStream::stream( TestContext::TraceLevel tl )
    {
        if( _traceLevel >= tl )
        {
    ...
    TestSuite*
    TestGraph::suite( const std::string& path,
                      TestSuite*         tsuite,
                      bool               createIfNecessary )
    {
        ...
        return suite( pathComponents.begin(),
                      pathComponents.end(),
                      tsuite,
                      createIfNecessary );
    }
    ...
    TestSuite*
    TestGraph::suite( std::list<std::string>::iterator it,
    ...
    bool
    TestQualifier::visitEnter( TestSuite* pSuite )
    {
    ...
    bool
    TestQualifier::visitLeave( TestSuite* pSuite )
    {
    ...
    const std::string&
    TestQualifier::currentPath() const
    {
        return _path;
    }
    ...
    void
    TestRecord::log( const TestFailureX& e )
    {
        stop();
        result_  = Failure;
        problem_ = e.what();
    ...
    void
    TestRecord::log( const TestErrorX& e )
    {
        stop();
        result_  = Error;
        problem_ = e.what();
    ...
    void
    TestRecord::log( const std::exception& e )
    {
        stop();
        result_  = Error;
        problem_ = e.what();
    ...
    void
    TestRecord::log( const std::string& s )
    {
        stop();
        result_  = Error;
        problem_ = s;
    ...
    TestRunner::TestRunner( TestContext& ctx ) :
        _ctx( ctx )
    {
    }

    void
    TestRunner::specify( const std::string& sQualifiedName )
    {
        _tests.push_back( sQualifiedName );
    }

    bool
    TestRunner::visitEnter( TestSuite* pSuite )
    {
    ...
    namespace osgUtx
    {
        ...
        struct isSpecified
        {
                ...
                isSpecified( const isSpecified& ) = default;

                isSpecified( const std::string& s ) :
                    pTestName_( s )
                {
                }

                bool
                operator()( const std::string& specifiedTest )
                {
    ...
    bool
    TestRunner::visit( TestCase* pTest )
    {
        if( std::find_if( _tests.begin(),
                          _tests.end(),
                          osgUtx::isSpecified( currentPath() + pTest->name() ) ) !=
            _tests.end() )
        {
            perform( pTest );
        }
    ...
    bool
    TestRunner::visitLeave( TestSuite* pSuite )
    {
    ...
    void
    TestRunner::perform( TestCase* pTest )
    {
    ...
    void
    TestSuite::add( Test* pTest )
    {
        _tests.push_back( pTest );
    }

    Test*
    TestSuite::findChild( const std::string& name )
    {
        for( Tests::iterator it = _tests.begin(); it != _tests.end(); ++it )
        {
    ...
    bool
    TestSuite::accept( Test::Visitor& v )
    {
    ...
    bool
    QualifiedTestPrinter::visit( TestCase* pTest )
    {
