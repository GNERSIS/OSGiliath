...
const char* DepthPeeling::PeelingShader = {
    ...
    "                    gl_FragCoord.z);\n"
    "    return texture(depthtex, r0) < 0.5;\n"
    "  }\n"
    "  return false;\n"
    "}\n"
};
...
class PreDrawFBOCallback : public osg::Camera::DrawCallback
{
    public:
        ...
        PreDrawFBOCallback( osg::FrameBufferObject* fbo,
                            osg::FrameBufferObject* source_fbo,
                            unsigned int            width,
                            unsigned int            height,
                            osg::Texture*           dt,
                            osg::Texture*           ct ) :
            _fbo( fbo ),
            _source_fbo( source_fbo ),
            _depthTexture( dt ),
            _colorTexture( ct ),
            _width( width ),
            _height( height )
        ...
        {
            // switching only the frame buffer attachments is actually faster than
            // switching the framebuffer
            ...
            _fbo->setAttachment(
                ...
                osg::FrameBufferAttachment( ( osg::Texture2D* )( _colorTexture.get() ) )
            );

            // check if we need to do some depth buffer copying from a source FBO into
            // the current FBO
            if( _source_fbo.get() != NULL )
            {
                ...
                {
                    // blit the depth buffer from the solid geometry fbo into the current
                    // transparency fbo
                    ( _fbo.get() )
                    ...
                    // glReadBuffer(GL_COLOR_ATTACHMENT0); // only needed to blit the
                    // color buffer glDrawBuffer(GL_COLOR_ATTACHMENT0); // only needed to
                    // blit the color buffer
                    ext->glBlitFramebuffer( 0,
                                            0,
...
class PostDrawFBOCallback : public osg::Camera::DrawCallback
{
    public:
        ...
        PostDrawFBOCallback( bool restore ) :
            _restore( restore )
        {
        }
        ...
        operator()( osg::RenderInfo& renderInfo ) const
        {
            ...
            if( _restore )
            {
                renderInfo.getState()->get<osg::GLExtensions>()->glBindFramebuffer(
                    GL_FRAMEBUFFER,
                    0
                );
            }
...
DepthPeeling::CullCallback::CullCallback( unsigned int texUnit,
...
void
DepthPeeling::CullCallback::operator()( osg::Node*        node,
                                        osg::NodeVisitor* nv )
{
    ...
    osg::dmat4            m( *cullVisitor->getProjectionMatrix() );
    osg::postMultTranslate( m, osg::dvec3( 1, 1, 1 ) );
    osg::postMultScale( m, osg::dvec3( 0.5, 0.5, 0.5 ) );

    // With NPOT textures, no coordinate scaling needed
    ...
    if( _texUnit != 0 && _offsetValue )
    {
        ...
        osg::postMultTranslate(
            m,
            osg::dvec3( 0, 0, -ldexp( double( _offsetValue ), -24 ) )
        );
    }
    ...
    if( _texUnit != 0 )
    {
        ...
        osg::Uniform* depthUniform =
            new osg::Uniform( osg::Uniform::SAMPLER_2D_SHADOW, "depthtex" );
        depthUniform->set( ( int )_texUnit );
        osg::Uniform* invWidthUniform =
            new osg::Uniform( "invWidth", 1.0F / ( float )_texWidth );
        osg::Uniform* invHeightUniform =
            new osg::Uniform( "invHeight", 1.0F / ( float )_texHeight );
        osg::Uniform* offsetXUniform =
            new osg::Uniform( "offsetX", ( float )viewport->x() );
        osg::Uniform* offsetYUniform =
            new osg::Uniform( "offsetY", ( float )viewport->y() );
...
osg::Node*
DepthPeeling::createQuad( unsigned int layerNumber,
                          unsigned int numTiles )
{
    ...
    // Vertex order for GL_TRIANGLE_STRIP: BL, TL, BR, TR
    osg::Vec3Array* vertices = new osg::Vec3Array;

    vertices->push_back( osg::vec3( tileOffsetX, tileOffsetY, 0 ) );
    vertices->push_back( osg::vec3( tileOffsetX, tileOffsetY + tileSpan, 0 ) );
    vertices->push_back( osg::vec3( tileOffsetX + tileSpan, tileOffsetY, 0 ) );
    vertices->push_back(
        osg::vec3( tileOffsetX + tileSpan, tileOffsetY + tileSpan, 0 )
    );
...
// Simple pass-through shader for compositing textured quads
static const char* compositeVertexShader = {
    "#version 460 core\n"
...
static const char* compositeFragmentShader = {
    "#version 460 core\n"
    "uniform sampler2D layerTex;\n"
    "in vec2 vTexCoord;\n"
    "out vec4 fragColor;\n"
    "void main() {\n"
    "    fragColor = texture(layerTex, vTexCoord);\n"
    "}\n"
};

void
DepthPeeling::createPeeling()
{
...
#ifdef USE_PACKED_DEPTH_STENCIL
        _depthTextures[i]->setInternalFormat( GL_DEPTH24_STENCIL8 );
        _depthTextures[i]->setSourceFormat( GL_DEPTH_STENCIL );
        _depthTextures[i]->setSourceType( GL_UNSIGNED_INT_24_8 );
#else
    ...
    CullCallback* ccb =
        new CullCallback( _texUnit, _offsetValue, _texWidth, _texHeight );
    ...
    transparentNodeNoPeel->getOrCreateStateSet()->setRenderBinDetails(
        99,
        "RenderBin",
    ...
    osg::ref_ptr<osg::FrameBufferObject> fbos[2] = {
    ...
    for( unsigned int i = 0; i < _numPasses; ++i )
    {
        ...
        // get the pointers to the required fbo, color and depth textures for each camera
        // instance we perform ping ponging between two depth textures
        ...
        osg::Texture* depthTexture = ( i >= 1 ) ? _depthTextures[1 + ( i - 1 ) % 2].get()
                                                : _depthTextures[i].get();
        osg::Texture* prevDepthTexture =
            ( i >= 2 ) ? _depthTextures[1 + ( i - 2 ) % 2].get() : NULL;

        // all our peeling layer cameras are post render
        osg::Camera* camera = new osg::Camera;
        camera->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        // the peeling passes also need read access to prevDepthTexture and a cull
        // callback
        if( 0 == i )
        {    // solid geometry
        ...
        {    // topmost layer peeling pass
            camera->addChild( transparentNodeNoPeel );
        }
        else
        {    // behind layers peeling passes
    ...
    _compositeCamera->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    _compositeCamera->setComputeNearFarMode(
    ...
    _compositeCamera->setViewMatrix( osg::dmat4() );
    _compositeCamera->setProjectionMatrix(
        osg::ortho2D( 0.0, ( double )1, 0.0, ( double )1 )
    );
    _compositeCamera->setCullCallback( new CullCallback( 0, 0 ) );
    osg::StateSet* ss = _compositeCamera->getOrCreateStateSet();
    ss->setRenderBinDetails( 100,
                             "TraversalOrderBin",
                             osg::StateSet::OVERRIDE_RENDERBIN_DETAILS );

    // Add composite shader for GL 4.6 core profile
    osg::Program* compositeProgram = new osg::Program;
    compositeProgram->setName( "composite" );
    compositeProgram->addShader( new osg::Shader( osg::Shader::VERTEX,
                                                  compositeVertexShader ) );
    compositeProgram->addShader( new osg::Shader( osg::Shader::FRAGMENT,
                                                  compositeFragmentShader ) );
    ss->setAttributeAndModes( compositeProgram, osg::StateAttribute::ON );
    osg::Uniform* layerTexUniform =
        new osg::Uniform( osg::Uniform::SAMPLER_2D, "layerTex" );
    layerTexUniform->set( 0 );
    ss->addUniform( layerTexUniform );

    _root->addChild( _compositeCamera.get() );

    // solid geometry is blended first, transparency layers are blended in back to front
    // order. this order is achieved by rendering using a TraversalOrderBin (see camera
    // stateset).
    for( unsigned int i = _numPasses; i > 0; --i )
    {
        ...
        stateSet->setTextureAttributeAndModes( 0,
                                               _colorTextures[i % _numPasses].get(),
...
DepthPeeling::DepthPeeling( unsigned int width,
                            unsigned int height ) :
    _numPasses( 9 ),
    _texUnit( 2 ),
    _texWidth( width ),
...
void
DepthPeeling::setSolidScene( osg::Node* scene )
{
...
void
DepthPeeling::setTransparentScene( osg::Node* scene )
{
...
osg::Node*
DepthPeeling::getRoot()
{
    return _root.get();
}

void
DepthPeeling::resize( int width,
                      int height )
{
    for( unsigned int i = 0; i < 3; ++i )
    {
...
void
DepthPeeling::setNumPasses( unsigned int numPasses )
{
...
unsigned int
DepthPeeling::getNumPasses() const
{
    return _numPasses;
}

void
DepthPeeling::setTexUnit( unsigned int texUnit )
{
    if( texUnit == _texUnit )
    {
        return;
...
void
DepthPeeling::setShowAllLayers( bool showAllLayers )
{
...
bool
DepthPeeling::getShowAllLayers() const
{
    return _showAllLayers;
}

void
DepthPeeling::setOffsetValue( unsigned int offsetValue )
{
...
unsigned int
DepthPeeling::getOffsetValue() const
{
    return _offsetValue;
}
...
bool
DepthPeeling::EventHandler::handle( const osgGA::GUIEventAdapter& ea,
