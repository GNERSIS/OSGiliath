...
//             glReadBuffer(GL_COLOR_ATTACHMENT0); // only needed to blit the color buffer
//             glDrawBuffer(GL_COLOR_ATTACHMENT0); // only needed to blit the color buffer
...
public:
  ...
  virtual void operator () (osg::RenderInfo& renderInfo) const
  {
    ...
    if (_restore)
    {
        renderInfo.getState()->get<osg::GLExtensions>()->glBindFramebuffer( GL_FRAMEBUFFER, 0 );
    }
  }
protected:
  bool _restore;
...
DepthPeeling::CullCallback::CullCallback(unsigned int texUnit, unsigned int offsetValue, unsigned int texWidth, unsigned int texHeight) :
    _texUnit(texUnit),
...
void DepthPeeling::CullCallback::operator()(osg::Node* node, osg::NodeVisitor* nv)
{
    ...
    if (_texUnit != 0)
    {
        ...
        osg::Uniform *invWidthUniform = new osg::Uniform("invWidth", 1.0f / (float)_texWidth);
        osg::Uniform *invHeightUniform = new osg::Uniform("invHeight", 1.0f / (float)_texHeight);
...
void DepthPeeling::createPeeling()
{
    ...
    CullCallback *ccb      = new CullCallback(_texUnit, _offsetValue, _texWidth, _texHeight);
    ...
    compositeProgram->addShader(new osg::Shader(osg::Shader::VERTEX, compositeVertexShader));
    compositeProgram->addShader(new osg::Shader(osg::Shader::FRAGMENT, compositeFragmentShader));
    ss->setAttributeAndModes(compositeProgram, osg::StateAttribute::ON);
    osg::Uniform* layerTexUniform = new osg::Uniform(osg::Uniform::SAMPLER_2D, "layerTex");
