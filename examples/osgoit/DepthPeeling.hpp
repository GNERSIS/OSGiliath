...
#include <osgGA/events/GUIEventHandler.hpp>
    ...
    // USE_TEXTURE_RECTANGLE disabled for Core Profile - regular Texture2D with NPOT
    // works fine
...
class DepthPeeling : public osg::Referenced
{
    public:
        ...
        DepthPeeling( unsigned int width,
                      unsigned int height );
        void
        setSolidScene( osg::Node* scene );
        void
        setTransparentScene( osg::Node* scene );
        osg::Node*
        getRoot();
        void
        resize( int width,
                int height );
        void
        setNumPasses( unsigned int numPasses );
        unsigned int
        getNumPasses() const;
        void
        setTexUnit( unsigned int texUnit );
        void
        setShowAllLayers( bool showAllLayers );
        bool
        getShowAllLayers() const;
        void
        setOffsetValue( unsigned int offsetValue );
        unsigned int
                           getOffsetValue() const;

        static const char* PeelingShader; /* use this to support depth peeling in GLSL
                                             shaders in transparent objects */
    ...
    protected:
        ...
        osg::Node*
        createQuad( unsigned int layerNumber,
        ...
        class CullCallback : public osg::NodeCallback
        {
            public:
                ...
                CullCallback( unsigned int texUnit,
