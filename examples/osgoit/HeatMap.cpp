...
static const char* VertexShader = {
    ...
    "layout(location = 6) in vec2  xypos;\n"
    "out vec4 vColor;\n"
    "void main(void)\n"
    "{\n"
    "    float tmp     = min(texture(datatex, xypos).x / maximum, 1.0);\n"
    "    gl_Position   = osg_ModelViewProjectionMatrix * (osg_Vertex + vec4(0.0, 0.0, "
    "maxheight * tmp, 0.0));\n"
    "    vec4 color    = texture(colortex, tmp);\n"
    "    color.w       = color.w * transparency;\n"
    "    vColor        = color;\n"
    "}\n"
};

static const char* FragmentShader = { "#version 460 core\n"
                                      "in vec4 vColor;\n"
                                      "out vec4 fragColor;\n"
                                      "bool depthpeeling();\n"
                                      "void main(void)\n"
...
class MyGeometry : public osg::Geometry
{
    public:
        ...
        MyGeometry( osg::box bounds )
        {
            m_bounds  = bounds;
            m_bsphere = osg::sphere( bounds );
        }

        // an attempt to return a reasonable bounding box. Still does not prevent
        // clipping of the heat map.
        virtual const osg::box&
    ...
    protected:
        ...
        osg::box    m_bounds;
        osg::sphere m_bsphere;
};

Heatmap::Heatmap( float        width,
                  float        depth,
                  float        maxheight,
                  unsigned int K,
                  unsigned int N,
                  ...
                  float        transparency )
{
    ...
    osg::Geometry* meshGeom =
        new MyGeometry( osg::box( osg::vec3( -width / 2, -depth / 2, 0 ),
                                  osg::vec3( width / 2, depth / 2, maxheight ) ) );
    ...
    for( unsigned int y = 0; y < O * N; y++ )
    {
        if( y % 2 == 0 )
        {
            ...
            {
                vertices->push_back( osg::vec3( width * x / ( O * K - 1 ),
                                                depth * y / ( O * N - 1 ),
                                                0.0 ) +
                                     off );
                xypositions->push_back( osg::vec2( ( ( float )x + 0.5F ) / ( O * K ),
                                                   ( ( float )y + 0.5F ) / ( O * N ) ) );
            }
        }
        else
        {
            ...
            xypositions->push_back( osg::vec2( 0.5F / ( O * K ),
                                               ( ( float )y + 0.5F ) / ( O * N ) ) );
            for( unsigned int x = 0; x < O * K - 1; x++ )
            {
                vertices->push_back( osg::vec3( width * ( 0.5 + x ) / ( O * K - 1 ),
                                                depth * y / ( O * N - 1 ),
                                                0.0 ) +
                                     off );
                xypositions->push_back( osg::vec2( ( ( float )( x + 0.5F ) + 0.5F ) /
                                                       ( O * K ),
                                                   ( ( float )y + 0.5F ) / ( O * N ) ) );
            }
            vertices->push_back( osg::vec3( width, depth * y / ( O * N - 1 ), 0.0 ) +
                                 off );
            xypositions->push_back( osg::vec2( 1.0F - 0.5F / ( O * K ),
                                               ( ( float )y + 0.5F ) / ( O * N ) ) );
        }
    }
    ...
    *data++             = 255;    // red
    ...
    m_img2->setInternalTextureFormat( GL_RGB32F );
