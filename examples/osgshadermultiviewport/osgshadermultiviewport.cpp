 ...
 * osgshadermultiviewport example application
 */
...
void
createShaderCompositionStateSet( osg::StateSet* stateSet )
{
    ...
    *( ( int* )( imageData ) ) = 0XFF'FF'00'00;    // blue
    image->setImage( 1,
                     1,
                     1,
                     GL_RGBA,
    ...
    osg::ref_ptr<osg::Shader> ts =
        new osg::Shader( osg::Shader::GEOMETRY,
                         ...
                         "out vec2 texcoord;\n"
                         "void main(void) {\n"
                         "    for (int i = 0; i < osg::length(gl_in); i++)\n"
                         "    {"
                         "        basecolor = vec4(1,1,1,1);"
                         "        texcoord = vtexcoord[i];\n"
                         "        gl_ViewportIndex = gl_InvocationID;\n"
                         "        gl_Position = transform_block[gl_InvocationID] * "
                         "gl_in[i].gl_Position;\n"
                         "        EmitVertex();\n"
                         "    }\n"
                         "    EndPrimitive();\n"
      ...
      char** argv )
{
    ...
    osg::MatrixfArrayUniform*   LRuniform =
        stateSet->getOrCreateUniform<osg::MatrixfArrayUniform>( "transform_block" );
    LRuniform->resize( 2 );
    ...
    stateSet->setAttribute(
        new osg::ViewportIndexed( 0, 0.0, 0.0, width * 0.5, height )
    );    // left
    stateSet->setAttribute(
        new osg::ViewportIndexed( 1, width * 0.5, 0.0, width * 0.5, height )
    );    // right

    osg::Camera* cam     = viewer.getCamera();
    osg::dmat4   matrixL = cam->getProjectionMatrix();
    LRuniform->setElement( 0, osg::mat4( matrixL ) );
    double l, r, b, t, n, f;
    osg::getFrustum( matrixL, l, r, b, t, n, f );
    osg::dmat4 matrixR;
    matrixR = osg::frustum( l * 2.0, r * 2.0, b * 2.0, t * 2.0, n, f );
    LRuniform->setElement( 1, osg::mat4( matrixR ) );
    return viewer.run();
}
