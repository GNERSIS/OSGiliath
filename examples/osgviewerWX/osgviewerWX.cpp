...
wxOsgApp::OnInit()
{
    if( argc < 2 )
    {
        std::cout << wxString( argv[0] ).mb_str() << ": requires filename argument."
                  << std::endl;
        return false;
    }
    ...
    OSGCanvas*        canvas = new OSGCanvas( frame,
                                              wxID_ANY,
                                              wxDefaultPosition,
                                              wxSize( width, height ),
                                              wxSUNKEN_BORDER,
    ...
    osg::ref_ptr<osg::Node> loadedModel =
        osgDB::readRefNodeFile( std::string( fname.mb_str() ) );
    if( !loadedModel )
    {
...
void
MainFrame::SetViewer( osgViewer::Viewer* viewer )
{
    _viewer = viewer;
}
...
OSGCanvas::OnMouseDown( wxMouseEvent& event )
{
    if( _graphics_window.valid() )
    {
        _graphics_window->getEventQueue()->mouseButtonPress( event.GetX(),
                                                             event.GetY(),
                                                             event.GetButton() );
    }
}
...
OSGCanvas::OnMouseUp( wxMouseEvent& event )
{
    if( _graphics_window.valid() )
    {
        _graphics_window->getEventQueue()->mouseButtonRelease( event.GetX(),
                                                               event.GetY(),
                                                               event.GetButton() );
    }
}
...
void
OSGCanvas::OnMouseWheel( wxMouseEvent& event )
{
    int delta =
