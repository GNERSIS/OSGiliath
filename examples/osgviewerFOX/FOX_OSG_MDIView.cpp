...
FXIMPLEMENT( FOX_OSG_MDIView,
             FXMDIChild,
             FOX_OSG_MDIView_Map,
             ARRAYNUMBER( FOX_OSG_MDIView_Map ) )

FOX_OSG_MDIView::FOX_OSG_MDIView( FXMDIClient*    p,
                                  const FXString& name,
                                  FXIcon*         ic,
                                  FXPopup*        pup,
                                  FXuint          opt,
                ...
                h )
{
    ...
    FXGLVisual* glVisual =
        new FXGLVisual( getApp(), VISUAL_DOUBLEBUFFER | VISUAL_STEREO );

    m_gwFox                   = new GraphicsWindowFOX( this,
                                                       glVisual,
                                                       NULL,
                                                       0,
    ...
    // load the scene.
    osg::ref_ptr<osg::Node> loadedModel = osgDB::readRefNodeFile( "cow.glb" );
    if( !loadedModel )
    {
        return;
    }
...
long
FOX_OSG_MDIView::OnIdle( FXObject*  sender,
                         FXSelector sel,
                         void*      ptr )
{
    m_osgViewer->frame();
...
void
FOX_OSG_MDIView::SetViewer( osgViewer::Viewer* viewer )
{
    m_osgViewer = viewer;
}
