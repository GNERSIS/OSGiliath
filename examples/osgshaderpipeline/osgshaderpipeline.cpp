 ...
 * osgshaderpipeline example application
 */
#include <iostream>
...
bool
readShaderArguments( osg::ArgumentParser& arguments,
                     const std::string&   option,
                     osg::Program*        program,
                     const std::string&   fallbackShaderFilename )
{
    ...
    osg::ref_ptr<osg::Shader> shader =
        osgDB::readRefShaderFile( fallbackShaderFilename );
    if( shader )
    {
        shaderAssigned = true;
        program->addShader( shader );
    ...
    {
        OSG_NOTICE << "Unable to load shader file : " << fallbackShaderFilename
                   << std::endl;
        return false;
    }
}
...
createFallbackImage()
{
    ...
    *( reinterpret_cast<unsigned int*>( image->data() ) ) = 0XFF'FF'FF'FF;
    return image;
}

bool
setUpStateSet( osg::ArgumentParser& arguments,
               osg::StateSet*       stateset )
{
    osg::ref_ptr<osg::Program> program = new osg::Program;

    if( !readShaderArguments( arguments,
                              "--vert",
                              program.get(),
                              "shaders/shaderpipeline.vert" ) )
    {
        return false;
    }
    ...
    stateset->setDefine( "OSG_MAX_TEXTURE_UNITS", sstream.str() );

#define ADD_OSG_DEFINE( NAME, GL_ENUM )         \
    sstream.str( "" );                          \
    sstream << GL_ENUM;                         \
    stateset->setDefine( NAME, sstream.str() );

    if( maxTextureUnits > 0 )
    {
        osg::ref_ptr<osg::Texture2D> fallbackTexture =
            new osg::Texture2D( createFallbackImage() );
...
{
        RealizeOperation( osg::ArgumentParser& arguments,
                          osg::StateSet*       stateset ) :
            osg::GraphicsOperation( "RealizeOperation",
                                    false )
        {
            _stateset                          = stateset;
        ...
        {
            OSG_NOTICE << std::endl
                       << "---------- RealizeOperation() : Pushing StateSet on to "
                          "GraphicsContext's State. -----"
                       << std::endl
                       << std::endl;
            ;
      ...
      char** argv )
{
    ...
    viewer.setRealizeOperation(
        new RealizeOperation( arguments, useRootStateSet ? stateset.get() : 0 )
    );
