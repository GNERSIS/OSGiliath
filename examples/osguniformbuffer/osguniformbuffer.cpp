...
char vertexShaderSource[] =
    ...
    "void main(void)\n"
    "{\n"
    "vec3 ecPos      = vec3 (osg_ModelViewMatrix * osg_Vertex);\n"
...
char fragmentShaderSource[] =
    "#version 460 core\n"
    "// Fragment shader for Gooch shading, adapted for uniform buffer objects\n"
...
class UniformBufferCallback : public StateAttributeCallback
{
    public:
                    ...
                    NodeVisitor*    nv )
        {
            ...
            vec4        warmColor     = ( vec4( 0.0, 0.0, 1.0, 1 ) *
                                          frac +
                                          vec4( 1.0, 0.0, 0.0, 1 ) *
                                          ( 1 - frac ) );
      ...
      char** argv )
{
    ...
    // Ensure VBOs are used on all geometry (required for core profile)
    osgUtil::Optimizer::BufferObjectVisitor bufferObjectVisitor( true,
                                                                 true,
                                                                 false,
                                                                 false );
    loadedModel->accept( bufferObjectVisitor );
    const sphere bound        = loadedModel->getBound();
    ...
    ref_ptr<FloatArray> colorArray =
        new FloatArray( &colors1[0], &colors1[sizeof( colors1 ) / sizeof( GLfloat )] );
    ...
    ref_ptr<FloatArray> colorArray2 =
        new FloatArray( &colors2[0], &colors2[sizeof( colors2 ) / sizeof( GLfloat )] );
    ...
    dmat4 mat2 = osg::translate( static_cast<double>( -displacement ), 0.0, 0.0 );
    ...
    dmat4 mat3 = osg::translate( static_cast<double>( displacement ), 0.0, 0.0 );
    ...
    ubb3->setDataVariance( Object::DataVariance::DYNAMIC );
