 ...
 * osgoscdevice example application
 */
#include <iostream>
...
class PickHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        setLabel( const std::string& name,
                  float              x,
                  float              y )
        {
        ...
        case( osgGA::GUIEventAdapter::KEYUP ) :
            {
                if( ea.getKey() == 't' )
                {
                    osg::ref_ptr<osgGA::GUIEventAdapter> user_event =
                        new osgGA::GUIEventAdapter();
                    user_event->setEventType( osgGA::GUIEventAdapter::USER );
                    user_event->setUserValue( "vec2f", osg::vec2( 1.0F, 2.0F ) );
                    user_event->setUserValue( "vec3f", osg::vec3( 1.0F, 2.0F, 3.0F ) );
                    user_event->setUserValue( "vec4f",
                                              osg::vec4( 1.0F, 2.0F, 3.0F, 4.0F ) );

                    user_event->setUserValue( "vec2d", osg::dvec2( 1.0, 2.0 ) );
                    user_event->setUserValue( "vec3d", osg::dvec3( 1.0, 2.0, 3.0 ) );
                    user_event->setUserValue( "vec4d",
                                              osg::dvec4( 1.0, 2.0, 3.0, 4.0 ) );
...
class UserEventHandler : public osgGA::EventHandler
{
    public:
        ...
        UserEventHandler( osgText::Text* text ) :
            osgGA::EventHandler(),
            _text( text )
        {
        }
...
bool
UserEventHandler::handle( osgGA::Event* event,
                          osg::Object* /*object*/,
                          osg::NodeVisitor* nv )
{
    ...
    else if( event->getName() == "/osgga" )
    {
        ...
        osg::View*           view =
            ev ? dynamic_cast<osgViewer::View*>( ev->getActionAdapter() ) : NULL;
        if( view && ( rect[2] > 0 ) && ( rect[3] > 0 ) )
        {
            ...
            if( win )
            {
                win->setWindowRectangle( rect[2] + 10 + rect[0],
                                         rect[1],
                                         rect[2],
                                         rect[3] );
            }
    ...
    else
    {
        ...
        if( udc )
        {
            ...
            {
                const osg::ValueObject* vo =
                    dynamic_cast<const osg::ValueObject*>( udc->getUserObject( i ) );
...
createHUD()
{
    ...
    hudCamera->setProjectionMatrixAsOrtho2D( 0, 1'280, 0, 1'024 );
    hudCamera->setViewMatrix( osg::dmat4() );
    ...
    for( int i = 0; i < 5; i++ )
    {
        ...
        osg::DrawElementsUShort* elements =
            new osg::DrawElementsUShort( GL_TRIANGLES, 6 );
        ( *elements )[0] = 0;
        ( *elements )[1] = 1;
        ( *elements )[2] = 2;
        ( *elements )[3] = 0;
...
class OscServiceDiscoveredEventHandler : public ForwardToDeviceEventHandler
{
    public:
        ...
        OscServiceDiscoveredEventHandler() :
            ForwardToDeviceEventHandler( NULL )
        {
        }
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--zeroconf",
        "uses zeroconf to advertise the osc-plugin and to discover it"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--sender",
        "create a view which sends its events via osc"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--receiver",
        "create a view which receive its events via osc"
    );
    ...
    if( use_receiver )
    {
        ...
        osg::ref_ptr<osgGA::Device> device =
            osgDB::readRefFile<osgGA::Device>( address_port );
        if( device.valid() &&
            ( device->getCapabilities() & osgGA::Device::RECEIVE_EVENTS ) )
        {
            ...
            if( use_zeroconf )
            {
                osg::ref_ptr<osgGA::Device> zeroconf_device =
                    osgDB::readRefFile<osgGA::Device>(
                        "_osc._udp:9000.advertise.zeroconf"
                    );
                if( zeroconf_device )
                {
    ...
    if( use_sender )
    {
        ...
        osgViewer::GraphicsWindowCocoa* win =
            dynamic_cast<osgViewer::GraphicsWindowCocoa*>( gc.get() );
        if( win )
        {
        ...
        {
            osg::ref_ptr<osgGA::Device> zeroconf_device =
                osgDB::readRefFile<osgGA::Device>( "_osc._udp.discover.zeroconf" );
            if( zeroconf_device )
            {
                view->addDevice( zeroconf_device );
                view->getEventHandlers().push_front(
                    new OscServiceDiscoveredEventHandler()
                );
            }
        }
        else
        {
            ...
            OSG_NOTICE << "osc plugin address:port string : " << address_port
                       << std::endl;

            osg::ref_ptr<osgGA::Device> device =
                osgDB::readRefFile<osgGA::Device>( address_port );
            if( device.valid() &&
                ( device->getCapabilities() & osgGA::Device::SEND_EVENTS ) )
            {
                // add as first event handler, so it gets ALL events ...
                view->getEventHandlers().push_front(
                    new ForwardToDeviceEventHandler( device.get() )
                );
