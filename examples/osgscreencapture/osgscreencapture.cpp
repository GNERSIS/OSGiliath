 ...
 * osgscreencapture example application
 */
#include <iostream>
    ...
    public:
        ...
        struct ContextData : public osg::Referenced
        {
...

                {
                    ...
                    if( gc->getTraits() )
                    {
                        if( gc->getTraits()->alpha )
                        {
                            osg::notify( osg::NOTICE )
                                << "Select GL_BGRA read back format" << std::endl;
                            _pixelFormat = GL_BGRA;
                        }
                        else
                        {
                            osg::notify( osg::NOTICE )
                                << "Select GL_BGR read back format" << std::endl;
                            _pixelFormat = GL_BGR;
                        }
                    }
                    ...
                    std::cout << "Window size " << _width << ", " << _height
                              << std::endl;
                    ...
                    switch( _mode )
                    {
                        case( READ_PIXELS ) :
                            osg::notify( osg::NOTICE )
                                << "Reading window using glReadPixels, with out "
                                   "PixelBufferObject."
                                << std::endl;
                            break;
                        case( SINGLE_PBO ) :
                            osg::notify( osg::NOTICE )
                                << "Reading window using glReadPixels, with a single "
                                   "PixelBufferObject."
                                << std::endl;
                        ...
                        case( DOUBLE_PBO ) :
                            osg::notify( osg::NOTICE )
                                << "Reading window using glReadPixels, with a double "
                                   "buffer PixelBufferObject."
                                << std::endl;
                            _pboBuffer.push_back( 0 );
                            _pboBuffer.push_back( 0 );
                            break;
                        case( TRIPLE_PBO ) :
                            osg::notify( osg::NOTICE )
                                << "Reading window using glReadPixels, with a triple "
                                   "buffer PixelBufferObject."
                                << std::endl;
                            _pboBuffer.push_back( 0 );
                            _pboBuffer.push_back( 0 );
                                                   ...
                                                   unsigned int dataSize )
{
    ...
    if( _numTimeValuesRecorded == _reportTimingFrequency )
    {
        ...
        double averageFrameTime =
            osg::Timer::instance()->delta_s( _previousFrameTick, tick_afterMemCpy ) /
            double( _numTimeValuesRecorded );
        ...
        double numMb           = double( dataSize ) / ( 1'024 * 1'024 );

        int    prec            = osg::notify( osg::NOTICE ).precision( 5 );

        if( timeForMemCpy == 0.0 )
        {
            osg::notify( osg::NOTICE ) << "fps = " << fps
                                       << ", full frame copy = " << timeForFullCopy *
                1000.0F << "ms rate = " << numMPixels /
                timeForFullCopy
                                       << " Mpixel/sec, copy speed = " << numMb /
                timeForFullCopy << " Mb/sec"
                                       << std::endl;
        }
        else
        {
            osg::notify( osg::NOTICE ) << "fps = " << fps
                                       << ", full frame copy = " << timeForFullCopy *
                1000.0F << "ms rate = " << numMPixels /
                timeForFullCopy << " Mpixel/sec, "
                                       << numMb /
                timeForFullCopy << " Mb/sec "
                                       << "time for memcpy = " << timeForMemCpy *
                1000.0
                                       << "ms  memcpy speed = " << numMb /
                timeForMemCpy << " Mb/sec"
                                       << std::endl;
        }
                     ...
                     WindowCaptureCallback* callback )
{
    ...
    if( callback->getFramePosition() == WindowCaptureCallback::START_FRAME )
    {
             ...
             ++itr )
        {
            ...
            {
                osg::notify( osg::NOTICE )
                    << "First camera " << firstCamera << std::endl;
      ...
      char** argv )
{
    ...
    if( pbuffer.valid() )
    {
        ...
        camera->setFinalDrawCallback(
            new WindowCaptureCallback( mode, position, readBuffer )
        );

        if( pbufferOnly )
        {
            viewer.addSlave( camera.get(), osg::dmat4(), osg::dmat4() );
