...
class MovieEventHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        setMouseTracking( bool track )
        {
            _trackMouse = track;
        }

        bool
        getMouseTracking() const
        {
            return _trackMouse;
        }
    ...
    protected:
        ...
        class FindImageStreamsVisitor : public osg::DualModeVisitor
        {
            public:
                ...
                apply( osg::StateSet* stateset )
                {
                    ...
                    if( attr )
                    {
                        osg::Texture2D* texture2D =
                            dynamic_cast<osg::Texture2D*>( attr );
                        if( texture2D )
                        {
                            apply(
                                dynamic_cast<osg::ImageStream*>( texture2D->getImage() )
                            );
                        }
                        ...
                        {
                            apply(
                                dynamic_cast<osg::ImageStream*>( textureRec->getImage() )
                            );
                        }
                    }
                }
                ...
                inline void
                apply( osg::ImageStream* imagestream )
                {
                    if( imagestream )
                    {
            ...
            protected:
                ...
                operator=( const FindImageStreamsVisitor& )
                {
                    return *this;
                }
        };
...
void
MovieEventHandler::set( osg::Node* node )
{
                ...
                if( _trackMouse )
                {
                    ...
                    if( foundIntersection )
                    {
                        ...
                        if( vertices )
                        {
                            ...
                            const osgUtil::LineSegmentIntersector::Intersection::
                                IndexList& indices = intersection.indexList;
                            const osgUtil::LineSegmentIntersector::Intersection::
                                RatioList& ratios = intersection.ratioList;
                            ...
                            if( indices.size() == 3 && ratios.size() == 3 )
                            {
                                ...
                                osg::Array*  texcoords =
                                    ( geometry->getNumTexCoordArrays() > 0 )
                                ...
                                {
                                    // we have tex coord array so now we can compute the
                                    // final tex coord at the point of intersection.
                            ...
                            {
                                osg::notify( osg::NOTICE )
                                    << "Intersection has insufficient indices to work "
                                       "with";
                            }
                        }
                    }
        ...
        case( osgGA::GUIEventAdapter::KEYDOWN ) :
            {
                ...
                else if( ea.getKey() == 'L' )
                {
                         ...
                         ++itr )
                    {
                        ...
                        {
                            std::cout << ( *itr ).get() << " Toggle Looping Off"
                                      << std::endl;
                            ( *itr )->setLoopingMode( osg::ImageStream::NO_LOOPING );
                        }
                        else
                        {
                            std::cout << ( *itr ).get() << " Toggle Looping On"
                                      << std::endl;
                ...
                else if( ea.getKey() == 'o' )
                {
                    for( ImageStreamList::iterator itr = _imageStreamList.begin();
                         ...
                         ++itr )
                    {
                        std::cout << ( *itr ).get() << " Frame rate  "
                                  << ( *itr )->getFrameRate() << std::endl;
                    }
                    return true;
                }
                return false;
...
osg::Geometry*
myCreateTexturedQuadGeometry( const osg::vec3& pos,
                              float            width,
                              float            height,
                              osg::Image*      image,
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--texture2D",
        "Use Texture2D rather than TextureRectangle."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--shader",
        "Use shaders to post process the video."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--interactive",
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--flip",
        "Flip the movie so top becomes bottom."
    );
    ...
    bool                     fullscreen = arguments.read( "--fullscreen" );
    ...
    if( useShader )
    {
        ...
        static const char* shaderSourceTextureRec = {
            ...
            "void main(void)\n"
            "{\n"
            "    vec4 texture_color = texture(movie_texture, texcoord.st); \n"
        ...
        program->addShader( new osg::Shader( osg::Shader::VERTEX,
                                             shaderSourceTextureVertex ) );

        program->addShader( new osg::Shader( osg::Shader::FRAGMENT,
                                             useTextureRectangle
                                                 ? shaderSourceTextureRec
                                                 : shaderSourceTexture2D ) );

        stateset->addUniform( new osg::Uniform( "cutoff_color",
        ...
        if( arguments.isString( i ) )
        {
            ...
            if( image )
            {
                ...
                osg::ref_ptr<osg::Drawable> drawable =
                    myCreateTexturedQuadGeometry( pos,
                                                  width,
                                                  height,
                                                  image.get(),
                                                  useTextureRectangle,
    ...
    if( fullscreen )
    {
        ...
        viewer.getCamera()->setViewMatrix( osg::dmat4() );
        ...
        viewer.getCamera()->setProjectionMatrixAsOrtho2D( topleft.x,
                                                          bottomright.x,
                                                          topleft.y,
                                                          bottomright.y );
...
static void
soundReadCallback( void*    user_data,
                   uint8_t* data,
                   int      datalen )
{
...
SDLAudioSink::play()
{
    ...
    osg::notify( osg::NOTICE )
        << "  audioFrequency()=" << _audioStream->audioFrequency() << std::endl;
    osg::notify( osg::NOTICE )
        << "  audioNbChannels()=" << _audioStream->audioNbChannels() << std::endl;
    osg::notify( osg::NOTICE )
        << "  audioSampleFormat()=" << _audioStream->audioSampleFormat() << std::endl;
    ...
    switch( _audioStream->audioSampleFormat() )
    {
        ...
        case( osg::AudioStream::SAMPLE_FORMAT_S24 ) :
            {
                OSG_NOTICE
                    << "    SampleFormat  = SAMPLE_FORMAT_S24 NOT Supported by SDL."
                    << std::endl;
                break;
            }
