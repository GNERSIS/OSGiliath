 ...
 * osgthirdpersonview example application
 */
// This code is originally Copyright (c) 2008 Skew dmat4  Software LLC,
...
// specify no command line arguments, the code attempts to load duck.glb.
...
makeFrustumFromCamera( osg::Camera* camera )
{
    ...
    else
    {
        ...
        proj = osg::perspective( 30., 1., 1., 10. );
      ...
      char** argv )
{
    ...
    if( !scene )
    {
        ...
        // Try to load the duck...
        osg::notify( osg::WARN )
            << arguments.getApplicationName()
            << ": Could not find specified files. Trying \"duck.glb\" instead."
            << std::endl;
    ...
    // Create View 0 -- Just the loaded model.
    {
        ...
        view->setUpViewInWindow( 600, 100, 640, 480 );
        view->setSceneData( scene.get() );
        view->setCameraManipulator( new osgGA::TrackballManipulator );
    }

    // Create view 1 -- Contains the loaded moel, as well as a wireframe frustum derived
    // from View 0's Camera.
    {
        osgViewer::View* view = new osgViewer::View;
        viewer.addView( view );

        view->setUpViewInWindow( 600, 600, 640, 480 );
