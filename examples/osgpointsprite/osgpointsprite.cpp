 ...
 * osgpointsprite example application
 */
...
makeGalaxy( unsigned nvertices )
{
    ...
    for( unsigned i = 0; i < nvertices / 2; i++ )
    {
        ...
        float modz1 =
            ( ( rand() - RAND_MAX / 2 ) / ( float )( RAND_MAX ) ) * 3 / ( val + 1 );
        float modz2 =
            ( ( rand() - RAND_MAX / 2 ) / ( float )( RAND_MAX ) ) * 3 / ( val + 1 );
        vertices->push_back(
            osg::vec3( cos( val ) * val + modx1, sin( val ) * val + mody1, modz1 )
        );
        vertices->push_back(
            osg::vec3( -cos( val ) * val + modx2, -sin( val ) * val + mody2, modz2 )
        );
    ...
    galaxy->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POINTS, 0, nvertices )
    );
...
makeStateSet( float size )
{
    ...
    /// In Core Profile, point sprites are always enabled.
    /// GL_POINT_SPRITE texture mode tells the shader pipeline to use
    /// gl_PointCoord for texture lookup instead of vertex tex coords.
    set->setTextureMode( 0, GL_POINT_SPRITE, osg::StateAttribute::ON );
      ...
      char* argv[] )
{
    ...
    osg::Node*        node = makeGalaxy( 5'000 );
