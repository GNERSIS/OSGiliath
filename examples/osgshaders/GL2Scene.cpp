...
#include <osgUtil/utils/PerlinNoise.hpp>
...
CreateModel()
{
    ...
    osg::ShapeDrawable* sphere =
        new osg::ShapeDrawable( new osg::Sphere( osg::vec3( 0.0F, 0.0F, 0.0F ), 1.0F ) );
    sphere->setUseVertexBufferObjects( true );
    geode->addDrawable( sphere );

    osg::ShapeDrawable* cone = new osg::ShapeDrawable(
        new osg::Cone( osg::vec3( 2.2F, 0.0F, -0.4F ), 0.9F, 1.8F )
    );
    cone->setUseVertexBufferObjects( true );
    geode->addDrawable( cone );

    osg::ShapeDrawable* cylinder = new osg::ShapeDrawable(
        new osg::Cylinder( osg::vec3( 4.4F, 0.0F, 0.0F ), 1.0F, 1.4F )
    );
...
ModelInstance()
{
    ...
    xform->setPosition( osg::dvec3( 0.0F, -1.0F, zvalue ) );
    zvalue = zvalue + 2.2F;
...
static void
LoadShaderSource( osg::Shader*       shader,
            ...
            if( _enabled )
            {
                ...
                switch( _operation )
                {
                    ...
                    case SIN :
                        ...
                        break;
                    case COLOR1 :
                        uniform->set( osg::vec3( v10, 0.0F, 0.0F ) );
                        break;
                    case COLOR2 :
                        uniform->set( osg::vec3( v01, v01, v10 ) );
                        break;
                }
            }
        }
...
GL2Scene::buildScene()
{
    ...
    {
        osg::Uniform* OffsetUniform =
            new osg::Uniform( "Offset", osg::vec3( 0.0F, 0.0F, 0.0F ) );
        osg::Uniform* SineUniform = new osg::Uniform( "Sine", 0.0F );
        osg::Uniform* Color1Uniform =
            new osg::Uniform( "Color1", osg::vec3( 0.0F, 0.0F, 0.0F ) );
        osg::Uniform* Color2Uniform =
            new osg::Uniform( "Color2", osg::vec3( 0.0F, 0.0F, 0.0F ) );

        OffsetUniform->setUpdateCallback(
            new AnimateCallback( AnimateCallback::OFFSET )
        );
        SineUniform->setUpdateCallback( new AnimateCallback( AnimateCallback::SIN ) );
        Color1Uniform->setUpdateCallback(
            new AnimateCallback( AnimateCallback::COLOR1 )
        );
        Color2Uniform->setUpdateCallback(
            new AnimateCallback( AnimateCallback::COLOR2 )
        );
    ...
    // the "eroded" shader, uses a noise texture to discard fragments
    {
        ...
        ss->addUniform( new osg::Uniform( "LightPosition",
                                          osg::vec3( 0.0F, 0.0F, 4.0F ) ) );
