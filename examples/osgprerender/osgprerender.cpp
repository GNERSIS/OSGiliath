 ...
 * osgprerender example application
 */
#include <iostream>
...
class MyGeometryCallback : public osg::DrawableUpdateCallback,
               ...
               osg::vec3*                   begin )
        {
            if( type == osg::Drawable::VERTICES )
            {
                ...
                for( osg::vec3* itr = begin; itr < end; ++itr )
                {
                    ...
                    osg::vec3 local( osg::dot( dv, _xAxis ),
                                     osg::dot( dv, _yAxis ),
                                     osg::dot( dv, _zAxis ) );
                    ...
                    ( *itr ) =
                        _origin + _xAxis * local.x + _yAxis * local.y + _zAxis * local.z;
                }
            }
        }
                         ...
                         unsigned int                            colorSamples )
{
    ...
    // first create the geometry of the flag of which to view.
    {
        ...
        polyGeom->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        // note, when we use TextureRectangle we have to scale the tex coords up to
        // compensate.
        osg::vec2       bottom_texcoord( 0.0F, 0.0F );
        osg::vec2       top_texcoord( 0.0F, useTextureRectangle ? tex_height : 1.0F );
        osg::vec2       dv_texcoord( ( useTextureRectangle ? tex_width : 1.0F ) /
                                         ( float )( noSteps - 1 ),
                                     0.0F );
        ...
        polyGeom->setUpdateCallback(
            new MyGeometryCallback( origin, xAxis, yAxis, zAxis, 1.0, 1.0 / width, 0.2F )
        );
    ...
    // then create the camera node to do the render to texture
    {
        ...
        // set up projection.
        camera->setProjectionMatrixAsFrustum( -proj_right,
                                              proj_right,
                                              -proj_top,
                                              proj_top,
                                              znear,
        ...
        if( useImage )
        {
            ...
            // meaning we'll need to copy image to main memory, then copy it back to the
            // graphics card to the texture in one frame). The long way round allows us
            // to manually modify the copied image via the callback and then let this
            // modified image by reloaded back.
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--image",
        "Render to an image, then apply a post draw callback to it, and use this image "
        "to update a texture."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--texture-rectangle",
        "Use osg::TextureRectangle for doing the render to texture to."
    );
