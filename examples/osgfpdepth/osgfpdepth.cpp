 ...
 * osgfpdepth example application
 */
#include <iostream>
...
struct BufferConfig
{
        ...
        BufferConfig( const string& name_,
                      GLenum        format_,
                      int           bits_ ) :
            name( name_ ),
            format( format_ ),
...
void
getPossibleConfigs( GraphicsContext*  gc,
...
bool
checkFramebufferStatus( GraphicsContext* gc,
                        bool             silent = false )
{
    ...
    switch( ext->glCheckFramebufferStatus( GL_FRAMEBUFFER_EXT ) )
    {
        ...
        case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT :
            if( !silent )
            {
                cout << "Framebuffer incomplete, attached images must have same "
                        "dimensions\n";
            }
            return false;
           ...
           FboData&         data )
{
    ...
    if( multisample )
    {
        ...
        colorRB = new RenderBuffer( texWidth,
                                    texHeight,
                                    config.colorFormat,
                                    config.coverageSamples,
                                    config.depthSamples );
        data.fb->setAttachment( Camera::COLOR_BUFFER, FrameBufferAttachment( colorRB ) );
        depthRB = new RenderBuffer( texWidth,
                                    texHeight,
                                    config.depthFormat,
                                    config.coverageSamples,
                                    config.depthSamples );
        data.fb->setAttachment( Camera::DEPTH_BUFFER, FrameBufferAttachment( depthRB ) );
    }
    else
    {
        data.depthTex = makeDepthTexture( texWidth, texHeight, config.depthFormat );
            ...
            FboData&         data )
{
    ...
    osg::get<GLFrameBufferObjectManager>( state.getContextID() )
        ->flushAllDeletedGLObjects();
}

void
setAttachmentsFromConfig( Camera*          camera,
                          const FboConfig& config );
Switch*
makeTexturesAndGeometry( int     width,
                         int     height,
                         Switch* sw = 0 );
...
struct AppState : public Referenced
{
        ...
        void
        setStateFromConfig( const FboConfig& config );
        void
        advanceConfig( int increment );
        void
    ...
    zNear( 0.03125 )
{
    ...
    configText->setDataVariance( Object::DataVariance::DYNAMIC );
    ...
    zNearText->setDataVariance( Object::DataVariance::DYNAMIC );
    ...
    textProjection->setMatrix( osg::ortho2D( 0.0, 1280.0, 0.0, 1024.0 ) );
...
void
AppState::setStateFromConfig( const FboConfig& config )
{
...
void
AppState::advanceConfig( int increment )
{
...
void
AppState::updateDisplayedTexture()
{
    if( displayScene )
    {
        sw->setSingleChildOn( 0 );
    }
    else if( validConfigs[currentConfig].depthSamples >
             0 ||
    ...
    if( displayScene || ( validConfigs[currentConfig].depthSamples ==
                          0 &&
                          validConfigs[currentConfig].coverageSamples == 0 ) )
    {
...
AppState::updateNear()
{
    ...
    viewer->getCamera()->getProjectionMatrixAsPerspective( fovy,
                                                           ...
                                                           cNear,
                                                           cFar );
    viewer->getCamera()->setProjectionMatrixAsPerspective( fovy,
                                                           aspectRatio,
                                                           zNear,
                                                           cFar );
...
class ConfigHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        getUsage( ApplicationUsage& usage ) const
        {
...
class DepthHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        DepthHandler( AppState* appState,
                      Depth*    depth ) :
            _appState( appState ),
            _depth( depth )
        {
            depth->setDataVariance( Object::DataVariance::DYNAMIC );
        }
                ...
                case( osgGA::GUIEventAdapter::KEYUP ) :
                    {
                        if( ea.getKey() == 'i' )
                        {
                            ...
                            if( !_appState->invertRange )
                            {
                                ...
                                depth->setFunction( Depth::Function::LESS );
                            ...
                            else
                            {
                                ...
                                depth->setFunction( Depth::Function::GEQUAL );
        ...
        getUsage( ApplicationUsage& usage ) const
        {
            ...
            usage.addKeyboardMouseBinding( "up arrow", "double near plane distance" );
            usage.addKeyboardMouseBinding( "down arrow", "half near plane distance" );
        }
...
Geode*
createTextureQuad( Texture2D* texture )
{
    ...
    geom->addPrimitiveSet( new DrawArrays( GL_TRIANGLES, 0, 6 ) );
...
void
ResizedCallback::resizedImplementation( GraphicsContext* gc,
                                        int              x,
                                        int              y,
                                        int              width,
                                        int              height )
{
    ...
    _appState->setStateFromConfig( validConfigs[_appState->currentConfig] );
    ...
    if( vp )
    {
        ...
        double aspectRatioChange = ( width / oldWidth ) / ( height / oldHeight );
...
GraphicsContext*
setupGC( osgViewer::Viewer& viewer,
         ArgumentParser&    arguments )
{
    ...
    if( !wsi )
    {
        OSG_NOTIFY( NOTICE ) << "View::setUpViewOnSingleScreen() : Error, no "
    ...
    OSG_NOTICE << "x = " << x << ", y = " << y << ", width = " << width
    ...
    if( gw )
    {
        OSG_NOTIFY( INFO ) << "View::setUpViewOnSingleScreen - GraphicsWindow has been "
                              "created successfully."
                           << std::endl;
        gw->getEventQueue()->getCurrentEventState()->setWindowRectangle( 0,
                                                                         0,
                                                                         width,
                                                                         height );
    }
...
Camera*
makeRttCamera( GraphicsContext* gc,
               int              width,
               int              height )
{
                         ...
                         Switch* sw )
{
    ...
    colorTexture->setBorderColor( dvec4( 0, 0, 0, 0 ) );
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        ...
        " demonstrates using a floating point depth buffer.\nThe user can invert the "
        "depth buffer range and choose among available multi-sample configurations."
    );
    arguments.getApplicationUsage()->setCommandLineUsage(
        arguments.getApplicationName() + " [options] filename ..."
    );
    arguments.getApplicationUsage()->addCommandLineOption( "--far <number>",
                                                           "Set far plane value" );
    ...
    // Headless check: load model early for offscreen capture
    {
        std::string headlessOutput;
        if( arguments.read( "--headless", headlessOutput ) )
        {
            ref_ptr<Node> model = osgDB::readRefNodeFiles( arguments );
            return osg::headlessCapture( model.get(), headlessOutput, 640, 480 ) ? 0 : 1;
        }
    }
    ...
    // setupGC().
    viewer.getCamera()->setProjectionMatrixAsPerspective( 40.0,
                                                          1.0,
                                                          appState->zNear,
                                                          zFar );
