 ...
 * osgocclusionquery example application
 */
...
class OcclusionQueryVisitor : public osg::DualModeVisitor
{
    public:
        ...
        void
        setOccluderThreshold( int vertices );
        int
        getOccluderThreshold() const;

        virtual void
        apply( osg::OcclusionQueryNode& oqn );
        virtual void
...
class VisibilityThresholdVisitor : public osg::DualModeVisitor
{
    public:
...
//   between queries.
class QueryFrameCountVisitor : public osg::DualModeVisitor
{
    public:
...
class EnableQueryVisitor : public osg::DualModeVisitor
{
    public:
...
class DebugDisplayVisitor : public osg::DualModeVisitor
{
    public:
...
class RemoveOcclusionQueryVisitor : public osg::DualModeVisitor
{
    public:
...
class StatisticsVisitor : public osg::DualModeVisitor
{
    public:

        StatisticsVisitor( osg::NodeVisitor::TraversalMode mode =
...
unsigned int
countGeometryVertices( osg::Geometry* geom )
{
...
class VertexCounter : public osg::DualModeVisitor
{
    public:
...
OcclusionQueryVisitor::~OcclusionQueryVisitor()
{
    osg::notify( osg::INFO ) << "osgOQ: OcclusionQueryVisitor: Added " << getNameIdx()
                             << " OQNodes." << std::endl;
}
    ...
    if( vc.exceeded() )
    {
        ...
        while( np-- )
        {
            ...
            if( parent != NULL )
            {
                ...
                // Set all OQNs to use the same query StateSets (instead of multiple
                // copies
                ...
                case( osgGA::GUIEventAdapter::KEYUP ) :
                    {
                        ...
                        else if( ea.getKey() == 'o' )
                        {
                            ...
                            //     osg::notify( osg::ALWAYS ) << "osgOQ: Wrote scene
                            //     graph to \"saved_model.osgt\"" << std::endl;
                            // else
...
createBox()
{
    ...
    osg::PolygonMode* pm = new osg::PolygonMode( osg::PolygonMode::Face::FRONT_AND_BACK,
    ...
    // Convert 5 quads to triangles via index buffer
    osg::ref_ptr<osg::DrawElementsUShort> indices =
        new osg::DrawElementsUShort( GL_TRIANGLES );
    for( int face = 0; face < 5; ++face )
    {
        unsigned short base = face * 4;
...
createRandomTriangles( unsigned int numTriangles )
{
    ...
    // Use VBOs, disable display lists to decrease performance.
    geom->setUseVertexBufferObjects( true );
...
// Simple color passthrough shaders for core profile
static const char* colorVertSource =
    "#version 460 core\n"
...
static const char* colorFragSource = "#version 460 core\n"
                                     "in vec4 vColor;\n"
                                     "out vec4 fragColor;\n"
                                     "void main()\n"
                                     "{\n"
...
createStockScene()
{
    ...
    osg::ref_ptr<osg::OcclusionQueryNode> oqn = new osg::OcclusionQueryNode;
    oqn->addChild( createRandomTriangles( 20'000 ).get() );
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setCommandLineUsage(
        arguments.getApplicationName() + " [options] [filename(s)]"
    );
    ...
    if( arguments.argc() > 1 )
    {
        ...
        else
        {
            std::cout << arguments.getApplicationName()
                      << ": unable to load specified data." << std::endl;
            return 1;
        }
    }
    else
    {
        ...
        {
            std::cout << arguments.getApplicationName()
                      << ": Failed to create stock scene." << std::endl;
            return 1;
        }
    }
