 ...
 * osgmultitouch example application
 */
           ...
           unsigned int h )
{
    ...
    camera->setProjectionMatrix( osg::ortho2D( 0.0, ( double )w, 0.0, ( double )h ) );
...
class TestMultiTouchEventHandler : public osgGA::GUIEventHandler
{
    ...
    private:
        ...
        void
        createTouchRepresentations( osg::Group*  parent_group,
                                    unsigned int num_objects )
        {
            ...
            for( unsigned int i = 0; i != num_objects; ++i )
            {
                ...
                osg::ShapeDrawable* drawable =
                    new osg::ShapeDrawable( new osg::Box( osg::vec3( 0, 0, 0 ), 100 ) );
                ...
                osgText::Text* text = new osgText::Text;
                geode->addDrawable( text );
                drawable->setDataVariance( osg::Object::DataVariance::DYNAMIC );
                ...
                case osgGA::GUIEventAdapter::RELEASE :
                    {
                        ...
                        for( osgGA::GUIEventAdapter::TouchData::iterator i =
                                 ea.getTouchData()->begin();
                             i != ea.getTouchData()->end();
                             ++i, ++j )
                        {
                            ...
                            // std::cout << j << ": " << tp.x << "/" << tp.y <<" "<< x <<
                            // " " << y << " " << _w << " " << _h << std::endl;

                            _mats[j]->setMatrix(
                                osg::translate( ( double )( ( 1 + x ) * 0.5 * _w ),
                                                ( double )( ( 1 + y ) * 0.5 * _h ),
                                                0.0 )
                            );
                            _mats[j]->setNodeMask( 0XFF'FF );
                            ...
                            switch( tp.phase )
                            {
                                ...
                                case osgGA::GUIEventAdapter::TOUCH_STATIONERY :
                                    _drawables[j]->setColor(
                                        osg::vec4( 0.8, 0.8, 0.8, 1 )
                                    );
                                    break;
                        ...
                        if( ( ea.getTouchData()->getNumTouchPoints() > 0 ) &&
                            ( ea.getTouchData()->getNumTouchPoints() ==
                              num_touch_ended ) )
                        {
                        ...
                        aa.requestWarpPointer(
                            ( ea.getWindowX() + ea.getWindowWidth() ) / 2.0,
                            ( ea.getWindowY() + ea.getWindowHeight() ) / 2.0
                        );
                    }
                    break;
      ...
      char** argv )
{
    ...
    if( !scene )
    {
        scene = osgDB::readRefNodeFile( "milk_truck.glb" );
    }
    ...
    osgViewer::GraphicsWindowCocoa* win =
        dynamic_cast<osgViewer::GraphicsWindowCocoa*>( gc );
    if( win )
    {
    ...
    std::cout << "creating hud with " << gc->getTraits()->width << "x"
              << gc->getTraits()->height << std::endl;
    osg::Camera* hud_camera =
        createHUD( gc->getTraits()->width, gc->getTraits()->height );
