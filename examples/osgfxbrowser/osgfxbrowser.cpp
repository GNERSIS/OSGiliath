 ...
 * osgfxbrowser example application
 */
#include "Frame.hpp"
    ...
    public:
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( xform && enabled_ )
            {
                ...
                xform->setMatrix( osg::rotate( t, osg::dvec3( 0, 0, 1 ) ) );
            }
            traverse( node, nv );
        }
...
class EffectPanel : public osgfxbrowser::Frame
{
    public:
        ...
        class KeyboardHandler : public osgGA::GUIEventHandler
        {
            public:
                ...
                KeyboardHandler( EffectPanel* ep ) :
                    ep_( ep )
                {
                }
                        ...
                        osgGA::GUIActionAdapter& )
                {
                    if( ea.getEventType() == osgGA::GUIEventAdapter::KEYDOWN )
                    {
                        ...
                        {
                            ep_->setNodeMask( 0XFF'FF'FF'FF - ep_->getNodeMask() );
                            return true;
                        }
                         ...
                         1 )
        {
            ...
            osgFX::Registry::EffectMap emap =
                osgFX::Registry::instance()->getEffectMap();
            for( osgFX::Registry::EffectMap::const_iterator i = emap.begin();
                 i != emap.end();
                 ++i )
            {
                std::cout << "INFO: \t" << i->first << "\n";
                osg::ref_ptr<osgFX::Effect> effect =
                    static_cast<osgFX::Effect*>( i->second->cloneType() );
        ...
        inline void
        setScene( osg::Node* node )
        {
            _scene = node;
        }

        inline bool
        getEffectsEnabled() const
        {
            return _fxen;
        }

        inline void
        setEffectsEnabled( bool v )
        {
            _fxen = v;
        ...
        inline int
        getEffectIndex() const
        {
            return _selected_fx;
        }

        inline void
        setEffectIndex( int i )
        {
    ...
    protected:
        ...
        void
        rebuild_client_area( const osgfxbrowser::Rect& client_rect )
        {
            float zPos = -0.1;    // note from Robert, was 0.1f, but now must be -0.1f to
                                  // keep text visible??#!? due to some other change in
                                  // the OSG not tracked down yet...
            ...
            hints->setPosition( osg::vec3( ( client_rect.x0 + client_rect.x1 ) / 2,
                                           client_rect.y0 + 4,
                                           zPos ) );
            hints->setText( "<RETURN> show/hide this panel      <LEFT> previous effect  "
                            "    <RIGHT> next effect      <DEL> enable/disable effects  "
                            "    'x' save to file      'r' rotate/stop" );
            ...
            if( _selected_fx >= 0 && _selected_fx < static_cast<int>( _effects.size() ) )
            {
                ...
                {
                    effect_description = author_name =
                        "AUTHOR: " +
                        std::string( _effects[_selected_fx]->effectAuthor() ) +
                        std::string( "\n\n" );
                }
                effect_description +=
                    "DESCRIPTION:\n" +
            ...
            ename->setPosition( osg::vec3( ( client_rect.x0 + client_rect.x1 ) / 2,
                                           client_rect.y1 - 22,
                                           zPos ) );
            ...
            edesc->setPosition(
                osg::vec3( client_rect.x0 + 8, client_rect.y1 - 60, zPos )
            );
...
osg::Group*
build_hud_base( osg::Group* root )
{
    osg::ref_ptr<osg::Projection> proj =
        new osg::Projection( osg::ortho2D( 0.0, 1024.0, 0.0, 768.0 ) );
    proj->setCullingActive( false );
    root->addChild( proj.get() );

    osg::ref_ptr<osg::MatrixTransform> xform = new osg::MatrixTransform( osg::dmat4() );
...
build_gui( osg::Group* root )
{
    ...
    effect_panel->setRect( osgfxbrowser::Rect( 20, 20, 1'000, 280 ) );
      ...
      char* argv[] )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        arguments.getApplicationName() + " is a simple browser that allows you to apply "
                                         "osgFX effects to models interactively."
    );
    ...
    arguments.getApplicationUsage()->addKeyboardMouseBinding( "Right",
                                                              "Apply next effect" );
    arguments.getApplicationUsage()->addKeyboardMouseBinding(
        "Del",
        "Enable or disable osgFX"
    );
    arguments.getApplicationUsage()->addKeyboardMouseBinding(
        "Return",
        "Show or hide the effect information panel"
    );
    arguments.getApplicationUsage()->addKeyboardMouseBinding(
        "x",
        "Save the scene graph with current effect applied"
    );
