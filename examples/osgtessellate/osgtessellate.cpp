 ...
 * osgtessellate example application
 */
/* osgTessellator
...
class tessellateDemoGeometry : public osg::Geometry,
                               public osgUtil::Tessellator
{
        ...
        // the contours could be discarded since the geometry does not need to be
        // retessellated.
...
makePolsTwo( void )
{
    ...
    {
        coords->push_back(
            osg::vec3( quadstrip[i][0], quadstrip[i][2], quadstrip[i][1] )
        );
        tcs->push_back( osg::vec2( quadstrip[i][0], quadstrip[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    for( i = 0; i < 8; i++ )
    {
        coords->push_back(
            osg::vec3( innerquadstrip[i][0], innerquadstrip[i][2], innerquadstrip[i][1] )
        );
        tcs->push_back( osg::vec2( innerquadstrip[i][0], innerquadstrip[i][1] ) /
                        200.0F );
        nrms->push_back( nrm );
    }
    ...
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::TRIANGLE_STRIP, nstart, 8 )
    );
    nstart += 8;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::TRIANGLE_STRIP, nstart, 8 )
    ...
    gtess->setWindingType(
        osgUtil::Tessellator::TESS_WINDING_ABS_GEQ_TWO
    );    // so that first change in wind type makes the commonest tessellation - ODD.

    return gtess;
}

osg::Geometry*
makeSideWall( const float xpos )
{
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        tcs->push_back( osg::vec2( wall[i][1], wall[i][0] ) / 100.0F );
        nrms->push_back( nrm );
    }
    int nstart = 0;

    gtess->addPrimitiveSet( new osg::DrawArrays( osg::PrimitiveSet::QUADS, nstart, 4 ) );
    nstart += 4;
    for( i = 0; i < 24; i++ )
    {    // make an ellipse hole
        float y = 150 + 50 * cos( i * 2 * osg::PI / 24.0 );
        float z = 1'300 + 30 * sin( i * 2 * osg::PI / 24.0 );
        coords->push_back( osg::vec3( xpos, y, z ) );
        tcs->push_back( osg::vec2( y, z ) / 100.0F );
        nrms->push_back( nrm );
    }
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POLYGON, nstart, 24 )
    );
    nstart += 24;
    for( i = 0; i < 5; i++ )
    {    // make a pentagonal hole
        float y = 150 + 50 * cos( i * 2 * osg::PI / 5.0 );
        float z = 1'200 + 40 * sin( i * 2 * osg::PI / 5.0 );
        coords->push_back( osg::vec3( xpos, y, z ) );
        tcs->push_back( osg::vec2( y, z ) / 100.0F );
        nrms->push_back( nrm );
    }
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POLYGON, nstart, 5 )
    );
    nstart += 5;
    ...
    osg::ref_ptr<osgUtil::Tessellator> tscx =
        new osgUtil::Tessellator;    // the v1.2 multi-contour Tessellator.
    // we use the geometry primitives to describe the contours which are tessellated.
    // Winding odd means leave hole in surface where there are 2,4,6... contours circling
    // the point.
...
osg::Geometry*
makeFrontWall( const float zpos )
{
    ...
    // tessellate the set of contours to make a 'house wall' from the Boolean-like
    // operations.
    ...
    for( i = 0; i < 5; i++ )
    {
        ...
        tcs->push_back( osg::vec2( wall[i][0], wall[i][1] ) / 100.0F );
        nrms->push_back( nrm );
    }
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POLYGON, nstart, 5 )
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        tcs->push_back( osg::vec2( door[i][0], door[i][1] ) / 100.0F );
        nrms->push_back( nrm );
    }
    // and windows
    for( i = 0; i < 16; i++ )
    {
        ...
        tcs->push_back( osg::vec2( windows[i][0], windows[i][1] ) / 100.0F );
        nrms->push_back( nrm );
    }
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::QUADS, nstart, 20 )
    );
    nstart += 20;
...
makePols( void )
{
    ...
    osg::Vec2Array*         tcs    = new osg::Vec2Array;
    osg::vec3               nrm( 0, -1, 0 );
    // coordinates from red book code but shifted by 1000 & 2000 for alternate
    // Tessellatory things.
    ...
    static GLdouble rectsMidanti[12][3] =    // the centre 2 contours are traversed
                                             // opposite order to outer contour.
        {
            {1050.0,  50.0, 0.0},
            {1300.0,  50.0, 0.0},
            {1300.0, 300.0, 0.0},
    ...
    static GLdouble spiral[16][3] =    // shift by 1000; nb the order of vertices is
                                       // reversed from that of the red book
        {
            {3400.0, 250.0, 0.0},
            {3400.0,  50.0, 0.0},
            {3050.0,  50.0, 0.0},
    ...
    for( i = 0; i < 12; i++ )
    {
        ...
        tcs->push_back( osg::vec2( rects[i][0], rects[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    for( i = 0; i < 12; i++ )
    {
        coords->push_back(
            osg::vec3( rectsMidanti[i][0], rectsMidanti[i][2], rectsMidanti[i][1] )
        );
        tcs->push_back( osg::vec2( rectsMidanti[i][0], rectsMidanti[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    for( i = 0; i < 16; i++ )
    {    // and reverse spiral to make same as that of red book ch 11
        coords->push_back(
            osg::vec3( spiral[15 - i][0], spiral[15 - i][2], spiral[15 - i][1] )
        );
        tcs->push_back( osg::vec2( spiral[15 - i][0], spiral[15 - i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    for( i = 0; i < 4; i++ )
    {
        coords->push_back( osg::vec3( quad1[i][0], quad1[i][2], quad1[i][1] ) );
        tcs->push_back( osg::vec2( quad1[i][0], quad1[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    for( i = 0; i < 4; i++ )
    {
        coords->push_back( osg::vec3( quad2[i][0], quad2[i][2], quad2[i][1] ) );
        tcs->push_back( osg::vec2( quad2[i][0], quad2[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    for( i = 0; i < 3; i++ )
    {
        coords->push_back( osg::vec3( tri[i][0], tri[i][2], tri[i][1] ) );
        tcs->push_back( osg::vec2( tri[i][0], tri[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        tcs->push_back( osg::vec2( quad3[i][0], quad3[i][1] ) / 200.0F );
        nrms->push_back( nrm );
    }
    {
        osg::vec3 centre( 300, 0, 1'500 );
        for( i = 0; i < 18; i++ )
        {
            osg::vec3 rim = centre +
                            osg::vec3( -cos( osg::radians( ( float )i * 20.0 ) ),
                                       0.0,
                                       sin( osg::radians( ( float )i * 20.0 ) ) ) *
                            150.0;
            coords->push_back( rim );
            tcs->push_back( osg::vec2( rim.x, rim.z ) / 200.0F );
            nrms->push_back( nrm );
        }
    }
    {
        osg::vec3 centre( 400, 0, 1'800 );
        for( i = 0; i < 18; i++ )
        {
            osg::vec3 rim = centre +
                            osg::vec3( -cos( osg::radians( ( float )i * 15.0 ) ),
                                       0.0,
                                       sin( osg::radians( ( float )i * 15.0 ) ) ) *
                            250.0;
            coords->push_back( rim );
            tcs->push_back( osg::vec2( rim.x, rim.z ) / 200.0F );
            nrms->push_back( nrm );
    ...
    {
        osg::vec3 centre( 600, 0, 1'400 );
        for( i = 0; i < 18; i++ )
        {
            osg::vec3 rim = centre +
                            osg::vec3( -cos( osg::radians( ( float )i * 12.0 ) ),
                                       0.0,
                                       sin( osg::radians( ( float )i * 12.0 ) ) ) *
                            250.0;
            coords->push_back( rim );
            tcs->push_back( osg::vec2( rim.x, rim.z ) / 200.0F );
            nrms->push_back( nrm );
    ...
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::QUADS, nstart, 12 )
    );
    nstart += 12;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::QUADS, nstart, 12 )
    ...
    nstart += 12;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POLYGON, nstart, 16 )
    );
    nstart += 16;
    gtess->addPrimitiveSet( new osg::DrawArrays( osg::PrimitiveSet::QUADS, nstart, 4 ) );
    nstart += 4;
    ...
    nstart += 4;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::TRIANGLES, nstart, 3 )
    );
    nstart += 3;
    // A rectabngle with multiple holes
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::LINE_LOOP, nstart, 4 )
    );
    nstart += 4;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::TRIANGLE_FAN, nstart, 18 )
    );
    nstart += 18;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POLYGON, nstart, 18 )
    );
    nstart += 18;
    gtess->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POLYGON, nstart, 18 )
    ...
    // We need to access the tessellatable contours again to demonstrate all types of
    // tessellation. I could add the Tessellator to the geometry as userdata, but here I
    // use the derived tessellateDemoGeometry to hold both the drawable geode and the
    // original contours.
...
{    // add a string reporting the type of winding rule tessellation applied
    ...
    // disable depth test to ensure its always ontop.
...

    {
        ...
        text->setText(
            "Tessellation example - no tessellation (use 'W' wireframe to visualise)"
        );
...
class setTessellateVisitor : public osg::DualModeVisitor
{    // searches a loaded model tree for tessellatable geometries.
     ...
     // or you can force a type of tess with special names or a sub-class of Geode could
     // have extra information of course you can use any name to detect what is to be
     // tessellated! all the polygons within the specific node are deemed to be contours,
     // so any tessellation can be requested.
    public:

        setTessellateVisitor() :
            ...
            if( geode.getName().compare( 0, 9, "tessellate" ) == 0 )
            {
                for( unsigned int i = 0; i < geode.getNumDrawables(); ++i )
                {
                    ...
                    if( geom )
                    {
                        ...
                        if( tscx.valid() )
                        {
                            tscx->setTessellationType(
                                osgUtil::Tessellator::TESS_TYPE_GEOMETRY
                            );
                            if( geode.getName() == "tessellate" )
                            {
                                // add a Tessellator so that this geom is retessellated
                                // when N is pressed
                                tscx->setBoundaryOnly( true );
                                tscx->setWindingType(
                                    osgUtil::Tessellator::TESS_WINDING_ABS_GEQ_TWO
                                );    // so that first change in wind type makes the
                                      // commonest tessellation - ODD.
                                geom->setUserData( tscx.get() );
                            }
                            else if( geode.getName() == "tessellate odd" )
                            {
                                // OR you can just apply the Tessellator once only, using
                            ...
                            else if( geode.getName() == "tessellate odd bound" )
                            {
                                ...
                                tscx->setWindingType(
                                    osgUtil::Tessellator::TESS_WINDING_ODD
                                );    // tessellation - ODD, only show boundary.
                                tscx->retessellatePolygons( *geom );
                            }
                            ...
                            {
                                tscx->setWindingType(
                                    osgUtil::Tessellator::TESS_WINDING_NEGATIVE
                                );
                                tscx->retessellatePolygons( *geom );
                            }
...
class cxTessellateVisitor : public osg::DualModeVisitor
{    // special to this demo, traverses SG and finds nodes which have been tessellated
    ...
    public:
        ...
        apply( osg::Geode& geode )
        {
            ...
            for( unsigned int i = 0; i < geode.getNumDrawables(); ++i )
            {
                tessellateDemoGeometry* geom =
                    dynamic_cast<tessellateDemoGeometry*>( geode.getDrawable( i ) );
                if( geom )
                {
                    if( !geom->getBoundaryOnly() )
                    {    // turn on bounds only
                        // NB this shows only the true boundary of the curves, no
                        // internal edges
                        geom->setBoundaryOnly( true );
                    }
                    else
                    {    // change to next type of tessellation...
                    ...
                    switch( geom->getWindingType() )
                    {    // a text to be added to the scene.
                        case osgUtil::Tessellator::TESS_WINDING_ODD :
                ...
                osgText::Text* txt =
                    dynamic_cast<osgText::Text*>( geode.getDrawable( i ) );
                if( txt )
                {
                    const osg::vec4& ct =
                        txt->getColor();    // pick the text to be changed by its color
                    if( ct.z < 0.9 )
                    {
                ...
                case( osgGA::GUIEventAdapter::KEYDOWN ) :
                    {
                        if( _scene && ea.getKey() == 'n' )
                        {
                            ...
                            // the same contours are re-tessellated using a new method.
                            // Old contours & tessellation type are held internally in
                            // the derived Geode class tessellateDemoGeometry.
