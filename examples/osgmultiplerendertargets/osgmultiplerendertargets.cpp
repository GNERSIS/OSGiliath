 ...
 * osgmultiplerendertargets example application
 */
#include <iostream>
...
//
// Below is relatively straight forward example of using the OpenGL multiple render
// targets (MRT) extension to FrameBufferObjects/GLSL shaders.
//
...
// The quad geometry is used by the render to texture camera to generate multiple
// textures.
osg::Group*
createRTTQuad( unsigned int tex_width,
               ...
               bool         useHDR )
{
    ...
    // counter-clockwise
    quad_coords->push_back( osg::vec3( 0, 0, -1 ) );
    quad_coords->push_back( osg::vec3( 1, 0, -1 ) );
    quad_coords->push_back( osg::vec3( 1, 1, -1 ) );
    quad_coords->push_back( osg::vec3( 0, 1, -1 ) );
    ...
    osg::ref_ptr<osg::DrawElementsUByte> quad_da =
    ...
    quad_geom->setUseVertexBufferObjects( true );
    ...
    // Vertex shader for RTT quad (no varyings needed, just pass-through)
    static const char* rttVertSource =
        "#version 460 core\n"
    ...
    if( useHDR )
    {
        static const char*        shaderSource = "#version 460 core\n"
                                                 ...
                                                 "void main(void)\n"
                                                 "{\n"
                                                 "    fragData0 = vec4(-1e-12,0,0,1);\n"
                                                 "    fragData1 = vec4(0,1e-12,0,1);\n"
                                                 "    fragData2 = vec4(0,0,1e-12,1);\n"
                                                 "    fragData3 = vec4(0,0,1e-12,1);\n"
                                                 "}\n";

        osg::ref_ptr<osg::Shader> vshader =
            new osg::Shader( osg::Shader::VERTEX, rttVertSource );
        osg::ref_ptr<osg::Shader> fshader =
            new osg::Shader( osg::Shader::FRAGMENT, shaderSource );
...
// Here a scene consisting of a single quad is created. This scene is viewed by the
// screen camera. The quad is textured using a shader and the multiple textures generated
// in the RTT stage.
osg::Node*
createScene( osg::Node*   cam_subgraph,
             ...
             bool         useMultiSample )
{
    ...
    // first create the geometry of the quad
    {
        ...
        // Vertex shader for scene quad - passes texture coordinate to fragment shader
        static const char* sceneVertSource =
            "#version 460 core\n"
            "layout(location = 0) in vec4 osg_Vertex;\n"
        ...
        if( useHDR )
        {
            static const char* shaderSource =
                ...
                "{\n"
                "    fragData0 = \n"
                "     vec4(  -1e12 * texture( textureID0, vTexCoord0.st ).rgb, 1) + \n"
                "     vec4(   1e12 * texture( textureID1, vTexCoord0.st ).rgb, 1) + \n"
                "     vec4(   1e12 * texture( textureID2, vTexCoord0.st ).rgb, 1) + \n"
                "     vec4(-0.5e12 * texture( textureID3, vTexCoord0.st ).rgb, 1);  \n"
                "}\n";
            osg::ref_ptr<osg::Shader> vshader =
                new osg::Shader( osg::Shader::VERTEX, sceneVertSource );
            osg::ref_ptr<osg::Shader> fshader =
                new osg::Shader( osg::Shader::FRAGMENT, shaderSource );
            osg::ref_ptr<osg::Program> program = new osg::Program;
            program->addShader( vshader.get() );
        ...
        else
        {
            static const char* shaderSource =
                ...
                "{\n"
                "    fragData0 = \n"
                "          vec4(texture( textureID0, vTexCoord0.st ).rgb, 1) + \n"
                "          vec4(texture( textureID1, vTexCoord0.st ).rgb, 1) + \n"
                "          vec4(texture( textureID2, vTexCoord0.st ).rgb, 1) + \n"
                "     -0.5*vec4(texture( textureID3, vTexCoord0.st ).rgb, 1);  \n"
                "}\n";
            osg::ref_ptr<osg::Shader> vshader =
                new osg::Shader( osg::Shader::VERTEX, sceneVertSource );
            osg::ref_ptr<osg::Shader> fshader =
    ...
    // now create the camera to do the multiple render to texture
    {
        ...
        // we can also read back any of the targets as an image, modify this image and
        // push it back
        if( useImage )
        {
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        ...
        " demonstrates the use of multiple render targets (MRT) with frame buffer "
        "objects (FBOs). A render to texture (RTT) camera is used to render to four "
        "textures using a single shader. The four textures are then combined to texture "
        "the viewed geometry."
    );
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--image",
        "Render one of the targets to an image, then apply a post draw callback to "
        "modify it and use this image to update the final texture. Print some texture "
        "values when using HDR."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--hdr",
        "Use high dynamic range (HDR). Create floating point textures to render to."
    );
    ...
    rootNode->addChild(
        createScene( subGraph, tex_width, tex_height, useHDR, useImage, useMultiSample )
    );
