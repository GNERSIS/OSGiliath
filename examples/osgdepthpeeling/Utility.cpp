...
bool
Utility::readFile( const char*  fName,
                   std::string& s )
{
...
osg::Program*
Utility::createProgram( std::string vs,
                        std::string fs )
{
    // setup shader
                   ...
                   unsigned random )
{
    ...
    double noise =
        ( 1.0F -
          ( ( n * ( n * n * 15'731 + 789'221 ) + 1'376'312'589 ) & 0X7F'FF'FF'FF ) *
          0.000000000931322574615478515625F );
    return noise;
}

double
Utility::smoothNoise( unsigned       width,
                      unsigned       height,
                      unsigned       x,
                      unsigned       y,
                      unsigned char* noise )
...
osg::Texture2D*
Utility::newColorTexture2D( unsigned width,
                            unsigned height,
                            unsigned accuracy )
{
...
osg::Geode*
Utility::getCanvasQuad( unsigned width,
                        unsigned height,
                        double   depth )
{
    ...
    quad->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::QUAD_STRIP, 0, vertices->size() )
    );
