              ...
              unsigned    height )
{
    ...
    _normalDepthMapProgram =
        Utility::createProgram( "shaders/depthpeel_normaldepthmap.vert",
                                "shaders/depthpeel_normaldepthmap.frag" );
    _colorMapProgram = Utility::createProgram( "shaders/depthpeel_colormap.vert",
                                               "shaders/depthpeel_colormap.frag" );
    _edgeMapProgram  = Utility::createProgram( "shaders/depthpeel_edgemap.vert",
                                               "shaders/depthpeel_edgemap.frag" );
...
void
DePee::setSketchiness( double sketchiness )
{
    _sketchiness->set( ( float )sketchiness );
}

void
DePee::setColored( bool colored )
{
...
void
DePee::setFPS( double* fps )
{
    _fps = fps;
}

unsigned int
DePee::getNumberOfRenderPasses()
{
    unsigned int n = 0;
...
DePee::createNoiseMap()
{
    ...
    int            random  = rand() % 5'000;
    for( unsigned y = 0; y < _height; y++ )
    {
        for( unsigned x = 0; x < _width; x++ )
        {
            data[y * _width + x] = ( unsigned char )( 0.5 *
                                                      255.0 +
                                                      Utility::getNoise( x, y, random ) *
                                                      0.5 *
                                                      255.0 );
        }
    }
    ...
    if( !_isCrayon )
    {
        for( unsigned i = 0; i < 4; i++ )
        {
            for( unsigned y = 0; y < _height; y++ )
            {
                for( unsigned x = 0; x < _width; x++ )
                {
                    tmpData[y * _width + x] = ( unsigned char )
                        Utility::smoothNoise( _width, _height, x, y, data );
                }
            }
            ...
            for( unsigned y = 0; y < _height; y++ )
            {
                for( unsigned x = 0; x < _width; x++ )
                {
                    data[y * _width + x] = ( unsigned char )
                        Utility::smoothNoise( _width, _height, x, y, tmpData );
                }
            }
        }
    }

    image->setImage( _width,
                     _height,
                     1,
                     1,
                     GL_LUMINANCE,
...
DePee::createHUD()
{
...

    {
        ...
        _hudText->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
bool
DePee::createMap( MapMode mapMode,
                  bool    first )
{
    switch( mapMode )
    {
...
DePee::createFinal()
{
    ...
    screenAlignedProjectionMatrix->setMatrix(
        osg::ortho2D( 0.0, ( double )_width, 0.0, ( double )_height )
    );
    ...
    screenAlignedModelViewMatrix->setMatrix( osg::dmat4() );
...
bool
DePee::createEdgeMap( bool first )
// create the edge map of the first normal and depth map
{
    _dePeePasses.back()->newRenderPass( EDGE_MAP );

    // set up the background color and clear mask.
    _dePeePasses.back()->Cameras[EDGE_MAP]->setClearColor(
        osg::vec4( 0.3, 0.3F, 0.3F, 1.0F )
    );
    _dePeePasses.back()->Cameras[EDGE_MAP]->setClearMask( GL_COLOR_BUFFER_BIT |
                                                          GL_DEPTH_BUFFER_BIT );

    const osg::sphere& bs = _quadGeode->getBound();
    if( !bs.valid() )
    {
        return false;
    }
    ...
    _dePeePasses.back()->Cameras[EDGE_MAP]->setProjectionMatrixAsOrtho( 0,
                                                                        _width,
                                                                        0,
                                                                        _height,
                                                                        znear,
    ...
    // set view
    _dePeePasses.back()->Cameras[EDGE_MAP]->setReferenceFrame(
        osg::Transform::ABSOLUTE_RF
    );

    _dePeePasses.back()->Cameras[EDGE_MAP]->setViewMatrixAsLookAt(
        osg::dvec3( osg::vec3( 0.0F, 0.0F, 2.0F ) * bs.radius ),
    ...
    _dePeePasses.back()->Cameras[EDGE_MAP]->setRenderTargetImplementation(
    ...
    _dePeePasses.back()->Cameras[EDGE_MAP]->attach( osg::Camera::COLOR_BUFFER,
                                                    _edgeMap.get() );
    stateset->addUniform( new osg::Uniform( "edgeMap", 3 ) );

    // GL_LIGHTING removed: not in core profile
    // setup shader
    stateset->setAttributeAndModes( _edgeMapProgram.get(),
                                    osg::StateAttribute::OVERRIDE |
                                        osg::StateAttribute::ON );
...
bool
DePee::createNormalDepthColorMap( MapMode mapMode,
                                  bool    first )
{
    DePeePass* pass;
    ...
    const osg::sphere& bs = _subgraph->getBound();
    if( !bs.valid() )
    {
        return false;
    }
    ...
    // set up projection.
    pass->Cameras[mapMode]->setProjectionMatrixAsFrustum( -projRight,
                                                          projRight,
                                                          -projTop,
                                                          projTop,
                                                          znear,
    ...
    pass->Cameras[mapMode]->setViewMatrixAsLookAt(
        osg::dvec3( bs.center - osg::vec3( 0.0F, 2.0F, 0.0F ) * bs.radius ),
        osg::dvec3( bs.center ),
    ...
    pass->Cameras[mapMode]->setRenderTargetImplementation(
    ...
    switch( mapMode )
    {
        case NORMAL_DEPTH_MAP :
            ...
            if( _renderToFirst )
            {
                pass->Cameras[mapMode]->attach( osg::Camera::COLOR_BUFFER,
                                                _normalDepthMap0.get() );
                stateset->addUniform( new osg::Uniform( "normalDepthMap", 2 ) );
            }
            else
            ...
            // GL_LIGHTING removed: not in core profile
            stateset->setAttributeAndModes( _normalDepthMapProgram.get(),
                                            osg::StateAttribute::OVERRIDE |
                                                osg::StateAttribute::ON );
            break;
        ...
        case COLOR_MAP :
            ...
            stateset->setAttributeAndModes( _colorMapProgram.get(),
                                            osg::StateAttribute::OVERRIDE |
                                                osg::StateAttribute::ON );
