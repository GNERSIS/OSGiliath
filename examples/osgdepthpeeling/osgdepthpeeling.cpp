...
class KeyboardEventHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        registerAnimationPathCallback( osg::AnimationPathCallback* apc )
        {
            _apc = apc;
        }
...
class MouseEventHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        void
        registerModelGroupTransform( osg::MatrixTransform* modelGroupTransform )
        {
    ...
    private:
        ...
        rotate( float x,
                float y )
        {
            osg::dmat4 baseMatrix = _modelGroupTransform->getMatrix();

            osg::preMultTranslate( baseMatrix, osg::dvec3( _rotCenter ) );
            osg::preMultRotate( baseMatrix,
                                osg::dquat( ( double )( ( x - _prevX ) * 3 ),
                                            osg::dvec3( 0.0, 0.0, 1.0 ) ) );
            osg::preMultRotate( baseMatrix,
                                osg::dquat( ( double )( -( y - _prevY ) * 3 ),
                                            baseMatrix * osg::dvec3( 1.0, 0.0, 0.0 ) ) );
            osg::preMultTranslate( baseMatrix, osg::dvec3( -_rotCenter ) );
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        ...
        " is the example which demonstrates Depth Peeling"
    );
    arguments.getApplicationUsage()->setCommandLineUsage(
        arguments.getApplicationName() + " filename"
    );
    ...
    osg::ref_ptr<DePee> dePee =
        new DePee( rootNode, modelGroupTransform, width, height );
    ...
    osg::AnimationPathCallback* apc = new osg::AnimationPathCallback(
        osg::dvec3( modelGroupTransform->getBound().center ),
        osg::dvec3( 0.0, 0.0, 1.0 ),
        osg::radians( 45.0F )
    );
    apc->setPause( true );
