 ...
 * osgbindlesstext example application
 */
...
std::string vertShader = "#version 460 core                                          \n"
...
std::string fragShader =
    ...
    "    if (tex[tIndex] == 0) fragColor.r = 1.0;                  \n"
    "}                                                             \n";
...
class BindlessBuffer : public osg::Referenced
{
    public:
        ...
        Make( size_t count )
        {
            ...
            val->_ssbb = new osg::UniformBufferBinding( 0,
                                                        val->_handles.get(),
                                                        0,
                                                        sizeof( GLuint64 ) * count );
            return val;
        }

        BindlessBuffer&
        operator=( const BindlessBuffer& rhs )
        {
            if( this != &rhs )
            {
                _count   = rhs._count;
        ...
        BindlessBuffer( const BindlessBuffer& rhs ) :
            osg::Referenced( rhs )
        {
            if( this != &rhs )
            {
...
class BindlessTexture : public osg::Inherit<osg::Texture2D, BindlessTexture>
{
    public:
        ...
        BindlessTexture();
        BindlessTexture( BufferRef,
                         TextureList );
        BindlessTexture( const BindlessTexture& rhs,
        ...
        void
        setBidlessIndex( unsigned int index );
    ...
    protected:
        ...
        void
                                                 applyOnce( osg::State& state ) const;
...
BindlessTexture::BindlessTexture( const BindlessTexture& rhs,
                                  const osg::CopyOp&     copy ) :
    Inherit( rhs,
             copy )
{
    _isBound.resize( 5, false );
...
BindlessTexture::BindlessTexture( BufferRef   ref,
                                  TextureList textureList ) :
    Inherit( textureList[0] ),
...
void
BindlessTexture::setBidlessIndex( unsigned int index )
{
...
void
BindlessTexture::applyOnce( osg::State& state ) const
{
    if( !_buffer )
    {
        return;
    ...
    // for each actual texture we have, bind it, get the texture hande, assign the value
    // to our UBO
    for( int i = 0; i < txtcount; i++ )
    {
        ...
        // Here is where the "magic" happens, we get the texture handle for our texture,
        // copy it to our UBO, and then tell OpenGL to keep the handle resident
        ...
        if( _handles[contextID][i] !=
            0L ||
            extensions->glIsTextureHandleResident( _handles[contextID][i] ) == GL_FALSE )
        {
...
void
BindlessTexture::apply( osg::State& state ) const
{
    unsigned contextID = state.getContextID();
...
void
BindlessTexture::releaseGLObjects( osg::State* state ) const
{
    if( state )
    {
        unsigned contextID = state->getContextID();
...
///
void
createImageArray( osg::StateSet* attachPnt )
{
...
///
osg::Geometry*
createCube( float     scale,
            osg::vec3 origin = osg::vec3( 0.0F,
                                          0.0F,
                                          0.0F ) )
{
    ...
    // Convert quads to triangles for core profile (GL_QUADS not available)
    // Each quad (4 verts: 0,1,2,3) becomes 2 triangles (0,1,2) and (0,2,3)
    osg::DrawElementsUShort* primSet  = new osg::DrawElementsUShort( GL_TRIANGLES );
    unsigned int             numQuads = vertices->size() / 4;
    for( unsigned int i = 0; i < numQuads; ++i )
    {
        unsigned short base = i * 4;
        primSet->push_back( base + 0 );
        primSet->push_back( base + 1 );
        primSet->push_back( base + 2 );
        primSet->push_back( base + 0 );
        primSet->push_back( base + 2 );
        primSet->push_back( base + 3 );
    }
