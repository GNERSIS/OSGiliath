 ...
 */
class PosterVisitor : public osg::DualModeVisitor
{
    public:
        ...
        const PagedNodeNameSet&
        getPagedNodeNames() const
        {
            return _pagedNodeNames;
        }

        unsigned int
        getNeedToApplyCount() const
        {
            return _needToApplyCount;
        }

        unsigned int
        getAppliedCount() const
        {
        ...
        inQueue() const
        {
            return _needToApplyCount > _appliedCount ? _needToApplyCount - _appliedCount
                                                     : 0;
        }

        void
        setAddingCallbacks( bool b )
        {
            _addingCallbacks = b;
        }

        bool
        getAddingCallbacks() const
        {
    ...
    protected:
        ...
        bool
        hasCullCallback( osg::Callback* nc,
                         osg::Callback* target )
        {
            if( nc == target )
            {
...
class PosterIntersector : public osgUtil::Intersector
{
    public:
        ...
        PosterIntersector( double xMin,
                           double yMin,
                           double xMax,
                           double yMax );

        void
        setPosterVisitor( PosterVisitor* pcv )
        {
            _visitor = pcv;
        }
...
class PosterPrinter : public osg::Referenced
{
    public:
        ...
        void
        setOutputTiles( bool b )
        {
            _outputTiles = b;
        }

        bool
        getOutputTiles() const
        {
            return _outputTiles;
        }
        ...
        void
        setOutputTileExtension( const std::string& ext )
        {
            _outputTileExt = ext;
        }

        const std::string&
        getOutputTileExtension() const
        {
        ...
        void
        setOutputPosterName( const std::string& name )
        {
            _outputPosterName = name;
        }

        const std::string&
        getOutputPosterName() const
        {
        ...
        void
        setFinalPoster( osg::Image* image )
        {
            _finalPoster = image;
        }

        const osg::Image*
        getFinalPoster() const
        {
        ...
        void
        init( const osg::Camera* camera );
        void
        init( const osg::dmat4& view,
              const osg::dmat4& proj );
        void
        frame( const osg::FrameStamp* fs,
               osg::Node*             node );
    ...
    protected:
        ...
        bool
        addCullCallbacks( const osg::FrameStamp* fs,
                          osg::Node*             node );
        void
        removeCullCallbacks( osg::Node* node );
        void
        bindCameraToImage( osg::Camera* camera,
                           int          row,
                           int          col );
        void
                                        recordImages();
        ...
        osg::dmat4                      _currentViewMatrix;
        osg::dmat4                      _currentProjectionMatrix;
