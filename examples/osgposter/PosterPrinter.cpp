...
static osg::ref_ptr<PagedLoadingCallback> g_pagedLoadingCallback =
    new PagedLoadingCallback;
    ...
    public:
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( pagedLOD && pagedLOD->getNumChildren() > 0 )
            {
                ...
                {
                    double       timeStamp = nv->getFrameStamp()
                    ...
                    numChildren < pagedLOD->getNumRanges() )
                {
                    if( pagedLOD->getDatabasePath().empty() )
                    {
                        nv->getDatabaseRequestHandler()->requestNodeFile(
                            pagedLOD->getFileName( numChildren ),
                            nv->getNodePath(),
                            1.0,
                            nv->getFrameStamp(),
...
static osg::ref_ptr<PagedCullingCallback> g_pagedCullingCallback =
    new PagedCullingCallback;
...
void
PosterVisitor::apply( osg::LOD& node )
{
...
void
PosterVisitor::apply( osg::PagedLOD& node )
{
    if( !hasCullCallback( node.getCullCallback(), g_pagedCullingCallback.get() ) )
    {
        for( unsigned int i = 0; i < node.getNumFileNames(); ++i )
        {
            ...
            PagedNodeNameSet::iterator itr =
                _pagedNodeNames.find( node.getFileName( i ) );
            if( itr != _pagedNodeNames.end() )
            {
...
/* PosterIntersector: A simple polytope intersector for updating pagedLODs in each
 * image-tile */
PosterIntersector::PosterIntersector( const osg::Polytope& polytope ) :
    _intersectionVisitor( 0 ),
    _parent( 0 ),
    _polytope( polytope )
{
}

PosterIntersector::PosterIntersector( double xMin,
                                      double yMin,
                                      double xMax,
                                      double yMax ) :
...
osgUtil::Intersector*
PosterIntersector::clone( osgUtil::IntersectionVisitor& iv )
{
    osg::dmat4 matrix;
    if( iv.getProjectionMatrix() )
    {
        osg::preMult( matrix, *iv.getProjectionMatrix() );
    }
    if( iv.getViewMatrix() )
    {
        osg::preMult( matrix, *iv.getViewMatrix() );
    }
    if( iv.getModelMatrix() )
    {
        osg::preMult( matrix, *iv.getModelMatrix() );
    }
...
bool
PosterIntersector::enter( const osg::Node& node )
{
    ...
    if( _polytope.contains( node.getBound() ) )
    {
        if( node.getCullCallback() )
        {
            const osg::ClusterCullingCallback* cccb =
                dynamic_cast<const osg::ClusterCullingCallback*>(
...
void
PosterIntersector::intersect( osgUtil::IntersectionVisitor& iv,
                              osg::Drawable*                drawable )
{
    ...
    for( osg::NodePath::iterator itr = nodePath.begin(); itr != nodePath.end(); ++itr )
    {
        ...
        if( pagedLOD )
        {
            // FIXME: The first non-empty getFileName() is used as the identity of this
            // paged node. This should work with VPB-generated terrains but maybe
            // unusable with others.
...
void
PosterPrinter::init( const osg::Camera* camera )
{
    if( _camera.valid() )
    {
...
void
PosterPrinter::init( const osg::dmat4& view,
                     const osg::dmat4& proj )
{
    if( _isRunning )
    {
        return;
...
void
PosterPrinter::frame( const osg::FrameStamp* fs,
                      osg::Node*             node )
{
    ...
    if( _isRunning )
    {
        ...
        if( ( fs->getFrameNumber() - _lastBindingFrame ) > 2 )
        {
            ...
            if( _camera.valid() )
            {
                std::cout << "Binding sub-camera " << _currentRow << "_"
                          << _currentColumn << " to image..." << std::endl;
...
bool
PosterPrinter::addCullCallbacks( const osg::FrameStamp* fs,
                                 osg::Node* /*node*/ )
{
    ...
    std::cout << "Dispatching callbacks to paged nodes... " << _visitor->inQueue()
              << std::endl;
    return true;
}

void
PosterPrinter::removeCullCallbacks( osg::Node* /*node*/ )
{
...
void
PosterPrinter::bindCameraToImage( osg::Camera* camera,
                                  int          row,
                                  int          col )
{
    ...
    image->allocateImage( ( int )_tileSize.x,
                          ( int )_tileSize.y,
                          1,
                          GL_RGBA,
                          GL_UNSIGNED_BYTE );
    ...
    osg::dmat4 offsetMatrix =
        osg::scale( ( double )_tileColumns, ( double )_tileRows, 1.0 ) *
        osg::translate( ( double )( _tileColumns - 1 - 2 * col ),
                        ( double )( _tileRows - 1 - 2 * row ),
                        0.0 );
    ...
    {
        // Apply a cull callback to every paged node obtained, to force the highest level
        // displaying. This will be done by the PosterVisitor, who already records all
        // the paged nodes.
    }

    // Reattach cameras and new allocated images
    camera->setRenderingCache(
        NULL
