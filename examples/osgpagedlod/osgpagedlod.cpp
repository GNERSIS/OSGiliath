 ...
 * osgpagedlod example application
 */
#include <iostream>
...
class NameVistor : public osg::DualModeVisitor
{
    public:
...
class CheckVisitor : public osg::DualModeVisitor
{
    public:
        ...
        {
            std::cout << "PagedLOD " << plod.getName()
                      << "  numRanges = " << plod.getNumRanges()
                      << "  numFiles = " << plod.getNumFileNames() << std::endl;
...
class WriteOutPagedLODSubgraphsVistor : public osg::DualModeVisitor
{
    public:
...
class ConvertToPageLODVistor : public osg::DualModeVisitor
{
    public:

        ConvertToPageLODVistor( const std::string& basename,
                                const std::string& extension,
                                bool               makeAllChildrenPaged ) :
            osg::DualModeVisitor( osg::NodeVisitor::TRAVERSE_ALL_CHILDREN ),
            _basename( basename ),
        ...
        convert()
        {
            ...
            for( LODSet::iterator itr = _lodSet.begin(); itr != _lodSet.end();
                 ++itr, ++lodNum )
            {
                ...
                {
                    osg::notify( osg::NOTICE )
                        << "Warning can't operator on root node." << std::endl;
                    break;
                }
                ...
                {
                    osg::notify( osg::NOTICE )
                        << "Leaving LOD with one child as is." << std::endl;
                    break;
                }
                ...
                unsigned int                                              pos = 0;
                for( osg::LOD::RangeList::const_iterator ritr =
                         originalRangeList.begin();
                     ritr != originalRangeList.end();
                     ++ritr, ++pos )
                {
                    rangeMap.insert(
                        std::multimap<osg::LOD::MinMaxPair, unsigned int>::value_type(
                            *ritr,
                            pos
                        )
                    );
                     ...
                     ++mitr, ++pos )
                {
                    ...
                    {
                        plod->addChild( lod->getChild( mitr->second ),
                                        mitr->first.first,
                                        mitr->first.second );
                    }
                    else
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        arguments.getApplicationName() +
        " creates a hierarchy of files for paging which can be later loaded by viewers."
    );
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "-o",
        "set the output file (defaults to output.ive)"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--makeAllChildrenPaged",
        "Force all children of LOD to be written out as external PagedLOD children"
    );
