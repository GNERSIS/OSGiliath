...
class MyCopyOp : public osg::CopyOp
{
    public:
        ...
        virtual osg::Referenced*
        operator()( const osg::Referenced* ref ) const
        {
            writeIndent();
        ...
        virtual osg::Object*
        operator()( const osg::Object* obj ) const
        {
            writeIndent();
        ...
        virtual osg::Node*
        operator()( const osg::Node* node ) const
        {
            writeIndent();
        ...
        operator()( const osg::Drawable* drawable ) const
        {
            writeIndent();
        ...
        operator()( const osg::StateSet* stateset ) const
        {
            writeIndent();
        ...
        operator()( const osg::StateAttribute* attr ) const
        {
            writeIndent();
        ...
        virtual osg::Texture*
        operator()( const osg::Texture* text ) const
        {
            writeIndent();
                ...
                if( node->getNumParents() > 1 )
                {
                    if( _nodeCopyMap.find( node ) != _nodeCopyMap.end() )
                    {
                        std::cout << "Copy of node " << node << " " << node->getName()
                                  << ", " << _nodeCopyMap[node] << ", will be reused"
                                  << std::endl;
                    ...
                    {
                        osg::Node* newNode =
                            dynamic_cast<osg::Node*>( node->clone( *this ) );
      ...
      char** argv )
{
    ...
    osg::ref_ptr<osg::Node> mycopy =
        dynamic_cast<osg::Node*>( rootnode->clone( osg::CopyOp::DEEP_COPY_ALL ) );
    ...
    osg::ref_ptr<osg::Node> deep_copy = dynamic_cast<osg::Node*>(
        rootnode->clone( MyCopyOp( osg::CopyOp::DEEP_COPY_ALL ) )
    );
    ...
    osg::ref_ptr<osg::Node> graph_copy = dynamic_cast<osg::Node*>(
        rootnode->clone( GraphCopyOp( osg::CopyOp::DEEP_COPY_NODES ) )
    );

    // do a shallow copy.
    std::cout << "Doing a shallow copy of scene graph" << std::endl;
    osg::ref_ptr<osg::Node> shallow_copy = dynamic_cast<osg::Node*>(
        rootnode->clone( MyCopyOp( osg::CopyOp::SHALLOW_COPY ) )
    );
    ...
    // You can use a bit mask to control which parts of the scene graph are shallow
    // copied vs deep copied.  The options are (from include/osg/CopyOp) : enum Options {
    //        SHALLOW_COPY = 0,
    //        DEEP_COPY_OBJECTS = 1,
    //        DEEP_COPY_NODES = 2,
    ...
    //    osg::Node* mycopy =
    //    dynamic_cast<osg::Node*>(rootnode->clone(osg::CopyOp::DEEP_COPY_NODES |
    //    DEEP_COPY_DRAWABLES));
    // Which shares state but creates copies of all nodes and drawables (which contain
    // the geometry).
    //
    // You may also want to subclass from CopyOp to provide finer grained control of what
    // gets shared (shallow copy) vs cloned (deep copy).
