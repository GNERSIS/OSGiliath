...
bool
scrollWindow( osgWidget::Event& ev )
{
    ...
    // The user wants to scroll up; make sure that the visible area's Y origin isn't
    // already at 0.0f, 0.0f.
...
bool
changeTheme( osgWidget::Event& ev )
{
    std::string theme;
      ...
      char** argv )
{
    ...
    frame->addCallback( new osgWidget::Callback( &scrollWindow,
                                                 osgWidget::EVENT_MOUSE_SCROLL ) );
    frame->addCallback( new osgWidget::Callback( &changeTheme,
                                                 osgWidget::EVENT_KEY_DOWN ) );
