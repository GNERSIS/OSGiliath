...
Widget::Widget( const Widget&      widget,
    ...
    _hasEventFocus( false ),
    _graphicsInitialized( false ),
    _alignmentSettings( osg::clone( widget._alignmentSettings.get(),
                                    copyop ) ),
    _frameSettings( osg::clone( widget._frameSettings.get(),
                                copyop ) ),
    _textSettings( osg::clone( widget._textSettings.get(),
                               copyop ) ),
                ...
                if( _focusBehaviour == CLICK_TO_FOCUS )
                {
                    if( ea->getEventType() == osgGA::GUIEventAdapter::PUSH )
                    {
                        if( numButtonsPressed == 1 )
                        {
                            ...
                            bool       withinWidget =
                                computeExtentsPositionInLocalCoordinates( ev,
                                                                          ea,
                                                                          intersection );
...
Widget::traverseImplementation( osg::NodeVisitor& nv )
{
    ...
    if( ev )
    {
        if( _visible && _enabled )
        {
            ...
            // OSG_NOTICE<<"EventTraversal
            // getHasEventFocus()="<<getHasEventFocus()<<std::endl;
...
Widget::handle( osgGA::EventVisitor* ev,
                osgGA::Event*        event )
{
    // currently lua scripting takes a ref count so messes up handling of NodeVisitor's
    // created on stack, so don't attempt to call the sctipt.
...
void
Widget::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
Widget::releaseGLObjects( osg::State* state ) const
{
                                           ...
                                           osg::dvec3&             localPosition ) const
{
    ...
    if( aa && aa->computeIntersections( *event, ev->getNodePath(), intersections ) )
    {
        ...
        return ( _extents.contains( osg::vec3( localPosition ), 1E-6F ) );
    }
    else
    {
        return false;
...
{
        bool
        operator()( const osgUtil::LineSegmentIntersector::Intersection* lhs,
                    const osgUtil::LineSegmentIntersector::Intersection* rhs ) const
        {
            double epsilon = 1E-6;
            ...
            while( itr_lhs != end_lhs && itr_rhs != end_rhs )
            {
                ...
                {
                    OSG_NOTICE
                        << "SortTraversalOrder::operator() NodePath has no parent, just "
                           "have to use default less than operator for Intersection"
                        << std::endl;
                    return ( *lhs ) < ( *rhs );
                }
                else
                {
                    const osgUI::Widget* widget =
                        dynamic_cast<const osgUI::Widget*>( parent );

                    unsigned int lhs_index = parent->getChildIndex( *itr_lhs );
                    double       lhs_sort_value =
                        static_cast<double>( lhs_index ) /
                        static_cast<double>( parent->getNumChildren() );

                    unsigned int rhs_index = parent->getChildIndex( *itr_rhs );
                    double       rhs_sort_value =
                        ( static_cast<double>( rhs_index ) + epsilon ) /
                        static_cast<double>( parent->getNumChildren() );

                    if( widget )
                    {
                        const osgUI::Widget::GraphicsSubgraphMap& gsm =
                            widget->getGraphicsSubgraphMap();
                        for( osgUI::Widget::GraphicsSubgraphMap::const_iterator itr =
                                 gsm.begin();
                             itr != gsm.end();
                             ++itr )
                        {
...
bool
Widget::computeIntersections( osgGA::EventVisitor*    ev,
                              osgGA::GUIEventAdapter* event,
                              Intersections&          intersections,
                              osg::Node::NodeMask     traversalMask ) const
{
    osgGA::GUIActionAdapter*                       aa = ev ? ev->getActionAdapter() : 0;
    osgUtil::LineSegmentIntersector::Intersections source_intersections;
    if( aa && aa->computeIntersections( *event,
                                        ev->getNodePath(),
                                        source_intersections,
                                        traversalMask ) )
    {
        typedef std::vector<const osgUtil::LineSegmentIntersector::Intersection*>
                                IntersectionPointerList;
        IntersectionPointerList intersectionsToSort;

        // populate the temporary vector of pointers to the original intersection
        // pointers.
        for( osgUtil::LineSegmentIntersector::Intersections::iterator itr =
                 source_intersections.begin();
        ...
        // sort the pointer list into order based on child traversal order, to be
        // consistent with osgUI rendering order.
        std::sort( intersectionsToSort.begin(),
                   intersectionsToSort.end(),
                   SortTraversalOrder() );
...
bool
Widget::computeExtentsPositionInLocalCoordinates( osgGA::EventVisitor*    ev,
                                                  osgGA::GUIEventAdapter* event,
                                                  osg::dvec3&             localPosition,
                                                  bool withinExtents ) const
{
    ...
    osg::postMult( matrix, camera->getViewMatrix() );
    osg::postMult( matrix, camera->getProjectionMatrix() );

    double     zNear       = -1.0;
    double     zFar        = 1.0;

    osg::dmat4 inverse     = osg::inverse( matrix );

    osg::dvec3 startVertex = osg::dvec3( x, y, zNear ) * inverse;
    osg::dvec3 endVertex   = osg::dvec3( x, y, zFar ) * inverse;

    // OSG_NOTICE<<"   startVertex("<<startVertex<<"(,
    ...
    osg::dvec3 intersection = startVertex + ( endVertex - startVertex ) * r;
    // OSG_NOTICE<<"    intersection = "<<intersection<<std::endl;
    localPosition = intersection;

    return withinExtents ? _extents.contains( osg::vec3( localPosition ), 1E-6F ) : true;
}
