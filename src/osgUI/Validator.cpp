...
Validator::Validator( const osgUI::Validator& validator,
...
Validator::State
Validator::validate( std::string& text,
                     int&         cursorpos ) const
{
    const osg::CallbackObject* co = getCallbackObject( this, "validate" );
    if( co )
    {
        osg::ref_ptr<osg::StringValueObject> textInput =
            new osg::StringValueObject( "text", text );
        osg::ref_ptr<osg::IntValueObject> cursorposInput =
            new osg::IntValueObject( "cursorpos", cursorpos );
        ...
        if( co->run( const_cast<Validator*>( this ),
                     inputParameters,
                     outputParameters ) )
        {
            if( textInput->getValue() != text )
            {
                OSG_NOTICE << "Updating text in CallbackObject " << textInput->getValue()
                           << std::endl;
                text = textInput->getValue();
            }
            if( cursorposInput->getValue() != cursorpos )
            {
                OSG_NOTICE << "Updating cursor pos in CallbackObject "
                           << cursorposInput->getValue() << std::endl;
            ...
            if( outputParameters.size() >= 1 )
            {
                ...
                osg::StringValueObject* svo =
                    dynamic_cast<osg::StringValueObject*>( object );
                if( svo )
                {
                    OSG_NOTICE << "Have string return value from validate "
                               << svo->getValue() << std::endl;
                ...
                OSG_NOTICE << "Called validate CallbackObject but didn't get string "
                              "return value."
                           << object->className() << std::endl;
            }
        }
    }
...
Validator::State
Validator::validateImplementation( std::string& text,
                                   int&         cursorpos ) const
{
    OSG_NOTICE << "Validator::validateImplemetation(" << text << ", " << cursorpos << ")"
               << std::endl;
    return ACCEPTABLE;
}

void
Validator::fixup( std::string& text ) const
{
...
void
Validator::fixupImplementation( std::string& text ) const
{
...
IntValidator::IntValidator( const IntValidator& validator,
                            const osg::CopyOp&  copyop ) :
    Inherit( validator,
             copyop ),
...
IntValidator::State
IntValidator::validateImplementation( std::string& str,
                                      int& /*cursorpos*/ ) const
{
...
void
IntValidator::fixupImplementation( std::string& str ) const
{
    if( str.empty() )
    {
        return;
...
DoubleValidator::State
DoubleValidator::validateImplementation( std::string& str,
                                         int& /*cursorpos*/ ) const
{
    std::string newstring;
    bool        canBeNegative = _bottom < 0.0;
    int         maxNumDecimalPlaces =
        _decimals >= 0 ? _decimals : static_cast<int>( str.size() );
...
void
DoubleValidator::fixupImplementation( std::string& str ) const
{
    if( str.empty() )
    {
        return;
