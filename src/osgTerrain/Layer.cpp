...
void
osgTerrain::extractSetNameAndFileName( const std::string& compoundstring,
                                       std::string&       setname,
                                       std::string&       filename )
{
    ...
    std::string::size_type secondcolonpos =
        compoundstring.find_first_of( ':', setcolonpos + 4 );
...
std::string
osgTerrain::createCompoundSetNameAndFileName( const std::string& setname,
...
osg::sphere
Layer::computeBound( bool treatAsElevationLayer ) const
{
    osg::sphere bs;
    if( !getLocator() )
    {
    ...
    else
    {
        ...
        osg::dvec3 v;
        if( getLocator()->convertLocalToModel( osg::dvec3( 0.5, 0.5, 0.0 ), v ) )
        {
            bs.center = osg::vec3( v );
        }

        if( getLocator()->convertLocalToModel( osg::dvec3( 0.0, 0.0, 0.0 ), v ) )
        {
            bs.radius = osg::length( osg::dvec3( bs.center ) - v );
        }
    }
            ...
            const O&       operation )
{
    ...
    {
        case( GL_BYTE ) :
            _processRow( num, pixelFormat, ( char* )data, operation );
            break;
        case( GL_UNSIGNED_BYTE ) :
            _processRow( num, pixelFormat, ( unsigned char* )data, operation );
            break;
        case( GL_SHORT ) :
            _processRow( num, pixelFormat, ( short* )data, operation );
            break;
        case( GL_UNSIGNED_SHORT ) :
            _processRow( num, pixelFormat, ( unsigned short* )data, operation );
            break;
        case( GL_INT ) :
            _processRow( num, pixelFormat, ( int* )data, operation );
            break;
        case( GL_UNSIGNED_INT ) :
            _processRow( num, pixelFormat, ( unsigned int* )data, operation );
            break;
        case( GL_FLOAT ) :
            _processRow( num, pixelFormat, ( float* )data, operation );
            break;
    }
}
...
{
        TransformOperator( float offset,
                           float scale ) :
            _offset( offset ),
            _scale( scale )
        {
        ...
        inline void
        operator()( unsigned short& v ) const
        {
...
bool
ImageLayer::transform( float offset,
                       float scale )
{
    if( !_image.valid() )
    {
...
bool
ImageLayer::getValue( unsigned int i,
                      unsigned int j,
                      float&       value ) const
{
...
ContourLayer::ContourLayer( const ContourLayer& contourLayer,
                            const osg::CopyOp&  copyop ) :
    Inherit( contourLayer,
             copyop ),
...
void
ContourLayer::setTransferFunction( osg::TransferFunction1D* tf )
{
    _tf = tf;
}

bool
ContourLayer::transform( float offset,
                         float scale )
{
    if( !_tf )
    {
...
bool
ContourLayer::getValue( unsigned int i,
                        unsigned int /*j*/,
                        float& value ) const
{
    if( !_tf )
...
void
ContourLayer::setModifiedCount( unsigned int value )
{
    if( getImage() )
    {
...
unsigned int
ContourLayer::getModifiedCount() const
{
    if( !getImage() )
    {
        return 0;
...
HeightFieldLayer::HeightFieldLayer( const HeightFieldLayer& hfLayer,
                                    const osg::CopyOp&      copyop ) :
    Inherit( hfLayer,
             copyop ),
    _modifiedCount( 0 ),
...
void
HeightFieldLayer::setHeightField( osg::HeightField* hf )
{
    _heightField = hf;
    dirty();
}
...
bool
HeightFieldLayer::transform( float offset,
                             float scale )
{
    if( !_heightField )
    {
    ...
    OSG_INFO << "HeightFieldLayer::transform(" << offset << "," << scale << ")"
             << std::endl;
    ;

    for( osg::FloatArray::iterator itr = heights->begin(); itr != heights->end(); ++itr )
    {
...
bool
HeightFieldLayer::getValue( unsigned int i,
                            unsigned int j,
                            float&       value ) const
{
...
void
HeightFieldLayer::dirty()
{
    ++_modifiedCount;
}

void
HeightFieldLayer::setModifiedCount( unsigned int value )
{
    _modifiedCount = value;
}

unsigned int
HeightFieldLayer::getModifiedCount() const
{
...
ProxyLayer::ProxyLayer( const ProxyLayer&  proxyLayer,
                        const osg::CopyOp& copyop ) :
    Inherit( proxyLayer,
             copyop )
{
...
void
ProxyLayer::setFileName( const std::string& filename )
{
    _filename = filename;
...
unsigned int
ProxyLayer::getNumColumns() const
{
    if( _implementation.valid() )
    {
...
bool
ProxyLayer::transform( float offset,
                       float scale )
{
    if( _implementation.valid() )
    {
...
bool
ProxyLayer::getValue( unsigned int i,
                      unsigned int j,
                      float&       value ) const
{
    if( _implementation.valid() )
...
void
ProxyLayer::setModifiedCount( unsigned int value )
{
    if( _implementation.valid() )
    {
...
unsigned int
ProxyLayer::getModifiedCount() const
{
    return _implementation.valid() ? _implementation->getModifiedCount() : 0;
}

osg::sphere
ProxyLayer::computeBound( bool treatAsElevationLayer ) const
{
    if( _implementation.valid() )
    {
...
void
CompositeLayer::setCompoundName( unsigned int       i,
...
std::string
CompositeLayer::getCompoundName( unsigned int i ) const
{
    return createCompoundSetNameAndFileName( _layers[i].setname, _layers[i].filename );
}

void
CompositeLayer::addLayer( const std::string& compoundname )
{
    std::string setname;
    std::string filename;
    extractSetNameAndFileName( compoundname, setname, filename );
...
void
CompositeLayer::addLayer( const std::string& setname,
                          const std::string& filename )
{
    _layers.push_back( CompoundNameLayer( setname, filename, 0 ) );
}
...
SwitchLayer::SwitchLayer( const SwitchLayer& switchLayer,
                          const osg::CopyOp& copyop ) :
    Inherit( switchLayer,
             copyop ),
