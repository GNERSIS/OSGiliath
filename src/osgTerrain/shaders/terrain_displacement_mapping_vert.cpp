char terrain_displacement_mapping_vert[] =
    "#version 460 core\n"
    ...
    "void main(void)\n"
    "{\n"
    "    vec2 texcoord_center = osg_MultiTexCoord0.xy;\n"
    "\n"
    "#ifdef HEIGHTFIELD_LAYER\n"
    "    float height_center = texture(terrainTexture, texcoord_center).r;\n"
    "#else\n"
    "    float height_center = 0.0;\n"
    "#endif\n"
    "\n"
    "#if defined(LIGHTING) && defined(HEIGHTFIELD_LAYER)\n"
    "    vec2 texelWorldRatio = osg_MultiTexCoord0.zw;\n"
    "    vec2 texelTexcoordSize = osg_Color.xy;\n"
    "\n"
    "    vec2 texcoord_right = vec2(texcoord_center.x+texelTexcoordSize.x, "
    "texcoord_center.y);\n"
    "    vec2 texcoord_left = vec2(texcoord_center.x-texelTexcoordSize.x, "
    "texcoord_center.y);\n"
    "    vec2 texcoord_up = vec2(texcoord_center.x, "
    "texcoord_center.y+texelTexcoordSize.y);\n"
    "    vec2 texcoord_down = vec2(texcoord_center.x, "
    "texcoord_center.y-texelTexcoordSize.y);\n"
    "\n"
    "    float dz_dx = 0.0;\n"
    "    float dx = 0.0;\n"
    ...
    "    {\n"
    "        float height = texture(terrainTexture, texcoord_left).r;\n"
    ...
    "    {\n"
    "        float height = texture(terrainTexture, texcoord_right).r;\n"
    ...
    "    {\n"
    "        float height = texture(terrainTexture, texcoord_down).r;\n"
    ...
    "    {\n"
    "        float height = texture(terrainTexture, texcoord_up).r;\n"
    ...
    "\n"
    "\n"
    "    vec3 normal = normalize(rotate_x_mat * (rotate_y_mat * osg_Normal.xyz));\n"
    "    //vec3 normal = normalize(osg_Normal.xyz);\n"
    "    vec4 color = vec4(1.0,1.0,1.0,1.0);\n"
    "    directionalLight( 0, normal, color);\n"
    "\n"
    "#elif defined(LIGHTING)\n"
    "    vec3 normal = osg_Normal.xyz;\n"
    "    vec4 color = vec4(1.0, 1.0, 1.0, 1.0);\n"
    "    directionalLight( 0, normal, color);\n"
    "#else\n"
    "    vec3 normal = osg_Normal.xyz;\n"
    ...
    "#endif\n"
    "\n"
    "    vec3 position = osg_Vertex.xyz + osg_Normal.xyz * height_center;\n"
