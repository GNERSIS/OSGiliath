...
GeometryTechnique::GeometryTechnique( const GeometryTechnique& gt,
...
void
GeometryTechnique::setFilterBias( float filterBias )
{
...
void
GeometryTechnique::setFilterWidth( float filterWidth )
{
...
void
GeometryTechnique::setFilterMatrix( const osg::Matrix3& matrix )
{
...
void
GeometryTechnique::setFilterMatrixAs( FilterType filterType )
{
    switch( filterType )
    {
        case( SMOOTH ) :
            setFilterMatrix( osg::Matrix3( 0.0,
                                           0.5 / 2.5,
                                           0.0,
                                           0.5 / 2.5,
                                           0.5 / 2.5,
        ...
        case( SHARPEN ) :
            setFilterMatrix(
                osg::Matrix3( 0.0, -1.0, 0.0, -1.0, 5.0, -1.0, 0.0, -1.0, 0.0 )
            );
            break;
    };
}
...
void
GeometryTechnique::init( int  dirtyMask,
                         bool assumeMultiThreaded )
{
    ...
    std::lock_guard<std::mutex> lock( _writeBufferMutex );
    ...
    osg::dvec3               centerModel = computeCenterModel( *buffer, masterLocator );
    ...
    {
        // there is already an active _currentBufferData so we'll request that this gets
        // swapped on next frame.
...
Locator*
GeometryTechnique::computeMasterLocator()
{
    ...
    if( !masterLocator )
    {
        OSG_NOTICE << "Problem, no locator found in any of the terrain layers"
                   << std::endl;
        return 0;
    }
...
osg::dvec3
GeometryTechnique::computeCenterModel( BufferData& buffer,
                                       Locator*    masterLocator )
{
    if( !masterLocator )
    {
        return osg::dvec3( 0.0, 0.0, 0.0 );
    }
    ...
    osg::dvec3 bottomLeftNDC( DBL_MAX, DBL_MAX, 0.0 );
    osg::dvec3 topRightNDC( -DBL_MAX, -DBL_MAX, 0.0 );

    if( elevationLayer )
    {
        if( elevationLocator != masterLocator )
        {
            masterLocator->computeLocalBounds( *elevationLocator,
                                               bottomLeftNDC,
                                               topRightNDC );
        }
        else
        {
            bottomLeftNDC.x = std::min( bottomLeftNDC.x, 0.0 );
            bottomLeftNDC.y = std::min( bottomLeftNDC.y, 0.0 );
            topRightNDC.x   = std::max( topRightNDC.x, 1.0 );
            topRightNDC.y   = std::max( topRightNDC.y, 1.0 );
        }
    }
    ...
    if( colorLayer )
    {
        ...
        {
            masterLocator->computeLocalBounds( *colorLocator,
                                               bottomLeftNDC,
                                               topRightNDC );
        }
        else
    ...
    osg::dvec3 centerNDC   = ( bottomLeftNDC + topRightNDC ) * 0.5;
    osg::dvec3 centerModel = ( bottomLeftNDC + topRightNDC ) * 0.5;
    masterLocator->convertLocalToModel( centerNDC, centerModel );

    buffer._transform->setMatrix( osg::translate( centerModel ) );
...
class VertexNormalGenerator
{
    public:
        ...
        VertexNormalGenerator( Locator*          masterLocator,
        ...
        void
        populateCenter( osgTerrain::Layer*  elevationLayer,
                        LayerToTexCoordMap& layerToTexCoordMap );
        void
        populateLeftBoundary( osgTerrain::Layer* elevationLayer );
        void
        populateRightBoundary( osgTerrain::Layer* elevationLayer );
        void
        populateAboveBoundary( osgTerrain::Layer* elevationLayer );
        void
        populateBelowBoundary( osgTerrain::Layer* elevationLayer );
                   ...
                   const osg::vec3& n )
        {
            ...
            if( i == 0 )
            {
                if( r < 0 || r >= _numRows || c < 0 || c >= _numColumns )
                {
                    ...
                    // OSG_NOTICE<<"setVertex("<<c<<", "<<r<<", ["<<v<<"], ["<<n<<"]),
                    // i="<<i<<"
                    // _boundaryVertices["<<-i-1<<"]="<<(*_boundaryVertices)[-i-1]<<"]"<<std::endl;
                }
                else
                {
                    i = _vertices->size() + 1;
                    ...
                    // OSG_NOTICE<<"setVertex("<<c<<", "<<r<<", ["<<v<<"], ["<<n<<"]),
                    // i="<<i<<"
                    // _vertices["<<i-1<<"]="<<(*_vertices)[i-1]<<"]"<<std::endl;
                }
            }
            else if( i < 0 )
            {
            ...
            {
                // OSG_NOTICE<<"Overwriting setVertex("<<c<<", "<<r<<", ["<<v<<"],
                // ["<<n<<"]"<<std::endl; OSG_NOTICE<<"     previous values ( vertex
                // ["<<(*_vertices)[i-1]<<"], normal (*_normals)[i-1]
                // ["<<n<<"]"<<std::endl;
                // (*_vertices)[i-1] = v;
        ...
        inline bool
        computeNormalWithNoDiagonals( int        c,
                                      int        r,
                                      osg::vec3& n ) const
        {
            osg::vec3 center;
            ...
            n = dx ^ dy;
            n = osg::normalize( n );
            return n != osg::vec3( 0.0F, 0.0F, 0.0F );
        }

        inline bool
        computeNormalWithDiagonals( int        c,
                                    int        r,
                                    osg::vec3& n ) const
        {
            osg::vec3 center;
...
VertexNormalGenerator::VertexNormalGenerator( Locator*          masterLocator,
                                              const osg::dvec3& centerModel,
                                              int               numRows,
                                              int               numColumns,
                                              float             scaleHeight,
                                       ...
                                       LayerToTexCoordMap& layerToTexCoordMap )
{
    ...
    bool sampled =
        ...
        ( ( elevationLayer->getNumRows() != static_cast<unsigned int>( _numRows ) ) ||
          ( elevationLayer->getNumColumns() !=
            static_cast<unsigned int>( _numColumns ) ) );

    for( int j = 0; j < _numRows; ++j )
    {
        for( int i = 0; i < _numColumns; ++i )
        {
            osg::dvec3 ndc( ( ( double )i ) / ( double )( _numColumns - 1 ),
                            ( ( double )j ) / ( double )( _numRows - 1 ),
                            0.0 );
            ...
            if( elevationLayer )
            {
                ...
                {
                    validValue =
                        elevationLayer->getInterpolatedValidValue( ndc.x, ndc.y, value );
                }
                else
                {
            ...
            if( validValue )
            {
                ...
                for( VertexNormalGenerator::LayerToTexCoordMap::iterator itr =
                         layerToTexCoordMap.begin();
                     itr != layerToTexCoordMap.end();
                     ++itr )
                {
                    osg::Vec2Array*         texcoords = itr->second.first.get();
                    osgTerrain::ImageLayer* imageLayer(
                        dynamic_cast<osgTerrain::ImageLayer*>( itr->first )
                    );
                    ...
                    if( imageLayer != NULL )
                    {
                        ...
                        {
                            osg::dvec3 color_ndc;
                            Locator::convertLocalCoordBetween( *_masterLocator,
                                                               ndc,
                                                               *colorLocator,
                                                               color_ndc );
                            ( *texcoords )
                                .push_back( osg::vec2( color_ndc.x, color_ndc.y ) );
                        }
                        else
                        {
                    ...
                    {
                        osgTerrain::ContourLayer* contourLayer(
                            dynamic_cast<osgTerrain::ContourLayer*>( itr->first )
                        );
                        ...
                        if( contourLayer )
                        {
                            osg::TransferFunction1D* transferFunction =
                                contourLayer->getTransferFunction();
                            if( transferFunction )
                            {
                                float difference = transferFunction->getMaximum() -
                                                   transferFunction->getMinimum();
                                if( difference != 0.0F )
                                {
                                    osg::dvec3           color_ndc;
                                    osgTerrain::Locator* colorLocator(
                                        itr->second.second
                                    );
                ...
                osg::dvec3 ndc_one  = ndc;
                ndc_one.z          += 1.0;
                osg::dvec3 model_one;
                _masterLocator->convertLocalToModel( ndc_one, model_one );
                model_one = model_one - model;
                model_one = osg::normalize( model_one );

                setVertex( i,
                           j,
                           osg::vec3( model - _centerModel ),
                           osg::vec3( model_one ) );
            }
        }
    }
...
void
VertexNormalGenerator::populateLeftBoundary( osgTerrain::Layer* elevationLayer )
{
    // OSG_NOTICE<<"
    // VertexNormalGenerator::populateLeftBoundary("<<elevationLayer<<")"<<std::endl;
    ...
    for( int j = 0; j < _numRows; ++j )
    {
        for( int i = -1; i <= 0; ++i )
        {
            osg::dvec3 ndc( ( ( double )i ) / ( double )( _numColumns - 1 ),
                            ( ( double )j ) / ( double )( _numRows - 1 ),
                            0.0 );
            osg::dvec3 left_ndc( 1.0 + ndc.x, ndc.y, 0.0 );
            ...
            if( elevationLayer )
            {
                ...
                if( sampled )
                {
                    validValue = elevationLayer->getInterpolatedValidValue( left_ndc.x,
                                                                            left_ndc.y,
                                                                            value );
                }
                else
                {
                    validValue = elevationLayer->getValidValue( ( _numColumns - 1 ) + i,
                                                                j,
                                                                value );
                }
                ndc.z  = value * _scaleHeight;
...
void
VertexNormalGenerator::populateRightBoundary( osgTerrain::Layer* elevationLayer )
{
    // OSG_NOTICE<<"
    // VertexNormalGenerator::populateRightBoundary("<<elevationLayer<<")"<<std::endl;
    ...
    for( int j = 0; j < _numRows; ++j )
    {
        for( int i = _numColumns - 1; i < _numColumns + 1; ++i )
        {
            osg::dvec3 ndc( ( ( double )i ) / ( double )( _numColumns - 1 ),
                            ( ( double )j ) / ( double )( _numRows - 1 ),
                            0.0 );
            osg::dvec3 right_ndc( ndc.x - 1.0, ndc.y, 0.0 );
            ...
            if( elevationLayer )
            {
                ...
                if( sampled )
                {
                    validValue = elevationLayer->getInterpolatedValidValue( right_ndc.x,
                                                                            right_ndc.y,
                                                                            value );
                }
                else
                {
                    validValue = elevationLayer->getValidValue( i - ( _numColumns - 1 ),
                                                                j,
                                                                value );
                }
                ndc.z = value * _scaleHeight;
...
void
VertexNormalGenerator::populateAboveBoundary( osgTerrain::Layer* elevationLayer )
{
    // OSG_NOTICE<<"
    // VertexNormalGenerator::populateAboveBoundary("<<elevationLayer<<")"<<std::endl;
    ...
    for( int j = _numRows - 1; j < _numRows + 1; ++j )
    {
        for( int i = 0; i < _numColumns; ++i )
        {
            osg::dvec3 ndc( ( ( double )i ) / ( double )( _numColumns - 1 ),
                            ( ( double )j ) / ( double )( _numRows - 1 ),
                            0.0 );
            osg::dvec3 above_ndc( ndc.x, ndc.y - 1.0, 0.0 );
            ...
            if( elevationLayer )
            {
                ...
                if( sampled )
                {
                    validValue = elevationLayer->getInterpolatedValidValue( above_ndc.x,
                                                                            above_ndc.y,
                                                                            value );
                }
                else
                {
                    validValue =
                        elevationLayer->getValidValue( i, j - ( _numRows - 1 ), value );
                }
                ndc.z = value * _scaleHeight;
            }
...
void
VertexNormalGenerator::populateBelowBoundary( osgTerrain::Layer* elevationLayer )
{
    // OSG_NOTICE<<"
    // VertexNormalGenerator::populateBelowBoundary("<<elevationLayer<<")"<<std::endl;
    ...
    for( int j = -1; j <= 0; ++j )
    {
        for( int i = 0; i < _numColumns; ++i )
        {
            osg::dvec3 ndc( ( ( double )i ) / ( double )( _numColumns - 1 ),
                            ( ( double )j ) / ( double )( _numRows - 1 ),
                            0.0 );
            osg::dvec3 below_ndc( ndc.x, 1.0 + ndc.y, 0.0 );
            ...
            if( elevationLayer )
            {
                ...
                if( sampled )
                {
                    validValue = elevationLayer->getInterpolatedValidValue( below_ndc.x,
                                                                            below_ndc.y,
                                                                            value );
                }
                else
                {
                    validValue =
                        elevationLayer->getValidValue( i, ( _numRows - 1 ) + j, value );
                }
                ndc.z = value * _scaleHeight;
            }
...
void
VertexNormalGenerator::computeNormals()
{
...
void
GeometryTechnique::generateGeometry( BufferData&       buffer,
                                     Locator*          masterLocator,
                                     const osg::dvec3& centerModel )
{
    ...
    if( ( sampleRatio != 1.0F ) &&
        ( numColumns > minimumNumColumns ) &&
        ( numRows > minimumNumRows ) )
    {
        unsigned int originalNumColumns = numColumns;
        unsigned int originalNumRows    = numRows;

        numColumns = std::max( ( unsigned int )( float( originalNumColumns ) *
                                                 sqrtf( sampleRatio ) ),
                               minimumNumColumns );
        numRows    = std::max( ( unsigned int )( float( originalNumRows ) *
                                                 sqrtf( sampleRatio ) ),
                               minimumNumRows );
    }

    bool treatBoundariesToValidDataAsDefaultValue =
        _terrainTile->getTreatBoundariesToValidDataAsDefaultValue();
    OSG_INFO << "TreatBoundariesToValidDataAsDefaultValue="
             << treatBoundariesToValidDataAsDefaultValue << std::endl;
    ...
    // construct the VertexNormalGenerator which will manage the generation and the
    // vertices and normals
    VertexNormalGenerator VNG( masterLocator,
                               centerModel,
                               numRows,
                               numColumns,
                               scaleHeight,
    ...
    for( unsigned int layerNum = 0; layerNum < _terrainTile->getNumColorLayers();
         ++layerNum )
    {
        osgTerrain::Layer* colorLayer = _terrainTile->getColorLayer( layerNum );
        if( colorLayer )
        {
            VertexNormalGenerator::LayerToTexCoordMap::iterator itr =
                layerToTexCoordMap.find( colorLayer );
            ...
            else
            {
                ...
                if( !locator )
                {
                    osgTerrain::SwitchLayer* switchLayer =
                        dynamic_cast<osgTerrain::SwitchLayer*>( colorLayer );
                    if( switchLayer )
                    {
                        if( switchLayer->getActiveLayer() >=
                            0 &&
                            static_cast<unsigned int>( switchLayer->getActiveLayer() ) <
                            switchLayer->getNumLayers() &&
                ...
                VertexNormalGenerator::TexCoordLocatorPair& tclp =
                    layerToTexCoordMap[colorLayer];
    ...
    if( terrain && terrain->getEqualizeBoundaries() )
    {
        ...
        osg::ref_ptr<TerrainTile> left_tile =
            terrain->getTile( TileID( tileID.level, tileID.x - 1, tileID.y ) );
        osg::ref_ptr<TerrainTile> right_tile =
            terrain->getTile( TileID( tileID.level, tileID.x + 1, tileID.y ) );
        osg::ref_ptr<TerrainTile> top_tile =
            terrain->getTile( TileID( tileID.level, tileID.x, tileID.y + 1 ) );
        osg::ref_ptr<TerrainTile> bottom_tile =
            terrain->getTile( TileID( tileID.level, tileID.x, tileID.y - 1 ) );
...
#endif
        VNG.populateLeftBoundary( left_tile.valid() ? left_tile->getElevationLayer()
                                                    : 0 );
        VNG.populateRightBoundary( right_tile.valid() ? right_tile->getElevationLayer()
                                                      : 0 );
        VNG.populateAboveBoundary( top_tile.valid() ? top_tile->getElevationLayer()
                                                    : 0 );
        VNG.populateBelowBoundary( bottom_tile.valid() ? bottom_tile->getElevationLayer()
                                                       : 0 );
        ...
        if( left_tile.valid() )
        {
            if( left_tile->getTerrainTechnique() ==
                0 ||
                !(
                    left_tile->getTerrainTechnique()->containsNeighbour( _terrainTile )
                ) )
            {
        ...
        if( right_tile.valid() )
        {
            if( right_tile->getTerrainTechnique() ==
                0 ||
                !(
                    right_tile->getTerrainTechnique()->containsNeighbour( _terrainTile )
                ) )
            {
                int dirtyMask =
                    right_tile->getDirtyMask() | TerrainTile::RIGHT_EDGE_DIRTY;
                if( updateNeighboursImmediately )
                {
        ...
        if( top_tile.valid() )
        {
            if( top_tile->getTerrainTechnique() ==
                0 ||
                !( top_tile->getTerrainTechnique()->containsNeighbour( _terrainTile ) ) )
            {
        ...
        if( bottom_tile.valid() )
        {
            if( bottom_tile->getTerrainTechnique() ==
                0 ||
                !(
                    bottom_tile->getTerrainTechnique()->containsNeighbour( _terrainTile )
                ) )
            {
                int dirtyMask =
                    bottom_tile->getDirtyMask() | TerrainTile::BOTTOM_EDGE_DIRTY;
                if( updateNeighboursImmediately )
                {
    ...
    for( j = 0; j < numRows - 1; ++j )
    {
        for( i = 0; i < numColumns - 1; ++i )
        {
            ...
            if( numValid == 4 )
            {
                ...
                float dot_00_11 =
                    osg::dot( ( *VNG._normals )[i00], ( *VNG._normals )[i11] );
                float dot_01_10 =
                    osg::dot( ( *VNG._normals )[i01], ( *VNG._normals )[i10] );
    ...
    if( createSkirt )
    {
        ...
        osg::ref_ptr<osg::DrawElements> skirtDrawElements =
            smallTile ? static_cast<osg::DrawElements*>(
        ...
        for( c = 0; c < static_cast<int>( numColumns ); ++c )
        {
            ...
            {
                unsigned int new_i =
                    vertices->size();    // index of new index of added skirt point
                osg::vec3 new_v =
                    ( *vertices )[orig_i] - ( ( *skirtVectors )[orig_i] ) * skirtHeight;
        ...
        for( r = numRows - 1; r >= 0; --r )
        {
            ...
            else
            {
                ...
                {
                    geometry->addPrimitiveSet( skirtDrawElements.get() );
                    skirtDrawElements = new osg::DrawElementsUShort( GL_TRIANGLE_STRIP );
                }
            }
        }
        ...
        osgDB::Registry::instance()->getKdTreeBuilder() )
    {
        ...
        // OSG_NOTICE<<"KdTree build time "<<osg::Timer::instance()->delta_m(before,
        // after)<<std::endl;
    }
}

void
GeometryTechnique::applyColorLayers( BufferData& buffer )
{
         ...
         ++layerNum )
    {
        ...
        osgTerrain::ImageLayer* imageLayer =
            dynamic_cast<osgTerrain::ImageLayer*>( colorLayer );
        osgTerrain::ContourLayer* contourLayer =
            dynamic_cast<osgTerrain::ContourLayer*>( colorLayer );
        if( imageLayer )
        {
            osg::StateSet*  stateset = buffer._geode->getOrCreateStateSet();

            osg::Texture2D* texture2D =
                dynamic_cast<osg::Texture2D*>( layerToTextureMap[colorLayer] );
            if( !texture2D )
            {
                ...
                texture2D->setFilter( osg::Texture::MIN_FILTER,
                                      colorLayer->getMinFilter() );
                texture2D->setFilter( osg::Texture::MAG_FILTER,
                                      colorLayer->getMagFilter() );

                texture2D->setWrap( osg::Texture::WRAP_S, osg::Texture::CLAMP_TO_EDGE );
                texture2D->setWrap( osg::Texture::WRAP_T, osg::Texture::CLAMP_TO_EDGE );

                bool mipMapping = !( texture2D->getFilter( osg::Texture::MIN_FILTER ) ==
                                     osg::Texture::LINEAR ||
                                     texture2D->getFilter( osg::Texture::MIN_FILTER ) ==
                                     osg::Texture::NEAREST );
                bool s_NotPowerOfTwo =
                    image->s() == 0 || ( image->s() & ( image->s() - 1 ) );
                bool t_NotPowerOfTwo =
                    image->t() == 0 || ( image->t() & ( image->t() - 1 ) );

                if( mipMapping && ( s_NotPowerOfTwo || t_NotPowerOfTwo ) )
                {
                    OSG_INFO << "Disabling mipmapping for non power of two tile size("
                             << image->s() << ", " << image->t() << ")" << std::endl;
                    texture2D->setFilter( osg::Texture::MIN_FILTER,
                                          osg::Texture::LINEAR );
                }
                ...
                // OSG_NOTICE<<"Creating new ImageLayer texture "<<layerNum<<"
                // image->s()="<<image->s()<<"  image->t()="<<image->t()<<std::endl;
            }
            else
            {
            ...
            stateset->setTextureAttributeAndModes( layerNum,
                                                   texture2D,
        ...
        else if( contourLayer )
        {
            ...
            osg::Texture1D* texture1D =
                dynamic_cast<osg::Texture1D*>( layerToTextureMap[colorLayer] );
            if( !texture1D )
            {
                ...
                texture1D->setFilter( osg::Texture::MAG_FILTER,
                                      colorLayer->getMagFilter() );
...
void
GeometryTechnique::applyTransparency( BufferData& buffer )
{
    ...
    {
        OSG_INFO
            << "GeometryTechnique::applyTransparency() inheriting policy from Terrain"
            << std::endl;
    ...
    {
        OSG_INFO << "GeometryTechnique::applyTransparency() policy is INHERIT, "
                    "defaulting to ENABLE_BLENDING_WHEN_ALPHA_PRESENT"
                 << std::endl;
    ...
    {
        OSG_INFO << "blendingPolicy == TerrainTile::ENABLE_BLENDING_WHEN_ALPHA_PRESENT"
                 << std::endl;
        for( unsigned int i = 0; i < _terrainTile->getNumColorLayers(); ++i )
        {
            osg::Image* image = ( _terrainTile->getColorLayer( i ) != 0 )
                                  ? _terrainTile->getColorLayer( i )->getImage()
                                  : 0;
            if( image )
            {
...
void
GeometryTechnique::update( osgUtil::UpdateVisitor* uv )
{
    if( _terrainTile )
    {
...
void
GeometryTechnique::cull( osgUtil::CullVisitor* cv )
{
...
void
GeometryTechnique::traverse( osg::NodeVisitor& nv )
{
    if( !_terrainTile )
    {
        return;
...
void
GeometryTechnique::cleanSceneGraph()
{
}

void
GeometryTechnique::releaseGLObjects( osg::State* state ) const
{
