...
const osgTerrain::Locator*
osgTerrain::computeMasterLocator( const osgTerrain::TerrainTile* tile )
{
...
bool
GeometryPool::createKeyForTile( TerrainTile* tile,
                                GeometryKey& key )
{
    const osgTerrain::Locator* masterLocator = computeMasterLocator( tile );
    if( masterLocator )
    {
        const osg::dmat4& matrix       = masterLocator->getTransform();
        osg::dvec3        bottom_left  = matrix * osg::dvec3( 0.0, 0.0, 0.0 );
        osg::dvec3        bottom_right = matrix * osg::dvec3( 1.0, 0.0, 0.0 );
        osg::dvec3        top_left     = matrix * osg::dvec3( 1.0, 1.0, 0.0 );
        key.sx = static_cast<float>( osg::length( bottom_right - bottom_left ) );
        key.sy = static_cast<float>( osg::length( top_left - bottom_left ) );

        if( masterLocator->getCoordinateSystemType() == osgTerrain::Locator::GEOCENTRIC )
        {
            // need to differentiate between tiles based of latitude, so use y position
            // of bottom left corner.
            key.y = static_cast<float>( bottom_left.y );
        }
        else
        {
            // when the projection is linear there is no need to differentiate tiles
            // according to their latitude
            key.y = 0.0;
        }
    }

    osgTerrain::HeightFieldLayer* layer =
        dynamic_cast<osgTerrain::HeightFieldLayer*>( tile->getElevationLayer() );
    if( layer )
    {
...
GeometryPool::getOrCreateGeometry( osgTerrain::TerrainTile* tile )
{
    ...
    SharedGeometry::VertexToHeightFieldMapping& vthfm =
        geometry->getVertexToHeightFieldMapping();

    osg::ref_ptr<osg::Vec3Array> vertices =
        new osg::Vec3Array( osg::Array::BIND_PER_VERTEX );
    ...
    osg::ref_ptr<osg::Vec4Array> colours =
        new osg::Vec4Array( osg::Array::BIND_OVERALL );
    geometry->setColorArray( colours.get() );
    colours->push_back( osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ) );

    osg::ref_ptr<osg::Vec4Array> texcoords =
        new osg::Vec4Array( osg::Array::BIND_PER_VERTEX );
    ...
    typedef std::vector<osg::dvec4> LocationCoords;
    ...
    {
        osg::dvec3 bottom_left( 0.0, 0.0, 0.0 );
        osg::dvec3 top_right( 1.0, 1.0, 0.0 );
        ...
        if( em && locator->getCoordinateSystemType() == osgTerrain::Locator::GEOCENTRIC )
        {
            ...
            em->convertLatLongHeightToXYZ( bottom_left.y,
                                           bottom_left.x,
                                           bottom_left.z,
                                           bottom_left.x,
                                           bottom_left.y,
                                           bottom_left.z );
            em->convertLatLongHeightToXYZ( top_right.y,
                                           top_right.x,
                                           top_right.z,
                                           top_right.x,
                                           top_right.y,
        ...
        double diagonalLength = osg::length( top_right - bottom_left );
    ...
    {
        osg::dvec3 pos( 0.0, 0.0, 0.0 );
        osg::dvec3 normal( 0.0, 0.0, 1.0 );
        osg::vec2  delta( 1.0F / static_cast<float>( nx ),
                          1.0F / static_cast<float>( ny ) );
        ...
        for( int c = 0; c < nx; ++c )
        {
            ...
            vertices->push_back( osg::vec3( pos ) );
            normals->push_back( osg::vec3( normal ) );
            texcoords->push_back( osg::vec4( pos.x, pos.y, 1.0F, 1.0F ) );
            locationCoords.push_back( osg::dvec4( pos.x, pos.y, c_mult, r_mult ) );
    ...
    GLenum                          primitiveTypes = GL_TRIANGLES;

    osg::ref_ptr<osg::DrawElements> elements =
        smallTile ? static_cast<osg::DrawElements*>(
                        new osg::DrawElementsUShort( primitiveTypes )
                    )
                  : static_cast<osg::DrawElements*>(
                        new osg::DrawElementsUInt( primitiveTypes )
                    );

    elements->reserveElements(
        ( nx - 1 ) * ( ny - 1 ) * 6 + ( nx - 1 ) * 2 * 6 + ( ny - 1 ) * 2 * 6
    );
    ...
    if( locator )
    {
        ...
        osg::dvec3 center( 0.5, 0.5, 0.0 );

        osg::dvec3 bottom_left( 0.0, 0.0, 0.0 );
        osg::dvec3 bottom_right( 1.0, 0.0, 0.0 );
        osg::dvec3 top_left( 0.0, 1.0, 0.0 );
        ...
        if( em && locator->getCoordinateSystemType() == osgTerrain::Locator::GEOCENTRIC )
        {
            osg::dmat4 localToWorldTransform;
            // note y axis maps to latitude, x axis to longitude
            em->computeLocalToWorldTransformFromLatLongHeight( center.y,
                                                               center.x,
                                                               center.z,
                                                               localToWorldTransform );
            // OSG_NOTICE<<"We have a EllipsoidModel to take account of
            // "<<localToWorldTransform<<std::endl;

            // note y axis maps to latitude, x axis to longitude
            em->convertLatLongHeightToXYZ( center.y,
                                           center.x,
                                           center.z,
                                           center.x,
                                           center.y,
            ...
            em->convertLatLongHeightToXYZ( bottom_left.y,
                                           ...
                                           bottom_left.y,
                                           bottom_left.z );
            em->convertLatLongHeightToXYZ( bottom_right.y,
                                           bottom_right.x,
                                           bottom_right.z,
                                           bottom_right.x,
                                           bottom_right.y,
                                           bottom_right.z );
            em->convertLatLongHeightToXYZ( top_left.y,
                                           top_left.x,
                                           top_left.z,
                                           top_left.x,
                                           top_left.y,
            ...
            osg::dmat4 worldToLocalTransform;
            worldToLocalTransform = osg::inverse( localToWorldTransform );
            ...
            {
                const osg::dvec4& location = locationCoords[i];
                double            height   = ( *vertices )[i].z;
                osg::dvec3 pos = matrix * osg::dvec3( location.x, location.y, 0.0 );
                em->convertLatLongHeightToXYZ( pos.y,
                                               pos.x,
                                               height,
                                               pos.x,
                                               pos.y,
                ...
                osg::dvec3 pos_right =
                    matrix * osg::dvec3( location.x + location[2], location.y, 0.0 );
                em->convertLatLongHeightToXYZ( pos_right.y,
                                               pos_right.x,
                                               height,
                                               pos_right.x,
                                               pos_right.y,
                ...
                osg::dvec3 pos_up =
                    matrix * osg::dvec3( location.x, location.y + location[3], 0.0 );
                em->convertLatLongHeightToXYZ( pos_up.y,
                                               pos_up.x,
                                               height,
                                               pos_up.x,
                                               pos_up.y,
                ...
                double length_right = osg::length( pos_right - pos );
                double length_up    = osg::length( pos_up - pos );
                tc[2]               = 1.0 / length_right;
                tc[3]               = 1.0 / length_up;

                osg::dvec3 normal( pos );
                normal            = osg::transform3x3( localToWorldTransform, normal );
    ...
    // double tileWidth = osg::length(bottom_right-bottom_left);
...
GeometryPool::getTileSubgraph( osgTerrain::TerrainTile* tile )
{
    ...
    osgTerrain::HeightFieldLayer*     hfl =
        dynamic_cast<osgTerrain::HeightFieldLayer*>( tile->getElevationLayer() );
    ...
    if( locator )
    {
        osg::dmat4                 matrix = locator->getTransform();

        osg::dvec3                 center = matrix * osg::dvec3( 0.5, 0.5, 0.0 );
    ...
    osg::Vec3Array* shared_vertices =
        dynamic_cast<osg::Vec3Array*>( geometry->getVertexArray() );
    osg::Vec3Array* shared_normals =
        dynamic_cast<osg::Vec3Array*>( geometry->getNormalArray() );
    osg::FloatArray* heights = hf ? hf->getFloatArray() : 0;
    const SharedGeometry::VertexToHeightFieldMapping& vthfm =
        geometry->getVertexToHeightFieldMapping();

    if( hf &&
        shared_vertices &&
        shared_normals &&
        ( shared_vertices->size() == shared_normals->size() ) )
    {
        if( vthfm.size() == shared_vertices->size() )
        {
            ...
            for( unsigned int i = 0; i < numVertices; ++i )
            {
                ...
                ( *vertices )[i] =
                    ( *shared_vertices )[i] + ( *shared_normals )[i] * ( *heights )[hi];
            }
        ...
        else
        {
            ...
            // OSG_NOTICE<<"Assigning initial bound ("<<bb.xMin()<<", "<<bb.xMax()<<") ,
            // ("<<bb.yMin()<<", "<<bb.yMax()<<") ("<<bb.zMin()<<", "<<bb.zMax()<<")"<<
            // std::endl; bb = hfDrawable->getBoundingBox();
            // OSG_NOTICE<<"         getBoundingBox ("<<bb.xMin()<<", "<<bb.xMax()<<") ,
            // ("<<bb.yMin()<<", "<<bb.yMax()<<") ("<<bb.zMin()<<", "<<bb.zMax()<<")"<<
            // std::endl;
        }
    }
...
GeometryPool::getOrCreateProgram( LayerTypes& layerTypes )
{
    // std::lock_guard<std::mutex>  lock(_programMapMutex);
    ...
    for( LayerTypes::iterator itr = layerTypes.begin(); itr != layerTypes.end(); ++itr )
    {
        switch( *itr )
        {
            case( HEIGHTFIELD_LAYER ) :
                ++num_HeightField;
                break;
            case( COLOR_LAYER ) :
                ++num_Color;
...
#include "shaders/lighting_vert.cpp"
        program->addShader(
            osgDB::readRefShaderFileWithFallback( osg::Shader::VERTEX,
...
#include "shaders/terrain_displacement_mapping_vert.cpp"
        program->addShader( osgDB::readRefShaderFileWithFallback(
            osg::Shader::VERTEX,
            "shaders/terrain_displacement_mapping.vert",
            terrain_displacement_mapping_vert
        ) );
    }
...
#include "shaders/terrain_displacement_mapping_geom.cpp"
        program->addShader( osgDB::readRefShaderFileWithFallback(
            osg::Shader::GEOMETRY,
            "shaders/terrain_displacement_mapping.geom",
            terrain_displacement_mapping_geom
        ) );
...
#include "shaders/terrain_displacement_mapping_frag.cpp"
        program->addShader( osgDB::readRefShaderFileWithFallback(
            osg::Shader::FRAGMENT,
            "shaders/terrain_displacement_mapping.frag",
...
void
GeometryPool::applyLayers( osgTerrain::TerrainTile* tile,
                           osg::StateSet*           stateset )
{
    ...
    if( hfl )
    {
        osg::Texture2D* texture2D =
            dynamic_cast<osg::Texture2D*>( layerToTextureMap[hfl] );
        if( !texture2D )
        {
            ...
            const void*              dataPtr =
                hfl->getHeightField()->getFloatArray()->getDataPointer();
            ...
            texture2D->setBorderColor( osg::dvec4( 0.0, 0.0, 0.0, 0.0 ) );
    ...
    for( unsigned int layerNum = 0; layerNum < tile->getNumColorLayers(); ++layerNum )
    {
        ...
        {
            OSG_NOTICE
                << "Warning : GeometryPool does not presently support ContourLayers."
                << std::endl;
        }
    }
#endif
    ...
    //    create new StateSet for Program required and assigned with layers required
    //    enabled
    //
    ...
    // stateset->setDefine("GL_LIGHTING", osg::StateAttribute::ON);
    {
        ...
        if( !_rootStateSetAssigned )
        {
            ...
            int num_Color = 0;
            for( LayerTypes::iterator itr = layerTypes.begin(); itr != layerTypes.end();
                 ++itr )
            {
                switch( *itr )
                {
                    case( HEIGHTFIELD_LAYER ) :
                        _rootStateSet->setDefine( "HEIGHTFIELD_LAYER" );
                        break;
                    case( COLOR_LAYER ) :
                        ++num_Color;
                        break;
...
osg::StateSet*
GeometryPool::getRootStateSetForTerrain( Terrain* /*terrain*/ )
{
...
SharedGeometry::SharedGeometry( const SharedGeometry& rhs,
...
SharedGeometry::createVertexArrayStateImplementation( osg::RenderInfo& renderInfo ) const
{
...
SharedGeometry::compileGLObjects( osg::RenderInfo& renderInfo ) const
{
    ...
    if( _vertexArray->getVertexBufferObject() )
    {
        ...
        osg::GLBufferObject* vbo_glBufferObject =
            vbo->getOrCreateGLBufferObject( contextID );
        ...
        osg::GLBufferObject* ebo_glBufferObject =
            ebo->getOrCreateGLBufferObject( contextID );
        ...
        if( state.useVertexArrayObject( _useVertexArrayObject ) )
        {
            ...
            if( vbo_glBufferObject )
            {
                vas->bindVertexBufferObject( vbo_glBufferObject );
            }
            if( ebo_glBufferObject )
            {
                vas->bindElementBufferObject( ebo_glBufferObject );
            }
...
void
SharedGeometry::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
SharedGeometry::releaseGLObjects( osg::State* state ) const
{
...
void
SharedGeometry::drawImplementation( osg::RenderInfo& renderInfo ) const
{
    bool computeDiagonals =
        renderInfo.getState()->supportsShaderRequirement( "COMPUTE_DIAGONALS" );
    ...
    {
        // OSG_NOTICE<<"   sending vertex arrays
        // vas->getRequiresSetArrays()="<<vas->getRequiresSetArrays()<<std::endl;
        ...
        if( _texcoordArray.valid() &&
            _texcoordArray->getBinding() == osg::Array::BIND_PER_VERTEX )
        {
    ...
    //
    GLenum primitiveType = computeDiagonals ? GL_LINES_ADJACENCY : GL_TRIANGLES;

    bool   request_bind_unbind =
        !state.useVertexArrayObject( _useVertexArrayObject ) ||
        state.getCurrentVertexArrayState()->getRequiresSetArrays();

    osg::GLBufferObject* ebo =
        _drawElements->getOrCreateGLBufferObject( state.getContextID() );
...
void
SharedGeometry::accept( osg::Drawable::AttributeFunctor& af )
{
...
void
SharedGeometry::accept( osg::Drawable::ConstAttributeFunctor& af ) const
{
...
void
SharedGeometry::accept( osg::PrimitiveFunctor& pf ) const
{
    pf.setVertexArray( _vertexArray->getNumElements(),
                       static_cast<const osg::vec3*>( _vertexArray->getDataPointer() ) );

    _drawElements->accept( pf );
}

void
SharedGeometry::accept( osg::PrimitiveIndexFunctor& pif ) const
{
    pif.setVertexArray(
        _vertexArray->getNumElements(),
        static_cast<const osg::vec3*>( _vertexArray->getDataPointer() )
    );
...
HeightFieldDrawable::HeightFieldDrawable( const HeightFieldDrawable& rhs,
                                          const osg::CopyOp&         copyop ) :
    Inherit( rhs,
             copyop ),
...
void
HeightFieldDrawable::drawImplementation( osg::RenderInfo& renderInfo ) const
{
    if( _geometry.valid() )
    {
...
void
HeightFieldDrawable::compileGLObjects( osg::RenderInfo& renderInfo ) const
{
    if( _geometry.valid() )
    {
...
void
HeightFieldDrawable::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _geometry.valid() )
    {
...
void
HeightFieldDrawable::releaseGLObjects( osg::State* state ) const
{
    if( _geometry.valid() )
    {
...
void
HeightFieldDrawable::accept( osg::Drawable::AttributeFunctor& af )
{
    if( _geometry )
    {
        _geometry->accept( af );
...
void
HeightFieldDrawable::accept( osg::Drawable::ConstAttributeFunctor& caf ) const
{
    if( _geometry )
    {
...
void
HeightFieldDrawable::accept( osg::PrimitiveFunctor& pf ) const
{
    ...
    if( _vertices.valid() )
    {
        ...
        const osg::DrawElementsUShort* deus =
            dynamic_cast<const osg::DrawElementsUShort*>( _geometry->getDrawElements() );
        if( deus )
        {
            pf.drawElements( GL_TRIANGLES, deus->size(), &( ( *deus )[0] ) );
        }
        else
        {
            const osg::DrawElementsUInt* deui =
                dynamic_cast<const osg::DrawElementsUInt*>(
                    _geometry->getDrawElements()
                );
            if( deui )
            {
                pf.drawElements( GL_TRIANGLES, deui->size(), &( ( *deui )[0] ) );
            }
        }
    }
    else
...
void
HeightFieldDrawable::accept( osg::PrimitiveIndexFunctor& pif ) const
{
    if( _vertices.valid() )
    {
        ...
        if( deus )
        {
            pif.drawElements( GL_TRIANGLES, deus->size(), &( ( *deus )[0] ) );
        }
        else
        {
            const osg::DrawElementsUInt* deui =
            ...
            if( deui )
            {
                pif.drawElements( GL_TRIANGLES, deui->size(), &( ( *deui )[0] ) );
            }
        }
    }
    else
