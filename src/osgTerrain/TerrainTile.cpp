...
//
void
TerrainTile::setTileLoadedCallback( TerrainTile::TileLoadedCallback* lc )
{
    getTileLoadedCallback() = lc;
}

osg::ref_ptr<TerrainTile::TileLoadedCallback>&
TerrainTile::getTileLoadedCallback()
{
...
TerrainTile::TerrainTile( const TerrainTile& terrain,
    ...
    _treatBoundariesToValidDataAsDefaultValue(
        terrain._treatBoundariesToValidDataAsDefaultValue
    ),
    _blendingPolicy( terrain._blendingPolicy )
{
    if( terrain.getTerrainTechnique() )
    {
        setTerrainTechnique( dynamic_cast<TerrainTechnique*>(
...
void
TerrainTile::setTerrain( Terrain* ts )
{
    if( _terrain == ts )
    {
        return;
...
void
TerrainTile::setTileID( const TileID& tileID )
{
    if( _tileID == tileID )
    {
        return;
...
void
TerrainTile::traverse( osg::NodeVisitor& nv )
{
    ...
    {
        osg::ClusterCullingCallback* ccc =
            dynamic_cast<osg::ClusterCullingCallback*>( getCullCallback() );
        if( ccc )
        {
...
void
TerrainTile::init( int  dirtyMask,
    ...
    if( !_terrainTechnique )
    {
        if( _terrain && _terrain->getTerrainTechniquePrototype() )
        {
            osg::ref_ptr<osg::Object> object =
                _terrain->getTerrainTechniquePrototype()->clone(
...
void
TerrainTile::setTerrainTechnique( TerrainTechnique* terrainTechnique )
{
...
void
TerrainTile::setDirtyMask( int dirtyMask )
{
    ...
    // setNumChildrenRequeingUpdateTraversal() isn't thread safe so should avoid using
    // it.
    if( dirtyDelta > 0 )
    {
...
void
TerrainTile::setElevationLayer( Layer* layer )
{
    _elevationLayer = layer;
}

void
TerrainTile::setColorLayer( unsigned int i,
                            Layer*       layer )
{
...
osg::sphere
TerrainTile::computeBound() const
{
    osg::sphere bs;
...
bool
WhiteListTileLoadedCallback::layerAcceptable( const std::string& setname ) const
{
    if( _allowAll )
    {
        return true;
...
bool
WhiteListTileLoadedCallback::readImageLayer(
    osgTerrain::ImageLayer*             imageLayer,
    const osgDB::ReaderWriter::Options* options
) const
{
    if( !imageLayer->getImage() && !imageLayer->getFileName().empty() )
    {
        if( layerAcceptable( imageLayer->getSetName() ) )
        {
            osg::ref_ptr<osg::Image> image =
                osgDB::readRefImageFile( imageLayer->getFileName(), options );
...
bool
WhiteListTileLoadedCallback::deferExternalLayerLoading() const
{
    return true;
}

void
WhiteListTileLoadedCallback::loaded( osgTerrain::TerrainTile*            tile,
                                     const osgDB::ReaderWriter::Options* options ) const
{
    ...
    for( unsigned int i = 0; i < tile->getNumColorLayers(); ++i )
    {
        ...
        osgTerrain::ImageLayer* color_imageLayer =
            dynamic_cast<osgTerrain::ImageLayer*>( layer );
        if( color_imageLayer )
        {
        ...
        osgTerrain::SwitchLayer* switchLayer =
            dynamic_cast<osgTerrain::SwitchLayer*>( layer );
        if( switchLayer )
        {
            for( unsigned int si = 0; si < switchLayer->getNumLayers(); ++si )
            {
                osgTerrain::ImageLayer* imageLayer =
                    dynamic_cast<osgTerrain::ImageLayer*>( switchLayer->getLayer( si ) );
                if( imageLayer )
                {
        ...
        osgTerrain::CompositeLayer* compositeLayer =
            dynamic_cast<osgTerrain::CompositeLayer*>( layer );
        if( compositeLayer )
        {
            for( unsigned int ci = 0; ci < compositeLayer->getNumLayers(); ++ci )
            {
                osgTerrain::ImageLayer* imageLayer =
                    dynamic_cast<osgTerrain::ImageLayer*>(
                        compositeLayer->getLayer( ci )
                    );
                if( imageLayer )
                {
    ...
    for( unsigned int i = 0; i < tile->getNumColorLayers(); ++i )
    {
        ...
        if( compositeLayer )
        {
            ...
            {
                osgTerrain::ImageLayer* imageLayer =
                    dynamic_cast<osgTerrain::ImageLayer*>( switchLayer->getLayer( ci ) );
                if( imageLayer && imageLayer->getImage() != 0 )
                {
                    validLayer = imageLayer;
                }
    ...
    if( validLayer )
    {
        ...
        {
            for( unsigned int i = tile->getNumColorLayers(); i < _minumumNumberOfLayers;
                 ++i )
            {
                tile->setColorLayer( i, validLayer );
            }
...
void
TerrainTile::releaseGLObjects( osg::State* state ) const
{
