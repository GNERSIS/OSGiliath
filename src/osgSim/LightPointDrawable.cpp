 ...
 * Utility functions for osgSim: color, position, drawLightPoints.
 */
    ...
    _simulationTimeInterval( 0.0 )
{
    ...
    _blendOneMinusSrcAlpha->setFunction( osg::BlendFunc::SRC_ALPHA,
...
LightPointDrawable::LightPointDrawable( const LightPointDrawable& lpd,
...
void
LightPointDrawable::drawImplementation( osg::RenderInfo& renderInfo ) const
{
    ...
    auto drawLightPoints = [&]( const SizedLightPointList& list )
    {
        ...
        for( auto sitr = list.begin(); sitr != list.end(); ++sitr, ++pointsize )
        {
            ...
            if( !lpl.empty() )
            {
                ...
                // Each element: 4 bytes color (GLubyte[4]) + 12 bytes position
                // (GLfloat[3]) = 16 bytes stride
...
osg::box
LightPointDrawable::computeBoundingBox() const
{
    osg::box                            bbox;
    ...
    {
        const LightPointList& lpl = *sitr;
        for( LightPointList::const_iterator litr = lpl.begin(); litr != lpl.end();
             ++litr )
        {
