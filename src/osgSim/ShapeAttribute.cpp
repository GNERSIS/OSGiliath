...
namespace osgSim
{
    ...
    void
    ShapeAttribute::setValue( const char* value )
    {
        free();
        _type   = STRING;
        _string = ( value ? strdup( value ) : 0 );
    ...
    void
    ShapeAttribute::copy( const ShapeAttribute& sa )
    {
        _name = sa._name;
        _type = sa._type;
    ...
    int
    ShapeAttribute::compare( const osgSim::ShapeAttribute& sa ) const
    {
        if( _name < sa._name )
        {
            return -1;
    ...
    int
    ShapeAttributeList::compare( const osgSim::ShapeAttributeList& sal ) const
    {
        const_iterator salIt, thisIt, thisEnd = end();

        int            ret;
        for( thisIt = begin(), salIt = sal.begin(); thisIt != thisEnd;
             ++thisIt, ++salIt )
        {
