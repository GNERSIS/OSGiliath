...
MultiSwitch::MultiSwitch( const MultiSwitch& sw,
...
void
MultiSwitch::traverse( osg::NodeVisitor& nv )
{
...
bool
MultiSwitch::addChild( osg::Node* child )
{
    unsigned int childPosition = static_cast<unsigned int>( _children.size() );
    if( Group::addChild( child ) )
    {
        for( SwitchSetList::iterator itr = _values.begin(); itr != _values.end(); ++itr )
        {
...
bool
MultiSwitch::insertChild( unsigned int index,
                          osg::Node*   child )
{
...
bool
MultiSwitch::removeChild( osg::Node* child )
{
...
void
MultiSwitch::setValue( unsigned int switchSet,
                       unsigned int pos,
                       bool         value )
{
...
void
MultiSwitch::setChildValue( const osg::Node* child,
...
bool
MultiSwitch::getValue( unsigned int switchSet,
...
bool
MultiSwitch::getChildValue( const osg::Node* child,
                            unsigned int     switchSet ) const
{
    if( switchSet >= _values.size() )
    {
        return false;
...
void
MultiSwitch::expandToEncompassSwitchSet( unsigned int switchSet )
{
    if( switchSet >= _values.size() )
    {
        // need to expand arrays.
        unsigned int originalSize = static_cast<unsigned int>( _values.size() );
...
bool
MultiSwitch::setAllChildrenOff( unsigned int switchSet )
{
    ...
    ValueList& values = _values[switchSet];
    for( ValueList::iterator itr = values.begin(); itr != values.end(); ++itr )
    {
        *itr = false;
    }
    return true;
...
bool
MultiSwitch::setAllChildrenOn( unsigned int switchSet )
{
    _newChildDefaultValue = true;
...
bool
MultiSwitch::setSingleChildOn( unsigned int switchSet,
                               unsigned int pos )
{
    expandToEncompassSwitchSet( switchSet );
...
void
MultiSwitch::setSwitchSetList( const SwitchSetList& switchSetList )
{
    expandToEncompassSwitchSet( static_cast<unsigned int>( switchSetList.size() ) );

    _values = switchSetList;
}

void
MultiSwitch::setValueList( unsigned int     switchSet,
...
void
MultiSwitch::setValueName( unsigned int       switchSet,
