...
SphereSegment::SphereSegment( const SphereSegment& rhs,
                              const osg::CopyOp&   co ) :
    Inherit( rhs,
             co ),
    _centre( rhs._centre ),
    ...
    _azMin( rhs._azMin ),
    _azMax( rhs._azMax ),
    _elevMin( rhs._elevMin ),
    _elevMax( rhs._elevMax ),
...
void
SphereSegment::traverse( osg::NodeVisitor& nv )
{
...
void
SphereSegment::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _surfaceGeometry.valid() )
    {
        _surfaceGeometry->resizeGLObjectBuffers( maxSize );
    }
    if( _spokesGeometry.valid() )
    {
        _spokesGeometry->resizeGLObjectBuffers( maxSize );
    }
    if( _edgeLineGeometry.valid() )
    {
        _edgeLineGeometry->resizeGLObjectBuffers( maxSize );
    }
    if( _sidesGeometry.valid() )
    {
        _sidesGeometry->resizeGLObjectBuffers( maxSize );
    }

    if( _litOpaqueState.valid() )
    {
        _litOpaqueState->resizeGLObjectBuffers( maxSize );
    }
    if( _unlitOpaqueState.valid() )
    {
        _unlitOpaqueState->resizeGLObjectBuffers( maxSize );
    }
    if( _litTransparentState.valid() )
    {
        _litTransparentState->resizeGLObjectBuffers( maxSize );
    }
    if( _unlitTransparentState.valid() )
    {
        _unlitTransparentState->resizeGLObjectBuffers( maxSize );
    }
}

void
SphereSegment::releaseGLObjects( osg::State* state ) const
{
    if( _surfaceGeometry.valid() )
    {
        _surfaceGeometry->releaseGLObjects( state );
    }
    if( _spokesGeometry.valid() )
    {
        _spokesGeometry->releaseGLObjects( state );
    }
    if( _edgeLineGeometry.valid() )
    {
        _edgeLineGeometry->releaseGLObjects( state );
    }
    if( _sidesGeometry.valid() )
    {
        _sidesGeometry->releaseGLObjects( state );
    }

    if( _litOpaqueState.valid() )
    {
        _litOpaqueState->releaseGLObjects( state );
    }
    if( _unlitOpaqueState.valid() )
    {
        _unlitOpaqueState->releaseGLObjects( state );
    }
    if( _litTransparentState.valid() )
    {
        _litTransparentState->releaseGLObjects( state );
    }
    if( _unlitTransparentState.valid() )
    {
        _unlitTransparentState->releaseGLObjects( state );
    }
}

osg::sphere
SphereSegment::computeBound() const
{
    _bbox.init();
    ...
    return osg::sphere( _centre, _radius );
}

void
SphereSegment::setCentre( const osg::vec3& c )
{
    _centre = c;
...
const osg::vec3&
SphereSegment::getCentre() const
{
    return _centre;
}

void
SphereSegment::setRadius( float r )
{
    _radius = r;
...
float
SphereSegment::getRadius() const
{
    return _radius;
}

void
SphereSegment::setArea( const osg::vec3& v,
                        float            azRange,
                        float            elevRange )
{
    osg::vec3 vec( v );

    vec = osg::normalize( vec );    // Make sure we're unit length

    // Calculate the elevation range
    float xyLen  = sqrtf( vec.x * vec.x + vec.y * vec.y );
    float elev   = atan2f( vec.z, xyLen );    // Elevation angle
...
void
SphereSegment::getArea( osg::vec3& vec,
                        float&     azRange,
                        float&     elevRange ) const
{
    ...
    vec.set( cosf( elev ) * sinf( az ), cosf( elev ) * cosf( az ), sinf( elev ) );
}

void
SphereSegment::setArea( float azMin,
                        float azMax,
                        float elevMin,
                        float elevMax )
{
...
void
SphereSegment::getArea( float& azMin,
                        float& azMax,
                        float& elevMin,
                        float& elevMax ) const
{
...
void
SphereSegment::setDensity( int density )
{
    _density = density;
...
int
SphereSegment::getDensity() const
{
    return _density;
}
...
SphereSegment::init()
{
    ...
    _litTransparentState->setAttributeAndModes(
    ...
    unsigned int numVertices =
        static_cast<unsigned int>( ( _density + 1 ) * ( _density + 1 ) + 1 );
...
void
SphereSegment::setDrawMask( int dm )
{
    _drawMask = dm;
}

void
SphereSegment::setSurfaceColor( const osg::vec4& c )
{
    ( *_surfaceColor )[0] = c;
...
void
SphereSegment::setSpokeColor( const osg::vec4& c )
{
    ( *_spokeColor )[0] = c;
...
void
SphereSegment::setEdgeLineColor( const osg::vec4& c )
{
    ( *_edgeLineColor )[0] = c;
...
void
SphereSegment::setSideColor( const osg::vec4& c )
{
    ( *_sideColor )[0] = c;
...
void
SphereSegment::setAllColors( const osg::vec4& c )
{
    setSurfaceColor( c );
    setSpokeColor( c );
    setEdgeLineColor( c );
...
class PolytopeVisitor : public osg::DualModeVisitor
{
    public:

        typedef std::pair<osg::dmat4, osg::Polytope> MatrixPolytopePair;
        typedef std::vector<MatrixPolytopePair>      PolytopeStack;

        struct Hit
        {
                Hit( const osg::dmat4& matrix,
        ...
        PolytopeVisitor( const osg::dmat4&    matrix,
            ...
            osg::DualModeVisitor( osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN )
        {
            ...
            _polytopeStack.back().second.setAndTransformProvidingInverse(
                polytope,
                _polytopeStack.back().first
            );
        }
        ...
        apply( osg::Transform& transform )
        {
            ...
            {
                osg::dmat4 matrix = _polytopeStack.back().first;
                transform.computeLocalToWorldMatrix( matrix, this );

                _polytopeStack.push_back( MatrixPolytopePair() );
                _polytopeStack.back().first = matrix;
                _polytopeStack.back().second.setAndTransformProvidingInverse(
                    _polytopeStack.front().second,
                    matrix
                );
        ...
        apply( osg::Drawable& drawable )
        {
            ...
            {
                _hits.push_back(
                    Hit( _polytopeStack.back().first, getNodePath(), &drawable )
                );
            }
        }
...
SphereSegment::LineList
SphereSegment::computeIntersection( const osg::dmat4& transform,
                                    osg::Node*        subgraph )
{
    OSG_INFO << "Creating line intersection between sphere segment and subgraph."
             << std::endl;

    osg::box      bb = getBoundingBox();
    ...
    for( PolytopeVisitor::HitList::iterator itr = hits.begin(); itr != hits.end();
         ++itr )
    {
        SphereSegment::LineList lines =
            computeIntersection( itr->_matrix, itr->_drawable.get() );
...
osg::Node*
SphereSegment::computeIntersectionSubgraph( const osg::dmat4& transform,
                                            osg::Node*        subgraph )
{
    OSG_INFO << "Creating line intersection between sphere segment and subgraph."
             << std::endl;
    ...
    for( PolytopeVisitor::HitList::iterator itr = hits.begin(); itr != hits.end();
         ++itr )
    {
        group->addChild( computeIntersectionSubgraph( itr->_matrix,
                                                      itr->_drawable.get() ) );
    }
...
namespace SphereSegmentIntersector
{
    ...
    struct SortFunctor
    {
            ...
            SortFunctor( const SortFunctor& ) = default;
    ...
    struct TriangleIntersectOperator
    {
            ...
            struct Edge : public osg::Referenced
            {
                    ...
                    Edge( unsigned int p1,
                          unsigned int p2,
                          SurfaceType  intersectionEdge = NO_SURFACE ) :
                    ...
                    addTriangle( Triangle* tri )
                    {
                        TriangleList::iterator itr =
                            std::find( _triangles.begin(), _triangles.end(), tri );
                    ...
                    void
                    removeFromToTraverseList( Triangle* tri )
                    {
                        TriangleList::iterator itr =
                            std::find( _toTraverse.begin(), _toTraverse.end(), tri );
            ...
            struct Triangle : public osg::Referenced
            {
                    ...
                    oppositeActiveEdge( Edge* edge )
                    {
                        ...
                        if( edge !=
                            _e1 &&
                            _e1 &&
                            _e1->_intersectionType != Edge::NO_INTERSECTION )
                        {
                            return _e1;
                        }
                        if( edge !=
                            _e2 &&
                            _e2 &&
                            _e2->_intersectionType != Edge::NO_INTERSECTION )
                        {
                            return _e2;
                        }
                        if( edge !=
                            _e3 &&
                            _e3 &&
                            _e3->_intersectionType != Edge::NO_INTERSECTION )
                        {
                            return _e3;
                        }
                        return 0;
            ...
            struct Region
            {
                    ...
                    void
                    classify( const osg::vec3& vertex,
                              double           radius2,
                              double           azimMin,
                              double           azimMax,
                              double           elevMin,
                              double           elevMax )
                    {
                        ...
                        double rad2       = osg::length2( vertex );
                        double length_xy =
                            sqrtf( vertex.x * vertex.x + vertex.y * vertex.y );
                        double elevation = atan2f( ( double )vertex.z, length_xy );
                        ...
                        double dot_alphaMin =
                            cos( azimMin ) * vertex.x - sin( azimMin ) * vertex.y;
                        if( dot_alphaMin < 0.0 )
                        {
                            _leftSurface = OUTSIDE;
                        }
                        ...
                        double dot_alphaMax =
                            cos( azimMax ) * vertex.x - sin( azimMax ) * vertex.y;
                        if( dot_alphaMax > 0.0 )
                        {
                            _rightSurface = OUTSIDE;
                        }
            ...
            struct RegionCounter
            {
                    ...
                    overallClassification() const
                    {
                        // if all vertices are outside any of the surfaces then we are
                        // completely outside
                        if( _outside_radiusSurface ==
                            _numVertices ||
                            _outside_leftRightSurfaces ==
                            _numVertices ||
                            _outside_topSurface ==
                        ...
                        // if all the vertices on all the sides and inside then we are
                        // completely inside
                        if( _inside_radiusSurface ==
                            _numVertices &&
                            _inside_leftRightSurfaces ==
                            _numVertices &&
                            _inside_topSurface ==
                    ...
                    bool
                    intersecting( SurfaceType surfaceType ) const
                    {
                        // if all vertices are outside any of the surfaces then we are
                        // completely outside
                        if( ( surfaceType !=
                    ...
                    int
                    numberOfIntersectingSurfaces() const
                    {
                        int sidesThatIntersect = 0;
                        if( _outside_radiusSurface !=
                            _numVertices &&
                            _inside_radiusSurface != _numVertices )
                        {
                            ++sidesThatIntersect;
                        }
                        if( _outside_leftSurface !=
                            _numVertices &&
                            _inside_leftSurface != _numVertices )
                        {
                            ++sidesThatIntersect;
                        }
                        if( _outside_rightSurface !=
                            _numVertices &&
                            _inside_rightSurface != _numVertices )
                        {
                            ++sidesThatIntersect;
                        }
                        if( _outside_topSurface !=
                            _numVertices &&
                            _inside_topSurface != _numVertices )
                        {
                            ++sidesThatIntersect;
                        }
                        if( _outside_bottomSurface !=
                            _numVertices &&
                            _inside_bottomSurface != _numVertices )
                        {
                            ++sidesThatIntersect;
            ...
            void
            computePositionAndRegions( const osg::dmat4& matrix,
                                       osg::Vec3Array&   array )
            {
                ...
                {
                    osg::vec3 vertex =
                        osg::vec3( matrix * osg::dvec3( array[i] ) ) - _centre;
                    _originalVertices[i] = vertex;
                    _regions[i]
                        .classify( vertex, radius2, _azMin, _azMax, _elevMin, _elevMax );
                }
            }
            ...
            {
                OSG_INFO << "Removing duplicates : num vertices in "
                         << _candidateVertexIndices.size() << std::endl;
                ...
                std::sort( _candidateVertexIndices.begin(),
                           _candidateVertexIndices.end(),
                           SortFunctor( _originalVertices ) );
                ...
                for( ; itr != _candidateVertexIndices.end(); ++itr )
                {
                    // unsigned int i = *itr;
                    //  OSG_INFO<<"  i="<<i<<"
                    //  lastUniqueIndex="<<lastUniqueIndex<<std::endl;
                    if( _originalVertices[*itr] == _originalVertices[lastUniqueIndex] )
                    {
                        OSG_INFO << "Combining vertex " << *itr << " with "
                                 << lastUniqueIndex << std::endl;
            ...
            {
                OSG_INFO << "Removing duplicate triangles : num triangles in "
                         << _triangles.size() << std::endl;
                ...
                {
                    _triangles.erase( _triangles.begin() + lastUniqueTriangle + 1,
                                      _triangles.end() );
                }

                OSG_INFO << "Removed duplicate triangles : num duplicates found "
                         << numDuplicates << std::endl;
                OSG_INFO << "Removed duplicate triangles : num triangles out "
                         << _triangles.size() << std::endl;
            }

            void
            buildEdges( Triangle* tri )
            {
            ...
            buildEdges()
            {
                ...
                OSG_INFO << "Number of edges " << _edges.size() << std::endl;
                for( EdgeSet::iterator eitr = _edges.begin(); eitr != _edges.end();
                     ++eitr )
                {
                    const Edge*  edge = eitr->get();
                    unsigned int numConnections =
                        static_cast<unsigned int>( edge->_triangles.size() );
                    if( numConnections == 0 )
                    {
                        ++numZeroConnections;
                    }
                ...
                OSG_INFO << "Number of numZeroConnections " << numZeroConnections
                         << std::endl;
                OSG_INFO << "Number of numSingleConnections " << numSingleConnections
                         << std::endl;
                OSG_INFO << "Number of numDoubleConnections " << numDoubleConnections
                         << std::endl;
                OSG_INFO << "Number of numMultiConnections " << numMultiConnections
                         << std::endl;
            }

            Edge*
            addEdge( unsigned int p1,
                     unsigned int p2,
                     Triangle*    tri )
            {
            ...
            SphereSegment::LineList
            connectIntersections( EdgeList& hitEdges )
            {
                SphereSegment::LineList lineList;

                OSG_INFO << "Number of edge intersections " << hitEdges.size()
                         << std::endl;
                ...
                // but should only contain triangles that actually hit the intersection
                // surface
                EdgeList::iterator hitr;
                for( hitr = hitEdges.begin(); hitr != hitEdges.end(); ++hitr )
                {
                    Edge* edge = hitr->get();
                         ...
                         ++titr )
                    {
                        ...
                        ( void )numEdges;
                        if( tri->_e1 &&
                            tri->_e1->_intersectionType != Edge::NO_INTERSECTION )
                        {
                            ++numActiveEdges;
                        }
                        if( tri->_e2 &&
                            tri->_e2->_intersectionType != Edge::NO_INTERSECTION )
                        {
                            ++numActiveEdges;
                        }
                        if( tri->_e3 &&
                            tri->_e3->_intersectionType != Edge::NO_INTERSECTION )
                        {
                            ++numActiveEdges;
                        }
                        ...
                        // if we have one or more then add it into the edges to traverse
                        // list
                        if( numActiveEdges > 1 )
                        {
                        ...
                        // OSG_INFO<<"Number active edges "<<numActiveEdges<<" num
                        // original edges "<<numEdges<<std::endl;
                    }
                }
                ...
                while( !hitEdges.empty() )
                {
                    ...
                    while( edge )
                    {
                        ...
                        newLine->push_back( edge->_intersectionVertex +
                                            _centre /*+osg::vec3(0.0f,0.0f,200.0f)*/ );

                        Edge*     newEdge = 0;

                        Triangle* tri     = !( edge->_toTraverse.empty() )
                                              ? edge->_toTraverse.back()
                                              : 0;
                        if( tri )
                        {
                            ...
                            // OSG_INFO<<"   tri="<<tri<<" edge="<<edge<<"
                            // newEdge="<<newEdge<<std::endl;

                            if( edge == newEdge )
                            {
                                OSG_INFO << "   edge returned to itself problem "
                                         << std::endl;
                            }
                        }
                        else
            ...
            template<class I>
            SphereSegment::LineList
            computeIntersections( I intersector )
            {
                // collect all the intersecting edges
                EdgeList hitEdges;
                for( EdgeSet::iterator itr = _edges.begin(); itr != _edges.end(); ++itr )
                {
                    Edge* edge = const_cast<Edge*>( itr->get() );
                    if( intersector( edge ) )
                    {
            ...
            template<class I>
            void
            trim( SphereSegment::LineList& lineList,
                  osg::Vec3Array*          sourceLine,
                  I                        intersector )
            {
                if( sourceLine->empty() )
                {
                ...
                while( first < sourceLine->size() )
                {
                    ...
                    {
                        if( intersector.distance( ( *sourceLine )[first] - _centre ) >=
                            0.0 )
                        {
                            break;
                        }
                    ...
                    {
                        if( intersector.distance( ( *sourceLine )[last] - _centre ) <
                            0.0 )
                        {
                            break;
                        }
                    ...
                    {
                        // OSG_INFO<<"Copying partial line line"<<first<<"
                        // "<<last<<std::endl;
                        ...
                        if( first > 0 )
                        {
                            newLine->push_back( intersector.intersectionPoint(
                                                    ( *sourceLine )[first - 1] - _centre,
                                                    ( *sourceLine )[first] - _centre
                                                ) +
                                                _centre );
                        }
                        ...
                        {
                            newLine->push_back( intersector.intersectionPoint(
                                                    ( *sourceLine )[last - 1] - _centre,
                                                    ( *sourceLine )[last] - _centre
                                                ) +
                                                _centre );
                        }
            ...
            // handle a paired of surfaces that work to enclose a convex region, which
            // means that points can be inside either surface to be valid, and be outside
            // both surfaces to be invalid.
            template<class I>
            void
            trim( SphereSegment::LineList& lineList,
                  osg::Vec3Array*          sourceLine,
                  ...
                  I                        intersector2 )
            {
                ...
                while( first < sourceLine->size() )
                {
                    ...
                    for( ; first < sourceLine->size(); ++first )
                    {
                        ...
                        if( intersector1.distance( v ) >=
                            0.0 ||
                            intersector2.distance( v ) >= 0.0 )
                        {
                            break;
                        }
                    }
                    ...
                    {
                        OSG_INFO << "Copying partial line line" << first << " " << last
                                 << std::endl;
                        ...
                        if( first > 0 )
                        {
                            ...
                            // work out which intersector to use by discounting the one
                            // that isn't a plausible candidate.
                            ...
                            if( possible1 && possible2 )
                            {
                                ...
                                if( r1 < r2 )
                                {
                                    OSG_INFO << "start point, 1 near to end than 2" << r1
                                             << " " << r2 << std::endl;
                                    possible1 = true;
                                    possible2 = false;
                                }
                                ...
                                {
                                    OSG_INFO << "start point, 2 near to end than 1"
                                             << std::endl;
                                    possible1 = false;
                                    possible2 = true;
                                }
                            ...
                            {
                                newLine->push_back(
                                    intersector1.intersectionPoint( start, end ) +
                                    _centre
                                );
                            }
                            else
                            {
                                newLine->push_back(
                                    intersector2.intersectionPoint( start, end ) +
                                    _centre
                                );
                            }
                        }
                        ...
                        if( last < sourceLine->size() )
                        {
                            ...
                            if( possible1 && possible2 )
                            {
                                ...
                                if( possible1 && possible2 )
                                {
                                    ...
                                    double r1 =
                                        fabs( delta1 ) > 0.0 ? start1 / delta1 : 0.0;
                                    double r2 =
                                        fabs( delta2 ) > 0.0 ? start2 / delta2 : 0.0;

                                    // choose intersection which is nearest the end
                                    // point.
                                    if( r1 > r2 )
                                    {
                                        OSG_INFO << "end point, 1 near to end than 2"
                                                 << r1 << " " << r2 << std::endl;
                                        possible1 = true;
                                        possible2 = false;
                                    }
                                    ...
                                    {
                                        OSG_INFO << "end point, 2 near to end than 1"
                                                 << std::endl;
                                        possible1 = false;
                                        possible2 = true;
                                    }
            ...
            struct LinePair
            {
                    ...
                    bool
                    operator<( const LinePair& linePair ) const
                    {
                        return _distance < linePair._distance;
                    }

                    void
                    consider( osg::Vec3Array* testline )
                    {
                        if( _neighbourLine.valid() )
                        {
                            float distance =
                                osg::length( ( *_line )[0] - ( *testline )[0] );
                            ...
                            distance =
                                osg::length( ( *_line )[0] -
                                             ( *testline )[testline->size() - 1] );
                            if( distance < _distance )
                            {
                                _lineEnd       = 0;
                                _neighbourLine = testline;
                                _neighbourLineEnd =
                                    static_cast<unsigned int>( testline->size() - 1 );
                                _distance = distance;
                            }

                            distance = osg::length( ( *_line )[_line->size() - 1] -
                                                    ( *testline )[0] );
                            if( distance < _distance )
                            {
                                _lineEnd =
                                    static_cast<unsigned int>( _line->size() - 1 );
                                _neighbourLine    = testline;
                                _neighbourLineEnd = 0;
                                _distance         = distance;
                            }
                        ...
                        else
                        {
                            ...
                            if( _neighbourLine == _line )
                            {
                                ...
                                _distance         = osg::length(
                                    ( *_line )[_lineEnd] -
                                    ( *_neighbourLine )[_neighbourLineEnd]
                                );
                            }
                            else
                            {
                                _distance = osg::length( ( *_line )[0] -
                                                         ( *_neighbourLine )[0] );
                                _lineEnd  = 0;
                                _neighbourLineEnd = 0;

                                float distance    = osg::length(
                                    ( *_line )[0] -
                                    ( *_neighbourLine )[_neighbourLine->size() - 1]
                                );
                                if( distance < _distance )
                                {
                                    _lineEnd          = 0;
                    ...
                    bool
                    contains( osg::Vec3Array* line ) const
                    {
            ...
            void
            joinEnds( float fuseDistance,
                      bool  doFuse,
                      bool  allowJoinToSelf )
            {
                ...
                while( unfusedLines.size() >= 1 )
                {
                    ...
                    for( LinePairSet::iterator itr = linePairs.begin();
                         ...
                         ++itr )
                    {
                        OSG_INFO << "Line " << itr->_line.get() << " " << itr->_lineEnd
                                 << "  neighbour " << itr->_neighbourLine.get() << " "
                                 << itr->_neighbourLineEnd
                                 << " distance=" << itr->_distance << std::endl;
                    }

                    LinePair linePair = *linePairs.begin();
                    if( linePair._distance > fuseDistance )
                    {
                        OSG_INFO << "Completed work, shortest distance left is "
                                 << linePair._distance << std::endl;
                        break;
                    }
                    ...
                    if( linePair._line == linePair._neighbourLine )
                    {
                        ...
                        SphereSegment::LineList::iterator fitr =
                            std::find( unfusedLines.begin(), unfusedLines.end(), line );
                        ...
                        {
                            OSG_INFO << "Error couldn't find line in unfused list, "
                                        "exiting fusing loop."
                                     << std::endl;
                            break;
                        }
                    ...
                    else
                    {
                        ...
                        for( int i = openEnd1; i != fuseEnd1; i += direction1 )
                        {
                        ...
                        {
                            osg::vec3 average =
                                ( ( *line1 )[fuseEnd1] + ( *line2 )[fuseEnd2] ) * 0.5F;
                        ...
                        // copy across from the next point in from fuseEnd2 to the
                        // openEnd2.
                        for( int j  = fuseEnd2 + direction2; j != openEnd2 + direction2;
                             j     += direction2 )
                        {
                        ...
                        SphereSegment::LineList::iterator fitr =
                            std::find( unfusedLines.begin(), unfusedLines.end(), line1 );
                        if( fitr != unfusedLines.end() )
                        {
                            unfusedLines.erase( fitr );
                        }
                        ...
                        fitr =
                            std::find( unfusedLines.begin(), unfusedLines.end(), line2 );
                        if( fitr != unfusedLines.end() )
                        {
                            unfusedLines.erase( fitr );
                        }
                    ...
                    _generatedLines.insert( _generatedLines.end(),
                                            unfusedLines.begin(),
                                            unfusedLines.end() );
                }
            }
    ...
    bool
    computeQuadraticSolution( double  a,
                              double  b,
                              double  c,
                              double& s1,
                              double& s2 )
    ...
    {
            AzimPlaneIntersector( TriangleIntersectOperator& tif,
                                  double                     azim,
                                  bool                       lowerOutside ) :
                _tif( tif ),
            ...
            inline bool
            operator()( TriangleIntersectOperator::Edge* edge )
            {
                edge->_intersectionType =
                    TriangleIntersectOperator::Edge::NO_INTERSECTION;
                ...
                if( d1 == 0.0 )
                {
                    ...
                    {
                        edge->_intersectionType =
                            TriangleIntersectOperator::Edge::BOTH_ENDS;
                    }
                    else
                    {
                        edge->_intersectionType =
                            TriangleIntersectOperator::Edge::POINT_1;
                    }
                }
                else if( d2 == 0.0 )
                {
            ...
            osg::vec3
            intersectionPoint( const osg::vec3& v1,
                               const osg::vec3& v2 )
            {
                double d1  = _plane.distance( v1 );
                double d2  = _plane.distance( v2 );
        ...
        protected:
            ...
            operator=( const AzimPlaneIntersector& )
            {
                return *this;
            }
    };
    ...
    {
            ElevationIntersector( TriangleIntersectOperator& tif,
                                  double                     elev,
                                  bool                       lowerOutside ) :
                _tif( tif ),
                _elev( elev ),
            ...
            operator()( TriangleIntersectOperator::Edge* edge )
            {
                ...
                double     elev1      = atan2f( ( double )v1.z, length_xy1 );

                double     length_xy2 = sqrt( v2.x * v2.x + v2.y * v2.y );
                double     elev2      = atan2f( ( double )v2.z, length_xy2 );
                ...
                else
                {
                    ...
                    {
                        OSG_INFO << "neither segment intersects s1=" << s1
                                 << " s2=" << s2 << std::endl;
                               ...
                               const osg::vec3& v2 )
            {
                ...
                if( !computeQuadraticSolution( a, b, c, s1, s2 ) )
                {
                    OSG_INFO << "Warning::neither segment intersects s1=" << s1
                             << " s2=" << s2 << std::endl;
                    return v1;
                }
                double r = 0.0;
            ...
            distance( const osg::vec3& v )
            {
                ...
                double computedElev = atan2f( ( double )v.z, length_xy );
        ...
        protected:
            ...
            ElevationIntersector&
            operator=( const ElevationIntersector& )
            {
                return *this;
            }
    };
    ...
    struct RadiusIntersector
    {
            ...
            operator()( TriangleIntersectOperator::Edge* edge )
            {
                ...
                osg::vec3& v1      = _tif._originalVertices[edge->_p1];
                osg::vec3& v2      = _tif._originalVertices[edge->_p2];
                double     radius1 = osg::length( v1 );
                double     radius2 = osg::length( v2 );
            ...
            distance( const osg::vec3& v )
            {
                return _tif._radius - osg::length( v );
            }

        protected:

            RadiusIntersector&
            operator=( const RadiusIntersector& )
            {
                return *this;
            }
    };
...
SphereSegment::LineList
SphereSegment::computeIntersection( const osg::dmat4& matrix,
                                    osg::Drawable*    drawable )
{
    ...
    typedef osg::TriangleIndexFunctor<TriangleIntersectOperator>
                             TriangleIntersectFunctor;
    ...
    SphereSegment::LineList elevMinLines =
        tif.computeIntersections( elevMinIntersector );
    SphereSegment::LineList elevMaxLines =
        tif.computeIntersections( elevMaxIntersector );
    ...
    tif._generatedLines.insert( tif._generatedLines.end(),
                                radiusLines.begin(),
                                radiusLines.end() );
    tif._generatedLines.insert( tif._generatedLines.end(),
                                azMinLines.begin(),
    ...
    OSG_INFO << "number of separate lines after fuse = " << tif._generatedLines.size()
             << std::endl;

    float joinDistance = 1E8;
    tif.joinEnds( joinDistance, false, false );
    OSG_INFO << "number of separate lines after join = " << tif._generatedLines.size()
             << std::endl;

    tif.joinEnds( joinDistance, false, true );
    OSG_INFO << "number of separate lines after second join = "
             << tif._generatedLines.size() << std::endl;

    return tif._generatedLines;
}

osg::Node*
SphereSegment::computeIntersectionSubgraph( const osg::dmat4& matrix,
                                            osg::Drawable*    drawable )
{
         ...
         ++itr )
    {
        ...
        geom->addPrimitiveSet(
            new osg::DrawArrays( GL_LINE_STRIP, 0, vertices->getNumElements() )
        );
    }
...
SphereSegment::updatePositions()
{
    ...
    for( unsigned int i = 0; i < rowSize; i++ )
    {
        ...
        for( unsigned int j = 0; j < rowSize; j++ )
        {
            ...
            ( *_normals )[pos] = osg::normalize( ( *_normals )[pos] );
...
void
SphereSegment::updatePrimitives()
{
    // surface
    {
        unsigned int                          rowSize = _density + 1;
    ...
    // edge line
    {
        ...
        // add primitive set
        osg::ref_ptr<osg::DrawElementsUShort> elements =
            new osg::DrawElementsUShort( GL_TRIANGLE_FAN );
