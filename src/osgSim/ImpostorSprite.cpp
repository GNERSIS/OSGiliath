 ...
 * Billboard sprite used by Impostor. Holds the cached texture
 * and geometry for the impostor image.
 */
#if defined( _MSC_VER )
...
float
ImpostorSprite::calcPixelError( const osg::dmat4& MVPW ) const
{
    // find the maximum screen space pixel error between the control coords and the quad
    // coners.
    ...
    for( int i = 0; i < 4; ++i )
    {
        ...
        osg::vec3 projected_coord =
            osg::vec3( ( *_coords )[static_cast<std::size_t>( i )] * MVPW );
        osg::vec3 projected_control =
            osg::vec3( _controlcoords[static_cast<std::size_t>( i )] * MVPW );
...
void
ImpostorSprite::setTexture( osg::Texture2D* tex,
                            int             s,
                            int             t )
{
    _texture = tex;
...
void
ImpostorSpriteManager::push_back( ImpostorSprite* is )
{
    if( is == NULL || is == _last )
    {
        return;
...
void
ImpostorSpriteManager::remove( ImpostorSprite* is )
{
    if( is == NULL )
    {
        return;
...
ImpostorSprite*
ImpostorSpriteManager::createOrReuseImpostorSprite( int          s,
                                                    int          t,
                                                    unsigned int frameNumber )
{
    ...
    // AlphaFunc removed: use shader discard instead
...
osg::StateSet*
ImpostorSpriteManager::createOrReuseStateSet()
{
    ...
    _reuseStateSetIndex = static_cast<unsigned int>( _stateSetList.size() );
    return _stateSetList.back().get();
}

void
ImpostorSpriteManager::reset()
{
    _reuseStateSetIndex = 0;
}
