...
namespace osgSim
{
    ...
    class CustomPolytope
    {
        public:
            ...
            typedef std::vector<dvec3> Vertices;
        ...
        OSG_NOTICE<<"                        "<<bb.zMin()<<" "<<bb.zMax()<<std::endl;
            ...
            void
            insertVertex( const osg::dvec3&    vertex,
                          osg::EllipsoidModel* em        = 0,
                          double               minHeight = 0.0 )
            {
                ...
                for( Edges::iterator eitr = edges.begin(); eitr != edges.end(); ++eitr )
                {
                    if( eitr->second == 1 )
                    {
                        ...
                        if( face.plane.distance( faces_center ) < 0.0 )
                        {
                            face.plane.flip();
                        }
                        _faces.push_back( face );
                ...
                if( em )
                {
                    ...
                    for( VertexSet::iterator itr = uniqueVertices.begin();
                         ...
                         ++itr )
                    {
                        const osg::dvec3& point = *itr;
                        double            latitude, longitude, height;
                        em->convertXYZToLatLongHeight( point.x,
                                                       point.y,
                                                       point.z,
                                                       latitude,
                                                       longitude,
                                                       height );
                        osg::dvec3 normal( point );
                        normal = osg::normalize( normal );
                        baseVertices.push_back(
                            point - normal * ( height - minHeight )
                        );
                    }
                    ...
                    {
                        const osg::dvec3& a = baseVertices[i];
                        const osg::dvec3& b = baseVertices[i + 1];
                        const osg::dvec3& c =
                            baseVertices[( i + 2 ) % baseVertices.size()];
                        double     area = osg::length( ( a - b ) ^ ( b - c ) ) * 0.5;
                        osg::dvec3 localCenter  = ( a + b + c ) / 3.0;
                        center                 += localCenter * area;
                        totalArea              += area;
                    }
                    center /= totalArea;
                    osg::dvec3 normal( center );
                    normal = osg::normalize( normal );
            ...
            void
            projectDowntoBase( const osg::dvec3& control,
                               const osg::dvec3& normal )
            {
                // OSG_NOTICE<<"CustomPolytope::projectDowntoBase not implemented
                // yet."<<std::endl;

                Faces           removedFaces;

                Faces::iterator itr;
                for( itr = _faces.begin(); itr != _faces.end(); )
                {
                    Face& face = *itr;
                    if( osg::dot( face.plane.getNormal(), normal ) >= 0.0 )
                    {
                        removedFaces.push_back( face );
                        itr = _faces.erase( itr );
                    }
                ...
                double                                    numVerticesAdded = 0.0;
                osg::dvec3                                center;
                for( itr = removedFaces.begin(); itr != removedFaces.end(); ++itr )
                {
                    Face&     face     = *itr;
                    Vertices& vertices = face.vertices;
                    for( unsigned int i = 0; i < vertices.size(); ++i )
                ...
                for( Edges::iterator eitr = edges.begin(); eitr != edges.end(); ++eitr )
                {
                    if( eitr->second == 1 )
                    {
                        ...
                        double      h_first = osg::dot( edge.first - control, normal );
                        osg::dvec3  projected_first = edge.first - normal * h_first;

                        double      h_second = osg::dot( edge.second - control, normal );
                        osg::dvec3  projected_second = edge.second - normal * h_second;
                ...
                osg::dvec3 v    = normal ^ side;
                v               = osg::normalize( v );

                osg::dvec3 u    = v ^ normal;
                u               = osg::normalize( u );

                typedef std::map<double, dvec3> AnglePositions;
                AnglePositions                  anglePositions;
                for( VertexSet::iterator vitr = uniqueVertices.begin();
                     vitr != uniqueVertices.end();
                     ++vitr )
                {
                    osg::dvec3 delta = *vitr - center;
                    double angle = atan2( osg::dot( delta, u ), osg::dot( delta, v ) );
                    if( angle < 0.0 )
                    {
                        angle += 2.0 * osg::PI;
                    }
            ...
            void
            computeSilhoette( const osg::dvec3& normal,
                              Vertices&         vertices )
            {

                typedef std::pair<osg::dvec3, osg::dvec3> EdgePair;
                ...
                for( EdgeMap::iterator eitr = edgeMap.begin(); eitr != edgeMap.end();
                     ++eitr )
                {
                    ...
                    else if( edgeFaces.size() == 2 )
                    {
                        ...
                        double dotProduct0 =
                            osg::dot( edgeFaces[0]->plane.getNormal(), normal );
                        double dotProduct1 =
                            osg::dot( edgeFaces[1]->plane.getNormal(), normal );
                        if( dotProduct0 * dotProduct1 < 0.0 )
                        {
                            // OSG_NOTICE<<"  Silhoette edge found
                        ...
                        {
                            // OSG_NOTICE<<"  Non silhoette edge found
                            // "<<edgeFaces[0]->name<<" "<<edgeFaces[1]->name<<std::endl;
                        }
                    }
                    else
                    {
                        // OSG_NOTICE<<"Confused edge found
                ...
                for( vitr = uniqueVertices.begin(); vitr != uniqueVertices.end();
                     ++vitr )
                {
                    center += *vitr;
                }
            ...
            void
            cut( const CustomPolytope& polytope )
            {
                // OSG_NOTICE<<"Cutting with polytope "<<std::endl;
                 ...
                 const std::string& name = std::string() )
            {
                ...
                for( Faces::iterator itr = _faces.begin(); itr != _faces.end(); )
                {
                    ...
                    {
                        // OSG_NOTICE<<"    Face intersecting - before
                ...
                {
                    // OSG_NOTICE<<"    inserting newFace intersecting
                    ...
                    osg::dvec3 side =
                        ( fabs( plane.getNormal().x ) < fabs( plane.getNormal().y ) )
                            ? osg::dvec3( 1.0, 0.0, 0.0 )
                            : osg::dvec3( 0.0, 1.0, 0.0 );

                    osg::dvec3 v = plane.getNormal() ^ side;
                    v            = osg::normalize( v );

                    osg::dvec3 u = v ^ plane.getNormal();
                    u            = osg::normalize( u );
                    ...
                    for( vitr = vertices.begin(); vitr != vertices.end(); ++vitr )
                    {
                        osg::dvec3 delta = *vitr - center;
                        double     angle =
                            atan2( osg::dot( delta, u ), osg::dot( delta, v ) );
                        if( angle < 0.0 )
                        {
                            angle += 2.0 * osg::PI;
                        }
            ...
            osg::Drawable*
            createDrawable( const osg::dvec4& colour )
            {
                ...
                for( Faces::iterator itr = _faces.begin(); itr != _faces.end(); ++itr )
                {
                    ...
                    geometry->addPrimitiveSet( new osg::DrawArrays(
                        ...
                        static_cast<GLsizei>( face.vertices.size() )
                    ) );
                    for( Vertices::iterator vitr = face.vertices.begin();
                         vitr != face.vertices.end();
                         ++vitr )
                    {
                        vertices->push_back( osg::vec3( *vitr ) );
                    }
                }

                osg::Vec4Array* colours = new osg::Vec4Array;
                colours->push_back( osg::vec4( colour ) );
...
OverlayNode::OverlayNode( OverlayTechnique technique ) :
    ...
    _textureUnit( 1 ),
    _textureSizeHint( 1'024 ),
    _overlayClearColor( 0.0F,
                        0.0F,
                        0.0F,
                        0.0F ),
...
OverlayNode::OverlayNode( const OverlayNode& copy,
...
void
OverlayNode::OverlayData::setThreadSafeRefUnref( bool threadSafe )
{
    if( _camera.valid() )
    {
...
void
OverlayNode::OverlayData::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _camera.valid() )
    {
...
void
OverlayNode::OverlayData::releaseGLObjects( osg::State* state ) const
{
    if( _camera.valid() )
    {
...
void
OverlayNode::setThreadSafeRefUnref( bool threadSafe )
{
...
void
OverlayNode::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
OverlayNode::releaseGLObjects( osg::State* state ) const
{
...
void
OverlayNode::setOverlayTechnique( OverlayTechnique technique )
{
...
void
OverlayNode::setRenderTargetImplementation(
...
OverlayNode::getOverlayData( osgUtil::CullVisitor* cv )
{
    std::lock_guard<std::mutex> lock( _overlayDataMapMutex );
    ...
    if( !overlayData->_texture )
    {
        ...
        texture->setTextureSize( static_cast<int>( tex_width ),
...
#if 1
        texture->setBorderColor( osg::dvec4( _overlayClearColor.x,
                                             _overlayClearColor.y,
                                             _overlayClearColor.z,
                                             _overlayClearColor.w ) );
#else
    ...
    if( !overlayData->_camera ||
        overlayData->_camera->getRenderTargetImplementation() != _renderTargetImpl )
    {
        ...
        // set viewport
        overlayData->_camera->setViewport( 0,
                                           0,
                                           static_cast<int>( tex_width ),
                                           static_cast<int>( tex_height ) );

        overlayData->_camera->setComputeNearFarMode(
        ...
        overlayData->_camera->attach( osg::Camera::COLOR_BUFFER,
                                      overlayData->_texture.get() );
    ...
    if( !overlayData->_overlayStateSet )
    {
        ...
        std::string vertexShaderFile =
            osgDB::findDataFile( "shaders/overlay_perspective_rtt.vert" );
        if( !vertexShaderFile.empty() )
        {
            program->addShader( osgDB::readRefShaderFile( osg::Shader::VERTEX,
                                                          vertexShaderFile ) );
        }
        else
        {
            char vertexShaderSource[] =
                ...
                "    return vec4(source.x * (1.0 + y0 ), source.y * y0 + 1.0, (source.z "
                "* y0 + 1.0)*0.01, source.w * divisor); \n"
                "} \n"
                " \n"
                "vec3 fnormal(void) \n"
                "{ \n"
                "    //Compute the normal  \n"
                "    vec3 normal = osg_NormalMatrix * osg_Normal; \n"
                ...
                "(osg_LightSource.position)))); \n"
                "   vec3 halfVector = normalize(normalize(osg_LightSource.position.xyz) "
                "+ vec3(0.0, 0.0, 1.0)); \n"
                ...
                "void main() \n"
                "{ \n"
                "    gl_Position = warp(osg_ModelViewProjectionMatrix * osg_Vertex); \n"
                " \n"
                "    if (lightingEnabled) \n"
                "    {     \n"
                ...
                " \n"
                "        vec4 sceneColor = osg_FrontMaterial.emission + "
                "osg_FrontMaterial.ambient * osg_LightSource.ambient; \n"
                "        vec4 color = sceneColor + \n"
                "                     ambient  * osg_FrontMaterial.ambient + \n"
                "                     diffuse  * osg_FrontMaterial.diffuse + \n"
                "                     specular * osg_FrontMaterial.specular; \n"
                " \n"
                "        vertexColor = color; \n"
                " \n"
                "    } \n"
    ...
    if( !overlayData->_mainSubgraphProgram )
    {
        ...
        std::string fragmentShaderFile =
            osgDB::findDataFile( "shaders/overlay_perspective_main.frag" );
        if( !fragmentShaderFile.empty() )
        {
            overlayData->_mainSubgraphProgram->addShader(
                osgDB::readRefShaderFile( osg::Shader::FRAGMENT, fragmentShaderFile )
            );
        }
        else
        {
            char fragmentShaderSource[] =
                ...
                "    return vec2(source.x * (1.0 + y0 ) * inv_divisor , (source.y * y0 "
                "+ 1.0 ) * inv_divisor); \n"
                "} \n"
                " \n"
                "void main() \n"
                "{ \n"
                "    vec2 coord = texCoord_1.xy; \n"
                ...
                " \n"
                "    vec4 base_color = texture(texture_0, texCoord_0.xy); \n"
                "    vec4 overlay_color = texture(texture_1, warped ); \n"
                "    vec3 mixed_color = mix(base_color.rgb, overlay_color.rgb, "
                "overlay_color.a); \n"
                "    fragColor = vec4(mixed_color, base_color.a); \n"
                "} \n";
    ...
    if( !overlayData->_mainSubgraphStateSet )
    {
        ...
        overlayData->_mainSubgraphStateSet->addUniform( new osg::Uniform( "texture_0",
                                                                          0 ) );
        overlayData->_mainSubgraphStateSet->addUniform( new osg::Uniform( "texture_1",
                                                                          1 ) );

        overlayData->_mainSubgraphStateSet->setTextureAttributeAndModes(
            _textureUnit,
            overlayData->_texture.get(),
...
void
OverlayNode::init_OBJECT_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY()
{
    OSG_INFO << "OverlayNode::init() - OBJECT_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY"
             << std::endl;
...
void
OverlayNode::init_VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY()
{
    OSG_INFO << "OverlayNode::init() - VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY"
             << std::endl;
}

void
OverlayNode::init_VIEW_DEPENDENT_WITH_PERSPECTIVE_OVERLAY()
{
    OSG_INFO << "OverlayNode::init() - VIEW_DEPENDENT_WITH_PERSPECTIVE_OVERLAY"
             << std::endl;
}
...
void
OverlayNode::traverse_OBJECT_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY( osg::NodeVisitor& nv )
{
    ...
    if( nv.getVisitorType() == osg::NodeVisitor::UPDATE_VISITOR )
    {
        ...
        if( _continuousUpdate || _updateCamera )
        {
            ...
            {
                bs.expandBy(
                    static_cast<const osg::sphere&>( camera->getChild( i )->getBound() )
                );
            }
            ...
            if( bs.valid() )
            {
                ...
                if( em )
                {
                    osg::dvec3 eyePoint( 0.0, 0.0, 0.0 );    // center of the planet
                    double     centerDistance =
                        osg::length( eyePoint - osg::dvec3( bs.center ) );
                    ...
                    camera->setProjectionMatrixAsFrustum( -right,
                                                          right,
                                                          -top,
                                                          top,
                                                          znear,
                                                          zfar );
                    camera->setViewMatrixAsLookAt( eyePoint,
                                                   osg::dvec3( bs.center ),
                                                   osg::dvec3( 0.0, 1.0, 0.0 ) );
                }
                else
                {
                    osg::dvec3 upDirection( 0.0, 1.0, 0.0 );
                    osg::dvec3 viewDirection( 0.0, 0.0, 1.0 );

                    double     viewDistance = 2.0 * bs.radius;
                    osg::dvec3 center       = osg::dvec3( bs.center );
                    osg::dvec3 eyePoint     = center + viewDirection * viewDistance;
                    ...
                    camera->setProjectionMatrixAsOrtho( -right,
                                                        right,
                                                        -top,
                                                        top,
                                                        znear,
                ...
                // coords
                osg::dmat4 MVP = camera->getViewMatrix() * camera->getProjectionMatrix();
    ...
    // now set up the drawing of the main scene.
    {
        ...
        const osg::dmat4 modelView = *( cv->getModelViewMatrix() );
        osg::Polytope    viewTextureFrustum;
        viewTextureFrustum.setAndTransformProvidingInverse( overlayData._textureFrustum,
                                                            osg::inverse( modelView ) );

        cv->getProjectionCullingStack().back().addStateFrustum(
            overlayData._mainSubgraphStateSet.get(),
            viewTextureFrustum
        );
        cv->getCurrentCullingSet().addStateFrustum(
            overlayData._mainSubgraphStateSet.get(),
            overlayData._textureFrustum
        );
...
void
OverlayNode::traverse_VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY( osg::NodeVisitor& nv )
{
    // OSG_NOTICE<<"OverlayNode::traverse() -
    // VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY"<<std::endl;
    ...
    if( _overlaySubgraph.valid() )
    {
        ...
        osg::sphere          bs = _overlaySubgraph->getBound();
        ...
        osg::dmat4 pm    = *( cv->getProjectionMatrix() );
        ...
        osg::dmat4 MVP = *( cv->getModelViewMatrix() ) * pm;
        osg::dmat4 inverseMVP;
        inverseMVP = osg::inverse( MVP );
        ...
        if( em )
        {
            overlayPolytope.insertVertex( osg::dvec3( 0.0, 0.0, 0.0 ),
                                          em,
                                          _overlayBaseHeight );
        }
        else
        {
            overlayPolytope.projectDowntoBase(
                osg::dvec3( 0.0, 0.0, _overlayBaseHeight ),
        ...
        {
            overlayData._geode->addDrawable(
                overlayPolytope.createDrawable( osg::dvec4( 1.0F, 1.0F, 0.0F, 1.0F ) )
            );
            overlayData._geode->addDrawable(
                frustum.createDrawable( osg::dvec4( 0.0F, 0.0F, 1.0F, 1.0F ) )
            );
        }

        // OSG_NOTICE<<"AFTER CUT corners = "<<corners.size()<<std::endl;

        osg::dvec3 center = osg::dvec3( _overlaySubgraph->getBound().center );

        osg::dvec3 lookVector( 0.0, 0.0, -1.0 );
        if( em )
        {
            lookVector = -center;
            lookVector = osg::normalize( lookVector );
        }

        osg::dvec3 sideVectorLV = lookVector ^ cv->getLookVectorLocal();
        osg::dvec3 sideVectorUP = lookVector ^ cv->getUpLocal();

        osg::dvec3 sideVector = osg::length( sideVectorLV ) > osg::length( sideVectorUP )
                                  ? sideVectorLV
                                  : sideVectorUP;

        sideVector            = osg::normalize( sideVector );

        osg::dvec3 upVector   = sideVector ^ lookVector;
        upVector              = osg::normalize( upVector );

        osg::dvec3 overlayLookVector = upVector ^ sideVector;
        overlayLookVector            = osg::normalize( overlayLookVector );
        ...
        {
            overlayData._geode->addDrawable(
                overlayPolytope.createDrawable( osg::dvec4( 1.0F, 1.0F, 1.0F, 1.0F ) )
            );
        }
        ...
        typedef std::vector<osg::dvec2> ProjectedVertices;
        ...
        {
            double distanceEye = osg::length( corners[i] - eyeLocal );
            ...
            osg::dvec3 delta         = corners[i] - center;
            double     distance_side = osg::dot( delta, sideVector );
            double     distance_up   = osg::dot( delta, upVector );
            projectedVertices.push_back( osg::dvec2( distance_side, distance_up ) );
        ...
        bool   usePerspectiveShaders =
            ( _overlayTechnique == VIEW_DEPENDENT_WITH_PERSPECTIVE_OVERLAY );
        ...
        if( usePerspectiveShaders )
        {
            ...
            osg::dvec3 v000            = osg::dvec3( -1.0, -1.0, -1.0 ) * inverseMVP;
            osg::dvec3 v010            = osg::dvec3( -1.0, 1.0, -1.0 ) * inverseMVP;
            osg::dvec3 v100            = osg::dvec3( 1.0, -1.0, -1.0 ) * inverseMVP;
            osg::dvec3 v110            = osg::dvec3( 1.0, 1.0, -1.0 ) * inverseMVP;

            osg::dvec3 v001            = osg::dvec3( -1.0, -1.0, 1.0 ) * inverseMVP;
            osg::dvec3 v011            = osg::dvec3( -1.0, 1.0, 1.0 ) * inverseMVP;
            osg::dvec3 v101            = osg::dvec3( 1.0, -1.0, 1.0 ) * inverseMVP;
            osg::dvec3 v111            = osg::dvec3( 1.0, 1.0, 1.0 ) * inverseMVP;

            osg::dvec3 edgeBottomLeft  = v001 - v000;
            osg::dvec3 edgeBottomRight = v101 - v100;
            osg::dvec3 edgeTopRight    = v111 - v110;
            osg::dvec3 edgeTopLeft     = v011 - v010;
            edgeBottomLeft             = osg::normalize( edgeBottomLeft );
            edgeBottomRight            = osg::normalize( edgeBottomRight );
            edgeTopLeft                = osg::normalize( edgeTopLeft );
            edgeTopRight               = osg::normalize( edgeTopRight );

            double frustumDiagonal =
                osg::degrees( acos( osg::dot( edgeBottomLeft, edgeBottomRight ) ) );

            // OSG_NOTICE<<"  width ratio  = "<<new_width/original_width<<std::endl;
            // OSG_NOTICE<<"  near ratio  = "<<ratio *
            // new_width/original_width<<std::endl;
            double angle = 2.0 * osg::degrees( atan( max_side_over_up ) );

            if( angle > frustumDiagonal )
            {
                double maxHalfAngle = osg::radians( 30.0 );
                ...
                // OSG_NOTICE<<"  originalRatio  = "<<ratio<<"
            ...
            overlayData._mainSubgraphStateSet->setAttribute(
                overlayData._mainSubgraphProgram.get()
            );

            osg::dmat4 projection;
            projection =
                osg::orthographic( min_side, max_side, min_up, max_up, zNear, zFar );

            camera->setProjectionMatrix( projection );
        }
        else
        {
            overlayData._mainSubgraphStateSet->removeAttribute(
                osg::StateAttribute::Type::PROGRAM
            );
            camera->setProjectionMatrixAsOrtho( min_side,
                                                max_side,
                                                min_up,
                                                max_up,
                                                zNear,
        ...
        if( em )
        {
            camera->setViewMatrixAsLookAt( osg::dvec3( bs.center ),
        ...
        {
            camera->setViewMatrixAsLookAt( osg::dvec3( bs.center ),
                                           osg::dvec3( bs.center ) + overlayLookVector,
                                           upVector );
        }
        ...
        unsigned int contextID =
            cv->getState() != 0 ? cv->getState()->getContextID() : 0;
        ...
        if( usePerspectiveShaders )
        {
                 ...
                 ++itr )
            {
                osg::StateAttribute::GLModeValue mode =
                    static_cast<osg::StateAttribute::GLModeValue>(
                        ( *itr )->getMode( GL_LIGHTING )
                    );
                if( ( mode & ~static_cast<osg::StateAttribute::GLModeValue>(
                                 osg::StateAttribute::INHERIT
                             ) ) != 0 )
                {
                    if( ( mode & osg::StateAttribute::PROTECTED ) !=
                        0 ||
            ...
            overlayData._lightingEnabled->set( ( base_mode & osg::StateAttribute::ON ) !=
                                               0 );
        }
...
void
OverlayNode::traverse_VIEW_DEPENDENT_WITH_PERSPECTIVE_OVERLAY( osg::NodeVisitor& nv )
{
    traverse_VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY( nv );
}

void
OverlayNode::setOverlaySubgraph( osg::Node* node )
{
...
void
OverlayNode::dirtyOverlayTexture()
{
...
void
OverlayNode::setOverlayTextureUnit( unsigned int unit )
{
    _textureUnit = unit;
...
void
OverlayNode::setOverlayTextureSizeHint( unsigned int size )
{
         ...
         ++itr )
    {
        ...
        {
            itr->second->_texture->setTextureSize(
                static_cast<int>( _textureSizeHint ),
                static_cast<int>( _textureSizeHint )
            );
        }
        if( itr->second->_camera.valid() )
        {
            itr->second->_camera->setViewport( 0,
                                               0,
                                               static_cast<int>( _textureSizeHint ),
                                               static_cast<int>( _textureSizeHint ) );
        }
...
void
OverlayNode::updateMainSubgraphStateSet()
{
         ...
         ++itr )
    {
        ...
        if( mainSubgraphStateSet )
        {
            ...
            mainSubgraphStateSet->setTextureAttributeAndModes(
                _textureUnit,
                itr->second->_texture.get(),
                osg::StateAttribute::ON
            );
        }
    }
