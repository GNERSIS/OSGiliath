...
namespace osgSim
{
    ...
    osg::StateSet*
    getSingletonLightPointSystemSet()
    {
        static osg::ref_ptr<osg::StateSet> s_stateset = 0;
        if( !s_stateset )
        {
            s_stateset = new osg::StateSet;
            // force light point nodes to be drawn after everything else by picking a
            // rendering bin number after the transparent bin.
            s_stateset->setRenderBinDetails( 20, "DepthSortedBin" );
            // Disable lighting so the shader pipeline passes through per-vertex colors
            // without applying the lighting equation (light points have no normals).
            s_stateset->setMode( GL_LIGHTING,
    ...
    LightPointNode::LightPointNode( const LightPointNode& lpn,
    ...
    unsigned int
    LightPointNode::addLightPoint( const LightPoint& lp )
    {
        unsigned int num = static_cast<unsigned int>( _lightPointList.size() );
    ...
    void
    LightPointNode::removeLightPoint( unsigned int pos )
    {
    ...
    osg::sphere
    LightPointNode::computeBound() const
    {
        osg::sphere bsphere;
        bsphere.init();
        _bbox.init();
        ...
        for( itr = _lightPointList.begin(); itr != _lightPointList.end(); ++itr )
        {
        ...
        for( itr = _lightPointList.begin(); itr != _lightPointList.end(); ++itr )
        {
            ...
            float     r = osg::length( dv ) + itr->_radius;
            if( bsphere.radius < r )
            {
                bsphere.radius = r;
            }
    ...
    void
    LightPointNode::traverse( osg::NodeVisitor& nv )
    {
        if( _lightPointList.empty() )
        {
...
// #define USE_TIMER
        ...
        if( cv /*&& !cv->isCulled(_bbox)*/ )
        {
            ...
            osg::dmat4           matrix     = *( cv->getModelViewMatrix() );
            ...
            if( object )
            {
                ...
                else
                {
                    ...
                    OSG_WARN
                        << "Warning: Replacing osgUtil::StateGraph::_userData to "
                           "support osgSim::LightPointNode, may have undefined results."
                        << std::endl;
                }
            }
            ...
            if( !drawable )
            {
                drawable = _pointSprites ? new LightPointSpriteDrawable
                                         : new LightPointDrawable;
                rg->setUserData( drawable );

                if( cv->getFrameStamp() )
                {
                    drawable->setSimulationTime(
                        cv->getFrameStamp()->getSimulationTime()
                    );
                }
            }

            // search for a drawable in the RenderLeaf list equal to the attached the one
            // attached to StateGraph user data as this will be our special light point
            // drawable.
            ...
            {
                // haven't found the drawable added in the RenderLeaf list, therefore
                // this may be the first time through LightPointNode in this frame, so
                // need to add drawable into the StateGraph RenderLeaf list and update
                // its time signatures.

                drawable->reset();
                rg->addLeaf(
                    new osgUtil::RenderLeaf( drawable, &projection, NULL, FLT_MAX )
                );

                // need to update the drawable's frame count.
                if( cv->getFrameStamp() )
                {
                    drawable->updateSimulationTime(
                        cv->getFrameStamp()->getSimulationTime()
                    );
                }
            }
                 ...
                 ++itr )
            {
                ...
                float intensity = ( _lightSystem.valid() ) ? _lightSystem->getIntensity()
                                                           : lp._intensity;
                ...
                {
                    if( osg::length2( dv ) > _maxVisibleDistance2 )
                    {
                        continue;
                    }
                    else if( _maxVisibleDistance2 > 0 )
                    {
                        distanceFactor = 1.0F - osg::square( osg::length2( dv ) /
                ...
                osg::vec3 xpos( matrix * osg::dvec3( position ) );
                ...
                if( lp._blendingMode == LightPoint::BLENDED )
                {
                    ...
                    else if( pixelSize < _maxPixelSize )
                    {
                        ...
                        float        remainder =
                            osg::square( pixelSize - ( float )lowerBoundPixelSize );
                        ...
                        if( orgPixelSize < _minPixelSize )
                        {
                            color[3] *=
                                static_cast<float>( ( 2.0 / 3.0 ) +
                                                    ( 1.0 / 3.0 ) *
                                                    sqrt( orgPixelSize / pixelSize ) );
                        }

                        drawable->addBlendedLightPoint( lowerBoundPixelSize - 1,
                                                        xpos,
                                                        color );
                        color[3] *= remainder;
                        drawable->addBlendedLightPoint( lowerBoundPixelSize,
                                                        xpos,
                                                        color );
                    }
                    else    // use a billboard geometry.
                    {
                        drawable->addBlendedLightPoint( ( unsigned int )( _maxPixelSize -
                                                                          1.0 ),
                                                        xpos,
                                                        color );
                    }
                ...
                else    // ADDITIVE blending.
                {
                    ...
                    else if( pixelSize < _maxPixelSize )
                    {
                        ...
                        drawable->addAdditiveLightPoint( lowerBoundPixelSize - 1,
                                                         xpos,
                                                         color );
                        color[3] = alpha * remainder;
                        drawable->addAdditiveLightPoint( lowerBoundPixelSize,
                                                         xpos,
                                                         color );
                    }
                    else    // use a billboard geometry.
                    {
                        drawable->addAdditiveLightPoint(
                            ( unsigned int )( _maxPixelSize - 1.0 ),
                            xpos,
                            color
                        );
                    }
        ...
        cout << "  t8-t7/size = "
             << ( float )( t8 - t7 ) / ( float )_lightPointList.size() << endl;
#endif
    }
