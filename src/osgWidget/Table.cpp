...
namespace osgWidget
{
    ...
    // TODO: There is a serious, outstanding bug with regards to USING a table before ALL
    // Widgets are set! FIX THIS!!!
    ...
    unsigned int
    Table::_calculateIndex( unsigned int row,
    ...
    void
    Table::_getRows( CellSizes& rows,
                     Getter     get ) const
    {
        for( unsigned int i = 0; i < _rows; i++ )
        {
            rows.push_back( _compare<Greater>( get, i * _cols, ( i * _cols ) + _cols ) );
        }
    }

    void
    Table::_getColumns( CellSizes& cols,
                        Getter     get ) const
    {
        for( unsigned int i = 0; i < _cols; i++ )
        {
            cols.push_back( _compare<Greater>( get, i, 0, _cols ) );
        }
    }

    void
    Table::_resizeImplementation( point_type width,
                                  point_type height )
    {
    ...
    Window::Sizes
    Table::_getWidthImplementation() const
    {
        CellSizes cols;
        CellSizes minCols;
        ...
        return Sizes( std::accumulate( cols.begin(), cols.end(), 0.0F, Plus() ),
                      std::accumulate( minCols.begin(), minCols.end(), 0.0F, Plus() ) );
    }

    Window::Sizes
    Table::_getHeightImplementation() const
    {
        CellSizes rows;
        CellSizes minRows;
        ...
        return Sizes( std::accumulate( rows.begin(), rows.end(), 0.0F, Plus() ),
                      std::accumulate( minRows.begin(), minRows.end(), 0.0F, Plus() ) );
    }

    bool
    Table::addWidget( Widget* widget )
    {
    ...
    void
    Table::getRowHeights( CellSizes& rowHeights ) const
    {
        _getRows( rowHeights, &Widget::getHeightTotal );
    }

    void
    Table::getRowMinHeights( CellSizes& rowMinHeights ) const
    {
        _getRows( rowMinHeights, &Widget::getMinHeightTotal );
    }

    void
    Table::getColumnWidths( CellSizes& colWidths ) const
    {
        _getColumns( colWidths, &Widget::getWidthTotal );
    }

    void
    Table::getColumnMinWidths( CellSizes& colMinWidths ) const
    {
        _getColumns( colMinWidths, &Widget::getMinWidthTotal );
    }

    void
    Table::addHeightToRow( unsigned int row,
                           point_type   height )
    {
        for( Iterator i = begin() + ( row * _cols );
    ...
    void
    Table::addWidthToColumn( unsigned int col,
                             point_type   width )
    {
        // See the documentation in include/osgWidget/Window::_forEachApplyOrAssign if
        // you want to know why we need this variable.
        unsigned int c = col;
    ...
    bool
    Table::isRowVerticallyFillable( unsigned int row ) const
    {
        return static_cast<point_type>( _cols ) ==
               _getNumFill( row * _cols, ( row * _cols ) + _cols );
    }

    bool
    Table::isColumnHorizontallyFillable( unsigned int col ) const
    {
