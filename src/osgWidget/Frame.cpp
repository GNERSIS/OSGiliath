...
namespace osgWidget
{
    ...
    std::string
    Frame::cornerTypeToString( CornerType c )
    {
    ...
    std::string
    Frame::borderTypeToString( BorderType b )
    {
        if( b == BORDER_LEFT )
        {
            return "BorderLeft";
    ...
    Frame::Corner::Corner( CornerType corner,
                           point_type width,
                           point_type height ) :
        Inherit( cornerTypeToString( corner ),
                 width,
                 height ),
        _corner( corner )
    {
    ...
    Frame::Corner::Corner( const Corner&      corner,
                           const osg::CopyOp& co ) :
        Inherit( corner,
                 co ),
    ...
    void
    Frame::Corner::parented( Window* /*window*/ )
    {
    ...
    bool
    Frame::Corner::mouseDrag( double x,
                              double y,
    ...
    Frame::Border::Border( BorderType border,
                           point_type width,
                           point_type height ) :
        Inherit( borderTypeToString( border ),
                 width,
                 height ),
        _border( border )
    {
    ...
    Frame::Border::Border( const Border&      border,
                           const osg::CopyOp& co ) :
        Inherit( border,
                 co ),
    ...
    void
    Frame::Border::parented( Window* /*window*/ )
    {
        Frame* parent = dynamic_cast<Frame*>( getParent() );
    ...
    bool
    Frame::Border::mouseDrag( double x,
                              double y,
                              const WindowManager* /*wm*/ )
    {
        Frame* parent = dynamic_cast<Frame*>( getParent() );
    ...
    Frame*
    Frame::createSimpleFrame( const std::string& name,
                              point_type         cw,
                              point_type         ch,
                              point_type         w,
                              point_type         h,
                              ...
                              Frame*             exFrame )
    {
        ...
        // Use an "existing frame" if we have it (for example, if you've in inherited
        // from Frame and want to use this stuff.
        if( !exFrame )
        {
    ...
    Frame* Frame::createSimpleFrameWithSingleTexture(
        ...
        frame->getEmbeddedWindow()->setTexCoordRegion(cw, ch, tw - (cw * 2.0f), th - (ch
    * 2.0f));
    ...
    */
    ...
    Frame*
    Frame::createSimpleFrameWithSingleTexture( const std::string&       name,
                                               ...
                                               Frame*                   exFrame )
    {
        ...
        if( image )
        {
            ...
            frame->getCorner( CORNER_UPPER_RIGHT )
                ->setTexCoordRegion( w * 2, 0.0F, twh );
            ...
            frame->getCorner( CORNER_LOWER_RIGHT )
                ->setTexCoordRegion( w * 7, 0.0F, twh );

            // We set all of these to wrap vertically, but the REAL texture coordinates
            // will be generated properly in the positioned() method.
        ...
        {
            OSG_WARN
                << "createSimpleFrameWithSingleTexture with a null image, the frame "
                << name << " will be use texture" << std::endl;
        }
    ...
    bool
    Frame::resizeFrame( point_type w,
                        point_type h )
    {
        ...
        return resize( left->getWidth() + right->getWidth() + w,
                       top->getHeight() + bottom->getHeight() + h );
    }

    osg::Image*
    createNatifEdgeImageFromTheme( osg::Image* theme );

    Frame*
    Frame::createSimpleFrameFromTheme( const std::string&       name,
                                       osg::ref_ptr<osg::Image> image,
                                       point_type               width,
                                       point_type               height,
                                       unsigned int             flags,
                                       Frame*                   exFrame )
    {
        ...
        osg::ref_ptr<osg::Image> natifImage =
            createNatifEdgeImageFromTheme( image.get() );
        Frame* frame;

        frame = createSimpleFrameWithSingleTexture( name,
                                                    natifImage.get(),
                                                    width,
                                                    height,
                                                    flags,
        ...
        if( frame && image && natifImage.valid() )
        {
            ...
            {
                color[d] =
                    srcdata[one_third_s * image->s() * bpps + ( one_third_s )*bpps + d] *
                    1.0 /
                    255.0;
            }
    ...
    template<typename T>
    void
    copyDataImpl( const osg::Image*  source,
                  const unsigned int x1,
                  const unsigned int y1,
                  const unsigned int x2,
                  const unsigned int y2,
                  osg::Image*        destination,
                  const unsigned int xd = 0,
                  const unsigned int yd = 0 )
    {
        if( ( unsigned int )destination->s() >=
            xd +
            ( x2 - x1 ) &&
            ( unsigned int )destination->t() >=
            yd +
            ( y2 - y1 ) )
        {
            ...
            for( unsigned int y = 0; y < y2 - y1; ++y )
            {
                for( unsigned int x = 0; x < x2 - x1; ++x )
                {
                    ...
                    {
                        T v = srcdata
                            [( y + y1 ) * source->s() * bpps + ( x + x1 ) * bpps + d];
                        dstdata[( yd + y ) *
                                destination->s() *
                                bpps +
                                ( xd + x ) *
    ...
    void
    copyData( const osg::Image*  source,
              const unsigned int x1,
              const unsigned int y1,
              const unsigned int x2,
              const unsigned int y2,
    ...
    template<typename T>
    osg::Image*
    rotateImageImpl( osg::Image* image )
    {
        if( image->s() == image->t() )
        {
            ...
            for( unsigned int y = 0; y < s; ++y )
            {
                for( unsigned int x = 0; x < s; ++x )
                {
                    ...
                    {
                        dstdata[y * s * bpp + x * bpp + p] =
                            srcdata[x * s * bpp + y * bpp + p];
                    }
                }
            }
    ...
    createNatifEdgeImageFromTheme( osg::Image* theme )
    {
        ...
        {
            OSG_WARN
                << "can't create a natif edge image from null image theme as argument"
                << std::endl;
            return 0;
        }
        ...
        if( s != t )
        {
            OSG_WARN << "width and height are different, bad format theme image "
                     << theme->getFileName() << std::endl;
            return 0;
        }
        ...
        {
            OSG_WARN
                << "the size of theme file " << theme->getFileName()
                << " can not be divided by 3, check the documentation about theme format"
                << std::endl;
            return 0;
        }
        ...
        final->allocateImage( 8 * one_third_s,
                              one_third_t,
                              1,
                              pixelFormat,
                              dataType,
        ...
        rotateandcopy2->allocateImage( one_third_s,
                                       one_third_t,
                                       1,
                                       pixelFormat,
                                       dataType,
        ...
        rotateandcopy7->allocateImage( one_third_s,
                                       one_third_t,
                                       1,
                                       pixelFormat,
                                       dataType,
