...
namespace osgWidget
{
    ...
    WindowManager::WindowManager( osgViewer::View* view,
                                  point_type       width,
                                  point_type       height,
    ...
    void
    WindowManager::setEventFromInterface( Event&          ev,
                                          EventInterface* ei )
    {
    ...
    bool
    WindowManager::_handleMousePushed( float x,
                                       float y,
                                       bool& down )
    {
        down = true;
        ...
        // grabFocus(). bool handled = _lastPush->callMethodAndCallbacks(ev);
    ...
    bool
    WindowManager::_handleMouseReleased( float /*x*/,
                                         float /*y*/,
                                         bool& down )
    {
        down = false;
    ...
    void
    WindowManager::_getPointerXYDiff( float& x,
                                      float& y )
    {
        x -= _lastX;
        y -= _lastY;
    ...
    void
    WindowManager::_updatePickWindow( const WidgetList* wl,
                                      point_type        x,
                                      point_type        y )
    {
        ...
        ss << "At XY Coords: " << x << ", " << y << " ( diff " << xdiff << ", " << ydiff
           << " )" << std::endl;

        const Window* parent = wl->back()->getParent();

        ss << "Window: " << parent->getName() << " ( xyz " << parent->getPosition()
           << " )"
           << " { zRange " << parent->getZRange() << " }"
           << " < size " << parent->getSize() << " >"
           << " EventMask: " << std::hex << parent->getEventMask() << std::endl;

        for( WidgetList::const_iterator i = wl->begin(); i != wl->end(); i++ )
        {
            Widget* widget = i->get();

            ss << "   - " << widget->getName() << " ( xyz " << widget->getPosition()
               << " )"
               << " [ XYZ " << widget->getPosition() *
                parent->getMatrix()
               << " ] < size " << widget->getSize() << " >"
               << " EventMask: " << std::hex << widget->getEventMask() << std::endl;
        }
    ...
    void
    WindowManager::childInserted( unsigned int i )
    {
    ...
    void
    WindowManager::childRemoved( unsigned int start,
    ...
    // This method performs intersection testing at the given XY coords, and returns true
    // if any intersections were found. It will break after processing the first pickable
    // Window it finds.
    bool
    WindowManager::pickAtXY( float       x,
                             float       y,
                             WidgetList& wl )
    {
        Intersections              intr;
        ...
        osgViewer::GraphicsWindow* gw =
            dynamic_cast<osgViewer::GraphicsWindow*>( camera->getGraphicsContext() );
        if( gw )
        {
            _view->computeIntersections( camera,
        ...
        if( !intr.empty() )
        {
            ...
            for( Intersections::iterator i = intr.begin(); i != intr.end(); i++ )
            {
                ...
                for( osg::NodePath::const_reverse_iterator np_itr = nodePath.rbegin();
                     np_itr != nodePath.rend();
                     ++np_itr )
                {
    ...
    bool
    WindowManager::setFocused( Window* window )
    {
        Event ev( this );
        ...
        // Build a vector of every Window that is focusable, in the foreground, and in
        // the background. All these Windows are handled differently.
        Vector focusable;
        Vector bg;
        Vector fg;
        ...
        // This is the depth range for each Window. Each Window object must be informed
        // of the Z space allocated to it so that it can properly arrange it's children.
        // We add 2 additional Windows here for anything that should appear in the
    ...
    void
    WindowManager::setPointerXY( float x,
                                 float y )
    {
        float xdiff = x;
        float ydiff = y;
    ...
    void
    WindowManager::setStyleManager( StyleManager* sm )
    {
        _styleManager = sm;
    ...
    void
    WindowManager::resizeAllWindows( bool visible )
    {
        for( Iterator i = begin(); i != end(); i++ )
        {
            if( i->valid() )
    ...
    XYCoord
    WindowManager::windowXY( double x,
                             double y ) const
    {
    ...
    XYCoord
    WindowManager::localXY( double x,
                            double y ) const
    {
        return XYCoord( ( _width / _windowWidth ) * x, ( _height / _windowHeight ) * y );
    }

    // This is called by a ViewerEventHandler/MouseHandler (or whatever) as the pointer
    // moves around and intersects with objects. It also resets our state data (_widget,
    // _leftDown, etc.) The return value of this method is mostly useless.
    bool
    WindowManager::pointerMove( float x,
                                float y )
    {
        WidgetList wl;
        Event      ev( this );
        ...
        EventInterface* ei =
            getFirstEventInterface( wl, ev.makeMouse( x, y, EVENT_MOUSE_OVER ) );
        ...
        if( _lastEvent != ei )
        {
            ...
            _lastEvent->callMethodAndCallbacks(
                ev.makeMouse( x, y, EVENT_MOUSE_ENTER )
            );
        }
    ...
    bool
    WindowManager::pointerDrag( float x,
                                float y )
    {
        WidgetList widgetList;
        Event      ev( this );
    ...
    bool
    WindowManager::mouseScroll( float x,
                                float y )
    {
        WidgetList wl;
    ...
    // width and height.
    osg::Camera*
    WindowManager::createParentOrthoCamera()
    {
