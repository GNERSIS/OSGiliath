...
namespace osgWidget
{
    ...
    // an empty namespace. This is 100% for code clarity, as this namespace is internal
    // and not visible to the outside C/C++ world.
    namespace lua
    {
    ...
    bool
    noLuaFail( const std::string& err )
    {
    ...
    bool
    LuaEngine::eval( const std::string& /*code*/ )
    {
#ifdef OSGWIDGET_USELUA
        if( luaL_dostring( _data->lua, code.c_str() ) )
        {
            warn() << "LuaEngine::eval - " << lua_tostring( _data->lua, -1 )
                   << std::endl;
    ...
    bool
    LuaEngine::runFile( const std::string& /*filePath*/ )
    {
#ifdef OSGWIDGET_USELUA
        if( !osgDB::fileExists( filePath ) )
        {
            warn() << "Couldn't find file \"" << filePath << "\" for LuaEngine."
                   << std::endl;
        ...
        {
            warn() << "LuaEngine::runFile - " << lua_tostring( _data->lua, -1 )
                   << std::endl;
