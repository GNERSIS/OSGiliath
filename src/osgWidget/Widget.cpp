...
namespace osgWidget
{
        ...
        _minHeight( 0.0F )
    {
        ...
        if( !_norms.valid() )
        {
            ...
            ( *_norms )[0] = osg::normalize( ( *_norms )[0] );
        }

        TexCoordArray* texs = new TexCoordArray( 4 );

        // Fill our texture coordinates with null stuff for now, since we aren't using
        // them until an Image is set at some later point.
    ...
    point_type
    Widget::_calculateZ( unsigned int layer ) const
    {
        ...
        return ( static_cast<point_type>( layer ) /
                 static_cast<point_type>( LAYER_TOP + 1 ) ) *
               zRange;
    }

    WindowManager*
    Widget::_getWindowManager() const
    {
        if( !_parent )
        {
            return 0;
    ...
    void
    Widget::setDimensions( point_type x,
                           point_type y,
                           point_type w,
                           point_type h,
                           point_type z )
    {
        ...
        {
            warn() << "Widget [" << _name << "] was asked to set it's width to " << w
                   << ", but the minimum width is " << _minWidth << "." << std::endl;
        ...
        {
            warn() << "Widget [" << _name << "] was asked to set it's height to " << h
                   << ", but the minimum height is " << _minHeight << "." << std::endl;
        ...
        if( _coordMode == CM_ABSOLUTE )
        {
            ...
            // We could just call getWidth(), etc., but all those dynamic_casts could
            // eventually get expensive, so we just use the already-created verts() array
            // directly.
            if( x < 0.0F )
            {
        ...
        // Now, we need to determine if the dimensions are actually percentage of the
        // parent's size, rather than an absolute values. The Widget must be parented for
        // this to be valid, however.
        ...
        {
            getOrCreateStateSet()->setRenderBinDetails( static_cast<int>( z ),
                                                        "RenderBin" );
    ...
    void
    Widget::setTexCoord( texcoord_type tx,
                         texcoord_type ty,
                         Corner        p )
    {
    ...
    void
    Widget::setTexCoordRegion( point_type x,
                               point_type y,
                               point_type w,
                               point_type h )
    {
    ...
    void
    Widget::setTexCoordWrapHorizontal()
    {
        osg::Image*   image   = _image();
    ...
    void
    Widget::setTexCoordWrapVertical()
    {
        osg::Image*   image   = _image();
        osg::Texture* texture = _texture();
    ...
    bool
    Widget::setImage( osg::Image* image,
                      bool        setTexCoords,
                      bool        useTextRect )
    {
        if( !image )
    ...
    bool
    Widget::setImage( const std::string& filePath,
                      bool               setTexCoords,
                      bool               useTextRect )
    {
        if( !osgDB::findDataFile( filePath ).size() )
        {
            warn() << "Widget [" << _name << "] cannot find file " << filePath
    ...
    bool
    Widget::setTexture( osg::Texture* texture,
                        bool          setTexCoords,
                        bool          useTextRect )
    {
        if( !texture )
    ...
    void
    Widget::setPadding( point_type pad )
    {
        _padLeft = _padRight = _padTop = _padBottom = pad;
    }

    void
    Widget::addX( point_type x )
    {
        if( _coordMode == CM_ABSOLUTE )
        {
    ...
    void
    Widget::addHeight( point_type h )
    {
        if( _coordMode == CM_ABSOLUTE )
        {
            setDimensions( -1.0F, -1.0F, -1.0F, MACRO_WIDGET_H( _verts() ) + h );
        }
                       ...
                       point_type y )
    {
        if( _coordMode == CM_ABSOLUTE )
        {
            ...
            setDimensions( MACRO_WIDGET_X( verts ) + x, MACRO_WIDGET_Y( verts ) + y );
        }
    ...
    point_type
    Widget::getPadHorizontal() const
    {
        return _padLeft + _padRight;
    }

    point_type
    Widget::getPadVertical() const
    {
        return _padTop + _padBottom;
    }

    const Point&
    Widget::getPoint( Corner p ) const
    {
        Corner point = p;
    ...
    const Color&
    Widget::getColor( Corner p ) const
    {
        Corner point = p;
    ...
    const TexCoord&
    Widget::getTexCoord( Corner p ) const
    {
        Corner point = p;
    ...
    Color
    Widget::getImageColorAtXY( point_type x,
                               point_type y ) const
    {
        ...
        */
        ...
        {
            OSG_INFO << "Widget::getImageColorAtXY(" << x << ", " << y
                     << ") Texture coordinate out of range, X=" << X << ", Y=" << Y
                     << std::endl;
            return Color();
        }
    ...
    bool
    Widget::isPaddingUniform() const
    {
        return _padLeft == _padRight && _padLeft == _padTop && _padLeft == _padBottom;
    }
