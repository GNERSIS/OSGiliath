...
namespace osgWidget
{
    ...
    class BlinkCursorCallback : public osg::Drawable::DrawCallback
    {
        public:
                                ...
                                const osg::Drawable* drawable ) const
            {
                ...
                if( osg::Timer::instance()->delta_s( startTime, now ) >
                    ( _insertMode ? 0.125 : 0.25 ) )
                {
                    on        = !on;
                    startTime = now;
                }
        ...
        _mouseClickX( 0 )
    {
        ...
        _cursor->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        _cursor->setColor( 0.0F, 0.0F, 0.0F, 1.0F );

        _selection->setCanClone( false );
        _selection->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        setEventMask(
            ...
            EVENT_MASK_KEY |
            // For "click" focusing.
            EVENT_MOUSE_PUSH | EVENT_MASK_MOUSE_DRAG
        );
    ...
    void
    Input::_calculateSize( const XYCoord& /*size*/ )
    {
        // An Input cannot currently set it's own size RELIABLY until the osgText
        // implementation is dratiscally improved. I'm getting wildly crazy results. :(
   ...
   if(height > getHeight()) setHeight(osg::round(height));
    ...
    Input::_calculateCursorOffsets()
    {
        ...
        osgText::Text::TextureGlyphQuadMap& tgqm =
            const_cast<osgText::Text::TextureGlyphQuadMap&>(
    ...
    bool
    Input::focus( const WindowManager* )
    {
    ...
    bool
    Input::unfocus( const WindowManager* )
    {
    ...
    Input::positioned()
    {
        ...
        // point_type th = (_text->getCharacterHeight() * ln) + (_text->getLineSpacing()
        // * (ln - 1.0f));
        ...
        if( _insertMode )
        {
            ...
            else
            {
                ...
                // so get the advance for any character, perhaps a large one, I chose
                // 'A'.
                osgText::Glyph* glyph =
                    const_cast<osgText::Font*>( _text->getFont() )
                        ->getGlyph( osgText::FontResolution( _text->getFontWidth(),
                                                             _text->getFontHeight() ),
                                    'A' );
        ...
        unsigned int selectionMin = std::min( _selectionStartIndex, _selectionEndIndex );
        unsigned int selectionMax = std::max( _selectionStartIndex, _selectionEndIndex );
                      ...
                      const WindowManager* )
    {
        ...
        for( unsigned int i = 0; i < _offsets.size(); ++i )
        {
            ...
            if( ( x >= offset1 && x <= offset2 ) ||
                i ==
                _offsets.size() -
                1 )    // If we're at the last one, obviously it will be there.
            {
            ...
            case osgGA::GUIEventAdapter::KEY_Delete :
                {
                    unsigned int deleteMin =
                        std::min( _selectionStartIndex, _selectionEndIndex );
                    unsigned int deleteMax =
                        std::max( _selectionStartIndex, _selectionEndIndex );
...
// Data from clipboard
                    ...
                    if( !data.empty() )
                    {
                        ...
                        if( deleteMax - deleteMin > 0 )
                        {
                            data = data.substr(
                                0,
                                _maxSize - s.size() - ( deleteMax - deleteMin )
                            );

                            s.erase( s.begin() + deleteMin, s.begin() + deleteMax );
                            std::copy( data.begin(),
                                       data.end(),
                                       std::inserter( s, s.begin() + deleteMin ) );
                ...
                else if( ( ( key == 'c' || key == 'C' || key == 'x' || key == 'X' ) &&
                           ( mask & osgGA::GUIEventAdapter::MODKEY_CTRL ) ) ||
                         ( key == 3 ) ||
                         ( key == 24 ) )
                {
                    ...
                    if( selectionMax - selectionMin > 0 )
                    {
                        ...
                        std::copy( s.begin() + selectionMin,
                                   s.begin() + selectionMax,
...
// Data to clipboard
                        ...
                        {
                            ::EmptyClipboard();
                            int     wchar_count = MultiByteToWideChar( CP_UTF8,
                                                                       0,
                                                                       data.c_str(),
                                                                       -1,
                                                                       NULL,
    ...
    void
    Input::setCursor( Widget* )
    {
    }

    unsigned int
    Input::calculateBestYOffset( const std::string& s )
    {
        if( !_text->getFont() )
        {
            return 0;
        ...
        const osgText::FontResolution fr(
