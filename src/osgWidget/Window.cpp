...
namespace osgWidget
{
    ...
    bool
    callbackWindowTabFocus( Event& ev )
    {
    ...
    Window::EmbeddedWindow::EmbeddedWindow( const std::string& name,
                                            point_type         w,
                                            point_type         h ) :
        Inherit( name,
                 w,
    ...
    Window::EmbeddedWindow::EmbeddedWindow( const EmbeddedWindow& wiw,
                                            const osg::CopyOp&    co ) :
        Inherit( wiw,
                 co )
    {
    ...
    void
    Window::EmbeddedWindow::parented( Window* parent )
    {
        if( !_window.valid() )
        {
            return;
        ...
        {
            warn() << "EmbeddedWindow Widget [" << _name
                   << "] cannot embed itself in Window [" << _window->getName()
                   << "], since it is already a child of ["
                   << _window->_parent->getName() << "]" << std::endl;
        }
    }

    void
    Window::EmbeddedWindow::unparented( Window* )
    {
        if( _window.valid() )
        {
            _window->_parent = 0;
    ...
    void
    Window::EmbeddedWindow::unmanaged( WindowManager* wm )
    {
        _window->unmanaged( wm );
    }

    void
    Window::EmbeddedWindow::positioned()
    {
        if( !_window.valid() )
        {
            return;
    ...
    bool
    Window::EmbeddedWindow::setWindow( Window* win )
    {
        if( !win )
        {
            warn() << "EmbeddedWindow [" << _name << "] attempted to set a NULL Window."
                   << std::endl;
    ...
    void
    Window::EmbeddedWindow::updateSizeFromWindow()
    {
        ...
        _hAnchor( HA_NONE )
    {
        ...
        getOrCreateStateSet()->setAttributeAndModes( new osg::Scissor( 0, 0, 0, 0 ),
                                                     osg::StateAttribute::ON );
    }

    Window::Window( const Window&      window,
                    const osg::CopyOp& co ) :
        MatrixTransform( window,
                         co ),
        ...
        _visibleArea( window._visibleArea )
    {
        ...
        // eventually for things to work 100% properly. For example, some Geodes may
        // contain labels, etc. Also, any widget that doesn't support simple addWidget
        // probably won't work (Table?)
    ...
    // This is the method by which all Windows are redrawn/resized. Keep in mind that not
    // all Windows are required to absolutely honor a resize request, which is why we
    // call the _getWidthImplementation() and _getHeightImplementation() functions
    // instead of using the values passed in.
    bool
    Window::resize( point_type width,
                    point_type height )
    {
    ...
    bool
    Window::resizeAdd( point_type diffWidth,
                       point_type diffHeight )
    {
        if( _width.current +
            diffWidth <
            _width.minimum ||
            _height.current +
            diffHeight < _height.minimum )
        {
            warn() << "Window [" << _name << "] can't call resizeAdd() with the "
    ...
    bool
    Window::resizePercent( point_type width,
                           point_type height )
    {
        if( !_parent && !_wm )
        {
        ...
        if( !_parent )
        {
            return resize( _wm->getWidth() * ( width / 100.0F ),
                           _wm->getHeight() * ( height / 100.0F ) );
        }

        else
        {
            return resize( _parent->getWidth() * ( width / 100.0F ),
    ...
    Window::update()
    {
        ...
        if( ( _vAnchor != VA_NONE || _hAnchor != HA_NONE ) && !_parent && _wm )
        {
            ...
            {
                y = std::round( _wm->getHeight() / 2.0F );
            }
            ...
            {
                x = std::round( ( _wm->getWidth() - _width.current ) / 2.0F );
            }
        ...
        // WindowManager, and we need to determine the sorting method we want to use.
        if( _wm )
        {
        ...
        // Update the Window itself, setting it's matrix according to translate, rotate,
        // and scale values.
        osg::dmat4 r = osg::rotate( osg::radians( _r ), osg::dvec3( 0.0, 0.0, 1.0 ) );

        osg::dmat4 s = osg::scale( _s, _s, 1.0 );
        osg::dmat4 t = osg::translate( x - _visibleArea[0], y - _visibleArea[1], z );

        setMatrix( r * s * t );
    }

    void
    Window::_setWidthAndHeightUnknownSizeError( const std::string& size,
                                                point_type         val )
    {
        warn() << "Window [" << _name << "] doesn't know its " << size << " (" << val
               << ")." << std::endl;
    }

    void
    Window::_setWidthAndHeightNotPAError( const std::string& size,
                                          point_type         val )
    {
        warn() << "Window [" << _name
    ...
    // Since there is so much error-checking associated with setting the width and height
    // properly of a Window, this function attempts to abstract some of that tedium.
    void
    Window::_setWidthAndHeight()
    {
    ...
    void
    Window::_removeFromGeode( Widget* widget )
    {
        if( !widget )
        {
            return;
    ...
    // This is a somewhat complicated function designed to only be called by derived
    // classes, allowing them to insert Widgets (which can be added to the Window in any
    // way the derived class sees fit) into the REAL internal _objects container.
    // TODO: This doesn't handle insertion properly!!!
    bool
    Window::_setWidget( Widget* widget,
                        int     index )
    {
        if( !widget )
        {
        ...
        if( widget->_parent )
        {
            warn() << "Window [" << _name << "] attempted to parent Widget ["
                   << widget->getName() << "], which is already parented by ["
                   << widget->_parent->getName() << "]." << std::endl;
        ...
        if( index >= 0 && index >= static_cast<int>( size() ) )
        {
            warn() << "Window [" << _name
                   ...
                   << "] at position " << index
                   << ", but there is not enough space available." << std::endl;
    ...
    bool
    Window::_setVisible( bool visible )
    {
        if( !_wm )
        {
            return false;
    ...
    void
    Window::_setFocused( Widget* widget )
    {
        if( widget && _wm )
        {
            Event ev( _wm );
    ...
    void
    Window::_setStyled( Widget* widget )
    {
        if( !widget || !_wm )
        {
            return;
    ...
    void
    Window::_setParented( Widget* widget,
                          bool    setUnparented )
    {
        if( !widget )
        {
    ...
    void
    Window::_setManaged( Widget* widget,
                         bool    setUnmanaged )
    {
        if( !widget || !_wm )
        {
    ...
    Window::_getBackground() const
    {
        ...
        if( geode )
        {
            return dynamic_cast<Widget*>(
                const_cast<osg::Drawable*>( geode->getDrawable( 0 ) )
            );
        }
    ...
    void
    Window::_positionWidget( Widget*    widget,
                             point_type width,
                             point_type height )
    {
        ...
        if( widget->canFill() )
        {
            point_type nw = std::round( width - pr - pl );
            point_type nh = std::round( height - pt - pb );
        ...
        point_type ha = std::round( ( width - w - pl - pr ) / 2.0F );
        point_type va = std::round( ( height - h - pt - pb ) / 2.0F );
    ...
    bool
    Window::isXYWithinVisible( float x,
                               float y ) const
    {
        return ( x >= _visibleArea[0] && x <= ( _visibleArea[0] + _visibleArea[2] ) ) &&
               ( y >= _visibleArea[1] && y <= ( _visibleArea[1] + _visibleArea[3] ) );
    }

    void
    Window::setVisibleArea( int x,
                            int y,
                            int w,
                            int h )
    {
    ...
    // The topmost Window always has this method called, instead of the embedded window
    // directly.
    bool
    Window::setFocused( const Widget* widget )
    {
        // TODO: I've turned on the warn() here, but perhaps I shouldn't? I need to
        // define the conditions under which it's okay to call setFocus() with a NULL
        // widget.
        if( !widget )
        {
        ...
        if( i == end() )
        {
            ...
            {
                ConstIterator ii =
                    std::find( w->get()->begin(), w->get()->end(), widget );
        ...
        if( !found )
        {
            warn() << "Window [" << _name << "] couldn't find the Widget ["
                   << widget->getName() << "] in it's object list." << std::endl;
    ...
    bool
    Window::setFocused( const std::string& name )
    {
        ...
        if( !w1 )
        {
            // Just like above, we couldn't find the widget in the toplevel, so lets see
            // if one of our EmbeddedWindow objects has it. The difference here is that
            // we search by name.
            WindowList wl;
        ...
        if( !found )
        {
            warn() << "Window [" << _name << "] couldn't find a Widget named [" << name
    ...
    Window::setNextFocusable()
    {
        ...
        for( witr = focusList.begin(); witr != focusList.end(); ++witr )
        {
            if( *witr == _focused )
            {
                // found current focused widget, move to next widget the one we want to
                // focus on
                ++witr;
                break;
            }
        ...
        if( witr != focusList.end() )
        {
            _setFocused( witr->get() );
        }
        else
        {
            _setFocused( focusList.front().get() );
    ...
    XYCoord
    Window::getAbsoluteOrigin() const
    {
        XYCoord    xy( 0, 0 );
    ...
    Window::embed( const std::string& newName,
                   Widget::Layer      layer,
                   unsigned int       layerOffset )
    {
        EmbeddedWindow* ew =
            new EmbeddedWindow( newName.size() > 0 ? newName : _name + "Embedded",
                                getWidth(),
                                getHeight() );
    ...
    bool
    Window::getFocusList( WidgetList& wl ) const
    {
    ...
    bool
    Window::getEmbeddedList( WindowList& wl ) const
    {
        for( ConstIterator i = begin(); i != end(); i++ )
        {
            if( i->valid() )
    ...
    void
    Window::getParentList( WindowList& wl ) const
    {
    ...
    void
    Window::managed( WindowManager* wm )
    {
        _wm = wm;
    ...
    void
    Window::unmanaged( WindowManager* /*wm*/ )
    {
        for( Iterator i = begin(); i != end(); i++ )
        {
    ...
    bool
    Window::addWidget( Widget* widget )
    {
        return _setWidget( widget );
    }

    bool
    Window::insertWidget( Widget*      widget,
                          unsigned int pos )
    {
    ...
    bool
    Window::replaceWidget( Widget* /*oldWidget*/,
                           Widget* /*newWidget*/ )
    {
        return false;
    }
    ...
    unsigned int
    Window::addDrawableAndGetIndex( osg::Drawable* drawable )
    {
        osg::Geode* geode = _geode();
    ...
    unsigned int
    Window::addChildAndGetIndex( osg::Node* node )
    {
        if( addChild( node ) )
        {
    ...
    // hopefully self-explanatory. They simply wrap calls to _compare<>() with the proper
    // templates, and forward the optional iteration ranges...

    point_type
    Window::_getMinWidgetWidth( int begin,
                                int end,
                                int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetHeight( int begin,
                                 int end,
                                 int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetWidth( int begin,
                                int end,
                                int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetHeight( int begin,
                                 int end,
                                 int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetMinWidth( int begin,
                                   int end,
                                   int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetMinHeight( int begin,
                                    int end,
                                    int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetMinWidth( int begin,
                                   int end,
                                   int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetMinHeight( int begin,
                                    int end,
                                    int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetWidthTotal( int begin,
                                     int end,
                                     int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetHeightTotal( int begin,
                                      int end,
                                      int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetWidthTotal( int begin,
                                     int end,
                                     int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetHeightTotal( int begin,
                                      int end,
                                      int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetMinWidthTotal( int begin,
                                        int end,
                                        int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetMinHeightTotal( int begin,
                                         int end,
                                         int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetMinWidthTotal( int begin,
                                        int end,
                                        int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetMinHeightTotal( int begin,
                                         int end,
                                         int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetPadHorizontal( int begin,
                                        int end,
                                        int add ) const
    {
    ...
    point_type
    Window::_getMinWidgetPadVertical( int begin,
                                      int end,
                                      int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetPadHorizontal( int begin,
                                        int end,
                                        int add ) const
    {
    ...
    point_type
    Window::_getMaxWidgetPadVertical( int begin,
                                      int end,
                                      int add ) const
    {
    ...
    Window::Sizes
    Window::_getWidthImplementation() const
    {
        osg::box   bb = getGeode()->getBoundingBox();

        point_type w  = std::round( bb.xMax() - bb.xMin() );

        return Sizes( w, 0.0F );
    }

    Window::Sizes
    Window::_getHeightImplementation() const
    {
        osg::box   bb = getGeode()->getBoundingBox();

        point_type h  = std::round( bb.yMax() - bb.yMin() );
