...
namespace osgWidget
{
    ...
    bool
    Style::applyStyle( Widget* widget,
                       Reader  r )
    {
        std::string str;
        osg::vec2   vec2;
    ...
    bool
    Style::applyStyle( Window* window,
                       Reader  r )
    {
        osg::vec2 vec2;
        float     f;
    ...
    bool
    Style::applyStyle( Window::EmbeddedWindow*,
                       Reader /*r*/ )
    {
        return false;
    }
    ...
    Widget::Layer
    Style::strToLayer( const std::string& layer )
    {
        ...
        {
            warn() << "Unknown Layer name [" << layer << "]; using LAYER_MIDDLE."
                   << std::endl;
    ...
    Style::strToVAlign( const std::string& valign )
    {
        ...
        {
            warn() << "Unknown VAlign name [" << valign << "]; using VA_CENTER."
                   << std::endl;
    ...
    Style::strToHAlign( const std::string& halign )
    {
        ...
        {
            warn() << "Unknown HAlign name [" << halign << "]; using HA_CENTER."
                   << std::endl;
    ...
    Widget::CoordinateMode
    Style::strToCoordMode( const std::string& coordmode )
    {
        ...
        {
            warn() << "Unknown CoordMode name [" << coordmode << "]; using CM_ABSOLUTE."
                   << std::endl;
    ...
    bool
    Style::strToFill( const std::string& fill )
    {
        ...
        {
            warn() << "Unknown Fill name [" << fill << "]; using false." << std::endl;
    ...
    StyleManager::StyleManager( const StyleManager& manager,
                                const osg::CopyOp&  co ) :
        Inherit( manager,
                 co )
    {
        for( ConstIterator i = _styles.begin(); i != _styles.end(); i++ )
        {
            ...
            {
                _styles[i->first] =
                    new Style( *i->second.get(), osg::CopyOp::DEEP_COPY_ALL );
            }
        }
    }

    bool
    StyleManager::_applyStyleToObject( osg::Object*       obj,
                                       const std::string& style )
    {
        ...
        {
            return _coerceAndApply<Widget>( obj, style, c );
        }

        else if( !std::string( "Label" ).compare( c ) )
        {
            return _coerceAndApply<Label>( obj, style, c );
        }

        else if( !std::string( "Box" ).compare( c ) )
        {
            return _coerceAndApply<Box>( obj, style, c );
        }
        ...
        {
            return _coerceAndApply<Window>( obj, style, c );
        }
        else if( !std::string( "Input" ).compare( c ) )
        {
            return _coerceAndApply<Input>( obj, style, c );
        }
        else if( !std::string( "Corner" ).compare( c ) )
        {
            return _coerceAndApply<Frame::Corner>( obj, style, c );
        }
        else if( !std::string( "Border" ).compare( c ) )
        {
            return _coerceAndApply<Frame::Border>( obj, style, c );
        }
        else if( !std::string( "EmbeddedWindow" ).compare( c ) )
        {
            return _coerceAndApply<Window::EmbeddedWindow>( obj, style, c );
        }

        else
        {
            warn() << "StyleManager does not support coercion of objects of type " << c
                   << "." << std::endl;
        }
