...
namespace osgWidget
{
    ...
    namespace py
    {
        ...
        // TODO: Until I can find a way to move data around inside of the context, this
        // will have to do. I don't really like it, but I've got no choice.
        static PyObject* G_ERR = 0;

        PyObject*
        newWindow( PyObject* self,
                   PyObject* args )
        {
            PyObject*   buffer = 0;
            const char* name   = 0;
            ...
            /*
            if(PyArg_ParseTuple(args, "sO!ii", &name, &PyBuffer_Type, &buffer, &width,
            &height)) { const void* buf = 0; int         len = 0;
        ...
        static PyMethodDef methods[] = {
            {"newWindow", newWindow, METH_VARARGS, "docstring"},
            {          0,         0,            0,           0}
        };
    ...
    // A helper function for all those cases where we need to inform the user that there
    // isn't a LUA engine available.
    bool
    noPythonFail( const std::string& err )
    {
    ...
    bool
    PythonEngine::eval( const std::string& code )
    {
#ifdef OSGWIDGET_USEPYTHON
        PyObject* r =
            PyRun_String( code.c_str(), Py_file_input, _data->main, _data->main );
    ...
    bool
    PythonEngine::runFile( const std::string& filePath )
    {
#ifdef OSGWIDGET_USEPYTHON
        if( !osgDB::fileExists( filePath ) )
        {
            warn() << "Couldn't find file \"" << filePath << "\" for PythonEngine."
                   << std::endl;
        ...
        PyObject* r =
            PyRun_File( f, filePath.c_str(), Py_file_input, _data->main, _data->main );
