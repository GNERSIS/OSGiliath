...
namespace osgWidget
{
    ...
    )
    {
        ...
        if( ma )
        {
            ...
            // osgWidget assumes origin is bottom left of window so make sure mouse
            // coordinate are increaseing y upwards and are scaled to window size.
            float x = ( gea.getX() - gea.getXmin() ) /
                      ( gea.getXmax() - gea.getXmin() ) *
                      static_cast<float>( gea.getWindowWidth() );
            float y = ( gea.getY() - gea.getYmin() ) /
                      ( gea.getYmax() - gea.getYmin() ) *
                      static_cast<float>( gea.getWindowHeight() );
            if( gea.getMouseYOrientation() ==
                osgGA::GUIEventAdapter::Y_INCREASING_DOWNWARDS )
            {
                y = static_cast<float>( gea.getWindowHeight() - 1 ) - y;
            }
    ...
    bool
    MouseHandler::_handleMousePush( float x,
                                    float y,
                                    int   button )
    {
        if( button == osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON )
        {
            return _doMouseEvent( x, y, &WindowManager::mousePushedLeft );
        }

        else if( button == osgGA::GUIEventAdapter::RIGHT_MOUSE_BUTTON )
        {
            return _doMouseEvent( x, y, &WindowManager::mousePushedRight );
        }

        else if( button == osgGA::GUIEventAdapter::MIDDLE_MOUSE_BUTTON )
        {
            return _doMouseEvent( x, y, &WindowManager::mousePushedMiddle );
        }
    ...
    bool
    MouseHandler::_handleMouseRelease( float x,
                                       float y,
                                       int   button )
    {
        if( button == osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON )
        {
            return _doMouseEvent( x, y, &WindowManager::mouseReleasedLeft );
        }

        else if( button == osgGA::GUIEventAdapter::RIGHT_MOUSE_BUTTON )
        {
            return _doMouseEvent( x, y, &WindowManager::mouseReleasedRight );
        }

        else if( button == osgGA::GUIEventAdapter::MIDDLE_MOUSE_BUTTON )
        {
            return _doMouseEvent( x, y, &WindowManager::mouseReleasedMiddle );
        }
    ...
    bool
    MouseHandler::_handleMouseDoubleClick( float /*x*/,
                                           float /*y*/,
                                           int /*button*/ )
    {
        return false;
    ...
    bool
    MouseHandler::_handleMouseDrag( float x,
                                    float y,
                                    int /*button*/ )
    {
    ...
    bool
    MouseHandler::_handleMouseMove( float x,
                                    float y,
                                    int /*button*/ )
    {
    ...
    bool
    MouseHandler::_handleMouseScroll( float x,
                                      float y,
                                      int )
    {
    ...
    MouseHandler::_isMouseEvent( osgGA::GUIEventAdapter::EventType ev ) const
    {
        if( ev == osgGA::GUIEventAdapter::PUSH )
        {
            return &MouseHandler::_handleMousePush;
        }

        else if( ev == osgGA::GUIEventAdapter::RELEASE )
        {
            return &MouseHandler::_handleMouseRelease;
        }

        else if( ev == osgGA::GUIEventAdapter::DOUBLECLICK )
        {
            return &MouseHandler::_handleMouseDoubleClick;
        }

        else if( ev == osgGA::GUIEventAdapter::DRAG )
        {
            return &MouseHandler::_handleMouseDrag;
        }

        else if( ev == osgGA::GUIEventAdapter::MOVE )
        {
            return &MouseHandler::_handleMouseMove;
        }

        else if( ev == osgGA::GUIEventAdapter::SCROLL )
        {
            return &MouseHandler::_handleMouseScroll;
        }
    ...
    bool
    MouseHandler::_doMouseEvent( float      x,
                                 float      y,
                                 MouseEvent me )
    {
        bool handled = ( _wm.get()->*me )( x, y );

        // This is called LAST for things like drag, which needs to calculate a mouse
        // difference.
    ...
    ResizeHandler::ResizeHandler( WindowManager* wm,
                                  osg::Camera*   camera ) :
        _wm( wm ),
        _camera( camera )
    {
    ...
    )
    {
        ...
        osg::dmat4::value_type w = gea.getWindowWidth();
        osg::dmat4::value_type h = gea.getWindowHeight();

        if( _camera.valid() )
        {
            _camera->setProjectionMatrix( osg::ortho2D( 0.0, w, 0.0, h ) );
    ...
    CameraSwitchHandler::CameraSwitchHandler( WindowManager* wm,
                                              osg::Camera*   camera ) :
        _wm( wm ),
        _camera( camera )
    {
    ...
    )
    {
        ...
        if( !oldTrans )
        {
            ...
            mt->setMatrix( osg::translate( width / 2.0, 0.0, 0.0 ) *
                           osg::scale( 1.0, 1.0, scale ) *
                           osg::rotate( osg::radians( 45.0 ), 0.0, 1.0, 0.0 ) );

            mt->addChild( _wm.get() );
            mt->getOrCreateStateSet()->setMode( GL_LIGHTING,
                                                osg::StateAttribute::PROTECTED |
                                                    osg::StateAttribute::OFF );
            mt->getOrCreateStateSet()->setMode( GL_SCISSOR_TEST,
            ...
            mm->setHomePosition(
                // eye
                osg::dvec3( width / 2.0, height, 100.0 ),
                // center
                osg::dvec3( 0.0, 0.0, -( scale / 2.0 ) ),
                // up
