...
namespace osgWidget
{
        ...
        _textIndex( 0 )
    {
        ...
        _text->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    void
    Label::_calculateSize( const XYCoord& size )
    {
    ...
    void
    Label::parented( Window* parent )
    {
        osg::Geode*    geode = parent->getGeode();

        // If we've been cloned, use the index of the old text Drawable if it's already
        // there. However, we have a problem here: imagine a Label gets cloned AFTER
        // being added to a Window; it'll have a _textIndex, but that _textIndex won't
        // apply to the currently cloned object. In this case, we'll need to check to be
        // SURE.
        osgText::Text* text =
            dynamic_cast<osgText::Text*>( geode->getDrawable( _textIndex ) );
    ...
    void
    Label::unparented( Window* parent )
    {
        if( _textIndex )
        {
    ...
    Label::positioned()
    {
        ...
        /*
        warn() << "Label widget size : " << getWidth() << " x " << getHeight() <<
        std::endl; warn() << "Label widget tsize: " << getWidthTotal() << " x " <<
        getHeightTotal() << std::endl; warn() << "Label XY coords   : " << getX() << " x
        " << getY() << std::endl; warn() << "Label BB in size  : " << size.x << " x " <<
        size.y << std::endl; warn() << "Label xy position : " << x << " y " << y <<
        std::endl; warn() << "------------------------------------" << std::endl;
        */
    ...
    void
    Label::setLabel( const osgText::String& label )
    {
        _text->setText( label );
    ...
    void
    Label::setFontColor( const Color& c )
    {
        _text->setColor( c );
    }

    void
    Label::setShadow( point_type offset )
    {
    ...
    {
        const osg::box& bb = _text->getBoundingBox();

        return XYCoord( osg::round( bb.xMax() - bb.xMin() ),
