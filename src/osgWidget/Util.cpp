...
namespace osgWidget
{
    ...
    std::string
    generateRandomName( const std::string& base )
    {
    ...
    osg::Camera*
    createOrthoCamera( matrix_type width,
                       matrix_type height )
    {
        ...
        camera->setProjectionMatrix(
            osg::ortho2D( 0.0, ( double )width, 0.0, ( double )height )
        );
    ...
    int
    createExample( osgViewer::Viewer& viewer,
                   WindowManager*     wm,
                   osg::Node*         node )
    {
        if( !wm )
    ...
    bool
    writeWindowManagerNode( WindowManager* wm )
    {
