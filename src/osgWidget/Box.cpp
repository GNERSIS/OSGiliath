...
namespace osgWidget
{
    ...
    void
    Box::_resizeImplementation( point_type w,
                                point_type h )
    {
        // Get the number of Widgets that agree to fill. Also perform some casting to
        // integers in case we're being request to resize with pixel perfection.
        ...
        // into account the padding, and will be affected by any resizing that occurred
        // above.
    ...
    Window::Sizes
    Box::_getWidthImplementation() const
    {
        ...
        if( _boxType == HORIZONTAL )
        {
            ...
            if( _uniform )
            {
                return Sizes( _getMaxWidgetWidthTotal() * size(),
                              _getMaxWidgetMinWidthTotal() * size() );
            }

            // Othweriwse, our width is all of the widths added together, and our
            // minWidth is all of the minWidths added together.
            else
            {
                return Sizes( _accumulate<Plus>( &Widget::getWidthTotal ),
                              _accumulate<Plus>( &Widget::getMinWidthTotal ) );
            }
        }

        // If we're a vertical Box, our width is the width of the larget Widget in the
        // group. Our minWidth is the largest minWidth of the Widgets in the group.
        else
        {
            return Sizes( _getMaxWidgetWidthTotal(), _getMaxWidgetMinWidthTotal() );
        }
    }

    Window::Sizes
    Box::_getHeightImplementation() const
    {
        if( _boxType == VERTICAL )
        {
            if( _uniform )
            {
                return Sizes( _getMaxWidgetHeightTotal() * size(),
                              _getMaxWidgetMinHeightTotal() * size() );
            }

            else
            {
                return Sizes( _accumulate<Plus>( &Widget::getHeightTotal ),
        ...
        {
            return Sizes( _getMaxWidgetHeightTotal(), _getMaxWidgetMinHeightTotal() );
        }
    }
