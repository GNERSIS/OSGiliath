...
bool
isBasicRootNode( const osg::Node& node )
{
    const osg::Group* osgGroup = node.asGroup();
    if( !osgGroup ||
        node.asGeode() )    // WriterNodeVisitor handles Geodes the "old way" (= Derived
                            // from Node, not Group as for now). Geodes may be considered
                            // "basic root nodes" when WriterNodeVisitor will be adapted.
    {
    ...
    if( transform )
    {
        if( const osg::MatrixTransform* matrixTransform =
                     ...
                     transform->asPositionAttitudeTransform() )
        {
            if( pat->getPosition() !=
                ...
                osg::quat() ||
                pat->getScale() !=
                osg::dvec3( 1.0F, 1.0F, 1.0F ) ||
                pat->getPivotPoint() != osg::dvec3() )
            {
...
class CleanUpFbx
{
    ...
    public:
        ...
        explicit CleanUpFbx( FbxManager* pSdkManager ) :
...
void
findLinkedFbxSkeletonNodes( FbxNode*                  pNode,
                            std::set<const FbxNode*>& fbxSkeletons )
{
    if( const FbxGeometry* pMesh = FbxCast<FbxGeometry>( pNode->GetNodeAttribute() ) )
    {
        ...
        {
            const FbxSkin* pSkin =
                ( const FbxSkin* )pMesh->GetDeformer( i, FbxDeformer::eSkin );
...
void
resolveBindMatrices( osg::Node&                  root,
                     const BindMatrixMap&        boneBindMatrices,
                     const std::map<FbxNode*,
                                    osg::Node*>& nodeMap )
{
    std::set<std::string> nodeNames;
    for( std::map<FbxNode*, osg::Node*>::const_iterator it = nodeMap.begin();
         it != nodeMap.end();
         ++it )
    {
         ...
         ++it )
    {
        ...
        {
            osgAnimation::Bone* originalBone =
                dynamic_cast<osgAnimation::Bone*>( nodeIt->second );

            // Iterate bind matrices and create new bones if needed
            const BindMatrixGeometryMap& bindMatrixGeom = it->second;
            for( BindMatrixGeometryMap::const_iterator bindIt = bindMatrixGeom.begin();
                 bindIt != bindMatrixGeom.end();
                 ++bindIt )
            {
                ...
                else
                {
                    ...
                    for( std::set<osgAnimation::RigGeometry*>::const_iterator rigIt =
                             bindIt->second.begin();
                         ...
                         ++rigIt )
                    {
                        ...
                        osgAnimation::VertexInfluenceMap* vertexInfluences =
                            pRigGeometry->getInfluenceMap();

                        osgAnimation::VertexInfluenceMap::iterator vimIt =
                            vertexInfluences->find( originalBone->getName() );
                        if( vimIt != vertexInfluences->end() )
                        {
                            osgAnimation::VertexInfluence vi;
                            vi.swap( vimIt->second );
                            vertexInfluences->erase( vimIt );
                            osgAnimation::VertexInfluence& vi2 =
                                ( *vertexInfluences )[name];
                            vi.swap( vi2 );
                            vi2.setName( name );
                        }
                        else
                        {
                            OSG_WARN << "No vertex influences found for \""
                                     << originalBone->getName() << "\"" << std::endl;
                        }
                    }
                }
            }
        ...
        {
            OSG_WARN << "No bone found for \"" << fbxBone->GetName() << "\""
                     << std::endl;
            ++it;
        }
    }
                           ...
                           const Options*     options ) const
{
    try
    {
        ...
        std::string utf8filename(
            osgDB::convertStringFromCurrentCodePageToUTF8( filename )
        );
#endif
        ...
        // FbxAxisSystem::OpenGL.ConvertScene(pScene);        // Doesn't work as
        // expected. Still need to transform vertices.
        ...
        if( FbxNode* pNode = pScene->GetRootNode() )
        {
            ...
            else
            {
                ...
                localOptions->setObjectCacheHint(
                    osgDB::ReaderWriter::Options::CACHE_IMAGES
                );
            }

            std::string              filePath = osgDB::getFilePath( filename );
            FbxMaterialToOsgStateSet fbxMaterialToOsgStateSet( filePath,
                                                               localOptions.get(),
                                                               lightmapTextures );
            ...
            if( FbxDocumentInfo* pDocInfo = pScene->GetDocumentInfo() )
            {
                ...
                for( unsigned int i = 0;
                     i < sizeof( authoringTools ) / sizeof( authoringTools[0] );
                     ++i )
                {
                    if( 0 ==
#ifdef _WIN32
...
#endif
                        ( appName,
                          authoringTools[i].name,
                          strlen( authoringTools[i].name ) ) )
                    {
            ...
            if( res.success() )
            {
                ...
                resolveBindMatrices( *res.getNode(),
                                     reader.boneBindMatrices,
                                     reader.nodeMap );
                ...
                if( refCoordSysChange )
                {
                    ...
                    osg::MatrixTransform* pMatrixTransform =
                        pTransformTemp ? pTransformTemp->asMatrixTransform() : NULL;
                    if( pMatrixTransform )
                    {
                        pMatrixTransform->setMatrix( pMatrixTransform->getMatrix() *
                                                     mat );
                    }
                    else
                    {
                        ...
                        {
                            // If root node is a simple group, put all FBX elements under
                            // the OSG root
                            osg::Group* osgGroup = osgNode->asGroup();
                            for( unsigned int i = 0; i < osgGroup->getNumChildren();
                                 ++i )
                            {
    ...
    {
        OSG_WARN << "Exception thrown while importing \"" << filenameInit << '\"'
                 << std::endl;
    }
...
ReaderWriterFBX::writeNode( const osg::Node&   node,
                            ...
                            const Options*     options ) const
{
    try
    {
        ...
        osg::ref_ptr<Options> localOptions =
            options
        ...
        if( options )
        {
            ...
            {
                float unit = *static_cast<const float*>(
                    options->getPluginData( "FBX-AssetUnitMeter" )
                );
        ...
        pluginfbx::WriterNodeVisitor writerNodeVisitor(
            pScene,
            pSdkManager,
            filename,
            options,
            osgDB::getFilePath( node.getName().empty() ? filename : node.getName() )
        );
        if( useFbxRoot && isBasicRootNode( node ) )
        {
            ...
            {
                const_cast<osg::Node*>( osgGroup->getChild( child ) )
                    ->accept( writerNodeVisitor );
            }
        }
        else
        {
        ...
        {
            OSG_NOTICE << "Can't create directory for file '" << filename
                       << "'. FBX SDK may fail creating the file." << std::endl;
        }
        ...
        // GetReaderFormatCount() gives the following list:
        // FBX binary (*.fbx)
        // FBX ascii (*.fbx)
        // FBX encrypted (*.fbx)
        // FBX 6.0 binary (*.fbx)
        ...
        // Acclaim ASF (*.asf)
        int format = ascii
                       ? pSdkManager->GetIOPluginRegistry()->FindWriterIDByDescription(
                             "FBX ascii (*.fbx)"
                         )
                       : -1;    // -1 = Default
        ...
        if( !exportVersion.empty() &&
            !lExporter->SetFileExportVersion( FbxString( exportVersion.c_str() ),
                                              FbxSceneRenamer::eNone ) )
        {
            ...
            OSG_WARN << "Can't set FBX export version to '" << exportVersion
                     << "'. Using default. Available export versions are:"
                     << versionsStr.str() << std::endl;
        }
