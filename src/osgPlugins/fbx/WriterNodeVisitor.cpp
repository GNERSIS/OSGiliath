...
namespace pluginfbx
{
    ...
    /** writes all primitives of a primitive-set out to a stream, decomposes quads to
     * triangles, line-strips to lines etc */
    class PrimitiveIndexWriter : public osg::PrimitiveIndexFunctor
    {
        public:
                ...
                _mesh( 0 )
            {
                ...
                {
                    _normalBinding =
                        osg::Geometry::BIND_OFF;    // Turn off binding if there is no
                                                    // normal data
                }
                reset();
            }
        ...
        protected:
            ...
            template<typename T>
            void
            drawElementsImplementation( GLenum   mode,
                                        GLsizei  count,
                                        const T* indices )
            {
        ...
        private:
            ...
            PrimitiveIndexWriter&
            operator=( const PrimitiveIndexWriter& );    // { return *this; }
    ...
    void
    PrimitiveIndexWriter::drawArrays( GLenum  mode,
                                      GLint   first,
                                      GLsizei count )
    {
    ...
    WriterNodeVisitor::Material::Material( WriterNodeVisitor&         writerNodeVisitor,
        ...
        _osgImage( NULL )
    {
        osg::vec4 diffuse( 1, 1, 1, 1 ), ambient( 0.2, 0.2, 0.2, 1 ),
            specular( 0, 0, 0, 1 ), emission( 1, 1, 1, 1 );
        ...
        if( mat )
        {
            ...
            const osg::StateAttribute* attribute =
                stateset->getAttribute( osg::StateAttribute::Type::CULLFACE );
            if( attribute )
            {
                assert( dynamic_cast<const osg::CullFace*>( attribute ) );
                osg::CullFace::Mode mode =
                    static_cast<const osg::CullFace*>( attribute )->getMode();
                if( mode == osg::CullFace::Mode::FRONT )
                {
                    OSG_WARN
                        << "FBX Writer: Reversed face (culled FRONT) not supported yet."
                        << std::endl;
                }
                else if( mode != osg::CullFace::Mode::BACK )
                {
                    assert( mode == osg::CullFace::Mode::FRONT_AND_BACK );
                    OSG_WARN << "FBX Writer: Invisible face (culled FRONT_AND_BACK) not "
                                "supported yet."
                             << std::endl;
                }
            }
            ...
            _fbxMaterial =
                FbxSurfacePhong::Create( pSdkManager, mat->getName().c_str() );
            if( _fbxMaterial )
            {
                _fbxMaterial->DiffuseFactor.Set( 1 );
                _fbxMaterial->Diffuse.Set(
                    FbxDouble3( diffuse.x, diffuse.y, diffuse.z )
                );

                _fbxMaterial->TransparencyFactor.Set( transparency );

                _fbxMaterial->Ambient.Set(
                    FbxDouble3( ambient.x, ambient.y, ambient.z )
                );

                _fbxMaterial->Emissive.Set(
                    FbxDouble3( emission.x, emission.y, emission.z )
                );

                _fbxMaterial->Specular.Set(
                    FbxDouble3( specular.x, specular.y, specular.z )
                );
        ...
        if( tex && tex->getImage( 0 ) )
        {
            ...
            {
                _fbxMaterial =
                    FbxSurfacePhong::Create( pSdkManager, relativePath.c_str() );
            }
            // Connect texture to material's diffuse
            // Note there should be no reason FbxSurfacePhong::Create() would return
            // NULL, but as previous code made this secirity test, here we keep the same
            // way.
            if( _fbxMaterial )
            {
    ...
    int
    WriterNodeVisitor::processStateSet( const osg::StateSet* ss )
    {
        // OSG_ALWAYS << "Trying Adding " <<
        // ss->getAttribute(osg::StateAttribute::Type::MATERIAL)->getName() << std::endl;
    ...
    unsigned int
    addPolygon( MapIndices&  index_vert,
    ...
    void
    addPolygon( FbxMesh*        mesh,
                MapIndices&     index_vert,
                const Triangle& tri,
                unsigned int    drawableNum )
    {
        mesh->AddPolygon(
            addPolygon( index_vert, tri.t1, tri.normalIndex1, drawableNum )
        );
        mesh->AddPolygon(
            addPolygon( index_vert, tri.t2, tri.normalIndex2, drawableNum )
        );
        mesh->AddPolygon(
            addPolygon( index_vert, tri.t3, tri.normalIndex3, drawableNum )
        );
    }
    ...
    {
        FbxLayerElementTexture* lTextureDiffuseLayer =
            FbxLayerElementTexture::Create( mesh, "Diffuse" );
        ...
        FbxLayerElementMaterial* lMaterialLayer =
            FbxLayerElementMaterial::Create( mesh, "materialLayer" );
        ...
        for( MaterialMap::iterator it = _materialMap.begin(); it != _materialMap.end();
             ++it )
        {
        ...
        mesh->GetLayer( 0 )->SetTextures( FbxLayerElement::eTextureDiffuse,
                                                       ...
                                                       FbxMesh*            mesh )
    {
        ...
        FbxLayerElementNormal* lLayerElementNormal =
            FbxLayerElementNormal::Create( mesh, "" );
        ...
        FbxLayerElementUV* lUVDiffuseLayer =
            FbxLayerElementUV::Create( mesh, "DiffuseUV" );
        ...
        for( MapIndices::iterator it = index_vert.begin(); it != index_vert.end(); ++it )
        {
            ...
            {
                const osg::vec3& vec =
                    ( *static_cast<const osg::Vec3Array*>( basevecs ) )[vertexIndex];
                vertex.Set( vec.x, vec.y, vec.z );
            }
            else if( basevecs->getType() == osg::Array::Vec3dArrayType )
            {
                const osg::dvec3& vec =
                    ( *static_cast<const osg::Vec3dArray*>( basevecs ) )[vertexIndex];
                vertex.Set( vec.x, vec.y, vec.z );
            }
            else
            {
                OSG_NOTIFY( osg::FATAL )
                    << "Vertex array is not vec3 or dvec3. Not implemented" << std::endl;
                throw "Vertex array is not vec3 or dvec3. Not implemented";
                //_succeeded = false;
                // return;
            }
            ...
            if( basenormals && basenormals->getNumElements() > 0 )
            {
                ...
                {
                    const osg::dvec3& vec = ( *static_cast<const osg::Vec3dArray*>(
                        basenormals
                    ) )[normalIndex];
                    normal.Set( vec.x, vec.y, vec.z, 0 );
                }
                ...
                {
                    OSG_NOTIFY( osg::FATAL )
                        << "Normal array is not vec3 or dvec3. Not implemented"
                        << std::endl;
                    throw "Normal array is not vec3 or dvec3. Not implemented";
                    //_succeeded = false;
                    // return;
                }
            ...
            if( texcoords )
            {
                ...
                if( basetexcoords && basetexcoords->getNumElements() > 0 )
                {
                    ...
                    {
                        const osg::dvec2& vec = ( *static_cast<const osg::Vec2dArray*>(
                            basetexcoords
                        ) )[vertexIndex];
                        texcoord.Set( vec.x, vec.y );
                    }
                    else
                    {
                        OSG_NOTIFY(
                            osg::FATAL
                        ) << "Texture coords array is not vec2 or dvec2. Not implemented"
                          << std::endl;
                        throw "Texture coords array is not vec2 or dvec2. Not "
                              "implemented";
                        //_succeeded = false;
                        // return;
                    }
    ...
    void
    WriterNodeVisitor::buildFaces( const std::string&  name,
                                   ...
                                   bool                texcoords )
    {
        ...
        lLayer->GetTextures( FbxLayerElement::eTextureDiffuse )
            ->GetIndexArray()
            .SetCount( listTriangles.size() );
        lLayer->GetMaterials()->GetIndexArray().SetCount( listTriangles.size() );

        unsigned int i = 0;
        for( ListTriangle::iterator it = listTriangles.begin();
             it != listTriangles.end();
             ++it, ++i )    // Go through the triangle list to define meshs
        {
    ...
    void
    WriterNodeVisitor::createListTriangle( const osg::Geometry* geo,
                                           ListTriangle&        listTriangles,
                                           bool&                texcoords,
                                           unsigned int         drawable_n )
    {
        unsigned int nbVertices = 0;
        {
            ...
            if( vecs )
            {
                ...
                if( texvec )
                {
                    ...
                    {
                        OSG_WARN << "There are more/less texture coords than vertices! "
    ...
    void
    WriterNodeVisitor::apply( osg::Geometry& geometry )
    {
    ...
    void
    WriterNodeVisitor::apply( osg::Group& node )
    {
        if( _firstNodeProcessed )
        {
            ...
            FbxNode* nodeFBX =
                FbxNode::Create( _pSdkManager,
                                 node.getName().empty() ? "DefaultName"
                                                        : node.getName().c_str() );
    ...
    void
    WriterNodeVisitor::apply( osg::MatrixTransform& node )
    {
        FbxNode* parent = _curFbxNode;
        _curFbxNode = FbxNode::Create( _pSdkManager,
                                       node.getName().empty() ? "DefaultName"
                                                              : node.getName().c_str() );
        parent->AddChild( _curFbxNode );

        const osg::dmat4& matrix = node.getMatrix();
        osg::dvec3        pos, scl;
        osg::quat         rot, so;

        osg::decompose( matrix, pos, rot, scl, so );
