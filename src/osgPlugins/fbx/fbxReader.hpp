...
typedef std::map<osg::dmat4, std::set<osgAnimation::RigGeometry*>> BindMatrixGeometryMap;
...
class OsgFbxReader
{
    public:
        ...
        OsgFbxReader( FbxManager&                     pSdkManager1,
                      FbxScene&                       fbxScene1,
        ...
        osgDB::ReaderWriter::ReadResult
        readFbxCamera( FbxNode* pNode );

        osgDB::ReaderWriter::ReadResult
        readFbxLight( FbxNode* pNode,
                      int&     nLightCount );

        osgDB::ReaderWriter::ReadResult
        readMesh( FbxNode*                      pNode,
                  FbxMesh*                      fbxMesh,
                  std::vector<StateSetContent>& stateSetList,
                  const char*                   szName );

        osgDB::ReaderWriter::ReadResult
        readFbxMesh( FbxNode* pNode,
