#include "fbxMaterialToOsgStateSet.hpp"
...
bool
TextureDetails::transparent() const
{
...
void
TextureDetails::assignTextureIfRequired( osg::StateSet* stateSet,
                                         unsigned int   unit )
{
    if( !texture )
    {
...
static osg::Texture::WrapMode
convertWrap( FbxFileTexture::EWrapMode wrap )
{
    return wrap == FbxFileTexture::eRepeat ? osg::Texture2D::REPEAT
...
FbxMaterialToOsgStateSet::convert( const FbxSurfaceMaterial* pFbxMat )
{
    ...
    // diffuse map...
    result.diffuse =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sDiffuse ) );

    // opacity map...
    double            transparencyColorFactor    = 1.0;
    bool              useTransparencyColorFactor = false;

    const FbxProperty lOpacityProperty =
        pFbxMat->FindProperty( FbxSurfaceMaterial::sTransparentColor );
    if( lOpacityProperty.IsValid() )
    {
        FbxDouble3 transparentColor = lOpacityProperty.Get<FbxDouble3>();
        // If transparent color is defined set the transparentFactor to gray scale value
        // of transparentColor
        if( transparentColor[0] <
            1.0 ||
            transparentColor[1] <
            1.0 ||
            transparentColor[2] < 1.0 )
        {
            transparencyColorFactor    = transparentColor[0] *
                                         0.30 +
                                         transparentColor[1] *
                                         0.59 +
                                         transparentColor[2] *
    ...
    // reflection map...
    result.reflection =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sReflection ) );

    // emissive map...
    result.emissive =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sEmissive ) );

    // ambient map...
    result.ambient =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sAmbient ) );

    // normal map...
    result.normalMap =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sNormalMap ) );

    // specular map...
    result.specular =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sSpecular ) );

    // shininess map...
    result.shininess =
        selectTextureDetails( pFbxMat->FindProperty( FbxSurfaceMaterial::sShininess ) );

    if( pFbxLambert )
    {
        FbxDouble3 color              = pFbxLambert->Diffuse.Get();
        double     factor             = pFbxLambert->DiffuseFactor.Get();
        double     transparencyFactor = useTransparencyColorFactor
                                          ? transparencyColorFactor
                                          : pFbxLambert->TransparencyFactor.Get();
        pOsgMat->setDiffuse( osg::Material::FRONT_AND_BACK,
                             osg::vec4( static_cast<float>( color[0] * factor ),
        ...
        pOsgMat->setAmbient( osg::Material::FRONT_AND_BACK,
                             osg::vec4( static_cast<float>( color[0] * factor ),
                                        static_cast<float>( color[1] * factor ),
                                        static_cast<float>( color[2] * factor ),
                                        1.0F ) );
        ...
        pOsgMat->setEmission( osg::Material::FRONT_AND_BACK,
                              osg::vec4( static_cast<float>( color[0] * factor ),
                                         static_cast<float>( color[1] * factor ),
                                         static_cast<float>( color[2] * factor ),
                                         1.0F ) );
        ...
        // get maps factors...
        if( result.diffuse.valid() )
        {
            result.diffuse->factor = pFbxLambert->DiffuseFactor.Get();
        }
        if( result.emissive.valid() )
        {
            result.emissive->factor = pFbxLambert->EmissiveFactor.Get();
        }
        if( result.ambient.valid() )
        {
            result.ambient->factor = pFbxLambert->AmbientFactor.Get();
        }
        if( result.normalMap.valid() )
        {
            result.normalMap->factor = pFbxLambert->BumpFactor.Get();
        }
        ...
        if( const FbxSurfacePhong* pFbxPhong = FbxCast<FbxSurfacePhong>( pFbxLambert ) )
        {
            ...
            pOsgMat->setSpecular( osg::Material::FRONT_AND_BACK,
                                  osg::vec4( static_cast<float>( color[0] * factor ),
                                             static_cast<float>( color[1] * factor ),
                                             static_cast<float>( color[2] * factor ),
                                             1.0F ) );
            // Since Maya and 3D studio Max stores their glossiness values in exponential
            // format (2^(log2(x)) We need to linearize to values between 0-100 and then
            // scale to values between 0-128. Glossiness values above 100 will result in
            // shininess larger than 128.0 and will be clamped
            double shininess =
                ( 64.0 * log( pFbxPhong->Shininess.Get() ) ) / ( 5.0 * log( 2.0 ) );
            ...
            {
                result.reflection->factor = pFbxPhong->ReflectionFactor.Get();
            }
            if( result.specular.valid() )
            {
                result.specular->factor = pFbxPhong->SpecularFactor.Get();
            }
...
FbxMaterialToOsgStateSet::fbxTextureToOsgTexture( const FbxFileTexture* fbx )
{
    ...
    if( osgDB::fileExists(
            osgDB::concatPaths( _dir, fbx->GetFileName() )
        ) )    // First try "export dir/name"
    {
        filename = osgDB::concatPaths( _dir, fbx->GetFileName() );
    }
    else if( osgDB::fileExists(
                 fbx->GetFileName()
             ) )    // Then try "name" (if absolute)
    {
        filename = fbx->GetFileName();
                 ...
                 osgDB::concatPaths( _dir, fbx->GetRelativeFileName() )
             ) )    // Else try  "current dir + relative filename"
    {
        filename = osgDB::concatPaths( _dir, fbx->GetRelativeFileName() );
    }
    else if( osgDB::fileExists(
                 osgDB::concatPaths( _dir,
                                     osgDB::getSimpleFileName( fbx->GetFileName() ) )
             ) )    // Else try "current dir + simple filename"
    {
        filename =
            osgDB::concatPaths( _dir, osgDB::getSimpleFileName( fbx->GetFileName() ) );
    }
    else
    {
...
FbxFileTexture*
FbxMaterialToOsgStateSet::selectFbxFileTexture( const FbxProperty& lProperty )
{
    if( lProperty.IsValid() )
    {
        ...
        if( layeredTextureCount )
        {
            ...
            for( int layeredTextureIndex = 0; layeredTextureIndex < layeredTextureCount;
                 ++layeredTextureIndex )
            {
                FbxLayeredTexture* layered_texture = FbxCast<FbxLayeredTexture>(
                    lProperty.GetSrcObject<FbxLayeredTexture>( layeredTextureIndex )
                );
                int lNbTex = layered_texture->GetSrcObjectCount<FbxFileTexture>();
                for( int lTextureIndex = 0; lTextureIndex < lNbTex; lTextureIndex++ )
                {
                    FbxFileTexture* lTexture = FbxCast<FbxFileTexture>(
                        layered_texture->GetSrcObject<FbxFileTexture>( lTextureIndex )
                    );
                    if( lTexture )
                    {
                        return lTexture;
        ...
        else
        {
            ...
            {
                FbxFileTexture* lTexture = FbxCast<FbxFileTexture>(
                    lProperty.GetSrcObject<FbxFileTexture>( lTextureIndex )
                );
                if( lTexture )
                {
                    return lTexture;
...
TextureDetails*
FbxMaterialToOsgStateSet::selectTextureDetails( const FbxProperty& lProperty )
{
    if( lProperty.IsValid() )
    {
        ...
        if( fileTexture )
        {
            ...
            textureDetails->scale.set( fileTexture->GetScaleU(),
                                       fileTexture->GetScaleV() );
...
FbxMaterialToOsgStateSet::checkInvertTransparency()
{
    ...
    for( FbxMaterialMap::const_iterator it = _fbxMaterialMap.begin();
         it != _fbxMaterialMap.end();
         ++it )
    {
