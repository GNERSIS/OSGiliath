...
// The only things we need to create a new StateSet are texture and materials. So we
// store that in a pair. We Don't store directly in stateSet because getOrCreateStateSet
// function set some parameters.
...
struct TextureDetails : public osg::Referenced
{
        ...
        void
                                     assignTextureIfRequired( osg::StateSet* stateSet,
                                                              unsigned int   unit );
...
class FbxMaterialToOsgStateSet
{
    public:
        ...
        StateSetContent
        convert( const FbxSurfaceMaterial* pFbxMat );

        // dir is the directory where fbx is stored (for relative path).
        FbxMaterialToOsgStateSet( const std::string&    dir,
    ...
    private:
        ...
        FbxFileTexture*
        selectFbxFileTexture( const FbxProperty& lProperty );

        TextureDetails*
                              selectTextureDetails( const FbxProperty& lProperty );
