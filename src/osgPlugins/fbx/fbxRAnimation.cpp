...
osg::quat
makeQuat( const FbxDouble3&,
          EFbxRotationOrder );

osg::quat
makeQuat( const osg::vec3&  radians,
          EFbxRotationOrder fbxRotOrder )
{
    FbxDouble3 degrees( osg::degrees( radians.x ),
                        osg::degrees( radians.y ),
                        osg::degrees( radians.z ) );
    return makeQuat( degrees, fbxRotOrder );
}

void
readKeys( FbxAnimCurve*                                       curveX,
          FbxAnimCurve*                                       curveY,
          FbxAnimCurve*                                       curveZ,
          const FbxDouble3&                                   defaultValue,
          osgAnimation::TemplateKeyframeContainer<osg::vec3>& keyFrameCntr,
          float                                               scalar = 1.0F )
{
...
readKeys( FbxAnimCurve*     curveX,
          ...
          osgAnimation::TemplateKeyframeContainer<
              osgAnimation::TemplateCubicBezier<osg::vec3>>& keyFrameCntr,
          float                                              scalar = 1.0F )
{
    FbxAnimCurve*            curves[3] = { curveX, curveY, curveZ };
    ...
    for( int nCurve = 0; nCurve < 3; ++nCurve )
    {
        ...
        {
            times.insert( 0.0 );
            curveTimeMap[nCurve][0.0] =
                osgAnimation::FloatCubicBezier( defaultValue[nCurve] * scalar );
        }
        ...
        for( int i = 0; i < nKeys; ++i )
        {
            ...
            FbxAnimCurveTangentInfo rightTangent =
                pCurve->KeyGetRightDerivativeInfo( i );

            if( i > 0 )
            {
                leftTangent.mDerivative *=
                    fTime - pCurve->KeyGetTime( i - 1 ).GetSecondDouble();
            }
            if( i + 1 < pCurve->KeyGetCount() )
            {
                rightTangent.mDerivative *=
                    pCurve->KeyGetTime( i + 1 ).GetSecondDouble() - fTime;
            }
    ...
    for( TimeSet::iterator it = times.begin(); it != times.end(); ++it )
    {
        ...
        keyFrameCntr.push_back( osgAnimation::Vec3CubicBezierKeyframe(
            fTime,
...
readFbxChannels( FbxAnimCurve*     curveX,
                 FbxAnimCurve*     curveY,
                 FbxAnimCurve*     curveZ,
                 const FbxDouble3& defaultValue,
                 ...
                 const char*       channelName )
{
    ...
    {
        osgAnimation::Vec3CubicBezierKeyframeContainer* pKeyFrameCntr =
            new osgAnimation::Vec3CubicBezierKeyframeContainer;
        readKeys( curveX, curveY, curveZ, defaultValue, *pKeyFrameCntr );
        reorderControlPoints( *pKeyFrameCntr );

        osgAnimation::Vec3CubicBezierChannel* pCubicChannel =
    ...
    else
    {
        ...
        {
            osgAnimation::Vec3StepChannel* pStepChannel =
        ...
        {
            osgAnimation::Vec3LinearChannel* pLinearChannel =
...
osgAnimation::Channel*
readFbxChannels( FbxPropertyT<FbxDouble3>& fbxProp,
                 ...
                 const char*               channelName )
{
    ...
    return readFbxChannels( fbxProp.GetCurve( pAnimLayer, "X" ),
...
osgAnimation::Channel*
readFbxChannelsQuat( FbxAnimCurve*     curveX,
                     FbxAnimCurve*     curveY,
                     FbxAnimCurve*     curveZ,
                     const FbxDouble3& defaultValue,
                     const char*       targetName,
                     EFbxRotationOrder rotOrder )
{
    if( !( curveX && curveX->KeyGetCount() ) &&
        !( curveY && curveY->KeyGetCount() ) &&
        !( curveZ && curveZ->KeyGetCount() ) )
    ...
    osgAnimation::QuatSphericalLinearChannel* pChannel =
        new osgAnimation::QuatSphericalLinearChannel;
    pChannel->setTargetName( targetName );
    pChannel->setName( "quaternion" );
    typedef osgAnimation::TemplateKeyframeContainer<osg::vec3> KeyFrameCntr;
         ...
         ++it )
    {
        ...
        quatFrameCntr.push_back(
            osgAnimation::QuatKeyframe( it->getTime(), makeQuat( euler, rotOrder ) )
        );
    }
...
addChannels( osgAnimation::Channel*                            pTranslationChannel,
...
void
readFbxRotationAnimation( osgAnimation::Channel* channels[3],
                          FbxNode*               pNode,
                          FbxAnimLayer*          pAnimLayer,
                          const char*            targetName )
{
    ...
    EFbxRotationOrder rotOrder =
        pNode->RotationOrder.IsValid() ? pNode->RotationOrder.Get() : eEulerXYZ;
    ...
    else
    {
        ...
        {
            FbxAnimCurve* curve =
                pNode->LclRotation.GetCurve( pAnimLayer, curveNames[i] );
            if( !curve )
            {
                continue;
            }
            ...
            if( interpolationType == FbxAnimCurveDef::eInterpolationCubic )
            {
                osgAnimation::FloatCubicBezierKeyframeContainer* pKeyFrameCntr =
                    new osgAnimation::FloatCubicBezierKeyframeContainer;

                for( int j = 0; j < curve->KeyGetCount(); ++j )
                {
                    double fTime = curve->KeyGetTime( j ).GetSecondDouble();
                    float  angle = curve->KeyGetValue( j );
                    // FbxAnimCurveDef::EWeightedMode tangentWeightMode =
                    // curve->KeyGet(j).GetTangentWeightMode();

                    FbxAnimCurveTangentInfo leftTangent =
                        curve->KeyGetLeftDerivativeInfo( j );
                    FbxAnimCurveTangentInfo rightTangent =
                        curve->KeyGetRightDerivativeInfo( j );
                    if( j > 0 )
                    {
                        leftTangent.mDerivative *=
                            fTime - curve->KeyGetTime( j - 1 ).GetSecondDouble();
                    }
                    if( j + 1 < curve->KeyGetCount() )
                    {
                        rightTangent.mDerivative *=
                            curve->KeyGetTime( j + 1 ).GetSecondDouble() - fTime;
                    }
                    osgAnimation::FloatCubicBezier key(
                        osg::radians( angle ),
                        osg::radians( angle - leftTangent.mDerivative / 3.0 ),
                        osg::radians( angle + rightTangent.mDerivative / 3.0 )
                    );

                    pKeyFrameCntr->push_back(
                        osgAnimation::FloatCubicBezierKeyframe( fTime, key )
                    );
                }

                reorderControlPoints( *pKeyFrameCntr );

                osgAnimation::FloatCubicBezierChannel* pCubicChannel =
                    new osgAnimation::FloatCubicBezierChannel;
                pCubicChannel->getOrCreateSampler()->setKeyframeContainer(
                    pKeyFrameCntr
                );
            ...
            else
            {
                ...
                for( int j = 0; j < curve->KeyGetCount(); ++j )
                {
                    ...
                    keys->push_back( osgAnimation::FloatKeyframe(
                        ...
                        static_cast<float>( osg::radians( key.GetValue() ) )
                    ) );
                }

                if( interpolationType == FbxAnimCurveDef::eInterpolationConstant )
                {
                    osgAnimation::FloatStepChannel* pStepChannel =
                        new osgAnimation::FloatStepChannel();
                    pStepChannel->getOrCreateSampler()->setKeyframeContainer( keys );
                    channels[i] = pStepChannel;
                }
                else
                {
                    osgAnimation::FloatLinearChannel* pLinearChannel =
...
osgAnimation::Animation*
readFbxAnimation( FbxNode*                                          pNode,
                  FbxAnimLayer*                                     pAnimLayer,
                  const char*                                       pTakeName,
                  const char*                                       targetName,
                  osg::ref_ptr<osgAnimation::AnimationManagerBase>& pAnimManager )
{
    ...
    osgAnimation::Channel* pScaleChannel =
        readFbxChannels( pNode->LclScaling, pAnimLayer, targetName, "scale" );

    return addChannels( pTranslationChannel,
                        pRotationChannels,
                        pScaleChannel,
                        pAnimManager,
                        pTakeName );
...
std::string
OsgFbxReader::readFbxAnimation( FbxNode*    pNode,
                                const char* targetName )
{
    std::string result;
