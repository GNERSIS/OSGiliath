...
struct VertexIndex
{
        VertexIndex( unsigned int in_vertexIndex,
                     unsigned int in_drawableIndex,
                     unsigned int in_normalIndex ) :
            vertexIndex( in_vertexIndex ),
            drawableIndex( in_drawableIndex ),
            normalIndex( in_normalIndex )
        {
        }

        VertexIndex( const VertexIndex& v ) :
        ...
        bool
        operator<( const VertexIndex& v ) const
        {
...
typedef std::vector<std::pair<Triangle, int>>
    ListTriangle;    // the int is the drawable of the triangle
typedef std::map<VertexIndex, unsigned int>
    MapIndices;      ///< Map OSG indices to FBX mesh indices
typedef std::vector<const osg::Geometry*>
    GeometryList;    // a list of geometries to process in batch

namespace pluginfbx
{

    ///\author Capo (Thibault Caporal), Sukender (Benoit Neil)
    class WriterNodeVisitor : public osg::DualModeVisitor
    {
        public:
            ...
            void
            popStateSet( const osg::StateSet* ss )
            {
                if( ss )
                {
            ...
            typedef std::set<std::string>
                ImageFilenameSet;    // Sub-optimal because strings are doubled (in
                                     // ImageSet). Moreover, an unordered_set (= hashset)
                                     // would be more efficient (Waiting for
                                     // unordered_set to be included in C++ standard ;)
                                     // ).

            ///\todo Add support for 2nd texture, opacity_map, bump_map, specular_map,
            /// shininess_map, self_illum_map, reflection_map.
            class Material
            {
                public:
        ...
        protected:
            ...
            /// Compares StateSets.
            ///\todo It may be useful to compare stack of pointers (see pushStateset())
            /// in order to keep the same number of FBX materials when doing reading and
            /// then writing without further processing.
            struct CompareStateSet
            {
                    bool
                    operator()( const osg::ref_ptr<const osg::StateSet>& ss1,
        ...
        private:
             ...
             *  \param geometryList is the list of geometries which contains the vertices
             * and faces.
            ...
            void
            setLayerTextureAndMaterial( FbxMesh* mesh );

            /// Set Vertices, normals, and UVs
            void
            setControlPointAndNormalsAndUV( const GeometryList& geometryList,
             ...
             */
            void
            createListTriangle( const osg::Geometry* geo,
                                ListTriangle&        listTriangles,
                                bool&                texcoords,
            ...
            int
            processStateSet( const osg::StateSet* stateset );

            typedef std::stack<osg::ref_ptr<osg::StateSet>> StateSetStack;
            typedef std::
                map<osg::ref_ptr<const osg::StateSet>, Material, CompareStateSet>
                                                MaterialMap;
