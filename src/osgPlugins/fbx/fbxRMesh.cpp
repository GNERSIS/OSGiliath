...
osg::dvec3
convertVec3( const FbxVector4& v )
{
    return osg::dvec3( v[0], v[1], v[2] );
}

template<typename T>
bool
layerElementValid( const FbxLayerElementTemplate<T>* pLayerElement )
{
    if( !pLayerElement )
    {
        return false;
...
template<typename T>
int
getVertexIndex( const FbxLayerElementTemplate<T>* pLayerElement,
...
template<typename T>
int
getPolygonIndex( const FbxLayerElementTemplate<T>* pLayerElement,
                 int                               nPolygon )
{
    if( pLayerElement && pLayerElement->GetMappingMode() == FbxLayerElement::eByPolygon )
    {
        switch( pLayerElement->GetReferenceMode() )
        {
            case FbxLayerElement::eDirect :
...
FbxT
getElement( const FbxLayerElementTemplate<FbxT>* pLayerElement,
            const FbxMesh*                       fbxMesh,
            int                                  nPolygon,
            int                                  nPolyVertex,
            int                                  nMeshVertex )
{
    return pLayerElement->GetDirectArray().GetAt(
        getVertexIndex( pLayerElement, fbxMesh, nPolygon, nPolyVertex, nMeshVertex )
    );
}
...
osg::Array*
createVec2Array( bool doublePrecision )
{
    if( doublePrecision )
    {
...
osg::Array*
createVec3Array( bool doublePrecision )
{
    if( doublePrecision )
    {
...
osg::Array*
createVec4Array( bool doublePrecision )
{
    if( doublePrecision )
    {
             ...
             bool                          lightmapTextures )
{
    ...
    pGeometry->setVertexArray(
        createVec3Array( ( precision & osgDB::Options::DOUBLE_PRECISION_VERTEX ) != 0 )
    );
    if( bNormal )
    {
        pGeometry->setNormalArray(
            createVec3Array( ( precision & osgDB::Options::DOUBLE_PRECISION_NORMAL ) !=
                             0 ),
    ...
    if( useDiffuseMap )
    {
        pGeometry->setTexCoordArray(
            StateSetContent::DIFFUSE_TEXTURE_UNIT,
    ...
    {
        pGeometry->setTexCoordArray(
            StateSetContent::OPACITY_TEXTURE_UNIT,
            createVec2Array( ( precision &
                               osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0 ),
            osg::Array::BIND_PER_VERTEX
        );
    ...
    {
        pGeometry->setTexCoordArray(
            StateSetContent::EMISSIVE_TEXTURE_UNIT,
            createVec2Array( ( precision &
                               osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0 ),
            osg::Array::BIND_PER_VERTEX
        );
    ...
    {
        pGeometry->setTexCoordArray(
            StateSetContent::AMBIENT_TEXTURE_UNIT,
            createVec2Array( ( precision &
                               osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0 ),
            osg::Array::BIND_PER_VERTEX
        );
    ...
    {
        pGeometry->setTexCoordArray(
            StateSetContent::NORMAL_TEXTURE_UNIT,
            createVec2Array( ( precision &
                               osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0 ),
            osg::Array::BIND_PER_VERTEX
        );
    ...
    {
        pGeometry->setTexCoordArray(
            StateSetContent::SPECULAR_TEXTURE_UNIT,
            createVec2Array( ( precision &
                               osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0 ),
            osg::Array::BIND_PER_VERTEX
        );
    ...
    {
        pGeometry->setTexCoordArray(
            StateSetContent::SHININESS_TEXTURE_UNIT,
            createVec2Array( ( precision &
                               osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0 ),
            osg::Array::BIND_PER_VERTEX
        );
    ...
    if( bColor )
    {
        pGeometry->setColorArray(
            createVec4Array( ( precision & osgDB::Options::DOUBLE_PRECISION_COLOR ) !=
                             0 ),
            osg::Array::BIND_PER_VERTEX
        );
    }
    ...
    if( mti < stateSetList.size() )
    {
        ...
        {
            ssc.diffuse->assignTextureIfRequired(
                stateSet,
                StateSetContent::DIFFUSE_TEXTURE_UNIT
            );
            ...
            if( lightmapTextures )
            {
                // Texture blending factor now handled by shaders
            }

            if( !transparent )
            {
                transparent = ssc.diffuse->transparent();
            }
        }

        if( ssc.opacity.valid() )
        {
            ssc.opacity->assignTextureIfRequired(
                stateSet,
                StateSetContent::OPACITY_TEXTURE_UNIT
            );
            ...
            if( !transparent )
            {
                transparent = ssc.opacity->transparent();
            }
        }

        if( ssc.reflection.valid() )
        {
            ssc.reflection->assignTextureIfRequired(
                stateSet,
                StateSetContent::REFLECTION_TEXTURE_UNIT
            );
        }

        if( ssc.emissive.valid() )
        {
            ssc.emissive->assignTextureIfRequired(
                stateSet,
                StateSetContent::EMISSIVE_TEXTURE_UNIT
            );
        }
        ...
        {
            ssc.ambient->assignTextureIfRequired(
                stateSet,
                StateSetContent::AMBIENT_TEXTURE_UNIT
            );
        }
        ...
        {
            ssc.normalMap->assignTextureIfRequired(
                stateSet,
                StateSetContent::NORMAL_TEXTURE_UNIT
            );
        }
        ...
        {
            ssc.specular->assignTextureIfRequired(
                stateSet,
                StateSetContent::SPECULAR_TEXTURE_UNIT
            );
        }
        ...
        {
            ssc.shininess->assignTextureIfRequired(
                stateSet,
                StateSetContent::SHININESS_TEXTURE_UNIT
            );
        }
...
getVertexInfluence( osgAnimation::VertexInfluenceMap& vim,
                    const std::string&                name )
{
    ...
    {
        it = vim.insert( it,
                         osgAnimation::VertexInfluenceMap::value_type(
                             name,
                             osgAnimation::VertexInfluence()
                         ) );
...
void
addChannel( osgAnimation::Channel*                            pChannel,
...
void
readAnimation( FbxNode*                                          pNode,
               FbxScene&                                         fbxScene,
               const std::string&                                targetName,
               osg::ref_ptr<osgAnimation::AnimationManagerBase>& pAnimationManager,
               FbxMesh*                                          pMesh,
               int                                               nBlendShape,
               int                                               nBlendShapeChannel,
               int                                               nShape )
{
    for( int i = 0; i < fbxScene.GetSrcObjectCount<FbxAnimStack>(); ++i )
    {
        FbxAnimStack* pAnimStack =
            FbxCast<FbxAnimStack>( fbxScene.GetSrcObject<FbxAnimStack>( i ) );
        ...
        for( int j = 0; j < nbAnimLayers; j++ )
        {
            ...
            FbxAnimCurve* pCurve     = pMesh->GetShapeChannel( nBlendShape,
                                                               nBlendShapeChannel,
                                                               pAnimLayer,
                                                               false );
            ...
            osgAnimation::FloatLinearChannel* pChannel =
                new osgAnimation::FloatLinearChannel;
            osgAnimation::TemplateKeyframeContainer<float>& keyFrameCntr =
...
void
addBindMatrix( BindMatrixMap&             boneBindMatrices,
               FbxNode*                   pBone,
               const osg::dmat4&          bindMatrix,
...
void
addVec2ArrayElement( osg::Array&       a,
                     const FbxVector2& v )
{
    if( a.getType() == osg::Array::Vec2dArrayType )
    {
        static_cast<osg::Vec2dArray&>( a ).push_back( osg::dvec2( v[0], v[1] ) );
    }
    else
    {
        static_cast<osg::Vec2Array&>( a ).push_back(
            osg::vec2( static_cast<float>( v[0] ), static_cast<float>( v[1] ) )
        );
    }
}

void
addVec3ArrayElement( osg::Array&       a,
                     const FbxVector4& v )
{
    if( a.getType() == osg::Array::Vec3dArrayType )
    {
        static_cast<osg::Vec3dArray&>( a ).push_back( osg::dvec3( v[0], v[1], v[2] ) );
    }
    else
    {
        static_cast<osg::Vec3Array&>( a ).push_back(
            osg::vec3( static_cast<float>( v[0] ),
...
void
addColorArrayElement( osg::Array&     a,
                      const FbxColor& c )
{
    if( a.getType() == osg::Array::Vec4dArrayType )
    {
        static_cast<osg::Vec4dArray&>( a ).push_back(
            osg::dvec4( c.mRed, c.mGreen, c.mBlue, c.mAlpha )
        );
    }
    else
    {
        static_cast<osg::Vec4Array&>( a ).push_back(
            osg::vec4( static_cast<float>( c.mRed ),
...
std::string
getUVChannelForTextureMap( std::vector<StateSetContent>& stateSetList,
                           const char*                   pName )
{
    ...
    {
        if( stateSetList[i].diffuse.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sDiffuse ) ) )
        {
            return stateSetList[i].diffuse->channel;
        }
        if( stateSetList[i].opacity.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sTransparentColor ) ) )
        {
            return stateSetList[i].opacity->channel;
        }
        if( stateSetList[i].reflection.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sReflection ) ) )
        {
            return stateSetList[i].reflection->channel;
        }
        if( stateSetList[i].emissive.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sEmissive ) ) )
        {
            return stateSetList[i].emissive->channel;
        }
        if( stateSetList[i].ambient.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sAmbient ) ) )
        {
            return stateSetList[i].ambient->channel;
        }
        if( stateSetList[i].normalMap.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sNormalMap ) ) )
        {
            return stateSetList[i].normalMap->channel;
        }
        if( stateSetList[i].specular.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sSpecular ) ) )
        {
            return stateSetList[i].specular->channel;
        }
        if( stateSetList[i].shininess.valid() &&
            ( 0 == strcmp( pName, FbxSurfaceMaterial::sShininess ) ) )
        {
...
// scans mesh layers looking for the UV element corresponding to the specified channel
// name...
const FbxLayerElementUV*
getUVElementForChannel( std::string            uvChannelName,
                        FbxLayerElement::EType elementType,
                        FbxMesh*               pFbxMesh )
{
...
void
readMeshTriangle( const FbxMesh*                    fbxMesh,
                  int                               i /*polygonIndex*/,
                  int                               posInPoly0,
                  int                               posInPoly1,
                  int                               posInPoly2,
                  ...
                  osg::Array*                       pColors )
{
    ...
    fbxToOsgVertMap.insert(
        FbxToOsgVertexMap::value_type( v0,
                                       GIPair( pGeometry, pVertices->getNumElements() ) )
    );
    fbxToOsgVertMap.insert( FbxToOsgVertexMap::value_type(
        v1,
        GIPair( pGeometry, pVertices->getNumElements() + 1 )
    ) );
    fbxToOsgVertMap.insert( FbxToOsgVertexMap::value_type(
        v2,
        GIPair( pGeometry, pVertices->getNumElements() + 2 )
    ) );
    ...
    if( pNormals )
    {
        ...
        osgToFbxNormMap.insert( OsgToFbxNormalMap::value_type(
            GIPair( pGeometry, pNormals->getNumElements() ),
            n0
        ) );
        osgToFbxNormMap.insert( OsgToFbxNormalMap::value_type(
            GIPair( pGeometry, pNormals->getNumElements() + 1 ),
            n1
        ) );
        osgToFbxNormMap.insert( OsgToFbxNormalMap::value_type(
            GIPair( pGeometry, pNormals->getNumElements() + 2 ),
            n2
        ) );
    ...
    if( pTexCoords_diffuse )
    {
        addVec2ArrayElement(
            *pTexCoords_diffuse,
            getElement( pFbxUVs_diffuse, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_diffuse,
            getElement( pFbxUVs_diffuse, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addVec2ArrayElement(
            *pTexCoords_diffuse,
            getElement( pFbxUVs_diffuse, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
    if( pTexCoords_opacity && ( pTexCoords_opacity != pTexCoords_diffuse ) )
    {
        addVec2ArrayElement(
            *pTexCoords_opacity,
            getElement( pFbxUVs_opacity, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_opacity,
            getElement( pFbxUVs_opacity, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addVec2ArrayElement(
            *pTexCoords_opacity,
            getElement( pFbxUVs_opacity, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }

    // Only spherical reflection maps are supported (so do not add coordinates for the
    // reflection map)

    if( pTexCoords_emissive &&
        ( pTexCoords_emissive != pTexCoords_opacity ) &&
        ( pTexCoords_emissive != pTexCoords_diffuse ) )
    {
        addVec2ArrayElement(
            *pTexCoords_emissive,
            getElement( pFbxUVs_emissive, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_emissive,
            getElement( pFbxUVs_emissive, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addVec2ArrayElement(
            *pTexCoords_emissive,
            getElement( pFbxUVs_emissive, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
    if( pTexCoords_ambient &&
        ( pTexCoords_ambient != pTexCoords_opacity ) &&
        ( pTexCoords_ambient != pTexCoords_diffuse ) &&
        ( pTexCoords_ambient != pTexCoords_emissive ) )
    {
        addVec2ArrayElement(
            *pTexCoords_ambient,
            getElement( pFbxUVs_ambient, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_ambient,
        ...
        addVec2ArrayElement(
            *pTexCoords_ambient,
            getElement( pFbxUVs_ambient, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
    if( pTexCoords_normal &&
        ( pTexCoords_normal != pTexCoords_opacity ) &&
        ( pTexCoords_normal != pTexCoords_diffuse ) &&
        ( pTexCoords_normal != pTexCoords_emissive ) &&
        ( pTexCoords_normal != pTexCoords_ambient ) )
    {
        addVec2ArrayElement(
            *pTexCoords_normal,
            getElement( pFbxUVs_normal, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_normal,
            getElement( pFbxUVs_normal, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addVec2ArrayElement(
            *pTexCoords_normal,
            getElement( pFbxUVs_normal, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
    if( pTexCoords_specular &&
        ( pTexCoords_specular != pTexCoords_opacity ) &&
        ( pTexCoords_specular != pTexCoords_diffuse ) &&
        ( pTexCoords_specular != pTexCoords_emissive ) &&
        ( pTexCoords_specular != pTexCoords_ambient ) &&
        ( pTexCoords_specular != pTexCoords_normal ) )
    {
        addVec2ArrayElement(
            *pTexCoords_specular,
            getElement( pFbxUVs_specular, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_specular,
            getElement( pFbxUVs_specular, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addVec2ArrayElement(
            *pTexCoords_specular,
            getElement( pFbxUVs_specular, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
    if( pTexCoords_shininess &&
        ( pTexCoords_shininess != pTexCoords_opacity ) &&
        ( pTexCoords_shininess != pTexCoords_diffuse ) &&
        ( pTexCoords_shininess != pTexCoords_emissive ) &&
        ( pTexCoords_shininess != pTexCoords_ambient ) &&
        ( pTexCoords_shininess != pTexCoords_normal ) &&
        ( pTexCoords_shininess != pTexCoords_specular ) )
    {
        addVec2ArrayElement(
            *pTexCoords_shininess,
            getElement( pFbxUVs_shininess, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addVec2ArrayElement(
            *pTexCoords_shininess,
            getElement( pFbxUVs_shininess, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addVec2ArrayElement(
            *pTexCoords_shininess,
            getElement( pFbxUVs_shininess, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
    // add more texture maps here...
    ...
    if( pColors )
    {
        addColorArrayElement(
            *pColors,
            getElement( pFbxColors, fbxMesh, i, posInPoly0, meshVertex0 )
        );
        addColorArrayElement(
            *pColors,
            getElement( pFbxColors, fbxMesh, i, posInPoly1, meshVertex1 )
        );
        addColorArrayElement(
            *pColors,
            getElement( pFbxColors, fbxMesh, i, posInPoly2, meshVertex2 )
        );
    }
}
...
bool
quadSplit02( const FbxMesh*    fbxMesh,
             int               i /*polygonIndex*/,
             int               posInPoly0,
             int               posInPoly1,
             int               posInPoly2,
             ...
             const FbxVector4* pFbxVertices )
{
    ...
    // Here we test each of the 4 triangles and see if there is one in the opposite
    // direction.
    //        Triangles: 012, 023, 013, 123
    // For this, we do a cross product to get normals. We say here the first triangle is
    // the reference, and do a dot product to see the direction.
    ...
    osg::dvec3 p0( pFbxVertices[v0][0], pFbxVertices[v0][1], pFbxVertices[v0][2] );
    osg::dvec3 p1( pFbxVertices[v1][0], pFbxVertices[v1][1], pFbxVertices[v1][2] );
    osg::dvec3 p2( pFbxVertices[v2][0], pFbxVertices[v2][1], pFbxVertices[v2][2] );
    osg::dvec3 p3( pFbxVertices[v3][0], pFbxVertices[v3][1], pFbxVertices[v3][2] );

    osg::dvec3 na( ( p1 - p0 ) ^ ( p2 - p1 ) );
    osg::dvec3 nb( ( p2 - p0 ) ^ ( p3 - p2 ) );
    ...
    osg::dvec3 nc( ( p1 - p0 ) ^ ( p3 - p1 ) );
    osg::dvec3 nd( ( p2 - p1 ) ^ ( p3 - p2 ) );
    double     rc( na * nc );
    double     rd( na * nd );
    return ( rc >= 0 || rd >= 0 );
}
...
struct PolygonRef
{
        PolygonRef( osg::Geometry* in_pGeometry,
                    int            in_numPoly,
                    int            in_nVertex ) :
            pGeometry( in_pGeometry ),
            numPoly( in_numPoly ),
            nVertex( in_nVertex )
        {
        }
...
OsgFbxReader::readMesh( FbxNode*                      pNode,
                        FbxMesh*                      fbxMesh,
                        std::vector<StateSetContent>& stateSetList,
                        const char*                   szName )
{
    ...
    // scan layers for Normals, Colors and Materials elements (this will get the first
    // available elements)...
    ...
    // look for UV elements (diffuse, opacity, reflection, emissive, ...) and get their
    // channels names...
    std::string diffuseChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sDiffuse );
    std::string opacityChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sTransparentColor );
    std::string emissiveChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sEmissive );
    std::string ambientChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sAmbient );
    std::string normalChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sNormalMap );
    std::string specularChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sSpecular );
    std::string shininessChannel =
        getUVChannelForTextureMap( stateSetList, FbxSurfaceMaterial::sShininess );
    // look for more UV elements here...

    // UV elements...
    const FbxLayerElementUV* pFbxUVs_diffuse =
        getUVElementForChannel( diffuseChannel,
    ...
    const FbxLayerElementUV* pFbxUVs_opacity =
        getUVElementForChannel( opacityChannel,
                                ...
                                fbxMesh );
    const FbxLayerElementUV* pFbxUVs_emissive =
        getUVElementForChannel( emissiveChannel,
                                FbxLayerElement::eTextureEmissive,
                                fbxMesh );
    const FbxLayerElementUV* pFbxUVs_ambient =
        getUVElementForChannel( ambientChannel,
                                FbxLayerElement::eTextureAmbient,
                                fbxMesh );
    const FbxLayerElementUV* pFbxUVs_normal =
        getUVElementForChannel( normalChannel,
                                FbxLayerElement::eTextureNormalMap,
                                fbxMesh );
    const FbxLayerElementUV* pFbxUVs_specular =
        getUVElementForChannel( specularChannel,
                                FbxLayerElement::eTextureSpecular,
                                fbxMesh );
    const FbxLayerElementUV* pFbxUVs_shininess =
        getUVElementForChannel( shininessChannel,
                                FbxLayerElement::eTextureShininess,
                                fbxMesh );
    ...
    if( !layerElementValid( pFbxUVs_normal ) )
    {
        pFbxUVs_normal = 0;
    }
    if( !layerElementValid( pFbxUVs_specular ) )
    {
        pFbxUVs_specular = 0;
    }
    if( !layerElementValid( pFbxUVs_shininess ) )
    {
        pFbxUVs_shininess = 0;
    }
    // more here...
    ...
    for( int i = 0, nVertex = 0; i < nPolys; ++i )
    {
        ...
        osg::Geometry* pGeometry     = getGeometry( pGeode,
                                                    geometryMap,
                                                    stateSetList,
                                                    geomType,
                                                    materialIndex,
        ...
        osg::Array*    pTexCoords_diffuse =
            pGeometry->getTexCoordArray( StateSetContent::DIFFUSE_TEXTURE_UNIT );
        osg::Array* pTexCoords_opacity =
            pGeometry->getTexCoordArray( StateSetContent::OPACITY_TEXTURE_UNIT );
        osg::Array* pTexCoords_emissive =
            pGeometry->getTexCoordArray( StateSetContent::EMISSIVE_TEXTURE_UNIT );
        osg::Array* pTexCoords_ambient =
            pGeometry->getTexCoordArray( StateSetContent::AMBIENT_TEXTURE_UNIT );
        osg::Array* pTexCoords_normal =
            pGeometry->getTexCoordArray( StateSetContent::NORMAL_TEXTURE_UNIT );
        osg::Array* pTexCoords_specular =
            pGeometry->getTexCoordArray( StateSetContent::SPECULAR_TEXTURE_UNIT );
        osg::Array* pTexCoords_shininess =
            pGeometry->getTexCoordArray( StateSetContent::SHININESS_TEXTURE_UNIT );
    ...
    for( unsigned i = 0; i < pGeode->getNumDrawables(); ++i )
    {
        ...
        osg::DrawArrays* pDrawArrays =
            new osg::DrawArrays( GL_TRIANGLES,
                                 0,
                                 pGeometry->getVertexArray()->getNumElements() );
         ...
         ++it )
    {
        ...
        for( int j = 0, nVertex = it->nVertex; j < lPolygonSize; ++j, ++nVertex )
        {
            ...
            if( pTexCoords_diffuse )
            {
                addVec2ArrayElement(
                    *pTexCoords_diffuse,
                    getElement( pFbxUVs_diffuse, fbxMesh, i, j, nVertex )
                );
            }
            if( pTexCoords_opacity && ( pTexCoords_opacity != pTexCoords_diffuse ) )
            {
                addVec2ArrayElement(
                    *pTexCoords_opacity,
                    getElement( pFbxUVs_opacity, fbxMesh, i, j, nVertex )
                );
            }

            // Only spherical reflection maps are supported (so do not add coordinates
            // for the reflection map)
                ...
                ( pTexCoords_emissive != pTexCoords_diffuse ) )
            {
                addVec2ArrayElement(
                    *pTexCoords_emissive,
                    getElement( pFbxUVs_emissive, fbxMesh, i, j, nVertex )
                );
            }
            if( pTexCoords_ambient &&
                ( pTexCoords_ambient != pTexCoords_opacity ) &&
                ...
                ( pTexCoords_normal != pTexCoords_ambient ) )
            {
                addVec2ArrayElement(
                    *pTexCoords_normal,
                    getElement( pFbxUVs_normal, fbxMesh, i, j, nVertex )
                );
            }
            if( pTexCoords_specular &&
                ( pTexCoords_specular != pTexCoords_opacity ) &&
                ...
                ( pTexCoords_specular != pTexCoords_normal ) )
            {
                addVec2ArrayElement(
                    *pTexCoords_specular,
                    getElement( pFbxUVs_specular, fbxMesh, i, j, nVertex )
                );
            }
            if( pTexCoords_shininess &&
                ( pTexCoords_shininess != pTexCoords_opacity ) &&
                ...
                ( pTexCoords_shininess != pTexCoords_specular ) )
            {
                addVec2ArrayElement(
                    *pTexCoords_shininess,
                    getElement( pFbxUVs_shininess, fbxMesh, i, j, nVertex )
                );
            }
            // add more texture maps here...
    ...
    if( geomType & GEOMETRY_MORPH )
    {
        for( unsigned i = 0; i < pGeode->getNumDrawables(); ++i )
        {
            ...
            pGeode->addUpdateCallback(
                new osgAnimation::UpdateMorph( morph.getName() )
            );

            // read morph geometry
            for( int nBlendShape = 0; nBlendShape < nDeformerBlendShapeCount;
                 ++nBlendShape )
            {
                FbxBlendShape* pBlendShape = FbxCast<FbxBlendShape>(
                    fbxMesh->GetDeformer( nBlendShape, FbxDeformer::eBlendShape )
                );
                const int nBlendShapeChannelCount =
                    pBlendShape->GetBlendShapeChannelCount();

                for( int nBlendShapeChannel = 0;
                     nBlendShapeChannel < nBlendShapeChannelCount;
                     ++nBlendShapeChannel )
                {
                    FbxBlendShapeChannel* pBlendShapeChannel =
                        pBlendShape->GetBlendShapeChannel( nBlendShapeChannel );
                    ...
                    {
                        OSG_WARN
                            << "Multiple FBX Target Shapes, only the first will be used"
                            << std::endl;
                    }
                    const FbxGeometryBase* pMorphShape =
                        pBlendShapeChannel->GetTargetShape( 0 );
                    ...
                    pMorphTarget->setVertexArray(
                        static_cast<osg::Array*>( pMorphTarget->getVertexArray()->clone(
                    ...
                    readAnimation( pNode,
                                   ...
                                   nBlendShape,
                                   nBlendShapeChannel,
                                   ( int )morph.getMorphTargetList().size() - 1 );
                }
            }
        }
        ...
        for( int nBlendShape = 0; nBlendShape < nDeformerBlendShapeCount; ++nBlendShape )
        {
                 ...
                 ++nBlendShapeChannel )
            {
                ...
                {
                    osg::dvec3 vPos = convertVec3( pControlPoints[fbxIndex] );
                    for( FbxToOsgVertexMap::const_iterator it =
                             fbxToOsgVertMap.find( fbxIndex );
                         it != fbxToOsgVertMap.end() && it->first == fbxIndex;
                         ++it )
                    {
                        GIPair                       gi = it->second;
                        ...
                        osg::Geometry* pGeometry =
                            morphGeom.getMorphTarget( nMorphTarget ).getGeometry();

                        if( pGeometry->getVertexArray()->getType() ==
                            osg::Array::Vec3dArrayType )
                        {
                            osg::Vec3dArray* pVertices = static_cast<osg::Vec3dArray*>(
                        ...
                        if( pFbxShapeNormals && pGeometry->getNormalArray() )
                        {
                                ...
                                osg::Array::Vec3dArrayType )
                            {
                                osg::Vec3dArray* pNormals =
                                    static_cast<osg::Vec3dArray*>(
                                        pGeometry->getNormalArray()
                                    );
                                ( *pNormals )[gi.second] =
                                    convertVec3( pFbxShapeNormals->GetDirectArray()
                                                     .GetAt( osgToFbxNormMap[gi] ) );
                            }
                            else
                            {
                ...
                // don't put this in the for loop as we don't want to do it if the loop
                // continues early
                ++nMorphTarget;
            }
        }
    ...
    if( geomType & GEOMETRY_RIG )
    {
        typedef std::map<osg::ref_ptr<osg::Geometry>,
                         osg::ref_ptr<osgAnimation::RigGeometry>>
                               GeometryRigGeometryMap;
        ...
        for( unsigned i = 0; i < pGeode->getNumDrawables(); ++i )
        {
            ...
            old2newGeometryMap.insert( GeometryRigGeometryMap::value_type( pGeometry,
                                                                           pRig ) );
            pRig->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        for( int i = 0; i < nDeformerCount; ++i )
        {
            ...
            for( int j = 0; j < nClusters; ++j )
            {
                ...
                osg::dmat4    bindMatrix( pTransformLinkInverse );
                ...
                for( int k = 0; k < nIndices; ++k )
                {
                         ...
                         ++it )
                    {
                        ...
                        vi.push_back( osgAnimation::VertexIndexWeight( gi.second,
                                                                       weight ) );
                    }
                }
            }
    ...
    fbxGeometricTransform.SetTRS( pNode->GeometricTranslation.Get(),
                                  pNode->GeometricRotation.Get(),
                                  pNode->GeometricScaling.Get() );
    const double* pGeometricMat = fbxGeometricTransform;
    osg::dmat4    osgGeometricTransform( pGeometricMat );
    ...
    if( geomType & GEOMETRY_RIG )
    {
        ...
        if( pSkin->GetClusterCount() )
        {
            ...
            osgGeometricTransform.postMult(
                osg::rotate( osg::dquat( pTransformMatrix ) )
            );
        }
    }
    ...
    if( !osgGeometricTransform.isIdentity() )
    {
        osg::MatrixTransform* pMatTrans =
            new osg::MatrixTransform( osgGeometricTransform );
    ...
    if( geomType & GEOMETRY_RIG )
    {
        ...
        if( pSkin->GetClusterCount() )
        {
            osgAnimation::Skeleton* pSkeleton =
                getSkeleton( pSkin->GetCluster( 0 )->GetLink(),
                             fbxSkeletons,
                             skeletonMap );
...
osgDB::ReaderWriter::ReadResult
OsgFbxReader::readFbxMesh( FbxNode*                      pNode,
                           std::vector<StateSetContent>& stateSetList )
{
    ...
    return readMesh( pNode, lMesh, stateSetList, pNode->GetName() );
}
