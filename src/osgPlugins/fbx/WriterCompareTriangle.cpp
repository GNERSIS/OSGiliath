#include "WriterCompareTriangle.hpp"

WriterCompareTriangle::WriterCompareTriangle( const osg::Geode& in_geode,
                                              unsigned int      in_nbVertices ) :
    geode( geode )
{
...
bool
WriterCompareTriangle::operator()( const std::pair<Triangle,
                                                   int>& t1,
...
{
    osg::box::vec_type        length      = sceneBox._max - sceneBox._min;
    ...
    osg::box::value_type blocX =
        length.x / nbVerticesX;    // These 3 lines set the size of a box in x, y and z
    osg::box::value_type blocY = length.y / nbVerticesY;
    osg::box::value_type blocZ = length.z / nbVerticesZ;
    ...
    for( unsigned int z = 0; z < nbVerticesZ; ++z )
    {
        while( x < nbVerticesX && x >= 0 )
        {
            ...
            {
                osg::box::value_type xMin = sceneBox.xMin() + x * blocX;
                if( x == 0 )    // to prevent from mesh with no case
                {
                    xMin -= 10;
                }

                osg::box::value_type yMin = sceneBox.yMin() + y * blocY;
                if( y == 0 )    // to prevent from mesh with no case
                {
                    yMin -= 10;
                }

                osg::box::value_type zMin = sceneBox.zMin() + z * blocZ;
                if( z == 0 )    // to prevent from mesh with no case
                {
                    zMin -= 10;
                }

                osg::box::value_type xMax = sceneBox.xMin() + ( x + 1 ) * blocX;
                if( x == nbVerticesX - 1 )    // to prevent from mesh with no case
                {
                    xMax += 10;
                }

                osg::box::value_type yMax = sceneBox.yMin() + ( y + 1 ) * blocY;
                if( y == nbVerticesY - 1 )    // to prevent from mesh with no case
                {
                    yMax += 10;
                }

                osg::box::value_type zMax = sceneBox.zMin() + ( z + 1 ) * blocZ;
                if( z == nbVerticesZ - 1 )    // to prevent from mesh with no case
                {
                    zMax += 10;
                }

                boxList.push_back( osg::box( xMin,    // Add a box to the list
                                             yMin,
                                             zMin,
                                             xMax,
                                             yMax,
