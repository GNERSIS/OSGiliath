...
osg::quat
makeQuat( const FbxDouble3& degrees,
          EFbxRotationOrder fbxRotOrder )
{
    double radiansX = osg::radians( degrees[0] );
    double radiansY = osg::radians( degrees[1] );
    double radiansZ = osg::radians( degrees[2] );

    switch( fbxRotOrder )
    {
        case eEulerXYZ :
            return osg::quat( radiansX,
                              osg::dvec3( 1, 0, 0 ),
                              radiansY,
                              osg::dvec3( 0, 1, 0 ),
                              radiansZ,
        ...
        case eSphericXYZ :
            {
                ...
                osg::quat quat;
                quat = osg::quat( osg::dvec3( 1.0, osg::vec3( 0.0, 0.0 ) ),
                                  osg::dvec3( degrees[0], degrees[1], degrees[2] ) );
                return quat;
            }
        default :
...
void
makeLocalMatrix( const FbxNode* pNode,
                 osg::dmat4&    m )
{
    /*From
    http://area.autodesk.com/forum/autodesk-fbx/fbx-sdk/the-makeup-of-the-local-matrix-of-an-kfbxnode/

    Local dmat4 = LclTranslation * RotationOffset * RotationPivot *
    ...
    RotationOffset: translation compensates for the change in the rotate pivot point
    (xform -q -rotateTranslation) RotationPivot: current rotate pivot position (xform -q
    -rotatePivot) PreRotation : joint orientation(pre rotation) LocalRotation: rotate
    transform (xform -q -rotation & xform -q -rotateOrder) PostRotation : rotate axis
    (xform -q -rotateAxis) RotationPivotInverse: inverse of RotationPivot ScalingOffset:
    translation compensates for the change in the scale pivot point (xform -q
    -scaleTranslation) ScalingPivot: current scale pivot position (xform -q -scalePivot)
    ...
    */
    ...
    EFbxRotationOrder fbxRotOrder =
        rotationActive ? pNode->RotationOrder.Get() : eEulerXYZ;
    ...
    m.makeTranslate( osg::dvec3( fbxLclPos[0] + fbxRotOff[0] + fbxRotPiv[0],
                                 fbxLclPos[1] + fbxRotOff[1] + fbxRotPiv[1],
                                 fbxLclPos[2] + fbxRotOff[2] + fbxRotPiv[2] ) );
    if( rotationActive )
    {
        m.preMultRotate( makeQuat( fbxPostRot, eEulerXYZ ) *
    ...
    m.preMultTranslate( osg::dvec3( fbxSclOff[0] + fbxSclPiv[0] - fbxRotPiv[0],
                                    fbxSclOff[1] + fbxSclPiv[1] - fbxRotPiv[1],
                                    fbxSclOff[2] + fbxSclPiv[2] - fbxRotPiv[2] ) );
    m.preMultScale( osg::dvec3( fbxLclScl[0], fbxLclScl[1], fbxLclScl[2] ) );
    m.preMultTranslate( osg::dvec3( -fbxSclPiv[0], -fbxSclPiv[1], -fbxSclPiv[2] ) );
}

void
readTranslationElement( FbxPropertyT<FbxDouble3>&            prop,
                        osgAnimation::UpdateMatrixTransform* pUpdate,
                        osg::dmat4&                          staticTransform,
                        FbxScene&                            fbxScene )
{
    FbxDouble3 fbxPropValue = prop.Get();
    osg::dvec3 val( fbxPropValue[0], fbxPropValue[1], fbxPropValue[2] );
    ...
    if( isAnimated( prop, fbxScene ) )
    {
        if( !staticTransform.isIdentity() )
        {
            ...
            staticTransform = osg::dmat4();
        }
        pUpdate->getStackedTransforms().push_back(
            new osgAnimation::StackedTranslateElement( "translate", val )
        );
    }
    else
    {
...
void
getRotationOrder( EFbxRotationOrder fbxRotOrder,
                  int               order[/*3*/] )
{
    switch( fbxRotOrder )
    {
...
void
readRotationElement( FbxPropertyT<FbxDouble3>&            prop,
                     EFbxRotationOrder                    fbxRotOrder,
                     ...
                     FbxScene&                            fbxScene )
{
    if( isAnimated( prop, fbxScene ) )
    {
        if( quatInterpolate )
        {
            ...
            pUpdate->getStackedTransforms().push_back(
                new osgAnimation::StackedQuaternionElement( "quaternion",
                                                            makeQuat( prop.Get(),
                                                                      fbxRotOrder ) )
            );
        }
        else
        {
            ...
            osg::vec3 axes[3] =
                { osg::vec3( 1, 0, 0 ), osg::vec3( 0, 1, 0 ), osg::vec3( 0, 0, 1 ) };

            FbxDouble3 fbxPropValue = prop.Get();
            fbxPropValue[0]         = osg::radians( fbxPropValue[0] );
            fbxPropValue[1]         = osg::radians( fbxPropValue[1] );
            fbxPropValue[2]         = osg::radians( fbxPropValue[2] );
            ...
            for( int i = 0; i < 3; ++i )
            {
                ...
                if( isAnimated( prop, curveNames[j], fbxScene ) )
                {
                    ...
                    pUpdate->getStackedTransforms().push_back(
                        new osgAnimation::StackedRotateAxisElement(
                            std::string( "rotate" ) + curveNames[j],
                            axes[j],
                            fbxPropValue[j]
                        )
                    );
                ...
                {
                    staticTransform.preMultRotate( osg::quat( fbxPropValue[j],
                                                              axes[j] ) );
                }
            }
        }
...
void
readScaleElement( FbxPropertyT<FbxDouble3>&            prop,
                  osgAnimation::UpdateMatrixTransform* pUpdate,
                  osg::dmat4&                          staticTransform,
                  FbxScene&                            fbxScene )
{
    ...
    if( isAnimated( prop, fbxScene ) )
    {
        ...
        pUpdate->getStackedTransforms().push_back(
            new osgAnimation::StackedScaleElement( "scale", val )
        );
    }
    else
    {
...
void
readUpdateMatrixTransform( osgAnimation::UpdateMatrixTransform* pUpdate,
                           FbxNode*                             pNode,
                           FbxScene&                            fbxScene )
{
    osg::dmat4 staticTransform;
    ...
    FbxDouble3 fbxRotPiv    = pNode->RotationPivot.Get();
    staticTransform.preMultTranslate( osg::dvec3( fbxRotPiv[0] + fbxRotOffset[0],
    ...
    readRotationElement( pNode->LclRotation,
                         fbxRotOrder,
    ...
    if( rotationActive )
    {
        staticTransform.preMultRotate( makeQuat( pNode->PostRotation.Get(),
                                                 eEulerXYZ ) );
    }

    FbxDouble3 fbxSclOffset = pNode->ScalingOffset.Get();
    FbxDouble3 fbxSclPiv    = pNode->ScalingPivot.Get();
    staticTransform.preMultTranslate(
        osg::dvec3( fbxSclOffset[0] + fbxSclPiv[0] - fbxRotPiv[0],
    ...
    staticTransform.preMultTranslate(
        osg::dvec3( -fbxSclPiv[0], -fbxSclPiv[1], -fbxSclPiv[2] )
    );
...
osg::Group*
createGroupNode( FbxManager&           pSdkManager,
                 FbxNode*              pNode,
                 const std::string&    animName,
                 const osg::dmat4&     localMatrix,
                 bool                  bNeedSkeleton,
                 std::map<FbxNode*,
                          osg::Node*>& nodeMap,
                 FbxScene&             fbxScene )
{
    if( bNeedSkeleton )
    {
        ...
        osgBone->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    else
    {
        ...
        {
            osgAnimation::UpdateMatrixTransform* pUpdate =
                new osgAnimation::UpdateMatrixTransform( animName );
            readUpdateMatrixTransform( pUpdate, pNode, fbxScene );
...
OsgFbxReader::readFbxNode( FbxNode* pNode,
                           bool&    bIsBone,
                           int&     nLightCount )
{
    ...
    for( int i = 0; i < nChildCount; ++i )
    {
        ...
        osgDB::ReaderWriter::ReadResult childResult =
            readFbxNode( pChildNode, bChildIsBone, nLightCount );
        ...
        case FbxNodeAttribute::eMesh :
            {
                ...
                osgDB::ReaderWriter::ReadResult meshRes =
                    readFbxMesh( pNode, stateSetList );
                if( meshRes.error() )
                {
                    return meshRes;
                }
                else if( osg::Node* node = meshRes.getNode() )
                {
                    ...
                    {
                        // The mesh is skinned therefore the bind matrix will handle all
                        // transformations.
                        localMatrix          = osg::dmat4();
        ...
        case FbxNodeAttribute::eLight :
            {
                osgDB::ReaderWriter::ReadResult res =
                    lAttributeType == FbxNodeAttribute::eCamera
                        ? readFbxCamera( pNode )
                        : readFbxLight( pNode, nLightCount );
                if( res.error() )
                {
                    return res;
                }
                else if( osg::Group* resGroup =
                             dynamic_cast<osg::Group*>( res.getObject() ) )
                {
                    bEmpty = false;
                    if( animName.empty() && bLocalMatrixIdentity )
                    {
                        osgGroup = resGroup;
                    }
                    else
    ...
    if( !osgGroup )
    {
        osgGroup = createGroupNode( pSdkManager,
                                    pNode,
                                    animName,
                                    localMatrix,
                                    bIsBone,
    ...
    if( bCreateSkeleton )
    {
        osgAnimation::Skeleton* osgSkeleton =
            getSkeleton( pNode, fbxSkeletons, skeletonMap );
...
getSkeleton( FbxNode*                           fbxNode,
             const std::set<const FbxNode*>&    fbxSkeletons,
             std::map<FbxNode*,
                      osgAnimation::Skeleton*>& skeletonMap )
{
    // Find the first non-skeleton ancestor of the node.
    while( fbxNode && ( ( fbxNode->GetNodeAttribute() &&
    ...
    std::map<FbxNode*, osgAnimation::Skeleton*>::const_iterator it =
        skeletonMap.find( fbxNode );
    if( it == skeletonMap.end() )
    {
        osgAnimation::Skeleton* skel = new osgAnimation::Skeleton;
        skel->setDefaultUpdateCallback();
        skeletonMap.insert( std::pair<FbxNode*, osgAnimation::Skeleton*>( fbxNode,
                                                                          skel ) );
        return skel;
    }
    else
