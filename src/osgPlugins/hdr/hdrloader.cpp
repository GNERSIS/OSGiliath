...
static void
rawRGBEData( RGBE*  scan,
             int    len,
             float* cols );
static void
workOnRGBE( RGBE*  scan,
            int    len,
            float* cols );
static bool
decrunch( RGBE* scanline,
          int   len,
          FILE* file );
static bool
oldDecrunch( RGBE* scanline,
             int   len,
             FILE* file );
...
read_unsigned_char( FILE* stream,
                    int&  error )
{
    if( error )
    {
...
bool
HDRLoader::isHDRFile( const char* _fileName )
{
    FILE* file;
...
bool
HDRLoader::load( const char*      _fileName,
                 const bool       _rawRGBE,
                 HDRLoaderResult& _res )
{
    int   i;
    char  str[200];
    FILE* file;
             ...
             FILE* _file )
{
    ...
    while( _len > 0 && !error )
    {
        ...
        if( _scanline[0][R] == 1 && _scanline[0][G] == 1 && _scanline[0][B] == 1 )
        {
