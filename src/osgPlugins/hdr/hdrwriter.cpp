/*
 ...
 Modified for OSG September 2007 david.spilling@gmail.com :
    The file format is described fully in
 http://radsite.lbl.gov/radiance/refer/filefmts.pdf For the moment, we don't output most
...
bool
HDRWriter::writeRAW( const osg::Image* img,
                     std::ostream&     fout )
{
    bool result = true;
...
#define MAXELEN        0X7F'FF    // maximum scanline length for encoding

/* default minimal header. modify if you want more information in header */
bool
HDRWriter::writeHeader( const osg::Image* img,
                        std::ostream&     fout )
{
    std::stringstream stream;    // for conversion to strings

    stream << "#?RADIANCE\n";    // Could be RGBE, but some 3rd party software doesn't
                                 // interpret the file correctly if it is.
    stream << "FORMAT=32-bit_rle_rgbe\n\n";

    // Our image data is usually arranged row major, with the origin at the bottom left
    // of the image. Based on the Radiance file format, this would be "+Y blah +X blah".
    // However, no software (including HDRShop v1!) seems to support this; they all
    // expect -Y blah +X blah, and then flip the image. This is unfortunate, and is what
    // provokes the default behaviour of OSG having to flip the image prior to write.
...
bool
HDRWriter::writeNoRLE( std::ostream&     fout,
                       const osg::Image* img )
{
...
bool
HDRWriter::writePixelsRAW( std::ostream&  fout,
                           unsigned char* data,
                           int            numpixels )
{
    unsigned char rgbe[4];
    ...
    while( numpixels-- > 0 )
    {
        ...
        data    += RGBE_DATA_SIZE;
        fout.write( reinterpret_cast<const char*>( rgbe ),
                    sizeof( rgbe ) );    // img->getTotalSizeInBytesIncludingMipmaps()
    }
    return true;
}
...
bool
HDRWriter::writeBytesRLE( std::ostream&  fout,
                          unsigned char* data,
                          int            numbytes )
{
#define MINRUNLENGTH 4
    ...
    while( cur < numbytes )
    {
        ...
        while( cur < beg_run )
        {
            ...
            fout.write( reinterpret_cast<const char*>( &data[cur] ),
                        sizeof( data[0] ) * nonrun_count );
            // if (fwrite(&data[cur],sizeof(data[0])*nonrun_count,1,fp) < 1) return
            // false;
            cur += nonrun_count;
        }
...
bool
HDRWriter::writeRLE( const osg::Image* img,
                     std::ostream&     fout )
{
    ...
    for( int row = 0; row < num_scanlines; ++row )
    {
        ...
        */
        ...
        for( int i = 0; i < 4; i++ )
        {
            if( writeBytesRLE( fout, &buffer[i * scanline_width], scanline_width ) !=
                true )
            {
                free( buffer );
                return false;
