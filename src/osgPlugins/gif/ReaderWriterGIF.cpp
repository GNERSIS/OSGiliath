...
class GifImageStream : public osg::ImageStream,
                       public osg::Thread
{
    public:
        ...
        run()
        {
            ...
            while( !_done )
            {
                if( _status == PLAYING && ( *_dataIter ) )
                {
                    ...
                    osg::Thread::microSleep( static_cast<int>( 10000.0F *
                                                               _multiplier ) );
                }
                else
                {
                    osg::Thread::microSleep( 150'000L );
                }
            }
        }

        void
        addToImageStream( int            ss,
                          int            tt,
                          int            rr,
                          int            numComponents,
                          int            delayTime,
                          unsigned char* imgData )
        {
            if( isRunning() )
            {
                OSG_WARN << "GifImageStream::addToImageStream: thread is running!"
                         << std::endl;
                return;
            }
            ...
            GLenum pixelFormat    = numComponents == 1 ? GL_RED
                                  : numComponents == 2 ? GL_RG
                                  : numComponents == 3 ? GL_RGB
                                  : numComponents == 4 ? GL_RGBA
                                                       : ( GLenum )-1;
                     ...
                     int                  headerlen )
{
    ...
    if( memcmp( ( const void* )header, ( const void* )gifcmp, 3 ) == 0 )
    {
        return 1;
    }
    return 0;
...
int
gif_read_stream( GifFileType* gfile,
                 GifByteType* gdata,
                 int          glength )
{
        ...
        switch( recordtype )
        {
            case IMAGE_DESC_RECORD_TYPE :
                ...
                if( gif_num == 2 )
                {
                    *obj = new GifImageStream;
                    ( *obj )->addToImageStream( giffile->SWidth,
                                                giffile->SHeight,
                                                1,
                                                4,
                                                delaytime,
                ...
                if( giffile->Image.Left +
                    giffile->Image.Width >
                    giffile->SWidth ||
                    giffile->Image.Top +
                    giffile->Image.Height > giffile->SHeight )
                {
            ...
            case EXTENSION_RECORD_TYPE :
                ...
                else if( extcode == 0XF9 )
                {
                    ...
                    delaytime =
                        ( extension[3] << 8 ) +
                        extension[2];    // minimum unit 1/100s, so 8 here means 8/100s
                }
...
class ReaderWriterGIF : public osgDB::ReaderWriter
{
    public:
        ...
        ReadResult
        readGIFStream( std::istream& fin ) const
        {
