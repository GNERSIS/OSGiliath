...
EXTERN_GUID( CLSID_WMMUTEX_Language,
             0XD6'E2'2A'00,
             0X35'DA,
             0X11'D1,
             0X90,
             ...
             0X49,
             0XBE );
EXTERN_GUID( CLSID_WMMUTEX_Bitrate,
             0XD6'E2'2A'01,
             0X35'DA,
             0X11'D1,
             0X90,
             ...
             0X49,
             0XBE );
EXTERN_GUID( CLSID_WMMUTEX_Presentation,
             0XD6'E2'2A'02,
             0X35'DA,
             0X11'D1,
             0X90,
             ...
             0X49,
             0XBE );
EXTERN_GUID( CLSID_WMMUTEX_Unknown,
             0XD6'E2'2A'03,
             0X35'DA,
             0X11'D1,
             0X90,
             ...
             0X49,
             0XBE );
EXTERN_GUID( CLSID_WMBandwidthSharing_Exclusive,
             0XAF'60'60'AA,
             0X51'97,
             0X11'D2,
             0XB6,
             ...
             0X08,
             0XE9 );
EXTERN_GUID( CLSID_WMBandwidthSharing_Partial,
             0XAF'60'60'AB,
             0X51'97,
             0X11'D2,
             0XB6,
...
const NamedGuid MediaType[] = {
    ...
    {&CLSID_AnalogVideoDecoderPropertyPage,
     TEXT( "CLSID_AnalogVideoDecoderPropertyPage\0" )                                   },
    ...
    { &CLSID_VideoStreamConfigPropertyPage,
     TEXT( "CLSID_VideoStreamConfigPropertyPage\0" )                                    },
    ...
    {  &CLSID_WMBandwidthSharing_Exclusive,
     TEXT( "CLSID_WMBandwidthSharing_Exclusive\0" )                                     },
    ...
    if( pFilter && pFormat && ppPin )
    {
        ...
        if( SUCCEEDED( hr ) )
        {
            ...
            while( S_OK == pIEnumPins->Next( 1, &pIPin, NULL ) )
            {
                ...
                if( pinDir == PinDir )
                {
                    // match pin direction check the first media type returned from the
                    // upstream pin
...
//-----------------------------------------------------------------------------
// FindPinInterface
// Attempt to locate the interface on the pin with the specified format or on the first
// pin if no format is provided.
HRESULT
FindPinInterface( IBaseFilter*  pFilter,
                  const GUID*   pFormat,
                  PIN_DIRECTION PinDir,
                  const IID&    riid,
                  void**        ppvInterface )
...
std::string
getStringFromGUID( const GUID* pGUID )
{
    int i = 0;
...
const GUID*
getGUIDFromString( const std::string& str )
{
    int i = 0;
    // Find format GUID's name in the named guids table
    while( MediaType[i].pguid != 0 )
...
static bool
checkError( const std::string& prefix,
            HRESULT            hr )
{
    if( FAILED( hr ) )
    {
...
bool
CTextureRenderer::initBuildGraph()
{
    ...
    checkError( prefixForMessage,
                hr );    // May be no sound so don't effect return result
...
struct ListDeviceAvailable
{
        ...
        ListDeviceAvailable( IEnumMoniker* enumMoniker ) :
            _enumMoniker( enumMoniker )
        {
            createList();
        }
        ...
        void
        displayDevicesFound( const std::string&  prefixForMessage,
                             osg::NotifySeverity severity = osg::NOTICE ) const
        {
            for( int i = 0; i < ( int )_listDevice.size(); i++ )
            {
                OSG_NOTIFY( severity )
                    << prefixForMessage << " device \"" << _listDevice[i]._name
                    << "\" clsid " << _listDevice[i]._clsid << std::endl;
            }
        }

        DeviceEntry
        getDevice( const std::string& name )
        {
            for( int i = 0; i < ( int )_listDevice.size(); i++ )
            {
        ...
        createList()
        {
            ...
            while( _enumMoniker->Next( 1, &device, NULL ) == S_OK )
            {
                ...
                hr =
                    device->BindToStorage( 0, 0, IID_IPropertyBag, ( void** )&pPropBag );
                ...
                if( SUCCEEDED( hr ) )
                {
                    ...
                    if( !FAILED( hr ) )
                    {
                        ...
                        WideCharToMultiByte( CP_ACP,
                                             ...
                                             NULL,
                                             NULL );
                        // OSG_NOTICE << "device \"" << truename << "\" id " << szCLSID
                        // << std::endl;
...
struct ListCapDeviceAvailable
{
        ...
        ListCapDeviceAvailable( IAMStreamConfig* config ) :
            _config( config )
        {
            createList();
        }
        ...
        void
        displayCapsFound( const std::string& prefixForMessage = "" ) const
        {
            for( int i = 0; i < ( int )_capsList.size(); i++ )
            {
        ...
        static void
        displayCap( VIDEOINFOHEADER*   video,
                    const std::string& prefix = "" )
        {
            ...
            GUID      g1 = ( GUID )fccMap;
            OSG_NOTICE << prefix << " cap " << video->bmiHeader.biWidth << " x "
                       << video->bmiHeader.biHeight << " bit per pixel "
                       << video->bmiHeader.biBitCount << " (" << getStringFromGUID( &g1 )
                       << ") at " << fps << " fps" << std::endl;
        }

        std::pair<AM_MEDIA_TYPE*,
                  VIDEOINFOHEADER*>
        getCaps( int    width,
                 int    height,
                 double fps )
        {
            ...
            for( int i = 0; i < ( int )filterResolution.size(); i++ )
            {
                ...
                filterFrameRate.insert(
                    std::pair<double, CapEntry>( error, filterResolution[i] )
                );
            }

            CapEntry nullCapEntry( static_cast<AM_MEDIA_TYPE*>( NULL ),
                                   static_cast<VIDEOINFOHEADER*>( NULL ) );
        ...
        createList()
        {
            ...
            {
                // GetStreamCaps allocats the AM_MEDIA_TYPE, which must be deleted by
                // using DeleteMediaType
                AM_MEDIA_TYPE* pmt = NULL;
...
bool
CTextureRenderer::setupOutputSoundDevice( ICreateDevEnum* devs )
{
    if( !devs )
    {
        return false;
    ...
    IEnumMoniker* audioRenderer = 0;
    hr = devs ? devs->CreateClassEnumerator( CLSID_AudioRendererCategory,
                                             &audioRenderer,
                                             0 )
              : 0;
    if( !checkError( prefixForMessage, hr ) )
    ...
    if( !device._device )
    {
        OSG_WARN << prefixForMessage << " no output sound device \"" << outputdevice
                 << "\" found" << std::endl;
        return false;
    }
    ...
    hr = mon ? mon->BindToObject( 0, 0, IID_IBaseFilter, ( void** )&_soundOutputDevice )
             : 0;
...
bool
CTextureRenderer::openVideoCaptureDevice( const std::string& capture,
                                          int                wantWidth,
                                          int                wantHeight,
                                          double             wantFps )
{
    ...
    if( !device._device )
    {
        OSG_WARN << _imageStream.get() << " no capture device \"" << capture
                 << "\" found" << std::endl;
        return false;
    }
    _videoCaptureDeviceName = device._name;
    OSG_NOTICE << _imageStream.get() << " use capture device \""
               << getVideoCaptureDeviceName() << "\"" << std::endl;
    ...
    hr = mon ? mon->BindToObject( 0, 0, IID_IBaseFilter, ( void** )&_videoCaptureDevice )
             : 0;
    if( !checkError( prefixForMessage, hr ) )
    {
        return false;
    ...
    IEnumPins* pins = 0;
    hr              = _videoCaptureDevice ? _videoCaptureDevice->EnumPins( &pins ) : 0;
    if( !checkError( prefixForMessage, hr ) )
    {
        return false;
    }
    ...
    IAMStreamConfig* capConfig = 0;
    hr = cap->QueryInterface( IID_IAMStreamConfig, ( void** )&capConfig );
    if( capConfig )
    {
        ListCapDeviceAvailable capsDevice( capConfig );
        capsDevice.displayCapsFound( prefixForMessage );
        ListCapDeviceAvailable::CapEntry found =
            capsDevice.getCaps( wantWidth, wantHeight, wantFps );
        if( found.first )
        {
            ListCapDeviceAvailable::displayCap( found.second,
                                                prefixForMessage + " use " );
...
bool
CTextureRenderer::openSoundCaptureDevice( const std::string& capture,
                                          int                nbChannels )
{
    std::string prefixForMessage;
    {
    ...
    IEnumMoniker* captureDevices = 0;
    hr = devs ? devs->CreateClassEnumerator( CLSID_AudioInputDeviceCategory,
                                             &captureDevices,
                                             0 )
              : 0;
    if( !checkError( prefixForMessage, hr ) )
    ...
    if( !device._device )
    {
        OSG_WARN << _imageStream.get() << " no sound capture device \"" << capture
                 << "\" found" << std::endl;
        return false;
    }
    _soundCaptureDeviceName = device._name;
    OSG_NOTICE << _imageStream.get() << " use sound capture device \""
               << getSoundCaptureDeviceName() << "\"" << std::endl;
    ...
    hr = mon ? mon->BindToObject( 0, 0, IID_IBaseFilter, ( void** )&_soundCaptureDevice )
             : 0;
    if( FAILED( hr ) )
    {
        checkError( prefixForMessage, hr );
    ...
    hr                    = FindPinInterface( _soundCaptureDevice,
                                              &MEDIATYPE_Audio,
                                              PINDIR_OUTPUT,
                                              IID_IAMStreamConfig,
                                              reinterpret_cast<void**>( &pISC ) );
    if( FAILED( hr ) )
    {
        checkError( prefixForMessage, hr );
        return false;
    ...
    if( SUCCEEDED( hr ) )
    {
        if( sizeof( AUDIO_STREAM_CONFIG_CAPS ) == size )
        {
            ...
            {
                hr =
                    pISC->GetStreamCaps( index, &pmt, reinterpret_cast<BYTE*>( &ascc ) );
                if( SUCCEEDED( hr ) )
                {
                    TCHAR buffer[32];
                    ...
                    {
                        StringCbPrintf( buffer,
                                        sizeof( buffer ),
                                        TEXT( "%d channel, %2.1fkHz, %d-bit" ),
                                        ( int )pwfex->nChannels,
                                        ( float )pwfex->nSamplesPerSec / 1'000,
                                        ( int )pwfex->wBitsPerSample );
                    }
                    else
                    {
                        StringCbPrintf( buffer,
                                        sizeof( buffer ),
                                        TEXT( "%d channels, %2.1fkHz, %d-bit" ),
                                        ( int )pwfex->nChannels,
                                        ( float )pwfex->nSamplesPerSec / 1'000,
                                        ( int )pwfex->wBitsPerSample );
                    }
                    ...
                    {
                        pISC->SetFormat( pmt );
                        OSG_NOTICE << prefixForMessage << " use format " << buffer
                                   << std::endl;
                        break;
                    }
                    else
        ...
        {
            OSG_WARN << prefixForMessage << " can t retrieve information pins"
                     << std::endl;
        }
    }
    if( pISC )
    ...
    pISC                            = 0;
    IPin* captureOutputDevicePinOut = 0;
    hr = _soundCaptureDevice
    ...
    hr = _soundOutputDevice->FindPin( L"Audio Input pin (rendered)",
                                      &soundOutputDevicePinIn );
    if( FAILED( hr ) )
    {
...
bool
CTextureRenderer::openCaptureDevices( const DirectShowImageStream::Options& o )
{
    std::string prefixForMessage;
    {
        std::stringstream ss;
    ...
    for( DirectShowImageStream::Options::iterator it = options.begin();
         it != options.end();
         it++ )
    {
        OSG_NOTICE << prefixForMessage << " option " << it->first << " = " << it->second
                   << std::endl;
    }
    ...
    OSG_NOTICE << prefixForMessage << " try to open video capture device "
               << videoCaptureDevice;
    ...
    {
        OSG_WARN << prefixForMessage << " failed to setup sound capture device "
                 << soundCaptureDevice << std::endl;
    }
    return true;
}

bool
CTextureRenderer::openFile( const std::string& file )
{
    syncStreams( true );
    ...
    // Find the video pin
    hr = _fileSource
           ? ::GetPin( _fileSource, &MEDIATYPE_Video, PINDIR_OUTPUT, &videoOutputPin )
           : 0;
    ...
    if( soundFilter )
    {
        ...
        hr =
            ::GetPin( soundFilter, &MEDIATYPE_Audio, PINDIR_OUTPUT, &soundStreamPinOut );
        ...
        if( FAILED( hr ) )
        {
            OSG_WARN << prefixForMessage << " can't find audio pin"
                     << getErrorMessage( hr ) << std::endl;
        }
...
//-----------------------------------------------------------------------------
void
CTextureRenderer::releaseRessources()
{
    // Shut down the graph
    if( _mediaControl )
    {
...
CTextureRenderer::CTextureRenderer( DirectShowImageStream* is,
                                    HRESULT*               valid ) :
...
CTextureRenderer::CheckMediaType( const CMediaType* pmt )
{
    ...
    {
        OSG_NOTICE << _imageStream.get() << " Texture Renderer use media "
                   << getStringFromGUID( pmt->Subtype() ) << std::endl;
        hr = S_OK;
    }
    else
    {
        OSG_INFO << _imageStream.get() << " Texture Renderer check media "
                 << getStringFromGUID( pmt->Subtype() ) << std::endl;
    }
...
void
CTextureRenderer::syncStreams( bool state )
{
    if( state == true )
    {
        _dropFrame = !S_OK;
...
HRESULT
CTextureRenderer::ShouldDrawSampleNow( IMediaSample*   sample,
...
DirectShowImageStream::DirectShowImageStream()
{
    ...
    if( FAILED( hr ) )
    {
        OSG_WARN << this << " error in constructor " << getErrorMessage( hr )
                 << std::endl;
    }
}

DirectShowImageStream::DirectShowImageStream( const DirectShowImageStream& d,
                                              const osg::CopyOp&           c ) :
    osg::ImageStream( d,
                      c )
{
...
bool
DirectShowImageStream::openFile( const std::string& file )
{
    HRESULT valid = S_OK;
...
bool
DirectShowImageStream::openCaptureDevices()
{
    HRESULT valid = S_OK;
    _renderer     = new CTextureRenderer( this, &valid );
    if( FAILED( valid ) )
...
DirectShowImageStream::play()
{
    ...
    if( _renderer.valid() && _renderer->_mediaControl )
    {
        ...
        if( FAILED( hr ) )
        {
            OSG_WARN << this << " can't run the graph " << getErrorMessage( hr )
                     << std::endl;
        }
        else
        {
...
void
DirectShowImageStream::rewind()
{
    seek( 0 );
}

osg::ImageStream::StreamStatus
DirectShowImageStream::getStatus()
{
    std::lock_guard<std::mutex> lock( _mutex );
    return _status;
}
...
void
DirectShowImageStream::seek( double time )
{
    std::lock_guard<std::mutex> lock( _mutex );
    if( _renderer.valid() && _renderer->_mediaSeeking )
    {
        ...
        HRESULT  hr =
            _renderer->_mediaSeeking->SetPositions( &start2,
...
double
DirectShowImageStream::getCurrentTime() const
{
    std::lock_guard<std::mutex> lock( _mutex );
    ...
    if( _renderer.valid() && _renderer->_mediaSeeking )
    {
        ...
        else
        {
            ...
            currentTime =
                currentTime *
                ( 100.0 * 1E-9 );    // default unit in directshow IMediaSeeking
        }
    }
    return currentTime;
}
...
void
DirectShowImageStream::setOptions( const Options& map )
{
...
void
DirectShowImageStream::quit( bool waitForThreadToExit )
{
    stop();
}

double
DirectShowImageStream::getLength() const
{
    std::lock_guard<std::mutex> lock( _mutex );
    double                      duration = 0;
    if( _renderer.valid() && _renderer->_mediaSeeking )
    {
        ...
        duration =
            duration * ( 100.0 * 1E-9 );    // default unit in directshow IMediaSeeking
    }
    return duration;
}

double
DirectShowImageStream::getFrameRate() const
{
    std::lock_guard<std::mutex> lock( _mutex );
    int                         frameRate = 0;
    if( _renderer.valid() )
...
void
DirectShowImageStream::setTimeMultiplier( double rate )
{
    std::lock_guard<std::mutex> lock( _mutex );
    if( _renderer.valid() && _renderer->_mediaSeeking )
    {
...
double
DirectShowImageStream::getTimeMultiplier() const
{
    std::lock_guard<std::mutex> lock( _mutex );
    double                      rate = 1.0;
    if( _renderer.valid() && _renderer->_mediaSeeking )
...
void
DirectShowImageStream::setVolume( float vol )
{
    std::lock_guard<std::mutex> lock( _mutex );
...
float
DirectShowImageStream::getVolume() const
{
    std::lock_guard<std::mutex> lock( _mutex );
    double                      vol = 0;
    if( _renderer.valid() && _renderer->_basicAudio )
