 ...
 * ReaderWriterGZ, derived from ReaderWriter.
 * Provides: className, openArchive, readObject, readFile, readImage, readFile.
 */
#include <fstream>
#include <iostream>
#include <iterator>
...
class ReaderWriterGZ : public osgDB::ReaderWriter
{
    public:
        ...
        /// return the reader for fullFileName and a outuncompessed stream fetching this
        /// file in the archive
        osgDB::ReaderWriter*
        getStreamAndReader( std::stringstream& outuncompessed,
                            std::istream&      fin,
                            const std::string& fullFileName ) const
        {
            ...
            OSG_INFO << className() << "::getStreamAndReader:" << baseExt
                     << " ReaderWriter " << rw << std::endl;
        ...
        readImage( std::istream&                       fin,
                   const osgDB::ReaderWriter::Options* local_opt = NULL ) const
        {
            std::string fullFileName =
                local_opt->getPluginStringData( "STREAM_FILENAME" );
            std::string ext = osgDB::getLowerCaseFileExtension( fullFileName );
        ...
        WriteResult
        writeFile( ObjectType                          objectType,
        ...
        read( std::istream&      fin,
              std::stringstream& destination ) const;
        bool
...
ReaderWriterGZ::readFile( ObjectType                          objectType,
                          osgDB::ReaderWriter*                rw,
                          std::istream&                       fin,
                          const osgDB::ReaderWriter::Options* options ) const
{
                          ...
                          const osgDB::ReaderWriter::Options* options ) const
{
    ...
    osg::ref_ptr<Options> local_opt =
        options ? static_cast<Options*>( options->clone( osg::CopyOp::SHALLOW_COPY ) )
                : new Options;
...
ReaderWriterGZ::writeFile( ObjectType                          objectType,
                           const osg::Object*                  object,
                           osgDB::ReaderWriter*                rw,
                           std::ostream&                       fout,
                           const osgDB::ReaderWriter::Options* options ) const
{
    switch( objectType )
    {
        ...
        case( IMAGE ) :
            return rw->writeImage( *( dynamic_cast<const osg::Image*>( object ) ),
                                   fout,
                                   options );
        case( HEIGHTFIELD ) :
            return rw->writeHeightField(
                *( dynamic_cast<const osg::HeightField*>( object ) ),
                fout,
                options
            );
        case( NODE ) :
            return rw->writeNode( *( dynamic_cast<const osg::Node*>( object ) ),
                                  fout,
                                  options );
        default :
            break;
...
bool
ReaderWriterGZ::read( std::istream&      fin,
                      std::stringstream& destination ) const
{
    int           ret;
    unsigned      have;
...
bool
ReaderWriterGZ::write( std::ostream&            fout,
                       const std::stringstream& sourcestream ) const
{
