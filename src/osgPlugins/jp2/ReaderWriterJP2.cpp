...
extern "C"
{
             ...
             int           numcmpts )
    {
        ...
        for( y = height - 1; y >= 0; --y )
        {
            for( cmptno = 0; cmptno < numcmpts; ++cmptno )
            {
                if( jas_image_readcmpt( image, cmptno, 0, y, width, 1, data[cmptno] ) )
                {
                    goto done;
                }
             ...
             jas_image_t*  image )
    {
        ...
        // for (y = 0; y < height; ++y)
        {
            ...
            for( cmptno = 0; cmptno < numcmpts; ++cmptno )
            {
                if( jas_image_writecmpt( image, cmptno, 0, y, width, 1, data[cmptno] ) )
                {
                    goto done;
                }
            }
...
class ReaderWriterJP2 : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterJP2()
        {
            ...
            // little dance here to get around warnings created by jas_image_strtofmt use
            // of char* rather than const char* as a parameted and modern compilers
            // deprecating "jp2" string being treated as char*.
            char* jp2 = strdup( "jp2" );
                   ...
                   const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            jas_stream_t* in = jas_stream_freopen(
                fileName.c_str(),
                "rb",
                fileHandle
            );    // Replacement for jas_stream_fopen() to be able to support UTF8
            if( !in )
            {
                fclose( fileHandle );
                return ReadResult::ERROR_IN_READING_FILE;
            ...
            jas_image_t* jimage = jas_image_decode(
                in,
                -1,
                opt
            );    // last is the option string whatto put there?
            if( opt )
            {
                delete[] opt;
            }
            ...
            jas_stream_t*  mem =
                jas_stream_memopen( ( char* )data, internalFormat * s * t );
            ...
            unsigned int pixelFormat = internalFormat == 1 ? GL_RED
                                     : internalFormat == 2 ? GL_RG
                                     : internalFormat == 3 ? GL_RGB
                                     : internalFormat == 4 ? GL_RGBA
                                                           : ( GLenum )-1;
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( !img.isDataContiguous() )
            {
                OSG_WARN << "Warning: Writing of image data, that is non contiguous, is "
                            "not supported by JPEG2000 plugin."
                         << std::endl;
            ...
            int                   internalFormat =
                osg::Image::computeNumComponents( img.getPixelFormat() );

            jas_stream_t* mem = jas_stream_memopen( ( char* )img.data(),
                                                    internalFormat * img.s() * img.t() );
            ...
            if( !( jimage = jas_image_create( internalFormat,
                                              cmptparms,
                                              JAS_CLRSPC_UNKNOWN ) ) )
            {
                return WriteResult::ERROR_IN_WRITING_FILE;
            ...
            jas_stream_t* out = jas_stream_freopen(
                fileName.c_str(),
                "wb",
                fileHandle
            );    // Replacement for jas_stream_fopen() to be able to support UTF8
                    ...
                    const Options*    options ) const
        {
            ...
            // now the encoded jp2 image resides in the out->buf_ member with size
            // out->len_ we now need to stream it to a std::ostream
