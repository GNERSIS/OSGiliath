 ...
 * LuaCallbackObject, derived from CallbackObject.
 * Provides: setName, run, lse, lua_rawgeti, getRef, PushStackValueVisitor.
 */
...
class LuaCallbackObject : public osg::CallbackObject
{
    public:
        ...
        LuaCallbackObject( const std::string&     methodName,
                           const LuaScriptEngine* lse,
                           int                    ref ) :
            _lse( lse ),
            _ref( ref )
        {
             ...
             osg::Parameters& outputParameters ) const
        {
            if( !_lse )
            {
                OSG_NOTICE << "Warning: Ignoring call to Lua by an expired callback"
                           << std::endl;
                return false;
            }

            // a strong reference is necessary as the lua call might trigger deletion of
            // the LuaScriptEngine object avoid overhead by observer_ptr<>::lock as a
            // race on run/destruction never is valid
            ...
            {
                OSG_NOTICE << "Lua error : " << lua_tostring( lse->getLuaState(), -1 )
                           << std::endl;
                return false;
            }
            ...
            {
                outputParameters.insert( outputParameters.begin(),
                                         lse->popParameterObject() );
            }
            return true;
        }
...
static int
getProperty( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
//
static int
getContainerProperty( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( n == 2 && lua_type( _lua, 1 ) == LUA_TTABLE )
    {
        if( lua_type( _lua, 2 ) == LUA_TSTRING )
        {
            std::string  propertyName = lua_tostring( _lua, 2 );
            osg::Object* object       = lse->getObjectFromTable<osg::Object>( 1 );
            std::string  containerPropertyName =
                lse->getStringFromTable( 1, "containerPropertyName" );
        ...
        else if( lua_type( _lua, 2 ) == LUA_TNUMBER )
        {
            ...
            osgDB::BaseSerializer*      bs =
                lse->getClassInterface().getSerializer( object,
...
static int
setContainerProperty( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
static int
getContainerSize( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    osgDB::BaseSerializer::Type type;
    osgDB::BaseSerializer*      bs =
        lse->getClassInterface().getSerializer( object, containerPropertyName, type );
...
static int
callVectorClear( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
static int
callVectorResize( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
static int
callVectorReserve( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
        ...
        else
        {
            ...
            if( ms )
            {
                ...
                if( keysp.dataType == ms->getKeyType() )
                {
                    ...
                    if( dataPtr )
                    {
                        SerializerScratchPad valuesp( ms->getElementType(),
                                                      dataPtr,
                                                      ms->getElementSize() );
                        return lse->pushDataToStack( &valuesp );
                    }
    ...
    if( n == 3 && lua_type( _lua, 1 ) == LUA_TTABLE )
    {
        ...
        else
        {
            ...
            if( ms )
            {
                ...
                if( keysp.dataType ==
                    ms->getKeyType() &&
                    ms->getElementType() == valuesp.dataType )
                {
                ...
                {
                    OSG_NOTICE << "Warning: Lua setMapProperty() : Failed to matched "
                                  "map element "
                               << std::endl;
                    OSG_NOTICE << "                                keysp.dataType="
                               << keysp.dataType << std::endl;
                    OSG_NOTICE << "                                valuesp.dataType="
                               << valuesp.dataType << std::endl;
                    return 0;
                }
            }
        }
...
static int
createMapIterator( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
static int
createMapReverseIterator( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
//
static int
callMapIteratorAdvance( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    osgDB::MapIteratorObject* mio =
        lse->getObjectFromTable<osgDB::MapIteratorObject>( 1 );
    if( mio )
    {
...
static int
callMapIteratorValid( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
...
static int
getMapIteratorKey( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( mio )
    {
        ...
        if( dataPtr )
        {
            SerializerScratchPad valuesp( mio->getKeyType(),
                                          dataPtr,
                                          mio->getKeySize() );
            return lse->pushDataToStack( &valuesp );
        }
...
static int
getMapIteratorElement( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( mio )
    {
        ...
        if( dataPtr )
        {
            SerializerScratchPad valuesp( mio->getElementType(),
                                          dataPtr,
                                          mio->getElementSize() );
            return lse->pushDataToStack( &valuesp );
        }
...
static int
setMapIteratorElement( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( mio )
    {
        ...
        {
            OSG_NOTICE << "Warning: Lua setMapIteratorElement() : Failed to matched map "
                          "element type, valuesp.dataType="
                       << valuesp.dataType << std::endl;
            return 0;
        }
    }
...
//
static int
convertStringToStateAttributeValue( const std::string&                 valueString,
                                    osg::StateAttribute::OverrideValue defaultValue,
                                    bool&                              setOnOff )
{
...
static std::string
convertStateAttributeValueToString( unsigned int value,
                                    bool         withOnOffCheck )
{
...
static int
callStateSetSet( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( !stateset )
    {
        OSG_NOTICE << "Warning: StateSet:add() can only be called on a StateSet"
                   << std::endl;
        return 0;
    }
    ...
    if( lua_type( _lua, 2 ) == LUA_TTABLE )
    {
        ...
        {
            value = convertStringToStateAttributeValue( lua_tostring( _lua, 3 ),
                                                        value,
                                                        setOnOff );
        }
    ...
    else if( lua_type( _lua, 2 ) == LUA_TNUMBER )
    {
        ...
        if( n >= 3 )
        {
            if( lua_type( _lua, 3 ) == LUA_TTABLE )
            {
                ...
                {
                    value = convertStringToStateAttributeValue( lua_tostring( _lua, 4 ),
                                                                value,
                                                                setOnOff );
                }
            ...
            else if( lua_type( _lua, 3 ) == LUA_TSTRING )
            {
                ...
                stateset->setTextureMode( static_cast<unsigned int>( index ),
                                          mode,
                                          value );
                return 0;
            }
    ...
    OSG_NOTICE << "Warning: StateSet:set() inappropriate parameters, use form:"
               << std::endl;
    OSG_NOTICE << "   StateSet:set(modestring [,value=\"ON,OFF,OVERRIDE,PROTECTED\"]); "
               << std::endl;
    OSG_NOTICE << "   StateSet:set(uniform [,value=\"ON,OFF,OVERRIDE,PROTECTED\"]); "
               << std::endl;
    OSG_NOTICE << "   StateSet:set(attribute [,value=\"ON,OFF,OVERRIDE,PROTECTED\"]); "
               << std::endl;
    OSG_NOTICE << "   StateSet:set(textureUnit, textureAttribute "
                  "[,value=\"ON,OFF,OVERRIDE,PROTECTED\"]); "
               << std::endl;
    return 0;
}

static int
callStateSetGet( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( !stateset )
    {
        OSG_NOTICE << "Warning: StateSet:get() can only be called on a StateSet"
                   << std::endl;
        return 0;
    }
    ...
    if( lua_type( _lua, 2 ) == LUA_TNUMBER )
    {
        if( n < 3 )
        {
            OSG_NOTICE << "Warning: StateSet:get() must be in form get(textureUnit, "
                          "ClassName|ModeName|ObjectName)"
                       << std::endl;
            return 0;
        }
        ...
        if( lua_type( _lua, 3 ) == LUA_TTABLE )
        {
            ...
            if( sa && sa->isTextureAttribute() )
            {
                ...
                {
                    const osg::StateSet::AttributeList& al =
                        stateset->getTextureAttributeList()[index];
                         ...
                         ++itr )
                    {
                        if( itr->second.first == sa )
                        {
                            ...
                            lua_pushstring(
                                _lua,
                                convertStateAttributeValueToString( itr->second.second,
                                                                    false )
                                    .c_str()
                            );
                            lua_settable( _lua, -3 );
                ...
                OSG_NOTICE << "Warning: StateSet:get() Could not find attribute : "
                           << sa->className() << std::endl;
                lua_pushnil( _lua );
                return 1;
            }
            lua_pushnil( _lua );
        ...
        else if( lua_type( _lua, 3 ) == LUA_TSTRING )
        {
            ...
            if( stateset->getTextureModeList().size() > index )
            {
                ...
                const osg::StateSet::ModeList& ml =
                    stateset->getTextureModeList()[index];
                     ...
                     ++itr )
                {
                    ...
                    {
                        lua_pushstring( _lua,
                                        convertStateAttributeValueToString( itr->second,
                                                                            true )
                                            .c_str() );
                        return 1;
                    }
            ...
            OSG_NOTICE << "Warning: StateSet:get() Could not find attribute : " << value
                       << std::endl;
            lua_pushnil( _lua );
            return 1;
        }
    ...
    else if( lua_type( _lua, 2 ) == LUA_TTABLE )
    {
        ...
        if( sa &&
            sa->isTextureAttribute() &&
            stateset->getTextureAttributeList().size() > 0 )
        {
            const osg::StateSet::AttributeList& al =
                stateset->getTextureAttributeList()[0];
            for( osg::StateSet::AttributeList::const_iterator itr = al.begin();
                 itr != al.end();
                 ++itr )
            {
                if( itr->second.first == sa )
                {
                    ...
                    lua_pushstring(
                        _lua,
                        convertStateAttributeValueToString( itr->second.second, false )
                            .c_str()
                    );
                    lua_settable( _lua, -3 );
                    return 1;
            ...
            OSG_NOTICE << "Warning: StateSet:get(" << sa->className()
                       << ") Could not find attribute" << std::endl;
            lua_pushnil( _lua );
            return 1;
        }
        else if( sa )
        ...
        else if( uniform )
        {
            ...
            OSG_NOTICE << "Warning: StateSet:get(" << sa->className()
                       << ") Could not find uniform" << std::endl;
            lua_pushnil( _lua );
            return 1;
        }
    }
    else if( lua_type( _lua, 2 ) == LUA_TSTRING )
    {
        ...
        for( osg::StateSet::ModeList::const_iterator itr = ml.begin(); itr != ml.end();
             ++itr )
        {
            if( mode == itr->first )
            {
                lua_pushstring(
                    _lua,
                    convertStateAttributeValueToString( itr->second, true ).c_str()
                );
                return 1;
            }
        }
        ...
        for( osg::StateSet::DefineList::const_iterator itr = dl.begin(); itr != dl.end();
             ++itr )
        {
            if( value == itr->first )
            {
        ...
        OSG_NOTICE << "Warning: StateSet:get(" << value
                   << ") Could not find matching mode or attribute" << std::endl;
        lua_pushnil( _lua );
        return 1;
    }

    OSG_NOTICE << "Warning: StateSet:get() inappropriate parameters, use form:"
               << std::endl;
...
static int
callStateSetRemove( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( !stateset )
    {
        OSG_NOTICE << "Warning: StateSet:remove() can only be called on a StateSet"
                   << std::endl;
        return 0;
    }
    ...
    if( lua_type( _lua, 2 ) == LUA_TNUMBER )
    {
        if( n < 3 )
        {
            OSG_NOTICE << "Warning: StateSet:remove() must be in form "
                          "remove(textureUnit, textureAttribute)"
                       << std::endl;
            return 0;
        }
        ...
        else if( lua_type( _lua, 3 ) == LUA_TSTRING )
        {
            ...
            if( stateset->getTextureAttributeList().size() > index )
            {
                     ...
                     ++itr )
                {
                    if( value ==
                        ...
                        value == itr->second.first->getName() )
                    {
                        stateset->removeTextureAttribute( index,
            ...
            if( stateset->getTextureModeList().size() > index )
            {
                osg::StateAttribute::GLMode    mode = lse->lookUpGLenumValue( value );
                const osg::StateSet::ModeList& ml =
                    stateset->getTextureModeList()[static_cast<unsigned int>( index )];
                for( osg::StateSet::ModeList::const_iterator itr = ml.begin();
                     itr != ml.end();
                     ++itr )
                {
                    if( mode == itr->first )
                    {
                        stateset->removeTextureMode( static_cast<unsigned int>( index ),
                                                     mode );
                        return 1;
                    }
                }
            ...
            OSG_NOTICE << "Warning: StateSet:remove(" << index << ", " << value
                       << ") could not find entry to remove." << std::endl;
            return 0;
        }
    }
    else if( lua_type( _lua, 2 ) == LUA_TTABLE )
    ...
    else if( lua_type( _lua, 2 ) == LUA_TSTRING )
    {
        ...
        OSG_NOTICE << "Warning: StateSet:remove(" << value
                   << ") could not find entry to remove." << std::endl;
        return 0;
    }

    OSG_NOTICE << "Warning: StateSet:remove() inappropriate parameters, use form:"
               << std::endl;
...
//
static int
callImageAllocate( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( !image )
    {
        OSG_NOTICE << "Warning: Image:allocate() can only be called on a Image"
                   << std::endl;
        return 0;
    }
    ...
    {
        OSG_NOTICE << "Warning: Cannot not image:allocator(" << s << ", " << t << ", "
                   << r << ", " << pixelFormat << ", " << dataType
                   << ") a zero sized image, use non zero, positive values for s,t,r, "
                      "pixelFormat and dataType."
                   << std::endl;
        return 0;
    }
...
// conversion of a lua value/table to a std::string, supports recursion when tables
// contain tables
static std::string
cpp_tostring( lua_State* _lua,
              int        index )
{
...
callImageGet( lua_State* _lua )
{
    ...
    // OSG_NOTICE<<"Need to implement Image::get("<<i<<", "<<j<<", "<<k<<")
    // ptr="<<(void*)ptr<<", numComponents="<<numComponents<<std::endl;
    ...
    switch( image->getDataType() )
    {
        case( GL_BYTE ) :
            ...
            {
                colour[i] =
                    static_cast<double>( *( reinterpret_cast<const char*>( ptr ) + i ) );
            }
            break;
        ...
        case( GL_SHORT ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                colour[i] = static_cast<double>(
                    *( reinterpret_cast<const short*>( ptr ) + i )
                );
            }
            break;
        case( GL_UNSIGNED_SHORT ) :
            ...
            {
                colour[i] = static_cast<double>(
                    *( reinterpret_cast<const unsigned short*>( ptr ) + i )
                );
            }
            break;
        case( GL_INT ) :
            ...
            {
                colour[i] =
                    static_cast<double>( *( reinterpret_cast<const int*>( ptr ) + i ) );
            }
            break;
        case( GL_UNSIGNED_INT ) :
            ...
            {
                colour[i] = static_cast<double>(
                    *( reinterpret_cast<const unsigned int*>( ptr ) + i )
                );
            }
            break;
        case( GL_FLOAT ) :
...
static void
setImageColour( osg::Image*       image,
                int               image_i,
                int               image_j,
                int               image_k,
                const osg::dvec4& colourToWrite )
{
    if( image_i >= image->s() || image_j >= image->t() || image_k >= image->r() )
    {
        OSG_NOTICE << "Warning: Image::set(" << image_i << ", " << image_j << ", "
                   << image_k << ") out of range" << std::endl;
        return;
    }
        ...
        case( GL_BYTE ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<char*>( ptr ) + i ) =
                    static_cast<char>( colourToWrite[i] );
            }
            break;
        case( GL_UNSIGNED_BYTE ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<unsigned char*>( ptr ) + i ) =
                    static_cast<unsigned char>( colourToWrite[i] );
            }
            break;
        case( GL_SHORT ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<short*>( ptr ) + i ) =
                    static_cast<short>( colourToWrite[i] );
            }
            break;
        case( GL_UNSIGNED_SHORT ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<unsigned short*>( ptr ) + i ) =
                    static_cast<unsigned short>( colourToWrite[i] );
            }
            break;
        case( GL_INT ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<int*>( ptr ) + i ) =
                    static_cast<int>( colourToWrite[i] );
            }
            break;
        case( GL_UNSIGNED_INT ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<unsigned int*>( ptr ) + i ) =
                    static_cast<unsigned int>( colourToWrite[i] );
            }
            break;
        case( GL_FLOAT ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<float*>( ptr ) + i ) =
                    static_cast<float>( colourToWrite[i] );
            }
            break;
        case( GL_DOUBLE ) :
            for( unsigned int i = 0; i < numComponents; ++i )
            {
                *( reinterpret_cast<double*>( ptr ) + i ) =
                    static_cast<double>( colourToWrite[i] );
            }
            break;
        default :
...
callImageSet( lua_State* _lua )
{
    ...
    osg::dvec4 colour( 1.0, 1.0, 1.0, 1.0 );
    ...
    osg::dvec4 colourToWrite = colour;
    switch( image->getPixelFormat() )
    {
        case( GL_RED ) :
...
callGetParent( lua_State* _lua )
{
    ...
    if( !node )
    {
        OSG_NOTICE << "Warning: Node::getParent() can only be called on a Node"
                   << std::endl;
        return 0;
    }
    ...
    if( n >= 2 && lua_isnumber( _lua, 2 ) )
    {
        ...
        {
            OSG_NOTICE
                << "Warning: Call to node:getParent(index) has an out of range index."
                << std::endl;
            return 0;
        }
    }
    ...
    {
        OSG_NOTICE << "Warning: node:getParent() requires a integer parameter."
                   << std::endl;
        return 0;
    }
}
...
static int
callGetNumParents( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( !node )
    {
        OSG_NOTICE << "Warning: Node::getNumParents() can only be called on a Node"
                   << std::endl;
        return 0;
    }
...
//
static int
callClassMethod( lua_State* _lua )
{
    const LuaScriptEngine* lse = reinterpret_cast<const LuaScriptEngine*>(
        lua_topointer( _lua, lua_upvalueindex( 1 ) )
    );
    ...
    if( n >= 1 && lua_type( _lua, 1 ) == LUA_TTABLE )
    {
        ...
        const std::string compoundClassName =
            lse->getObjectCompoundClassName( 1 );    // object->getCompoundClassName();
        // OSG_NOTICE<<"callClassMethod() on "<<object->className()<<" method name
        // "<<methodName<<", stored compoundClassName "<<compoundClassName<<std::endl;

        // need to put within a c function
        osg::Parameters inputParameters, outputParameters;
        for( int i = 2; i <= n; ++i )
        {
            // OSG_NOTICE<<" need to push parameter "<<lua_typename(_lua, lua_type(_lua,
            // n))<<std::endl;
            inputParameters.insert( inputParameters.begin(), lse->popParameterObject() );
        }

        if( lse->getClassInterface().run( object,
                                          compoundClassName,
                                          methodName,
                                          inputParameters,
                                          outputParameters ) )
    ...
    {
        OSG_NOTICE << "Warning: lua method called without passing object, use "
                      "object::method() convention."
                   << std::endl;
    }
...
static int
garabageCollectObject( lua_State* _lua )
{
    int n = lua_gettop( _lua ); /* number of arguments */
    if( n == 1 )
    {
        if( lua_type( _lua, 1 ) == LUA_TUSERDATA )
        {
            osg::Object* object = *const_cast<osg::Object**>(
                reinterpret_cast<const osg::Object**>( lua_touserdata( _lua, 1 ) )
            );
            object->unref();
        }
    }
...
LuaScriptEngine::LuaScriptEngine( const LuaScriptEngine& /*rhs*/,
...
std::string
LuaScriptEngine::createUniquieScriptName()
{
...
LuaScriptEngine::initialize()
{
    ...
    // Set up the __newindex and __index methods for looking up implementations of Object
    // properties
    {
    ...
    // Set up the __tostring methods to be able to convert tables into strings so they
    ...
    // Set up the __gc methods for looking up implementations of Object pointer to do the
    // unref when the associated Lua object is destroyed.
    {
...
bool
LuaScriptEngine::loadScript( osg::Script* script )
{
    ...
    {
        OSG_NOTICE << "LuaScriptEngine::luaL_loadstring(Script*) error: "
...
bool
LuaScriptEngine::run( osg::Script*       script,
                      ...
                      osg::Parameters&   outputParameters )
{
    ...
    if( _loadedScripts.count( script ) == 0 )
    {
        ...
        if( !entryPoint.empty() )
        {
            ...
            {
                OSG_NOTICE << "error initialize script " << lua_tostring( _lua, -1 )
                           << std::endl;
                return false;
            }
        }
...
class PushStackValueVisitor : public osg::ValueObject::GetValueVisitor
{
    public:
        ...
        PushStackValueVisitor( const LuaScriptEngine* lse ) :
            _lse( lse )
        {
        ...
        virtual void
        apply( const osg::bvec2& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::bvec3& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::bvec4& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::ubvec2& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::ubvec3& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::ubvec4& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::svec2& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::svec3& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::svec4& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::usvec2& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::usvec3& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::usvec4& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::ivec2& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::ivec3& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::ivec4& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::uivec2& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::uivec3& value )
        {
            _lse->pushValue( value );
        }

        virtual void
        apply( const osg::uivec4& value )
        {
            _lse->pushValue( value );
        }
...
class GetStackValueVisitor : public osg::ValueObject::SetValueVisitor
{
    public:
        ...
        GetStackValueVisitor( const LuaScriptEngine* lse,
                              int                    index ) :
            _lse( lse ),
            _lua( 0 ),
            _index( index ),
        ...
        virtual void
        apply( std::string& value )
        {
            if( lua_isstring( _lua, _index ) )
            {
                value        = std::string( lua_tostring( _lua, _index ),
                                            lua_rawlen( _lua, _index ) );
                _numberToPop = 1;
            }
        }

        virtual void
        apply( osg::vec2& value )
        {
...
int
LuaScriptEngine::pushPropertyToStack( osg::Object*       object,
                                      const std::string& propertyName ) const
{
    ...
    if( !_ci.getPropertyType( object, propertyName, type ) )
    {
        ...
        OSG_INFO << "LuaScriptEngine::pushPropertyToStack(" << object << ", "
                 << propertyName << ") no property found." << std::endl;
        return 0;
    }
    ...
    switch( type )
    {
        ...
        case( osgDB::BaseSerializer::RW_ENUM ) :
            {
                ...
                if( _ci.getProperty( object, propertyName, value ) )
                {
                    osgDB::BaseSerializer* serializer =
                        _ci.getSerializer( object, propertyName, type );
                    osgDB::IntLookup* lookup =
                        serializer ? serializer->getIntLookup() : 0;
                    if( lookup )
                    {
        ...
        case( osgDB::BaseSerializer::RW_VEC2B ) :
            if( getPropertyAndPushValue<osg::bvec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3B ) :
            if( getPropertyAndPushValue<osg::bvec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4B ) :
            if( getPropertyAndPushValue<osg::bvec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UB ) :
            if( getPropertyAndPushValue<osg::ubvec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UB ) :
            if( getPropertyAndPushValue<osg::ubvec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UB ) :
            if( getPropertyAndPushValue<osg::ubvec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2S ) :
            if( getPropertyAndPushValue<osg::svec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3S ) :
            if( getPropertyAndPushValue<osg::svec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4S ) :
            if( getPropertyAndPushValue<osg::svec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2US ) :
            if( getPropertyAndPushValue<osg::usvec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3US ) :
            if( getPropertyAndPushValue<osg::usvec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4US ) :
            if( getPropertyAndPushValue<osg::usvec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2I ) :
            if( getPropertyAndPushValue<osg::ivec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3I ) :
            if( getPropertyAndPushValue<osg::ivec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4I ) :
            if( getPropertyAndPushValue<osg::ivec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UI ) :
            if( getPropertyAndPushValue<osg::uivec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UI ) :
            if( getPropertyAndPushValue<osg::uivec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UI ) :
            if( getPropertyAndPushValue<osg::uivec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2F ) :
            if( getPropertyAndPushValue<osg::vec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3F ) :
            if( getPropertyAndPushValue<osg::vec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4F ) :
            if( getPropertyAndPushValue<osg::vec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2D ) :
            if( getPropertyAndPushValue<osg::dvec2>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3D ) :
            if( getPropertyAndPushValue<osg::dvec3>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4D ) :
            if( getPropertyAndPushValue<osg::dvec4>( object, propertyName ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_QUAT ) :
            if( getPropertyAndPushValue<osg::quat>( object, propertyName ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_PLANE ) :
            if( getPropertyAndPushValue<osg::Plane>( object, propertyName ) )
            {
                return 1;
            }
            break;
    ...
    OSG_NOTICE << "LuaScriptEngine::pushPropertyToStack(" << object << ", "
               << propertyName << ") property of type = " << _ci.getTypeName( type )
               << " error, not supported." << std::endl;
    return 0;
}

std::string
LuaScriptEngine::lookUpGLenumString( GLenum value ) const
{
    osgDB::ObjectWrapperManager* ow =
    ...
    OSG_NOTICE
        << "Warning: LuaScriptEngine did not find valid GL enum value for GLenum value: "
        << value << std::endl;
...
GLenum
LuaScriptEngine::lookUpGLenumValue( const std::string& str ) const
{
    osgDB::ObjectWrapperManager* ow =
        osgDB::Registry::instance()->getObjectWrapperManager();
    ...
    OSG_NOTICE
        << "Warning: LuaScriptEngine did not find valid GL enum value for string value: "
        << str << std::endl;
...
int
LuaScriptEngine::pushDataToStack( SerializerScratchPad* ssp ) const
{
    switch( ssp->dataType )
    {
        case( osgDB::BaseSerializer::RW_BOOL ) :
        ...
        case( osgDB::BaseSerializer::RW_VEC2B ) :
            if( pushValueToStack<osg::bvec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3B ) :
            if( pushValueToStack<osg::bvec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4B ) :
            if( pushValueToStack<osg::bvec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UB ) :
            if( pushValueToStack<osg::ubvec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UB ) :
            if( pushValueToStack<osg::ubvec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UB ) :
            if( pushValueToStack<osg::ubvec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2S ) :
            if( pushValueToStack<osg::svec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3S ) :
            if( pushValueToStack<osg::svec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4S ) :
            if( pushValueToStack<osg::svec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2US ) :
            if( pushValueToStack<osg::usvec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3US ) :
            if( pushValueToStack<osg::usvec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4US ) :
            if( pushValueToStack<osg::usvec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2I ) :
            if( pushValueToStack<osg::ivec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3I ) :
            if( pushValueToStack<osg::ivec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4I ) :
            if( pushValueToStack<osg::ivec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UI ) :
            if( pushValueToStack<osg::uivec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UI ) :
            if( pushValueToStack<osg::uivec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UI ) :
            if( pushValueToStack<osg::uivec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2F ) :
            if( pushValueToStack<osg::vec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3F ) :
            if( pushValueToStack<osg::vec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4F ) :
            if( pushValueToStack<osg::vec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2D ) :
            if( pushValueToStack<osg::dvec2>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3D ) :
            if( pushValueToStack<osg::dvec3>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4D ) :
            if( pushValueToStack<osg::dvec4>( ssp ) )
            {
                return 1;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_QUAT ) :
            if( pushValueToStack<osg::quat>( ssp ) )
            {
                return 1;
            }
            break;
        case( osgDB::BaseSerializer::RW_PLANE ) :
            if( pushValueToStack<osg::Plane>( ssp ) )
            {
                return 1;
            }
            break;
    ...
    OSG_NOTICE << "LuaScriptEngine::pushDataToStack() property of type = "
               << _ci.getTypeName( ssp->dataType ) << " error, not supported."
               << std::endl;
    return 0;
}

int
LuaScriptEngine::getDataFromStack( SerializerScratchPad*       ssp,
                                   osgDB::BaseSerializer::Type type,
                                   int                         pos ) const
{
    ...
    switch( type )
    {
        ...
        case( osgDB::BaseSerializer::RW_GLENUM ) :
            {
                ...
                else if( lua_isstring( _lua, pos ) )
                {
                    ...
                    GLenum      value =
                        lookUpGLenumValue( enumString );    // getValue("GL",enumString);

                    ssp->set( value );
                    return 0;
                }
                OSG_NOTICE << "LuaScriptEngine::getDataFromStack() "
                              "osgDB::BaseSerializer::RW_GLENUM Failed"
                           << std::endl;
                break;
            }
        case( osgDB::BaseSerializer::RW_ENUM ) :
            {
                ...
                {
                    OSG_NOTICE
                        << "LuaScriptEngine::getDataFromStack() "
                           "osgDB::BaseSerializer::RW_ENUM Failed to convert string"
                        << std::endl;
                    return 0;
                }
                break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2B ) :
            if( getDataFromStack<osg::bvec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3B ) :
            if( getDataFromStack<osg::bvec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4B ) :
            if( getDataFromStack<osg::bvec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UB ) :
            if( getDataFromStack<osg::ubvec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UB ) :
            if( getDataFromStack<osg::ubvec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UB ) :
            if( getDataFromStack<osg::ubvec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2S ) :
            if( getDataFromStack<osg::svec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3S ) :
            if( getDataFromStack<osg::svec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4S ) :
            if( getDataFromStack<osg::svec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2US ) :
            if( getDataFromStack<osg::usvec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3US ) :
            if( getDataFromStack<osg::usvec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4US ) :
            if( getDataFromStack<osg::usvec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2I ) :
            if( getDataFromStack<osg::ivec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3I ) :
            if( getDataFromStack<osg::ivec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4I ) :
            if( getDataFromStack<osg::ivec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UI ) :
            if( getDataFromStack<osg::uivec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UI ) :
            if( getDataFromStack<osg::uivec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UI ) :
            if( getDataFromStack<osg::uivec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2F ) :
            if( getDataFromStack<osg::vec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3F ) :
            if( getDataFromStack<osg::vec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4F ) :
            if( getDataFromStack<osg::vec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2D ) :
            if( getDataFromStack<osg::dvec2>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3D ) :
            if( getDataFromStack<osg::dvec3>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4D ) :
            if( getDataFromStack<osg::dvec4>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_QUAT ) :
            if( getDataFromStack<osg::quat>( ssp, pos ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_PLANE ) :
            if( getDataFromStack<osg::Plane>( ssp, pos ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_OBJECT ) :
            {
                if( lua_istable( _lua, pos ) )
                {
                    ...
                    if( lua_type( _lua, -1 ) == LUA_TUSERDATA )
                    {
                        value = *const_cast<osg::Object**>(
                            reinterpret_cast<const osg::Object**>( lua_touserdata( _lua,
                                                                                   -1 ) )
                        );
                    }
                    lua_pop( _lua, 1 );
                    ...
                    {
                        OSG_NOTICE << "Error: lua type '"
                                   << lua_typename( _lua, lua_type( _lua, pos ) )
                                   << "' cannot be assigned." << std::endl;
                    }
                }
                else if( lua_isnil( _lua, pos ) )
                {
                    OSG_NOTICE << "Assigning property object (nil) to to object"
                               << std::endl;
                    osg::Object* value = 0;
                    ssp->set( value );
                    return 0;
    ...
    OSG_NOTICE << "LuaScriptEngine::getDataFromStack() property of type = "
               << _ci.getTypeName( type ) << " not matched" << std::endl;
    return 0;
}

int
LuaScriptEngine::setPropertyFromStack( osg::Object*       object,
                                       const std::string& propertyName ) const
{
    osgDB::BaseSerializer::Type type;
    if( !_ci.getPropertyType( object, propertyName, type ) )
    {
        if( lua_type( _lua, -1 ) == LUA_TFUNCTION )
        {
            ...
            osg::ref_ptr<LuaCallbackObject> lco =
                new LuaCallbackObject( propertyName, this, ref );
                                       ...
                                       osgDB::BaseSerializer::Type type ) const
{
    switch( type )
    {
        case( osgDB::BaseSerializer::RW_BOOL ) :
            {
                ...
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<bool>( lua_toboolean( _lua, -1 ) !=
                                                        0 ) );
                    return 0;
                }
                else if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<bool>( lua_tonumber( _lua, -1 ) !=
                                                        0 ) );
                    return 0;
                }
                break;
        ...
        case( osgDB::BaseSerializer::RW_STRING ) :
            {
                if( lua_isstring( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     std::string( lua_tostring( _lua, -1 ) ) );
                    return 0;
                }
                break;
            }
        case( osgDB::BaseSerializer::RW_GLENUM ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<GLenum>( lua_tonumber( _lua, -1 ) ) );
                    return 0;
                }
                ...
                OSG_NOTICE << "LuaScriptEngine::setPropertyFromStack(" << propertyName
                           << ") osgDB::BaseSerializer::RW_GLENUM Failed" << std::endl;
                break;
            }
        case( osgDB::BaseSerializer::RW_ENUM ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<int>( lua_tonumber( _lua, -1 ) ) );
                    return 0;
                }
                else if( lua_isstring( _lua, -1 ) )
                {
        ...
        case( osgDB::BaseSerializer::RW_SHORT ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<short>( lua_tonumber( _lua, -1 ) ) );
                    return 0;
                }
                break;
            }
        case( osgDB::BaseSerializer::RW_USHORT ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<unsigned short>( lua_tonumber( _lua,
                                                                                -1 ) ) );
                    return 0;
                }
                break;
        ...
        case( osgDB::BaseSerializer::RW_UINT ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<unsigned int>( lua_tonumber( _lua,
                                                                              -1 ) ) );
                    return 0;
                }
                break;
        ...
        case( osgDB::BaseSerializer::RW_FLOAT ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<float>( lua_tonumber( _lua, -1 ) ) );
                    return 0;
                }
                break;
            }
        case( osgDB::BaseSerializer::RW_DOUBLE ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    _ci.setProperty( object,
                                     propertyName,
                                     static_cast<double>( lua_tonumber( _lua, -1 ) ) );
                    return 0;
                }
                break;
            }
        ...
        case( osgDB::BaseSerializer::RW_VEC2B ) :
            if( getValueAndSetProperty<osg::bvec2>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3B ) :
            if( getValueAndSetProperty<osg::bvec3>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4B ) :
            if( getValueAndSetProperty<osg::bvec4>( object, propertyName ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2UB ) :
            if( getValueAndSetProperty<osg::ubvec2>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3UB ) :
            if( getValueAndSetProperty<osg::ubvec3>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4UB ) :
            if( getValueAndSetProperty<osg::ubvec4>( object, propertyName ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2F ) :
            if( getValueAndSetProperty<osg::vec2>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3F ) :
            if( getValueAndSetProperty<osg::vec3>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4F ) :
            if( getValueAndSetProperty<osg::vec4>( object, propertyName ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_VEC2D ) :
            if( getValueAndSetProperty<osg::dvec2>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC3D ) :
            if( getValueAndSetProperty<osg::dvec3>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_VEC4D ) :
            if( getValueAndSetProperty<osg::dvec4>( object, propertyName ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_QUAT ) :
            if( getValueAndSetProperty<osg::quat>( object, propertyName ) )
            {
                return 0;
            }
            break;
        case( osgDB::BaseSerializer::RW_PLANE ) :
            if( getValueAndSetProperty<osg::Plane>( object, propertyName ) )
            {
                return 0;
            }
            break;
        ...
        case( osgDB::BaseSerializer::RW_OBJECT ) :
            {
                if( lua_istable( _lua, -1 ) )
                {
                    ...
                    {
                        OSG_NOTICE << "Error: lua type '"
                                   << lua_typename( _lua, lua_type( _lua, -1 ) )
                                   << "' cannot be assigned to " << object->className()
    ...
    OSG_NOTICE << "LuaScriptEngine::setPropertyFromStack(" << object << ", "
               << propertyName << ") property of type = " << _ci.getTypeName( type )
...
bool
LuaScriptEngine::getfields( int         pos,
                            const char* f1,
                            const char* f2,
                            int         type ) const
{
...
bool
LuaScriptEngine::getelements( int pos,
                              int numElements,
                              int type ) const
{
    int abs_pos = getAbsolutePos( pos );
...
LuaScriptEngine::getType( int pos ) const
{
    int abs_pos = getAbsolutePos( pos );
        ...
        case( LUA_TTABLE ) :
            {
                ...
                if( ( numStringKeys == 2 || numNumberKeys == 2 ) &&
                    ( numNumberFields == 2 ) )
                {
                    return osgDB::BaseSerializer::RW_VEC2D;
                }
                else if( ( numStringKeys == 3 || numNumberKeys == 3 ) &&
                         ( numNumberFields == 3 ) )
                {
                    return osgDB::BaseSerializer::RW_VEC3D;
                }
                else if( ( numStringKeys == 4 || numNumberKeys == 4 ) &&
                         ( numNumberFields == 4 ) )
                {
                ...
                // not supported
                OSG_NOTICE << "Warning: LuaScriptEngine::getType() Lua table "
                              "configuration not supported."
                           << std::endl;
                break;
            }
        default :
            OSG_NOTICE << "Warning: LuaScriptEngine::getType() Lua type "
                       << lua_typename( _lua, lua_type( _lua, abs_pos ) )
...
bool
LuaScriptEngine::getvec3( int pos ) const
{
    int abs_pos = getAbsolutePos( pos );
    if( lua_istable( _lua, abs_pos ) )
    {
...
bool
LuaScriptEngine::getmatrix( int pos ) const
{
    int abs_pos = getAbsolutePos( pos );
    if( lua_istable( _lua, abs_pos ) )
    {
...
bool
LuaScriptEngine::getboundingbox( int pos ) const
{
    int abs_pos = getAbsolutePos( pos );
    if( lua_istable( _lua, abs_pos ) )
    {
        if( getfields( abs_pos,
                       "xMin",
                       "yMin",
                       "zMin",
                       "xMax",
...
bool
LuaScriptEngine::getboundingsphere( int pos ) const
{
    int abs_pos = getAbsolutePos( pos );
    if( lua_istable( _lua, abs_pos ) )
    {
...
bool
LuaScriptEngine::getValue( int        pos,
                           osg::mat4& value ) const
{
    if( !getmatrix( pos ) )
    {
        return false;
...
bool
LuaScriptEngine::getValue( int          pos,
                           osg::sphere& value ) const
{
    if( !getboundingsphere( pos ) )
    {
        return false;
    }
    value.set( osg::vec3( lua_tonumber( _lua, -4 ),
                          lua_tonumber( _lua, -3 ),
                          lua_tonumber( _lua, -2 ) ),
               lua_tonumber( _lua, -1 ) );
    lua_pop( _lua, 4 );
...
bool
LuaScriptEngine::getValue( int           pos,
                           osg::dsphere& value ) const
{
    if( !getboundingsphere( pos ) )
    {
        return false;
    }
    value.set( osg::dvec3( lua_tonumber( _lua, -4 ),
                           lua_tonumber( _lua, -3 ),
                           lua_tonumber( _lua, -2 ) ),
               lua_tonumber( _lua, -1 ) );
    lua_pop( _lua, 4 );
...
void
LuaScriptEngine::pushValue( const osg::mat4& value ) const
{
    lua_newtable( _lua );
    lua_newtable( _lua );
    luaL_getmetatable( _lua, "LuaScriptEngine.Table" );
...
void
LuaScriptEngine::pushValue( const osg::dmat4& value ) const
{
    lua_newtable( _lua );
    lua_newtable( _lua );
    luaL_getmetatable( _lua, "LuaScriptEngine.Table" );
...
void
LuaScriptEngine::pushValue( const osg::box& value ) const
{
    lua_newtable( _lua );
    lua_newtable( _lua );
    luaL_getmetatable( _lua, "LuaScriptEngine.Table" );
...
void
LuaScriptEngine::pushValue( const osg::dbox& value ) const
{
    lua_newtable( _lua );
    lua_newtable( _lua );
    luaL_getmetatable( _lua, "LuaScriptEngine.Table" );
...
void
LuaScriptEngine::pushValue( const osg::sphere& value ) const
{
    lua_newtable( _lua );
    lua_newtable( _lua );
    luaL_getmetatable( _lua, "LuaScriptEngine.Table" );
...
void
LuaScriptEngine::pushValue( const osg::dsphere& value ) const
{
    lua_newtable( _lua );
    lua_newtable( _lua );
    luaL_getmetatable( _lua, "LuaScriptEngine.Table" );
...
bool
LuaScriptEngine::pushParameter( osg::Object* object ) const
{
...
bool
LuaScriptEngine::popParameter( osg::Object* object ) const
{
    osg::ValueObject* vo = dynamic_cast<osg::ValueObject*>( object );
    if( vo )
    {
...
osg::Object*
LuaScriptEngine::popParameterObject() const
{
    ...
    switch( type )
    {
        case( osgDB::BaseSerializer::RW_BOOL ) :
            {
                ...
                {
                    object =
                        new osg::BoolValueObject( "", lua_toboolean( _lua, -1 ) != 0 );
                }
                break;
            }
        case( osgDB::BaseSerializer::RW_STRING ) :
        ...
        case( osgDB::BaseSerializer::RW_UINT ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    object = new osg::UIntValueObject(
                        "",
                        static_cast<unsigned int>( lua_tonumber( _lua, -1 ) )
                    );
                }
                break;
            }
        case( osgDB::BaseSerializer::RW_FLOAT ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    object = new osg::FloatValueObject(
                        "",
                        static_cast<float>( lua_tonumber( _lua, -1 ) )
                    );
                }
        ...
        case( osgDB::BaseSerializer::RW_DOUBLE ) :
            {
                if( lua_isnumber( _lua, -1 ) )
                {
                    object = new osg::DoubleValueObject(
                        "",
                        static_cast<double>( lua_tonumber( _lua, -1 ) )
                    );
                }
        ...
        case( osgDB::BaseSerializer::RW_VEC2B ) :
            object = getValueObject<osg::bvec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3B ) :
            object = getValueObject<osg::bvec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4B ) :
            object = getValueObject<osg::bvec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2UB ) :
            object = getValueObject<osg::ubvec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3UB ) :
            object = getValueObject<osg::ubvec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4UB ) :
            object = getValueObject<osg::ubvec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2S ) :
            object = getValueObject<osg::svec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3S ) :
            object = getValueObject<osg::svec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4S ) :
            object = getValueObject<osg::svec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2US ) :
            object = getValueObject<osg::usvec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3US ) :
            object = getValueObject<osg::usvec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4US ) :
            object = getValueObject<osg::usvec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2I ) :
            object = getValueObject<osg::ivec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3I ) :
            object = getValueObject<osg::ivec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4I ) :
            object = getValueObject<osg::ivec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2UI ) :
            object = getValueObject<osg::uivec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3UI ) :
            object = getValueObject<osg::uivec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4UI ) :
            object = getValueObject<osg::uivec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2F ) :
            object = getValueObject<osg::vec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3F ) :
            object = getValueObject<osg::vec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4F ) :
            object = getValueObject<osg::vec4>( -1 );
            break;

        case( osgDB::BaseSerializer::RW_VEC2D ) :
            object = getValueObject<osg::dvec2>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC3D ) :
            object = getValueObject<osg::dvec3>( -1 );
            break;
        case( osgDB::BaseSerializer::RW_VEC4D ) :
            object = getValueObject<osg::dvec4>( -1 );
            break;
...
void
LuaScriptEngine::pushContainer( osg::Object*       object,
                                const std::string& propertyName ) const
{
    if( object )
    {
        ...
        // set up objbect_ptr to handle ref/unref of the object
        {
            ...
            // increment the reference count as the lua now will unreference it once it's
            // finished with the userdata for the pointer
            object->ref();
        }
...
void
LuaScriptEngine::createAndPushObject( const std::string& compoundName ) const
{
...
void
LuaScriptEngine::pushObject( osg::Object* object ) const
{
    if( object )
    {
        lua_newtable( _lua );
...
void
LuaScriptEngine::pushAndCastObject( const std::string& compoundClassName,
                                    osg::Object*       object ) const
{
    if( object && _ci.isObjectOfType( object, compoundClassName ) )
    {
        ...
        std::string libraryName          = ( separator == std::string::npos )
                                             ? object->libraryName()
                                             : compoundClassName.substr( 0, separator );
        std::string className =
            ( separator == std::string::npos )
                ? object->className()
                : compoundClassName.substr( separator + 2, std::string::npos );
...
void
LuaScriptEngine::assignClosure( const char*   name,
                                lua_CFunction fn ) const
{
...
void
LuaScriptEngine::addPaths( const osgDB::FilePathList& paths )
{
    ...
    OSG_INFO << "LuaScriptEngine::addPaths() original package.path = " << path
             << std::endl;

    for( osgDB::FilePathList::const_iterator itr = paths.begin(); itr != paths.end();
         ++itr )
    {
...
void
LuaScriptEngine::addPaths( const osgDB::Options* options )
{
    if( !options )
    {
        return;
