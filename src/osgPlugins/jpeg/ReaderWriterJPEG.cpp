...
#define INPUT_BUF_SIZE 4'096 /* choose an efficiently fread'able size */
    ...
    void
    jpeg_istream_src( j_decompress_ptr cinfo,
                      std::istream*    infile )
    {
        stream_src_ptr src;
        ...
        { /* first time for this JPEG object? */
            ...
            src->buffer =
                ( JOCTET* )( *cinfo->mem->alloc_small )( ( j_common_ptr )cinfo,
                                                         JPOOL_PERMANENT,
                                                         INPUT_BUF_SIZE *
                                                             sizeof( JOCTET ) );
        }
...
#define OUTPUT_BUF_SIZE 4'096 /* choose an efficiently fwrite'able size */
                          ...
                          int                  headerlen )
    {
        ...
        if( memcmp( ( const void* )&header[6], ( const void* )jpgcmp, 4 ) == 0 )
        {
            return 1;
        }
        return 0;
    ...
    copyScanline( unsigned char* currPtr,
                  unsigned char* from,
                  int            cnt )
    {
                      ...
                      unsigned int* exif_orientation )
    {
        ...
        // used to be before setjump above, but have moved to after to avoid compile
        // warnings.
        ...
        jpeg_save_markers( &cinfo, EXIF_JPEG_MARKER, 0XFF'FF );
        ...
        *exif_orientation = EXIF_Orientation( &cinfo );
        ...
        rowbuffer = ( *cinfo.mem->alloc_sarray )( ( j_common_ptr )&cinfo,
                                                  JPOOL_IMAGE,
                                                  row_stride,
                                                  1 );
...
class ReaderWriterJPEG : public osgDB::ReaderWriter
{
        ...
        write_JPEG_file( std::ostream&     fout,
                         const osg::Image& img,
                         int               quality = 100 ) const
        {
            if( !img.isDataContiguous() )
            {
                OSG_WARN << "Warning: Writing of image data, that is non contiguous, is "
                            "not supported by JPEG plugin."
                         << std::endl;
            ...
            {
                OSG_DEBUG << "ReaderWriterJPEG::write_JPEG_file - Error no size"
                          << std::endl;
                return WriteResult::ERROR_IN_WRITING_FILE;
            }
            ...
            switch( img.getPixelFormat() )
            {
                ...
                default :
                    {
                        OSG_DEBUG << "ReaderWriterJPEG::write_JPEG_file - Error pixel "
            ...
            cinfo.input_components =
                image_components;             /* # of color components per pixel */
             ...
             */
            row_stride =
                image_width * image_components; /* JSAMPLEs per row in image_buffer */
        ...
        int
        getQuality( const osgDB::ReaderWriter::Options* options ) const
        {
            if( options )
            {
    ...
    public:
        ...
        ReadResult
        readJPGStream( std::istream& fin ) const
        {
            ...
            imageData = osgDBJPEG::simage_jpeg_load( fin,
                                                     &width_ret,
                                                     &height_ret,
                                                     &numComponents_ret,
                                                     &exif_orientation );
            ...
            int          internalFormat        = numComponents_ret == 1 ? GL_RED
            ...
            if( exif_orientation > 0 )
            {
                // guide for meaning of exif_orientation provided by webpage:
                // http://sylvana.net/jpegcrop/exif_orientation.html
                switch( exif_orientation )
                {
                    case( 1 ) :
                        OSG_INFO << "EXIF_Orientation 1 (top, left side), No need to "
                                    "rotate image. "
                                 << std::endl;
                        break;
                    case( 2 ) :
                        OSG_INFO << "EXIF_Orientation 2 (top, right side), flip x."
                                 << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                            osg::ivec3( pOsgImage->s() - 1, 0, 0 ),
                            osg::ivec3( -pOsgImage->s(), 0, 0 ),
                            osg::ivec3( 0, pOsgImage->t(), 0 ),
                            osg::ivec3( 0, 0, 1 )
                        );
                        break;
                    case( 3 ) :
                        OSG_INFO
                            << "EXIF_Orientation 3 (bottom, right side), rotate 180."
                            << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                            osg::ivec3( pOsgImage->s() - 1, pOsgImage->t() - 1, 0 ),
                            osg::ivec3( -pOsgImage->s(), 0, 0 ),
                            osg::ivec3( 0, -pOsgImage->t(), 0 ),
                            osg::ivec3( 0, 0, 1 )
                        );
                        break;
                    case( 4 ) :
                        OSG_INFO << "EXIF_Orientation 4 (bottom, left side). flip y, "
                                    "rotate 180."
                                 << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                            osg::ivec3( 0, pOsgImage->t() - 1, 0 ),
                            osg::ivec3( pOsgImage->s(), 0, 0 ),
                            osg::ivec3( 0, -pOsgImage->t(), 0 ),
                            osg::ivec3( 0, 0, 1 )
                        );
                    ...
                    case( 5 ) :
                        OSG_INFO
                            << "EXIF_Orientation 5 (left side, top). flip y, rotate 90."
                            << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                            osg::ivec3( pOsgImage->s() - 1, pOsgImage->t() - 1, 0 ),
                        ...
                        break;
                    case( 6 ) :
                        OSG_INFO << "EXIF_Orientation 6 (right side, top). rotate 90."
                                 << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                            osg::ivec3( pOsgImage->s() - 1, 0, 0 ),
                        ...
                        break;
                    case( 7 ) :
                        OSG_INFO << "EXIF_Orientation 7 (right side, bottom), flip Y, "
                                    "rotate 270."
                                 << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                        ...
                        break;
                    case( 8 ) :
                        OSG_INFO << "EXIF_Orientation 8 (left side, bottom). rotate 270."
                                 << std::endl;
                        pOsgImage = osg::createImageWithOrientationConversion(
                            pOsgImage.get(),
                            osg::ivec3( 0, pOsgImage->t() - 1, 0 ),
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            WriteResult::WriteStatus ws =
                write_JPEG_file( fout, *( tmp_img.get() ), getQuality( options ) );
            return ws;
        }
