...
EXIF_Orientation( j_decompress_ptr cinfo )
{
    ...
    */
    ...
    OSG_INFO << "Found TIFF header = " << i << " endian = "
             << ( ( tiffHeaderEndian == osg::BigEndian ) ? "BigEndian" : "LittleEndian" )
             << std::endl;
    ...
    unsigned int orient_tag_id = 0X1'12;
    ...
    while( tags-- )
    {
        ...
        OSG_INFO << "  tag=0x" << std::hex << tag << std::dec << ", type=" << type
