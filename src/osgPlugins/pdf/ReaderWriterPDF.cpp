 ...
 * CairoImage, derived from Referenced.
 * Provides: _image, _surface, _context, create, destroy, getSurface.
 */
#include <cairo.h>
...
class PopplerPdfImage : public osgWidget::PdfImage
{
    public:
        ...
        open( const std::string& filename )
        {
            ...
            PopplerDocument* doc =
                poppler_document_new_from_file( uri.c_str(), NULL, NULL );
            if( !doc )
            {
                OSG_NOTICE << " could not open(" << filename << "), uri=" << uri
                           << std::endl;
        ...
        page( int pageNum )
        {
            ...
            _cairoImage->create( ( unsigned int )( w * 2.0 ),
                                 ( unsigned int )( h * 2.0 ) );
            ...
            cairo_rectangle( _cairoImage->getContext(),
                             0.0,
                             0.0,
                             double( s() ),
                             double( t() ) );
            cairo_scale( _cairoImage->getContext(),
                         double( s() ) / w,
                         double( t() ) / h );
...
class ReaderWriterPDF : public osgDB::ReaderWriter
{
    public:
                  ...
                  const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( pdfReader->assign(
                    dynamic_cast<osgWidget::PdfImage*>( result.getImage() )
                ) )
            {
