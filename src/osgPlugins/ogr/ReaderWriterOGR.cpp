 ...
 * ReaderWriterOGR, derived from ReaderWriter.
 * Provides: supportsExtension, supportsOption, supportsOption, CPLSetErrorHandler,
...
void CPL_STDCALL
CPLOSGErrorHandler( CPLErr      eErrClass,
                    int         nError,
...
triangulizeGeometry( osg::Geometry* src )
{
    if( src->getNumPrimitiveSets() ==
        1 &&
        src->getPrimitiveSet( 0 )->getType() ==
...
class ReaderWriterOGR : public osgDB::ReaderWriter
{
    ...
    public:
        ...
        ReaderWriterOGR()
        {
            ...
            supportsOption( "useRandomColorByFeature",
                            "Assign a random color to each feature." );
            supportsOption( "addGroupPerFeature",
                            "Places each feature in a separate group." );
                  ...
                  const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            // Try to open data source
            GDALDataset* file = ( GDALDataset* )
                GDALOpenEx( fileName.c_str(), GDAL_OF_VECTOR, NULL, NULL, NULL );
#endif
            ...
            if( options )
            {
                if( options->getOptionString().find( "UseRandomColorByFeature" ) !=
                    std::string::npos )
                {
                ...
                if( options->getOptionString().find( "addGroupPerFeature" ) !=
                    std::string::npos )
                {
            ...
            for( int i = 0; i < GDALDatasetGetLayerCount( file ); i++ )
            {
                ...
                osg::Group* node  = readLayer( layer,
                                               layer->GetName(),
                                               useRandomColorByFeature,
                                               addGroupPerFeature );
                if( node )
        ...
        osg::Group*
        readLayer( OGRLayer* ogrLayer,
                   const std::string& /*name*/,
                   bool useRandomColorByFeature,
                   bool addGroupPerFeature ) const
        {
            if( !ogrLayer )
            {
                return 0;
        ...
        osg::Geometry*
        pointsToDrawable( const OGRPoint* points ) const
        {
            osg::Geometry*  pointGeom = new osg::Geometry();
            osg::Vec3Array* vertices  = new osg::Vec3Array();
            vertices->push_back(
                osg::vec3( points->getX(), points->getY(), points->getZ() )
            );
        ...
        osg::Geometry*
        linearRingToDrawable( OGRLinearRing* ring ) const
        {
            ...
            for( int j = 0; j < ring->getNumPoints(); j++ )
            {
                ...
                vertices->push_back(
                    osg::vec3( point.getX(), point.getY(), point.getZ() )
                );
            }
            contourGeom->setVertexArray( vertices );
            contourGeom->addPrimitiveSet(
        ...
        osg::Geometry*
        lineStringToDrawable( OGRLineString* lineString ) const
        {
            osg::Geometry*  contourGeom = new osg::Geometry();
            osg::Vec3Array* vertices    = new osg::Vec3Array();
            OGRPoint        point;
        ...
        osg::Geometry*
        multiPointToDrawable( OGRMultiPoint* mpoint ) const
        {
            osg::Geometry*  pointGeom = new osg::Geometry();
            osg::Vec3Array* vertices  = new osg::Vec3Array();
            ...
            {
                OSG_INFO << "osgOgrFeature::multiPointToDrawable "
                         << pointGeom->getVertexArray()->getNumElements() << " vertices"
                         << std::endl;
            }
        ...
        osg::Geometry*
        multiPolygonToDrawable( OGRMultiPolygon* mpolygon ) const
        {
            ...
            for( int i = 0; i < mpolygon->getNumGeometries(); i++ )
            {
                    ...
                    geometry->getVertexArray()->getType() == osg::Array::Vec3ArrayType )
                {
                    ...
                    {    // already a polygon then append
                        ...
                        osg::Vec3Array* arrayDst =
                            static_cast<osg::Vec3Array*>( geom->getVertexArray() );
                        osg::ref_ptr<osg::Vec3Array> triangulized =
                            triangulizeGeometry( geometry.get() );
                        if( triangulized.valid() )
                        {
                            arrayDst->insert( arrayDst->end(),
                                              triangulized->begin(),
                                              triangulized->end() );
                            // shift index
            ...
            if( geom->getVertexArray() )
            {
                OSG_INFO << "osgOgrFeature::multiPolygonToDrawable "
                         << geom->getVertexArray()->getNumElements() << " vertices"
                         << std::endl;
            }
        ...
        osg::Geometry*
        polygonToDrawable( OGRPolygon* polygon ) const
        {
            osg::Geometry*  geom     = new osg::Geometry();
            osg::Vec3Array* vertices = new osg::Vec3Array();
                ...
                for( int i = 0; i < polygon->getNumInteriorRings(); i++ )
                {
                    ...
                    geom->addPrimitiveSet(
                        new osg::DrawArrays( osg::PrimitiveSet::LINE_LOOP,
                                             vertices->size() - ring->getNumPoints(),
                                             ring->getNumPoints() )
                    );
                }
            }
            ...
            geom->addPrimitiveSet(
                new osg::DrawArrays( osg::PrimitiveSet::TRIANGLES, 0, array->size() )
            );
        ...
        osg::Geometry*
        multiLineStringToDrawable( OGRMultiLineString* mlineString ) const
        {
            osg::Geometry* geom = new osg::Geometry;
            ...
            for( int i = 0; i < mlineString->getNumGeometries(); i++ )
            {
                ...
                osg::ref_ptr<osg::Geometry> geometry =
                    lineStringToDrawable( lineString );
                if( geometry.valid() &&
                    geometry->getVertexArray() &&
                    geometry->getNumPrimitiveSets() &&
                    geometry->getVertexArray()->getType() == osg::Array::Vec3ArrayType )
        ...
        osg::Geode*
        readFeature( OGRFeature* ogrFeature,
                     bool        useRandomColorByFeature ) const
        {
            ...
            switch( ogrFeature->GetGeometryRef()->getGeometryType() )
            {
                ...
                case wkbPoint25D :
                    // point to drawable
                    drawable = pointsToDrawable(
                        static_cast<OGRPoint*>( ogrFeature->GetGeometryRef() )
                    );
                    disableCulling = true;
                    break;

                case wkbLinearRing :
                    drawable = linearRingToDrawable(
                        static_cast<OGRLinearRing*>( ogrFeature->GetGeometryRef() )
                    );
                    break;

                case wkbLineString :
                case wkbLineString25D :
                    drawable = lineStringToDrawable(
                        static_cast<OGRLineString*>( ogrFeature->GetGeometryRef() )
                    );
                    break;

                case wkbPolygon :
                case wkbPolygon25D :
                    drawable = polygonToDrawable(
                        static_cast<OGRPolygon*>( ogrFeature->GetGeometryRef() )
                    );
                    break;

                case wkbMultiPoint :
                case wkbMultiPoint25D :
                    drawable = multiPointToDrawable(
                        static_cast<OGRMultiPoint*>( ogrFeature->GetGeometryRef() )
                    );
                    disableCulling = true;
                    break;
                ...
                case wkbMultiLineString25D :
                    drawable = multiLineStringToDrawable(
                        static_cast<OGRMultiLineString*>( ogrFeature->GetGeometryRef() )
                    );
                    break;

                case wkbMultiPolygon :
                case wkbMultiPolygon25D :
                    drawable = multiPolygonToDrawable(
                        static_cast<OGRMultiPolygon*>( ogrFeature->GetGeometryRef() )
                    );
                    break;

                case wkbGeometryCollection :
                case wkbGeometryCollection25D :
                    OSG_WARN << "This geometry is not yet implemented "
            ...
            if( disableCulling )
            {
                geode->setCullingActive(
                    false
                );    // because culling on one points geode is always true, so i disable
                      // it
            }
            ...
            {
                geode->addDescription(
                    std::string( ogrFeature->GetFieldDefnRef( i )->GetNameRef() ) +
                    " : " +
                    std::string( ogrFeature->GetFieldAsString( i ) )
                );
            }
            return geode;
        }
