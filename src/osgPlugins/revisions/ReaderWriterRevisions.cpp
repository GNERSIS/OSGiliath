...
class ReaderWriterRevisions : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterRevisions()
        {
            ...
            supportsExtension( "removed",
                               "revision file containing list of removed files" );
            supportsExtension( "modified",
                               "revision file containing list of modified files" );
        }
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( fileName.empty() )
            {
                OSG_NOTICE << "Error: ReaderWriterRevision unable to determine stream "
                              "type, cannot not read file."
                           << std::endl;
                return ReadResult::FILE_NOT_HANDLED;
            }
        ...
        ReadResult
        readRevisions( std::istream&                       fin,
                       const std::string&                  name,
                       const osgDB::ReaderWriter::Options* options ) const
        {
            osg::ref_ptr<osgDB::DatabaseRevisions> revisions =
                new osgDB::DatabaseRevisions;
            revisions->setName( name );

            typedef std::map<std::string, osg::ref_ptr<osgDB::DatabaseRevision>>
                        RevisionMap;
                    ...
                    if( !revisionName.empty() )
                    {
                        ...
                        if( loadFileLists )
                        {
                            std::string complete_path =
                                osgDB::concatPaths( revisions_path, filename );
                            OSG_INFO << "    complete_path=" << complete_path
                                     << std::endl;
                            fileList =
                                osgDB::readRefFile<osgDB::FileList>( complete_path,
                                                                     options );
                        }
        ...
        {
            const osgDB::FileList* fileList =
                dynamic_cast<const osgDB::FileList*>( &object );
            if( fileList )
            {
            ...
            const osgDB::DatabaseRevisions* revisions =
                dynamic_cast<const osgDB::DatabaseRevisions*>( &object );
            if( revisions )
            {
                     ...
                     ++itr )
                {
                    ...
                    if( revision->getFilesAdded() )
                    {
                        ...
                        {
                            fout << "FilesAdded entry had no name assigned."
                                 << std::endl;
                        }
                    }
