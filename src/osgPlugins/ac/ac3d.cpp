...
class SurfaceBin : public PrimitiveBin {
  ...
  public:
    ...
    virtual osg::Geode* finalize(const MaterialData& material, const TextureData& textureData)
    {
        ...
        // handle quads - decompose to triangles for Core Profile compatibility
        if (!_quads.empty())
        {
            osg::ref_ptr<osg::DrawElementsUInt> drawElements = new osg::DrawElementsUInt(osg::PrimitiveSet::TRIANGLES);
            for (unsigned i = 0; i < _quads.size(); ++i)
            {
                ...
                {
                    idx[j] = pushVertex(_quads[i].index[j], vertexArray, normalArray, texcoordArray);
                }
                // Triangle 1: (0, 1, 3)
                drawElements->addElement(idx[0]);
                drawElements->addElement(idx[1]);
                drawElements->addElement(idx[3]);
                // Triangle 2: (1, 2, 3)
                drawElements->addElement(idx[1]);
                drawElements->addElement(idx[2]);
                drawElements->addElement(idx[3]);
            }
            geometry->addPrimitiveSet(createOptimalDrawElements(drawElements.get()));
        }

        // handle polygons - use TRIANGLE_FAN for Core Profile compatibility (convex polygons)
        if (!_polygons.empty())
        {
