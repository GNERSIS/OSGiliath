...
class ReaderWriterLAS : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterLAS()
        {
            ...
            supportsOption( "noScale",
                            "don't scale vertices according to las header - put scale "
                            "in matrixTransform" );
            supportsOption(
                "noReCenter",
                "don't transform vertex coords to re-center the pointcloud"
            );
        }
                  ...
                  const Options* options ) const
        {
            ...
            const liblas::Header& h      = reader.GetHeader();
            ...
            unsigned int      targetNumVertices = 10'000;
            ...
            while( reader.ReadNextPoint() )
            {
                ...
                uint32_t             a =
                    255;    // default value, since LAS point has no alpha information
                ...
                else
                {
                    if( singleColor )
                    {
                        singleColor = singleColorValue ==
                                      c;    // set false if different color found
                    }
                }
                ...
                colours->push_back( osg::ubvec4( r, g, b, a ) );
            ...
            if( _recenter )
            {
                ...
                for( unsigned int geomIndex = 0; geomIndex < geode->getNumDrawables();
                     ++geomIndex )
                {
                    osg::Geometry* geom =
                        dynamic_cast<osg::Geometry*>( geode->getDrawable( geomIndex ) );
                    if( geom )
                    {
                        osg::Vec3Array* vertArray =
                            dynamic_cast<osg::Vec3Array*>( geom->getVertexArray() );
                        size_t vertArraySize = vertArray->size();
                        for( size_t vertexIndex = 0; vertexIndex < vertArraySize;
                             ++vertexIndex )
                        {
            ...
            const double d2 = t.stop();

            if( _verbose )
            {
                std::cout << "Read points: " << i << " Elapsed Time: " << d2 << std::endl
                          << std::endl;
            }
            ...
            if( _scale )    // vertex positions are scaled already
            {
                if( _recenter )
                {
                    mt->setMatrix( osg::translate( osg::dvec3( h.GetOffsetX() + mid_x,
                                                               h.GetOffsetY() + mid_y,
                                                               h.GetOffsetZ() +
                                                                   mid_z ) ) );
                }
                ...
                {
                    mt->setMatrix( osg::translate(
                        osg::dvec3( h.GetOffsetX(), h.GetOffsetY(), h.GetOffsetZ() )
                    ) );
                }
            }
            else
            {
                if( _recenter )
                {
                    ...
                    mt->setMatrix(
                        osg::scale(
                            osg::dvec3( h.GetScaleX(), h.GetScaleY(), h.GetScaleZ() )
                        ) *
                        osg::translate( osg::dvec3( h.GetOffsetX() + mid_x,
                                                    h.GetOffsetY() + mid_y,
                                                    h.GetOffsetZ() + mid_z ) )
                    );
                }
                ...
                {
                    mt->setMatrix( osg::scale( osg::dvec3( h.GetScaleX(),
                                                           h.GetScaleY(),
                                                           h.GetScaleZ() ) ) *
                                   osg::translate( osg::dvec3( h.GetOffsetX(),
                                                               h.GetOffsetY(),
                                                               h.GetOffsetZ() ) ) );
                }
            }
