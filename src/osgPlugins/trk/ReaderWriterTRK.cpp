 ...
 * ReaderWriterTRK, derived from ReaderWriter.
 * Provides: supportsExtension, className, readObject, readNode, readObject, readNode.
 */
...
const char vert_shader_str[] =
    ...
    "void main(void)\n"
    "{\n"
    "    vec4 eye = osg_ModelViewMatrixInverse * vec4(0.0,0.0,0.0,1.0);\n"
    "    vec3 rayVector = normalize(osg_Vertex.xyz-eye.xyz);\n"
    "\n"
    "    vec3 dv = osg_Normal;\n"
    ...
    "    float half_l = l*0.5;\n"
    "\n"
    "    vertexColor = vec4( abs(dv.x), abs(dv.y), abs(dv.z), 1.0);\n"
    "    gl_Position = osg_ModelViewProjectionMatrix * osg_Vertex;\n"
    "}\n";

const char frag_shader_str[] = "#version 460 core\n"
                               "in vec4 vertexColor;\n"
                               "out vec4 fragColor;\n"
                               "void main(void)\n"
                               "{\n"
        ...
        assign( osg::Geometry* geometry )
        {
            ...
            for( unsigned int i = 0; i < geometry->getNumPrimitiveSets(); ++i )
            {
                ...
                if( pset->getMode() == GL_LINES )
                {
                    for( unsigned int pi = 0; pi < pset->getNumIndices() - 1; pi += 2 )
                    {
                        ...
                        osg::vec4  colour( dv.x + 1.0F / 2.0F,
                                           dv.y + 1.0F / 2.0F,
                                           dv.z + 1.0F / 2.0F,
                                           1.0F );
            ...
            for( unsigned int vi = 0; vi < vertices->size(); ++vi )
            {
                if( divisors[vi] > 0.0F )
                {
                    ...
                    ( *normals )[vi]  = osg::normalize( ( *normals )[vi] );
                }
            }
            ...
            osg::ref_ptr<osg::Shader>   vertexShader =
                osgDB::readRefShaderFile( osg::Shader::VERTEX, vertexShaderFile );
            if( !vertexShader )
            {
            ...
            osg::ref_ptr<osg::Shader> fragmentShader =
                new osg::Shader( osg::Shader::FRAGMENT, frag_shader_str );
...
class ReaderWriterTRK : public osgDB::ReaderWriter
{
    public:
                  ...
                  const osgDB::ReaderWriter::Options* = NULL ) const
        {
            ...
            OSG_NOTICE << "Read header successfully [" << header.id_string << "]"
                       << std::endl;
            bool requiresByteSwap = header.hdr_size != 1'000;
            if( requiresByteSwap )
            {
            ...
            OSG_NOTICE << "dim = " << header.dim[0] << ", " << header.dim[1] << ", "
                       << header.dim[2] << std::endl;
            OSG_NOTICE << "voxel_size = " << header.voxel_size[0] << ", "
                       << header.voxel_size[1] << ", " << header.voxel_size[2]
                       << std::endl;
            OSG_NOTICE << "origin = " << header.origin[0] << ", " << header.origin[1]
                       << ", " << header.origin[2] << std::endl;
            OSG_NOTICE << "n_scalars = " << header.n_scalars << std::endl;
            OSG_NOTICE << "n_properties = " << header.n_properties << std::endl;

            float vtr[16];
            memcpy( vtr, header.vox_to_ras, sizeof( float ) * 16 );
            osg::mat4 matrix( vtr );
            OSG_NOTICE << "vox_to_ras=" << matrix << std::endl;

            OSG_NOTICE << "invert_x=" << static_cast<int>( header.invert_x )
                       << std::endl;
            OSG_NOTICE << "invert_y=" << static_cast<int>( header.invert_y )
                       << std::endl;
            OSG_NOTICE << "invert_z=" << static_cast<int>( header.invert_z )
                       << std::endl;
