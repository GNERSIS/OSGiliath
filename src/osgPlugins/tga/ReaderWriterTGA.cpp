...
static void
convert_16_to_24( const unsigned char* const src,
...
static void
convert_16_to_32( const unsigned char* const src,
                  unsigned char* const       dest )
{
...
static void
convert_24_to_24( const unsigned char* const src,
                  unsigned char* const       dest )
{
    /* RGB for opengl */
    /* BGR for TGA */
...
static void
convert_32_to_32( const unsigned char* const src,
                  unsigned char* const       dest )
{
...
static void
convert_32_to_24( const unsigned char* const src,
                  unsigned char* const       dest )
{
...
static void
convert_data( const unsigned char* const src,
              unsigned char* const       dest,
              const int                  x,
              const int                  srcformat,
              const int                  destformat )
{
    if( destformat > 2 ) /* color */
    {
        if( srcformat == 2 )
        {
            if( destformat == 3 )
            {
                convert_16_to_24( src + x * srcformat, dest + x * destformat );
            }
            else
            {
                assert( destformat == 4 );
                convert_16_to_32( src + x * srcformat, dest + x * destformat );
            }
        }
        else if( srcformat == 3 )
        {
            assert( destformat == 3 );
            convert_24_to_24( src + x * srcformat, dest + x * destformat );
        }
        else
        {
            assert( srcformat == 4 );
            if( destformat == 3 )
            {
                convert_32_to_24( src + x * srcformat, dest + x * destformat );
            }
            else
            {
                assert( destformat == 4 );
                convert_32_to_32( src + x * srcformat, dest + x * destformat );
            }
        }
    }
    else
                 ...
                 bool          ignoreTGA2Fields )
{
    ...
    // Add 7 as R5G5B5 images with no alpha data take up two bytes per pixel, but only 15
    // bits
    depth    = ( header[16] + 7 ) >> 3;
    flags    = header[17];
    alphaBPP = flags & 0X0F;
    ...
    if( !ignoreTGA2Fields )
    {
        ...
        if( strcmp( ( char* )&footer[8], "TRUEVISION-XFILE." ) == 0 )
        {
            ...
            {
                endOfImage =
                    std::min( endOfImage, ( std::streampos )extensionAreaOffset );
                ...
                {
                    endOfImage =
                        std::min( endOfImage, ( std::streampos )colorCorrectionOffset );
                }
                if( postageStampOffset != 0 )
                {
                    endOfImage =
                        std::min( endOfImage, ( std::streampos )postageStampOffset );
                }
                if( scanLineOffset != 0 )
                {
                    endOfImage =
                        std::min( endOfImage, ( std::streampos )scanLineOffset );
                }
            ...
            {
                endOfImage =
                    std::min( endOfImage, ( std::streampos )developerAreaOffset );
            }
        }
    }
    ...
    if( header[1] == 1 ) /* there is a colormap */
    {
        ...
        if( colormapFirst != 0 )
        {
            // Error on non-zero colormapFirst as it's unclear from the specification
        ...
        fin.read( reinterpret_cast<char*>( colormap.data() ),
                  colormapLen * colormapDepth );
        ...
        if( colormapDepth == 2 ) /* 16 bits */
        {
            if( alphaBPP ==
                ...
                ( attributeType ==
                  ...
                  attributeType ==
                  ALPHA_PREMULTIPLIED ||
                  attributeType == ATTRIBUTE_TYPE_UNSET ) )
            {
                format = 4;
            }
            else
                ...
                for( y = 0; y < height; y++ )
                {
                    ...
                    for( x = 0; x < width; x++ )
                    {
                        ...
                        switch( depth )
                        {
                            ...
                            default :
                                tgaerror = ERR_UNSUPPORTED;
                                return NULL; /* unreachable code - (depth < 1 || depth >
                                                4) rejected by "check for reasonable
                                                values in case this is not a tga file"
        ...
        case 3 : /* greyscale, uncompressed */
            {
                ...
                for( y = 0; y < height; y++ )
                {
                    ...
                    for( x = 0; x < width; x++ )
                    {
                        convert_data( linebuf.data(),
                                      dest,
                                      bLeftToRight ? x : ( width - 1 ) - x,
                                      depth,
                                      format );
                    }
                    dest += lineoffset;
                    ...
                    for( y = 0; y < height; y++ )
                    {
                        ...
                        for( x = 0; x < width; x++ )
                        {
                            ...
                            switch( depth )
                            {
                                ...
                                default :
                                    tgaerror = ERR_UNSUPPORTED;
                                    return NULL; /* unreachable code - (depth < 1 ||
                                                    depth > 4) rejected by "check for
                                                    reasonable values in case this is not
                                                    a tga file" near the start of this
                                                    function*/
                            }
...
class ReaderWriterTGA : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterTGA()
        {
            ...
            supportsOption( "ignoreTga2Fields",
                            "(Read option) Ignore TGA 2.0 fields, even if present. "
                            "Makes it possible to read files as a TGA 1.0 reader would, "
                            "helpful when dealing with malformed TGA 2.0 files which "
                            "are still valid TGA 1.0 files, such as when an image ends "
                            "with data resembling a TGA 2.0 footer by coincidence." );
        }
        ...
        ReadResult
        readTGAStream( std::istream& fin,
                       bool          ignoreTGA2Fields ) const
        {
            ...
            unsigned int pixelFormat    = numComponents_ret == 1 ? GL_RED
                                        : numComponents_ret == 2 ? GL_RG
                                        : numComponents_ret == 3 ? GL_RGB
                                        : numComponents_ret == 4 ? GL_RGBA
                                                                 : ( GLenum )-1;
        ...
        bool
        saveTGAStream( const osg::Image& image,
                       std::ostream&     fout ) const
        {
            ...
            fout.put( 0 );
            fout.put( 0 );                            // Color map origin
            fout.put( 0 );
            fout.put( 0 );                            // Color map length
            fout.put( 0 );                            // Color map entry size
            fout.put( 0 );
            fout.put( 0 );                            // X origin of image
            fout.put( 0 );
            fout.put( 0 );                            // Y origin of image
            fout.put( width & 0XFF );
            fout.put( ( width & 0XFF'00 ) >> 8 );     // Width of image
            fout.put( height & 0XFF );
            fout.put( ( height & 0XFF'00 ) >> 8 );    // Height of image
            ...
            for( int y = 0; y < height; ++y )
            {
                ...
                for( int x = 0; x < width; ++x )
                {
                    ...
                    {
                        case 3 :    // BGR
                            fout.put( ptr[off + b] * pixelMultiplier );
                            fout.put( ptr[off + g] * pixelMultiplier );
