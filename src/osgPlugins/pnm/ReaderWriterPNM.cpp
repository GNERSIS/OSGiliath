...
template<class T>
unsigned char*
read_bitmap_ascii( std::istream& fin,
                   int           width,
                   int           height )
{
...
template<class T>
unsigned char*
read_grayscale_ascii( std::istream& fin,
                      int           width,
                      int           height,
                      int           max_value )
{
...
template<class T>
unsigned char*
read_color_ascii( std::istream& fin,
                  int           width,
                  int           height,
                  int           max_value )
{
...
template<class T>
unsigned char*
read_bitmap_binary( std::istream& fin,
                    int           width,
                    int           height )
{
    T* data = new T[width * height];
...
template<class T>
unsigned char*
read_grayscale_binary( std::istream& fin,
                       int           width,
                       int           height )
{
    T* data = new T[width * height];
...
template<class T>
unsigned char*
read_color_binary( std::istream& fin,
                   int           width,
                   int           height )
{
    T* data = new T[3 * width * height];
...
class ReaderWriterPNM : public osgDB::ReaderWriter
{
    public:
                   ...
                   const osgDB::ReaderWriter::Options* options = NULL ) const
        {
            ...
            if( max_value > 255 )
            {
                ...
                switch( ppmtype )
                {
                    case 2 :    // grayscale ascii
                        pixelFormat = GL_RED;
                        data        = read_grayscale_ascii<unsigned short>( fin,
                                                                            width,
                                                                            height,
                                                                            max_value );
                        break;
                    case 3 :    // color ascii
                        pixelFormat = GL_RGB;
                        data        = read_color_ascii<unsigned short>( fin,
                                                                        width,
                                                                        height,
                                                                        max_value );
                        break;
                    case 5 :    // grayscale binary
                        pixelFormat = GL_RED;
                        data =
                            read_grayscale_binary<unsigned short>( fin, width, height );
                        break;
                    case 6 :    // color binary
                        pixelFormat = GL_RGB;
            ...
            else
            {
                ...
                switch( ppmtype )
                {
                    ...
                    case 2 :    // grayscale ascii
                        pixelFormat = GL_RED;
                        data        = read_grayscale_ascii<unsigned char>( fin,
                                                                           width,
                                                                           height,
                                                                           max_value );
                        break;
                    case 3 :    // color ascii
                        pixelFormat = GL_RGB;
                        data        = read_color_ascii<unsigned char>( fin,
                                                                       width,
                                                                       height,
                                                                       max_value );
                        break;
                    ...
                    case 5 :    // grayscale binary
                        pixelFormat = GL_RED;
                        data =
                            read_grayscale_binary<unsigned char>( fin, width, height );
                        break;
                    case 6 :    // color binary
                        pixelFormat = GL_RGB;
            ...
            if( options &&
                options->getOptionString().find( "flip" ) != std::string::npos )
            {
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            bool ascii =
                ( options &&
                  options->getOptionString().find( "ascii" ) != std::string::npos );
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            {
                return WriteResult(
                    "Error image pixel format not supported by pnm writer."
                );
            }
