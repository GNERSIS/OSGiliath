...
tsize_t
libtiffStreamReadProc( thandle_t fd,
                       tdata_t   buf,
                       tsize_t   size )
{
...
tsize_t
libtiffStreamWriteProc( thandle_t,
                        tdata_t,
                        tsize_t )
{
    return 0;
...
toff_t
libtiffStreamSeekProc( thandle_t fd,
                       toff_t    off,
                       int       i )
{
    std::istream* fin = ( std::istream* )fd;
...
int
libtiffStreamCloseProc( thandle_t )
{
    return 0;
}

toff_t
libtiffStreamSizeProc( thandle_t fd )
{
    std::istream*  fin    = ( std::istream* )fd;
...
int
libtiffStreamMapProc( thandle_t,
                      tdata_t*,
                      toff_t* )
{
    return 0;
...
void
libtiffStreamUnmapProc( thandle_t,
                        tdata_t,
                        toff_t )
{
}
...
tsize_t
libtiffOStreamReadProc( thandle_t,
                        tdata_t,
                        tsize_t )
{
    return 0;
...
tsize_t
libtiffOStreamWriteProc( thandle_t fd,
                         tdata_t   buf,
                         tsize_t   size )
{
...
toff_t
libtiffOStreamSizeProc( thandle_t fd )
{
    std::ostream*  fout   = ( std::ostream* )fd;
...
toff_t
libtiffOStreamSeekProc( thandle_t fd,
                        toff_t    off,
                        int       i )
{
    std::ostream* fout         = ( std::ostream* )fd;
    ...
    {
        // position required past the end of the stream so we need to insert extra
        // characters to ensure the stream is big enough to encompass the new the
        // position.
        fout->seekp( 0, std::ios::end );
...
doFormat( const char* fmt,
          va_list     ap )
{
    static const int MSG_BUFSIZE =
        256;            // Initial size of the buffer used for formatting
    static const int MAX_BUFSIZE =
        256 * 1'024;    // Maximum size of the buffer used for formatting
    for( int size = MSG_BUFSIZE; size < MAX_BUFSIZE; )
    {
        // Sukender: Here we could try/catch(std::bad_alloc &), but this is clearly an
        // and-of-all-things condition knowing the fact 'size' is kept small.
    ...
    return std::string( fmt, fmt + MSG_BUFSIZE ) +
           "...";    // Fallback: Message is not formatted and truncated, but that's
                     // better than no message
}

static void
tiff_error( const char*,
            const char* fmt,
            va_list     ap )
{
                  ...
                  uint16&       bitspersample )
{
    ...
    if( TIFFGetField( in, TIFFTAG_PHOTOMETRIC, &photometric ) == 1 )
    {
        ...
        {
            OSG_NOTICE
                << "Photometric type " << photometric
                << " not handled; can only handle Grayscale, RGB and Palette images"
                << std::endl;
            TIFFClose( in );
    ...
    if( TIFFGetField( in, TIFFTAG_IMAGEWIDTH, &w ) !=
        1 ||
    ...
    switch( pack( photometric, config ) )
    {
        ...
        case pack( PHOTOMETRIC_PALETTE, PLANARCONFIG_SEPARATE ) :
            ...
            if( !tifferror &&
                bitspersample !=
                32 &&
                checkcmap( 1 << bitspersample, red, green, blue ) == 16 )
            {
                int i;
...
class ReaderWriterTIFF : public osgDB::ReaderWriter
{
    public:
        ...
        ReadResult
        readTIFStream( std::istream& fin ) const
        {
            ...
            imageData                        = simage_tiff_load( fin,
                                                                 width_ret,
                                                                 height_ret,
                                                                 numComponents_ret,
                                                                 bitspersample_ret );
            ...
            unsigned int dataType       = bitspersample_ret == 8  ? GL_UNSIGNED_BYTE
                                        : bitspersample_ret == 16 ? GL_UNSIGNED_SHORT
                                        : bitspersample_ret == 32 ? GL_FLOAT
                                                                  : ( GLenum )-1;
        ...
        writeTIFStream( std::ostream&                       fout,
                        const osg::Image&                   img,
                        const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            image = TIFFClientOpen( "outputstream",
                                    "w",
                                    ( thandle_t )&fout,
