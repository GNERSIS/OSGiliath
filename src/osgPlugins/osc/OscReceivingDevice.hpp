 ...
 * OscReceivingDevice, derived from Device.
 * Provides: RequestHandler, getRequestPath, describeTo, SendKeystrokeRequestHandler,
 * getDevice, getDevice.
 */
#pragma once

/**
 * OscReceivingDevice can be used to receive osg-events via OSC from other hosts/
 applications
 ...
 *
 * This device adds to every message-bundle a message-id, so you can check, if you miss
 events or similar.
 *
 ...
 *
 * this will result in one osgGA:Event (when both messages are bundled) with the name
 "/my_user_event",
 ...
 *
 * The receiving device will try to combine multiple osc arguments intelligently,
 multiple osc-arguments are
 * bundled into vec2, vec3, vec4 or dmat4, it depends on the number of arguments.

 * TUIO-specific notes:
 * If multiple TUIO-applications are transmitting their touch-points to one
 oscReceivingDevice, all
 ...
 * are specific to an application, the lower 16bits contain the touch-id for that
 application.
 * If you need "better" separation, use multiple oscReceivingDevices listening on
 different ports.
 *
    ...
    public:
        ...
        class RequestHandler : public osg::Referenced
        {
            ...
            protected:
                ...
                void
                setRequestPath( const std::string& request_path )
                {
                    _requestPath = request_path;
                }

                void
                handleException( const osc::Exception& e )
                {
                    OSG_WARN << "OscDevice :: error while handling " << getRequestPath()
                             << ": " << e.what() << std::endl;
                }
        ...
        typedef std::multimap<std::string, osg::ref_ptr<RequestHandler>>
            RequestHandlerMap;

        OscReceivingDevice( const std::string& server_address,
                            int                listening_port );
        ~OscReceivingDevice();
        ...
        operator<<( std::ostream&             out,
                    const OscReceivingDevice& device )
        {
        ...
        void
        addHandleOnCheckEvents( RequestHandler* handler )
        {
        ...
        checkEvents()
        {
            ...
            for( std::vector<RequestHandler*>::iterator i = _handleOnCheckEvents.begin();
                 i != _handleOnCheckEvents.end();
                 ++i )
            {
                ( *i )->operator()( queue );
            }
...
class SendKeystrokeRequestHandler : public OscReceivingDevice::RequestHandler
{
    public:
        ...
        SendKeystrokeRequestHandler( const std::string& request_path,
                                     int                key ) :
            OscReceivingDevice::RequestHandler( request_path ),
            _key( key )
        {
        }
        ...
        operator()( const std::string& /*request_path*/,
                    const std::string& /*full_request_path*/,
                    const osc::ReceivedMessage& /*arguments*/,
                    const IpEndpointName& /*remoteEndPoint*/ )
        {
        ...
        describeTo( std::ostream& out ) const
        {
            out << getRequestPath() << ": send KEY_DOWN + KEY_UP, code: 0x" << std::hex
                << _key << std::dec;
        }
