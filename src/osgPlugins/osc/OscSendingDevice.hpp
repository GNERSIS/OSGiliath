 ...
 * OscSendingDevice, derived from Device.
 * Provides: sendEvent, className.
 */
#pragma once
 ...
 *
 * The sending device knows how to handle vec2, vec3, vec4, quat, Plane and dmat4 values
 *
 * TUIO-specific notes:
 * If you want to explicitly set the application-name when sending multi-touch-events,
 * use osc_device->setUserValue("tuio_application_name", "<your application name>");
...
#include <osc/OscOutboundPacketStream.hpp>
...
class OscSendingDevice : public osgGA::Device
{
    public:
        ...
        OscSendingDevice( const std::string& address,
                          int                port,
                          unsigned int       numMessagesPerEvent              = 1,
                          unsigned int       delay_between_sends_in_millisecs = 0 );
        ~OscSendingDevice();
        virtual void
        sendEvent( const osgGA::Event& ea );
    ...
    private:
        ...
        bool
        sendEventImpl( const osgGA::Event& ea,
                       MsgIdType           msg_id );
        bool
        sendUIEventImpl( const osgGA::GUIEventAdapter& ea,
                         MsgIdType                     msg_id );
        void
        beginBundle( MsgIdType msg_id );
        void
        beginSendInputRange( const osgGA::GUIEventAdapter& ea,
                             MsgIdType                     msg_id );
        void
        beginMultiTouchSequence();
        bool
        sendMultiTouchData( const osgGA::GUIEventAdapter& ea );
        int
        getButtonNum( const osgGA::GUIEventAdapter& ea );
        void
        sendUserDataContainer( const std::string&            key,
                               const osg::UserDataContainer* udc,
                               bool                          asBundle,
                               MsgIdType                     msg_id );
        std::string
                                  transliterateKey( const std::string& key ) const;
