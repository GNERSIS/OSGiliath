 ...
 * ReaderWriterOsc, derived from ReaderWriter.
 ...
 *  osgGA::Device* device =
 * dynamic_cast<osgGA::Device*>(osgDB::readObjectFile(filename));
 *
 ...
 *  osgGA::GUIEventHandler* event_handler =
 * dynamic_cast<osgGA::GUIEventHandler*>(osgDB::readObjectFile(filename));
 *
...
class ReaderWriterOsc : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterOsc()
        {
            ...
            supportsOption(
                "documentRegisteredHandlers",
                "dump a documentation of all registered REST-handler to the console"
            );
            supportsOption(
                "numMessagesPerEvent",
                "set the number of osc-messages to send for one event (sender-only)"
            );
            supportsOption( "delayBetweenSendsInMillisecs",
                            "when sending multiple msgs per event you can specify an "
                            "optional delay between the sends (sender-only)" );
        }
                    ...
                    const osgDB::ReaderWriter::Options* options = NULL ) const
        {
            if( osgDB::getFileExtension( file ) == "osc" )
            {
                ...
                if( osgDB::getFileExtension( file_name ) == "sender" )
                {
                    ...
                    if( options &&
                        !options->getPluginStringData( "numMessagesPerEvent" ).empty() )
                    {
                        std::string num_messages_per_event_str =
                            options->getPluginStringData( "numMessagesPerEvent" );
                        num_messages_per_event =
                            std::max( 1, atoi( num_messages_per_event_str.c_str() ) );
                    }

                    unsigned int delay_between_sends_in_millisecs = 0;
                    if( options &&
                        !options->getPluginStringData( "delayBetweenSendsInMillisecs" )
                             .empty() )
                    {
                        std::string delay_between_sends_in_millisecs_str =
                            options->getPluginStringData(
                                "delayBetweenSendsInMillisecs"
                            );
                        delay_between_sends_in_millisecs =
                            atoi( delay_between_sends_in_millisecs_str.c_str() );
                    }

                    return new OscSendingDevice( server_address,
                ...
                else
                {
                    ...
                    if( port <= 0 )
                    {
                        OSG_WARN << "ReaderWriterOsc :: can't get valid port from "
                                 << osgDB::getNameLessAllExtensions( file ) << std::endl;
                        port = 8'000;
                    }
                    try
                    {

                        osg::ref_ptr<OscReceivingDevice> device =
                            new OscReceivingDevice( server_address, port );

                        device->addRequestHandler( new SendKeystrokeRequestHandler(
                            "/p3d/slide/first",
                        ...
                        device->addRequestHandler( new SendKeystrokeRequestHandler(
                            "/p3d/slideorlayer/next",
                            osgGA::GUIEventAdapter::KEY_Page_Down
                        ) );
                        device->addRequestHandler( new SendKeystrokeRequestHandler(
                            "/p3d/slideorlayer/previous",
                            osgGA::GUIEventAdapter::KEY_Page_Up
                        ) );

                        device->addRequestHandler(
                            new SendKeystrokeRequestHandler( "/p3d/unpause", 'o' )
                        );
                        device->addRequestHandler(
                            new SendKeystrokeRequestHandler( "/p3d/pause", 'p' )
                        );

                        device->addRequestHandler(
                            new SendKeystrokeRequestHandler( "/osgviewer/home", ' ' )
                        );
                        device->addRequestHandler(
                            new SendKeystrokeRequestHandler( "/osgviewer/stats", 's' )
                        );

                        if( options && ( options->getPluginStringData(
                                             "documentRegisteredHandlers"
                                         ) == "true" ) )
                        {
                    ...
                    {
                        OSG_WARN << "OscDevice :: could not register UDP listener : "
                                 << e.what() << std::endl;
