 ...
 * StandardRequestHandler, derived from RequestHandler.
 * Provides: describeTo, SetMouseInputRangeRequestHandler, x_min, getDevice, catch,
 * handleException.
 */
#include "osc/OscHostEndianness.hpp"
#include "osc/OscPrintReceivedElements.hpp"
...
namespace OscDevice
{
    ...
    struct NativeTypeTraits<float, 2>
    {
            ...
            static type
            create( const std::vector<float>& t )
            {
                return type( t[0], t[1] );
            }
    };
    ...
    {
            typedef osg::dvec2 type;

            static type
            create( const std::vector<double>& t )
            {
                return type( t[0], t[1] );
            }
    };
    ...
    class StandardRequestHandler : public OscReceivingDevice::RequestHandler
    {
        ...
        public:
            ...
            StandardRequestHandler( const std::string& request_handler,
            ...
            virtual bool
            operator()( const std::string&          request_path,
            ...
            {
                out << getRequestPath()
                    << ": add all transmitted arguments as ValueObjects to an event";
                if( _treatFirstArgumentAsValueName )
                {
                    out << ", the first argument is used as the name of the value, if "
                           "it's a string";
                }
            }
        ...
        private:
            ...
            void
            addArgumentToUdc( osg::UserDataContainer*                     udc,
                              const std::string&                          key,
                              const osc::ReceivedMessageArgumentIterator& itr );

            template<class T>
            bool
            addNativeTypeFromVector( osg::UserDataContainer* udc,
                                     const std::string&      key,
    ...
    bool
    StandardRequestHandler::operator()( const std::string& /*request_path*/,
                                        const std::string&          full_request_path,
                                        const osc::ReceivedMessage& m,
                                        const IpEndpointName& /*remoteEndPoint*/ )
    {
        try
        {
            ...
            else
            {
                ...
                if( ( _treatFirstArgumentAsValueName ) &&
                    ( start->TypeTag() == osc::STRING_TYPE_TAG ) )
                {
                ...
                for( osc::ReceivedMessageArgumentIterator itr = start;
                     itr != m.ArgumentsEnd();
                     ++itr, ++i )
                {
    ...
    void
    StandardRequestHandler::addArgumentToUdc(
    ...
    )
    {
        switch( ( *itr ).TypeTag() )
        {
            ...
            case osc::RGBA_COLOR_TYPE_TAG :
                ...
                udc->setUserValue(
                    key,
                    static_cast<unsigned int>( ( *itr ).AsRgbaColorUnchecked() )
                );
                break;

            case osc::INT64_TYPE_TAG :
                // TODO 64bit ints not supported by ValueObject
                udc->setUserValue( key,
                                   static_cast<double>( ( *itr ).AsInt64Unchecked() ) );
                break;
            ...
            case osc::TIME_TAG_TYPE_TAG :
                ...
                udc->setUserValue(
                    key,
                    static_cast<double>( ( *itr ).AsTimeTagUnchecked() )
                );
                break;
        ...
        public:
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                try
                {
                    ...
                    getDevice()->getEventQueue()->setMouseInputRange( x_min,
                                                                      y_min,
                                                                      x_max,
                                                                      y_max );
            ...
            {
                out << getRequestPath()
                    << "(float x_min, float y_min, float x_max, float y_max): sets the "
                       "mouse-input-range"
                    << std::dec;
            }
    };
        ...
        public:
            ...
            SetMouseOrientationRequestHandler() :
                OscReceivingDevice::RequestHandler(
                    "/osgga/mouse/y_orientation_increasing_upwards"
                )
            {
            }
    ...
    class KeyCodeRequestHandler : public OscReceivingDevice::RequestHandler
    {
        public:
            ...
            KeyCodeRequestHandler( bool handle_key_press ) :
                OscReceivingDevice::RequestHandler(
                    std::string( "/osgga/key/" ) +
                    ( ( handle_key_press ) ? "press" : "release" )
                ),
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                try
                {
                    ...
                    if( _handleKeyPress )
                    {
                        getDevice()->getEventQueue()->keyPress( keycode,
                                                                getLocalTime() );
                    }
                    else
                    {
                        getDevice()->getEventQueue()->keyRelease( keycode,
                                                                  getLocalTime() );
                    }
            ...
            describeTo( std::ostream& out ) const
            {
                out << getRequestPath() << "(int keycode): send KEY_"
                    << ( _handleKeyPress ? "DOWN" : "UP" );
            }
        ...
        public:
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                ...
                try
                {
                    ...
                    getDevice()->getEventQueue()->mouseMotion( _lastX,
                                                               _lastY,
                                                               getLocalTime() );
        ...
        public:
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                ...
                try
                {
                    ...
                    {
                        getDevice()->getEventQueue()->mouseScroll(
                    ...
                    {
                        getDevice()->getEventQueue()->mouseScroll2D( delta_x,
                                                                     delta_y,
                                                                     getLocalTime() );
                    }
            ...
            {
                out << getRequestPath()
                    << "(int scroll_motion, float x, float y): send mouse scroll-motion";
            }
    };
    ...
    class MouseButtonToggleRequestHandler : public OscReceivingDevice::RequestHandler
    {
        public:
            ...
            MouseButtonToggleRequestHandler( const std::string&         btn_name,
                                             MouseMotionRequestHandler* mm_handler ) :
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                ...
                try
                {
                    ...
                    if( down > 0 )
                    {
                        getDevice()->getEventQueue()->mouseButtonPress(
                            _mmHandler->getLastX(),
                            _mmHandler->getLastY(),
                            _btnNum,
                            getLocalTime()
                    ...
                    {
                        getDevice()->getEventQueue()->mouseButtonRelease(
                            _mmHandler->getLastX(),
                            _mmHandler->getLastY(),
                            _btnNum,
                            getLocalTime()
                ...
                try
                {
                    ...
                    switch( _mode )
                    {
                        case PRESS :
                            getDevice()
                                ->getEventQueue()
                                ->mouseButtonPress( x, y, btn, getLocalTime() );
                            break;
                        case RELEASE :
                            getDevice()
                                ->getEventQueue()
                                ->mouseButtonRelease( x, y, btn, getLocalTime() );
                            break;
                        case DOUBLE_PRESS :
                            getDevice()
                                ->getEventQueue()
                                ->mouseDoubleButtonPress( x, y, btn, getLocalTime() );
                            break;
                    }
        ...
        public:
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                try
                {
                    ...
                    getDevice()->getEventQueue()->penPressure( pressure,
                                                               getLocalTime() );
    ...
    class PenProximityRequestHandler : public OscReceivingDevice::RequestHandler
    {
        public:
            ...
            PenProximityRequestHandler( bool handle_enter ) :
                OscReceivingDevice::RequestHandler(
                    std::string( "/osgga/pen/proximity/" ) +
                    ( ( handle_enter ) ? std::string( "enter" )
                                       : std::string( "leave" ) )
                ),
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                try
                {
                    ...
                    getDevice()->getEventQueue()->penProximity(
                        ( osgGA::GUIEventAdapter::TabletPointerType )pt,
                        _handleEnter,
                        getLocalTime()
                    );
            ...
            {
                out << getRequestPath()
                    << "(int table_pointer_type): send pen proximity "
                    << ( _handleEnter ? "enter" : "leave" );
            }
        ...
        public:
                        ...
                        const IpEndpointName& /*remoteEndPoint*/ )
            {
                try
                {
                    ...
                    getDevice()->getEventQueue()->penOrientation( tilt_x,
                                                                  tilt_y,
                                                                  rotation,
                                                                  getLocalTime() );
            ...
            {
                out << getRequestPath()
                    << "(float rotation, float tilt_x, float tilt_y): send pen "
                       "orientation";
            }
    };
    ...
    class TUIO2DCursorRequestHandler : public OscReceivingDevice::RequestHandler
    {
        ...
        public:
            ...
            virtual void
            setDevice( OscReceivingDevice* device )
            {
                        ...
                        const IpEndpointName&       remoteEndPoint )
            {
                ...
                std::string end_point( ' ',
                                       IpEndpointName::ADDRESS_AND_PORT_STRING_LENGTH );
            ...
            virtual void
            operator()( osgGA::EventQueue* queue )
            {
                ...
                for( ApplicationCursorMap::iterator i = _alive.begin();
                     i != _alive.end();
                     ++i )
                {
                    ...
                    /*
                    std::cout << source << ": ";
                    for(std::set<unsigned int>::iterator k =
                    _endpointData[source].unhandled.begin(); k !=
                    _endpointData[source].unhandled.end();
                        ++k)
                    {
                        std::cout << *k << " ";
                    }
                    ...
                    */
                    ...
                    // remove all touchpoints which are not transmitted via
                    // alive-message, dispatching TOUCH_ENDED

                    unsigned int              source_id = getSourceId( source );

                    std::vector<unsigned int> to_delete;

                    for( CursorMap::iterator k = cursormap.begin(); k != cursormap.end();
                         ++k )
                    {
                        EndpointData& endpoint_data(
                            _endpointData[k->second.end_point]
                        );
                        ...
                        {
                            // std::cout << "deleting: " << k->first << " from " <<
                            // k->second.end_point << std::endl;
                    ...
                    // remove "dead" cursors
                    for( std::vector<unsigned int>::iterator k = to_delete.begin();
                         k != to_delete.end();
                         ++k )
                    {
                     ...
                     ++i )
                {
                    ...
                    for( CursorMap::iterator k = i->second.begin(); k != i->second.end();
                         ++k )
                    {
                        ...
                        bool down = c.phase !=
                                    ...
                                    c.phase != osgGA::GUIEventAdapter::TOUCH_STATIONERY;
                        if( !event )
                        {
                            if( down )
                            {
                                event = queue->touchBegan(
                                    touch_id,
                        ...
                        else
                        {
                            event->addTouchPoint(
                                touch_id,
                                down ? osgGA::GUIEventAdapter::TOUCH_BEGAN
                                     : osgGA::GUIEventAdapter::TOUCH_MOVED,
                                win_x,
                                win_y
                            );
            ...
            inline void
            updateSourceIdMap( const std::string& source )
            {
            ...
            inline unsigned int
            getSourceId( const std::string& source )
            {
...
OscReceivingDevice::OscReceivingDevice( const std::string& server_address,
                                        int                listening_port ) :
    osgGA::Device(),
    osg::Thread(),
    ...
    _lastMsgId( 0 )
{
    ...
    _socket = new UdpListeningReceiveSocket( IpEndpointName( server_address.c_str(),
                                                             listening_port ),
                                             this );
    ...
    OscDevice::MouseMotionRequestHandler* mm_handler =
        new OscDevice::MouseMotionRequestHandler();
    addRequestHandler( mm_handler );
    addRequestHandler( new OscDevice::MouseButtonRequestHandler(
        OscDevice::MouseButtonRequestHandler::PRESS
    ) );
    addRequestHandler( new OscDevice::MouseButtonRequestHandler(
        OscDevice::MouseButtonRequestHandler::RELEASE
    ) );
    addRequestHandler( new OscDevice::MouseButtonRequestHandler(
        OscDevice::MouseButtonRequestHandler::DOUBLE_PRESS
    ) );
    addRequestHandler( new OscDevice::MouseScrollRequestHandler() );

    addRequestHandler( new OscDevice::MouseButtonToggleRequestHandler( "1",
                                                                       mm_handler ) );
    addRequestHandler( new OscDevice::MouseButtonToggleRequestHandler( "2",
                                                                       mm_handler ) );
    addRequestHandler( new OscDevice::MouseButtonToggleRequestHandler( "3",
                                                                       mm_handler ) );
    ...
    addRequestHandler( new OscDevice::StandardRequestHandler( "/osg/set_user_value",
                                                              true ) );
...
void
OscReceivingDevice::ProcessMessage( const osc::ReceivedMessage& m,
                                    const IpEndpointName&       remoteEndpoint )
{
    ...
    do
    {
        ...
        if( pos != std::string::npos )
        {
            ...
            std::pair<RequestHandlerMap::iterator, RequestHandlerMap::iterator> range =
                _map.equal_range( mangled_path );

            for( RequestHandlerMap::iterator i = range.first; i != range.second; ++i )
            {
                // OSG_INFO << "OscDevice :: handling " << mangled_path << " with " <<
                // i->second << std::endl;

                if( i->second->operator()( mangled_path,
                                           in_request_path,
                                           m,
                                           remoteEndpoint ) &&
                    !handled )
...
void
OscReceivingDevice::ProcessBundle( const osc::ReceivedBundle& b,
                                   const IpEndpointName&      remoteEndpoint )
{
    // find msg-id
    MsgIdType msg_id( 0 );

    for( osc::ReceivedBundle::const_iterator i = b.ElementsBegin(); i != b.ElementsEnd();
         ++i )
    {
    ...
    if( msg_id )
    {
        ...
        {
            // already handled
            // OSG_WARN << "OscReceiver :: message with lower id received: " << msg_id <<
            // std::endl;
            return;
        }
        else
        {
            ...
            {
                OSG_WARN << "OscReceiver :: missed " << ( msg_id - _lastMsgId )
                         << " messages, (" << msg_id << "/" << _lastMsgId << ")"
                         << std::endl;
            }
            _lastMsgId = msg_id;
        }
...
void
OscReceivingDevice::ProcessPacket( const char*           data,
                                   int                   size,
                                   const IpEndpointName& remoteEndpoint )
{
    try
    ...
    {
        OSG_WARN << "OscDevice :: could not process UDP-packet: " << e.what()
                 << std::endl;
    }
    catch( ... )
    {
        OSG_WARN << "OscDevice :: could not process UDP-packet because of an exception!"
                 << std::endl;
    }
...
void
OscReceivingDevice::addRequestHandler( RequestHandler* handler )
{
    if( handler )
    {
...
void
OscReceivingDevice::describeTo( std::ostream& out ) const
{
    out << "OscDevice :: listening on " << _listeningAddress << ":" << _listeningPort
        << std::endl;
    out << std::endl;
