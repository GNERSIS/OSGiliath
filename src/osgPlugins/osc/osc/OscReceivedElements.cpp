 ...
 * Utility functions for osgPlugins: FindStr4End, RoundUp4, ToInt32, ToUInt32, ToInt64.
 */
...
namespace osc
{
    ...
    bool
    ReceivedBundleElement::IsBundle() const
    {
        return ( Size() > 0 && Contents()[0] == '#' );
    }
    ...
    bool
    ReceivedMessageArgument::AsBool() const
    {
        if( !typeTag_ )
        {
    ...
    bool
    ReceivedMessageArgument::AsBoolUnchecked() const
    {
        if( !typeTag_ )
        {
            throw MissingArgumentException();
    ...
    float
    ReceivedMessageArgument::AsFloat() const
    {
        if( !typeTag_ )
        {
            throw MissingArgumentException();
    ...
    float
    ReceivedMessageArgument::AsFloatUnchecked() const
    {
#ifdef OSC_HOST_LITTLE_ENDIAN
        union
        {
    ...
    char
    ReceivedMessageArgument::AsChar() const
    {
        if( !typeTag_ )
        {
            throw MissingArgumentException();
    ...
    char
    ReceivedMessageArgument::AsCharUnchecked() const
    {
    ...
    double
    ReceivedMessageArgument::AsDouble() const
    {
        if( !typeTag_ )
        {
            throw MissingArgumentException();
    ...
    double
    ReceivedMessageArgument::AsDoubleUnchecked() const
    {
#ifdef OSC_HOST_LITTLE_ENDIAN
        union
        {
    ...
    const char*
    ReceivedMessageArgument::AsString() const
    {
        if( !typeTag_ )
        {
            throw MissingArgumentException();
    ...
    const char*
    ReceivedMessageArgument::AsSymbol() const
    {
        if( !typeTag_ )
        {
            throw MissingArgumentException();
    ...
    void
    ReceivedMessageArgument::AsBlob( const void*&   data,
    ...
    void
    ReceivedMessageArgument::AsBlobUnchecked( const void*&   data,
                                              unsigned long& size ) const
    {
    ...
    void
    ReceivedMessageArgumentIterator::Advance()
    {
        if( !value_.typeTag_ )
        {
            return;
            ...
            case BLOB_TYPE_TAG :
                {
                    ...
                    value_.argument_ =
                        value_.argument_ + 4 + RoundUp4( ( unsigned long )blobSize );
                }
                break;
    ...
    bool
    ReceivedMessage::AddressPatternIsUInt32() const
    {
    ...
    void
    ReceivedMessage::Init( const char*   message,
                           unsigned long size )
    {
        if( size == 0 )
        {
                        ...
                        case BLOB_TYPE_TAG :
                            {
                                ...
                                argument =
                                    argument + 4 + RoundUp4( ( unsigned long )blobSize );
                                if( argument > end )
                                {
                                    MalformedMessageException(
                                        "arguments exceed message size"
    ...
    void
    ReceivedBundle::Init( const char*   bundle,
                          unsigned long size )
    {
        if( size < 16 )
        {
