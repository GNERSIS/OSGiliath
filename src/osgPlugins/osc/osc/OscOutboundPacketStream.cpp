 ...
 * Utility functions for osgPlugins: defined, FromInt32, FromUInt32, FromInt64,
 * FromUInt64.
 */
...
namespace osc
{
    ...
    OutboundPacketStream::OutboundPacketStream( char*         buffer,
    ...
    char*
    OutboundPacketStream::BeginElement( char* beginPtr )
    {
    ...
    void
    OutboundPacketStream::EndElement( char* endPtr )
    {
    ...
    bool
    OutboundPacketStream::ElementSizeSlotRequired() const
    {
        return ( elementSizePtr_ != 0 );
    }

    void
    OutboundPacketStream::CheckForAvailableBundleSpace()
    {
    ...
    void
    OutboundPacketStream::CheckForAvailableMessageSpace( const char* addressPattern )
    {
        // plus 4 for at least four bytes of type tag
        unsigned long required =
            Size() +
            ( ( ElementSizeSlotRequired() ) ? 4 : 0 ) +
            RoundUp4( static_cast<unsigned long>( strlen( addressPattern ) ) + 1 ) +
            4;
    ...
    void
    OutboundPacketStream::CheckForAvailableArgumentSpace( long argumentLength )
    {
        // plus three for extra type tag, comma and null terminator
        unsigned long required =
            static_cast<unsigned long>( ( argumentCurrent_ - data_ ) +
                                        argumentLength +
    ...
    unsigned int
    OutboundPacketStream::Capacity() const
    {
        return static_cast<unsigned int>( end_ - data_ );
    }

    unsigned int
    OutboundPacketStream::Size() const
    {
        ...
        if( IsMessageInProgress() )
        {
            ...
            result +=
                static_cast<unsigned int>( RoundUp4( ( end_ - typeTagsCurrent_ ) + 2 ) );
        }
    ...
    const char*
    OutboundPacketStream::Data() const
    {
        return data_;
    }

    bool
    OutboundPacketStream::IsReady() const
    {
        return ( !IsMessageInProgress() && !IsBundleInProgress() );
    }

    bool
    OutboundPacketStream::IsMessageInProgress() const
    {
        return messageIsInProgress_;
    }

    bool
    OutboundPacketStream::IsBundleInProgress() const
    {
        return ( elementSizePtr_ != 0 );
    }
    ...
    OutboundPacketStream::operator<<( const BeginMessage& rhs )
    {
        ...
        unsigned long rhsLength =
            static_cast<unsigned long>( strlen( rhs.addressPattern ) );
    ...
    OutboundPacketStream::operator<<( const MessageTerminator& rhs )
    {
        ...
        if( typeTagsCount )
        {
            ...
            uint32 argumentsSize =
                static_cast<uint32>( argumentCurrent_ - messageCursor_ );
    ...
    {
        CheckForAvailableArgumentSpace( static_cast<long>( RoundUp4( strlen( rhs ) +
                                                                     1 ) ) );
