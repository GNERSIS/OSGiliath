 ...
 * SocketReceiveMultiplexer class.
 * Provides: AttachSocketListener, DetachSocketListener, AttachPeriodicTimerListener,
 * AttachPeriodicTimerListener, DetachPeriodicTimerListener, Run.
 */
#pragma once
...
class SocketReceiveMultiplexer
{
    ...
    public:
        ...
        void
        Run();    // loop and block processing messages indefinitely
        void
        RunUntilSigInt();
        void
        Break();    // call this from a listener to exit once the listener returns
        void
        AsynchronousBreak();    // call this from another thread or signal handler to
                                // exit the Run() state
};
...
class UdpListeningReceiveSocket : public UdpSocket
{
    ...
    public:
        ...
        UdpListeningReceiveSocket( const IpEndpointName& localEndpoint,
                                   PacketListener*       listener ) :
            listener_( listener )
        {
            Bind( localEndpoint );
