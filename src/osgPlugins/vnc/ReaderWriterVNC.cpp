 ...
 * LibVncImage, derived from VncImage.
 * Provides: connect, close, sendPointerEvent, getTimeOfLastRender, time, sendKeyEvent.
 */
...
class LibVncImage : public osgWidget::VncImage
{
    public:
        ...
        static rfbBool
        resizeImage( rfbClient* client );

        static void
        updateImage( rfbClient* client,
                     int        x,
                     int        y,
                     int        w,
                     int        h );
        ...
        static void
        passwordCheck( rfbClient*  client,
                       const char* encryptedPassWord,
                       int         len );
        static char*
                                    getPassword( rfbClient* client );
    ...
    protected:
        ...
        class RfbThread : public osg::Referenced,
                          public osg::Thread
        {
            public:
                ...
                run()
                {
                    do
                    {
                        ...
                        int i = WaitForMessage( _client, 1'000'000 );
                        if( i )
                        {
                            if( !HandleRFBServerMessage( _client ) )
                            {
                                OSG_NOTICE
                                    << "HandleRFBServerMessage returned non zero value."
                                    << std::endl;
                            }
                            // _image->updated();
                        }
...
static rfbBool
rfbInitConnection( rfbClient* client )
{
    ...
    if( !client->listenSpecified )
    {
        if( !client->serverHost ||
            !ConnectToRFBServer( client, client->serverHost, client->serverPort ) )
        {
            return FALSE;
        }
    }
...
void
LibVncImage::passwordCheck( rfbClient* /*client*/,
                            const char* /*encryptedPassWord*/,
                            int /*len*/ )
{
...
char*
LibVncImage::getPassword( rfbClient* client )
{
...
bool
LibVncImage::connect( const std::string& hostname )
{
    if( hostname.empty() )
    {
        return false;
...
rfbBool
LibVncImage::resizeImage( rfbClient* client )
{
    LibVncImage* image  = ( LibVncImage* )( rfbClientGetClientData( client, 0 ) );
    ...
    OSG_NOTICE << "resize " << width << ", " << height << ", " << depth
    ...
    {
        if( image->_optionString.find( "swap" ) !=
            std::string::npos ||
            image->_optionString.find( "swop" ) != std::string::npos )
        {
            swap = true;
        }
    }
...
void
LibVncImage::updateImage( rfbClient* client,
                          int /*x*/,
                          int /*y*/,
                          int /*w*/,
                          int /*h*/ )
...
bool
LibVncImage::sendPointerEvent( int x,
                               int y,
                               int buttonMask )
{
    if( _client )
...
bool
LibVncImage::sendKeyEvent( int  key,
                           bool keyDown )
{
    if( _client )
    {
...
void
LibVncImage::setFrameLastRendered( const osg::FrameStamp* )
{
...
class ReaderWriterVNC : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterVNC()
        {
            ...
            supportsOption(
                "swap",
                "Swaps the pixel format order, exchanging the red and blue channels."
            );
                   ...
                   const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            image->setDataVariance( osg::Object::DataVariance::DYNAMIC );
            ...
            if( authenticationMap != NULL )
            {
                ...
                if( details == NULL )
                {
                    ...
                    if( pos != std::string::npos )
                    {
                        details = authenticationMap->getAuthenticationDetails(
                            hostname.substr( 0, pos )
                        );
                    }
                }
                ...
                if( details != NULL )
                {
                    OSG_NOTICE << "Passing in password = " << details->password
                               << std::endl;
                  ...
                  const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( vncClient->assign(
                    dynamic_cast<osgWidget::VncImage*>( result.getImage() )
                ) )
            {
