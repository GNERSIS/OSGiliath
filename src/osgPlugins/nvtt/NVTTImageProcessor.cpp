...
class NVTTProcessor : public osgDB::ImageProcessor
{
    ...
    protected:
        ...
        void
        convertRGBAToBGRA( std::vector<unsigned char>& outputData,
                           const osg::Image&           image );

        // Convert RGB to BGRA : nvtt only accepts BGRA pixel format
        void
        convertRGBToBGRA( std::vector<unsigned char>& outputData,
                          const osg::Image&           image );
};

/// Error handler.
void
NVTTProcessor::VPBErrorHandler::error( nvtt::Error e )
{
    switch( e )
    {
...
/// Output handler.
NVTTProcessor::OSGImageOutputHandler::OSGImageOutputHandler( nvtt::Format format,
                                                             bool discardAlpha ) :
    _format( format ),
...
// create the osg image from the given format
bool
NVTTProcessor::OSGImageOutputHandler::assignImage( osg::Image& image )
{
...
void
NVTTProcessor::OSGImageOutputHandler::beginImage( int size,
                                                  int width,
                                                  int height,
                                                  int /*depth*/,
                                                  int /*face*/,
...
/// Output data. Compressed data is output as soon as it's generated to minimize memory
/// allocations.
bool
NVTTProcessor::OSGImageOutputHandler::writeData( const void* data,
                                                 int         size )
{
    // Copy mipmap data
...
// Convert RGBA to BGRA : nvtt only accepts BGRA pixel format
void
NVTTProcessor::convertRGBAToBGRA( std::vector<unsigned char>& outputData,
                                  const osg::Image&           image )
{
    unsigned int n = 0;
...
// Convert RGB to BGRA : nvtt only accepts BGRA pixel format
void
NVTTProcessor::convertRGBToBGRA( std::vector<unsigned char>& outputData,
                                 const osg::Image&           image )
{
    unsigned int n = 0;
    for( int row = 0; row < image.t(); ++row )
...
void
NVTTProcessor::process( osg::Image&        image,
                        nvtt::Format       format,
                        bool               generateMipMap,
                        bool               resizeToPowerOfTwo,
                        CompressionMethod  method,
                        CompressionQuality quality )
{
    ...
    if( format == nvtt::Format_RGBA )
    {
        if( image.getPixelFormat() == GL_RGB )
        {
            compressionOptions.setPixelFormat( 24, 0XFF, 0XFF'00, 0XFF'00'00, 0 );
        }
        else
        {
    ...
    if( method == USE_GPU )
    {
        ...
        {
            OSG_WARN << "CUDA acceleration was enabled but it is not available. CPU "
                        "will be used."
                     << std::endl;
        }
    }
...
void
NVTTProcessor::compress( osg::Image&                      image,
                         osg::Texture::InternalFormatMode compressedFormat,
                         bool                             generateMipMap,
                         bool                             resizeToPowerOfTwo,
                         CompressionMethod                method,
...
void
NVTTProcessor::generateMipMap( osg::Image&       image,
                               bool              resizeToPowerOfTwo,
                               CompressionMethod method )
{
...
REGISTER_OSGIMAGEPROCESSOR( nvtt,
                            NVTTProcessor )
