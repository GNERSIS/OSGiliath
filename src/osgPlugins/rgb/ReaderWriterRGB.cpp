 ...
 * ReaderWriterRGB, derived from ReaderWriter.
...
struct refImageRec : public rawImageRec,
                     public osg::Referenced
{
};

static void
ConvertShort( unsigned short* array,
              long            length )
{
    unsigned long  b1, b2;
    unsigned char* ptr;
...
static osg::ref_ptr<refImageRec>
RawImageOpen( std::istream& fin )
{
    ...
    raw->rowStart                                            = 0;
    raw->rowSize                                             = 0;
    raw->bpc                                                 = ( raw->type & 0X00'FF );

    raw->tmp = new unsigned char[static_cast<size_pos>( raw->sizeX ) *
                                 256 *
                                 static_cast<size_pos>( raw->bpc )];
    if( raw->tmp == NULL )
    {
    ...
    if( raw->sizeZ >= 1 )
    {
        if( ( raw->tmpR = new unsigned char[static_cast<size_pos>( raw->sizeX ) *
                                            static_cast<size_pos>( raw->bpc )] ) ==
            NULL )
        {
            OSG_FATAL << "Out of memory!" << std::endl;
    ...
    if( raw->sizeZ >= 2 )
    {
        if( ( raw->tmpG = new unsigned char[static_cast<size_pos>( raw->sizeX ) *
                                            static_cast<size_pos>( raw->bpc )] ) ==
            NULL )
        {
            OSG_FATAL << "Out of memory!" << std::endl;
    ...
    if( raw->sizeZ >= 3 )
    {
        if( ( raw->tmpB = new unsigned char[static_cast<size_pos>( raw->sizeX ) *
                                            static_cast<size_pos>( raw->bpc )] ) ==
            NULL )
        {
            OSG_FATAL << "Out of memory!" << std::endl;
    ...
    if( raw->sizeZ >= 4 )
    {
        if( ( raw->tmpA = new unsigned char[static_cast<size_pos>( raw->sizeX ) *
                                            static_cast<size_pos>( raw->bpc )] ) ==
            NULL )
        {
            OSG_FATAL << "Out of memory!" << std::endl;
    ...
    if( ( raw->type & 0XFF'00 ) == 0X01'00 )
    {
        unsigned int ybyz =
            static_cast<size_pos>( raw->sizeY ) * static_cast<size_pos>( raw->sizeZ );
...
static void
RawImageGetRow( rawImageRec&   raw,
                unsigned char* buf,
                int            y,
                int            z )
{
    ...
    if( ( raw.type & 0XFF'00 ) == 0X01'00 )
    {
        size_pos pos    = raw.rowStart[static_cast<size_pos>( y ) +
                                       static_cast<size_pos>( z ) *
                                       static_cast<size_pos>( raw.sizeY )];

        size_pos amount = raw.rowSize[static_cast<size_pos>( y ) +
                                      static_cast<size_pos>( z ) *
                                      static_cast<size_pos>( raw.sizeY )];
        ...
        while( !done )
        {
            ...
            if( ( static_cast<size_pos>( raw.sizeX ) *
                  static_cast<size_pos>( raw.bpc ) ) <= ( oPtr - buf ) )
            {
                count = static_cast<size_pos>( raw.sizeX ) -
                        ( oPtr - buf ) /
                        static_cast<size_pos>( raw.bpc );
            }
    ...
    else
    {
        size_pos pos = static_cast<size_pos>( 512 ) +
                       ( static_cast<size_pos>( y ) *
                         static_cast<size_pos>( raw.sizeX ) *
                         static_cast<size_pos>( raw.bpc ) ) +
                       ( static_cast<size_pos>( z ) *
                         static_cast<size_pos>( raw.sizeX ) *
                         static_cast<size_pos>( raw.sizeY ) *
        ...
        size_pos amount =
            static_cast<size_pos>( raw.sizeX ) * static_cast<size_pos>( raw.bpc );
...
static void
RawImageGetData( rawImageRec&    raw,
                 unsigned char** data )
{
    unsigned char*  ptr;
    int             i, j;
    ...
    *data =
        new unsigned char[( raw.sizeX ) * ( raw.sizeY ) * ( raw.sizeZ ) * ( raw.bpc )];
...
class ReaderWriterRGB : public osgDB::ReaderWriter
{
    public:
        ...
        ReadResult
        readRGBStream( std::istream& fin ) const
        {
            ...
            unsigned int pixelFormat    = raw->sizeZ == 1 ? GL_RED
                                        : raw->sizeZ == 2 ? GL_RG
                                        : raw->sizeZ == 3 ? GL_RGB
                                        : raw->sizeZ == 4 ? GL_RGBA
                                                          : ( GLenum )-1;
            ...
            unsigned int dataType = raw->bpc == 1 ? GL_UNSIGNED_BYTE : GL_UNSIGNED_SHORT;
        ...
        WriteResult
        writeRGBStream( const osg::Image&  img,
                        std::ostream&      fout,
                        const std::string& name ) const
        {
            ...
            raw.type           = dataType == GL_UNSIGNED_BYTE               ? 1
                               ...
                               : dataType == GL_UNSIGNED_INT_2_10_10_10_REV ? 4
                                                                            : 4;
            ...
            raw.sizeX          = img.s();
            raw.sizeY          = img.t();
            raw.sizeZ          = pixelFormat == GL_COLOR_INDEX ? 1
            ...
            {    // bpc == 2
                ...
                for( i = 0; i < raw.sizeZ; ++i )
                {
                    const unsigned short* ptr =
                        reinterpret_cast<const unsigned short*>( img.data() );
                    ptr += i;
                    ...
                    const osgDB::ReaderWriter::Options* ) const
        {
            ...
            {
                OSG_NOTICE << "Warning: RGB plugin does not supporting writing non "
                              "contiguous imagery."
                           << std::endl;
                return WriteResult::ERROR_IN_WRITING_FILE;
            }
