 ...
 * readerBase, derived from Referenced.
 * Provides: readGroup, readerText, readerBinary, dxfReader, openFile, nextGroupCode.
 */
/*	File handling.
...
class readerBase : public osg::Referenced
{
    public:
        ...
        bool
        readGroup( std::ifstream& f,
                   codeValue&     cv );

    protected:

        virtual bool
        readGroupCode( std::ifstream& f,
                       int&           groupcode ) = 0;
        virtual bool
        readValue( std::ifstream& f,
                   std::string&   s ) = 0;
        virtual bool
        readValue( std::ifstream& f,
                   bool&          b ) = 0;
...
class readerText : public readerBase
{
    ...
    protected:
        ...
        bool
        success( bool        inSuccess,
                 std::string type );
        bool
        getTrimmedLine( std::ifstream& f );
