 ...
 * Utility functions for osgPlugins: acos.
 */
...
#include <algorithm>
#include <osg/core/io_utils.hpp>    // just for debugging
...
void
dxfVertex::assign( dxfFile*   dxf,
                   codeValue& cv )
{
    double d = cv._double;
    // 2005.12.13 pdr: learned today that negative indices mean something and were
    // possible
...
dxfCircle::drawScene( scene* sc )
{
    ...
    double             theta =
        5.0;    // we generate polyline from "spokes" at theta degrees at arc's center

    if( _useAccuracy )
    {
        // we generate points on a polyline where each point lies on the arc, thus the
        // maximum error occurs at the midpoint of each line segment where it lies
        // furthest inside the arc If we divide the segment in half and connect the
        // bisection point to the arc's center, we have two rightangled triangles with
        // one side=r-maxError, hypotenuse=r, and internal angle at center is half the
        // angle we will step with:
        double maxError =
            min( _maxError,
                 _radius );    // Avoid offending acos() in the edge case where allowable
                               // deviation is greater than radius.
        double newtheta = acos( ( _radius - maxError ) / _radius );
        newtheta        = osg::degrees( newtheta ) * 2.0;

        // Option to only use the new accuracy code when it would improve on the accuracy
        // of the old method
    ...
    theta = osg::radians( theta );

    // We create an anglestep<=theta so that the line's points are evenly distributed
    // around the circle
    ...
    {
        b       = a + dvec3( _radius * ( double )sin( angle1 ),
                             _radius * ( double )cos( angle1 ),
                             0 );
        angle1 += anglestep;
        vlist.push_back( b );
    }
    ...
    sc->addLineStrip(
        ...
        vlist,
        _lineWidth
    );    // Should really add LineLoop implementation and save a vertex
    sc->ocs_clear();
}

void
dxfArc::assign( dxfFile*   dxf,
                codeValue& cv )
{
    double d = cv._double;
    // unsigned short s = cv._short;
...
dxfArc::drawScene( scene* sc )
{
    ...
    // cout<<"arc theta="<<osg::degrees(theta)<<" end="<<end<<" start="<<start<<"
    // numsteps="<<numsteps<<" e-s/theta="<<((end-start)/theta)<<"
    ...
    numsteps =
        max( numsteps,
             2 );    // Whatever else, minimum representation of an arc is a straightline
...
void
dxfPolyline::assign( dxfFile*   dxf,
                     codeValue& cv )
{
    string s = cv._string;
    if( cv._groupCode == 0 )
    ...
    else if( _currentVertex )
    {
            ...
            ( cv._int & 128 /*i.e. vertex is actually a face*/ ) )
        {
            _indices.push_back(
                _currentVertex
            );    // Add the index only if _currentvertex is actually an index
        }
    }
    else
    {
        ...
        switch( cv._groupCode )
        {
            ...
            case 70 :
                _flag =
                    cv._int;    // 2005.12.13 pdr: group codes [70,78] now signed int.
                break;
            case 71 :
                ...
                // group specifies the number of faces. Although these counts are correct
                // for all meshes created with the PFACE command, applications are not
                // required to place correct values in these fields.)" Amusing isn't it ?
                _mcount =
                    cv._int;    // 2005.12.13 pdr: group codes [70,78] now signed int.
                break;
            case 72 :
...
dxfPolyline::drawScene( scene* sc )
{
    ...
    sc->ocs( matrx );
    std::vector<dvec3> vlist;
    std::vector<dvec3> qlist;
    dvec3              a, b, c, d;
    bool               invert_order = false;
    if( _flag & 16 )
    {
        std::vector<dvec3> nlist;
        dvec3              nr;
        bool               nset = false;
        // dxfVertex* v = NULL;
        unsigned int       ncount;
        ...
        /*
                ...
                for (; itr != nlist.end(); ++itr) {
                    ...
                    dvec3 diff = ((*itr)-lastn);
                    diff = osg::normalize(diff);
                    float dot = diff * nr;
                    if (dot > 0.0) {
                        bad_c += dot;
                        ++bad;
...
void
dxfLWPolyline::assign( dxfFile*   dxf,
                       codeValue& cv )
{
    string s = cv._string;
...
dxfInsert::drawScene( scene* sc )
{
    ...
    // sanity check (useful when no block remains after all unsupported entities have
    // been filtered out)
    if( !_block )
    {
        return;
    ...
    dmat4 back = sc->backMatrix();
    dmat4 m;
    m = osg::dmat4();
    sc->pushMatrix( m, true );
    dvec3 trans = _block->getPosition();
    sc->blockOffset( -trans );
    if( _rotation )
    {
        sc->pushMatrix( osg::rotate( osg::radians( _rotation ), 0.0, 0.0, 1.0 ) );
    }
    sc->pushMatrix( osg::scale( _scale.x, _scale.y, _scale.z ) );
    sc->pushMatrix( osg::translate( _point.x, _point.y, _point.z ) );
    getOCSMatrix( _ocs, m );
    sc->pushMatrix( m );
    sc->pushMatrix( back );
    ...
    sc->popMatrix();        // back
    sc->blockOffset( dvec3( 0, 0, 0 ) );
}

void
dxfText::assign( dxfFile*   dxf,
                 codeValue& cv )
{
    switch( cv._groupCode )
    {
...
dxfText::drawScene( scene* sc )
{
    ...
    quat qr( static_cast<float>( DegreesToRadians( _rotation ) ),
             osg::vec3( 0.0F, 0.0F, 1.0F ) );

    if( _flags & 2 )
    {
        qr = quat( static_cast<float>( PI ), osg::vec3( 0.0F, 1.0F, 0.0F ) ) * qr;
    }
    if( _flags & 4 )
    {
        qr = quat( static_cast<float>( PI ), osg::vec3( 1.0F, 0.0F, 0.0F ) ) * qr;
    }
