...
void
dxfEntities::assign( dxfFile*   dxf,
                     codeValue& cv )
{
    if( cv._groupCode == 0 )
    {
...
{
    for( EntityList::iterator itr = _entityList.begin(); itr != _entityList.end();
         ++itr )
    {
        ( *itr )->drawScene( sc );
    }
