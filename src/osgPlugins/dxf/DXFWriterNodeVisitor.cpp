...
/** writes all values of an array out to a stream, applies a matrix beforehand if
 * necessary */

// I think this is a bit over the top for just a simple vertex array - but if anyone
// knows different?
/*
class ValueVisitor : public osg::ValueVisitor {
    public:
        ValueVisitor(std::ostream& fout, const Layer &layer,const osg::dmat4& m =
osg::dmat4()) : osg::ValueVisitor(), _fout(fout), _layer(layer), _m(m)
        {
            //_applyMatrix = (_m != osg::dmat4());
        }
...
class DxfPrimitiveIndexWriter : public osg::PrimitiveIndexFunctor
{
    ...
    public:
        ...
        DxfPrimitiveIndexWriter( std::ostream&     fout,
                                 osg::Geometry*    geo,
                                 const Layer&      layer,
                                 AcadColor&        acad,
                                 const osg::dmat4& m                     = osg::dmat4(),
                                 bool              writeTriangleAs3DFace = true ) :
               ...
               int          c )
        {
            const osg::vec3 point =
                osg::vec3( ( ( osg::Vec3Array* )_geo->getVertexArray() )->at( i ) * _m );
            _fout << c +
                10 << "\n "
                  << point.x << "\n"
                  << 20 +
                       ...
                       unsigned int i3 )
        {
            if( _writeTriangleAs3DFace )
            {
                ...
                {
                    _fout << "62\n"
                          << _acad.findColor( DXFWriterNodeVisitor::getNodeRGB( _geo,
                                                                                i1 ) )
                          << "\n";
    ...
    private:
        ...
        DxfPrimitiveIndexWriter&
        operator=( const DxfPrimitiveIndexWriter& )
        {
            return *this;
        }
...
void
DxfPrimitiveIndexWriter::drawArrays( GLenum  mode,
                                     GLint   first,
                                     GLsizei count )
{
    switch( mode )
    {
        ...
        default :
            OSG_WARN << "DXFWriterNodeVisitor :: can't handle mode " << mode
                     << std::endl;
            break;
    }
}
...
std::string
DXFWriterNodeVisitor::getLayerName( const std::string& defaultvalue )
{
    ...
    for( std::vector<Layer>::iterator itr = _layers.begin(); itr != _layers.end();
         itr++ )
    {
...
// void DXFWriterNodeVisitor::processArray(osg::Array* array, const Layer &layer,const
// osg::dmat4& m)
//{
//    if (array == NULL)
//        return;
...
void
DXFWriterNodeVisitor::processStateSet( osg::StateSet* ss )
{
    // anything to do if no material/texture?

    osg::PolygonMode* pm = dynamic_cast<osg::PolygonMode*>(
        ss->getAttribute( osg::StateAttribute::Type::POLYGONMODE )
    );
    if( pm )
    {
        if( pm->getMode( osg::PolygonMode::Face::FRONT ) ==
    ...
    if( mat )
    {
        ...
        _layer._color          = _acadColor.findColor( osg::asABGR( color ) >> 8 );
    }
}

void
DXFWriterNodeVisitor::processGeometry( osg::Geometry* geo,
                                       osg::dmat4&    m )
{
    ...
    if( geo->getVertexArray() && geo->getVertexArray()->getNumElements() )
    {
        ...
        if( _firstPass )
        {
            ...
            _layer._name =
                getLayerName( geo->getName().empty() ? geo->getParent( 0 )->getName()
                                                     : geo->getName() );
            ...
            {
                _layer._color =
                    _acadColor.findColor( getNodeRGB( geo ) );    // per layer color
            }
            else if( osg::Array::BIND_OFF == colorBinding )
            {
                _layer._color =
                    0XFF;    // use white - or can we easily lookup in texture?
            }
            else
            {
        ...
        else
        {
            ...
            if( geo->getNumPrimitiveSets() )
            {
                for( unsigned int i = 0; i < geo->getNumPrimitiveSets(); ++i )
                {
                    ...
                    DxfPrimitiveIndexWriter pif( _fout,
                                                 geo,
                                                 _layer,
                                                 _acadColor,
                                                 m,
                                                 _writeTriangleAs3DFace );
                    ps->accept( pif );
                }
            }
            else
            {
                ...
                {
                    osg::Vec3Array* data =
                        static_cast<osg::Vec3Array*>( geo->getVertexArray() );
                    for( unsigned int ii = 0; ii < data->getNumElements(); ii++ )
                    {
                        osg::vec3 point = osg::vec3( data->at( ii ) * m );
                        _fout << "0 \nVERTEX\n 8\n" << _layer._name << "\n";
                        if( _layer._color )
                        {
                            _fout << "62\n" << _layer._color << "\n";
...
void
DXFWriterNodeVisitor::apply( osg::Geode& node )
{

    pushStateSet( node.getStateSet() );
    osg::dmat4   m     = osg::computeLocalToWorld( getNodePath() );
...
bool
DXFWriterNodeVisitor::writeHeader( const osg::sphere& bound )
{
    if( _layers.empty() )
    {
        return false;
         ...
         itr++ )
    {
        ...
        else
        {
            _fout << "0\nLAYER\n2\n"
                  << itr->_name
                  << "\n70\n0\n62\n255\n6\nContinuous\n";    // most apps won't read
                                                             // 24bit color without a
                                                             // color value in header
        }
    }
...
void
DXFWriterNodeVisitor::writeFooter()
{
    _fout << "0\nENDSEC\n0\nEOF";
    _fout << std::endl;
}
