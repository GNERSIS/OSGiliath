 ...
 * Utility functions for osgPlugins: trim, readValue, success.
 */
...
*/
bool
readerBase::readGroup( std::ifstream& f,
                       codeValue&     cv )
{
    cv.reset();
...
bool
readerText::success( bool   inSuccess,
                     string type )
{
    if( !inSuccess )
    {
...
bool
readerText::getTrimmedLine( std::ifstream& f )
{
    std::string line;
...
bool
readerText::readGroupCode( std::ifstream& f,
                           int&           groupcode )
{
    if( getTrimmedLine( f ) )
    {
...
bool
readerText::readValue( std::ifstream& f,
                       string&        s )
{
    if( getTrimmedLine( f ) )
    {
...
dxfReader::openFile( std::string fileName )
{
    ...
    _ifs.open( fileName.c_str(),
               ios::binary );    // found mac autocad with \r delimiters
    if( !_ifs )
    {
