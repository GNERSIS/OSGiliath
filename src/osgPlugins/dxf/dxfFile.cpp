 ...
 * Utility functions for osgPlugins: scene.
 */
...
dxfFile::dxf2osg()
{
    ...
    if( !_tables )
    {    // a dxfTable is needed to create undefined layers
...
dxfFile::assign( codeValue& cv )
{
    ...
    else if( _isNewSection )
    {
        ...
        std::cout << "No groupcode for changing section " << cv._groupCode
                  << " value: " << s << std::endl;
        return -1;
    }
