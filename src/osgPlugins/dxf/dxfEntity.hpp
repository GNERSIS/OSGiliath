 ...
 * dxfBasicEntity, derived from Referenced.
 * Provides: create, name, assign, drawScene, getLayer, setAccuracy.
 */
#pragma once
...
static inline void
getOCSMatrix( const osg::dvec3& ocs,
              osg::dmat4&       m )
{
    static const double one_64th = 1.0 / 64.0;
    m                            = osg::dmat4();
    if( ocs == osg::dvec3( 0, 0, 1 ) )
    {
        return;
    }
    osg::dvec3 ax( 1, 0, 0 ), ay( 0, 1, 0 ), az( 0, 0, 1 );
    osg::dvec3 ocsaxis( ocs );
    ocsaxis = osg::normalize( ocsaxis );
...
class dxfBasicEntity : public osg::Referenced
{
    ...
    protected:
        ...
        bool   _improveAccuracyOnly;    // if true only use _maxError where it would
                                        // increase the quality of curves compared to the
                                        // previous algorithm
...
class dxfArc : public dxfBasicEntity
{
    public:
        ...
        {    // we create a copy which uses our accuracy settings
            ...
            // std::cout<<"dxfArc::create with _useAccuracy="<<_useAccuracy<<"
            // maxError="<<_maxError<<"
            // improveAccuracyOnly="<<_improveAccuracyOnly<<std::endl;
            return arc;
        }
...
class dxf3DFace : public dxfBasicEntity
{
    public:
        ...
        dxf3DFace()
        {
            _vertices[0] = osg::dvec3( 0, 0, 0 );
            _vertices[1] = osg::dvec3( 0, 0, 0 );
            _vertices[2] = osg::dvec3( 0, 0, 0 );
            _vertices[3] = osg::dvec3( 0, 0, 0 );
        }
...
class dxfLWPolyline : public dxfBasicEntity
{
    ...
    protected:
        ...
        osg::dvec3              _ocs;       // 210 220 230
        osg::dvec3              _lastv;
        std::vector<osg::dvec3> _vertices;
};
...
class dxfText : public dxfBasicEntity
{
    ...
    protected:
        ...
        osg::dvec3  _point1;      // 10,20,30
        osg::dvec3  _point2;      // 11,21,31
        osg::dvec3  _ocs;         // 210,220,230
        double      _height;      // 40
        double      _xscale;      // 41
        double      _rotation;    // 50
...
class dxfEntity : public osg::Referenced
{
    public:
        ...
        static void
        unregisterEntity( dxfBasicEntity* );

        static dxfBasicEntity*
        findByName( std::string s )
        {
        ...
        // Returns the exemplar from the registry - all other entities of this type are
        // created by this one via entity->create
        static dxfBasicEntity*
        getRegistryEntity( std::string s )
        {
