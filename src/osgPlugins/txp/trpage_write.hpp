...
TX_EXDECL class TX_CLDECL trpgwGeomStats
{
    public:
        ...
        // Helper functions
        inline void
        AddStripStat( int val )
        {
            stripStat[MIN( 14, val )]++;
            totalStripTri += val;
            totalTri      += val;
            numStrip++;
        }
...
TX_EXDECL class TX_CLDECL trpgwGeomHelper
{
    public:
        ...
        trpgwGeomHelper( trpgWriteBuffer*,
                         int dataType = UseDouble );
        void
        init( trpgWriteBuffer*,
              int dataType = UseDouble );
        virtual void
        SetMode( int );    // Takes a trpgGeometry primitive type (triangle by default)
        virtual void
        Reset( void );
        ...
        EndPolygon( void );
        virtual void
        ResetPolygon( void );    // If you change your mind about the current poly
        // Set the current state
        // Note: Currently you *must* set all of these
        virtual void
        SetColor( trpgColor& );
        virtual void
        SetTexCoord( trpg2dPoint& );
        virtual void
        AddTexCoord( trpg2dPoint& );    // for multiple textures
        virtual void
                     SetNormal( trpg3dPoint& );
...
TX_EXDECL class TX_CLDECL trpgwImageHelper
{
    public:
        ...
        trpgwImageHelper( trpgEndian ness,
                          char*      dir,
                          trpgTexTable&,
        ...
        virtual trpgwAppFile*
        GetNewWAppFile( trpgEndian  inNess,
                        const char* fileName,
                        bool        reuse = false );

        // Write the given texture data into one our local archives
        bool
        WriteToArchive( const trpgTexture& tex,
                        char*              data,
                        trpgwAppAddress&   addr,
                        bool               geotyp = false );
...
TX_EXDECL class TX_CLDECL trpgwArchive : public trpgCheckable
{
    public:
        ...
        trpgwArchive( trpgEndian ness     = LittleEndian,
                      ...
                      int        minorVer = TRPG_VERSION_MINOR );
        virtual void
        Init( trpgEndian ness     = LittleEndian,
              TileMode   tileMode = TileLocal,
              int        majorVer = TRPG_VERSION_MAJOR,
              int        minorVer = TRPG_VERSION_MINOR );
        ...
        virtual void
        SetMaxTileFileLength( int len );

        // Set functions.  Have to fill all these out before writing
        virtual bool
        SetHeader( const trpgHeader& );
        virtual bool
        SetMaterialTable( const trpgMatTable& );
        virtual bool
        SetTextureTable( const trpgTexTable& );
        virtual bool
        SetModelTable( const trpgModelTable& );
        virtual bool
        SetLightTable( const trpgLightTable& );
        virtual bool
        SetRangeTable( const trpgRangeTable& );
        virtual bool
        SetLabelPropertyTable( const trpgLabelPropertyTable& );
        virtual bool
        SetSupportStyleTable( const trpgSupportStyleTable& );
        virtual bool
        SetTextStyleTable( const trpgTextStyleTable& );

        // Get functions.  If we're doing a regenerate we need to get at these
        virtual trpgHeader*
        GetHeader();
        virtual trpgMatTable*
        GetMatTable();
        virtual trpgTexTable*
        GetTextureTable();
        virtual trpgModelTable*
        GetModelTable();
        virtual trpgLightTable*
        GetLightTable();
        virtual trpgRangeTable*
        GetRangeTable();
        virtual trpgLabelPropertyTable*
        GetLabelPropertyTable();
        virtual trpgTextStyleTable*
        GetTextStyleTable();
        virtual trpgSupportStyleTable*
        GetSupportStyleTable();

        virtual bool
        IncrementTileFile( void );
        virtual bool
        DesignateTileFile( int );
        ...
        WriteHeader( void );
        virtual bool
        CheckpointHeader( void );
        virtual bool
        WriteTile( unsigned int,
                   unsigned int,
                   unsigned int,
                   float zmin,
                   float zmax,
        ...
        virtual trpgwImageHelper*
        GetNewWImageHelper( trpgEndian ness,
                            char*      dir,
                            trpgTexTable& );
        virtual trpgwAppFile*
        GetNewWAppFile( trpgEndian  inNess,
        ...
        virtual int32
        WriteHeaderData( const char* dataPtr,
                         int32       length,
                         FILE*       filehandle );
    ...
    protected:
        ...
        char                       dir[1'024];    // Directory where we're doing all this
