...
void
trpgRange::SetCategory( const char* cat,
                        const char* subCat )
{
    if( category )
    {
...
void
trpgRange::GetCategory( char* cat,
                        int   catLen,
                        char* subCat,
                        int   subCatLen ) const
{
...
void
trpgRange::SetLodInfo( double in,
                       double out )
{
    inLod  = in;
    outLod = out;
...
void
trpgRange::GetLodInfo( double& in,
                       double& out ) const
{
    in  = inLod;
    out = outLod;
...
void
trpgRange::SetPriority( int prior )
{
    priority = prior;
}

void
trpgRange::GetPriority( int& prior ) const
{
    prior = priority;
}

trpgRange&
trpgRange::operator=( const trpgRange& other )
{
    Reset();
    inLod  = other.inLod;
    outLod = other.outLod;
...
bool
trpgRange::operator==( const trpgRange& in ) const
{
    ...
    {
        if( ( category && !in.category ) || ( !category && in.category ) )
        {
            return false;
        }
    }
    ...
    {
        if( ( subCategory && !in.subCategory ) || ( subCategory && in.subCategory ) )
        {
            return false;
        }
    }
...
bool
trpgRange::Write( trpgWriteBuffer& buf )
{
...
bool
trpgRange::Read( trpgReadBuffer& buf )
{
    char catStr[1'024], subStr[1'024];
    ...
    try
    {
        ...
        buf.Get( outLod );
        buf.Get( priority );
        buf.Get( catStr, 1'024 );
        buf.Get( subStr, 1'024 );
...
bool
trpgRangeTable::GetRange( int        id,
                          trpgRange& ret ) const
{
    if( !isValid() )
    {
...
bool
trpgRangeTable::SetRange( int        id,
                          trpgRange& inRange )
{
    if( !isValid() )
    {
...
int
trpgRangeTable::AddRange( trpgRange& range )
{
...
int
trpgRangeTable::FindAddRange( trpgRange& range )
{
...
bool
trpgRangeTable::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
bool
trpgRangeTable::Read( trpgReadBuffer& buf )
{
    int32     numRange;
    trpgToken tok;
    int32     len;
...
trpgRangeTable&
trpgRangeTable::operator=( const trpgRangeTable& inTab )
{
    Reset();
