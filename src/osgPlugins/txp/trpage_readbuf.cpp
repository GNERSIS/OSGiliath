...
bool
trpgReadBuffer::Get( std::string& str )
{
    int32 len;
...
bool
trpgReadBuffer::Get( float32& ret )
{
    char cval[4];
...
bool
trpgReadBuffer::Get( float64& ret )
{
    char cval[8];
...
bool
trpgReadBuffer::Get( trpgDiskRef& ret )
{
    trpgDiskRef val;
...
bool
trpgReadBuffer::Get( trpgToken& ret )
{
    trpgToken val;
...
// Array Get functions
bool
trpgReadBuffer::GetArray( int       len,
                          float32** arr )
{
...
bool
trpgReadBuffer::Get( trpg2iPoint& pt )
{
...
bool
trpgReadBuffer::Get( trpg2dPoint& pt )
{
    if( !Get( pt.x ) || !Get( pt.y ) )
    {
        return false;
...
bool
trpgReadBuffer::Get( trpg3dPoint& pt )
{
...
bool
trpgReadBuffer::Get( trpgColor& color )
{
...
bool
trpgReadBuffer::GetToken( trpgToken& tok,
                          int32&     len )
{
    if( !Get( tok ) || !Get( len ) )
    {
...
void
trpgReadBuffer::PushLimit( int limit )
{
...
bool
trpgReadBuffer::TestLimit( int len )
{
...
void
trpgReadBuffer::UpdateLimits( int len )
{
    for( unsigned int i = 0; i < limits.size(); i++ )
    {
        limits[i] -= len;
...
void
trpgMemReadBuffer::SetLength( int newLen )
{
    if( newLen > totLen )
    {
        if( data )
...
bool
trpgMemReadBuffer::GetData( char* ret,
                            int   rlen )
{
    if( rlen < 0 )
    {
...
bool
trpgMemReadBuffer::GetDataRef( char** ret,
                               int    rlen )
{
    if( rlen < 0 )
    {
...
bool
trpgMemReadBuffer::Skip( int rlen )
{
    if( rlen == 0 )
    {
        return true;
...
trpgrAppFile::trpgrAppFile( trpgEndian  inNess,
                            const char* fileName )
{
...
void
trpgrAppFile::Init( trpgEndian  inNess,
                    const char* fileName )
{
    valid   = false;
    ness    = inNess;
...
bool
trpgrAppFile::isValid( void ) const
{
    return valid;
}
...
bool
trpgrAppFile::Read( trpgMemReadBuffer* buf,
                    int32              offset )
{
    if( !valid )
    {
...
trpgrAppFileCache::trpgrAppFileCache( const char* inPre,
                                      const char* inExt,
                                      int         noFiles )
{
...
void
trpgrAppFileCache::Init( const char* inPre,
                         const char* inExt,
                         int         noFiles )
{
...
trpgrAppFile*
trpgrAppFileCache::GetNewRAppFile( trpgEndian  ness,
                                   const char* fileName )
{
...
trpgrAppFile*
trpgrAppFileCache::GetFile( trpgEndian ness,
                            int        id )
{
                            ...
                            int        row )
{
    ...
    {
        if( ( files[i].id == id ) && ( files[i].col == col ) && ( files[i].row == row ) )
        {
            foundID = i;
            break;
        }
    ...
    else
    {
        ...
        sprintf( fileName,
                 "%s" PATHSEPERATOR "%d" PATHSEPERATOR "%d" PATHSEPERATOR "%s_%d.%s",
                 dir,
                 col,
                 row,
                 filebase,
