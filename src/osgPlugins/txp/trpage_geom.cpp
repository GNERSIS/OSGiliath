...
// Set functions
void
trpgGeometry::SetPrimType( PrimType type )
{
    primType = type;
}

void
trpgGeometry::SetPrimLengths( int        num,
                              const int* len )
{
    if( num < 0 )
    {
...
void
trpgGeometry::AddPrimLength( int len )
{
    if( len < 0 )
    {
        return;
...
void
trpgGeometry::AddPrim()
{
    numPrim++;
}

void
trpgGeometry::SetNumPrims( int num )
{
    numPrim = num;
}

void
trpgGeometry::SetNumMaterial( int no )
{
    if( no < 0 )
    {
        return;
...
void
trpgGeometry::SetMaterial( int  which,
                           int  mat,
                           bool isLocal )
{
...
void
trpgGeometry::SetMaterials( int32        num,
                            const int32* mat )
{
...
int
trpgGeometry::AddMaterial( int mat )
{
...
void
trpgGeometry::SetVertices( int            num,
                           const float32* data )
{
    if( num < 0 )
    {
...
void
trpgGeometry::AddVertex( DataType     type,
                         trpg3dPoint& pt )
{
    if( type == FloatData )
    {
...
void
trpgGeometry::SetNormals( int            num,
                          BindType       bind,
                          const float32* data )
{
    if( num < 0 )
...
void
trpgGeometry::AddNormal( DataType     type,
                         trpg3dPoint& pt )
{
    if( type == FloatData )
    {
...
void
trpgGeometry::SetColors( int              num,
                         ColorType        type,
                         BindType         bind,
                         const trpgColor* data )
{
    trpgColorInfo ci;
...
void
trpgGeometry::SetTexCoords( int            num,
                            BindType       bind,
                            const float32* data )
{
    if( num < 0 )
...
void
trpgGeometry::AddTexCoord( DataType     type,
                           trpg2dPoint& pt,
                           int          n )
{
...
void
trpgGeometry::AddTexCoords( BindType bind )
{
    trpgTexData td;
    td.bind = bind;
    texData.push_back( td );
...
void
trpgGeometry::SetEdgeFlags( int         num,
                            const char* flags )
{
    if( num < 0 )
    {
...
// Get methods
bool
trpgGeometry::GetPrimType( PrimType& t ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetNumPrims( int& n ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetPrimLengths( int* ret ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetNumMaterial( int& n ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetMaterial( int    id,
                           int32& m,
                           bool&  isLocal ) const
{
    isLocal = false;
                         ...
                         trpg3dPoint& pt ) const
{
    ...
    if( id <
        0 ||
        ( idMax >=
          ( int )vertDataFloat.size() &&
          idMax >= ( int )vertDataDouble.size() ) )
    {
        return false;
    }
...
bool
trpgGeometry::GetNumNormal( int32& n ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetNumColorSets( int& n ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetColorSet( int            id,
                           trpgColorInfo* ci ) const
{
...
bool
trpgGeometry::GetNumTexCoordSets( int& n ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetTexCoordSet( int          id,
                              trpgTexData* tx ) const
{
...
const trpgTexData*
trpgGeometry::GetTexCoordSet( int id ) const
{
    if( !isValid() || id < 0 || id >= ( int )texData.size() )
    {
        return 0;
...
bool
trpgGeometry::GetNumEdgeFlag( int& n ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::GetEdgeFlags( char* e ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgGeometry::Write( trpgWriteBuffer& buf )
{
    unsigned int i, j;
        ...
        switch( tok )
        {
            ...
            case TRPG_GEOM_COLOR :
                ...
                if( ALIGNMENT_WORKAROUND )
                {
                    ...
                    geom->SetColors( num,
                                     ( trpgGeometry::ColorType )type,
                                     ( trpgGeometry::BindType )bind,
                                     aligned );
                    free( aligned );
                }
                else
...
// Read Geometry
bool
trpgGeometry::Read( trpgReadBuffer& buf )
{
    trpgr_Parser parse;
    geomCB       gcb;
