...
TXPNode::TXPNode( const TXPNode&     txpNode,
...
TXPNode::~TXPNode()
{
    if( _archive.get() )
    {
        ...
        {
            if( ReaderWriterTXP* rwTXP = dynamic_cast<ReaderWriterTXP*>( rw ) )
            {
...
void
TXPNode::traverse( osg::NodeVisitor& nv )
{
...
#ifdef PRINT_TILEMAPP_TIMEINFO
                    std::cout << "Completed visible tile search in "
                              << timer.delta_m( start, timer.tick() ) << std::endl;
#endif    // PRINT_TILEMAPP_TIMEINFO
                }
...
osg::sphere
TXPNode::computeBound() const
{
#if 1

    // Steve Lunsford 10/28/05 : - Had to avoid using the child group nodes for bounds
    // calculation because apparently the loader doesn't load all the sub-tiles for this
    // node, resulting in a Group::computeBounds call which returns a size smaller than
    // the actual size represented by this node so consequently this node gets culled
    // out. note, submission merged and rearranged by Robert Osfield as an #if #else just
...
void
TXPNode::setArchiveName( const std::string& archiveName )
{
    _archiveName = archiveName;
}

void
TXPNode::setOptions( const std::string& options )
{
    _options = options;
}

const std::string&
TXPNode::getOptions() const
{
    return _options;
}

const std::string&
TXPNode::getArchiveName() const
{
    return _archiveName;
}

bool
TXPNode::loadArchive( TXPArchive* archive )
{
    // if (_archive.get())
    //{
    ...
    if( archive == NULL )
    {
        ...
        {
            TXPNodeERROR( "loadArchive()" )
                << "failed to load archive: \"" << _archiveName << "\"" << std::endl;
            return false;
        }
    }
    else
...
void
TXPNode::updateEye( osg::NodeVisitor& nv )
{
    if( !_pageManager )
    {
    ...
    if( _pageManager->SetLocation( loc ) )
    {
        ...
        while( ( tile = _pageManager->GetNextUnload() ) )
        {
            ...
            if( lod == 0 )
            {
                ...
                // OSG_NOTICE << "Tile unload: " << x << " " << y << " " << lod <<
                // std::endl;
            }
                          ...
                          int y )
{
    ...
    // lod = 0. So all is OK
    int  lod = 0;
    char pagedLODfile[1'070];
    sprintf( pagedLODfile,
             "%s\\tile%d_%dx%d_%d.txp",
             _archive->getDir(),
             lod,
             x,
             y,
    ...
    {
        // add in MatrixTransform node with dmat4 offsets
