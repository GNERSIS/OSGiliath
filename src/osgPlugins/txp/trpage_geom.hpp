...
TX_EXDECL class TX_CLDECL trpgHeader : public trpgReadWriteable
{
    public:
        ...
        */
        ...
        void
        SetOrigin( const trpg3dPoint& );
        // 2D archive extents.  Must be set.
        void
        SetExtents( const trpg2dPoint& sw,
        ...
        */
        ...
        SetLodRange( int,
                     float64 );
        void
        SetLodRange( const float64* );
        // Increase the number of terrain LODs, adding a new one with the given size and
        // range
        void
        AddLod( const trpg2iPoint& size,
                const trpg2dPoint& ext,
                float64            range );

        // Set details for an LOD, resizing if necessary
        void
        SetLod( const trpg2iPoint& size,
                const trpg2dPoint& ext,
                float64            range,
                unsigned int       lod );
        ...
        */
        ...
        /* This is the extents, in X/Y of a
           single tile.  All tiles within an LOD should be the same size (although this
           is not enforced).  It's also assumed that a given tile lives entirely within
        ...
        */
        bool
        GetExtents( trpg2dPoint& sw,
                    trpg2dPoint& ne ) const;
        /* The tile origin type tells you the coordinate system of each database
           ...
           * TileLocal - Each tile has its own coordinate system starting from the lower
           left corner.  We do this to get around floating point accuracy problems
           (although we can do Double coordinates if necessary, as well).  To determine
           the real world coordinate value do: tileID*tileSize + coord.
        */
        bool
        GetTileOriginType( trpgTileType& ) const;
        /* Group IDs are used by TerraPage to hook level of detail structures together.
           ...
           separately.  To hook them together we use trpgAttach nodes and number each
           group & LOD node.  This returns the maximum group ID in the file, which is
           important to know if you're keeping an array of them. */
        bool
        GetMaxGroupID( int& ) const;

        /* A TerraPage archive can contain any number of terrain LODs (a typical number
           is 4). Each of these terrain LODs is accessed separately (as are the tiles
           within them). This returns the number of terrain LODs in the file.  It will be
           at least 1. See trpgAttach for instructions on how to hook the terrain LODs
           together.

           For version 2.1 and over, this number represent the depest lod that was found
           in the gaming area. With variable lod, tiles will not ncessarily fill out all
           of the gaming area for all of the lods. trpgAttach node are no longer used,
           instead see trpgChildRef
        */
        bool
        GetNumLods( int32& ) const;
        ...
        */
        ...
        bool
        Write( trpgWriteBuffer& );
        // Reads this class from a read buffer
        bool
        Read( trpgReadBuffer& );
        // Prints this class to a print buffer
        bool
        Print( trpgPrintBuffer& ) const;
        // {secret}
        bool
        ReadLodInfo( trpgReadBuffer& );

        void
        SetMaster( bool isMaster )
        {
            if( ( verMajor >= TRPG_NOMERGE_VERSION_MAJOR ) &&
                ( verMinor >= TRPG_NOMERGE_VERSION_MINOR ) )
            {
                if( isMaster )
                {
...
TX_EXDECL class TX_CLDECL trpgTextureEnv : public trpgReadWriteable
{
    ...
    public:
        ...
        void
        SetBorderColor( const trpgColor& );
        ...
        */
        bool
        GetMinFilter( int32& ) const;
        // Get the magnification filter
        bool
        GetMagFilter( int32& ) const;
        ...
        */
        ...
        bool
        GetBorderColor( trpgColor& ) const;
...
TX_EXDECL class TX_CLDECL trpgMaterial : public trpgReadWriteable
{
    ...
    public:
        ...
        void
        SetColor( const trpgColor& );
        // Ambient color
        void
        SetAmbient( const trpgColor& );
        // Diffuse color (the most commonly used)
        void
        SetDiffuse( const trpgColor& );
        // Specular color used in lighting
        void
        SetSpecular( const trpgColor& );
        // Emissive color used in lighting
        void
             SetEmission( const trpgColor& );
        ...
        */
        ...
        void
        SetNumTiles( int );
        // Adds a count to the number of tiles this material is used in and returns that
        // number
        int
        AddTile( void );
        ...
        void
        SetAttr( int attrCode,
                 int val );

        // Return the current color
        bool
        GetColor( trpgColor& ) const;
        // Returns the ambient color
        bool
        GetAmbient( trpgColor& ) const;
        // Returns the diffuse color (the most commonly used color)
        bool
        GetDiffuse( trpgColor& ) const;
        // Specular color used for lighting
        bool
        GetSpecular( trpgColor& ) const;
        // Emissive color used for lighting
        bool
        GetEmission( trpgColor& ) const;
        // Shininess used for lighting
        bool
        GetShininess( float64& ) const;

        // The shading model can be either Smooth or Flat
        bool
        GetShadeModel( int& ) const;
        // Point size
        bool
        GetPointSize( float64& ) const;
        // Line width
        bool
        GetLineWidth( float64& ) const;
        /* Cull mode determines whether geometry will be rejected if it's Front facing,
           Back facing, or neither (FrontAndBack)
        */
        bool
        GetCullMode( int& ) const;
        ...
        */
        bool
        GetAlphaFunc( int& ) const;
        /* The Alpha Ref is a value used in some of the Alpha Functions */
        bool
        GetAlphaRef( float64& ) const;
        // Whether or not to generate normals from geometry
        bool
        GetAutoNormal( bool& ) const;
        // A single Alpha value that applies to any polygons using this material
        bool
        GetAlpha( float64& ) const;
        /* One of the useful things about TerraPage is that it contains enough
           information to page textures & materials as well as terrain.  This is part of
           that. It returns the number of tiles this material is used in.  The
           trpgTexture has its own which is used for paging textures.  You only want to
           pay attention to this if you have some costly material definition in your
           hardware and so have to swap them in and out.
        */
        bool
        GetNumTile( int& ) const;

        /* There can be multiple textures per material.  This returns the number.
           The first is the base texture, so just use that if you can only do 1 texture
           per poly.
        */
        bool
        GetNumTexture( int& ) const;
        /* TerraPage supports multiple textures per polygon.  Some hardware can do this,
           ...
           This method returns the texture definition for the Nth texture used in this
           material. That consists of a texture ID which points into a trpgTexTable and a
           trpgTextureEnv which contains the texture application information. Multiple
           materials can also appear per trpgGeometry, so be aware that there are two
           ways to have multiple textures per polygon.
        */
        bool
        GetTexture( int  no,
                    int& id,
                    trpgTextureEnv& ) const;

        // Return whether or not this material is a bump map
        bool
        GetIsBumpMap( bool& ) const;

        // Get an attribute (e.g. fid/smc)
        bool
        GetAttr( int  attrCode,
                 int& val ) const;
...
TX_EXDECL class TX_CLDECL trpgLightAttr : public trpgReadWriteable
{
    public:
        ...
        enum LightFlags
        {
            ...
            trpg_Dusk        = 0X00'02,
            trpg_Night       = 0X00'04,
            trpg_Directional = 0X00'08,
            trpg_BackColor   = 0X00'10,
            trpg_Reflective  = 0X00'20,

            // Animation flags
            trpg_Flashing      = 0X01'00,
            trpg_Rotating      = 0X02'00,
            trpg_ClockWise     = 0X04'00,
            trpg_AnimationMask = 0X0F'00,

            // Performer light flags
            trpg_Perspective   = 0X10'00,
            trpg_Fade          = 0X20'00,
            trpg_ZBuffer       = 0X40'00,
            trpg_FogPunch      = 0X80'00,
            trpg_PerformerMask = 0XF0'00,
        };
        ...
        struct PerformerAttr
        {
                PerformerAttr() :
                    ...
                    maxPixelSize( 0 ),
                    actualSize( 0 ),
                    transparentPixelSize( 0 ),
                    transparentFallofExp( 0 ),
                    transparentScale( 0 ),
                    transparentClamp( 0 ),
                    fogScale( 0 ) {};
                int32   flags;
        ...
        void
             SetCalligraphicAttr( trpgLightAttr::CalligraphicAttr& );
        ...
        void
             SetPerformerAttr( trpgLightAttr::PerformerAttr& );
        ...
        void
             SetAnimationAttr( trpgLightAttr::AnimationAttr& );
        ...
        void
        SetComment( const char* );

        // Getters
        // See the comments for the Setters
        void
        GetType( trpgLightAttr::LightType& );
        void
        GetDirectionality( trpgLightAttr::LightDirectionality& );
        void
        GetFrontColor( trpgColor& );
        void
        GetFrontIntensity( float64& );
        void
        GetBackColor( trpgColor& );
        void
        GetBackIntensity( float64& );
        void
        GetNormal( trpg3dPoint& );
        void
        GetSMC( int32& );
        void
        GetFID( int32& );
        ...
        GetFlags( int32& );
        void
        GetHLobeAngle( float64& );
        void
        GetVLobeAngle( float64& );
        void
        GetLobeRollAngle( float64& );
        void
        GetLobeFalloff( float64& );
        void
        GetAmbient( float64& );
        void
        GetQuality( trpgLightAttr::LightQuality& );
        void
        GetRascalSignificance( float64& );
        void
        GetRandomIntensity( trpgLightAttr::LightQuality& );
        void
        GetCalligraphicAttr( trpgLightAttr::CalligraphicAttr& );
        void
        GetCalligraphicDrawOrder( int32& );
        void
        GetCalligraphicMinDefocus( float64& );
        void
        GetCalligraphicMaxDefocus( float64& );
        void
        GetPerformerAttr( trpgLightAttr::PerformerAttr& );
        void
        GetPerformerFlags( int32& );
        void
        GetPerformerMinPixelSize( float64& );
        void
        GetPerformerMaxPixelSize( float64& );
        void
        GetPerformerActualSize( float64& );
        void
        GetPerformerTpPixelSize( float64& );
        void
        GetPerformerTpFalloffExp( float64& );
        void
        GetPerformerTpScale( float64& );
        void
        GetPerformerTpClamp( float64& );
        void
        GetPerformerFogScale( float64& );
        void
        GetAnimationAttr( trpgLightAttr::AnimationAttr& );
        void
        GetAnimationPeriod( float64& );
        void
        GetAnimationPhaseDelay( float64& );
        void
        GetAnimationTimeOn( float64& );
        void
        GetAnimationVector( trpg3dPoint& );
        void
        GetAnimationFlags( int32& );
        const char*
        GetComment();

        // operators
        trpgLightAttr&
        operator=( const trpgLightAttr& );
        bool
        operator==( const trpgLightAttr& );
        bool
        operator!=( const trpgLightAttr& );
...
TX_EXDECL class TX_CLDECL trpgLight : public trpgReadWriteable
{
    public:
        ...
        bool
        GetVertices( trpg3dPoint* ) const;
        bool
        GetVertices( float64* ) const;
        bool
        GetVertices( float32* ) const;

        // Returns the index of the Light Attributes in the Light Table
        void
        GetAttrIndex( int& ) const;
...
TX_EXDECL class TX_CLDECL trpgLightTable : public trpgReadWriteable
{
    public:
        ...
        // Adds the given light attrib to the table and increments the total light attrib
        // count.
        int
        AddLightAttr( const trpgLightAttr& );

        /* This is the same as AddLightAttr except that it searches for a matching light
           attrib first.  This is convenient for writers who aren't keeping track of
           their own light attrib internally.
        */
        int
        FindAddLightAttr( const trpgLightAttr& );

        // Returns the number of light attrib in this table
        bool
        GetNumLightAttrs( int& ) const;

        // Returns a pointer to a light attrib from a given index
        const trpgLightAttr*
        GetLightAttrRef( int ) const;
...
TX_EXDECL class TX_CLDECL trpgRange : public trpgReadWriteable
{
    public:
        ...
        */
        void
        GetCategory( char* cat,
                     int   maxCatLen,
                     char* subCat,
                     int   maxSubCatLen ) const;

        // Set the lod info
        void
        SetLodInfo( double in,
                    double out );
        ...
        */
        ...
        // Equality operator
        bool
        operator==( const trpgRange& ) const;

        // Assignment operator
        trpgRange&
        operator=( const trpgRange& );
...
/* All materials are centrally indexed in TerraPage.  There is one material
   ...
   representation.  The next table (1) might the Infra-Red version.  It's up to the
   run-time system to switch between these two.  TerraPage simply provides the means for
   keeping track of it.

   If you're doing a TerraPage reader you'll get a trpgMatTable from the trpgr_Archive.
   This is your central index for materials.  If you can handle the multiple
   channels/sub-tables then you can access those as you need.  If you can't, just use 0
   for the sub-table index where appropriate.
   ...
   for a given material are copied when you call the add function.  So you can have a
   single trpgMaterial, modify just a few fields and call AddMaterial repeatedly.
   {group:Read/Write Classes}
*/
TX_EXDECL class TX_CLDECL trpgMatTable : public trpgReadWriteable
{
    public:
        ...
        */
        int
        AddMaterial( const trpgMaterial&,
        ...
        /* This function should be used when adding materials to multiple sub-tables.
           ...
           find a match.  The new (or old) ID is returned.
        */
        // int    AddMaterialInSubtable(const trpgMaterial &,int table,bool
    ...
    public:
        ...
        */
        typedef enum
        {
            ...
            trpg_INTA8,
            trpg_FXT1,
            trpg_Filler,    // This is not a texture format.  It's here to keep the
            ...
            trpg_DXT3,
            trpg_DXT5,
            // This is a bit ugly, but we can't change the size of the texture record
            // without breaking existing readers.  So there will be multiple MCM types to
            // indicate the number of bands and the nature of the extra channels, if any
            trpg_MCM5,
            trpg_MCM6R,
            trpg_MCM6A,
            trpg_MCM7RA,
        ...
        */
        ...
        void
        SetImageSize( const trpg2iPoint& );

        // Set the image location (For Local images only)
        void
        SetImageAddr( const trpgwAppAddress& );
        ...
        */
        ...
        trpgTexture&
        operator=( const trpgTexture& );
        int
        operator==( const trpgTexture& ) const;
...
/* The texture table keeps track of all the textures in a TerraPage archive.
   ...
   Textures don't live in isolation and must be applied to geometry through a
   trpgMaterial. After the trpgTexTable is built it will get passed to a trpgwArchive for
   writing.  That can be done right before you close the archive. {group:Read/Write
   Classes}
*/
TX_EXDECL class TX_CLDECL trpgTexTable : public trpgReadWriteable
{
    public:
        ...
        */
        int
        FindAddTexture( const trpgTexture& );
        ...
        */
        ...
        bool
        GetNumTextures( int& ) const;
        // This returns the trpgTexture corresponding to the given ID (from a
        // trpgMaterial)
        bool
        GetTexture( int texID,
                    trpgTexture& ) const;
        ...
        */
        ...
        trpgTexTable&
        operator=( const trpgTexTable& );
        const trpgTexture*
        FindByName( const char* name,
                    int&        texid ) const;

        void
        SetCurrentBlock( int row,
                         int col )
        {
            currentRow = row;
            currentCol = col;
...
/* Models are basically just references in TerraPage.  This class just points
   ...
   to by trpgModelRef objects.  Those model references are the only things that
   explicitly use trpgModel objects.
...
TX_EXDECL class TX_CLDECL trpgModel : public trpgReadWriteable
{
    public:
        ...
        void
             SetName( const char* );
        // Sets the on-disk reference to an internal model and sets this model to
        // Internal type. 5.28.04 - It will not set the model to be as of internal type.
        // Use the method from below to set the type of the model
        ...
        bool
        GetReference( trpgDiskRef& ) const;
        ...
        */
        ...
        trpgModel&
        operator=( const trpgModel& );
        int
        operator==( const trpgModel& ) const;
...
TX_EXDECL class TX_CLDECL trpgModelTable : public trpgReadWriteable
{
    public:
        ...
        */
        ...
        int
        FindAddModel( trpgModel& );
        /* Sets the model definition corresponding to the given ID.  Use this in
           conjunction with SetNumModels. */
        void
        SetModel( int,
                  const trpgModel& );
...
TX_EXDECL class TX_CLDECL trpgTileTable : public trpgReadWriteable
{
    public:
        ...
        // Tiles can be stored individually (External and ExternalSaved) or in grouped
        // files (Local). ExternalSaved still have an entry in the table. In this case
        // the address data is not valid.
         ...
         * 1. When building a block specific tile archive (MMB and non-MMB). (local is
         * TRUE)
    ...
    protected:
        ...
        int                  currentRow;
        int                  currentCol;
        bool localBlock;    // if true, this tile table should only contain entries for
                            // one block.
};
...
TX_EXDECL class TX_CLDECL trpgLocalMaterial : public trpgReadWriteable
{
    public:
        ...
        void
        SetBaseMaterial( int32 matSubTable,
                         int32 matID );
        ...
        /* Get the base material for this local material.  Base materials
           ...
           */
        bool
        GetBaseMaterial( int32& matSubTable,
                         int32& matID ) const;
        ...
        void
        SetSubImageInfo( const SubImageInfo& );
        ...
        */
        bool
        GetSubImageInfo( SubImageInfo& ) const;

        // Set the address info (only for writers)
        void
        SetAddr( const trpgwAppAddress& );
...
TX_EXDECL class TX_CLDECL trpgTileHeader : public trpgReadWriteable
{
    public:
        ...
        */
        void
             AddLocalMaterial( trpgLocalMaterial& );
        // {secret}
        void SetDate( int32 );
        ...
        */
        ...
        bool
        GetNumLocalMaterial( int32& ) const;
...
TX_EXDECL class TX_CLDECL trpgTexData
{
    public:
        ...
        */
        ...
        /* Initialize the texture coordinate data set with floating point or double
           values. num should correspond to the correct bind. */
        void
        set( int num,
             int bind,
             const float32* );
    ...
    public:
        ...
        */
        ...
        /* This just increments the number of primitives in the structure.  Use this
           if you're adding geometry one vertex at a time for a Triangle array, for
           example.
        */
        void
        AddPrim( void );
        ...
        */
        void
        SetNumPrims( int );    // Only when there are no primitive lengths
        ...
        */
        void
        SetMaterial( int  which,
                     int  matID,
                     bool isLocal = false );
        /* This is the same as repeated SetMaterial() calls.
         */
        void
        SetMaterials( int32        numMat,
                      const int32* matIDs );
        ...
        */
        bool
        GetPrimLengths( int* lenArray ) const;
        ...
        */
        bool
        GetNormals( float32* ) const;
        bool
        GetNormals( float64* ) const;
        ...
        */
        bool
        GetTexCoordSet( int id,
                        trpgTexData* ) const;
        /* This method returns this trpgGeometry's texture coordinate set specified by
           the given index.  GetNumTexCoordSets determines the total number of texture
           coordinate sets.
        */
        const trpgTexData*
        GetTexCoordSet( int id ) const;
        /* Returns the number of edge flags in this geometry node.
           Edge flags are used on certain primitive types, but are rather rare.
        */
        bool
        GetNumEdgeFlag( int& num ) const;
        /* This returns the edge flags for the current set of primitives.
         */
        bool
        GetEdgeFlags( char* ) const;
...
/* Structurally, an attach is just like a trpgGroup.  It does everything a group
   ...
   tile.  That trpgAttach can point to any group within the scene graph (with a group
   ID). Level of detail for the terrain is controlled by trpgLod nodes as if everything
   was loaded in all the time.  That is, you'll see the same thing no matter whether
   every node is loaded into memory or just the nearby ones.  The theoretical scene graph
   structure is the same no matter what.  It's the ranges in your trpgHeader that tell
   you when things ought to be loaded in, but the actual display logic is contained
   within the trpgLod objects.  It sounds complicated and it is... for the writer.  But
   for the reader it's really simple.

   If you're doing a TerraPage reader all you'll need to do is keep a mapping from group
   ID's to your own internal scene graph node representation.  Then when a trpgAttach
   shows up you can turn it into your own group-like thing and stuff it and its children
   into the scene graph.  When it wanders out of range (the trpgHeader tells you that for
   a given terrain LOD) you simply delete it.  If you out-run your paging you've got two
   options: (1) Display holes.  That's what will happen when the LOD above a given tile
   trpgAttach turns on without the children in memory; or (2) Don't switch LODs that
   don't have all their children loaded in yet.  Be aware that a trpgAttach inherits from
   trpgGroup and so can have trpgAttach children.  So don't absorb the trpgAttach even
   though it's extra hierarchy.  Also, don't make any assumptions that there is really
   terrain in a given tile.  The writer may have chosen to page buildings or trees.  You
   never know and there's no reason to assume.
   ...
   do is give all your groups (or whatever will become groups) unique IDs, keeping in
   mind to update the trpgHeader max group ID as you go.  Start at the lowest terrain
   lod.  This one doesn't need to have a trpgAttach node since it's at the top.  Traverse
   toward the higher lods.  When you hit one, spit out a trpgAttach, giving it the group
   ID of the trpgGroup directly above it.  Then treat the node you just created as a
   trpgGroup (i.e. do its children as normal).  You will also need to keep the
   trpgTileHeader for each tile around.  It's best to index these by (x,y,lod) index.
   You'll need to build that tile header up *just for this tile geometry*.  That means
   you have to stop adding material/model references when you start defining the next
   tile.  Depending on how you write out your scene graph it may make sense to keep a
   stack of trpgTileHeader and trpgMemWriteBuffer objects around indexed by tile
   (x,y,lod).
...
TX_EXDECL class TX_CLDECL trpgChildRef : public trpgReadWriteable
{
    public:
        ...
        void
        SetTileAddress( const trpgwAppAddress& gAddr );
        void
        SetTileAddress( int32 file,
                        int32 offset );
        void
        SetTileZValue( float gZmin,
                       float gZmax );
        bool
        GetTileAddress( int32& file,
                        int32& offset ) const;
        bool
        GetTileAddress( trpgwAppAddress& gAddr ) const;
        bool
        GetTileZValue( float& gZmin,
                       float& gZmax ) const;
...
/* The billboard inherits from the standard trpgGroup.  It tells the reader that
   ...
   If you're doing a TerraPage reader treat everything underneath this group as
   rotatable. Pay attention to the Type in particular.  There's a shorthand for rotating
   a bunch of objects that is a little confusing.

   If you're doing a TerraPage write this is pretty simple.  For the standard tree
   example use one of these with one or more trpgGeometry children. {group:Read/Write
   Classes}
*/
TX_EXDECL class TX_CLDECL trpgBillboard : public trpgGroup
{
    public:
        ...
        /* The type controls how the billboard node relates to its children.  There
           ...
           this node rotates around the center in the way specified by GetMode.  (2)
           Individual - This is a little weirder.  Basically, it's here for trees.  It's
           assumed that there will be one or more trpgGeometry nodes below this node.
           Each single primitive is supposed to rotate "separately".  That is, you must
           take into account the unique center of each one and rotate it around that.  If
           you have some optimization scheme where you can deal with groups of billboards
           (ala Performer) it is valid to do so in the Individual case. */
        bool
        GetType( int& ) const;
        /* Center of the thing to be rotated.  For Group this does the obvious thing.
           For Individual it should be the center of the group of things you want to
           rotate. This has no use if you're going to rotate each primitive separately,
           but if you've got some sort of optimized scheme for doing so (ala Performer)
           this information is useful.
        */
        bool
        GetCenter( trpg3dPoint& ) const;
        /* The mode will be one of: (1) Axial - rotate around the Axis.  This is the
           normal one for tree. (2) Eye - Always rotate toward the eye point.  (3) world.
        */
        bool
        GetMode( int& ) const;
        /* The axis used when GetMode returns Axial. */
        bool
        GetAxis( trpg3dPoint& ) const;
...
/* TerraPage level of detail nodes are pretty simple.  Even though they don't inherit
   from trpgGroup, they have many of the same calls and act, structurally at least, in
   much the same way.  These act as a switch.  When the user's eye point is within a
   distance then the children of this node should be turned on for display.  Otherwise,
   the children will be invisible.
...
TX_EXDECL class TX_CLDECL trpgLod : public trpgReadWriteable
{
    public:
        ...
        // Set the range Index
        void
        SetRangeIndex( int ri );
        // Get the range index
        bool
        GetRangeIndex( int& ri ) const;
...
TX_EXDECL class TX_CLDECL trpgTransform : public trpgGroup
{
    public:
        ...
        // Set the 4x4 matrix
        void
        SetMatrix( const float64* );

        // Get the 4x4 matrix
        bool
        GetMatrix( float64* ) const;
...
TX_EXDECL class TX_CLDECL trpgTextStyle : public trpgReadWriteable
{
    public:
        ...
        bool
        operator==( const trpgTextStyle& ) const;
...
TX_EXDECL class TX_CLDECL trpgTextStyleTable : public trpgReadWriteable
{
    public:
        ...
        int
        AddStyle( const trpgTextStyle& );

        int
        FindAddStyle( const trpgTextStyle& );

        // Figure out how many text styles there are
        int
        GetNumStyle( void ) const;

        // Return a reference to the given style
        const trpgTextStyle*
        GetStyleRef( int ) const;
...
TX_EXDECL class TX_CLDECL trpgSupportStyle : public trpgReadWriteable
{
    public:
        ...
        bool
        operator==( const trpgSupportStyle& ) const;
...
TX_EXDECL class TX_CLDECL trpgSupportStyleTable : public trpgReadWriteable
{
    public:
        ...
        // Add a single text style
        int
        AddStyle( const trpgSupportStyle& );

        int
        FindAddStyle( const trpgSupportStyle& );
...
TX_EXDECL class TX_CLDECL trpgLabelProperty : public trpgReadWriteable
{
    public:
        ...
        bool
             operator==( const trpgLabelProperty& ) const;
...
TX_EXDECL class TX_CLDECL trpgLabelPropertyTable : public trpgReadWriteable
{
    public:
        ...
        int
        AddProperty( const trpgLabelProperty& );

        // Find or Add a single label property
        int
        FindAddProperty( const trpgLabelProperty& property );

        // Figure out how many properties there are
        int
        GetNumProperty( void ) const;

        // Return a reference to the given property
        const trpgLabelProperty*
        GetPropertyRef( int ) const;
...
TX_EXDECL class TX_CLDECL trpgLabel : public trpgReadWriteable
{
    public:
        ...
        */
        ...
        void
        SetLocation( const trpg3dPoint& );
        const trpg3dPoint&
        GetLocation() const;

        // Add a support.  See GetSupports() for more information
        void
        AddSupport( const trpg3dPoint& );
        ...
        */
        const std::vector<trpg3dPoint>*
        GetSupports( void ) const;
