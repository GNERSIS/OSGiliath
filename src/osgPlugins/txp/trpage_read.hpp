...
TX_EXDECL class TX_CLDECL trpgr_Token
{
    public:
        ...
        void
        Destruct( void );           // Not quite like delete
};
...
TX_EXDECL class TX_CLDECL trpgr_Parser
{
    public:
        ...
        virtual void
        AddCallback( trpgToken,
                     trpgr_Callback*,
                     bool destroy = true );
        virtual void
        AddCallback( trpgToken,
                     trpgReadWriteable* );
        virtual const trpgr_Callback*
        GetCallback( trpgToken tok ) const;
        virtual trpgr_Callback*
                     GetCallback( trpgToken tok );
        virtual void RemoveCallback( trpgToken );
        virtual void
        SetDefaultCallback( trpgr_Callback*,
                            bool destroy = true );
        // Parse a read buffer
        virtual bool
                     Parse( trpgReadBuffer& );
...
TX_EXDECL class TX_CLDECL trpgrImageHelper
{
    public:
        ...
        trpgrImageHelper( trpgEndian ness,
                          char*      dir,
                          const trpgMatTable&,
                          const trpgTexTable&,
                          bool separateGeoTyp );
        ...
        */
        virtual bool
        GetMipLevelLocalGL( int miplevel,
                            const trpgTexture*,
                            char* data,
                            int32 dataSize );
        ...
        /* Do the lookups to figure out the correct material
           ...
           This routine also calculates the total size, including mipmaps if they're
           there.
        */
        virtual bool
        GetImageInfoForLocalMat( const trpgLocalMaterial* locMat,
         ...
         */
        virtual bool
        GetNthImageInfoForLocalMat( const trpgLocalMaterial* locMat,
                                    int                      index,
                                    const trpgMaterial**     retMat,
                                    const trpgTexture**      retTex,
                                    int&                     totSize );
        ...
        */
        virtual bool
        GetImageForLocalMat( const trpgLocalMaterial* locMat,
                             char*                    data,
                             int                      dataSize );
         ...
         */
        virtual bool
        GetNthImageForLocalMat( const trpgLocalMaterial* locMat,
                                int                      index,
                                char*                    data,
                                int                      dataSize );
         ...
         */
        virtual bool
        GetMipLevelForLocalMat( int                      miplevel,
         ...
         */
        virtual bool
        GetNthImageMipLevelForLocalMat( int                      miplevel,
                                        const trpgLocalMaterial* locMat,
                                        int                      index,
                                        char*                    data,
                                        int                      dataSize );
        ...
        */
        virtual bool
        GetImagePath( const trpgTexture*,
                      char*,
                      int len );

        virtual trpgrAppFileCache*
        GetNewRAppFileCache( const char* fullBase,
                             const char* ext );
...
TX_EXDECL class TX_CLDECL trpgr_Archive : public trpgCheckable
{
    public:
        ...
        virtual bool
        OpenFile( const char* );    // Open File
        virtual void
        CloseFile( void );
        virtual bool
        ReadHeader( void );    // Read header (materials, tile table. etc..)
        // overload that lets you specify if you want to read all the blocks now,
        // or defer reading them for later.
        virtual bool
        ReadHeader( bool readAllBlocks );
        bool
        ReadSubArchive( int        row,
                        int        col,
                        trpgEndian cpuNess );
        ...
        ReadTile( uint32 x,
                  ...
                  trpgMemReadBuffer& );
        virtual bool
        ReadTile( const trpgwAppAddress& addr,
                  trpgMemReadBuffer&     buf );
        virtual bool
        ReadExternalTile( uint32             x,
                          uint32             y,
                          uint32             lod,
                          trpgMemReadBuffer& buf );

        // Get access to header info
        virtual const trpgHeader*
        GetHeader( void ) const;
        virtual const trpgMatTable*
        GetMaterialTable( void ) const;
        virtual trpgTexTable*
        GetTexTable( void );
        virtual const trpgModelTable*
        GetModelTable( void ) const;
        virtual const trpgTileTable*
        GetTileTable( void ) const;
        virtual const trpgLightTable*
        GetLightTable( void ) const;
        virtual const trpgRangeTable*
        GetRangeTable( void ) const;
        virtual const trpgLabelPropertyTable*
        GetLabelPropertyTable() const;
        virtual const trpgTextStyleTable*
        GetTextStyleTable() const;
        virtual const trpgSupportStyleTable*
        GetSupportStyleTable() const;

        // Utility routine to calculate the MBR of a given tile
        virtual bool
        trpgGetTileMBR( uint32       x,
                        uint32       y,
                        uint32       lod,
                        trpg3dPoint& ll,
        ...
        virtual trpgrImageHelper*
        GetNewRImageHelper( trpgEndian          ness,
                            char*               dir,
                            const trpgMatTable& matTable,
                            const trpgTexTable& texTable );
        virtual trpgrAppFileCache*
