...
// Set functions
void
trpgModel::SetType( int t )
{
    type = t;
}

void
trpgModel::SetName( const char* nm )
{
    if( name )
    {
        delete[] name;
...
void
trpgModel::SetReference( trpgDiskRef pos )
{
    diskRef = pos;
}

void
trpgModel::SetNumTiles( int num )
{
    useCount = num;
}
...
trpgModel&
trpgModel::operator=( const trpgModel& in )
{
    if( name )
    {
        delete[] name;
...
// Compare two models
int
trpgModel::operator==( const trpgModel& in ) const
{
    if( type != in.type )
    {
        return 0;
...
bool
trpgModel::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
bool
trpgModel::GetNumTiles( int& ret ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgModel::GetReference( trpgDiskRef& ref ) const
{
...
bool
trpgModel::Read( trpgReadBuffer& buf,
                 bool            hasHandle )
{
...
// Set functions
void
trpgModelTable::SetNumModels( int /*no*/ )
{
...
void
trpgModelTable::SetModel( int              id,
                          const trpgModel& mod )
{
    if( id < 0 )
    {
...
int
trpgModelTable::AddModel( trpgModel& mod )
{
...
int
trpgModelTable::FindAddModel( trpgModel& mod )
{
...
bool
trpgModelTable::FindByName( const char*   name,
                            unsigned int& mId )
{
    ...
    {
        char theName[1'023];
        itr->second.GetName( theName, 1'023 );
...
bool
trpgModelTable::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
// Get methods
bool
trpgModelTable::GetNumModels( int& nm ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgModelTable::GetModel( int        id,
                          trpgModel& model ) const
{
...
trpgModel*
trpgModelTable::GetModelRef( int id )
{
...
bool
trpgModelTable::Read( trpgReadBuffer& buf )
{
    int32     numModel;
    trpgToken tok;
    int32     len;
