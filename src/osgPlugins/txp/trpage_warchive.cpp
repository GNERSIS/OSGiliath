...
trpgwArchive::trpgwArchive( trpgEndian             inNess,
                            trpgwArchive::TileMode inTileMode,
                            int                    majorVer,
                            int                    minorVer )
{
...
void
trpgwArchive::Init( trpgEndian             inNess,
                    trpgwArchive::TileMode inTileMode,
                    int                    majorVer,
                    int                    minorVer )
{
...
trpgwArchive::trpgwArchive( char*        inDir,
                            char*        inFile,
                            trpg2dPoint& sw,
                            trpg2dPoint& ne,
                            int          majorVer,
                    ...
                    int          minorVer )
{
    ...
    if( !extentsUnchanged )
    {
        ...
        addOffset.x = ( int )dx;
        addOffset.y = ( int )dy;
        if( dx - addOffset.x > 10E-4 || dy - addOffset.y > 10E-4 )
        {
            delete inArch;
            throw 1;
        }
        ...
        if( majorVersion == 2 && minorVersion >= 1 )
        {
            ...
            for( int ix = 0; ix < oldTileSize.x; ix++ )
            {
                for( int iy = 0; iy < oldTileSize.y; iy++ )
                {
                    ...
                    tileTable.SetTile( ix + addOffset.x,
                                       iy + addOffset.y,
                                       0,
                                       addr,
                                       zmin,
...
// WriteHeaderData
int32
trpgwArchive::WriteHeaderData( const char* dataPtr,
                               int         length,
                               FILE* /*filehandle*/ )
{
...
void
trpgwArchive::SetMaxTileFileLength( int max )
{
    maxTileFileLen = max;
}
...
*/
bool
trpgwArchive::SetHeader( const trpgHeader& head )
{
    header = head;
    return true;
}
...
bool
trpgwArchive::SetMaterialTable( const trpgMatTable& mat )
{
    matTable = mat;
    return true;
}
...
bool
trpgwArchive::SetTextureTable( const trpgTexTable& tex )
{
    texTable = tex;
    return true;
}
...
bool
trpgwArchive::SetModelTable( const trpgModelTable& models )
{
    modelTable = models;
    return true;
}
...
bool
trpgwArchive::SetLightTable( const trpgLightTable& lights )
{
    lightTable = lights;
    return true;
}
...
bool
trpgwArchive::SetRangeTable( const trpgRangeTable& ranges )
{
    rangeTable = ranges;
    return true;
}
...
bool
trpgwArchive::SetTextStyleTable( const trpgTextStyleTable& styles )
{
...
bool
trpgwArchive::SetLabelPropertyTable( const trpgLabelPropertyTable& properties )
{
...
bool
trpgwArchive::SetSupportStyleTable( const trpgSupportStyleTable& styles )
{
...
*/
trpgHeader*
trpgwArchive::GetHeader()
{
    return &header;
}

trpgMatTable*
trpgwArchive::GetMatTable()
{
    return &matTable;
}

trpgTexTable*
trpgwArchive::GetTextureTable()
{
    return &texTable;
}

trpgModelTable*
trpgwArchive::GetModelTable()
{
    return &modelTable;
}

trpgLightTable*
trpgwArchive::GetLightTable()
{
    return &lightTable;
}

trpgRangeTable*
trpgwArchive::GetRangeTable()
{
    return &rangeTable;
}

trpgTextStyleTable*
trpgwArchive::GetTextStyleTable()
{
    return &textStyleTable;
}

trpgSupportStyleTable*
trpgwArchive::GetSupportStyleTable()
{
    return &supportStyleTable;
}

trpgLabelPropertyTable*
trpgwArchive::GetLabelPropertyTable()
{
...
bool
trpgwArchive::OpenFile( const char* in_dir,
                        const char* name )
{
    char filename[1'025];

    strncpy( dir, in_dir, 1'023 );
...
trpgwAppFile*
trpgwArchive::GetNewWAppFile( trpgEndian  inNess,
                              const char* fileName,
                              bool        reuse )
{
 ...
 */
trpgwImageHelper*
trpgwArchive::GetNewWImageHelper( trpgEndian    endian,
                                  char*         imageDir,
                                  trpgTexTable& inTexTable )
{
...
*/
bool
trpgwArchive::DesignateTileFile( int id )
{
    if( tileMode != TileLocal )
    {
        return false;
...
*/
bool
trpgwArchive::WriteTile( unsigned int              x,
                         unsigned int              y,
                         unsigned int              lod,
                         float                     zmin,
                         float                     zmax,
                         const trpgMemWriteBuffer* head,
                         const trpgMemWriteBuffer* buf,
                         int32&                    fileId,
                         int32&                    fileOffset )
{
    FILE* tfp = NULL;
...
trpgwGeomHelper::trpgwGeomHelper( trpgWriteBuffer* ibuf,
                                  int              dtype )
{
    init( ibuf, dtype );
}
...
void
trpgwGeomHelper::init( trpgWriteBuffer* ibuf,
                       int              dtype )
{
    buf      = ibuf;
    dataType = dtype;
...
// Note: Required
void
trpgwGeomHelper::SetColor( trpgColor& /*col*/ )
{
    // tmpColor = col;
}
...
// Note: Required
void
trpgwGeomHelper::SetTexCoord( trpg2dPoint& pt )
{
    tmpTex.resize( 0 );
    tmpTex.push_back( pt );
}
...
void
trpgwGeomHelper::AddTexCoord( trpg2dPoint& pt )
{
    tmpTex.push_back( pt );
}
...
// Note: required
void
trpgwGeomHelper::SetNormal( trpg3dPoint& pt )
{
    tmpNorm = pt;
}
...
// Note: required
void
trpgwGeomHelper::SetMaterial( int32 imat )
{
    matPoly.resize( 0 );
...
void
trpgwGeomHelper::AddMaterial( int32 imat )
{
    matPoly.push_back( imat );
}
...
void
trpgwGeomHelper::GetZMinMax( double& outZmin,
                             double& outZmax )
{
    outZmin = zmin;
    outZmax = zmax;
...
void
trpgwGeomHelper::AddVertex( trpg3dPoint& pt )
{
        ...
        case trpgGeometry::Quads :
            {
                ...
                if( numVert % 4 == 0 )
                {
                    int dtype = ( dataType == UseDouble ? trpgGeometry::DoubleData
                                                        : trpgGeometry::FloatData );
                    ...
                    for( unsigned int i = 0; i < numVert; i++ )
                    {
                        ...
                        for( loop = 0; loop < numMat; loop++ )
                        {
                            quads.AddTexCoord( ( trpgGeometry::DataType )dtype,
                                               tex[i * numMat + loop],
                                               loop );
                        }
                    }
...
class optVert
{
    public:
        ...
        optVert( int                       numMat,
                 int                       vid,
                 std::vector<trpg3dPoint>& iv,
                 std::vector<trpg3dPoint>& in,
                 std::vector<trpg2dPoint>& itex );
        trpg3dPoint              v;
        trpg3dPoint              n;
        ...
        int
        operator==( const optVert& in ) const
        {
...
void
trpgwGeomHelper::Optimize()
{
    int dtype =
        ( dataType == UseDouble ? trpgGeometry::DoubleData : trpgGeometry::FloatData );
...
trpgwImageHelper::trpgwImageHelper( trpgEndian    inNess,
                                    char*         inDir,
                                    trpgTexTable& inTable,
                                    bool          sepGeoTypical )
{
...
void
trpgwImageHelper::Init( trpgEndian    inNess,
                        char*         inDir,
                        trpgTexTable& inTable,
                        bool          sepGeoTypical )
{
...
bool
trpgwImageHelper::AddExternal( char* name,
                               int&  texID,
...
void
trpgwImageHelper::SetMaxTexFileLength( int len )
{
    maxTexFileLen = len;
}

bool
trpgwImageHelper::AddLocal( char*                  name,
...
bool
trpgwImageHelper::ReplaceLocal( char* data,
                                int&  texID )
{
...
bool
trpgwImageHelper::AddTileLocal( char*                  name,
                                trpgTexture::ImageType type,
                                int                    sizeX,
                                int                    sizeY,
                                bool                   isMipmap,
 ...
 */
trpgwAppFile*
trpgwImageHelper::GetNewWAppFile( trpgEndian  inNess,
                                  const char* fileName,
                                  bool        reuse )
{
    return new trpgwAppFile( inNess, fileName, reuse );
...
*/
trpgwAppFile*
trpgwImageHelper::IncrementTextureFile( bool geotyp )
{
    char          filename[1'049];
    ...
    if( geotyp && separateGeoTypical )
    {
        thefile = geotypFile;
        sprintf( filename,
                 "%s" PATHSEPERATOR "geotypFile_%d.txf",
                 dir,
                 static_cast<int>( geotypFileIDs.size() ) );
    }
    else
    {
        sprintf( filename,
                 "%s" PATHSEPERATOR "texFile_%d.txf",
                 dir,
                 static_cast<int>( texFileIDs.size() ) );
    }
...
*/
bool
trpgwImageHelper::DesignateTextureFile( int id )
{
 ...
 */
bool
trpgwImageHelper::WriteToArchive( const trpgTexture& tex,
                                  char*              data,
                                  trpgwAppAddress&   addr,
                                  bool               geotyp )
{
