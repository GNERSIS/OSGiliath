...
int32
trpgr_Archive::GetHeaderData( char* dataPtr,
                              int   length,
                              FILE* filehandle )
{
...
void
trpgr_Archive::SetDirectory( const char* in_dir )
{
    strncpy( dir, in_dir, DIR_SIZE - 1 );
    dir[DIR_SIZE - 1] = 0;
}
...
bool
trpgr_Archive::OpenFile( const char* name )
{
    char file[1'025];
...
trpgrAppFileCache*
trpgr_Archive::GetNewRAppFileCache( const char* fullBase,
                                    const char* ext )
{
...
trpgrImageHelper*
trpgr_Archive::GetNewRImageHelper( trpgEndian          endian,
                                   char*               imagedir,
                                   const trpgMatTable& matTable,
                                   const trpgTexTable& tTable )
{
...
/**
 * Read a sub block from a 2.2 TXP database. This can be called any time after ReadHeader
 * is called if ReadHeader is called with the false parameter to specify not to read all
 * the sub-archives. This can make a huge improvement in startup time for loading a very
 * large archive with many blocks.
 **/
bool
trpgr_Archive::ReadSubArchive( int        row,
                               int        col,
                               trpgEndian cpuNess )
{
    int          ret;
    ...
    if( ( bmagic != GetMagicNumber() ) &&
        ( trpg_byteswap_int( bmagic ) != GetMagicNumber() ) )
    {
        // close the block archive
        fclose( bfp );
        return false;
    ...
    bparser.AddCallback( TRPGMATTABLE,
                         &materialTable );    // Went back to oldest style for 2.0
...
bool
trpgr_Archive::ReadHeader( bool readAllBlocks )
{
    int ret;
    ...
    parser.AddCallback( TRPGMATTABLE,
                        &materialTable );    // Went back to oldest style for 2.0
    parser.AddCallback( TRPGMATTABLE2,
                        &oldMatTable );      // Added 11-14-98 (1.0 material table)
...
bool
trpgr_Archive::ReadTile( uint32             x,
                         uint32             y,
                         uint32             lod,
...
bool
trpgr_Archive::ReadExternalTile( uint32             x,
                                 uint32             y,
                                 uint32             lod,
                                 trpgMemReadBuffer& buf )
{
        ...
        ( minorVer >= TRPG_NOMERGE_VERSION_MINOR ) )
    {
        ...
        blocky             = y / denom;
        sprintf( filename,
                 "%s" PATHSEPERATOR "%d" PATHSEPERATOR "%d" PATHSEPERATOR
                 "tile_%d_%d_%d.tpt",
                 dir,
                 blockx,
                 blocky,
...
bool
trpgr_Archive::ReadTile( const trpgwAppAddress& addr,
                         trpgMemReadBuffer&     buf )
{
...
// Get methods
const trpgHeader*
trpgr_Archive::GetHeader() const
{
    return &header;
}

const trpgMatTable*
trpgr_Archive::GetMaterialTable() const
{
    return &materialTable;
}

trpgTexTable*
trpgr_Archive::GetTexTable()
{
    return &texTable;
}

const trpgModelTable*
trpgr_Archive::GetModelTable() const
{
    return &modelTable;
}

const trpgTileTable*
trpgr_Archive::GetTileTable() const
{
    return &tileTable;
}

const trpgLightTable*
trpgr_Archive::GetLightTable() const
{
    return &lightTable;
}

const trpgRangeTable*
trpgr_Archive::GetRangeTable() const
{
    return &rangeTable;
}

const trpgTextStyleTable*
trpgr_Archive::GetTextStyleTable() const
{
    return &textStyleTable;
}

const trpgSupportStyleTable*
trpgr_Archive::GetSupportStyleTable() const
{
    return &supportStyleTable;
}

const trpgLabelPropertyTable*
trpgr_Archive::GetLabelPropertyTable() const
{
    return &labelPropertyTable;
}

trpgEndian
trpgr_Archive::GetEndian() const
{
    return ness;
}
...
// Utility MBR routine
bool
trpgr_Archive::trpgGetTileMBR( uint32       x,
                               uint32       y,
                               uint32       lod,
                               trpg3dPoint& ll,
                               trpg3dPoint& ur ) const
{
    if( !header.isValid() )
    {
        return false;
...
trpgrImageHelper::trpgrImageHelper( trpgEndian          inNess,
                                    char*               inDir,
                                    const trpgMatTable& inMatTable,
                                    const trpgTexTable& inTexTable,
                                    bool                sepGeoTyp )
{
...
void
trpgrImageHelper::Init( trpgEndian          inNess,
                        char*               inDir,
                        const trpgMatTable& inMatTable,
                        const trpgTexTable& inTexTable,
                        bool                sepGeoTyp )
...
trpgrAppFileCache*
trpgrImageHelper::GetNewRAppFileCache( const char* fullBase,
                                       const char* /*ext*/ )
{
...
bool
trpgrImageHelper::GetLocalGL( const trpgTexture* tex,
                              char*              data,
                              int32              size )
{
...
bool
trpgrImageHelper::GetMipLevelLocalGL( int                miplevel,
...
bool
trpgrImageHelper::GetImageInfoForLocalMat( const trpgLocalMaterial* locMat,
...
bool
trpgrImageHelper::GetNthImageInfoForLocalMat( const trpgLocalMaterial* locMat,
                                              int                      index,
                                              const trpgMaterial**     retMat,
                                              const trpgTexture**      retTex,
                                              int&                     totSize )
...
bool
trpgrImageHelper::GetImageForLocalMat( const trpgLocalMaterial* locMat,
                                       char*                    data,
                                       int                      dataSize )
{
...
bool
trpgrImageHelper::GetNthImageForLocalMat( const trpgLocalMaterial* locMat,
                                          int                      index,
                                          char*                    data,
                                          int                      dataSize )
{
        ...
        case trpgTexture::Template :
            {
                ...
                trpgrAppFile* af =
                    texCache->GetFile( ness, addr.file, addr.col, addr.row );
                if( !af )
                {
                    return false;
                }
...
bool
trpgrImageHelper::GetMipLevelForLocalMat( int                      miplevel,
...
bool
trpgrImageHelper::GetNthImageMipLevelForLocalMat( int                      miplevel,
                                                  const trpgLocalMaterial* locMat,
                                                  int                      index,
                                                  char*                    data,
                                                  int                      dataSize )
        ...
        case trpgTexture::Template :
            {
                ...
                int level_offset =
                    ( const_cast<trpgTexture*>( tex ) )->MipLevelOffset( miplevel );
                if( !af->Read( data, addr.offset, level_offset, dataSize ) )
                {
                    return false;
                }
...
bool
trpgrImageHelper::GetImagePath( const trpgTexture* tex,
                                char*              fullPath,
                                int                pathLen )
{
    char name[1'024];
