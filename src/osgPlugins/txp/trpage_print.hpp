...
TX_EXDECL class TX_CLDECL trpgPrintGraphParser : public trpgSceneParser
{
    public:
        ...
        trpgPrintGraphParser( trpgr_Archive*,
                              trpgrImageHelper*,
                              trpgPrintBuffer* );
        ...
        */
        class ReadHelper : public trpgr_Callback
        {
            public:
                ...
                // The const in the template parameter was removed because it causes GCC
                // to freak out.  I am of the opinion that const doesn't make sense in a
                // template parameter for std::vector anyway... const prevents you from
                // changing the value, so what exactly is the point?  How does one add
                // entries to the vector without giving them a value?  -ADS
                typedef std::vector<trpgChildRef> ChildRefList;

                ReadHelper( trpgPrintGraphParser* inPG,
...
trpgPrintArchive( char*            filename,
                  trpgPrintBuffer& pBuf,
                  int              flags = TRPGPRN_ALL );
TX_CPPDECL bool
trpgPrintArchive( trpgr_Archive*,
                  trpgPrintBuffer& pBuf,
                  int              flags = TRPGPRN_ALL );
