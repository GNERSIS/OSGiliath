...
void
trpgMBR::AddPoint( const trpg3dPoint& pt )
{
    if( valid )
    {
        ll.x = MIN( pt.x, ll.x );
...
void
trpgMBR::GetMBR( trpg3dPoint& oll,
                 trpg3dPoint& our ) const
{
    oll = ll;
    our = ur;
}
...
void
trpgMBR::Union( const trpgMBR& in )
{
    if( valid )
    {
        if( in.isValid() )
...
bool
trpgMBR::Overlap( const trpg2dPoint& ill,
                  const trpg2dPoint& iur ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMBR::Within( const trpg2dPoint& pt ) const
{
...
void
trpgReadGroupBase::AddChild( trpgReadNode* n )
{
...
void
trpgReadGroupBase::unRefChild( int id )
{
...
void
trpgReadGroupBase::unRefChildren()
{
    for( unsigned int i = 0; i < children.size(); i++ )
    {
        unRefChild( i );
...
// Calculate an MBR
trpgMBR
trpgReadGroupBase::GetMBR() const
{
    if( mbr.isValid() )
    {
        return mbr;
...
trpgReadGroupBase*
trpgSceneGraphParser::GetCurrTop()
{
    if( !currTop )
    {
        return NULL;
...
trpgReadTileHeader*
trpgSceneGraphParser::GetTileHeaderRef()
{
    return &tileHead;
}
...
trpgReadNode*
trpgSceneGraphParser::ParseScene( trpgReadBuffer& buf,
                                  GroupMap&       inGmap )
{
    gmap                 = &inGmap;
...
bool
trpgSceneGraphParser::StartChildren( void* in_node )
{
...
/* This is called whent he parser hits a pop.
   ...
   */
bool
trpgSceneGraphParser::EndChildren( void* /*in_node*/ )
{
...
trpgSceneGraphParser::GroupMap*
trpgSceneGraphParser::GetGroupMap()
{
    return gmap;
}
...
bool
trpgTestArchive( trpgr_Archive& archive )
{
    int                            numLod;
