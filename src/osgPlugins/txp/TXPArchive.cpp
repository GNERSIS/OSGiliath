...
TXPArchive::GetTexMapEntry( int key )
{
    return _texmap[key];
}

void
TXPArchive::SetStatesMap( int                         key,
...
TXPArchive::GetStatesMapEntry( int key )
{
...
void
TXPArchive::getExtents( osg::box& extents )
{
    TileInfo    sw, ne;
    ...
    extents.set( sw.bbox.min, sw.bbox.max );
...
TXPArchive::openFile( const std::string& archiveName )
{
    ...
    {
        TXPArchiveERROR( "openFile()" )
            << "couldn't open archive: " << archiveName << std::endl;
        return false;
    }
    ...
    {
        TXPArchiveERROR( "openFile()" )
            << "couldn't read header for archive: " << archiveName << std::endl;
        return false;
    }
...
bool
TXPArchive::loadMaterial( int ix )
{
    int i = ix;
    ...
    if( numMatTex )
    {
        ...
        osg_state_set->setAttributeAndModes( osg_material.get(),
            ...
            if( osg_texture )
            {
                ...
                switch( minFilter )
                {
                    ...
                    case trpgTextureEnv::Nearest :
                        osg_texture->setFilter( osg::Texture2D::MIN_FILTER,
                ...
                switch( magFilter )
                {
                    case trpgTextureEnv::Point :
                    case trpgTextureEnv::Nearest :
                        osg_texture->setFilter( osg::Texture2D::MAG_FILTER,
                                                osg::Texture2D::NEAREST );
                        break;
                    case trpgTextureEnv::Linear :
                    default :
                ...
                osg_state_set->setTextureAttributeAndModes( ntex,
                                                            osg_texture,
                                                            osg::StateAttribute::ON );

                if( osg_texture->getImage() &&
                    osg_texture->getImage()->isImageTranslucent() )
                {
                    osg_state_set->setMode( GL_BLEND, osg::StateAttribute::ON );
                    osg_state_set->setRenderingHint( osg::StateSet::TRANSPARENT_BIN );
                }
        ...
        if( cullMode != trpgMaterial::FrontAndBack )
        {
            ...
            osg_state_set->setAttributeAndModes( cull_face.get(),
                                                 osg::StateAttribute::ON );
        }
    }
...
TXPArchive::loadTexture( int i )
{
    ...
    trpgrImageHelper   image_helper( this->GetEndian(),
                                     getDir(),
                                     materialTable,
                                     texTable,
                                     separateGeo );
    ...
    if( type == trpgModel::External )
    {
        ...
        if( !osg_model )
        {
            OSG_WARN << "TrPageArchive::LoadModels() error: "
                     << "failed to load model: " << name << std::endl;
        }
...
TXPArchive::loadLightAttributes()
{
    ...
    for( ; itr != lm->end(); itr++ )
    {
        ...
        addLightAttribute( osgLight,
                           stateSet,
                           osg::vec3( normal.x, normal.y, normal.z ),
                           itr->first );
    }
    ...
    for( ; itr != smap->end(); itr++ )
    {
        ...
        if( matTable )
        {
            ...
            if( mat )
            {
                ...
                _fcolors[itr->first] =
                    osg::vec4( faceColor.red, faceColor.green, faceColor.blue, alpha );
            }
        }
    }
...
void
TXPArchive::addLightAttribute( osgSim::LightPointNode* lpn,
...
bool
TXPArchive::getTileInfo( const TileLocationInfo& loc,
                         TileInfo&               info )
{
    info.minRange = 0.0;
    info.maxRange = 0.0;
    ...
    info.center.set( sw.x + ( loc.x * size.x ) + ( size.x / 2.F ),
                     sw.y + ( loc.y * size.y ) + ( size.y / 2.F ),
                     ( loc.zmin + loc.zmax ) / 2.F );
    info.bbox.set( osg::vec3( info.center.x - ( size.x / 2.F ),
                              info.center.y - ( size.y / 2.F ),
                              loc.zmin ),
                   osg::vec3( info.center.x + ( size.x / 2.F ),
                              info.center.y + ( size.y / 2.F ),
                              loc.zmax ) );
    info.radius = osg::length( osg::vec3( size.x / 2.F, size.y / 2.F, 0.F ) ) * 1.3;

    return true;
}

bool
TXPArchive::getTileInfo( int       x,
                         int       y,
                         int       lod,
                         TileInfo& info )
{
...
osg::Group*
TXPArchive::getTileContent( int                            x,
                            int                            y,
                            int                            lod,
                            double                         realMinRange,
                            double                         realMaxRange,
...
class ModelVisitor : public osg::DualModeVisitor
{
        TXPArchive*                  _archive;
    ...
    public:
        ...
        ModelVisitor( TXPArchive*                         archive,
        ...
        virtual void
        apply( osg::MatrixTransform& xform )
        {
            ...
            const TileIdentifier*  tileID =
                dynamic_cast<const txp::TileIdentifier*>( ref );
            ...
            if( tileType == trpgHeader::TileLocal && tileID->lod == 9'999 )
            {
                trpg2dPoint tileExtents;
                header->GetTileSize( 0, tileExtents );
                osg::box bbox;
                _archive->getExtents( bbox );
                osg::vec3 offset( osg::getTrans( xform.getMatrix() ) );
                offset[0] -= bbox.min[0];
                offset[1] -= bbox.min[1];
                ...
                osg::dmat4 mat( xform.getMatrix() );
                osg::setTrans( mat, osg::dvec3( offset ) );
                xform.setMatrix( mat );
            }
        }
};
...
osg::Group*
TXPArchive::getTileContent( const TileLocationInfo&        loc,
                            double                         realMinRange,
                            double                         realMaxRange,
                            double                         usedMaxRange,
                            osg::vec3&                     tileCenter,
                            std::vector<TileLocationInfo>& childInfoList )
{
    ...
    if( ( majVersion >= TRPG_NOMERGE_VERSION_MAJOR ) &&
        ( minVersion >= TRPG_NOMERGE_VERSION_MINOR ) &&
        ( _isMaster ) )
    {
        if( tilehdr )
        {
    ...
    osg::Group* tileGroup = _parser->parseScene( buf,
                                                 _statesMap,
                                                 _models,
                                                 realMinRange,
                                                 realMaxRange,
    ...
    {
        if( mitr->second.valid() && ( mitr->second->referenceCount() == 1 ) )
        {
            // unreference it.
            mitr->second                     = NULL;
