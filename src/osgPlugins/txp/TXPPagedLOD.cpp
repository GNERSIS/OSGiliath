...
TXPPagedLOD::TXPPagedLOD( const TXPPagedLOD& plod,
...
void
TXPPagedLOD::traverse( osg::NodeVisitor& nv )
{

    // TileMapper* tileMapper = dynamic_cast<TileMapper*>(nv.getUserData());
    // Modified by Brad Anderegg (May-27-08) because the black listing process appears to
    // make tiles switch lods when they clearly shouldn't, in the worst cases a tile will
    // page out that is right in front of you.
    bool forceUseOfFirstChild =
        /*tileMapper ? (tileMapper->isNodeBlackListed(this)) :*/ false;
        ...
        case( osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN ) :
            {
                ...
                {
                    if( forceUseOfFirstChild || ( _rangeList[i].first <=
                                                  distance &&
                ...
                if( needToLoadChild )
                {
                        ...
                        numChildren < _perRangeDataList.size() )
                    {
                        ...
                        // distance falls.
                        float priority = ( _rangeList[numChildren].second - distance ) /
                        ...
                        // "<<_fileNameList[numChildren]<<" priority =
                        // "<<priority<<std::endl;
...
osg::sphere
TXPPagedLOD::computeBound() const
{
