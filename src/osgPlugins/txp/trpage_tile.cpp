...
void
trpgTileTable::SetMode( TileMode inMode )
{
    Reset();
    mode = inMode;
}
...
void
trpgTileTable::SetNumLod( int numLod )
{
    lodInfo.resize( numLod );
}

void
trpgTileTable::SetNumTiles( int nx,
                            int ny,
                            int lod )
{
...
bool
trpgTileTable::GetMode( TileMode& outMode ) const
{
    if( !isValid() )
    {
        return false;
...
// Write tile table
bool
trpgTileTable::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTileTable::Read( trpgReadBuffer& buf )
{
    valid = false;
...
// Set functions
void
trpgTileHeader::SetMaterial( int no,
                             int id )
{
...
void
trpgTileHeader::SetModel( int no,
                          int id )
{
...
// Set functions
void
trpgTileHeader::AddMaterial( int id )
{
    // Look for it first
...
void
trpgTileHeader::AddModel( int id )
{
...
void
trpgTileHeader::SetDate( int32 d )
{
    date = d;
}
...
void
trpgTileHeader::AddLocalMaterial( trpgLocalMaterial& locMat )
{
    locMats.push_back( locMat );
}

bool
trpgTileHeader::GetNumLocalMaterial( int32& retNum ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTileHeader::GetLocalMaterial( int32              id,
                                  trpgLocalMaterial& retMat ) const
{
...
trpgTileHeader::GetLocalMaterialList() const
{
    if( !isValid() )
    {
        return NULL;
...
// Get methods
bool
trpgTileHeader::GetNumMaterial( int32& no ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTileHeader::GetMaterial( int32  id,
                             int32& mat ) const
{
...
bool
trpgTileHeader::GetNumModel( int32& no ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTileHeader::GetModel( int32  id,
                          int32& m ) const
{
...
bool
trpgTileHeader::GetDate( int32& d ) const
{
    if( !isValid() )
    {
        return false;
...
// Write to a buffer
bool
trpgTileHeader::Write( trpgWriteBuffer& buf )
{
    unsigned int i;
...
void*
tileHeaderCB::Parse( trpgToken       tok,
                     trpgReadBuffer& buf )
{
    int32 no, id, date, i;
            ...
            case TRPG_TILE_LOCMATLIST :
                {
                    ...
                    locMats = const_cast<std::vector<trpgLocalMaterial>*>(
                        head->GetLocalMaterialList()
                    );
...
// Read tile header
bool
trpgTileHeader::Read( trpgReadBuffer& buf )
{
    tileHeaderCB tcb;
    trpgr_Parser parse;
