...
#define ReaderWriterTXPERROR( s )                                 \
    OSG_NOTICE << "txp::ReaderWriterTXP::" << ( s ) << " error: "
...
ReaderWriterTXP::local_readNode( const std::string&                  file,
                                 const osgDB::ReaderWriter::Options* options )
{
    ...
    if( strncmp( name.c_str(), "archive", 7 ) == 0 )
    {
        ...
        // calling getArchive will create a new TXPArchive if the specified one does not
        // exist we will set our osgdb loader options on the archive and set the
        // appropriate archive on the txpNode.
        int                      id = ++_archiveId;
        osg::ref_ptr<TXPArchive> archive =
            createArchive( id, osgDB::getFilePath( fileName ) );
        ...
        if( archive != NULL )
        {
            ...
            if( options &&
                options->getOptionString().find( "loadMaterialsToStateSet" ) !=
                std::string::npos )
            {
    ...
    else if( strncmp( name.c_str(), "tile", 4 ) == 0 )
    {
        ...
        if( lod != 0 )
        {
            ReaderWriterTXPERROR( "ReaderWriterTXP::local_readNode()" )
                << "paged 'tile' should be at lod 0" << std::endl;
            return ReadResult::ERROR_IN_READING_FILE;
        }
        ...
        osg::ref_ptr<osg::Node>                   tileContent =
            getTileContent( info, x, y, lod, archive.get(), childrenLoc );
        ...
        if( asChildren )
        {
            ...
            pagedLODfile << archive->getDir() << "\\subtiles" << lod << "_" << x << "x"
                         << y << "_" << archive->getId() << childrenInfoStr << ".txp";
            ...
            if( info.center.z == 0 )
            {
                osg::sphere bSphere = tileContent->getBound();
            ...
            // note: use maximum(info.maxRange,1e7) as just maxRange would result in some
            // corner tiles from being culled out.
            pagedLOD->addChild( tileContent.get(),
                                info.minRange,
                                std::max( info.maxRange, 1E7 ) );
    ...
    // For 2.1 and over  we load
    // subtilesLOD_XxY_ID_NBCHILD_{X_Y_FID_FOFFSET_ZMIN_ZMAX_X_Y_ADDR ....}.txp
    else if( strncmp( name.c_str(), "sub", 3 ) == 0 )
    {
        ...
        if( majorVersion == 2 && minorVersion >= 1 )
        {
            ...
            int numItemsRead = sscanf( name.c_str(),
                                       "subtiles%d_%dx%d_%u_%d",
                                       &lod,
                                       &x,
                                       &y,
                                       &id,
            ...
            {
                ReaderWriterTXPERROR( "ReaderWriterTXP::local_readNode()" )
                    << "'subtile' filename children parsing failed " << std::endl;
                return ReadResult::ERROR_IN_READING_FILE;
            }
            ...
            if( majorVersion >=
                TRPG_NOMERGE_VERSION_MAJOR &&
                minorVersion >=
                TRPG_NOMERGE_VERSION_MINOR &&
                archive->GetHeader()->GetIsMaster() )
            {
                for( int idx = 0; idx < nbChild; idx++ )
                {
                    ...
                    int          blockx, blocky;
                    unsigned int denom =
                        ( 1 << locs[idx].lod );    // this should work up to lod 31
            ...
            for( int idx = 0; idx < nbChild; ++idx )
            {
                ...
                osg::ref_ptr<osg::Node> tileContent =
                    getTileContent( info, loc, archive.get(), childrenChildLoc );

                tileContent->setName( "TileContent" );

                subtiles->setUserData(
                    new TileIdentifier( loc.x, loc.y, loc.lod )
                ...
                if( childrenChildLoc.size() > 0 )
                {
                    ...
                    char pagedLODfile[1'084];
                    sprintf( pagedLODfile,
                             "%s\\subtiles%d_%dx%d_%d%s.txp",
                             archive->getDir(),
                             loc.lod,
                             loc.x,
                             loc.y,
                    ...
                    // note: use maximum(info.maxRange,1e7) as just maxRange would result
                    // in some corner tiles from being culled out.
                    pagedLOD->addChild( tileContent.get(),
                                        info.minRange,
                                        std::max( info.maxRange, 1E7 ) );
                    ...
                    if( tileType == trpgHeader::TileLocal )
                    {
                        osg::vec3 center( info.center - parentInfo.bbox.min );
                        osg::vec3 sw( info.bbox.min - parentInfo.bbox.min );
                        sw[2] = 0.0F;
                        pagedLOD->setCenter( center - sw );
                        osg::dmat4 offset;
                        osg::setTrans( offset, osg::dvec3( sw ) );
                ...
                else
                {
                    ...
                    if( tileType == trpgHeader::TileLocal )
                    {
                        osg::dvec3 center( info.center - parentInfo.bbox.min );
                        osg::dvec3 sw( info.bbox.min - parentInfo.bbox.min );
                        sw[2] = 0.0;
                        osg::dmat4 offset;
                        osg::setTrans( offset, sw );
                        osg::MatrixTransform* tform = new osg::MatrixTransform( offset );
        ...
        else
        {
            ...
            for( int ix = 0; ix < 2; ix++ )
            {
                for( int iy = 0; iy < 2; iy++ )
                {
                    ...
                    osg::ref_ptr<osg::Node> tileContent = getTileContent( info,
                                                                          tileX,
                                                                          tileY,
                                                                          tileLOD,
                                                                          archive.get(),
                    ...
                    {
                        char pagedLODfile[1'094];
                        sprintf( pagedLODfile,
                                 "%s\\subtiles%d_%dx%d_%d.txp",
                                 archive->getDir(),
                                 tileLOD,
                                 tileX,
                                 tileY,
                        ...
                        // note: use maximum(info.maxRange,1e7) as just maxRange would
                        // result in some corner tiles from being culled out.
                        pagedLOD->addChild( tileContent.get(),
                                            info.minRange,
                                            std::max( info.maxRange, 1E7 ) );
                        pagedLOD->setFileName( 1, pagedLODfile );
                        ...
                        if( tileType == trpgHeader::TileLocal )
                        {
                            ...
                            osg::dmat4 offset;
                            osg::setTrans( offset, osg::dvec3( sw ) );
                            osg::MatrixTransform* tform =
                                new osg::MatrixTransform( offset );
                            tform->addChild( pagedLOD.get() );
                            subtiles->addChild( tform );
                        }
                        else
                    ...
                    {
                        subtiles->setUserData(
                            new TileIdentifier( tileX, tileY, tileLOD )
                        );
                        if( tileType == trpgHeader::TileLocal )
                        {
                            osg::dvec3 center( info.center - parentInfo.bbox.min );
        ...
        // OSG_NOTICE << "Subtiles for " << x << " " << y << " " << lod << " lodaded" <<
        // std::endl;
        return subtiles.get();
    }
...
void
ReaderWriterTXP::createChildrenLocationString(
    const std::vector<TXPArchive::TileLocationInfo>& locs,
...
) const
{
    ...
    else
    {
        ...
        for( unsigned int idx = 0; idx < locs.size(); ++idx )
        {
            ...
            theLoc << loc.x << "_" << loc.y << "_" << loc.addr.file << "_"
                   << loc.addr.offset << "_" << loc.zmin << "_" << loc.zmax;
            if( idx != locs.size() - 1 )
            {
                theLoc << "_";
            }
...
bool
ReaderWriterTXP::extractChildrenLocations(
...
std::string
ReaderWriterTXP::getArchiveName( const std::string& dir )
{
#ifdef _WIN32
    const char _PATHD = '\\';
...
osg::ref_ptr<TXPArchive>
ReaderWriterTXP::getArchive( int                id,
                             const std::string& dir )
{
    ...
    else
    {
        ...
        ReaderWriterTXPERROR( "getArchive()" ) << "archive id " << id << " not found: \""
...
osg::ref_ptr<TXPArchive>
ReaderWriterTXP::createArchive( int                id,
                                const std::string& dir )
{
    std::string              archiveName = getArchiveName( dir );
    ...
    if( archive != NULL )
    {
        ReaderWriterTXPERROR( "createArchive()" )
            << "archive id " << id << " already exists: \"" << archiveName << "\""
            << std::endl;
        return NULL;
    }
    ...
    {
        ReaderWriterTXPERROR( "createArchive()" )
            << "failed to load archive: \"" << archiveName << "\"" << std::endl;
        return NULL;
    }
    ...
    {
        ReaderWriterTXPERROR( "createArchive()" )
            << "failed to load materials from archive: \"" << archiveName << "\""
            << std::endl;
        return NULL;
    }
    ...
    {
        ReaderWriterTXPERROR( "createArchive()" )
            << "failed to load models from archive: \"" << archiveName << "\""
            << std::endl;
        return NULL;
    }
    ...
    {
        ReaderWriterTXPERROR( "createArchive()" )
            << "failed to load light attributes from archive: \"" << archiveName << "\""
            << std::endl;
        return NULL;
    }
    ...
    {
        ReaderWriterTXPERROR( "createArchive()" )
            << "failed to load text styles from archive: \"" << archiveName << "\""
            << std::endl;
        return NULL;
    }
...
bool
ReaderWriterTXP::removeArchive( int id )
{
    OSG_INFO << "ReaderWriterTXP::removeArchive(id=" << id << ")" << std::endl;
    // return (_archives.erase(id) >= 1);
    bool result = _archives.erase( id ) >= 1;
    OSG_WARN << "remove archive " << id << " size " << _archives.size() << " result "
             << result << std::endl;
    return result;
}

class SeamFinder : public osg::DualModeVisitor
{
    public:
        ...
        SeamFinder( int                         x,
                    int                         y,
                    int                         lod,
                    const TXPArchive::TileInfo& info,
                    TXPArchive*                 archive ) :
    ...
    protected:
        ...
        osg::Node*
        seamReplacement( osg::Node* node );

        SeamFinder&
        operator=( const SeamFinder& )
        {
            return *this;
        }
...
osg::Node*
SeamFinder::seamReplacement( osg::Node* node )
{
    ...
    for( unsigned int i = 0; i < group->getNumChildren(); i++ )
    {
        ...
        if( !_info.bbox.contains( lodCenter ) )
        {
            ...
            if( equalDoubles( lod_plus_oneSwitchInDistance, rangeList.at( 0 ).first ) &&
                equalDoubles( lod0SwitchInDistance, rangeList.at( 0 ).second ) )
            {
                if( loRes == 0 )
                {
                    loRes        = lod;
                ...
                if( rangeList.at( 0 ).first ==
                    0.0 &&
                    equalDoubles( lod_plus_oneSwitchInDistance,
                                  rangeList.at( 0 ).second ) )
                {
                    if( hiRes == 0 )
                    {
                        hiRes        = lod;
...
osg::Node*
ReaderWriterTXP::getTileContent( const TXPArchive::TileInfo&                info,
                                 int                                        x,
                                 int                                        y,
                                 int                                        lod,
                                 TXPArchive*                                archive,
                                 std::vector<TXPArchive::TileLocationInfo>& childrenLoc )
{
    ...
    double      usedMaxRange = std::max( info.maxRange, 1E7 );
    osg::vec3   tileCenter;
    osg::Group* tileGroup = archive->getTileContent( x,
                                                     y,
                                                     lod,
                                                     realMinRange,
                                                     realMaxRange,
    ...
    // if the node is a transform, then stop processing so as to not loose the
    // transformation
    while( tileGroup &&
           !tileGroup->asTransform() &&
           tileGroup->getNumChildren() ==
           1 &&
           tileGroup->getChild( 0 )->asGroup() )
    {
                                 ...
                                 std::vector<TXPArchive::TileLocationInfo>& childrenLoc )
{
    ...
    double      usedMaxRange = std::max( info.maxRange, 1E7 );
    osg::vec3   tileCenter;
    osg::Group* tileGroup = archive->getTileContent( loc,
                                                     realMinRange,
                                                     realMaxRange,
                                                     usedMaxRange,
                                                     tileCenter,
