...
/* Set up the callbacks for the scene graph parser.
    ...
    a switch statement.
 */
trpgPrintGraphParser::trpgPrintGraphParser( trpgr_Archive*    inArch,
                                            trpgrImageHelper* inImg,
 ...
 */
bool
trpgPrintGraphParser::StartChildren( void* )
{
 ...
 */
bool
trpgPrintGraphParser::EndChildren( void* )
{
...
unsigned int
trpgPrintGraphParser::GetNbChildrenRef() const
{
    if( childRefCB )
    {
...
const trpgChildRef*
trpgPrintGraphParser::GetChildRef( unsigned int idx ) const
{
    if( childRefCB )
    {
...
void
trpgPrintGraphParser::ReadHelper::Reset()
{
    childRefList.clear();
}

unsigned int
trpgPrintGraphParser::ReadHelper::GetNbChildrenRef() const
{
    return childRefList.size();
}

const trpgChildRef*
trpgPrintGraphParser::ReadHelper::GetChildRef( unsigned int idx ) const
{
 ...
 */
void*
trpgPrintGraphParser::ReadHelper::Parse( trpgToken       tok,
                                         trpgReadBuffer& buf )
{
        ...
        if( tok == TRPGTILEHEADER )
        {
            ...
            for( int i = 0; i < numMat; i++ )
            {
                ...
                for( int imgN = 0; imgN < numImages; imgN++ )
                {
                    ...
                    parse_imageHelp->GetNthImageInfoForLocalMat( &locMat,
                                                                 imgN,
                                                                 &baseMat,
                                                                 &baseTex,
                                                                 totSize );
                    ...
                    // Fetch the whole image
                    {
                        ...
                        try
                        {
                            failed = !parse_imageHelp->GetNthImageForLocalMat( &locMat,
                                                                               imgN,
                                                                               pixels,
                                                                               totSize );
                        }
                        ...
                        {
                            fprintf( stderr,
                                     "Failed to read local image %d from local material "
                                     "%d.\n",
                                     imgN,
                                     i );
                        }
                        ...
                        {
                            fprintf( stderr,
                                     "Read local image %d from local material %d "
                                     "successfully.\n",
                                     imgN,
                                     i );
                        }
                    ...
                    // Fetch the individual mipmap levels
                    {
                        ...
                        {
                            // int mipOffset = (const_cast<trpgTexture
                            // *>(baseTex))->MipLevelOffset(j);
                            int mipSize = ( const_cast<trpgTexture*>( baseTex ) )
                                              ->MipLevelSize( j );
                            if( mipSize )
                            {
                                ...
                                {
                                    failed =
                                        !parse_imageHelp->GetNthImageMipLevelForLocalMat(
                                            j,
                                            &locMat,
                                            imgN,
                                            pixels,
                                ...
                                {
                                    fprintf( stderr,
                                             "Failed to read mipmap level %d for local "
                                             "image %d from local material %d.\n",
                                             j,
                                             imgN,
                                             i );
                                }
                                ...
                                {
                                    fprintf( stderr,
                                             "Read mipmap level %d for local image %d "
                                             "from local material %d.\n",
                                             j,
                                             imgN,
                                             i );
                                }
 ...
 */
bool
trpgPrintArchive( char*            filename,
                  trpgPrintBuffer& pBuf,
                  int              flags )
{
    trpgr_Archive archive;
    ...
    char          drive[100], dir[1'024], fname[1'024], ext[1'024];
    _splitpath( filename, drive, dir, fname, ext );

    char rname[1'024], baseDir[1'024];
...
bool
trpgPrintArchive( trpgr_Archive*   archive,
                  trpgPrintBuffer& pBuf,
                  int              flags )
{
    char ls[1'024];
    ...
    trpgrImageHelper* imageHelp =
        archive->GetNewRImageHelper( archive->GetEndian(),
                                     archive->getDir(),
                                     *archive->GetMaterialTable(),
                                     *archive->GetTexTable() );
                    ...
                    if( archive->ReadTile( x, y, nl, buf ) )
                    {
                        if( flags & TRPGPRN_BODY )
                        {
                            ...
                            if( !parser.Parse( buf ) )
                            {
                                char errString[80];
                                sprintf( errString,
                                         "**** Warning: tile anomaly detected: (%d) "
                                         "(%d,%d) ****",
                                         nl,
                                         x,
                                         y );
