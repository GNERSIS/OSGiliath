...
bool
trpgManagedTile::ParseTileHeader( trpgReadBuffer& buf )
{
    isLoaded = false;
...
bool
trpgManagedTile::IsLoaded()
{
    return isLoaded;
}

bool
trpgManagedTile::SetTileLoc( int inX,
                             int inY,
                             int inLod )
{
    location.x = inX;
...
bool
trpgManagedTile::GetTileLoc( int& retx,
                             int& rety,
                             int& retLod ) const
{
    retx   = location.x;
...
void
trpgManagedTile::SetTileAddress( const trpgwAppAddress& gAddr )
{
    location.addr = gAddr;
}

void
trpgManagedTile::SetTileAddress( int32 file,
                                 int32 offset )
{
...
trpgManagedTile::GetTileAddress() const
{
    return location.addr;
}

bool
trpgManagedTile::SetChildLocationInfo( int                    childIdx,
                                       int                    x,
                                       int                    y,
...
trpgManagedTile::GetChildLocationInfo( int childIdx ) const
{
...
bool
trpgManagedTile::GetChildTileLoc( int  childIdx,
                                  int& x,
                                  int& y,
                                  int& lod ) const
{
    ...
    const TileLocationInfo& info = childLocationInfo[childIdx];
...
const trpgwAppAddress&
trpgManagedTile::GetChildTileAddress( int childIdx ) const
{
    if( childIdx < 0 || childIdx >= ( int )childLocationInfo.size() )
    {
        throw std::invalid_argument(
...
const trpgTileHeader*
trpgManagedTile::GetTileHead()
{
    return &tileHead;
}

const std::vector<trpgLocalMaterial>*
trpgManagedTile::GetLocMatList() const
{
    return tileHead.GetLocalMaterialList();
}

const trpgLocalMaterial*
trpgManagedTile::GetLocMaterial( int id ) const
{
...
void
trpgManagedTile::SetLocalData( void* inData )
{
    localData = inData;
}

void*
trpgManagedTile::GetLocalData() const
{
    return localData;
}

bool
trpgManagedTile::SetMatData( int   id,
                             void* info )
{
...
void*
trpgManagedTile::GetMatData( int id ) const
{
    if( id < 0 || id >= ( int )localMatData.size() )
    {
        return NULL;
...
void
trpgManagedTile::AddGroupID( int id )
{
    groupIDs.push_back( id );
}

const std::vector<int>*
trpgManagedTile::GetGroupIDs() const
{
    return &groupIDs;
}

void
trpgManagedTile::Print( trpgPrintBuffer& buf )
{
    char line[1'024];
    sprintf( line, "x = %d, y = %d, lod = %d", location.x, location.y, location.lod );
    buf.prnLine( line );
...
void
trpgPageManager::LodPageInfo::Clean()
{
...
bool
trpgPageManager::LodPageInfo::Init( trpgr_Archive* archive,
                                    int            myLod,
                                    double         scale,
                                    int            freeListDivider )
{
...
bool
trpgPageManager::LodPageInfo::SetLocation( trpg2dPoint& loc )
{
...
trpgManagedTile*
trpgPageManager::LodPageInfo::GetNextLoad()
{
...
void
trpgPageManager::LodPageInfo::AckLoad()
{
    if( activeLoad )
    {
...
trpgManagedTile*
trpgPageManager::LodPageInfo::GetNextUnload()
{
...
void
trpgPageManager::LodPageInfo::AckUnload()
{
    if( activeUnload )
    {
...
bool
trpgPageManager::LodPageInfo::isWithin( trpgManagedTile* tile,
                                        trpg2iPoint&     sw,
                                        trpg2iPoint&     ne )
{
    int tileX, tileY, tileLod;
    tile->GetTileLoc( tileX, tileY, tileLod );
    if( tileX >= sw.x && tileX <= ne.x && tileY >= sw.y && tileY <= ne.y )
    {
        return true;
    }
    else
...
bool
trpgPageManager::LodPageInfo::Stop()
{
    // Empty the load list
    unsigned int i;
    for( i = 0; i < load.size(); i++ )
...
void
trpgPageManager::LodPageInfo::AddChildrenToLoadList(
...
)
{
    ...
    for( unsigned int parentIdx = 0; parentIdx < parentList.size(); ++parentIdx )
    {
        ...
        for( unsigned int childIdx = 0; childIdx < nbChildren; ++childIdx )
        {
            ...
            const TileLocationInfo& childLoc =
                parentTile->GetChildLocationInfo( childIdx );
...
bool
trpgPageManager::LodPageInfo::AddToLoadList( int                    x,
                                             int                    y,
                                             const trpgwAppAddress& addr )
{
    trpg2iPoint sw, ne;
    ...
    if( x >= sw.x && x <= ne.x && y >= sw.y && y <= ne.y )
    {
...
void
trpgPageManager::LodPageInfo::Update()
{
    trpg2iPoint sw, ne;
        ...
        for( int x = 0; x < dx; x++ )
        {
            for( int y = 0; y < dy; y++ )
            {
                if( !tmpCurrent[y * dx + x] )
                {
                    ...
                    if( tileTable &&
                        tileTable->GetTile( x + sw.x, y + sw.y, lod, addr, zmin, zmax ) )
                    {
                        tile->SetTileAddress( addr );
                    }
...
void
trpgPageManager::LodPageInfo::GetLoadedTileWithin(
    double                         pagingDistance,
...
void
trpgPageManager::LodPageInfo::Print( trpgPrintBuffer& buf )
{
    char         line[1'024];
    unsigned int i;

    sprintf( line, "lod = %d,  valid = %s", lod, valid ? "yes" : "no" );
    buf.prnLine( line );
    sprintf( line, "pageDist = %f,  maxNumTiles = %d", pageDist, maxNumTiles );
    buf.prnLine( line );
    sprintf( line, "cellSize = (%f,%f)", cellSize.x, cellSize.y );
    buf.prnLine( line );
    sprintf( line,
             "cell = (%d,%d),  aoiSize = (%d,%d),  lodSize = (%d,%d)",
             cell.x,
             cell.y,
             aoiSize.x,
             aoiSize.y,
    ...
    sprintf( line, "Loads:  (activeLoad = %s)", activeLoad ? "yes" : "no" );
    buf.prnLine( line );
    buf.IncreaseIndent();
    for( i = 0; i < load.size(); i++ )
    {
    ...
    sprintf( line, "Unloads:  (activeUnload = %s)", activeUnload ? "yes" : "no" );
    buf.prnLine( line );
    buf.IncreaseIndent();
    for( i = 0; i < unload.size(); i++ )
    {
    ...
    sprintf( line, "Free list size = %d", ( int )freeList.size() );
    buf.prnLine( line );
}
...
void
trpgPageManager::Init( trpgr_Archive* inArch )
{
    archive = inArch;
...
bool
trpgPageManager::SetPageDistFactor( double inFact )
{
...
bool
trpgPageManager::SetLocation( trpg2dPoint& pt )
{
...
void
trpgPageManager::AckLoad( const std::vector<TileLocationInfo>& children )
{
    ...
    if( majorVersion >= 2 && minorVersion >= 1 )
    {
        if( children.size() > 0 )
        {
            ...
            {
                const TileLocationInfo& childLocInfo = children[idx];
                if( childLocInfo.lod != lastLod + 1 )
                {
                    continue;    // Something wrong here
                }
                childInfo.AddToLoadList( childLocInfo.x,
                                         childLocInfo.y,
                                         childLocInfo.addr );
...
void
trpgPageManager::AddGroupID( trpgManagedTile* tile,
                             int              groupID,
                             void*            data )
{
...
void*
trpgPageManager::GetGroupData( int groupID )
{
...
void
trpgPageManager::Print( trpgPrintBuffer& buf )
{
    char line[1'024];
    sprintf( line, "Paging pos = (%f,%f),  scale = %f", pagePt.x, pagePt.y, scale );
    buf.prnLine( line );
    buf.prnLine( "Terrain LODs:" );

    for( unsigned int i = 0; i < pageInfo.size(); i++ )
    {
        sprintf( line, "----Terrain lod %d---", i );
        buf.prnLine( line );
        buf.IncreaseIndent();
...
void*
trpgr_ChildRefCB::Parse( trpgToken       tok,
                         trpgReadBuffer& rbuf )
{
...
void
trpgr_ChildRefCB::Reset()
{
    childList.clear();
}

unsigned int
trpgr_ChildRefCB::GetNbChildren() const
{
    return childList.size();
}

const trpgChildRef&
trpgr_ChildRefCB::GetChildRef( unsigned int idx ) const
{
...
void
trpgPageManageTester::Init( trpgPrintBuffer* pBuf,
                            trpgPageManager* pMan,
                            trpgr_Archive*   inArch )
{
    archive  = inArch;
    manager  = pMan;
    printBuf = pBuf;
...
void
trpgPageManageTester::RandomTest( int num,
                                  int seed )
{
    ...
    for( i = 0; i < num; i++ )
    {
        ...
        sprintf( line,
                 "Jumped to (%f,%f).  Tiles to load/unload = %s",
                 pt.x,
                 pt.y,
                 changes ? "yes" : "no" );
        printBuf->prnLine( line );
...
void
trpgPageManageTester::Fly_LL_to_UR( double dist )
{
    char line[1'024];
    ...
    do
    {
        ...
        sprintf( line,
                 "Moved to (%f,%f).  Tiles to load/unload = %s",
                 loc.x,
                 loc.y,
                 changes ? "yes" : "no" );
        printBuf->prnLine( line );
...
void
trpgPageManageTester::ProcessChanges()
{
    char             line[1'024];
    int              x, y, lod;
    ...
    while( ( unloadTile = manager->GetNextUnload() ) )
    {
        ...
        sprintf( line, "x = %d, y = %d, lod = %d", x, y, lod );
        printBuf->prnLine( line );
        manager->AckUnload();
    }
