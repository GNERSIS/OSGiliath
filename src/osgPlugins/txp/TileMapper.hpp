 ...
 * Provides: _containsGeode, getEyePoint, getEyeLocal, getDistanceToEyePoint,
 * getDistanceFromEyePoint, apply.
 */
...
namespace txp
{
    ...
    struct TileIdentifier : public osg::Referenced
    {
            ...
            TileIdentifier&
            operator=( const TileIdentifier& rhs )
            {
                if( this == &rhs )
                {
                    return *this;
            ...
            bool
            operator<( const TileIdentifier& rhs ) const
            {
                if( lod < rhs.lod )
                {
                    return true;
    ...
    class TileMapper : public osg::DualModeVisitor,
                       public osg::CullStack
    {
        public:
            ...
            void
            insertTile( const TileIdentifier& tid );

            bool
            isTileNeighbourALowerLODLevel( const TileIdentifier& tid,
                                           int                   dx,
                                           int                   dy ) const;
