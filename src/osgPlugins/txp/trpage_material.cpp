...
// Set functions
void
trpgMatTable::SetNumTable( int no )
{
    if( ( no < 0 ) || ( no == numTable ) )
    {
        return;
...
void
trpgMatTable::SetNumMaterial( int /*no*/ )
{
...
void
trpgMatTable::SetMaterial( int                 nm,
...
int
trpgMatTable::AddMaterial( const trpgMaterial& mat,
                           bool                lookForExisting )
{
    ...
    // work fine, until I realized that evidently most of the time the shademodel isn't
    // set at all.  Now my kludge takes so much work it's almost worth doing it right.
    ...
    if( lookForExisting )
    {
        ...
        for( ; itr != materialMap.end(); itr++ )
        {
            ...
            // Compare structures
            if( CEQ( cmat.color, bm.color ) &&
                CEQ( cmat.ambient, bm.ambient ) &&
                CEQ( cmat.diffuse, bm.diffuse ) &&
                CEQ( cmat.specular, bm.specular ) &&
                CEQ( cmat.emission, bm.emission ) &&
                cmat.shininess ==
                bm.shininess &&
                cmat.shadeModel ==
                bm.shadeModel &&
                ...
                cmat.autoNormal ==
                bm.autoNormal &&
                cmat.texEnvs.size() == bm.texEnvs.size() )
            {
...
bool
trpgMatTable::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
// Get functions
bool
trpgMatTable::GetNumTable( int& no ) const
{
    if( !isValid() )
    {
...
bool
trpgMatTable::GetNumMaterial( int& no ) const
{
    if( !isValid() )
    {
        no = 0;
...
bool
trpgMatTable::GetMaterial( int           nt,
                           int           nm,
                           trpgMaterial& mat ) const
{
    if( !isValid() )
    {
        return false;
...
const trpgMaterial*
trpgMatTable::GetMaterialRef( int nt,
                              int nm ) const
{
    MaterialMapType::const_iterator itr = materialMap.find( ( nt * numMat ) + nm );
    if( itr == materialMap.end() )
...
bool
trpgMatTable::Read( trpgReadBuffer& buf )
{
    trpgMaterial mat;
    trpgToken    matTok;
    int32        len;
...
// Set functions
void
trpgTextureEnv::SetEnvMode( int mode )
{
    envMode = mode;
}

void
trpgTextureEnv::SetMinFilter( int fl )
{
    minFilter = fl;
}

void
trpgTextureEnv::SetMagFilter( int fl )
{
    magFilter = fl;
}

void
trpgTextureEnv::SetWrap( int s,
                         int t )
{
    wrapS = s;
    wrapT = t;
...
void
trpgTextureEnv::SetBorderColor( const trpgColor& col )
{
    borderCol = col;
}
...
// Write function
bool
trpgTextureEnv::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
// Get functions
bool
trpgTextureEnv::GetEnvMode( int32& ret ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTextureEnv::GetMinFilter( int32& ret ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTextureEnv::GetMagFilter( int32& ret ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgTextureEnv::GetBorderColor( trpgColor& col ) const
{
    if( !isValid() )
    {
        return false;
...
void*
textureEnvCB::Parse( trpgToken       tok,
                     trpgReadBuffer& buf )
{
    int       envMode;
    int       minFilter;
...
bool
trpgTextureEnv::Read( trpgReadBuffer& buf )
{
    trpgr_Parser parse;
    textureEnvCB teCb;
...
// Set functions
void
trpgMaterial::SetColor( const trpgColor& col )
{
    color = col;
}

void
trpgMaterial::SetAmbient( const trpgColor& col )
{
    ambient = col;
}

void
trpgMaterial::SetDiffuse( const trpgColor& col )
{
    diffuse = col;
}

void
trpgMaterial::SetSpecular( const trpgColor& col )
{
    specular = col;
}

void
trpgMaterial::SetEmission( const trpgColor& col )
{
    emission = col;
}

void
trpgMaterial::SetShininess( float64 val )
{
    shininess = val;
}

void
trpgMaterial::SetShadeModel( int val )
{
    shadeModel = val;
}

void
trpgMaterial::SetPointSize( float64 val )
{
    pointSize = val;
}

void
trpgMaterial::SetLineWidth( float64 val )
{
    lineWidth = val;
}

void
trpgMaterial::SetCullMode( int val )
{
    cullMode = val;
}

void
trpgMaterial::SetAlphaFunc( int val )
{
    alphaFunc = val;
}

void
trpgMaterial::SetAlphaRef( float64 val )
{
    alphaRef = val;
}

void
trpgMaterial::SetAlpha( float64 val )
{
    alpha = val;
}

void
trpgMaterial::SetAutoNormal( bool val )
{
    autoNormal = val;
}

void
trpgMaterial::SetNumTexture( int no )
{
    if( no < 0 )
    {
        return;
...
void
trpgMaterial::SetTexture( int                   no,
                          int                   id,
                          const trpgTextureEnv& env )
{
...
int
trpgMaterial::AddTexture( int                   id,
                          const trpgTextureEnv& env )
{
    texids.push_back( id );
...
void
trpgMaterial::SetNumTiles( int no )
{
    numTile = no;
}

int
trpgMaterial::AddTile()
{
    return ( ++numTile );
}

void
trpgMaterial::SetIsBumpMap( bool val )
{
    isBump = val;
}

void
trpgMaterial::SetAttr( int attrCode,
                       int val )
{
    switch( attrCode )
    {
...
// Write to buffer
bool
trpgMaterial::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
// Get methods
bool
trpgMaterial::GetColor( trpgColor& col ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetAmbient( trpgColor& col ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetDiffuse( trpgColor& col ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetSpecular( trpgColor& col ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetEmission( trpgColor& col ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetShininess( float64& shin ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetShadeModel( int& sm ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetPointSize( float64& ps ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetLineWidth( float64& lw ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetCullMode( int32& cull ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetAlphaFunc( int& af ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetAlphaRef( float64& ar ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetAlpha( float64& a ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetAutoNormal( bool& an ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetNumTexture( int& no ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetTexture( int             no,
                          int&            id,
                          trpgTextureEnv& te ) const
{
...
bool
trpgMaterial::GetNumTile( int& no ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetIsBumpMap( bool& ret ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgMaterial::GetAttr( int  attrCode,
                       int& ret ) const
{
    switch( attrCode )
    {
...
void*
materialCB::Parse( trpgToken       tok,
                   trpgReadBuffer& buf )
{
    trpgColor      color;
    float64        shininess;
...
bool
trpgMaterial::Read( trpgReadBuffer& buf )
{
    trpgr_Parser parse;
    materialCB   matCb;
...
// Set Name
void
trpgTexture::SetName( const char* inName )
{
    if( name )
    {
        delete[] name;
...
// Get Name
bool
trpgTexture::GetName( char* outName,
                      int   outLen ) const
{
    if( !isValid() )
    {
...
void
trpgTexture::SetImageMode( ImageMode inMode )
{
    mode = inMode;
}

bool
trpgTexture::GetImageMode( ImageMode& outMode ) const
{
    outMode = mode;
...
void
trpgTexture::SetImageType( ImageType inType )
{
    type = inType;
}

bool
trpgTexture::GetImageType( ImageType& outType ) const
{
    outType = type;
...
void
trpgTexture::SetImageSize( const trpg2iPoint& inSize )
{
    sizeX = inSize.x;
    sizeY = inSize.y;
}
...
bool
trpgTexture::GetImageSize( trpg2iPoint& outSize ) const
{
...
void
trpgTexture::SetIsMipmap( bool val )
{
    isMipmap = val;
}

bool
trpgTexture::GetIsMipmap( bool& ret ) const
{
    ret = isMipmap;
    return true;
}
...
bool
trpgTexture::GetImageAddr( trpgwAppAddress& outAddr ) const
{
    if( mode != Local )
    {
        return false;
...
void
trpgTexture::SetImageAddr( const trpgwAppAddress& inAddr )
{
    addr = inAddr;
}

bool
trpgTexture::GetImageDepth( int32& depth ) const
{
    switch( type )
    {
        case trpg_RGB8 :
...
void
trpgTexture::SetNumLayer( int layers )
{
    numLayer = layers;
}

bool
trpgTexture::GetNumLayer( int& layers ) const
{
    if( !isValid() )
    {
        return false;
...
void
trpgTexture::SetNumTile( int num )
{
    useCount = num;
}
...
bool
trpgTexture::GetNumTile( int& num ) const
{
    if( !isValid() )
    {
        return false;
...
// Copy operator
trpgTexture&
trpgTexture::operator=( const trpgTexture& in )
{
    mode = in.mode;
    type = in.type;
...
// Equality operator
int
trpgTexture::operator==( const trpgTexture& in ) const
{
    if( mode != in.mode )
    {
        return 0;
...
int32
trpgTexture::CalcNumMipmaps() const
{
...
int32
trpgTexture::CalcTotalSize() const
{
...
int32
trpgTexture::MipLevelSize( int miplevel )
{
...
int32
trpgTexture::MipLevelOffset( int miplevel )
{
    if( miplevel > 0 && miplevel < CalcNumMipmaps() )
    {
        if( !levelOffset.size() )
...
// Write function
bool
trpgTexture::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
// Read function
bool
trpgTexture::Read( trpgReadBuffer& buf )
{
    char texName[1'024];

    try
    {
        buf.Get( texName, 1'023 );
        SetName( texName );
        buf.Get( useCount );
        ...
        unsigned char bval;
        buf.Get( bval );
        mode = ( trpgTexture::ImageMode )bval;
        buf.Get( bval );
        type = ( trpgTexture::ImageType )bval;
...
// Set functions
void
trpgTexTable::SetNumTextures( int /*no*/ )
{
    // obsolete. This method doesn't need to do anything since we're using a map instead
    // of a vector.
    //    texList.resize(no);
}

int
trpgTexTable::AddTexture( const trpgTexture& inTex )
{

    TeAttrHdl hdl = inTex.GetHandle();
    if( hdl == -1 )
    {
        // if no handle is specified, we will use an index as the handle (just like
        // before 2.3)
        hdl = textureMap.size();
    }
...
int
trpgTexTable::FindAddTexture( const trpgTexture& inTex )
{
...
void
trpgTexTable::SetTexture( int                id,
                          const trpgTexture& inTex )
{
    if( id < 0 )
    {
        return;
...
// Copy operator
trpgTexTable&
trpgTexTable::operator=( const trpgTexTable& in )
{
    Reset();
...
// Write Texture table
bool
trpgTexTable::Write( trpgWriteBuffer& buf )
{
    int32 numTex;
...
// Get functions
bool
trpgTexTable::GetNumTextures( int& no ) const
{
    no = textureMap.size();
    if( !isValid() )
    {
...
bool
trpgTexTable::GetTexture( int          id,
                          trpgTexture& ret ) const
{
    if( !isValid() )
    {
        return false;
...
const trpgTexture*
trpgTexTable::GetTextureRef( int id ) const
{
    if( id < 0 )
    {
        return 0;
...
const trpgTexture*
trpgTexTable::FindByName( const char* name,
                          int&        texid ) const
{
    TextureMapType::const_iterator itr = textureMap.begin();
    for( ; itr != textureMap.end(); itr++ )
    {
        char thisName[1'024];
        thisName[0] = '\0';
        itr->second.GetName( thisName, 1'023 );
...
bool
trpgTexTable::Read( trpgReadBuffer& buf )
{
    int32     numTex;
    trpgToken texTok;
    int32     len;
...
void
trpgLocalMaterial::SetBaseMaterial( int32 subTable,
                                    int32 inMatID )
{
...
bool
trpgLocalMaterial::GetBaseMaterial( int32& subTable,
                                    int32& matID ) const
{
    if( !isValid() )
    {
...
void
trpgLocalMaterial::SetSubImageInfo( const SubImageInfo& info )
{
    sx         = info.sx;
    sy         = info.sy;
    ex         = info.ex;
...
bool
trpgLocalMaterial::GetSubImageInfo( SubImageInfo& info ) const
{
    if( !isValid() )
    {
        return false;
...
void
trpgLocalMaterial::SetAddr( const trpgwAppAddress& inAddr )
{
    addr[0] = inAddr;
}

void
trpgLocalMaterial::SetNthAddr( unsigned int           subtable,
                               const trpgwAppAddress& inAddr )
{
...
bool
trpgLocalMaterial::GetAddr( trpgwAppAddress& inAddr ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgLocalMaterial::GetNthAddr( unsigned int     subtable,
                               trpgwAppAddress& inAddr ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgLocalMaterial::GetNumLocals( int& numLocals ) const
{
    if( !isValid() )
    {
        return false;
...
bool
trpgLocalMaterial::isValid() const
{
    if( baseMat < 0 )
    {
        return false;
...
bool
trpgLocalMaterial::Write( trpgWriteBuffer& buf )
{
    if( !isValid() )
    {
        return false;
...
bool
trpgLocalMaterial::Read( trpgReadBuffer& buf )
{
    try
    {
