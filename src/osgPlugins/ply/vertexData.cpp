...
void
VertexData::readVertices( PlyFile*  file,
                          const int nVertices,
                          const int fields )
{
    ...
    PlyProperty vertexProps[] = {
        ...
        {   "ambient_red",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,    ambient_red ),
         0, 0,
         0, 0                                                                                     },
        { "ambient_green",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,  ambient_green ),
         0, 0,
         0, 0                                                                                     },
        {  "ambient_blue",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,   ambient_blue ),
         0, 0,
         0, 0                                                                                     },
        {   "diffuse_red",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,    diffuse_red ),
         0, 0,
         0, 0                                                                                     },
        { "diffuse_green",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,  diffuse_green ),
         0, 0,
         0, 0                                                                                     },
        {  "diffuse_blue",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,   diffuse_blue ),
         0, 0,
         0, 0                                                                                     },
        {  "specular_red",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,   specular_red ),
         0, 0,
         0, 0                                                                                     },
        {"specular_green",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex, specular_green ),
         0, 0,
         0, 0                                                                                     },
        { "specular_blue",
         PLY_UCHAR, PLY_UCHAR,
         offsetof( _Vertex,  specular_blue ),
         0, 0,
         0, 0                                                                                     },
        {"specular_coeff",
         PLY_FLOAT, PLY_FLOAT,
         offsetof( _Vertex, specular_coeff ),
         0, 0,
         0, 0                                                                                     },
        {"specular_power",
         PLY_FLOAT, PLY_FLOAT,
         offsetof( _Vertex, specular_power ),
         0, 0,
         0, 0                                                                                     },
        {     "texture_u",
         PLY_FLOAT, PLY_FLOAT,
         offsetof( _Vertex,      texture_u ),
         0, 0,
         0, 0                                                                                     },
        {     "texture_v",
         PLY_FLOAT, PLY_FLOAT,
         offsetof( _Vertex,      texture_v ),
         0, 0,
         0, 0                                                                                     },
    };
    ...
    for( int i = 0; i < nVertices; ++i )
    {
        ...
        if( fields & AMBIENT )
        {
            _ambient->push_back( osg::vec4( ( unsigned int )vertex.ambient_red / 255.0,
                                            ...
                                            ( unsigned int )vertex.ambient_blue / 255.0,
                                            1.0 ) );
        }
        ...
        if( fields & DIFFUSE )
        {
            _diffuse->push_back( osg::vec4( ( unsigned int )vertex.diffuse_red / 255.0,
                                            ...
                                            ( unsigned int )vertex.diffuse_blue / 255.0,
                                            1.0 ) );
        }
        ...
        {
            _specular->push_back(
                osg::vec4( ( unsigned int )vertex.specular_red / 255.0,
                           ( unsigned int )vertex.specular_green / 255.0,
                           ( unsigned int )vertex.specular_blue / 255.0,
                           1.0 )
            );
        }
        if( fields & TEXCOORD )
...
void
VertexData::readTriangles( PlyFile*  file,
                           const int nFaces )
{
    ...
    // Quads are decomposed into triangles for Core Profile compatibility
    if( !_quads.valid() )
    {
        _quads = new osg::DrawElementsUInt( osg::PrimitiveSet::TRIANGLES );
    }
    ...
    for( int i = 0; i < nFaces; i++ )
    {
        ...
        if( face.vertices )
        {
            if( face.nVertices ==
                ...
                face.nVertices == NUM_VERTICES_QUAD )
            {
                if( face.nVertices == 4 )
                {
                    // Decompose quad into two triangles: (0,1,3) + (1,2,3)
                    unsigned short i0 = ( _invertFaces ? 3 : 0 );
                    unsigned short i1 = ( _invertFaces ? 2 : 1 );
                    unsigned short i2 = ( _invertFaces ? 1 : 2 );
                    unsigned short i3 = ( _invertFaces ? 0 : 3 );
                    _quads->push_back( face.vertices[i0] );
                    _quads->push_back( face.vertices[i1] );
                    _quads->push_back( face.vertices[i3] );
                    _quads->push_back( face.vertices[i1] );
                    _quads->push_back( face.vertices[i2] );
                    _quads->push_back( face.vertices[i3] );
                }
                else
                {
...
osg::Node*
VertexData::readPlyFile( const char* filename,
                         const bool  ignoreColors )
{
    int      nPlyElems;
    char**   elemNames;
    int      fileType;
    ...
    {
        MESHERROR << "Unable to read PLY file, an exception occurred:  " << e.what()
                  << endl;
    }

    if( !file )
    {
        MESHERROR << "Unable to open PLY file " << filename << " for reading." << endl;
        return NULL;
    }
...
#ifndef NDEBUG
    MESHINFO << filename << ": " << nPlyElems << " elements, file type = " << fileType
    ...
    for( int i = 0; i < nComments; i++ )
    {
        ...
        if( strncmp( comments[i], "TextureFile", 11 ) == 0 )
        {
            ...
            {
                textureFile =
                    osgDB::concatPaths( osgDB::getFilePath( filename ), textureFile );
            }
        }
    }
    for( int i = 0; i < nPlyElems; ++i )
    {
        ...
        try
        {
            props = ply_get_element_description( file, elemNames[i], &nElems, &nProps );
        }
        catch( exception& e )
        {
...
#ifndef NDEBUG
        MESHINFO << "element " << i << ": name = " << elemNames[i] << ", " << nProps
                 << " properties, " << nElems << " elements" << endl;
        ...
        if( equal_strings( elemNames[i], "vertex" ) )
        {
            ...
            {
                MESHERROR
                    << "Unable to read vertex in PLY file, an exception occurred:  "
                    << e.what() << endl;
        ...
        else if( equal_strings( elemNames[i], "face" ) )
        {
            try
            {
                ...
                unsigned int nbTriangles =
                    ( _triangles.valid() ? _triangles->size() / 3 : 0 );
    ...
    if( result )
    {
        ...
        if( !hasTriOrQuads )
        {
            geom->addPrimitiveSet(
                new osg::DrawArrays( GL_POINTS, 0, _vertices->size() )
            );
        }
        ...
        if( !textureFile.empty() &&
            ( image = osgDB::readRefImageFile( textureFile ) ) != NULL )
        {
