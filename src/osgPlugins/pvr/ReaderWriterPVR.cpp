...
class ReaderWriterPVR : public osgDB::ReaderWriter
{
    public:
        ...
        ReadResult
        readPVRStream( std::istream& fin ) const
        {
            PVRTexHeader header;
            ...
            if( gPVRTexIdentifier[0] !=
                static_cast<char>( ( header.pvrTag >> 0 ) & 0XFF ) ||
                gPVRTexIdentifier[1] !=
                static_cast<char>( ( header.pvrTag >> 8 ) & 0XFF ) ||
                gPVRTexIdentifier[2] !=
                static_cast<char>( ( header.pvrTag >> 16 ) & 0XFF ) ||
                gPVRTexIdentifier[3] !=
                static_cast<char>( ( header.pvrTag >> 24 ) & 0XFF ) )
            {
                osg::notify( osg::WARN )
                    << "Failed to verify pvr header: "
                    << ( ( header.pvrTag >> 0 ) & 0XFF ) << ", "
                    << ( ( header.pvrTag >> 8 ) & 0XFF ) << ", "
                    << ( ( header.pvrTag >> 16 ) & 0XFF ) << ", "
                    << ( ( header.pvrTag >> 24 ) & 0XFF ) << std::endl;
            ...
            if( formatFlags ==
                ...
                kPVRTextureFlagTypeOGLPVRTC_4 ||
                formatFlags ==
                kPVRTextureFlagTypeOGLPVRTC_2 ||
                formatFlags == kPVRTextureFlagTypeETC )
            {
                if( formatFlags ==
                    kPVRTextureFlagTypePVRTC_4 ||
                    formatFlags == kPVRTextureFlagTypeOGLPVRTC_4 )
                {
                    internalFormat = GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG;
                }
                else if( formatFlags ==
                         kPVRTextureFlagTypePVRTC_2 ||
                         formatFlags == kPVRTextureFlagTypeOGLPVRTC_2 )
                {
                ...
                while( dataOffset < header.dataLength )
                {
                    ...
                    dataOffset +=
                        widthBlocks * heightBlocks * ( ( blockSize * bpp ) / 8 );
