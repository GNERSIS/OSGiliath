 ...
 * ReaderWriterProducerCFG, derived from ReaderWriter.
...
static osg::GraphicsContext::Traits*
buildTrait( RenderSurface& rs )
{
         ...
         it++ )
    {
        ...
        {
            case( VisualChooser::UseGL ) :
                break;    // on by default in osgViewer
            case( VisualChooser::BufferSize ) :
                break;    // no present mapping
            case( VisualChooser::Level ) :
                traits->level = it->_parameter;
                break;
            case( VisualChooser::RGBA ) :
                break;    // automatically set in osgViewer
            case( VisualChooser::DoubleBuffer ) :
                traits->doubleBuffer = true;
                break;
            case( VisualChooser::Stereo ) :
                traits->quadBufferStereo = true;
                break;
            case( VisualChooser::AuxBuffers ) :
                break;    // no present mapping
            case( VisualChooser::RedSize ) :
                traits->red = it->_parameter;
                break;
            case( VisualChooser::GreenSize ) :
                traits->green = it->_parameter;
                break;
            case( VisualChooser::BlueSize ) :
                traits->blue = it->_parameter;
                break;
            case( VisualChooser::AlphaSize ) :
                traits->alpha = it->_parameter;
                break;
            case( VisualChooser::DepthSize ) :
                traits->depth = it->_parameter;
                break;
            case( VisualChooser::StencilSize ) :
                traits->stencil = it->_parameter;
                break;
            case( VisualChooser::AccumRedSize ) :
                break;    // no present mapping
            case( VisualChooser::AccumGreenSize ) :
                break;    // no present mapping
            case( VisualChooser::AccumBlueSize ) :
                break;    // no present mapping
            case( VisualChooser::AccumAlphaSize ) :
                break;    // no present mapping
            case( VisualChooser::Samples ) :
                traits->samples = it->_parameter;
                break;
            case( VisualChooser::SampleBuffers ) :
                traits->sampleBuffers = 1;
                break;
        }
    }

    OSG_INFO << "ReaderWriterCFG buildTrait traits->depth=" << traits->depth
             << std::endl;
    OSG_INFO << "ReaderWriterCFG buildTrait traits->samples=" << traits->samples
             << std::endl;
    OSG_INFO << "ReaderWriterCFG buildTrait traits->sampleBuffers="
             << traits->sampleBuffers << std::endl;
...
static osgViewer::View*
load( const std::string& file,
      const osgDB::ReaderWriter::Options* /*option*/ )
{
    ...
    if( config->getNumberOfCameras() == 1 )
    {
        ...
        if( gc.valid() )
        {
            ...
            camera->setProjectionMatrix( osg::dmat4( cm->getProjectionMatrix() ) );
            camera->setViewMatrix( osg::dmat4( cm->getPositionAndAttitudeMatrix() ) );

#if 0
            std::cout << "dmat4 Projection " << projection << std::endl;
            std::cout << "dmat4 Projection master " << _view->getCamera()->getProjectionMatrix() << std::endl;
            // will work only if it's a post multyply in the producer camera
            std::cout << "dmat4 View " << view << std::endl;
            std::cout << _view->getCamera()->getProjectionMatrix() * offsetProjection << std::endl;
#endif
        }
        else
        {
            OSG_INFO << "  GraphicsWindow has not been created successfully."
                     << std::endl;
            return 0;
        }
    }
    else
    {
        for( int i = 0; i < ( int )config->getNumberOfCameras(); i++ )
        {
            ...
            if( gc.valid() )
            {
                ...
                osg::dmat4 projection( cm->getProjectionMatrix() );

                osg::dmat4 offset = osg::dmat4();
                cm->setViewByMatrix( offset );
                osg::dmat4 view = osg::dmat4( cm->getPositionAndAttitudeMatrix() );

                // setup projection from parent
                osg::dmat4 offsetProjection =
                    osg::inverse( _view->getCamera()->getProjectionMatrix() ) *
                    projection;
...
class ReaderWriterProducerCFG : public osgDB::ReaderWriter
{
    public:
                    ...
                    const Options*     options = NULL ) const
        {
            ...
            if( options )
            {
                filePathList = const_cast<osgDB::FilePathList*>(
