 ...
 * Utility functions for osgPlugins: fullScreen, realize.
 */
...
const std::string RenderSurface::defaultWindowName =
    std::string( " *** Producer::RenderSurface *** " );

const unsigned int RenderSurface::UnknownDimension    = 0XFF'FF'FF'FF;
const unsigned int RenderSurface::UnknownAmount       = 0XFF'FF'FF'FF;
...
void
RenderSurface::shareAllGLContexts( bool flag )
{
    _shareAllGLContexts = flag;
}

bool
RenderSurface::allGLContextsAreShared()
{
    return _shareAllGLContexts;
}

const std::string&
RenderSurface::getDefaultWindowName()
{
...
RenderSurface::RenderSurface( void )
{
    ...
    {
        if( strcmp( override_envptr, "true" ) ==
            0 ||
            strcmp( override_envptr, "True" ) ==
            0 ||
            strcmp( override_envptr, "TRUE" ) == 0 )
        {
...
void
RenderSurface::setDrawableType( RenderSurface::DrawableType drawableType )
{
    if( _realized )
    {
...
RenderSurface::DrawableType
RenderSurface::getDrawableType()
{
    return _drawableType;
}

void
RenderSurface::setReadDrawable( osgProducer::RenderSurface* rs )
{
...
void
RenderSurface::setWindowName( const std::string& name )
{
    // _setWindowName( name );
    _windowName = name;
}
...
const std::string&
RenderSurface::getWindowName( void ) const
{
    return _windowName;
}

void
RenderSurface::setHostName( const std::string& name )
{
    _hostname = name;
}

const std::string&
RenderSurface::getHostName( void ) const
{
    return _hostname;
}
...
void
RenderSurface::setDisplayNum( int num )
{
    _displayNum = num;
}

int
RenderSurface::getDisplayNum( void ) const
{
    return _displayNum;
}

void
RenderSurface::setScreenNum( int num )
{
    _screen = num;
}

int
RenderSurface::getScreenNum( void ) const
{
    return _screen;
}

void
RenderSurface::setWindowRectangle( int          x,
                                   int          y,
                                   unsigned int width,
                                   unsigned int height,
                                   bool /*resize*/ )
{
    ...
    fullScreen(
        false
    );    // STH: this may break other implementations, but it is necessary for OSX
#else
    _isFullScreen = false;
#endif
#if 0
...
void
RenderSurface::getWindowRectangle( int&          x,
                                   int&          y,
                                   unsigned int& width,
...
void
RenderSurface::useDefaultFullScreenRectangle()
{
    _useCustomFullScreen = false;
}

void
RenderSurface::setCustomFullScreenRectangle( int          x,
                                             int          y,
                                             unsigned int width,
                                             unsigned int height )
{
...
int
RenderSurface::getWindowOriginX() const
{
    if( _isFullScreen )
    {
        if( _useCustomFullScreen == true )
...
int
RenderSurface::getWindowOriginY() const
{
    if( _isFullScreen )
    {
        if( _useCustomFullScreen == true )
...
unsigned int
RenderSurface::getWindowWidth() const
{
    if( _isFullScreen )
    {
        if( _useCustomFullScreen == true )
...
unsigned int
RenderSurface::getWindowHeight() const
{
    if( _isFullScreen )
    {
        if( _useCustomFullScreen == true )
...
void
RenderSurface::setInputRectangle( const InputRectangle& inputRectangle )
{
    _inputRectangle = inputRectangle;
}

const RenderSurface::InputRectangle&
RenderSurface::getInputRectangle() const
{
    return _inputRectangle;
}

void
RenderSurface::bindInputRectangleToWindowSize( bool flag )
{
...
void
RenderSurface::useBorder( bool flag )
{
    _decorations = flag;
#if 0
    if( _realized )
...
void
RenderSurface::setVisualChooser( VisualChooser* vc )
{
    if( _realized )
    {
        std::cerr << "RenderSurface::setVisualChooser():Warning - has no effect after "
                     "RenderSurface has been realized\n";
        return;
    }
    _visualChooser = vc;
...
VisualChooser*
RenderSurface::getVisualChooser( void )
{
    return _visualChooser.get();
}

const VisualChooser*
RenderSurface::getVisualChooser( void ) const
{
    return _visualChooser.get();
}
...
RenderSurface::RenderToTextureMode
RenderSurface::getRenderToTextureMode() const
{
    return _rtt_mode;
}

void
RenderSurface::setRenderToTextureMode( RenderToTextureMode mode )
{
    _rtt_mode = mode;
}

RenderSurface::RenderToTextureTarget
RenderSurface::getRenderToTextureTarget() const
{
    return _rtt_target;
}

void
RenderSurface::setRenderToTextureTarget( RenderToTextureTarget target )
{
    _rtt_target = target;
}

RenderSurface::RenderToTextureOptions
RenderSurface::getRenderToTextureOptions() const
{
    return _rtt_options;
}

void
RenderSurface::setRenderToTextureOptions( RenderToTextureOptions options )
{
    _rtt_options = options;
}

int
RenderSurface::getRenderToTextureMipMapLevel() const
{
    return _rtt_mipmap;
}

void
RenderSurface::setRenderToTextureMipMapLevel( int level )
{
    _rtt_mipmap       = level;
    _rtt_dirty_mipmap = true;
}

RenderSurface::CubeMapFace
RenderSurface::getRenderToTextureFace() const
{
    return _rtt_face;
}

void
RenderSurface::setRenderToTextureFace( CubeMapFace face )
{
    _rtt_face       = face;
    _rtt_dirty_face = true;
}
...
RenderSurface::getPBufferUserAttributes() const
{
    return _user_pbattr;
}

std::vector<int>&
RenderSurface::getPBufferUserAttributes()
{
    return _user_pbattr;
}

void
RenderSurface::useOverrideRedirect( bool flag )
{
    _overrideRedirectFlag = flag;
}

bool
RenderSurface::usesOverrideRedirect()
{
