 ...
 * Utility functions for osgPlugins: addAttribute, defined, malloc, new.
 */
#include "VisualChooser.hpp"
...
void
VisualChooser::addAttribute( AttributeName attribute )
{
    resetVisualInfo();
...
void
VisualChooser::addExtendedAttribute( unsigned int attribute )
{
    resetVisualInfo();
    _visual_attributes.push_back( VisualAttribute( attribute ) );
}
...
void
VisualChooser::setBufferSize( unsigned int size )
{
    addAttribute( BufferSize, size );
}

void
VisualChooser::setLevel( int level )
{
    addAttribute( Level, level );
}

void
VisualChooser::useRGBA()
{
    addAttribute( RGBA );
}

void
VisualChooser::useDoubleBuffer()
{
    addAttribute( DoubleBuffer );
}

void
VisualChooser::useStereo()
{
    addAttribute( Stereo );
}

void
VisualChooser::setAuxBuffers( unsigned int num )
{
    addAttribute( AuxBuffers, num );
}

void
VisualChooser::setRedSize( unsigned int size )
{
    addAttribute( RedSize, size );
}

void
VisualChooser::setGreenSize( unsigned int size )
{
    addAttribute( GreenSize, size );
}

void
VisualChooser::setBlueSize( unsigned int size )
{
    addAttribute( BlueSize, size );
}

void
VisualChooser::setAlphaSize( unsigned int size )
{
    addAttribute( AlphaSize, size );
}

void
VisualChooser::setDepthSize( unsigned int size )
{
    addAttribute( DepthSize, size );
}

void
VisualChooser::setStencilSize( unsigned int size )
{
    addAttribute( StencilSize, size );
}

void
VisualChooser::setAccumRedSize( unsigned int size )
{
    addAttribute( AccumRedSize, size );
}

void
VisualChooser::setAccumGreenSize( unsigned int size )
{
    addAttribute( AccumGreenSize, size );
}

void
VisualChooser::setAccumBlueSize( unsigned int size )
{
    addAttribute( AccumBlueSize, size );
}

void
VisualChooser::setAccumAlphaSize( unsigned int size )
{
    addAttribute( AccumAlphaSize, size );
}

void
VisualChooser::setSamples( unsigned int size )
{
    addAttribute( Samples, size );
}

void
VisualChooser::setSampleBuffers( unsigned int size )
{
    addAttribute( SampleBuffers, size );
}

void
VisualChooser::setVisualID( unsigned int id )
{
    _visual_id = id;
}

void
VisualChooser::setSimpleConfiguration( bool doublebuffer )
{
    clear();
    addAttribute( RGBA );
...
void
VisualChooser::applyAttribute( const VisualAttribute& va,
                               std::vector<int>&      attribs )
{
    ...
    else
    {
        ...
        {
            case UseGL :
                attribs.push_back( AGL_COMPLIANT );
                break;
            case BufferSize :
                attribs.push_back( AGL_BUFFER_SIZE );
                break;
            case Level :
                attribs.push_back( AGL_LEVEL );
                break;
            case RGBA :
    ...
    #endif
            default :
                attribs.push_back( ( int )va.attribute() );
                break;
        }
    }
...
VisualInfo*
VisualChooser::choose( Display* dpy,
                       int      screen,
                       bool     strict_adherence )
{
    if( _vinfo != NULL )
    {
                               ...
                               std::vector<int>&      attribs )
{
    ...
    else
    {
        ...
        {
            case UseGL :
                attribs.push_back( kCGLPFACompliant );
                break;
            case BufferSize :
            ...
            case Stereo :
                ...
                break;
            case AuxBuffers :
                attribs.push_back( kCGLPFAAuxBuffers );
                break;
            case RedSize :
                attribs.push_back( kCGLPFAColorSize );
                break;
            ...
            case AccumAlphaSize :
                ...
                break;
            case SampleBuffers :
                attribs.push_back( kCGLPFASampleBuffers );
                break;
            case Samples :
                attribs.push_back( kCGLPFASamples );
                break;
            default :
                attribs.push_back( va.attribute() );
                break;
        }
    }
                       ...
                       bool     strict_adherence )
{
    ...
    if( err )
    {
        std::cerr << "CGLChoosePixelFormat failed: " << CGLErrorString( err )
                  << std::endl;
        std::vector<int>::iterator i;
        for( i = va.begin(); i != va.end(); ++i )
        {
    ...
    else
    {
        switch( va.attribute() )
        {
            ...
            case StencilSize :
                ...
                break;
            case AccumRedSize :
                attribs.push_back( GLX_ACCUM_RED_SIZE );
                break;
            case AccumGreenSize :
                attribs.push_back( GLX_ACCUM_GREEN_SIZE );
                break;
            case AccumBlueSize :
                attribs.push_back( GLX_ACCUM_BLUE_SIZE );
                break;
            case AccumAlphaSize :
                attribs.push_back( GLX_ACCUM_ALPHA_SIZE );
                break;
                               ...
                               std::vector<int>&      attribs )
{
    ...
    std::pair<int, int> attr =
        std::make_pair( static_cast<int>( va.attribute() ), va.parameter() );
    ...
    switch( va.attribute() )
    {
        case Level :
            return;
        case BufferSize :
            attr.first = WGL_COLOR_BITS_ARB;
            break;
        case RGBA :
            attr.first  = WGL_PIXEL_TYPE_ARB;
            attr.second = WGL_TYPE_RGBA_ARB;
            break;
        case Stereo :
            attr.first  = WGL_STEREO_ARB;
            attr.second = 1;
            break;
        case AuxBuffers :
            attr.first = WGL_AUX_BUFFERS_ARB;
            break;
        case RedSize :
            attr.first = WGL_RED_BITS_ARB;
            break;
        case GreenSize :
            attr.first = WGL_GREEN_BITS_ARB;
            break;
        case BlueSize :
            attr.first = WGL_BLUE_BITS_ARB;
            break;
        case AlphaSize :
            attr.first = WGL_ALPHA_BITS_ARB;
            break;
        case DepthSize :
            attr.first = WGL_DEPTH_BITS_ARB;
            break;
        case StencilSize :
            attr.first = WGL_STENCIL_BITS_ARB;
            break;
        case AccumRedSize :
            attr.first = WGL_ACCUM_RED_BITS_ARB;
            break;
        case AccumGreenSize :
            attr.first = WGL_ACCUM_GREEN_BITS_ARB;
            break;
        case AccumBlueSize :
            attr.first = WGL_ACCUM_BLUE_BITS_ARB;
            break;
        case AccumAlphaSize :
            attr.first = WGL_ACCUM_ALPHA_BITS_ARB;
            break;
    #if defined( WGL_SAMPLE_BUFFERS_ARB ) && defined( WGL_SAMPLES_ARB )
        case SampleBuffers :
            attr.first = WGL_SAMPLE_BUFFERS_ARB;
            break;
        case Samples :
            attr.first = WGL_SAMPLES_ARB;
            break;
    #endif
        default :;
    }
                       ...
                       bool     strict_adherence )
{
    ...
    if( ext && ext->isSupported( WGLExtensions::ARB_pixel_format ) )
    {
        ...
        {
            std::vector<int> attribs;
            for( std::vector<VisualAttribute>::const_iterator i =
                     _visual_attributes.begin();
                 i != _visual_attributes.end();
                 ++i )
            {
            ...
            if( failed )
            {
                ...
                for( std::vector<int>::iterator k = attribs.begin(); k != attribs.end();
                     ++k )
                {
            ...
            if( failed )
            {
                ...
                std::cerr << "Producer::VisualChooser: the requested visual is not "
                             "available, trying to relax attributes..."
                          << std::endl;
    ...
    {
        std::cerr << "Producer::VisualChooser: unable to setup a valid visual with WGL "
    ...
    if( failed )
    {
        std::cerr << "Producer::VisualChooser: could not choose the pixel format"
                  << std::endl;
        return 0;
    }
...
bool
VisualChooser::isDoubleBuffer() const
{
    for( std::vector<VisualAttribute>::const_iterator i = _visual_attributes.begin();
         i != _visual_attributes.end();
         ++i )
    {
        if( i->attribute() == DoubleBuffer )
