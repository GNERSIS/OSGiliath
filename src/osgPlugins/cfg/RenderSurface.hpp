 ...
 * RenderSurface, derived from Referenced.
 * Provides: Callback, InputRectangle, InputRectangle, _left, InputRectangle, set.
 */
#pragma once
...
namespace osgProducer
{
    ...
    /**
        ...
        in that it is not a user input/output device, but rather a context and screen
       area specifically designed for 3D applications.  Consequently, a RenderSurface
       does not provide or impose a requirement on the caller to structure the
       application around the capturing or handling of events.  Further, RenderSurface
       provides increased control over the quality of pixel formats.
     */
    ...
    class RenderSurface : public osg::Referenced
    {
        public:
            ...
            class Callback : public osg::Referenced
            {
                public:
                    ...
                    virtual void
                    operator()( const RenderSurface& ) = 0;
            ...
            static unsigned int
                 getNumberOfScreens( void );

            void setDrawableType( DrawableType );
            DrawableType
            getDrawableType();
            void
            setReadDrawable( RenderSurface* );
            ...
            void
            setInputRectangle( const InputRectangle& ir );
            const InputRectangle&
            getInputRectangle() const;
            void
            bindInputRectangleToWindowSize( bool );
             ...
             * XServer.  Ignored on Win32   */
            int
            getScreenNum( void ) const;

            /** Get the size of the screen in pixels the render surface
             * is to be created on.  */
            void
                                     getScreenSize( unsigned int& width,
            ...
            /** Get the height of the RenderSurface in windowing system screen
             * coordinates */
            unsigned int
        ...
        Window getParentWindow( void ) const;
            ...
            void
            setVisualChooser( VisualChooser* vc );
            VisualChooser*
            getVisualChooser( void );
            const VisualChooser*
            getVisualChooser( void ) const;
        ...
        bool isRealized( void ) const;
            ...
            void
            useOverrideRedirect( bool );
            bool
            usesOverrideRedirect();

            /** Request whether the window should have a visible cursor.  If true, the
             * windowing system's default cursor will be assigned to the window.  If
             * false the window will not have a visible cursor. */
            void
            useCursor( bool flag );
        ...
        void unmapWindow();
            ...
            /** setCustomFullScreenRencangle(x,y,width,height).  Programmer may set a
             * customized rectangle to be interpreted as "fullscreen" when
             * fullscreen(true) is called.  This allows configurations that have large
             * virtual screens that span more than one monitor to define a "local" full
             * screen for each monitor.
             */
            void
            setCustomFullScreenRectangle( int          x,
                                          int          y,
                                          unsigned int width,
                                          unsigned int height );
            /** useDefaultFullScreenRetangle().  Sets the application back to using the
             * default screen size as fullscreen rather than the custom full screen
             * rectangle
             */
            void
            useDefaultFullScreenRectangle();

            /** isFullScreen() returns true if the RenderSurface's window fills the
             * entire screen and has no border. */
            bool
            isFullScreen() const
            {
             ...
             * This method has no effect if it is called after realize(). */
            void
            setRenderToTextureMode( RenderToTextureMode mode );

            /**
             * Get the render-to-texture target (PBuffer drawables only).
             * This method has no effect if it is called after realize(). */
            RenderToTextureTarget
            getRenderToTextureTarget() const;
             ...
             * This method has no effect if it is called after realize(). */
            void
            setRenderToTextureTarget( RenderToTextureTarget target );

            /**
             * Get the render-to-texture options (PBuffer drawables only).
             * This method has no effect if it is called after realize(). */
            RenderToTextureOptions
            getRenderToTextureOptions() const;
             ...
             * This method has no effect if it is called after realize(). */
            void
            setRenderToTextureOptions( RenderToTextureOptions options );

            /**
             * Get which mipmap level on the target texture will be
             * affected by rendering. */
            int
            getRenderToTextureMipMapLevel() const;
            ...
            void
            setRenderToTextureMipMapLevel( int level );

            /**
             * Get which face on the target cube map texture will be
             * affected by rendering. */
            CubeMapFace
            getRenderToTextureFace() const;
             ...
             * PBuffer has been realized. */
            void
            setRenderToTextureFace( CubeMapFace face );
        ...
        protected:
            ...
            int          _displayNum;
            float        _windowLeft, _windowRight, _windowBottom, _windowTop;
        ...
        unsigned int       _frameCount;
            ...
            // void _computeScreenSize( unsigned int &width, unsigned int &height )
            // const;
        ...
        static  void  _initThreads();
            ...
            LONG WINAPI                             proc( Window,
                                                          UINT,
                                                          WPARAM,
                                                          LPARAM );
            static LONG WINAPI                      s_proc( Window,
                                                            UINT,
                                                            WPARAM,
                                                            LPARAM );
