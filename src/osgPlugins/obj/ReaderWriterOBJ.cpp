...
#include "obj.hpp"
#include "OBJWriterNodeVisitor.hpp"
...
class ReaderWriterOBJ : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterOBJ()
        {
            ...
            supportsOption( "noTesselateLargePolygons",
                            "Do not do the default tessellation of large polygons" );
            supportsOption( "noTriStripPolygons",
                            "Do not do the default tri stripping of polygons" );
            supportsOption( "generateFacetNormals",
                            "generate facet normals for vertices without normals" );
            supportsOption( "noReverseFaces",
                            "avoid to reverse faces when normals and triangles "
            ...
            supportsOption( "SPECULAR_EXPONENT=<unit>",
                            "Set texture unit for specular exponent texture" );
            supportsOption( "OPACITY=<unit>",
                            "Set texture unit for opacity/dissolve texture" );
            supportsOption( "BUMP=<unit>", "Set texture unit for bumpmap texture" );
            supportsOption( "DISPLACEMENT=<unit>",
                            "Set texture unit for displacement texture" );
            supportsOption( "REFLECTION=<unit>",
                            "Set texture unit for reflection texture" );
            supportsOption( "NsIfNotPresent=<value>",
                            "set specular exponent if not present" );

            supportsOption( "precision=<digits>",
                            "Set the floating point precision when writing out files" );
            supportsOption( "OutputTextureFiles",
                   ...
                   const Options*     options = NULL ) const
        {
            ...
            OBJWriterNodeVisitor nv( f,
                                     osgDB::getSimpleFileName( materialFile ),
                                     localOptions.outputTextureFiles,
                                     options );

            node.accept( static_cast<osg::ConstNodeVisitor&>( nv ) );
    ...
    protected:
        ...
        class ObjOptionsStruct
        {
            public:
                ...
                bool noReverseFaces;
                // This is the order in which the materials will be assigned to texture
                // maps, unless otherwise overridden
        ...
        void
        buildMaterialToStateSetMap( obj::Model&            model,
                                    MaterialToStateSetMap& materialToSetSetMapObj,
                                    ObjOptionsStruct&      localOptions,
                                    const Options*         options ) const;

        osg::Geometry*
        convertElementListToGeometry( obj::Model&              model,
                                      obj::Model::ElementList& elementList,
                                      ObjOptionsStruct&        localOptions ) const;

        osg::Node*
        convertModelToSceneGraph( obj::Model&       model,
                                  ObjOptionsStruct& localOptions,
                                  const Options*    options ) const;

        inline osg::vec3
        transformVertex( const osg::vec3& vec,
                         const bool       rotate ) const;
        inline osg::vec3
        transformNormal( const osg::vec3& vec,
                         const bool       rotate ) const;

        ObjOptionsStruct
        parseOptions( const Options* options ) const;
};

inline osg::vec3
ReaderWriterOBJ::transformVertex( const osg::vec3& vec,
                                  const bool       rotate ) const
{
    if( rotate == true )
    {
...
inline osg::vec3
ReaderWriterOBJ::transformNormal( const osg::vec3& vec,
                                  const bool       rotate ) const
{
    if( rotate == true )
    {
...
REGISTER_OSGPLUGIN( obj,
                    ReaderWriterOBJ )

static void
load_material_texture( obj::Model&           model,
                       ...
                       const osgDB::Options* options )
{
    ...
    if( !filename.empty() )
    {
        ...
        if( image.valid() )
        {
            ...
            stateset->setTextureAttributeAndModes( texture_unit,
                                                   texture,
...
void
ReaderWriterOBJ::buildMaterialToStateSetMap(
    obj::Model&            model,
...
) const
{
         ...
         ++itr )
    {
        ...
        if( material.illum != 0 )
        {
            ...
            int ns = material.Ns != -1                   ? material.Ns
                   : localOptions.specularExponent != -1 ? localOptions.specularExponent
                                                         : 0;
            osg_material->setShininess( osg::Material::FRONT_AND_BACK,
                                        ( ns / 1000.0F ) *
                                            128.0F );    // note OBJ shiniess is 0..1000.

            if( material.ambient[3] !=
                1.0 ||
                material.diffuse[3] !=
                1.0 ||
                material.specular[3] !=
        ...
        // If the user has explicitly set the required texture type to unit map via the
        // options string, then we load ONLY those textures that are in the map.
        if( localOptions.textureUnitAllocation.size() > 0 )
        {
            for( unsigned int i = 0; i < localOptions.textureUnitAllocation.size(); i++ )
            {
                ...
                obj::Material::Map::TextureMapType type =
                    localOptions.textureUnitAllocation[i].second;
                // secondly, see if this texture type (e.g. DIFFUSE) is one of those in
                // the material
                int index = -1;
        ...
        // If the user has set no options, then we load them up in the order contained in
        // the enum. This latter method is an attempt not to break user's existing code
        else
        {
            int unit = 0;
            for( int i = 0; i < ( int )obj::Material::Map::UNKNOWN;
                 i++ )    // for each type
            {
                obj::Material::Map::TextureMapType type =
                    ( obj::Material::Map::TextureMapType )i;
                // see if this texture type (e.g. DIFFUSE) is one of those in the
                // material
                int index = -1;
                for( unsigned int j = 0; j < material.maps.size(); j++ )
                {
...
osg::Geometry*
ReaderWriterOBJ::convertElementListToGeometry( obj::Model&              model,
                                               obj::Model::ElementList& elementList,
                                               ObjOptionsStruct& localOptions ) const
{
    ...
    if( localOptions.generateFacetNormals == true )
    {
        for( itr = elementList.begin(); itr != elementList.end(); ++itr )
        {
            ...
            if( element.dataType ==
                obj::Element::POINTS ||
                element.dataType == obj::Element::POLYLINE )
            {
                continue;
            }
            ...
            if( element.normalIndices.size() == 0 )
            {
                ...
                osg::vec3 ab( model.vertices[b] );
                osg::vec3 ac( model.vertices[c] );

                ab -= model.vertices[a];
                ac -= model.vertices[a];

                osg::vec3 Norm( ab ^ ac );
                Norm           = osg::normalize( Norm );
    ...
    if( numPointElements > 0 )
    {
        ...
        for( itr = elementList.begin(); itr != elementList.end(); ++itr )
        {
            ...
            {
                for( obj::Element::IndexList::iterator index_itr =
        ...
        osg::DrawArrays* drawArrays =
            new osg::DrawArrays( GL_POINTS, startPos, numPoints );
    ...
    if( numPolylineElements > 0 )
    {
        ...
        osg::DrawArrayLengths* drawArrayLengths =
            new osg::DrawArrayLengths( GL_LINES, startPos );
    ...
    if( numPolygonElements > 0 )
    {
        ...
        osg::DrawArrayLengths* drawArrayLengths =
            new osg::DrawArrayLengths( GL_TRIANGLE_FAN, startPos );
        geometry->addPrimitiveSet( drawArrayLengths );
#endif
        ...
        for( itr = elementList.begin(); itr != elementList.end(); ++itr )
        {
            ...
            if( element.dataType == obj::Element::POLYGON )
            {
...
#else
                {
                    // Use GL_TRIANGLE_FAN for Core Profile compatibility (works for
                    // convex polygons of any size)
                    osg::DrawArrays* drawArrays =
                ...
                if( model.needReverse( element ) && !localOptions.noReverseFaces )
                {
                    ...
                    // need to reverse so add to OSG arrays in same order as in OBJ, as
                    // OSG assume anticlockwise ordering.
                    for( obj::Element::IndexList::reverse_iterator index_itr =
                             element.vertexIndices.rbegin();
                    ...
                    {
                        for( obj::Element::IndexList::reverse_iterator index_itr =
                                 element.normalIndices.rbegin();
                             index_itr != element.normalIndices.rend();
                             ++index_itr )
                        {
                            normals->push_back(
                                transformNormal( model.normals[*index_itr],
                                                 localOptions.rotate )
                            );
                        }
                    }
                    ...
                    {
                        for( obj::Element::IndexList::reverse_iterator index_itr =
                                 element.texCoordIndices.rbegin();
    ...
    if( hasReversedFaces )
    {
        OSG_WARN << "Warning: [ReaderWriterOBJ::convertElementListToGeometry] Some "
                    "faces from geometry '"
                 << geometry->getName() << "' were reversed by the plugin" << std::endl;
    }
...
osg::Node*
ReaderWriterOBJ::convertModelToSceneGraph( obj::Model&       model,
                                           ObjOptionsStruct& localOptions,
                                           const Options*    options ) const
{
         ...
         ++itr )
    {
        ...
        osg::Geometry*           geometry =
            convertElementListToGeometry( model, el, localOptions );

        if( geometry )
        {
            MaterialToStateSetMap::const_iterator it =
                materialToStateSetMap.find( es.materialName );
            if( it == materialToStateSetMap.end() )
            {
                OSG_WARN << "Obj unable to find material '" << es.materialName << "'"
                         << std::endl;
            }
            ...
            if( localOptions.generateFacetNormals ==
                false &&
                ( !geometry->getNormalArray() ||
                  geometry->getNormalArray()->getNumElements() == 0 ) )
            {
...
ReaderWriterOBJ::ObjOptionsStruct
ReaderWriterOBJ::parseOptions( const osgDB::ReaderWriter::Options* options ) const
{
    ObjOptionsStruct localOptions;
    ...
    if( options != NULL )
    {
        ...
        while( iss >> opt )
        {
            ...
            else if( pre_equals == "precision" )
            {
                ...
                if( val <= 0 )
                {
                    OSG_NOTICE << "Warning: invalid precision value: " << post_equals
                               << std::endl;
                }
                else
                {
            ...
            else if( post_equals.length() > 0 )
            {
                ...
                if( type != obj::Material::Map::UNKNOWN )
                {
                    int unit = atoi(
                        post_equals.c_str()
                    );    // (probably should use istringstream rather than atoi)
                    localOptions.textureUnitAllocation.push_back(
                        std::make_pair( unit,
                                        ( obj::Material::Map::TextureMapType )type )
                    );
                    OSG_NOTICE << "Obj Found map in options, [" << pre_equals
                               << "]=" << unit << std::endl;
                }
            }
        }
...
ReaderWriterOBJ::readNode( const std::string&                  file,
                           const osgDB::ReaderWriter::Options* options ) const
{
    ...
    if( fin )
    {
        ...
        osg::Node*       node =
            convertModelToSceneGraph( model, localOptions, local_opt.get() );
        return node;
    }
...
osgDB::ReaderWriter::ReadResult
ReaderWriterOBJ::readNode( std::istream&  fin,
                           const Options* options ) const
{
    if( fin )
    {
