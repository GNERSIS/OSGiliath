...
class OBJWriterNodeVisitor : public osg::DualModeVisitor
{

    public:

        OBJWriterNodeVisitor( std::ostream&         fout,
                              const std::string     materialFileName   = "",
                              bool                  outputTextureFiles = false,
        ...
        apply( osg::Group& node )
        {
            ...
            _nameStack.push_back( node.getName().empty() ? node.className()
                                                         : node.getName() );
        ...
        void
        writeMaterials( std::ostream& fout );
    ...
    private:
        ...
        OBJWriterNodeVisitor&
        operator=( const OBJWriterNodeVisitor& )
        {
            return *this;
        }
        ...
        processGeometry( osg::Geometry* geo,
                         osg::dmat4&    m );
        void
        processArray( const std::string& key,
                      osg::Array*        array,
        ...
        std::string
        getUniqueName( const std::string& defaultValue = "" );

        typedef std::stack<osg::ref_ptr<osg::StateSet>> StateSetStack;
        typedef std::map<osg::ref_ptr<osg::StateSet>, OBJMaterial, CompareStateSet>
                                            MaterialMap;
