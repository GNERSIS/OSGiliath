...
class ValueVisitor : public osg::ValueVisitor
{
    public:
        ...
        apply( osg::dvec3& inv )
        {
            osg::dvec3 v( inv[0], inv[1], inv[2] );
            osg::dvec3 orign_d( ( double )_origin.x,
                                ( double )_origin.y,
                                ( double )_origin.z );
            if( _applyMatrix )
            {
                v = ( _isNormal ) ? ( v * _m ) - orign_d : v * _m;
            }
...
class ObjPrimitiveIndexWriter : public osg::PrimitiveIndexFunctor
{
    ...
    public:
        ...
        ObjPrimitiveIndexWriter( std::ostream&  fout,
                                 osg::Geometry* geo,
                                 unsigned int   normalIndex,
                                 unsigned int   lastVertexIndex,
                                 unsigned int   lastNormalIndex,
                                 unsigned int   lastTexIndex ) :
        ...
        write( unsigned int i )
        {
            ...
            if( _hasTexCoords || _hasNormalCoords )
            {
                ...
                if( _hasNormalCoords )
                {
                    if( osg::getBinding( _geo->getNormalArray() ) ==
    ...
    private:
        ...
        ObjPrimitiveIndexWriter&
        operator=( const ObjPrimitiveIndexWriter& )
        {
            return *this;
        }
...
void
ObjPrimitiveIndexWriter::drawArrays( GLenum  mode,
                                     GLint   first,
                                     GLsizei count )
{
    switch( mode )
    {
        ...
        default :
            OSG_WARN << "OBJWriterNodeVisitor :: can't handle mode " << mode
                     << std::endl;
            break;
    }
}

OBJWriterNodeVisitor::OBJMaterial::OBJMaterial( osg::Material*        mat,
                                                osg::Texture*         tex,
...
operator<<( std::ostream&                            fout,
            const OBJWriterNodeVisitor::OBJMaterial& mat )
{
...
void
OBJWriterNodeVisitor::writeMaterials( std::ostream& fout )
{
...
std::string
OBJWriterNodeVisitor::getUniqueName( const std::string& defaultvalue )
{

    std::string name = "";
    for( std::list<std::string>::iterator i = _nameStack.begin(); i != _nameStack.end();
         ++i )
    {
        if( !name.empty() )
        {
...
void
OBJWriterNodeVisitor::processArray( const std::string& key,
                                    osg::Array*        array,
                                    const osg::dmat4&  m,
                                    bool               isNormal )
{
...
void
OBJWriterNodeVisitor::processStateSet( osg::StateSet* ss )
{
    ...
    if( mat || tex )
    {
        _materialMap.insert( std::make_pair(
            osg::ref_ptr<osg::StateSet>( ss ),
            OBJMaterial( mat, tex, _outputTextureFiles, _options.get() )
        ) );
        _fout << "usemtl " << _materialMap[ss].name << std::endl;
    }
}
...
void
OBJWriterNodeVisitor::processGeometry( osg::Geometry* geo,
                                       osg::dmat4&    m )
{
    _fout << std::endl;
    _fout << "o "
          << getUniqueName( geo->getName().empty() ? geo->className() : geo->getName() )
          << std::endl;
    ...
    for( unsigned int i = 0; i < geo->getNumPrimitiveSets(); ++i )
    {
        ...
        ObjPrimitiveIndexWriter pif( _fout,
                                     geo,
                                     normalIndex,
                                     _lastVertexIndex,
                                     _lastNormalIndex,
        ...
        if( geo->getNormalArray() &&
            geo->getNormalArray()->getBinding() == osg::Array::BIND_PER_PRIMITIVE_SET )
        {
            ++normalIndex;
        }
    }
...
void
OBJWriterNodeVisitor::apply( osg::Geometry& geometry )
{
...
void
OBJWriterNodeVisitor::apply( osg::Geode& node )
{
