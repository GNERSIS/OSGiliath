 ...
 * Utility functions for osgPlugins: strip, parseTextureMap, s.
 */
#include "obj.hpp"
...
static std::string
strip( const std::string& ss )
{
    // std::not1 / std::ptr_fun were removed in C++17; use a lambda. The
    // unsigned-char cast avoids passing a negative char to isspace (UB).
 ...
 */
static Material::Map
parseTextureMap( const std::string&            ss,
                 Material::Map::TextureMapType type )
{
    Material::Map map;
    std::string   s( ss );
    for( ;; )
...
bool
Model::readline( std::istream& fin,
                 char*         line,
                 const int     LINE_SIZE )
{
    if( LINE_SIZE < 1 )
...
std::string
Model::lastComponent( const char* linep )
{
...
bool
Model::readMTL( std::istream& fin )
{
    OSG_INFO << "Reading MTL file" << std::endl;

    fin.imbue( std::locale::classic() );

    const int LINE_SIZE = 4'096;
    char      line[LINE_SIZE];
    ...
    while( fin )
    {
        ...
        else if( strlen( line ) > 0 )
        {
            ...
            else if( material )
            {
                ...
                {
                    unsigned int fieldsRead =
                        sscanf( line + 3, "%f %f %f %f", &r, &g, &b, &a );
                ...
                // Tr - transparency
                //
                // Seems that the world did not agreed about the specification of the
                // item.
                //
                // Some thinks that value of 1 means opaque material and 0 transparent
                // material, such as
                ...
                // carry the following signature as the first line in the file (*.obj,
                // *.mtl): # 3ds Max Wavefront OBJ Exporter v0.97b - (c)2007 guruware
                //
                // Moreover, at least one model uses Tr followed by two numbers.
                // Such model can be downloaded from
                // http://graphics.cs.williams.edu/data/meshes/cube.zip (part of the
                // following collection:
                // http://graphics.cs.williams.edu/data/meshes.xml).
                //
                // Current solution: As we do not know what is the correct interpretation
                // of the value 0 and value 1 for Tr, we will rely on d (dissolve)
                // parameter instead whenever it is present. This seems to fix the
                // problem on large number of models.
                //
                ...
                else if( strncasecmp( line, "map_Ka ", 7 ) == 0 )
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 7 ), Material::Map::AMBIENT )
                    );
                }
                // diffuse map
                ...
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 7 ), Material::Map::DIFFUSE )
                    );
                }
                ...
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 7 ), Material::Map::SPECULAR )
                    );
                }
                ...
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 12 ), Material::Map::OPACITY )
                    );
                }
                ...
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 6 ), Material::Map::OPACITY )
                    );
                }
                ...
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 7 ),
                                         Material::Map::SPECULAR_EXPONENT )
                    );
                }
                // modelling tools and convertors variously produce bump, map_bump, and
                // map_Bump so parse them all
                else if( strncasecmp( line, "bump ", 5 ) == 0 )
                {
                    material->maps.push_back( parseTextureMap( strip( line + 5 ),
                                                               Material::Map::BUMP ) );
                }
                else if( strncasecmp( line, "map_bump ", 9 ) == 0 )
                {
                    material->maps.push_back( parseTextureMap( strip( line + 9 ),
                                                               Material::Map::BUMP ) );
                }
                else if( strncasecmp( line, "map_Bump ", 9 ) == 0 )
                {
                ...
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 5 ), Material::Map::DISPLACEMENT )
                    );
                }
                // reflection map (the original code had the possibility of a blank
                // "refl" line which isn't correct according to the spec, so this bit
                // might break for some modelling packages...
                else if( strncasecmp( line, "refl ", 5 ) == 0 )
                {
                    material->maps.push_back(
                        parseTextureMap( strip( line + 5 ), Material::Map::REFLECTION )
                    );
                }
                else
                {
...
inline bool
isZBrushColorField( char* line )
{
    return strncmp( line, "#MRGB", 5 ) == 0;
}

bool
Model::readOBJ( std::istream&                       fin,
    ...
    while( fin )
    {
        ...
        else if( isZBrushColorField( line ) )
        {
            ...
            while( colorFields.size() >= 8 )
            {
                ...
                r = static_cast<float>( strtol( currentValue.c_str(), NULL, 16 ) ) /
                    255.;
                colorFields  = colorFields.substr( 2 );

                currentValue = colorFields.substr( 0, 2 );
                g = static_cast<float>( strtol( currentValue.c_str(), NULL, 16 ) ) /
                    255.;
                colorFields  = colorFields.substr( 2 );

                currentValue = colorFields.substr( 0, 2 );
                b = static_cast<float>( strtol( currentValue.c_str(), NULL, 16 ) ) /
                    255.;
                colorFields = colorFields.substr( 2 );
        ...
        else if( strlen( line ) > 0 )
        {
            ...
            {
                unsigned int fieldsRead = sscanf( line + 2,
                                                  "%f %f %f %f %f %f %f",
                                                  &x,
                                                  &y,
                                                  &z,
                     ...
                     strncmp( line, "f ", 2 ) == 0 )
            {
                ...
                Element* element =
                    new Element( ( line[0] == 'p' )   ? Element::POINTS
                ...
                while( *ptr != 0 )
                {
                    ...
                    if( sscanf( ptr, "%d/%d/%d", &vi, &ti, &ni ) == 3 )
                    {
                        ...
                        if( normals.size() >
                            0 &&
                            remapNormalIndex( ni ) < static_cast<int>( normals.size() ) )
                        {
                            element->normalIndices.push_back( remapNormalIndex( ni ) );
                        }
                        if( texcoords.size() >
                            0 &&
                            remapTexCoordIndex( ti ) <
                            static_cast<int>( texcoords.size() ) )
                        {
                            element->texCoordIndices.push_back(
                                remapTexCoordIndex( ti )
                            );
                        }
                    }
                ...
                if( !element->normalIndices.empty() &&
                    element->normalIndices.size() != element->vertexIndices.size() )
                {
                    element->normalIndices.clear();
                }

                if( !element->texCoordIndices.empty() &&
                    element->texCoordIndices.size() != element->vertexIndices.size() )
                {
                ...
                if( !element->vertexIndices.empty() )
                {
                    ...
                    if( coordateCombination !=
                        currentElementState.coordinateCombination )
                    {
                        currentElementState.coordinateCombination = coordateCombination;
                        currentElementList =
                            0;    // reset the element list to force a recompute of which
                                  // ElementList to use
                    }
                    addElement( element );
                }
            ...
            else if( strncmp( line, "usemtl ", 7 ) == 0 )
            {
                ...
                if( currentElementState.materialName != materialName )
                {
                    ...
                    currentElementList = 0;    // reset the element list to force a
                                               // recompute of which ElementList to use
                }
            }
            else if( strncmp( line, "mtllib ", 7 ) == 0 )
            {
                ...
                std::string fullPathFileName =
                    osgDB::findDataFile( materialFileName, options );
            ...
            else if( strncmp( line, "s ", 2 ) == 0 )
            {
                ...
                else
                {
                    ...
                    if( result != 1 )
                    {
                        OSG_NOTICE << "*** error reading smoothing group ***"
                                   << std::endl;
                    }
                }
...
osg::vec3
Model::averageNormal( const Element& element ) const
{
    osg::vec3 normal;
...
osg::vec3
Model::computeNormal( const Element& element ) const
{
    osg::vec3 normal;
...
bool
Model::needReverse( const Element& element ) const
{
    if( element.normalIndices.empty() )
    {
        return false;
    ...
    return osg::dot( computeNormal( element ), averageNormal( element ) ) < 0.0F;
}
