...
#include "AutoDiscoveryWinImpl.hpp"
#include "dns_sd.hpp"
...
RegisterService( DNSServiceRef*           sdref,
                 const char*              nam,
                 const char*              typ,
                 const char*              dom,
                 const char*              host,
                 const char*              port,
                 AutoDiscoveryServerImpl* context )
{
    DNSServiceFlags flags        = 0;
    uint16_t        PortAsNumber = atoi( port );
    Opaque16        registerPort = {
        ( unsigned char )( PortAsNumber >> 8 ),
        ( unsigned char )( PortAsNumber & 0XFF )
    };
    unsigned char  txt[2'048] = "";
    ...
    OSG_INFO << "AutoDiscoveryImpl :: Registering Service "
             << ( nam[0] ? nam : "<<Default>>" ) << " " << typ << " "
             << ( dom[0] ? "." : "", dom ) << std::endl;
    if( host && *host )
    {
        OSG_INFO << "AutoDiscoveryImpl :: host " << host << " port " << port
                 << std::endl;
    }
    ...
    return ( DNSServiceRegister( sdref,
                                 flags,
...
AutoDiscoveryServerImpl::AutoDiscoveryServerImpl( const std::string& type,
                                                  unsigned int       port )
{
    ...
    DNSServiceErrorType err =
        RegisterService( &client, "", type.c_str(), "", NULL, ss.str().c_str(), this );
    if( !client || err != kDNSServiceErr_NoError )
    {
        OSG_WARN << "AutoDiscoveryImpl :: DNSService call failed " << ( long int )err
                 << std::endl;
    }
}

void
AutoDiscoveryServerImpl::update()
{
    ...
    if( result > 0 )
    {
        ...
        // else if (client_pa && FD_ISSET(dns_sd_fd2, &readfds)) err =
        // DNSServiceProcessResult(client_pa); if (err) { fprintf(stderr,
        // "DNSServiceProcessResult returned %d\n", err); stopNow = 1; }
    }
}

void
AutoDiscoveryServerImpl::serviceAdded()
{
    OSG_INFO << "AutoDiscoveryImpl :: Service added" << std::endl;
}

void
AutoDiscoveryServerImpl::serviceRemoved()
{
                  ...
                  void*                 context )
{
    ...
    ContextDNSServiceRefPair* ref_pair =
        static_cast<ContextDNSServiceRefPair*>( context );
    if( !ref_pair )
    {
        return;
    }
    ...
    if (CopyLabels(n, n + kDNSServiceMaxDomainName, &p, 3)) return;        // Fetch
    name+type p = fullname; if (CopyLabels(t, t + kDNSServiceMaxDomainName, &p, 1))
    return;        // Skip first label if (CopyLabels(t, t + kDNSServiceMaxDomainName,
    &p, 2)) return;        // Fetch next two labels (service type)

    if (num_printed++ == 0)
        {
        printf("\n");
        printf("; To direct clients to browse a different domain, substitute that domain
    in place of '@'\n"); printf("%-47s PTR     %s\n", "lb._dns-sd._udp", "@");
        printf("\n");
        printf("; In the list of services below, the SRV records will typically reference
    dot-local Multicast DNS names.\n"); printf("; When transferring this zone file data
    to your unicast DNS server, you'll need to replace those dot-local\n"); printf(";
    ...
    PortAsNumber, hosttarget); printf("%-47s TXT    ", n);
              ...
              void*                 context )
{
    ...
    DNSServiceErrorType err = DNSServiceResolve( &ref_pair->ref,
                                                 0,
                                                 ifIndex,
                                                 replyName,
                                                 replyType,
    ...
    {
        OSG_WARN << "AutoDiscoveryImpl :: DNSServiceResolve call failed "
                 << ( long int )err << std::endl;
    }
    else
    {
...
AutoDiscoveryClientImpl::AutoDiscoveryClientImpl( const std::string&          type,
                                                  DiscoveredServicesCallback* cb )
{
    ...
    if( !client || err != kDNSServiceErr_NoError )
    {
        OSG_WARN << "AutoDiscoveryImpl :: DNSServiceBrowse call failed "
                 << ( long int )err << std::endl;
    }
}

void
AutoDiscoveryClientImpl::update()
{
    updateRef( client );
    for( std::list<DNSServiceRef>::iterator i = resolveRefs.begin();
         i != resolveRefs.end();
         ++i )
    {
        updateRef( *i );
    }
    for( std::list<DNSServiceRef>::iterator i = resolveRefsToDelete.begin();
         i != resolveRefsToDelete.end();
         ++i )
    {
...
void
AutoDiscoveryClientImpl::updateRef( DNSServiceRef& ref )
{
...
void
AutoDiscoveryClientImpl::serviceRemoved( const std::string& replyName,
                                         ...
                                         const std::string& replyDomain )
{
    ...
    {
        OSG_INFO << "AutoDiscoveryImpl :: no services found to remove? " << full_name
                 << std::endl;
        return;
    }
    if( !_cb.valid() )
...
void
AutoDiscoveryClientImpl::serviceAdded( const std::string& fullname,
