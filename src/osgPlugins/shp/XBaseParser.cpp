...
namespace ESRIShape
{
    ...
    XBaseHeader::print()
    {
        OSG_INFO << "VersionNumber = " << ( int )_versionNumber << std::endl
                 << "LastUpdate    = " << 1'900 +
            ( int )_lastUpdate[0]
                 << "/" << ( int )_lastUpdate[1] << "/" << ( int )_lastUpdate[2]
                 << std::endl
    ...
    XBaseHeader::read( int fd )
    {
        ...
        if( ( nbytes = ::read( fd,
                               ...
                               sizeof( _incompleteTransaction ) ) ) <= 0 )
        {
            return false;
        }
        ...
        if( ( nbytes =
                  ::read( fd, &_reservedMultiUser, sizeof( _reservedMultiUser ) ) ) <=
            0 )
        {
            return false;
        }
    ...
    bool
    XBaseFieldDescriptor::read( int fd )
    {
        int nbytes = 0;
        ...
        if( ( nbytes =
                  ::read( fd, &_reservedMultiUser2, sizeof( _reservedMultiUser2 ) ) ) <=
            0 )
        {
            return false;
        }
    ...
    XBaseParser::parse( int fd )
    {
        ...
        while( fieldDescriptorDone == false )
        {
            ...
            else
            {
                ...
                {
                    OSG_WARN << "File parsing failed, lseek return errno=" << errno
                             << std::endl;
                    return false;
                }
            }
        ...
        std::vector<XBaseFieldDescriptor>::iterator it,
            end = _xBaseFieldDescriptorList.end();
        for( Integer i = 0; i < _xBaseHeader._numRecord; ++i )
        {
            ...
            osgSim::ShapeAttributeList* shapeAttributeList =
                    ...
                    case 'C' :
                        {
                            ...
                            shapeAttributeList->push_back(
                                osgSim::ShapeAttribute( ( const char* )it->_name,
                                                        ( char* )str )
                            );
                            delete[] str;
                            break;
                    ...
                    default :
                        {
                            OSG_WARN << "ESRIShape::XBaseParser : record type "
                                     << it->_fieldType << "not supported, skipped"
                                     << std::endl;
                            shapeAttributeList->push_back(
                                osgSim::ShapeAttribute( ( const char* )it->_name,
                                                        ( double )0 )
