...
class ESRIShapeReaderWriter : public osgDB::ReaderWriter
{
    public:
        ...
        ESRIShapeReaderWriter()
        {
            ...
            supportsOption(
                "double",
                "Read x,y,z data as double an stored as geometry in osg::Vec3dArray's."
            );
            supportsOption( "keepSeparatePoints",
                            "Avoid combining point features into multi-point." );
        }
                  ...
                  const Options*     options ) const
        {
            ...
            if( options &&
                options->getOptionString().find( "keepSeparatePoints" ) !=
                std::string::npos )
            {
            ...
            if( sp.getGeode() && ( xbp.getAttributeList().empty() == false ) )
            {
                ...
                {
                    OSG_WARN << "ESRIShape loader : .dbf file containe different record "
                                "number that .shp file."
                             << std::endl
                ...
                else
                {
                    ...
                    ESRIShape::XBaseParser::ShapeAttributeListList::const_iterator it,
                        end = xbp.getAttributeList().end();
                ...
                if( osgDB::fileExists( projFileName ) )
                {
                    ...
                    if( fin )
                    {
                        ...
                        if( !projstring.empty() )
                        {
                            ...
                            {
                                locator->setCoordinateSystemType(
