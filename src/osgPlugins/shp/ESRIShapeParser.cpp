...
osg::Geode*
ESRIShapeParser::getGeode()
{
    return _geode.get();
}

void
ESRIShapeParser::_combinePointToMultipoint()
{
    ...
    geometry->addPrimitiveSet(
        new osg::DrawArrays( osg::PrimitiveSet::POINTS, 0, coords.size() )
    );
    _geode->addDrawable( geometry.get() );
}

void
ESRIShapeParser::_process( const std::vector<ESRIShape::Point>& pts )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::MultiPoint>& mpts )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PolyLine>& lines )
{
    if( !_valid )
    {
        return;
    ...
    for( p = lines.begin(); p != lines.end(); p++ )
    {
        ...
        {
            int index = p->parts[i];
            int len   = i < p->numParts - 1 ? p->parts[i + 1] - p->parts[i]
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::Polygon>& polys )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PointM>& ptms )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::MultiPointM>& mptms )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PolyLineM>& linems )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PolygonM>& polyms )
{
    if( !_valid )
    {
        return;
    ...
    for( p = polyms.begin(); p != polyms.end(); p++ )
    {
        ...
        for( i = 0; i < p->numParts; i++ )
        {
            ...
            geometry->addPrimitiveSet(
                new osg::DrawArrays( osg::PrimitiveSet::TRIANGLE_FAN, index, len )
            );
        }
        _geode->addDrawable( geometry.get() );
    }
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PointZ>& ptzs )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::MultiPointZ>& mptzs )
{
    if( !_valid )
    {
        return;
    ...
    for( p = mptzs.begin(); p != mptzs.end(); p++ )
    {
        ...
        {
            coords->push_back(
                osg::vec3( p->points[i].x, p->points[i].y, p->zArray[i] )
            );
        }
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PolyLineZ>& linezs )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::PolygonZ>& polyzs )
{
    if( !_valid )
    {
        return;
...
void
ESRIShapeParser::_process( const std::vector<ESRIShape::MultiPatch>& mpatches )
{
    if( !_valid )
    {
        return;
    ...
    for( p = mpatches.begin(); p != mpatches.end(); p++ )
    {
        ...
        for( i = 0; i < p->numParts; i++ )
        {
            ...
            int mode  = p->partTypes[i] == TriangleStrip
                ...
                p->partTypes[i] == Ring )
            {
                OSG_WARN << "ESRIShapeParser - MultiPatch type "
                         << ( p->partTypes[i] == TriangleStrip ? "TriangleStrip"
                              ...
                              : p->partTypes[i] == Ring        ? "Ring"
                                                               : "Dunno" )
                         << " poorly supported.  Will be represented by a red line strip"
                         << std::endl;
            }
