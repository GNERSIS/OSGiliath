...
Normals::MakeNormalsVisitor::MakeNormalsVisitor( float normalScale,
                                                 Mode  mode ) :
...
void
Normals::MakeNormalsVisitor::apply( osg::MatrixTransform& tx )
{
    _matStack.push( _mat );
...
void
Normals::MakeNormalsVisitor::apply( Geode& geode )
{
                    ...
                    else
                    {
                        switch( ( *itr )->getMode() )
                        {
                            ...
                            case( PrimitiveSet::POLYGON ) :
                                {
                                    DrawArrayLengths* dal =
                                        dynamic_cast<DrawArrayLengths*>(
                                            ( *itr ).get()
                                        );
                                    if( dal )
                                    {
                                        for( unsigned int j = 0; j < dal->size(); ++j )
                                        {
                                            ...
                                            // OSG_WARN << "j=" << j << " num_prim=" <<
                                            // num_prim << std::endl;
                                            _processPrimitive( num_prim,
                                                               coord_index,
                                                               normals_index,
                                                               binding );
...
void
Normals::MakeNormalsVisitor::_processPrimitive( unsigned int        nv,
...
#ifdef DEBUG
void
Normals::_printPrimitiveType( osg::PrimitiveSet* pset )
{
