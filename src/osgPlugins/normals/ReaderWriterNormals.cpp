...
class NormalsReader : public osgDB::ReaderWriter
{
                  ...
                  const Options*     options ) const
        {
            ...
            if( !nodeName.empty() )
            {
                ...
                if( node.valid() )
                {
                    ...
                    const osg::sphere& bsph = group->getBound();
                    scale = bsph.radius *
                            0.05F *
                            scale;    // default is 5% of bounding-sphere radius
    ...
    private:
        ...
        usage() const
        {
            OSG_INFO << "Normals Plugin usage:  <application> [-O options] "
                        "<model.ext>.normals\n"
                        "     options: \"scale=<scale>\"                        "
                        "(default = 1.0)\n"
                        "              \"mode=<VertexNormals|SurfaceNormals>\"  "
                        "(default = VertexNormals)"
                     << std::endl;
        }
};

REGISTER_OSGPLUGIN( normals,
                    NormalsReader )
