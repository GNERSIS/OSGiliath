...
namespace osgFFmpeg
{
    ...
    class FFmpegDecoderAudio : public osg::Thread
    {
        public:

            typedef BoundedMessageQueue<FFmpegPacket> PacketQueue;
            typedef void ( *PublishFunc )( const FFmpegDecoderAudio& decoder,
                                           void*                     user_data );

            FFmpegDecoderAudio( PacketQueue&  packets,
                                FFmpegClocks& clocks );
            ~FFmpegDecoderAudio();
            ...
            void
            setAudioSink( osg::ref_ptr<osg::AudioSink> audio_sink );
            void
        ...
        private:
            ...
            decodeLoop();
            void
            adjustBufferEndPts( size_t buffer_size );
            size_t
                            decodeFrame( void*  buffer,
                                         size_t size );
    ...
    inline bool
    FFmpegDecoderAudio::validContext() const
    {
        return m_context != 0;
    }

    inline int
    FFmpegDecoderAudio::frequency() const
    {
        return m_out_sample_rate;
    }

    inline int
    FFmpegDecoderAudio::nbChannels() const
    {
