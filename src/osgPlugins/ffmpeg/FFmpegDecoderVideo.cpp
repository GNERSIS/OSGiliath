...
namespace osgFFmpeg
{
    ...
    // Note from Jason Daly in a osg-submissions thread, "The pix_fmt field of
    // AVCodecContext will indicate the pixel format of the decoded video"

    FFmpegDecoderVideo::FFmpegDecoderVideo( PacketQueue&  packets,
                                            FFmpegClocks& clocks ) :
        m_packets( packets ),
        m_clocks( clocks ),
        m_stream( 0 ),
    ...
    void
    FFmpegDecoderVideo::open( AVStream* const stream )
    {
        m_stream  = stream;
...
// Find out the framerate
        ...
        m_buffer_rgba[0].resize(
            avpicture_get_size( AV_PIX_FMT_RGB24, width(), height() )
        );
        ...
        // Override get_buffer()/release_buffer() from codec context in order to retrieve
        // the PTS of each frame.
        m_context->opaque      = this;
        m_context->get_buffer2 = getBuffer;
    }

    void
    FFmpegDecoderVideo::close( bool waitForThreadToExit )
    {
        if( isRunning() )
        {
            m_exit = true;
    ...
    void
    FFmpegDecoderVideo::pause( bool pause )
    {
        if( pause )
        {
            m_paused = true;
    ...
    FFmpegDecoderVideo::decodeLoop()
    {
        ...
        while( !m_exit )
        {
            ...
            while( m_bytes_remaining > 0 )
            {
                ...
                // We want to use the entire packet since some codecs will require extra
                ...
                if( frame_finished )
                {
                    ...
                    const double synched_pts = m_clocks.videoSynchClock(
                        m_frame.get(),
                        av_q2d( av_inv_q( m_context->framerate ) ),
                        pts
                    );
    ...
    void
    FFmpegDecoderVideo::findAspectRatio()
    {
        float ratio = 0.0F;
    ...
    int
    FFmpegDecoderVideo::convert( AVPicture* dst,
                                 int        dst_pix_fmt,
                                 AVPicture* src,
                                 int        src_pix_fmt,
                                 int        src_width,
                                 int        src_height )
    {
        ...
        if( m_swscale_ctx == 0 )
        {
            m_swscale_ctx = sws_getContext( src_width,
                                            src_height,
        ...
        int result =
            img_convert( dst, dst_pix_fmt, src, src_pix_fmt, src_width, src_height );

#endif
        osg::Timer_t endTick = osg::Timer::instance()->tick();
        OSG_DEBUG << " time = " << osg::Timer::instance()->delta_m( startTick, endTick )
                  << "ms" << std::endl;
    ...
    void
    FFmpegDecoderVideo::publishFrame( const double delay,
                                      bool         audio_disabled )
    {
...
#if 1
        // new code from Jean-Sebasiten Guay - needs testing as we're unclear on the best
        // solution If the display delay is too small, we better skip the frame.
        ...
        int i_delay = static_cast<int>( delay * 1'000'000 + 0.5 );
        ...
        while( i_delay > 1'000 )
        {
            ...
            const int micro_delay = ( std::min )( 1'000'000, i_delay );

            osg::Thread::microSleep( micro_delay );
    ...
    void
    FFmpegDecoderVideo::yuva420pToRgba( AVPicture* const dst,
                                        AVPicture* const src,
                                        int              width,
                                        int              height )
    {
    ...
    int
    FFmpegDecoderVideo::getBuffer( AVCodecContext* const context,
                                   AVFrame* const        picture,
                                   int                   flags )
    {
        AVBufferRef*                    ref;
        const FFmpegDecoderVideo* const this_ =
            reinterpret_cast<const FFmpegDecoderVideo*>( context->opaque );
        ...
        *p_pts           = this_->m_packet_pts;
        ...
        ref              = av_buffer_create( ( uint8_t* )picture->opaque,
                                             sizeof( int64_t ),
                                             FFmpegDecoderVideo::freeBuffer,
                                             picture->buf[0],
                                             flags );
        picture->buf[0]  = ref;
    ...
    void
    FFmpegDecoderVideo::freeBuffer( void*    opaque,
                                    uint8_t* data )
    {
