...
// #include <iostream>
    ...
    #define AVCODEC_MAX_AUDIO_FRAME_SIZE 192'000
#endif
...
namespace osgFFmpeg
{
    ...
    static int
    decode_audio( AVCodecContext* avctx,
                  int16_t*        samples,
                  int*            frame_size_ptr,
                  const uint8_t*  buf,
                  int             buf_size,
    ...
    53 ||                                                                \
    ( LIBAVCODEC_VERSION_MAJOR == 52 && LIBAVCODEC_VERSION_MINOR >= 32 )
    ...
    #ifdef USE_AVRESAMPLE    // libav's AVFrame structure does not contain a 'channels'
                             // field
        if( ret >= 0 && got_frame )
        {
    #else
        if( ret >= 0 && got_frame && av_frame_get_channels( frame ) > 0 )
        {
            ...
            {
                // out_samples = av_rescale_rnd(swr_get_delay(swr_context,
                // avctx->sample_rate) +
                //                      frame->nb_samples, out_sample_rate,
                //                      avctx->sample_rate, AV_ROUND_UP);
                out_samples = av_rescale_rnd( frame->nb_samples,
                                              out_sample_rate,
                                              avctx->sample_rate,
                                              AV_ROUND_UP );
            }
            ...
            int output_data_size = av_samples_get_buffer_size( &plane_size,
                                                               out_nb_channels,
                                                               out_samples,
                                                               out_sample_format,
                                                               1 );
            ...
            if( swr_context != NULL )
            {
                ...
                out_samples = swr_convert( swr_context,
                                           ( uint8_t** )&samples,
                                           out_samples,
                                           ( const uint8_t** )frame->extended_data,
                                           frame->nb_samples );

                // recompute output_data_size following swr_convert result (number of
                // samples actually converted)
                output_data_size = av_samples_get_buffer_size( &plane_size,
                                                               out_nb_channels,
                                                               out_samples,
                                                               out_sample_format,
                                                               1 );
    ...
    FFmpegDecoderAudio::FFmpegDecoderAudio( PacketQueue&  packets,
                                            FFmpegClocks& clocks ) :
        m_packets( packets ),
        m_clocks( clocks ),
        m_stream( 0 ),
    ...
    void
    FFmpegDecoderAudio::open( AVStream* const   stream,
                              FFmpegParameters* parameters )
    {
        try
        {
            ...
            AVDictionaryEntry* opt_out_sample_rate =
                av_dict_get( *parameters->getOptions(), "out_sample_rate", NULL, 0 );
            ...
            AVDictionaryEntry* opt_out_sample_format =
                av_dict_get( *parameters->getOptions(), "out_sample_format", NULL, 0 );
            if( opt_out_sample_format )
            {
                m_out_sample_format =
                    ( AVSampleFormat )atoi( opt_out_sample_format->value );
            }
            else
            {
            ...
            AVDictionaryEntry* opt_out_nb_channels =
                av_dict_get( *parameters->getOptions(), "out_nb_channels", NULL, 0 );
...
printf("### CONVERTING from sample format %s TO %s\n\t\tFROM %d TO %d channels\n\t\tFROM %d Hz to %d Hz\n",
            ...
            m_out_sample_rate);
                ...
                if( err )
                {
                    ...
                    OSG_WARN << "FFmpegDecoderAudio - WARNING: Error initializing "
                                "resampling context : "
    ...
    void
    FFmpegDecoderAudio::pause( bool pause )
    {
        if( pause != m_paused )
        {
            m_paused = pause;
    ...
    void
    FFmpegDecoderAudio::close( bool waitForThreadToExit )
    {
        if( isRunning() )
        {
            m_exit = true;
    ...
    void
    FFmpegDecoderAudio::setVolume( float volume )
    {
        if( m_audio_sink.valid() )
        {
    ...
    float
    FFmpegDecoderAudio::getVolume() const
    {
        if( m_audio_sink.valid() )
        {
    ...
    void
    FFmpegDecoderAudio::setAudioSink( osg::ref_ptr<osg::AudioSink> audio_sink )
    {
        // The FFmpegDecoderAudio object takes the responsibility of destroying the
        // audio_sink.
    ...
    void
    FFmpegDecoderAudio::fillBuffer( void* const buffer,
                                    size_t      size )
    {
        ...
        while( size != 0 )
        {
            if( m_audio_buf_index == m_audio_buf_size )
            {
                ...
                const size_t bytes_decoded =
                    decodeFrame( &m_audio_buffer[0], m_audio_buffer.size() );

                // If nothing could be decoded (e.g. error or no packet available),
                // output a bit of silence
                if( bytes_decoded == 0 )
                {
                    m_audio_buf_size =
                        std::min( Buffer::size_type( 1'024 ), m_audio_buffer.size() );
            ...
            const size_t fill_size =
                std::min( m_audio_buf_size - m_audio_buf_index, size );
    ...
    FFmpegDecoderAudio::decodeLoop()
    {
        ...
        while( !m_exit )
        {
            ...
            // Note: If m_audio_sink has an audio callback, this thread will still be
            // awaken from time to time to refill the audio buffer.
            else
            {
                osg::Thread::microSleep( 10'000 );
            }
        }
    }

    void
    FFmpegDecoderAudio::adjustBufferEndPts( const size_t buffer_size )
    {
        ...
        m_clocks.audioAdjustBufferEndPts( double( buffer_size ) /
                                          double( bytes_per_second ) );
    }

    size_t
    FFmpegDecoderAudio::decodeFrame( void* const  buffer,
                                     const size_t size )
    {
        for( ;; )
        {
            ...
            while( m_bytes_remaining > 0 )
            {
                ...
                const int bytes_decoded =
                    decode_audio( m_context,
            ...
            if( m_packet.type == FFmpegPacket::PACKET_DATA )
            {
                ...
                {
                    const double pts =
                        av_q2d( m_stream->time_base ) * m_packet.packet.pts;
    ...
    FFmpegDecoderAudio::sampleFormat() const
    {
