 ...
 * ReaderWriterFFmpeg, derived from ReaderWriter.
 * Provides: supportsProtocol, supportsProtocol, supportsProtocol, supportsProtocol,
...
class ReaderWriterFFmpeg : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterFFmpeg()
        {
            ...
            supportsOption(
                "format",
                "Force setting input format (e.g. vfwcap for Windows webcam)"
            );
            ...
            // WARNING:  This option is kept for backwards compatibility only, use
            // out_sample_rate instead!
            supportsOption( "audio_sample_rate",
                            "Set audio sampling rate (e.g. 44100)" );
            supportsOption( "out_sample_format",
                            "Set the output sample format (e.g. AV_SAMPLE_FMT_S16)" );
            supportsOption(
                "out_sample_rate",
                "Set the output sample rate or frequency in Hz (e.g. 48000)"
            );
            supportsOption( "out_nb_channels",
                            "Set the output number of channels (e.g. 2 for stereo)" );
            supportsOption( "context", "AVIOContext* for custom IO" );
            supportsOption( "mad", "Max analyze duration (seconds)" );
            supportsOption( "rtsp_transport",
                            "RTSP transport (udp, tcp, udp_multicast or http)" );
                   ...
                   const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( !acceptsExtension( ext ) && !acceptsProtocol( pro ) )
            {
            ...
            osg::ref_ptr<osgFFmpeg::FFmpegParameters> parameters(
                new osgFFmpeg::FFmpegParameters
            );
...
#if 1
            // NOTE: The original code checks parameters->isFormatAvailable() which
            // returns false when a format is not explicitly specified. In these cases,
            // the extension is used, which is a problem for videos served from URLs
        ...
        readImageStream( const std::string&           filename,
                         osgFFmpeg::FFmpegParameters* parameters ) const
        {
            OSG_INFO << "ReaderWriterFFmpeg::readImage " << filename << std::endl;

            osg::ref_ptr<osgFFmpeg::FFmpegImageStream> image_stream(
                new osgFFmpeg::FFmpegImageStream
            );
    ...
    private:
        ...
        void
        parseOptions( osgFFmpeg::FFmpegParameters*        parameters,
                      const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            {
                AVIOContext* context =
                    ( AVIOContext* )options->getPluginData( "context" );
                if( context != NULL )
                {
