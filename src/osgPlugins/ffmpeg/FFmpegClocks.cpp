...
namespace osgFFmpeg
{
    ...
    void
    FFmpegClocks::reset( const double start_time )
    {
    ...
    void
    FFmpegClocks::seek( double seek_time )
    {
        ScopedLock lock( m_mutex );
    ...
    void
    FFmpegClocks::audioSetBufferEndPts( const double pts )
    {
        ScopedLock lock( m_mutex );
    ...
    void
    FFmpegClocks::audioAdjustBufferEndPts( double increment )
    {
        ScopedLock lock( m_mutex );
    ...
    void
    FFmpegClocks::audioSetDelay( const double delay )
    {
        m_audio_delay = delay;
    }

    void
    FFmpegClocks::audioDisable()
    {
        ScopedLock lock( m_mutex );
    ...
    double
    FFmpegClocks::videoSynchClock( const AVFrame* const frame,
    ...
    double
    FFmpegClocks::videoRefreshSchedule( const double pts )
    {
        ScopedLock lock( m_mutex );
        ...
        // Ideally the frame time should be incremented after the actual delay is
        // computed. But because of the sound latency, it seems better to keep some
        // latency in the video too.
        m_frame_time            += delay;

        const double audio_time  = getAudioTime();
        const double actual_delay =
    ...
    double
    FFmpegClocks::getStartTime() const
    {
        return m_start_time;
    }

    void
    FFmpegClocks::setPauseTime( double pause_time )
    {
        m_pause_time += pause_time;
    }

    void
    FFmpegClocks::setSeekTime( double seek_time )
    {
        m_seek_time += getAudioTime() - seek_time;
    }

    double
    FFmpegClocks::getAudioTime() const
    {
        if( m_audio_disabled )
        {
            return m_audio_buffer_end_pts +
