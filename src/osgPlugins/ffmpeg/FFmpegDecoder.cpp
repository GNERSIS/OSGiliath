...
namespace osgFFmpeg
{
    ...
    FFmpegDecoder::FFmpegDecoder() :
        ...
        m_audio_queue( 100 ),
        m_video_queue( 100 ),
        m_audio_decoder( m_audio_queue,
                         m_clocks ),
        m_video_decoder( m_video_queue,
                         m_clocks ),
        m_state( NORMAL ),
        m_loop( false )
    {
    ...
    bool
    FFmpegDecoder::open( const std::string& filename,
                         FFmpegParameters*  parameters )
    {
        try
        {
            ...
            int error = avformat_open_input( &p_format_context,
                                             filename.c_str(),
                                             iformat,
            ...
            if( error != 0 )
            {
                ...
                switch( error )
                {
                    // case AVERROR_UNKNOWN: error_str = "AVERROR_UNKNOWN"; break;   //
                    // same value as AVERROR_INVALIDDATA
                    case AVERROR_IO :
                        error_str = "AVERROR_IO";
                        break;
                    case AVERROR_NUMEXPECTED :
                        error_str = "AVERROR_NUMEXPECTED";
                        break;
                    case AVERROR_INVALIDDATA :
                        error_str = "AVERROR_INVALIDDATA";
                        break;
                    case AVERROR_NOMEM :
                        error_str = "AVERROR_NOMEM";
                        break;
                    case AVERROR_NOFMT :
                        error_str = "AVERROR_NOFMT";
                        break;
                    case AVERROR_NOTSUPP :
                        error_str = "AVERROR_NOTSUPP";
                        break;
                    case AVERROR_NOENT :
                        error_str = "AVERROR_NOENT";
                        break;
                    case AVERROR_PATCHWELCOME :
                        error_str = "AVERROR_PATCHWELCOME";
                        break;
                    default :
            ...
            AVDictionaryEntry* mad =
                av_dict_get( *parameters->getOptions(), "mad", NULL, 0 );
            if( mad )
            {
            ...
            // Find and open the first video and audio streams (note that audio stream is
            // optional and only opened if possible)
            if( ( m_video_index = av_find_best_stream( m_format_context.get(),
                                                       AVMEDIA_TYPE_VIDEO,
                                                       -1,
                                                       -1,
                                                       NULL,
            ...
            if( ( m_audio_index = av_find_best_stream( m_format_context.get(),
                                                       AVMEDIA_TYPE_AUDIO,
                                                       -1,
                                                       -1,
                                                       NULL,
            ...
            {
                OSG_WARN << "FFmpegImageStream::open audio failed, audio stream will be "
                            "disabled: "
                         << rerror.what() << std::endl;
            }
        }
    ...
    void
    FFmpegDecoder::close( bool waitForThreadToExit )
    {
        flushAudioQueue();
        flushVideoQueue();
    ...
    void
    FFmpegDecoder::seek( double time )
    {
        m_pending_packet.clear();
    ...
    bool
    FFmpegDecoder::readNextPacketNormal()
    {
        AVPacket packet;
        ...
        if( !m_pending_packet )
        {
            ...
            if( error < 0 )
            {
                ...
                {
                    OSG_FATAL << "av_read_frame() returned " << AvStrError( error )
                              << std::endl;
    ...
    bool
    FFmpegDecoder::readNextPacketEndOfStream()
    {
    ...
    bool
    FFmpegDecoder::readNextPacketRewinding()
    {
        const FFmpegPacket packet( FFmpegPacket::PACKET_FLUSH );

        if( m_audio_queue.timedPush( packet, 10 ) &&
            m_video_queue.timedPush( packet, 10 ) )
        {
            m_state = NORMAL;
        }
    ...
    FFmpegDecoder::rewindButDontFlushQueues()
    {
        ...
        const int64_t seek_target =
            av_rescale_q( pos, AvTimeBaseQ, m_video_stream->time_base );

        int error = 0;
        if( ( error =
                  av_seek_frame( m_format_context.get(),
                                 m_video_index,
                                 seek_target,
                                 0 /*AVSEEK_FLAG_BYTE |*/ /*AVSEEK_FLAG_BACKWARD*/ ) ) <
            0 )
        {
    ...
    bool
    FFmpegDecoder::readNextPacketSeeking()
    {
        const FFmpegPacket packet( FFmpegPacket::PACKET_FLUSH );
    ...
    void
    FFmpegDecoder::seekButDontFlushQueues( double time )
    {
        const AVRational AvTimeBaseQ = { 1, AV_TIME_BASE };    // = AV_TIME_BASE_Q

        const int64_t    pos =
            int64_t( m_clocks.getStartTime() + time * double( AV_TIME_BASE ) );
        const int64_t seek_target =
            av_rescale_q( pos, AvTimeBaseQ, m_video_stream->time_base );
