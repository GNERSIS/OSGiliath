...
namespace osgFFmpeg
{
    ...
    class FormatContextPtr
    {
        public:
            ...
            explicit FormatContextPtr() :
                _ptr( 0 )
            {
            }

            explicit FormatContextPtr( T* ptr ) :
                _ptr( ptr )
            {
            }
            ...
            cleanup()
            {
                if( _ptr )
                {
                    ...
                    OSG_NOTICE << "Calling avformat_close_input(" << &_ptr << ")"
                               << std::endl;
                    avformat_close_input( &_ptr );
#else
                    OSG_NOTICE << "Calling av_close_input_file(" << _ptr << ")"
                               << std::endl;
    ...
    class FFmpegDecoder : public osg::Referenced
    {
        ...
        protected:
            ...
            flushVideoQueue();
            bool
            readNextPacketNormal();
            bool
            readNextPacketEndOfStream();
            bool
            readNextPacketRewinding();
            bool
            readNextPacketSeeking();
            bool
            readNextPacketPause();
            void
            rewindButDontFlushQueues();
            void
                               seekButDontFlushQueues( double time );
    ...
    inline void
    FFmpegDecoder::loop( const bool loop )
    {
        m_loop = loop;
    }

    inline bool
    FFmpegDecoder::loop() const
    {
        return m_loop;
    }

    inline double
    FFmpegDecoder::creation_time() const
    {
        if( m_format_context )
        {
    ...
    inline double
    FFmpegDecoder::duration() const
    {
        return double( m_format_context->duration ) / AV_TIME_BASE;
    }

    inline double
    FFmpegDecoder::reference()
    {
        return m_clocks.getCurrentTime();
    }

    inline FFmpegDecoderAudio&
    FFmpegDecoder::audio_decoder()
    {
        return m_audio_decoder;
    }

    inline FFmpegDecoderVideo&
    FFmpegDecoder::video_decoder()
    {
        return m_video_decoder;
    }

    inline const FFmpegDecoderAudio&
    FFmpegDecoder::audio_decoder() const
    {
        return m_audio_decoder;
    }

    inline const FFmpegDecoderVideo&
    FFmpegDecoder::video_decoder() const
    {
        return m_video_decoder;
    }
