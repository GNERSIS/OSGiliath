...
void
user_error_fn( png_structp /*png_ptr*/,
...
void
user_warning_fn( png_structp /*png_ptr*/,
...
void
png_read_istream( png_structp png_ptr,
                  png_bytep   data,
                  png_size_t  length )
{
    std::istream* stream =
        ( std::istream* )png_get_io_ptr( png_ptr );    // Get pointer to istream
    stream->read( ( char* )data, length );             // Read requested amount of data
}

void
png_write_ostream( png_structp png_ptr,
                   png_bytep   data,
                   png_size_t  length )
{
    std::ostream* stream =
        ( std::ostream* )png_get_io_ptr( png_ptr );    // Get pointer to ostream
    stream->write( ( char* )data, length );            // Write requested amount of data
}

void
png_flush_ostream( png_structp png_ptr )
{
    std::ostream* stream =
        ( std::ostream* )png_get_io_ptr( png_ptr );    // Get pointer to ostream
    stream->flush();
...
class ReaderWriterPNG : public osgDB::ReaderWriter
{
    public:
        ...
        writePngStream( std::ostream&     fout,
                        const osg::Image& img,
                        int               compression_level ) const
        {
            ...
            unsigned int numComponents =
                Image::computeNumComponents( img.getPixelFormat() );
            bitDepth = ( numComponents > 0 )
                         ? ( Image::computePixelSizeInBits( img.getPixelFormat(),
                                                            img.getDataType() ) /
                             numComponents )
                         : 0;
            ...
            // must take place after png_write_info: png_set_swap verifies 16bit depth
            // before setting transformation flag.
        ...
        ReadResult
        readPNGStream( std::istream& fin ) const
        {
            int           trans = PNG_ALPHA;
            pngInfo       pInfo;
            pngInfo*      pinfo = &pInfo;
...
#endif
            {
                ...
                if( fin.gcount() == 8 && png_sig_cmp( header, 0, 8 ) == 0 )
                {
                    png_set_read_fn(
                        ...
                        &fin,
                        png_read_istream
                    );    // Use custom read function that will get data from istream
                }
                else
                {
                ...
                OSG_DEBUG << "width=" << width << " height=" << height
                          << " depth=" << depth << std::endl;
                if( color == PNG_COLOR_TYPE_RGB )
                {
                    OSG_DEBUG << "color == PNG_COLOR_TYPE_RGB " << std::endl;
                }
                if( color == PNG_COLOR_TYPE_GRAY )
                {
                    OSG_DEBUG << "color == PNG_COLOR_TYPE_GRAY " << std::endl;
                }
                if( color == PNG_COLOR_TYPE_GRAY_ALPHA )
                {
                    OSG_DEBUG << "color ==  PNG_COLOR_TYPE_GRAY_ALPHA" << std::endl;
                }

                // png default to big endian, so we'll need to swap bytes if on a little
                // endian machine.
                ...
                data  = ( png_bytep ) new unsigned char[png_get_rowbytes( png, info ) *
                                                        height];
        ...
        int
        getCompressionLevel( const osgDB::ReaderWriter::Options* options ) const
        {
            if( options )
            {
