 ...
 * Utility functions for osgPlugins: defined, S_ISDIR, S_ISREG, _zipLoaded, archive.
 */
#include "unzip.hpp"
...
ZipArchive::close()
{
    if( _zipLoaded )
    {
        std::lock_guard<std::mutex> exclusive( _zipMutex );
        if( _zipLoaded )
        {
...
bool
ZipArchive::fileExists( const std::string& filename ) const
{
...
std::string
ZipArchive::getMasterFileName() const
{
    return std::string();
}

std::string
ZipArchive::getArchiveFileName() const
{
    std::string result;
    if( _zipLoaded )
    {
...
bool
ZipArchive::getFileNames( osgDB::Archive::FileNameList& fileNameList ) const
{
    if( _zipLoaded )
    {
                  ...
                  const osgDB::ReaderWriter::Options* options )
{
    if( !_zipLoaded )
    {
        ...
        std::lock_guard<std::mutex> exclusiveLock( _zipMutex );
...
bool
ZipArchive::open( std::istream&                       fin,
                  const osgDB::ReaderWriter::Options* options )
{
    if( !_zipLoaded )
    {
        ...
        if( !_zipLoaded )    // double-check avoids race condition
        {
            osgDB::ReaderWriter::ReadResult result = osgDB::ReaderWriter::ReadResult(
...
ZipArchive::readObject( const std::string&                  file,
                        const osgDB::ReaderWriter::Options* options ) const
{
    osgDB::ReaderWriter::ReadResult rresult =
        osgDB::ReaderWriter::ReadResult::FILE_NOT_HANDLED;
    ...
    if( ze != NULL )
    {
        ...
        if( rw != NULL )
        {
            ...
            osg::ref_ptr<osgDB::ReaderWriter::Options> local_opt =
                options ? static_cast<osgDB::ReaderWriter::Options*>(
                              options->clone( osg::CopyOp::SHALLOW_COPY )
                          )
                        : new osgDB::ReaderWriter::Options;

            local_opt->setPluginStringData( "STREAM_FILENAME",
                                            osgDB::getSimpleFileName( ze->name ) );

            osgDB::ReaderWriter::ReadResult readResult =
                rw->readObject( buffer, local_opt.get() );
...
osgDB::ReaderWriter::ReadResult
ZipArchive::readImage( const std::string&                  file,
                       const osgDB::ReaderWriter::Options* options ) const
{
    osgDB::ReaderWriter::ReadResult rresult =
        osgDB::ReaderWriter::ReadResult::FILE_NOT_HANDLED;
    ...
    if( ze != NULL )
    {
        ...
        if( rw != NULL )
        {
            ...
            osgDB::ReaderWriter::ReadResult readResult =
                rw->readImage( buffer, local_opt.get() );
            if( readResult.success() )
            {
                return readResult;
            }
...
osgDB::ReaderWriter::ReadResult
ZipArchive::readHeightField( const std::string&                  file,
                             const osgDB::ReaderWriter::Options* options ) const
{
    osgDB::ReaderWriter::ReadResult rresult =
        osgDB::ReaderWriter::ReadResult::FILE_NOT_HANDLED;
    ...
    if( ze != NULL )
    {
        ...
        if( rw != NULL )
        {
            // Setup appropriate options
            osg::ref_ptr<osgDB::ReaderWriter::Options> local_opt =
                options ? options->cloneOptions() : new osgDB::ReaderWriter::Options;
...
osgDB::ReaderWriter::ReadResult
ZipArchive::readNode( const std::string&                  file,
                      const osgDB::ReaderWriter::Options* options ) const
{
    osgDB::ReaderWriter::ReadResult rresult =
        osgDB::ReaderWriter::ReadResult::FILE_NOT_HANDLED;
...
osgDB::ReaderWriter::ReadResult
ZipArchive::readScript( const std::string&                  file,
                        const osgDB::ReaderWriter::Options* options ) const
{
    osgDB::ReaderWriter::ReadResult rresult =
        osgDB::ReaderWriter::ReadResult::FILE_NOT_HANDLED;
    ...
    if( ze != NULL )
    {
        ...
        if( rw != NULL )
        {
            ...
            osgDB::ReaderWriter::ReadResult readResult =
                rw->readScript( buffer, local_opt.get() );
            if( readResult.success() )
            {
                return readResult;
            }
...
osgDB::ReaderWriter::ReadResult
ZipArchive::readShader( const std::string&                  file,
                        const osgDB::ReaderWriter::Options* options ) const
{
    osgDB::ReaderWriter::ReadResult rresult =
        osgDB::ReaderWriter::ReadResult::FILE_NOT_HANDLED;
    ...
    if( ze != NULL )
    {
        ...
        if( rw != NULL )
        {
            ...
            osgDB::ReaderWriter::ReadResult readResult =
                rw->readShader( buffer, local_opt.get() );
            if( readResult.success() )
            {
                return readResult;
            }
...
ZipArchive::writeObject( const osg::Object& /*obj*/,
                         const std::string& /*fileName*/,
                         const osgDB::ReaderWriter::Options* ) const
{
    return osgDB::ReaderWriter::WriteResult(
        osgDB::ReaderWriter::WriteResult::FILE_NOT_HANDLED
    );
}

osgDB::ReaderWriter::WriteResult
ZipArchive::writeScript( const osg::Script& /*obj*/,
                         const std::string& /*fileName*/,
                         const osgDB::ReaderWriter::Options* ) const
{
    return osgDB::ReaderWriter::WriteResult(
...
osgDB::ReaderWriter::WriteResult
ZipArchive::writeImage( const osg::Image& /*image*/,
                        const std::string& /*fileName*/,
                        const osgDB::ReaderWriter::Options* ) const
{
    return osgDB::ReaderWriter::WriteResult(
...
osgDB::ReaderWriter::WriteResult
ZipArchive::writeHeightField( const osg::HeightField& /*heightField*/,
                              const std::string& /*fileName*/,
                              const osgDB::ReaderWriter::Options* ) const
{
    return osgDB::ReaderWriter::WriteResult(
...
osgDB::ReaderWriter::WriteResult
ZipArchive::writeNode( const osg::Node& /*node*/,
                       const std::string& /*fileName*/,
                       const osgDB::ReaderWriter::Options* ) const
{
    return osgDB::ReaderWriter::WriteResult(
...
osgDB::ReaderWriter::WriteResult
ZipArchive::writeShader( const osg::Shader& /*shader*/,
                         const std::string& /*fileName*/,
                         const osgDB::ReaderWriter::Options* ) const
{
    return osgDB::ReaderWriter::WriteResult(
...
osgDB::ReaderWriter*
ZipArchive::ReadFromZipEntry( const ZIPENTRY* ze,
                              const osgDB::ReaderWriter::Options* /*options*/,
                              std::stringstream& buffer ) const
{
    if( ze != 0 )
    {
        ...
        if( ibuf )
        {
            ...
            {
                ZRESULT result =
                    UnzipItem( data._zipHandle, ze->index, ibuf, ze->unc_size );
        ...
        {
            // std::cout << "Error- failed to allocate enough memory to unzip file '" <<
            // ze->name << ", with size '" << ze->unc_size << std::endl;
        }
    }
...
void
ZipArchive::IndexZipFiles( HZIP hz )
{
...
ZIPENTRY*
ZipArchive::GetZipEntry( const std::string& filename )
{
    ZIPENTRY*   ze         = NULL;
...
osgDB::FileType
ZipArchive::getFileType( const std::string& filename ) const
{
...
ZipArchive::getDirectoryContents( const std::string& dirName ) const
{
        ...
        if( iter->first.size() > searchPath.size() )
        {
            ...
            if( endSubElement == 0 )
            {
                std::string remainingFile =
                    iter->first.substr( searchPath.size() + 1, std::string::npos );
...
std::string
ZipArchive::ReadPassword( const osgDB::ReaderWriter::Options* options ) const
{
    ...
    if( options != NULL )
    {
        ...
        if( credentials != NULL )
        {
            const osgDB::AuthenticationDetails* details =
                credentials->getAuthenticationDetails( "ZipPlugin" );
            if( details != NULL )
            {
...
bool
ZipArchive::CheckZipErrorCode( ZRESULT result ) const
{
    if( result == ZR_OK )
    {
        return true;
    ...
    else
    {
        ...
        if( buf )
        {
            ...
            // print error message
            // sprintf(buf, "%s");
            OSG_WARN << "Error loading zip file: " << getArchiveFileName()
...
{
    std::lock_guard<std::mutex> exclusive( const_cast<ZipArchive*>( this )->_zipMutex );
...
ZipArchive::getDataNoLock() const
{
    ...
    size_t                           current = osg::Thread::CurrentThreadId();
    ...
    if( i == _perThreadData.end() || i->second._zipHandle == NULL )
    {
        ...
        // data does not already exist, so open the ZIP with a handle exclusively for
        // this thread:
        ...
        {
            data._zipHandle = OpenZip( ( void* )_membuffer.c_str(),
                                       _membuffer.length(),
                                       _password.c_str() );
        }
        else
