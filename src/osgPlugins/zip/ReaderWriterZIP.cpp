...
class ReaderWriterZIP : public osgDB::ReaderWriter
{
    public:
        ...
        virtual ReadResult
        openArchive( const std::string& file,
                     ArchiveStatus /*status*/,
                     unsigned int /*indexBlockSize*/ = 4'096,
                     const Options* options          = NULL ) const
        {
            ...
            // copy the incoming options if possible so that plugin options can be
            // applied to files inside the archive
        ...
        readNodeFromArchive( osgDB::Archive&                     archive,
                             const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            else
            {
                ...
                if( archive.getFileNames( fileNameList ) )
                {
                    ...
                    Nodes                                      nodes;
                    for( osgDB::Archive::FileNameList::iterator itr =
                             fileNameList.begin();
                    ...
                    if( !nodes.empty() )
                    {
                        ...
                        {
                            result =
                                osgDB::ReaderWriter::ReadResult( nodes.front().get() );
                        }
                        else
                        {
                            osg::ref_ptr<osg::Group> group = new osg::Group;
                            for( Nodes::iterator itr = nodes.begin(); itr != nodes.end();
                                 ++itr )
                            {
        ...
        {
            osgDB::ReaderWriter::ReadResult result =
                openArchive( file, osgDB::Archive::READ );
            ...
            if( !options || ( options->getObjectCacheHint() &
                              osgDB::ReaderWriter::Options::CACHE_ARCHIVES ) )
            {
        ...
        osgDB::ReaderWriter::ReadResult
        readImageFromArchive( osgDB::Archive&                     archive,
                              const osgDB::ReaderWriter::Options* options ) const
        {
            osgDB::ReaderWriter::ReadResult result(
                osgDB::ReaderWriter::ReadResult::FILE_NOT_FOUND
