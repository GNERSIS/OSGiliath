...
class ReaderWriterGLSL : public osgDB::ReaderWriter
{
    public:
        ...
        void
        processIncludes( osg::Shader&   shader,
                         const Options* options ) const
        {
            ...
            while( ( pos != std::string::npos ) &&
                   ( ( ( pragma_pos = code.find( "#pragma", pos ) ) !=
                       std::string::npos ) ||
                     ( include_pos = code.find( "#include", pos ) ) !=
                     std::string::npos ) )
            {
                ...
                {
                    // we have #pragma usage so skip to the start of the first non white
                    // space
                ...
                std::string::size_type num_characters =
                    ( end_of_line == std::string::npos ) ? code.size() - pos
                                                         : end_of_line - pos;
                if( num_characters == 0 )
                {
                    continue;
                ...
                code.erase( start_of_pragma_line,
                            ( end_of_line == std::string::npos )
                                ? code.size() - start_of_pragma_line
                                : end_of_line - start_of_pragma_line );
                pos = start_of_pragma_line;

                osg::ref_ptr<osg::Shader> innerShader =
                    osgDB::readRefShaderFile( filename, options );
                    ...
                    const Options* options ) const
        {
            ...
            if( options )
            {
                ...
                if( options->getOptionString().find( "tesscontrol" ) !=
                    std::string::npos )
                {
                    shader->setType( osg::Shader::TESSCONTROL );
                }
                if( options->getOptionString().find( "tessevaluation" ) !=
                    std::string::npos )
                {
        ...
        virtual ReadResult
        readShader( const std::string&                  file,
                    const osgDB::ReaderWriter::Options* options ) const
        {
