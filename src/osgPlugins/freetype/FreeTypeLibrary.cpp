 ...
 * Utility functions for osgPlugins: FT_Init_FreeType, lock, FreeTypeFont.
 */
...
FreeTypeLibrary::FreeTypeLibrary()
{
    ...
    if( error )
    {
        OSG_WARN << "Warning: an error occurred during FT_Init_FreeType(..) "
                    "initialisation, error code = "
...
bool
FreeTypeLibrary::getFace( const std::string& fontfile,
                          unsigned int       index,
                          FT_Face&           face )
{
    ...
    {
        OSG_WARN << " .... the font file could be opened and read, but it appears"
                 << std::endl;
    ...
    {
        OSG_WARN << " .... another error code means that the font file could not"
                 << std::endl;
    ...
    for( unsigned int i = 0; i < count; ++i )
    {
        ...
        std::string name( ( char* )names.string,
                          ( char* )names.string + names.string_len );
...
FT_Byte*
FreeTypeLibrary::getFace( std::istream& fontstream,
                          unsigned int  index,
                          FT_Face&      face )
{
    std::lock_guard<std::mutex> lock( getMutex() );
    ...
    /* empty stream into memory, open that, and keep the pointer in a FreeTypeFont for
     * cleanup */
...
osgText::Font*
FreeTypeLibrary::getFont( const std::string& fontfile,
                          unsigned int       index,
                          unsigned int       flags )
{
    FT_Face face;
...
osgText::Font*
FreeTypeLibrary::getFont( std::istream& fontstream,
                          unsigned int  index,
                          unsigned int  flags )
{
    FT_Face  face   = 0;
...
void
FreeTypeLibrary::verifyCharacterMap( FT_Face face )
{
    //
