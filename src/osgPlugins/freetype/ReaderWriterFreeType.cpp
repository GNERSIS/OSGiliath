...
class ReaderWriterFreeType : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterFreeType()
        {
            ...
            supportsOption(
                "index=<uint>",
                "Select index of font within ttc collection. Defaults to 0."
            );
        }
        ...
        static unsigned int
        getFlags( const osgDB::ReaderWriter::Options* options )
        {
            unsigned int flags = 0;
            if( options &&
                options->getOptionString().find( "monochrome" ) != std::string::npos )
            {
        ...
        static unsigned int
        getIndex( const osgDB::ReaderWriter::Options* options )
        {
            if( !options )
            {
                return 0;
            ...
            {
                OSG_WARN
                    << "Warning: invalid index string (" << indexstr
                    << ") when loading freetype font. Attempting to use default index 0."
                    << std::endl;
                return 0;
            }
            else
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( !freeTypeLibrary )
            {
                OSG_WARN << "Warning:: cannot create freetype font after freetype "
                            "library has been deleted."
                         << std::endl;
                return ReadResult::ERROR_IN_READING_FILE;
            }

            return freeTypeLibrary->getFont( fileName,
                                             getIndex( options ),
                                             getFlags( options ) );
        }
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            return freeTypeLibrary->getFont( stream,
                                             getIndex( options ),
                                             getFlags( options ) );
        }
};
