 ...
 * Utility functions for osgPlugins: _verts, completeCurrentPrimitiveSet, get, addVertex,
 * moveTo.
 */
...
namespace FreeType
{
    ...
    struct Char3DInfo
    {
            ...
            void
            lineTo( const osg::vec2& pos )
            {
                addVertex( osg::vec3( pos.x, pos.y, 0 ) );
            }

            void
            conicTo( const osg::vec2& control,
                     const osg::vec2& pos )
            {
                     ...
                     const osg::vec2& pos )
            {
                ...
                for( int i = 0; i <= _numSteps; ++i )
                {
                    osg::vec3 p = osg::vec3( ax * u * u * u + bx * u * u + cx * u + p0.x,
    ...
    {
        Char3DInfo* char3d = ( Char3DInfo* )user;
        char3d->conicTo( osg::vec2( control->x, control->y ),
                         osg::vec2( to->x, to->y ) );
        return 0;
    }
    ...
    {
        Char3DInfo* char3d = ( Char3DInfo* )user;
        char3d->cubicTo( osg::vec2( control1->x, control1->y ),
...
FreeTypeFont::FreeTypeFont( const std::string& filename,
                            FT_Face            face,
                            unsigned int       flags ) :
    _currentRes( osgText::FontResolution( 0,
                                          0 ) ),
    _filename( filename ),
    _buffer( 0 ),
    _face( face ),
...
FreeTypeFont::FreeTypeFont( FT_Byte*     buffer,
                            FT_Face      face,
                            unsigned int flags ) :
    _currentRes( osgText::FontResolution( 0,
                                          0 ) ),
...
void
FreeTypeFont::setFontResolution( const osgText::FontResolution& fontSize )
{
    ...
    if( error )
    {
        OSG_WARN << "FT_Set_Pixel_Sizes() - error 0x" << std::hex << error << std::dec
                 << std::endl;
    }
    else
    {
...
osgText::Glyph*
FreeTypeFont::getGlyph( const osgText::FontResolution& fontRes,
                        unsigned int                   charcode )
{
    std::lock_guard<std::mutex> lock( FreeTypeLibrary::instance()->getMutex() );
    ...
    // returned, for symbol fonts in windows (FT_ENCONDING_MS_SYMBOL in freetype) the
    // correct values are from 0xF000 to 0xF0FF not from 0x000 to 0x00FF (0 to 255) as
    // you would expect. Microsoft uses a private field for its symbol fonts
    //
    ...
    FT_Error error =
        FT_Load_Char( _face, charindex, FT_LOAD_RENDER | FT_LOAD_NO_BITMAP | _flags );
    if( error )
    {
        OSG_WARN << "FT_Load_Char(...) error 0x" << std::hex << error << std::dec
                 << std::endl;
        return 0;
    }
    ...
    switch( glyphslot->bitmap.pixel_mode )
    {
        ...
        default :
            OSG_WARN << "FT_Load_Char(...) returned bitmap with unknown pixel_mode "
                     << glyphslot->bitmap.pixel_mode << std::endl;
    }
    ...
    glyph->setHorizontalBearing(
        osg::vec2( ( float )metrics->horiBearingX * coord_scale,
                   ( float )( metrics->horiBearingY - metrics->height ) * coord_scale )
    );    // bottom left.
    glyph->setHorizontalAdvance( ( float )metrics->horiAdvance * coord_scale );
    glyph->setVerticalBearing(
        osg::vec2( ( float )metrics->vertBearingX * coord_scale,
                   ( float )( metrics->vertBearingY - metrics->height ) * coord_scale )
    );    // top middle.
    ...
    // cout << "      in getGlyph() implementation="<<this<<"  "<<_filename<<"
    // facade="<<_facade<<endl;
...
osgText::Glyph3D*
FreeTypeFont::getGlyph3D( const osgText::FontResolution& fontRes,
                          unsigned int                   charcode )
{
    std::lock_guard<std::mutex> lock( FreeTypeLibrary::instance()->getMutex() );
    ...
    FT_Error error =
        FT_Load_Char( _face, charindex, FT_LOAD_DEFAULT | FT_LOAD_NO_HINTING | _flags );
    if( error )
    {
        OSG_WARN << "FT_Load_Char(...) error 0x" << std::hex << error << std::dec
                 << std::endl;
    ...
    if( _error )
    {
        OSG_WARN << "FreeTypeFont3D::getGlyph : - outline decompose failed ..."
                 << std::endl;
        return 0;
    }
    ...
    for( osg::Geometry::PrimitiveSetList::iterator itr =
             char3d.get()->getPrimitiveSetList().begin();
         itr != char3d.get()->getPrimitiveSetList().end();
         ++itr )
    {
        rawPrimitives.push_back( dynamic_cast<osg::PrimitiveSet*>(
            ( *itr )->clone( osg::CopyOp::DEEP_COPY_ALL )
        ) );
    }

    // ** save vertices and PrimitiveSetList of each face in the Glyph3D PrimitiveSet
    // face list
    ...
    osg::box bb( float( ftbb.xMin ) * coord_scale,
                 float( ftbb.yMin ) * coord_scale,
                 0.0F,
                 float( ftbb.xMax ) * coord_scale,
                 float( ftbb.yMax ) * coord_scale,
                 0.0F );
...
osg::vec2
FreeTypeFont::getKerning( const osgText::FontResolution& fontRes,
                          ...
                          osgText::KerningType           kerningType )
{
    ...
    FT_Kerning_Mode mode = ( kerningType == osgText::KERNING_DEFAULT )
                             ? ft_kerning_default
                             : ft_kerning_unfitted;
    ...
    if( error )
    {
        OSG_WARN << "FT_Get_Kerning(...) returned error code " << std::hex << error
                 << std::dec << std::endl;
        return osg::vec2( 0.0F, 0.0F );
    }
    ...
    return osg::vec2( ( float )kerning.x * coord_scale,
                      ( float )kerning.y * coord_scale );
}

bool
FreeTypeFont::hasVertical() const
{
    std::lock_guard<std::mutex> lock( FreeTypeLibrary::instance()->getMutex() );
...
bool
FreeTypeFont::getVerticalSize( float& ascender,
...
float
FreeTypeFont::getCoordScale() const
{
