    ...
    public:
        ...
        struct logosCullCallback : public osg::DrawableCullCallback
        {
                      ...
                      osg::State* ) const
                {
                    ...
                    {
                        // logo not appropriate for window assigned to the cull visitor
                        // so cull it.
                        return true;
                    }
                    ...
                    {
                        if( vp->width() !=
                            logos->getViewport()->width() ||
                            vp->height() != logos->getViewport()->height() )
                        {
                            logos->getViewport()->setViewport( vp->x(),
                                                               vp->y(),
                                                               vp->width(),
                                                               vp->height() );
...
#if 1
            // for now we'll crudely set the bin number to 100 to force it to draw later
            // and ontop of the scene
        ...
        {
            OSG_NOTICE << "Warning: Logos::drawImplementation(..) not supported."
                       << std::endl;
        }

        void
        addLogo( RelativePosition pos,
                 std::string      name )
        {
        ...
        {
            return osg::box( -1, -1, -1, 1, 1, 1 );
        }

    protected:

        Logos&
        operator=( const Logos& )
        {
            return *this;
        }
                  ...
                  const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            while( fin )
            {
                ...
                else if( str == "Camera" )
                {
                    ...
                    if( tn < 0 )
                    {
                        OSG_WARN << "Error... Camera requires an positive or null value "
                                    "argument\n";
                        break;
                    }
