 ...
 * ReaderWriterDICOM, derived from ReaderWriter.
...
class ReaderWriterDICOM : public osgDB::ReaderWriter
{
    public:
        ...
        bool
        isFileADicom( const std::string& filename ) const
        {
            ...
            return (
                str[128] == 'D' && str[129] == 'I' && str[130] == 'C' && str[131] == 'M'
            );
        }
        ...
        bool
        getDicomFilesInDirectory( const std::string& path,
                                  Files&             files ) const
        {
            osgDB::DirectoryContents contents =
                osgDB::getSortedDirectoryContents( path );
                  ...
                  const osgDB::ReaderWriter::Options* options = NULL ) const
        {
            ...
            osg::ref_ptr<osgVolume::ImageLayer> layer =
                new osgVolume::ImageLayer( result.getImage() );
            ...
            osgVolume::AlphaFuncProperty* ap =
                new osgVolume::AlphaFuncProperty( alphaFunc );
            ...
            osgVolume::ImageDetails* details = dynamic_cast<osgVolume::ImageDetails*>(
                result.getImage()->getUserData()
            );
                   ...
                   const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            for( Images::iterator itr = images.begin(); itr != images.end(); ++itr )
            {
                osg::Image*              image = itr->get();
                osgVolume::ImageDetails* details =
                    dynamic_cast<osgVolume::ImageDetails*>( image->getUserData() );
                osg::RefMatrix* matrix = details ? details->getMatrix() : 0;
                if( matrix )
                {
                    osg::vec3 p0 =
                        osg::vec3( ( *matrix ) * osg::dvec3( 0.0, 0.0, 0.0 ) );
                    osg::vec3 p1 =
                        osg::vec3( ( *matrix ) * osg::dvec3( 0.0, 0.0, 1.0 ) );
                    osg::vec3 direction = p1 - p0;
                    direction           = osg::normalize( direction );
                    float distance      = p0 * direction;
                    info() << " direction=" << direction << "  distance = " << distance
                           << std::endl;
            ...
            int               height         = 0;
            int               depth          = 0;
            for( DistanceImageMap::iterator itr = dim.begin(); itr != dim.end(); ++itr )
            {
            ...
            image3D->allocateImage( width, height, depth, GL_RED, GL_UNSIGNED_BYTE, 1 );
            int r = 0;
            for( DistanceImageMap::iterator itr = dim.begin(); itr != dim.end(); ++itr )
            {
                osg::Image* image = itr->second.get();
            ...
            osgVolume::ImageDetails* details =
                dynamic_cast<osgVolume::ImageDetails*>( firstImage->getUserData() );
            osg::RefMatrix* matrix = details ? details->getMatrix() : 0;
            if( matrix )
            {
                osgVolume::ImageDetails* details3D =
                    new osgVolume::ImageDetails( *details );
                details3D->getMatrix()->preMult(
                    osg::scale( 1.0, 1.0, totalThickness )
                );
        ...
        virtual ReadResult
        readSingleITKImage( const std::string&                  fileName,
                            const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            image->allocateImage( width, height, depth, GL_RED, GL_UNSIGNED_BYTE, 1 );
            ...
            matrix->preMult( osg::scale( double( image->s() ),
                                         double( image->t() ),
                                         double( image->r() ) ) );
        ...
        void
        convertPixelTypes( const DiPixel*     pixelData,
        ...
        struct SeriesIdentifier
        {
                ...
                void
                set( DcmDataset* dataset )
                {
                    OFString seriesInstanceUIDStr;
                    if( dataset
                            ->findAndGetOFString( DCM_SeriesInstanceUID,
                    ...
                    if( dataset
                            ->findAndGetOFString( DCM_SeriesDescription,
                                                  seriesDescriptionStr )
                            .good() )
                    {
                ...
                bool
                operator==( const SeriesIdentifier& rhs ) const
                {
                   ...
                   const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            for( Files::iterator itr = files.begin(); itr != files.end(); ++itr )
            {
                ...
                // code for reading the intercept and scale that is required to convert
                // to Hounsfield units.
                ...
                if( fileformat.getDataset()
                        ->findAndGetString( DCM_SOPClassUID, classUID )
                        .good() )
                {
                ...
                rescaling =
                    fileformat.getDataset()
                        ->findAndGetFloat64( DCM_RescaleIntercept, rescaleIntercept )
                        .good();
                rescaling &= fileformat.getDataset()
                                 ->findAndGetFloat64( DCM_RescaleSlope, rescaleSlope )
                                 .good();
                if( rescaling )
                {
                ...
                double value = 0.0;
                if( fileformat.getDataset()
                        ->findAndGetFloat64( DCM_PixelSpacing, value, 0 )
                        .good() )
                {
                ...
                if( fileformat.getDataset()
                        ->findAndGetFloat64( DCM_PixelSpacing, value, 1 )
                        .good() )
                {
                ...
                if( fileformat.getDataset()
                        ->findAndGetFloat64( DCM_SpacingBetweenSlices, value, 0 )
                        .good() )
                {
                ...
                // Get slice thickness
                if( fileformat.getDataset()
                        ->findAndGetFloat64( DCM_SliceThickness, value )
                        .good() )
                {
                ...
                info() << "tagExistsWithValue(DCM_NumberOfFrames)="
                       << fileformat.getDataset()->tagExistsWithValue(
                              DCM_NumberOfFrames
                          )
                       << std::endl;
                info() << "tagExistsWithValue(DCM_NumberOfSlices)="
                       << fileformat.getDataset()->tagExistsWithValue(
                              DCM_NumberOfSlices
                          )
                       << std::endl;
                ...
                Uint32 numFrames   = 1;
                if( fileformat.getDataset()
                        ->findAndGetUint32( DCM_NumberOfFrames, numFrames )
                        .good() )
                {
                ...
                if( fileformat.getDataset()
                        ->findAndGetOFString( DCM_NumberOfFrames, numFramesStr )
                        .good() )
                {
                ...
                if( fileformat.getDataset()
                        ->findAndGetUint16( DCM_NumberOfFrames, numOfSlices )
                        .good() )
                {
                ...
                if( fileformat.getDataset()
                        ->findAndGetUint16( DCM_NumberOfSlices, numOfSlices )
                        .good() )
                {
                    // fileInfo.numSlices = numOfSlices;
                ...
                {
                    if( fileformat.getDataset()
                            ->findAndGetFloat64( DCM_ImagePositionPatient,
                                                 imagePositionPatient[i],
                                                 i )
                            .good() )
                    {
                        info() << "Read DCM_ImagePositionPatient[" << i << "], "
                               << imagePositionPatient[i] << std::endl;
                    }
                    else
                    {
                        info() << "Have not read DCM_ImagePositionPatient[" << i << "]"
                               << std::endl;
                    }
                }
                fileInfo.position.set( imagePositionPatient[0],
                ...
                for( int i = 0; i < 6; ++i )
                {
                    value = 0.0;
                    if( fileformat.getDataset()
                            ->findAndGetFloat64( DCM_ImageOrientationPatient, value, i )
                            .good() )
                    {
                        imageOrientationPatient[i] = value;
                        info() << "Read imageOrientationPatient[" << i << "], "
                               << imageOrientationPatient[i] << std::endl;
                    }
                    else
                    {
                        info() << "Have not read imageOrientationPatient[" << i << "]"
                               << std::endl;
                    }
                }

                fileInfo.dirX.set( imageOrientationPatient[0],
                                   imageOrientationPatient[1],
                                   imageOrientationPatient[2] );
                fileInfo.dirY.set( imageOrientationPatient[3],
                                   imageOrientationPatient[4],
                                   imageOrientationPatient[5] );
                fileInfo.dirZ     = fileInfo.dirX ^ fileInfo.dirY;
                fileInfo.dirZ     = osg::normalize( dirZ );
                ...
                info() << "osg::length(dirX) = " << fileInfo.osg::length( dirX )
                       << std::endl;
                info() << "osg::length(dirY) = " << fileInfo.osg::length( dirY )
                       << std::endl;
            ...
            for( SeriesFileInfoMap::iterator itr = seriesFileInfoMap.begin();
                 ...
                 ++itr )
            {
                notice() << "Description = " << itr->first.SeriesDescription
                         << ", Orientation = " << itr->first.Orientation << std::endl;
                     ...
                     ++ditr )
                {
                    ...
                    info() << "   d = " << fileInfo.distance << " " << fileInfo.filename
                           << " fileInfo.numSlices=" << fileInfo.numSlices << std::endl;
                }
                ...
                {
                    totalDistance = dfim.begin()->second.sliceThickness *
                                    double( dfim.begin()->second.numSlices );
                }
                ...
                double averageThickness =
                    totalNumSlices <= 1 ? 1.0
                                        : totalDistance / double( totalNumSlices - 1 );
                     ...
                     ++ditr )
                {
                    ...
                    if( dcmImage.getStatus() == EIS_Normal )
                    {
                        ...
                        {
                            warning()
                                << "Error: no data in DicomImage object." << std::endl;
                            return ReadResult::ERROR_IN_READING_FILE;
                        }
                        ...
                        {
                            imageAdapter->allocateImage( dcmImage.getWidth(),
                                                         dcmImage.getHeight(),
                                                         dcmImage.getFrameCount(),
                                                         curr_pixelFormat,
                                                         curr_dataType );
                            ...
                            unsigned long size = dcmImage.createWindowsDIB(
                                data,
                            ...
                            if( size == 0 )
                            {
                                info() << "  dcmImage.createWindowsDIB() failed to "
                                          "create required imagery."
                                       << std::endl;
                                continue;
                            }
                        }
                        ...
                        if( !image )
                        {
                            ...
                            matrix->preMultScale(
                                osg::dvec3( fileInfo.pixelSize_x * dcmImage.getWidth(),
                            ...
                            // note from Robert Osfield, testing various dicom files I
                            // have found that the rescaleIntercept for CT data doesn't
                            // look to be applicable as an straight value offset, so
                            ...
                            image->allocateImage( dcmImage.getWidth(),
                                                  dcmImage.getHeight(),
                                                  totalNumSlices,
                                                  pixelFormat,
                                                  dataType );
                            ...
                            // matrix->preMult(osg::scale(double(image->s()),
                            // double(image->t()), double(image->r())));

                            info()
                                << "Image dimensions = " << image->s() << ", "
                                << image->t() << ", " << image->r() << " pixelFormat=0x"
                                << std::hex << pixelFormat << " dataType=0x" << std::hex
                                << dataType << std::dec << std::endl;
                        }
                        else if( pixelData->getPlanes() >
                                 numPlanes ||
                                 pixelData->getRepresentation() > pixelRep )
                        {
                            info() << "Need to reallocated " << image->s() << ", "
                                   << image->t() << ", " << image->r() << std::endl;

                            // record the previous image settings to use when we copy
                            // back the content.
                            ...
                            image->allocateImage( dcmImage.getWidth(),
                                                  ...
                                                  pixelFormat,
                                                  dataType );
                            osg::copyImage( previous_image.get(),
                                            0,
                                            0,
                                            0,
                                            previous_image->s(),
                        ...
                        info() << "copyImage(, fileInfo.distance" << fileInfo.distance
                               << ", imageNum=" << imageNum << std::endl;

                        osg::copyImage( imageAdapter.get(),
                                        0,
                                        0,
                                        0,
                                        imageAdapter->s(),
                    ...
                    {
                        warning() << "Error in reading dicom file " << fileInfo.filename
                                  << ", error = "
                                  << DicomImage::getString( dcmImage.getStatus() )
                                  << std::endl;
                        info() << "    dcmImage.getPhotometricInterpretation()="
                               << DicomImage::getString(
                                      dcmImage.getPhotometricInterpretation()
                                  )
                               << std::endl;
                        info()
                            << "    dcmImage.width=" << dcmImage.getWidth()
                            << ", height=" << dcmImage.getHeight()
                            << " FrameCount=" << dcmImage.getFrameCount() << std::endl;
                    }
                }
            ...
            if( !errorMap.empty() )
            {
                for( ErrorMap::iterator itr = errorMap.begin(); itr != errorMap.end();
                     ++itr )
                {
