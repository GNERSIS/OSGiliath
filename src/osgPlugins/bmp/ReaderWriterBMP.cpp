...
static unsigned int
findLeastSignificantBit( unsigned short mask )
{
...
static unsigned int
findMostSignificantBit( unsigned short mask )
{
    unsigned int shift = 16;
    while( ( mask & 0X80'00 ) == 0 )
    {
        mask <<= 1;
        --shift;
    }
 ...
 * BMP header
 */
const unsigned short BMP_MAGIC_BM = 0X42'4D;    // 'BM'
const unsigned short BMP_MAGIC_MB = 0X4D'42;    // 'MB'
          ...
          int&          numComponents_ret )
{
    ...
    BMPHeader bmp;
    {
        ...
        swap = ( bmp.magic ==
                 BMP_MAGIC_BM );    // means machine is big-endian and must swap
        if( swap )
        {
        ...
        {
            OSG_DEBUG << "Stored BMP fileSize=" << bmp.fileSize
    ...
    if( dibHdrSize == 12 )
    {
        ...
        {
            OSG_WARN << "Invalid BMP OS/2 v1 header size " << expectHdrSize
                     << " != " << dibHdrSize << std::endl;
            return 0;
        }
    ...
    {
        OSG_WARN << "Invalid BMP number of color planes=" << dib.colorPlanes
                 << std::endl;
        return 0;
    }
    ...
    if( dib.bitsPerPixel < 16 )
    {
        ...
        imageBytesPerPixel =
            ( dibHdrSize == 12 ? 3 : 4 );    // OS/2 v1 stores RGB, else RGBA
    ...
    if( where != bmp.imageOffset )
    {
        ...
        OSG_DEBUG << "BMP streampos out-of-sync where=" << where
                  << " imageOffset=" << bmp.imageOffset << std::endl;
        fin.seekg( bmp.imageOffset,
                   std::ios::beg );    // seek to imageOffset and hope for the best
    }
    ...
    const unsigned int bytesPerRowAlign =
        ( unsigned int )ceilf( bytesPerRow / 4.0F ) * 4;
    // printf("bytesPerPixel=%u bytesPerRow=%u bytesPerRowAlign=%u\n", bytesPerPixel,
    // bytesPerRow, bytesPerRowAlign);
    ...
    else
    {
        ...
        for( int i = 0; i < dib.height; ++i )
        {
            ...
            while( j < dib.width )
            {
                ...
                {
                    unsigned int idx =
                        ( val >> ( ( idxPerByte - 1 - k ) * dib.bitsPerPixel ) ) &
                        idxMask;
          ...
          std::ostream&     fout )
{
    ...
    // DIB header
    {
        ...
        dib.horizontalPixelPerMeter = 1'000;
        dib.verticalPixelPerMeter   = 1'000;
