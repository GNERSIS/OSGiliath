 ...
 * Utility functions for osgPlugins: SDL_VERSION_ATLEAST, addMouseButtonMapping,
 * addKeyMapping.
 */
...
void
JoystickDevice::capture( ValueList& axisValues,
...
JoystickDevice::checkEvents()
{
    if( _joystick )
    {
        ...
        float projected_mx = previous_event->getXmin() +
                             ( mx + 1.0 ) *
                             0.5 *
                             ( previous_event->getXmax() - previous_event->getXmin() );
        float projected_my = previous_event->getYmin() +
                             ( my + 1.0 ) *
                             0.5 *
                             ( previous_event->getYmax() - previous_event->getYmin() );
        ...
        OSG_NOTICE << "mx=" << mx << ", my=" << my << ", projected_mx=" << projected_mx
                   << ", projected_my=" << projected_my << std::endl;
        ...
        if( _verbose )
        {
            for( int ai = 0; ai < _numAxes; ++ai )
            {
                ...
                {
                    std::cout << "axis " << ai << " moved to " << newAxisValues[ai]
                              << std::endl;
                }
            }
        }
        ...
        for( int bi = 0; bi < _numButtons; ++bi )
        {
            if( newButtonValues[bi] != _buttonValues[bi] )
            {
                if( _verbose )
                {
                    std::cout << "button " << bi << " changed to " << newButtonValues[bi]
                              << std::endl;
                }
                ...
                if( mouseButton > 0 )
                {
                    ...
                    {
                        eq->mouseButtonRelease( projected_mx,
                                                projected_my,
                                                mouseButton,
                                                time );
                    }
                    ...
                    {
                        eq->mouseButtonPress( projected_mx,
                                              projected_my,
                                              mouseButton,
                                              time );
                    }
