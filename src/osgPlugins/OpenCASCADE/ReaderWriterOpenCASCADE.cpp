...
#include "ReaderWriterOpenCASCADE.hpp"
...
REGISTER_OSGPLUGIN( OpenCASCADE,
...
ReaderWritterOpenCASCADE::readNode( const std::string&                  fileName,
...
ReaderWritterOpenCASCADE::writeNode( const osg::Node& /*node*/,
...
void
ReaderWritterOpenCASCADE::OCCTKReader::_healShape( TopoDS_Shape& shape )
{
#ifdef _LOG_DEBUG_
...
/// \brief takes and OpenCascadeShape and returns OSG geometry(drawable), which further
/// can be added to a geode
/// \detail it iterates shape and breaks it into faces, builds vertex list, color list
/// and creates geometry
///         transformation is applied to each vertex before storing it into vertex list
///         all vertices are assigned same color
/// \param shape shape to be converted in geometry. Not a const because it needs to be
/// modified if healing
///        is enabled
/// \param color color of geometry
/// \param transformation matrix with which vertex position has to be transformed
osg::ref_ptr<osg::Geometry>
ReaderWritterOpenCASCADE::OCCTKReader::_createGeometryFromShape(
    TopoDS_Shape&    shape,
    const osg::vec3& geomColor,
...
)
{
    ...
    osg::ref_ptr<osg::DrawElementsUInt> triangleStrip =
        ...
        for( TopExp_Explorer ex( shape, TopAbs_FACE ); ex.More(); ex.Next() )
        {
            ...
            if( !triangulation.IsNull() )
            {
                ...
                for( int j = 1; j <= triangulation->NbNodes(); j++ )
                {
                    ...
                    gp_Pnt pt = ( triangulation->Nodes() )( j ).Transformed(
                ...
                {
                    /// If face direction is reversed then we add verticews in reverse
                    /// order order of vertices is important for normal calculation later
...
///        which directly can be used anywhere. It calculates normals using
///        osgUtil::smoother
osg::ref_ptr<osg::Geode>
ReaderWritterOpenCASCADE::OCCTKReader::igesToOSGGeode( const std::string& filePath )
{
    ...
    if( ext == "stp" || ext == "step" )
    {
        OSG_NOTICE << "Using STEPCAFControl_Reader to read file : " << filePath
                   << std::endl;
    ...
    {
        OSG_NOTICE << "Using IGESCAFControl_Reader to read file : " << filePath
                   << std::endl;
    ...
    // To get a node considered as an Assembly from an XDE structure, you can use the
    // Label of the node.
...
/// calculate normals
    ...
    sv.setCreaseAngle( osg::radians( 20.0 ) );
...
///        this function also finds color for leaf node shapes and calculates
///        transformation from parent to leaf
/// \param shapeTree its a OCT(OpenCascade Technology) XDE document label which might
/// contain children or referred shapes
/// \param transformation contains transformation matrix to be applied
/// \note Simple Shape: is a shape which is not a compound. Its can be a free or non free
/// shape
/// \note Support Thread: http://www.opencascade.org/org/forum/thread_25512/?forum=3
void
ReaderWritterOpenCASCADE::OCCTKReader::_traverse( const TDF_Label& shapeTree,
                                                  gp_Trsf&         transformation )
{
    ...
    {
        /// Find out if this simple shape has any color store that color as color of
        /// geometry
...
#ifdef _LOG_DEBUG_
            std::cout << std::endl
                      << "Free Shape has a color !! " << color.Red() << " "
        ...
        osg::ref_ptr<osg::Geometry> geom =
            _createGeometryFromShape( shape, geomColor, transformation );
