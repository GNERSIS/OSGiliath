 ...
 * LightPoint, derived from PrimaryRecord.
 * Provides: _material, _feature, _backColor, _displayMode, _intensityFront,
 * _intensityBack.
 */
//
...
class LightPoint : public PrimaryRecord
{
    ...
    public:
        ...
        META_Record( LightPoint ) META_setID( _lpn ) META_setComment(
            _lpn
        ) META_dispose( _lpn )
            ...
            virtual void addVertex( Vertex& vertex )
        {
            ...
            // color
            lp._color =
                ( vertex.validColor() ) ? vertex._color : osg::vec4( 1, 1, 1, 1 );

            // sector
            bool directional = ( _directionality == UNIDIRECTIONAL ) ||
                               ( _directionality == BIDIRECTIONAL );
            if( directional && vertex.validNormal() )
            {
                lp._sector =
                    new osgSim::DirectionalSector( vertex._normal,
                                                   osg::radians( _lobeHorizontal ),
                                                   osg::radians( _lobeVertical ),
                                                   osg::radians( _lobeRoll ) );
            }
            ...
            if( ( _flags & FLASHING ) || ( _flags & ROTATING ) )
            {
                ...
                {
                    lp._blinkSequence->setDataVariance(
                        osg::Object::DataVariance::DYNAMIC
                    );
                    lp._blinkSequence->setPhaseShift( _animationPhaseDelay );
                    lp._blinkSequence->addPulse( _animationPeriod -
                                                     _animationPeriodEnable,
...
class IndexedLightPoint : public PrimaryRecord
{
        ...
        // flags
        static const unsigned int            NO_BACK_COLOR_BIT = 0X80'00'00'00U >> 1;
    ...
    public:
        ...
        META_Record( IndexedLightPoint ) META_setID( _lpn ) META_setComment(
            _lpn
        ) META_dispose( _lpn )
            ...
            virtual void addVertex( Vertex& vertex )
        {
            ...
            if( _appearance.valid() )
            {
                ...
                // sector
                bool directional = ( _appearance->directionality == UNIDIRECTIONAL ) ||
                                   ( _appearance->directionality == BIDIRECTIONAL );
                if( directional && vertex.validNormal() )
                {
                    lp._sector = new osgSim::DirectionalSector(
                        vertex._normal,
                        osg::radians( _appearance->horizontalLobeAngle ),
                        osg::radians( _appearance->verticalLobeAngle ),
                        osg::radians( _appearance->lobeRollAngle )
                    );
                }
                ...
                if( _animation.valid() )
                {
                    ...
                    switch( _animation->animationType )
                    {
                        ...
                        case LPAnimation::STROBE :
                            blinkSequence->setPhaseShift(
                                _animation->animationPhaseDelay
                            );
                            blinkSequence->addPulse(
                                _animation->animationPeriod -
                                    _animation->animationEnabledPeriod,
                                osg::vec4( 0, 0, 0, 0 )
                            );
                        ...
                        case LPAnimation::FLASHING_SEQUENCE :
                            {
                                ...
                                for( LPAnimation::PulseArray::iterator itr =
                                         _animation->sequence.begin();
                ...
                // Create a new lightpoint if bi-directional.
                if( ( _appearance->directionality == BIDIRECTIONAL ) &&
                    vertex.validNormal() )
                {
                    // back intensity
            ...
            if( _appearance.valid() )
            {
                ...
                if( _appearance->texturePatternIndex != -1 )
                {
                    ...
                    osg::StateSet* textureStateSet =
                        tp->get( _appearance->texturePatternIndex );
                    if( textureStateSet )
                    {
...
REGISTER_FLTRECORD( IndexedLightPoint,
                    INDEXED_LIGHT_POINT_OP )
...
class LightPointSystem : public PrimaryRecord
{
    ...
    META_Record( LightPointSystem ) META_addChild( _switch ) protected
                    ...
                    Document& /*document*/ )
        {
            ...
            switch( _animationState )
            {
                ...
                case 1 :
                    ...
                    break;
                case 2 :
                    _lps->setAnimationState(
                        osgSim::LightPointSystem::ANIMATION_RANDOM
                    );
                    break;
            }
        ...
        dispose( Document& /*document*/ )
        {
            ...
            unsigned int initialSet = ( ( _flags & 0X80'00'00'00 ) != 0 ) ? 1 : 0;
            _switch->setActiveSwitchSet( initialSet );

            for( unsigned int i = 0; i < _switch->getNumChildren(); i++ )
            {
                osg::Node* child = _switch->getChild( i );
                if( osgSim::LightPointNode* lpn =
                        dynamic_cast<osgSim::LightPointNode*>( child ) )
                {
...
REGISTER_FLTRECORD( LightPointSystem,
                    LIGHT_POINT_SYSTEM_OP )
