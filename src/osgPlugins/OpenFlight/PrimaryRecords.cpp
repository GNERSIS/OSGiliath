 ...
 * Header, derived from PrimaryRecord.
...
namespace flt
{
    ...
    {
            static const unsigned int SAVE_VERTEX_NORMALS_BIT = 0X80'00'00'00U >> 0;
            static const unsigned int PACKED_COLOR_MODE_BIT   = 0X80'00'00'00U >> 1;
            static const unsigned int CAD_VIEW_MODE_BIT       = 0X80'00'00'00U >> 2;
        ...
        META_Record( Header ) META_setID( _header ) META_setComment(
            _header
        ) META_setMultitexture( _header ) META_addChild( _header ) protected
            : virtual ~Header()
            {
            }
                        ...
                        Document&          document )
            {
                ...
                uint8 units = in.readUInt8();    // 0=Meters 1=Kilometers 4=Feet 5=Inches
                                                 // 8=Nautical miles
                ...
                {
                    document._unitScale = unitsToMeters( ( CoordUnits )units ) /
                                          unitsToMeters( document.getDesiredUnits() );
                }
            ...
            dispose( Document& document )
            {
                if( _header.valid() )
                {
                    ...
                    if( sp && !sp->empty() )
                    {
                        _header->getOrCreateStateSet()->addUniform(
                            new osg::Uniform( "TextureUnit0", 0 )
                        );
                        _header->getOrCreateStateSet()->addUniform(
                            new osg::Uniform( "TextureUnit1", 1 )
                        );
                        _header->getOrCreateStateSet()->addUniform(
                            new osg::Uniform( "TextureUnit2", 2 )
                        );
                        _header->getOrCreateStateSet()->addUniform(
                            new osg::Uniform( "TextureUnit3", 3 )
                        );
                    }
                }
            }
    ...
    {
            static const unsigned int FORWARD_ANIM     = 0X80'00'00'00U >> 1;
            static const unsigned int SWING_ANIM       = 0X80'00'00'00U >> 2;
            static const unsigned int BOUND_BOX_FOLLOW = 0X80'00'00'00U >> 3;
            static const unsigned int FREEZE_BOUND_BOX = 0X80'00'00'00U >> 4;
            static const unsigned int DEFAULT_PARENT   = 0X80'00'00'00U >> 5;
            static const unsigned int BACKWARD_ANIM    = 0X80'00'00'00U >> 6;
        ...
        public:
            ...
            META_Record( Group ) META_setID( _group ) META_setComment(
                _group
            ) META_setMultitexture( _group ) META_addChild( _group ) bool hasAnimation() const
            {
        ...
        protected:
            ...
            dispose( Document& document )
            {
                ...
                {
                    // Regardless of forwards or backwards, animation could have swing
                    // bit set.
                    ...
                    if( document.version() >= VERSION_15_8 )
                    {
                        // Set frame duration.
                        float frameDuration =
                            _loopDuration / float( sequence->getNumChildren() );
    ...
    {
            // Flags
            static const unsigned long LIMIT_TRANSLATION_X = 0X80'00'00'00U >> 0;
            static const unsigned long LIMIT_TRANSLATION_Y = 0X80'00'00'00U >> 1;
            static const unsigned long LIMIT_TRANSLATION_Z = 0X80'00'00'00U >> 2;
            static const unsigned long LIMIT_PITCH         = 0X80'00'00'00U >> 3;
            static const unsigned long LIMIT_ROLL          = 0X80'00'00'00U >> 4;
            static const unsigned long LIMIT_YAW           = 0X80'00'00'00U >> 5;
            static const unsigned long LIMIT_SCALE_X       = 0X80'00'00'00U >> 6;
            static const unsigned long LIMIT_SCALE_Y       = 0X80'00'00'00U >> 7;
            static const unsigned long LIMIT_SCALE_Z       = 0X80'00'00'00U >> 8;

            struct Range
            {
                    float64
                        min;    // Minimum value with respect to the local coord system
                    float64
                        max;    // Maximum value with respect to the local coord system
                    float64 current;    // Current value with respect to the local coord
                                        // system
        ...
        META_Record( DegreeOfFreedom ) META_setID( _dof ) META_setComment(
            _dof
        ) META_setMultitexture( _dof ) META_addChild( _dof ) META_dispose( _dof ) protected
                        ...
                        Document&          document )
            {
                std::string id = in.readString( 8 );
                in.forward( 4 );    // Reserved
                osg::dvec3 localOrigin    = in.readVec3d();
                osg::dvec3 pointOnXAxis   = in.readVec3d();
                osg::dvec3 pointInXYPlane = in.readVec3d();
                Range      rangeZ         = readRange(
                    in
                );    // Legal z values with respect to the local coord system
                Range rangeY = readRange(
                    in
                );    // Legal y values with respect to the local coord system
                Range rangeX = readRange(
                    in
                );    // Legal x values with respect to the local coord system
                Range rangePitch =
                    readRange( in );    // Legal pitch values (rotation about the x-axis)
                Range rangeRoll =
                    readRange( in );    // Legal roll values( rotation about the y-axis)
                Range rangeYaw =
                    readRange( in );    // Legal yaw values (rotation about the z-axis)
                Range rangeScaleZ =
                    readRange( in );    // Legal z scale values (about local origin)
                Range rangeScaleY =
                    readRange( in );    // Legal y scale values about local origin)
                Range rangeScaleX =
                    readRange( in );    // Legal x scale values (about local origin)
                uint32 flags =
                    in.readUInt32();    // Flags, bits from left to right (see OF doc)

                // out of range check, required for racecar.flt (Creator Gallery)
                if( !valid( localOrigin ) )
                {
                    localOrigin = osg::dvec3( 0, 0, 0 );
                }
                if( !valid( pointOnXAxis ) )
                {
                    pointOnXAxis = osg::dvec3( 1, 0, 0 );
                }
                if( !valid( pointInXYPlane ) )
                {
                    pointInXYPlane = osg::dvec3( 0, 1, 0 );
                }

                _dof->setName( id );

                // tranlsations:
                _dof->setMinTranslate( osg::vec3( rangeX.min, rangeY.min, rangeZ.min ) *
                                       document.unitScale() );
                _dof->setMaxTranslate( osg::vec3( rangeX.max, rangeY.max, rangeZ.max ) *
                                       document.unitScale() );
                _dof->setCurrentTranslate(
                    osg::vec3( rangeX.current, rangeY.current, rangeZ.current ) *
                    document.unitScale()
                );
                _dof->setIncrementTranslate(
                    osg::vec3( rangeX.increment, rangeY.increment, rangeZ.increment ) *
                    document.unitScale()
                );

                // rotations:
                _dof->setMinHPR( osg::vec3( osg::radians( rangeYaw.min ),
                                            osg::radians( rangePitch.min ),
                                            osg::radians( rangeRoll.min ) ) );
                _dof->setMaxHPR( osg::vec3( osg::radians( rangeYaw.max ),
                                            osg::radians( rangePitch.max ),
                                            osg::radians( rangeRoll.max ) ) );
                _dof->setCurrentHPR( osg::vec3( osg::radians( rangeYaw.current ),
                                                osg::radians( rangePitch.current ),
                                                osg::radians( rangeRoll.current ) ) );
                _dof->setIncrementHPR(
                    osg::vec3( osg::radians( rangeYaw.increment ),
                               osg::radians( rangePitch.increment ),
                               osg::radians( rangeRoll.increment ) )
                );

                // scales:
                _dof->setMinScale(
                    osg::vec3( rangeScaleX.min, rangeScaleY.min, rangeScaleZ.min )
                );
                _dof->setMaxScale(
                    osg::vec3( rangeScaleX.max, rangeScaleY.max, rangeScaleZ.max )
                );
                _dof->setCurrentScale( osg::vec3( rangeScaleX.current,
                                                  rangeScaleY.current,
                                                  rangeScaleZ.current ) );
                _dof->setIncrementScale( osg::vec3( rangeScaleX.increment,
                                                    rangeScaleY.increment,
                                                    rangeScaleZ.increment ) );

                // compute axis.
                osg::dvec3 xAxisD         = pointOnXAxis - localOrigin;
                osg::dvec3 xyPlaneVectorD = pointInXYPlane - localOrigin;
                osg::dvec3 zAxisD         = osg::cross( xAxisD, xyPlaneVectorD );
                osg::dvec3 yAxisD         = osg::cross( zAxisD, xAxisD );

                // normalize
                float      length_x = ( float )osg::length( xAxisD );
                xAxisD              = osg::normalize( xAxisD );
                float length_y      = ( float )osg::length( yAxisD );
                yAxisD              = osg::normalize( yAxisD );
                float length_z      = ( float )osg::length( zAxisD );
                zAxisD              = osg::normalize( zAxisD );
                osg::vec3 xAxis( ( float )xAxisD.x,
                                 ( float )xAxisD.y,
                                 ( float )xAxisD.z );
                osg::vec3 yAxis( ( float )yAxisD.x,
                                 ( float )yAxisD.y,
                                 ( float )yAxisD.z );
                osg::vec3 zAxis( ( float )zAxisD.x,
                                 ( float )zAxisD.y,
                                 ( float )zAxisD.z );

                if( ( length_x * length_y * length_z ) == 0.0F )
                {
                    OSG_NOTICE << "Warning: OpenFlight DegreeOfFreedom::readRecord() "
                                  "found erroneous axis definition:"
                               << std::endl;
                ...
                // scale origin
                osg::vec3  origin( osg::vec3( localOrigin ) *
                                   ( float )document.unitScale() );

                // create putmatrix
                osg::dmat4 inv_putmat( xAxis.x,
                                       xAxis.y,
                                       xAxis.z,
                                       0.0,
                                       yAxis.x,
                ...
                _dof->setPutMatrix( osg::inverse( inv_putmat ) );
            ...
            Range
            readRange( RecordInputStream& in ) const
            {
                Range range;
            ...
            bool
            valid( const osg::dvec3& v ) const
            {
                const osg::dvec3 bad( -1.0E8, -1.0E8, -1.0E8 );
                const float64    epsilon = 1.0E-7;
    ...
    REGISTER_FLTRECORD( DegreeOfFreedom,
                        DOF_OP )

    /** LevelOfDetail - To recreate the LevelOfDetail record in OSG we have to create a
     * LOD node with one Group node under it. OSG representation Children of the
     * LevelOfDetail record will be added to
     */
    class LevelOfDetail : public PrimaryRecord
    {
        ...
        META_Record( LevelOfDetail ) META_setID( _lod ) META_setComment(
            _lod
        ) META_setMultitexture( _lod ) META_addChild( _impChild0 ) META_dispose( _lod ) protected
                        ...
                        Document&          document )
            {
                ...
                /*uint32 flags =*/in.readUInt32();
                osg::dvec3 center = in.readVec3d();

                _lod              = new osg::LOD;
                _lod->setName( id );
                _lod->setCenter( osg::vec3( center * document.unitScale() ) );
    ...
    REGISTER_FLTRECORD( LevelOfDetail,
                        LOD_OP )
    ...
    class OldLevelOfDetail : public PrimaryRecord
    {
        ...
        META_Record( OldLevelOfDetail ) META_setID( _lod ) META_setComment(
            _lod
        ) META_setMultitexture( _lod ) META_addChild( _impChild0 ) META_dispose( _lod ) protected
    ...
    REGISTER_FLTRECORD( OldLevelOfDetail,
                        OLD_LOD_OP )
    ...
    class Switch : public PrimaryRecord
    {
        ...
        public:
            ...
            META_Record( Switch ) META_setID( _multiSwitch ) META_setComment( _multiSwitch ) META_setMultitexture(
                _multiSwitch
            ) META_dispose( _multiSwitch ) virtual void addChild( osg::Node& child )
            {
        ...
        protected:
                        ...
                        Document& /*document*/ )
            {
                ...
                In this example numberOfMasks=2 and wordsInMask=3, currentMask can be 0
                or 1.
                */
    ...
    class ExternalReference : public PrimaryRecord
    {
            ...
            static const unsigned long   COLOR_PALETTE_OVERRIDE    = 0X80'00'00'00U >> 0;
            static const unsigned long   MATERIAL_PALETTE_OVERRIDE = 0X80'00'00'00U >> 1;
            static const unsigned long   TEXTURE_PALETTE_OVERRIDE  = 0X80'00'00'00U >> 2;
            static const unsigned long LINE_STYLE_PALETTE_OVERRIDE = 0X80'00'00'00U >> 3;
            static const unsigned long SOUND_PALETTE_OVERRIDE      = 0X80'00'00'00U >> 4;
            static const unsigned long LIGHT_SOURCE_PALETTE_OVERRIDE =
                0X80'00'00'00U >> 5;
            static const unsigned long LIGHT_POINT_PALETTE_OVERRIDE =
                0X80'00'00'00U >> 6;
            static const unsigned long SHADER_PALETTE_OVERRIDE = 0X80'00'00'00U >> 7;
        ...
        META_Record( ExternalReference ) META_setID( _external ) META_setComment(
            _external
        ) META_setMultitexture( _external ) META_addChild( _external ) META_dispose( _external ) protected
                        ...
                        Document&          document )
            {
                ...
                if( document.version() >= VERSION_14_2 )
                {
                    ...
                    if( ( document.version() >= VERSION_15_1 ) &&
                        ( ( mask & LIGHT_SOURCE_PALETTE_OVERRIDE ) == 0 ) )
                    {
                    ...
                    {
                        parentPools->setLPAppearancePool(
                            document.getLightPointAppearancePool()
                        );
                    }
    ...
    REGISTER_FLTRECORD( ExternalReference,
    ...
    class InstanceDefinition : public PrimaryRecord
    {
        ...
        META_Record( InstanceDefinition ) META_setID( _instanceDefinition ) META_setComment(
            _instanceDefinition
        ) META_setMultitexture( _instanceDefinition ) META_addChild( _instanceDefinition ) protected
            ...
            dispose( Document& document )
            {
                ...
                if( _matrix.valid() )
                {
                    osg::ref_ptr<osg::MatrixTransform> transform =
                        new osg::MatrixTransform( *_matrix );
    ...
    REGISTER_FLTRECORD( InstanceDefinition,
                        INSTANCE_DEFINITION_OP )
    ...
    class InstanceReference : public PrimaryRecord
    {
        ...
        META_Record( InstanceReference ) protected : virtual ~InstanceReference()
            {
            }
    ...
    REGISTER_FLTRECORD( InstanceReference,
                        INSTANCE_REFERENCE_OP )
    ...
    class Extension : public PrimaryRecord
    {
        ...
        META_Record( Extension ) META_setID( _extension ) META_setComment(
            _extension
        ) META_setMultitexture( _extension ) META_addChild( _extension ) META_dispose( _extension ) protected
            : virtual ~Extension()
            {
            }
    ...
    {
            static const unsigned int HIDE_IN_DAYLIGHT = 0X80'00'00'00U >> 0;
            static const unsigned int HIDE_AT_DUSK     = 0X80'00'00'00U >> 1;
            static const unsigned int HIDE_AT_NIGHT    = 0X80'00'00'00U >> 2;
            static const unsigned int NO_ILLUMINATION  = 0X80'00'00'00U >> 3;
            static const unsigned int FLAT_SHADED      = 0X80'00'00'00U >> 4;
            static const unsigned int SHADOW_OBJECT    = 0X80'00'00'00U >> 5;
        ...
        META_Record( Object ) META_setID( _object ) META_setComment(
            _object
        ) META_addChild( _object ) protected
            : virtual void readRecord( RecordInputStream& in,
                                       Document&          document )
            {
                std::string id = in.readString( 8 );
            ...
            bool
            isSafeToRemoveObject() const
            {
                // The following tests need a valid parent.
                if( _parent.valid() )
                {
                    // LODs adds an empty child group so it is safe to remove this object
                    // record.
    ...
    {
            static const unsigned int      ENABLED = 0X80'00'00'00U >> 0;
            static const unsigned int      GLOBAL  = 0X80'00'00'00U >> 1;
            static const unsigned int      EXPORT  = 0X80'00'00'00U >> 3;
        ...
        META_Record( LightSource ) META_setID( _lightSource ) META_setComment(
            _lightSource
        ) META_dispose( _lightSource ) protected : virtual ~LightSource()
            {
            }
                        ...
                        Document&          document )
            {
                ...
                if( lightFromPool )
                {
                    ...
                    if( ( w == 0.0 ) || ( light->getSpotCutoff() < 180.0 ) )
                    {
                        ...
                        float cos_yaw   = cosf( osg::radians( -yaw ) );
                        float sin_yaw   = sinf( osg::radians( -yaw ) );
                        float cos_pitch = cosf( osg::radians( pitch ) );
                        float sin_pitch = sinf( osg::radians( pitch ) );
                    ...
                    _lightSource->setLocalStateSetModes(
                        ( flags & ENABLED ) ? osg::StateAttribute::ON
                    ...
                    if( flags & GLOBAL )
                    {
                        ...
                        if( header )
                        {
                            _lightSource->setStateSetModes(
                                *( header->getOrCreateStateSet() ),
    ...
    REGISTER_FLTRECORD( LightSource,
                        LIGHT_SOURCE_OP )
