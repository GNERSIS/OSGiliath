 ...
 * VertexPalette, derived from Record.
 * Provides: META_Record, ColorPalette, META_Record, NameTable, META_Record,
 * MaterialPalette.
 */
//
...
namespace flt
{
    ...
    class VertexPalette : public Record
    {
        ...
        META_Record( VertexPalette ) protected : virtual ~VertexPalette()
            {
            }
    ...
    REGISTER_FLTRECORD( VertexPalette,
                        VERTEX_PALETTE_OP )
    ...
    class ColorPalette : public Record
    {
        ...
        META_Record( ColorPalette ) protected : virtual ~ColorPalette()
            {
            }
                        ...
                        Document&          document )
            {
                ...
                if( document.version() > VERSION_13 )
                {
                    ...
                    bool colorNameSection = in.getRecordSize() > 4'228;
                    int maxColors = ( document.version() >= VERSION_15_1 ) ? 1'024 : 512;
                ...
                else    // version <= 13
                {
                    ...
                    for( int i = 0; i < 56; i++ )
                    {
                        ...
                        uint16 green    = in.readUInt16( 1 );
                        uint16 blue     = in.readUInt16( 1 );
                        ( *cp )[i + 32] = osg::vec4( ( float )red / 255,
                                                     ( float )green / 255,
                                                     ( float )blue / 255,
                                                     1 );
                    }
    ...
    REGISTER_FLTRECORD( ColorPalette,
                        COLOR_PALETTE_OP )
    ...
    class NameTable : public Record
    {
        ...
        META_Record( NameTable ) protected : virtual ~NameTable()
            {
            }
    ...
    class MaterialPalette : public Record
    {
        ...
        META_Record( MaterialPalette ) protected : virtual ~MaterialPalette()
            {
            }
                        ...
                        Document&          document )
            {
                ...
                osg::vec3      ambient   = in.readVec3f();
                osg::vec3      diffuse   = in.readVec3f();
                osg::vec3      specular  = in.readVec3f();
                osg::vec3      emissive  = in.readVec3f();
                ...
                material->setAmbient( osg::Material::FRONT_AND_BACK,
                                      osg::vec4( ambient, alpha ) );
                material->setDiffuse( osg::Material::FRONT_AND_BACK,
                                      osg::vec4( diffuse, alpha ) );
                material->setSpecular( osg::Material::FRONT_AND_BACK,
                                       osg::vec4( specular, alpha ) );
                material->setEmission( osg::Material::FRONT_AND_BACK,
                                       osg::vec4( emissive, alpha ) );
                ...
                {
                    OSG_INFO << "Warning: OpenFlight shininess value out of range: "
                             << shininess << std::endl;
                }
    ...
    REGISTER_FLTRECORD( MaterialPalette,
                        MATERIAL_PALETTE_OP )
    ...
    class OldMaterialPalette : public Record
    {
        ...
        META_Record( OldMaterialPalette ) protected : virtual ~OldMaterialPalette()
            {
            }
    ...
    REGISTER_FLTRECORD( OldMaterialPalette,
                        OLD_MATERIAL_PALETTE_OP )
    ...
    class TexturePalette : public Record
    {
        ...
        META_Record( TexturePalette ) protected : virtual ~TexturePalette()
            {
            }

            osg::Texture2D::WrapMode
            convertWrapMode( int32           attrWrapMode,
                         ...
                         const Document&    document ) const
            {
                osg::ref_ptr<osg::Image> image =
                    osgDB::readRefImageFile( filename, document.getOptions() );
                if( !image )
                {
                    return NULL;
                }
                ...
                // Since the .attr file is optional according to the OpenFlight spec,
                // check to see if the file exists before reading it, to avoid printing
                ...
                osg::ref_ptr<AttrData> attr =
                    osgDB::readRefFile<AttrData>( attrname, document.getOptions() );
                if( attr.valid() )
                {
                    // Wrap mode
                    osg::Texture2D::WrapMode wrap_s =
                        convertWrapMode( attr->wrapMode_u, document );
                    texture->setWrap( osg::Texture2D::WRAP_S, wrap_s );

                    osg::Texture2D::WrapMode wrap_t =
                        convertWrapMode( attr->wrapMode_v, document );
                    ...
                    switch( attr->minFilterMode )
                    {
                        ...
                        case AttrData::MIN_FILTER_MIPMAP_POINT :
                            texture->setFilter( osg::Texture2D::MIN_FILTER,
                                                osg::Texture2D::NEAREST_MIPMAP_NEAREST );
                            break;
                        case AttrData::MIN_FILTER_MIPMAP_LINEAR :
                            texture->setFilter( osg::Texture2D::MIN_FILTER,
                                                osg::Texture2D::NEAREST_MIPMAP_LINEAR );
                            break;
                        case AttrData::MIN_FILTER_MIPMAP_BILINEAR :
                            texture->setFilter( osg::Texture2D::MIN_FILTER,
                                                osg::Texture2D::LINEAR_MIPMAP_NEAREST );
                            break;
                    ...
                    switch( attr->intFormat )
                    {
                        ...
                        case AttrData::INTERNAL_FORMAT_TX_IA_8 :
                            texture->setInternalFormat( GL_RG );
                            break;
                        ...
                        case AttrData::INTERNAL_FORMAT_TX_IA_12 :
                            texture->setInternalFormat( GL_RG16 );
                            break;
                        ...
                        case AttrData::INTERNAL_FORMAT_TX_I_16 :
                            texture->setInternalFormat( GL_R16 );
                            break;
                        ...
                        Document&          document )
            {
                ...
                if( pathname.empty() )
                {
                    OSG_WARN << "Can't find texture (" << index << ") " << filename
                             << std::endl;
                    return;
                }
                ...
                if( !stateset.valid() )
                {
                    ...
                    flt::Registry::instance()->addTextureToLocalCache( pathname,
                                                                       stateset.get() );
                }
    ...
    class EyepointAndTrackplanePalette : public Record
    {
        ...
        META_Record( EyepointAndTrackplanePalette ) protected
    ...
    REGISTER_FLTRECORD( EyepointAndTrackplanePalette,
                        EYEPOINT_AND_TRACKPLANE_PALETTE_OP )
    ...
    class LinkagePalette : public Record
    {
        ...
        META_Record( LinkagePalette ) protected : virtual ~LinkagePalette()
            {
            }
    ...
    REGISTER_FLTRECORD( LinkagePalette,
                        LINKAGE_PALETTE_OP )
    ...
    class SoundPalette : public Record
    {
        ...
        META_Record( SoundPalette ) protected : virtual ~SoundPalette()
            {
            }
    ...
    REGISTER_FLTRECORD( SoundPalette,
                        SOUND_PALETTE_OP )
    ...
    class LightSourcePalette : public Record
    {
        public:
            ...
            META_Record( LightSourcePalette ) enum LightType
            {
                INFINITE_LIGHT = 0,
                LOCAL_LIGHT    = 1,
                SPOT_LIGHT     = 2,
        ...
        protected:
                        ...
                        Document&          document )
            {
                ...
                in.forward( 4 );
                osg::vec4 ambient  = in.readVec4f();
                osg::vec4 diffuse  = in.readVec4f();
                osg::vec4 specular = in.readVec4f();
    ...
    REGISTER_FLTRECORD( LightSourcePalette,
                        LIGHT_SOURCE_PALETTE_OP )
    ...
    class LightPointAppearancePalette : public Record
    {
        ...
        META_Record( LightPointAppearancePalette ) protected
    ...
    class LightPointAnimationPalette : public Record
    {
        ...
        META_Record( LightPointAnimationPalette ) protected
                        ...
                        Document&          document )
            {
                ...
                animation->morseCodeString = in.readString( 1'024 );
                ...
                // Add to pool
                LightPointAnimationPool* lpaPool =
                    document.getOrCreateLightPointAnimationPool();
                ( *lpaPool )[animation->index] = animation.get();
            }
    };

    REGISTER_FLTRECORD( LightPointAnimationPalette,
    ...
    class LineStylePalette : public Record
    {
        ...
        META_Record( LineStylePalette ) protected : virtual ~LineStylePalette()
            {
            }
    ...
    REGISTER_FLTRECORD( LineStylePalette,
                        LINE_STYLE_PALETTE_OP )
    ...
    class TextureMappingPalette : public Record
    {
        ...
        META_Record( TextureMappingPalette ) protected : virtual ~TextureMappingPalette()
            {
            }
    ...
    REGISTER_FLTRECORD( TextureMappingPalette,
                        TEXTURE_MAPPING_PALETTE_OP )
    ...
    class ShaderPalette : public Record
    {
        public:
            ...
            META_Record( ShaderPalette ) enum ShaderType
            {
                CG   = 0,
                CGFX = 1,
                GLSL = 2,
        ...
        protected:
                        ...
                        Document&          document )
            {
                ...
                std::string name  = in.readString( 1'024 );

                if( type == CG )
                {
                    // CG support is currently not implemented. Just parse.
                    std::string vertexProgramFilename   = in.readString( 1'024 );
                    std::string fragmentProgramFilename = in.readString( 1'024 );
                ...
                else if( type == GLSL )
                {
                    ...
                    {
                        // In 16.1, possibly multiple filenames for each vertex and
                        // fragment program.
                    ...
                    for( idx = 0; idx < vertexProgramFileCount; idx++ )
                    {
                        ...
                        std::string vertexProgramFilePath =
                            osgDB::findDataFile( vertexProgramFilename,
                                                 document.getOptions() );
                        if( !vertexProgramFilePath.empty() )
                        {
                            osg::ref_ptr<osg::Shader> vertexShader =
                                osgDB::readRefShaderFile( osg::Shader::VERTEX,
                                                          vertexProgramFilePath );
                            if( vertexShader )
                            {
                    ...
                    for( idx = 0; idx < fragmentProgramFileCount; idx++ )
                    {
                        ...
                        std::string fragmentProgramFilePath =
                            osgDB::findDataFile( fragmentProgramFilename,
                                                 document.getOptions() );
                        if( !fragmentProgramFilePath.empty() )
                        {
                            osg::ref_ptr<osg::Shader> fragmentShader =
                                osgDB::readRefShaderFile( osg::Shader::FRAGMENT,
                                                          fragmentProgramFilePath );
                            if( fragmentShader )
                            {
    ...
    REGISTER_FLTRECORD( ShaderPalette,
                        SHADER_PALETTE_OP )
