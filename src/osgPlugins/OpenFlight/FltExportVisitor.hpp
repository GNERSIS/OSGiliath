 ...
 * FltExportVisitor, derived from DualModeVisitor.
 * Provides: complete, apply, apply, apply, apply, apply.
 */
//
...
namespace flt
{
     ...
     */
    class FltExportVisitor : public osg::DualModeVisitor
    {
        public:

            FltExportVisitor( DataOutputStream* dos,
                              ExportOptions*    fltOpt );
            ~FltExportVisitor();

            bool
            complete( const osg::Node& node );
            ...
            // Primary records
            void
            writeHeader( const std::string& headerName );
            void
            writeGroup( const osg::Group& node );
            void
            writeGroup( const osg::Group& group,
                        int32             flags,
                        int32             loopCount,
                        float32           loopDuration,
                        float32           lastFrameDuration );
            void
            writeSequence( const osg::Sequence& node );
            void
            writeObject( const osg::Group&         node,
                         osgSim::ObjectRecordData* ord );
            void
            writeDegreeOfFreedom( const osgSim::DOFTransform* dof );
            void
            writeExternalReference( const osg::ProxyNode& node );
            void
            writeLevelOfDetail( const osg::LOD&   lod,
                                const osg::dvec3& center,
                                double            switchInDist,
                                double            switchOutDist );
            void
            writeLightSource( const osg::LightSource& ls );
            void
            writeSwitch( const osgSim::MultiSwitch* ms );
            void
            writeSwitch( const osg::Switch* ms );
            void
            writeLightPoint( const osgSim::LightPointNode* lpn );

            // Ancillary records
            void
            writeComment( const osg::Node&  node,
                          DataOutputStream* dos = NULL );
            void
            writeLongID( const std::string& id,
                         DataOutputStream*  dos = NULL );
            void
            writeMatrix( const osg::Referenced* ref );
            void
            writeContinuationRecord( const unsigned short length );
            ...
            void
            writePushTraverseWritePop( osg::Node& node )
            {
                writePush();
                traverse( node );
                writePop();
            ...
            void
            writePushTraverseChildWritePop( osg::Node& node )
            {
                writePush();
                node.accept( *this );
                writePop();
            ...
            // Geometry records
            void
            writeFace( const osg::Geode&    geode,
                       const osg::Geometry& geom,
                       GLenum               mode );
            void
            writeMesh( const osg::Geode&    geode,
                       const osg::Geometry& geom );
            int
            writeVertexList( int          first,
                             unsigned int count );
            int
            writeVertexList( const std::vector<unsigned int>& indices,
                             unsigned int                     count );
            void
            writeMeshPrimitive( const std::vector<unsigned int>& indices,
                                GLenum                           mode );
            void
            writeLocalVertexPool( const osg::Geometry& geom );
            void
            writeMultitexture( const osg::Geometry& geom );
            void
            writeUVList( int                              numVerts,
                         const osg::Geometry&             geom,
            ...
            // Exporter doesn't currently support color palette; write a dummy in its
            // place to support loaders that require it.
            void
            writeColorPalette();

            // StateSet stack support
            void
            pushStateSet( const osg::StateSet* rhs );
            void
            popStateSet();
            const osg::StateSet*
            getCurrentStateSet() const;
            void
            clearStateSetStack();

            // Write a .attr file if none exists in the data file path.
            void
            writeATTRFile( int                   unit,
                           const osg::Texture2D* texture ) const;
        ...
        private:
            ...
            void
            handleDrawArrays( const osg::DrawArrays* da,
                              const osg::Geometry&   geom,
                              const osg::Geode&      geode );
            void
            handleDrawArrayLengths( const osg::DrawArrayLengths* dal,
                                    const osg::Geometry&         geom,
                                    const osg::Geode&            geode );
            void
            handleDrawElements( const osg::DrawElements* de,
                                const osg::Geometry&     geom,
                                const osg::Geode&        geode );

            bool
            isLit( const osg::Geometry& geom ) const;
            bool
            isTextured( int                  unit,
                        const osg::Geometry& geom ) const;
            bool
            isMesh( const GLenum mode ) const;
            bool
            atLeastOneFace( const osg::Geometry& geom ) const;
            bool
            atLeastOneMesh( const osg::Geometry& geom ) const;
    ...
    class ScopedStatePushPop
    {
        public:
            ...
            ScopedStatePushPop( FltExportVisitor*    fnv,
    ...
    struct IdHelper
    {
            IdHelper( flt::FltExportVisitor& v,
                      const std::string&     id ) :
                v_( v ),
                id_( id ),
                dos_( NULL )
    ...
    {
            SubfaceHelper( flt::FltExportVisitor& v,
                           const osg::StateSet*   ss ) :
                v_( v )
            {
                _polygonOffsetOn =
                    ( ss->getMode( GL_POLYGON_OFFSET_FILL ) == osg::StateAttribute::ON );
                if( _polygonOffsetOn )
                {
                    v_.writePushSubface();
                }
