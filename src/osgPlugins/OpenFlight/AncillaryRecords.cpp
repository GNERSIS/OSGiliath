 ...
 * Comment, derived from Record.
 * Provides: META_Record, LongID, META_Record, dmat4, META_Record, Multitexture.
 */
//
...
namespace flt
{
    ...
    class Comment : public Record
    {
        ...
        META_Record( Comment ) protected : virtual ~Comment()
            {
            }
                        ...
                        Document& /*document*/ )
            {
                ...
                _parent->setComment(commentfield);
                    ...
                    while( end_of_line < commentfield.size() )
                    {
                        ...
                        {
                            _parent->setComment( std::string( commentfield,
                                                              front_of_line,
                                                              end_of_line -
                                                                  front_of_line ) );
    ...
    class LongID : public Record
    {
        ...
        META_Record( LongID ) protected : virtual ~LongID()
            {
            }
    ...
    REGISTER_FLTRECORD( LongID,
                        LONG_ID_OP )

    /** dmat4 -
     */
    class dmat4 : public Record
    {
        public:
        ...
        META_Record( dmat4 ) protected : virtual ~dmat4()
            {
            }
                        ...
                        Document&          document )
            {
                ...
                // scale position.
                osg::dvec3 pos  = osg::getTrans( matrix );
                matrix          = osg::translate( -pos ) * matrix;
                pos            *= document.unitScale();
                matrix          = osg::translate( pos ) * matrix;
    ...
    class Multitexture : public Record
    {
        ...
        protected:
                        ...
                        Document&          document )
            {
                ...
                {
                    uint32 layerBit = 0X80'00'00'00U >> ( layer - 1 );
                    if( mask & layerBit )
                    {
                        ...
                        osg::ref_ptr<osg::StateSet> texturePoolStateset =
                            document.getOrCreateTexturePool()->get( textureIndex );
                        if( stateset.valid() && texturePoolStateset.valid() )
                        {
                            // Apply texture from texture pool.
                            osg::Texture* texture = dynamic_cast<osg::Texture*>(
                                texturePoolStateset->getTextureAttribute(
                                    0,
                            ...
                            if( texture )
                            {
                                ...
                                if( document.getPreserveNonOsgAttrsAsUserData() )
                                {
                                    ...
                                    texture->setUserValue( "<UA::TexMappingIdx>",
                                                           mappingIndex );
                            ...
                            switch( effect )
                            {
                                case TEXTURE_ENVIRONMENT :
                                    {
                                        // TexEnv removed (fixed-function) - texture
                                        // environment now handled by shaders
                                    }
                                    break;
                                case BUMP_MAP :
                                    {
                                        // Dot3 bumpmap - now handled by shaders
                                    }
                                    break;
                            }
                        }
    ...
    REGISTER_FLTRECORD( Multitexture,
                        MULTITEXTURE_OP )
    ...
    class UVList : public Record
    {
        ...
        META_Record( UVList ) protected : virtual ~UVList()
            {
            }

            // count number of 1's in mask.
            int
            bitCount( uint32 mask )
            {
                int count = 0;
                while( mask )
                {
    ...
    class Replicate : public Record
    {
        ...
        META_Record( Replicate ) protected : virtual ~Replicate()
            {
            }
    ...
    class IndexedString : public Record
    {
        ...
        META_Record( IndexedString ) protected : virtual ~IndexedString()
            {
            }
    ...
    REGISTER_FLTRECORD( IndexedString,
                        INDEXED_STRING_OP )
    ...
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_TRANSLATE2_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_ROTATE_ABOUT_POINT_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_ROTATE_ABOUT_EDGE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_SCALE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_TRANSLATE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_NONUNIFORM_SCALE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_ROTATE_ABOUT_POINT2_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_ROTATE_SCALE_TO_POINT_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_PUT_TRANSFORM_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        OLD_BOUNDING_BOX_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        ROAD_ZONE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        ROTATE_ABOUT_EDGE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        TRANSLATE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        NONUNIFORM_SCALE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        ROTATE_ABOUT_POINT_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        ROTATE_SCALE_TO_POINT_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        PUT_TRANSFORM_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        GENERAL_MATRIX_OP )
    ...
    REGISTER_FLTRECORD( DummyRecord,
                        BOUNDING_SPHERE_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        BOUNDING_CYLINDER_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        BOUNDING_CONVEX_HULL_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        BOUNDING_HISTOGRAM )
    REGISTER_FLTRECORD( DummyRecord,
                        BOUNDING_VOLUME_CENTER_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        BOUNDING_VOLUME_ORIENTATION_OP )
    REGISTER_FLTRECORD( DummyRecord,
                        HISTOGRAM_BOUNDING_VOLUME_OP )
