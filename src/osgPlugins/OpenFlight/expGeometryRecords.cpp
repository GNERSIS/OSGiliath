 ...
 * Utility functions for osgPlugins: LAYER_1, LAYER_2, LAYER_3, LAYER_4, LAYER_5.
 */
//
...
namespace flt
{
    ...
    static unsigned int LAYER_1( 0X80'00'00'00 >> 0 );
#if 0
// unused so if'deffing out
static unsigned int LAYER_2( 0X80'00'00'00 >> 1 );
static unsigned int LAYER_3( 0X80'00'00'00 >> 2 );
static unsigned int LAYER_4( 0X80'00'00'00 >> 3 );
static unsigned int LAYER_5( 0X80'00'00'00 >> 4 );
static unsigned int LAYER_6( 0X80'00'00'00 >> 5 );
static unsigned int LAYER_7( 0X80'00'00'00 >> 6 );
#endif
    ...
    bool
    FltExportVisitor::isTextured( int                  unit,
                                  const osg::Geometry& geom ) const
    {
        const osg::StateSet* ss = getCurrentStateSet();
        // Detect if a texture is present on this unit
        bool                 texOn(
            ss->getTextureAttribute( unit, osg::StateAttribute::Type::TEXTURE ) != NULL
        );
    ...
    void
    FltExportVisitor::writeFace( const osg::Geode&    geode,
                                 ...
                                 GLenum               mode )
    {
        ...
        const unsigned int PACKED_COLOR_BIT = 0X80'00'00'00U >> 3;
        // const unsigned int FOOTPRINT_BIT    = 0x80000000u >> 4;    // Terrain culture
        // cutout
        const unsigned int HIDDEN_BIT = 0X80'00'00'00U >> 5;
        ...
        else
        {
            ...
            {
                packedColorRaw = ( *c )[0];
                transparency = uint16( ( 1. - packedColorRaw[3] ) * ( double )0XFF'FF );
            }
        ...
        packedColor   = ( int )( packedColorRaw[3] * 255 ) << 24 |
                        ( int )( packedColorRaw[2] * 255 ) << 16 |
                        ( int )( packedColorRaw[1] * 255 ) << 8 |
            ...
            case GL_POLYGON :
                {
                    ...
                    // If facet-culling isn't *dis*abled, check whether the CullFace mode
                    // is BACK
                    if( ss->getMode( GL_CULL_FACE ) & osg::StateAttribute::ON )
                    {
                        const osg::CullFace* cullFace =
                            static_cast<const osg::CullFace*>(
                                ss->getAttribute( osg::StateAttribute::Type::CULLFACE )
                            );
                        if( cullFace->getMode() == osg::CullFace::Mode::BACK )
                        {
                            drawType = SOLID_BACKFACE;
                        }

                        // Note: OpenFlt can't handle FRONT or FRONT_AND_BACK settings,
                        // so ignore these(??)
                    }
                    break;
                }
        ...
        if( isLit( geom ) )
        {
            const osg::Material* currMaterial = static_cast<const osg::Material*>(
        ...
        if( isTextured( 0, geom ) )
        {
            ...
            {
                std::string warning(
                    "fltexp: Face is textured, but Texture2D StateAttribute is NULL."
                );
                OSG_WARN << warning << std::endl;
                _fltOpt->getWriteResult().warn( warning );
            }
        ...
        _records->writeUInt32( 0X00'FF'FF'FF );    // Packed color, alternate
    ...
    void
    FltExportVisitor::writeMesh( const osg::Geode&    geode,
                                 const osg::Geometry& geom )
    {
        enum DrawMode
        {
...

        {
            ...
            if( ss->getMode( GL_CULL_FACE ) & osg::StateAttribute::ON )
            {
                const osg::CullFace* cullFace = static_cast<const osg::CullFace*>(
                    ss->getAttribute( osg::StateAttribute::Type::CULLFACE )
                );
                if( cullFace->getMode() == osg::CullFace::Mode::BACK )
                {
                ...
                // Note: OpenFlt can't handle FRONT or FRONT_AND_BACK settings, so ignore
                // these(??)
            }
        }
        ...
        if( isTextured( 0, geom ) )
        {
            ...
            {
                std::string warning(
                    "fltexp: Mesh is textured, but Texture2D StateAttribute is NULL."
                );
                OSG_WARN << warning << std::endl;
                _fltOpt->getWriteResult().warn( warning );
            }
    ...
    int
    FltExportVisitor::writeVertexList( int          first,
                                       unsigned int count )
    {
    ...
    int
    FltExportVisitor::writeVertexList( const std::vector<unsigned int>& indices,
                                       unsigned int                     count )
    {
        _records->writeInt16( ( int16 )VERTEX_LIST_OP );
        _records->writeUInt16( 4 + ( count * 4 ) );
    ...
    void
    FltExportVisitor::writeMeshPrimitive( const std::vector<unsigned int>& indices,
                                          GLenum                           mode )
    {
        int16 primType;
        switch( mode )
    ...
    {
        // Attribute Mask
        static const unsigned int           HAS_POSITION = 0X80'00'00'00U >> 0;
        // static const unsigned int HAS_COLOR_INDEX   = 0x80000000u >> 1;
        static const unsigned int           HAS_RGBA_COLOR = 0X80'00'00'00U >> 2;
        static const unsigned int           HAS_NORMAL     = 0X80'00'00'00U >> 3;
        static const unsigned int           HAS_BASE_UV    = 0X80'00'00'00U >> 4;
        static const unsigned int           HAS_UV_LAYER1  = 0X80'00'00'00U >> 5;
        static const unsigned int           HAS_UV_LAYER2  = 0X80'00'00'00U >> 6;
        static const unsigned int           HAS_UV_LAYER3  = 0X80'00'00'00U >> 7;
        static const unsigned int           HAS_UV_LAYER4  = 0X80'00'00'00U >> 8;
        static const unsigned int           HAS_UV_LAYER5  = 0X80'00'00'00U >> 9;
        static const unsigned int           HAS_UV_LAYER6  = 0X80'00'00'00U >> 10;
        static const unsigned int           HAS_UV_LAYER7  = 0X80'00'00'00U >> 11;

        const osg::Array*                   v              = geom.getVertexArray();
        uint32                              numVerts( v->getNumElements() );
        osg::ref_ptr<const osg::Vec3dArray> v3 =
            VertexPaletteManager::asVec3dArray( v, numVerts );
        if( !v3 )
        {
            std::string warning(
                "fltexp: writeLocalVertexPool: VertexArray is not Vec3Array."
            );
            OSG_WARN << warning << std::endl;
            _fltOpt->getWriteResult().warn( warning );
            return;
        ...
        osg::ref_ptr<const osg::Vec4Array> c4 =
            VertexPaletteManager::asVec4Array( c, numVerts );
        osg::ref_ptr<const osg::Vec3Array> n3 =
            VertexPaletteManager::asVec3Array( n, numVerts );
        osg::ref_ptr<const osg::Vec2Array> t2 =
            VertexPaletteManager::asVec2Array( t, numVerts );
        if( c && !c4 )
        {
            return;
        }
        ...
        for( ; unit < 8; unit++ )
        {
            mtc[unit] = VertexPaletteManager::asVec2Array( geom.getTexCoordArray( unit ),
                                                           numVerts );
        }
        ...
        if( ( c4 != NULL ) &&
            ( osg::getBinding( geom.getColorArray() ) == osg::Array::BIND_PER_VERTEX ) )
        {
            attr     |= HAS_RGBA_COLOR;
            vertSize += sizeof( unsigned int );
        }
        if( ( n3 != NULL ) &&
            ( osg::getBinding( geom.getNormalArray() ) == osg::Array::BIND_PER_VERTEX ) )
        {
            attr     |= HAS_NORMAL;
        ...
        unsigned int maxVerts          = ( 0XFF'FF - 12 ) / vertSize;
    ...
    FltExportVisitor::writeMultitexture( const osg::Geometry& geom )
    {
        ...
        for( idx = 1; idx < 8; idx++ )
        {
            if( isTextured( idx, geom ) )
            {
                int16                 textureIndex( -1 );
                const osg::Texture2D* texture = static_cast<const osg::Texture2D*>(
                    ss->getTextureAttribute( idx, osg::StateAttribute::Type::TEXTURE )
                );
                if( texture != NULL )
                {
                ...
                // texture index (this value is an unsigned int, but has a -1 default per
                // oflt spec: ugh)
                _records->writeUInt16( static_cast<uint16>( textureIndex ) );
                _records->writeUInt16(
                    0
                );    // TBD effect
                      // mapping index (this value is an unsigned int, but has a -1
                      // default per oflt spec: ugh)
    ...
    void
    FltExportVisitor::writeUVList( int                              numVerts,
                                   const osg::Geometry&             geom,
                                   const std::vector<unsigned int>& indices )
    {
        ...
        for( int vertexIdx = 0; vertexIdx < numVerts; vertexIdx++ )
        {
            for( idx = 1; idx < 8; idx++ )
            {
                ...
                {
                    osg::Array* t =
                        const_cast<osg::Array*>( geom.getTexCoordArray( idx ) );
            ...
            for( idx = 1; idx < 8; idx++ )
            {
                if( isTextured( idx, geom ) )
                {
                    ...
                    else if( t2->getNumElements() < first + numVerts )
                    {
                        std::ostringstream warning;
                        warning
                            << "fltexp: Invalid number of texture coordinates for unit "
                            << idx;
                        OSG_WARN << warning.str() << std::endl;
                        _fltOpt->getWriteResult().warn( warning.str() );
                    }
    ...
    void
    FltExportVisitor::handleDrawArrays( const osg::DrawArrays* da,
                                        const osg::Geometry&   geom,
    ...
    void
    FltExportVisitor::handleDrawArrayLengths( const osg::DrawArrayLengths* dal,
                                              const osg::Geometry&         geom,
                                              const osg::Geode&            geode )
    {
        if( !dal )
    ...
    void
    FltExportVisitor::handleDrawElements( const osg::DrawElements* de,
                                          const osg::Geometry&     geom,
                                          const osg::Geode&        geode )
    {
        if( !de )
