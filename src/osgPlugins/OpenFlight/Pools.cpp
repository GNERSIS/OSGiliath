 ...
 * Utility functions for osgPlugins: finalColor, materialParam.
 */
//
...
osg::vec4
ColorPool::getColor( int indexIntensity ) const
{
    if( _old )    // version <= 13
    {
        // bit 0-6:  intensity
        ...
        bool         fixedIntensity = ( indexIntensity & 0X10'00 ) ? true : false;
        unsigned int index          = ( fixedIntensity )
                                        ? ( indexIntensity & 0X0F'FF ) + ( 4'096 >> 7 )
                                        : indexIntensity >> 7;
...
MaterialPool::MaterialPool()
{
    ...
    _defaultMaterial->setAmbient( osg::Material::FRONT_AND_BACK,
                                  osg::vec4( 1, 1, 1, 1 ) );
    _defaultMaterial->setDiffuse( osg::Material::FRONT_AND_BACK,
                                  osg::vec4( 1, 1, 1, 1 ) );
    _defaultMaterial->setSpecular( osg::Material::FRONT_AND_BACK,
                                   osg::vec4( 0, 0, 0, 1 ) );
    _defaultMaterial->setEmission( osg::Material::FRONT_AND_BACK,
                                   osg::vec4( 0, 0, 0, 1 ) );
    _defaultMaterial->setShininess( osg::Material::FRONT_AND_BACK, 0 );
}

osg::Material*
MaterialPool::get( int index )
{
...
namespace
{
    ...
    osg::vec4
    finalColor( const osg::vec4& materialColor,
                const osg::vec4& faceColor )
    {
        return osg::vec4( materialColor.r * faceColor.r,
...
osg::Material*
MaterialPool::getOrCreateMaterial( int              index,
                                   const osg::vec4& faceColor )
{
    ...
    material->setAmbient( osg::Material::FRONT_AND_BACK,
                          finalColor( materialPaletteAmbient, faceColor ) );
    material->setDiffuse( osg::Material::FRONT_AND_BACK,
                          finalColor( materialPaletteDiffuse, faceColor ) );
    material->setAlpha( osg::Material::FRONT_AND_BACK,
                        materialPaletteAmbient.a * faceColor.a );
