 ...
 * Face, derived from PrimaryRecord.
 * Provides: _primaryColor, _drawFlag, _template, _transparency, _flags, _lightMode.
 */
//
...
namespace flt
{
    ...
    void
    reverseWindingOrder( ARRAY* data,
                         GLenum mode,
                         GLint  first,
                         GLint  last )
    {
    ...
    void
    addDrawableAndReverseWindingOrder( osg::Geode* geode )
    {
        // Replace double sided polygons by duplicating the drawables and inverting the
        // normals.
        ...
        for( size_t di = 0; di < geode->getNumDrawables(); ++di )
        {
            const osg::Geometry* geometry =
                dynamic_cast<const osg::Geometry*>( geode->getDrawable( di ) );
            if( geometry )
            {
                osg::Geometry* geom =
                    new osg::Geometry( *geometry,
                ...
                for( size_t pi = 0; pi < geom->getNumPrimitiveSets(); ++pi )
                {
                    osg::DrawArrays* drawarray =
                        dynamic_cast<osg::DrawArrays*>( geom->getPrimitiveSet( pi ) );
                    if( drawarray )
                    {
                        ...
                        {
                            osg::Vec2Array* UVs = dynamic_cast<osg::Vec2Array*>(
                                geom->getTexCoordArray( i )
                            );
                            if( UVs )
                            {
                                reverseWindingOrder( UVs,
                                                     drawarray->getMode(),
                                                     first,
                                                     last );
                            }
    ...
    {
            // flags
            static const unsigned int TERRAIN_BIT      = 0X80'00'00'00U >> 0;
            static const unsigned int NO_COLOR_BIT     = 0X80'00'00'00U >> 1;
            static const unsigned int NO_ALT_COLOR_BIT = 0X80'00'00'00U >> 2;
            static const unsigned int PACKED_COLOR_BIT = 0X80'00'00'00U >> 3;
            static const unsigned int FOOTPRINT_BIT =
                0X80'00'00'00U >> 4;    // Terrain culture cutout
            static const unsigned int   HIDDEN_BIT   = 0X80'00'00'00U >> 5;
            static const unsigned int   ROOFLINE_BIT = 0X80'00'00'00U >> 6;
        ...
        public:
            ...
            META_Record( Face ) META_setID( _geode ) META_setComment(
                _geode
            ...
            isLit() const
            {
                return ( _lightMode == FACE_COLOR_LIGHTING ) ||
            ...
            isGouraud() const
            {
                return ( _lightMode == VERTEX_COLOR ) ||
                       ( _lightMode == VERTEX_COLOR_LIGHTING );
            }
            ...
            addVertex( Vertex& vertex )
            {
                ...
                if( isGouraud() )
                {
                    ...
                    else
                    {
                        ...
                        // Flight characteristic not reflected in the current OSG"
                        // (Sept/Oct 2011)
                ...
                for( int layer = 0; layer < Vertex::MAX_LAYERS; layer++ )
                {
                    ...
                    {
                        osg::Vec2Array* UVs =
                            getOrCreateTextureArray( *_geometry, layer );
        ...
        protected:
                        ...
                        Document&          document )
            {
                ...
                _geode->setDataVariance( osg::Object::DataVariance::STATIC );
                _geode->setName( id );

                _geometry = new osg::Geometry;
                _geometry->setDataVariance( osg::Object::DataVariance::STATIC );
                ...
                if( isLit() || materialIndex >= 0 )
                {
                    ...
                    osg::Material* material =
                        document.getOrCreateMaterialPool()->getOrCreateMaterial(
                            materialIndex,
                            col
                        );
                ...
                switch( _drawFlag )
                {
                    case SOLID_BACKFACED :    // Enable backface culling
                        {
                            static osg::ref_ptr<osg::CullFace> cullFace =
                ...
                if( document.subfaceLevel() > 0 )
                {
                    stateset->setAttributeAndModes(
                        document.getSubSurfacePolygonOffset( document.subfaceLevel() ),
                        osg::StateAttribute::ON
                    );
                    stateset->setAttribute( document.getSubSurfaceDepth() );

                    stateset->setRenderBinDetails( document.subfaceLevel(),
                                                   "RenderBin" );
                }
            ...
            getPrimitiveSetMode( int numVertices )
            {
                switch( getDrawMode() )
                {
            ...
            dispose( Document& document )
            {
                if( _geode.valid() )
                {
                    ...
                    if( _matrix.valid() )
                    {
                        insertMatrixTransform( *_geode,
                                               *_matrix,
                                               _numberOfReplications );
                    }
                    ...
                    for( unsigned int i = 0; i < _geode->getNumDrawables(); ++i )
                    {
                        ...
                        if( geometry )
                        {
                            ...
                            if( isGouraud() )
                            {
                                ...
                                {
                                    geometry->getColorArray()->setBinding(
                                        osg::Array::BIND_PER_VERTEX
                                    );
                                }
                            }
                            ...
                            if( isLit() )
                            {
                                ...
                                {
                                    geometry->getNormalArray()->setBinding(
                                        osg::Array::BIND_PER_VERTEX
                                    );
                                }
                            }
                    ...
                    if( document.getUseBillboardCenter() )
                    {
                        ...
                        if( billboard )
                        {
                            for( unsigned int i = 0; i < billboard->getNumDrawables();
                                 ++i )
                            {
                                const osg::box& bb =
                                ...
                                osgUtil::TransformAttributeFunctor tf(
                                    osg::translate( osg::dvec3( -bb.center() ) )
                                );
    ...
    class VertexListRecord : public PrimaryRecord
    {
        public:
            ...
            META_Record( VertexListRecord ) virtual void addVertex( Vertex& vertex )
            {
    ...
    REGISTER_FLTRECORD( VertexListRecord,
                        VERTEX_LIST_OP )
    ...
    class MorphVertexList : public PrimaryRecord
    {
        ...
        public:
            ...
            META_Record( MorphVertexList ) virtual void addVertex( Vertex& vertex )
            {
                switch( _mode )
                {
                    case MORPH_0 :
    ...
    REGISTER_FLTRECORD( MorphVertexList,
                        MORPH_VERTEX_LIST_OP )
    ...
    class Mesh : public PrimaryRecord
    {
        ...
        public:
            ...
            META_Record( Mesh ) META_setID( _geode ) META_setComment(
                _geode
            ) META_setMultitexture( _geode )
            ...
            virtual void
            addGeometry( osg::Geometry& geometry )
            {
    ...
    class LocalVertexPool : public Record
    {
            ...
            static const unsigned int HAS_COLOR_INDEX = 0X80'00'00'00U >> 1;
        ...
        META_Record( LocalVertexPool ) protected : virtual ~LocalVertexPool()
            {
            }
                        ...
                        Document&          document )
            {
                ...
                for( unsigned int n = 0; n < vertices; n++ )
                {
                    ...
                    if( mask & HAS_POSITION )
                    {
                        ...
                        if( !osg::valid( coord ) )
                        {
                            OSG_NOTICE << "Warning: data error detected in "
                                          "LocalVertexPool::readRecord coord="
                    ...
                    if( mask & HAS_COLOR_INDEX )
                    {
                        ...
                        int       index      = alphaIndex & 0X00'FF'FF'FF;
                    ...
                    if( mask & HAS_NORMAL )
                    {
                        ...
                        {
                            OSG_NOTICE << "Warning: data error detected in "
                                          "LocalVertexPool::readRecord normal="
    ...
    REGISTER_FLTRECORD( LocalVertexPool,
                        LOCAL_VERTEX_POOL_OP )
    ...
    class MeshPrimitive : public PrimaryRecord
    {
        ...
        META_Record( MeshPrimitive ) protected : virtual ~MeshPrimitive()
            {
            }
                        ...
                        Document& /*document*/ )
            {
                ...
                for( unsigned int n = 0; n < vertexCount; n++ )
                {
                    ...
                    if( index < vertexList->size() )
                    {
                        ...
                        {
                            osg::Vec3Array* normals =
                                getOrCreateNormalArray( *geometry );
                            normals->push_back( vertex._normal );
                        }
    ...
    REGISTER_FLTRECORD( MeshPrimitive,
                        MESH_PRIMITIVE_OP )
