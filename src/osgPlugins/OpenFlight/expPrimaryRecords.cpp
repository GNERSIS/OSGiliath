 ...
 * Utility functions for osgPlugins: flags, id, length, writeGroup.
 */
//
...
#include "Opcodes.hpp"
#include "VertexPaletteManager.hpp"
...
namespace flt
{
    ...
    FltExportVisitor::writeHeader( const std::string& headerName )
    {
        ...
        _dos.writeString( std::string( " " ),
                          32 );                   // date and time string for last rev
    ...
    void
    FltExportVisitor::writeGroup(
        const osg::Group& group,
        int32             flags,
        int32             loopCount,
    ...
    void
    FltExportVisitor::writeObject( const osg::Group&         group,
    ...
    {
        const osg::dmat4& invPut = dof->getInversePutMatrix();

        // Origin of DOF coord sys
        osg::dvec3        origin( osg::getTrans( invPut ) );
        ...
        osg::dvec3        pointOnXAxis = origin + xAxis;
        // Reference point in DOF coord sys's X-Y plane
        osg::dvec3        pointInXYPlane = origin + yAxis;

        // Translations
        osg::dvec3        minTranslate( dof->getMinTranslate() );
        osg::dvec3        maxTranslate( dof->getMaxTranslate() );
        osg::dvec3        currTranslate( dof->getCurrentTranslate() );
        osg::dvec3        incrTranslate( dof->getIncrementTranslate() );

        // Rotations
        osg::dvec3        minHPR( dof->getMinHPR() );
        osg::dvec3        maxHPR( dof->getMaxHPR() );
        osg::dvec3        currHPR( dof->getCurrentHPR() );
        osg::dvec3        incrHPR( dof->getIncrementHPR() );

        // Scaling
        osg::dvec3        minScale( dof->getMinScale() );
        osg::dvec3        maxScale( dof->getMaxScale() );
        osg::dvec3        currScale( dof->getCurrentScale() );
        osg::dvec3        incrScale( dof->getIncrementScale() );
        ...
        _records->writeFloat64( osg::degrees( minHPR[1] ) );
        _records->writeFloat64( osg::degrees( maxHPR[1] ) );
        _records->writeFloat64( osg::degrees( currHPR[1] ) );
        _records->writeFloat64( osg::degrees( incrHPR[1] ) );

        _records->writeFloat64( osg::degrees( minHPR[2] ) );
        _records->writeFloat64( osg::degrees( maxHPR[2] ) );
        _records->writeFloat64( osg::degrees( currHPR[2] ) );
        _records->writeFloat64( osg::degrees( incrHPR[2] ) );

        _records->writeFloat64( osg::degrees( minHPR[0] ) );
        _records->writeFloat64( osg::degrees( maxHPR[0] ) );
        _records->writeFloat64( osg::degrees( currHPR[0] ) );
        _records->writeFloat64( osg::degrees( incrHPR[0] ) );
    ...
    static const unsigned long LIGHT_SOURCE_PALETTE_OVERRIDE = 0X80'00'00'00U >> 5;
    static const unsigned long LIGHT_POINT_PALETTE_OVERRIDE  = 0X80'00'00'00U >> 6;
    ...
    FltExportVisitor::writeLightSource( const osg::LightSource& node )
    {
        ...
        const osg::Light*         light    = node.getLight();
        int                       index    = _lightSourcePalette->add( light );

        const osg::vec4&          lightPos = light->getPosition();
        const osg::vec3&          lightDir = light->getDirection();
        ...
        _records->writeInt32( 0 );         // Reserved
        _records->writeVec3d( osg::dvec3( lightPos.x, lightPos.y, lightPos.z ) );
    ...
    FltExportVisitor::writeLightPoint( const osgSim::LightPointNode* lpn )
    {
        ...
        enum Flags
        {
            NO_BACK_COLOR = 0X80'00'00'00U >> 1,
            CALLIGRAPHIC  = 0X80'00'00'00U >> 3,
            REFLECTIVE    = 0X80'00'00'00U >> 4,
            PERSPECTIVE   = 0X80'00'00'00U >> 8,
            FLASHING      = 0X80'00'00'00U >> 9,
            ROTATING      = 0X80'00'00'00U >> 10,
            ROTATE_CC     = 0X80'00'00'00U >> 11,
            VISIBLE_DAY   = 0X80'00'00'00U >> 15,
            VISIBLE_DUSK  = 0X80'00'00'00U >> 16,
            VISIBLE_NIGHT = 0X80'00'00'00U >> 17,
        };
        ...
        // parameters, but in OpenFlight, a Light Point Record contains a list of
        // homogeneous vertices. To be correct, we'd have to look at all LightPoints in
        // the LightPointNode and spew out multiple FLT records for each group that
        // shared common appearance parameters. Instead, we cheat: We take the first
        // LightPoint and use its appearance parameters for all LightPoints in the
        // LightPointNode.
        ...
        // Note that true bidirectional light points are currently unsupported (they are
        // unavailable in OSG, so we never write them out to FLT as BIDIRECTIONAL.
        int32                            directionality( OMNIDIRECTIONAL );
        float32                          horizLobe( 360.F );
        float32                          vertLobe( 360.F );
        float32                          lobeRoll( 0.F );
        const osgSim::DirectionalSector* dirSector =
            dynamic_cast<osgSim::DirectionalSector*>( lp0._sector.get() );
        if( dirSector )
        {
            directionality = UNIDIRECTIONAL;
            horizLobe      = osg::degrees( dirSector->getHorizLobeAngle() );
            vertLobe       = osg::degrees( dirSector->getVertLobeAngle() );
            lobeRoll       = osg::degrees( dirSector->getLobeRollAngle() );
        }

        {
            ...
            _records->writeUInt32(
                ~0U
            );    // OpenFlight erroneously say -1, so will assume ~0u is OK.  Back color
                  // for bidirectional
            ...
            _records->writeFloat32(
                animPhaseDelay
            );                                // Animation phase delay in seconds
            _records->writeFloat32(
                animEnabled
            );                                // Animation enabled period in seconds
            _records->writeFloat32( 1.F );    // Significance
            _records->writeInt32( 0 );        // Calligraphic draw order
            _records->writeInt32( flags );    // Flags
            _records->writeVec3f( osg::vec3( 0.F, 0.F, 0.F ) );    // Axis of rotation
        }

        {
            osg::ref_ptr<osg::Vec3dArray> v =
                new osg::Vec3dArray( lpn->getNumLightPoints() );
            osg::ref_ptr<osg::Vec4Array> c =
                new osg::Vec4Array( lpn->getNumLightPoints() );
            osg::ref_ptr<osg::Vec3Array> n =
                new osg::Vec3Array( lpn->getNumLightPoints() );
            osg::vec3    normal( 0.F, 0.F, 1.F );
            ...
            for( idx = 0; idx < lpn->getNumLightPoints(); idx++ )
            {
                ...
                const osgSim::DirectionalSector* ds =
                    dynamic_cast<osgSim::DirectionalSector*>( lp._sector.get() );
                if( ds )
                {
            ...
            _vertexPalette->add( ( const osg::Array* )NULL,
                                 v.get(),
                                 c.get(),
                                 n.get(),
                                 NULL,
    ...
    FltExportVisitor::writeColorPalette()
    {
        ...
        int idx;
        for( idx = 0; idx < 1'024; idx++ )
        {
            _dos.writeUInt32( 0XFF'FF'FF'FF );    // Color n
        }
    }
