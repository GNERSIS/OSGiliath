 ...
 * Utility functions for osgPlugins: _currentSizeBytes, needsInit, DataOutputStream,
 * flags, packedColor.
 */
//
...
namespace flt
{
    ...
    VertexPaletteManager::~VertexPaletteManager()
    {
        if( !_verticesTempName.empty() )
        {
            ...
            {
                OSG_WARN
                    << "fltexp: VertexPaletteManager destructor has an open temp file."
                    << std::endl;
    ...
    VertexPaletteManager::add( const osg::Geometry& geom )
    {
        ...
        {
            OSG_WARN
                << "fltexp: Attempting to add NULL vertex array in VertexPaletteManager."
                << std::endl;
            return;
        }
                               ...
                               bool                   allowSharing )
    {
        ...
        if( needsInit )
        {
            ...
            if( !_vertices )
            {
                ...
                _verticesStr.open( _verticesTempName.c_str(),
    ...
    VertexPaletteManager::byteOffset( unsigned int idx ) const
    {
        if( !_current )
        {
            OSG_WARN << "fltexp: No current vertex array in VertexPaletteManager."
                     << std::endl;
            return 4;
        }
        if( idx >= _current->_idxCount )
        {
            OSG_WARN << "fltexp: Index out of range in VertexPaletteManager."
                     << std::endl;
            return 4;
        }
    ...
    VertexPaletteManager::recordType( const osg::Array* /*v*/,
                                      const osg::Array* /*c*/,
                                      const osg::Array* n,
                                      const osg::Array* t )
    {
    ...
    void
    VertexPaletteManager::writeRecords( const osg::Vec3dArray* v,
                                        const osg::Vec4Array*  c,
                                        const osg::Vec3Array*  n,
                                        const osg::Vec2Array*  t,
                                        bool                   colorPerVertex,
    ...
    VertexPaletteManager::asVec2Array( const osg::Array*  in,
                                       const unsigned int n )
    {
        ...
        const unsigned int nToCopy =
            ( ( n < in->getNumElements() ) ? n : in->getNumElements() );
            ...
            case osg::Array::Vec2dArrayType :
                {
                    ...
                    {
                        ( *ret )[idx] =
                            ( *v2d )[idx];    // convert vec2 double to vec2 float
                    }
                    return ret.get();
                }
            default :
                {
                    OSG_WARN
                        << "fltexp: Unsupported array type in conversion to Vec2Array: "
                        << arrayType << std::endl;
                    return NULL;
                }
        }
    ...
    VertexPaletteManager::asVec3Array( const osg::Array*  in,
                                       const unsigned int n )
    {
        if( !in )
        {
            ...
            case osg::Array::Vec3dArrayType :
                {
                    ...
                    {
                        ( *ret )[idx] =
                            ( *v3d )[idx];    // convert vec3 double to vec3 float
                    }
                    return ret.get();
                }
            default :
                {
                    OSG_WARN
                        << "fltexp: Unsupported array type in conversion to Vec3Array: "
                        << arrayType << std::endl;
                    return NULL;
                }
        }
    ...
    VertexPaletteManager::asVec3dArray( const osg::Array*  in,
                                        const unsigned int n )
    {
        if( !in )
        {
            ...
            case osg::Array::Vec3ArrayType :
                {
                    ...
                    {
                        ( *ret )[idx] =
                            ( *v3f )[idx];    // convert vec3 float to vec3 double
                    }
                    return ret.get();
                }
            default :
                {
                    OSG_WARN
                        << "fltexp: Unsupported array type in conversion to Vec3dArray: "
                        << arrayType << std::endl;
                    return NULL;
                }
        }
    ...
    VertexPaletteManager::asVec4Array( const osg::Array*  in,
                                       const unsigned int n )
    {
        if( !in )
        {
        ...
        switch( arrayType )
        {
            ...
            default :
                {
                    OSG_WARN
                        << "fltexp: Unsupported array type in conversion to Vec4Array: "
                        << arrayType << std::endl;
                    return NULL;
                }
        }
