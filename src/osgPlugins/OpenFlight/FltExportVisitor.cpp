 ...
 * Utility functions for osgPlugins: warning, MaterialPaletteManager,
 * TexturePaletteManager, LightSourcePaletteManager, VertexPaletteManager.
 */
//
...
namespace flt
{
    ...
    FltExportVisitor::FltExportVisitor( DataOutputStream* dos,
        ...
        _texturePalette( new TexturePaletteManager( *this,
                                                    *fltOpt ) ),
        ...
        _firstNode( true )
    {
        ...
        _records =
            new DataOutputStream( _recordsStr.rdbuf(), fltOpt->getValidateOnly() );
    ...
    FltExportVisitor::~FltExportVisitor()
    {
        ...
        {
            OSG_WARN << "fltexp: FltExportVisitor destructor has an open temp file."
                     << std::endl;
    ...
    FltExportVisitor::apply( osg::Group& node )
    {
        ...
        if( _firstNode )
        {
            ...
            // On successive roundtrips, this results in increased redundant top-level
            // Group nodes/records. Avoid this by NOT outputting anything for a top-level
            // Group node.
            _firstNode = false;
            traverse( node );
            return;
        ...
        //   InstanceDefinition/InstanceReference -- MED --  multiparented Group is an
        //   instance Extension record -- MED Object record -- MED LightPointSystem
        //   record (if psgSim::MultiSwitch) -- LOW
    ...
    FltExportVisitor::apply( osg::LOD& lodNode )
    {
        ...
        osg::dvec3         center( lodNode.getCenter() );
    ...
    {
        // Importer reads a dmat4 record and inserts a MatrixTransform above
        //   the current node. We need to do the opposite: Write a dmat4 record
        ...
        //   UserData and writes a dmat4 record if UserData is a MatrixTransform.
        ...
        if( node.getUserData() )
        {
            const osg::RefMatrix* rm =
                dynamic_cast<const osg::RefMatrix*>( node.getUserData() );
            if( rm )
            {
                ( *m ) *= *rm;
            }
    ...
    FltExportVisitor::apply( osg::PositionAttitudeTransform& node )
    {
        ...
        osg::ref_ptr<osg::RefMatrix> m =
            new osg::RefMatrix( osg::translate( -node.getPivotPoint() ) *
                                osg::scale( node.getScale() ) *
                                osg::rotate( node.getAttitude() ) *
                                osg::translate( node.getPosition() ) );
    ...
    FltExportVisitor::apply( osg::Geode& node )
    {
        ...
        for( idx = 0; idx < node.getNumDrawables(); idx++ )
        {
            ...
            {
                std::string warning(
                    "fltexp: Non-Geometry Drawable encountered. Ignoring."
                );
                ...
                for( jdx = 0; jdx < geom->getNumPrimitiveSets(); jdx++ )
                {
                             ...
                             osg::PrimitiveSet::DrawArrayLengthsPrimitiveType )
                    {
                        handleDrawArrayLengths(
                            dynamic_cast<osg::DrawArrayLengths*>( prim ),
                            *geom,
                            node
                        );
                    }
    ...
    void
    FltExportVisitor::writeATTRFile( int                   unit,
        ...
        if( osgDB::findDataFile( name ).empty() )
        {
            ...
            switch( texture->getWrap( osg::Texture::WRAP_S ) )
            {
                ...
                case osg::Texture::MIRROR :
                    if( _fltOpt->getFlightFileVersionNumber() >= 1'610 )
                    {
