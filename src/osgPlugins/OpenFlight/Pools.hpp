 ...
 * VertexPool, derived from Referenced.
 * Provides: ColorPool, _old, getColor, TexturePool, get, MaterialPool.
 */
//
...
namespace flt
{
    ...
    class VertexPool : public osg::Referenced,
    ...
    class ColorPool : public osg::Referenced,
                      public std::vector<osg::vec4>
    {
        public:

            explicit ColorPool( bool old,
                                int  size ) :
            ...
            osg::vec4
            getColor( int indexIntensity ) const;
    ...
    class TexturePool : public osg::Referenced,
                        public std::map<int, osg::ref_ptr<osg::StateSet>>
    {
        public:
    ...
    class MaterialPool : public osg::Referenced,
                         public std::map<int, osg::ref_ptr<osg::Material>>
    {
        public:
            ...
            osg::Material*
            getOrCreateMaterial( int              index,
                                 const osg::vec4& faceColor );
        ...
        protected:
            ...
            struct MaterialParameters
            {
                    ...
                    bool
                    operator<( const MaterialParameters& rhs ) const
                    {
                        if( index < rhs.index )
                        {
                            return true;
            ...
            typedef std::map<MaterialParameters, osg::ref_ptr<osg::Material>>
                             FinalMaterialMap;
            FinalMaterialMap _finalMaterialMap;
    };

    class LightSourcePool : public osg::Referenced,
                            public std::map<int, osg::ref_ptr<osg::Light>>
    {
        public:
    ...
    class LightPointAppearancePool : public osg::Referenced,
                                     public std::map<int, osg::ref_ptr<LPAppearance>>
    {
        public:
    ...
    struct LPAnimation : public osg::Referenced
    {
            ...
            float32 animationPhaseDelay;       // animation phase delay, in seconds from
                                               // start of period
            float32 animationEnabledPeriod;    // animation enabled period (time on), in
                                               // seconds
            osg::vec3
                axisOfRotation;    // axis of rotation for rotating animation (i, j, k)
    ...
    class LightPointAnimationPool : public osg::Referenced,
                                    public std::map<int, osg::ref_ptr<LPAnimation>>
    {
        public:
    ...
    class ShaderPool : public osg::Referenced,
                       public std::map<int, osg::ref_ptr<osg::Program>>
    {
        public:
    ...
    class ParentPools : public osg::Referenced
    {
        public:
            ...
            void
            setTexturePool( TexturePool* pool )
            {
                _texturePool = pool;
            }

            TexturePool*
            getTexturePool() const
            {
                return _texturePool.get();
            }

            void
            setMaterialPool( MaterialPool* pool )
            {
                _materialPool = pool;
            }

            MaterialPool*
            getMaterialPool() const
            {
                return _materialPool.get();
            }

            void
            setLightSourcePool( LightSourcePool* pool )
            {
                _lightSourcePool = pool;
            }

            LightSourcePool*
            getLightSourcePool() const
            {
                return _lightSourcePool.get();
            }

            void
            setLPAppearancePool( LightPointAppearancePool* pool )
            {
                _lpAppearancePool = pool;
            }

            LightPointAppearancePool*
            getLPAppearancePool() const
            {
                return _lpAppearancePool.get();
            }

            void
            setLPAnimationPool( LightPointAnimationPool* pool )
            {
                _lpAnimationPool = pool;
            }

            LightPointAnimationPool*
            getLPAnimationPool() const
            {
                return _lpAnimationPool.get();
            }

            void
            setShaderPool( ShaderPool* pool )
            {
                _shaderPool = pool;
            }
