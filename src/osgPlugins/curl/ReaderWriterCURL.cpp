 ...
 * Utility functions for osgPlugins: _outputStream, processResponse, result, afterwards,
 * peer.
 */
#include <curl/curl.h>
#include <fstream>
...
#if LIBCURL_VERSION_NUM < 0X07'15'03
...
//  StreamObject
//
EasyCurl::StreamObject::StreamObject( std::ostream*      outputStream,
...
void
EasyCurl::StreamObject::write( const char* ptr,
                               size_t      realsize )
{
    if( _outputStream )
    {
...
size_t
EasyCurl::StreamObject::read( char*  ptr,
                              size_t maxsize )
{
    if( !_inputStream )
    {
...
std::string
EasyCurl::getResultMimeType( const StreamObject& sp ) const
{
    return sp._resultMimeType;
}

std::string
EasyCurl::getMimeTypeForExtension( const std::string& ext ) const
{
    const osgDB::Registry::MimeTypeExtensionMap& mimeMap =
        osgDB::Registry::instance()->getMimeTypeExtensionMap();
...
std::string
EasyCurl::getFileNameFromURL( const std::string& url )
{
...
size_t
EasyCurl::StreamMemoryCallback( void*  ptr,
                                size_t size,
                                size_t nmemb,
                                void*  data )
{
...
EasyCurl::read( const std::string&                  proxyAddress,
...
EasyCurl::write( const std::string&                  proxyAddress,
                 const std::string&                  fileName,
                 StreamObject&                       sp,
                 const osgDB::ReaderWriter::Options* options )
{
...
void
EasyCurl::setOptions( const std::string&                  proxyAddress,
                      const std::string&                  fileName,
                      StreamObject&                       sp,
                      const osgDB::ReaderWriter::Options* options )
{
    const osgDB::AuthenticationMap* authenticationMap =
        ( options && options->getAuthenticationMap() )
    ...
    // WARNING: here we make the assumption that if someone starts using the timeouts
    // settings he will not try to disable them afterwards (a value must be provided or
    ...
    if( !proxyAddress.empty() )
    {
        ...
        curl_easy_setopt(
            ...
            CURLOPT_PROXY,
            proxyAddress.c_str()
        );    // Sets proxy address and port on libcurl
    }

    // setting ssl verify peer (default is enabled)
    curl_easy_setopt( _curl, CURLOPT_SSL_VERIFYPEER, _sslVerifyPeer );

    const osgDB::AuthenticationDetails* details =
        authenticationMap ? authenticationMap->getAuthenticationDetails( fileName ) : 0;
...
#if LIBCURL_VERSION_NUM >= 0X07'0A'07
...
osgDB::ReaderWriter::ReadResult
EasyCurl::processResponse( CURLcode           res,
...
#if LIBCURL_VERSION_NUM >= 0X07'0C'00
        OSG_NOTICE << "Error: libcurl read error, file=" << fileName
                   << " error = " << curl_easy_strerror( res ) << std::endl;
#else
        OSG_NOTICE << "Error: libcurl read error, file=" << fileName
                   << " error no = " << res << std::endl;
#endif
...
ReaderWriterCURL::ReaderWriterCURL()
{
    ...
    supportsExtension(
        "*",
        "Passes all read files to other plugins to handle actual model loading."
    );
    ...
    supportsEnvironment(
        "OSG_CURL_CONNECTTIMEOUT",
        "Specify the connection timeout duration in seconds [default = 0 = not set]."
    );
    supportsEnvironment( "OSG_CURL_TIMEOUT",
                         "Specify the timeout duration of the whole transfer in seconds "
                         "[default = 0 = not set]." );
    supportsEnvironment( "OSG_CURL_SSL_VERIFYPEER",
                         "Specify ssl verification peer [default = 1 = set]." );
}
...
osgDB::ReaderWriter::WriteResult
ReaderWriterCURL::writeFile( const osg::Object&                  obj,
                             ...
                             const Options*     options ) const
{
    ...
    std::stringstream
        requestBuffer;    // Buffer to be filled then output via http request.
    std::stringstream responseBuffer;    // Buffer to contain content of http response.

    // Serialize obj into an std::stringstream buffer which will be uploaded via HTTP
    // post request.
    ...
    osgDB::ReaderWriter::WriteResult result =
        writeFile( obj, writer, requestBuffer, options );
    if( !result.success() )
    {
        return result;
    }
...
osgDB::ReaderWriter::ReadResult
ReaderWriterCURL::readFile( ObjectType                          objectType,
                            osgDB::ReaderWriter*                rw,
                            std::istream&                       fin,
                            const osgDB::ReaderWriter::Options* options ) const
{
...
void
ReaderWriterCURL::getConnectionOptions( const osgDB::ReaderWriter::Options* options,
                                        std::string&                        proxyAddress,
                                        long& connectTimeout,
                                        long& timeout,
                                        long& sslVerifyPeer ) const
{
    if( options )
    {
        ...
        while( iss >> opt )
        {
            ...
            {
                connectTimeout = atol(
                    opt.substr( index + 1 ).c_str()
                );    // this will return 0 in case of improper format.
            }
    ...
    {
        const char* proxyEnvPort =
            getenv( "OSG_CURL_PROXYPORT" );    // Searching Proxy Port on Env

        if( proxyEnvPort )
        {
            proxyAddress =
                std::string( proxyEnvAddress ) + ":" + std::string( proxyEnvPort );
        }
        else
        {
                            ...
                            const osgDB::ReaderWriter::Options* options ) const
{
    ...
    if( curlResult.status() == ReadResult::FILE_LOADED )
    {
        ...
        if( !reader )
        {
            ...
            OSG_INFO << "CURL: Looking up extension for mime-type " << mimeType
                     << std::endl;
            if( mimeType.length() > 0 )
            {
                reader =
                    osgDB::Registry::instance()->getReaderWriterForMimeType( mimeType );
            }
        }
        ...
        local_opt->setPluginStringData( "STREAM_FILENAME",
...
bool
ReaderWriterCURL::read( std::istream& fin,
...
size_t
empty_write_data( const char* /*buffer*/,
                  size_t size,
                  size_t nmemb,
                  char* /*userp*/ )
{
...
bool
ReaderWriterCURL::fileExists( const std::string&    filename,
                              const osgDB::Options* options ) const
{
    if( osgDB::containsServerAddress( filename ) )
    {
        std::string data;
        OSG_NOTICE << "Checking if file exists using curl plugin: " << filename
                   << std::endl;
        ...
        return ( ( result == 0 ) &&
                 ( ( http_return_code == 200 ) || ( http_return_code == 0 ) ) );
    }
    else
    {
