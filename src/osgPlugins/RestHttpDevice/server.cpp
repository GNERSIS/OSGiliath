...
namespace http
{
    namespace server
    {
        ...
        server::server( const std::string& address,
            ...
            new_connection_( new connection( io_service_pool_.get_io_service(),
                                             request_handler_ ) ),
        ...
        void
        server::handle_accept( const asio::error_code& e )
        {
            if( !e )
            {
            ...
            {
                OSG_WARN << "RestHttpDevice :: server::handle_accept error: "
                         << e.message() << std::endl;
            }
        }
