 ...
 * RestHttpDevice, derived from Device.
...
class RestHttpDevice : public osgGA::Device,
                       osg::Thread
{
    ...
    public:
        ...
        class RequestHandler : public osg::Referenced
        {
            ...
            protected:
                ...
                void
                setDevice( RestHttpDevice* device )
                {
                    _device = device;
                }
                ...
                void
                reportMissingArgument( const std::string&   argument,
                                       http::server::reply& reply ) const
                {
                    OSG_WARN << "RequestHandler :: missing argument '" << argument
                             << "' for " << getRequestPath() << std::endl;
                    reply.content = "{ \"result\": 0, \"error\": \"missing argument '" +
                                    argument +
                                    "'\"}";
                ...
                bool
                sendOkReply( http::server::reply& reply )
                {
                ...
                bool
                getStringArgument( const Arguments&     arguments,
                ...
                bool
                getHexArgument( const Arguments&     arguments,
                                const std::string&   argument,
                                http::server::reply& reply,
                                int&                 value ) const
                {
                ...
                bool
                getIntArgument( const Arguments&     arguments,
                                const std::string&   argument,
                                http::server::reply& reply,
                                int&                 value ) const
                {
                ...
                bool
                getDoubleArgument( const Arguments&     arguments,
                                   const std::string&   argument,
                                   http::server::reply& reply,
                                   double&              value ) const
                {
            ...
            protected:
                ...
                double
                getTimeStamp( const Arguments&     arguments,
                              http::server::reply& reply )
                {
                ...
                double
                getLocalTime( const Arguments&     arguments,
                              http::server::reply& reply )
                {
                ...
                double
                getLocalTime( double time_stamp )
                {
        ...
        RestHttpDevice( const std::string& listening_address,
        ...
        bool
        handleRequest( const std::string&   request_path,
                       http::server::reply& reply );
        ...
        getLocalTime( double time_stamp )
        {
            ...
            double local_time =
                _firstEventLocalTimeStamp + ( time_stamp - _firstEventRemoteTimeStamp );
            // std::cout << "ts: "<< time_stamp << " -> " << local_time << std::endl;
            return local_time;
        }

        bool
        isNewer( double time_stamp )
        {
        ...
        checkEvents()
        {
            if( _targetMouseChanged &&
                ( ( fabs( _currentMouseX - _targetMouseY ) > 0.1F ) ||
                  ( fabs( _currentMouseY - _targetMouseY ) > 0.1F ) ) )
            {
                static const float scalar = 0.2F;
                _currentMouseX =
                    ( 1.0F - scalar ) * _currentMouseX + scalar * _targetMouseX;
                _currentMouseY =
                    ( 1.0F - scalar ) * _currentMouseY + scalar * _targetMouseY;
                getEventQueue()->mouseMotion( _currentMouseX,
                                              _currentMouseY,
        ...
        void
        setTargetMousePosition( float x,
                                float y,
                                bool  force = false )
        {
    ...
    private:
        ...
        void
                             parseArguments( const std::string          request_path,
...
class SendKeystrokeRequestHandler : public RestHttpDevice::RequestHandler
{
    public:
        ...
        SendKeystrokeRequestHandler( const std::string& request_path,
                                     int                key ) :
            RestHttpDevice::RequestHandler( request_path ),
            _key( key )
        {
        }
