...
class ReaderWriterRestHttp : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterRestHttp()
        {
            supportsExtension(
                "resthttp",
                "Virtual Device Integration via a HTTP-Server and a REST-interface"
            );

            supportsOption(
                "documentRoot",
                "document root of asset files to server via the http-server"
            );
            supportsOption( "serverAddress",
                            "server address to listen for incoming requests" );
            supportsOption( "serverPort",
                    ...
                    const osgDB::ReaderWriter::Options* options = NULL ) const
        {
            if( osgDB::getFileExtension( file ) == "resthttp" )
            {
                std::string document_root =
                    options ? options->getPluginStringData( "documentRoot" ) : "htdocs/";
                std::string server_address =
                    options ? options->getPluginStringData( "serverAddress" )
                            : "localhost";
                std::string server_port =
                    options ? options->getPluginStringData( "serverPort" ) : "8080";

                // supported file-name scheme to get address, port and document-root:
                // <server-address>:<server-port>/<document-root> example:
                // '192.168.1.1:88888/var/www/htdocs'

                std::string file_wo_ext = osgDB::getNameLessAllExtensions( file );
                if( ( file_wo_ext.find( '/' ) != std::string::npos ) &&
                    ( file_wo_ext.find( ':' ) != std::string::npos ) )
                {
                    std::string server_part =
                        file_wo_ext.substr( 0, file_wo_ext.find( '/' ) );
                ...
                try
                {
                    osg::ref_ptr<RestHttpDevice> device =
                        new RestHttpDevice( server_address, server_port, document_root );
                    ...
                    device->addRequestHandler(
                        new SendKeystrokeRequestHandler( "/layer/previous",
                    ...
                    device->addRequestHandler( new SendKeystrokeRequestHandler(
                        "/slideorlayer/previous",
                        osgGA::GUIEventAdapter::KEY_Page_Up
                    ) );

                    device->addRequestHandler(
                        new SendKeystrokeRequestHandler( "/unpause", 'o' )
                    );
                    device->addRequestHandler( new SendKeystrokeRequestHandler( "/pause",
                                                                                'p' ) );

                    if( options &&
                        ( options->getPluginStringData( "documentRegisteredHandlers" ) ==
                          "true" ) )
                    {
                ...
                {
                    OSG_WARN << "ReaderWriterRestHttpDevice : could not create "
                                "http-server! Reason: "
                             << e.what() << std::endl;
                ...
                {
                    OSG_WARN << "ReaderWriterRestHttpDevice : could not create "
                                "http-server, unknown exception thrown "
                             << std::endl;
                    return ReadResult::ERROR_IN_READING_FILE;
                }
            }
