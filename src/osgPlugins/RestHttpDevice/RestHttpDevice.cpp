 ...
 * UserEventRequestHandler, derived from RequestHandler.
 * Provides: getDevice, sendOkReply, describeTo, HomeRequestHandler, getDevice,
 * getDevice.
 */
...
namespace RestHttp
{
    ...
    class UserEventRequestHandler : public RestHttpDevice::RequestHandler
    {
        public:
            ...
            UserEventRequestHandler() :
                RestHttpDevice::RequestHandler( "/user-event" )
            {
            }
            ...
            {
                OSG_INFO << "RestHttpDevice :: handling request " << full_request_path
                         << " as user-event" << std::endl;
            ...
            {
                out << getRequestPath()
                    << ": fall-through request-handler, catches all requests w/o "
                       "registered handler and report them to the console"
                    << std::dec;
            }
    };
    ...
    class HomeRequestHandler : public RestHttpDevice::RequestHandler
    {
        public:
            ...
            {
                out << getRequestPath()
                    << ": sets the mouse-input-range arguments: 'x_min','y_min', "
                       "'x_max' and 'y_max'"
                    << std::dec;
            }
    };
    ...
    class KeyCodeRequestHandler : public RestHttpDevice::RequestHandler
    {
        public:
            ...
            KeyCodeRequestHandler( bool handle_key_press ) :
                RestHttpDevice::RequestHandler( std::string( "/key/" ) +
                ...
                if( getHexArgument( arguments, "code", reply, keycode ) )
                {
                    if( _handleKeyPress )
                    {
                        getDevice()->getEventQueue()->keyPress( keycode,
                                                                getLocalTime( arguments,
                                                                              reply ) );
                    }
                    else
                    {
                        getDevice()->getEventQueue()->keyRelease(
                            keycode,
                            getLocalTime( arguments, reply )
                        );
                    }
                }
            ...
            describeTo( std::ostream& out ) const
            {
                out << getRequestPath() << ": send KEY_"
                    << ( _handleKeyPress ? "DOWN" : "UP" )
                    << ", using hex-argument 'code' as keycode" << std::dec;
            }
        ...
        public:
                        ...
                        http::server::reply& reply )
            {
                    ...
                    getIntArgument( arguments, "y", reply, y ) )
                {
                    ...
                    {
                        // getDevice()->getEventQueue()->mouseMotion(x,y,
            ...
            {
                out << getRequestPath()
                    << ": send mouse motion using arguments 'x' and 'y' as coordinates"
                    << std::dec;
            }
    ...
    class MouseButtonRequestHandler : public RestHttpDevice::RequestHandler
    {
        public:
                    ...
                    getIntArgument( arguments, "button", reply, button ) )
                {
                    ...
                    switch( _mode )
                    {
                        ...
                        case RELEASE :
                            ...
                            break;
                        case DOUBLE_PRESS :
                            getDevice()->getEventQueue()->mouseDoubleButtonPress(
                                x,
                                y,
                                button,
                                getLocalTime( arguments, reply )
            ...
            describeTo( std::ostream& out ) const
            {
                ...
                out << " using arguments 'x', 'y' and 'button' as coordinates"
                    << std::dec;
            }
    ...
    class RequestHandlerDispatcherCallback
        : public http::server::request_handler::Callback
    {
        public:
            ...
            virtual std::string
            applyTemplateVars( const std::string& txt )
            {
                return txt;
            }
    ...
    bool
    RequestHandlerDispatcherCallback::operator()( const std::string&   request_path,
                                                  http::server::reply& reply )
    {
...
RestHttpDevice::RestHttpDevice( const std::string& listening_address,
                                const std::string& listening_port,
                                const std::string& doc_root ) :
    osgGA::Device(),
    osg::Thread(),
    _server( listening_address,
             listening_port,
             osgDB::findDataFile( doc_root ),
             std::max( osg::GetNumberOfProcessors() - 1,
                       1 ) ),
    ...
    _targetMouseChanged( false )
{
    ...
    OSG_NOTICE << "RestHttpDevice :: listening on " << listening_address << ":"
               << listening_port << ", document root: " << doc_root << std::endl;

    if( osgDB::findDataFile( doc_root ).empty() )
    {
        OSG_WARN << "RestHttpDevice :: warning, can't locate document-root '" << doc_root
                 << "'for the http-server, starting anyway" << std::endl;
    }
    ...
    addRequestHandler( new RestHttp::MouseButtonRequestHandler(
        RestHttp::MouseButtonRequestHandler::PRESS
    ) );
    addRequestHandler( new RestHttp::MouseButtonRequestHandler(
        RestHttp::MouseButtonRequestHandler::RELEASE
    ) );
    addRequestHandler( new RestHttp::MouseButtonRequestHandler(
        RestHttp::MouseButtonRequestHandler::DOUBLE_PRESS
    ) );
...
void
RestHttpDevice::addRequestHandler( RequestHandler* handler )
{
    if( handler )
    {
...
void
RestHttpDevice::parseArguments( const std::string          request_path,
...
bool
RestHttpDevice::handleRequest( const std::string&   in_request_path,
                               http::server::reply& reply )
{
...
void
RestHttpDevice::describeTo( std::ostream& out ) const
{
