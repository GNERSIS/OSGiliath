 ...
 * ReaderWriterOsgShadow, derived from ReaderWriter.
 * Provides: supportsExtension, supportsExtension, className, readObject, readNode,
 * readNode.
 */
...
class ReaderWriterOsgShadow : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterOsgShadow()
        {
            supportsExtension(
                "osgShadow",
                "OpenSceneGraph osgShadow extension to .osg ascii format"
            );
            supportsExtension( "shadow",
                               "OpenSceneGraph osgShadow extension pseudo loader" );
        }
                  ...
                  const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( !params.empty() )
            {
                if( params == "ShadowTexture" || params == "st" )
                {
                    technique = new osgShadow::ShadowTexture;
                }
                else if( params == "ViewDependentShadowMap" || params == "vdsm" )
                {
                    technique = new osgShadow::ViewDependentShadowMap;
                }
