          ...
          unsigned int* dataType_ret )
{
    ...
    try
    {
        ...
        rgbafile.setFrameBuffer( ( &pixels )[0][0] - dw.min.x - dw.min.y * width,
                                 1,
                                 width );
...
class ReaderWriterEXR : public osgDB::ReaderWriter
{
    ...
    protected:
        ...
        bool
        writeEXRStream( const osg::Image& img,
                        std::ostream&     fout,
                        const std::string& /*fileName*/ ) const
        {
            ...
            if( !( dataType == GL_HALF_FLOAT || dataType == GL_FLOAT ) )
            {
                writeOK = false;
                return false;
            }
        ...
        ReadResult
        readEXRStream( std::istream& fin ) const
        {
            ...
            {
                interNalTextureFormat = numComponents_ret == 1 ? GL_R16F
                                      : numComponents_ret == 2 ? GL_RG16F
