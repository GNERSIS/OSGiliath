...
class BvhMotionBuilder : public osg::Referenced
{
    public:
                        ...
                        osgAnimation::Bone* parent )
        {
            ...
            if( fr.matchSequence( "OFFSET %f %f %f" ) )
            {
                ...
                if( fr.readSequence( offset ) )
                {
                    ...
                    parent->setMatrixInSkeletonSpace(
                        osg::translate( offset ) * parent->getMatrixInSkeletonSpace()
                    );
                    osgAnimation::UpdateBone* updateBone =
                        dynamic_cast<osgAnimation::UpdateBone*>(
                            parent->getUpdateCallback()
                        );
                    if( updateBone )
                    {
                        osgAnimation::StackedTransform& stack =
                            updateBone->getStackedTransforms();
                        stack.push_back(
                    ...
                    {
                        parent->getParent( 0 )->addChild(
                            createRefGeometry( offset, 0.5 ).get()
                        );
                    }
                }
            }
                ...
                if( fr.matchSequence( "OFFSET %f %f %f" ) )
                {
                    ...
                    if( fr.readSequence( offsetEndSite ) )
                    {
                        ...
                        osg::ref_ptr<osgAnimation::Bone> bone =
                            new osgAnimation::Bone( parent->getName() + "End" );
                        bone->setMatrixInSkeletonSpace(
                            osg::translate( offsetEndSite ) *
                            bone->getMatrixInSkeletonSpace()
                        );
                        bone->setDataVariance( osg::Object::DataVariance::DYNAMIC );
                        parent->insertChild( 0, bone.get() );

                        if( _drawingFlag )
                        {
                            parent->addChild(
                                createRefGeometry( offsetEndSite, 0.5 ).get()
                            );
                        }
                    }
                }
            ...
            if( fr.matchSequence( "ROOT %w {" ) || fr.matchSequence( "JOINT %w {" ) )
            {
                ...
                osg::ref_ptr<osgAnimation::Bone> bone =
                    new osgAnimation::Bone( fr[1].getStr() );
                bone->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        void
        buildMotion( osgDB::Input&            fr,
                     osgAnimation::Animation* anim )
        {
            int   i = 0, frames = 0;
            ...
            {
                OSG_WARN
                    << "BVH Reader: Frame number setting not found, but an unexpected "
            ...
            {
                OSG_WARN
                    << "BVH Reader: Frame time setting not found, but an unexpected "
            ...
            for( JointList::iterator itr = _joints.begin(); itr != _joints.end(); ++itr )
            {
                ...
                osg::ref_ptr<osgAnimation::Vec3KeyframeContainer> posKey =
                    new osgAnimation::Vec3KeyframeContainer;
                osg::ref_ptr<osgAnimation::Vec3LinearSampler> posSampler =
                    new osgAnimation::Vec3LinearSampler;
                osg::ref_ptr<osgAnimation::Vec3LinearChannel> posChannel =
                    new osgAnimation::Vec3LinearChannel( posSampler.get() );
                posSampler->setKeyframeContainer( posKey.get() );
                posChannel->setName( "position" );
                posChannel->setTargetName( name );

                osg::ref_ptr<osgAnimation::QuatKeyframeContainer> rotKey =
                    new osgAnimation::QuatKeyframeContainer;
                ...
                for( unsigned int n = 0; n < _joints.size(); ++n )
                {
                    ...
                    setKeyframe( fr,
                                 ...
                                 dynamic_cast<osgAnimation::Vec3KeyframeContainer*>(
                                     posChannel->getSampler()->getKeyframeContainer()
                                 ),
                                 dynamic_cast<osgAnimation::QuatKeyframeContainer*>(
                                     rotChannel->getSampler()->getKeyframeContainer()
                                 ) );
                }
                  ...
                  const osgDB::ReaderWriter::Options* options )
        {
            if( options )
            {
                ...
                else if( options->getOptionString().find( "solids" ) !=
                         std::string::npos )
                {
                    _drawingFlag = 2;
                }
    ...
    protected:
                     ...
                     osgAnimation::QuatKeyframeContainer* rotKey )
        {
            ...
            if( ch & 0X38 && rotKey )    // Rotation keyframe
            {
                ...
                // So we should not create quat directly from input values, which makes a
                // wrong X*Y*Z
                osg::mat4 rotMat = osg::rotate( osg::radians( keyValue[1] ),
                                                osg::vec3( 0.0, 1.0, 0.0 ) ) *
                                   osg::rotate( osg::radians( keyValue[0] ),
                                                osg::vec3( 1.0, 0.0, 0.0 ) ) *
                                   osg::rotate( osg::radians( keyValue[2] ),
                                                osg::vec3( 0.0, 0.0, 1.0 ) );
                osg::quat quat   = osg::getRotate( rotMat );
                rotKey->push_back( osgAnimation::QuatKeyframe( time, quat ) );
            }
        }

        osg::ref_ptr<osg::Geode>
        createRefGeometry( osg::vec3 p,
                           double    len )
        {
            ...
            else if( _drawingFlag == 2 )
            {
                ...
                osg::ref_ptr<osg::Box> box =
                    new osg::Box( p * 0.5, osg::length( p ), len, len );
                quat = osg::quat( osg::vec3( 1.0F, 0.0F, 0.0F ), p );
