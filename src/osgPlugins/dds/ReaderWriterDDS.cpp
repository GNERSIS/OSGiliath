...
// Macro similar to what's in FLT/TRP plugins (except it uses wide char under Windows if
...
struct DDSURFACEDESC2
{
        ...
        UI32 dwAlphaBitDepth;
        UI32 dwReserved;
        UI32 lpSurface;    // Fred Marmond: removed from pointer type to UI32 for 64bits
                           // compatibility. it is unused data
...
//
#define DDSD_CAPS            0X00'00'00'01L    // default
#define DDSD_HEIGHT          0X00'00'00'02L    // default
#define DDSD_WIDTH           0X00'00'00'04L    // default
#define DDSD_PIXELFORMAT     0X00'00'10'00L    // default
#define DDSD_PITCH           0X00'00'00'08L    // For uncompressed formats
#define DDSD_MIPMAPCOUNT     0X00'02'00'00L
#define DDSD_LINEARSIZE      0X00'08'00'00L    // For compressed formats
#define DDSD_DEPTH           0X00'80'00'00L    // Volume Textures

//
// DDPIXELFORMAT flags
//
#define DDPF_ALPHAPIXELS     0X00'00'00'01L
#define DDPF_FOURCC          0X00'00'00'04L    // Compressed formats
#define DDPF_PALETTEINDEXED8 0X00'00'00'20L
#define DDPF_RGB             0X00'00'00'40L    // Uncompressed formats
#define DDPF_ALPHA           0X00'00'00'02L
#define DDPF_COMPRESSED      0X00'00'00'80L
#define DDPF_LUMINANCE       0X00'02'00'00L
#define DDPF_BUMPLUMINANCE   0X00'04'00'00L    // L,U,V
#define DDPF_BUMPDUDV        0X00'08'00'00L    // U,V

//
// DDSCAPS flags
//
#define DDSCAPS_TEXTURE      0X00'00'10'00L    // default
#define DDSCAPS_COMPLEX      0X00'00'00'08L
#define DDSCAPS_MIPMAP       0X00'40'00'00L
#define DDSCAPS2_VOLUME      0X00'20'00'00L
...
typedef enum OSG_DXGI_FORMAT
{
    ...
    OSG_DXGI_FORMAT_FORCE_UINT                 = 0XFF'FF'FF'FFUL,
} OSG_DXGI_FORMAT;
                         ...
                         int          image_packing = 1 )
{
    ...
    return osg::Image::computeImageSizeInBytes( width,
                                                height,
                                                depth,
                                                pixelFormat,
                                                pixelType,
...
osg::Image*
ReadDDSFile( std::istream& _istream,
             bool          flipDDSRead )
{
    DDSURFACEDESC2 ddsd;
    ...
    {
        OSG_WARN << "ReadDDSFile warning: DDPF_BUMPDUDV format is not supported"
                 << std::endl;
        return NULL;
    ...
    {
        OSG_WARN << "ReadDDSFile warning: DDPF_BUMPLUMINANCE format is not supported"
                 << std::endl;
        return NULL;
    ...
    if( ddsd.ddpfPixelFormat.dwFlags & DDPF_FOURCC )
    {
        ...
        {
            case FOURCC_DXT1 :
                OSG_INFO << "ReadDDSFile info : format = DXT1, usingAlpha=" << usingAlpha
                         << std::endl;
                if( usingAlpha )
                {
            ...
            case FOURCC_DX10 :
                OSG_INFO << "ReadDDSFile info : format = DX10 file" << std::endl;
                {
                    ...
                    switch( header10.dxgiFormat )
                    {
                        ...
                        default :
                            OSG_WARN
                                << "ReadDDSFile warning: unhandled DX10 pixel format 0x"
            ...
            default :
                OSG_WARN
                    << "ReadDDSFile warning: unhandled FOURCC pixel format ("
                    << ( char )( ddsd.ddpfPixelFormat.dwFourCC & 0X00'00'00'FF )
                    << ( char )( ( ddsd.ddpfPixelFormat.dwFourCC & 0X00'00'FF'00 ) >> 8 )
                    << ( char )( ( ddsd.ddpfPixelFormat.dwFourCC & 0X00'FF'00'00 ) >>
                                 16 )
                    << ( char )( ( ddsd.ddpfPixelFormat.dwFourCC & 0XFF'00'00'00 ) >>
                                 24 )
    ...
    else if( ddsd.ddpfPixelFormat.dwFlags & DDPF_RGB )
    {
        ...
        static const RGBFormat rgbFormats[] = {
            ...
            {     "R8G8B8",
             24,    0XFF'00'00,
             0X00'FF'00,    0X00'00'FF,
             0X00'00'00,        GL_RGB,
             GL_BGR,               GL_UNSIGNED_BYTE     },

            {     "B8G8R8",
             24,    0X00'00'FF,
             0X00'FF'00,    0XFF'00'00,
             0X00'00'00,        GL_RGB,
             GL_RGB,               GL_UNSIGNED_BYTE     },

            {   "A8R8G8B8",
             32, 0X00'FF'00'00,
             0X00'00'FF'00, 0X00'00'00'FF,
             0XFF'00'00'00,       GL_RGBA,
             GL_BGRA,               GL_UNSIGNED_BYTE    },
            {   "X8R8G8B8",
             32, 0X00'FF'00'00,
             0X00'00'FF'00, 0X00'00'00'FF,
             0X00'00'00'00,        GL_RGB,
             GL_BGRA,               GL_UNSIGNED_BYTE    },
            {   "A8B8G8R8",
             32, 0X00'00'00'FF,
             0X00'00'FF'00, 0X00'FF'00'00,
             0XFF'00'00'00,       GL_RGBA,
            ...
            {"A2R10G10B10",
             32, 0X00'00'03'FF,
             0X00'0F'FC'00, 0X3F'F0'00'00,
             0XC0'00'00'00,       GL_RGBA,
             GL_BGRA, GL_UNSIGNED_INT_2_10_10_10_REV    },
            {"A2B10G10R10",
             32, 0X3F'F0'00'00,
             0X00'0F'FC'00, 0X00'00'03'FF,
             0XC0'00'00'00,       GL_RGBA,
             GL_RGBA, GL_UNSIGNED_INT_2_10_10_10_REV    },
            {     "G16R16",
             32, 0X00'00'FF'FF,
             0XFF'FF'00'00, 0X00'00'00'00,
             0X00'00'00'00,        GL_RGB,
             UNSUPPORTED,              GL_UNSIGNED_SHORT},
            {  "B16G16R16",
             48,    0X00'00'FF,
             0X00'FF'00,    0XFF'00'00,
             0X00'00'00, GL_RGB16F_ARB,
             GL_RGB,                  GL_HALF_FLOAT     },
            {  "B32G32R32",
             96,    0X00'00'FF,
             0X00'FF'00,    0XFF'00'00,
             0X00'00'00, GL_RGB32F_ARB,
             GL_RGB,                       GL_FLOAT     },
        };
        ...
        if( !found )
        {
            OSG_WARN << "ReadDDSFile warning: unhandled RGB pixel format in dds file, "
                        "image not loaded"
                     << std::endl;
    ...
    {
        internalFormat = usingAlpha ? GL_RG : GL_RED;
        pixelFormat    = usingAlpha ? GL_RG : GL_RED;
    ...
    unsigned int size =
        ComputeImageSizeInBytes( s, t, r, pixelFormat, dataType, packing );
    ...
    if( ddsd.dwMipMapCount > 1 )
    {
        ...
        for( unsigned int k = 0; k < mipmap_offsets.size(); ++k )
        {
            ...
            mip_depth          = std::max( mip_depth >> 1, 1 );

            sizeWithMipmaps   += ComputeImageSizeInBytes( mip_width,
                                                          mip_height,
                                                          mip_depth,
                                                          pixelFormat,
                                                          dataType,
    ...
    unsigned char palette[1'024];
    if( ddsd.ddpfPixelFormat.dwFlags & DDPF_PALETTEINDEXED8 )
    {
        if( !_istream.read( ( char* )palette, 1'024 ) )
        {
    ...
    if( size <
        sizeWithMipmaps &&
        !_istream.read( ( char* )imageData + size, sizeWithMipmaps - size ) )
    {
        sizeWithMipmaps = size;
        mipmap_offsets.resize( 0 );
        OSG_WARN << "ReadDDSFile warning: couldn't read mipmapData" << std::endl;

        // if mipmaps read failed we leave some not used overhead memory allocated past
        // main image this memory will not be used but it will not cause leak in worst
    ...
    if( ddsd.ddpfPixelFormat.dwFlags & DDPF_PALETTEINDEXED8 )
    {
        ...
        {
            memcpy( pconvertedData,
                    &palette[imageData[i] * 4],
    ...
    if( flipDDSRead )
    {
        ...
        if( !isDXTC ||
            ( ( s > 4 && s % 4 == 0 && t > 4 && t % 4 == 0 ) ||
              s <=
              4 ) )    // Flip may crash (access violation) or fail for non %4 dimensions
                       // (except for s<4). Tested with revision trunk 2013-02-22.
        {
        ...
        else
        {
            OSG_WARN << "ReadDDSFile warning: Vertical flip was skipped. Image "
                        "dimensions have to be multiple of 4."
                     << std::endl;
        }
    }
...
bool
WriteDDSFile( const osg::Image* img,
              std::ostream&     fout,
              bool              autoFlipDDSWrite )
{
    bool           isDXTC( false );
    ...
    // Check that theorical image size (computation taking into account DXTC blocks) is
    // not bigger than actual image size. This may happen, for instance, if some
    // operation tuncated the data buffer non block-aligned. Example:
    //  - Read DXT1 image, size = 8x7. Actually, image data is 8x8 because it stores 4x4
    //  blocks.
    //  - Some hypothetical operation wrongly assumes the data buffer is 8x7 and
    //  truncates the buffer. This may even lead to access violations.
    //  - Then we write the DXT1 image: last block(s) is (are) corrupt.
    // Actually what could be very nice is to handle some "lines packing" (?) in DDS
    // reading, indicating that the image buffer has "additional lines to reach a
    // multiple of 4". Please note this can also produce false positives (ie. when data
    // buffer is large enough, but getImageSizeInBytes() returns a smaller value). There
    // is no way to detect this, until we fix getImageSizeInBytes() with "line packing".
    ...
    {
        OSG_FATAL << "Image cannot be written as DDS (Maybe a corrupt S3TC-DXTC image, "
                     "with non %4 dimensions)."
                  << std::endl;
        return false;
    }
    ...
    switch( pixelFormat )
    {
        ...
        case GL_RGBA :
            {
                ddpf.dwRBitMask         = 0X00'00'00'FF;
                ddpf.dwGBitMask         = 0X00'00'FF'00;
                ddpf.dwBBitMask         = 0X00'FF'00'00;
                ddpf.dwRGBAlphaBitMask  = 0XFF'00'00'00;
        ...
        case GL_BGRA :
            {
                ddpf.dwBBitMask         = 0X00'00'00'FF;
                ddpf.dwGBitMask         = 0X00'00'FF'00;
                ddpf.dwRBitMask         = 0X00'FF'00'00;
                ddpf.dwRGBAlphaBitMask  = 0XFF'00'00'00;
                PF_flags               |= ( DDPF_ALPHAPIXELS | DDPF_RGB );
                ddpf.dwRGBBitCount      = pixelSize;
                ddsd.lPitch             = img->getRowSizeInBytes();
        ...
        case GL_RG :
            {
                ddpf.dwRBitMask         = 0X00'00'00'FF;
                ddpf.dwRGBAlphaBitMask  = 0X00'00'FF'00;
        ...
        default :
            OSG_WARN
                << "Warning:: unhandled pixel format in image, file cannot be written."
                << std::endl;
            return false;
    }
    ...
    {    // image contains mipmaps and has 1 byte alignment
        ...
        OSG_INFO << "writing out with mipmaps ddsd.dwMipMapCount" << ddsd.dwMipMapCount
                 << std::endl;
    }
    ...
    if( autoFlipDDSWrite && img->getOrigin() == osg::Image::BOTTOM_LEFT )
    {
        ...
        osg::ref_ptr<osg::Image> copy( new osg::Image( *img,
    ...
    fout.write( reinterpret_cast<char*>( &ddsd ),
                sizeof( ddsd ) ); /* write file header */
...
class ReaderWriterDDS : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterDDS()
        {
            ...
            supportsOption(
                "dds_dxt1_rgb",
                "Set the pixel format of DXT1 encoded images to be RGB variant of DXT1"
            );
            supportsOption(
                "dds_dxt1_rgba",
                "Set the pixel format of DXT1 encoded images to be RGBA variant of DXT1"
            );
            supportsOption( "dds_dxt1_detect_rgba",
                            "For DXT1 encode images set the pixel format according to "
                            "presence of transparent pixels" );
            supportsOption( "dds_flip", "Flip the image about the horizontal axis" );
            supportsOption(
                "ddsNoAutoFlipWrite",
                "(Write option) Avoid automatically flipping the image vertically when "
                "writing, depending on the origin (Image::getOrigin())."
            );
        }
                   ...
                   const Options* options ) const
        {
                ...
                osgImage->getPixelFormat() == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT )
            {
                if( dds_dxt1_rgba )
                {
                    ...
                    osgImage->setInternalTextureFormat(
                ...
                {
                    // check to see if DXT1c (RGB_S3TC_DXT1) format image might actually
                    // be a DXT1a format image

                    // temporarily set pixel format to GL_COMPRESSED_RGBA_S3TC_DXT1_EXT
                    // so that the isImageTranslucent() method assumes that RGBA is
                    // present and then checks the alpha values to see if they are
                    // all 1.0.
                    osgImage->setPixelFormat( GL_COMPRESSED_RGBA_S3TC_DXT1_EXT );
                    osgImage->setInternalTextureFormat(
                        GL_COMPRESSED_RGBA_S3TC_DXT1_EXT
                    ...
                    if( !osgImage->isImageTranslucent() )
                    {
                        ...
                        OSG_INFO
                            << "Image with PixelFormat==GL_COMPRESSED_RGB_S3TC_DXT1_EXT "
                               "is opaque."
                            << std::endl;
                        osgImage->setPixelFormat( GL_COMPRESSED_RGB_S3TC_DXT1_EXT );
                        osgImage->setInternalTextureFormat(
                    ...
                    else
                    {
                        ...
                        OSG_INFO
                            << "Image with PixelFormat==GL_COMPRESSED_RGB_S3TC_DXT1_EXT "
                               "has transparency, setting format to "
                               "GL_COMPRESSED_RGBA_S3TC_DXT1_EXT."
                            << std::endl;
                    }
                }
        ...
        writeImage( const osg::Image& image,
                    ...
                    const Options*    options ) const
        {
            bool noAutoFlipDDSWrite =
                options &&
                options->getOptionString().find( "ddsNoAutoFlipWrite" ) !=
                std::string::npos;
