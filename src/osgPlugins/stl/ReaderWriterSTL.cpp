...
STLOptionsStruct
parseOptions( const osgDB::ReaderWriter::Options* options )
{
...
class ReaderWriterSTL : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterSTL()
        {
            ...
            supportsOption( "separateFiles",
                            "Save each geode in a different file. Can result in a huge "
                            "amount of files!" );
            supportsOption( "dontSaveNormals",
                            "Set all normals to [0 0 0] when saving to a file." );
        }
    ...
    private:
        ...
        class ReaderObject : public osg::Referenced
        {
            public:
                ...
                ReaderObject( bool noTriStripPolygons,
                ...
                asGeometry() const
                {
                    ...
                    if( _normal.valid() )
                    {
                        ...
                        osg::ref_ptr<osg::Vec3Array> perVertexNormals =
                            new osg::Vec3Array;
                    ...
                    if( _color.valid() )
                    {
                        ...
                        osg::ref_ptr<osg::Vec4Array> perVertexColours =
                            new osg::Vec4Array;
        ...
        class BinaryReaderObject : public ReaderObject
        {
            public:
                ...
                BinaryReaderObject( unsigned int expectNumFacets,
                                    bool         noTriStripPolygons,
                                    bool         generateNormals = true ) :
                    ReaderObject( noTriStripPolygons,
                                  generateNormals ),
        ...
        class CreateStlVisitor : public osg::DualModeVisitor
        {
            public:

                CreateStlVisitor( const std::string&                  fout,
                    ...
                    m_f( 0 )
                {
                    ...
                    {
                        OSG_INFO
                            << "ReaderWriterSTL::writeNode: Files are written separately"
                            << std::endl;
                ...
                {
                    osg::dmat4 mat = osg::computeLocalToWorld( getNodePath() );
                ...
                ~CreateStlVisitor()
                {
                    if( m_localOptions.separateFiles )
                    {
                        OSG_INFO << "ReaderWriterSTL::writeNode: "
                                 << counter - 1 << " files were written" << std::endl;
                    }
                    else
                    {
                        m_f->close();
            ...
            private:
                ...
                struct PushPoints
                {
                        ...
                        operator()( const osg::vec3& _v1,
                                    ...
                                    const osg::vec3& _v3 )
                        {
                            osg::vec3 v1      = osg::vec3( _v1 * m_mat );
                            osg::vec3 v2      = osg::vec3( _v2 * m_mat );
                            osg::vec3 v3      = osg::vec3( _v3 * m_mat );
                            osg::vec3 vV1V2   = v2 - v1;
                            osg::vec3 vV1V3   = v3 - v1;
                            osg::vec3 vNormal = osg::cross( vV1V2, vV1V3 );
                            ...
                            *m_stream << "vertex " << v1[0] << " " << v1[1] << " "
                                      << v1[2] << std::endl;
                            *m_stream << "vertex " << v2[0] << " " << v2[1] << " "
                                      << v2[2] << std::endl;
                            *m_stream << "vertex " << v3[0] << " " << v3[1] << " "
                                      << v3[2] << std::endl;
...
const unsigned short StlHasColor     = 0X80'00;
...
bool
fileComesFromMagics( FILE*      fp,
                     osg::vec4& magicsColor )
{
...
ReaderWriterSTL::readNode( const std::string&                  file,
                           const osgDB::ReaderWriter::Options* options ) const
{
    ...
    {
        OSG_NOTICE << "Warning: STL reading not supported as unsigned int is not 4 "
    ...
    if( header_text.find( "solid" ) == std::string::npos || stb.st_size == expectLen )
    {
        if( stb.st_size < expectLen )
        {
            ...
            OSG_WARN << "Warning: [[stl]] Incomplete file. "
                     << "Attempting to read " << facets << " out of " << expectFacets
                     << " facets expected." << std::endl;
            expectFacets = facets;
        }
        isBinary = true;
    }
    ...
    {
        readerObject =
            new BinaryReaderObject( expectFacets, localOptions.noTriStripPolygons );
    }
    else
    {
...
ReaderWriterSTL::BinaryReaderObject::read( FILE* fp )
{
    ...
    for( unsigned int i = 0; i < _expectNumFacets; ++i )
    {
        ...
        {
            OSG_FATAL << "ReaderWriterSTL::readStlBinary: Failed to read facet " << i
                      << std::endl;
            return ReadError;
        }
         ...
         * for a given face, according to the value of the last bit (0 = per-face, 1 =
         * per-object) Moreover, magics uses RGB instead of BGR (as the other software)
         */
        if( !_color.valid() )
        {
        ...
        {
            if( facet.color &
                StlHasColor )    // The last bit is 1, the per-object color is used
            {
        ...
        // Case of a generic file
                 ...
                 StlHasColor )    // The color is valid if the last bit is 1
        {
...
ReaderWriterSTL::writeNode( const osg::Node&   node,
                            const std::string& fileName,
                            const Options*     opts ) const
{
    ...
    if( ext != "stl" )
    {
        OSG_FATAL << "ReaderWriterSTL::writeNode: Only STL ASCII files supported"
                  << std::endl;
        return WriteResult::FILE_NOT_HANDLED;
    }
