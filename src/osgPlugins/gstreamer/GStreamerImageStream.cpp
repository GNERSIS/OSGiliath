...
namespace osgGStreamer
{
    ...
    GStreamerImageStream::GStreamerImageStream( const GStreamerImageStream& image,
    ...
    GStreamerImageStream::~GStreamerImageStream()
    {
        ...
        gst_element_get_state( _pipeline,
                               NULL,
                               NULL,
                               GST_CLOCK_TIME_NONE );    // wait until the state changed
    ...
    bool
    GStreamerImageStream::open( const std::string& filename )
    {
    ...
    void
    GStreamerImageStream::seek( double time )
    {
    ...
    GstFlowReturn
    GStreamerImageStream::on_new_sample( GstAppSink*           appsink,
    ...
    GstFlowReturn
    GStreamerImageStream::on_new_preroll( GstAppSink*           appsink,
                                          GStreamerImageStream* user_data )
    {
        ...
        if( width <= 0 || height <= 0 )
        {
            OSG_NOTICE << "Error: video size invalid width=" << width
        ...
        if( user_data->_width != width || user_data->_height != height )
        {
            ...
            // allocate buffer
            user_data->_internal_buffer =
                ( unsigned char* )malloc( sizeof( unsigned char ) * row_width * height );

            // assign buffer to image
            user_data->setImage( user_data->_width,
                                 user_data->_height,
                                 1,
                                 GL_RGB,
                                 GL_RGB,
    ...
    gboolean
    GStreamerImageStream::on_message( GstBus* /*bus*/,
                                      GstMessage*           message,
                                      GStreamerImageStream* user_data )
    {
        if( GST_MESSAGE_TYPE( message ) == GST_MESSAGE_EOS )
        {
            OSGGST_INFO << "Video '" << user_data->getFileName() << "' finished."
                        << std::endl;
