...
daeReader::processAnimationLibraries( domCOLLADA* document )
{
    domLibrary_animation_clips_Array domAnimationClipsLibraries =
        document->getLibrary_animation_clips_array();

    domLibrary_animations_Array domAnimationsLibraries =
        document->getLibrary_animations_array();
    osgAnimation::BasicAnimationManager* pOsgAnimationManager = NULL;

    // Only create an animationmanager if we have  animation clip libraries or animation
    // libraries
    if( ( domAnimationClipsLibraries.getCount() > 0 ) ||
        ( domAnimationsLibraries.getCount() > 0 ) )
    {
        ...
        {
            domAnimation_clip_Array domAnimationClips =
                domAnimationClipsLibraries[i]->getAnimation_clip_array();
        ...
        if( domAnimationClipsLibraries.getCount() ==
            0 &&
            domAnimationsLibraries.getCount() )
        {
            ...
            {
                domAnimation_Array domAnimations =
                    domAnimationsLibraries[i]->getAnimation_array();
...
// 0..* <extra>
void
daeReader::processAnimationClip(
    osgAnimation::BasicAnimationManager* pOsgAnimationManager,
    domAnimation_clip*                   pDomAnimationClip
)
{
    // an <animation_clip> groups animations
    osgAnimation::Animation* pOsgAnimation = new osgAnimation::Animation;
    std::string              name =
        pDomAnimationClip->getId() ? pDomAnimationClip->getId() : "AnimationClip";
    ...
    // 1..* <instance_animation>
    domInstanceWithExtra_Array domInstanceArray =
        pDomAnimationClip->getInstance_animation_array();
    for( size_t i = 0; i < domInstanceArray.getCount(); i++ )
    {
        domAnimation* pDomAnimation = daeSafeCast<domAnimation>(
            getElementFromURI( domInstanceArray[i]->getUrl() )
        );
        if( pDomAnimation )
        {
        ...
        {
            OSG_WARN << "Failed to locate animation "
                     << domInstanceArray[i]->getUrl().getURI() << std::endl;
        }
    }
...
{
        bool
        operator()( const osgAnimation::Keyframe& a,
                    const osgAnimation::Keyframe& b ) const
        {
...
template<typename T>
void
deCasteljau( osgAnimation::TemplateCubicBezier<T>& l,
             osgAnimation::TemplateCubicBezier<T>& n,
             osgAnimation::TemplateCubicBezier<T>& r,
             float                                 t )
{
    T q1 = l.getPosition() + t * ( l.getControlPointOut() - l.getPosition() );
    T q2 =
        l.getControlPointOut() + t * ( r.getControlPointIn() - l.getControlPointOut() );
...
void
mergeKeyframeContainers( osgAnimation::Vec3CubicBezierKeyframeContainer*   to,
                         ...
                         const osg::vec3&             defaultValue )
{
    ...
    for( TimeSet::const_iterator it = times.begin(), end = times.end(); it != end; ++it )
    {
        ...
        for( int i = 0; i < 3; ++i )
        {
            ...
            {
                --next;
                value.getPosition().data()[i] = next->getValue().getPosition();
                value.getControlPointIn().data()[i] =
                    next->getValue().getControlPointIn();
                value.getControlPointOut().data()[i] =
                    next->getValue().getControlPointOut();
            }
            ...
            else
            {
                ...
                switch( interpolationType )
                {
                    case daeReader::INTERPOLATION_STEP :
                        value.getPosition().data()[i] = prev->getValue().getPosition();
                        break;
                    case daeReader::INTERPOLATION_LINEAR :
                        {
                            ...
                            value.getPosition().data()[i] =
                                yp + ( yn - yp ) * ( time - xp ) / ( xn - xp );
                        }
                        break;
                    case daeReader::INTERPOLATION_BEZIER :
                        {
                            ...
                            osgAnimation::FloatCubicBezier l( prev->getValue() ), n,
                                r( next->getValue() );
                            deCasteljau( l, n, r, ( time - xp ) / ( xn - xp ) );

                            value.getPosition().data()[i]       = n.getPosition();
                            value.getControlPointIn().data()[i] = n.getControlPointIn();
                            value.getControlPointOut().data()[i] =
                                n.getControlPointOut();

                            osgAnimation::Vec3CubicBezier prevValue =
                                to->back().getValue();
                            prevValue.getControlPointOut().data()[i] =
                                l.getControlPointOut();
                            ...
                            from[i]->insert(
                                next,
                                osgAnimation::FloatCubicBezierKeyframe( time, n )
                            );
                        }
                        break;
                    default :
...
void
daeReader::processAnimationChannels( domAnimation*         pDomAnimation,
                                     TargetChannelPartMap& tcm )
{
    // 1..* <source>
    SourceMap       sources;
    ...
    {
        sources.insert( std::make_pair( ( daeElement* )domSources[i],
                                        domSourceReader( domSources[i] ) ) );
    }
...
convertKeyframeContainerToLinear( osgAnimation::Vec3CubicBezierKeyframeContainer& from )
{
    osgAnimation::Vec3KeyframeContainer* linearKeyframes =
        new osgAnimation::Vec3KeyframeContainer;
    for( size_t i = 0; i < from.size(); ++i )
    {
        linearKeyframes->push_back(
            osgAnimation::Vec3Keyframe( from[i].getTime(),
                                        from[i].getValue().getPosition() )
        );
    }
...
template<typename T>
void
convertHermiteToBezier( osgAnimation::TemplateKeyframeContainer<T>& keyframes )
{
...
template<typename T>
void
reorderControlPoints( osgAnimation::TemplateKeyframeContainer<
                      osgAnimation::TemplateCubicBezier<T>>& vkfCont )
{
    if( vkfCont.size() <= 1 )
    {
        if( vkfCont.size() == 1 )
...
// 0..* <extra>
void
daeReader::processAnimationMap( const TargetChannelPartMap& tcm,
                                osgAnimation::Animation*    pOsgAnimation )
{
    for( TargetChannelPartMap::const_iterator lb = tcm.begin(), end = tcm.end();
         lb != end; )
    {
        ...
        if( osgAnimation::Vec3Target* pTarget =
                dynamic_cast<osgAnimation::Vec3Target*>( lb->first ) )
        {
            osgAnimation::FloatCubicBezierKeyframeContainer* fkfConts[3] =
                { NULL, NULL, NULL };
            ...
            for( TargetChannelPartMap::const_iterator it = lb; it != ub; ++it )
            {
                ...
                extractTargetName( channelPart->name,
                                   channelName,
                                   targetName,
                                   componentName );
                interpolationType = channelPart->interpolation;

                if( osgAnimation::Vec3CubicBezierKeyframeContainer* v3cnt =
                        dynamic_cast<osgAnimation::Vec3CubicBezierKeyframeContainer*>(
                            channelPart->keyframes.get()
                        ) )
                {
                    vkfCont = v3cnt;
                    break;
                }
                else if( osgAnimation::FloatCubicBezierKeyframeContainer* fcnt =
                             dynamic_cast<
                             ...
                             ) )
                {
                    ...
                    {
                        OSG_WARN << "Unrecognised vector component \"" << componentName
                                 << "\"" << std::endl;
                    }
                }
                else
                {
                    OSG_WARN << "Unrecognised keyframe container \"" << channelPart->name
                             << "\"" << std::endl;
                }
            }
            ...
            if( !pOsgAnimationChannel && ( fkfConts[0] || fkfConts[1] || fkfConts[2] ) )
            {
                ...
                mergeKeyframeContainers( vkfCont,
                                         fkfConts,
                                         interpolationType,
                                         pTarget->getValue() );
            }
            ...
            if( vkfCont )
            {
                ...
                {
                    osgAnimation::Vec3StepChannel* channel =
                        new osgAnimation::Vec3StepChannel;
                    pOsgAnimationChannel = channel;
                    channel->getOrCreateSampler()->setKeyframeContainer(
                        convertKeyframeContainerToLinear( *vkfCont )
                    );
                }
                else if( interpolationType == INTERPOLATION_LINEAR )
                {
                    osgAnimation::Vec3LinearChannel* channel =
                ...
                {
                    osgAnimation::Vec3CubicBezierChannel* channel =
                        new osgAnimation::Vec3CubicBezierChannel;
                    pOsgAnimationChannel = channel;
        ...
        else
        {
            ...
            typedef osgAnimation::TemplateKeyframe<
                osgAnimation::TemplateCubicBezier<osg::mat4>>
                MatrixCubicBezierKeyframe;
            typedef osgAnimation::TemplateKeyframeContainer<
                osgAnimation::TemplateCubicBezier<osg::mat4>>
                MatrixCubicBezierKeyframeContainer;

            if( osgAnimation::FloatCubicBezierKeyframeContainer* kfCntr =
                    dynamic_cast<osgAnimation::FloatCubicBezierKeyframeContainer*>(
                        channelPart->keyframes.get()
                    ) )
            {
                if( dynamic_cast<osgAnimation::MatrixTarget*>( lb->first ) )
                {
                    OSG_WARN << "Animating elements of matrices not supported."
                             << std::endl;
                }
                else
                {
                    osgAnimation::FloatCubicBezierChannel* channel =
                        new osgAnimation::FloatCubicBezierChannel;
                    ...
                    pOsgAnimationChannel = channel;
                         ...
                         dynamic_cast<MatrixCubicBezierKeyframeContainer*>(
                             channelPart->keyframes.get()
                         ) )
            {
                osgAnimation::MatrixKeyframeContainer* cbCntr =
                    new osgAnimation::MatrixKeyframeContainer;
                for( size_t i = 0; i < cbkfCntr->size(); ++i )
                {
                    const MatrixCubicBezierKeyframe& cbkf = cbkfCntr->at( i );
                    cbCntr->push_back(
                        osgAnimation::MatrixKeyframe( cbkf.getTime(),
                                                      cbkf.getValue().getPosition() )
                    );
                }
                osgAnimation::MatrixLinearChannel* channel =
...
makeKeyframes( const osg::FloatArray*        pOsgTimesArray,
               TArray*                       pOsgPointArray,
               TArray*                       pOsgInTanArray,
               TArray*                       pOsgOutTanArray,
               daeReader::InterpolationType& interpolationType )
{
    osgAnimation::TemplateKeyframeContainer<osgAnimation::TemplateCubicBezier<T>>*
...
//        1    semantic
//        1    source
daeReader::ChannelPart*
daeReader::processSampler( domChannel* pDomChannel,
                           SourceMap&  sources )
{
    ...
    domSampler* pDomSampler =
        daeSafeCast<domSampler>( getElementFromURI( pDomChannel->getSource() ) );
    if( !pDomSampler )
    {
        return NULL;
    }
    ...
    if( findInputSourceBySemantic( domInputArray,
                                   COMMON_PROFILE_INPUT_INPUT,
                                   input_source,
                                   &tmp ) )
    {
        ...
        if( pDomSource )
        {
            domSource::domTechnique_common* pDomTechnique =
                pDomSource->getTechnique_common();
            if( pDomTechnique )
            {
                ...
                if( domParams.getCount() > 0 )
                {
                    ...
                    {
                        pOsgTimesArray =
                            sources[input_source].getArray<osg::FloatArray>();
                    }
                    else
                    {
                        OSG_WARN << "Only TIME based animations are supported"
                                 << std::endl;
                        return NULL;
                    }
                }
            ...
            {
                OSG_WARN << "Unable to find <technique_common> in <source> "
                         << pDomSource->getName() << std::endl;
                return NULL;
            }
        }
        else
    ...
    // const bool readDoubleKeyframes = (_precisionHint &
    // osgDB::Options::DOUBLE_PRECISION_KEYFRAMES) != 0;
    static const bool readDoubleKeyframes = false;

    findInputSourceBySemantic( domInputArray,
                               COMMON_PROFILE_INPUT_OUTPUT,
                               output_source,
                               &tmp );
    findInputSourceBySemantic( domInputArray,
                               COMMON_PROFILE_INPUT_IN_TANGENT,
                               output_intangent_source,
                               &tmp );
    findInputSourceBySemantic( domInputArray,
                               COMMON_PROFILE_INPUT_OUT_TANGENT,
                               output_outtangent_source,
                               &tmp );
    domSourceReader::ArrayType arrayType =
        sources[output_source].getArrayType( readDoubleKeyframes );
    ...
    if( findInputSourceBySemantic( domInputArray,
                                   COMMON_PROFILE_INPUT_INTERPOLATION,
                                   input_source,
                                   &tmp ) )
    {
        domSource* pDomSource = daeSafeCast<domSource>( input_source );
        if( pDomSource )
        {
            ...
            if( pDomNames )
            {
                ...
                // Take a look at the first element in the array to see what kind of
                // interpolation is needed multiple interpolation types inside an
            ...
            {
                OSG_WARN << "Unable to find <Name_array> in <source> "
                         << pDomSource->getName() << std::endl;
                return NULL;
            }
        }
    ...
    switch( arrayType )
    {
        ...
        case domSourceReader::vec2 :
            keyframes = makeKeyframes<osg::vec2>(
                pOsgTimesArray,
        ...
        case domSourceReader::vec3 :
            keyframes = makeKeyframes<osg::vec3>(
                pOsgTimesArray,
        ...
        case domSourceReader::vec4 :
            keyframes = makeKeyframes<osg::vec4>(
                pOsgTimesArray,
            ...
            break;
        case domSourceReader::dvec2 :
            keyframes = makeKeyframes<osg::dvec2>(
                pOsgTimesArray,
            ...
            break;
        case domSourceReader::dvec3 :
            keyframes = makeKeyframes<osg::dvec3>(
                pOsgTimesArray,
            ...
            break;
        case domSourceReader::dvec4 :
            keyframes = makeKeyframes<osg::dvec4>(
                pOsgTimesArray,
            ...
            break;
        case domSourceReader::dmat4 :
            keyframes = makeKeyframes<osg::mat4>(
                pOsgTimesArray,
...
osgAnimation::Target*
findChannelTarget( osg::Callback*     nc,
                   const std::string& targetName,
                   bool&              rotation )
{
    if( osgAnimation::UpdateMatrixTransform* umt =
            dynamic_cast<osgAnimation::UpdateMatrixTransform*>( nc ) )
    {
        for( osgAnimation::StackedTransform::const_iterator
                 it  = umt->getStackedTransforms().begin(),
                 end = umt->getStackedTransforms().end();
             it != end;
             ++it )
        {
            ...
            {
                rotation =
                    dynamic_cast<osgAnimation::StackedRotateAxisElement*>( te ) != NULL;
...
void
convertDegreesToRadians( osgAnimation::KeyframeContainer* pKeyframeContainer )
{
            ...
            dynamic_cast<osgAnimation::FloatKeyframeContainer*>( pKeyframeContainer ) )
    {
        for( size_t i = 0; i < fkc->size(); ++i )
        {
            ...
            fk.setValue( osg::radians( fk.getValue() ) );
        }
    }
                 ...
                 ) )
    {
        for( size_t i = 0; i < fcbkc->size(); ++i )
        {
            ...
            fcb.setPosition( osg::radians( fcb.getPosition() ) );
            fcb.setControlPointIn( osg::radians( fcb.getControlPointIn() ) );
            fcb.setControlPointOut( osg::radians( fcb.getControlPointOut() ) );
            fcbk.setValue( fcb );
        }
    }
    else
...
// 1 target
void
daeReader::processChannel( domChannel*           pDomChannel,
                           SourceMap&            sources,
                           TargetChannelPartMap& tcm )
{
    domSampler* pDomSampler =
    ...
    if( pDomSampler )
    {
        ...
        if( pChannelPart )
        {
            domChannelOsgAnimationUpdateCallbackMap::iterator iter =
                _domChannelOsgAnimationUpdateCallbackMap.find( pDomChannel );
            if( iter != _domChannelOsgAnimationUpdateCallbackMap.end() )
            {
                osg::Callback* nc = iter->second.get();

                std::string    channelName, targetName, componentName;
                extractTargetName( pDomChannel->getTarget(),
                                   channelName,
                                   targetName,
                                   componentName );
                // assert(targetName == nc->getName());
                bool bRotationChannel = false;
                if( osgAnimation::Target* pTarget =
                        findChannelTarget( nc, channelName, bRotationChannel ) )
                {
                    if( bRotationChannel )
                    {
                        convertDegreesToRadians( pChannelPart->keyframes.get() );
                    }
                    tcm.insert( TargetChannelPartMap::value_type( pTarget,
                                                                  pChannelPart ) );
                }
                else
                {
                    OSG_WARN << "Target \"" << channelName << "\" not found."
                             << std::endl;
                }
            }
            else
            {
                OSG_WARN << "Could not locate UpdateCallback for <channel> target "
                         << pDomChannel->getTarget() << std::endl;
            }
        }
        else
        {
            OSG_WARN << "<channel> source " << pDomChannel->getSource().getURI()
                     << " has no corresponding osgAnimation::Channel" << std::endl;
        }
    }
    else
    {
        OSG_WARN << "Could not locate <channel> source "
                 << pDomChannel->getSource().getURI() << std::endl;
    }
}

void
daeReader::extractTargetName( const std::string& daeTarget,
                              ...
                              std::string&       component )
{
    ...
    else
    {
        ...
        {
            OSG_WARN << "Couldn't extract a proper name for <channel> target "
                     << daeTarget << std::endl;
        }
    }
        ...
        if( first_parenthesis != std::string::npos )
        {
            ...
            do
            {
                ...
                size_t close_parenthesis =
                    channelName.find_first_of( ")", open_parenthesis );
                component +=
                    channelName.substr( open_parenthesis + 1,
