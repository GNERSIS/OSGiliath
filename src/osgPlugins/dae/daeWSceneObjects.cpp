...
void
daeWriter::writeNodeExtra( osg::Node& node )
{
    ...
    if( _pluginOptions.writeExtras && ( numDesc > 0 ) )
    {
        ...
        domExtra* extra =
            daeSafeCast<domExtra>( currentNode->add( COLLADA_ELEMENT_EXTRA ) );
        extra->setType( "Node" );
        domTechnique* teq =
...
void
daeWriter::apply( osg::Group& node )
{
    debugPrint( node );
    updateCurrentDaeNode();
    currentNode = daeSafeCast<domNode>( currentNode->add( COLLADA_ELEMENT_NODE ) );

    // If a multiswitch node, store it's data as extra "MultiSwitch" data in the
    // "OpenSceneGraph" technique
    ...
    if( _pluginOptions.writeExtras && multiswitch )
    {
        ...
        activeSwitchSet->setValue(
            toString<unsigned int>( multiswitch->getActiveSwitchSet() ).c_str()
        );

        domAny*      valueLists = ( domAny* )teq->add( "ValueLists" );

        unsigned int pos        = 0;
        const osgSim::MultiSwitch::SwitchSetList& switchset =
            multiswitch->getSwitchSetList();
...
void
daeWriter::apply( osg::Switch& node )
{
    debugPrint( node );
    updateCurrentDaeNode();
    currentNode = daeSafeCast<domNode>( currentNode->add( COLLADA_ELEMENT_NODE ) );
...
void
daeWriter::apply( osg::Sequence& node )
{
    debugPrint( node );
    updateCurrentDaeNode();
    currentNode = daeSafeCast<domNode>( currentNode->add( COLLADA_ELEMENT_NODE ) );
    ...
    // If a sequence node, store it's data as extra "Sequence" data in the
    // "OpenSceneGraph" technique
    if( _pluginOptions.writeExtras )
    {
        // Adds the following to a node
        ...
        sequenceMode->setValue(
            toString<osg::Sequence::SequenceMode>( node.getMode() ).c_str()
        );
    }
...
daeWriter::apply( osg::LOD& node )
{
    ...
    if( _pluginOptions.writeExtras )
    {
              ...
              osg::LOD::UNION_OF_BOUNDING_SPHERE_AND_USER_DEFINED ) )
        {
            ...
            radius->setValue(
                toString<osg::LOD::value_type>( node.getRadius() ).c_str()
            );
        }

        domAny* rangeMode = ( domAny* )teq->add( "RangeMode" );
        rangeMode->setValue(
            toString<osg::LOD::RangeMode>( node.getRangeMode() ).c_str()
        );
...
void
daeWriter::apply( osg::ProxyNode& node )
{
    OSG_WARN << "ProxyNode. Missing " << node.getNumChildren() << " children"
             << std::endl;
}

void
daeWriter::apply( osg::LightSource& node )
{
    debugPrint( node );
    updateCurrentDaeNode();

    domInstance_light* il = daeSafeCast<domInstance_light>(
        currentNode->add( COLLADA_ELEMENT_INSTANCE_LIGHT )
    );
    ...
    domLight*   pDomLight =
        daeSafeCast<domLight>( lib_lights->add( COLLADA_ELEMENT_LIGHT ) );
    pDomLight->setId( name.c_str() );

    domLight::domTechnique_common* domTechniqueCommon =
        daeSafeCast<domLight::domTechnique_common>(
            pDomLight->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
        );
    ...
    {
        // Directional light
        domLight::domTechnique_common::domDirectional* domDirectional =
            daeSafeCast<domLight::domTechnique_common::domDirectional>(
                domTechniqueCommon->add( COLLADA_ELEMENT_DIRECTIONAL )
            );

        if( ( position.x != 0 ) || ( position.y != 0 ) || ( position.z != 0 ) )
        {
            osg::vec3 dir( -position.x, -position.y, -position.z );
            // TODO wrap instance_light in a rotating node to translate default light
            // [0,0,-1] into proper direction
        }
    ...
    else if( osg::length( direction ) == 0 )
    {
        // Omni/point light
        domLight::domTechnique_common::domPoint* domPoint =
            daeSafeCast<domLight::domTechnique_common::domPoint>(
                domTechniqueCommon->add( COLLADA_ELEMENT_POINT )
            );
        domPoint->add( COLLADA_ELEMENT_CONSTANT_ATTENUATION );
        domPoint->getConstant_attenuation()->setValue(
            pOsgLight->getConstantAttenuation()
        );
        ...
        domPoint->getQuadratic_attenuation()->setValue(
            pOsgLight->getQuadraticAttenuation()
        );

        if( ( position.x != 0 ) || ( position.y != 0 ) || ( position.z != 0 ) )
        {
            // TODO wrap instance_light in a transforming node to translate default light
            // [0,0,0] into proper position
        }
    ...
    {
        // Spot light
        domLight::domTechnique_common::domSpot* domSpot =
            daeSafeCast<domLight::domTechnique_common::domSpot>(
                domTechniqueCommon->add( COLLADA_ELEMENT_SPOT )
            );
        domSpot->add( COLLADA_ELEMENT_CONSTANT_ATTENUATION );
        domSpot->getConstant_attenuation()->setValue(
            pOsgLight->getConstantAttenuation()
        );
        ...
        domSpot->getQuadratic_attenuation()->setValue(
            pOsgLight->getQuadraticAttenuation()
        );
        ...
        {
            // TODO wrap instance_light in a transforming node to translate default light
            // [0,0,0] into proper position and rotate default direction [0,0,-1] into
            // proper dir
        }
    ...
    {
        domInstance_light* ambientDomInstanceLight = daeSafeCast<domInstance_light>(
            currentNode->add( COLLADA_ELEMENT_INSTANCE_LIGHT )
        );
        name = node.getName();
        if( name.empty() )
        ...
        domLight* ambientDomLight =
            daeSafeCast<domLight>( lib_lights->add( COLLADA_ELEMENT_LIGHT ) );
        ambientDomLight->setId( name.c_str() );

        domLight::domTechnique_common* ambientDomTechniqueCommon =
            daeSafeCast<domLight::domTechnique_common>(
                ambientDomLight->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
            );

        // Ambient light
        domLight::domTechnique_common::domAmbient* domAmbient =
            daeSafeCast<domLight::domTechnique_common::domAmbient>(
                ambientDomTechniqueCommon->add( COLLADA_ELEMENT_AMBIENT )
            );
...
void
daeWriter::apply( osg::Camera& node )
{
    debugPrint( node );
    updateCurrentDaeNode();

    domInstance_camera* ic = daeSafeCast<domInstance_camera>(
        currentNode->add( COLLADA_ELEMENT_INSTANCE_CAMERA )
    );
    std::string name = node.getName();
    if( name.empty() )
    {
    ...
    if( lib_cameras == NULL )
    {
        lib_cameras = daeSafeCast<domLibrary_cameras>(
            dom->add( COLLADA_ELEMENT_LIBRARY_CAMERAS )
        );
    }
    domCamera* cam =
        daeSafeCast<domCamera>( lib_cameras->add( COLLADA_ELEMENT_CAMERA ) );
...
void
daeWriter::apply( osg::CameraView& node )
{
    debugPrint( node );
    updateCurrentDaeNode();
    ...
    domCamera::domOptics* optics =
        daeSafeCast<domCamera::domOptics>( cam->add( COLLADA_ELEMENT_OPTICS ) );
    domCamera::domOptics::domTechnique_common* techniqueCommon =
        daeSafeCast<domCamera::domOptics::domTechnique_common>(
            optics->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
        );
    domCamera::domOptics::domTechnique_common::domPerspective* pDomPerspective =
        daeSafeCast<domCamera::domOptics::domTechnique_common::domPerspective>(
            techniqueCommon->add( COLLADA_ELEMENT_PERSPECTIVE )
        );
    ...
    switch( node.getFieldOfViewMode() )
    {
        case( osg::CameraView::UNCONSTRAINED ) :
            pXfov = daeSafeCast<domTargetableFloat>(
                pDomPerspective->add( COLLADA_ELEMENT_XFOV )
            );
    ...
    domTargetableFloat* pAspectRatio = daeSafeCast<domTargetableFloat>(
        pDomPerspective->add( COLLADA_ELEMENT_ASPECT_RATIO )
    );
    pAspectRatio->setValue( 1.0 );

    domTargetableFloat* pNear =
        daeSafeCast<domTargetableFloat>( pDomPerspective->add( COLLADA_ELEMENT_ZNEAR ) );
    pNear->setValue( 1 );

    domTargetableFloat* pFar =
        daeSafeCast<domTargetableFloat>( pDomPerspective->add( COLLADA_ELEMENT_ZFAR ) );
    pFar->setValue( 1'000 );
}
