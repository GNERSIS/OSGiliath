...
void
daeWriter::processMaterial( osg::StateSet*     ss,
                            domBind_material*  pDomBindMaterial,
                            const std::string& geoName )
{
    osg::ref_ptr<osg::StateSet> ssClean = CleanStateSet(
        ss
    );    // Need to hold a ref to this or the materialMap.find() will delete it
    domBind_material::domTechnique_common* tc =
        daeSafeCast<domBind_material::domTechnique_common>(
            pDomBindMaterial->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
        );
    domInstance_material* pDomInstanceMaterial = daeSafeCast<domInstance_material>(
        tc->add( COLLADA_ELEMENT_INSTANCE_MATERIAL )
    );
    const std::string symbol(
        _pluginOptions.namesUseCodepage
    ...
    if( lib_mats == NULL )
    {
        lib_mats = daeSafeCast<domLibrary_materials>(
            dom->add( COLLADA_ELEMENT_LIBRARY_MATERIALS )
        );
    }

    domMaterial* mat =
        daeSafeCast<domMaterial>( lib_mats->add( COLLADA_ELEMENT_MATERIAL ) );
    ...
    domInstance_effect* ie =
        daeSafeCast<domInstance_effect>( mat->add( COLLADA_ELEMENT_INSTANCE_EFFECT ) );

    if( lib_effects == NULL )
    {
        lib_effects = daeSafeCast<domLibrary_effects>(
            dom->add( COLLADA_ELEMENT_LIBRARY_EFFECTS )
        );
    }
    domEffect* effect =
        daeSafeCast<domEffect>( lib_effects->add( COLLADA_ELEMENT_EFFECT ) );
    ...
    domProfile_COMMON* pc =
        daeSafeCast<domProfile_COMMON>( effect->add( COLLADA_ELEMENT_PROFILE_COMMON ) );
    domProfile_COMMON::domTechnique* pc_teq =
        daeSafeCast<domProfile_COMMON::domTechnique>(
            pc->add( COLLADA_ELEMENT_TECHNIQUE )
        );
    pc_teq->setSid( "t0" );
    domProfile_COMMON::domTechnique::domPhong* phong =
        daeSafeCast<domProfile_COMMON::domTechnique::domPhong>(
            pc_teq->add( COLLADA_ELEMENT_PHONG )
        );

    osg::Texture* tex = static_cast<osg::Texture*>(
        ssClean->getTextureAttribute( 0, osg::StateAttribute::Type::TEXTURE )
    );
    if( ssClean->getTextureAttribute( 1, osg::StateAttribute::Type::TEXTURE ) != NULL )
    {
        tex = static_cast<osg::Texture*>(
            ssClean->getTextureAttribute( 1, osg::StateAttribute::Type::TEXTURE )
        );
    }
    if( tex != NULL && tex->getImage( 0 ) != NULL )
    {
        ...
        domImage::domInit_from* imgif =
            daeSafeCast<domImage::domInit_from>( img->add( COLLADA_ELEMENT_INIT_FROM ) );
        ...
        {
            // We do not link to original images but to the ones in memory. Then must
            // ensure to write the images.
        ...
        if( !_pluginOptions.earthTex )
        {
            domCommon_newparam_type* np = daeSafeCast<domCommon_newparam_type>(
                pc->add( COLLADA_ELEMENT_NEWPARAM )
            );
            std::string surfName = efName + "-surface";
            np->setSid( surfName.c_str() );
            domFx_surface_common* surface =
                daeSafeCast<domFx_surface_common>( np->add( COLLADA_ELEMENT_SURFACE ) );
            domFx_surface_init_from_common* sif =
                daeSafeCast<domFx_surface_init_from_common>(
                    surface->add( COLLADA_ELEMENT_INIT_FROM )
                );
            sif->setValue( iName.c_str() );
            surface->setType( FX_SURFACE_TYPE_ENUM_2D );

            np = daeSafeCast<domCommon_newparam_type>(
                pc->add( COLLADA_ELEMENT_NEWPARAM )
            );
            ...
            domFx_sampler2D_common* sampler = daeSafeCast<domFx_sampler2D_common>(
                np->add( COLLADA_ELEMENT_SAMPLER2D )
            );
            domFx_sampler2D_common_complexType::domSource* source =
                daeSafeCast<domFx_sampler2D_common_complexType::domSource>(
                    sampler->add( COLLADA_ELEMENT_SOURCE )
                );
            source->setValue( surfName.c_str() );

            // set sampler state
            domFx_sampler2D_common_complexType::domWrap_s* wrap_s =
                daeSafeCast<domFx_sampler2D_common_complexType::domWrap_s>(
                    sampler->add( COLLADA_ELEMENT_WRAP_S )
                );
            ...
            domFx_sampler2D_common_complexType::domWrap_t* wrap_t =
                daeSafeCast<domFx_sampler2D_common_complexType::domWrap_t>(
                    sampler->add( COLLADA_ELEMENT_WRAP_T )
                );
            ...
            domFx_sampler2D_common_complexType::domBorder_color* dbcol =
                daeSafeCast<domFx_sampler2D_common_complexType::domBorder_color>(
                    sampler->add( COLLADA_ELEMENT_BORDER_COLOR )
                );
            ...
            domFx_sampler2D_common_complexType::domMinfilter* minfilter =
                daeSafeCast<domFx_sampler2D_common_complexType::domMinfilter>(
                    sampler->add( COLLADA_ELEMENT_MINFILTER )
                );
            ...
            domFx_sampler2D_common_complexType::domMagfilter* magfilter =
                daeSafeCast<domFx_sampler2D_common_complexType::domMagfilter>(
                    sampler->add( COLLADA_ELEMENT_MAGFILTER )
                );
            ...
            domCommon_color_or_texture_type* cot =
                daeSafeCast<domCommon_color_or_texture_type>(
                    phong->add( COLLADA_ELEMENT_DIFFUSE )
                );
            domCommon_color_or_texture_type_complexType::domTexture* dtex =
                daeSafeCast<domCommon_color_or_texture_type_complexType::domTexture>(
                    cot->add( COLLADA_ELEMENT_TEXTURE )
                );
        ...
        domInstance_material::domBind_vertex_input* bvi =
            daeSafeCast<domInstance_material::domBind_vertex_input>(
                pDomInstanceMaterial->add( COLLADA_ELEMENT_BIND_VERTEX_INPUT )
            );
        bvi->setSemantic( "texcoord0" );
        bvi->setInput_semantic( COMMON_PROFILE_INPUT_TEXCOORD );
        bvi->setInput_set( 0 );
    }
    osg::Material* osgmat = static_cast<osg::Material*>(
        ssClean->getAttribute( osg::StateAttribute::Type::MATERIAL )
    );
    if( osgmat != NULL )
    {
        const osg::vec4& eCol = osgmat->getEmissionFrontAndBack()
                                  ? osgmat->getEmission( osg::Material::FRONT_AND_BACK )
                                  : osgmat->getEmission( osg::Material::FRONT );
        const osg::vec4& aCol = osgmat->getAmbientFrontAndBack()
                                  ? osgmat->getAmbient( osg::Material::FRONT_AND_BACK )
                                  : osgmat->getAmbient( osg::Material::FRONT );
        const osg::vec4& dCol = osgmat->getDiffuseFrontAndBack()
                                  ? osgmat->getDiffuse( osg::Material::FRONT_AND_BACK )
                                  : osgmat->getDiffuse( osg::Material::FRONT );
        const osg::vec4& sCol = osgmat->getSpecularFrontAndBack()
                                  ? osgmat->getSpecular( osg::Material::FRONT_AND_BACK )
                                  : osgmat->getSpecular( osg::Material::FRONT );
        float shininess       = osgmat->getShininessFrontAndBack()
                                  ? osgmat->getShininess( osg::Material::FRONT_AND_BACK )
                                  : osgmat->getShininess( osg::Material::FRONT );

        domCommon_color_or_texture_type* cot =
            daeSafeCast<domCommon_color_or_texture_type>(
                phong->add( COLLADA_ELEMENT_EMISSION )
            );
        domCommon_color_or_texture_type_complexType::domColor* col =
            daeSafeCast<domCommon_color_or_texture_type_complexType::domColor>(
                cot->add( COLLADA_ELEMENT_COLOR )
            );
        ...
        cot = daeSafeCast<domCommon_color_or_texture_type>(
            phong->add( COLLADA_ELEMENT_AMBIENT )
        );
        ...
        else
        {
            ...
            if( _pluginOptions.writeExtras )
            {
                ...
                domCommon_color_or_texture_type_complexType::domTexture* dtex =
                    cot->getTexture();
                domExtra* extra =
                    daeSafeCast<domExtra>( dtex->add( COLLADA_ELEMENT_EXTRA ) );
                extra->setType( "color" );
                domTechnique* teq =
                    daeSafeCast<domTechnique>( extra->add( COLLADA_ELEMENT_TECHNIQUE ) );
                teq->setProfile( "SCEI" );
                domAny* any =
                    ( domAny* )( daeElement* )teq->add( COLLADA_ELEMENT_COLOR );
        ...
        cot = daeSafeCast<domCommon_color_or_texture_type>(
            phong->add( COLLADA_ELEMENT_SPECULAR )
        );
        col = daeSafeCast<domCommon_color_or_texture_type_complexType::domColor>(
            cot->add( COLLADA_ELEMENT_COLOR )
        );
        ...
        domCommon_float_or_param_type* fop = daeSafeCast<domCommon_float_or_param_type>(
            phong->add( COLLADA_ELEMENT_SHININESS )
        );
        domCommon_float_or_param_type_complexType::domFloat* f =
            daeSafeCast<domCommon_float_or_param_type_complexType::domFloat>(
                fop->add( COLLADA_ELEMENT_FLOAT )
            );
        f->setValue( shininess );
    }

    // Do common transparency stuff here
    // if (osg::StateSet::TRANSPARENT_BIN == ssClean->getRenderingHint()) cannot do this
    // at the moment because stateSet::merge does does not handle the hints
    if( osg::StateSet::TRANSPARENT_BIN == m_CurrentRenderingHint )
    {
        osg::BlendFunc* pBlendFunc = static_cast<osg::BlendFunc*>(
            ssClean->getAttribute( osg::StateAttribute::Type::BLENDFUNC )
        );
        osg::BlendColor* pBlendColor = static_cast<osg::BlendColor*>(
            ssClean->getAttribute( osg::StateAttribute::Type::BLENDCOLOR )
        );
        if( pBlendFunc != NULL )
        {
            if( pBlendColor != NULL )
            {
                if( ( GL_CONSTANT_ALPHA == pBlendFunc->getSource() ) &&
                    ( GL_ONE_MINUS_CONSTANT_ALPHA == pBlendFunc->getDestination() ) )
                {
                    // A_ONE opaque mode
                    domCommon_transparent_type* pTransparent =
                        daeSafeCast<domCommon_transparent_type>(
                            phong->add( COLLADA_ELEMENT_TRANSPARENT )
                        );
                    pTransparent->setOpaque( FX_OPAQUE_ENUM_A_ONE );
                    domCommon_color_or_texture_type_complexType::domColor* pColor =
                        daeSafeCast<
                            domCommon_color_or_texture_type_complexType::domColor>(
                            pTransparent->add( COLLADA_ELEMENT_COLOR )
                        );
                    domCommon_float_or_param_type* pFop =
                        daeSafeCast<domCommon_float_or_param_type>(
                            phong->add( COLLADA_ELEMENT_TRANSPARENCY )
                        );
                    domCommon_float_or_param_type_complexType::domFloat* pTransparency =
                        daeSafeCast<domCommon_float_or_param_type_complexType::domFloat>(
                            pFop->add( COLLADA_ELEMENT_FLOAT )
                        );
                    if( _pluginOptions.googleMode )
                    {
                        ...
                        pTransparency->setValue( 1.0 -
                                                 pBlendColor->getConstantColor().a );
                    }
                    else
                    {
                ...
                else if( ( GL_ONE_MINUS_CONSTANT_COLOR == pBlendFunc->getSource() ) &&
                         ( GL_CONSTANT_COLOR == pBlendFunc->getDestination() ) )
                {
                ...
                {
                    OSG_WARN << "Unsupported BlendFunction parameters in transparency "
                                "processing."
                             << std::endl;
                }
            }
            ...
            {
                domCommon_transparent_type* ctt =
                    daeSafeCast<domCommon_transparent_type>(
                        phong->add( COLLADA_ELEMENT_TRANSPARENT )
                    );
                ...
                domCommon_color_or_texture_type_complexType::domTexture* dtex =
                    daeSafeCast<domCommon_color_or_texture_type_complexType::domTexture>(
                        ctt->add( COLLADA_ELEMENT_TEXTURE )
                    );
            ...
            {
                OSG_WARN << "Transparency processing - No texture or BlendColor."
                         << std::endl;
            }
        }
        else
        {
            OSG_WARN << "Transparency processing - BlendFunction not found."
                     << std::endl;
        }
    }
    ...
    if( _pluginOptions.writeExtras )
    {
        ...
        if( osg::StateAttribute::INHERIT != ssClean->getMode( GL_CULL_FACE ) )
        {
            ...
            domTechnique* pTechnique =
                daeSafeCast<domTechnique>( pExtra->add( COLLADA_ELEMENT_TECHNIQUE ) );
...
osg::StateSet*
daeWriter::CleanStateSet( osg::StateSet* pStateSet ) const
{
    ...
    if( NULL != pStateSet->getAttribute( osg::StateAttribute::Type::BLENDFUNC ) )
    {
        pCleanedStateSet->setAttribute(
            pStateSet->getAttribute( osg::StateAttribute::Type::BLENDFUNC )
        );
    }
    if( NULL != pStateSet->getAttribute( osg::StateAttribute::Type::BLENDCOLOR ) )
    {
        pCleanedStateSet->setAttribute(
            pStateSet->getAttribute( osg::StateAttribute::Type::BLENDCOLOR )
        );
    }
    if( NULL != pStateSet->getAttribute( osg::StateAttribute::Type::MATERIAL ) )
    {
        pCleanedStateSet->setAttribute(
            pStateSet->getAttribute( osg::StateAttribute::Type::MATERIAL )
        );
    }
    // pCleanedStateSet->setRenderingHint(pStateSet->getRenderingHint()); does not work
    // at the moment due to stateSet::merge
