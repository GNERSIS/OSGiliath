...
void daeReader::processProfileCOMMON(osg::StateSet *ss, domProfile_COMMON *pc )
{
    ...
    if (b != NULL )
    {
        ...
        processColorOrTextureType(ss, b->getSpecular(), osg::Material::SPECULAR, mat.get(), b->getShininess() );
    ...
    else if (p != NULL )
    {
        ...
        processColorOrTextureType(ss, p->getSpecular(), osg::Material::SPECULAR, mat.get(), p->getShininess() );
...
osg::Image* daeReader::processImageTransparency(const osg::Image* srcImg, domFx_opaque_enum opaque, float transparency) const
{
    ...
    transparentImage->setImage(s, t, 1, GL_RED, GL_RED, GL_UNSIGNED_BYTE, pixels, osg::Image::USE_NEW_DELETE);
