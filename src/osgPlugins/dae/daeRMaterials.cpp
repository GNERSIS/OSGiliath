...
#include "daeReader.hpp"
#include "ReaderWriterDAE.hpp"
...
template<typename T>
void
daeReader::getTransparencyCounts( daeDatabase* database,
                                  ...
                                  int&         opaqueCount ) const
{
    ...
    {
        if( const domCommon_transparent_type* pTransparent =
                constantVec[i]->getTransparent() )
        {
        ...
        if( const domCommon_float_or_param_type* pTransparency =
                constantVec[i]->getTransparency() )
        {
            float    transparency;
            ...
            if( pTransparency->getFloat() )
            {
                transparency = pTransparency->getFloat()->getValue();
                     ...
                     GetFloatParam( pTransparency->getParam()->getRef(),
                                    transparencyParam ) )
            {
...
bool
daeReader::findInvertTransparency( daeDatabase* database ) const
{
    int transparentCount = 0, opaqueCount = 0;
    getTransparencyCounts<domProfile_COMMON::domTechnique::domConstant>(
        database,
        transparentCount,
        opaqueCount
    );
...
//        type
void
daeReader::processBindMaterial( domBind_material* bm,
                                domGeometry*      geom,
                                osg::Geode*       geode,
                                osg::Geode*       cachedGeode )
{
    ...
    for( size_t i = 0; i < geode->getNumDrawables(); i++ )
    {
        ...
        osg::Geometry* cachedGeometry =
            dynamic_cast<osg::Geometry*>( cachedGeode->getDrawable( i )->asGeometry() );
        ...
        for( size_t j = 0; j < ima.getCount(); j++ )
        {
            ...
            if( symbol.compare( materialName ) == 0 )
            {
                found = true;
                domMaterial* mat =
                    daeSafeCast<domMaterial>( getElementFromURI( ima[j]->getTarget() ) );
                if( mat )
                {
                    ...
                    // 1. Clear the texcoord arrays and associated texcoord vertex
                    // indices from the current (cloned) drawable.
                    osg::Geometry* clonedGeometry = drawable->asGeometry();
                    if( NULL == clonedGeometry )
                    {
                        OSG_WARN << "Failed to convert drawable to geometry object"
                                 << std::endl;
                        break;
                    }
                    clonedGeometry->getTexCoordArrayList().clear();

                    // 2. For each possible texture unit find the correct texcoord array
                    // and indices from the cached drawable and place in the cloned
                    // drawable in the correct texture unit slot
                    unsigned int textureUnit( 0 );
                    if( copyTextureCoordinateSet( ss,
                                                  cachedGeometry,
                                                  clonedGeometry,
                                                  ima[j],
                ...
                {
                    OSG_WARN << "Failed to locate <material> with id "
                             << ima[i]->getTarget().getURI() << std::endl;
                }
        ...
        if( !found )
        {
            OSG_WARN << "Failed to locate <instance_material> with symbol "
...
// 0..* <extra>
void
daeReader::processMaterial( osg::StateSet* ss,
                            domMaterial*   mat )
{
    if( !mat )
    {
    ...
    domEffect* effect =
        daeSafeCast<domEffect>( getElementFromURI( _currentInstance_effect->getUrl() ) );
    if( effect )
    {
        processEffect( ss, effect );

        // TODO: process all of the setParams that could happen here in the material.
        // ESP. the textures
    }
    else
    {
        OSG_WARN << "Failed to locate effect "
...
// 0..* <extra>
void
daeReader::processEffect( osg::StateSet* ss,
                          domEffect*     effect )
{
    ...
    {
        domProfile_COMMON* pc =
            daeSafeCast<domProfile_COMMON>( effect->getFx_profile_abstract_array()[i] );
        if( pc != NULL )
        {
            if( hasCOMMON )
            {
                OSG_WARN << "Effect already has a profile_COMMON. Skipping this one"
                         << std::endl;
                continue;
            }
        ...
        OSG_WARN << "unsupported effect profile "
                 << effect->getFx_profile_abstract_array()[i]->getTypeName()
                 << std::endl;
    }
}
...
// 0..* <extra>
void
daeReader::processProfileCOMMON( osg::StateSet*     ss,
                                 domProfile_COMMON* pc )
{
    ...
    for( CurrentExtra = 0; CurrentExtra < NumberOfExtras; CurrentExtra++ )
    {
             ...
             CurrentTechnique++ )
        {
            ...
            if( TechniqueRef->getProfile() &&
                strcmp( TechniqueRef->getProfile(), "GOOGLEEARTH" ) == 0 )
            {
    ...
    if( b != NULL )
    {
        ...
        processColorOrTextureType( ss,
                                   b->getEmission(),
                                   osg::Material::EMISSION,
                                   mat.get(),
                                   NULL,
                                   &EmissionStateAttribute );
        if( NULL != EmissionStateAttribute )
        {
            OSG_WARN << "Currently no support for <texture> in Emission channel "
                     << std::endl;
        }
        ...
        if( DiffuseStateAttribute != NULL )
        {
            ...
            {
                // Set the ambient and diffuse colour white so that the incoming fragment
                // colour ends up as a lit white colour. I modulate both textures onto
                // this to approximate the lighting equation. Using a zero diffuse and
                // then an ADD of the diffuse texture seems overlit to me.
                ...
                unsigned int textureUnit( _pluginOptions.usePredefinedTextureUnits
                                              ? AMBIENT_OCCLUSION_UNIT
                                              : 0 );
                ss->setTextureAttribute( textureUnit, AmbientStateAttribute );
                // Modulate in the diffuse texture
                textureUnit =
                    _pluginOptions.usePredefinedTextureUnits ? MAIN_TEXTURE_UNIT : 1;
                ss->setTextureAttribute( textureUnit, DiffuseStateAttribute );
            }
            else
            {
                // Set the diffuse colour white so that the incoming fragment colour ends
                // up as the global diffuse lighting colour plus any constant ambient
                // contribution after the lighting calculation. This means that I am
                // modulating the the ambient with the texture as well but I cannot see a
        ...
        else
        {
            ...
            {
                OSG_WARN << "Ambient occlusion map only supported when diffuse texture "
                            "also specified"
                         << std::endl;
            }
        }
        ...
        processTransparencySettings( b->getTransparent(),
                                     b->getTransparency(),
                                     ss,
                                     mat.get(),
                                     _pluginOptions.usePredefinedTextureUnits
                                         ? MAIN_TEXTURE_UNIT
                                         : 0 );
    }
    // <phong>
    ...
    else if( p != NULL )
    {
        ...
        processTransparencySettings( p->getTransparent(),
                                     p->getTransparency(),
                                     ss,
                                     mat.get(),
                                     _pluginOptions.usePredefinedTextureUnits
    ...
    else if( l != NULL )
    {
        ...
        processTransparencySettings( l->getTransparent(),
                                     l->getTransparency(),
                                     ss,
                                     mat.get(),
                                     _pluginOptions.usePredefinedTextureUnits
    ...
    else if( c != NULL )
    {
        ...
        mat->setDiffuse( osg::Material::FRONT_AND_BACK,
                         mat->getEmission( osg::Material::FRONT_AND_BACK ) );

        processTransparencySettings( c->getTransparent(),
                                     c->getTransparency(),
                                     ss,
                                     mat.get(),
                                     _pluginOptions.usePredefinedTextureUnits
...
//        0..*    extra
bool
daeReader::processColorOrTextureType( const osg::StateSet*             ss,
                                      ...
                                      bool                             blinn )
{
    ...
    if( channel == osg::Material::EMISSION )
    {
        if( cot->getColor() != NULL )
        {
            ...
            mat->setEmission(
                ...
                osg::vec4( f4[0], f4[1], f4[2], ( f4.getCount() == 4 ) ? f4[3] : 1.0 )
            );
            retVal = true;
        }
        ...
        {
            OSG_WARN << "Missing <color>, <param> or <texture> in Emission channel "
                     << std::endl;
        }
    }
    else if( channel == osg::Material::AMBIENT )
    {
        ...
        else if( cot->getTexture() != NULL )
        {
            ...
            {
                OSG_WARN << "Currently no support for <texture> in Ambient channel "
                         << std::endl;
                mat->setAmbient( osg::Material::FRONT_AND_BACK,
        ...
        {
            OSG_WARN << "Missing <color>, <param> or <texture> in Ambient channel "
                     << std::endl;
        }
    }
    else if( channel == osg::Material::DIFFUSE )
    {
        ...
        else if( cot->getTexture() != NULL )
        {
            ...
            {
                OSG_WARN << "Currently no support for <texture> in Diffuse channel "
                         << std::endl;
                mat->setDiffuse( osg::Material::FRONT_AND_BACK,
                ...
                strcmp( extra->getType(), "color" ) == 0 )
            {
                ...
                for( unsigned int i = 0; i < extra->getTechnique_array().getCount();
                     i++ )
                {
                    ...
                    if( strcmp( teq->getProfile(), "SCEI" ) == 0 )
                    {
                        ...
                        std::istringstream diffuse_colour(
                            ( const char* )dcol->getValue()
                        );
        ...
        {
            OSG_WARN << "Missing <color>, <param> or <texture> in Diffuse channel "
                     << std::endl;
        }
    }
    else if( channel == osg::Material::SPECULAR )
    {
        ...
        {
            OSG_WARN << "Currently no support for <texture> in Specular channel "
                     << std::endl;
        }
        else
        {
            OSG_WARN << "Missing <color>, <param> or <texture> in Specular channel "
                     << std::endl;
        }
...
bool
daeReader::GetFloat4Param( xsNCName   Reference,
                           domFloat4& f4 ) const
{
    ...
    if( NULL != _currentInstance_effect )
    {
        ...
        const domInstance_effect::domSetparam_Array& SetParamArray =
            _currentInstance_effect->getSetparam_array();
        ...
        for( size_t i = 0; i < NumberOfSetParams; i++ )
        {
            ...
            {
                if( NULL !=
                    SetParamArray[i]->getFx_basic_type_common() &&
                    ( NULL !=
                      SetParamArray[i]->getFx_basic_type_common()->getFloat4() ) )
                {
                    f4 = SetParamArray[i]
                             ->getFx_basic_type_common()
                             ->getFloat4()
                             ->getValue();
                    return true;
                }
    ...
    if( ( cnp != NULL ) && ( NULL != cnp->getFloat4() ) )
    {
        ...
        return true;
             ...
             ( NULL != npc->getFx_basic_type_common() ) &&
             ( NULL != npc->getFx_basic_type_common()->getFloat4() ) )
    {
...
bool
daeReader::GetFloatParam( xsNCName  Reference,
                          domFloat& f ) const
{
    std::string MyReference = Reference;
    ...
    if( ( cnp != NULL ) && ( NULL != cnp->getFloat() ) )
    {
        ...
        return true;
             ...
             ( NULL != npc->getFx_basic_type_common() ) &&
             ( NULL != npc->getFx_basic_type_common()->getFloat() ) )
    {
...
getWrapMode( domFx_sampler_wrap_common domWrap )
{
    switch( domWrap )
    {
...
getFilterMode( domFx_sampler_filter_common domFilter,
               bool                        allowMipMap )
{
    switch( domFilter )
    {
        ...
        case FX_SAMPLER_FILTER_COMMON_NEAREST_MIPMAP_NEAREST :
            return allowMipMap ? osg::Texture::NEAREST_MIPMAP_NEAREST
                               : osg::Texture::NEAREST;
        case FX_SAMPLER_FILTER_COMMON_LINEAR_MIPMAP_NEAREST :
            return allowMipMap ? osg::Texture::LINEAR_MIPMAP_NEAREST
                               : osg::Texture::LINEAR;
        case FX_SAMPLER_FILTER_COMMON_NEAREST_MIPMAP_LINEAR :
            return allowMipMap ? osg::Texture::NEAREST_MIPMAP_LINEAR
                               : osg::Texture::NEAREST;
        case FX_SAMPLER_FILTER_COMMON_LINEAR_MIPMAP_LINEAR :
            return allowMipMap ? osg::Texture::LINEAR_MIPMAP_LINEAR
                               : osg::Texture::LINEAR;
        default :
...
std::string
daeReader::processImagePath( const domImage* pDomImage ) const
{
    if( pDomImage == NULL )
    {
    ...
    // Got a sampler and a surface and an imaged. Time to create the texture stuff for
    // osg
    if( pDomImage->getInit_from() )
    {
        ...
        {
            OSG_WARN << "Only images with a \"file\" scheme URI are supported in this "
                        "version."
                     << std::endl;
        }
    }
...
float
luminance( const osg::vec4& color )
{
    return color.r * 0.212671F + color.g * 0.715160F + color.b * 0.072169F;
}

osg::Image*
daeReader::processImageTransparency( const osg::Image* srcImg,
        ...
        if( texHasAlpha )
        {
            for( int i = 0; i < t; ++i )
            {
                for( int j = 0; j < s; ++j )
                {
                    ...
                    pixels[i * s + j] =
                        static_cast<unsigned char>( color.a * transparency * 255.0F );
                }
            }
        }
        else
...
osg::Texture2D*
daeReader::processTexture( domCommon_color_or_texture_type_complexType::domTexture* tex,
                           ...
                           float             transparency )
{
    ...
    if( el == NULL )
    {
        OSG_WARN << "Could not locate newparam for texture sampler2D \""
                 << tex->getTexture()
                 << "\". Checking if data does incorrect linking straight to the image"
                 << std::endl;
        _dae->getDatabase()->getElement( ( daeElement** )&dImg,
                                         0,
                                         tex->getTexture(),
                                         "image" );
        if( dImg != NULL )
        {
            OSG_WARN << "Direct image link found. Data is incorrect but will continue "
                        "to load texture"
                     << std::endl;
        }
    }
    else
    {
        ...
        if( el == NULL )
        {
            OSG_WARN << "Could not locate newparam for source "
    ...
    if( sampler )
    {
        ...
        if( sampler->getMinfilter() &&
            sampler->getMinfilter()->getValue() != FX_SAMPLER_FILTER_COMMON_NONE )
        {
            parameters.filter_min =
                getFilterMode( sampler->getMinfilter()->getValue(), true );
        }
        if( sampler->getMagfilter() &&
            sampler->getMagfilter()->getValue() != FX_SAMPLER_FILTER_COMMON_NONE )
        {
            parameters.filter_mag =
                getFilterMode( sampler->getMagfilter()->getValue(), false );
        }
    ...
    else
    {
        osg::ref_ptr<osg::Image> img =
            osgDB::readRefImageFile( parameters.filename, _pluginOptions.options.get() );
        ...
        OSG_INFO << "  processTexture(..) - readImage(" << parameters.filename << ")"
                 << std::endl;
...
In <blinn>, <constant>, <lambert>, and <phong>, the child element <transparent> now has
an optional opaque attribute whose valid values are: A_ONE (the default): Takes the
transparency information from the colors alpha channel, where the value 1.0 is opaque.
 RGB_ZERO: Takes the transparency information from the colors red, green, and blue
channels, where the value 0.0 is opaque, with each channel modulated independently.

 In the Specification, this is described in the FX Reference chapter in the
common_color_or_texture_type entry, along with a description of how transparency works in
the Getting Started with COLLADA FX chapter in the Determining Transparency section.


Collada Digital Asset Schema Release 1.5.0 Release Notes

The <transparent> elements opaque attribute now allows, in addition to A_ONE and
RGB_ZERO, the following values: A_ZERO: Takes the transparency information from the
colors alpha channel, where the value 0.0 is opaque. RGB_ONE: Takes the transparency
information from the colors red, green, and blue channels, where the value 1.0 is opaque,
with each channel modulated independently.

When we update to a version of the dom using that schema we will need to modify the code
below
*/

void
daeReader::processTransparencySettings( domCommon_transparent_type*    ctt,
                                        ...
                                        unsigned int diffuseTextureUnit )
{
    ...
    if( ctt )
    {
        ...
        if( ctt->getColor() )
        {
            ...
            transparentColor.set( domColorValue.get( 0 ),
                                  domColorValue.get( 1 ),
                                  domColorValue.get( 2 ),
                                  domColorValue.get( 3 ) );
        ...
        {
            pTransparentTexture = processTexture( ctt->getTexture(),
                                                  ss,
                                                  TRANSPARENCY_MAP_UNIT,
                                                  opaque,
                                                  transparency );
    ...
    else
    {
        ...
        if( !strictTransparency )
        {
            const osg::Texture* pMainTexture = dynamic_cast<osg::Texture*>(
                ss->getTextureAttribute( diffuseTextureUnit,
                                         osg::StateAttribute::Type::TEXTURE )
            );
            bool haveTranslucentTexture =
                pMainTexture &&
                pMainTexture->getImage( 0 ) &&
                pMainTexture->getImage( 0 )->isImageTranslucent();
        ...
        if( strictTransparency )
        {
            ...
            ss->setAttributeAndModes(
                new osg::BlendFunc( GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR )
            );
        }
        else
        {
...
bool
daeReader::copyTextureCoordinateSet( const osg::StateSet*        ss,
                                     const osg::Geometry*        cachedGeometry,
                                     osg::Geometry*              clonedGeometry,
                                     const domInstance_material* im,
                                     TextureUnitUsage            tuu,
                                     unsigned int                textureUnit )
{
    unsigned int localTextureUnit( _pluginOptions.usePredefinedTextureUnits
                                       ? tuu
                                       : textureUnit );
    if( !ss->getTextureAttribute( localTextureUnit,
                                  osg::StateAttribute::Type::TEXTURE ) )
    {
        return false;
    }

    const std::string& texCoordSetName =
        _texCoordSetMap[TextureToCoordSetMap::key_type( ss, tuu )];
    if( texCoordSetName.empty() )
    {
        return false;
    }

    const domInstance_material::domBind_vertex_input_Array& bvia =
        im->getBind_vertex_input_array();
    size_t k;
    for( k = 0; k < bvia.getCount(); k++ )
    {
        if( !strcmp( bvia[k]->getSemantic(), texCoordSetName.c_str() ) &&
            !strcmp( bvia[k]->getInput_semantic(), COMMON_PROFILE_INPUT_TEXCOORD ) )
        {
            unsigned int set = bvia[k]->getInput_set();
            if( set < cachedGeometry->getNumTexCoordArrays() )
            {
                clonedGeometry->setTexCoordArray(
                    localTextureUnit,
                    const_cast<osg::Array*>( cachedGeometry->getTexCoordArray( set ) )
                );
            }
            else
            {
                OSG_WARN << "Texture coordinate set " << set << " not found."
                         << std::endl;
            }
            break;
        }
    ...
    if( k == bvia.getCount() )
    {
        OSG_WARN << "Failed to find matching <bind_vertex_input> for " << texCoordSetName
                 << std::endl;
        ...
        for( k = 0; k < ba.getCount(); k++ )
        {
            ...
            {
                IdToCoordIndexMap::const_iterator it =
                    _texCoordIdMap.find( ba[k]->getTarget() );
                if( it !=
                    _texCoordIdMap.end() &&
                    ( cachedGeometry->getNumTexCoordArrays() > it->second ) )
                {
                    clonedGeometry->setTexCoordArray(
                        localTextureUnit,
                        const_cast<osg::Array*>(
                            cachedGeometry->getTexCoordArray( it->second )
                        )
                    );
                }
                else
                {
                    OSG_WARN << "Texture coordinate set " << ba[k]->getTarget()
                             << " not found." << std::endl;
                }
                break;
            }
        ...
        if( k == ba.getCount() )
        {
            if( cachedGeometry->getNumTexCoordArrays() )
            {
                clonedGeometry->setTexCoordArray(
                    localTextureUnit,
                    const_cast<osg::Array*>( cachedGeometry->getTexCoordArray( 0 ) )
                );
            }
        }
    }
