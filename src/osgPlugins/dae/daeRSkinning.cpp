...
domNode*
daeReader::getRootJoint( domNode* joint ) const
{
    int depth = 0;
...
domNode*
daeReader::findJointNode( daeElement*             searchFrom,
                          domInstance_controller* pDomInstanceController ) const
{
    ...
    if( domIDREF_array* pDomIDREFs = pDomJointsSource->getIDREF_array() )
    {
        ...
        {
            return daeSafeCast<domNode>(
                getElementFromIDRef( pDomIDREFs->getValue().get( 0 ) )
            );
        }
    }
...
domNode*
daeReader::findSkeletonNode( daeElement*             searchFrom,
                             domInstance_controller* pDomInstanceController ) const
{
...
daeReader::processSkins()
{
    ...
    for( size_t i = 0; i < _skinInstanceControllers.size(); ++i )
    {
        ...
        {
            domNode* skelNode =
                findSkeletonNode( _skeletonMap.begin()->first, pDomInstanceController );
            if( skelNode )
            {
        ...
        {
            if( daeElement* pDaeElement =
                    pDomSkeletons.get( 0 )->getValue().getElement() )
            {
                if( domNode* skelNode =
                        findSkeletonNode( pDaeElement, pDomInstanceController ) )
                {
                    skelSkinMap[skelNode].push_back( pDomInstanceController );
                }
            }
...
void
getJointsAndInverseObjectspaceBindMatrices(
    domInstance_controller*             pDomInstanceController,
    domNode*                            pDomSkeletonNode,
    std::vector<std::pair<domNode*,
                          osg::dmat4>>& jointsAndBindMatrices
)
{
    domController* pDomController = daeSafeCast<domController>(
        getElementFromURI( pDomInstanceController->getUrl() )
    ...
    {
        OSG_WARN << "Only a single pair of skin joints inputs is supported."
                 << std::endl;
    }
        ...
        if( !strcmp( domInputs[i]->getSemantic(), COMMON_PROFILE_INPUT_JOINT ) )
        {
            ...
            if( !pDomJointsSource )
            {
                ...
                return;
                          ...
                          COMMON_PROFILE_INPUT_INV_BIND_MATRIX ) )
        {
            pDomInvBindMatricesSource =
                daeSafeCast<domSource>( getElementFromURI( domInputs[i]->getSource() ) );
            if( !pDomInvBindMatricesSource )
            {
                OSG_WARN << "Could not find skin inverse bind matrices source '"
                         << domInputs[i]->getSource().getURI() << "'" << std::endl;
                return;
            }
        }
    ...
    osg::dmat4      parentInverseSkeletonBindMatrix;

    if( domIDREF_array* pDomIDREFs = pDomJointsSource->getIDREF_array() )
    {
        // IDREFS refer to an absolute joint and therefore do not allow a different
        // skeleton
        xsIDREFS* pIDREFS = &( pDomIDREFs->getValue() );
        for( size_t i = 0; i < pIDREFS->getCount(); i++ )
        {
            domNode* pDomNode =
                daeSafeCast<domNode>( getElementFromIDRef( pIDREFS->get( i ) ) );

            if( pDomNode )
            {
                jointsAndBindMatrices.push_back(
                    std::pair<domNode*, osg::dmat4>( pDomNode, osg::dmat4() )
                );
            }
            else
            {
                OSG_WARN << "Failed to locate joint '" << pIDREFS->get( i ).getID()
                         << "'" << std::endl;
            }
        }
    }
    else if( domName_array* pDomNames = pDomJointsSource->getName_array() )
    {
        ...
        for( size_t i = 0; i < pNames->getCount(); i++ )
        {
            ...
            {
                OSG_WARN << "Failed to locate joint '" << pNames->get( i ) << "'"
                         << std::endl;
            }
        }
    }
    ...
    {
        osg::dmat4 invMat( matrices.get( i * 16 + 0 ),
                           matrices.get( i * 16 + 4 ),
                           matrices.get( i * 16 + 8 ),
                           matrices.get( i * 16 + 12 ),
                           matrices.get( i * 16 + 1 ),
                           matrices.get( i * 16 + 5 ),
                           matrices.get( i * 16 + 9 ),
                           matrices.get( i * 16 + 13 ),
                           matrices.get( i * 16 + 2 ),
                           matrices.get( i * 16 + 6 ),
                           matrices.get( i * 16 + 10 ),
                           matrices.get( i * 16 + 14 ),
                           matrices.get( i * 16 + 3 ),
                           matrices.get( i * 16 + 7 ),
                           matrices.get( i * 16 + 11 ),
                           matrices.get( i * 16 + 15 ) );
        jointsAndBindMatrices[i].second = invMat;
    }
}

void
daeReader::processSkeletonSkins( domNode*                          skeletonRoot,
                                 const domInstance_controllerList& instanceControllers )
{
    for( size_t i = 0; i < instanceControllers.size(); ++i )
    {
        domInstance_controller* instanceController = instanceControllers[i];

        std::vector<std::pair<domNode*, osg::dmat4>> jointsAndInverseBindMatrices;
        getJointsAndInverseObjectspaceBindMatrices( instanceController,
                                                    skeletonRoot,
                                                    jointsAndInverseBindMatrices );

        for( size_t j = 0; j < jointsAndInverseBindMatrices.size(); ++j )
        {
            osgAnimation::Bone* pOsgBone =
                getOrCreateBone( jointsAndInverseBindMatrices[j].first );
            pOsgBone->setInvBindMatrixInSkeletonSpace(
                jointsAndInverseBindMatrices[j].second
            );
        }
    }
    ...
    for( size_t i = 0; i < instanceControllers.size(); ++i )
    {
        ...
        processSkin( pDomController->getSkin(),
                     skeletonRoot,
                     skeleton,
...
// 0..*    <extra>
void
daeReader::processSkin( domSkin*                pDomSkin,
                        domNode*                skeletonRoot,
                        osgAnimation::Skeleton* pOsgSkeleton,
                        domBind_material*       pDomBindMaterial )
{
    ...
    if( !pDaeSkinSource )
    {
        OSG_WARN << "Failed to locate geometry " << pDomSkin->getSource().getURI()
                 << std::endl;
        return;
    }
    ...
    if( !pDomGeometry )
    {
        OSG_WARN << "Skin source is of type " << pDaeSkinSource->getTypeName()
    ...
    osg::Geode*       pOsgGeode =
        getOrCreateGeometry( pDomGeometry, pDomBindMaterial, &pOriginalGeode );
    if( !pOsgGeode )
    {
        return;
    }
    ...
    pOsgRigGeode->setDataVariance( osg::Object::DataVariance::DYNAMIC );

    typedef std::map<const osg::Geometry*, osgAnimation::RigGeometry*>
                           GeometryRigGeometryMap;
    GeometryRigGeometryMap old2newGeometryMap;

    for( unsigned i = 0; i < pOsgGeode->getNumDrawables(); ++i )
    {
        if( osg::Geometry* pOsgGeometry =
                dynamic_cast<osg::Geometry*>( pOsgGeode->getDrawable( i ) ) )
        {
            const osg::Geometry* pOriginalGeometry =
                dynamic_cast<const osg::Geometry*>( pOriginalGeode->getDrawable( i ) );
            ...
            old2newGeometryMap.insert(
                GeometryRigGeometryMap::value_type( pOriginalGeometry, pOsgRigGeometry )
            );
            pOsgRigGeometry->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    // <bind_shape_matrix>
    if( domSkin::domBind_shape_matrix* pDomBindShapeMatrix =
            pDomSkin->getBind_shape_matrix() )
    {
        domFloat4x4 matrix = pDomBindShapeMatrix->getValue();
        osg::dmat4  bindMatrix( matrix.get( 0 ),
                                matrix.get( 4 ),
                                matrix.get( 8 ),
                                matrix.get( 12 ),
                                matrix.get( 1 ),
        ...
        for( unsigned d = 0; d < pOsgRigGeode->getNumDrawables(); ++d )
        {
            ...
            {
                OSG_NOTIFY( osg::WARN )
                    << "Vertices vector type isn't supported." << std::endl;
                continue;
            }
            ...
            if( norm )
            {
                ...
                if( norm->getType() == osg::Array::Vec3ArrayType )
                {
                    ...
                    {
                        ( *normf )[i] =
                            osg::dmat4::transform3x3( ( *normf )[i], bindMatrix );
                    }
                }
                else if( norm->getType() == osg::Array::Vec3dArrayType )
                {
                    ...
                    {
                        ( *normd )[i] =
                            osg::dmat4::transform3x3( ( *normd )[i], bindMatrix );
                    }
                }
                else
                {
                    OSG_NOTIFY( osg::WARN )
                        << "Normals vector type isn't supported." << std::endl;
                    // continue;
                }
            }
        }
    ...
    if( domInputs.getCount() > 2 )
    {
        OSG_WARN << "Only a single pair of skin vertex weights inputs is supported."
                 << std::endl;
    }
    ...
    for( size_t i = 0; i < 2; i++ )
    {
        ...
        else if( !strcmp( domInputs[i]->getSemantic(), COMMON_PROFILE_INPUT_WEIGHT ) )
        {
            ...
            {
                OSG_WARN << "Could not find skin weights source '"
                         << domInputs[i]->getSource().getURI() << "'" << std::endl;
                return;
            }
        }
    ...
    if( domName_array* pDomNames = pDomJointsSource->getName_array() )
    {
        ...
        for( size_t i = 0; i < pNames->getCount(); ++i )
        {
            ...
            osgAnimation::Bone* pOsgBone =
                _jointMap[daeSafeCast<domNode>( resolver.getElement() )].get();
            if( pOsgBone )
            {
    ...
    else if( domIDREF_array* pDomIDREFs = pDomJointsSource->getIDREF_array() )
    {
        ...
        {
            osgAnimation::Bone* pOsgBone =
                _jointMap[daeSafeCast<domNode>( pIDREFs->get( i ).getElement() )].get();
            if( pOsgBone )
            {
                jointNames.push_back( pOsgBone->getName() );
            }
            else
            {
                ...
                OSG_WARN << "Cannot find bone " << pIDREFs->get( i ).getID()
                         << std::endl;
            }
        }
    }
    ...
    for( size_t i = 0, vIndex = 0; i < influenceCounts.getCount(); i++ )
    {
        ...
        const OldToNewIndexMap::const_iterator end =
            _oldToNewIndexMap.upper_bound( indexID );
            ...
            if( weight > 0.0F )
            {
                ...
                for( OldToNewIndexMap::const_iterator it = start; it != end; ++it )
                {
                    osgAnimation::RigGeometry* pRigGeometry =
                        old2newGeometryMap[it->second.first.get()];

                    osgAnimation::VertexInfluenceMap* vim =
                        pRigGeometry->getInfluenceMap();
                    if( !vim )
                    {
                        pRigGeometry->setInfluenceMap(
                            vim = new osgAnimation::VertexInfluenceMap
                        );
                    }

                    getVertexInfluence( *vim, name )
                        .push_back( osgAnimation::VertexIndexWeight( it->second.second,
                                                                     weight ) );
                }
            }
        }
