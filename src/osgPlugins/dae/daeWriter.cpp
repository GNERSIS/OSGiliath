...
namespace osgDAE
{
    ...
    daeWriter::ArrayNIndices::ArrayNIndices( osg::Array*      vArray,
                                             osg::IndexArray* ind ) :
        vec2( 0 ),
        vec3( 0 ),
        vec4( 0 ),
    ...
    std::string
    toString( const osg::dvec2& value )
    {
        std::stringstream str;
        str << value.x << " " << value.y;
        return str.str();
    ...
    std::string
    toString( const osg::dvec3& value )
    {
        std::stringstream str;
        str << value.x << " " << value.y << " " << value.z;
        return str.str();
    ...
    std::string
    toString( const osg::dvec4& value )
    {
        std::stringstream str;
        str << value.x << " " << value.y << " " << value.z << " " << value.w;
        return str.str();
    ...
    template<typename T>
    std::string
    matrixToString( T value )
    {
        std::stringstream str;
        str << value( 0, 0 ) << " " << value( 1, 0 ) << " " << value( 2, 0 ) << " "
            << value( 3, 0 ) << " " << value( 0, 1 ) << " " << value( 1, 1 ) << " "
            << value( 2, 1 ) << " " << value( 3, 1 ) << " " << value( 0, 2 ) << " "
    ...
    std::string
    toString( const osg::mat4& value )
    {
        return matrixToString<osg::mat4>( value );
    }

    std::string
    toString( const osg::dmat4& value )
    {
        return matrixToString<osg::dmat4>( value );
    }
    ...
    daeWriter::daeWriter( DAE*                                dae_,
        ...
        _externalWriter( srcDirectory,
                         directory,
                         true,
                         pluginOptions ? pluginOptions->relativiseImagesPathNbUpDirs
                                       : 0 )
    {
        success = true;
        ...
        domCOLLADA::domScene* scene =
            daeSafeCast<domCOLLADA::domScene>( dom->add( COLLADA_ELEMENT_SCENE ) );
        domInstanceWithExtra* ivs = daeSafeCast<domInstanceWithExtra>(
        ...
        lib_vis_scenes = daeSafeCast<domLibrary_visual_scenes>(
            dom->add( COLLADA_ELEMENT_LIBRARY_VISUAL_SCENES )
        );
        vs = daeSafeCast<domVisual_scene>(
            lib_vis_scenes->add( COLLADA_ELEMENT_VISUAL_SCENE )
        );
    ...
    void
    daeWriter::debugPrint( osg::Node& node )
    {
#ifdef _DEBUG
        std::string indent = "";
    ...
    void
    daeWriter::setRootNode( const osg::Node& node )
    {
        std::string fname = osgDB::findDataFile( node.getName() );
        // create Asset with root node providing meta data
        createAssetTag( node );

        node.accept( static_cast<osg::ConstNodeVisitor&>( _animatedNodeCollector ) );
    }

    // ### provide a name to node
    std::string
    daeWriter::getNodeName( const osg::Node&   node,
    ...
    void
    daeWriter::updateCurrentDaeNode()
    {
    ...
    std::string
    daeWriter::uniquify( const std::string& uname )
    {
        const std::string baseName(
            _pluginOptions.namesUseCodepage
                ? osgDB::convertStringFromCurrentCodePageToUTF8( uname )
                : uname
        );
        ...
        if( _pluginOptions.renameIds )
        {
            ...
            // Note: ConvertFilePathToColladaCompatibleURI(newName) did not the job:
            // names such as "C:\somedir\somefile" become "/C:/somedir/somefile" and are
            // not interpreted as a simple ID within Google Earth.
            static const char* const REPLACE_CHARS = " /\\:#?=%&*";
            for( std::size_t found = newName.find_first_of( REPLACE_CHARS );
                 found != std::string::npos;
                 found = newName.find_first_of( REPLACE_CHARS ) )
            {
                newName[found] = '_';
            }
        }
        ...
        for( iter = uniqueNames.find( newName ); iter != uniqueNames.end();
             iter = uniqueNames.find( newName ) )
        {
            if( iter->second == std::numeric_limits<UniqueNames::mapped_type>::max() )
            {
                throw std::runtime_error(
                    "Not implemented: renaming DAE name above given limit."
                );
            }
    ...
    void
    daeWriter::createAssetTag( bool isZUpAxis )
    {
        domAsset* asset = daeSafeCast<domAsset>( dom->add( COLLADA_ELEMENT_ASSET ) );
        domAsset::domCreated* c =
            daeSafeCast<domAsset::domCreated>( asset->add( COLLADA_ELEMENT_CREATED ) );
        domAsset::domModified* m =
            daeSafeCast<domAsset::domModified>( asset->add( COLLADA_ELEMENT_MODIFIED ) );
        domAsset::domUnit* u =
            daeSafeCast<domAsset::domUnit>( asset->add( COLLADA_ELEMENT_UNIT ) );
        domAsset::domUp_axis* up =
            daeSafeCast<domAsset::domUp_axis>( asset->add( COLLADA_ELEMENT_UP_AXIS ) );
    ...
    void
    daeWriter::createAssetTag( const osg::Node& node )
    {
        domAsset* asset = daeSafeCast<domAsset>( dom->add( COLLADA_ELEMENT_ASSET ) );
        domAsset::domCreated* c =
            daeSafeCast<domAsset::domCreated>( asset->add( "created" ) );
        domAsset::domModified* m =
            daeSafeCast<domAsset::domModified>( asset->add( "modified" ) );
        domAsset::domUnit*    u = daeSafeCast<domAsset::domUnit>( asset->add( "unit" ) );
        domAsset::domUp_axis* up_axis =
            daeSafeCast<domAsset::domUp_axis>( asset->add( "up_axis" ) );

        domAsset::domContributor* contributor =
            daeSafeCast<domAsset::domContributor>( asset->add( "contributor" ) );

        // set date and time
        // Generate the date like this
        // "YYYY-mm-ddTHH:MM:SSZ"  ISO 8601  Date Time format
        const size_t bufSize = 1'024;
        ...
        u->setName(
            "meter"
        );    // NOTE: SketchUp incorrectly sets this to "meters" but it does not really
              // matter.
              //  Also note that since the default is: <unit meter="1.0" name="meter"/>
              //  this is equivalent to <unit/>
        u->setMeter( 1.0 );    // this is the important units setting as it tells
                               // consuming apps how to convert to meters.
        up_axis->setValue( UPAXISTYPE_Z_UP );

        // get description info as name value pairs
        if( node.getDescriptions().size() % 2 == 0 )
        {
            for( osg::Node::DescriptionList::const_iterator ditr =
                     node.getDescriptions().begin();
                 ditr != node.getDescriptions().end();
                 ++ditr )
            {
                std::string attrName( *ditr );
                ++ditr;
                ...
                {
                    domAsset::domKeywords* keywords =
                        daeSafeCast<domAsset::domKeywords>( asset->add( "keywords" ) );
                    keywords->setValue( attrValue.c_str() );
                }
                else if( attrName == "collada_revision" && !attrValue.empty() )
                {
                    domAsset::domRevision* revision =
                        daeSafeCast<domAsset::domRevision>( asset->add( "revision" ) );
                    revision->setValue( attrValue.c_str() );
                }
                else if( attrName == "collada_subject" && !attrValue.empty() )
                {
                    domAsset::domSubject* subject =
                        daeSafeCast<domAsset::domSubject>( asset->add( "subject" ) );
                    subject->setValue( attrValue.c_str() );
                }
                else if( attrName == "collada_title" && !attrValue.empty() )
                {
                    domAsset::domTitle* title =
                        daeSafeCast<domAsset::domTitle>( asset->add( "title" ) );
                ...
                else if( attrName == "collada_unit_meter_length" && !attrValue.empty() )
                {
                    ...
                    catch( ... )
                    {
                        ...
                        continue;
                         ...
                         "collada_contributor{0}.author" &&
                         !attrValue.empty() )
                {
                    domAsset::domContributor::domAuthor* author =
                        daeSafeCast<domAsset::domContributor::domAuthor>(
                            contributor->add( "author" )
                        );
                    author->setValue( attrValue.c_str() );
                }
                else if( attrName ==
                         "collada_contributor{0}.authoring_tool" &&
                         !attrValue.empty() )
                {
                    domAsset::domContributor::domAuthoring_tool* authoring_tool =
                        daeSafeCast<domAsset::domContributor::domAuthoring_tool>(
                            contributor->add( "authoring_tool" )
                        );
                    authoring_tool->setValue( attrValue.c_str() );
                }
                else if( attrName ==
                         "collada_contributor{0}.comments" &&
                         !attrValue.empty() )
                {
                    domAsset::domContributor::domComments* comments =
                        daeSafeCast<domAsset::domContributor::domComments>(
                            contributor->add( "comments" )
                        );
                    comments->setValue( attrValue.c_str() );
                }
                else if( attrName ==
                         "collada_contributor{0}.source_data" &&
                         !attrValue.empty() )
                {
                    domAsset::domContributor::domSource_data* source_data =
                        daeSafeCast<domAsset::domContributor::domSource_data>(
                            contributor->add( "source_data" )
                        );
                    source_data->setValue( attrValue.c_str() );
                }
                else if( attrName ==
                         "collada_contributor{0}.copyright" &&
                         !attrValue.empty() )
                {
                    domAsset::domContributor::domCopyright* copyright =
                        daeSafeCast<domAsset::domContributor::domCopyright>(
                            contributor->add( "copyright" )
                        );
                    copyright->setValue( attrValue.c_str() );
                }

                // TODO:  handle array of contributor data rather that just the first.
                // also there is probably a better way to pass attribute data as
                // DescriptionList is a bit fragile
            }
        }
    }

    void
    daeWriter::traverse( osg::Node& node )
    {
    ...
    void
    daeWriter::pushStateSet( osg::StateSet* ss )
    {
        if( NULL != ss )
        {
            ...
            currentStateSet = static_cast<osg::StateSet*>(
                currentStateSet->clone( osg::CopyOp::SHALLOW_COPY )
            );
    ...
    void
    daeWriter::popStateSet( osg::StateSet* ss )
    {
        if( NULL != ss )
        {
