...
namespace osgDAE
{
    ...
    class FindAnimatedNodeVisitor : public osg::DualModeVisitor
    {
        public:
            ...
            apply( osg::Node& node )
            {
                ...
                if( ncb )
                {
                    osgAnimation::AnimationUpdateCallback<osg::NodeCallback>* ut =
                        dynamic_cast<
                    ...
                    if( ut )
                    {
                        ...
                        {
                            // TODO store in a multimap and let the exporter create
                            // multiple <channel>s for each connected node
                            OSG_WARN << "Multiple nodes using the same update callback "
                                        "not supported"
                                     << std::endl;
                        }
                    }
            ...
            osg::Node*
            getTargetNode( const std::string& targetName )
            {
                UpdateCallbackNameNodeMap::iterator it =
                    _updateCallbackNameNodeMap.find( targetName );
    ...
    */
    class daeWriter : public osg::DualModeVisitor
    {
        protected:
        ...
        public:
            ...
            struct Options
            {
                    ...
                    bool         usePolygons;
                    /** work in Google compatibility mode. In daeWMaterials, change
                     * transparency color. And in daeWGeometry, replace tristrip and
                     * trifans by triangles*/
                    bool         googleMode;
                    ...
                    /** force the use an image for a texture, even if the file is not
                     * found (when m_linkOrignialTextures). */
                    bool         forceTexture;
                    bool         namesUseCodepage;
                    unsigned int relativiseImagesPathNbUpDirs;
                    /** Rename all IDs (geometries, materials, etc.) to remove characters
                     * which may be interpreted as an URI. */
                    bool         renameIds;
            };
            ...
            daeWriter( DAE*                                dae_,
                       ...
                       const Options*                      pluginOptions = NULL );
            virtual ~daeWriter();

            void
            setRootNode( const osg::Node& node );
            ...
            virtual void
            apply( osg::PositionAttitudeTransform& node );
            virtual void
            apply( osg::Switch& node );
            virtual void
            apply( osg::Sequence& node );
            virtual void
            apply( osg::LOD& node );
            ...
            virtual void
            apply( osg::CoordinateSystemNode& node );
            // virtual void  apply( osg::ClipNode &node)
            virtual void
            apply( osg::Transform& node );
            virtual void
            apply( osg::CameraView& node );
        ...
        protected:    // methods
            ...
            void
            writeAnimations( osg::Node& node );
            void
            writeNodeExtra( osg::Node& node );
            void
            writeUpdateTransformElements( const osg::vec3& pos,
                                          const osg::quat& q,
                                          const osg::vec3& s );
            void
            writeRigGeometry( osgAnimation::RigGeometry* pOsgRigGeometry );
            void
            writeMorphGeometry( osgAnimation::MorphGeometry* pOsgMorphGeometry );

            void
            debugPrint( osg::Node& node );

            domGeometry*
            getOrCreateDomGeometry( osg::Geometry* pOsgGeometry );
            bool
            processGeometry( osg::Geometry*     geom,
                             domGeometry*       geo,
                             const std::string& name );
            domSource*
            createSource( daeElement*        parent,
            ...
            Ty*
            createPrimGroup( daeString                      type,
                             domMesh*                       mesh,
                             domSource*                     norm,
                             domSource*                     color,
        ...
        protected:    // members
            ...
            struct CompareStateSet
            {
                    ...
                    {
                        // std::cout << "CompareStateSet: " << ss1->compare(*ss2, false)
                        // << " " << ss1 << " " << ss2 << std::endl;
                        return ss1->compare( *ss2, true ) < 0;
                    }
            };

            typedef std::map<osg::ref_ptr<osg::StateSet>, domMaterial*, CompareStateSet>
                                                            MaterialMap;
            typedef std::stack<osg::ref_ptr<osg::StateSet>> StateSetStack;
            typedef std::map<osg::Geometry*, domGeometry*>  OsgGeometryDomGeometryMap;
            typedef std::map<osgAnimation::RigGeometry*, domController*>
                OsgRigGeometryDomControllerMap;
            typedef std::map<osgAnimation::MorphGeometry*, domController*>
                                               OsgMorphGeometryDomControllerMap;
            ...
            osg::StateSet*
            CleanStateSet( osg::StateSet* pStateSet ) const;
        ...
        protected:    // inner classes
            ...
            class ArrayNIndices
            {
                public:
                    ...
                    ArrayNIndices( osg::Array*      valArray,
                                   osg::IndexArray* ind );
        ...
        private:    // members
            ...
            void
            appendGeometryIndices( osg::Geometry*                    geom,
                                   domP*                             p,
                                   unsigned int                      vindex,
                                   domSource*                        norm,
                                   domSource*                        color,
            ...
            std::string
            getNodeName( const osg::Node&   node,
                         const std::string& defaultName );

            /** provide an unique name */
            std::string
                                                uniquify( const std::string& name );

            /** Current RenderingHint */
            /** This are needed because the stateSet merge code currently does not handle
             * it */
