...
osg::Group*
daeReader::processOsgMultiSwitch( domTechnique* teq )
{
    ...
    if( any )
    {
        ...
        for( unsigned int currChild = 0; currChild < numChildren; currChild++ )
        {
            ...
            if( child )
            {
                ...
                {
                    OSG_WARN
                        << "Child of element 'ValueLists' is not of type 'ValueList'"
                        << std::endl;
                }
            }
            else
            {
                OSG_WARN << "Element 'ValueLists' does not contain expected elements."
                         << std::endl;
            }
        }
    }
...
osg::Group*
daeReader::processOsgSwitch( domTechnique* teq )
{
...
osg::Group*
daeReader::processOsgSequence( domTechnique* teq )
{
    ...
    {
        sq->setMode(
            ( osg::Sequence::SequenceMode )parseString<int>( any->getValue() )
        );
    }
    else
    {
...
osg::Group*
daeReader::processOsgLOD( domTechnique* teq )
{
        ...
        for( unsigned int currChild = 0; currChild < numChildren; currChild++ )
        {
            ...
            if( child )
            {
                if( strcmp( child->getElementName(), "MinMax" ) == 0 )
                {
                    ...
                    {
                        OSG_WARN << "'MinMax' does not contain a valid minimum value"
                                 << std::endl;
                    }
                    ...
                    {
                        OSG_WARN << "'MinMax' does not contain a valid maximum value"
                                 << std::endl;
                    }
                ...
                {
                    OSG_WARN << "Child of element 'RangeList' is not of type 'MinMax'"
                             << std::endl;
                }
            }
            else
            {
                OSG_WARN << "Element 'RangeList' does not contain expected elements."
                         << std::endl;
            }
        }
...
// 0..* <extra>
osg::Node*
daeReader::processLight( domLight* dlight )
{
    if( _numlights >= 7 )
    {
        OSG_WARN << "More than 8 lights may not be supported by OpenGL driver."
                 << std::endl;
    }
    ...
    else if( directional != NULL )
    {
        ...
        light->setDiffuse( osg::vec4( directional->getColor()->getValue()[0],
                                      ...
                                      directional->getColor()->getValue()[2],
                                      1.0F ) );
...
// 0..* <extra>
osg::Node*
daeReader::processCamera( domCamera* dcamera )
{
    osg::CameraView* pOsgCameraView = new osg::CameraView;
    pOsgCameraView->setName( dcamera->getId() );

    domCamera::domOptics::domTechnique_common* pDomTechniqueCommon =
        dcamera->getOptics()->getTechnique_common();
    domCamera::domOptics::domTechnique_common::domPerspective* pDomPerspective =
        pDomTechniqueCommon->getPerspective();
    domCamera::domOptics::domTechnique_common::domOrthographic* pDomOrthographic =
        pDomTechniqueCommon->getOrthographic();
    if( pDomPerspective )
    {
        // <perspective>
        // 1    <xfov>, <yfov>, <xfov> and <yfov>, <xfov> and <aspect_ratio>, <yfov> and
        // <aspect_ratio> 1    <znear> 1    <zfar>
        domTargetableFloat* pXfov =
            daeSafeCast<domTargetableFloat>( pDomPerspective->getXfov() );
        domTargetableFloat* pYfov =
            daeSafeCast<domTargetableFloat>( pDomPerspective->getYfov() );
        domTargetableFloat* pAspectRatio =
            daeSafeCast<domTargetableFloat>( pDomPerspective->getAspect_ratio() );
        ...
        if( pXfov )
        {
            if( pYfov )
            {
                ...
                if( pAspectRatio )
                {
                    OSG_WARN << "Unexpected <aspectratio> in <camera> '"
                             << dcamera->getId() << "'" << std::endl;
                }
            }
        ...
        else
        {
            ...
            OSG_WARN << "Expected <xfov> or <yfov> in <camera> '" << dcamera->getId()
                     << "'" << std::endl;
        }

        // domTargetableFloat *pZnear = daeSafeCast< domTargetableFloat
        // >(pDomPerspective->getZnear()); domTargetableFloat *pZfar = daeSafeCast<
        // domTargetableFloat >(pDomPerspective->getZfar());
    ...
    {
        // <orthographic>
        // 1    <xmag>, <ymag>, <xmag> and <ymag>, <xmag> and <aspect_ratio>, <ymag> and
        // <aspect_ratio> 1    <znear> 1    <zfar>

        // domTargetableFloat *pXmag = daeSafeCast< domTargetableFloat
        // >(pDomOrthographic->getXmag()); domTargetableFloat *pYmag = daeSafeCast<
        // domTargetableFloat >(pDomOrthographic->getYmag()); domTargetableFloat
        // *pAspectRatio = daeSafeCast< domTargetableFloat
        // >(pDomOrthographic->getAspect_ratio());

        // TODO The current osg::CameraView does not support an orthographic view
        OSG_WARN << "Orthographic in <camera> '" << dcamera->getId() << "' not supported"
                 << std::endl;

        // domTargetableFloat *pZnear = daeSafeCast< domTargetableFloat
        // >(pDomOrthographic->getZnear()); domTargetableFloat *pZfar = daeSafeCast<
        // domTargetableFloat >(pDomOrthographic->getZfar());
