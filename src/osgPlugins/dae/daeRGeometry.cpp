...
osg::Geode*
daeReader::getOrCreateGeometry( domGeometry*       pDomGeometry,
                                domBind_material*  pDomBindMaterial,
                                const osg::Geode** ppOriginalGeode )
{
    ...
    // Compute optimized geometry by expanding all indexed arrays so we are no longer
    // rendering with the slow path
    ...
    if( pDomBindMaterial )
    {
        processBindMaterial( pDomBindMaterial,
                             pDomGeometry,
                             pCopiedOsgGeode,
                             pOsgGeode );
    }
...
osgAnimation::Bone*
daeReader::getOrCreateBone( domNode* pDomNode )
{
    ...
    pOsgBone->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
daeReader::getOrCreateSkeleton( domNode* pDomNode )
{
    ...
    pOsgSkeleton->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
osg::Geode*
daeReader::processInstanceGeometry( domInstance_geometry* pDomInstanceGeometry )
{
    domGeometry* pDomGeometry =
        daeSafeCast<domGeometry>( getElementFromURI( pDomInstanceGeometry->getUrl() ) );
    if( !pDomGeometry )
    {
...
// 0..* <extra>
osg::Node*
daeReader::processMorph( domMorph*         pDomMorph,
                         domBind_material* pDomBindMaterial )
{
    domGeometry* pDomGeometry =
        daeSafeCast<domGeometry>( getElementFromURI( pDomMorph->getSource() ) );

    if( !pDomGeometry )
    {
        OSG_WARN << "Failed to locate geometry " << pDomMorph->getSource().getURI()
                 << std::endl;
        return NULL;
    }
    ...
    osgAnimation::MorphGeometry* pOsgMorphGeometry =
        new osgAnimation::MorphGeometry( *pOsgGeometry );
                ...
                if( const domName_array* pDomNames = pDomSource->getName_array() )
                {
                    ...
                    for( size_t j = 0; j < names.getCount(); j++ )
                    {
                        ...
                        if( pDomGeometry )
                        {
                            ...
                            // Expects a single geometry inside the geode, should change
                            // this
                            osg::Geometry* geometry = dynamic_cast<osg::Geometry*>(
                                targetgeode->getDrawable( 0 )
                            );
                            if( geometry )
                            {
                          ...
                          COMMON_PROFILE_INPUT_MORPH_WEIGHT ) )
        {
            ...
            if( pDomSource )
            {
                ...
                if( iter != _daeElementDomChannelMap.end() )
                {
                    ...
                    osgAnimation::UpdateMorph* pUpdateCallback =
                        new osgAnimation::UpdateMorph( name );
                    pOsgGeode->setUpdateCallback( pUpdateCallback );
                    pOsgGeode->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
// 0..* <extra>
osg::Node*
daeReader::processInstanceController( domInstance_controller* pDomInstanceController )
{
...
// 1..* <source>
// 1    <vertices>
// 0..*    <lines>, <linestrips>, <polygons>, <polylist>, <triangles>, <trifans>,
// <tristrips> 0..* <extra>
osg::Geode*
daeReader::processMesh( domMesh* pDomMesh )
{
    ...
    {
        processSinglePPrimitive<domLines>( pOsgGeode,
                                           pDomMesh,
                                           linesArray[i],
                                           sources,
                                           GL_LINES );
    ...
    {
        processMultiPPrimitive<domLinestrips>( pOsgGeode,
                                               pDomMesh,
                                               linestripsArray[i],
                                               sources,
                                               GL_LINE_STRIP );
    ...
    {
        processPolygons<domPolygons>( pOsgGeode,
                                      pDomMesh,
                                      polygonsArray[i],
                                      sources,
                                      GL_POLYGON,
                                      _pluginOptions.tessellateMode );
    }
    ...
    {
        processSinglePPrimitive<domTriangles>( pOsgGeode,
                                               pDomMesh,
                                               trianglesArray[i],
                                               sources,
                                               GL_TRIANGLES );
    ...
    {
        processPolygons<domTrifans>( pOsgGeode,
                                     pDomMesh,
                                     trifansArray[i],
                                     sources,
                                     GL_TRIANGLE_FAN,
    ...
    {
        processMultiPPrimitive<domTristrips>( pOsgGeode,
                                              pDomMesh,
                                              tristripsArray[i],
                                              sources,
                                              GL_TRIANGLE_STRIP );
...
// <convexmesh>
osg::Geode*
daeReader::processConvexMesh( domConvex_mesh* pDomConvexMesh )
{
    // OSG_WARN << "Unsupported geometry convex mesh '" << pDomConvexMesh->getId() << "'"
    // << std::endl;
    return NULL;
}
...
// <spline>
osg::Geode*
daeReader::processSpline( domSpline* pDomSpline )
{
...
// 0..* <extra>
osg::Geode*
daeReader::processGeometry( domGeometry* pDomGeometry )
{
    ...
    OSG_WARN << "Unexpected geometry type in geometry '" << pDomGeometry->getId() << "'"
             << std::endl;
    return NULL;
}
...
template<typename T>
void
daeReader::processSinglePPrimitive( osg::Geode*    geode,
                                    const domMesh* pDomMesh,
                                    const T*       group,
                                    SourceMap&     sources,
                                    GLenum         mode )
{
    ...
    osg::ref_ptr<osg::DrawElementsUInt> pDrawElements =
        new osg::DrawElementsUInt( mode );
...
template<typename T>
void
daeReader::processMultiPPrimitive( osg::Geode*    geode,
                                   const domMesh* pDomMesh,
                                   const T*       group,
                                   SourceMap&     sources,
                                   GLenum         mode )
{
    ...
    std::vector<std::vector<GLuint>> indexLists;
    resolveMeshArrays( group->getP_array(),
                       group->getInput_array(),
                       pDomMesh,
                       geometry,
                       sources,
...
void
daeReader::processPolylist( osg::Geode*        geode,
                            const domMesh*     pDomMesh,
                            const domPolylist* group,
                            SourceMap&         sources,
                            TessellateMode     tessellateMode )
{
    ...
    if( tessellateMode == TESSELLATE_POLYGONS_AS_TRIFAN )
    {
        ...
        for( size_t i = 0, j = 0; i < vCount.getCount(); ++i )
        {
            ...
            {
                OSG_WARN
                    << "Error: vertex counts are greater than the number of indices."
                    << std::endl;
                return;
            }
    ...
    else
    {
        ...
        // Use GL_POLYGON when tessellation will follow (tessellator needs it),
        // otherwise use GL_TRIANGLE_FAN for Core Profile compatibility
        GLenum polyMode =
            ( tessellateMode == TESSELLATE_POLYGONS ) ? GL_POLYGON : GL_TRIANGLE_FAN;
        const domListOfUInts& vCount = pDomVcount->getValue();
        for( size_t i = 0, j = 0; i < vCount.getCount(); ++i )
        {
            size_t primitiveLength = vCount[i];
            ...
            osg::DrawElementsUInt* pDrawElements = new osg::DrawElementsUInt( polyMode );
            geometry->addPrimitiveSet( pDrawElements );
...
template<typename T>
void
daeReader::processPolygons( osg::Geode*    geode,
                            const domMesh* pDomMesh,
                            const T*       group,
                            SourceMap&     sources,
                            GLenum         mode,
                            TessellateMode tessellateMode )
{
    ...
    else
    {
        ...
        // otherwise use GL_TRIANGLE_FAN for Core Profile compatibility
        GLenum polyMode =
            ( tessellateMode == TESSELLATE_POLYGONS ) ? mode : GL_TRIANGLE_FAN;
        for( size_t i = 0; i < indexLists.size(); ++i )
        {
            const std::vector<GLuint>& indices   = indexLists[i];
...
void
processVertices( domVertices* vertices,
...
void
resolveMeshInputs( const domInputLocalOffset_Array& inputs,
                   daeElement*&                     position_source,
                   daeElement*&                     color_source,
                   daeElement*&                     normal_source,
                   ...
                   int         texcoord_offsets[MAX_TEXTURE_COORDINATE_SETS] )
{
    ...
    for( size_t i = 0; i < inputs.getCount(); i++ )
    {
        ...
        if( strcmp( COMMON_PROFILE_INPUT_COLOR, semantic ) == 0 )
        {
            ...
            {
                OSG_WARN << "Overwriting vertices input(COLOR) with input from primitive"
                         << std::endl;
            }
            color_source = pDaeElement;
            color_offset = offset;
        ...
        else if( strcmp( COMMON_PROFILE_INPUT_NORMAL, semantic ) == 0 )
        {
            ...
            {
                OSG_WARN
                    << "Overwriting vertices input(NORMAL) with input from primitive"
                    << std::endl;
            }
            normal_source = pDaeElement;
        ...
        else if( strcmp( COMMON_PROFILE_INPUT_TEXCOORD, semantic ) == 0 )
        {
            ...
            {
                OSG_WARN
                    << "Overwriting vertices input(TEXCOORD) with input from primitive"
                    << std::endl;
            }
...
struct VertexIndices
{
        VertexIndices( int       p,
                       int       c,
                       int       n,
                       const int t[MAX_TEXTURE_COORDINATE_SETS] ) :
            position_index( p ),
            color_index( c ),
            normal_index( n )
        {
        ...
        bool
        operator<( const VertexIndices& rhs ) const
        {
...
template<>
inline int
VertexIndices::get<VertexIndices::POSITION>() const
{
...
template<>
inline int
VertexIndices::get<VertexIndices::COLOR>() const
{
    return color_index;
}
...
template<>
inline int
VertexIndices::get<VertexIndices::NORMAL>() const
{
    return normal_index;
}
...
template<int Value>
inline int
VertexIndices::get() const
{
    // TEXCOORD has not to be implemented here as we need compile-time constants for
    // texcoord number
    return -1;
}
...
template<class ArrayType,
         int Value>
ArrayType*
createGeometryArray( domSourceReader&             sourceReader,
                     const VertexIndicesIndexMap& vertexIndicesIndexMap,
                     int                          texcoordNum = -1 )
{
...
template<class ArrayTypeSingle,
         class ArrayTypeDouble,
         int Value>
inline osg::Array*
createGeometryArray( domSourceReader&             sourceReader,
                     const VertexIndicesIndexMap& vertexIndicesIndexMap,
                     ...
                     int                          texcoordNum = -1 )
{
    ...
    {
        return createGeometryArray<ArrayTypeDouble, Value>( sourceReader,
                                                            vertexIndicesIndexMap,
                                                            texcoordNum );
    }
    else
    {
        return createGeometryArray<ArrayTypeSingle, Value>( sourceReader,
                                                            vertexIndicesIndexMap,
                                                            texcoordNum );
    }
}
...
void
daeReader::resolveMeshArrays( const domP_Array&                 domPArray,
                              ...
                              std::vector<std::vector<GLuint>>& vertexLists )
{
    ...
    for( size_t j = 0; j < domPArray.getCount(); ++j )
    {
        ...
        for( size_t i = 0; i < p.getCount(); i += stride )
        {
            ...
            VertexIndices v( p.get( i + position_offset ),
    ...
    {
        VertexIndicesIndexMap::iterator it  = vertexIndicesIndexMap.begin(),
                                        end = vertexIndicesIndexMap.end();
    ...
    const bool readDoubleVertices =
        ( _pluginOptions.precisionHint & osgDB::Options::DOUBLE_PRECISION_VERTEX ) != 0;
    const bool readDoubleColors =
        ( _pluginOptions.precisionHint & osgDB::Options::DOUBLE_PRECISION_COLOR ) != 0;
    const bool readDoubleNormals =
        ( _pluginOptions.precisionHint & osgDB::Options::DOUBLE_PRECISION_NORMAL ) != 0;
    const bool readDoubleTexcoords = ( _pluginOptions.precisionHint &
                                       osgDB::Options::DOUBLE_PRECISION_TEX_COORD ) != 0;

    // Vertices
    {
        osg::ref_ptr<osg::Array> array(
            createGeometryArray<osg::Vec3Array,
                                osg::Vec3dArray,
    ...
    if( color_source )
    {
        // first try Vec4Array
        osg::ref_ptr<osg::Array> array(
            createGeometryArray<osg::Vec4Array, osg::Vec4dArray, VertexIndices::COLOR>(
        ...
        if( !array )
        {
            array = createGeometryArray<osg::Vec3Array,
                                        osg::Vec3dArray,
    ...
    {
        osg::ref_ptr<osg::Array> array(
            createGeometryArray<osg::Vec3Array, osg::Vec3dArray, VertexIndices::NORMAL>(
    ...
    for( unsigned int texcoord_set = 0; texcoord_set < MAX_TEXTURE_COORDINATE_SETS;
         ++texcoord_set )
    {
        if( daeElement* texcoord_source = texcoord_sources[texcoord_set] )
        {
            ...
            // 2D Texcoords
            osg::ref_ptr<osg::Array> array2d(
                createGeometryArray<osg::Vec2Array,
                                    osg::Vec2dArray,
            ...
            {
                // 3D Textcoords
                osg::ref_ptr<osg::Array> array3d(
                    createGeometryArray<osg::Vec3Array,
                                        osg::Vec3dArray,
                                        VertexIndices::TEXCOORD>(
                        sources[texcoord_source],
                        vertexIndicesIndexMap,
                        readDoubleTexcoords,
                        texcoord_set
                    )
