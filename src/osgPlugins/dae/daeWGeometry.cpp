...
unsigned int
daeWriter::ArrayNIndices::getDAESize()
{
    switch( mode )
    {
        case VEC2F :
...
bool
daeWriter::ArrayNIndices::append( domListOfFloats& list )
{
    switch( getMode() )
    {
        case VEC2F :
            for( osg::Vec2Array::const_iterator it = vec2->begin(), itEnd = vec2->end();
                 it != itEnd;
                 ++it )
            {
            ...
            break;
        case VEC2D :
            for( osg::Vec2dArray::const_iterator it    = vec2d->begin(),
                                                 itEnd = vec2d->end();
                 it != itEnd;
                 ++it )
            {
                ::append<osg::dvec2>( list, *it );
            }
            break;
        case VEC3F :
            for( osg::Vec3Array::const_iterator it = vec3->begin(), itEnd = vec3->end();
                 it != itEnd;
                 ++it )
            {
            ...
            break;
        case VEC3D :
            for( osg::Vec3dArray::const_iterator it    = vec3d->begin(),
                                                 itEnd = vec3d->end();
                 it != itEnd;
                 ++it )
            {
                ::append<osg::dvec3>( list, *it );
            }
            break;
        case VEC4F :
            for( osg::Vec4Array::const_iterator it = vec4->begin(), itEnd = vec4->end();
                 it != itEnd;
                 ++it )
            {
            ...
            break;
        case VEC4D :
            for( osg::Vec4dArray::const_iterator it    = vec4d->begin(),
                                                 itEnd = vec4d->end();
                 it != itEnd;
                 ++it )
            {
                ::append<osg::dvec4>( list, *it );
            }
            break;
        case VEC4_UB :
            for( osg::Vec4ubArray::const_iterator it    = vec4ub->begin(),
                                                  itEnd = vec4ub->end();
                 it != itEnd;
                 ++it )
            {
                ::append<osg::ubvec4>( list, *it );
            }
            break;
        default :
            return false;
...
domGeometry*
daeWriter::getOrCreateDomGeometry( osg::Geometry* pOsgGeometry )
{
    ...
    else
    {
        if( !lib_geoms )
        {
            lib_geoms = daeSafeCast<domLibrary_geometries>(
                dom->add( COLLADA_ELEMENT_LIBRARY_GEOMETRIES )
            );
        }
        domGeometry* pDomGeometry =
            daeSafeCast<domGeometry>( lib_geoms->add( COLLADA_ELEMENT_GEOMETRY ) );
...
void
daeWriter::writeRigGeometry( osgAnimation::RigGeometry* pOsgRigGeometry )
{
    // See if controller exists in cache
    OsgRigGeometryDomControllerMap::iterator iter =
        _osgRigGeometryDomControllerMap.find( pOsgRigGeometry );
    ...
    else
    {
        ...
        if( pDomGeometry )
        {
            if( !lib_controllers )
            {
                lib_controllers = daeSafeCast<domLibrary_controllers>(
                    dom->add( COLLADA_ELEMENT_LIBRARY_CONTROLLERS )
                );
            }
            ...
            //   1      <vertex_weights>
            //   0..1    <extra>
            pDomController = daeSafeCast<domController>(
                lib_controllers->add( COLLADA_ELEMENT_CONTROLLER )
            );
            ...
            _osgRigGeometryDomControllerMap.insert( std::make_pair( pOsgRigGeometry,
                                                                    pDomController ) );

            // Link <skin> to cache hit or created <geometry>
            domSkin* pDomSkin =
                daeSafeCast<domSkin>( pDomController->add( COLLADA_ELEMENT_SKIN ) );
            std::string url = "#" + std::string( pDomGeometry->getId() );
            pDomSkin->setSource( url.c_str() );

            domSource* pDomJointsSource =
                daeSafeCast<domSource>( pDomSkin->add( COLLADA_ELEMENT_SOURCE ) );
            ...
            domName_array* pDomJointsNameArray = daeSafeCast<domName_array>(
                pDomJointsSource->add( COLLADA_ELEMENT_NAME_ARRAY )
            );
            ...
            {
                domSource::domTechnique_common* pDomSourceTechniqueCommon =
                    daeSafeCast<domSource::domTechnique_common>(
                        pDomJointsSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
                    );

                domAccessor* pDomAccessor = daeSafeCast<domAccessor>(
                    pDomSourceTechniqueCommon->add( COLLADA_ELEMENT_ACCESSOR )
                );
                ...
                domParam* pDomParam =
                    daeSafeCast<domParam>( pDomAccessor->add( COLLADA_ELEMENT_PARAM ) );
                pDomParam->setType( COLLADA_TYPE_NAME );
            }

            domSource* pDomSkinBindPoseSource =
                daeSafeCast<domSource>( pDomSkin->add( COLLADA_ELEMENT_SOURCE ) );
            ...
            domFloat_array* pDomMatricesArray = daeSafeCast<domFloat_array>(
                pDomSkinBindPoseSource->add( COLLADA_ELEMENT_FLOAT_ARRAY )
            );
            ...
            pDomMatricesArray->setValue( matrices );
            {
                domSource::domTechnique_common* pDomSourceTechniqueCommon =
                    daeSafeCast<domSource::domTechnique_common>(
                        pDomSkinBindPoseSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
                    );
            ...
            domSource* pDomSkinWeightsSource =
                daeSafeCast<domSource>( pDomSkin->add( COLLADA_ELEMENT_SOURCE ) );
            ...
            domFloat_array* pDomWeightsArray = daeSafeCast<domFloat_array>(
                pDomSkinWeightsSource->add( COLLADA_ELEMENT_FLOAT_ARRAY )
            );
            ...
            pDomWeightsArray->setValue( weights );
            {
                domSource::domTechnique_common* pDomSourceTechniqueCommon =
                    daeSafeCast<domSource::domTechnique_common>(
                        pDomSkinWeightsSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
                    );
            ...
            domSkin::domJoints* pDomJoints = daeSafeCast<domSkin::domJoints>(
                pDomSkin->add( COLLADA_ELEMENT_JOINTS )
            );

            domInputLocal* pDomInput =
                daeSafeCast<domInputLocal>( pDomJoints->add( COLLADA_ELEMENT_INPUT ) );
            ...
            domSkin::domVertex_weights* pDomVertexWeights =
                daeSafeCast<domSkin::domVertex_weights>(
                    pDomSkin->add( COLLADA_ELEMENT_VERTEX_WEIGHTS )
                );
            pDomVertexWeights->setCount( 0 );    // TODO set number of vertex weights

            domInputLocalOffset* pDomInputLocalOffset = daeSafeCast<domInputLocalOffset>(
                pDomVertexWeights->add( COLLADA_ELEMENT_INPUT )
            );
            ...
            pDomInputLocalOffset = daeSafeCast<domInputLocalOffset>(
                pDomVertexWeights->add( COLLADA_ELEMENT_INPUT )
            );
            ...
            domSkin::domVertex_weights::domVcount* pDomVcount =
                daeSafeCast<domSkin::domVertex_weights::domVcount>(
                    pDomVertexWeights->add( COLLADA_ELEMENT_VCOUNT )
                );
            domListOfUInts valueCounts;
            // TODO
            pDomVcount->setValue( valueCounts );
            domSkin::domVertex_weights::domV* pDomV =
                daeSafeCast<domSkin::domVertex_weights::domV>(
                    pDomVertexWeights->add( COLLADA_ELEMENT_V )
                );
    ...
    if( pDomController )
    {
        ...
        domInstance_controller* pDomInstanceController =
            daeSafeCast<domInstance_controller>(
                currentNode->add( COLLADA_ELEMENT_INSTANCE_CONTROLLER )
            );
...
void
daeWriter::writeMorphGeometry( osgAnimation::MorphGeometry* pOsgMorphGeometry )
{
    // See if controller exists in cache
    OsgMorphGeometryDomControllerMap::iterator iter =
        _osgMorphGeometryDomControllerMap.find( pOsgMorphGeometry );
    domController* pDomController = NULL;
    ...
    else
    {
        ...
        if( pDomGeometry )
        {
            ...
            pDomController->setId( name.c_str() );
            _osgMorphGeometryDomControllerMap.insert( std::make_pair( pOsgMorphGeometry,
                                                                      pDomController ) );

            // Link <morph> to cache hit or created <geometry>
            domMorph* pDomMorph =
                daeSafeCast<domMorph>( pDomController->add( COLLADA_ELEMENT_MORPH ) );
            std::string url = "#" + std::string( pDomGeometry->getId() );
            pDomMorph->setSource( url.c_str() );
            pDomMorph->setMethod( MORPHMETHODTYPE_NORMALIZED );
            // pDomMorph->setMethod(MORPHMETHODTYPE_RELATIVE);

            domSource* pDomTargetsSource =
                daeSafeCast<domSource>( pDomMorph->add( COLLADA_ELEMENT_SOURCE ) );
            std::string targetsName = name + "_morph_targets";
            pDomTargetsSource->setId( targetsName.c_str() );

            domIDREF_array* pDomIDREFArray = daeSafeCast<domIDREF_array>(
                pDomTargetsSource->add( COLLADA_ELEMENT_IDREF_ARRAY )
            );
            xsIDREFS                                     idrefs;
            osgAnimation::MorphGeometry::MorphTargetList morphTargetList =
                pOsgMorphGeometry->getMorphTargetList();
            for( unsigned int i = 0; i < morphTargetList.size(); i++ )
            {
                domGeometry* pRefDomGeometry =
                    getOrCreateDomGeometry( morphTargetList[i].getGeometry() );
            ...
            domSource::domTechnique_common* pDomTechniqueCommon =
                daeSafeCast<domSource::domTechnique_common>(
                    pDomTargetsSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
                );
            domAccessor* pDomAccessor = daeSafeCast<domAccessor>(
                pDomTechniqueCommon->add( COLLADA_ELEMENT_ACCESSOR )
            );
            ...
            domSource* pDomWeightsSource =
                daeSafeCast<domSource>( pDomMorph->add( COLLADA_ELEMENT_SOURCE ) );
            std::string weightsName = name + "_morph_weights";
            pDomWeightsSource->setId( weightsName.c_str() );

            domFloat_array* pDomFloatArray = daeSafeCast<domFloat_array>(
                pDomWeightsSource->add( COLLADA_ELEMENT_FLOAT_ARRAY )
            );
            ...
            pDomTechniqueCommon = daeSafeCast<domSource::domTechnique_common>(
                pDomWeightsSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
            );
            pDomAccessor = daeSafeCast<domAccessor>(
                pDomTechniqueCommon->add( COLLADA_ELEMENT_ACCESSOR )
            );
            pDomAccessor->setCount( morphTargetList.size() );
            url = "#" + weightsName;
            ...
            domMorph::domTargets* pDomTargets = daeSafeCast<domMorph::domTargets>(
                pDomMorph->add( COLLADA_ELEMENT_TARGETS )
            );

            domInputLocal* pDomTargetsInput =
                daeSafeCast<domInputLocal>( pDomTargets->add( COLLADA_ELEMENT_INPUT ) );
            pDomTargetsInput->setSemantic( COMMON_PROFILE_INPUT_MORPH_TARGET );
            url = "#" + targetsName;
            pDomTargetsInput->setSource( url.c_str() );

            domInputLocal* pDomWeightsInput =
                daeSafeCast<domInputLocal>( pDomTargets->add( COLLADA_ELEMENT_INPUT ) );
    ...
    if( pDomController )
    {
        ...
        if( pOsgMorphGeometry->getStateSet() )
        {
            m_CurrentRenderingHint =
                pOsgMorphGeometry->getStateSet()->getRenderingHint();
        }
        ...
        {
            domBind_material* pDomBindMaterial = daeSafeCast<domBind_material>(
                pDomInstanceController->add( COLLADA_ELEMENT_BIND_MATERIAL )
            );
            processMaterial( currentStateSet.get(),
                             pDomBindMaterial,
                             pOsgMorphGeometry->getName() );
        }
...
void
daeWriter::apply( osg::Geode& node )
{
    debugPrint( node );
    ...
    // TODO
    // Write a Geode as a single instance_geometry if all drawables use the same vertex
    // streams Reuse an existing Geode if only statesets differ
    ...
    for( unsigned int i = 0; i < count; i++ )
    {
        ...
        if( g != NULL )
        {
            osgAnimation::RigGeometry* pOsgRigGeometry =
                dynamic_cast<osgAnimation::RigGeometry*>( g );
            if( pOsgRigGeometry )
            {
            ...
            {
                osgAnimation::MorphGeometry* pOsgMorphGeometry =
                    dynamic_cast<osgAnimation::MorphGeometry*>( g );
                ...
                else
                {
                    ...
                    if( pDomGeometry )
                    {
                        ...
                        domInstance_geometry* pDomInstanceGeometry =
                            daeSafeCast<domInstance_geometry>(
                                currentNode->add( COLLADA_ELEMENT_INSTANCE_GEOMETRY )
                            );
                        std::string url = "#" + std::string( pDomGeometry->getId() );
                        ...
                        if( !stateSetStack.empty() )
                        {
                            domBind_material* pDomBindMaterial =
                                daeSafeCast<domBind_material>( pDomInstanceGeometry->add(
...
void
daeWriter::appendGeometryIndices( osg::Geometry*                    geom,
                                  domP*                             p,
                                  unsigned int                      vindex,
                                  domSource*                        norm,
                                  domSource*                        color,
                                  ...
                                  unsigned int                      ccount )
{
    ...
    if( norm != NULL )
    {
        ...
        {
            p->getValue().append( normals.inds != NULL ? normals.inds->index( vindex )
                                                       : vindex );
        }
        else
        {
            p->getValue().append( normals.inds != NULL ? normals.inds->index( ncount )
                                                       : ncount );
        }
    }
    ...
    if( color != NULL )
    {
        ...
        {
            p->getValue().append( colors.inds != NULL ? colors.inds->index( vindex )
                                                      : vindex );
        }
        else
        {
            p->getValue().append( colors.inds != NULL ? colors.inds->index( ccount )
                                                      : ccount );
        }
    }
    ...
    for( unsigned int ti = 0; ti < texcoords.size(); ti++ )
    {
        ...
        p->getValue().append( texcoords[ti].inds != NULL
                                  ? texcoords[ti].inds->index( vindex )
                                  : vindex );
    }
}

bool
daeWriter::processGeometry( osg::Geometry*     geom,
                            domGeometry*       geo,
                            const std::string& name )
{
    ...
    for( unsigned int i = 0; i < geom->getNumVertexAttribArrays(); i++ )
    {
        ...
        {
            vertexAttributes.push_back( ArrayNIndices( geom->getVertexAttribArray( i ),
                                                       0 ) );
        }
    }
    ...
    std::string sName;
    {
        ...
        domVertices* vertices =
            daeSafeCast<domVertices>( mesh->add( COLLADA_ELEMENT_VERTICES ) );
        std::string vName = name + "-vertices";
        vertices->setId( vName.c_str() );

        // make a POSITION input in it
        domInputLocal* il =
            daeSafeCast<domInputLocal>( vertices->add( COLLADA_ELEMENT_INPUT ) );
    ...
    if( normals.getMode() != ArrayNIndices::NONE )
    {
        ...
        unsigned int numElements =
            normals.valArray ? normals.valArray->getNumElements() : 0;
    ...
    if( colors.getMode() != ArrayNIndices::NONE )
    {
        ...
        unsigned int numElements =
            colors.valArray ? colors.valArray->getNumElements() : 0;
    ...
    // process TEXCOORD
    // TODO: Do the same as normal and colors for texcoods. But in a loop since you can
    // have many
    for( unsigned int ti = 0; ti < texcoords.size(); ti++ )
    {
        ...
        unsigned int numElements =
            texcoords[ti].valArray ? texcoords[ti].valArray->getNumElements() : 0;
    ...
    for( unsigned int ti = 0; ti < vertexAttributes.size(); ti++ )
    {
        ...
        unsigned int numElements = vertexAttributes[ti].valArray
                                     ? vertexAttributes[ti].valArray->getNumElements()
                                     : 0;
        domSource*   t           = createSource(
            mesh,
            sName,
            elementSize,
            false,
            true
        );    // Sukender: should we *REALLY* call createSource(... false, true)? (I mean
              // with such flags)
        t->getFloat_array()->setCount( numElements * elementSize );
        t->getTechnique_common()->getAccessor()->setCount( numElements );
    ...
    for( unsigned int i = 0; i < geom->getNumPrimitiveSets(); i++ )
    {
        ...
        switch( mode )
        {
            case GL_POINTS :
                {
                    OSG_WARN << "Geometry contains points rendering. COLLADA does not"
                             << std::endl;
                    continue;
                }
            case GL_LINES :
                {
                    if( lines == NULL )
                    {
                        lines = createPrimGroup<domLines>( COLLADA_ELEMENT_LINES,
                                                           mesh,
                                                           norm,
                                                           color,
                                                           texcoord );
            ...
            case GL_TRIANGLES :
                {
                    if( tris == NULL )
                    {
                        tris = createPrimGroup<domTriangles>( COLLADA_ELEMENT_TRIANGLES,
                                                              mesh,
                                                              norm,
                                                              color,
                                                              texcoord );
            ...
            case GL_QUADS :
                {
                    if( polys == NULL )
                    {
                        ...
                        {
                            polys =
                                createPrimGroup<domPolygons>( COLLADA_ELEMENT_POLYGONS,
                                                              mesh,
                                                              norm,
                                                              color,
                                                              texcoord );
                        ...
                        {
                            polylist =
                                createPrimGroup<domPolylist>( COLLADA_ELEMENT_POLYLIST,
                                                              mesh,
                                                              norm,
                                                              color,
                                                              texcoord );
            ...
            case GL_LINE_STRIP :
                {
                    ...
                    {
                        linestrips =
                            createPrimGroup<domLinestrips>( COLLADA_ELEMENT_LINESTRIPS,
                                                            mesh,
                                                            norm,
                                                            color,
                                                            texcoord );
            ...
            case GL_TRIANGLE_STRIP :
                {
                    ...
                    {
                        tristrips =
                            createPrimGroup<domTristrips>( COLLADA_ELEMENT_TRISTRIPS,
                                                           mesh,
                                                           norm,
                                                           color,
                                                           texcoord );
            ...
            case GL_TRIANGLE_FAN :
                {
                    if( trifans == NULL )
                    {
                        trifans = createPrimGroup<domTrifans>( COLLADA_ELEMENT_TRIFANS,
                                                               mesh,
                                                               norm,
                                                               color,
                                                               texcoord );
            ...
            case osg::PrimitiveSet::DrawArraysPrimitiveType :
                {
                    ...
                    unsigned int     indexEnd =
                        drawArray->getFirst() + drawArray->getCount();
                    ...
                    switch( mode )
                    {
                        ...
                        case GL_TRIANGLES :
                            {
                                ...
                                tris->setCount(
                                    tris->getCount() + drawArray->getCount() / primLength
                                );
                                break;
                            }
                        case GL_LINE_STRIP :
                            {
                                p.push_back( daeSafeCast<domP>(
                                    linestrips->add( COLLADA_ELEMENT_P )
                                ) );
                        ...
                        case GL_TRIANGLE_STRIP :
                            {
                                p.push_back( daeSafeCast<domP>(
                                    tristrips->add( COLLADA_ELEMENT_P )
                                ) );
                        ...
                        default :
                            {
                                // TODO : test this case
                                unsigned int nbPolygons =
                                    drawArray->getCount() / primLength;
                                if( _pluginOptions.usePolygons )
                                {
                                ...
                                else
                                {
                                    for( unsigned int idx = 0; idx < nbPolygons; ++idx )
                                    {
                                        polylist->getVcount()->getValue().append(
                                            primLength
                                        );
                                    }
                                    p.push_back( polylist->getP() );
                                    polylist->setCount( polylist->getCount() +
                                                        nbPolygons );
                                }
                                break;
                            }
                    ...
                    unsigned int nbVerticesPerPoly =
                        ( indexEnd - indexBegin ) / p.size();
                    unsigned int indexPolyEnd = indexBegin + nbVerticesPerPoly;
                    for( unsigned int iPoly = 0; iPoly < p.size(); ++iPoly )
                    {
                        for( unsigned int vindex = indexBegin; vindex < indexPolyEnd;
                             vindex++ )
                        {

                            appendGeometryIndices( geom,
                                                   p[iPoly],
                                                   vindex,
                                                   norm,
                                                   color,
            ...
            case osg::PrimitiveSet::DrawArrayLengthsPrimitiveType :
                {
                    ...
                    osg::DrawArrayLengths* drawArrayLengths =
                        static_cast<osg::DrawArrayLengths*>( ps );
                         ...
                         ++primItr )
                    {
                            ...
                            case GL_TRIANGLES :
                                {
                                    p.push_back( tris->getP() );
                                    tris->setCount(
                                        tris->getCount() + ( *primItr ) / localPrimLength
                                    );
                                    break;
                                }
                            case GL_LINE_STRIP :
                            ...
                            default :
                                {
                                    ...
                                    if( _pluginOptions.usePolygons )
                                    {
                                        // for( unsigned int idx = 0; idx < nbPolygons;
                                        // ++idx )
                                        p.push_back( polys->getP_array()[0] );
                        ...
                        for( unsigned int iPoly = 0; iPoly < p.size(); ++iPoly )
                        {
                            ...
                            // \n",indexBegin,indexEnd);
                            for( unsigned int primCount = indexBegin;
                                 primCount < indexEnd;
                                 ++primCount )
                            {
                                appendGeometryIndices( geom,
            ...
            case osg::PrimitiveSet::DrawElementsUBytePrimitiveType :
                {
                    ...
                    osg::DrawElementsUByte* drawElements =
                        static_cast<osg::DrawElementsUByte*>( ps );
                    ...
                    switch( mode )
                    {
                        case GL_LINES :
                            {
                                p.push_back( lines->getP() );
                                lines->setCount(
                                    lines->getCount() + drawElements->size() / primLength
                                );
                                break;
                            }
                        case GL_TRIANGLES :
                            {
                                p.push_back( tris->getP() );
                                tris->setCount(
                                    tris->getCount() + drawElements->size() / primLength
                                );
                                break;
                            }
                        case GL_LINE_STRIP :
                        ...
                        default :
                            {
                                unsigned int nbPolygons =
                                    drawElements->size() / primLength;
                                if( _pluginOptions.usePolygons )
                                {
                                    // for( unsigned int idx = 0; idx < nbPolygons; ++idx
                                    // ) /*idx*/ huh ? why only one ?
                                    p.push_back( polys->getP_array()[0] );
                                    polys->setCount( polys->getCount() + nbPolygons );
                                }
                                else
                    ...
                    osg::DrawElementsUByte::iterator primItrBegin =
                        drawElements->begin();
                    unsigned int nbVerticesPerPoly = drawElements->size() / p.size();
                    osg::DrawElementsUByte::iterator primItrEnd =
                        primItrBegin + nbVerticesPerPoly;
                    for( unsigned int iPoly = 0; iPoly < p.size(); ++iPoly )
                    {
                        for( osg::DrawElementsUByte::iterator primItr = primItrBegin;
                             primItr != primItrEnd;
                             ++primCount, ++primItr )
                        {
            ...
            case osg::PrimitiveSet::DrawElementsUShortPrimitiveType :
                {
                    ...
                    osg::DrawElementsUShort* drawElements =
                        static_cast<osg::DrawElementsUShort*>( ps );
                    ...
                    unsigned int                      primCount = 0;
                    osg::DrawElementsUShort::iterator primItrBegin =
                        drawElements->begin();
                    unsigned int nbVerticesPerPoly = drawElements->size() / p.size();
                    osg::DrawElementsUShort::iterator primItrEnd =
                        primItrBegin + nbVerticesPerPoly;

                    for( unsigned int iPoly = 0; iPoly < p.size(); ++iPoly )
                    {
                        for( osg::DrawElementsUShort::iterator primItr = primItrBegin;
                             primItr != primItrEnd;
                             ++primCount, ++primItr )
                        {
            ...
            case osg::PrimitiveSet::DrawElementsUIntPrimitiveType :
                {
                    ...
                    osg::DrawElementsUInt* drawElements =
                        static_cast<osg::DrawElementsUInt*>( ps );
                    ...
                    unsigned int nbVerticesPerPoly = drawElements->size() / p.size();
                    osg::DrawElementsUInt::iterator primItrEnd =
                        primItrBegin + nbVerticesPerPoly;

                    for( unsigned int iPoly = 0; iPoly < p.size(); ++iPoly )
                    {

                        for( osg::DrawElementsUInt::iterator primItr = primItrBegin;
                             primItr != primItrEnd;
                             ++primCount, ++primItr )
                        {
...
domSource*
daeWriter::createSource( daeElement*        parent,
                         ...
                         bool               uv )
{
    ...
    domFloat_array* fa =
        daeSafeCast<domFloat_array>( src->add( COLLADA_ELEMENT_FLOAT_ARRAY ) );
    std::string fName = baseName + "-array";
    fa->setId( fName.c_str() );

    domSource::domTechnique_common* teq = daeSafeCast<domSource::domTechnique_common>(
        src->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
    );
...
Ty*
daeWriter::createPrimGroup( daeString                      type,
                            domMesh*                       mesh,
                            domSource*                     norm,
                            domSource*                     color,
                            const std::vector<domSource*>& texcoord )
{
    unsigned int         offset = 0;
    Ty*                  retVal = daeSafeCast<Ty>( mesh->add( type ) );
    domInputLocalOffset* ilo =
        daeSafeCast<domInputLocalOffset>( retVal->add( COLLADA_ELEMENT_INPUT ) );
