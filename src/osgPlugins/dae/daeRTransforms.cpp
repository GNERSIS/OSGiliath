...
// Note <lookat>, <matrix>, <rotate>, <scale>, <skew> and <translate> may appear in any
// order These transformations can be combined in any number and ordering to produce the
// desired coordinate system for the parent <node> element. The COLLADA specification
// requires that the transformation elements are processed in order and accumulate the
// result as if they were converted to column-order matrices and concatenated using
// matrix post-multiplication.
osg::Transform*
daeReader::processOsgMatrixTransform( domNode* node,
                                      bool     isBone )
{
    ...
    for( size_t i = 0; i < count; i++ )
    {
        ...
        if( domRotate* pDomRotate = daeSafeCast<domRotate>( pDaeElement ) )
        {
            ...
            pTransformElement = new osgAnimation::StackedRotateAxisElement(
                pDomRotate->getSid() ? pDomRotate->getSid() : "",
        ...
        else if( domTranslate* pDomTranslate = daeSafeCast<domTranslate>( pDaeElement ) )
        {
            ...
            pTransformElement = new osgAnimation::StackedTranslateElement(
                pDomTranslate->getSid() ? pDomTranslate->getSid() : "",
        ...
        else if( domScale* pDomScale = daeSafeCast<domScale>( pDaeElement ) )
        {
            ...
            pTransformElement = new osgAnimation::StackedScaleElement(
                pDomScale->getSid() ? pDomScale->getSid() : "",
        ...
        else if( domMatrix* pDomMatrix = daeSafeCast<domMatrix>( pDaeElement ) )
        {
            ...
            pTransformElement = new osgAnimation::StackedMatrixElement(
                pDomMatrix->getSid() ? pDomMatrix->getSid() : "",
                osg::dmat4( pDomMatrix->getValue()[0],
                            pDomMatrix->getValue()[4],
                            pDomMatrix->getValue()[8],
                            pDomMatrix->getValue()[12],
                            pDomMatrix->getValue()[1],
                            pDomMatrix->getValue()[5],
                            pDomMatrix->getValue()[9],
                            pDomMatrix->getValue()[13],
                            pDomMatrix->getValue()[2],
                            pDomMatrix->getValue()[6],
                            pDomMatrix->getValue()[10],
                            pDomMatrix->getValue()[14],
                            pDomMatrix->getValue()[3],
                            pDomMatrix->getValue()[7],
                            pDomMatrix->getValue()[11],
                            pDomMatrix->getValue()[15] )
            );
        }
        else if( domLookat* pDomLookat = daeSafeCast<domLookat>( pDaeElement ) )
        {
            ...
            pTransformElement = new osgAnimation::StackedMatrixElement(
                pDomLookat->getSid() ? pDomLookat->getSid() : "",
                osg::dmat4::lookAt( osg::vec3( pDomLookat->getValue()[0],
                                               pDomLookat->getValue()[1],
                                               pDomLookat->getValue()[2] ),
                                    osg::vec3( pDomLookat->getValue()[3],
                                               pDomLookat->getValue()[4],
                                               pDomLookat->getValue()[5] ),
                                    osg::vec3( pDomLookat->getValue()[6],
                                               pDomLookat->getValue()[7],
                                               pDomLookat->getValue()[8] ) )
            );
        }
        else if( domSkew* pDomSkew = daeSafeCast<domSkew>( pDaeElement ) )
        {
            ...
            float            shear = sin( osg::radians( s[0] ) );
            // axis of rotation
            osg::vec3        around( s[1], s[2], s[3] );
            // axis of translation
            osg::vec3        along( s[4], s[5], s[6] );

            // This maths is untested so may be transposed or negated or just completely
            // wrong.
            osg::vec3        normal  = along ^ around;
            normal                   = osg::normalize( normal );
            around                   = osg::normalize( around );
            along                   *= shear / osg::length( along );

            pTransformElement        = new osgAnimation::StackedMatrixElement(
                pDomSkew->getSid() ? pDomSkew->getSid() : "",
                osg::dmat4( normal.x * along.x + 1.0F,
                            normal.x * along.y,
                            normal.x * along.z,
                            0.0F,
                            normal.y * along.x,
                            normal.y * along.y + 1.0F,
                            normal.y * along.z,
                            0.0F,
                            normal.z * along.x,
                            normal.z * along.y,
                            normal.z * along.z + 1.0F,
                            0.0F,
                            0.0F,
                            0.0F,
                            0.0F,
        ...
        if( pTransformElement )
        {
            daeElementDomChannelMap::iterator iter =
                _daeElementDomChannelMap.find( pDaeElement );
            if( iter != _daeElementDomChannelMap.end() )
            {
                ...
                if( !pNodeCallback )
                {
                    std::string name = node->getId()  ? node->getId()
                                     : node->getSid() ? node->getSid()
                                                      : "";
                    resultNode->setDataVariance( osg::Object::DataVariance::DYNAMIC );
                ...
                do
                {
                    ...
                    ++iter;
                         ...
                         _daeElementDomChannelMap.end() &&
                         iter->first == pDaeElement );
            }
            else if( pLastStaticTransformElement )
            {
                ...
                if( !pTransformElement->isIdentity() )
                {
                    ...
                    osg::dmat4 matrix = pLastStaticTransformElement->getAsMatrix();
                    pTransformElement->applyToMatrix( matrix );
                    pLastStaticTransformElement =
                        new osgAnimation::StackedMatrixElement( "collapsed", matrix );
                }
            }
    ...
    for( size_t i = 0; i < transformElements.size(); i++ )
    {
        ...
        {
            pUpdateStackedTransform->getStackedTransforms().push_back(
                transformElements[i].get()
            );
        }
    }
    ...
    // GL_RESCALE_NORMAL and GL_NORMALIZE removed: not available in core profile

    return resultNode;
}

osg::Group*
daeReader::processOsgDOFTransform( domTechnique* teq )
{
    ...
    {
        dof->setHPRMultOrder(
            ( osgSim::DOFTransform::MultOrder )parseString<int>( any->getValue() )
        );
    }
    else
    {
    ...
    if( any )
    {
        osg::dmat4 mat = parseMatrixString( any->getValue() );
        dof->setPutMatrix( mat );
        dof->setInversePutMatrix( osg::inverse( mat ) );
    }
    else
    {
