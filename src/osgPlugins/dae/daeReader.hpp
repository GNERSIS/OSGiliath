...
namespace osgDAE
{
    ...
    inline daeElement*
    getElementFromURI( daeURI& uri )
    {
        if( uri.getState() ==
            daeURI::uri_loaded ||
            uri.getState() == daeURI::uri_pending )
        {
            uri.resolveElement();
        }
    ...
    inline daeElement*
    getElementFromIDRef( daeIDRef& idref )
    {
        if( idref.getState() ==
            daeIDRef::id_loaded ||
            idref.getState() == daeIDRef::id_pending )
        {
    ...
    template<typename TInputArray,
             typename TInputType>
    bool
    findInputSourceBySemantic( TInputArray& inputs,
                               const char*  semantic,
                               daeElement*& element,
    ...
    template<typename T>
    T
    parseString( const std::string& valueAsString )
    {
    ...
    inline osg::vec3
    parseVec3String( const std::string& valueAsString )
    {
        std::stringstream str;
        str << valueAsString;
        osg::vec3 result;
    ...
    inline osg::dmat4
    parseMatrixString( const std::string& valueAsString )
    {
        std::stringstream str;
        str << valueAsString;
        osg::dmat4 result;
    ...
    */
    class daeReader
    {
        public:
            ...
            enum TessellateMode
            {
                TESSELLATE_NONE,    ///< Do not tessellate at all (Polygons are stored as
                                    ///< GL_POLYGON - not suitable for concave polygons)
                TESSELLATE_POLYGONS_AS_TRIFAN,    ///< Tessellate the old way,
                                                  ///< interpreting polygons as triangle
                                                  ///< fans (faster, but does not work
                                                  ///< for concave polygons)
                TESSELLATE_POLYGONS,    ///< Use full tessellation of polygons (slower,
            ...
            struct Options
            {
                    ...
                    int  precisionHint;    ///< Precision hint flags, as specified in
                                           ///< osgDB::Options::PrecisionHint
            ...
            bool
            convert( std::istream& fin );
            bool
            convert( const std::string& fileURI );
            ...
            const std::string&
            getAssetUnitName() const
            {
                return _assetUnitName;
            }

            float
            getAssetUnitMeter() const
            {
                return _assetUnitMeter;
            }

            domUpAxisType
            getAssetUpAxis() const
            {
            ...
            enum AuthoringTool
            {
                ...
                DAZ_STUDIO,
                FBX_CONVERTER,
                AUTODESK_3DS_MAX = FBX_CONVERTER,    // 3ds Max exports to DAE via
            ...
            class TextureParameters
            {
                public:
                    ...
                    TextureParameters() :
                        wrap_s( osg::Texture::REPEAT ),
                        wrap_t( osg::Texture::REPEAT ),
                        filter_min( osg::Texture::LINEAR_MIPMAP_LINEAR ),
                        filter_mag( osg::Texture::LINEAR ),
                        transparent( false ),
                        opaque( FX_OPAQUE_ENUM_A_ONE ),
                        transparency( 1.0F )
                    {
                    }

                    bool
                    operator<( const TextureParameters& rhs ) const
                    {
            ...
            typedef std::map<domMaterial*, osg::ref_ptr<osg::StateSet>>
                domMaterialStateSetMap;
            typedef std::map<std::string, osg::ref_ptr<osg::StateSet>>
                                                            MaterialStateSetMap;
            typedef std::multimap<daeElement*, domChannel*> daeElementDomChannelMap;
            typedef std::map<domChannel*, osg::ref_ptr<osg::Callback>>
                domChannelOsgAnimationUpdateCallbackMap;
            typedef std::map<domNode*, osg::ref_ptr<osgAnimation::Bone>>
                domNodeOsgBoneMap;
            typedef std::map<domNode*, osg::ref_ptr<osgAnimation::Skeleton>>
                domNodeOsgSkeletonMap;
            typedef std::map<TextureParameters, osg::ref_ptr<osg::Texture2D>>
                TextureParametersMap;
            typedef std::map<std::pair<const osg::StateSet*, TextureUnitUsage>,
                             std::string>
                                                                  TextureToCoordSetMap;
            ...
            typedef std::multimap<osgAnimation::Target*, osg::ref_ptr<ChannelPart>>
                TargetChannelPartMap;
            typedef std::multimap<std::pair<const domMesh*, size_t>,
                                  std::pair<osg::ref_ptr<osg::Geometry>, GLuint>>
                OldToNewIndexMap;

        private:

            bool
            processDocument( const std::string& );
            void
            clearCaches();
            ...
            turnZUp();
            osg::Group*
            processVisualScene( domVisual_scene* scene );
            osg::Node*
            processNode( domNode* node,
                         bool     skeleton );
            osg::Transform*
            processOsgMatrixTransform( domNode* node,
                                       bool     isBone );

            template<typename T>
            inline void
            getTransparencyCounts( daeDatabase*,
                                   int& zero,
                                   int& one ) const;
            ...
            bool
            findInvertTransparency( daeDatabase* ) const;

            osgAnimation::BasicAnimationManager*
            processAnimationLibraries( domCOLLADA* document );
            void
            ...
            void
            processAnimationMap( const TargetChannelPartMap&,
                                 osgAnimation::Animation* pOsgAnimation );
            ChannelPart*
            processSampler( domChannel* pDomChannel,
                            SourceMap&  sources );
            void
            processAnimationChannels( domAnimation*         pDomAnimation,
                                      TargetChannelPartMap& tcm );
            void
            processChannel( domChannel*           pDomChannel,
                            SourceMap&            sources,
                            TargetChannelPartMap& tcm );
            void
            extractTargetName( const std::string&,
                               std::string&,
                               std::string&,
                               std::string& );
            ...
            osg::Group*
            processExtras( domNode* node );
            void
            processNodeExtra( osg::Node* osgNode,
                              domNode*   node );
            domTechnique*
            getOpenSceneGraphProfile( domExtra* extra );
            void
            processAsset( domAsset* node );

            osg::Group*
            processOsgSwitch( domTechnique* teq );
            osg::Group*
            processOsgMultiSwitch( domTechnique* teq );
            osg::Group*
            processOsgLOD( domTechnique* teq );
            osg::Group*
            processOsgDOFTransform( domTechnique* teq );
            osg::Group*
            processOsgSequence( domTechnique* teq );

            // geometry processing
            osg::Geode*
            getOrCreateGeometry( domGeometry*       geom,
                                 domBind_material*  pDomBindMaterial,
                                 const osg::Geode** ppOriginalGeode = NULL );
            osgAnimation::Bone*
            getOrCreateBone( domNode* pDomNode );
            osgAnimation::Skeleton*
            getOrCreateSkeleton( domNode* pDomNode );
            osg::Geode*
            processInstanceGeometry( domInstance_geometry* ig );

            osg::Geode*
            processMesh( domMesh* pDomMesh );
            osg::Geode*
            processConvexMesh( domConvex_mesh* pDomConvexMesh );
            osg::Geode*
            processSpline( domSpline* pDomSpline );
            osg::Geode*
            processGeometry( domGeometry* pDomGeometry );
            ...
            void
            processSkeletonSkins( domNode* skeletonRoot,
                                  const domInstance_controllerList& );
            void
            processSkin( domSkin* pDomSkin,
                         domNode* skeletonRoot,
                         osgAnimation::Skeleton*,
                         domBind_material* pDomBindMaterial );
            osg::Node*
            processMorph( domMorph*         pDomMorph,
                          domBind_material* pDomBindMaterial );
            osg::Node*
            processInstanceController( domInstance_controller* ictrl );

            template<typename T>
            void
            processSinglePPrimitive( osg::Geode*    geode,
            ...
            template<typename T>
            void
            processMultiPPrimitive( osg::Geode*    geode,
                                    const domMesh* pDomMesh,
                                    const T*       group,
                                    SourceMap&     sources,
                                    GLenum         mode );
            ...
            void
            processPolylist( osg::Geode*        geode,
                             const domMesh*     pDomMesh,
            ...
            template<typename T>
            void
            processPolygons( osg::Geode*    geode,
                             const domMesh* pDomMesh,
                             const T*       group,
                             SourceMap&     sources,
                             GLenum         mode,
            ...
            void
            resolveMeshArrays( const domP_Array&,
            ...
            void
            processBindMaterial( domBind_material* bm,
                                 domGeometry*      geom,
                                 osg::Geode*       geode,
                                 osg::Geode*       cachedGeode );
            void
            processMaterial( osg::StateSet* ss,
                             domMaterial*   mat );
            void
            processEffect( osg::StateSet* ss,
                           domEffect*     effect );
            void
            processProfileCOMMON( osg::StateSet*     ss,
                                  domProfile_COMMON* pc );
            bool
            processColorOrTextureType( const osg::StateSet*,
            ...
            void
            processTransparencySettings( domCommon_transparent_type*    ctt,
                                         domCommon_float_or_param_type* pTransparency,
                                         osg::StateSet*,
                                         osg::Material* material,
                                         unsigned int   diffuseTextureUnit );
            bool
            GetFloat4Param( xsNCName   Reference,
                            domFloat4& f4 ) const;
            bool
            GetFloatParam( xsNCName  Reference,
                           domFloat& f ) const;

            std::string
            processImagePath( const domImage* ) const;
            osg::Image*
            processImageTransparency( const osg::Image*,
                                      domFx_opaque_enum,
                                      float transparency ) const;
            osg::Texture2D*
            processTexture( domCommon_color_or_texture_type_complexType::domTexture* tex,
                            const osg::StateSet*,
                            TextureUnitUsage,
                            domFx_opaque_enum  = FX_OPAQUE_ENUM_A_ONE,
                            float transparency = 1.0F );
            bool
            copyTextureCoordinateSet( const osg::StateSet*        ss,
            ...
            // scene objects
            osg::Node*
            processLight( domLight* dlight );
            osg::Node*
            processCamera( domCamera* dcamera );

            domNode*
            getRootJoint( domNode* ) const;
            domNode*
            findJointNode( daeElement* searchFrom,
                           domInstance_controller* ) const;
            domNode*
            findSkeletonNode( daeElement* searchFrom,
                              domInstance_controller* ) const;
            ...
            bool
            isJoint( const domNode* node ) const
            {
        ...
        private:
            ...
            /// Maps an animated element to a domchannel to quickly find which animation
            /// influence this element
            // TODO a single element can be animated by multiple channels (with different
            // members like translate.x or morphweights(2) )
            daeElementDomChannelMap     _daeElementDomChannelMap;
            /// Maps a domchannel to an animationupdatecallback
            domChannelOsgAnimationUpdateCallbackMap
                                       _domChannelOsgAnimationUpdateCallbackMap;
