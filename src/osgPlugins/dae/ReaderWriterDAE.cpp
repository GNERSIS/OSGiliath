 ...
 * Utility functions for osgPlugins: SERIALIZER, lock, iss.
 */
                           ...
                           const osgDB::ReaderWriter::Options* options ) const
{
    ...
    if( options )
    {
        ...
        while( iss >> opt )
        {
            ...
            {
                pluginOptions.tessellateMode =
                    osgDAE::daeReader::TESSELLATE_POLYGONS_AS_TRIFAN;
            }
    ...
    smart_ptr<DAE> scopedDae(
        bOwnDAE ? pDAE : NULL
    );    // Deallocates locally created structure at scope exit
                           ...
                           const osgDB::ReaderWriter::Options* options ) const
{
    ...
    pluginOptions.options =
        options ? osg::clone( options, osg::CopyOp::SHALLOW_COPY ) : new Options;
    pluginOptions.options->getDatabasePathList().push_front(
                            ...
                            const osgDB::ReaderWriter::Options* options ) const
{
    ...
    std::string                srcDirectory(
        osgDB::getFilePath( node.getName().empty() ? fname : node.getName() )
    );    // Base dir when relativising images paths
    if( options )
    {
        pDAE                       = ( DAE* )options->getPluginData( "DAE" );

        const std::string& baseDir = options->getPluginStringData(
            "baseImageDir"
        );    // Rename "srcModelPath" (and call getFilePath() on it)?
        if( !baseDir.empty() )
        {
            srcDirectory = baseDir;
        }

        const std::string& relativiseImagesPathNbUpDirs =
            options->getPluginStringData( "DAE-relativiseImagesPathNbUpDirs" );
        ...
        // Sukender's note: I don't know why DAE seems to accept comma-sparated options
        // instead of space-separated options as other ReaderWriters. However, to avoid
        // breaking compatibility, here's a workaround:
        ...
        while( std::getline( iss, opt, ',' ) )
        {
            ...
            {
                pluginOptions.linkOrignialTextures = true;
                pluginOptions.forceTexture         = false;
            }
            else if( opt == "daeLinkOriginalTexturesForce" )
            {
                pluginOptions.linkOrignialTextures = true;
                pluginOptions.forceTexture         = true;
            }
            else if( opt == "daeNamesUseCodepage" )
            {
                pluginOptions.namesUseCodepage = true;
            }
            else if( opt == "daeRenameIds" )
            {
                pluginOptions.renameIds = true;
            }
            else if( !opt.empty() )
            {
                OSG_NOTICE << std::endl
    ...
    osg::NodeVisitor::TraversalMode traversalMode =
        pluginOptions.writeExtras ? osg::NodeVisitor::TRAVERSE_ALL_CHILDREN
                                  : osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN;

    osgDAE::daeWriter daeWriter( pDAE,
                                 fileURI,
                                 osgDB::getFilePath( fname ),
                                 srcDirectory,
                                 options,
                                 traversalMode,
                                 &pluginOptions );
    daeWriter.setRootNode( node );
    node.accept( static_cast<osg::ConstNodeVisitor&>( daeWriter ) );
...
std::string
ReaderWriterDAE::ConvertFilePathToColladaCompatibleURI( const std::string& FilePath )
{
    ...
    // Unfortunately, cdom::nativePathToUri() does not convert '#' characters to "%23" as
    // expected. So having /a/#b/c will generate a wrong conversion, as '#' will be
    // misinterpreted as an URI fragment. Here are listed all special chars, but only #
    // was found problematic. I (Sukender) tested #{}^~[]`;@=&$ under Windows. Uncomment
    // lines if you find issues with some other special characters.
...
std::string
ReaderWriterDAE::ConvertColladaCompatibleURIToFilePath( const std::string& uri )
{
...
#else
    std::string path(
        osgDB::convertStringFromCurrentCodePageToUTF8( cdom::uriToNativePath( uri ) )
    );
#endif
    replace( path, "%23", "#" );
    return path;
