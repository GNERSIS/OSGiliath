...
daeReader::Options::Options() :
    ...
    tessellateMode(
        TESSELLATE_POLYGONS_AS_TRIFAN
    )    // Use old tessellation behaviour as default
{
}
...
bool
daeReader::processDocument( const std::string& fileURI )
{
    ...
    if( _document->getAsset() )
    {
             ...
             CurrentContributor++ )
        {
            if( ContributorArray[CurrentContributor]->getAuthoring_tool() )
            {
                ...
                xsString   Tool          = ContributorArray[CurrentContributor]
                                               ->getAuthoring_tool()
                                               ->getValue();
    ...
    if( daeDatabase* database = _dae->getDatabase() )
    {
        ...
        // build a std::map for lookup if Group or PositionAttitudeTransform should be
        // created, i.e, make it easy to check if a instance_rigid_body targets a visual
        // node
        domInstance_rigid_body* pDomInstanceRigidBody;
        count =
            database->getElementCount( NULL, COLLADA_TYPE_INSTANCE_RIGID_BODY, NULL );
        for( int i = 0; i < count; i++ )
        {
            result = database->getElement( &colladaElement,
                                           i,
                                           NULL,
            ...
            if( result == DAE_OK )
            {
                ...
                {
                    domNode* node = daeSafeCast<domNode>(
                        pDomInstanceRigidBody->getTarget().getElement()
                    );
                    if( node && node->getId() )
                    {
        ...
        {
            result =
                database->getElement( &colladaElement, i, NULL, COLLADA_TYPE_CHANNEL );
            ...
            if( result == DAE_OK )
            {
                ...
                if( pDomChannel )
                {
                    ...
                    if( pDaeElement )
                    {
                        _daeElementDomChannelMap.insert(
                            daeElementDomChannelMap::value_type( pDaeElement,
                                                                 pDomChannel )
                        );
                    }
                    else
        ...
        for( size_t controller = 0; controller < instanceControllers.size();
             ++controller )
        {
            domInstance_controller* pInstanceController =
                instanceControllers[controller];

            domController* pDomController = daeSafeCast<domController>(
                getElementFromURI( pInstanceController->getUrl() )
            );
            if( !pDomController )
            {
            ...
            for( size_t i = 0; i < inputURIs.getCount(); i++ )
            {
                ...
                {
                    pDomJointsSource = daeSafeCast<domSource>(
                        getElementFromURI( inputURIs[i]->getSource() )
                    );
                    if( !pDomJointsSource )
                    {
                        OSG_WARN << "Could not find skin joints source '"
                                 << inputURIs[i]->getSource().getURI() << "'"
                                 << std::endl;
                    }
                }
            }
            ...
            else if( domIDREF_array* pDomIDREFs = pDomJointsSource->getIDREF_array() )
            {
                ...
                {
                    if( domNode* pJoint = daeSafeCast<domNode>(
                            getElementFromIDRef( pDomIDREFs->getValue().get( i ) )
                        ) )
                    {
                        _jointSet.insert( pJoint );
                    }
...
bool
daeReader::convert( std::istream& fin )
{
    clearCaches();
...
bool
daeReader::convert( const std::string& fileURI )
{
    clearCaches();
...
void
daeReader::addChild( osg::Group* group,
                     osg::Node*  node )
{
...
daeReader::turnZUp()
{
    ...
    if( _assetUp_axis != UPAXISTYPE_Z_UP )
    {
        ...
        {
            pat->setAttitude( osg::quat( osg::radians( 90.0F ),
...
osg::Group*
daeReader::processVisualScene( domVisual_scene* scene )
{
    osg::Group* retVal;
...
osg::Group*
daeReader::processExtras( domNode* node )
{
...
void
daeReader::processNodeExtra( osg::Node* osgNode,
                             domNode*   node )
{
    // See if one of the extras contains OpenSceneGraph specific information
    unsigned int numExtras = node->getExtra_array().getCount();
                ...
                if( any )
                {
                         ...
                         currChild++ )
                    {
                        ...
                        if( child )
                        {
                            ...
                            {
                                OSG_WARN << "Child of element 'Descriptions' is not of "
                                            "type 'Description'"
                                         << std::endl;
                            }
                        }
                        ...
                        {
                            OSG_WARN << "Element 'Descriptions' does not contain "
                                        "expected elements."
                                     << std::endl;
                        }
                    }
...
domTechnique*
daeReader::getOpenSceneGraphProfile( domExtra* extra )
{
...
// 0..* <extra>
osg::Node*
daeReader::processNode( domNode* node,
                        bool     skeleton )
{
    // First we need to determine what kind of OSG node we need
    // If there exist any of the <lookat>, <matrix>, <rotate>, <scale>, <skew>,
    // <translate> elements or if a COLLADA_TYPE_INSTANCE_RIGID_BODY targets this node we
    // need a MatrixTransform
