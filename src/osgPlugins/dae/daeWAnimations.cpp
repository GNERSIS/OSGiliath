...
void
daeWriter::writeAnimations( osg::Node& wnode )
{
    const std::string nodeNameUTF(
        _pluginOptions.namesUseCodepage
            ? osgDB::convertStringFromCurrentCodePageToUTF8( wnode.getName() )
            : wnode.getName()
    );
    osg::Callback* ncb = wnode.getUpdateCallback();
    if( ncb )
    {
        osgAnimation::AnimationManagerBase* am =
            dynamic_cast<osgAnimation::AnimationManagerBase*>( ncb );
        if( am )
        {
            // Create library of animations if not existing yet
            if( !_domLibraryAnimations )
            {
                _domLibraryAnimations = daeSafeCast<domLibrary_animations>(
                    dom->add( COLLADA_ELEMENT_LIBRARY_ANIMATIONS )
                );
            }
            ...
            for( size_t ai = 0; ai < animationList.size(); ai++ )
            {
                domAnimation* pDomAnimation = daeSafeCast<domAnimation>(
                    _domLibraryAnimations->add( COLLADA_ELEMENT_ANIMATION )
                );
                ...
                for( size_t j = 0; j < animationChannels.size(); j++ )
                {
                    ...
                    // Wrap each animation channel into it's own child <animation> when
                    // more than 1 channel
                    if( animationChannels.size() > 1 )
                    {
                        pDomAnimation = daeSafeCast<domAnimation>(
                            pMainDomAnimation->add( COLLADA_ELEMENT_ANIMATION )
                        );
                    ...
                    std::string interpolationSourceName( channelNameUTF +
                                                         "_interpolation" );

                    // Fill dom sources based on sampler
                    osgAnimation::Sampler* animationSampler = channel->getSampler();
                    osgAnimation::KeyframeContainer* kfc =
                        animationSampler->getKeyframeContainer();
                    int                                  size        = kfc->size();
                    ...
                    osgAnimation::Vec3KeyframeContainer* v3kc =
                        dynamic_cast<osgAnimation::Vec3KeyframeContainer*>( kfc );
                    if( v3kc )
                    {
                        valueStride = 3;
                        for( size_t i = 0; i < v3kc->size(); i++ )
                        {
                            ...
                            {
                                frameValues.append( osg::degrees( vec.x ) );
                                frameValues.append( osg::degrees( vec.y ) );
                                frameValues.append( osg::degrees( vec.z ) );
                            }
                            else
                            {
                    ...
                    osgAnimation::FloatKeyframeContainer* fkc =
                        dynamic_cast<osgAnimation::FloatKeyframeContainer*>( kfc );
                    if( fkc )
                    {
                        valueStride = 1;
                    ...
                    // time values
                    domSource* pDomSource = daeSafeCast<domSource>(
                        pDomAnimation->add( COLLADA_ELEMENT_SOURCE )
                    );
                    pDomSource->setId( inputSourceName.c_str() );

                    domFloat_array* pDomFloatArray = daeSafeCast<domFloat_array>(
                        pDomSource->add( COLLADA_ELEMENT_FLOAT_ARRAY )
                    );
                    ...
                    domSource::domTechnique_common* pDomSourceTechniqueCommon =
                        daeSafeCast<domSource::domTechnique_common>(
                            pDomSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
                        );
                    ...
                    domParam* pDomParam = daeSafeCast<domParam>(
                        pDomAccessor->add( COLLADA_ELEMENT_PARAM )
                    );
                    pDomParam->setName( "TIME" );
                    pDomParam->setType( COLLADA_TYPE_FLOAT );

                    // <source> interpolator type
                    pDomSource = daeSafeCast<domSource>(
                        pDomAnimation->add( COLLADA_ELEMENT_SOURCE )
                    );
                    pDomSource->setId( interpolationSourceName.c_str() );

                    domName_array* pDomNameArray = daeSafeCast<domName_array>(
                        pDomSource->add( COLLADA_ELEMENT_NAME_ARRAY )
                    );
                    std::string interpolationArrayName =
                        interpolationSourceName + "_array";
                    pDomNameArray->setId( interpolationArrayName.c_str() );
                    pDomNameArray->setCount( size );
                    pDomNameArray->setValue( interpolationValues );

                    // <technique_common>
                    pDomSourceTechniqueCommon =
                        daeSafeCast<domSource::domTechnique_common>(
                            pDomSource->add( COLLADA_ELEMENT_TECHNIQUE_COMMON )
                        );
                    ...
                    pDomParam = daeSafeCast<domParam>(
                        pDomAccessor->add( COLLADA_ELEMENT_PARAM )
                    );
                    pDomParam->setName( "INTERPOLATION" );
                    pDomParam->setType( COLLADA_TYPE_NAME );

                    // Split up access to the euler float array into three sources,
                    // because we need to target three separate transforms
                    if( channelNameUTF.find( "euler" ) != std::string::npos )
                    {
                        pDomSource = daeSafeCast<domSource>(
                            pDomAnimation->add( COLLADA_ELEMENT_SOURCE )
                        ...
                        pDomFloatArray = daeSafeCast<domFloat_array>(
                            pDomSource->add( COLLADA_ELEMENT_FLOAT_ARRAY )
                        );
                        std::string outputArrayNameX = outputSourceNameX + "_array";
                        pDomFloatArray->setId( outputArrayNameX.c_str() );

                        // TODO, flexible handling of different keyframe types, see osg
                        // exporter for inspiration
                        ...
                        {
                            domSampler* pDomSampler = daeSafeCast<domSampler>(
                                pDomAnimation->add( COLLADA_ELEMENT_SAMPLER )
                            );
                            ...
                            {
                                domInputLocal* pDomInput = daeSafeCast<domInputLocal>(
                                    pDomSampler->add( COLLADA_ELEMENT_INPUT )
                                );
                                pDomInput->setSemantic( COMMON_PROFILE_INPUT_INPUT );
                                url = "#" + inputSourceName;
                                pDomInput->setSource( url.c_str() );

                                pDomInput = daeSafeCast<domInputLocal>(
                                    pDomSampler->add( COLLADA_ELEMENT_INPUT )
                                );
                            ...
                            domChannel* pDomChannel = daeSafeCast<domChannel>(
                                pDomAnimation->add( COLLADA_ELEMENT_CHANNEL )
                            );
                            url = "#" + sourceNameX;
                            ...
                            // based on the type of updateCallback we can determine the
                            // transform element to target eg. "nodeName/translation"
                            osg::Node*  node =
                                _animatedNodeCollector.getTargetNode( targetName );
                            if( node )
                            {
                                std::string domChannelTargetName =
                                    nodeNameUTF + "/rotateX.ANGLE";
                                pDomChannel->setTarget( domChannelTargetName.c_str() );
                            }
                            else
                            {
                                OSG_WARN << "Could not find animation target '"
                                         << targetName << "'" << std::endl;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if( node )
                        {
                            ...
                            else if( channelNameUTF.find( "scale" ) !=
                                     std::string::npos )
                            {
                        ...
                        {
                            OSG_WARN << "Could not find animation target '" << targetName
                                     << "'" << std::endl;
                        }
                    }
                }
