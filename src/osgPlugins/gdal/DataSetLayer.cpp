 ...
 * Utility functions for osgPlugins: _dataset, Inherit.
 */
...
DataSetLayer::DataSetLayer( const DataSetLayer& dataSetLayer,
                            const osg::CopyOp&  copyop ) :
    Inherit( dataSetLayer,
             copyop ),
    _gdalReader( dataSetLayer._gdalReader )
{
...
DataSetLayer::open()
{
    ...
    _dataset =
        static_cast<GDALDataset*>( GDALOpen( getFileName().c_str(), GA_ReadOnly ) );
...
unsigned int
DataSetLayer::getNumColumns() const
{
    return _dataset != 0 ? _dataset->GetRasterXSize() : 0;
}

unsigned int
DataSetLayer::getNumRows() const
{
    return _dataset != 0 ? _dataset->GetRasterYSize() : 0;
}

osgTerrain::ImageLayer*
DataSetLayer::extractImageLayer( unsigned int sourceMinX,
                                 ...
                                 unsigned int targetHeight )
{
    ...
    osgDB::ReaderWriter::ReadResult result =
        _gdalReader->readImage( getFileName(), imageOptions.get() );
...
void
DataSetLayer::setGdalReader( const osgDB::ReaderWriter* rw )
{
    _gdalReader = const_cast<osgDB::ReaderWriter*>( rw );
}

void
DataSetLayer::setUpLocator()
{
    if( !isOpen() )
    {
        return;
    ...
    if( _dataset->GetGeoTransform( geoTransform ) == CE_None )
    {
        ...
        // shift the transform to the middle of the cell if a raster interpreted as
        // vector
        ...
        locator->setTransform( osg::scale( static_cast<double>( nPixels - 1 ),
                                           static_cast<double>( nLines - 1 ),
                                           1.0 ) *
                               matrix );
    ...
    else if( _dataset->GetGCPCount() > 0 && _dataset->GetGCPProjection() )
    {
        ...
        /* -------------------------------------------------------------------- */
        void* hTransformArg = GDALCreateGenImgProjTransformer( _dataset,
                                                               pszSourceSRS,
                                                               NULL,
                                                               pszSourceSRS,
                                                               TRUE,
        ...
        matrix.set( adfDstGeoTransform[1],
                    adfDstGeoTransform[4],
                    0.0,
                    0.0,
    ...
    {
        OSG_INFO << "DataSetLayer::setUpLocator(), No GeoTransform or GCP's - unable to "
