 ...
 * ReaderWriterGDAL, derived from ReaderWriter.
 * Provides: supportsExtension, className, readObject, readObject, lock, initGDAL.
 */
#include "DataSetLayer.hpp"
...
class ReaderWriterGDAL : public osgDB::ReaderWriter
{
    public:
                    ...
                    const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            std::lock_guard<std::recursive_mutex> lock( _serializerMutex );
            ...
            // open a DataSetLayer.
            osg::ref_ptr<GDALPlugin::DataSetLayer> dataset =
                new GDALPlugin::DataSetLayer( fileName );
                   ...
                   const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( osgDB::equalCaseInsensitive( osgDB::getFileExtension( fileName ),
                                             "gdal" ) )
            {
            ...
            std::lock_guard<std::recursive_mutex> lock( _serializerMutex );
            return const_cast<ReaderWriterGDAL*>( this )->local_readImage( fileName,
                                                                           options );
        }
                         ...
                         const osgDB::ReaderWriter::Options* options ) const
        {
            ...
            if( osgDB::equalCaseInsensitive( osgDB::getFileExtension( fileName ),
                                             "gdal" ) )
            {
                return readHeightField( osgDB::getNameLessExtension( fileName ),
                                        options );
            }

            std::lock_guard<std::recursive_mutex> lock( _serializerMutex );
            return const_cast<ReaderWriterGDAL*>( this )->local_readHeightField(
                fileName,
                options
            );
        }
        ...
        virtual ReadResult
        local_readImage( const std::string&                  file,
                         const osgDB::ReaderWriter::Options* options )
        {
            ...
            // hack to get around
            bool ecwLoad =
                osgDB::equalCaseInsensitive( osgDB::getFileExtension( file ), "ecw" );
            ...
            int destX        = 0;
            int destY        = 0;
            int destWidth    = std::min( dataWidth, 4'096 );
            int destHeight   = std::min( dataHeight, 4'096 );
            // int destWidth = std::min(dataWidth,4096);
            // int destHeight = std::min(dataHeight,4096);

            osg::ref_ptr<osgDB::ImageOptions::TexCoordRange> texCoordRange;

            osgDB::ImageOptions* imageOptions = dynamic_cast<osgDB::ImageOptions*>(
                const_cast<osgDB::ReaderWriter::Options*>( options )
            );
            if( imageOptions )
            {
                ...
                switch( imageOptions->_sourceImageWindowMode )
                {
                    case( osgDB::ImageOptions::RATIO_WINDOW ) :
                        {
                            double desiredX = ( double )dataWidth *
                                              imageOptions->_sourceRatioWindow.windowX;
                            double desiredY = ( double )dataHeight *
                                              imageOptions->_sourceRatioWindow.windowY;
                            double desiredWidth =
                                ( double )dataWidth *
                                imageOptions->_sourceRatioWindow.windowWidth;
                            double desiredHeight =
                                ( double )dataHeight *
                                imageOptions->_sourceRatioWindow.windowHeight;

                            windowX =
                                std::max( ( int )( floor( desiredX ) ) - margin, 0 );
                            windowY =
                                std::max( ( int )( floor( desiredY ) ) - margin, 0 );
                            windowWidth =
                                std::min( ( int )( ceil( desiredX + desiredWidth ) ) +
                                              margin,
                                          dataWidth ) -
                                windowX;
                            windowHeight =
                                std::min( ( int )( ceil( desiredY + desiredHeight ) ) +
                                              margin,
                                          dataHeight ) -
                                windowY;
                            ...
                            texCoordRange->set(
                                ( desiredX - ( double )windowX ) / ( double )windowWidth,
                                ( ( double )( windowY + windowHeight ) -
                                  ( desiredY + desiredHeight ) ) /
                                    ( double )windowHeight,
                                ( desiredWidth ) / ( double )windowWidth,
                                ( desiredHeight ) / ( double )windowHeight
                            );
                            OSG_INFO << "tex coord range " << texCoordRange->_x << " "
                                     << texCoordRange->_y << " " << texCoordRange->_w
                                     << " " << texCoordRange->_h << std::endl;
                        }
                        break;
                ...
                {
                    case( osgDB::ImageOptions::RATIO_WINDOW ) :
                        destX = ( unsigned int )( floor(
                            ( double )dataWidth *
                            imageOptions->_destinationRatioWindow.windowX
                        ) );
                        destY = ( unsigned int )( floor(
                            ( double )dataHeight *
                            imageOptions->_destinationRatioWindow.windowY
                        ) );
                        destWidth =
                            ( unsigned int )( ceil(
                                ( double )dataWidth *
                                ( imageOptions->_destinationRatioWindow.windowX +
                                  imageOptions->_destinationRatioWindow.windowWidth )
                            ) ) -
                            windowX;
                        destHeight =
                            ( unsigned int )( ceil(
                                ( double )dataHeight *
                                ( imageOptions->_destinationRatioWindow.windowY +
                                  imageOptions->_destinationRatioWindow.windowHeight )
                            ) ) -
                            windowY;
                        break;
                    case( osgDB::ImageOptions::PIXEL_WINDOW ) :
            ...
            OSG_INFO << "    GetProjectionRef() " << dataset->GetProjectionRef()
                     << std::endl;
            ...
            if( dataset->GetGeoTransform( geoTransform ) == CE_None )
            {
                ...
                OSG_INFO << "        Origin = " << geoTransform[0] << " "
                         << geoTransform[3] << std::endl;
                OSG_INFO << "        Pixel X = " << geoTransform[1] << " "
                         << geoTransform[4] << std::endl;
                OSG_INFO << "        Pixel Y = " << geoTransform[2] << " "
            ...
            unsigned int    pixelFormat          = GL_RED;
            ...
            for( int b = 1; b <= numBands; ++b )
            {
                ...
                OSG_INFO << "        GetOverviewCount() = " << band->GetOverviewCount()
                         << std::endl;
                OSG_INFO << "        GetColorTable() = " << band->GetColorTable()
                         << std::endl;
                OSG_INFO << "        DataTypeName() = "
                         << GDALGetDataTypeName( band->GetRasterDataType() )
                         << std::endl;
                OSG_INFO << "        ColorIntepretationName() = "
                ...
                if( bandPalette )
                {
                    OSG_INFO << "        Palette Interpretation: "
            ...
            if( bandRed && bandGreen && bandBlue )
            {
                if( bandAlpha )
                {
                    ...
                    bandGreen->RasterIO(
                        ...
                        windowWidth,
                        windowHeight,
                        ( void* )( imageData + 1 * numBytesPerComponent ),
                        destWidth,
                        destHeight,
                        targetGDALType,
                        pixelSpace,
                    ...
                    bandBlue->RasterIO(
                        ...
                        windowWidth,
                        windowHeight,
                        ( void* )( imageData + 2 * numBytesPerComponent ),
                        destWidth,
                        destHeight,
                        targetGDALType,
                        pixelSpace,
                    ...
                    bandAlpha->RasterIO(
                        ...
                        windowWidth,
                        windowHeight,
                        ( void* )( imageData + 3 * numBytesPerComponent ),
                        destWidth,
                        destHeight,
                        targetGDALType,
                        pixelSpace,
            ...
            else if( bandPalette )
            {
                ...
                OSG_INFO << "numBytesPerComponent: " << numBytesPerComponent
                         << std::endl;

                bandPalette->RasterIO( GF_Read,
                                       windowX,
                                       windowY,
                                       windowWidth,
                                       windowHeight,
                ...
                for( int i = 0; i < destWidth * destHeight; i++ )
                {
                    ...
                    if( !colorEntry )
                    {
                        ...
                        // OSG_INFO << "NO COLOR ENTRY FOR COLOR " << rawImageData[i] <<
                        // std::endl;
                        imageData[4 * i + 0] = 255;
                        imageData[4 * i + 1] = 0;
                        imageData[4 * i + 2] = 0;
                    ...
                    else
                    {
                        ...
                        {
                            imageData[4 * i + 0] =
                                static_cast<unsigned char>( colorEntry->c1 * 255.0F );
                            imageData[4 * i + 1] =
                                static_cast<unsigned char>( colorEntry->c1 * 255.0F );
                            imageData[4 * i + 2] =
                                static_cast<unsigned char>( colorEntry->c1 * 255.0F );
            ...
            if( imageData )
            {
                ...
                if( texCoordRange.valid() )
                {
                    image->setUserData( texCoordRange.get() );
                }
        ...
        ReadResult
        local_readHeightField( const std::string&                  fileName,
                               const osgDB::ReaderWriter::Options* options )
        {
            ...
            const osgDB::ImageOptions*                       imageOptions =
                dynamic_cast<const osgDB::ImageOptions*>( options );
            if( imageOptions )
            {
                OSG_INFO << "Got ImageOptions" << std::endl;
            ...
            OSG_INFO << "    GetGeoTransform " << std::endl;
            OSG_INFO << "        Origin = " << geoTransform[0] << " " << geoTransform[3]
                     << std::endl;
            OSG_INFO << "        Pixel X = " << geoTransform[1] << " " << geoTransform[4]
                     << std::endl;
            OSG_INFO << "        Pixel Y = " << geoTransform[2] << " " << geoTransform[5]
                     << std::endl;
            ...
            OSG_INFO << "BottomLeft  " << BottomLeft[0] << "\t" << BottomLeft[1]
                     << std::endl;
            OSG_INFO << "BottomRight " << BottomRight[0] << "\t" << BottomRight[1]
                     << std::endl;
            OSG_INFO << "TopRight    " << TopRight[0] << "\t" << TopRight[1]
                     << std::endl;
            ...
            for( int b = 1; b <= numBands; ++b )
            {
                ...
                OSG_INFO << std::endl;
                OSG_INFO << "        GetNoDataValue() = " << band->GetNoDataValue()
                         << std::endl;
                ...
                OSG_INFO << "        GetUnitType() = '" << band->GetUnitType() << "'"
                         << std::endl;
            ...
            if( bandSelected )
            {
                ...
                bandSelected->RasterIO( GF_Read,
                                        windowX,
                                        windowY,
                                        windowWidth,
                                        windowHeight,
                                        ( void* )( &( hf->getHeightList().front() ) ),
                                        destWidth,
                                        destHeight,
                                        GDT_Float32,
                                        0,
                ...
                hf->setXInterval( sqrt(
                    geoTransform[1] * geoTransform[1] + geoTransform[2] * geoTransform[2]
                ) );
                hf->setYInterval( sqrt(
                    geoTransform[4] * geoTransform[4] + geoTransform[5] * geoTransform[5]
                ) );

                hf->setRotation( osg::quat( rotation, osg::dvec3( 0.0, 0.0, 1.0 ) ) );
