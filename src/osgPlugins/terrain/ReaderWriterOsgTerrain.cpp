...
class ReaderWriterTerrain : public osgDB::ReaderWriter
{
    public:
        ...
        ReaderWriterTerrain()
        {
            supportsExtension( "osgTerrain",
                               "OpenSceneGraph terrain extension to .osg ascii format" );
        ...
        readNode( const std::string&                  file,
                  const osgDB::ReaderWriter::Options* opt ) const
        {
            ...
            osg::ref_ptr<Options> local_opt =
                opt ? static_cast<Options*>( opt->clone( osg::CopyOp::SHALLOW_COPY ) )
                    : new Options;
            ...
            while( !fr.eof() )
            {
                ...
                if( fr.matchSequence( "file %s" ) || fr.matchSequence( "file %w" ) )
                {
                    osg::ref_ptr<osg::Node> node =
                        osgDB::readRefNodeFile( fr[1].getStr() );
                ...
                if( !itrAdvanced )
                {
                    ...
                    {
                        OSG_NOTICE
                            << "Terrain file - unreconised token : " << fr[0].getStr()
                            << "" << std::endl;
                    }
                    ++fr;
                }
