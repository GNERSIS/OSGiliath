 ...
 * Utility functions for osgPlugins: plugins, defined, DELETEFILE, ReadResult.
 */
#include "ReaderWriterKTX.hpp"
...
const char*
ReaderWriterKTX::className() const
{
    return "KTX Image Reader/Writer";
}

bool
ReaderWriterKTX::correctByteOrder( KTXTexHeader& header ) const
{
...
ReaderWriterKTX::readKTXStream( std::istream& fin ) const
{
    KTXTexHeader header;
    ...
    uint32_t totalImageSize =
        fileLength - ( sizeof( KTXTexHeader ) +
    ...
    bool  byteswapImageData =
        ( header.glTypeSize > 1 ) && ( header.endianness != MyEndian );

    uint32_t                   totalOffset = 0;
    osg::Image::MipmapDataType mipmapData;

    for( uint32_t mipmapLevel = 0; mipmapLevel < header.numberOfMipmapLevels;
         mipmapLevel++ )
    {
        ...
        {
            OSG_WARN << "Failed to read mipmap: " << mipmapLevel
                     << " not enough bytes in file." << std::endl;
            delete[] totalImageData;
            return ReadResult::ERROR_IN_READING_FILE;
        }
    ...
    image->setImage( header.pixelWidth,
                     header.pixelHeight,
                     header.pixelDepth,
...
bool
ReaderWriterKTX::writeKTXStream( const osg::Image* img,
                                 std::ostream&     fout ) const
{
    ...
    fout.write( reinterpret_cast<char*>( &header ),
                sizeof( header ) ); /* write file header */
    uint32_t                 imageSize;
    int                      s = img->s();
    int                      t = img->t();
    int                      r = img->r();
    ...
    for( uint32_t mipmapLevel = 0; mipmapLevel < header.numberOfMipmapLevels;
         mipmapLevel++ )
    {
        imageSize = osg::Image::computeImageSizeInBytes( s,
                                                         t,
                                                         r,
                                                         img->getPixelFormat(),
                                                         img->getDataType(),
        ...
        fout.write( reinterpret_cast<char*>( &imageSize ), sizeof( imageSize ) );
        {
            ...
            while( imgData.valid() && ( bytesWritten < imageSize ) )
            {
                unsigned int blockSize =
                    std::min( imgData.size() - imgDataOffset, bytesToWrite );
...
osgDB::ReaderWriter::ReadResult
ReaderWriterKTX::readImage( std::istream& fin,
...
osgDB::ReaderWriter::ReadResult
ReaderWriterKTX::readImage( const std::string&                  file,
...
ReaderWriterKTX::writeObject( const osg::Object&                  object,
...
osgDB::ReaderWriter::WriteResult
ReaderWriterKTX::writeImage( const osg::Image&                   image,
                             const std::string&                  file,
                             const osgDB::ReaderWriter::Options* options ) const
{
    ...
    if( !res.success() )
    {
        ...
        OSG_WARN << "ReaderWriterKTX::writeImage Failed to write " << file << "."
                 << std::endl;
    }
    OSG_INFO << "ReaderWriterKTX::writeImage write " << file << " success;" << image.s()
             << "x" << image.t() << "x" << image.r() << std::endl;
    return res;
}
