...
class ConvertFromInventor
{
    ...
    private:
        ...
        postNode( void*             data,
                  ...
                  const SoNode*     node );
        static SoCallbackAction::Response
        preTransformSeparator( void*             data,
                               SoCallbackAction* action,
                               const SoNode*     node );
        static SoCallbackAction::Response
        postTransformSeparator( void*             data,
                                SoCallbackAction* action,
                                const SoNode*     node );
        static SoCallbackAction::Response
        preLOD( void*             data,
        ...
        preLight( void*             data,
                  ...
                  const SoNode*     node );
        static SoCallbackAction::Response
        preEnvironment( void*             data,
                        SoCallbackAction* action,
                        const SoNode*     node );
        static SoCallbackAction::Response
        preShaderProgram( void*             data,
                          SoCallbackAction* action,
                          const SoNode*     node );
        static SoCallbackAction::Response
        preRotor( void*             data,
                  ...
                  const SoNode*     node );
        static SoCallbackAction::Response
        prePendulum( void*             data,
                     SoCallbackAction* action,
                     const SoNode*     node );
        static SoCallbackAction::Response
        preShuttle( void*             data,
                    SoCallbackAction* action,
                    const SoNode*     node );
        static SoCallbackAction::Response
        preInfo( void*             data,
        ...
        static void
        addTriangleCB( void*                    data,
                       SoCallbackAction*        action,
                       const SoPrimitiveVertex* v0,
                       const SoPrimitiveVertex* v1,
                       const SoPrimitiveVertex* v2 );
        static void
        addLineSegmentCB( void*                    data,
                          SoCallbackAction*        action,
                          const SoPrimitiveVertex* v0,
                          const SoPrimitiveVertex* v1 );
        static void
        addPointCB( void*                    data,
                    SoCallbackAction*        action,
                    const SoPrimitiveVertex* v0 );

        static SoCallbackAction::Response
        restructure( void*             data,
                     SoCallbackAction* action,
                     const SoNode*     node );
        static SoCallbackAction::Response
        restructurePreNode( void*             data,
                            SoCallbackAction* action,
                            const SoNode*     node );
        static SoCallbackAction::Response
        restructurePostNode( void*             data,
                             SoCallbackAction* action,
                             const SoNode*     node );
    ...
    private:
        ...
        void
        addVertex( SoCallbackAction*        action,
                   const SoPrimitiveVertex* v,
                   int                      index );

        osg::ref_ptr<osg::StateSet>
        getStateSet( SoCallbackAction* action );

        osg::Texture2D*
        convertIVTexToOSGTex( const SoNode*     soNode,
                              SoCallbackAction* action );

        void
        transformLight( SoCallbackAction* action,
                        const SbVec3f&    vec,
                        osg::vec3&        transVec );

        // OSG doesn't seem to have a transpose function for matrices
        void
        transposeMatrix( osg::dmat4& mat );
    ...
    private:
        ...
        class IvStateItem
        {
            public:
                ...
                enum Flags
                {
                    ...
                    APPEND_AT_PUSH      = 4,
                    UPDATE_STATE        = 8,
                    UPDATE_STATE_EXCEPT_TRANSFORM =
                        0X10,    // this has the same
                ...
                // geosets). Supported types are SoTexture2 and SoVRMLImageTexture for
                // now. No multitexturing yet.
                ...
                IvStateItem( const SoNode* initiator,
                             osg::Group*   root = NULL ) :
                    ...
                    inheritedAmbientLight( SbColor( 0.2F,
                                                    0.2F,
                                                    0.2F ) ),
                    currentAmbientLight( SbColor( 0.2F,
                                                  0.2F,
                                                  0.2F ) ),
                ...
                IvStateItem( const IvStateItem&      i,
        ...
        void
        ivPushState( const SoCallbackAction* action,
                     const SoNode*           initiator,
                     const int               flags = IvStateItem::DEFAULT_FLAGS,
                     osg::Group*             root  = new osg::Group );
        void
        ivPopState( const SoCallbackAction* action,
                    const SoNode*           initator );

        void
        appendNode( osg::Node*              n,
                    const SoCallbackAction* action );
};
