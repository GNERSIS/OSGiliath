...
#include "ReaderWriterIV.hpp"

// forward declarations of static functions
static void
addSearchPaths( const osgDB::FilePathList* searchPaths );
static void
removeSearchPaths( const osgDB::FilePathList* searchPaths );
static void
errorCallback( const SoError* error,
               void*          data );

// Register with Registry to instantiate the inventor reader.
REGISTER_OSGPLUGIN( Inventor,
                    ReaderWriterIV )
 ...
 */
void
ReaderWriterIV::initInventor() const
{
    // Initialize Inventor
    SoDB::init();
    SoNodeKit::init();
                                     ...
                                     const osgDB::ReaderWriter::Options* options ) const
{
    ...
    const osgDB::FilePathList* searchPaths =
        options ? &options->getDatabasePathList() : NULL;
    if( options )
    {
    ...
    if( result.success() )
    {
        if( fileName.length() )
        {
            OSG_NOTICE << "osgDB::ReaderWriterIV::readNode() "
                       << "File " << fileName << " loaded successfully." << std::endl;
        }
        else
        {
            OSG_NOTICE << "osgDB::ReaderWriterIV::readNode() "
                          ...
                          const osgDB::ReaderWriter::Options* options ) const
{
    ...
    // Notify
    OSG_NOTICE << "osgDB::ReaderWriterIV::readNode() Reading file " << fileName.data()
               << std::endl;
                          ...
                          const osgDB::ReaderWriter::Options* options ) const
{
    ...
    size_t  bufSize  = 126 * 1'024;    // let's make it something below 128KB
...
ReaderWriterIV::writeNode( const osg::Node&   node,
                           ...
                           const osgDB::ReaderWriter::Options* /*options*/ ) const
{
    ...
    OSG_NOTICE << "osgDB::ReaderWriterIV::writeNode() Writing file " << fileName.data()
               << std::endl;

    // Convert OSG graph to Inventor graph
    ConvertToInventor osg2iv;
    osg2iv.setVRML1Conversion( useVRML1 );
    node.accept( static_cast<osg::ConstNodeVisitor&>( osg2iv ) );
