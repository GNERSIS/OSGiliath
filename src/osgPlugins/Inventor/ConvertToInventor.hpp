...
class ConvertToInventor : public osg::DualModeVisitor
{
    public:
        ...
        SoNode*
        getIvSceneGraph() const;

        void
        setVRML1Conversion( bool useVRML1 )
        {
    ...
    protected:
        ...
        struct InventorState
        {
                ...
                InventorState( SoGroup* root ) :
                    ivHead( root ),
                    ivTexture( NULL ),
                    ivMaterial( NULL ),
                    osgMaterial( NULL ),
                    osgTexture2Enabled( false ),
                    osgTexture( NULL ),
                    osgLighting( true ),
                    osgTwoSided( false ),
                    osgFrontFace( osg::FrontFace::Mode::COUNTER_CLOCKWISE ),
                    osgCullFaceEnabled( false ),
                    osgCullFace( osg::CullFace::Mode::BACK ),
                    osgBlendEnabled( false ),
                    osgBlendFunc( NULL )
                {
                }

                InventorState( const InventorState& s ) :
                    ivHead( s.ivHead ),
                    ivTexture( s.ivTexture ),
                    ivMaterial( s.ivMaterial ),
                    osgMaterial( s.osgMaterial ),
                    osgTexture2Enabled( s.osgTexture2Enabled ),
                    osgTexture( s.osgTexture ),
                    osgLighting( s.osgLighting ),
                    osgTwoSided( s.osgTwoSided ),
                    osgFrontFace( s.osgFrontFace ),
                    osgCullFaceEnabled( s.osgCullFaceEnabled ),
                    osgCullFace( s.osgCullFace ),
                    osgBlendEnabled( s.osgBlendEnabled ),
                    osgBlendFunc( s.osgBlendFunc )
                {
                }

                static InventorState
                createTopLevelState( SoSeparator* ivRoot )
                {
        ...
        std::map<const osg::Texture*, class SoTexture2*> ivTexturesMap;
        int                                              uniqueIdGenerator;

        void
        processDrawable( osg::Drawable* d );
        void
        processGeometry( const deprecated_osg::Geometry* g,
                         InventorState*                  ivState );
        void
        processShapeDrawable( const osg::ShapeDrawable* d,
                              InventorState*            ivState );

        virtual InventorState*
        createInventorState( const osg::StateSet* ss );
        virtual void
        popInventorState();
};
