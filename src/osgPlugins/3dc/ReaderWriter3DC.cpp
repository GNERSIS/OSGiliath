...
class Writer3DCNodeVisitor : public osg::DualModeVisitor
{
    ...
    protected:
        ...
        Writer3DCNodeVisitor&
        operator=( const Writer3DCNodeVisitor& )
        {
            return *this;
        }
...
void
Writer3DCNodeVisitor::apply( osg::Geode& node )
{
    osg::dmat4   matrix = osg::computeLocalToWorld( getNodePath() );
    ...
    for( unsigned int i = 0; i < count; i++ )
    {
        ...
        if( geometry )
        {
            ...
            osg::Vec3Array* colours =
                dynamic_cast<osg::Vec3Array*>( geometry->getColorArray() );
            ...
            if( vertices )
            {
                for( unsigned int ii = 0; ii < vertices->size(); ii++ )
                {
                    ...
                    osg::dvec3 v = matrix * osg::dvec3( vertices->at( ii ) );
                    _fout << v[0] << ' ' << v[1] << ' ' << v[2];
            ...
            while( fin )
            {
                ...
                else if( strlen( line ) > 0 )
                {
                    int matched = sscanf( line,
                                          "%f%c%f%c%f%c%d%c%d%c%d%c%f%c%f%c%f",
                                          &pos.x,
                                          &sep,
                                          &pos.y,
                                          &sep,
                    ...
                    if( matched )
                    {
                        ...
                        {
                            // finishing setting up the current geometry and add it to
                            // the geode.
