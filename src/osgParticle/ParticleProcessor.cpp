...
void
osgParticle::ParticleProcessor::setParticleSystem( ParticleSystem* ps )
{
    _ps = ps;
}
...
osgParticle::ParticleProcessor::traverse( osg::NodeVisitor& nv )
{
    ...
    if( nv.getVisitorType() == osg::NodeVisitor::CULL_VISITOR )
    {
        ...
        if( _ps.valid() )
        {
            if( nv.getFrameStamp() )
            {
                ...
                if( _frameNumber < nv.getFrameStamp()->getFrameNumber() )
                {
                    ...
                    if( _t0 != -1 )
                    {
                        ...
                        {
                            if( _endless ||
                                ( _currentTime < ( _startTime + _lifeTime ) ) )
                            {
                                alive = true;
                            }
                        }
                        ...
                        {
                            // The values of _previous_wtl_matrix and
                            // _previous_ltw_matrix will be invalid since processing was
            ...
            {
                OSG_WARN
                    << "osgParticle::ParticleProcessor::traverse(NodeVisitor&) requires "
                       "a valid FrameStamp to function, particles not updated.\n";
            }
        }
        else
        {
            OSG_WARN << "ParticleProcessor \"" << getName()
...
osg::sphere
osgParticle::ParticleProcessor::computeBound() const
{
    return osg::sphere();
}
