...

    {
...

        {
            ...
            if (!precipitationDrawableSet->_quadPrecipitationDrawable)
            {
                ...
                precipitationDrawableSet->_quadPrecipitationDrawable->setDrawType(GL_TRIANGLES);
...
void PrecipitationEffect::setUpGeometries(unsigned int numParticles)
{
    ...
    if (!_quadGeometry || _quadGeometry->getVertexArray()->getNumElements() != 6*numParticles)
    {
...
void PrecipitationEffect::cull(PrecipitationDrawableSet& pds, osgUtil::CullVisitor* cv) const
{
    ...
    pds._quadPrecipitationDrawable->setNumberOfVertices(numberOfParticles*6);
...
void PrecipitationEffect::PrecipitationDrawable::drawImplementation(osg::RenderInfo& renderInfo) const
{
    ...
    osg::Matrix originalModelView = state.getModelViewMatrix();
        ...
        ++itr)
    {
        ...
        if (_requiresPreviousMatrix && previousMVLoc >= 0)
        {
            ...
            if (pitr != _previousCellMatrixMap.end())
            {
                ...
                extensions->glUniform(previousMVLoc, osg::Matrixf(pitr->second.modelview));
            }
            else
            {
                // use current modelview matrix as "previous" frame value, cancelling motion blur effect
                extensions->glUniform(previousMVLoc, osg::Matrixf((*itr)->second.modelview));
            }
        }
