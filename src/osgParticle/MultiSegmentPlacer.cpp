...
osgParticle::MultiSegmentPlacer::MultiSegmentPlacer( const MultiSegmentPlacer& copy,
    ...
    Inherit( copy,
             copyop ),
    _vx( copy._vx ),
    _total_length( copy._total_length )
{
}

void
osgParticle::MultiSegmentPlacer::recompute_length()
{
    ...
    {
        _total_length += osg::length( i->first - i0->first );
...
void
osgParticle::MultiSegmentPlacer::place( Particle* P ) const
{
    if( _vx.size() >= 2 )
    {
