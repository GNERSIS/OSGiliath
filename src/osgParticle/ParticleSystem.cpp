          ...
          const osg::dmat4& matrix )
{
    ...
    // Row 2 of MV: z_eye = M(2,0)*x + M(2,1)*y + M(2,2)*z + M(2,3)
    return -( coord[0] *
              matrix( 2, 0 ) +
              coord[1] *
              matrix( 2, 1 ) +
...
osgParticle::ParticleSystem::~ParticleSystem()
{
    // OSG_NOTICE<<"ParticleSystem::~ParticleSystem() "<<std::dec<<this<<std::dec<<"
    // _particles.size()="<<_particles.size()<<",
    // _particles.capacity()="<<_particles.capacity()<<"
    // _estimatedMaxNumOfParticles="<<_estimatedMaxNumOfParticles<<std::endl;
}

osgParticle::Particle*
osgParticle::ParticleSystem::createParticle( const osgParticle::Particle* ptemplate )
{
    ...
    else
    {
        ...
        if( _particles.size() == _particles.capacity() )
        {
            ...
            {
                _particles.reserve(
                    static_cast<std::size_t>( _estimatedMaxNumOfParticles )
                );
            }
        }
...
void
osgParticle::ParticleSystem::update( double            dt,
                                     osg::NodeVisitor& nv )
{
    // reset bounds
    ...
    if( _useShaders )
    {
        ...
        // This slightly reduces the consumption of traversing the particle vector,
        // because we don't compute tile and angle attributes that are useleff for
        // shaders. At present, our lcoal shader implementation will ignore these
        // particle props:
        ...
        if( _dirty_uniforms )
        {
            osg::FloatUniform* u_vd =
                stateset->getOrCreateUniform<osg::FloatUniform>( "visibilityDistance" );
            if( u_vd )
            {
                u_vd->setValue( static_cast<float>( _visibilityDistance ) );
            }
            _dirty_uniforms = false;
        }
    }
    ...
    for( unsigned int i = 0; i < _particles.size(); ++i )
    {
        ...
        if( particle.isAlive() )
        {
            ...
            {
                reuseParticle( static_cast<int>( i ) );
            }
        }
    }
    ...
    if( _sortMode != NO_SORT )
    {
        ...
        if( cv )
        {
            osg::dmat4 modelview    = *( cv->getModelViewMatrix() );
            ...
            for( unsigned int i = 0; i < _particles.size(); ++i )
            {
                ...
                if( particle.isAlive() )
                {
                    particle.setDepth( distance( particle.getPosition(), modelview ) *
                                       scale );
                }
                else
                {
            ...
            unsigned int numDead = static_cast<unsigned int>( _deadparts.size() );
            if( numDead > 0 )
            {
                // clear the death stack
                _deadparts = Death_stack();

                // copy the tail of the _particles vector as this will contain all the
                // dead Particle thanks to the depth sort against DBL_MAX
                Particle* first_dead_ptr = &_particles[_particles.size() - numDead];
                Particle* last_dead_ptr  = &_particles[_particles.size() - 1];
                for( Particle* dead_ptr = first_dead_ptr; dead_ptr <= last_dead_ptr;
                     ++dead_ptr )
                {
...
void
osgParticle::ParticleSystem::drawImplementation( osg::RenderInfo& renderInfo ) const
{
    ...
    if( _useVertexArray )
    {
        // note from Robert Osfield, September 2016, this block implemented for backwards
        // compatibility but is pretty way vertex array/shaders were hacked into
        // osgParticle
        ...
        {
            ad.init3();
            ad.reserve( static_cast<unsigned int>( _particles.capacity() ) );
        }
        ...
        for( unsigned int i  = 0; i < static_cast<unsigned int>( _particles.size() );
             i              += static_cast<unsigned int>( _detail ) )
        {
            ...
            texcoords.push_back( osg::vec3( particle->_alive,
                                            particle->_current_size,
                                            particle->_current_alpha ) );
    ...
    else
    {
        ...
        {
            ad.init();
            ad.reserve( static_cast<unsigned int>( _particles.capacity() ) * 4 );
        }
        ...
        {
            xAxis          = osg::transform3x3( modelview, _align_X_axis );
            yAxis          = osg::transform3x3( modelview, _align_Y_axis );

            float lengthX2 = osg::length2( xAxis );
            float lengthY2 = osg::length2( yAxis );
             ...
             i              += static_cast<unsigned int>( _detail ) )
        {
            ...
            {
                insideDistance = ( currentParticle->getDepth() >=
                                   0.0 &&
                                   currentParticle->getDepth() <= _visibilityDistance );
            }
            ...
            if( currentParticle->isAlive() && insideDistance )
            {
                ...
                bool             requiresRotation =
                    ( angle.x != 0.0F || angle.y != 0.0F || angle.z != 0.0F );
                if( requiresRotation )
                {
                    osg::dmat4 R;
                    osg::makeRotate( R,
                                     ...
                                     osg::dvec3( 0, 1, 0 ),
                                     static_cast<double>( angle.z ),
                                     osg::dvec3( 0, 0, 1 ) );

                    if( _alignment == BILLBOARD )
                    {
                        xAxis = osg::transform3x3( scaled_aligned_xAxis, R );
                        xAxis = osg::transform3x3( modelview, xAxis );

                        yAxis = osg::transform3x3( scaled_aligned_yAxis, R );
                        yAxis = osg::transform3x3( modelview, yAxis );
                    }
                    else
                    {
                        xAxis = osg::transform3x3( scaled_aligned_xAxis, R );
                ...
                switch( currentParticle->getShape() )
                {
                    case osgParticle::Particle::POINT :
                        {
                            ...
                            if( !primitives.empty() &&
                                primitives.back().first == GL_POINTS )
                            {
                                primitives.back().second++;
                            }
                            else
                            {
                                primitives.push_back( ArrayData::ModeCount( GL_POINTS,
                                                                            1 ) );
                            }
                    ...
                    case osgParticle::Particle::QUAD :
                        {
                            ...
                            if( !primitives.empty() &&
                                primitives.back().first == GL_TRIANGLES )
                            {
                                primitives.back().second += 6;
                            }
                            else
                            {
                                primitives.push_back( ArrayData::ModeCount( GL_TRIANGLES,
                                                                            6 ) );
                            }
                    ...
                    case osgParticle::Particle::LINE :
                        {
                            // Get the normalized direction of the particle, to be used
                            // in the calculation of one of the linesegment endpoints.
                            const osg::vec3& velocity = currentParticle->getVelocity();
                            float            vl       = osg::length( velocity );
                            if( vl != 0 )
                            {
                                ...
                                if( !primitives.empty() &&
                                    primitives.back().first == GL_LINES )
                                {
                                    primitives.back().second += 2;
                                }
                                else
                                {
                                    primitives.push_back( ArrayData::ModeCount( GL_LINES,
                                                                                2 ) );
                                }
                            }
                            break;
...
void
osgParticle::ParticleSystem::setDefaultAttributes( const std::string& texturefile,
                                                   ...
                                                   int texture_unit )
{
    ...
    if( !texturefile.empty() )
    {
        ...
        stateset->setTextureAttributeAndModes( static_cast<unsigned int>( texture_unit ),
                                               texture,
    ...
    // Disable lighting for particles - they use vertex color directly
    stateset->setMode( GL_LIGHTING,
                       osg::StateAttribute::OFF | osg::StateAttribute::PROTECTED );

    // Disable depth writes for transparent particles
    osg::Depth* depth = new osg::Depth( osg::Depth::Function::LESS, 0.0, 1.0, false );
    stateset->setAttributeAndModes( depth, osg::StateAttribute::ON );

    setStateSet( stateset );

    // Don't use shader/vertex-array mode - let the non-shader rendering path
    // create proper QUAD geometry with tile UV coordinates. The shader pipeline
    // auto-generates shaders for this geometry.
...
)
{
    ...
    char vertexShaderSource[] =
        ...
        "uniform float particleScreenScale;\n"
        "out vec3 basic_prop;\n"
        "out vec4 vertexColor;\n"
        "\n"
        "void main(void)\n"
        "{\n"
        "    basic_prop = osg_MultiTexCoord0.xyz;\n"
        "    \n"
        ...
        "    float worldSize = basic_prop.y;\n"
        "    gl_PointSize = abs(particleScreenScale * osg_ProjectionMatrix[1][1] * "
        "worldSize / gl_Position.w);\n"
        "    \n"
    ...
    char fragmentShaderSource[] =
        ...
        "    fragColor = vertexColor * texture(baseTexture, gl_PointCoord);\n"
        "}\n";
    ...
    program->addShader(
        osg::Shader::readShaderFile( osg::Shader::VERTEX,
                                     osgDB::findDataFile( "shaders/particle.vert" ) )
    );
    program->addShader(
        osg::Shader::readShaderFile( osg::Shader::FRAGMENT,
                                     osgDB::findDataFile( "shaders/particle.frag" ) )
    );
#endif
    stateset->setAttributeAndModes( program, osg::StateAttribute::ON );

    stateset->addUniform(
        new osg::FloatUniform( "visibilityDistance",
                               static_cast<float>( _visibilityDistance ) )
    );
    stateset->addUniform( new osg::IntUniform( "baseTexture", texture_unit ) );
    stateset->addUniform( new osg::FloatUniform( "particleScreenScale",
                                                 480.0F * 0.5F ) );
    setStateSet( stateset );
...
osg::box
osgParticle::ParticleSystem::computeBoundingBox() const
{
    if( !_bounds_computed )
    {
        return _def_bbox;
    ...
    {
        return osg::box( _bmin, _bmax );
    }
}

void
osgParticle::ParticleSystem::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
osgParticle::ParticleSystem::releaseGLObjects( osg::State* state ) const
{
...
osgParticle::ParticleSystem::createVertexArrayStateImplementation(
...
void
osgParticle::ParticleSystem::ArrayData::init()
{
    ...
    vertices->setDataVariance( osg::Object::DataVariance::DYNAMIC );

    colors = new osg::Vec4Array( osg::Array::BIND_PER_VERTEX );
    colors->setBufferObject( vertexBufferObject.get() );
    colors->setDataVariance( osg::Object::DataVariance::DYNAMIC );

    texcoords2 = new osg::Vec2Array( osg::Array::BIND_PER_VERTEX );
    texcoords2->setBufferObject( vertexBufferObject.get() );
    texcoords2->setDataVariance( osg::Object::DataVariance::DYNAMIC );
}

void
osgParticle::ParticleSystem::ArrayData::init3()
{
    vertexBufferObject = new osg::VertexBufferObject;
    vertexBufferObject->setUsage( GL_DYNAMIC_DRAW );
    ...
    normals->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    texcoords3->setDataVariance( osg::Object::DataVariance::DYNAMIC );
}

void
osgParticle::ParticleSystem::ArrayData::reserve( unsigned int numVertices )
{
    ...
    if( texcoords2.valid() )
    {
        texcoords2->reserve( numVertices );
        vertex_size += 8;
    }
    if( texcoords3.valid() )
    {
        texcoords3->reserve( numVertices );
        vertex_size += 12;
    }
...
void
osgParticle::ParticleSystem::ArrayData::resize( unsigned int numVertices )
{
    if( vertices.valid() )
    {
    ...
    if( texcoords2.valid() )
    {
        texcoords2->resize( numVertices );
    }
    if( texcoords3.valid() )
    {
        texcoords3->resize( numVertices );
    }
}

void
osgParticle::ParticleSystem::ArrayData::resizeGLObjectBuffers( unsigned int maxSize )
{
    // OSG_NOTICE<<"osgParticle::ParticleSystem::resizeGLObjectBuffers("<<maxSize<<")
    // "<<this<<std::endl;

    if( vertexBufferObject.valid() )
    {
    ...
    if( normals.valid() )
    {
        normals->resizeGLObjectBuffers( maxSize );
    }
    if( colors.valid() )
    {
    ...
    if( texcoords2.valid() )
    {
        texcoords2->resizeGLObjectBuffers( maxSize );
    }
    if( texcoords3.valid() )
    {
        texcoords3->resizeGLObjectBuffers( maxSize );
    }
}

void
osgParticle::ParticleSystem::ArrayData::releaseGLObjects( osg::State* state )
{
    // OSG_NOTICE<<"osgParticle::ParticleSystem::releaseGLObjects("<<state<<")
    // "<<this<<std::endl;
    ...
    if( normals.valid() )
    {
        normals->releaseGLObjects( state );
    }
    if( colors.valid() )
    {
    ...
    if( texcoords2.valid() )
    {
        texcoords2->releaseGLObjects( state );
    }
    if( texcoords3.valid() )
    {
        texcoords3->releaseGLObjects( state );
    }
}

void
osgParticle::ParticleSystem::ArrayData::clear()
{
    if( vertices.valid() )
    {
        vertices->clear();
...
void
osgParticle::ParticleSystem::ArrayData::dirty()
{
    if( vertices.valid() )
    {
        vertices->dirty();
...
void
osgParticle::ParticleSystem::ArrayData::dispatchArrays( osg::State& state )
{
    ...
    if( vertices.valid() )
    {
        vas->setVertexArray( state, vertices.get() );
    }
    if( normals.valid() )
    {
        vas->setNormalArray( state, normals.get() );
    }
    if( colors.valid() )
    {
    ...
    if( texcoords2.valid() )
    {
        vas->setTexCoordArray( state, 0, texcoords2.get() );
    }
    if( texcoords3.valid() )
    {
        vas->setTexCoordArray( state, 0, texcoords3.get() );
    }
...
void
osgParticle::ParticleSystem::ArrayData::dispatchPrimitives()
{
    unsigned int base = 0;
         ...
         ++itr )
    {
        ...
        glDrawArrays( mc.first,
                      static_cast<GLint>( base ),
                      static_cast<GLsizei>( mc.second ) );
        base += mc.second;
    }
}
