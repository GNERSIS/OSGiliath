...
SlideEventHandler*
SlideEventHandler::instance()
{
    return s_seh.get();
}

bool
JumpData::jump( SlideEventHandler* seh ) const
{
    OSG_INFO << "Requires jump" << seh << ", " << relativeJump << ", " << slideNum
             << ", " << layerNum << ", " << slideName << ", " << layerName << std::endl;
...
void
LayerAttributes::callEnterCallbacks( osg::Node* node )
{
...
void
LayerAttributes::callLeaveCallbacks( osg::Node* node )
{
...
struct InteractiveImageSequenceOperator : public ObjectOperator
{
        ...
        virtual void
        maintain( SlideEventHandler* /*seh*/ )
        {
        }

        virtual void
        leave( SlideEventHandler* /*seh*/ )
        {
        }

        virtual void
        setPause( SlideEventHandler* /*seh*/,
                  bool /*pause*/ )
        {
        }

        virtual void
        reset( SlideEventHandler* seh )
        {
            set( seh );
        }

        void
        set( SlideEventHandler* /*seh*/ )
        {
...
struct ImageStreamOperator : public ObjectOperator
{
        ...
        frame( SlideEventHandler* seh )
        {
            if( _delayTime != 0.0 && !_started && !_stopped )
            {
                ...
                {
                    OSG_NOTICE << "ImageStreamOperator::frame("
                               << seh->getReferenceTime() << ") calling start"
                               << std::endl;
                    start( seh );
                }
            }
            if( _stopTime > 0.0 && _started && !_stopped )
            {
                ...
                {
                    OSG_NOTICE << "ImageStreamOperator::frame("
                               << seh->getReferenceTime() << ") calling stop"
                               << std::endl;
                    stop( seh );
                }
            }
        ...
        virtual void
        maintain( SlideEventHandler* )
        {
            OSG_NOTICE << "ImageStreamOperator::maintain()" << std::endl;
        }

        virtual void
        leave( SlideEventHandler* )
        {
        ...
        start( SlideEventHandler* )
        {
            ...
            float microSecondsToDelay =
                SlideEventHandler::instance()->getTimeDelayOnNewSlideWithMovies() *
                1000000.0F;
            osg::Thread::microSleep( static_cast<unsigned int>( microSecondsToDelay ) );
        }

        void
        stop( SlideEventHandler* /*seh*/ )
        {
            if( !_started )
            {
                return;
...
{
        CallbackOperator( osg::Node*       node,
                          osg::Referenced* callback ) :
            _node( node ),
            _callback( callback )
        {
        }
                  ...
                  bool pause )
        {
            ...
            osg::AnimationPathCallback* apc =
                dynamic_cast<osg::AnimationPathCallback*>( _callback.get() );
            osgUtil::TransformCallback* tc =
                dynamic_cast<osgUtil::TransformCallback*>( _callback.get() );
            AnimationMaterialCallback* amc =
                dynamic_cast<AnimationMaterialCallback*>( _callback.get() );
...
{
        LayerAttributesOperator( osg::Node*       node,
                                 LayerAttributes* la ) :
            _node( node ),
            _layerAttribute( la )
        {
        ...
        enter( SlideEventHandler* )
        {
            ...
            if( !_layerAttribute->_keys.empty() )
            {
                ...
                for( LayerAttributes::Keys::iterator itr =
                         _layerAttribute->_keys.begin();
            ...
            {
                for( LayerAttributes::RunStrings::iterator itr =
                         _layerAttribute->_runStrings.begin();
                     ...
                     ++itr )
                {
                    ...
                    double timeForRun = osg::Timer::instance()->delta_s(
                        startTick,
                        osg::Timer::instance()->tick()
                    );

                    osgGA::EventQueue* eq =
                        SlideEventHandler::instance()->getViewer()->getEventQueue();
                    if( eq )
                    {
                        osg::Timer_t new_startTick =
                            eq->getStartTick() +
                            osg::Timer_t( timeForRun /
...
class FindOperatorsVisitor : public osg::DualModeVisitor
{
    public:

        FindOperatorsVisitor( ActiveOperators::OperatorList&  operatorList,
                              osg::NodeVisitor::TraversalMode tm ) :
            osg::DualModeVisitor( tm ),
        ...
        {
            return "osgPresentation";
        }

        const char*
        className() const override
        {
            return "FindOperatorsVisitor";
        }
        ...
        apply( osg::Node& node )
        {
            ...
            {
                _operatorList.insert( new CallbackOperator( &node,
            ...
            if( la )
            {
                ...
                {
                    OSG_INFO << "LayerAttributeOperator for " << la
                             << " required, assigning one." << std::endl;
                    _operatorList.insert( new LayerAttributesOperator( &node, la ) );
                }
                else
                {
                    OSG_INFO << "LayerAttributeOperator for " << la
                             << " not required, as one already assigned." << std::endl;
                }
            }
        ...
        process( osg::StateSet* ss )
        {
            for( unsigned int i = 0; i < ss->getTextureAttributeList().size(); ++i )
            {
                ...
                osg::ImageSequence* imageSequence =
                    dynamic_cast<osg::ImageSequence*>( image );
                osg::ImageStream* imageStream = dynamic_cast<osg::ImageStream*>( image );
                if( imageSequence && imageSequence->getName() == "USE_MOUSE_X_POSITION" )
                {
                    if( ( _objectsHandled[image]++ ) == 0 )
                    {
                        OSG_INFO << "ImageSequenceOperator for" << imageSequence
                                 << " required, assigning one, name = '"
                                 << image->getName() << "'" << std::endl;
                        _operatorList.insert(
                            new InteractiveImageSequenceOperator( imageSequence )
                        );
                    }
                    else
                    {
                        OSG_INFO << "ImageSequenceOperator for" << imageSequence
                                 << " not required, as one already assigned"
                                 << std::endl;
                    }
                }
                else if( imageStream )
                {
                    if( ( _objectsHandled[image]++ ) == 0 )
                    {
                        OSG_INFO << "ImageStreamOperator for" << imageStream
                                 << " required, assigning one" << std::endl;
...
void
ActiveOperators::collect( osg::Node*                      incomingNode,
                          osg::NodeVisitor::TraversalMode tm )
{
    ...
    {
        OSG_NOTICE << "ActiveOperators::collect() incomingNode=" << incomingNode
                   << std::endl;
    }
    ...
    for( OperatorList::iterator itr = _previous.begin(); itr != _previous.end(); ++itr )
    {
    ...
    for( OperatorList::iterator itr = _current.begin(); itr != _current.end(); ++itr )
    {
...
void
ActiveOperators::frame( SlideEventHandler* seh )
{
    for( OperatorList::iterator itr = _current.begin(); itr != _current.end(); ++itr )
    {
        ( *itr )->frame( seh );
...
void
ActiveOperators::setPause( SlideEventHandler* seh,
                           bool               pause )
{
    _pause = pause;
    for( OperatorList::iterator itr = _current.begin(); itr != _current.end(); ++itr )
...
void
ActiveOperators::reset( SlideEventHandler* seh )
{
    for( OperatorList::iterator itr = _current.begin(); itr != _current.end(); ++itr )
    {
        ( *itr )->reset( seh );
...
void
ActiveOperators::process( SlideEventHandler* seh )
{
    processOutgoing( seh );
...
void
ActiveOperators::processOutgoing( SlideEventHandler* seh )
{
    OSG_INFO << "  outgoing.size()=" << _outgoing.size() << std::endl;
    for( OperatorList::iterator itr = _outgoing.begin(); itr != _outgoing.end(); ++itr )
    {
        ( *itr )->leave( seh );
    }
}
...
void
ActiveOperators::processMaintained( SlideEventHandler* seh )
{
    OSG_INFO << "  maintained.size()=" << _maintained.size() << std::endl;
    for( OperatorList::iterator itr = _maintained.begin(); itr != _maintained.end();
         ++itr )
    {
        ( *itr )->maintain( seh );
    }
...
void
ActiveOperators::processIncoming( SlideEventHandler* seh )
{
    OSG_INFO << "  incoming.size()=" << _incoming.size() << std::endl;
    for( OperatorList::iterator itr = _incoming.begin(); itr != _incoming.end(); ++itr )
    {
        ( *itr )->enter( seh );
        ( *itr )->setPause( seh, _pause );
    }
...
class FindHomePositionVisitor : public osg::DualModeVisitor
{
    public:
        ...
        apply( osg::Node& node )
        {
            HomePosition* homePosition =
                dynamic_cast<HomePosition*>( node.getUserData() );
            if( homePosition )
            {
...
class FindNamedSwitchVisitor : public osg::DualModeVisitor
{
    public:
        ...
        apply( osg::Switch& sw )
        {
            ...
            {
                _switch = &sw;
                return;    // note, no need to do traverse now we've located the relevant
                           // switch
            }
...
class FindFilePathDataVisitor : public osg::DualModeVisitor
{
    public:
...
class UpdateLightVisitor : public osg::DualModeVisitor
{
    public:

        UpdateLightVisitor( const osg::dmat4& viewMatrix,
                            float             currentX,
                            float             currentY ) :
            osg::DualModeVisitor( osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN ),
        ...
        void
        apply( osg::LightSource& lightsource )
        {
            if( lightsource.getLight() )
            {
                if( lightsource.getReferenceFrame() == osg::LightSource::RELATIVE_RF )
                {
                    apply( osg::dmat4(), lightsource.getLight() );
                }
                else
                {
                    apply( osg::computeEyeToLocal( _viewMatrix, _nodePath ),
               ...
               osg::Light*       light )
        {
            ...
            direction = osg::transform3x3( matrix, direction );
            direction = osg::normalize( direction );

            light->setPosition( osg::vec4( direction, 0.0F ) );
        }

        osg::dmat4 _viewMatrix;
        float      _currentX, _currentY;
};

class UpdateAlphaVisitor : public osg::DualModeVisitor
{
    public:

        UpdateAlphaVisitor( bool  modAlphaFunc,
                            bool  modMaterial,
                            float currentX,
                            float currentY ) :
            osg::DualModeVisitor( osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN ),
            _modAlphaFunc( modAlphaFunc ),
        ...
        apply( osg::StateSet& stateset )
        {
            if( _modMaterial )
            {
                osg::Material* material = dynamic_cast<osg::Material*>(
                    stateset.getAttribute( osg::StateAttribute::Type::MATERIAL )
                );
                if( material )
                {
                    OSG_INFO << "Adjusting material func" << std::endl;
                    float alpha = std::clamp( ( _currentY + 1.0F ) * 0.5F, 0.0F, 1.0F );
...
SlideEventHandler::SlideEventHandler( const SlideEventHandler& seh,
    ...
    _previousX( seh._previousX ),
    _previousY( seh._previousY ),
...
double
SlideEventHandler::getDuration( const osg::Node* node ) const
{
    const LayerAttributes* la =
        dynamic_cast<const LayerAttributes*>( node->getUserData() );
    return la ? la->_duration : -1.0;
}

void
SlideEventHandler::set( osg::Node* model )
{
#if 0
    ...
    if( !fullpath.empty() )
    {
        setUserValue( "fullpath", fullpath );
    }
    ...
    else
    {
        ...
        {
            OSG_INFO << "Found presentation slide" << findSlide._switch->getName()
                     << std::endl;
        ...
        {
            OSG_INFO << "No slides present in scene, unable to operate as a slideshow."
                     << std::endl;
        }
    }
}
...
double
SlideEventHandler::getCurrentTimeDelayBetweenSlides() const
{
        ...
        case( osgGA::GUIEventAdapter::FRAME ) :
            {
                if( _autoSteppingActive && !_pause )
                {
                    ...
                    else if( time - _previousTime >= getCurrentTimeDelayBetweenSlides() )
                    {
                        ...
                        if( !_hold )
                        {
                            // increment the previous by the required time delay, note
                            // relative to the current to keep the time relative to an
                            // absolute time signal, thus avoid drift of timing.
                        ...
                        {
                            // we're holding of the move to next layer to slide, but we
                            // need slip the time forward accordingly componensate for
                            // the extra time that this frame is receiving.
        ...
        case( osgGA::GUIEventAdapter::KEYDOWN ) :
            {
                ...
                /*
                   ...
                   _viewer->getCameraConfig()->getNumberOfCameras(); i++ )
                                {
                                    Producer::Camera* cam =
                   _viewer->getCameraConfig()->getCamera(i); Producer::RenderSurface* rs
                   = cam->getRenderSurface(); rs->useCursor(_cursorOn);
                                }
        ...
        case( osgGA::GUIEventAdapter::KEYUP ) :
            {
                ...
                else if( ea.getKey() == 'E' )
                {
                    ...
                    std::string filename;
                    if( editor &&
                        getUserValue( "fullpath", filename ) &&
                        !filename.empty() )
                    {
                        ...
                        OSG_INFO << "system(" << command.str() << ") result " << result
                                 << std::endl;
                    }
                    return true;
                }
...
unsigned int
SlideEventHandler::getNumSlides()
{
...
osg::Switch*
SlideEventHandler::getSlide( int slideNum )
{
    if( slideNum <
        0 ||
        slideNum > static_cast<int>( _presentationSwitch->getNumChildren() ) )
    {
        return 0;
    }
...
osg::Node*
SlideEventHandler::getLayer( int slideNum,
                             int layerNum )
{
    osg::Switch* slide = getSlide( slideNum );
    return ( slide && ( layerNum >=
                        0 &&
                        layerNum < static_cast<int>( slide->getNumChildren() ) ) )
             ? slide->getChild( layerNum )
             : 0;
}

bool
SlideEventHandler::selectSlide( int slideNum,
                                int layerNum )
{
    if( !_presentationSwitch || _presentationSwitch->getNumChildren() == 0 )
    {
        return false;
    }
    ...
    OSG_INFO << "selectSlide(" << slideNum << "," << layerNum << ") at time "
             << osg::Timer::instance()->delta_s( _tickAtFirstSlideOrLayerChange, tick )
             << " seconds, length ="
             << osg::Timer::instance()->delta_s( _tickAtLastSlideOrLayerChange, tick )
             << " seconds" << std::endl;
    ...
    // OSG_INFO<<"Selected slide
    // '"<<_presentationSwitch->getChild(_activeSlide)->getName()<<"'"<<std::endl;
...
bool
SlideEventHandler::selectLayer( int layerNum )
{
    if( !_slideSwitch || _slideSwitch->getNumChildren() == 0 )
    {
        return false;
    }
    ...
    OSG_INFO << "Selected layer '" << _slideSwitch->getChild( _activeLayer )->getName()
             << "' num=" << _activeLayer << std::endl;

    return withinSlide;
}

bool
SlideEventHandler::nextLayerOrSlide()
{
    if( nextLayer() )
    {
        return true;
...
bool
SlideEventHandler::previousLayerOrSlide()
{
...
bool
SlideEventHandler::nextSlide()
{
    OSG_INFO << "nextSlide()" << std::endl;
    LayerAttributes* la =
        _slideSwitch.valid()
            ? dynamic_cast<LayerAttributes*>( _slideSwitch->getUserData() )
            : 0;
...
bool
SlideEventHandler::previousSlide()
{
...
bool
SlideEventHandler::nextLayer()
{
    LayerAttributes* la =
        ( _slideSwitch.valid() &&
          _activeLayer < static_cast<int>( _slideSwitch->getNumChildren() ) )
            ? dynamic_cast<LayerAttributes*>(
                  _slideSwitch->getChild( _activeLayer )->getUserData()
              )
            : 0;
    if( la )
    {
    ...
    OSG_INFO << "nextLayer() calling selectLayer("
             << _activeLayer + 1 << ")" << std::endl;
    return selectLayer( _activeLayer + 1 );
}

bool
SlideEventHandler::previousLayer()
{
...
void
SlideEventHandler::updateOperators()
{
...
bool
SlideEventHandler::home( const osgGA::GUIEventAdapter& ea,
                         osgGA::GUIActionAdapter&      aa )
{
    ...
    if( _viewer->getCameraManipulator() )
    {
        if( fhpv._homePosition.valid() )
        {
            ...
            _viewer->getCameraManipulator()->setHomePosition( fhpv._homePosition->eye,
...
void
SlideEventHandler::updateAlpha( bool  modAlphaFunc,
                                bool  modMaterial,
                                float x,
                                float y )
{
...
void
SlideEventHandler::updateLight( float x,
                                float y )
{
    ...
    if( _viewer->getLightingMode() != osg::View::NO_LIGHT && _viewer->getLight() )
    {
        ...
        {
            uav.apply( osg::dmat4(), _viewer->getLight() );
        }
    }
}

void
SlideEventHandler::compileSlide( unsigned int slideNum )
{
...
void
SlideEventHandler::releaseSlide( unsigned int slideNum )
{
    osgUtil::GLObjectsVisitor globjVisitor(
        osgUtil::GLObjectsVisitor::RELEASE_VERTEX_BUFFER_OBJECTS |
...
void
SlideEventHandler::forwardEventToDevices( osgGA::Event* event )
{
    if( !event )
    {
        return;
    ...
    if( !_viewer )
    {
        OSG_NOTICE << "Warning: SlideEventHandler::forwardEventToDevices(Event*) error, "
                      "no Viewer to dispatch to."
                   << std::endl;
        return;
    }

    osgViewer::View::Devices& devices = _viewer->getDevices();
    for( osgViewer::View::Devices::iterator i = devices.begin(); i != devices.end();
         ++i )
    {
...
void
SlideEventHandler::dispatchEvent( osgGA::Event* event )
{
    if( !event )
    {
        return;
    ...
    if( !eq )
    {
        OSG_NOTICE << "Warning: SlideEventHandler::dispatchEvent(KeyPosition&) error, "
                      "no EventQueue to dispatch to."
                   << std::endl;
        return;
    }
...
SlideEventHandler::dispatchEvent( const KeyPosition& keyPosition )
{
    ...
    // reset the time of the last key press to ensure that the event is disgarded as a
    // key repeat.
    _timeLastKeyPresses = -1.0;

    if( keyPosition._x != FLT_MAX )
    {
        float xRescaled = eq->getCurrentEventState()->getXmin() +
                          ( keyPosition._x + 1.0F ) *
                          0.5F *
                          ( eq->getCurrentEventState()->getXmax() -
                            eq->getCurrentEventState()->getXmin() );
        eq->getCurrentEventState()->setX( xRescaled );
    }

    if( keyPosition._y != FLT_MAX )
    {
        float y         = ( eq->getCurrentEventState()->getMouseYOrientation() ==
                            osgGA::GUIEventAdapter::Y_INCREASING_UPWARDS )
                            ? keyPosition._y
                            : -keyPosition._y;

        float yRescaled = eq->getCurrentEventState()->getYmin() +
                          ( y + 1.0F ) *
                          0.5F *
                          ( eq->getCurrentEventState()->getYmax() -
                            eq->getCurrentEventState()->getYmin() );
...
void
SlideEventHandler::setRequestReload( bool flag )
{
    _requestReload = flag;
}

bool
SlideEventHandler::checkNeedToDoFrame()
{
    if( _viewer.valid() )
    {
        if( _viewer->getRequestRedraw() )
        {
            return true;
        }
        if( _viewer->getRequestContinousUpdate() )
        {
            return true;
        }

        // check if the database pager needs to update the scene
        if( _viewer->getDatabasePager()->requiresUpdateSceneGraph() )
        {
            return true;
        }

        // check if the image pager needs to update the scene
        if( _viewer->getImagePager()->requiresUpdateSceneGraph() )
        {
            return true;
        }

        // if there update callbacks then we need to do frame.
        if( _viewer->getCamera()->getUpdateCallback() )
        {
            return true;
        }
        ...
        if( !_pause )
        {
            if( _slideSwitch.valid() &&
                _activeLayer < static_cast<int>( _slideSwitch->getNumChildren() ) )
            {
                if( _slideSwitch->getChild( _activeLayer )
                        ->getNumChildrenRequiringUpdateTraversal() > 0 )
                {
                    return true;
                     ...
                     0 &&
                     ( _viewer->getSceneData()->getUpdateCallback() ||
            ...
            if( _autoSteppingActive )
            {
                ...
                else
                {
                    osg::Timer_t tick = osg::Timer::instance()->tick();
                    double       currentTime =
                        osg::Timer::instance()->delta_s( _viewer->getStartTick(), tick );
