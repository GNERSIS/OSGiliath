...
const osg::Object*
osgPresentation::getUserObject( const osg::NodePath& nodepath,
...
bool
osgPresentation::containsPropertyReference( const std::string& str )
{
    return ( str.find( '$' ) != std::string::npos );
}

void
PropertyAnimation::reset()
{
    _firstTime = DBL_MAX;
    _pauseTime = DBL_MAX;
...
void
PropertyAnimation::setPause( bool pause )
{
...
double
PropertyAnimation::getAnimationTime() const
{
    return _latestTime - _firstTime;
}

void
PropertyAnimation::operator()( osg::Node*        node,
                               osg::NodeVisitor* nv )
{
...
class MySetValueVisitor : public osg::ValueObject::SetValueVisitor
{
    public:
        ...
        MySetValueVisitor( double            in_r1,
                           double            in_r2,
                           osg::ValueObject* in_object2 ) :
            _r1( in_r1 ),
            _r2( in_r2 ),
            _object2( in_object2 )
        {
        ...
        template<typename T>
        void
        combineRealUserValue( T& value ) const
        {
            typedef osg::TemplateValueObject<T> UserValueObject;
            const UserValueObject*              uvo =
                _object2 ? dynamic_cast<const UserValueObject*>( _object2 ) : 0;
            if( uvo )
            {
                value = value * _r1 + uvo->getValue() * _r2;
            }
            OSG_NOTICE << "combineRealUserValue r1=" << _r1 << ", r2=" << _r2
                       << ", value=" << value << std::endl;
        }

        template<typename T>
        void
        combineIntegerUserValue( T& value ) const
        {
            typedef osg::TemplateValueObject<T> UserValueObject;
            const UserValueObject*              uvo =
                _object2 ? dynamic_cast<const UserValueObject*>( _object2 ) : 0;
            if( uvo )
            {
                value = static_cast<T>( static_cast<double>( value ) *
                                        _r1 +
                                        static_cast<double>( uvo->getValue() ) *
                                        _r2 );
            }
        ...
        template<typename T>
        void
        combineDiscretUserValue( T& value ) const
        {
        ...
        template<typename T>
        void
        combineRotationUserValue( T& /*value*/ ) const
        {
        ...
        template<typename T>
        void
        combinePlaneUserValue( T& /*value*/ ) const
        {
        ...
        template<typename T>
        void
        combineMatrixUserValue( T& /*value*/ ) const
        {
            OSG_NOTICE << "combineMatrixUserValue TODO - decomposs into translate, "
                          "rotation and scale and then interpolate."
                       << std::endl;
        }
...
void
PropertyAnimation::update( osg::Node& node )
{
    ...
    if( itr == _keyFrameMap.begin() )
    {
        ...
        OSG_NOTICE << "PropertyAnimation::update() : copy first UserDataContainer"
                   << std::endl;
        assign( node.getOrCreateUserDataContainer(), itr->second.get() );
    }
    else if( itr != _keyFrameMap.end() )
    {
        KeyFrameMap::const_iterator itr_1 = itr;
        --itr_1;
        ...
        osg::ref_ptr<osg::UserDataContainer> destination =
        ...
        for( unsigned int i2 = 0; i2 < p2->getNumUserObjects(); ++i2 )
        {
            ...
            if( i1 < p1->getNumUserObjects() )
            {
                ...
                osg::ValueObject* valueobject_1 =
                    dynamic_cast<osg::ValueObject*>( obj_1 );
                osg::ValueObject* valueobject_2 =
                    dynamic_cast<osg::ValueObject*>( obj_2 );
    ...
    {
        OSG_NOTICE << "PropertyAnimation::update() : copy last UserDataContainer"
                   << std::endl;
        assign( node.getOrCreateUserDataContainer(),
                _keyFrameMap.rbegin()->second.get() );
    }
}

void
PropertyAnimation::assign( osg::UserDataContainer* destination,
                           osg::UserDataContainer* source )
{
    if( !destination )
    {
        return;
...
void
PropertyAnimation::assign( osg::UserDataContainer* udc,
                           osg::Object*            obj )
{
    if( !obj )
    {
...
void
ImageSequenceUpdateCallback::operator()( osg::Node*        node,
                                         osg::NodeVisitor* nv )
{
    float x;
    if( _propertyManager->getProperty( _propertyName, x ) )
    {
        ...
        double xMax = 1.0;
        double position =
            ( ( double )x - xMin ) / ( xMax - xMin ) * _imageSequence->getLength();

        _imageSequence->seek( position );
    }
    else
    {
        OSG_INFO << "ImageSequenceUpdateCallback::operator() Could not find property : "
                 << _propertyName << std::endl;
    }
...
bool
PropertyEventCallback::handle( const osgGA::GUIEventAdapter& ea,
                               osgGA::GUIActionAdapter& )
{

    bool mouseEvent = ( ea.getEventType() ==
