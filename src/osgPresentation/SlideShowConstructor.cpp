...
class SetToTransparentBin : public osg::DualModeVisitor
{
    public:
        ...
        apply( osg::Geode& geode )
        {
            ...
            for( unsigned int i = 0; i < geode.getNumDrawables(); ++i )
            {
                ...
                {
                    geode.getDrawable( i )->getStateSet()->setMode(
                        GL_BLEND,
                        osg::StateAttribute::ON
                    );
                    geode.getDrawable( i )->getStateSet()->setRenderingHint(
...
bool
HUDTransform::computeLocalToWorldMatrix( osg::dmat4&       matrix,
...
bool
HUDTransform::computeWorldToLocalMatrix( osg::dmat4&       matrix,
                                         osg::NodeVisitor* nv ) const
{
    ...
    _options( options )
{
    ...
    _propertyEventCallback =
        new osgPresentation::PropertyEventCallback( _propertyManager.get() );
    ...
    _hudSettings   = new HUDSettings( _slideDistance,
                                      ds->getEyeSeparation() * 0.5,
                                      _leftEyeMask,
                                      _rightEyeMask );
...
void
SlideShowConstructor::setPresentationAspectRatio( float aspectRatio )
{
    _slideWidth = _slideHeight * aspectRatio;
}

void
SlideShowConstructor::setPresentationAspectRatio( const std::string& str )
{
    ...
    else
    {
        ...
        else
        {
            ...
            OSG_WARN << "       valid types are \"Reality Theatre\", \"Desktop\" or a "
                        "numerical value."
                     << std::endl;
        }
    }
...
void
SlideShowConstructor::createPresentation()
{
    ...
    OSG_INFO << "_titlePositionDataDefault.position="
             << _titlePositionDataDefault.position << std::endl;

    _textPositionDataDefault.position.set( 0.1F,
                                           _titlePositionDataDefault.position.y -
                                               _titleFontDataDefault.characterSize,
                                           0.0F );
    ...
    osg::vec3 slideCenter =
        _slideOrigin + osg::vec3( _slideWidth * 0.5F, 0.0F, _slideHeight * 0.5F );
    ...
    for( ScriptEngineMap::iterator itr = _scriptEngines.begin();
         ...
         ++itr )
    {
        OSG_NOTICE << "Assigning '" << itr->first
                   << "' ScriptEngine to Presentation in createPresentation()."
                   << std::endl;
        _presentationSwitch->getOrCreateUserDataContainer()->addUserObject(
            itr->second.get()
        );
    }
}
...
LayerAttributes*
SlideShowConstructor::getOrCreateLayerAttributes( osg::Node* node )
{
    ...
    if( !la )
    {
        if( node->getUserData() )
        {
            OSG_NOTICE << "UserData already assigned, overriding to set LayerAttributes."
                       << std::endl;
        }
...
void
SlideShowConstructor::setBackgroundColor( const osg::vec4& color,
...
void
SlideShowConstructor::setTextColor( const osg::vec4& color )
{
...
void
SlideShowConstructor::setPresentationName( const std::string& name )
{
    _presentationName = name;
    if( _presentationSwitch.valid() )
    {
        _presentationSwitch->setName( std::string( "Presentation_" ) +
                                      _presentationName );
    }
}

void
SlideShowConstructor::setPresentationDuration( double duration )
{
...
osg::ScriptEngine*
SlideShowConstructor::getOrCreateScriptEngine( const std::string& language )
{
...
void
SlideShowConstructor::addScriptEngine( const std::string& scriptEngineName )
{
    if( _scriptEngines.count( scriptEngineName ) != 0 )
    {
        OSG_NOTICE << "Script engine " << scriptEngineName << " already loaded."
                   << std::endl;
    }

    osg::ref_ptr<osg::ScriptEngine> scriptEngine =
        osgDB::readRefFile<osg::ScriptEngine>( std::string( "ScriptEngine." ) +
                                               scriptEngineName );
    ...
    {
        OSG_NOTICE << "Warning: Failed to load " << scriptEngineName
                   << " engine, scripts will not work." << std::endl;
    }
}

void
SlideShowConstructor::addScriptFile( const std::string& name,
                                     const std::string& filename )
{
    OSG_NOTICE << "addScriptFile() name=" << name << ", filename = " << filename
               << std::endl;
...
void
SlideShowConstructor::addScript( const std::string& name,
                                 const std::string& language,
                                 const std::string& scriptContents )
{
    OSG_NOTICE << "addScript() language=" << language << ", name=" << name
               << ", script = " << scriptContents << std::endl;
...
void
SlideShowConstructor::selectSlide( int slideNum )
{
    if( slideNum < 0 )
    {
        addSlide();
    ...
    {
        _slideClearNode =
            dynamic_cast<osg::ClearNode*>( _presentationSwitch->getChild( slideNum ) );
        if( !_slideClearNode ||
            _slideClearNode->getNumChildren() ==
            0 ||
            _slideClearNode->getChild( 0 )->asSwitch() == 0 )
        {
            addSlide();
        }
        else
...
void
SlideShowConstructor::setSlideDuration( double duration )
{
    if( !_slide )
    {
        addSlide();
...
void
SlideShowConstructor::pushCurrentLayer( osg::Group* group )
{
...
void
SlideShowConstructor::popCurrentLayer()
{
...
void
SlideShowConstructor::addLayer( bool inheritPreviousLayers,
                                bool defineAsBaseLayer )
{
    ...
    if( !_previousLayer || !inheritPreviousLayers )
    {
        ...
        // OSG_NOTICE<<"   new layer background =
        // "<<_slideBackgroundImageFileName<<std::endl;

        osg::ref_ptr<osg::Image> image =
            !_slideBackgroundImageFileName.empty()
                ? osgDB::readRefImageFile( _slideBackgroundImageFileName,
                                           _options.get() )
                : 0;
        ...
        if( image.valid() )
        {
            ...
            backgroundStateSet->setAttributeAndModes( new osg::PolygonOffset( 1.0F,
                                                                              2.0F ),
                                                      osg::StateAttribute::ON );
            ...
            osg::Geometry* backgroundQuad =
                osg::createTexturedQuadGeometry( _slideOrigin + backgroundShift,
                                                 osg::vec3( _slideWidth, 0.0F, 0.0F ),
                                                 osg::vec3( 0.0F, 0.0F, _slideHeight ),
                                                 s,
                                                 t );
            // OSG_NOTICE<<"Image loaded "<<image.get()<<"
            // "<<_slideBackgroundImageFileName<<std::endl;
        ...
        if( !_slideTitle.empty() )
        {
            ...
            text->setFont( osgText::readRefFontFile( _titleFontData.font,
                                                     _options.get() ) );
            ...
            addToCurrentLayer( decorateSubgraphForPosition( geode,
                                                            _titlePositionData ) );
        }
    }
    else
...
void
SlideShowConstructor::selectLayer( int layerNum )
{
    if( !_slide )
    {
        addSlide();
        addLayer();
             ...
             0 &&
             layerNum <
             static_cast<int>( _slide->getNumChildren() ) &&
             _slide->getChild( layerNum )->asGroup() )
    {
...
void
SlideShowConstructor::setLayerDuration( double duration )
{
    if( !_currentLayer )
    {
        addLayer();
...
void
SlideShowConstructor::addToCurrentLayer( osg::Node* subgraph )
{
    if( !subgraph )
    {
        return;
    ...
    {
        if( _layerToApplyEventCallbackTo ==
            0 ||
            _currentLayer == _layerToApplyEventCallbackTo )
        {
...
void
SlideShowConstructor::addEventHandler( PresentationContext presentationContext,
...
void
SlideShowConstructor::keyToDoOperation( PresentationContext presentationContext,
                                        int                 key,
                                        Operation           operation,
                                        const JumpData&     jumpData )
{
    OSG_INFO << "keyToDoOperation(key=" << key << ", operation=" << operation << ")"
             << std::endl;
    addEventHandler( presentationContext,
                     new KeyEventHandler( key, operation, jumpData ) );
}
...
SlideShowConstructor::keyToDoOperation( PresentationContext presentationContext,
                                        ...
                                        const JumpData&     jumpData )
{
    OSG_INFO << "keyToDoOperation(key=" << key << ",command=" << command << ")"
             << std::endl;
    addEventHandler( presentationContext,
                     new KeyEventHandler( key, command, operation, jumpData ) );
}

void
SlideShowConstructor::keyEventOperation( PresentationContext presentationContext,
                                         int                 key,
...
void
SlideShowConstructor::layerClickToDoOperation( Operation       operation,
                                               const JumpData& jumpData )
{
...
void
SlideShowConstructor::layerClickToDoOperation( const std::string& command,
                                               Operation          operation,
                                               const JumpData&    jumpData )
{
    addEventHandler( CURRENT_LAYER,
                     new PickEventHandler( command, operation, jumpData ) );
}

void
SlideShowConstructor::layerClickEventOperation( const KeyPosition& keyPos,
                                                const JumpData&    jumpData )
{
...
void
SlideShowConstructor::addPropertyAnimation( PresentationContext presentationContext,
...
void
SlideShowConstructor::addScriptCallback( PresentationContext presentationContext,
...
void
SlideShowConstructor::addScriptToNode( ScriptCallbackType scriptCallbackType,
                                       const std::string& name,
                                       osg::Node*         node )
{
    std::string::size_type colon_position = name.find( ':' );
    std::string            script_name    = ( colon_position == std::string::npos )
                                              ? name
                                              : name.substr( 0, colon_position );
    std::string entry_point = ( colon_position == std::string::npos )
                                ? std::string()
                                : name.substr( colon_position + 1, std::string::npos );
        ...
        switch( scriptCallbackType )
        {
            case( UPDATE_SCRIPT ) :
                node->addUpdateCallback(
                    new osg::ScriptNodeCallback( script_itr->second.get(), entry_point )
                );
                break;
            case( EVENT_SCRIPT ) :
...
void
SlideShowConstructor::addScriptsToNode( const ScriptData& scriptData,
                                        osg::Node*        node )
{
    if( !node )
    {
...
osg::Node*
SlideShowConstructor::decorateSubgraphForPosition( osg::Node*    node,
                                                   PositionData& positionData )
{
    ...
    if( positionData.rotation[0] != 0.0 )
    {
        ...
        animation_transform->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        animation_transform->setUpdateCallback(
            new osgUtil::TransformCallback( subgraph->getBound().center,
                                            osg::vec3( positionData.rotation[1],
                                                       positionData.rotation[2],
                                                       positionData.rotation[3] ),
                                            osg::radians( positionData.rotation[0] ) )
        );
...
void
SlideShowConstructor::addBullet( const std::string& bullet,
                                 PositionData&      positionData,
                                 FontData&          fontData,
                                 const ScriptData&  scriptData )
{
    ...
    const osg::box& bb = text->getBoundingBox();
...
void
SlideShowConstructor::addParagraph( const std::string& paragraph,
                                    PositionData&      positionData,
                                    FontData&          fontData,
                                    const ScriptData&  scriptData )
{
...
class FindImageStreamsVisitor : public osg::DualModeVisitor
{
    public:
        ...
        process( osg::StateSet* ss )
        {
            for( unsigned int i = 0; i < ss->getTextureAttributeList().size(); ++i )
            {
                osg::Texture* texture = dynamic_cast<osg::Texture*>(
                    ss->getTextureAttribute( i, osg::StateAttribute::Type::TEXTURE )
                );
                osg::Image*       image = texture ? texture->getImage( 0 ) : 0;
                osg::ImageStream* imageStream =
                    image ? dynamic_cast<osg::ImageStream*>( image ) : 0;
                if( imageStream )
                {
                    texture->setDataVariance( osg::Object::DataVariance::DYNAMIC );
...
void
SlideShowConstructor::findImageStreamsAndAddCallbacks( osg::Node* node )
{
...
osg::Geometry*
SlideShowConstructor::createTexturedQuadGeometry( const osg::vec3& pos,
                                                  ...
                                                  bool&            usedTextureRectangle )
{
    ...
    osg::mat4                  rotationMatrix = osg::rotate( osg::radians( rotation[0] ),
                                                             rotation[1],
                                                             rotation[2],
                                                             rotation[3] );
    ...
    if( texture )
    {
        float l = 0.0F;
        float b = 0.0F;
        float r = ( texture->getTextureTarget() == GL_TEXTURE_RECTANGLE )
                    ? static_cast<float>( image->s() )
                    : 1.0F;
        float t = ( texture->getTextureTarget() == GL_TEXTURE_RECTANGLE )
                    ? static_cast<float>( image->t() )
                    : 1.0F;
...
#if USE_CLIENT_STORAGE_HINT
        // make sure that OSX uses the client storage extension to accelerate performance
        // where possible.
        if( texture )
        {
            texture->setClientStorageHint( true );
...
SlideShowConstructor::readImage( const std::string& filename,
                                 const ImageData&   imageData )
{
    ...
    if( imageData.imageSequence )
    {
        ...
        else
        {
            ...
            if( fileType == osgDB::DIRECTORY )
            {
                ...
                // need to insert the directory path in front of the filenames so it's
    ...
    else
    {
             ...
             ++itr )
        {
            if( imageSequence->getMode() == osg::ImageSequence::PRE_LOAD_ALL_IMAGES )
            {
                ...
                osg::ref_ptr<osg::Image> loadedImage =
                    osgDB::readRefImageFile( *itr, options.get() );
            ...
            {
                OSG_INFO << "Adding filename for load image on demand " << *itr
                         << std::endl;
        ...
        else if( imageData.imageSequenceInteractionMode ==
                 ImageData::USE_MOUSE_Y_POSITION )
        {
...
struct VolumeCallback : public osg::NodeCallback
{
    public:
        ...
        VolumeCallback( osg::ImageStream*  movie,
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( pr.ok() )
            {
                OSG_NOTICE << "VolumeCallback : volume=" << volume << ", from "
                           << _source << std::endl;
                _movie->setVolume( volume );
            }
            else
            {
                OSG_NOTICE << "Problem in reading, VolumeCallback : volume=" << volume
                           << std::endl;
            }
...
void
SlideShowConstructor::setUpMovieVolume( osg::Node*        subgraph,
                                        osg::ImageStream* imageStream,
                                        const ImageData&  imageData )
{
    if( containsPropertyReference( imageData.volume ) )
    {
        subgraph->addUpdateCallback( new VolumeCallback( imageStream,
                                                         imageData.volume ) );
    }
    else
    {
...
void
SlideShowConstructor::addImage( const std::string&  filename,
                                ...
                                const ScriptData&   scriptData )
{
    ...
    if( imageStream )
    {
        ...
        isImageTranslucent = imageStream->getPixelFormat() ==
                             GL_RGBA ||
    ...
    float image_height =
        image_width * aspectRatio * positionData.scale.y / positionData.scale.x;
    float     offset          = 0.0F;

    osg::vec3 pos             = computePositionInModelCoords( positionData );
    osg::vec3 image_local_pos = osg::vec3( -image_width * 0.5F + offset,
                                           -offset,
                                           -image_height * 0.5F - offset );
    osg::vec3 image_pos =
        positionData.autoRotate ? image_local_pos : ( pos + image_local_pos );

    bool           usedTextureRectangle = false;
    osg::Geometry* pictureQuad     = createTexturedQuadGeometry( image_pos,
                                                                 positionData.rotate,
                                                                 image_width,
                                                                 image_height,
                                                                 image.get(),
                                                                 usedTextureRectangle );
    ...
    if( imageSequence )
    {
        if( imageData.imageSequenceInteractionMode == ImageData::USE_MOUSE_X_POSITION )
        {
            subgraph->setUpdateCallback(
                new osgPresentation::ImageSequenceUpdateCallback( imageSequence,
                                                                  _propertyManager.get(),
                                                                  "mouse.x_normalized" )
            );
        }
        else if( imageData.imageSequenceInteractionMode ==
    ...
    if( animation )
    {
        ...
        osg::dvec3 pivot = positionData.absolute_path
                             ? osg::dvec3( 0.0, 0.0, 0.0 )
                             : osg::dvec3( subgraph->getBound().center );

        osg::PositionAttitudeTransform* animation_transform =
...
void
SlideShowConstructor::addStereoImagePair( const std::string&  filenameLeft,
                                          ...
                                          const ScriptData&   scriptData )
{
    ...
    osg::ref_ptr<osg::Image> imageRight = ( filenameRight == filenameLeft )
                                            ? imageLeft
                                            : readImage( filenameRight, imageDataRight );
    ...
    osg::ImageStream* imageStreamLeft =
        dynamic_cast<osg::ImageStream*>( imageLeft.get() );
    if( imageStreamLeft )
    {
        imageStreamLeft->setLoopingMode( imageDataLeft.loopingMode );
        isImageTranslucent = imageStreamLeft->getPixelFormat() ==
                             GL_RGBA ||
    ...
    osg::ImageStream* imageStreamRight =
        dynamic_cast<osg::ImageStream*>( imageRight.get() );
    if( imageStreamRight )
    {
        imageStreamRight->setLoopingMode( imageDataRight.loopingMode );
        if( !isImageTranslucent )
        {
            isImageTranslucent = imageStreamRight->getPixelFormat() ==
                                 GL_RGBA ||
    ...
    if( imageDataLeft.blendingHint ==
        ImageData::ON ||
        imageDataRight.blendingHint == ImageData::ON )
    {
        isImageTranslucent = true;
    }
    else if( imageDataLeft.blendingHint ==
             ImageData::OFF ||
             imageDataRight.blendingHint == ImageData::OFF )
    {
        isImageTranslucent = false;
    }
    ...
    {
        osg::Geometry* pictureLeftQuad =
            createTexturedQuadGeometry( image_pos,
                                        positionData.rotate,
                                        image_width,
                                        image_height,
                                        imageLeft.get(),
    ...
    osg::ImageSequence* imageSequence =
        dynamic_cast<osg::ImageSequence*>( imageLeft.get() );
    if( imageSequence )
    {
        if( imageDataLeft.imageSequenceInteractionMode ==
            ImageData::USE_MOUSE_X_POSITION )
        {
            subgraph->setUpdateCallback(
                new osgPresentation::ImageSequenceUpdateCallback( imageSequence,
                                                                  _propertyManager.get(),
        ...
        else if( imageDataLeft.imageSequenceInteractionMode ==
                 ImageData::USE_MOUSE_Y_POSITION )
        {
            subgraph->setUpdateCallback(
                new osgPresentation::ImageSequenceUpdateCallback( imageSequence,
...
void
SlideShowConstructor::addGraph( const std::string&  contents,
                                const PositionData& positionData,
                                const ImageData&    imageData,
                                const ScriptData&   scriptData )
{
    ...
    if( ext.empty() )
    {
        ...
        dotFileNameStream << tmpDirectory << "graph_" << s_count
                          << std::string( ".dot" );
    ...
    svgFileNameStream << tmpDirectory << osgDB::getStrippedName( filename ) << s_count
                      << std::string( ".svg" );
    ...
    else
    {
        osg::ref_ptr<osg::Node> model =
            osgDB::readRefNodeFile( filename, _options.get() );
        if( !model )
        {
            return;
        }
        ...
        dotFileName =
            tmpDirectory + osgDB::getStrippedName( filename ) + std::string( ".dot" );

        osg::ref_ptr<osgDB::Options> opts =
            _options.valid() ? _options->cloneOptions() : ( new osgDB::Options );
        if( !imageData.options.empty() )
        {
    ...
    {
        OSG_NOTICE << "Error: SlideShowConstructor::addGraph() system(" << command.str()
                   << ") failed with return " << result << std::endl;
    }
}

void
SlideShowConstructor::addVNC( const std::string&  hostname,
                              const PositionData& positionData,
                              const ImageData&    imageData,
                              ...
                              const ScriptData&   scriptData )
{
    if( !password.empty() )
    {
        ...
        osgDB::Registry::instance()->getAuthenticationMap()->addAuthenticationDetails(
            hostname,
            new osgDB::AuthenticationDetails( "", password )
        );
    }
...
void
SlideShowConstructor::addBrowser( const std::string&  url,
                                  const PositionData& positionData,
                                  const ImageData&    imageData,
                                  const ScriptData&   scriptData )
{
...
void
SlideShowConstructor::addPDF( const std::string&  filename,
                              const PositionData& positionData,
                              const ImageData&    imageData,
                              const ScriptData&   scriptData )
{
...
class SetPageCallback : public LayerCallback
{
    public:
        ...
        SetPageCallback( osgWidget::PdfImage* pdfImage,
                         int                  pageNum ) :
            _pdfImage( pdfImage ),
            _pageNum( pageNum )
        {
...
osg::ref_ptr<osg::Image>
SlideShowConstructor::addInteractiveImage( const std::string&  filename,
                                           const PositionData& positionData,
                                           const ImageData&    imageData,
                                           const ScriptData&   scriptData )
{
    ...
    {
        getOrCreateLayerAttributes( _currentLayer.get() )
            ->addEnterCallback( new SetPageCallback( pdfImage, imageData.page ) );
...
std::string
SlideShowConstructor::findFileAndRecordPath( const std::string& filename )
{
    ...
    if( !path.empty() && _filePathData.valid() )
    {
        osgDB::FilePathList::iterator itr =
            std::find( _filePathData->filePathList.begin(),
                       _filePathData->filePathList.end(),
                       path );
...
struct ClipRegionCallback : public osg::NodeCallback
{
    public:
        ...
        ClipRegionCallback( const osg::dmat4&  originalMatrix,
                            const std::string& str ) :
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( transform )
            {
                ...
                if( pr.ok() )
                {
                    OSG_NOTICE << "ClipRegionCallback : xMin=" << xMin
                               << ", yMin=" << yMin << ", zMin=" << zMin
                               << ", xMax=" << xMax << ", yMax=" << yMax
                               << ", zMax=" << zMax << std::endl;
                }
                ...
                {
                    OSG_NOTICE
                        << "Problem in reading, ClipRegionCallback : xMin=" << xMin
                        << ", yMin=" << yMin << ", zMin=" << zMin << ", xMax=" << xMax
                        << ", yMax=" << yMax << ", zMax=" << zMax << std::endl;
                }

                osg::dmat4 tm =
                    osg::dmat4( osg::scale( xMax - xMin, yMax - yMin, zMax - zMin ) ) *
                    osg::translate( static_cast<double>( xMin ),
            ...
            {
                OSG_NOTICE << "ClipRegionCallback not attached to MatrixTransform, "
                              "unable to update any values."
                           << std::endl;
            }
...
void
SlideShowConstructor::addModel( const std::string&  filename,
                                const PositionData& positionData,
                                ...
                                const ScriptData&   scriptData )
{
    ...
    {
        // Note: ClipNode/ClipPlane were removed (non-functional in core profile).
        // Region-based clipping would need shader-based gl_ClipDistance instead.
        ...
        osg::dmat4 matrix = osg::translate( -0.5, -0.5, -0.5 ) *
                            osg::scale( width, length, height ) *
                            osg::translate( bb.center() );
        transform->setMatrix( matrix );

        if( containsPropertyReference( modelData.region ) )
        {
            transform->addUpdateCallback( new ClipRegionCallback( matrix,
                                                                  modelData.region ) );
        }
        else
        {
            ...
            osg::dmat4 tm = osg::translate( region[0], region[1], region[2] ) *
                            osg::scale( region[3] - region[0],
                                        region[4] - region[1],
                                        region[5] - region[2] );

            transform->setMatrix( matrix * tm );
        }
    ...
    OSG_INFO << "end of SlideShowConstructor::addModel(" << filename << ")" << std::endl
             << std::endl;
}

osg::Node*
SlideShowConstructor::decorateSubgraphForPositionAndAnimation(
    osg::Node*          node,
    const PositionData& positionData
)
{
    osg::Node*                subgraph = node;
    osg::Object::DataVariance defaultMatrixDataVariance =
        osg::Object::DataVariance::DYNAMIC;    // STATIC

    OSG_INFO << "SlideShowConstructor::decorateSubgraphForPositionAndAnimation() "
             << std::endl;
    ...
    if( positionData.frame == SLIDE )
    {
        ...
        float              slide_scale =
            _slideHeight * ( 1.0F - positionData.position.z ) * 0.7F / bs.radius;

        osg::MatrixTransform* transform = new osg::MatrixTransform;
        transform->setDataVariance( defaultMatrixDataVariance );
        transform->setMatrix(
            osg::dmat4( osg::translate( -bs.center ) *
                        osg::scale( positionData.scale.x * slide_scale,
                                    positionData.scale.y * slide_scale,
                                    positionData.scale.z * slide_scale ) *
                        osg::rotate( osg::radians( positionData.rotate[0] ),
                                     positionData.rotate[1],
                                     positionData.rotate[2],
                                     positionData.rotate[3] ) *
                        osg::translate( pos ) )
        );
    ...
    {
        osg::mat4             matf( osg::scale( 1.0F / positionData.scale.x,
                                                1.0F / positionData.scale.y,
                                                1.0F / positionData.scale.z ) *
                                    osg::rotate( osg::radians( positionData.rotate[0] ),
                                                 positionData.rotate[1],
                                                 positionData.rotate[2],
                                                 positionData.rotate[3] ) *
                                    osg::translate( positionData.position ) );
        osg::dmat4            matrix( matf );

        osg::MatrixTransform* transform = new osg::MatrixTransform;
        transform->setDataVariance( defaultMatrixDataVariance );
        transform->setMatrix( osg::inverse( matrix ) );

        OSG_INFO << "Position dmat4 " << transform->getMatrix() << std::endl;
    ...
    if( positionData.rotation[0] != 0.0 )
    {
        ...
        OSG_INFO << "Rotation dmat4 " << animation_transform->getMatrix() << std::endl;
    ...
    if( animation )
    {
        ...
        if( positionData.animation_name ==
            "wheel" &&
            ( path->getTimeControlPointMap() ).size() >= 2 )
        {
            OSG_INFO << "****  Need to handle special wheel animation" << std::endl;

            osg::AnimationPath::TimeControlPointMap& controlPoints =
                path->getTimeControlPointMap();

            osg::AnimationPath::TimeControlPointMap::iterator curr_itr =
                controlPoints.begin();
            ...
            osg::dvec3 delta_position  = curr_cp->getPosition() - prev_cp->getPosition();
            float      rotation_z_axis = atan2f( delta_position.y, delta_position.x );

            osg::quat  quat_y_axis, quat_z_axis, quat_combined;

            quat_y_axis   = osg::quat( rotation_y_axis, osg::vec3( 0.0F, 1.0F, 0.0F ) );
            quat_z_axis   = osg::quat( rotation_z_axis, osg::vec3( 0.0F, 0.0F, 1.0F ) );
            quat_combined = quat_y_axis * quat_z_axis;

            // set first rotation.
            prev_cp->setRotation( quat_combined );

            for( ; curr_itr != controlPoints.end(); ++curr_itr )
            {
                ...
                totalLength    += osg::length( delta_position );
                ...
                OSG_INFO << " rotation_y_axis=" << rotation_y_axis
                         << " rotation_z_axis=" << rotation_z_axis << std::endl;

                quat_y_axis =
                    osg::quat( rotation_y_axis, osg::vec3( 0.0F, 1.0F, 0.0F ) );
                quat_z_axis =
                    osg::quat( rotation_z_axis, osg::vec3( 0.0F, 0.0F, 1.0F ) );
                quat_combined = quat_y_axis * quat_z_axis;
...
void
SlideShowConstructor::addModel( osg::Node*          subgraph,
                                const PositionData& positionData,
                                const ModelData&    modelData,
                                const ScriptData&   scriptData )
{
...
class DraggerVolumeTileCallback : public osgManipulator::DraggerCallback
{
    public:
        ...
        virtual bool
        receive( const osgManipulator::MotionCommand& command );
...
bool
DraggerVolumeTileCallback::receive( const osgManipulator::MotionCommand& command )
{
    if( !_locator )
    {
        return false;
        ...
        case osgManipulator::MotionCommand::START :
            {
                ...
                _localToWorld =
                    _startMotionMatrix * osg::computeLocalToWorld( nodePathToRoot );
        ...
        case osgManipulator::MotionCommand::MOVE :
            {
                ...
                osg::dmat4 localMotionMatrix = _localToWorld *
...
class VolumeTileCallback : public osg::Inherit<osg::NodeCallback, VolumeTileCallback>
{
    public:
        ...
        VolumeTileCallback( const VolumeTileCallback& vtc,
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( tile )
            {
                OSG_NOTICE << "VolumeTileCallback : Have locator matrix "
                           << locator->getTransform() << std::endl;
            }
...
struct VolumeRegionCallback : public osg::NodeCallback
{
    public:
        ...
        VolumeRegionCallback( const osg::dmat4&  originalMatrix,
                              const std::string& str ) :
            _matrix( originalMatrix ),
            _source( str )
        {
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( locator )
            {
                ...
                if( pr.ok() )
                {
                    OSG_NOTICE << "VolumeRegionCallback : xMin=" << xMin
                               << ", yMin=" << yMin << ", zMin=" << zMin
                               << ", xMax=" << xMax << ", yMax=" << yMax
                               << ", zMax=" << zMax << std::endl;
                }
                ...
                {
                    OSG_NOTICE
                        << "Problem in reading, VolumeRegionCallback : xMin=" << xMin
                        << ", yMin=" << yMin << ", zMin=" << zMin << ", xMax=" << xMax
                        << ", yMax=" << yMax << ", zMax=" << zMax << std::endl;
                }
            ...
            {
                OSG_NOTICE << "VolumeRegionCallback not attached to VolumeTile, unable "
...
struct ScalarPropertyCallback : public osg::NodeCallback
{
    public:
        ...
        ScalarPropertyCallback( osgVolume::ScalarProperty* sp,
                                const std::string&         str ) :
            _sp( sp ),
            _source( str )
        {
                    ...
                    osg::NodeVisitor* nv )
        {
            ...
            if( pr.ok() )
            {
                OSG_NOTICE << "ScalarPropertyCallback : value [" << _source
                           << "]=" << value << std::endl;
                _sp->setValue( value );
            }
            else
            {
                OSG_NOTICE << "Problem in reading, ScalarPropertyCallback : value="
                           << value << std::endl;
            }
                                      ...
                                      public osg::DualModeVisitor
{
        ...
        virtual void
        apply( osgVolume::VolumeSettings& vs )
        {
...
struct VolumeSettingsCallback : public osgGA::GUIEventHandler
{
                ...
                osg::NodeVisitor* /*nv*/ )
        {
            ...
            if( !node )
            {
                OSG_NOTICE << "Warning: VolumeSettingsCallback assigned to a node other "
                              "than VolumeTile, cannot operate edit/save."
                           << std::endl;
                return false;
            }
            ...
            if( ea.getEventType() == osgGA::GUIEventAdapter::KEYUP )
            {
                ...
                if( ea.getKey() == _saveKey )
                {
                    ...
                    for( CollectVolumeSettingsVisitor::VolumeSettingsList::iterator itr =
                             cvsv._vsList.begin();
                         ...
                         ++itr )
                    {
                        ...
                        if( !filename.empty() )
                        {
                            OSG_NOTICE << "Save VolumeSettings " << vs << " to filename "
                                       << filename << std::endl;
                            osgDB::writeObjectFile( *vs, filename );
                        }
                        else
                        {
                            OSG_NOTICE << "VolumeSettings " << vs
                                       << " with blank filename, saving to "
                                          "'no_filename_vs.osgt'"
                                       << std::endl;
                ...
                if( ea.getKey() == _editKey )
                {
                    ...
                    for( CollectVolumeSettingsVisitor::WidgetList::iterator itr =
                             cvsv._widgets.begin();
                         itr != cvsv._widgets.end();
                         ++itr )
                    {
                        osgUI::Widget* widget = itr->get();
                        OSG_NOTICE << "Toggling visibility of Widget " << widget
                                   << std::endl;
...
void
SlideShowConstructor::setUpVolumeScalarProperty( osgVolume::VolumeTile*     tile,
...
void
SlideShowConstructor::addVolume( const std::string&  filename,
                                 const PositionData& in_positionData,
                                 ...
                                 const ScriptData&   scriptData )
{
    ...
    {
        osgDB::DirectoryContents filenames =
            osgDB::expandWildcardsInFilename( filename );
        if( filenames.empty() )
        {
            return;
        }
    ...
    if( volumeData.colorSpaceOperation != osg::NO_COLOR_SPACE_OPERATION )
    {
        ...
        osg::ref_ptr<osg::Image> converted_image =
            osg::colorSpaceConversion( volumeData.colorSpaceOperation,
                                       image.get(),
                                       volumeData.colorModulate );
    ...
    if( !layer )
    {
        ...
        osg::ref_ptr<osgVolume::ImageDetails> details =
            dynamic_cast<osgVolume::ImageDetails*>( image->getUserData() );
        osg::ref_ptr<osg::RefMatrix> matrix =
            details ? details->getMatrix()
                    : dynamic_cast<osg::RefMatrix*>( image->getUserData() );
        ...
        if( !volumeData.region.empty() )
        {
            ...
            {
                tile->addUpdateCallback(
                    new VolumeRegionCallback( matrix.valid() ? *matrix : osg::dmat4(),
                                              volumeData.region )
                );
            }
            else
            {
                ...
                osg::dmat4 tm = osg::translate( static_cast<double>( region[0] ),
                                                static_cast<double>( region[1] ),
                                                static_cast<double>( region[2] ) ) *
                                osg::dmat4( osg::scale( region[3] - region[0],
                                                        region[4] - region[1],
                                                        region[5] - region[2] ) );

                if( matrix.valid() )
                {
                    tile->setLocator( new osgVolume::Locator( ( *matrix ) * tm ) );
                }
                else
                {
        ...
        osg::ref_ptr<osgVolume::CompositeProperty> groupPropetry =
        ...
        osg::ref_ptr<osgVolume::SampleRatioWhenMovingProperty> srm =
            vs.valid() ? vs->getSampleRatioWhenMovingProperty() : 0;
        if( !volumeData.sampleRatioWhenMovingValue.empty() )
        {
            srm = new osgVolume::SampleRatioWhenMovingProperty( 1.0 );
            setUpVolumeScalarProperty( tile.get(),
                                       srm.get(),
                                       volumeData.sampleRatioWhenMovingValue );
        }
        ...
        if( !volumeData.exteriorTransparencyFactorValue.empty() )
        {
            ...
            setUpVolumeScalarProperty( tile.get(),
                                       etfp.get(),
                                       volumeData.exteriorTransparencyFactorValue );
        }
        ...
        if( !volumeData.sampleDensityWhenMovingValue.empty() )
        {
            ...
            setUpVolumeScalarProperty( tile.get(),
                                       sdm.get(),
                                       volumeData.sampleDensityWhenMovingValue );
        }

        osg::ref_ptr<osgVolume::TransferFunctionProperty> tfp =
            volumeData.transferFunction.valid()
                ? new osgVolume::TransferFunctionProperty(
                      volumeData.transferFunction.get()
                  )
                : 0;
        ...
        {
            // Isosurface
            osgVolume::IsoSurfaceProperty* isp =
                vs.valid() ? vs->getIsoSurfaceProperty()
                           : new osgVolume::IsoSurfaceProperty( 0.1 );
        ...
        else
        {
            ...
            {
                case( osgVolume::VolumeSettings::Standard ) :
                    sp->setActiveProperty( 0 );
                    break;
                case( osgVolume::VolumeSettings::Light ) :
                    sp->setActiveProperty( 1 );
                    break;
                case( osgVolume::VolumeSettings::Isosurface ) :
                    sp->setActiveProperty( 2 );
                    break;
                case( osgVolume::VolumeSettings::MaximumIntensityProjection ) :
                    sp->setActiveProperty( 3 );
                    break;
            }
        }
#else
    ...
    if( !volumeData.hull.empty() )
    {
        osg::ref_ptr<osg::Node> hull =
            osgDB::readRefNodeFile( volumeData.hull, _options.get() );
        if( hull.valid() )
        {
            hull =
                decorateSubgraphForPositionAndAnimation( hull.get(),
                                                         volumeData.hullPositionData );
    ...
    if( volumeData.useTabbedDragger || volumeData.useTrackballDragger )
    {
        ...
        dragger->addDraggerCallback(
            new DraggerVolumeTileCallback( tile.get(), tile->getLocator() )
        );
        dragger->setMatrix( osg::translate( 0.5, 0.5, 0.5 ) *
                            tile->getLocator()->getTransform() );

        group->addChild( dragger.get() );
    }

#if 1
    osg::ref_ptr<osgUI::Widget> widget =
        vs.valid() ? osgDB::readRefFile<osgUI::Widget>( "ui/VolumeSettings.lua" ) : 0;
    if( widget )
    {
        ...
        const osg::box& bb  = widget->getExtents();
        float           slide_scale     = 0.5F * _slideWidth / ( bb.xMax() - bb.xMin() );

        osg::MatrixTransform* transform = new osg::MatrixTransform;
        transform->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        transform->setMatrix( osg::dmat4(
            osg::rotate( osg::radians( 90.0F ), osg::vec3( 1.0F, 0.0F, 0.0F ) ) *
            osg::scale( slide_scale, slide_scale, slide_scale ) *
            osg::translate( pos )
        ) );
...
osg::Node*
SlideShowConstructor::attachMaterialAnimation( osg::Node*          model,
                                               const PositionData& positionData )
{
...
#else
        animationMaterial = osgDB::readRefFile<AnimationMaterial>(
            positionData.animation_material_filename,
            _options.get()
        );
#endif
    }
    ...
    if( animationMaterial )
    {
        ...
        AnimationMaterialCallback* animationMaterialCallback =
            new AnimationMaterialCallback( animationMaterial.get() );
        animationMaterialCallback->setTimeOffset(
            positionData.animation_material_time_offset
        );
        animationMaterialCallback->setTimeMultiplier(
...
SlideShowConstructor::getAnimationPathCallback( const PositionData& positionData )
{
    if( !positionData.path.empty() )
    {
        // need to create an Options object with cache off to prevent sharing of
        // animation paths
        ...
        osg::ref_ptr<osg::Object> object =
            osgDB::readRefObjectFile( positionData.path, options.get() );
        osg::AnimationPath* animation =
            dynamic_cast<osg::AnimationPath*>( object.get() );
        if( animation )
        {
            if( positionData.frame == SlideShowConstructor::SLIDE )
            {
                osg::AnimationPath::TimeControlPointMap& controlPoints =
                    animation->getTimeControlPointMap();
                for( osg::AnimationPath::TimeControlPointMap::iterator itr =
                         controlPoints.begin();
                     itr != controlPoints.end();
                     ++itr )
                {
                    osg::AnimationPath::ControlPoint& cp = itr->second;
                    cp.setPosition( osg::dvec3( convertSlideToModel(
                        osg::vec3( cp.getPosition() ) + positionData.position
                    ) ) );
                }
            }
...
osg::vec3
SlideShowConstructor::computePositionInModelCoords(
    const PositionData& positionData
) const
{
    if( positionData.frame == SLIDE )
...
osg::vec3
SlideShowConstructor::convertSlideToModel( const osg::vec3& position ) const
{
    return osg::vec3( _slideOrigin + osg::vec3( _slideWidth * position.x,
                                                0.0F,
                                                _slideHeight * position.y ) ) *
           ( 1.0F - position.z );
}

osg::vec3
SlideShowConstructor::convertModelToSlide( const osg::vec3& position ) const
{
    return osg::vec3(
        ( position.x * ( _slideOrigin.y / position.y ) - _slideOrigin.x ) / _slideWidth,
...
void
SlideShowConstructor::updatePositionFromInModelCoords( const osg::vec3& vertex,
...
void
SlideShowConstructor::recordOptionsFilePath( const osgDB::Options* options )
{
    if( options )
    {
        ...
        if( !path.empty() && _filePathData.valid() )
        {
            ...
            if( itr == _filePathData->filePathList.end() )
            {
                OSG_INFO << "SlideShowConstructor::recordOptionsFilePath(..) - new path "
                            "to record path="
                         << path << " filename_used=" << filename_used << std::endl;
                _filePathData->filePathList.push_front( path );
            }
        }
    }
