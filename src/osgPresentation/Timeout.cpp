...
class OperationVisitor : public osg::DualModeVisitor
{
    public:
...
HUDSettings::HUDSettings( double       slideDistance,
                          float        eyeOffset,
...
bool
HUDSettings::getModelViewMatrix( osg::dmat4&       matrix,
                                 osg::NodeVisitor* nv ) const
{
    matrix = osg::lookAt( osg::dvec3( 0.0, 0.0, 0.0 ),
                          osg::dvec3( 0.0, _slideDistance, 0.0 ),
    ...
    if( nv )
    {
        ...
        {
            osg::postMultTranslate( matrix, osg::dvec3( _eyeOffset, 0.0, 0.0 ) );
        }
...
bool
HUDSettings::getInverseModelViewMatrix( osg::dmat4&       matrix,
                                        osg::NodeVisitor* nv ) const
{
    osg::dmat4 modelView;
...
bool
Timeout::computeLocalToWorldMatrix( osg::dmat4&       matrix,
                                    osg::NodeVisitor* nv ) const
{
...
bool
Timeout::computeWorldToLocalMatrix( osg::dmat4&       matrix,
                                    osg::NodeVisitor* nv ) const
{
    if( _hudSettings.valid() )
    {
...
void
Timeout::broadcastEvent( osgViewer::Viewer*                  viewer,
...
void
Timeout::traverse( osg::NodeVisitor& nv )
{
    ...
    {
        int deltaFrameNumber =
            ( nv.getFrameStamp()->getFrameNumber() - _previousFrameNumber );
        ...
        osgViewer::Viewer*   viewer =
            ev ? dynamic_cast<osgViewer::Viewer*>( ev->getActionAdapter() ) : 0;
        if( ev )
        {
                 ...
                 ++itr )
            {
                ...
                bool keyEvent = event->getEventType() ==
                                osgGA::GUIEventAdapter::KEYDOWN ||
                                event->getEventType() == osgGA::GUIEventAdapter::KEYUP;
        ...
        double timeSinceLastEvent =
            nv.getFrameStamp()
                ? nv.getFrameStamp()->getReferenceTime() - _timeOfLastEvent
                : 0.0;
        ...
        {
            if( viewer && ( _displayBroadcastKeyPos._key !=
                            0 ||
                            _displayBroadcastKeyPos._x !=
                            FLT_MAX ||
                            _displayBroadcastKeyPos._y != FLT_MAX ) )
            {
                OSG_NOTICE << "Doing display broadcast key event"
                           << _displayBroadcastKeyPos._key << std::endl;
            ...
            if( leave.sleepTime() != 0.0 )
            {
                ...
                osg::Thread::microSleep(
                    static_cast<unsigned int>( 1000000.0 * leave.sleepTime() )
                );
        ...
        if( needToDismiss )
        {
            if( viewer && ( _dismissBroadcastKeyPos._key !=
                            0 ||
                            _dismissBroadcastKeyPos._x !=
                            FLT_MAX ||
                            _dismissBroadcastKeyPos._y != FLT_MAX ) )
            {
                OSG_NOTICE << "Doing dismiss broadcast key event"
                           << _dismissBroadcastKeyPos._key << std::endl;
        ...
        if( needToAction )
        {
            ...
            if( _actionKeyPos._key !=
                0 ||
                _actionKeyPos._x !=
                FLT_MAX ||
                _actionKeyPos._y != FLT_MAX )
            {
                OSG_NOTICE << "Doing timeout key event" << _actionKeyPos._key
                           << std::endl;
            ...
            if( viewer && ( _actionBroadcastKeyPos._key !=
                            0 ||
                            _actionBroadcastKeyPos._x !=
                            FLT_MAX ||
                            _actionBroadcastKeyPos._y != FLT_MAX ) )
            {
                OSG_NOTICE << "Doing timeout broadcast key event"
                           << _actionBroadcastKeyPos._key << std::endl;
    ...
    else
    {
        ...
        {
            OSG_NOTICE << "Timeout::traverse() " << nv.className()
                       << ", ignoring traversal" << std::endl;
        }
        else
        {
            Transform::traverse( nv );
