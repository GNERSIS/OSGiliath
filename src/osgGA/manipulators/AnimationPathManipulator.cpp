...
AnimationPathManipulator::AnimationPathManipulator( const std::string& filename )
{
    ...
    if( !in )
    {
        OSG_WARN << "AnimationPathManipulator: Cannot open animation path file \""
                 << filename << "\".\n";
        _valid = false;
        return;
    }
...
void
AnimationPathManipulator::home( double currentTime )
{
...
void
AnimationPathManipulator::home( const GUIEventAdapter& ea,
                                GUIActionAdapter& )
{
    home( ea.getTime() );
}
...
void
AnimationPathManipulator::init( const GUIEventAdapter& ea,
                                GUIActionAdapter&      aa )
{
    home( ea, aa );
}
...
bool
AnimationPathManipulator::handle( const osgGA::GUIEventAdapter& ea,
    ...
    switch( ea.getEventType() )
    {
        ...
        case GUIEventAdapter::KEYDOWN :
            ...
            else if( ea.getKey() == ')' )
            {
                ...
                OSG_NOTICE << "Animation speed = "
                           << _timeScale * 100 << "%" << std::endl;
...
AnimationPathManipulator::getUsage( osg::ApplicationUsage& usage ) const
{
    usage.addKeyboardMouseBinding( "AnimationPath: Space",
                                   "Reset the viewing position to start of animation" );
...
void
AnimationPathManipulator::handleFrame( double time )
{
    ...
    if( animDelta >= _animationPath->getPeriod() )
    {
        ...
        if( _printOutTimingInfo )
        {
            ...
            OSG_NOTICE << "AnimatonPath completed in " << delta
                       << " seconds, completing " << _numOfFramesSinceStartOfTimedPeriod
                       << " frames, average frame rate = " << frameRate << std::endl;
        }
