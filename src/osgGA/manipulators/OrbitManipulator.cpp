...
/// Constructor.
OrbitManipulator::OrbitManipulator( const OrbitManipulator& om,
                                    const CopyOp&           copyOp ) :
    osg::Object( om,
                 copyOp ),
    osg::Callback( om,
...
void
OrbitManipulator::setByMatrix( const osg::dmat4& matrix )
{
    _center   = matrix * osg::dvec3( 0., 0., -_distance );
...
/** Set the position of the manipulator using a 4x4 matrix.*/
void
OrbitManipulator::setByInverseMatrix( const osg::dmat4& matrix )
{
...
osg::dmat4
OrbitManipulator::getMatrix() const
{
    return osg::translate( _center ) *
           osg::rotate( _rotation ) *
           osg::translate( 0., 0., _distance );
}

/** Get the position of the manipulator as a inverse matrix of the manipulator,
    typically used as a model view matrix.*/
osg::dmat4
OrbitManipulator::getInverseMatrix() const
{
    return osg::translate( 0.0, 0.0, -_distance ) *
           osg::rotate( osg::inverse( _rotation ) ) *
           osg::translate( -_center );
}

// doc in parent
void
OrbitManipulator::setTransformation( const osg::dvec3& eye,
                                     const osg::quat&  rotation )
{
    _center   = eye + rotation * osg::dvec3( 0., 0., -_distance );
    _rotation = rotation;
...
// doc in parent
void
OrbitManipulator::getTransformation( osg::dvec3& eye,
                                     osg::quat&  rotation ) const
{
    eye      = _center - _rotation * osg::dvec3( 0., 0., -_distance );
    rotation = _rotation;
}
...
/** Sets the transformation by heading. Heading is given as an angle in radians giving a
   azimuth in xy plane. Its meaning is similar to longitude used in cartography and
   navigation. Positive number is going to the east direction.*/
void
OrbitManipulator::setHeading( double azimuth )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    dvec3           localUp         = getUpVector( coordinateFrame );
    dvec3           localRight      = getSideVector( coordinateFrame );

    dvec3           dir             = dquat( getElevation(), localRight ) *
                                      dquat( azimuth, localUp ) *
                                      dvec3( 0., -_distance, 0. );
...
double
OrbitManipulator::getHeading() const
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    dvec3           localFront      = getFrontVector( coordinateFrame );
    dvec3           localRight      = getSideVector( coordinateFrame );

    dvec3           center, eye, tmp;
...
/** Sets the transformation by elevation. Elevation is given as an angle in radians from
   xy plane. Its meaning is similar to latitude used in cartography and navigation.
    Positive number is going to the north direction, negative to the south.*/
void
OrbitManipulator::setElevation( double elevation )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    dvec3           localUp         = getUpVector( coordinateFrame );
    dvec3           localRight      = getSideVector( coordinateFrame );

    dvec3           dir             = dquat( -elevation, localRight ) *
                                      dquat( getHeading(), localUp ) *
                                      dvec3( 0., -_distance, 0. );
...
double
OrbitManipulator::getElevation() const
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    dvec3           localUp         = getUpVector( coordinateFrame );
    localUp                         = osg::normalize( localUp );
    ...
    return asin( -dist / osg::length( eye - center ) );
}

// doc in parent
bool
OrbitManipulator::handleMouseWheel( const GUIEventAdapter& ea,
                                    GUIActionAdapter&      us )
{
    ...
    switch( sm )
    {
        ...
        case GUIEventAdapter::SCROLL_UP :
            {
                // perform zoom
                zoomModel( static_cast<float>( _wheelZoomFactor ), true );
                us.requestRedraw();
...
// doc in parent
bool
OrbitManipulator::performMovementLeftMouseButton( const double eventTimeDelta,
                                                  const double dx,
                                                  const double dy )
{
    // rotate camera
...
// doc in parent
bool
OrbitManipulator::performMovementMiddleMouseButton( const double eventTimeDelta,
                                                    const double dx,
                                                    const double dy )
{
    // pan model
    float scale =
        static_cast<float>( -0.3 * _distance * getThrowScale( eventTimeDelta ) );
...
// doc in parent
bool
OrbitManipulator::performMovementRightMouseButton( const double eventTimeDelta,
                                                   const double /*dx*/,
                                                   const double dy )
{
    // zoom model
    zoomModel( static_cast<float>( dy * getThrowScale( eventTimeDelta ) ), true );
    return true;
}

bool
OrbitManipulator::performMouseDeltaMovement( const float dx,
                                             const float dy )
{
    // rotate camera
    if( getVerticalAxisFixed() )
...
void
OrbitManipulator::applyAnimationStep( const double currentProgress,
                                      const double prevProgress )
{
    ...
    // compute new center
    osg::dvec3 prevCenter, prevEye, prevUp;
    getTransformation( prevEye, prevCenter, prevUp );
    osg::dvec3 newCenter = osg::dvec3( prevCenter ) +
                           ( ad->_movement * ( currentProgress - prevProgress ) );
...
bool
OrbitManipulator::startAnimationByMousePointerIntersection(
...
)
{
    ...
    ad->start( osg::dvec3( _center ) - prevCenter, ea.getTime() );
...
void
OrbitManipulator::OrbitAnimationData::start( const osg::dvec3& movement,
...
void
OrbitManipulator::rotateTrackball( const float px0,
                                   const float py0,
                                   const float px1,
                                   const float py1,
                                   const float scale )
{
    ...
    quat new_rotate;
    new_rotate = quat( dquat( static_cast<double>( angle ), axis ) );
...
void
OrbitManipulator::rotateWithFixedVertical( const float dx,
                                           const float dy )
{
    CoordinateFrame coordinateFrame = getCoordinateFrame( _center );
    dvec3           localUp         = getUpVector( coordinateFrame );
...
OrbitManipulator::rotateWithFixedVertical( const float dx,
                                           ...
                                           const vec3& up )
{
    rotateYawPitch( _rotation, dx, dy, dvec3( up ) );
}

/** Moves camera in x,y,z directions given in camera local coordinates.*/
void
OrbitManipulator::panModel( const float dx,
                            const float dy,
                            const float dz )
{
    dmat4 rotation_matrix = osg::rotate( dquat( _rotation ) );
 ...
 */
void
OrbitManipulator::zoomModel( const float dy,
                             bool        pushForwardIfNeeded )
{
    ...
    // minimum distance
    float minDist = static_cast<float>( _minimumDistance );
    if( getRelativeFlag( _minimumDistanceFlagIndex ) )
    {
        minDist *= static_cast<float>( _modelSize );
    }
    ...
    else
    {
        if( pushForwardIfNeeded )
        {
            ...
            float yscale          = static_cast<float>( -_distance );
            dmat4 rotation_matrix = osg::rotate( dquat( _rotation ) );
            dvec3 dv =
                ( dvec3( 0.0F, 0.0F, -1.0F ) * rotation_matrix ) * ( dy * yscale );
            _center += dv;
        }
        else
        {
 ...
 */
void
OrbitManipulator::trackball( osg::dvec3& axis,
                             float&      angle,
                             float       p1x,
                             float       p1y,
                             float       p2x,
                             float       p2y )
{
    ...
    osg::dmat4 rotation_matrix = osg::rotate( dquat( _rotation ) );

    osg::dvec3 uv              = dvec3( 0.0F, 1.0F, 0.0F ) * rotation_matrix;
    osg::dvec3 sv              = dvec3( 1.0F, 0.0F, 0.0F ) * rotation_matrix;
    osg::dvec3 lv              = dvec3( 0.0F, 0.0F, -1.0F ) * rotation_matrix;
    ...
    osg::dvec3 p1 =
        ...
        p1y -
        lv *
        tb_project_to_sphere( static_cast<float>( _trackballSize ), p1x, p1y );
    osg::dvec3 p2 =
        sv *
        p2x +
        uv *
        p2y -
        lv *
        tb_project_to_sphere( static_cast<float>( _trackballSize ), p2x, p2y );
     ...
     */
    float t = static_cast<float>( osg::length( p2 - p1 ) / ( 2.0 * _trackballSize ) );
 ...
 */
float
OrbitManipulator::tb_project_to_sphere( float r,
                                        float x,
                                        float y )
{
    float d, t, z;
...
OrbitManipulator::getFusionDistanceMode() const
{
...
float
OrbitManipulator::getFusionDistanceValue() const
{
...
void
OrbitManipulator::setCenter( const dvec3& center )
{
    _center = center;
}
...
const dvec3&
OrbitManipulator::getCenter() const
{
    return _center;
}
...
void
OrbitManipulator::setRotation( const quat& rotation )
{
    _rotation = rotation;
}
...
const quat&
OrbitManipulator::getRotation() const
{
    return _rotation;
}
...
void
OrbitManipulator::setDistance( double distance )
{
    _distance = distance;
}
...
double
OrbitManipulator::getDistance() const
{
    return _distance;
}
...
void
OrbitManipulator::setTrackballSize( const double& size )
{
    /*
     ...
     */
    _trackballSize = size;
    clampBetweenRange( _trackballSize,
                       0.1,
                       1.0,
                       "TrackballManipulator::setTrackballSize(float)" );
}
...
/** Set the mouse wheel zoom factor.
    ...
    For example, value of 0.1 will short distance to center by 10% on each wheel up
   event. Use negative value for reverse mouse wheel direction.*/
void
OrbitManipulator::setWheelZoomFactor( double wheelZoomFactor )
{
...
void
OrbitManipulator::setMinimumDistance( const double& minimumDistance,
...
/** Get the minimum distance of the eye point from the center
    before the center is pushed forward.*/
double
OrbitManipulator::getMinimumDistance( bool* relativeToModelSize ) const
{
