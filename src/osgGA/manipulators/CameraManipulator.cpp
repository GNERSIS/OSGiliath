...
{
    _intersectTraversalMask  = 0XFF'FF'FF'FF;
...
CameraManipulator::CameraManipulator( const CameraManipulator& mm,
                                      const CopyOp&            copyOp ) :
    osg::Object( mm,
                 copyOp ),
    osg::Callback( mm,
    ...
    _homeUp( mm._homeUp ),
    _coordinateFrameCallback( dynamic_cast<CoordinateFrameCallback*>(
        copyOp( mm._coordinateFrameCallback.get() )
    ) )
{
}
...
std::string
CameraManipulator::getManipulatorName() const
{
    ...
    {
        return std::string( className, static_cast<size_t>( manipString - className ) );
    }
}

bool
CameraManipulator::handle( const GUIEventAdapter&,
                           GUIActionAdapter& )
{
    return false;
}
...
void
CameraManipulator::computeHomePosition( const osg::Camera* camera,
                                        bool               useBoundingBox )
{
    if( getNode() )
    {
        osg::sphere boundingSphere;

        OSG_INFO << " CameraManipulator::computeHomePosition(" << camera << ", "
                 << useBoundingBox << ")" << std::endl;
        ...
        if( useBoundingBox )
        {
            ...
            osg::box& bb = cbVisitor.getBoundingBox();
        ...
        OSG_INFO << "    boundingSphere.center = (" << boundingSphere.center << ")"
                 << std::endl;
        OSG_INFO << "    boundingSphere.radius = " << boundingSphere.radius << std::endl;

        double radius = std::max( double( boundingSphere.radius ), 1E-6 );
        ...
        if( camera )
        {
            ...
            if( camera->getProjectionMatrixAsFrustum( left,
                                                      right,
                                                      bottom,
                                                      top,
                                                      zNear,
            ...
            else
            {
                ...
                if( camera->getProjectionMatrixAsOrtho( left,
                                                        right,
                                                        bottom,
                                                        top,
                                                        zNear,
