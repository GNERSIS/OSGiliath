...
/// Constructor.
MultiTouchTrackballManipulator::MultiTouchTrackballManipulator(
    const MultiTouchTrackballManipulator& tm,
    const CopyOp&                         copyOp
) :
    osg::Object( tm,
                 copyOp ),
...
void
MultiTouchTrackballManipulator::handleMultiTouchDrag( const GUIEventAdapter* now,
                                                      const GUIEventAdapter* last,
                                                      const double eventTimeDelta )
{
    const osg::vec2 pt_1_now( now->getTouchData()->get( 0 ).x,
                              now->getTouchData()->get( 0 ).y );
    const osg::vec2 pt_2_now( now->getTouchData()->get( 1 ).x,
                              now->getTouchData()->get( 1 ).y );
    const osg::vec2 pt_1_last( last->getTouchData()->get( 0 ).x,
                               last->getTouchData()->get( 0 ).y );
    const osg::vec2 pt_2_last( last->getTouchData()->get( 1 ).x,
                               last->getTouchData()->get( 1 ).y );

    const float     gap_now( osg::length( pt_1_now - pt_2_now ) );
    const float     gap_last( osg::length( pt_1_last - pt_2_last ) );
    ...
    // drag gesture
    const osg::vec2 delta =
        ( ( pt_1_last - pt_1_now ) + ( pt_2_last - pt_2_now ) ) / 2.0F;
...
bool
MultiTouchTrackballManipulator::handle( const GUIEventAdapter& ea,
                                        GUIActionAdapter&      us )
{
    ...
    switch( ea.getEventType() )
    {
        ...
        case osgGA::GUIEventAdapter::RELEASE :
            ...
            {
                double eventTimeDelta =
                    1 / 60.0;    //_ga_t0->getTime() - _ga_t1->getTime();
                if( eventTimeDelta < 0. )
                {
                ...
                if( ( data->getNumTouchPoints() == 3 ) ||
                    ( ( data->getNumTouchPoints() == 1 ) &&
                      ( data->get( 0 ).tapCount >= 2 ) ) )
                {
                ...
                for( osgGA::GUIEventAdapter::TouchData::iterator i = data->begin();
                     i != data->end();
                     ++i )
                {
