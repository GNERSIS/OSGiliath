...
void
StateSetManipulator::setStateSet( StateSet* stateset )
{
    _stateset = stateset;
#if 0
    ...
    _stateset->setDataVariance(osg::Object::DataVariance::DYNAMIC);
#endif
}

StateSet*
StateSetManipulator::getStateSet()
{
    return _stateset.get();
}

const StateSet*
StateSetManipulator::getStateSet() const
{
    return _stateset.get();
}
...
StateSetManipulator::clone()
{
    ...
    // we clone the StateSet so that any draw traversals that might be running at the
    // time of the event traversal won't change the same StateSet that is being read. One
    // could just set the DataVariance to DYNAMIC to avoid this overlap, but this would
    // introduce a performance penalty.

    StateSet::ParentList        parents = _stateset->getParents();
    osg::ref_ptr<osg::StateSet> newStateSet =
        dynamic_cast<osg::StateSet*>( _stateset->clone( osg::CopyOp::SHALLOW_COPY ) );

    // change the parents of the original StateSet to point to the new stateset
    for( StateSet::ParentList::iterator itr = parents.begin(); itr != parents.end();
         ++itr )
    {
        osg::Node* node = *itr;
...
bool
StateSetManipulator::handle( const GUIEventAdapter& ea,
                             GUIActionAdapter&      aa )
{
    if( !_stateset.valid() )
    {
    ...
    if( !_initialized )
    {
        ...
        _texture = ( _stateset->getTextureMode( 0, GL_TEXTURE_RECTANGLE ) & mode ) !=
                   0 ||
...
void
StateSetManipulator::getUsage( osg::ApplicationUsage& usage ) const
{
    usage.addKeyboardMouseBinding(
        reinterpret_cast<const char*>( &_keyEventToggleBackfaceCulling ),
        "Toggle backface culling"
    );
    usage.addKeyboardMouseBinding(
        reinterpret_cast<const char*>( &_keyEventToggleLighting ),
        "Toggle lighting"
    );
    usage.addKeyboardMouseBinding(
        reinterpret_cast<const char*>( &_keyEventToggleTexturing ),
        "Toggle texturing"
    );
    usage.addKeyboardMouseBinding(
...
void
StateSetManipulator::setBackfaceEnabled( bool newbackface )
{
    ...
    {
        _stateset->setMode( GL_CULL_FACE,
                            osg::StateAttribute::OVERRIDE | osg::StateAttribute::OFF );
    }
}

void
StateSetManipulator::setLightingEnabled( bool newlighting )
{
...
void
StateSetManipulator::setTextureEnabled( bool newtexture )
{
    ...
    //     ( _stateset->getAttribute( osg::StateAttribute::Type::TEXTURE ) );
    ...
    // (texture enable/disable does not exist; textures are always available when bound)
...
void
StateSetManipulator::setPolygonMode( osg::PolygonMode::Mode newpolygonmode )
{
    clone();

    osg::PolygonMode* polyModeObj = getOrCreatePolygonMode();

    polyModeObj->setMode( osg::PolygonMode::Face::FRONT_AND_BACK, newpolygonmode );
}

void
StateSetManipulator::cyclePolygonMode()
{
    clone();
    ...
    switch( currentMode )
    {
        case osg::PolygonMode::Mode::FILL :
            polyModeObj->setMode( osg::PolygonMode::Face::FRONT_AND_BACK,
                                  osg::PolygonMode::Mode::LINE );
            break;
        case osg::PolygonMode::Mode::LINE :
            polyModeObj->setMode( osg::PolygonMode::Face::FRONT_AND_BACK,
                                  osg::PolygonMode::Mode::POINT );
            break;
        case osg::PolygonMode::Mode::POINT :
            polyModeObj->setMode( osg::PolygonMode::Face::FRONT_AND_BACK,
...
StateSetManipulator::getPolygonMode() const
{
    osg::PolygonMode* polyModeObj = dynamic_cast<osg::PolygonMode*>(
        _stateset->getAttribute( osg::StateAttribute::Type::POLYGONMODE )
    );
    if( polyModeObj )
    {
        return polyModeObj->getMode( osg::PolygonMode::Face::FRONT_AND_BACK );
    }
    else
    {
        return osg::PolygonMode::Mode::FILL;
    }
}

osg::PolygonMode*
StateSetManipulator::getOrCreatePolygonMode()
{
    osg::PolygonMode* polyModeObj = dynamic_cast<osg::PolygonMode*>(
        _stateset->getAttribute( osg::StateAttribute::Type::POLYGONMODE )
    );
