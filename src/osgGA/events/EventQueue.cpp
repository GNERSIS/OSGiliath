...
void
EventQueue::setEvents( Events& events )
{
    std::lock_guard<std::mutex> lock( _eventQueueMutex );
    _eventQueue = events;
}
...
void
EventQueue::appendEvents( Events& events )
{
    std::lock_guard<std::mutex> lock( _eventQueueMutex );
...
bool
EventQueue::takeEvents( Events& events )
{
    std::lock_guard<std::mutex> lock( _eventQueueMutex );
                        ...
                        double  cutOffTime )
{
    ...
    if( !_eventQueue.empty() )
    {
        ...
        for( ; ritr != _eventQueue.rend() && ( ( *ritr )->getTime() > cutOffTime );
             ++ritr )
        {
        }
        ...
        for( Events::iterator itr = _eventQueue.begin(); itr != ritr.base(); ++itr )
        {
            events.push_back( *itr );
        }

        // make sure that the events are in ascending time order, and any out of out
        // events have their time reset to the next valid time after them in the events
        // list.
        double previousTime = cutOffTime;
        for( Events::reverse_iterator itr = events.rbegin(); itr != events.rend();
             ++itr )
        {
            if( ( *itr )->getTime() > previousTime )
            {
                OSG_INFO << "Reset event time from " << ( *itr )->getTime() << " to "
...
bool
EventQueue::copyEvents( Events& events ) const
{
    std::lock_guard<std::mutex> lock( _eventQueueMutex );
    if( !_eventQueue.empty() )
    {
...
void
EventQueue::syncWindowRectangleWithGraphicsContext()
{
    const osg::GraphicsContext::Traits* traits =
        ( getGraphicsContext() != 0 ) ? getGraphicsContext()->getTraits() : 0;
    if( traits )
    {
        _accumulateEventState->setWindowRectangle( traits->x,
                                                   traits->y,
                                                   traits->width,
                                                   traits->height,
...
EventQueue::windowResize( int    x,
                          int    y,
                          int    width,
                          int    height,
                          double time )
{
    _accumulateEventState->setWindowRectangle( x,
                                               y,
                                               width,
                                               height,
                                               !_useFixedMouseInputRange );
...
osgGA::GUIEventAdapter*
EventQueue::penPressure( float  pressure,
                         double time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
...
osgGA::GUIEventAdapter*
EventQueue::penOrientation( float  tiltX,
                            float  tiltY,
                            float  rotation,
                            double time )
{
...
osgGA::GUIEventAdapter*
EventQueue::penProximity( GUIEventAdapter::TabletPointerType pt,
                          bool                               isEntering,
                          double                             time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
    event->setEventType( ( isEntering ) ? GUIEventAdapter::PEN_PROXIMITY_ENTER
                                        : GUIEventAdapter::PEN_PROXIMITY_LEAVE );
...
osgGA::GUIEventAdapter*
EventQueue::mouseScroll( GUIEventAdapter::ScrollingMotion sm,
                         double                           time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
...
osgGA::GUIEventAdapter*
EventQueue::mouseScroll2D( float  x,
                           float  y,
                           double time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
...
void
EventQueue::mouseWarped( float x,
                         float y )
{
...
osgGA::GUIEventAdapter*
EventQueue::mouseMotion( float  x,
                         float  y,
                         double time )
{
    _accumulateEventState->setX( x );
    ...
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
    event->setEventType( event->getButtonMask() ? GUIEventAdapter::DRAG
                                                : GUIEventAdapter::MOVE );
    event->setTime( time );
...
osgGA::GUIEventAdapter*
EventQueue::mouseButtonPress( float        x,
                              float        y,
                              unsigned int button,
                              double       time )
{
    ...
    {
        case( 1 ) :
            _accumulateEventState->setButtonMask(
...
osgGA::GUIEventAdapter*
EventQueue::mouseDoubleButtonPress( float        x,
                                    float        y,
                                    unsigned int button,
                                    double       time )
{
...
osgGA::GUIEventAdapter*
EventQueue::mouseButtonRelease( float        x,
                                float        y,
                                unsigned int button,
                                double       time )
{
    ...
    switch( unmodifiedKey )
    {
        case( GUIEventAdapter::KEY_Shift_L ) :
            _accumulateEventState->setModKeyMask(
                ...
                _accumulateEventState->getModKeyMask()
            );
            break;
        ...
        case( GUIEventAdapter::KEY_Control_L ) :
            ...
            break;
        case( GUIEventAdapter::KEY_Control_R ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_RIGHT_CTRL |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Meta_L ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_LEFT_META |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Meta_R ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_RIGHT_META |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Alt_L ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_LEFT_ALT | _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Alt_R ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_RIGHT_ALT |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Super_L ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_LEFT_SUPER |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Super_R ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_RIGHT_SUPER |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Hyper_L ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_LEFT_HYPER |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Hyper_R ) :
            _accumulateEventState->setModKeyMask(
                GUIEventAdapter::MODKEY_RIGHT_HYPER |
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Caps_Lock ) :
            {
                if( ( _accumulateEventState->getModKeyMask() &
                      GUIEventAdapter::MODKEY_CAPS_LOCK ) != 0 )
                {
                    _accumulateEventState->setModKeyMask(
                        ~GUIEventAdapter::MODKEY_CAPS_LOCK &
                        _accumulateEventState->getModKeyMask()
                    );
                }
                else
        ...
        case( GUIEventAdapter::KEY_Num_Lock ) :
            {
                if( ( _accumulateEventState->getModKeyMask() &
                      GUIEventAdapter::MODKEY_NUM_LOCK ) != 0 )
                {
                    _accumulateEventState->setModKeyMask(
                        ~GUIEventAdapter::MODKEY_NUM_LOCK &
                        _accumulateEventState->getModKeyMask()
                    );
                }
                else
                        ...
                        int    unmodifiedKey )
{
    switch( unmodifiedKey )
    {
        ...
        case( GUIEventAdapter::KEY_Alt_L ) :
            _accumulateEventState->setModKeyMask(
                ~GUIEventAdapter::MODKEY_LEFT_ALT &
                _accumulateEventState->getModKeyMask()
            );
            break;
        case( GUIEventAdapter::KEY_Alt_R ) :
...
GUIEventAdapter*
EventQueue::touchBegan( unsigned int                id,
...
GUIEventAdapter*
EventQueue::touchMoved( unsigned int                id,
                        GUIEventAdapter::TouchPhase phase,
                        float                       x,
                        float                       y,
                        double                      time )
...
GUIEventAdapter*
EventQueue::touchEnded( unsigned int                id,
                        GUIEventAdapter::TouchPhase phase,
                        float                       x,
                        float                       y,
...
osgGA::GUIEventAdapter*
EventQueue::closeWindow( double time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
...
osgGA::GUIEventAdapter*
EventQueue::quitApplication( double time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
...
GUIEventAdapter*
EventQueue::userEvent( osg::Referenced* userEventData,
                       double           time )
{
    GUIEventAdapter* event = new GUIEventAdapter( *_accumulateEventState );
