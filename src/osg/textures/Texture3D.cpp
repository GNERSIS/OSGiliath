...
int
Texture3D::compare( const StateAttribute& sa ) const
{
    // check the types are equal and then create the rhs variable
    // used by the COMPARE_StateAttribute_Parameter macros below.
    COMPARE_StateAttribute_Types( Texture3D,
                                  sa ) if( _image !=
...
void
Texture3D::setImage( Image* image )
{
    if( _image == image )
    {
        return;
...
void
Texture3D::computeRequiredTextureDimensions( State&            state,
                                             ...
                                             GLsizei&          numMipmapLevels ) const
{
    ...
    else
    {
        ...
        height = Image::computeNearestPowerOfTwo( image.t() - 2 * _borderWidth ) +
                 2 *
                 _borderWidth;
        depth  = Image::computeNearestPowerOfTwo( image.r() - 2 * _borderWidth ) +
                 2 *
                 _borderWidth;
    }
    ...
    bool useHardwareMipMapGeneration = !image.isMipmap() &&
...
void
Texture3D::apply( State& state ) const
{
    ...
    {
        OSG_WARN << "Warning: Texture3D::apply(..) failed, 3D texturing is not support "
                    "by OpenGL driver."
                 << std::endl;
        return;
    }
    ...
    if( textureObject )
    {
            ...
            getModifiedCount( contextID ) != _image->getModifiedCount() )
        {
            ...
            if( !textureObject->match( GL_TEXTURE_3D,
                                       new_numMipmapLevels,
             ...
             ( _internalFormat != 0 ) )
    {
        ...
        if( texStorageSizedInternalFormat != 0 )
        {
            ...
            {
                extensions->glTexStorage3D( GL_TEXTURE_3D,
        ...
        else
        {
            ...
            extensions->glTexImage3D( GL_TEXTURE_3D,
                                      0,
                                      static_cast<GLenum>( _internalFormat ),
                                      _textureWidth,
                                      _textureHeight,
...
void
Texture3D::computeInternalFormat() const
{
    if( _image.valid() )
    {
...
void
Texture3D::applyTexImage3D( GLenum   target,
                            Image*   image,
                            State&   state,
                            GLsizei& inwidth,
                            GLsizei& inheight,
                            ...
                            GLsizei& numMipmapLevels ) const
{
    ...
    if( compressed )
    {
        // OSG_WARN<<"Warning::cannot currently use compressed format with 3D
        // textures."<<std::endl; return;
    }
    ...
    if( _min_filter == LINEAR || _min_filter == NEAREST || useHardwareMipMapGeneration )
    {
        ...
        if( !compressed_image )
        {
            extensions->glTexImage3D( target,
                                      0,
                                      static_cast<GLenum>( _internalFormat ),
                                      inwidth,
                                      inheight,
        ...
        else if( extensions->isCompressedTexImage3DSupported() )
        {
            ...
            extensions->glCompressedTexImage3D( target,
                                                0,
                                                static_cast<GLenum>( _internalFormat ),
                                                inwidth,
                                                inheight,
        ...
        if( hardwareMipMapOn )
        {
            extensions->glGenerateMipmap( GL_TEXTURE_3D );
        }
    }
    else
    {
        ...
        else
        {
            ...
            for( GLsizei k = 0; k < numMipmapLevels && ( width || height || depth );
                 k++ )
            {
                ...
                extensions->glTexImage3D(
                    ...
                    ( GLenum )image->getPixelFormat(),
                    ( GLenum )image->getDataType(),
                    image->getMipmapData( static_cast<unsigned int>( k ) )
                );
...
void
Texture3D::copyTexSubImage3D( State& state,
                              int    xoffset,
                              int    yoffset,
                              int    zoffset,
                              int    x,
                              ...
                              int    height )
{
    ...
    if( textureObject != 0 )
    {
        ...
        applyTexParameters( GL_TEXTURE_3D, state );
        extensions->glCopyTexSubImage3D( GL_TEXTURE_3D,
                                         0,
                                         xoffset,
                                         yoffset,
                                         zoffset,
    ...
    {
        OSG_WARN << "Warning: Texture3D::copyTexSubImage3D(..) failed, cannot not copy "
                    "to a non existent texture."
                 << std::endl;
    }
}

void
Texture3D::allocateMipmap( State& state ) const
{
    const unsigned int contextID = state.getContextID();
