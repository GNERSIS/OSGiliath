...
Texture2DArray::Texture2DArray( const Texture2DArray& text,
                                const CopyOp&         copyop ) :
    Inherit( text,
             copyop ),
...
int
Texture2DArray::compare( const StateAttribute& sa ) const
{
    // check the types are equal and then create the rhs variable
    // used by the COMPARE_StateAttribute_Parameter macros below.
    COMPARE_StateAttribute_Types( Texture2DArray,
                                  sa ) if( _images.size() <
                                           rhs._images.size() ) return -1;
...
void
Texture2DArray::setImage( unsigned int layer,
                          Image*       image )
{
...
Image*
Texture2DArray::getImage( unsigned int layer )
{
    return ( layer < static_cast<unsigned int>( _images.size() ) ) ? _images[layer].get()
                                                                   : 0;
}

const Image*
Texture2DArray::getImage( unsigned int layer ) const
{
    return ( layer < static_cast<unsigned int>( _images.size() ) ) ? _images[layer].get()
                                                                   : 0;
}
...
bool
Texture2DArray::imagesValid() const
{
    if( _images.empty() )
    {
        return false;
    ...
    for( Images::const_iterator itr = _images.begin(); itr != _images.end(); ++itr )
    {
...
void
Texture2DArray::setTextureSize( int width,
                                int height,
                                int depth )
{
    // set dimensions
...
void
Texture2DArray::setTextureDepth( int depth )
{
    // if we decrease the number of layers, then delete non-used
    if( depth < static_cast<int>( _images.size() ) )
    {
        _images.resize( static_cast<std::size_t>( depth ) );
        _modifiedCount.resize( static_cast<std::size_t>( depth ) );
    }
...
void
Texture2DArray::computeInternalFormat() const
{
    if( imagesValid() )
    {
...
GLsizei
Texture2DArray::computeTextureDepth() const
{
...
void
Texture2DArray::apply( State& state ) const
{
    ...
    {
        OSG_WARN << "Warning: Texture2DArray::apply(..) failed, 2D texture arrays are "
    ...
    if( textureObject && textureDepth > 0 )
    {
        ...
        if( image && getModifiedCount( 0, contextID ) != image->getModifiedCount() )
        {
            ...
            if( !textureObject->match( GL_TEXTURE_2D_ARRAY,
                                       new_numMipmapLevels,
    ...
    if( textureObject )
    {
        ...
        else
        {
            ...
            GLsizei n               = 0;
            for( Images::const_iterator itr = _images.begin(); itr != _images.end();
                 ++itr )
            {
                osg::Image* image = itr->get();
                if( image )
                {
                    if( getModifiedCount( static_cast<unsigned int>( n ), contextID ) !=
                        image->getModifiedCount() )
                    {
                        getModifiedCount( static_cast<unsigned int>( n ), contextID ) =
                            image->getModifiedCount();
                        ...
                        applyTexImage2DArray_subload( state,
                                                      image,
                                                      n,
                                                      _textureWidth,
                                                      _textureHeight,
    ...
    // there is no texture object, but exists a subload callback, so use it to upload
    // images
    ...
    // nothing before, but we have valid images, so do manual upload and create texture
    // object manually
    ...
    else if( imagesValid() )
    {
        ...
        GLenum texStorageSizedInternalFormat =
            extensions->isTextureStorageEnabled && ( _borderWidth == 0 )
                ? selectSizedInternalFormat( _images[0].get() )
                : 0;
        ...
        {
            extensions->glTexStorage3D( GL_TEXTURE_2D_ARRAY,
                                        std::max( _numMipmapLevels, 1 ),
                                        texStorageSizedInternalFormat,
                                        _textureWidth,
                                        _textureHeight,
                                        textureDepth );
        ...
        else
        {
            ...
            if( isCompressedInternalFormat( static_cast<GLint>( sourceFormat ) ) &&
                sourceFormat ==
                static_cast<GLenum>( _internalFormat ) &&
                extensions->isCompressedTexImage3DSupported() )
            {
                extensions->glCompressedTexImage3D(
                    GL_TEXTURE_2D_ARRAY,
                    0,
                    static_cast<GLenum>( _internalFormat ),
                    _textureWidth,
                    ...
                    textureDepth,
                    _borderWidth,
                    static_cast<GLsizei>( _images[0]->getImageSizeInBytes() ) *
                        textureDepth,
                    0
                );
            }
            ...
            {
                // Override compressed source format with safe GL_RGBA value which not
                // generate error We can safely do this as source format is not important
                // when source data is NULL
                if( isCompressedInternalFormat( static_cast<GLint>( sourceFormat ) ) )
                {
                    sourceFormat = GL_RGBA;
                }

                extensions->glTexImage3D( GL_TEXTURE_2D_ARRAY,
                                          0,
                                          static_cast<GLenum>( _internalFormat ),
                                          _textureWidth,
                                          _textureHeight,
        ...
        // For certain we have to manually allocate memory for mipmaps if images are
        // compressed if not allocated OpenGL will produce errors on mipmap upload. I
        // have not tested if this is necessary for plain texture formats but common
        // sense suggests its required as well.
        ...
        if( _min_filter !=
            ...
            extensions->glGenerateMipmap )
        {
            _numMipmapLevels =
                Image::computeNumberOfMipmapLevels( _textureWidth, _textureHeight );
            generateMipmap( state );
        }

        textureObject->setAllocated( _numMipmapLevels,
                                     static_cast<GLenum>( _internalFormat ),
                                     _textureWidth,
                                     _textureHeight,
                                     textureDepth,
        ...
        if( isSafeToUnrefImageData( state ) )
        {
            ...
            {
                osg::Image* image = itr->get();
                if( image && image->getDataVariance() == DataVariance::STATIC )
                {
                    *itr = NULL;
                }
            }
...
void
Texture2DArray::applyTexImage2DArray_subload( State&   state,
                                              Image*   image,
                                              GLsizei  layer,
                                              GLsizei  inwidth,
                                              GLsizei  inheight,
                                              ...
                                              GLsizei& numMipmapLevels ) const
{
    ...
    OSG_NOTICE << "Texture2DArray::applyTexImage2DArray_subload() = " << ( void* )image
               << std::endl;
#endif
    ...
    if( indepth > extensions->maxLayerCount )
    {
        ...
        OSG_WARN << "Warning: Texture2DArray::applyTexImage2DArray_subload(..) the "
                    "given layer number exceeds the maximum number of supported layers."
                 << std::endl;
        return;
    }
    ...
    {
        OSG_WARN << "Warning: Texture2DArray::applyTexImage2DArray_subload(..) given "
                    "image do have wrong dimension or internal format."
                 << std::endl;
        return;
    }
...
#ifdef DO_TIMING
    OSG_NOTICE << "    Texture2DArray::applyTexImage2DArray_subload() pbo=" << pbo
               << ", before copy time = "
    ...
    if( _min_filter == LINEAR || _min_filter == NEAREST || useHardwareMipMapGeneration )
    {
...
#if 1
            extensions->glTexSubImage3D( target,
                                         0,
                                         0,
                                         0,
                                         layer,
        ...
        {
            // OSG_WARN<<"glCompressedTexImage3D "<<inwidth<<", "<<inheight<<",
            // "<<indepth<<std::endl;
            ...
            extensions->glCompressedTexSubImage3D( target,
                                                   0,
                                                   0,
                                                   0,
                                                   layer,
    ...
    else
    {
        ...
        {
            numMipmapLevels = 1;
            OSG_WARN << "Warning: Texture2DArray::applyTexImage2DArray_subload(..) "
                        "mipmap layer not passed, and auto mipmap generation turned off "
                        "or not available. Check texture's min/mag filters & hardware "
                        "mipmap generation."
                     << std::endl;

            // the image object does provide mipmaps, so upload the in the certain levels
            // of a layer
        }
        else
        {
...
void
Texture2DArray::copyTexSubImage2DArray( State& state,
                                        int    xoffset,
                                        int    yoffset,
                                        int    zoffset,
                                        int    x,
                                        ...
                                        int    height )
{
    ...
    if( textureObject != 0 )
    {
        ...
        applyTexParameters( GL_TEXTURE_2D_ARRAY, state );
        extensions->glCopyTexSubImage3D( GL_TEXTURE_2D_ARRAY,
                                         0,
                                         xoffset,
                                         yoffset,
                                         zoffset,
    ...
    {
        OSG_WARN << "Warning: Texture2DArray::copyTexSubImage2DArray(..) failed, cannot "
                    "not copy to a non existent texture."
                 << std::endl;
    }
}

void
Texture2DArray::allocateMipmap( State& state ) const
{
    const unsigned int contextID = state.getContextID();
    ...
    if( textureObject && _textureWidth != 0 && _textureHeight != 0 && textureDepth != 0 )
    {
        ...
        int                 safeSourceFormat =
            _sourceFormat ? static_cast<int>( _sourceFormat ) : _internalFormat;
        ...
        for( GLsizei k = 1; k < numMipmapLevels && ( width || height ); k++ )
        {
            ...
            else
            {
                extensions->glTexImage3D( GL_TEXTURE_2D_ARRAY,
                                          ...
                                          textureDepth,
                                          _borderWidth,
                                          static_cast<GLenum>( safeSourceFormat ),
                                          _sourceType ? _sourceType : GL_UNSIGNED_BYTE,
                                          NULL );
            }
