...
typedef void( GL_APIENTRY* MyCompressedTexImage1DArbProc )( GLenum        target,
                                                            GLint         level,
...
Texture1D::Texture1D( const Texture1D& text,
                      const CopyOp&    copyop ) :
    Inherit( text,
             copyop ),
...
int
Texture1D::compare( const StateAttribute& sa ) const
{
    // check the types are equal and then create the rhs variable
    // used by the COMPARE_StateAttribute_Parameter macros below.
    COMPARE_StateAttribute_Types( Texture1D,
                                  sa ) if( _image !=
    ...
    COMPARE_StateAttribute_Parameter( _textureWidth ) COMPARE_StateAttribute_Parameter(
        _subloadCallback
    ) return 0;    // passed all the above comparison macros, must be equal.
}

void
Texture1D::setImage( Image* image )
{
    if( _image == image )
    {
        return;
...
void
Texture1D::apply( State& state ) const
{
    ...
    if( textureObject )
    {
            ...
            getModifiedCount( contextID ) != _image->getModifiedCount() )
        {
            ...
            if( !textureObject->match( GL_TEXTURE_1D,
                                       new_numMipmapLevels,
    ...
    else if( ( _textureWidth != 0 ) && ( _internalFormat != 0 ) )
    {
        ...
        if( texStorageSizedInternalFormat != 0 )
        {
            ...
            {
                extensions->glTexStorage1D( GL_TEXTURE_1D,
...
void
Texture1D::computeInternalFormat() const
{
    if( _image.valid() )
    {
...
void
Texture1D::applyTexImage1D( GLenum   target,
                            Image*   image,
                            State&   state,
                            GLsizei& inwidth,
                            GLsizei& numMipmapLevels ) const
{
    ...
    {
        // this is not thread safe... should really create local image data and rescale
        // to that as per Texture2D.
    ...
    static MyCompressedTexImage1DArbProc glCompressedTexImage1D_ptr =
        convertPointerType<MyCompressedTexImage1DArbProc, void*>(
            getGLExtensionFuncPtr( "glCompressedTexImage1DARB" )
        );
    ...
    if( _min_filter == LINEAR || _min_filter == NEAREST )
    {
        ...
        else if( glCompressedTexImage1D_ptr )
        {
            numMipmapLevels = 1;
            GLint blockSize =
                ( _internalFormat == GL_COMPRESSED_RGB_S3TC_DXT1_EXT ? 8 : 16 );
            GLint size =
                ( ( image->s() + 3 ) / 4 ) * ( ( image->t() + 3 ) / 4 ) * blockSize;
            glCompressedTexImage1D_ptr( target,
                                        0,
                                        static_cast<GLenum>( _internalFormat ),
                                        image->s(),
                                        _borderWidth,
...
void
Texture1D::copyTexImage1D( State& state,
                           int    x,
                           int    y,
                           int    width )
{
...
void
Texture1D::copyTexSubImage1D( State& state,
                              int    xoffset,
                              int    x,
                              int    y,
                              int    width )
...
void
Texture1D::allocateMipmap( State& state ) const
{
    const unsigned int contextID = state.getContextID();
