...
int
TextureCubeMap::compare( const StateAttribute& sa ) const
{
    // check the types are equal and then create the rhs variable
    // used by the COMPARE_StateAttribute_Parameter macros below.
    COMPARE_StateAttribute_Types( TextureCubeMap, sa ) bool noImages = true;
    for( int n = 0; n < 6; n++ )
    {
...
void
TextureCubeMap::setImage( unsigned int face,
                          Image*       image )
{
...
Image*
TextureCubeMap::getImage( unsigned int face )
{
    return _images[face].get();
}

const Image*
TextureCubeMap::getImage( unsigned int face ) const
{
    return _images[face].get();
}

bool
TextureCubeMap::imagesValid() const
{
    for( int n = 0; n < 6; n++ )
    {
...
void
TextureCubeMap::computeInternalFormat() const
{
    if( imagesValid() )
    {
...
void
TextureCubeMap::apply( State& state ) const
{
    ...
    if( textureObject )
    {
        ...
        if( image && getModifiedCount( 0, contextID ) != image->getModifiedCount() )
        {
            ...
            if( !textureObject->match( GL_TEXTURE_CUBE_MAP,
                                       new_numMipmapLevels,
    ...
    if( textureObject )
    {
        ...
        else
        {
            ...
            for( int n = 0; n < 6; n++ )
            {
                ...
                if( image &&
                    getModifiedCount( ( Face )n, contextID ) !=
    ...
    else if( imagesValid() )
    {
        ...
        if( isSafeToUnrefImageData( state ) )
        {
            ...
            {
                if( _images[n].valid() &&
                    _images[n]->getDataVariance() == DataVariance::STATIC )
                {
...
void
TextureCubeMap::copyTexSubImageCubeMap( State& state,
                                        int    face,
                                        int    xoffset,
                                        int    yoffset,
                                        int    x,
                                        ...
                                        int    height )
{
    ...
    if( !textureObject )
    {
        ...
        if( !textureObject )
        {
            ...
            OSG_NOTICE << "Warning : failed to create TextureCubeMap texture obeject, "
                          "copyTexSubImageCubeMap abandoned."
                       << std::endl;
            return;
        }
    }
    ...
    if( textureObject )
    {
        ...
        if( needHardwareMipMap )
        {
            ...
            if( !hardwareMipMapOn )
            {
                ...
                OSG_NOTICE << "Warning: TextureCubeMap::copyTexImage2D(,,,,) switch off "
                              "mip mapping as hardware support not available."
                           << std::endl;
                _min_filter = LINEAR;
            }
        }
        ...
        GenerateMipmapMode mipmapResult =
            mipmapBeforeTexImage( state, hardwareMipMapOn );
...
void
TextureCubeMap::allocateMipmap( State& state ) const
{
    const unsigned int contextID = state.getContextID();
