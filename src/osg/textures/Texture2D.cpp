...
int
Texture2D::compare( const StateAttribute& sa ) const
{
    // check the types are equal and then create the rhs variable
    // used by the COMPARE_StateAttribute_Parameter macros below.
    COMPARE_StateAttribute_Types( Texture2D,
                                  sa ) if( _image !=
...
void
Texture2D::setImage( Image* image )
{
    if( _image == image )
    {
        return;
...
bool
Texture2D::textureObjectValid( State& state ) const
{
    ...
    return textureObject->match( GL_TEXTURE_2D,
                                 new_numMipmapLevels,
...
void
Texture2D::apply( State& state ) const
{
    ...
    if( textureObject )
    {
        ...
        {
            textureObjectInvalidated =
                !_subloadCallback->textureObjectValid( *this, state );
        }
    ...
    else if( _subloadCallback.valid() )
    {
        _textureObjectBuffer[contextID] =
            _subloadCallback->generateTextureObject( *this, state );
    ...
    else if( _image.valid() && _image->data() )
    {
        ...
        GLenum texStorageSizedInternalFormat =
            extensions->isTextureStorageEnabled && ( _borderWidth == 0 )
                ? selectSizedInternalFormat( _image.get() )
                : 0;
        ...
        if( isSafeToUnrefImageData( state ) &&
            image->getDataVariance() == DataVariance::STATIC )
        {
...
void
Texture2D::computeInternalFormat() const
{
    if( _image.valid() )
    {
...
void
Texture2D::copyTexImage2D( State& state,
                           int    x,
                           int    y,
                           int    width,
                           int    height )
{
    ...
    if( needHardwareMipMap )
    {
        ...
        if( !hardwareMipMapOn )
        {
            ...
            OSG_NOTICE << "Warning: Texture2D::copyTexImage2D(,,,,) switch off mip "
                          "mapping as hardware support not available."
                       << std::endl;
            _min_filter = LINEAR;
        }
    }
...
void
Texture2D::copyTexSubImage2D( State& state,
                              int    xoffset,
                              int    yoffset,
                              int    x,
                              int    y,
...
void
Texture2D::allocateMipmap( State& state ) const
{
    const unsigned int contextID = state.getContextID();
