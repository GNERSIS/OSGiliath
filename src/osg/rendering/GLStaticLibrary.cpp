 ...
 * Utility functions for osg: defined, void, ADD_FUNCTION, initGLES2ProcAddress,
 * initProcAddress.
 */
...
namespace
{
    ...
    #define ADD_FUNCTION( FunctionName )                                            \
        sProcAddressMap[#FunctionName] = reinterpret_cast<GLProc>( &FunctionName );
    ...
    initProcAddress()
    {
        OSG_NOTICE << "initProcAddress() not implemented for static GL lib yet."
                   << std::endl;
    }
...
void*
GLStaticLibrary::getProcAddress( const char* procName )
{
