...
void
GLRenderBufferManager::deleteGLObject( GLuint globj )
{
...
GLFrameBufferObjectManager::GLFrameBufferObjectManager( unsigned int contextID ) :
    GLObjectManager( "GLFrameBufferObjectManager",
                     contextID )
{
}

void
GLFrameBufferObjectManager::deleteGLObject( GLuint globj )
{
    const GLExtensions* extensions = GLExtensions::Get( _contextID, true );
    if( extensions->isGlslSupported )
    {
...
RenderBuffer::~RenderBuffer()
{
    for( unsigned i = 0; i < _objectID.size(); ++i )
    {
        if( _objectID[i] )
        {
            osg::get<GLRenderBufferManager>( i )->scheduleGLObjectForDeletion(
                _objectID[i]
            );
        }
    }
...
int
RenderBuffer::getMaxSamples( unsigned int        contextID,
...
GLuint
RenderBuffer::getObjectID( unsigned int        contextID,
                           const GLExtensions* ext ) const
{
    ...
    if( dirty )
    {
        ...
        {
            OSG_WARN
                << "Coverage samples must be greater than or equal to color samples."
        ...
        {
            ext->glRenderbufferStorage( GL_RENDERBUFFER_EXT,
                                        _internalFormat,
                                        _width,
                                        _height );
        }
...
void
RenderBuffer::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
RenderBuffer::releaseGLObjects( osg::State* state ) const
{
    if( state )
    {
        ...
        {
            osg::get<GLRenderBufferManager>( contextID )
                ->scheduleGLObjectForDeletion( _objectID[contextID] );
...
struct FrameBufferAttachment::Pimpl
{
        ...
        explicit Pimpl( TargetType   ttype = RENDERBUFFER,
                        unsigned int lev   = 0 ) :
            targetType( ttype ),
            cubeMapFace( 0 ),
            level( lev ),
...
FrameBufferAttachment::FrameBufferAttachment( Texture1D*   target,
                                              unsigned int level )
{
...
FrameBufferAttachment::FrameBufferAttachment( Texture2D*   target,
                                              unsigned int level )
{
...
FrameBufferAttachment::FrameBufferAttachment( Texture2DArray* target,
                                              unsigned int    layer,
                                              unsigned int    level )
{
...
FrameBufferAttachment::FrameBufferAttachment( TextureCubeMap* target,
                                              unsigned int    face,
                                              unsigned int    level )
{
...
FrameBufferAttachment::FrameBufferAttachment( Camera::Attachment& attachment )
{
    ...
    if( image )
    {
        ...
        {
            GLenum format =
                static_cast<GLenum>( attachment._image->getInternalTextureFormat() );
            if( format == 0 )
            {
                format = attachment._internalFormat;
            }
            _ximpl = new Pimpl( Pimpl::RENDERBUFFER );
            _ximpl->renderbufferTarget =
                new osg::RenderBuffer( image->s(), image->t(), format );
            return;
        }
        else
        {
            OSG_WARN
                << "Error: "
                   ...
                   "passed an empty osg::Image, image must be allocated first."
                << std::endl;
        }
    }
    else
    {
        OSG_WARN << "Error: "
                    "FrameBufferAttachment::FrameBufferAttachment(Camera::Attachment&) "
                    "passed an unrecognised Texture type."
                 << std::endl;
    }
...
bool
FrameBufferAttachment::isMultisample() const
{
...
void
FrameBufferAttachment::createRequiredTexturesAndApplyGenerateMipMap(
    State&              state,
    const GLExtensions* ext
) const
{
...
// TODO: DSA — could convert to glNamedFramebufferTexture/glNamedFramebufferRenderbuffer
// if the FBO ID were passed through (requires signature change and caller refactor)
void
FrameBufferAttachment::attach( State&              state,
                               GLenum              target,
                               ...
                               const GLExtensions* ext ) const
{
    ...
    switch( _ximpl->targetType )
    {
        case Pimpl::RENDERBUFFER :
            break;    // already handled above. case should never be hit, just here to
                      // quieten compiler warning.
        case Pimpl::TEXTURE1D :
            ext->glFramebufferTexture1D( target,
                                         attachment_point,
                                         GL_TEXTURE_1D,
                                         tobj->id(),
                                         static_cast<GLint>( _ximpl->level ) );
            break;
        case Pimpl::TEXTURE2D :
            ext->glFramebufferTexture2D( target,
                                         attachment_point,
                                         GL_TEXTURE_2D,
                                         tobj->id(),
                                         static_cast<GLint>( _ximpl->level ) );
        ...
        case Pimpl::TEXTURE3D :
            ...
            {
                ext->glFramebufferTexture( target,
                                           attachment_point,
                                           static_cast<GLint>( tobj->id() ),
                                           static_cast<GLint>( _ximpl->level ) );
            }
            else
            {
                ext->glFramebufferTexture3D( target,
                                             attachment_point,
                                             GL_TEXTURE_3D,
                                             tobj->id(),
                                             static_cast<GLint>( _ximpl->level ),
                                             static_cast<GLint>( _ximpl->zoffset ) );
            }
            break;
        case Pimpl::TEXTURE2DARRAY :
            ...
            {
                ext->glFramebufferTextureLayer( target,
                                                attachment_point,
                                                tobj->id(),
                                                static_cast<GLint>( _ximpl->level ),
                                                static_cast<GLint>( _ximpl->zoffset ) );
...
int
FrameBufferAttachment::compare( const FrameBufferAttachment& fa ) const
{
    if( &fa == this )
    {
        return 0;
...
RenderBuffer*
FrameBufferAttachment::getRenderBuffer()
{
    return _ximpl->renderbufferTarget.get();
}

Texture*
FrameBufferAttachment::getTexture()
{
    return _ximpl->textureTarget.get();
}

const RenderBuffer*
FrameBufferAttachment::getRenderBuffer() const
{
    return _ximpl->renderbufferTarget.get();
}

const Texture*
FrameBufferAttachment::getTexture() const
{
    return _ximpl->textureTarget.get();
}

unsigned int
FrameBufferAttachment::getCubeMapFace() const
{
    return _ximpl->cubeMapFace;
}

unsigned int
FrameBufferAttachment::getTextureLevel() const
{
    return _ximpl->level;
}

unsigned int
FrameBufferAttachment::getTexture3DZOffset() const
{
    return _ximpl->zoffset;
}

unsigned int
FrameBufferAttachment::getTextureArrayLayer() const
{
    return _ximpl->zoffset;
}

void
FrameBufferAttachment::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _ximpl->renderbufferTarget.valid() )
    {
...
void
FrameBufferAttachment::releaseGLObjects( osg::State* state ) const
{
    if( _ximpl->renderbufferTarget.valid() )
    {
...
FrameBufferObject::~FrameBufferObject()
{
    for( unsigned i = 0; i < _fboID.size(); ++i )
    {
        if( _fboID[i] )
        {
            osg::get<GLFrameBufferObjectManager>( i )->scheduleGLObjectForDeletion(
                _fboID[i]
            );
        }
    }
...
void
FrameBufferObject::resizeGLObjectBuffers( unsigned int maxSize )
{
    _fboID.resize( maxSize );
    _unsupported.resize( maxSize );
    _fboID.resize( maxSize );

    for( AttachmentMap::iterator itr = _attachments.begin(); itr != _attachments.end();
         ++itr )
    {
...
void
FrameBufferObject::releaseGLObjects( osg::State* state ) const
{
    if( state )
    {
        unsigned int contextID = state->getContextID();
        if( _fboID[contextID] )
        {
            osg::get<GLFrameBufferObjectManager>( contextID )
                ->scheduleGLObjectForDeletion( _fboID[contextID] );
            _fboID[contextID] = 0;
        }
    }
    else
    ...
    for( AttachmentMap::const_iterator itr = _attachments.begin();
         itr != _attachments.end();
         ++itr )
    {
...
void
FrameBufferObject::setAttachment( BufferComponent              attachment_point,
...
GLenum
FrameBufferObject::convertBufferComponentToGLenum(
    BufferComponent attachment_point
) const
{
    switch( attachment_point )
    {
        ...
        default :
            return GLenum( GL_COLOR_ATTACHMENT0_EXT +
                           ( attachment_point - Camera::COLOR_BUFFER0 ) );
    }
}

void
FrameBufferObject::updateDrawBuffers()
{
    _drawBuffers.clear();

    // create textures and mipmaps before we bind the frame buffer object
    for( AttachmentMap::const_iterator i = _attachments.begin(); i != _attachments.end();
         ++i )
    {
...
void
FrameBufferObject::apply( State& state ) const
{
    apply( state, READ_DRAW_FRAMEBUFFER );
}

void
FrameBufferObject::apply( State&     state,
                          BindTarget target ) const
{
    ...
    {
        // the set of of attachments appears to be thread sensitive, it shouldn't be
        // because OpenGL FBO handles osg::FrameBufferObject has are multi-buffered... so
        // as a temporary fix will stick in a mutex to ensure that only one thread passes
        // through here at one time.
    ...
    if( ( target == READ_DRAW_FRAMEBUFFER ) || ( target == DRAW_FRAMEBUFFER ) )
    {
        if( _drawBuffers.size() > 0 )
        {
            ...
            {
                gl2e->glDrawBuffers( static_cast<GLsizei>( _drawBuffers.size() ),
                                     &( _drawBuffers[0] ) );
            }
            else
            {
                OSG_WARN << "Warning: FrameBufferObject: could not set draw buffers, "
                            "glDrawBuffers is not supported!"
                         << std::endl;
            }
        }
    }
             ...
             ++i )
        {
            ...
            switch( i->first )
            {
                case( Camera::PACKED_DEPTH_STENCIL_BUFFER ) :
                    ...
                    {
                        OSG_WARN << "Warning: FrameBufferObject: could not attach "
                ...
                default :
                    fa.attach( state,
                               target,
                               convertBufferComponentToGLenum( i->first ),
                               ext );
                    break;
            }
        }
...
bool
FrameBufferObject::isMultisample() const
{
...
int
FrameBufferObject::compare( const StateAttribute& sa ) const
{
