 ...
 * Hardware occlusion query node. Renders a bounding proxy
 * and skips child rendering if no fragments passed.
 */
...
namespace osg
{
    ...
    QueryGeometry*
    createDefaultQueryGeometry( const std::string& name )
    {
        // 6 faces x 2 triangles x 3 verts = 36 indices
        // Each quad (a,b,c,d) becomes triangles (a,b,d) + (b,c,d)
        GLushort indices[] = {
            0,
            1,
            3,
            1,
            2,
            3,    // face: was quad 0,1,2,3
            4,
            5,
            7,
            5,
            6,
            7,    // face: was quad 4,5,6,7
            0,
            3,
            5,
            3,
            6,
            5,    // face: was quad 0,3,6,5
            2,
            1,
            7,
            1,
            4,
            7,    // face: was quad 2,1,4,7
            5,
            4,
            0,
            4,
            1,
            0,    // face: was quad 5,4,1,0
            2,
            7,
            3,
            7,
            6,
            3    // face: was quad 2,7,6,3
        };

        ref_ptr<QueryGeometry> geom = new QueryGeometry( name );
        geom->setDataVariance( Object::DataVariance::DYNAMIC );
        geom->addPrimitiveSet(
            new DrawElementsUShort( PrimitiveSet::TRIANGLES, 36, indices )
        );
    ...
    initOQState()
    {
        ...
        state->setAttributeAndModes( cm,
                                     StateAttribute::ON | StateAttribute::PROTECTED );

        Depth* d = new Depth( Depth::Function::LEQUAL, 0.F, 1.F, false );
        state->setAttributeAndModes( d, StateAttribute::ON | StateAttribute::PROTECTED );

        PolygonMode* pm = new PolygonMode( PolygonMode::Face::FRONT_AND_BACK,
    ...
    initOQDebugState()
    {
        ...
        debugState->setMode( GL_CULL_FACE,
                             StateAttribute::ON | StateAttribute::PROTECTED );

        PolygonMode* pm = new PolygonMode( PolygonMode::Face::FRONT_AND_BACK,
                                           PolygonMode::Mode::LINE );
        debugState->setAttributeAndModes( pm,
                                          StateAttribute::ON |
                                              StateAttribute::PROTECTED );

        PolygonOffset* po = new PolygonOffset( -1., -1. );
        debugState->setAttributeAndModes( po,
                                          StateAttribute::ON |
                                              StateAttribute::PROTECTED );
...
struct RetrieveQueriesCallback
    : public osg::Inherit<osg::Camera::DrawCallback, RetrieveQueriesCallback>
{
        OSG_REGISTER_TYPE( osgOQ,
        ...
        RetrieveQueriesCallback( const RetrieveQueriesCallback& rqc,
        ...
        virtual void
        operator()( const osg::Camera& camera ) const
        {
            if( _results.empty() )
            {
                return;
            ...
            else
            {
                ...
                OSG_DEBUG
                    << "osgOQ: RQCB: Using fallback path to obtain GLExtensions pointer."
                    << std::endl;
                ext = _extensionsFallback;
                if( !ext )
                {
                    OSG_FATAL << "osgOQ: RQCB: GLExtensions pointer fallback is NULL."
                              << std::endl;
                    return;
                }
            }
            ...
            while( it != _results.end() )
            {
                ...
                OSG_DEBUG << "osgOQ: RQCB: Retrieving..." << std::endl;

                GLint ready = 0;
                ext->glGetQueryObjectiv( tr->_id, GL_QUERY_RESULT_AVAILABLE, &ready );
                if( ready )
                {
                    ext->glGetQueryObjectiv( tr->_id,
                                             GL_QUERY_RESULT,
                                             &( tr->_numPixels ) );
                    if( tr->_numPixels < 0 )
                    {
            ...
            OSG_INFO << "osgOQ: RQCB: " << "Retrieved " << count << " queries in "
        ...
        reset()
        {
            ...
            {
                if( !( *it )->_active ||
                    !( *it )->_init )    // remove results that have already been
                                         // retrieved or their query objects deleted.
                {
...
struct ClearQueriesCallback
    : public osg::Inherit<osg::Camera::DrawCallback, ClearQueriesCallback>
{
        OSG_REGISTER_TYPE( osgOQ,
        ...
        ClearQueriesCallback( const ClearQueriesCallback& rhs,
                              const osg::CopyOp&          copyop ) :
            Inherit( rhs,
                     copyop ),
            _rqcb( rhs._rqcb )
        ...
        virtual void
        operator()( const osg::Camera& ) const
        {
            if( !_rqcb )
            {
...
namespace osg
{
    ...
    class QueryObjectManager : public GLObjectManager
    {
        public:
            ...
            QueryObjectManager( unsigned int contextID ) :
                GLObjectManager( "QueryObjectManager",
                                 contextID )
            {
            }
            ...
            deleteGLObject( GLuint globj )
            {
                ...
                if( extensions->isOcclusionQuerySupported ||
                    extensions->isARBOcclusionQuerySupported )
                {
                    extensions->glDeleteQueries( 1L, &globj );
                }
            }
    };
    ...
    QueryGeometry::drawImplementation( osg::RenderInfo& renderInfo ) const
    {
        ...
        RetrieveQueriesCallback* rqcb =
            cam->findPostDrawCallback<RetrieveQueriesCallback>();
        if( !rqcb )
        {
            rqcb = new RetrieveQueriesCallback( ext );
            cam->addPostDrawCallback( rqcb );

            ClearQueriesCallback* cqcb =
                cam->findPreDrawCallback<ClearQueriesCallback>();
            if( !cqcb )
            {
        ...
        OSG_DEBUG << "osgOQ: QG: Querying for: " << _oqnName << std::endl;
        ...
        OSG_DEBUG << "osgOQ: QG. OQNName: " << _oqnName << ", Ctx: " << contextID
                  << ", ID: " << tr->_id << std::endl;
#ifdef _DEBUG
        {
            GLenum err;
    ...
    QueryGeometry::QueryResult
    QueryGeometry::getQueryResult( const osg::Camera* cam ) const
    {
        osg::ref_ptr<osg::TestResult> tr;
        {
            std::lock_guard<std::mutex> lock( _mapMutex );
        ...
        return QueryResult( tr->_init && !tr->_active,
                            static_cast<unsigned int>( tr->_numPixels ) );
    }
    ...
    QueryGeometry::releaseGLObjects( osg::State* state ) const
    {
        ...
        else
        {
            ...
            while( it != _results.end() )
            {
                ...
                {
                    osg::get<QueryObjectManager>( contextID )
                        ->scheduleGLObjectForDeletion( tr->_id );
                    tr->_init = false;
                }
                it++;
            }
    ...
    void
    QueryGeometry::deleteQueryObject( unsigned int contextID,
                                      GLuint       handle )
    {
    ...
    {
        osg::get<QueryObjectManager>( contextID )
            ->flushDeletedGLObjects( currentTime, availableTime );
    }
    ...
    OcclusionQueryNode::OcclusionQueryNode( const OcclusionQueryNode& oqn,
                                            const CopyOp&             copyop ) :
        Inherit( oqn,
                 copyop ),
        _queryGeometryState( INVALID ),
    ...
    bool
    OcclusionQueryNode::getPassed( const Camera* camera,
                                   NodeVisitor&  nv )
    {
        if( !_enabled )
        {
...

        {
            ...
            //  2) we haven't rendered for an abnormally long time (probably because
            //  we're an out-of-range LOD child)
            // In these cases, assume we're visible to avoid blinking.
            std::lock_guard<std::mutex> lock( _frameCountMutex );
            const unsigned int&         lastQueryFrame( _frameCountMap[camera] );
            if( ( lastQueryFrame == 0 ) ||
                ( ( nv.getTraversalNumber() - lastQueryFrame ) >
                  ( _queryFrameCount + 1 ) ) )
            {
                _passed = true;
                return _passed;
        ...
        osg::dmat4::value_type nearPlane;
        const osg::dmat4&      proj( camera->getProjectionMatrix() );
        if( ( proj( 3, 3 ) != 1. ) ||
            ( proj( 2, 3 ) != 0. ) ||
            ( proj( 1, 3 ) != 0. ) ||
            ( proj( 0, 3 ) != 0. ) )
        ...
        // If the distance from the near plane to the bounding sphere shell is positive,
        // retrieve
        ...
        const osg::sphere&     bs = getBound();
        osg::dmat4::value_type distanceToEyePoint =
            nv.getDistanceToEyePoint( bs.center, false );

        osg::dmat4::value_type distance = distanceToEyePoint - nearPlane - bs.radius;
    ...
    void
    OcclusionQueryNode::traverseQuery( const Camera* camera,
                                       NodeVisitor&  nv )
    {
        ...
        bool issueQuery;
        {
            const unsigned int          curFrame = nv.getTraversalNumber();
    ...
    void
    OcclusionQueryNode::traverseDebug( NodeVisitor& nv )
    {
    ...
    {
        {
            ...
            std::lock_guard<std::mutex> lock( _computeBoundMutex );
    ...
    void
    OcclusionQueryNode::resetQueries()
    {
        std::lock_guard<std::mutex> lock( _frameCountMutex );
    ...
    void
    OcclusionQueryNode::setQueryStateSet( StateSet* ss )
    {
        if( !_queryGeode )
        {
            OSG_WARN << "osgOQ: OcclusionQueryNode:: Invalid query support node."
                     << std::endl;
            return;
        }
    ...
    void
    OcclusionQueryNode::setDebugStateSet( StateSet* ss )
    {
        if( !_debugGeode )
        {
            OSG_WARN << "osgOQ: OcclusionQueryNode:: Invalid debug support node."
                     << std::endl;
            return;
        }
    ...
    {
        {
            ...
            _queryGeode->setDataVariance( Object::DataVariance::DYNAMIC );
...

        {
            ...
            _debugGeode->setDataVariance( Object::DataVariance::DYNAMIC );
    ...
    void
    OcclusionQueryNode::setQueryGeometryInternal( QueryGeometry*     queryGeom,
    ...
    void
    OcclusionQueryNode::updateDefaultQueryGeometry()
    {
        if( _queryGeometryState == USER_DEFINED )
        {
            OSG_FATAL << "osgOQ: OcclusionQueryNode: Unexpected "
                         "QueryGeometryState=USER_DEFINED."
                      << std::endl;
            return;
        }
        ...
        if( bbValid )
        {
            ...
            ( *v )[2] = vec3( bb.max.x, bb.min.y, bb.max.z );
            ( *v )[3] = vec3( bb.min.x, bb.min.y, bb.max.z );
            ( *v )[4] = vec3( bb.max.x, bb.max.y, bb.min.z );
            ( *v )[5] = vec3( bb.min.x, bb.max.y, bb.min.z );
            ( *v )[6] = vec3( bb.min.x, bb.max.y, bb.max.z );
            ( *v )[7] = vec3( bb.max.x, bb.max.y, bb.max.z );
        }
    ...
    void
    OcclusionQueryNode::releaseGLObjects( State* state ) const
    {
    ...
    void
    OcclusionQueryNode::flushDeletedQueryObjects( unsigned int contextID,
                                                  double       currentTime,
                                                  double&      availableTime )
    {
    ...
    void
    OcclusionQueryNode::setQueryGeometry( QueryGeometry* geom )
    {
        std::lock_guard<std::mutex> lock( _computeBoundMutex );
    ...
    const osg::QueryGeometry*
    OcclusionQueryNode::getQueryGeometry() const
    {
        if( _queryGeode && _queryGeode->getDrawable( 0 ) )
        {
            QueryGeometry* qg =
                static_cast<QueryGeometry*>( _queryGeode->getDrawable( 0 ) );
            return qg;
        }
        return 0;
    }
