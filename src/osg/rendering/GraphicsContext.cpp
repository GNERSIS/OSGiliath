...
void
GraphicsContext::WindowingSystemInterfaces::addWindowingSystemInterface(
...
void
GraphicsContext::WindowingSystemInterfaces::removeWindowingSystemInterface(
    GraphicsContext::WindowingSystemInterface* wsi
)
{
...
)
{
    if( _interfaces.empty() )
    {
        OSG_WARN << "Warning: "
                    ...
                    "getWindowingSystemInterface() failed, no interfaces available."
                 << std::endl;
        return 0;
    }
    ...
    if( !name.empty() )
    {
        for( Interfaces::iterator itr = _interfaces.begin(); itr != _interfaces.end();
             ++itr )
        {
            if( ( *itr )->getName() == name )
            {
            ...
            OSG_NOTICE << "   tried interface " << typeid( *itr ).name()
                       << ", name= " << ( *itr )->getName() << std::endl;
        }

        OSG_WARN << "Warning: "
                    "GraphicsContext::WindowingSystemInterfaces::"
                    "getWindowingSystemInterface() failed, no interfaces matches name : "
                 << name << std::endl;
        return 0;
    }
    else
...
osg::ref_ptr<GraphicsContext::WindowingSystemInterfaces>&
GraphicsContext::getWindowingSystemInterfaces()
{
    static ref_ptr<GraphicsContext::WindowingSystemInterfaces>
        s_WindowingSystemInterface = new GraphicsContext::WindowingSystemInterfaces;
    return s_WindowingSystemInterface;
}

OSG_INIT_SINGLETON_PROXY( ProxyInitWindowingSystemInterfaces,
                          GraphicsContext::getWindowingSystemInterfaces() )

// GraphicsContext static method implementations
GraphicsContext::WindowingSystemInterface*
GraphicsContext::getWindowingSystemInterface( const std::string& name )
{
    return GraphicsContext::getWindowingSystemInterfaces()->getWindowingSystemInterface(
        name
    );
}
...
GraphicsContext::createGraphicsContext( Traits* traits )
{
    ref_ptr<GraphicsContext::WindowingSystemInterface> wsref =
        getWindowingSystemInterface( traits ? traits->windowingSystemPreference : "" );
    if( wsref.valid() )
    {
...
GraphicsContext::ScreenIdentifier::ScreenIdentifier( const std::string& in_hostName,
                                                     int                in_displayNum,
                                                     int                in_screenNum ) :
...
std::string
GraphicsContext::ScreenIdentifier::displayName() const
{
...
void
GraphicsContext::ScreenIdentifier::readDISPLAY()
{
    std::string str;
...
void
GraphicsContext::ScreenIdentifier::setScreenIdentifier( const std::string& displayName )
{
    std::string::size_type colon = displayName.find_last_of( ':' );
    std::string::size_type point = displayName.find_last_of( '.' );

    // handle the case where the host name is supplied with '.' such as 127.0.0.1:0  with
    // only DisplayNum provided here the point to picks up on the .1 from the host name,
    // rather then demarking the DisplayNum/ScreenNum as no ScreenNum is provided, hence
    // no . in the rhs of the :
    if( point != std::string::npos && colon != std::string::npos && point < colon )
    {
    ...
    std::string::size_type startOfDisplayNum =
        ( colon == std::string::npos ) ? 0 : colon + 1;
    std::string::size_type endOfDisplayNum =
        ( point == std::string::npos ) ? displayName.size() : point;

    if( startOfDisplayNum < endOfDisplayNum )
    {
        displayNum =
            atoi( displayName
                      .substr( startOfDisplayNum, endOfDisplayNum - startOfDisplayNum )
                      .c_str() );
    }
    else
    {
    ...
    {
        screenNum = atoi(
            displayName.substr( point + 1, displayName.size() - point - 1 ).c_str()
        );
    }
    else
    {
...
bool
GraphicsContext::Traits::getContextVersion( unsigned int& major,
...
unsigned int
GraphicsContext::createNewContextID()
{
...
void
GraphicsContext::incrementContextIDUsageCount( unsigned int contextID )
{
    return ContextData::incrementContextIDUsageCount( contextID );
}

void
GraphicsContext::decrementContextIDUsageCount( unsigned int contextID )
{
    return ContextData::decrementContextIDUsageCount( contextID );
}

void
GraphicsContext::registerGraphicsContext( GraphicsContext* gc )
{
    ContextData::registerGraphicsContext( gc );
}

void
GraphicsContext::unregisterGraphicsContext( GraphicsContext* gc )
{
...
void
GraphicsContext::setCompileContext( unsigned int     contextID,
                                    GraphicsContext* gc )
{
...
GraphicsContext::GraphicsContext( const GraphicsContext&,
                                  const osg::CopyOp& ) :
    _clearColor( osg::vec4( 0.0F,
                            0.0F,
                            0.0F,
...
void
GraphicsContext::close( bool callCloseImplementation )
{
    OSG_INFO << "GraphicsContext::close(" << callCloseImplementation << ")" << this
             << std::endl;
    ...
    for( Cameras::iterator itr = _cameras.begin(); itr != _cameras.end(); ++itr )
    {
        Camera* camera = ( *itr );
        if( camera )
        {
            OSG_INFO << "Releasing GL objects for Camera=" << camera
                     << " _state=" << _state.get() << std::endl;
    ...
    {
        OSG_INFO << "Closing still viable window " << sharedContextExists
        ...
        {
            OSG_INFO << "makeCurrent did not succeed, could not do flush/deletion of "
                        "OpenGL objects."
                     << std::endl;
        }
    }
    ...
    // now discard any deleted deleted OpenGL objects that the are still hanging around -
    // such as due to the flushDelete*() methods not being invoked, such as when using
    // GraphicContextEmbedded where makeCurrent does not work.
...
{
    _threadOfLastMakeCurrent = osg::Thread::CurrentThreadId();
...
bool
GraphicsContext::makeContextCurrent( GraphicsContext* readContext )
{
...
GraphicsContext::swapBuffers()
{
    if( isCurrent() )
    {
        ...
        clear();
             ...
             std::hash<std::thread::id>{}( _graphicsThread->getThreadId() ) )
    {
...
void
GraphicsContext::createGraphicsThread()
{
    if( !_graphicsThread )
    {
...
void
GraphicsContext::setGraphicsThread( GraphicsThread* gt )
{
...
void
GraphicsContext::add( Operation* operation )
{
    OSG_INFO << "Doing add" << std::endl;

    // acquire the lock on the operations queue to prevent anyone else for modifying it
    // at the same time
...
void
GraphicsContext::remove( Operation* operation )
{
...
void
GraphicsContext::remove( const std::string& name )
{
...
void
GraphicsContext::removeAllOperations()
{
...
GraphicsContext::runOperations()
{
    ...
    for( CameraVector::iterator itr = camerasCopy.begin(); itr != camerasCopy.end();
         ++itr )
    {
...
void
GraphicsContext::addCamera( osg::Camera* camera )
{
    _cameras.push_back( camera );
}

void
GraphicsContext::removeCamera( osg::Camera* camera )
{
    ...
    if( itr != _cameras.end() )
    {
        ...
        for( Cameras::iterator citr = _cameras.begin(); citr != _cameras.end(); ++citr )
        {
            if( citr != itr )
            {
        ...
        for( NodeSet::iterator nitr = nodes.begin(); nitr != nodes.end(); ++nitr )
        {
...
void
GraphicsContext::resizedImplementation( int x,
                                        int y,
                                        int width,
                                        int height )
{
    ...
    for( Cameras::iterator itr = _cameras.begin(); itr != _cameras.end(); ++itr )
    {
        ...
        if( viewport )
        {
            ...
            if( processedViewports.count( viewport ) == 0 )
            {
                ...
                {
                    viewport->x() =
                        static_cast<osg::Viewport::value_type>( double( viewport->x() ) *
                                                                widthChangeRatio );
                    viewport->y() =
                        static_cast<osg::Viewport::value_type>( double( viewport->y() ) *
                                                                heigtChangeRatio );
                    viewport->width() = static_cast<osg::Viewport::value_type>(
                        double( viewport->width() ) * widthChangeRatio
                    );
                    viewport->height() = static_cast<osg::Viewport::value_type>(
                        double( viewport->height() ) * heigtChangeRatio
                    );
                }
            }
        }
        ...
        if( aspectRatioChange != 1.0 )
        {
            ...
            {
                Camera::ProjectionResizePolicy policy =
                    view ? view->getCamera()->getProjectionResizePolicy()
                ...
                if( view && camera == master )
                {
                    for( unsigned int i = 0; i < view->getNumSlaves(); ++i )
                    {
                        ...
                        if( child._camera.valid() &&
                            child._camera->getReferenceFrame() ==
                            osg::Transform::RELATIVE_RF )
                        {
                            // scale the slaves by the inverse of the change that has
                            // been applied to master, to avoid them be scaled twice
                            // (such as when both master and slave are on the same
                            // GraphicsContexts) or by the wrong scale when master and
                            // slave are on different GraphicsContexts.
                            switch( policy )
                            {
                                case( osg::Camera::HORIZONTAL ) :
                                    child._projectionOffset =
                                        osg::scale( aspectRatioChange, 1.0, 1.0 ) *
                                        child._projectionOffset;
                                    break;
                                case( osg::Camera::VERTICAL ) :
                                    child._projectionOffset =
                                        osg::scale( 1.0, 1.0 / aspectRatioChange, 1.0 ) *
                                        child._projectionOffset;
                                    break;
                                default :
                                    break;
...
void
SyncSwapBuffersCallback::swapBuffersImplementation( osg::GraphicsContext* gc )
{
    ...
    if( ext->glClientWaitSync )
    {
        if( _previousSync )
        {
            ...
            GLuint64     timeout = num_seconds * ( ( GLuint64 )1'000 * 1'000 * 1'000 );
