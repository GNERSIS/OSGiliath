 ...
 * Per-graphics-context data store. Maps context IDs to associated
 * GL object managers and frame-stamped cleanup data.
 */
...
void
ContextData::newFrame( osg::FrameStamp* frameStamp )
{
    // OSG_NOTICE<<"ContextData::newFrame("<<frameStamp->getFrameNumber()<<")"<<std::endl;
    for( ManagerMap::iterator itr = _managerMap.begin(); itr != _managerMap.end();
         ++itr )
    {
        osg::GraphicsObjectManager* gom =
            dynamic_cast<osg::GraphicsObjectManager*>( itr->second.get() );
        if( gom )
        {
...
void
ContextData::reportStats( std::ostream& out )
{
    for( ManagerMap::iterator itr = _managerMap.begin(); itr != _managerMap.end();
         ++itr )
    {
...
void
ContextData::recomputeStats( std::ostream& out ) const
{
    for( ManagerMap::const_iterator itr = _managerMap.begin(); itr != _managerMap.end();
         ++itr )
    {
        osg::GraphicsObjectManager* gom =
            dynamic_cast<osg::GraphicsObjectManager*>( itr->second.get() );
...
void
ContextData::flushDeletedGLObjects( double  currentTime,
...
void
ContextData::flushAllDeletedGLObjects()
{
...
void
ContextData::deleteAllGLObjects()
{
...
void
ContextData::discardAllGLObjects()
{
...
ContextData*
osg::getContextData( unsigned int contextID )
{
    std::lock_guard<std::recursive_mutex> lock( s_contextIDMapMutex );
...
ContextData*
osg::getOrCreateContextData( unsigned int contextID )
{
    // std::lock_guard<std::recursive_mutex> lock(s_contextIDMapMutex);
...
unsigned int
ContextData::createNewContextID()
{
    std::lock_guard<std::recursive_mutex> lock( s_contextIDMapMutex );
         ...
         ++itr )
    {
        if( !itr->second || itr->second->getNumContexts() == 0 )
        {
            ...
            OSG_INFO << "ContextData::createNewContextID() : reusing contextID="
                     << itr->first << std::endl;
    ...
    unsigned int contextID    = static_cast<unsigned int>( s_contextIDMap.size() );
    s_contextIDMap[contextID] = new ContextData( contextID );
    s_contextIDMap[contextID]->setNumContexts( 1 );

    OSG_INFO << "ContextData::createNewContextID() creating contextID=" << contextID
             << std::endl;
    OSG_INFO << "Updating the MaxNumberOfGraphicsContexts to "
             << contextID + 1 << std::endl;

    // update the maximum number of graphics contexts,
    // to ensure that texture objects and display buffers are configured to the correct
    // size.
...
void
ContextData::incrementContextIDUsageCount( unsigned int contextID )
{
    std::lock_guard<std::recursive_mutex> lock( s_contextIDMapMutex );
    ...
    OSG_NOTICE << "ContextData::incrementContextIDUsageCount(" << contextID << ") to "
               << s_contextIDMap[contextID]->getNumContexts() << std::endl;
}

void
ContextData::decrementContextIDUsageCount( unsigned int contextID )
{
    std::lock_guard<std::recursive_mutex> lock( s_contextIDMapMutex );
...
void
ContextData::registerGraphicsContext( GraphicsContext* gc )
{
    ...
    GraphicsContexts::iterator            itr =
        std::find( s_registeredContexts.begin(), s_registeredContexts.end(), gc );
...
void
ContextData::unregisterGraphicsContext( GraphicsContext* gc )
{
...
ContextData::GraphicsContexts
ContextData::getAllRegisteredGraphicsContexts()
{
    OSG_INFO
        << "ContextData::getAllRegisteredGraphicsContexts s_registeredContexts.size()="
        << s_registeredContexts.size() << std::endl;
    std::lock_guard<std::recursive_mutex> lock( s_contextIDMapMutex );
...
ContextData::getRegisteredGraphicsContexts( unsigned int contextID )
{
    ...
    OSG_INFO << "ContextData::getRegisteredGraphicsContexts " << contextID
...
GraphicsContext*
ContextData::getOrCreateCompileContext( unsigned int contextID )
{
...
void
ContextData::setCompileContext( unsigned int     contextID,
                                GraphicsContext* gc )
{
    std::lock_guard<std::recursive_mutex> lock( s_contextIDMapMutex );
...
GraphicsContext*
ContextData::getCompileContext( unsigned int contextID )
{
