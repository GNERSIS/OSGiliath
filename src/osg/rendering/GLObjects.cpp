...
void
osg::flushDeletedGLObjects( unsigned int contextID,
                            double       currentTime,
                            double&      availableTime )
{
    osg::getContextData( contextID )
        ->flushDeletedGLObjects( currentTime, availableTime );
}

void
osg::flushAllDeletedGLObjects( unsigned int contextID )
{
    osg::getContextData( contextID )->flushAllDeletedGLObjects();
}

void
osg::deleteAllGLObjects( unsigned int contextID )
{
    osg::getContextData( contextID )->deleteAllGLObjects();
}

void
osg::discardAllGLObjects( unsigned int contextID )
{
...
//
GraphicsObjectManager::GraphicsObjectManager( const std::string& name,
...
// GLObjectManager
//
GLObjectManager::GLObjectManager( const std::string& name,
                                  unsigned int       contextID ) :
    GraphicsObjectManager( name,
                           contextID )
{
}
...
void
GLObjectManager::flushDeletedGLObjects( double,
                                        double& availableTime )
{
...
void
GLObjectManager::flushAllDeletedGLObjects()
{
    std::lock_guard<std::mutex> lock( _mutex );
...
void
GLObjectManager::deleteAllGLObjects()
{
    OSG_INFO << "void " << _name << "::deleteAllGLObjects() : Not Implemented"
             << std::endl;
}

void
GLObjectManager::discardAllGLObjects()
{
...
void
GLObjectManager::scheduleGLObjectForDeletion( GLuint globj )
{
    std::lock_guard<std::mutex> lock( _mutex );
...
GLuint
GLObjectManager::createGLObject()
{
