 ...
 * GPU buffer object (VBO, EBO, UBO, SSBO) wrapper. Manages
 * buffer allocation, data upload, and per-context GL handles.
 */
...
//
unsigned int
GLBufferObject::BufferEntry::getNumClients() const
{
...
void
GLBufferObject::setBufferObject( BufferObject* bufferObject )
{
    assign( bufferObject );
}

void
GLBufferObject::assign( BufferObject* bufferObject )
{
...
GLBufferObject::compileBuffer()
{
    ...
    for( ; i < _bufferObject->getNumBufferData(); ++i )
    {
        ...
        if( i < _bufferEntries.size() )
        {
            ...
            {
                unsigned int previousEndOfBufferDataMarker =
                    osg::computeBufferAlignment( entry.offset + entry.dataSize,
                                                 bufferAlignment );

                // OSG_NOTICE<<"GLBufferObject::compileBuffer(..) updating
                // BufferEntry"<<std::endl;

                entry.numRead        = 0;
                entry.modifiedCount  = 0XFF'FF'FF;
            ...
            {
                newTotalSize =
                    osg::computeBufferAlignment( newTotalSize + entry.dataSize,
                                                 bufferAlignment );
            }
        }
        else
...
#endif
            newTotalSize =
                computeBufferAlignment( newTotalSize + entry.dataSize, bufferAlignment );
    ...
    if( i < _bufferEntries.size() )
    {
        // triming end of bufferEntries as the source data is has less entries than the
        // originally held.
        _bufferEntries.erase( _bufferEntries.begin() + i, _bufferEntries.end() );
    }

    // DSA: operate on buffer by name instead of binding first
    ...
    if( _allocatedSize != _profile._size )
    {
        ...
        OSG_INFO << "    Allocating new glNamedBufferData(), _allocatedSize="
                 << _allocatedSize << std::endl;

        if( _profile._mappingbitfield != 0 )
        {
            _extensions->glNamedBufferStorage( _glObjectID,
                                               static_cast<GLsizei>( _profile._size ),
                                               NULL,
                                               _profile._usage );
            ...
            if( _profile._usage & GL_MAP_PERSISTENT_BIT )
            {
                ...
                if( _persistentDMA )
                {
                    glUnmapNamedBuffer( _glObjectID );
                    _persistentDMA = 0;
                }
                _persistentDMA = glMapNamedBufferRange( _glObjectID,
                                                        0,
                                                        _profile._size,
        ...
        {
            glNamedBufferData( _glObjectID, _profile._size, NULL, _profile._usage );
        }
        compileAll = true;
    }
         ...
         ++itr )
    {
        ...
        {
            // OSG_NOTICE<<"GLBufferObject::compileBuffer(..) downloading BufferEntry
            // "<<&entry<<std::endl;
            entry.numRead           = 0;
            entry.modifiedCount     = entry.dataSource->getModifiedCount();
            ...
            if( image && !( image->isDataContiguous() ) )
            {
                ...
                for( osg::Image::DataIterator img_itr( image ); img_itr.valid();
                     ++img_itr )
                {
                    glNamedBufferSubData( _glObjectID,
                                          ( GLintptr )offset,
                                          ( GLsizeiptr )img_itr.size(),
                                          img_itr.data() );
                    offset += img_itr.size();
                }
            }
            else
            {
                if( _profile._mappingbitfield != 0 )
                {
                    if( _profile._mappingbitfield & GL_MAP_PERSISTENT_BIT )
                    {
                        if( _persistentDMA )
                        {
                            GLvoid* src = const_cast<GLvoid*>(
                                entry.dataSource->getDataPointer()
                            );
                            memcpy( ( unsigned char* )_persistentDMA + entry.offset,
                                    src,
                                    entry.dataSize );
                    ...
                    else if( _profile._mappingbitfield & GL_MAP_WRITE_BIT )
                    {
                        GLvoid *src = const_cast<GLvoid*>(
                               ...
                               *dst =
                                   glMapNamedBufferRange( _glObjectID,
                                                          ( GLintptr )entry.offset,
                                                          ( GLsizeiptr )entry.dataSize,
                                                          _profile._mappingbitfield );
...
void
GLBufferObject::commitDMA( unsigned int entryidx )
{
    if( !( _profile._mappingbitfield & GL_MAP_PERSISTENT_BIT ) )
    {
        return;
    ...
    // DSA: flush by buffer name
    glFlushMappedNamedBufferRange( _glObjectID,
                                   ( GLintptr )entry.offset,
                                   ( GLsizeiptr )entry.dataSize );
}

void
GLBufferObject::downloadBuffer( unsigned int entryidx )
{
    ...
    BufferEntry& entry = _bufferEntries[entryidx];
    // DSA: map/unmap by buffer name
    GLvoid *     dst = const_cast<GLvoid*>( entry.dataSource->getDataPointer() ),
           *src      = glMapNamedBufferRange( _glObjectID,
                                              ( GLintptr )entry.offset,
                                              ( GLsizeiptr )entry.dataSize,
                                              _profile._mappingbitfield );
    memcpy( dst, src, entry.dataSize );
...
GLBufferObject::deleteGLObject()
{
    ...
    if( _glObjectID != 0 )
    {
        if( _persistentDMA )
        {
            // DSA: unmap by buffer name, no bind needed
            glUnmapNamedBuffer( _glObjectID );
            _persistentDMA = 0;
        }
...
bool
GLBufferObject::hasAllBufferDataBeenRead() const
{
    for( BufferEntries::const_iterator it = _bufferEntries.begin();
         it != _bufferEntries.end();
         ++it )
    {
...
void
GLBufferObject::setBufferDataHasBeenRead( const osg::BufferData* bd )
{
...
// GLBufferObjectSet
//
GLBufferObjectSet::GLBufferObjectSet( GLBufferObjectManager*     parent,
    ...
    _tail( 0 )
{
    OSG_INFO << "GLBufferObjectSet::GLBufferObjectSet _profile._size=" << _profile._size
             << std::endl;
}
...
bool
GLBufferObjectSet::checkConsistency() const
{
    ...
    while( to != 0 )
    {
        ...
        if( to->_next )
        {
            ...
            {
                OSG_NOTICE << "GLBufferObjectSet::checkConsistency() : Error "
        ...
        else
        {
            ...
            {
                OSG_NOTICE
                    << "GLBufferObjectSet::checkConsistency() : Error _trail != to"
                    << std::endl;
                return false;
            }
        }
    ...
    unsigned int totalNumber =
        numInList + static_cast<unsigned int>( _orphanedGLBufferObjects.size() );
    if( totalNumber != _numOfGLBufferObjects )
    {
        OSG_NOTICE << "Error numInList + _orphanedGLBufferObjects.size() != "
                      "_numOfGLBufferObjects"
                   << std::endl;
        OSG_NOTICE << "    numInList = " << numInList << std::endl;
        OSG_NOTICE << "    _orphanedGLBufferObjects.size() = "
                   << _orphanedGLBufferObjects.size() << std::endl;
        OSG_NOTICE << "    _pendingOrphanedGLBufferObjects.size() = "
                   << _pendingOrphanedGLBufferObjects.size() << std::endl;
        OSG_NOTICE << "    _numOfGLBufferObjects = " << _numOfGLBufferObjects
                   << std::endl;
        return false;
    }
...
void
GLBufferObjectSet::handlePendingOrphandedGLBufferObjects()
{
    ...
    unsigned int numOrphaned =
        static_cast<unsigned int>( _pendingOrphanedGLBufferObjects.size() );
    ...
    // update the GLBufferObjectManager's running total of active + orphaned
    // GLBufferObjects
...
void
GLBufferObjectSet::deleteAllGLBufferObjects()
{
...

    {
        ...
        if( !_pendingOrphanedGLBufferObjects.empty() )
        {
            // OSG_NOTICE<<"GLBufferObjectSet::flushDeletedGLBufferObjects(..) handling
            // orphans"<<std::endl;
...
void
GLBufferObjectSet::flushAllDeletedGLBufferObjects()
{
    {
        std::lock_guard<std::mutex> lock( _mutex );
        if( !_pendingOrphanedGLBufferObjects.empty() )
    ...
    unsigned int numDeleted =
        static_cast<unsigned int>( _orphanedGLBufferObjects.size() );
...
void
GLBufferObjectSet::discardAllDeletedGLBufferObjects()
{
    ...
    // update the GLBufferObjectManager's running total of current pool size
    _parent->setCurrGLBufferObjectPoolSize(
        _parent->getCurrGLBufferObjectPoolSize() - numDiscarded * _profile._size
    );
    ...
    // just clear the list as there is nothing else we can do with them when discarding
    // them
    _orphanedGLBufferObjects.clear();
}

void
GLBufferObjectSet::flushDeletedGLBufferObjects( double /*currentTime*/,
                                                double& availableTime )
{
    ...
    if( _parent->getCurrGLBufferObjectPoolSize() <=
    ...
    unsigned int sizeRequired          = _parent->getCurrGLBufferObjectPoolSize() -
                                         _parent->getMaxGLBufferObjectPoolSize();
    unsigned int maxNumObjectsToDelete = static_cast<unsigned int>(
        ceil( double( sizeRequired ) / double( _profile._size ) )
    );
    OSG_INFO << "_parent->getCurrGLBufferObjectPoolSize()="
             << _parent->getCurrGLBufferObjectPoolSize()
             << " _parent->getMaxGLBufferObjectPoolSize()="
             << _parent->getMaxGLBufferObjectPoolSize() << std::endl;
    OSG_INFO << "Looking to reclaim " << sizeRequired << ", going to look to remove "
             << maxNumObjectsToDelete << " from " << _orphanedGLBufferObjects.size()
             << " orphans" << std::endl;
    ...
    // OSG_NOTICE<<", after = "<<_orphanedGLBufferObjects.size()<<" numDeleted =
    // "<<numDeleted<<std::endl;
...
bool
GLBufferObjectSet::makeSpace( unsigned int& size )
{
    {
        std::lock_guard<std::mutex> lock( _mutex );
        if( !_pendingOrphanedGLBufferObjects.empty() )
...
GLBufferObjectSet::takeFromOrphans( BufferObject* bufferObject )
{
    ...
    // OSG_NOTICE<<"Reusing orphaned GLBufferObject,
    // _numOfGLBufferObjects="<<_numOfGLBufferObjects<<"
    // target="<<std::hex<<_profile._target<<std::dec<<std::endl;

    return glbo;
}

osg::ref_ptr<GLBufferObject>
GLBufferObjectSet::takeOrGenerate( BufferObject* bufferObject )
{
    ...
    // see if we can reuse GLBufferObject by taking the least recently used active
    // GLBufferObject
        ...
        ( _head->_frameLastUsed < minFrameNumber ) )
    {
        ...
        {
            original_BufferObject->setGLBufferObject( _contextID, 0 );
            OSG_INFO << "GLBufferObjectSet=" << this
                     << ": Reusing an active GLBufferObject " << glbo.get()
                     << " _numOfGLBufferObjects=" << _numOfGLBufferObjects
                     << " size=" << _profile._size << std::endl;
        }
        else
        {
            OSG_INFO << "Reusing a recently orphaned active GLBufferObject "
                     << glbo.get() << std::endl;
        }
    ...
    //
    GLBufferObject* glbo =
        new GLBufferObject( _contextID, const_cast<BufferObject*>( bufferObject ) );
    glbo->setProfile( _profile );
    glbo->_set = this;
    ++_numOfGLBufferObjects;
    ...
    // OSG_NOTICE<<"Created new GLBufferObject, _numOfGLBufferObjects
    // "<<_numOfGLBufferObjects<<std::endl;

    return glbo;
}

void
GLBufferObjectSet::moveToBack( GLBufferObject* to )
{
#if 0
...
void
GLBufferObjectSet::addToBack( GLBufferObject* to )
{
#if 0
...
void
GLBufferObjectSet::orphan( GLBufferObject* to )
{
    std::lock_guard<std::mutex> lock( _mutex );
...
void
GLBufferObjectSet::remove( GLBufferObject* to )
{
    if( to->_previous != 0 )
    {
        ( to->_previous )->_next = to->_next;
...
void
GLBufferObjectSet::moveToSet( GLBufferObject*    to,
...
unsigned int
GLBufferObjectSet::computeNumGLBufferObjectsInList() const
{
    unsigned int    num = 0;
...
void
GLBufferObjectManager::setMaxGLBufferObjectPoolSize( unsigned int size )
{
    ...
    if( size < _currGLBufferObjectPoolSize )
    {
        OSG_NOTICE << "Warning: new MaxGLBufferObjectPoolSize=" << size
                   << " is smaller than current GLBufferObjectPoolSize="
                   << _currGLBufferObjectPoolSize << std::endl;
    }
...
bool
GLBufferObjectManager::makeSpace( unsigned int size )
{
...
osg::ref_ptr<GLBufferObject>
GLBufferObjectManager::generateGLBufferObject( const BufferObject* bufferObject )
{
    ElapsedTime elapsedTime( &( getGenerateTime() ) );
    ++getNumberGenerated();

    unsigned int requiredBufferSize =
        std::max( bufferObject->computeRequiredBufferSize(),
                  bufferObject->getProfile()._size );
    ...
    // OSG_NOTICE<<"GLBufferObjectManager::generateGLBufferObject
    // size="<<requiredBufferSize<<std::endl;
...
GLBufferObjectSet*
GLBufferObjectManager::getGLBufferObjectSet( const BufferObjectProfile& profile )
{
...
void
GLBufferObjectManager::handlePendingOrphandedGLBufferObjects()
{
    for( GLBufferObjectSetMap::iterator itr = _glBufferObjectSetMap.begin();
...
void
GLBufferObjectManager::deleteAllGLObjects()
{
...
void
GLBufferObjectManager::discardAllGLObjects()
{
    for( GLBufferObjectSetMap::iterator itr = _glBufferObjectSetMap.begin();
         itr != _glBufferObjectSetMap.end();
         ++itr )
...
void
GLBufferObjectManager::flushAllDeletedGLObjects()
{
    ElapsedTime elapsedTime( &( getDeleteTime() ) );
...
void
GLBufferObjectManager::discardAllDeletedGLObjects()
{
    for( GLBufferObjectSetMap::iterator itr = _glBufferObjectSetMap.begin();
         itr != _glBufferObjectSetMap.end();
         ++itr )
...
void
GLBufferObjectManager::flushDeletedGLObjects( double  currentTime,
                                              double& availableTime )
{
    ElapsedTime elapsedTime( &( getDeleteTime() ) );
...
void
GLBufferObjectManager::newFrame( osg::FrameStamp* fs )
{
    if( fs )
    {
...
void
GLBufferObjectManager::reportStats( std::ostream& out )
{
    ...
    out << "   total _numApplied=" << _numApplied << ", _applyTime=" << _applyTime
        << ", averagePerFrame=" << _applyTime / numFrames * 1000.0 << "ms" << std::endl;
    out << "   getMaxGLBufferObjectPoolSize()=" << getMaxGLBufferObjectPoolSize()
        << " current/max size = " << double( _currGLBufferObjectPoolSize ) /
        double( getMaxGLBufferObjectPoolSize() )
        << std::endl;
    ;
...
void
GLBufferObjectManager::resetStats()
{
    _numFrames    = 0;
    _numDeleted   = 0;
    _deleteTime   = 0;
...
void
GLBufferObjectManager::recomputeStats( std::ostream& out ) const
{
         ...
         ++itr )
    {
        ...
        currentSize += os->getProfile()._size *
                       ( os->computeNumGLBufferObjectsInList() + os->getNumOrphans() );
        out << "   size=" << os->getProfile()._size
            << ", os->computeNumGLBufferObjectsInList()"
            << os->computeNumGLBufferObjectsInList() << ", os->getNumOfGLBufferObjects()"
            << os->getNumOfGLBufferObjects() << ", os->getNumOrphans()"
            << os->getNumOrphans() << ", os->getNumPendingOrphans()"
            << os->getNumPendingOrphans() << std::endl;
    }
    ( void )numPendingOrphans;
    out << "   numObjectsInLists=" << numObjectsInLists << ", numActive=" << numActive
        << ", numOrphans=" << numOrphans << " currentSize=" << currentSize << std::endl;
    out << "   getMaxGLBufferObjectPoolSize()=" << getMaxGLBufferObjectPoolSize()
        << " current/max size = "
        << double( currentSize ) / double( getMaxGLBufferObjectPoolSize() ) << std::endl;
}
...
GLBufferObject*
BufferObject::getOrCreateGLBufferObject( unsigned int contextID ) const
{
    if( !_glBufferObjects[contextID] )
    {
        _glBufferObjects[contextID] =
            osg::get<GLBufferObjectManager>( contextID )->generateGLBufferObject( this );
    }

    // OSG_NOTICE<<"BufferObject::getOrCreateGLBufferObject()
    // _glBufferObjects[contextID]->getProfile()._size() =
    // "<<_glBufferObjects[contextID]->getProfile()._size<<std::endl;

    return _glBufferObjects[contextID].get();
}

void
BufferObject::setBufferData( unsigned int index,
                             BufferData*  bd )
{
...
void
BufferObject::resizeGLObjectBuffers( unsigned int maxSize )
{
    _glBufferObjects.resize( maxSize );
}

void
BufferObject::releaseGLObjects( State* state ) const
{
    ...
    else
    {
        for( unsigned int i = 0; i < _glBufferObjects.size(); ++i )
        {
            if( _glBufferObjects[i].valid() )
            {
                // OSG_NOTICE<<"  GLBufferObject::releaseGLBufferObject("<<i<<",
                // _glBufferObjects["<<i<<"]="<<_glBufferObjects[i].get()<<")"<<std::endl;
...
unsigned int
BufferObject::addBufferData( BufferData* bd )
{
    if( !bd )
    {
        return 0;
    ...
    // check to see if bd exists in BufferObject already, is so return without doing
    // anything
    ...
    // OSG_NOTICE<<"BufferObject "<<this<<":"<<className()<<"::addBufferData("<<bd<<"),
    // bufferIndex= "<<_bufferDataList.size()-1<<std::endl;

    return static_cast<unsigned int>( _bufferDataList.size() ) - 1;
}

void
BufferObject::removeBufferData( unsigned int index )
{
    if( index >= _bufferDataList.size() )
    {
        OSG_WARN << "Error " << className() << "::removeBufferData(" << index
                 << ") out of range." << std::endl;
        return;
    }

    // OSG_NOTICE<<"BufferObject::"<<this<<":"<<className()<<"::removeBufferData("<<index<<"),
    // size= "<<_bufferDataList.size()<<std::endl;

    // alter the indices of the BufferData after the entry to be removed so their indices
...
void
BufferObject::removeBufferData( BufferData* bd )
{
    // OSG_NOTICE<<"BufferObject::"<<this<<":"<<className()<<"::removeBufferData("<<bd<<"),
    // index="<<bd->getBufferIndex()<<" size= "<<_bufferDataList.size()<<std::endl;
...
unsigned int
BufferObject::computeRequiredBufferSize() const
{
    unsigned int bufferAlignment = 4;
    unsigned int newTotalSize    = 0;
         ...
         ++itr )
    {
        ...
        {
            newTotalSize =
                osg::computeBufferAlignment( newTotalSize + bd->getTotalDataSize(),
                                             bufferAlignment );
        }
        else
        {
            OSG_NOTICE
                << "BufferObject::" << this << ":" << className()
                << "::BufferObject::computeRequiredBufferSize() error, BufferData is 0x0"
                << std::endl;
        }
    }
    // OSG_NOTICE<<"BufferObject::"<<this<<":"<<className()<<"::BufferObject::computeRequiredBufferSize()
    // size="<<newTotalSize<<std::endl;
    return newTotalSize;
}

void
BufferObject::deleteBufferObject( unsigned int contextID,
                                  GLuint       globj )
{
    // implement deleteBufferObject for backwards compatibility by adding
    // a GLBufferObject for the globj id to BufferObjectManager/Set for the specified
    // context.

    GLBufferObjectManager* bufferObjectManager =
        osg::get<GLBufferObjectManager>( contextID );
    if( !bufferObjectManager )
    {
        OSG_NOTICE << "Warning::BufferObject::deleteBufferObject(" << contextID << ", "
                   << globj << ") unable to get GLBufferObjectManager for context."
                   << std::endl;
        return;
    }
    osg::ref_ptr<GLBufferObject> glBufferObject =
        new GLBufferObject( contextID, 0, globj );

    GLBufferObjectSet* bufferObjectSet =
        bufferObjectManager->getGLBufferObjectSet( glBufferObject->getProfile() );
    if( !bufferObjectSet )
    {
        OSG_NOTICE << "Warning::BufferObject::deleteBufferObject(" << contextID << ", "
                   << globj << ") unable to get GLBufferObjectSet for context."
                   << std::endl;
        return;
    }
...
void
BufferData::setBufferObject( BufferObject* bufferObject )
{
...
void
BufferData::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _bufferObject.valid() )
    {
...
void
BufferData::releaseGLObjects( State* state ) const
{
...
unsigned int
VertexBufferObject::addArray( osg::Array* array )
{
    return addBufferData( array );
}

void
VertexBufferObject::removeArray( osg::Array* array )
{
    removeBufferData( array );
}

void
VertexBufferObject::setArray( unsigned int i,
                              Array*       array )
{
...
Array*
VertexBufferObject::getArray( unsigned int i )
{
    return dynamic_cast<osg::Array*>( getBufferData( i ) );
}

const Array*
VertexBufferObject::getArray( unsigned int i ) const
{
...
ElementBufferObject::ElementBufferObject( const ElementBufferObject& vbo,
                                          const CopyOp&              copyop ) :
    Inherit( vbo,
             copyop )
{
...
unsigned int
ElementBufferObject::addDrawElements( osg::DrawElements* drawElements )
{
    return addBufferData( drawElements );
}

void
ElementBufferObject::removeDrawElements( osg::DrawElements* drawElements )
{
    removeBufferData( drawElements );
}

void
ElementBufferObject::setDrawElements( unsigned int  i,
...
DrawElements*
ElementBufferObject::getDrawElements( unsigned int i )
{
    return dynamic_cast<DrawElements*>( getBufferData( i ) );
}

const DrawElements*
ElementBufferObject::getDrawElements( unsigned int i ) const
{
...
DrawIndirectBufferObject::DrawIndirectBufferObject( const DrawIndirectBufferObject& vbo,
                                                    const CopyOp& copyop ) :
    Inherit( vbo,
             copyop )
{
...
unsigned int
DrawIndirectBufferObject::addArray( osg::Array* array )
{
    return addBufferData( array );
}

void
DrawIndirectBufferObject::removeArray( osg::Array* array )
{
    removeBufferData( array );
}

void
DrawIndirectBufferObject::setArray( unsigned int i,
                                    Array*       array )
{
    setBufferData( i, array );
}
...
Array*
DrawIndirectBufferObject::getArray( unsigned int i )
{
    return dynamic_cast<osg::Array*>( getBufferData( i ) );
}

const Array*
DrawIndirectBufferObject::getArray( unsigned int i ) const
{
    return dynamic_cast<const osg::Array*>( getBufferData( i ) );
}
...
PixelBufferObject::PixelBufferObject( const PixelBufferObject& buffer,
                                      const CopyOp&            copyop ) :
    Inherit( buffer,
             copyop )
{
...
void
PixelBufferObject::setImage( osg::Image* image )
{
    setBufferData( 0, image );
}

Image*
PixelBufferObject::getImage()
{
    return dynamic_cast<Image*>( getBufferData( 0 ) );
}

const Image*
PixelBufferObject::getImage() const
{
...
//--------------------------------------------------------------------------------
PixelDataBufferObject::PixelDataBufferObject( const PixelDataBufferObject& buffer,
                                              const CopyOp&                copyop ) :
    Inherit( buffer,
             copyop )
{
...
//--------------------------------------------------------------------------------
void
PixelDataBufferObject::compileBuffer( State& state ) const
{
    unsigned int contextID = state.getContextID();
    if( _profile._size == 0 )
    {
    ...
    // DSA: allocate buffer by name, no bind needed
    glNamedBufferData( bo->getGLObjectID(), _profile._size, NULL, _profile._usage );
}

//--------------------------------------------------------------------------------
void
PixelDataBufferObject::bindBufferInReadMode( State& state )
{
    unsigned int    contextID = state.getContextID();
...
//--------------------------------------------------------------------------------
void
PixelDataBufferObject::bindBufferInWriteMode( State& state )
{
    unsigned int    contextID = state.getContextID();
...
//--------------------------------------------------------------------------------
void
PixelDataBufferObject::unbindBuffer( unsigned int contextID ) const
{
...
//--------------------------------------------------------------------------------
void
PixelDataBufferObject::resizeGLObjectBuffers( unsigned int maxSize )
{
...
UniformBufferObject::UniformBufferObject( const UniformBufferObject& ubo,
                                          const CopyOp&              copyop ) :
    Inherit( ubo,
             copyop )
{
...
AtomicCounterBufferObject::AtomicCounterBufferObject(
    const AtomicCounterBufferObject& ubo,
    const CopyOp&                    copyop
) :
    Inherit( ubo,
             copyop )
...
ShaderStorageBufferObject::ShaderStorageBufferObject(
    const ShaderStorageBufferObject& ubo,
    const CopyOp&                    copyop
) :
    Inherit( ubo,
             copyop )
