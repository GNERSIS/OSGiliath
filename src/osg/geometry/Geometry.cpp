 ...
 * Drawable that holds vertex arrays, normals, colors, texture coordinates,
 * and primitive sets. The primary class for defining renderable geometry
 * with VBO-backed vertex attribute arrays.
 */
    ...
    _containsDeprecatedData( false )
{
    ...
    // Core Profile requires all vertex data in VBOs.
    ...
    _containsDeprecatedData( geometry._containsDeprecatedData )
{
        ...
        ( copyop.getCopyFlags() & osg::CopyOp::DEEP_COPY_PRIMITIVES ) )
    {
        /*if (_useVertexBufferObjects)*/
        {
            ...
            // Geometry to assign these, we'll do this by changing the cached value to
            // false then re-enabling. note do not use setUseVertexBufferObjects(false)
            // as it might modify Arrays that we have not deep-copied.
...
Geometry::configureBufferObjects()
{
    ...
    for( Arrays::iterator itr = arrays.begin(); itr != arrays.end(); ++itr )
    {
...
void
Geometry::setVertexArray( Array* array )
{
...
void
Geometry::setNormalArray( Array*              array,
...
void
Geometry::setColorArray( Array*              array,
                         osg::Array::Binding binding )
{
    if( array && binding != osg::Array::BIND_UNDEFINED )
    {
...
void
Geometry::setSecondaryColorArray( Array*              array,
                                  osg::Array::Binding binding )
{
    if( array && binding != osg::Array::BIND_UNDEFINED )
    {
...
void
Geometry::setFogCoordArray( Array*              array,
                            osg::Array::Binding binding )
{
    if( array && binding != osg::Array::BIND_UNDEFINED )
    {
...
void
Geometry::setTexCoordArray( unsigned int        index,
                            Array*              array,
                            osg::Array::Binding binding )
{
    ...
    if( /*_useVertexBufferObjects && */ array )
    {
        _vertexArrayStateList.assignTexCoordArrayDispatcher(
            static_cast<unsigned int>( _texCoordList.size() )
        );
...
Array*
Geometry::getTexCoordArray( unsigned int index )
{
...
const Array*
Geometry::getTexCoordArray( unsigned int index ) const
{
    if( index < _texCoordList.size() )
    {
        return _texCoordList[index].get();
...
void
Geometry::setTexCoordArrayList( const ArrayList& arrayList )
{
    ...
    if( !_texCoordList.empty() )
    {
        ...
        for( ArrayList::iterator itr = _texCoordList.begin(); itr != _texCoordList.end();
             ++itr )
        {
            if( itr->get() )
            {
...
void
Geometry::setVertexAttribArray( unsigned int        index,
                                Array*              array,
                                osg::Array::Binding binding )
{
    ...
    if( /*_useVertexBufferObjects && */ array )
    {
        _vertexArrayStateList.assignVertexAttribArrayDispatcher(
            static_cast<unsigned int>( _vertexAttribList.size() )
        );
...
Array*
Geometry::getVertexAttribArray( unsigned int index )
{
...
const Array*
Geometry::getVertexAttribArray( unsigned int index ) const
{
    if( index < _vertexAttribList.size() )
    {
        return _vertexAttribList[index].get();
...
void
Geometry::setVertexAttribArrayList( const ArrayList& arrayList )
{
...
bool
Geometry::addPrimitiveSet( PrimitiveSet* primitiveset )
{
    if( primitiveset )
    {
    ...
    OSG_WARN << "Warning: invalid primitiveset passed to "
                "osg::Geometry::addPrimitiveSet(i, primitiveset), ignoring call."
             << std::endl;
    return false;
}

bool
Geometry::setPrimitiveSet( unsigned int  i,
                           PrimitiveSet* primitiveset )
{
    ...
    OSG_WARN << "Warning: invalid index i or primitiveset passed to "
                "osg::Geometry::setPrimitiveSet(i,primitiveset), ignoring call."
             << std::endl;
    return false;
}

bool
Geometry::insertPrimitiveSet( unsigned int  i,
                              PrimitiveSet* primitiveset )
{
    ...
    OSG_WARN << "Warning: invalid index i or primitiveset passed to "
                "osg::Geometry::insertPrimitiveSet(i,primitiveset), ignoring call."
             << std::endl;
    return false;
}

void
Geometry::setPrimitiveSetList( const PrimitiveSetList& primitives )
{
    _primitives = primitives;
    /*if (_useVertexBufferObjects)*/
    {
        for( unsigned int primitiveSetIndex = 0; primitiveSetIndex < _primitives.size();
             ++primitiveSetIndex )
        {
...
bool
Geometry::removePrimitiveSet( unsigned int i,
                              unsigned int numElementsToRemove )
{
    ...
    if( i < _primitives.size() )
    {
        if( i + numElementsToRemove <= _primitives.size() )
        {
            _primitives.erase( _primitives.begin() + i,
                               _primitives.begin() + i + numElementsToRemove );
        }
        else
        {
            ...
            OSG_WARN
                << "Warning: osg::Geometry::removePrimitiveSet(i,numElementsToRemove) "
                   "has been asked to remove more elements than are available,"
                << std::endl;
            OSG_WARN << "         removing on from i to the end of the list of "
                        "primitive sets."
                     << std::endl;
    ...
    OSG_WARN
        << "Warning: invalid index i passed to "
           "osg::Geometry::removePrimitiveSet(i,numElementsToRemove), ignoring call."
        << std::endl;
    return false;
}

unsigned int
Geometry::getPrimitiveSetIndex( const PrimitiveSet* primitiveset ) const
{
    for( unsigned int primitiveSetIndex = 0; primitiveSetIndex < _primitives.size();
         ++primitiveSetIndex )
    {
    ...
    return static_cast<unsigned int>( _primitives.size() );    // node not found.
}

unsigned int
Geometry::getGLObjectSizeHint() const
{
...
bool
Geometry::getArrayList( ArrayList& arrayList ) const
{
    unsigned int startSize = static_cast<unsigned int>( arrayList.size() );
...
bool
Geometry::getDrawElementsList( DrawElementsList& drawElementsList ) const
{
    unsigned int startSize = static_cast<unsigned int>( drawElementsList.size() );
...
void
Geometry::addVertexBufferObjectIfRequired( osg::Array* array )
{
    // Core Profile requires ALL vertex data in VBOs - not just per-vertex arrays.
    // BIND_OVERALL and BIND_PER_PRIMITIVE_SET arrays also need VBOs because
    // client-side vertex attribute pointers are invalid in Core Profile.
...
void
Geometry::addElementBufferObjectIfRequired( osg::PrimitiveSet* primitiveSet )
{
    /*if (_useVertexBufferObjects)*/
    {
...
Geometry::getOrCreateVertexBufferObject()
{
    ArrayList arrayList;
    getArrayList( arrayList );

    ArrayList::iterator vitr;
    for( vitr = arrayList.begin(); vitr != arrayList.end(); ++vitr )
    {
...
Geometry::getOrCreateElementBufferObject()
{
    DrawElementsList drawElementsList;
    getDrawElementsList( drawElementsList );

    DrawElementsList::iterator deitr;
    for( deitr = drawElementsList.begin(); deitr != drawElementsList.end(); ++deitr )
    {
...
void
Geometry::setUseVertexBufferObjects( bool flag )
{
    // flag = true;
    ...
    /*if (_useVertexBufferObjects)*/
    {
        if( !arrayList.empty() )
        {
            ...
            ArrayList::iterator                   vitr;
            for( vitr = arrayList.begin(); vitr != arrayList.end() && !vbo; ++vitr )
            {
                osg::Array* array = vitr->get();
                if( array->getVertexBufferObject() )
                {
        ...
        if( !drawElementsList.empty() )
        {
            ...
            DrawElementsList::iterator             deitr;
            for( deitr = drawElementsList.begin(); deitr != drawElementsList.end();
                 ++deitr )
            {
                osg::DrawElements* elements = *deitr;
                if( elements->getElementBufferObject() )
...
void
Geometry::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
Geometry::releaseGLObjects( State* state ) const
{
...
VertexArrayState*
Geometry::createVertexArrayStateImplementation( RenderInfo& renderInfo ) const
{
    ...
    if( !_texCoordList.empty() )
    {
        vas->assignTexCoordArrayDispatcher(
            static_cast<unsigned int>( _texCoordList.size() )
        );
    }
    if( !_vertexAttribList.empty() )
    {
        vas->assignVertexAttribArrayDispatcher(
            static_cast<unsigned int>( _vertexAttribList.size() )
        );
    }
...
void
Geometry::compileGLObjects( RenderInfo& renderInfo ) const
{
    State& state = *renderInfo.getState();
                                                      ...
                                                      _useVertexBufferObjects ) )
    {
             ...
             ++itr )
        {
            GLBufferObject* glBufferObject =
                ( *itr )->getOrCreateGLBufferObject( contextID );
...
void
Geometry::drawImplementation( RenderInfo& renderInfo ) const
{
    // OSG_NOTICE<<"Geometry::drawImplementation() "<<this<<std::endl;

    if( _containsDeprecatedData )
    {
        OSG_WARN << "Geometry::drawImplementation() unable to render due to deprecated "
                    "data, call geometry->fixDeprecatedData();"
                 << std::endl;
        return;
    }
    ...
    bool   usingVertexBufferObjects =
        state.useVertexBufferObject( _supportsVertexBufferObjects &&
                                     _useVertexBufferObjects );
    bool usingVertexArrayObjects =
        usingVertexBufferObjects && state.useVertexArrayObject( _useVertexArrayObject );
...
void
Geometry::drawVertexArraysImplementation( RenderInfo& renderInfo ) const
{
    State&                state                  = *renderInfo.getState();
    ...
    attributeDispatchers.setUseVertexAttribAlias(
    ...
    if( handleVertexAttributes )
    {
        ...
        {
            attributeDispatchers.activateVertexAttribArray(
                unit,
    ...
    // Core Profile: disabled vertex attributes default to (0,0,0,1).
    // Set defaults matching legacy GL behavior when arrays are missing.
    // GL_COLOR_MATERIAL emulation: vertex color replaces material ambient/diffuse
    if( _colorArray.valid() )
    {
        state.setColorMaterial( true );
    }
    else
    {
        state.setColorMaterial( false );
        state.Color( 1.0F, 1.0F, 1.0F, 1.0F );
    }
    if( !_normalArray.valid() )
    {
        // Set normal to zero to signal the shader that no normals are provided.
        // The shader detects zero-length normals and uses NdotL=1.0 for full
        // illumination, matching fixed-function behavior for unlit geometry.
...
void
Geometry::drawPrimitivesImplementation( RenderInfo& renderInfo ) const
{
    State&                state                = *renderInfo.getState();
    AttributeDispatchers& attributeDispatchers = state.getAttributeDispatchers();
    bool                  usingVertexBufferObjects =
    ...
    for( unsigned int primitiveSetNum = 0; primitiveSetNum != _primitives.size();
         ++primitiveSetNum )
    {
...
void
Geometry::accept( AttributeFunctor& af )
{
    ...
    {
        OSG_INFO
            << "Geometry::accept(AttributeFunctor& af): Using vertex attribute instead"
            << std::endl;
    ...
    {
        afav.applyArray( ( AttributeType )( TEXTURE_COORDS_0 + unit ),
                         _texCoordList[unit].get() );
    }
...
void
Geometry::accept( ConstAttributeFunctor& af ) const
{
    ...
    {
        OSG_INFO << "Geometry::accept(ConstAttributeFunctor& af): Using vertex "
                    "attribute instead"
                 << std::endl;
        afav.applyArray( VERTICES, _vertexAttribList[0].get() );
    }
...
Geometry::accept( PrimitiveFunctor& functor ) const
{
    ...
    {
        OSG_WARN << "Geometry::accept(PrimitiveFunctor& functor) unable to work due to "
                    "deprecated data, call geometry->fixDeprecatedData();"
                 << std::endl;
        return;
    }
    ...
    switch( vertices->getType() )
    {
        case( Array::Vec2ArrayType ) :
            functor.setVertexArray(
                vertices->getNumElements(),
                static_cast<const vec2*>( vertices->getDataPointer() )
            );
            break;
        case( Array::Vec3ArrayType ) :
            functor.setVertexArray(
                vertices->getNumElements(),
                static_cast<const vec3*>( vertices->getDataPointer() )
            );
            break;
        case( Array::Vec4ArrayType ) :
            functor.setVertexArray(
                vertices->getNumElements(),
                static_cast<const vec4*>( vertices->getDataPointer() )
            );
            break;
        case( Array::Vec2dArrayType ) :
            functor.setVertexArray(
                vertices->getNumElements(),
                static_cast<const dvec2*>( vertices->getDataPointer() )
            );
            break;
        case( Array::Vec3dArrayType ) :
            functor.setVertexArray(
                vertices->getNumElements(),
                static_cast<const dvec3*>( vertices->getDataPointer() )
            );
            break;
        case( Array::Vec4dArrayType ) :
            functor.setVertexArray(
                vertices->getNumElements(),
                static_cast<const dvec4*>( vertices->getDataPointer() )
            );
            break;
        default :
            OSG_WARN << "Warning: Geometry::accept(PrimitiveFunctor&) cannot handle "
                        "Vertex Array type"
                     << vertices->getType() << std::endl;
            return;
    }
...
void
Geometry::accept( PrimitiveIndexFunctor& functor ) const
{
    const osg::Array* vertices = _vertexArray.get();

    if( !vertices && _vertexAttribList.size() > 0 )
    {
        OSG_INFO << "Geometry::accept(PrimitiveIndexFunctor& functor): Using vertex "
                    "attribute instead"
                 << std::endl;
        vertices = _vertexAttribList[0].get();
    }
    ...
    if( _containsDeprecatedData &&
        dynamic_cast<const osg::IndexArray*>( vertices->getUserData() ) != 0 )
    {
        OSG_WARN << "Geometry::accept(PrimitiveIndexFunctor& functor) unable to work "
                    "due to deprecated data, call geometry->fixDeprecatedData();"
                 << std::endl;
        return;
    }
    ...
    switch( vertices->getType() )
    {
        ...
        case( Array::Vec4dArrayType ) :
            ...
            break;
        default :
            OSG_WARN << "Warning: Geometry::accept(PrimitiveIndexFunctor&) cannot "
...
Geometry*
osg::createTexturedQuadGeometry( const vec3& corner,
                                 const vec3& widthVec,
                                 ...
                                 float       t )
{
    ...
    ( *normals )[0]         = osg::cross( widthVec, heightVec );
    ( *normals )[0]         = osg::normalize( ( *normals )[0] );
...
void
Geometry::setNormalBinding( AttributeBinding ab )
{
    SET_BINDING( _normalArray.get(), ab )
}

void
Geometry::setColorBinding( AttributeBinding ab )
{
    SET_BINDING( _colorArray.get(), ab )
}

void
Geometry::setSecondaryColorBinding( AttributeBinding ab )
{
    SET_BINDING( _secondaryColorArray.get(), ab )
}

void
Geometry::setFogCoordBinding( AttributeBinding ab )
{
    SET_BINDING( _fogCoordArray.get(), ab )
}

void
Geometry::setVertexAttribBinding( unsigned int     index,
                                  AttributeBinding ab )
{
    osg::Array::Binding binding = static_cast<osg::Array::Binding>( ab );
...
void
Geometry::setVertexAttribNormalize( unsigned int index,
                                    GLboolean    norm )
{
    if( index < _vertexAttribList.size() && _vertexAttribList[index].valid() )
    {
...
Geometry::AttributeBinding
Geometry::getNormalBinding() const
{
    return GET_BINDING( _normalArray.get() );
}

Geometry::AttributeBinding
Geometry::getColorBinding() const
{
    return GET_BINDING( _colorArray.get() );
}

Geometry::AttributeBinding
Geometry::getSecondaryColorBinding() const
{
    return GET_BINDING( _secondaryColorArray.get() );
}

Geometry::AttributeBinding
Geometry::getFogCoordBinding() const
{
    return GET_BINDING( _fogCoordArray.get() );
}

Geometry::AttributeBinding
Geometry::getVertexAttribBinding( unsigned int index ) const
{
    if( index < _vertexAttribList.size() && _vertexAttribList[index].valid() )
    {
...
GLboolean
Geometry::getVertexAttribNormalize( unsigned int index ) const
{
    if( index < _vertexAttribList.size() && _vertexAttribList[index].valid() )
    {
...
//
bool
Geometry::containsSharedArrays() const
{
    ...
    {
        if( getVertexAttribArray( vi ) &&
            getVertexAttribArray( vi )->referenceCount() > 1 )
        {
            ++numSharedArrays;
        }
    }
...
#define DUPLICATE_IF_REQUIRED( A )                                       \
    ...
    for( unsigned int vi = 0; vi < _vertexAttribList.size(); ++vi )
    {
        if( getVertexAttribArray( vi ) &&
            getVertexAttribArray( vi )->referenceCount() > 1 )
        {
            setVertexAttribArray( vi,
                                  osg::clone( getVertexAttribArray( vi ),
                                              osg::CopyOp::DEEP_COPY_ARRAYS ) );
        }
    }
}
...
namespace GeometryUtilFunctions
{
    ...
    getIndexArray( osg::Array* array )
    {
        return array ? dynamic_cast<osg::IndexArray*>( array->getUserData() ) : 0;
    }

    inline bool
    containsDeprecatedUsage( osg::Array* array )
    {
        if( array )
        {
    ...
    static osg::Array*
    expandIndexArray( const osg::Array*      sourceArray,
                      const osg::IndexArray* indices )
    {
        ...
        const char*  sourcePtr =
            static_cast<const char*>( sourceArray->getDataPointer() );
        char* targetPtr = const_cast<char*>(
            static_cast<const char*>( targetArray->getDataPointer() )
        );
    ...
    static void
    duplicateArray( ArrayPairs&               pairs,
                    osg::ref_ptr<osg::Array>& sourceArray,
    ...
    struct PtrData
    {
            ...
            PtrData( osg::Array* s,
                     osg::Array* t ) :
                source(
                    const_cast<char*>( static_cast<const char*>( s->getDataPointer() ) )
                ),
                target(
                    const_cast<char*>( static_cast<const char*>( t->getDataPointer() ) )
                ),
...
bool
Geometry::checkForDeprecatedData()
{
    _containsDeprecatedData = false;
    ...
    {
        if( GeometryUtilFunctions::containsDeprecatedUsage(
                _vertexAttribList[vi].get()
            ) )
        {
            _containsDeprecatedData = true;
        }
...
Geometry::fixDeprecatedData()
{
    ...
    osg::IndexArray* indices =
        GeometryUtilFunctions::getIndexArray( _vertexArray.get() );

    if( indices )
    {
        setVertexArray( GeometryUtilFunctions::expandIndexArray( _vertexArray.get(),
                                                                 indices ) );
    }

    if( osg::getBinding( _normalArray.get() ) ==
        3 /*osg::Geometry::BIND_PER_PRIMITIVE*/ )
    {
    ...
    if( indices )
    {
        setNormalArray( GeometryUtilFunctions::expandIndexArray( getNormalArray(),
                                                                 indices ) );
    }
    ...
    if( indices )
    {
        setColorArray( GeometryUtilFunctions::expandIndexArray( getColorArray(),
                                                                indices ) );
    }

    if( osg::getBinding( getSecondaryColorArray() ) ==
        3 /*osg::Geometry::BIND_PER_PRIMITIVE*/ )
    {
        containsBindPerPrimitive = true;
    }
    ...
    if( indices )
    {
        setSecondaryColorArray(
            GeometryUtilFunctions::expandIndexArray( getSecondaryColorArray(), indices )
        );
    }

    if( osg::getBinding( getFogCoordArray() ) ==
        3 /*osg::Geometry::BIND_PER_PRIMITIVE*/ )
    {
        containsBindPerPrimitive = true;
    }
    ...
    if( indices )
    {
        setFogCoordArray( GeometryUtilFunctions::expandIndexArray( getFogCoordArray(),
                                                                   indices ) );
    }
    ...
    for( unsigned int ti = 0; ti < getNumTexCoordArrays(); ++ti )
    {
        ...
        if( indices )
        {
            setTexCoordArray(
                ti,
                GeometryUtilFunctions::expandIndexArray( getTexCoordArray( ti ),
                                                         indices )
            );
        }
    }
    ...
    for( unsigned int vi = 0; vi < _vertexAttribList.size(); ++vi )
    {
        if( osg::getBinding( getVertexAttribArray( vi ) ) ==
            3 /*osg::Geometry::BIND_PER_PRIMITIVE*/ )
        {
            containsBindPerPrimitive = true;
        }
        ...
        if( indices )
        {
            setVertexAttribArray(
                vi,
                GeometryUtilFunctions::expandIndexArray( getVertexAttribArray( vi ),
                                                         indices )
            );
        }
    }
    ...
    for( PrimitiveSetList::iterator itr = _primitives.begin(); itr != _primitives.end();
         ++itr )
    {
        ...
        switch( primitiveset->getType() )
        {
            case( PrimitiveSet::DrawArraysPrimitiveType ) :
                {
                    const DrawArrays* drawArray =
                        static_cast<const DrawArrays*>( primitiveset );
                    numVertices += static_cast<unsigned int>( drawArray->getCount() );
                    break;
                }
            case( PrimitiveSet::DrawArrayLengthsPrimitiveType ) :
                {
                    const DrawArrayLengths* drawArrayLengths =
                        static_cast<const DrawArrayLengths*>( primitiveset );
                    for( DrawArrayLengths::const_iterator primItr =
                             drawArrayLengths->begin();
                         primItr != drawArrayLengths->end();
                         ++primItr )
                    {
                        unsigned int localNumVertices =
                            static_cast<unsigned int>( *primItr );
            ...
            case( PrimitiveSet::DrawElementsUBytePrimitiveType ) :
                {
                    const DrawElementsUByte* drawElements =
                        static_cast<const DrawElementsUByte*>( primitiveset );
                    numVertices += drawElements->getNumIndices();
                    break;
                }
            case( PrimitiveSet::DrawElementsUShortPrimitiveType ) :
                {
                    const DrawElementsUShort* drawElements =
                        static_cast<const DrawElementsUShort*>( primitiveset );
                    numVertices += drawElements->getNumIndices();
                    break;
                }
            case( PrimitiveSet::DrawElementsUIntPrimitiveType ) :
                {
                    const DrawElementsUInt* drawElements =
                        static_cast<const DrawElementsUInt*>( primitiveset );
                    numVertices += drawElements->getNumIndices();
                    break;
                }
            default :
    ...
    if( _vertexArray.valid() )
    {
        GeometryUtilFunctions::duplicateArray( perVertexArrays,
                                               _vertexArray,
                                               numVertices );
    }
    ...
    if( _normalArray.valid() )
    {
        ...
        {
            GeometryUtilFunctions::duplicateArray( perPrimitiveArrays,
                                                   _normalArray,
                                                   numVertices );
        }
    }
    ...
    if( _secondaryColorArray.valid() )
    {
        ...
        else if( _secondaryColorArray->getBinding() ==
                 3 /*osg::Array::BIND_PER_PRIMITIVE*/ )
        {
            GeometryUtilFunctions::duplicateArray( perPrimitiveArrays,
                                                   _secondaryColorArray,
                                                   numVertices );
    ...
    for( GeometryUtilFunctions::ArrayPairs::iterator itr = perVertexArrays.begin();
         ...
         ++itr )
    {
        perVertexPtrs.push_back( GeometryUtilFunctions::PtrData( itr->first.get(),
                                                                 itr->second.get() ) );
    }
    ...
    for( GeometryUtilFunctions::ArrayPairs::iterator itr = perPrimitiveArrays.begin();
         ...
         ++itr )
    {
        perPrimitivePtrs.push_back(
            GeometryUtilFunctions::PtrData( itr->first.get(), itr->second.get() )
        );
    }
         ...
         ++prim_itr )
    {
        ...
        switch( primitiveset->getType() )
        {
            case( PrimitiveSet::DrawArraysPrimitiveType ) :
                {
                    ...
                    if( primLength == 0 )
                    {
                        primLength = static_cast<unsigned int>( drawArray->getCount() );
                    }

                    unsigned int primCount = 0;
                    unsigned int startindex =
                        static_cast<unsigned int>( drawArray->getFirst() );
                    drawArray->setFirst( target_vindex );
                    unsigned int indexEnd =
                        startindex + static_cast<unsigned int>( drawArray->getCount() );
                    for( unsigned int vindex = startindex; vindex < indexEnd;
                         ++vindex, ++target_vindex, ++primCount )
                    {
                             ...
                             ++itr )
                        {
                            ...
                            char* target = ptrs.target +
                                           static_cast<unsigned int>( target_vindex ) *
                                           ptrs.elementSize;
                        ...
                        {
                            GeometryUtilFunctions::PtrData& ptrs = *itr;
                            char* source = ptrs.source +
                                           static_cast<unsigned int>( source_pindex ) *
                                           ptrs.elementSize;
                            char* target = ptrs.target +
                                           static_cast<unsigned int>( target_vindex ) *
                                           ptrs.elementSize;
            ...
            case( PrimitiveSet::DrawArrayLengthsPrimitiveType ) :
                {
                    DrawArrayLengths* drawArrayLengths =
                        static_cast<DrawArrayLengths*>( primitiveset );
                    unsigned int vindex =
                        static_cast<unsigned int>( drawArrayLengths->getFirst() );
                         ...
                         ++primItr )
                    {
                        ...
                        if( primLength == 0 )
                        {
                            localPrimLength = static_cast<unsigned int>( *primItr );
                        }
                        else
                        {
                        ...
                        for( GLsizei primCount = 0; primCount < *primItr;
                             ++vindex, ++target_vindex, ++primCount )
                        {
                            if( ( static_cast<unsigned int>( primCount ) %
                                  localPrimLength ) == 0 )
                            {
                                ++source_pindex;
                            }
            ...
            case( PrimitiveSet::DrawElementsUBytePrimitiveType ) :
                {
                    ...
                    DrawElementsUByte* drawElements =
                        static_cast<DrawElementsUByte*>( primitiveset );
                    unsigned int primCount = 0;
                         ...
                         ++primCount, ++target_vindex, ++primItr )
                    {
                        ...
                        *primItr = static_cast<unsigned char>( target_vindex );
            ...
            case( PrimitiveSet::DrawElementsUShortPrimitiveType ) :
                {
                    ...
                    DrawElementsUShort* drawElements =
                        static_cast<DrawElementsUShort*>( primitiveset );
                    unsigned int primCount = 0;
                         ...
                         ++primCount, ++target_vindex, ++primItr )
                    {
                        ...
                        unsigned int vindex = *primItr;
                        *primItr = static_cast<unsigned short>( target_vindex );
            ...
            case( PrimitiveSet::DrawElementsUIntPrimitiveType ) :
                {
                    ...
                    DrawElementsUInt* drawElements =
                        static_cast<DrawElementsUInt*>( primitiveset );
                    unsigned int primCount = 0;
                         ...
                         ++primCount, ++target_vindex, ++primItr )
                    {
                        ...
                        unsigned int vindex = *primItr;
                        *primItr            = static_cast<unsigned int>( target_vindex );
