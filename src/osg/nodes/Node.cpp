...
namespace osg
{
    ...
    /// Helper class for generating NodePathList via const traversal.
    class CollectParentPaths : public ConstNodeVisitor
    {
        public:

            CollectParentPaths( const osg::Node* haltTraversalAtNode = 0 ) :
                osg::ConstNodeVisitor( osg::ConstNodeVisitor::TRAVERSE_PARENTS ),
...
void
Node::addParent( osg::Group* parent )
{
...
void
Node::removeParent( osg::Group* parent )
{
    std::unique_lock<std::mutex> lock( *getRefMutex() );
...
void
Node::ascend( NodeVisitor& nv )
{
    std::for_each( _parents.begin(), _parents.end(), NodeAcceptOp( nv ) );
}

void
Node::accept( ConstNodeVisitor& nv ) const
{
    if( nv.validNodeMask( *this ) )
    {
        nv.pushOntoNodePath( this );
...
void
Node::ascend( ConstNodeVisitor& nv ) const
{
    for( auto parent : _parents )
    {
        static_cast<const Node*>( parent )->accept( nv );
    }
}

void
Node::setStateSet( osg::StateSet* stateset )
{
    ...
    if( delta_update != 0 )
    {
        setNumChildrenRequiringUpdateTraversal(
            ...
            static_cast<unsigned int>( delta_update )
        );
    }
    ...
    if( delta_event != 0 )
    {
        setNumChildrenRequiringEventTraversal(
            ...
            static_cast<unsigned int>( delta_event )
        );
    }
}
...
/// Convert a ConstNodePath to a NodePath.
/// Safe because parent-path nodes originate from Group::_parents (mutable Group*).
static NodePath
toMutableNodePath( const ConstNodePath& cpath )
{
    NodePath result;
    result.reserve( cpath.size() );
    for( const Node* n : cpath )
    {
        result.push_back( const_cast<Node*>( n ) );
    }
    return result;
}

NodePathList
Node::getParentalNodePaths( osg::Node* haltTraversalAtNode ) const
{
    CollectParentPaths cpp( haltTraversalAtNode );
    this->accept( cpp );

    NodePathList result;
    result.reserve( cpp._nodePaths.size() );
    for( const auto& cpath : cpp._nodePaths )
    {
        result.push_back( toMutableNodePath( cpath ) );
    }
    return result;
}

MatrixList
Node::getWorldMatrices( const osg::Node* haltTraversalAtNode ) const
{
    CollectParentPaths cpp( haltTraversalAtNode );
    this->accept( cpp );
    ...
    for( const auto& cpath : cpp._nodePaths )
    {
        if( cpath.empty() )
        {
            matrices.push_back( osg::dmat4() );
        }
        else
        {
            matrices.push_back( osg::computeLocalToWorld( toMutableNodePath( cpath ) ) );
        }
    }
...
void
Node::setUpdateCallback( Callback* nc )
{
        ...
        if( delta != 0 )
        {
                 ...
                 ++itr )
            {
                ( *itr )->setNumChildrenRequiringUpdateTraversal(
                    ( *itr )->getNumChildrenRequiringUpdateTraversal() +
...
void
Node::setNumChildrenRequiringUpdateTraversal( unsigned int num )
{
    // if no changes just return.
...
void
Node::setEventCallback( Callback* nc )
{
    // if no changes just return.
...
void
Node::setNumChildrenRequiringEventTraversal( unsigned int num )
{
    // if no changes just return.
...
void
Node::setCullingActive( bool active )
{
    // if no changes just return.
        ...
        if( delta != 0 )
        {
                 ...
                 ++itr )
            {
                ( *itr )->setNumChildrenWithCullingDisabled(
                    ( *itr )->getNumChildrenWithCullingDisabled() +
                    static_cast<unsigned int>( delta )
                );
            }
        }
...
void
Node::setNumChildrenWithCullingDisabled( unsigned int num )
{
    // if no changes just return.
...
void
Node::setNumChildrenWithOccluderNodes( unsigned int num )
{
    // if no changes just return.
        ...
        if( delta != 0 )
        {
                 ...
                 ++itr )
            {
                ( *itr )->setNumChildrenWithOccluderNodes(
                    ( *itr )->getNumChildrenWithOccluderNodes() +
                    static_cast<unsigned int>( delta )
                );
            }
        }
...
bool
Node::containsOccluderNodes() const
{
    return _numChildrenWithOccluderNodes >
           0 ||
           dynamic_cast<const OccluderNode*>( this );
}

void
Node::setDescriptions( const DescriptionList& descriptions )
{
...
static std::mutex s_mutex_StaticDescriptionList;

static const Node::DescriptionList&
getStaticDescriptionList()
{
    std::lock_guard<std::mutex>  lock( s_mutex_StaticDescriptionList );
    static Node::DescriptionList s_descriptionList;
    return s_descriptionList;
}

const Node::DescriptionList&
Node::getDescriptions() const
{
...
std::string&
Node::getDescription( unsigned int i )
{
    return getOrCreateUserDataContainer()->getDescriptions()[i];
}

const std::string&
Node::getDescription( unsigned int i ) const
{
    if( _userDataContainer )
    {
...
unsigned int
Node::getNumDescriptions() const
{
    return _userDataContainer
             ? static_cast<unsigned int>( _userDataContainer->getDescriptions().size() )
             : 0;
}

void
Node::addDescription( const std::string& desc )
{
    getOrCreateUserDataContainer()->getDescriptions().push_back( desc );
}

sphere
Node::computeBound() const
{
    return sphere();
}
...
void
Node::setThreadSafeRefUnref( bool threadSafe )
{
...
void
Node::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _stateset.valid() )
    {
...
void
Node::releaseGLObjects( osg::State* state ) const
{
    if( _stateset.valid() )
    {
