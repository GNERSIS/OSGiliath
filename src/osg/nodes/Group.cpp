...
void
Group::traverse( ConstNodeVisitor& nv ) const
{
...
{
    return Group::insertChild( static_cast<unsigned int>( _children.size() ), child );
}

bool
Group::insertChild( unsigned int index,
                    Node*        child )
{
    if( !child )
    {
    ...
    if( index >= _children.size() )
    {
        index = static_cast<unsigned int>(
            _children.size()
        );    // set correct index value to be passed to the
    ...
    // tell any subclasses that a child has been inserted so that they can update
    // themselves.
    childInserted( index );
    ...
    {
        setNumChildrenWithCullingDisabled( getNumChildrenWithCullingDisabled() + 1 );
    }

    if( child->getNumChildrenWithOccluderNodes() > 0 || child->asOccluderNode() )
    {
        setNumChildrenWithOccluderNodes( getNumChildrenWithOccluderNodes() + 1 );
    }
...
bool
Group::removeChildren( unsigned int pos,
                       unsigned int numChildrenToRemove )
{
    if( pos < _children.size() && numChildrenToRemove > 0 )
    {
        ...
        {
            OSG_DEBUG << "Warning: Group::removeChild(i,numChildrenToRemove) has been "
                         "passed an excessive number"
                      << std::endl;
            OSG_DEBUG
                << "         of children to remove, trimming just to end of child list."
                << std::endl;
            endOfRemoveRange = static_cast<unsigned int>( _children.size() );
        }
        ...
        {
            setNumChildrenRequiringUpdateTraversal(
                getNumChildrenRequiringUpdateTraversal() - updateCallbackRemoved
            );
        }
        ...
        if( eventCallbackRemoved )
        {
            setNumChildrenRequiringEventTraversal(
                getNumChildrenRequiringEventTraversal() - eventCallbackRemoved
            );
        }

        if( numChildrenWithCullingDisabledRemoved )
        {
            setNumChildrenWithCullingDisabled( getNumChildrenWithCullingDisabled() -
                                               numChildrenWithCullingDisabledRemoved );
        }

        if( numChildrenWithOccludersRemoved )
        {
            setNumChildrenWithOccluderNodes( getNumChildrenWithOccluderNodes() -
                                             numChildrenWithOccludersRemoved );
        }
...
bool
Group::replaceChild( Node* origNode,
                     Node* newNode )
{
    ...
    if( i < _children.size() && newNode )
    {
        ...
        if( delta_numChildrenRequiringEventTraversal != 0 )
        {
            setNumChildrenRequiringEventTraversal( static_cast<unsigned int>(
                static_cast<int>( getNumChildrenRequiringEventTraversal() ) +
                delta_numChildrenRequiringEventTraversal
            ) );
        }
        ...
        {
            setNumChildrenWithCullingDisabled( static_cast<unsigned int>(
                static_cast<int>( getNumChildrenWithCullingDisabled() ) +
                delta_numChildrenWithCullingDisabled
            ) );
        }
        ...
        if( newNode->getNumChildrenWithOccluderNodes() > 0 || newNode->asOccluderNode() )
        {
            ++delta_numChildrenWithOccluderNodes;
        }
        ...
        {
            setNumChildrenWithOccluderNodes( static_cast<unsigned int>(
                static_cast<int>( getNumChildrenWithOccluderNodes() ) +
                delta_numChildrenWithOccluderNodes
            ) );
        }
...
Group::computeBound() const
{
    ...
    for( itr = _children.begin(); itr != _children.end(); ++itr )
    {
        ...
        if( !transform || transform->getReferenceFrame() == osg::Transform::RELATIVE_RF )
        {
            ...
            {
                const osg::sphere& bs = child->getBound();
                bb.expandBy( bs );
            }
        }
    }
    ...
    for( itr = _children.begin(); itr != _children.end(); ++itr )
    {
        ...
        if( !transform || transform->getReferenceFrame() == osg::Transform::RELATIVE_RF )
        {
            const sphere& bs = child->getBound();
...
void
Group::setThreadSafeRefUnref( bool threadSafe )
{
...
void
Group::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
Group::releaseGLObjects( osg::State* state ) const
{
