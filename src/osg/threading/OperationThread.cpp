...
unsigned int
OperationQueue::getNumOperationsInQueue()
{
    std::lock_guard<std::mutex> lock( _operationsMutex );
...
void
OperationQueue::remove( Operation* operation )
{
    ...
    for( Operations::iterator itr = _operations.begin(); itr != _operations.end(); )
    {
        if( ( *itr ) == operation )
        {
            ...
            {
                _currentOperationIterator =
                    ( itr == _operations.end() ) ? _operations.begin() : itr;
            }
        }
        else
        {
...
void
OperationQueue::remove( const std::string& name )
{
...
void
OperationQueue::removeAllOperations()
{
...
void
OperationQueue::runOperations( Object* callingObject )
{
    std::lock_guard<std::mutex> lock( _operationsMutex );
    ...
    for( ; _currentOperationIterator != _operations.end(); )
    {
...
void
OperationQueue::releaseOperationsBlock()
{
    _operationsBlock->release();
}

void
OperationQueue::releaseAllOperations()
{
    std::lock_guard<std::mutex> lock( _operationsMutex );
...
void
OperationQueue::addOperationThread( OperationThread* thread )
{
    _operationThreads.insert( thread );
}

void
OperationQueue::removeOperationThread( OperationThread* thread )
{
...
void
OperationThread::setOperationQueue( OperationQueue* opq )
{
...
void
OperationThread::setDone( bool done )
{
    unsigned d = done ? 1 : 0;
    if( _done == d )
    {
...
{
    OSG_INFO << "Cancelling OperationThread " << this << " isRunning()=" << isRunning()
             << std::endl;
    ...
    if( isRunning() )
    {
        ...
        while( isRunning() )
        {
            ...
            OSG_DEBUG << "   Waiting for OperationThread to cancel " << this
                      << std::endl;
            std::this_thread::yield();
        }

        // use join to appease valgrind as the above loop won't exit till the thread
        // stops running but valgrind doesn't reconginze this and assumes the thread is
        // still running
        join();
    }

    OSG_INFO << "  OperationThread::cancel() thread cancelled " << this
...
void
OperationThread::add( Operation* operation )
{
    std::lock_guard<std::mutex> lock( _threadMutex );
    if( !_operationQueue )
    {
...
void
OperationThread::remove( Operation* operation )
{
    std::lock_guard<std::mutex> lock( _threadMutex );
    if( _operationQueue.valid() )
    {
...
void
OperationThread::remove( const std::string& name )
{
    std::lock_guard<std::mutex> lock( _threadMutex );
    if( _operationQueue.valid() )
    {
...
void
OperationThread::removeAllOperations()
{
    std::lock_guard<std::mutex> lock( _threadMutex );
    if( _operationQueue.valid() )
    {
...
OperationThread::run()
{
    ...
    do
    {
        ...
        // OSG_NOTICE<<"operations.size()="<<_operations.size()<<" done="<<_done<<"
        // testCancel()"<<testCancel()<<std::endl;
