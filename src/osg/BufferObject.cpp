...
void GLBufferObject::compileBuffer()
{
    ...
    if (_allocatedSize != _profile._size)
    {
        ...
        OSG_INFO<<"    Allocating new glNamedBufferData(), _allocatedSize="<<_allocatedSize<<std::endl;

        if(_profile._mappingbitfield != 0)
        {
            _extensions->glNamedBufferStorage(_glObjectID, _profile._size, NULL, _profile._usage);
            ...
            if(_profile._usage & GL_MAP_PERSISTENT_BIT )
            {
                ...
                _persistentDMA = glMapNamedBufferRange(_glObjectID, 0, _profile._size, _profile._mappingbitfield);
            }
        }
        else
        {
        ...
        ++itr)
    {
        ...
        if (entry.dataSource && (compileAll || entry.modifiedCount != entry.dataSource->getModifiedCount()))
        {
            ...
            if (image && !(image->isDataContiguous()))
            {
                ...
                {
                    glNamedBufferSubData(_glObjectID, (GLintptr)offset, (GLsizeiptr)img_itr.size(), img_itr.data());
                    offset += img_itr.size();
                }
            }
            else
            {
                if(_profile._mappingbitfield != 0 )
                {
                    if(_profile._mappingbitfield & GL_MAP_PERSISTENT_BIT)
                    {
                        if(_persistentDMA)
                        {
                            ...
                            glFlushMappedNamedBufferRange(_glObjectID, (GLintptr)entry.offset, (GLsizeiptr)entry.dataSize);
                        }
                    ...
                    {
                        GLvoid *src = const_cast<GLvoid*>(entry.dataSource->getDataPointer()),
                               *dst = glMapNamedBufferRange( _glObjectID, (GLintptr)entry.offset,  (GLsizeiptr)entry.dataSize, _profile._mappingbitfield);
                ...
                {
                    glNamedBufferSubData(_glObjectID, (GLintptr)entry.offset, (GLsizeiptr)entry.dataSize, entry.dataSource->getDataPointer());
                }
            }
        }
    }
...
void GLBufferObject::downloadBuffer(unsigned int entryidx)
{
    ...
    GLvoid *dst = const_cast<GLvoid*>(entry.dataSource->getDataPointer()),
           *src = glMapNamedBufferRange( _glObjectID, (GLintptr)entry.offset, (GLsizeiptr)entry.dataSize, _profile._mappingbitfield);
    memcpy(dst, src, entry.dataSize);
    glUnmapNamedBuffer(_glObjectID);
}
