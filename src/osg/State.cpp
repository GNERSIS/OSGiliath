    ...
    Referenced(true)
{
    ...
    _lightSpecularUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_LightSource.specular");
    _lightPositionUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_LightSource.position");
    _lightSpotDirectionUniform = new Uniform(Uniform::FLOAT_VEC3, "osg_LightSource.spotDirection");
    _lightSpotExponentUniform = new Uniform(Uniform::FLOAT, "osg_LightSource.spotExponent");
    _lightSpotCutoffUniform = new Uniform(Uniform::FLOAT, "osg_LightSource.spotCutoff");
    _lightSpotCosCutoffUniform = new Uniform(Uniform::FLOAT, "osg_LightSource.spotCosCutoff");
    _lightConstantAttenuationUniform = new Uniform(Uniform::FLOAT, "osg_LightSource.constantAttenuation");
    _lightLinearAttenuationUniform = new Uniform(Uniform::FLOAT, "osg_LightSource.linearAttenuation");
    _lightQuadraticAttenuationUniform = new Uniform(Uniform::FLOAT, "osg_LightSource.quadraticAttenuation");
    ...
    _materialAmbientUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_FrontMaterial.ambient");
    _materialDiffuseUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_FrontMaterial.diffuse");
    _materialSpecularUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_FrontMaterial.specular");
    _materialEmissionUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_FrontMaterial.emission");
    _materialShininessUniform = new Uniform(Uniform::FLOAT, "osg_FrontMaterial.shininess");
    ...
    _lightModelAmbientUniform = new Uniform(Uniform::FLOAT_VEC4, "osg_LightModel_ambient");
...
void State::initUpModeDefineMaps()
{
    ...
    // TexGen modes for shader pipeline texture coordinate generation.
    // These are tracked (not sent to GL) and used for shader defines.
    _stringModeMap["GL_TEXTURE_GEN_S"] = GL_TEXTURE_GEN_S;
    _stringModeMap["GL_TEXTURE_GEN_T"] = GL_TEXTURE_GEN_T;
    _stringModeMap["GL_TEXTURE_GEN_R"] = GL_TEXTURE_GEN_R;
    _stringModeMap["GL_TEXTURE_GEN_Q"] = GL_TEXTURE_GEN_Q;
    ...
    _stringModeMap["OSG_LIGHTING"] = GL_LIGHTING;
    _stringModeMap["OSG_LIGHT0"] = GL_LIGHT0;
    ...
    for(unsigned int i=0; i<maxNumTextureUnits; ++i)
    {
        _textureModeDefineMapList[i][GL_TEXTURE_1D] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" TexCoord"<<i<<" = osg_MultiTexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler1D sampler"<<i<<"; in vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", TexCoord"<<i<<".s)"<<std::endl;

        _textureModeDefineMapList[i][GL_TEXTURE_2D] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" TexCoord"<<i<<" = osg_MultiTexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler2D sampler"<<i<<"; in vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", TexCoord"<<i<<".st)"<<std::endl;

        _textureModeDefineMapList[i][GL_TEXTURE_RECTANGLE] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" TexCoord"<<i<<" = osg_MultiTexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler2DRect sampler"<<i<<"; in vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", TexCoord"<<i<<".st)"<<std::endl;

        _textureModeDefineMapList[i][GL_TEXTURE_3D] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" TexCoord"<<i<<" = osg_MultiTexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler3D sampler"<<i<<"; in vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", TexCoord"<<i<<".stp)"<<std::endl;

        _textureModeDefineMapList[i][GL_TEXTURE_CUBE_MAP] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec3 ReflectCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" { vec3 ecPos3 = (osg_ModelViewMatrix * osg_Vertex).xyz; vec3 ecN = normalize(osg_NormalMatrix * osg_Normal); ReflectCoord"<<i<<" = reflect(normalize(ecPos3), ecN); }"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform samplerCube sampler"<<i<<"; in vec3 ReflectCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", ReflectCoord"<<i<<")"<<std::endl;

        _textureModeDefineMapList[i][GL_TEXTURE_2D_ARRAY] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" TexCoord"<<i<<" = osg_MultiTexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler2DArray sampler"<<i<<"; in vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", TexCoord"<<i<<".stp)"<<std::endl;
        ...
        _textureModeDefineMapList[i][GL_POINT_SPRITE] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler2D sampler"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", gl_PointCoord)"<<std::endl;

        // TexGen SPHERE_MAP: generates sphere-map texture coordinates in vertex shader.
        // Overrides regular texture coordinate pass-through when GL_TEXTURE_GEN_S is active.
        // The sphere map formula computes reflection-based 2D tex coords from the normal.
        _textureModeDefineMapList[i][GL_TEXTURE_GEN_S] = str.clear()
            <<"#define TEXTURE_VERT_DECLARE"<<i<<" out vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_VERT_BODY"<<i<<" { vec3 u = normalize((osg_ModelViewMatrix * osg_Vertex).xyz); vec3 n = normalize(osg_NormalMatrix * osg_Normal); vec3 r = reflect(u, n); float m = 2.0 * sqrt(r.x*r.x + r.y*r.y + (r.z+1.0)*(r.z+1.0)); TexCoord"<<i<<" = vec4(r.x/m + 0.5, r.y/m + 0.5, 0.0, 1.0); }"<<std::endl
            <<"#define TEXTURE_FRAG_DECLARE"<<i<<" uniform sampler2D sampler"<<i<<"; in vec4 TexCoord"<<i<<";"<<std::endl
            <<"#define TEXTURE_FUNCTION"<<i<<"() texture( sampler"<<i<<", TexCoord"<<i<<".st)"<<std::endl;
    }
}
...
void State::apply(const StateSet* dstate)
{
    ...
    if (dstate)
    {
        ...
        {
            // OSG_INFO<<"State::apply(StateSet*) Program re-apply: defineMap.changed="<<_defineMap.changed<<std::endl;
...
void State::applyModelViewAndProjectionUniformsIfRequired()
{
    ...
    if (_lightAmbientUniform.valid()) _lastAppliedProgramObject->apply(*_lightAmbientUniform);
    if (_lightDiffuseUniform.valid()) _lastAppliedProgramObject->apply(*_lightDiffuseUniform);
    if (_lightSpecularUniform.valid()) _lastAppliedProgramObject->apply(*_lightSpecularUniform);

    if (_lightPositionUniform.valid()) _lastAppliedProgramObject->apply(*_lightPositionUniform);
    if (_lightSpotDirectionUniform.valid()) _lastAppliedProgramObject->apply(*_lightSpotDirectionUniform);
    if (_lightSpotExponentUniform.valid()) _lastAppliedProgramObject->apply(*_lightSpotExponentUniform);
    if (_lightSpotCutoffUniform.valid()) _lastAppliedProgramObject->apply(*_lightSpotCutoffUniform);
    if (_lightSpotCosCutoffUniform.valid()) _lastAppliedProgramObject->apply(*_lightSpotCosCutoffUniform);
    if (_lightConstantAttenuationUniform.valid()) _lastAppliedProgramObject->apply(*_lightConstantAttenuationUniform);
    if (_lightLinearAttenuationUniform.valid()) _lastAppliedProgramObject->apply(*_lightLinearAttenuationUniform);
    if (_lightQuadraticAttenuationUniform.valid()) _lastAppliedProgramObject->apply(*_lightQuadraticAttenuationUniform);

    // Front material uniforms
    if (_materialAmbientUniform.valid()) _lastAppliedProgramObject->apply(*_materialAmbientUniform);
    if (_materialDiffuseUniform.valid()) _lastAppliedProgramObject->apply(*_materialDiffuseUniform);
    if (_materialSpecularUniform.valid()) _lastAppliedProgramObject->apply(*_materialSpecularUniform);
    if (_materialEmissionUniform.valid()) _lastAppliedProgramObject->apply(*_materialEmissionUniform);
    if (_materialShininessUniform.valid()) _lastAppliedProgramObject->apply(*_materialShininessUniform);

    // Color material and light model
    if (_colorMaterialUniform.valid()) _lastAppliedProgramObject->apply(*_colorMaterialUniform);
    if (_lightModelAmbientUniform.valid()) _lastAppliedProgramObject->apply(*_lightModelAmbientUniform);
}

void State::setLightUniforms(const Light& light)
{
    ...
    float spotCutoffRad = osg::DegreesToRadians(light.getSpotCutoff());
...
void State::setMaterialUniforms(const Material& material)
{
...
void State::updateModelViewAndProjectionMatrixUniforms()
{
    ...
    if (_normalMatrixUniform.valid())
    {
        ...
        // Store transpose(inverse(MV_rot)) in OSG row-major format.
        // GLSL upload (GL_FALSE = column-major) implicitly transposes this,
        // so the shader receives inverse(MV_rot) which is the correct NormalMatrix
        // for GLSL column-vector convention with OSG's row-vector matrices.
    ...
    if (!shaderPragmas.modes.empty())
    {
        ...
        for(unsigned int i=0; i<_textureModeMapList.size(); ++i)
        {
            ...
            bool pointSpriteActive = false;
            {
                ...
                {
                    ModeDefineMap::const_iterator mdm_itr = modeDefineMap.find(GL_POINT_SPRITE);
            ...
            if (!pointSpriteActive)
            {
                ...
                {
                    ModeDefineMap::const_iterator mdm_itr = modeDefineMap.find(GL_TEXTURE_GEN_S);
                    if (mdm_itr!=modeDefineMap.end())
                    {
                        shaderDefineStr += mdm_itr->second;
                        texGenActive = true;
            ...
            // Generate default TEXTURE_ENV_FUNCTION if no user override via setDefine().
            // This is separate from _textureModeDefineMapList so users can override
            // the texture environment function (e.g. texenv_ADD for highlight maps).
            if (pointSpriteActive || texGenActive || !modeMap.empty())
            {
                std::string envDefName = "TEXTURE_ENV_FUNCTION";
                envDefName += char('0' + char(i));
                if (_defineMap.currentDefines.find(envDefName) == _defineMap.currentDefines.end())
                {
                    shaderDefineStr += "#define ";
                    shaderDefineStr += envDefName;
                    shaderDefineStr += " texenv_MODULATE";
                    shaderDefineStr += s_LineEnding;
                }
            }
        }
