 ...
 * Utility functions for osg: gluBeginCurve, gluEndCurve, gluEndSurface, gluBeginTrim,
 * gluEndTrim.
 */
...
namespace osg
{
    ...
    const unsigned char*
    __gluNURBSErrorString( int errnum )
    {
    ...
    const unsigned char*
    __gluTessErrorString( int errnum )
    {
    ...
    gluErrorString( GLenum errorCode )
    {
        ...
        {
            return ( const GLubyte* )__gluTessErrorString(
                static_cast<int>( errorCode - ( GLU_TESS_ERROR1 - 1 ) )
            );
        }
