...
OSG_INIT_SINGLETON_PROXY(GLExtensionDisableStringInitializationProxy, osg::getGLExtensionDisableString())
    ...
    {
        // GLEW handles all GL function pointer resolution.
        // Use platform-specific fallback only for non-standard extensions.
    #if defined(_WIN32)
        return convertPointerType<void*, PROC>(wglGetProcAddress(funcName));
    #elif defined(__linux__) || defined(__FreeBSD__) || defined(__sun) || defined(__sgi) || defined(__QNX__)
...
{
    // Ensure GLEW is initialized (should have been done by GraphicsWindow::makeCurrent)
    if (glGetString == NULL)
    {
        OSG_WARN << "GLExtensions: GL context not available, skipping initialization" << std::endl;
        return;
    }
    ...
    setGLExtensionFuncPtr(glStencilFuncSeparate, "glStencilFuncSeparate", validContext);
    ...
    isInvalidateFramebufferSupported = validContext && (isGLExtensionSupported(contextID, "GL_ARB_invalidate_subdata") || glVersion >= 4.3);
