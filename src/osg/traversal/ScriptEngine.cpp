...
ScriptEngine*
ScriptNodeCallback::getScriptEngine( osg::NodePath& nodePath )
{
    if( !_script )
    {
        return 0;
         ...
         ++itr )
    {
        ...
        if( udc )
        {
            ScriptEngine* engine = dynamic_cast<ScriptEngine*>(
                udc->getUserObject( _script->getLanguage() )
            );
            if( engine )
            {
                return engine;
...
void
ScriptNodeCallback::operator()( Node*        node,
                                NodeVisitor* nv )
{
    ScriptEngine* engine = getScriptEngine( nv->getNodePath() );
    if( engine && _script.valid() )
    {
        // To handle the case where a NodeVisitor is created on the stack and can't be
        // automatically ref counted we take a reference to prevent the inputParameters
        // reference to the NodeVisitor making it's ref count going to zero and causing a
        // delete.
