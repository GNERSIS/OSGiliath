    ...
    else
    {
        ...
        case(USE_ARB_COMPRESSION):
            if (extensions->isTextureCompressionARBSupported)
            {
                ...
                {
                    case(1): internalFormat = GL_COMPRESSED_RED_RGTC1_EXT; break;
                    case(2): internalFormat = GL_COMPRESSED_RED_GREEN_RGTC2_EXT; break;
                    case(3): internalFormat = GL_COMPRESSED_RGB_ARB; break;
                    case(4): internalFormat = GL_COMPRESSED_RGBA_ARB; break;
                    case(GL_RED): internalFormat = GL_COMPRESSED_RED_RGTC1_EXT; break;
                    case(GL_RG): internalFormat = GL_COMPRESSED_RED_GREEN_RGTC2_EXT; break;
...
void Texture::applyTexParameters(GLenum target, State& state) const
{
    ...
    if (texId == 0)
    {
        OSG_WARN << "Texture::applyTexParameters: no valid texture object for DSA" << std::endl;
    ...
    if( image &&
        ...
        int( image->getNumMipmapLevels() ) <
            ...
            glTextureParameteri( texId, GL_TEXTURE_MAX_LEVEL, image->getNumMipmapLevels() - 1 );


    glTextureParameteri( texId, GL_TEXTURE_WRAP_S, ws );

    if (target!=GL_TEXTURE_1D) glTextureParameteri( texId, GL_TEXTURE_WRAP_T, wt );

    if (target==GL_TEXTURE_3D) glTextureParameteri( texId, GL_TEXTURE_WRAP_R, wr );
    ...
    if (extensions->isTextureSwizzleSupported)
    {
        ...
        // This replicates the old GL_LUMINANCE/GL_LUMINANCE_ALPHA behavior at zero runtime cost.
        bool isDefaultSwizzle = (_swizzle[0] == GL_RED && _swizzle[1] == GL_GREEN &&
                                 _swizzle[2] == GL_BLUE && _swizzle[3] == GL_ALPHA);
        ...
        if (isDefaultSwizzle)
        {
            switch(_internalFormat)
            {
                ...
                case GL_COMPRESSED_SIGNED_RED_RGTC1_EXT:
                {
                    ...
                    glTextureParameteriv(texId, GL_TEXTURE_SWIZZLE_RGBA, swizzle);
                    break;
                }
                ...
                default:
                    glTextureParameteriv(texId, GL_TEXTURE_SWIZZLE_RGBA, _swizzle.ptr());
                    break;
            }
        }
        else
        ...
        _internalFormatType != SIGNED_INTEGER && _internalFormatType != UNSIGNED_INTEGER)
    {
        ...
        {
            glTextureParameteri(texId, GL_TEXTURE_COMPARE_MODE_ARB, GL_COMPARE_R_TO_TEXTURE_ARB);
            glTextureParameteri(texId, GL_TEXTURE_COMPARE_FUNC_ARB, _shadow_compare_func);
            ...
            {
                glTextureParameterf(texId, TEXTURE_COMPARE_FAIL_VALUE_ARB, _shadow_ambient);
            }
        }
        else
        {
