 ...
 * Command-line argument parser. Extracts typed options, flags,
 * and positional parameters from argc/argv.
 */
...
bool
ArgumentParser::isOption( const char* str )
{
    return str && str[0] == '-';
}

bool
ArgumentParser::isString( const char* str )
{
    if( !str )
    {
        return false;
...
bool
ArgumentParser::isBool( const char* str )
{
    if( !str )
    {
        return false;
...
bool
ArgumentParser::isNumber( const char* str )
{
    if( !str )
    {
        return false;
...
bool
ArgumentParser::Parameter::valid( const char* str ) const
{
    switch( _type )
    {
...
bool
ArgumentParser::Parameter::assign( const char* str )
{
    if( valid( str ) )
    {
        switch( _type )
        {
            ...
            case Parameter::INT_PARAMETER :
                *_value._int = atoi( str );
                break;
            case Parameter::UNSIGNED_INT_PARAMETER :
                *_value._uint = static_cast<unsigned int>( atoi( str ) );
                break;
            case Parameter::STRING_PARAMETER :
                *_value._string = str;
                break;
...
std::string
ArgumentParser::getApplicationName() const
{
    if( _argc && *_argc > 0 )
    {
...
bool
ArgumentParser::isOption( int pos ) const
{
    return pos < *_argc && isOption( _argv[pos] );
}

bool
ArgumentParser::isString( int pos ) const
{
...
int
ArgumentParser::find( const std::string& str ) const
{
...
bool
ArgumentParser::containsOptions() const
{
    for( int pos = 1; pos < *_argc; ++pos )
    {
        if( isOption( pos ) )
...
bool
ArgumentParser::read( const std::string& str )
{
    int pos = find( str );
    if( pos <= 0 )
    {
 ...
 * Parameters are also matched then set the Parameter values and remove the from the list
 * of arguments.*/
bool
ArgumentParser::read( int                pos,
                      ...
                      Parameter          value2 )
{
    if( match( pos, str ) )
    {
        if( ( pos + 2 ) < *_argc )
        {
            if( value1.valid( _argv[pos + 1] ) && value2.valid( _argv[pos + 2] ) )
            {
                value1.assign( _argv[pos + 1] );
...
bool
ArgumentParser::errors( ErrorSeverity severity ) const
{
...
void
ArgumentParser::reportError( const std::string& message,
...
void
ArgumentParser::reportRemainingOptionsAsUnrecognized( ErrorSeverity severity )
{
    std::set<std::string> options;
    if( _usage.valid() )
    {
        // parse the usage options to get all the option that the application can
        // potential handle.
        for( ApplicationUsage::UsageMap::const_iterator itr =
                 _usage->getCommandLineOptions().begin();
    ...
    for( int pos = 1; pos < argc(); ++pos )
    {
        ...
        {
            reportError( std::string( "unrecognized option " ) +
                             std::string( _argv[pos] ),
                         severity );
        }
    }
}
...
ApplicationUsage::Type
ArgumentParser::readHelpType()
{
    getApplicationUsage()->addCommandLineOption( "-h or --help",
                                                 "Display command line parameters" );
    getApplicationUsage()->addCommandLineOption(
        "--help-env",
