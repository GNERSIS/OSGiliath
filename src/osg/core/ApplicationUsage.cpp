...
void
ApplicationUsage::addUsageExplanation( Type               type,
...
void
ApplicationUsage::addCommandLineOption( const std::string& option,
                                        const std::string& explanation,
...
void
ApplicationUsage::addEnvironmentalVariable( const std::string& option,
                                            const std::string& explanation,
                                            const std::string& defaultValue )
{
...
void
ApplicationUsage::addKeyboardMouseBinding( const std::string& prefix,
                                           int                key,
                                           const std::string& explanation )
{
    if( key != 0 )
...
void
ApplicationUsage::addKeyboardMouseBinding( int                key,
                                           const std::string& explanation )
{
...
void
ApplicationUsage::addKeyboardMouseBinding( const std::string& option,
                                           const std::string& explanation )
{
...
void
ApplicationUsage::getFormattedString( std::string&    str,
                                      const UsageMap& um,
                                      ...
                                      const UsageMap& ud )
{
    ...
    {
        maxNumCharsInOptions =
            maximum( maxNumCharsInOptions, ( unsigned int )citr->first.length() );
    }
    ...
    double       ratioOfExplanationToOutputWidth =
        float( explanationPos ) / float( widthOfOutput );
    double maxRatioOfExplanationToOutputWidth = 0.25F;

    if( ratioOfExplanationToOutputWidth > maxRatioOfExplanationToOutputWidth )
    {
        explanationPos = static_cast<unsigned int>( maxRatioOfExplanationToOutputWidth *
                                                    float( widthOfOutput ) );
    }
    ...
    for( citr = um.begin(); citr != um.end(); ++citr )
    {
        ...
        unsigned int currentEndPos =
            optionPos + static_cast<unsigned int>( citr->first.length() );
        ...
        if( !explanation.empty() )
        {
            ...
            while( pos < explanation.length() )
            {
                ...
                std::string::size_type width =
                    minimum( ( std::string::size_type )( explanation.length() - pos ),
                             ( std::string::size_type )( explanationWidth - offset ) );
                ...
                line.replace( explanationPos + offset,
                              explanationWidth,
                              explanation,
                              pos,
                              width );
...
void
ApplicationUsage::write( std::ostream&                     output,
...
void
ApplicationUsage::writeEnvironmentSettings( std::ostream& output )
{
         ...
         ++citr )
    {
        ...
        maxNumCharsInOptions =
            maximum( maxNumCharsInOptions, static_cast<unsigned int>( len ) );
    }
