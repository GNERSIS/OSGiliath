 ...
 * Logging and notification system. Provides severity-level filtered
 * output (ALWAYS, FATAL, WARN, NOTICE, INFO, DEBUG) to configurable
...
namespace osg
{
    ...
    class NullStreamBuffer : public std::streambuf
    {
        private:
            ...
            std::streamsize
            xsputn( const std::streambuf::char_type* /*str*/,
                    std::streamsize n )
            {
                return n;
            }
    ...
    /** Stream buffer calling notify handler when buffer is synchronized (usually on
     * std::endl). Stream stores last notification severity to pass it to handler call.
     */
    struct NotifyStreamBuffer : public std::stringbuf
    {
                ...
                _severity( osg::NOTICE )
            {
                 ...
                 * osg::Notify (causing multitreading issues) by pre-allocating 4095
                 * bytes */
                str( std::string( 4'095, 0 ) );
                pubseekpos( 0, std::ios_base::out );
            }

            void
            setNotifyHandler( osg::NotifyHandler* handler )
            {
                _handler = handler;
            }

            osg::NotifyHandler*
            getNotifyHandler() const
            {
                return _handler.get();
            }
            ...
            void
            setCurrentSeverity( osg::NotifySeverity severity )
            {
...
static osg::ApplicationUsageProxy
    Notify_e0( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
...
struct NotifySingleton
{
        NotifySingleton()
        {
            ...
            if( getEnvVar( "OSG_NOTIFY_LEVEL", OSGNOTIFYLEVEL ) ||
                getEnvVar( "OSGNOTIFYLEVEL", OSGNOTIFYLEVEL ) )
            {
                ...
                {
                    std::cout << "Warning: invalid OSG_NOTIFY_LEVEL set ("
                              << stringOSGNOTIFYLEVEL << ")" << std::endl;
                }
            }

            // Setup standard notify handler
            osg::NotifyStreamBuffer* buffer =
                dynamic_cast<osg::NotifyStreamBuffer*>( _notifyStream.rdbuf() );
...
// Use a proxy to force the initialization of the NotifySingleton during static
// initialization
OSG_INIT_SINGLETON_PROXY( NotifySingletonProxy,
                          osg::initNotifyLevel() )
...
void
osg::setNotifyHandler( osg::NotifyHandler* handler )
{
    osg::NotifyStreamBuffer* buffer = static_cast<osg::NotifyStreamBuffer*>(
        getNotifySingleton()._notifyStream.rdbuf()
    );
    if( buffer )
    {
...
bool
osg::isNotifyEnabled( osg::NotifySeverity severity )
{
...
std::ostream&
osg::notify( const osg::NotifySeverity severity )
{
...
void
osg::StandardNotifyHandler::notify( osg::NotifySeverity severity,
                                    const char*         message )
{
...
void
osg::WinDebugNotifyHandler::notify( osg::NotifySeverity severity,
                                    const char*         message )
{
    OSG_UNUSED( severity );
