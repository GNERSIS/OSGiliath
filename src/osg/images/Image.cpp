...
void
Image::UpdateCallback::operator()( osg::StateAttribute* attr,
                                   osg::NodeVisitor*    nv )
{
...
Image::DataIterator::assign()
{
    ...
    if( _image->isMipmap() )
    {
        ...
        unsigned int rowWidthInBytes =
            Image::computeRowWidthInBytes( rowLength,
                                           _image->getPixelFormat(),
                                           _image->getDataType(),
                                           static_cast<int>( _image->getPacking() ) );
        unsigned int imageSizeInBytes =
            rowWidthInBytes * static_cast<unsigned int>( imageHeight );

        _currentPtr  = ptr +
                       rowWidthInBytes *
                       static_cast<unsigned int>( _rowNum ) +
                       imageSizeInBytes *
                       static_cast<unsigned int>( _imageNum );
...
{
    setDataVariance( DataVariance::STATIC );
}
    ...
    _dimensionsChangedCallbacks( image._dimensionsChangedCallbacks )
{
    if( image._data )
    {
        ...
        {
            OSG_WARN << "Warning: Image::Image(const Image&, const CopyOp&) out of "
                        "memory, no image copy made."
                     << std::endl;
        }
    }
}
...
int
Image::compare( const Image& rhs ) const
{
    ...
    COMPARE_StateAttribute_Parameter( _s ) COMPARE_StateAttribute_Parameter( _t )
    ...
    COMPARE_StateAttribute_Parameter( getFileName() ) return 0;
}
...
bool
Image::isPackedType( GLenum type )
{
    switch( type )
    {
...
GLenum
Image::computePixelFormat( GLenum format )
{
    switch( format )
    {
        case( GL_R16F ) :
...
GLenum
Image::computeFormatDataType( GLenum pixelFormat )
{
    switch( pixelFormat )
    {
        case GL_R32F :
        ...
        default :
            {
                OSG_WARN << "error computeFormatType = " << std::hex << pixelFormat
                         << std::dec << std::endl;
                return 0;
            }
    }
...
unsigned int
Image::computeNumComponents( GLenum pixelFormat )
{
    switch( pixelFormat )
    {
        ...
        default :
            {
                OSG_WARN << "error pixelFormat = " << std::hex << pixelFormat << std::dec
                         << std::endl;
                return 0;
            }
    }
...
unsigned int
Image::computePixelSizeInBits( GLenum format,
                               GLenum type )
{
    ...
    // compressed formats as internal texture modes.  This is very much speculation
    // though if get the below error then its time to revist this issue :-) Robert
    // Osfield, Jan 2005.
    switch( format )
    {
        case( GL_COMPRESSED_RGB ) :
        case( GL_COMPRESSED_RGBA ) :
            OSG_WARN << "Image::computePixelSizeInBits(format,type) : cannot compute "
                        "correct size of compressed format ("
                     << format << ") returning 0." << std::endl;
            return 0;
        default :
            break;
    }
    ...
    {    // handle GL_KHR_texture_compression_astc_hdr
        ...
        case( GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x12_KHR ) :
            {
                osg::ivec3   footprint = computeBlockFootprint( format );
                unsigned int pixelsPerBlock =
                    static_cast<unsigned int>( footprint.x * footprint.y );
                unsigned int bitsPerBlock =
                    computeBlockSize( format, 0 );    // 16 x 8 = 128
                unsigned int bitsPerPixel = bitsPerBlock / pixelsPerBlock;
                if( bitsPerBlock == bitsPerPixel * pixelsPerBlock )
                {
                    OSG_WARN
                        << "Image::computePixelSizeInBits(format,type) : bits per pixel "
                           "("
                        << bitsPerPixel
                        << ") is not an integer for GL_KHR_texture_compression_astc_hdr "
                           "sizes other than 4x4 and 8x8."
                        << std::endl;
                    return bitsPerPixel;
                }
                else
                {
                    OSG_WARN << "Image::computePixelSizeInBits(format,type) : bits per "
                                "pixel ("
                             << bitsPerBlock << "/" << pixelsPerBlock
                             << ") is not an integer for "
                                "GL_KHR_texture_compression_astc_hdr size"
                             << footprint.x << "x" << footprint.y << "." << std::endl;
                }
                return 0;
            }
        default :
...
osg::ivec3
Image::computeBlockFootprint( GLenum pixelFormat )
{
    switch( pixelFormat )
    {
        case( GL_COMPRESSED_RGB_S3TC_DXT1_EXT ) :
        ...
        case( GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ) :
            return osg::ivec3( 4,
                               ...
                               4 );    // opengl 3d dxt: r value means (max)4 consecutive
                                       // blocks in r direction packed into a slab.
        ...
        case( GL_COMPRESSED_RG11_EAC ) :
        case( GL_COMPRESSED_SIGNED_RG11_EAC ) :
            return osg::ivec3( 4, 4, 1 );    // not sure about r
        case( GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG ) :
        case( GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG ) :
            return osg::ivec3( 8, 4, 1 );    // no 3d texture support in pvrtc at all
        case( GL_COMPRESSED_RGBA_ASTC_4x4_KHR ) :
            return osg::ivec3( 4, 4, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_5x4_KHR ) :
            return osg::ivec3( 5, 4, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_5x5_KHR ) :
            return osg::ivec3( 5, 5, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_6x5_KHR ) :
            return osg::ivec3( 6, 5, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_6x6_KHR ) :
            return osg::ivec3( 6, 6, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_8x5_KHR ) :
            return osg::ivec3( 8, 5, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_8x6_KHR ) :
            return osg::ivec3( 8, 6, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_8x8_KHR ) :
            return osg::ivec3( 8, 8, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_10x5_KHR ) :
            return osg::ivec3( 10, 5, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_10x6_KHR ) :
            return osg::ivec3( 10, 6, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_10x8_KHR ) :
            return osg::ivec3( 10, 8, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_10x10_KHR ) :
            return osg::ivec3( 10, 10, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_12x10_KHR ) :
            return osg::ivec3( 12, 10, 1 );
        case( GL_COMPRESSED_RGBA_ASTC_12x12_KHR ) :
            return osg::ivec3( 12, 12, 1 );
        case( GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR ) :
            return osg::ivec3( 4, 4, 1 );
        case( GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x4_KHR ) :
            return osg::ivec3( 5, 4, 1 );
    ...
    return osg::ivec3( 1, 1, 1 );
}

// returns the max(size of a 2D block in bytes,packing)
unsigned int
Image::computeBlockSize( GLenum pixelFormat,
                         GLenum packing )
{
    switch( pixelFormat )
    {
        ...
        case( GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ) :
        case( GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT ) :
            return std::max( 8U, packing );    // block size of 8
        case( GL_COMPRESSED_RGBA_S3TC_DXT3_EXT ) :
        case( GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT ) :
        case( GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ) :
        case( GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT ) :
        ...
        case( GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG ) :
        case( GL_ETC1_RGB8_OES ) :
            return std::max( 16U, packing );    // block size of 16
        case( GL_COMPRESSED_SIGNED_RED_RGTC1_EXT ) :
        case( GL_COMPRESSED_RED_RGTC1_EXT ) :
            return std::max( 8U, packing );     // block size of 8
            break;
...
unsigned int
Image::computeRowWidthInBytes( int    width,
                               GLenum pixelFormat,
                               GLenum type,
                               int    packing )
{
    unsigned int pixelSize = computePixelSizeInBits( pixelFormat, type );
    int widthInBits = static_cast<int>( static_cast<unsigned int>( width ) * pixelSize );
    int packingInBits = packing != 0 ? packing * 8 : 8;
    // OSG_INFO << "width="<<width<<" pixelSize="<<pixelSize<<"  width in
    // bit="<<widthInBits<<" packingInBits="<<packingInBits<<"
    // widthInBits%packingInBits="<<widthInBits%packingInBits<<std::endl;
    return static_cast<unsigned int>(
        ( widthInBits / packingInBits + ( ( widthInBits % packingInBits ) ? 1 : 0 ) ) *
        packing
    );
}

unsigned int
Image::computeImageSizeInBytes( int    width,
                                int    height,
                                int    depth,
                                GLenum pixelFormat,
                                GLenum type,
                                ...
                                int    image_packing )
{
    ...
    int blockSize = static_cast<int>( computeBlockSize( pixelFormat, 0 ) );
    if( blockSize > 0 )
    {
        osg::ivec3 footprint = computeBlockFootprint( pixelFormat );
        width                = ( width + footprint.x - 1 ) / footprint.x;
        height               = ( height + footprint.y - 1 ) / footprint.y;
        unsigned int size    = static_cast<unsigned int>( blockSize * width );
        size  = static_cast<unsigned int>( roudUpToMultiple( static_cast<int>( size ),
                                                             packing ) );
        size *= static_cast<unsigned int>( height );
        size  = static_cast<unsigned int>( roudUpToMultiple( static_cast<int>( size ),
                                                             slice_packing ) );
        size *= static_cast<unsigned int>( depth );
        size  = static_cast<unsigned int>( roudUpToMultiple( static_cast<int>( size ),
                                                             image_packing ) );
        return size;
    }
    ...
    unsigned int size =
        osg::Image::computeRowWidthInBytes( width, pixelFormat, type, packing );

    // now compute size of slice
    size *= static_cast<unsigned int>( height );
    size += static_cast<unsigned int>( slice_packing ) - 1;
    size -= size % static_cast<unsigned int>( slice_packing );

    // compute size of whole image
    size *= static_cast<unsigned int>( depth );
    size += static_cast<unsigned int>( image_packing ) - 1;
    size -= size % static_cast<unsigned int>( image_packing );

    return std::max( size,
                     computeBlockSize( pixelFormat,
                                       static_cast<unsigned int>( packing ) ) );
}

int
Image::roudUpToMultiple( int s,
                         int pack )
{
    if( pack < 2 )
    {
...
int
Image::computeNearestPowerOfTwo( int   s,
                                 float bias )
{
    if( ( s & ( s - 1 ) ) != 0 )
    {
...
int
Image::computeNumberOfMipmapLevels( int s,
                                    int t,
                                    int r )
{
    int w = maximum( s, t );
...
unsigned int
Image::getTotalSizeInBytesIncludingMipmaps() const
{
    ...
    {
        totalSize  += computeImageSizeInBytes( s,
                                               t,
                                               r,
                                               _pixelFormat,
                                               _dataType,
...
void
Image::setRowLength( int length )
{
    _rowLength = length;
}

void
Image::setInternalTextureFormat( GLint internalFormat )
{
...
void
Image::setPixelFormat( GLenum pixelFormat )
{
    _pixelFormat = pixelFormat;
}

void
Image::setDataType( GLenum dataType )
{
    ...
    {
        OSG_WARN << "Image::setDataType(..) - warning, attempt to reset the data type "
                    "not permitted."
                 << std::endl;
    }
}
                      ...
                      int    packing )
{
    ...
    if( _data )
    {
        previousTotalSize = computeRowWidthInBytes( _s,
                                                    _pixelFormat,
                                                    _dataType,
                                                    static_cast<int>( _packing ) ) *
                            static_cast<unsigned int>( _t ) *
                            static_cast<unsigned int>( _r );
    }

    unsigned int newTotalSize = computeRowWidthInBytes( s, format, type, packing ) *
    ...
    if( _data )
    {
        ...
        _pixelFormat    = format;
        _dataType       = type;
        _packing        = static_cast<unsigned int>( packing );
        _rowLength      = 0;
        ...
        {
            _internalTextureFormat = static_cast<GLint>( format );
        }
    }
    else
    {
        ...
        // policy so that allocateImage honours previous settings of
                   ...
                   int    packing )
{
    ...
    glPixelStorei( GL_PACK_ALIGNMENT, static_cast<GLint>( _packing ) );
...
void
Image::readImageFromCurrentTexture( unsigned int contextID,
                                    ...
                                    unsigned int face )
{
    ...
    GLboolean binding1D = GL_FALSE, binding2D = GL_FALSE, bindingRect = GL_FALSE,
              binding3D = GL_FALSE, binding2DArray = GL_FALSE, bindingCubeMap = GL_FALSE;
    ...
    GLenum textureMode = binding1D      ? GL_TEXTURE_1D
                       ...
                       : binding2DArray ? GL_TEXTURE_2D_ARRAY
                                        : 0;
    if( bindingCubeMap )
    {
        switch( face )
    ...
    if( copyMipMapsIfAvailable )
    {
        for( ; numMipMaps < 20; ++numMipMaps )
        {
            ...
            glGetTexLevelParameteriv( textureMode,
                                      ...
                                      GL_TEXTURE_DEPTH,
                                      &depth );
            // OSG_NOTICE<<"   numMipMaps="<<numMipMaps<<" width="<<width<<"
    ...
    // OSG_NOTICE<<"Image::readImageFromCurrentTexture() : numMipMaps =
    // "<<numMipMaps<<std::endl;
    ...
    if( compressed == GL_TRUE )
    {
        ...
        for( i = 0; i < numMipMaps; ++i )
        {
            ...
            total_size += static_cast<unsigned int>( compressed_size );
        }

        unsigned char* data = new unsigned char[total_size];
        if( !data )
        {
            OSG_WARN << "Warning: Image::readImageFromCurrentTexture(..) out of memory, "
                        "no image read."
                     << std::endl;
            return;
        }
        ...
        _pixelFormat           = static_cast<GLenum>( internalformat );
        _dataType              = type;
        _internalTextureFormat = internalformat;
        ...
        for( i = 0; i < numMipMaps; ++i )
        {
            extensions->glGetCompressedTexImage(
                textureMode,
                i,
                getMipmapData( static_cast<unsigned int>( i ) )
            );
        }
    ...
    else
    {
        ...
        _pixelFormat = computePixelFormat( static_cast<GLenum>( internalformat ) );
        _dataType    = type;
        _internalTextureFormat = internalformat;
        _mipmapData            = mipMapData;
        _allocationMode        = USE_NEW_DELETE;
                   ...
                   GLenum newDataType )
{
    ...
    if( _data == NULL )
    {
        OSG_WARN << "Error Image::scaleImage() do not succeed : cannot scale NULL image."
                 << std::endl;
        return;
    }
    ...
    if( _r != 1 || r != 1 )
    {
        OSG_WARN << "Error Image::scaleImage() do not succeed : scaling of volumes not "
                    "implemented."
                 << std::endl;
        return;
    }

    unsigned int newTotalSize = computeRowWidthInBytes( s,
                                                        _pixelFormat,
                                                        newDataType,
                                                        static_cast<int>( _packing ) ) *
                                static_cast<unsigned int>( t );
    ...
    if( !newData )
    {
        ...
        OSG_FATAL << "Error Image::scaleImage() do not succeed : out of memory."
                  << newTotalSize << std::endl;
        return;
    }

    PixelStorageModes psm;
    psm.pack_alignment   = static_cast<GLint>( _packing );
    psm.pack_row_length  = static_cast<GLint>( _rowLength );
    psm.unpack_alignment = static_cast<GLint>( _packing );

    GLint status         = gluScaleImage( &psm,
                                          _pixelFormat,
                                          _s,
                                          _t,
                                          _dataType,
    ...
    else
    {
        ...
        OSG_WARN << "Error Image::scaleImage() did not succeed : errorString = "
                 << gluErrorString( ( GLenum )status )
...
void
Image::copySubImage( int               s_offset,
                     int               t_offset,
                     int               r_offset,
                     const osg::Image* source )
{
    ...
    {
        OSG_WARN << "Warning: negative offsets passed to Image::copySubImage(..) not "
                    "supported, operation ignored."
                 << std::endl;
        return;
    }
    ...
    if( !_data )
    {
        ...
        allocateImage( s_offset + source->s(),
                       t_offset + source->t(),
                       r_offset + source->r(),
                       source->getPixelFormat(),
                       source->getDataType(),
                       static_cast<int>( source->getPacking() ) );
    }

    if( s_offset >= _s || t_offset >= _t || r_offset >= _r )
    {
        OSG_WARN << "Warning: offsets passed to Image::copySubImage(..) outside "
                    "destination image, operation ignored."
                 << std::endl;
        return;
    }
    ...
    {
        OSG_WARN << "Warning: image with an incompatible pixel formats passed to "
                    "Image::copySubImage(..), operation ignored."
                 << std::endl;
        return;
    }

    unsigned char* data_destination = data( static_cast<unsigned int>( s_offset ),
                                            static_cast<unsigned int>( t_offset ),
                                            static_cast<unsigned int>( r_offset ) );
    if( isCompressed() )
    {
        osg::ivec3 footprint = computeBlockFootprint( _pixelFormat );
        if( footprint.x == 4 && footprint.y == 4 )
        {
            if( ( source->s() & 0X3 ) ||
                ...
                ( t_offset & 0X3 ) )
            {
                OSG_WARN << "Error Image::copySubImage() did not succeed : size "
                         << source->s() << "x" << source->t() << " or offset "
                         << s_offset << "," << t_offset << " not multiple of 4."
                         << std::endl;
                return;
            }
        }
        ...
        {
            if( ( source->s() % footprint.x ) ||
                ( source->t() % footprint.y ) ||
                ( s_offset % footprint.x ) ||
                ( t_offset % footprint.y ) )
            {
                OSG_WARN << "Error Image::copySubImage() did not succeed : size "
                         << source->s() << "x" << source->t() << " or offset "
                         << s_offset << "," << t_offset << " not multiple of footprint "
                         << footprint.x << "x" << footprint.y << std::endl;
                return;
            }
        }
        unsigned int rowWidthInBlocks =
            static_cast<unsigned int>( ( _s + footprint.x - 1 ) / footprint.x );
        unsigned int blockSize = computeBlockSize( _pixelFormat, 0 );
        data_destination       = _data +
                                 blockSize *
                                 ( rowWidthInBlocks *
                                   static_cast<unsigned int>( t_offset / footprint.y ) +
                                   static_cast<unsigned int>( s_offset / footprint.x ) );
        unsigned int copy_width = static_cast<unsigned int>(
            ( std::min( source->s(), _s - s_offset ) + footprint.x - 1 ) / footprint.x
        );
        unsigned int copy_height = static_cast<unsigned int>(
            ( std::min( source->t(), _t - t_offset ) + footprint.y - 1 ) / footprint.y
        );
        unsigned int dstRowStep = blockSize * rowWidthInBlocks;
        unsigned int srcRowStep =
            blockSize *
            static_cast<unsigned int>( ( source->_s + footprint.x - 1 ) / footprint.x );
        const unsigned char* data_source = source->data( 0, 0, 0 );
        for( unsigned int row = 0; row < copy_height; row += 1 )
        {    // copy blocks in a row, footprint.y rows at a time
    ...
    psm.unpack_alignment  = static_cast<GLint>( source->getPacking() );
    psm.unpack_row_length = static_cast<GLint>( source->getRowLength() );
...
Image::flipHorizontal()
{
    ...
    {
        OSG_WARN
            << "Error Image::flipHorizontal() did not succeed : cannot flip NULL image."
            << std::endl;
        return;
    }
    ...
    if( _mipmapData.empty() )
    {
        ...
        for( int r = 0; r < _r; ++r )
        {
            for( int t = 0; t < _t; ++t )
            {
                ...
                unsigned char* right =
                    rowData +
                    ( static_cast<unsigned int>( _s - 1 ) * getPixelSizeInBits() ) /
                    8;
    ...
    {
        OSG_WARN << "Error Image::flipHorizontal() did not succeed : cannot flip "
                    "mipmapped image."
                 << std::endl;
        return;
    }
...
void
flipImageVertical( unsigned char* top,
...
Image::flipVertical()
{
    ...
    {
        OSG_WARN
            << "Error Image::flipVertical() do not succeed : cannot flip NULL image."
            << std::endl;
        return;
    }
    ...
    {
        OSG_WARN << "Error Image::flipVertical() do not succeed : flipping of mipmap 3d "
    ...
    if( _mipmapData.empty() )
    {
        ...
        for( int r = 0; r < _r; ++r )
        {
            if( dxtc )
            {
                if( !dxtc_tool::VerticalFlip(
                        static_cast<std::size_t>( _s ),
                        static_cast<std::size_t>( _t ),
                        _pixelFormat,
                        data( 0, 0, static_cast<unsigned int>( r ) )
                    ) )
                {
                    OSG_NOTICE
                        << "Notice Image::flipVertical(): Vertical flip do not succeed"
                        << std::endl;
                }
            }
            else
            {
                if( isCompressed() )
                {
                    OSG_NOTICE << "Notice Image::flipVertical(): image is compressed "
                                  "but normal v-flip is used"
                               << std::endl;
                }
                // its not a compressed image, so implement flip oursleves.
                unsigned char* top = data( 0, 0, static_cast<unsigned int>( r ) );
                unsigned char* bottom =
                    top + static_cast<unsigned int>( _t - 1 ) * rowStep;
    ...
    else if( _r == 1 )
    {
        if( dxtc )
        {
            if( !dxtc_tool::VerticalFlip( static_cast<std::size_t>( _s ),
                                          static_cast<std::size_t>( _t ),
                                          _pixelFormat,
                                          _data ) )
            {
        ...
        else
        {
            if( isCompressed() )
            {
                OSG_NOTICE << "Notice Image::flipVertical(): image is compressed but "
                              "normal v-flip is used"
                           << std::endl;
            }
            // its not a compressed image, so implement flip oursleves.
            unsigned char* top    = data( 0, 0, 0 );
            unsigned char* bottom = top + static_cast<unsigned int>( _t - 1 ) * rowStep;
        ...
        for( unsigned int i = 0; i < _mipmapData.size() && _mipmapData[i]; ++i )
        {
            ...
            if( dxtc )
            {
                if( !dxtc_tool::VerticalFlip( static_cast<std::size_t>( s ),
                                              static_cast<std::size_t>( t ),
                                              _pixelFormat,
                                              _data + _mipmapData[i] ) )
                {
                    OSG_NOTICE
                        << "Notice Image::flipVertical(): Vertical flip did not succeed"
                        << std::endl;
                }
            }
            else
            {
                ...
                unsigned int mipRowSize =
                    computeRowWidthInBytes( s,
                                            _pixelFormat,
                                            _dataType,
                                            static_cast<int>( _packing ) );
                ...
                unsigned char* bottom =
                    top + static_cast<unsigned int>( t - 1 ) * mipRowStep;
...
Image::flipDepth()
{
    ...
    for( ; r_front < r_back; ++r_front, --r_back )
    {
        ...
        {
            unsigned char* front = data( 0,
                                         static_cast<unsigned int>( row ),
                                         static_cast<unsigned int>( r_front ) );
            unsigned char* back  = data( 0,
                                         static_cast<unsigned int>( row ),
                                         static_cast<unsigned int>( r_back ) );
...
void
Image::ensureValidSizeForTexturing( GLint maxTextureSize )
{
    ...
    if( new_s != _s || new_t != _t )
    {
        if( !_fileName.empty() )
        {
            OSG_NOTICE << "Scaling image '" << _fileName << "' from (" << _s << "," << _t
                       << ") to (" << new_s << "," << new_t << ")" << std::endl;
        }
        else
        {
            OSG_NOTICE << "Scaling image from (" << _s << "," << _t << ") to (" << new_s
                       << "," << new_t << ")" << std::endl;
        }
...
bool
Image::supportsTextureSubloading() const
{
...
template<typename T>
bool
_findLowerAlphaValueInRow( unsigned int num,
                           T*           data,
                           T            value,
                           unsigned int delta )
{
...
template<typename T>
bool
_maskedFindLowerAlphaValueInRow( unsigned int num,
                                 T*           data,
                                 T            value,
                                 T            mask,
                                 unsigned int delta )
...
bool
Image::isImageTranslucent() const
{
    ...
    switch( _pixelFormat )
    {
        ...
        case( GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ) :
        case( GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT ) :
            return dxtc_tool::isCompressedImageTranslucent(
                static_cast<std::size_t>( _s ),
                static_cast<std::size_t>( _t ),
                _pixelFormat,
                _data
    ...
    for( int ir = 0; ir < r(); ++ir )
    {
        ...
        {
            const unsigned char* d = data( 0,
                                           static_cast<unsigned int>( it ),
                                           static_cast<unsigned int>( ir ) );
            switch( _dataType )
            {
                case( GL_BYTE ) :
                    if( _findLowerAlphaValueInRow( static_cast<unsigned int>( s() ),
                                                   ( char* )d + offset,
                                                   ( char )127,
                                                   delta ) )
                    {
                ...
                case( GL_FLOAT ) :
                    ...
                    break;
                case( GL_UNSIGNED_SHORT_5_5_5_1 ) :
                    if( _maskedFindLowerAlphaValueInRow(
                            static_cast<unsigned int>( s() ),
                            ( unsigned short* )d,
                            ( unsigned short )0X00'01,
                            ( unsigned short )0X00'01,
                            1
                        ) )
                    {
...
Geode*
osg::createGeodeForImage( osg::Image* image )
{
    return createGeodeForImage( image,
                          ...
                          float       t )
{
    if( image )
    {
        if( s > 0 && t > 0 )
        {
            ...
            float texcoord_y_b =
                ( image->getOrigin() == osg::Image::BOTTOM_LEFT ) ? 0.0F : 1.0F;
            float texcoord_y_t =
                ( image->getOrigin() == osg::Image::BOTTOM_LEFT ) ? 1.0F : 0.0F;
            ...
            // set up the geoset.                unsigned int rowSize =
            // computeRowWidthInBytes(s,_pixelFormat,_dataType,_packing);
            ...
            // Convert quad (0,1,2,3) to two triangles for Core Profile compatibility
            DrawElementsUShort* elements = new DrawElementsUShort( GL_TRIANGLES );
            ...
            float  scale )
{
    switch( pixelFormat )
    {
        case(
            GL_DEPTH_COMPONENT
        ) :    // intentionally fall through and execute the code for GL_RED
        case( GL_RED ) :
            {
                 ...
                 unsigned     r ) const
{
    if( isCompressed() )
    {
        if( dxtc_tool::isDXTC( _pixelFormat ) )
        {
            ...
            if( dxtc_tool::CompressedImageGetColor( color,
                                                    s,
                                                    t,
                                                    r,
                                                    _s,
    ...
    else
    {
        ...
        switch( _dataType )
        {
            ...
            case( GL_UNSIGNED_SHORT ) :
                return _readColor( _pixelFormat,
                                   ( unsigned short* )ptr,
                                   1.0F / 65535.0F );
            case( GL_INT ) :
...
vec4
Image::getColor( const vec3& texcoord ) const
{
    unsigned int s = static_cast<unsigned int>(
        osg::clampTo( int( texcoord.x * float( _s - 1 ) ), 0, _s - 1 )
    );
    unsigned int t = static_cast<unsigned int>(
        osg::clampTo( int( texcoord.y * float( _t - 1 ) ), 0, _t - 1 )
    );
    unsigned int r = static_cast<unsigned int>(
        osg::clampTo( int( texcoord.z * float( _r - 1 ) ), 0, _r - 1 )
    );
...
template<typename T>
void
_writeColor( GLenum      pixelFormat,
             T*          data,
             float       scale,
             const vec4& c )
{
...
void
Image::setColor( const vec4&  color,
                 unsigned int s,
                 unsigned int t /*=0*/,
                 unsigned int r /*=0*/ )
{
    ...
    switch( getDataType() )
    {
        ...
        case( GL_UNSIGNED_SHORT ) :
            return _writeColor( getPixelFormat(),
                                ( unsigned short* )ptr,
                                65535.0F,
                                color );
        case( GL_INT ) :
...
void
Image::addDimensionsChangedCallback( DimensionsChangedCallback* cb )
{
    _dimensionsChangedCallbacks.push_back( cb );
}

void
Image::removeDimensionsChangedCallback( DimensionsChangedCallback* cb )
{
    DimensionsChangedCallbackVector::iterator itr =
        std::find( _dimensionsChangedCallbacks.begin(),
                   _dimensionsChangedCallbacks.end(),
                   cb );
