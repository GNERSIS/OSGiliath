 ...
 * Utility functions for osg: Textures, colors, values.
 */
...
namespace dxtc_tool
{
        ...
        if( m_Height > 4 )
        {
            for( size_t i = 0; i < ( ( m_Height + 7 ) / 8 ); ++i )
            {
                for( size_t j = 0; j < ( m_Width + 3 ) / 4; ++j )
                {
                    ...
                    BVF_Color( GetBlock( i, j, BSIZE_DXT1 ),
                               GetBlock( TargetRow, j, BSIZE_DXT1 ) );
                }
            }
        }
    }
    ...
    dxtc_pixels::VFlip_DXT3() const
    {
        ...
        if( m_Height == 2 )
        {
            for( size_t j = 0; j < ( m_Width + 3 ) / 4; ++j )
            {
                ...
                BVF_Color_H2( ( ( dxtc_int8* )m_pPixels ) +
                              ( j * BSIZE_DXT3 + BSIZE_ALPHA_DXT3 ) );
            }
        }
        ...
        if( m_Height == 4 )
        {
            for( size_t j = 0; j < ( m_Width + 3 ) / 4; ++j )
            {
                ...
                BVF_Color_H4( ( ( dxtc_int8* )m_pPixels ) +
                              ( j * BSIZE_DXT3 + BSIZE_ALPHA_DXT3 ) );
            }
        }
        ...
        if( m_Height > 4 )
        {
            for( size_t i = 0; i < ( ( m_Height + 7 ) / 8 ); ++i )
            {
                ...
                {
                    const size_t TargetRow = ( ( m_Height + 3 ) / 4 ) - ( i + 1 );
                    BVF_Alpha_DXT3( GetBlock( i, j, BSIZE_DXT3 ),
                                    GetBlock( TargetRow, j, BSIZE_DXT3 ) );
                    BVF_Color( ( ( dxtc_int8* )GetBlock( i, j, BSIZE_DXT3 ) ) +
                                   BSIZE_ALPHA_DXT3,
                               ( ( dxtc_int8* )GetBlock( TargetRow, j, BSIZE_DXT3 ) ) +
                                   BSIZE_ALPHA_DXT3 );
                }
            }
        }
    ...
    dxtc_pixels::VFlip_DXT5() const
    {
        ...
        if( m_Height > 4 )
        {
            for( size_t i = 0; i < ( ( m_Height + 7 ) / 8 ); ++i )
            {
                ...
                {
                    const size_t TargetRow = ( ( m_Height + 3 ) / 4 ) - ( i + 1 );
                    BVF_Alpha_DXT5( GetBlock( i, j, BSIZE_DXT5 ),
                                    GetBlock( TargetRow, j, BSIZE_DXT5 ) );
                    BVF_Color( ( ( dxtc_int8* )GetBlock( i, j, BSIZE_DXT5 ) ) +
                                   BSIZE_ALPHA_DXT5,
                               ( ( dxtc_int8* )GetBlock( TargetRow, j, BSIZE_DXT5 ) ) +
                                   BSIZE_ALPHA_DXT5 );
                }
            }
        }
    ...
    // http://msdn.microsoft.com url at time of writing
    ...
    isCompressedImageTranslucent( size_t width,
                                  ...
                                  void*  imageData )
    {
        // OSG_NOTICE<<"isCompressedImageTranslucent("<<width<<", "<<height<<",
        // "<<format<<", "<<imageData<<")"<<std::endl;
        int blockCount =
            static_cast<int>( ( ( width + 3 ) >> 2 ) * ( ( height + 3 ) >> 2 ) );
        switch( format )
        {
            ...
            case( GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT ) :
                {
                    const DXT3TexelsBlock* texelsBlock =
                        reinterpret_cast<const DXT3TexelsBlock*>( imageData );
                    // Only do the check on the first mipmap level, and stop when we see
                    // the first alpha texel
                    int i = blockCount;
                    while( i > 0 )
                    {
                        for( int j = 0; j < 4; ++j )
                        {
                            if( texelsBlock->alpha4[j] != 0XFF'FF )    // 4 pixels at
                                                                       // once
                            {
            ...
            case( GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT ) :
                {
                    const DXT5TexelsBlock* texelsBlock =
                        reinterpret_cast<const DXT5TexelsBlock*>( imageData );
                    // Only do the check on the first mipmap level, and stop when we see
                    // the first alpha texel
                    int           i = blockCount;
                    ...
                    while( i > 0 )
                    {
                        ...
                        if( eightStep )
                        {
                            if( texelsBlock->alpha_0 < 255 )
                            {
                                return true;    // not fully opaque
                            }
                            alphaBlock[2] =
                                ( 6 * alphaBlock[0] + 1 * alphaBlock[1] + 3 ) /
                                7;    // bit code 010
                            alphaBlock[3] =
                                ( 5 * alphaBlock[0] + 2 * alphaBlock[1] + 3 ) /
                                7;    // bit code 011
                            alphaBlock[4] =
                                ( 4 * alphaBlock[0] + 3 * alphaBlock[1] + 3 ) /
                                7;    // bit code 100
                            alphaBlock[5] =
                                ( 3 * alphaBlock[0] + 4 * alphaBlock[1] + 3 ) /
                                7;    // bit code 101
                            alphaBlock[6] =
                                ( 2 * alphaBlock[0] + 5 * alphaBlock[1] + 3 ) /
                                7;    // bit code 110
                            alphaBlock[7] =
                                ( 1 * alphaBlock[0] + 6 * alphaBlock[1] + 3 ) /
                                7;    // bit code 111
                        }
                        else
                        {
                            alphaBlock[2] =
                                ( 4 * alphaBlock[0] + 1 * alphaBlock[1] + 2 ) /
                                5;                  // bit code 010
                            alphaBlock[3] =
                                ( 3 * alphaBlock[0] + 2 * alphaBlock[1] + 2 ) /
                                5;                  // bit code 011
                            alphaBlock[4] =
                                ( 2 * alphaBlock[0] + 3 * alphaBlock[1] + 2 ) /
                                5;                  // bit code 100
                            alphaBlock[5] =
                                ( 1 * alphaBlock[0] + 4 * alphaBlock[1] + 2 ) /
                                5;                  // bit code 101
                        ...
                        unsigned short running_a_index = static_cast<unsigned short>(
                            texelsBlock->alpha3[0] +
                            ( ( ( unsigned short )texelsBlock->alpha3[last_added_byte] )
                              << 8 )
                        );
                        for( int j = 0; j < 16; ++j )
                        {
                            unsigned char alphaIndex = running_a_index & 0X7;
                            if( alphaBlock[alphaIndex] < 255 )
                            {
                                return true;    // not fully opaque
                            }
                            running_a_index >>= 3;
                            ...
                            {
                                ++last_added_byte;
                                //(&texelsBlock->alpha3[0]) to avoid gcc warning: array
                                // subscript is above array bounds [-Warray-bounds]
                                running_a_index += ( ( ( unsigned short )( &(
                                                         texelsBlock->alpha3[0]
                                                     ) )[last_added_byte] )
                                                     << ( 8 - ( 3 * j & 0X7 ) ) );
                            }
    ...
    {
        unsigned short result = static_cast<unsigned short>(
            ( ( ( color1 >> 11 ) * 2 + ( color2 >> 11 ) + 1 ) / 3 ) << 11
        );
        result = static_cast<unsigned short>(
            result +
            ( ( ( ( color1 >> 5 & 0X3F ) * 2 + ( color2 >> 5 & 0X3F ) + 1 ) / 3 ) << 5 )
        );
        result = static_cast<unsigned short>(
                         ...
                         unsigned short color2 )
    {
        unsigned short result =
            static_cast<unsigned short>( ( ( ( color1 >> 11 ) + ( color2 >> 11 ) ) / 2 )
                                         << 11 );
        result = static_cast<unsigned short>(
                             ...
                             unsigned char* imageData )
    {
        ...
        unsigned int   slab4Count =
            static_cast<unsigned int>( depth ) & ~0X3U;      // 4*floor(d/4)
        unsigned int col = ( s >> 2 );                       //(floor(x/4)
        unsigned int row = ( t >> 2 );                       //(floor(y/4)
        unsigned int blockWidth =
            static_cast<unsigned int>( width + 3 ) >> 2;     // ceil(w/4)
        unsigned int blockHeight =
            static_cast<unsigned int>( height + 3 ) >> 2;    // ceil(h/4)
        int blockNumber =
            static_cast<int>( col + blockWidth * row );      // block to jump to
        ...
        if( depth > 1 )
        {
            ...
            //        floor(x/4) + ceil(w/4) * (floor(y/4) + ceil(h/4) * (z-4*floor(d/4))
            //        ));
            //    } else {
            //        blockIndex = blocksize * 4 * (floor(x/4) + ceil(w/4) * (floor(y/4)
            //        + ceil(h/4) * floor(z/4)));
            //    }
            // note floor(a/4) = (a >> 2)
            ...
            // note ceil(a/4) = ((a + 3) >> 2)
            //
            //  rewrite: this describes the final blocks as consecutive 4x4x1 blocks -
            //  and thats not in the wording of the specs
            //    if (r >= slab4Count) {
            //        blockNumber = (blockWidth * blockHeight * slab4Count  + col +
            //        blockWidth * (row + blockHeight * (r-slab4Count) ));
            //    } else {
            ...
            //    if (z >= 4*floor(d/4)) {
            //        blockIndex = blocksize * (ceil(w/4) * ceil(h/4) * 4*floor(d/4) + (z
            //        - 4*floor(d/4)) * ( (floor(x/4) + ceil(w/4) * (floor(y/4) );
            //    } else {
            //        blockIndex = blocksize * 4 * (floor(x/4) + ceil(w/4) * (floor(y/4)
            //        + ceil(h/4) * floor(z/4)));
            //    }
            ...
            {                                // slice number beyond  4x4x4 slabs
                unsigned int blockDepth =
                    ...
                    0X3U;    // equals: depth - slab4Count;//depth of this final block:
                             // 1/2/3 in case of 4x4x1; 4x4x2 or 4x4x3 bricks
                blockNumber = static_cast<int>(
                    blockWidth *
                    blockHeight *
                    slab4Count    // jump full 4x4x4 slabs
            ...
            {
                blockNumber = static_cast<int>(
                    4 * ( col + blockWidth * ( row + blockHeight * ( r >> 2 ) ) ) + sub_r
                );
            }
        }
            ...
            case( GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ) :
                {
                    ...
                    char index =
                        ( texelsBlock->texels4x4 >> ( 2 * sub_s + 8 * sub_t ) ) &
                        0X3;                       // two bit "index value"
                    color[3] = 255;
                    switch( index )
                    {
                        case 0 :
                        ...
                        case 2 :
                            if( texelsBlock->color_0 > texelsBlock->color_1 )
                            {
                                color16 = interpolateColors21( texelsBlock->color_0,
                                                               texelsBlock->color_1 );
                            }
                            else
                            {
                                color16 = interpolateColors11( texelsBlock->color_0,
                                                               texelsBlock->color_1 );
                            }
                            break;
                        case 3 :
                            if( texelsBlock->color_0 > texelsBlock->color_1 )
                            {
                                color16 = interpolateColors21( texelsBlock->color_1,
                                                               texelsBlock->color_0 );
                            }
                            else
                            {
            ...
            case( GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ) :
                {
                    ...
                    char pixel = static_cast<char>(
                        sub_s + 4 * sub_t
                    );                            // pixel number in block: 0 - 15
                    char firstBit = 3 * pixel;    // least significant bit: range 0 - 45
                    unsigned char alpha_index;
                    if( ( firstBit & 0X7 ) < 6 )
                    {
                        alpha_index = texelsBlock->alpha3[firstBit >> 3] >>
                                      ( firstBit & 0X7 ) &
                                      0X7;    // grab byte containing least significant
                                              // bit; shift and get 3 bits
                    }
                    else
                    {
                        alpha_index =
                            texelsBlock->alpha3[firstBit >> 3] >> ( firstBit & 0X7 );
                        alpha_index |= texelsBlock->alpha3[1 + ( firstBit >> 3 )]
                                    << ( 8 - ( firstBit & 0X7 ) );
                        alpha_index &= 0X7;
                    }
                    if( alpha_index == 0 )
                        ...
                        else
                        {
                            if( texelsBlock->alpha_0 > texelsBlock->alpha_1 )
                            {
                                color[3] = static_cast<unsigned char>(
                                    ( ( unsigned short )texelsBlock->alpha_0 *
                                      ( 8 - alpha_index ) +
                                      ( unsigned short )texelsBlock->alpha_1 *
                                      ( alpha_index - 1 ) +
                                      3 ) /
                                    7
                                );
        ...
        color[0] = static_cast<unsigned char>( colorChannel << 3 | colorChannel >> 2 );
        colorChannel = color16 >> 5 & 0X3F;             // green - 6 bits
        color[1] = static_cast<unsigned char>( colorChannel << 2 | colorChannel >> 3 );
        colorChannel = color16 & 0X1F;                  // blue - 5 bits
        color[2] = static_cast<unsigned char>( colorChannel << 3 | colorChannel >> 2 );
        return true;
    }
                                          ...
                                          const osg::ivec3&    columnDelta )
    {
        ...
        switch( format )
        {
            ...
            case( GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ) :
                {
                    const DXT1TexelsBlock* src_texelsBlock =
                        reinterpret_cast<const DXT1TexelsBlock*>( src_block );
                    // make a copy as source might be equal to destination
                    src_texels4x4 =
                        src_texelsBlock
                            ->texels4x4;    // interpolated colors (2 bits per texel)
                    DXT1TexelsBlock* dst_texelsBlock =
                        reinterpret_cast<DXT1TexelsBlock*>( dst_block );
            ...
            case( GL_COMPRESSED_RGBA_S3TC_DXT3_EXT ) :
                {
                    const DXT3TexelsBlock* src_texelsBlock =
                        reinterpret_cast<const DXT3TexelsBlock*>( src_block );
                    // make a copy as source might be equal to destination
                    src_texels4x4 =
                        src_texelsBlock
                            ->texels4x4;    // interpolated colors (2 bits per texel)
                    DXT3TexelsBlock* dst_texelsBlock =
                        reinterpret_cast<DXT3TexelsBlock*>( dst_block );
                    dst_texels4x4 = &dst_texelsBlock->texels4x4;
                    unsigned short
                        src_alpha4[4];    // alpha values (4 bits per texel) - 64 bits

                    memcpy(
                        src_alpha4,
                        src_texelsBlock->alpha4,
                        4 * sizeof( unsigned short )
                    );    // make a copy as source might be equal to destination

                    memset( dst_texelsBlock->alpha4,
                            0,
                            4 * sizeof( unsigned short ) );    // clear
                    osg::ivec3 source_pixel( srcOrigin );
                    for( int r = 0; r < 4; r++ )               // rows
                    {
                        for( int c = 0; c < 4; c++ )           // columns
                        {
                            ...
                            unsigned int alpha_value   = src_alpha4[sub_t] >>
                                                         shiftBits &
                                                         0XF;    // four bit alpha values
            ...
            case( GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ) :
                {
                    const DXT5TexelsBlock* src_texelsBlock =
                        reinterpret_cast<const DXT5TexelsBlock*>( src_block );
                    // make a copy as source might be equal to destination
                    src_texels4x4 =
                        src_texelsBlock
                            ->texels4x4;    // interpolated colors (2 bits per texel)
                    DXT5TexelsBlock* dst_texelsBlock =
                        reinterpret_cast<DXT5TexelsBlock*>( dst_block );
                    dst_texels4x4 = &dst_texelsBlock->texels4x4;

                    unsigned char
                        src_alpha3[6];    // alpha index values (3 bits per texel)

                    memcpy(
                        src_alpha3,
                        src_texelsBlock->alpha3,
                        6 * sizeof( unsigned char )
                    );    // make a copy as source might be equal to destination

                    memset( dst_texelsBlock->alpha3,
                            0,
                            6 * sizeof( unsigned char ) );    // clear
                    osg::ivec3     source_pixel( srcOrigin );
                    unsigned int   last_added_byte = 1;
                    unsigned short running_a_index = static_cast<unsigned short>(
                        src_texelsBlock->alpha3[0] +
                        ( ( ( unsigned short )src_texelsBlock->alpha3[last_added_byte] )
                          << 8 )
                    );
                    unsigned int j = 0;
                    for( int r = 0; r < 4; r++ )        // rows
                    {
                        for( int c = 0; c < 4; c++ )    // columns
                        {
                            ...
                            dst_texelsBlock->alpha3[shiftBits >> 3] |=
                                alphaIndex << ( shiftBits & 0X7 );
                            if( ( shiftBits & 0X7 ) > 5 )
                            {
                                dst_texelsBlock->alpha3[1 + ( shiftBits >> 3 )] |=
                                    alphaIndex >> ( 8 - ( shiftBits & 0X7 ) );
                            }
        ...
        *dst_texels4x4 = 0;                 // clear
        ...
        for( int r = 0; r < 4; r++ )        // rows
        {
            for( int c = 0; c < 4; c++ )    // columns
            {
                ...
                unsigned int index =
                    ( src_texels4x4 >> ( shiftBits ) ) & 0X3;    // two bit "index value"
            ...
            case( GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ) :
                {
                    ...
                    {
                        // Four-color block
                        memcpy( dst_texelsBlock,
                                src_texelsBlock,
                                sizeof( DXT1TexelsBlock ) );
                    }
                    else
                    {
                        ...
                        {
                            unsigned char index = src_texels4x4 >>
                                                  shiftBits &
                                                  0X3;    // two bit "index value"
                            dst_texelsBlock->texels4x4 |=
                                static_cast<unsigned int>( reshuffle[index] )
                                << shiftBits;
                        }
                    }
                    break;
