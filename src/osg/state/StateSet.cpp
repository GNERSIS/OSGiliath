 ...
 * Encapsulates OpenGL state (modes, attributes, texture units, defines)
 * applied during rendering. Supports inheritance and override flags
...
class TextureGLModeSet
{
    ...
    public:
        ...
        TextureGLModeSet()
        {
            ...
            _textureModeSet.insert( GL_POINT_SPRITE );
        ...
        bool
        isTextureMode( StateAttribute::GLMode mode ) const
        {
...
bool
osg::isTextureMode( StateAttribute::GLMode mode )
{
...
//
bool
StateSet::Callback::run( osg::Object* object,
                         osg::Object* data )
{
...
StateSet::StateSet( const StateSet& rhs,
                    const CopyOp&   copyop ) :
    Object( rhs,
            copyop ),
...
StateSet::computeDataVariance()
{
    ...
    if( _updateCallback.valid() || _eventCallback.valid() )
    {
        dynamic = true;
    }
    ...
    for( AttributeList::iterator itr = _attributeList.begin();
         ...
         ++itr )
    {
        if( itr->second.first->getDataVariance() ==
            DataVariance::UNSPECIFIED &&
            ( itr->second.first->getUpdateCallback() ||
              itr->second.first->getEventCallback() ) )
        {
            itr->second.first->setDataVariance( DataVariance::DYNAMIC );
        }

        if( itr->second.first->getDataVariance() == DataVariance::DYNAMIC )
        {
            dynamic = true;
        }
    }
    ...
    for( UniformList::iterator uitr = _uniformList.begin(); uitr != _uniformList.end();
         ++uitr )
    {
        if( uitr->second.first->getDataVariance() ==
            DataVariance::UNSPECIFIED &&
            ( uitr->second.first->getUpdateCallback() ||
              uitr->second.first->getEventCallback() ) )
        {
            uitr->second.first->setDataVariance( DataVariance::DYNAMIC );
        }

        if( uitr->second.first->getDataVariance() == DataVariance::DYNAMIC )
        {
            dynamic = true;
        }
    }
    ...
    if( getDataVariance() == DataVariance::UNSPECIFIED )
    {
        setDataVariance( dynamic ? DataVariance::DYNAMIC : DataVariance::STATIC );
    }
}

void
StateSet::addParent( osg::Node* node )
{
...
void
StateSet::removeParent( osg::Node* node )
{
    std::unique_lock<std::mutex> lock( *getRefMutex() );
...
int
StateSet::compare( const StateSet& rhs,
                   bool            compareAttributeContents ) const
{
    ...
    for( unsigned int ai = 0; ai < _textureAttributeList.size(); ++ai )
    {
        ...
        if( compareAttributeContents )
        {
            ...
            while( lhs_attr_itr !=
                   lhs_attributeList.end() &&
                   rhs_attr_itr != rhs_attributeList.end() )
            {
                ...
                else if( *( rhs_attr_itr->second.first ) <
                         *( lhs_attr_itr->second.first ) )
                {
                    return 1;
                }
    ...
    if( compareAttributeContents )
    {
        ...
        while( lhs_attr_itr !=
               _attributeList.end() &&
               rhs_attr_itr != rhs._attributeList.end() )
        {
            if( lhs_attr_itr->first < rhs_attr_itr->first )
            {
                return -1;
    ...
    while( lhs_uniform_itr !=
           _uniformList.end() &&
           rhs_uniform_itr != rhs._uniformList.end() )
    {
    ...
    while( lhs_define_itr !=
           _defineList.end() &&
           rhs_define_itr != rhs._defineList.end() )
    {
...
int
StateSet::compareModes( const ModeList& lhs,
                        const ModeList& rhs )
{
...
int
StateSet::compareAttributePtrs( const AttributeList& lhs,
...
int
StateSet::compareAttributeContents( const AttributeList& lhs,
                                    const AttributeList& rhs )
{
    AttributeList::const_iterator lhs_attr_itr = lhs.begin();
    AttributeList::const_iterator rhs_attr_itr = rhs.begin();
...
StateSet::setGlobalDefaults()
{
    ...
    // Shader pipeline Program is now created in State (State-level default).
    // Here we only set up defines and fallback textures on the root StateSet.
    {
        ...
        if( maxTextureUnits > 0 )
        {
            ...
            setDefine( "OSG_MAX_TEXTURE_UNITS", sstream.str() );

// Map GL enum values to OSG_-prefixed shader defines.
// GLSL 4.6 Core Profile forbids macros starting with "GL_".
#define ADD_OSG_DEFINE( NAME, GL_ENUM ) \
    sstream.str( "" );                  \
    sstream << GL_ENUM;                 \
    setDefine( NAME, sstream.str() );

            osg::ref_ptr<osg::Texture2D> fallbackTexture = createDefaultTexture();
            fallbackTexture->setWrap( osg::Texture2D::WRAP_S,
                                      osg::Texture2D::CLAMP_TO_EDGE );
            fallbackTexture->setWrap( osg::Texture2D::WRAP_T,
                                      osg::Texture2D::CLAMP_TO_EDGE );
            fallbackTexture->setWrap( osg::Texture2D::WRAP_R,
                                      osg::Texture2D::CLAMP_TO_EDGE );
            fallbackTexture->setFilter( osg::Texture2D::MIN_FILTER,
                                        osg::Texture2D::LINEAR );
            fallbackTexture->setFilter( osg::Texture2D::MAG_FILTER,
                                        osg::Texture2D::LINEAR );
            ...
            ADD_OSG_DEFINE( "OSG_ALPHA", GL_ALPHA );
            ADD_OSG_DEFINE( "OSG_INTENSITY", GL_INTENSITY );
            ADD_OSG_DEFINE( "OSG_LUMINANCE", GL_LUMINANCE );
            ADD_OSG_DEFINE( "OSG_RED", GL_RED );
            ADD_OSG_DEFINE( "OSG_RG", GL_RG );
            ADD_OSG_DEFINE( "OSG_RGB", GL_RGB );
            ADD_OSG_DEFINE( "OSG_RGBA", GL_RGBA );
        }
    }
}
...
void
StateSet::merge( const StateSet& rhs )
{
         ...
         ++rhs_aitr )
    {
        ...
        {
            // take the rhs attribute unless the lhs is override and the rhs is not
            // protected
        ...
        {
            // entry doesn't exist so insert it, and then tell it about self by adding
            // self as parent.
    ...
    for( unsigned int mi = 0; mi < rhs._textureModeList.size(); ++mi )
    {
             ...
             ++rhs_mitr )
        {
            ...
            {
                // take the rhs mode unless the lhs is override and the rhs is not
                // protected
                if( !( lhs_mitr->second & StateAttribute::OVERRIDE ) ||
                    ( rhs_mitr->second & StateAttribute::PROTECTED ) )
                {
             ...
             ++rhs_aitr )
        {
            ...
            else
            {
                ...
                lhs_attributeList.insert( *rhs_aitr )
                    .first->second.first->addParent( this );
            }
        }
    }
         ...
         ++rhs_uitr )
    {
        ...
        {
            // take the rhs uniform unless the lhs is override and the rhs is not
            // protected
...
void
StateSet::setMode( StateAttribute::GLMode      mode,
                   StateAttribute::GLModeValue value )
{
    if( getTextureGLModeSet().isTextureMode( mode ) )
    {
        OSG_NOTICE << "Warning: texture mode '" << mode
                   << "'passed to setMode(mode,value), " << std::endl;
        OSG_NOTICE << "         assuming setTextureMode(unit=0,mode,value) instead."
                   << std::endl;
        OSG_NOTICE << "         please change calling code to use appropriate call."
                   << std::endl;
...
void
StateSet::removeMode( StateAttribute::GLMode mode )
{
    if( getTextureGLModeSet().isTextureMode( mode ) )
    {
        OSG_NOTICE << "Warning: texture mode '" << mode
                   << "'passed to setModeToInherit(mode), " << std::endl;
        OSG_NOTICE << "         assuming setTextureModeToInherit(unit=0,mode) instead."
                   << std::endl;
        OSG_NOTICE << "         please change calling code to use appropriate call."
                   << std::endl;
...
StateSet::getMode( StateAttribute::GLMode mode ) const
{
    if( !getTextureGLModeSet().isTextureMode( mode ) )
    {
    ...
    {
        OSG_NOTICE << "Warning: texture mode '" << mode << "'passed to getMode(mode), "
                   << std::endl;
        OSG_NOTICE << "         assuming getTextureMode(unit=0,mode) instead."
                   << std::endl;
        OSG_NOTICE << "         please change calling code to use appropriate call."
                   << std::endl;
                        ...
                        StateAttribute::OverrideValue value )
{
    if( attribute )
    {
        ...
        {
            OSG_NOTICE << "Warning: texture attribute '" << attribute->className()
                       << "'passed to setAttribute(attr,value), " << std::endl;
            OSG_NOTICE
                << "         assuming setTextureAttribute(unit=0,attr,value) instead."
                << std::endl;
            OSG_NOTICE << "         please change calling code to use appropriate call."
                       << std::endl;
...
void
StateSet::setAttributeAndModes( StateAttribute*             attribute,
                                StateAttribute::GLModeValue value )
{
    if( attribute )
    {
        ...
        {
            OSG_NOTICE << "Warning: texture attribute '" << attribute->className()
                       << "' passed to setAttributeAndModes(attr,value), " << std::endl;
            OSG_NOTICE << "         assuming "
                          "setTextureAttributeAndModes(unit=0,attr,value) instead."
                       << std::endl;
            OSG_NOTICE << "         please change calling code to use appropriate call."
                       << std::endl;
...
void
StateSet::removeAttribute( StateAttribute::Type type,
                           unsigned int         member )
{
    AttributeList::iterator itr =
        _attributeList.find( StateAttribute::TypeMemberPair( type, member ) );
...
void
StateSet::removeAttribute( StateAttribute* attribute )
{
    if( !attribute )
    {
        return;
...
StateAttribute*
StateSet::getAttribute( StateAttribute::Type type,
                        unsigned int         member )
{
...
StateSet::RefAttributePair*
StateSet::getAttributePair( StateAttribute::Type type,
                            unsigned int         member )
{
    return getAttributePair( _attributeList, type, member );
}

const StateSet::RefAttributePair*
StateSet::getAttributePair( StateAttribute::Type type,
                            unsigned int         member ) const
{
    return getAttributePair( _attributeList, type, member );
...
void
StateSet::addUniform( UniformBase*                  uniform,
                      StateAttribute::OverrideValue value )
{
    if( uniform )
    {
        ...
        else
        {
            if( itr->second.first == uniform )
            {
                ...
                itr->second.second =
                    value & ( StateAttribute::OVERRIDE | StateAttribute::PROTECTED );
            }
            else
            {
                itr->second.first->removeParent( this );
        ...
        if( delta_update != 0 )
        {
            setNumChildrenRequiringUpdateTraversal( static_cast<unsigned int>(
                static_cast<int>( getNumChildrenRequiringUpdateTraversal() ) +
                delta_update
            ) );
        }
        ...
        if( delta_event != 0 )
        {
            setNumChildrenRequiringEventTraversal( static_cast<unsigned int>(
                static_cast<int>( getNumChildrenRequiringEventTraversal() ) + delta_event
            ) );
        }
    }
}
...
void
StateSet::removeUniform( const std::string& name )
{
...
void
StateSet::removeUniform( UniformBase* uniform )
{
    if( !uniform )
    {
        return;
...
UniformBase*
StateSet::getUniformBase( const std::string& name )
{
    UniformList::iterator itr = _uniformList.find( name );
    if( itr != _uniformList.end() )
    {
...
Uniform*
StateSet::getOrCreateUniform( const std::string& name,
                              Uniform::Type      type,
                              unsigned int       numElements )
{
    ...
    if( itr != _uniformList.end() )
    {
        ...
        if( orig_uniform && orig_uniform->getType() == type )
        {
            return orig_uniform;
        }
    }
    ...
    Uniform* uniform = new Uniform( type, name, static_cast<int>( numElements ) );
    addUniform( uniform );
...
const UniformBase*
StateSet::getUniformBase( const std::string& name ) const
{
...
const StateSet::RefUniformPair*
StateSet::getUniformPair( const std::string& name ) const
{
    UniformList::const_iterator itr = _uniformList.find( name );
    if( itr != _uniformList.end() )
    {
...
void
StateSet::setDefine( const std::string&            defineName,
                     StateAttribute::OverrideValue value )
{
...
void
StateSet::removeDefine( const std::string& defineName )
{
...
void
StateSet::setTextureMode( unsigned int                unit,
                          ...
                          StateAttribute::GLModeValue value )
{
    ...
    {
        OSG_NOTICE << "Warning: non-texture mode '" << mode
                   << "'passed to setTextureMode(unit,mode,value), " << std::endl;
...
void
StateSet::removeTextureMode( unsigned int           unit,
                             StateAttribute::GLMode mode )
{
    if( getTextureGLModeSet().isTextureMode( mode ) )
    {
    ...
    {
        OSG_NOTICE << "Warning: non-texture mode '" << mode
                   << "'passed to setTextureModeToInherit(unit,mode), " << std::endl;
        OSG_NOTICE << "         assuming setModeToInherit(unit=0,mode) instead."
                   << std::endl;
        OSG_NOTICE << "         please change calling code to use appropriate call."
                   << std::endl;
...
StateAttribute::GLModeValue
StateSet::getTextureMode( unsigned int           unit,
                          StateAttribute::GLMode mode ) const
{
    ...
    {
        OSG_NOTICE << "Warning: non-texture mode '" << mode
                   << "'passed to geTexturetMode(unit,mode), " << std::endl;
...
void
StateSet::setTextureAttribute( unsigned int                  unit,
                               ...
                               StateAttribute::OverrideValue value )
{
    if( attribute )
    {
        ...
        {
            OSG_NOTICE << "Warning: texture attribute '" << attribute->className()
                       << "' passed to setTextureAttribute(unit,attr,value), "
                       << std::endl;
            OSG_NOTICE << "         assuming setAttribute(attr,value) instead."
                       << std::endl;
            OSG_NOTICE << "         please change calling code to use appropriate call."
                       << std::endl;
...
void
StateSet::setTextureAttributeAndModes( unsigned int                unit,
                                       StateAttribute*             attribute,
                                       StateAttribute::GLModeValue value )
{
    if( attribute )
    {
        ...
        if( attribute->isTextureAttribute() )
        {
            ...
            else
            {
                ...
                setAttribute( getOrCreateTextureAttributeList( unit ),
                              attribute,
                              value );
        ...
        else
        {
            OSG_NOTICE << "Warning: non texture attribute '" << attribute->className()
                       << "' passed to setTextureAttributeAndModes(unit,attr,value), "
                       << std::endl;
            OSG_NOTICE << "         assuming setAttributeAndModes(attr,value) instead."
                       << std::endl;
            OSG_NOTICE << "         please change calling code to use appropriate call."
                       << std::endl;
...
void
StateSet::removeTextureAttribute( unsigned int         unit,
                                  StateAttribute::Type type )
{
    ...
    AttributeList::iterator itr =
        attributeList.find( StateAttribute::TypeMemberPair( type, 0 ) );
    if( itr != attributeList.end() )
    {
        if( unit < _textureModeList.size() )
        {
            setAssociatedTextureModes( unit,
...
StateAttribute*
StateSet::getTextureAttribute( unsigned int         unit,
                               StateAttribute::Type type )
{
    if( unit >= _textureAttributeList.size() )
    {
...
StateSet::RefAttributePair*
StateSet::getTextureAttributePair( unsigned int         unit,
                                   StateAttribute::Type type )
{
    if( unit >= _textureAttributeList.size() )
    {
...
bool
StateSet::checkValidityOfAssociatedModes( osg::State& state ) const
{
    ...
    for( TextureAttributeList::const_iterator taitr = _textureAttributeList.begin();
         ...
         ++taitr )
    {
        for( AttributeList::const_iterator itr = taitr->begin(); itr != taitr->end();
             ++itr )
        {
            if( !itr->second.first->checkValidityOfAssociatedModes( state ) )
            {
...
void
StateSet::setThreadSafeRefUnref( bool threadSafe )
{
...
void
StateSet::compileGLObjects( State& state ) const
{
    bool checkForGLErrors =
        state.getCheckForGLErrors() == osg::State::ONCE_PER_ATTRIBUTE;
    if( checkForGLErrors )
    {
        state.checkGLErrors( "before StateSet::compileGLObejcts()" );
    }
         ...
         ++itr )
    {
        ...
        if( checkForGLErrors )
        {
            state.checkGLErrors( "StateSet::compileGLObejcts() compiling ",
...
void
StateSet::resizeGLObjectBuffers( unsigned int maxSize )
{
    for( AttributeList::const_iterator itr = _attributeList.begin();
         itr != _attributeList.end();
         ++itr )
...
void
StateSet::releaseGLObjects( State* state ) const
{
    for( AttributeList::const_iterator itr = _attributeList.begin();
         itr != _attributeList.end();
         ++itr )
...
void
StateSet::setRenderingHint( int hint )
{
    _renderingHint = hint;
...
void
StateSet::setRenderBinDetails( int                binNum,
...
void
StateSet::setMode( ModeList&                   modeList,
                   StateAttribute::GLMode      mode,
                   StateAttribute::GLModeValue value )
{
    if( value & StateAttribute::INHERIT )
...
void
StateSet::setModeToInherit( ModeList&              modeList,
                            StateAttribute::GLMode mode )
{
...
StateAttribute::GLModeValue
StateSet::getMode( const ModeList&        modeList,
                   StateAttribute::GLMode mode ) const
{
...
class SetAssociateModesHelper : public StateAttribute::ModeUsage
{
    public:
        ...
        SetAssociateModesHelper( StateSet*                   stateset,
                                 StateAttribute::GLModeValue value,
                                 unsigned int                unit = 0 ) :
            _stateset( stateset ),
            _value( value ),
        ...
        virtual void
        usesMode( StateAttribute::GLMode mode )
        {
            _stateset->setMode( mode, _value );
        }

        virtual void
        usesTextureMode( StateAttribute::GLMode mode )
        {
...
class RemoveAssociateModesHelper : public StateAttribute::ModeUsage
{
    public:
        ...
        RemoveAssociateModesHelper( StateSet*    stateset,
                                    unsigned int unit = 0 ) :
            _stateset( stateset ),
            _unit( unit )
        {
...
void
StateSet::setAssociatedModes( const StateAttribute*       attribute,
                              StateAttribute::GLModeValue value )
{
...
void
StateSet::removeAssociatedModes( const StateAttribute* attribute )
{
...
void
StateSet::setAssociatedTextureModes( unsigned int                unit,
...
void
StateSet::removeAssociatedTextureModes( unsigned int          unit,
                                        const StateAttribute* attribute )
{
...
void
StateSet::setAttribute( AttributeList&                attributeList,
                        StateAttribute*               attribute,
                        StateAttribute::OverrideValue value )
{
    if( attribute )
    {
        ...
        AttributeList::iterator itr =
            attributeList.find( attribute->getTypeMemberPair() );
        if( itr == attributeList.end() )
        {
            // new entry.
            attributeList[attribute->getTypeMemberPair()] = RefAttributePair(
                attribute,
                value & ( StateAttribute::OVERRIDE | StateAttribute::PROTECTED )
            );
...
StateAttribute*
StateSet::getAttribute( AttributeList&       attributeList,
                        StateAttribute::Type type )
{
    for( AttributeList::iterator itr = attributeList.begin(); itr != attributeList.end();
         ++itr )
    {
        if( itr->first.first == type )
        {
            return itr->second.first.get();
        }
    }
...
const StateAttribute*
StateSet::getAttribute( const AttributeList& attributeList,
                        StateAttribute::Type type ) const
{
...
{
    AttributeList::iterator itr =
        attributeList.find( StateAttribute::TypeMemberPair( type, member ) );
    if( itr != attributeList.end() )
    {
        return itr->second.first.get();
    }
...
StateSet::RefAttributePair*
StateSet::getAttributePair( AttributeList&       attributeList,
                            StateAttribute::Type type )
{
    for( AttributeList::iterator itr = attributeList.begin(); itr != attributeList.end();
         ++itr )
...
const StateSet::RefAttributePair*
StateSet::getAttributePair( const AttributeList& attributeList,
                            StateAttribute::Type type ) const
{
    for( AttributeList::const_iterator itr = attributeList.begin();
         itr != attributeList.end();
...
void
StateSet::setUpdateCallback( Callback* ac )
{
    ...
    if( delta != 0 && _numChildrenRequiringUpdateTraversal == 0 )
    {
        ...
        {
            osg::Node* node = *itr;
            node->setNumChildrenRequiringUpdateTraversal( static_cast<unsigned int>(
                static_cast<int>( node->getNumChildrenRequiringUpdateTraversal() ) +
                delta
            ) );
        }
    }
...
void
StateSet::runUpdateCallbacks( osg::NodeVisitor* nv )
{
...
void
StateSet::setEventCallback( Callback* ac )
{
    ...
    if( delta != 0 && _numChildrenRequiringEventTraversal == 0 )
    {
        ...
        {
            osg::Node* node = *itr;
            node->setNumChildrenRequiringEventTraversal( static_cast<unsigned int>(
                static_cast<int>( node->getNumChildrenRequiringEventTraversal() ) + delta
            ) );
        }
    }
}
...
void
StateSet::runEventCallbacks( osg::NodeVisitor* nv )
{
    if( _eventCallback.valid() )
    {
...
void
StateSet::setNumChildrenRequiringUpdateTraversal( unsigned int num )
{
    ...
    if( !_updateCallback && !_parents.empty() )
    {
        ...
        if( delta != 0 )
        {
            ...
            for( ParentList::iterator itr = _parents.begin(); itr != _parents.end();
                 ++itr )
            {
                osg::Node* node = *itr;
                node->setNumChildrenRequiringUpdateTraversal( static_cast<unsigned int>(
...
void
StateSet::setNumChildrenRequiringEventTraversal( unsigned int num )
{
    // if no changes just return.
    ...
    if( !_eventCallback && !_parents.empty() )
    {
        ...
        if( delta != 0 )
        {
                 ...
                 ++itr )
            {
                osg::Node* node = *itr;
                node->setNumChildrenRequiringEventTraversal( static_cast<unsigned int>(
                    static_cast<int>( node->getNumChildrenRequiringEventTraversal() ) +
                    delta
                ) );
            }
        }
