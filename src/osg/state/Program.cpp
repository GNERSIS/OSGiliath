...
class GLProgramManager : public GLObjectManager
{
    public:
        ...
        GLProgramManager( unsigned int contextID ) :
            GLObjectManager( "GLProgramManager",
                             contextID )
        {
        }
        ...
        deleteGLObject( GLuint globj )
        {
            ...
            {
                extensions->glDeleteProgram( globj );
            }
        }
};
...
Program::ProgramBinary::ProgramBinary() :
    _format( 0 )
{
}

Program::ProgramBinary::ProgramBinary( const ProgramBinary& rhs,
...
void
Program::ProgramBinary::allocate( unsigned int size )
{
    _data.clear();
    _data.resize( size );
}
...
void
Program::ProgramBinary::assign( unsigned int         size,
             ...
             copyop )
{
    ...
    for( osg::Program::AttribBindingList::const_iterator attribute = abl.begin();
         attribute != abl.end();
         ++attribute )
    {
    ...
    for( osg::Program::FragDataBindingList::const_iterator fragdata = fdl.begin();
         fragdata != fdl.end();
         ++fragdata )
    {
...
int
Program::compare( const osg::StateAttribute& sa ) const
{
    // check the types are equal and then create the rhs variable
    // used by the COMPARE_StateAttribute_Parameter macros below.
    COMPARE_StateAttribute_Types( Program, sa ) if( _shaderList.size() <
                                                    rhs._shaderList.size() ) return -1;
    ...
    for( ; litr != _shaderList.end(); ++litr, ++ritr )
    {
...
void
Program::compileGLObjects( osg::State& state ) const
{
    ...
    if( !_feedbackout.empty() )
    {
        ...
        GLsizei      numfeedback = static_cast<GLsizei>( _feedbackout.size() );
        const char** varyings    = new const char*[static_cast<size_t>( numfeedback )];
        ...
        extensions->glTransformFeedbackVaryings( pcp->getHandle(),
                                                 numfeedback,
                                                 varyings,
                                                 _feedbackmode );
        delete[] varyings;
...
void
Program::setThreadSafeRefUnref( bool threadSafe )
{
...
void
Program::resizeGLObjectBuffers( unsigned int maxSize )
{
    for( unsigned int i = 0; i < _shaderList.size(); ++i )
    {
        if( _shaderList[i].valid() )
...
void
Program::releaseGLObjects( osg::State* state ) const
{
    for( unsigned int i = 0; i < _shaderList.size(); ++i )
    {
        if( _shaderList[i].valid() )
...
bool
Program::removeShader( Shader* shader )
{
    if( !shader )
    {
        return false;
...
void
Program::setParameter( GLenum pname,
                       GLint  value )
{
    switch( pname )
    {
        ...
        case GL_PATCH_VERTICES :
            OSG_WARN << "Program::setParameter invalid param " << GL_PATCH_VERTICES
                     << ", use osg::PatchParameter when setting GL_PATCH_VERTICES."
                     << std::endl;
            break;
        default :
...
GLint
Program::getParameter( GLenum pname ) const
{
    switch( pname )
    {
        case GL_GEOMETRY_VERTICES_OUT :
...
void
Program::addBindAttribLocation( const std::string& name,
                                GLuint             index )
{
...
void
Program::removeBindAttribLocation( const std::string& name )
{
...
void
Program::addBindFragDataLocation( const std::string& name,
                                  GLuint             index )
{
...
void
Program::removeBindFragDataLocation( const std::string& name )
{
...
void
Program::addBindUniformBlock( const std::string& name,
                              GLuint             index )
{
...
void
Program::removeBindUniformBlock( const std::string& name )
{
...
void
Program::apply( osg::State& state ) const
{
    const GLExtensions* extensions = state.get<GLExtensions>();
    if( !extensions->isGlslSupported )
    {
...
Program::ProgramObjects::ProgramObjects( const osg::Program* program,
...
Program::ProgramObjects::getPCP( const std::string& defineStr ) const
{
         ...
         ++itr )
    {
        ...
        {
            // OSG_NOTICE<<"Returning PCP "<<itr->get()<<" DefineString =
...
Program::PerContextProgram*
Program::ProgramObjects::createPerContextProgram( const std::string& defineStr )
{
    ...
    // OSG_NOTICE<<"Creating PCP "<<pcp<<" PCP DefineString =
    // ["<<pcp->getDefineString()<<"]"<<std::endl;
    return pcp;
}

void
Program::ProgramObjects::requestLink()
{
...
void
Program::ProgramObjects::addShaderToAttach( Shader* shader )
{
    for( PerContextPrograms::iterator itr = _perContextPrograms.begin();
         itr != _perContextPrograms.end();
         ++itr )
...
void
Program::ProgramObjects::addShaderToDetach( Shader* shader )
{
    for( PerContextPrograms::iterator itr = _perContextPrograms.begin();
         itr != _perContextPrograms.end();
         ++itr )
...
bool
Program::ProgramObjects::getGlProgramInfoLog( std::string& log ) const
{
    bool result = false;
    for( PerContextPrograms::const_iterator itr = _perContextPrograms.begin();
         itr != _perContextPrograms.end();
...
Program::PerContextProgram*
Program::getPCP( State& state ) const
{
    unsigned int contextID = state.getContextID();
...
bool
Program::isFixedFunction() const
{
    // No fixed-function pipeline in GL 4.6 Core Profile.
    return false;
}
...
Program::PerContextProgram::PerContextProgram( const Program* program,
                                               unsigned int   contextID,
    ...
    _ownsProgramHandle( false )
{
    ...
    if( _glProgramHandle == 0 )
    {
        ...
        {
            OSG_WARN << "Unable to create osg::Program \"" << _program->getName() << "\""
...
Program::PerContextProgram::~PerContextProgram()
{
    ...
    {
        osg::get<GLProgramManager>( _contextID )
            ->scheduleGLObjectForDeletion( _glProgramHandle );
    }
}

void
Program::PerContextProgram::requestLink()
{
    _needsLink = true;
    _isLinked  = false;
}
...
Program::PerContextProgram::linkProgram( osg::State& state )
{
    ...
    OSG_INFO << "Linking osg::Program \"" << _program->getName() << "\""
             << " id=" << _glProgramHandle << " contextID=" << _contextID << std::endl;
    ...
    if( !_loadedBinary && _extensions->isGeometryShader4Supported )
    {
        ...
        _extensions->glProgramParameteri( _glProgramHandle,
                                          ...
                                          _program->_geometryInputType );
        _extensions->glProgramParameteri( _glProgramHandle,
                                          GL_GEOMETRY_OUTPUT_TYPE_EXT,
                                          _program->_geometryOutputType );
    }
    ...
    if( !_loadedBinary )
    {
        ...
        _extensions->glGetAttachedShaders( _glProgramHandle,
                                           shaderMaxCount,
                                           &shadersCount,
                                           shaderObjectHandle );
    ...
    if( !_loadedBinary )
    {
        ...
        for( AttribBindingList::const_iterator itr = programBindlist.begin();
             itr != programBindlist.end();
             ++itr )
        {
            OSG_INFO << "Program's vertex attrib binding " << itr->second << ", "
                     << itr->first << std::endl;
            _extensions->glBindAttribLocation(
                _glProgramHandle,
                itr->second,
        ...
        // set any explicit vertex attribute bindings that are set up via osg::State,
        // such as the vertex arrays
        ...
        for( FragDataBindingList::const_iterator itr = fdbindlist.begin();
             ...
             ++itr )
        {
            _extensions->glBindFragDataLocation(
                _glProgramHandle,
                itr->second,
                reinterpret_cast<const GLchar*>( itr->first.c_str() )
            );
        ...
        // if any program binary has been set then assume we want to retrieve a binary
        // later.
        if( programBinary )
        {
            _extensions->glProgramParameteri( _glProgramHandle,
    ...
    if( !_isLinked )
    {
        OSG_NOTICE << "glLinkProgram " << this << "\"" << _program->getName()
                   << "\" FAILED" << std::endl;
    ...
    else
    {
        ...
        _extensions->debugObjectLabel( GL_PROGRAM,
                                       _glProgramHandle,
                                       _program->getName() );
    }
    ...
    if( _extensions->isUniformBufferObjectSupported )
    {
        ...
        {
            std::vector<GLchar> blockName( static_cast<size_t>( maxBlockNameLen ) );
            for( GLuint i = 0; i < activeUniformBlocks; ++i )
            {
                GLsizei len       = 0;
                GLint   blockSize = 0;
                _extensions->glGetActiveUniformBlockName( _glProgramHandle,
                                                          i,
                                                          maxBlockNameLen,
                                                          &len,
                                                          &blockName[0] );
                _extensions->glGetActiveUniformBlockiv( _glProgramHandle,
                                                        i,
                                                        GL_UNIFORM_BLOCK_DATA_SIZE,
                                                        &blockSize );
                _uniformBlockMap.insert(
                    UniformBlockMap::value_type( &blockName[0],
    ...
    if( ( numUniforms > 0 ) && ( maxLen > 1 ) )
    {
        ...
        for( GLint i = 0; i < numUniforms; ++i )
        {
            ...
            int pos = static_cast<int>( strlen( name ) );
            if( pos > 0 && name[pos - 1] == ']' )
            {
                // need to trim [..] from end of name as some drivers append this causing
                // problems with look up.
                --pos;
            ...
            {
                atomicCounterMap[static_cast<GLuint>( i )] = name;
            }
            ...
            if( loc != -1 )
            {
                _uniformInfoMap[Uniform::getNameID(
    ...
    if( _extensions->isShaderAtomicCountersSupported && !atomicCounterMap.empty() )
    {
        ...
        _extensions->glGetActiveUniformsiv( _glProgramHandle,
                                            static_cast<GLsizei>( uniformIndex.size() ),
                                            &( uniformIndex[0] ),
        ...
        {
            OSG_INFO << "\tUniform atomic counter \""
                     << atomicCounterMap[uniformIndex[j]] << "\""
        ...
        for( unsigned int i = 0; i < bufferIndex.size(); ++i )
        {
            bufferIndexToUniformIndices[bufferIndex[i]].push_back(
                static_cast<int>( uniformIndex[i] )
            );
        }

        GLuint activeAtomicCounterBuffers = 0;
        _extensions->glGetProgramiv(
            _glProgramHandle,
        ...
        if( activeAtomicCounterBuffers > 0 )
        {
            ...
            {
                GLint bindID = 0;
                _extensions->glGetActiveAtomicCounterBufferiv(
                    _glProgramHandle,
                    i,
                ...
                if( num )
                {
                    std::vector<GLint> indices( static_cast<size_t>( num ) );
                    _extensions->glGetActiveAtomicCounterBufferiv(
                        _glProgramHandle,
                        i,
                    ...
                    for( GLsizei j = 0; j < num; ++j )
                    {
                        OSG_INFO << indices[static_cast<size_t>( j )];
                        if( j < ( num - 1 ) )
                        {
                            OSG_INFO << ", ";
                        }
...
bool
Program::PerContextProgram::validateProgram()
{
    if( !_glProgramHandle )
    {
        return false;
...
bool
Program::PerContextProgram::getInfoLog( std::string& infoLog ) const
{
    if( !_glProgramHandle )
    {
        return false;
...
Program::PerContextProgram::compileProgramBinary( osg::State& state )
{
    ...
    if( binaryLength )
    {
        ...
        programBinary->allocate( static_cast<unsigned int>( binaryLength ) );
        GLenum binaryFormat = 0;
        _extensions->glGetProgramBinary(
            _glProgramHandle,
            binaryLength,
            0,
            &binaryFormat,
            reinterpret_cast<GLvoid*>( programBinary->getData() )
        );
...
void
Program::PerContextProgram::useProgram() const
{
    if( !_glProgramHandle )
    {
        return;
