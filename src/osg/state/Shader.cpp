 ...
 * Single GLSL shader stage (vertex, fragment, geometry, etc.). Holds
 * source code with #pragma import_defines for conditional compilation.
 */
...
namespace osg
{
    ...
    find_first( const std::string&     str,
                const M&               match,
    ...
    // It does only one pass, i.e. new matches created in a position before the current
    // match are not replaced
    void
...
class GLShaderManager : public GLObjectManager
{
    public:
        ...
        GLShaderManager( unsigned int contextID ) :
            GLObjectManager( "GLShaderManager",
                             contextID )
        {
        }
        ...
        deleteGLObject( GLuint globj )
        {
            ...
            {
                extensions->glDeleteShader( globj );
            }
        }
};
...
ShaderComponent::ShaderComponent( const ShaderComponent& sc,
                                  const CopyOp&          copyop ) :
    Inherit( sc,
             copyop ),
...
unsigned int
ShaderComponent::addShader( osg::Shader* shader )
{
    ...
    return static_cast<unsigned int>( _shaders.size() ) - 1;
}

void
ShaderComponent::removeShader( unsigned int i )
{
    _shaders.erase( _shaders.begin() + i );
}

void
ShaderComponent::compileGLObjects( State& state ) const
{
    for( Shaders::const_iterator itr = _shaders.begin(); itr != _shaders.end(); ++itr )
    {
...
void
ShaderComponent::resizeGLObjectBuffers( unsigned int maxSize )
{
    for( Shaders::const_iterator itr = _shaders.begin(); itr != _shaders.end(); ++itr )
    {
...
void
ShaderComponent::releaseGLObjects( State* state ) const
{
    for( Shaders::const_iterator itr = _shaders.begin(); itr != _shaders.end(); ++itr )
    {
...
ShaderBinary::ShaderBinary( const ShaderBinary& rhs,
...
void
ShaderBinary::allocate( unsigned int size )
{
    _data.clear();
    _data.resize( size );
}
...
void
ShaderBinary::assign( unsigned int         size,
...
ShaderBinary*
ShaderBinary::readShaderBinaryFile( const std::string& fileName )
{
    std::ifstream fin;
    ...
    shaderBinary->allocate( static_cast<unsigned int>( length ) );
...
int
Shader::compare( const Shader& rhs ) const
{
    if( this == &rhs )
    {
        return 0;
...
void
Shader::setShaderSource( const std::string& sourceText )
{
...
Shader*
Shader::readShaderFile( Type               type,
...
bool
Shader::loadShaderSourceFromFile( const std::string& fileName )
{
    ...
    int   length = static_cast<int>( sourceFile.tellg() );
    char* text   = new char[static_cast<std::size_t>( length ) + 1];
...
Shader::Type
Shader::getTypeId( const std::string& tname )
{
    if( tname == "VERTEX" )
    {
        return VERTEX;
...
void
Shader::resizeGLObjectBuffers( unsigned int maxSize )
{
    _pcsList.resize( maxSize );
}

void
Shader::releaseGLObjects( osg::State* state ) const
{
    if( !state )
    {
...
void
Shader::compileShader( osg::State& state ) const
{
...
Shader::ShaderObjects::ShaderObjects( const osg::Shader* shader,
...
Shader::ShaderObjects::getPCS( const std::string& defineStr ) const
{
    ...
    // OSG_NOTICE<<"ShaderPtr = "<<_shader<<" FileName = '"<<_shader->getFileName()<<"'
    // returning NULL"<<std::endl;
    return 0;
}

Shader::PerContextShader*
Shader::ShaderObjects::createPerContextShader( const std::string& defineStr )
{
    ...
    // OSG_NOTICE<<"ShaderPtr = "<<_shader<<" FileName = '"<<_shader->getFileName()<<"'
    // Creating PCS "<<pcs<<" DefineString = ["<<pcs->getDefineString()<<"]"<<std::endl;
    return pcs;
}

void
Shader::ShaderObjects::requestCompile()
{
    for( PerContextShaders::const_iterator itr = _perContextShaders.begin();
         itr != _perContextShaders.end();
         ++itr )
...
Shader::PerContextShader*
Shader::getPCS( osg::State& state ) const
{
...
bool
Shader::addProgramRef( Program* program )
{
    std::lock_guard<std::mutex> lk( _programSetMutex );
...
bool
Shader::removeProgramRef( Program* program )
{
    std::lock_guard<std::mutex> lk( _programSetMutex );
    ProgramSet::iterator        itr = _programSet.find( program );
    if( itr == _programSet.end() )
...
Shader::dirtyShader()
{
    ...
    for( ProgramSet::iterator itr = _programSet.begin(); itr != _programSet.end();
         ++itr )
    {
        ( *itr )->dirtyProgram();
    }
...
Shader::PerContextShader::PerContextShader( const Shader* shader,
                                            unsigned int  contextID ) :
    osg::Referenced(),
    _contextID( contextID )
{
    ...
    _glShaderHandle =
        _extensions->glCreateShader( static_cast<GLenum>( shader->getType() ) );
    requestCompile();
}

Shader::PerContextShader::~PerContextShader()
{
    osg::get<GLShaderManager>( _contextID )
        ->scheduleGLObjectForDeletion( _glShaderHandle );
}

void
Shader::PerContextShader::requestCompile()
{
    _needsCompile = true;
    _isCompiled   = false;
}
...
void
Shader::PerContextShader::compileShader( osg::State& state )
{
    if( !_needsCompile )
    {
        return;
    ...
    else
    {
        ...
        do
        {
            std::string::size_type start_of_line =
                find_first( source, NoneOf( " \t" ), previous_pos );
            std::string::size_type end_of_line =
                ( start_of_line != std::string::npos )
                    ? find_first( source, OneOf( "\n\r" ), start_of_line )
                    : std::string::npos;
            if( end_of_line != std::string::npos )
            {
                // OSG_NOTICE<<"A Checking line "<<lineNum<<"
                // ["<<source.substr(start_of_line,
                // end_of_line-start_of_line)<<"]"<<std::endl;
                if( ( end_of_line - start_of_line ) >=
                    8 &&
                    source.compare( start_of_line, 8, "#version" ) == 0 )
                {
                    versionLine =
                        source.substr( start_of_line, end_of_line - start_of_line + 1 );
                    if( versionLine[versionLine.size() - 1] != '\n' )
                    {
                        versionLine.push_back( '\n' );
                    }

                    source.insert( start_of_line,
                                   "// following version spec has been automatically "
                                   "reassigned to start of source list: " );
        ...
        if( !versionLine.empty() )
        {
            if( printOutShaders )
            {
                std::string sourceWithLineNumbers =
                    insertLineNumbers( versionLine + _defineStr + source );
                OSG_NOTICE << "\nCompiling " << _shader->getTypename() << " source:\n"
                           << sourceWithLineNumbers << std::endl;
            }
        ...
        else
        {
            ...
            {
                std::string sourceWithLineNumbers =
                    insertLineNumbers( _defineStr + source );
                OSG_NOTICE << "\nCompiling " << _shader->getTypename() << " source:\n"
                           << sourceWithLineNumbers << std::endl;
            }
    ...
    if( !_isCompiled )
    {
        OSG_WARN << _shader->getTypename() << " glCompileShader \"" << _shader->getName()
                 << "\" FAILED" << std::endl;

        std::string infoLog;
        if( getInfoLog( infoLog ) )
        {
            OSG_WARN << _shader->getTypename() << " Shader \"" << _shader->getName()
                     << "\" infolog:\n"
                     << infoLog << std::endl;
        }
    }
    else
    {
        ...
        if( getInfoLog( infoLog ) )
        {
            OSG_INFO << _shader->getTypename() << " Shader \"" << _shader->getName()
                     << "\" infolog:\n"
                     << infoLog << std::endl;
        }
...
bool
Shader::PerContextShader::getInfoLog( std::string& infoLog ) const
{
    return _extensions->getShaderInfoLog( _glShaderHandle, infoLog );
}

void
Shader::PerContextShader::attachShader( GLuint program ) const
{
    _extensions->glAttachShader( program, _glShaderHandle );
}

void
Shader::PerContextShader::detachShader( GLuint program ) const
{
    _extensions->glDetachShader( program, _glShaderHandle );
}

bool
Shader::_parseShaderDefines( const std::string& str,
                             ...
                             unsigned int&      index )
{
    ...
    do
    {
        ...
        std::string::size_type end_of_parameter =
            find_first( str, OneOf( " \t,)" ), start_of_parameter );

        if( end_of_parameter != std::string::npos )
        {
            std::string::size_type start_of_open_brackets =
                find_first( str, EqualTo( '(' ), start_of_parameter );
        ...
        if( start_of_parameter < end_of_parameter )
        {
            std::string parameter =
                str.substr( start_of_parameter, end_of_parameter - start_of_parameter );
            ...
            if( firstString )
            {
                ...
                {
                    index    = static_cast<unsigned int>( atoi( parameter.c_str() ) );
                    indexSet = false;
...
{
    if( _shaderPragmaMode == USE_MANUAL_SETTINGS )
    {
        return;
    }
    ...
    while( ( pos = _shaderSource.find( "#pragma", pos ) ) != std::string::npos )
    {
        ...
        std::string::size_type first_chararcter =
            find_first( _shaderSource, NoneOf( " \t" ), pos, eol );

        OSG_INFO << "\nFound pragma line ["
                 << _shaderSource.substr( first_chararcter, eol - first_chararcter )
                 << "]" << std::endl;

        if( first_chararcter < eol )
        {
            std::string::size_type end_of_keyword =
                find_first( _shaderSource, OneOf( " \t(" ), first_chararcter, eol );
            ...
            std::string::size_type open_brackets =
                find_first( _shaderSource, EqualTo( '(' ), end_of_keyword, eol );
            if( open_brackets < eol )
            {
                std::string  str( _shaderSource,
                                  open_brackets + 1,
                                  eol - open_brackets - 1 );
                ...
                {
                    _parseShaderDefines( str, _shaderPragmas.defines, index );
                }
                else if( keyword == "import_modes" )
                {
                    _parseShaderDefines( str, _shaderPragmas.modes, index );
                }
                else if( keyword == "import_texture_modes" )
                {
                    ...
                    if( index >= _shaderPragmas.textureModes.size() )
                    {
                        _shaderPragmas.textureModes.resize( index + 1 );
                    }
                    _shaderPragmas.textureModes[index] = sd;
                }
                else if( keyword == "requires" )
                {
                    _parseShaderDefines( str, _shaderPragmas.requirements, index );
                }
                else
                {
                    OSG_NOTICE << "  keyword not matched [" << keyword << "]"
                               << std::endl;
                }
            }
#if 1
