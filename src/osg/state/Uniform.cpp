...
UniformBase::UniformBase( const UniformBase& rhs,
                          const CopyOp&      copyop ) :
    Inherit( rhs,
             copyop ),
    _nameID( rhs._nameID ),
...
int
UniformBase::compare( const UniformBase& ) const
{
    return 0;
}

int
UniformBase::compareData( const UniformBase& ) const
{
    return 0;
}

void
UniformBase::setName( const std::string& name )
{
    if( _name == name )
    {
        return;
    ...
    // need to copy parents list before iterating through it as addUniform/removeUniform
    // will modify the _parents list and invalidate the iterators.
...
void
UniformBase::setName( const std::string& baseName,
                      unsigned int       unit )
{
...
void
UniformBase::addParent( osg::StateSet* object )
{
...
void
UniformBase::removeParent( osg::StateSet* object )
{
    std::unique_lock<std::mutex> lock( *getRefMutex() );
...
void
UniformBase::setUpdateCallback( UniformCallback* uc )
{
...
void
UniformBase::setEventCallback( UniformCallback* ec )
{
...
void
UniformBase::apply( const GLExtensions*,
                    GLint ) const
{
    OSG_NOTICE << "Unimplemented : UniformBase::apply(const GLExtensions*, GLint)"
               << std::endl;
}
...
Uniform::Uniform( Type               type,
    ...
    _numElements( 0 )
{
    setNumElements( static_cast<unsigned int>( numElements ) );
    allocateDataArray();
}
...
void
Uniform::setNumElements( unsigned int numElements )
{
    if( numElements < 1 )
    {
    ...
    if( _numElements > 0 )
    {
        OSG_WARN << "Warning: Uniform::setNumElements() cannot change Uniform "
                    "numElements, size already fixed."
                 << std::endl;
        return;
    }
...
Uniform::allocateDataArray()
{
    ...
    unsigned int arrayNumElements = getInternalArrayNumElements();
    if( arrayNumElements )
    {
...
bool
Uniform::setArray( FloatArray* array )
{
    if( !array )
    {
        return false;
    ...
    if( getInternalArrayType( getType() ) !=
        GL_FLOAT ||
        getInternalArrayNumElements() != array->getNumElements() )
    {
...
bool
Uniform::setArray( DoubleArray* array )
{
    if( !array )
    {
        return false;
...
bool
Uniform::setArray( IntArray* array )
{
    if( !array )
    {
        return false;
...
bool
Uniform::setArray( UIntArray* array )
{
    if( !array )
    {
        return false;
...
bool
Uniform::setArray( UInt64Array* array )
{
    if( !array )
    {
        return false;
...
bool
Uniform::setArray( Int64Array* array )
{
    if( !array )
    {
        return false;
...
int
Uniform::compare( const UniformBase& ub_rhs ) const
{
    if( typeid( this ) != typeid( &ub_rhs ) )
    {
        return ( this < &ub_rhs );
    }
...
int
Uniform::compareData( const UniformBase& ub_rhs ) const
{
    ...
    else if( _doubleArray.valid() )
    {
        ...
        return memcmp( _doubleArray->getDataPointer(),
                       rhs._doubleArray->getDataPointer(),
    ...
    else if( _uint64Array.valid() )
    {
        ...
        if( _uint64Array == rhs._uint64Array )
        {
            return 0;
        }
        return memcmp( _uint64Array->getDataPointer(),
                       rhs._uint64Array->getDataPointer(),
    ...
    else if( _int64Array.valid() )
    {
        ...
        if( _int64Array == rhs._int64Array )
        {
            return 0;
        }
        return memcmp( _int64Array->getDataPointer(),
                       rhs._int64Array->getDataPointer(),
...
void
Uniform::copyData( const Uniform& rhs )
{
    // caller must ensure that _type==rhs._type
    _numElements = rhs._numElements;
    _nameID      = rhs._nameID;
    if( rhs._floatArray.valid() ||
        rhs._doubleArray.valid() ||
        rhs._intArray.valid() ||
        rhs._uintArray.valid() )
    {
        allocateDataArray();
    }
    ...
    if( _int64Array.valid() && rhs._int64Array.valid() )
    {
        *_int64Array = *rhs._int64Array;
    }
    if( _uint64Array.valid() && rhs._uint64Array.valid() )
    {
        *_uint64Array = *rhs._uint64Array;
    }
    dirty();
}

bool
Uniform::isCompatibleType( Type t ) const
{
    ...
    OSG_WARN << "Cannot assign between Uniform types " << getTypename( t ) << " and "
             << getTypename( getType() ) << std::endl;
    return false;
}

bool
Uniform::isCompatibleType( Type t1,
                           Type t2 ) const
{
    ...
    OSG_WARN << "Cannot assign between Uniform types " << getTypename( t1 ) << " or "
             << getTypename( t2 ) << " and " << getTypename( getType() ) << std::endl;
    return false;
}

unsigned int
Uniform::getInternalArrayNumElements() const
{
    ...
    return getNumElements() *
           static_cast<unsigned int>( getTypeNumComponents( getType() ) );
}
...
int
Uniform::getTypeNumComponents( Type t )
{
    switch( t )
    {
        case FLOAT :
...
Uniform::Type
Uniform::getTypeId( const std::string& tname )
{
    if( tname == "float" )
    {
        return FLOAT;
...
GLenum
Uniform::getInternalArrayType( Type t )
{
    switch( t )
    {
        case FLOAT :
...
unsigned int
Uniform::getNameID( const std::string& name )
{
    typedef std::map<std::string, unsigned int> UniformNameIDMap;
    static std::mutex                           s_mutex_uniformNameIDMap;
    static UniformNameIDMap                     s_uniformNameIDMap;

    std::lock_guard<std::mutex>                 lock( s_mutex_uniformNameIDMap );
    UniformNameIDMap::iterator                  it = s_uniformNameIDMap.find( name );
    if( it != s_uniformNameIDMap.end() )
    {
        return it->second;
    }
    unsigned int id = static_cast<unsigned int>( s_uniformNameIDMap.size() );
    s_uniformNameIDMap.insert( UniformNameIDMap::value_type( name, id ) );
    return id;
}

// Use a proxy to force the initialization of the static variables in the
// Uniform::getNameID() method during static initialization
...
bool
Uniform::set( const osg::Matrix2& m2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix3& m3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix2x3& m2x3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix2x4& m2x4 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix3x2& m3x2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix3x4& m3x4 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix4x2& m4x2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix4x3& m4x3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::dvec2& v2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::dvec3& v3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::dvec4& v4 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix2d& m2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix3d& m3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::dmat4& m4 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix2x3d& m2x3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix2x4d& m2x4 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix3x2d& m3x2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix3x4d& m3x4 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix4x2d& m4x2 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( const osg::Matrix4x3d& m4x3 )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::set( unsigned long long ull )
{
    if( getNumElements() == 0 )
    {
        setNumElements( 1 );
...
bool
Uniform::get( float& f ) const
{
    return isScalar() ? getElement( 0, f ) : false;
}

bool
Uniform::get( osg::vec2& v2 ) const
{
    return isScalar() ? getElement( 0, v2 ) : false;
}

bool
Uniform::get( osg::vec3& v3 ) const
{
    return isScalar() ? getElement( 0, v3 ) : false;
}

bool
Uniform::get( osg::vec4& v4 ) const
{
    return isScalar() ? getElement( 0, v4 ) : false;
}

bool
Uniform::get( osg::Matrix2& m2 ) const
{
    return isScalar() ? getElement( 0, m2 ) : false;
}

bool
Uniform::get( osg::Matrix3& m3 ) const
{
    return isScalar() ? getElement( 0, m3 ) : false;
}

bool
Uniform::get( osg::mat4& m4 ) const
{
    return isScalar() ? getElement( 0, m4 ) : false;
}

bool
Uniform::get( osg::Matrix2x3& m2x3 ) const
{
    return isScalar() ? getElement( 0, m2x3 ) : false;
}

bool
Uniform::get( osg::Matrix2x4& m2x4 ) const
{
    return isScalar() ? getElement( 0, m2x4 ) : false;
}

bool
Uniform::get( osg::Matrix3x2& m3x2 ) const
{
    return isScalar() ? getElement( 0, m3x2 ) : false;
}

bool
Uniform::get( osg::Matrix3x4& m3x4 ) const
{
    return isScalar() ? getElement( 0, m3x4 ) : false;
}

bool
Uniform::get( osg::Matrix4x2& m4x2 ) const
{
    return isScalar() ? getElement( 0, m4x2 ) : false;
}

bool
Uniform::get( osg::Matrix4x3& m4x3 ) const
{
    return isScalar() ? getElement( 0, m4x3 ) : false;
}

bool
Uniform::get( double& d ) const
{
    return isScalar() ? getElement( 0, d ) : false;
}

bool
Uniform::get( osg::dvec2& v2 ) const
{
    return isScalar() ? getElement( 0, v2 ) : false;
}

bool
Uniform::get( osg::dvec3& v3 ) const
{
    return isScalar() ? getElement( 0, v3 ) : false;
}

bool
Uniform::get( osg::dvec4& v4 ) const
{
    return isScalar() ? getElement( 0, v4 ) : false;
}

bool
Uniform::get( osg::Matrix2d& m2 ) const
{
    return isScalar() ? getElement( 0, m2 ) : false;
}

bool
Uniform::get( osg::Matrix3d& m3 ) const
{
    return isScalar() ? getElement( 0, m3 ) : false;
}

bool
Uniform::get( osg::dmat4& m4 ) const
{
    return isScalar() ? getElement( 0, m4 ) : false;
}

bool
Uniform::get( osg::Matrix2x3d& m2x3 ) const
{
    return isScalar() ? getElement( 0, m2x3 ) : false;
}

bool
Uniform::get( osg::Matrix2x4d& m2x4 ) const
{
    return isScalar() ? getElement( 0, m2x4 ) : false;
}

bool
Uniform::get( osg::Matrix3x2d& m3x2 ) const
{
    return isScalar() ? getElement( 0, m3x2 ) : false;
}

bool
Uniform::get( osg::Matrix3x4d& m3x4 ) const
{
    return isScalar() ? getElement( 0, m3x4 ) : false;
}

bool
Uniform::get( osg::Matrix4x2d& m4x2 ) const
{
    return isScalar() ? getElement( 0, m4x2 ) : false;
}

bool
Uniform::get( osg::Matrix4x3d& m4x3 ) const
{
    return isScalar() ? getElement( 0, m4x3 ) : false;
}

bool
Uniform::get( int& i ) const
{
...
bool
Uniform::get( unsigned int& ui ) const
{
    return isScalar() ? getElement( 0, ui ) : false;
}

bool
Uniform::get( unsigned int& ui0,
...
bool
Uniform::get( unsigned long long& ull ) const
{
    return isScalar() ? getElement( 0, ull ) : false;
}

bool
Uniform::get( long long& ll ) const
{
...
bool
Uniform::setElement( unsigned int index,
                     float        f )
{
    ...
    unsigned int j =
        index * static_cast<unsigned int>( getTypeNumComponents( getType() ) );
    ( *_floatArray )[j] = f;
    dirty();
    return true;
}
                     ...
                     const osg::Matrix2& m2 )
{
    ...
    for( int i = 0; i < 4; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m2[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix3& m3 )
{
    ...
    for( int i = 0; i < 9; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m3[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::mat4& m4 )
{
    ...
    unsigned int j =
        index * static_cast<unsigned int>( getTypeNumComponents( getType() ) );
    const mat4::value_type* p = m4.data();
    for( int i = 0; i < 16; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = p[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix2x3& m2x3 )
{
    ...
    for( int i = 0; i < 6; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m2x3[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix2x4& m2x4 )
{
    ...
    for( int i = 0; i < 8; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m2x4[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix3x2& m3x2 )
{
    ...
    for( int i = 0; i < 6; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m3x2[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix3x4& m3x4 )
{
    ...
    for( int i = 0; i < 12; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m3x4[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix4x2& m4x2 )
{
    ...
    for( int i = 0; i < 8; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m4x2[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix4x3& m4x3 )
{
    ...
    for( int i = 0; i < 12; ++i )
    {
        ( *_floatArray )[j + static_cast<unsigned int>( i )] = m4x3[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix2d& m2 )
{
    ...
    for( int i = 0; i < 4; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m2[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix3d& m3 )
{
    ...
    for( int i = 0; i < 9; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m3[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::dmat4& m4 )
{
    ...
    {
        const dmat4::value_type* p = m4.data();
        for( int i = 0; i < 16; ++i )
        {
            ( *_doubleArray )[j + static_cast<unsigned int>( i )] = p[i];
        }
    }
    else    // if (_type == FLOAT_MAT4) for backward compatibility only
    {
        ...
        for( int i = 0; i < 16; ++i )
        {
            ( *_floatArray )[j + static_cast<unsigned int>( i )] =
                     ...
                     const osg::Matrix2x3d& m2x3 )
{
    ...
    for( int i = 0; i < 6; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m2x3[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix2x4d& m2x4 )
{
    ...
    for( int i = 0; i < 8; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m2x4[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix3x2d& m3x2 )
{
    ...
    for( int i = 0; i < 6; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m3x2[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix3x4d& m3x4 )
{
    ...
    for( int i = 0; i < 12; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m3x4[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix4x2d& m4x2 )
{
    ...
    for( int i = 0; i < 8; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m4x2[i];
    }
    dirty();
    return true;
}
                     ...
                     const osg::Matrix4x3d& m4x3 )
{
    ...
    for( int i = 0; i < 12; ++i )
    {
        ( *_doubleArray )[j + static_cast<unsigned int>( i )] = m4x3[i];
    }
    dirty();
    return true;
}
...
Uniform::setElement( unsigned int       index,
                     unsigned long long ull )
{
    if( index >= getNumElements() || !isCompatibleType( UNSIGNED_INT64 ) )
    {
        return false;
    }
    unsigned int j =
...
Uniform::setElement( unsigned int index,
                     long long    ll )
{
    if( index >= getNumElements() || !isCompatibleType( INT64 ) )
    {
        return false;
    }
    unsigned int j =
...
bool
Uniform::getElement( unsigned int index,
                     float&       f ) const
{
    if( index >= getNumElements() || !isCompatibleType( FLOAT ) )
    {
                     ...
                     osg::dmat4&  m4 ) const
{
    ...
    if( _type == DOUBLE_MAT4 )
    {
        m4 = osg::dmat4( &( ( *_doubleArray )[j] ) );
    }
    else    // if (_type == FLOAT_MAT4) for backward compatibility only
    {
        m4 = osg::dmat4( osg::mat4( &( ( *_floatArray )[j] ) ) );
    }
    return true;
}
...
void
Uniform::apply( const GLExtensions* ext,
                GLint               location ) const
{
    ...
    GLsizei          num = static_cast<GLsizei>( getNumElements() );
    if( num < 1 )
    {
        return;
    }
    ...
    switch( getGlApiType( getType() ) )
    {
        ...
        case FLOAT_MAT2 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix2fv( location,
                                         num,
                                         GL_FALSE,
                                         &_floatArray->front() );
            }
        ...
        case FLOAT_MAT3 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix3fv( location,
                                         num,
                                         GL_FALSE,
                                         &_floatArray->front() );
            }
        ...
        case FLOAT_MAT4 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix4fv( location,
                                         num,
                                         GL_FALSE,
                                         &_floatArray->front() );
            }
        ...
        case FLOAT_MAT2x3 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix2x3fv( location,
                                           num,
                                           GL_FALSE,
                                           &_floatArray->front() );
            }
        ...
        case FLOAT_MAT2x4 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix2x4fv( location,
                                           num,
                                           GL_FALSE,
                                           &_floatArray->front() );
            }
        ...
        case FLOAT_MAT3x2 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix3x2fv( location,
                                           num,
                                           GL_FALSE,
                                           &_floatArray->front() );
            }
        ...
        case FLOAT_MAT3x4 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix3x4fv( location,
                                           num,
                                           GL_FALSE,
                                           &_floatArray->front() );
            }
        ...
        case FLOAT_MAT4x2 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix4x2fv( location,
                                           num,
                                           GL_FALSE,
                                           &_floatArray->front() );
            }
        ...
        case FLOAT_MAT4x3 :
            if( _floatArray.valid() )
            {
                ext->glUniformMatrix4x3fv( location,
                                           num,
                                           GL_FALSE,
                                           &_floatArray->front() );
            }
        ...
        case DOUBLE_MAT2 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix2dv( location,
                                         num,
                                         GL_FALSE,
                                         &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT3 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix3dv( location,
                                         num,
                                         GL_FALSE,
                                         &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT4 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix4dv( location,
                                         num,
                                         GL_FALSE,
                                         &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT2x3 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix2x3dv( location,
                                           num,
                                           GL_FALSE,
                                           &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT2x4 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix2x4dv( location,
                                           num,
                                           GL_FALSE,
                                           &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT3x2 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix3x2dv( location,
                                           num,
                                           GL_FALSE,
                                           &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT3x4 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix3x4dv( location,
                                           num,
                                           GL_FALSE,
                                           &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT4x2 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix4x2dv( location,
                                           num,
                                           GL_FALSE,
                                           &_doubleArray->front() );
            }
        ...
        case DOUBLE_MAT4x3 :
            if( _doubleArray.valid() )
            {
                ext->glUniformMatrix4x3dv( location,
                                           num,
                                           GL_FALSE,
                                           &_doubleArray->front() );
            }
        ...
        case INT64 :
            if( _int64Array.valid() )
            {
                ext->glUniform1i64v( location, num, &_int64Array->front() );
            }
            break;
