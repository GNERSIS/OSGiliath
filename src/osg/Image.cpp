...
GLenum Image::computeFormatDataType(GLenum pixelFormat)
{
    switch (pixelFormat)
    {
        ...
        case GL_RGBA32UI_EXT:
        case GL_RGB32UI_EXT: return GL_UNSIGNED_INT;

        case GL_RGB16UI_EXT:
        case GL_RGBA16UI_EXT: return GL_UNSIGNED_SHORT;

        case GL_RGBA8UI_EXT:
        case GL_RGB8UI_EXT: return GL_UNSIGNED_BYTE;

        case GL_RGBA32I_EXT:
        case GL_RGB32I_EXT: return GL_INT;

        case GL_RGBA16I_EXT:
        case GL_RGB16I_EXT: return GL_SHORT;

        case GL_RGB8I_EXT:
        case GL_RGBA8I_EXT: return GL_BYTE;

        case GL_RGBA:
        case GL_RGB:
        case GL_RED:
        case GL_RG: return GL_UNSIGNED_BYTE;
...
Geode* osg::createGeodeForImage(osg::Image* image,float s,float t)
{
    if (image)
    {
        if (s>0 && t>0)
        {
            ...
            elements->push_back(0); elements->push_back(1); elements->push_back(3);
            elements->push_back(1); elements->push_back(2); elements->push_back(3);
...
Vec4 _readColor(GLenum pixelFormat, T* data,float scale)
{
    switch(pixelFormat)
    {
        case(GL_DEPTH_COMPONENT):   //intentionally fall through and execute the code for GL_RED
        case(GL_RED):               { float l = float(*data++)*scale; return Vec4(l, l, l, 1.0f); }
        case(GL_RG):                { float l = float(*data++)*scale; float a = float(*data++)*scale; return Vec4(l, l, l, a); }
...
void _writeColor(GLenum pixelFormat, T* data, float scale, const Vec4& c)
{
    ...
    {
    case(GL_DEPTH_COMPONENT):   //intentionally fall through and execute the code for GL_RED
    case(GL_RED):               { (*data++) = (T)(c[0] * scale); } break;
    case(GL_RG):                { (*data++) = (T)(c[0] * scale);  (*data++) = (T)(c[3] * scale); } break;
