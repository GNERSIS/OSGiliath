...
{
        bool
        operator()( const osg::ref_ptr<ImagePager::ImageRequest>& lhs,
                    const osg::ref_ptr<ImagePager::ImageRequest>& rhs ) const
        {
...
//  ReadQueue
//
ImagePager::ReadQueue::ReadQueue( ImagePager*        pager,
...
void
ImagePager::ReadQueue::add( ImagePager::ImageRequest* imageRequest )
{
    std::lock_guard<std::mutex> lock( _requestMutex );
    ...
    OSG_INFO << "ImagePager::ReadQueue::add(" << imageRequest->_fileName
             << "), size()=" << _requestList.size() << std::endl;

    updateBlock();
}

void
ImagePager::ReadQueue::takeFirst( osg::ref_ptr<ImageRequest>& databaseRequest )
{
    std::lock_guard<std::mutex> lock( _requestMutex );
    ...
    if( !_requestList.empty() )
    {
        ...
        OSG_INFO << "ImagePager::ReadQueue::takeFirst(..), size()="
...
ImagePager::ImageThread::ImageThread( const ImageThread& dt,
                                      ImagePager*        pager ) :
    _done( false ),
    _mode( dt._mode ),
    _pager( pager ),
...
ImagePager::ImageThread::cancel()
{
    ...
    if( isRunning() )
    {
        ...
        // release the frameBlock and _databasePagerThreadBlock in case its holding up
        // thread cancellation. _databasePagerThreadBlock->release();
...
void
ImagePager::signalBeginFrame( const osg::FrameStamp* framestamp )
{
    if( framestamp )
    {
...
void
ImagePager::signalEndFrame()
{
}

void
ImagePager::ImageThread::run()
{
    ...
    do
    {
        ...
        {
            // OSG_NOTICE<<"doing readImageFile("<<imageRequest->_fileName<<") index to
            // assign = "<<imageRequest->_attachmentIndex<<std::endl;
            osg::ref_ptr<osg::Image> image =
                osgDB::readRefImageFile( imageRequest->_fileName,
                                         imageRequest->_readOptions.get() );
            if( image.valid() )
            {
                // OSG_NOTICE<<"   successful
                // readImageFile("<<imageRequest->_fileName<<") index to assign =
                // "<<imageRequest->_attachmentIndex<<std::endl;

                osg::ImageSequence* is = dynamic_cast<osg::ImageSequence*>(
                    imageRequest->_attachmentPoint.get()
                );
                if( is )
                {
                    ...
                    {
                        is->setImage(
                            static_cast<unsigned int>( imageRequest->_attachmentIndex ),
                            image.get()
                        );
                    }
                    else
                ...
                else
                {
                    ...
                    std::lock_guard<std::mutex> lock(
                        _pager->_completedQueue->_requestMutex
                    );
    ...
    _done( false )
{
    ...
    _imageThreads.push_back(
        new ImageThread( this, ImageThread::HANDLE_ALL_REQUESTS, "Image Thread 1" )
    );
    _imageThreads.push_back(
        new ImageThread( this, ImageThread::HANDLE_ALL_REQUESTS, "Image Thread 2" )
    );
    _imageThreads.push_back(
        new ImageThread( this, ImageThread::HANDLE_ALL_REQUESTS, "Image Thread 3" )
    );
#if 0
...
ImagePager::cancel()
{
    ...
    for( ImageThreads::iterator itr = _imageThreads.begin(); itr != _imageThreads.end();
         ++itr )
    {
        ( *itr )->setDone( true );
    }

    // release the frameBlock and _databasePagerThreadBlock in case its holding up thread
    // cancellation.
    _readQueue->release();
...
ImagePager::readRefImageFile( const std::string&     fileName,
                              const osg::Referenced* options )
{
    osgDB::Options* readOptions =
        dynamic_cast<osgDB::Options*>( const_cast<osg::Referenced*>( options ) );
    return osgDB::readRefImageFile( fileName, readOptions );
}

void
ImagePager::requestImageFile( const std::string& fileName,
                              ...
                              const osg::Referenced*         options )
{
    ...
    bool alreadyAssigned = dynamic_cast<ImageRequest*>( imageRequest.get() ) &&
                           ( imageRequest->referenceCount() > 1 );
    if( alreadyAssigned )
    {
        // OSG_NOTICE<<"ImagePager::requestImageFile("<<fileName<<")
        // alreadyAssigned"<<std::endl;
        return;
    }
    ...
    // OSG_NOTICE<<"ImagePager::requestImageFile("<<fileName<<") new
    // request."<<std::endl;
...
bool
ImagePager::requiresUpdateSceneGraph() const
{
...
void
ImagePager::updateSceneGraph( const osg::FrameStamp& )
{
    std::lock_guard<std::mutex> lock( _completedQueue->_requestMutex );
         ...
         ++itr )
    {
        ...
        osg::Texture* texture =
            dynamic_cast<osg::Texture*>( imageRequest->_attachmentPoint.get() );
        if( texture )
        {
            int attachmentIndex =
                imageRequest->_attachmentIndex > 0 ? imageRequest->_attachmentIndex : 0;
            texture->setImage( static_cast<unsigned int>( attachmentIndex ),
                               imageRequest->_loadedImage.get() );
        }
        else
        {
            OSG_NOTICE << "ImagePager::updateSceneGraph() : error, image request "
                          "attachment type not handled yet."
                       << std::endl;
        }
    }
