 ...
 * Thread-safe cache of loaded objects keyed by filename.
 * Prevents duplicate loading and provides explicit expiry.
 */
...
bool
ObjectCache::ClassComp::operator()( const ObjectCache::FileNameOptionsPair& lhs,
...
void
ObjectCache::addObjectCache( ObjectCache* objectCache )
{
    ...
    // lock both ObjectCache to prevent their contents from being modified by other
    // threads while we merge.
    std::lock_guard<std::mutex> lock1( _objectCacheMutex );
    std::lock_guard<std::mutex> lock2( objectCache->_objectCacheMutex );

    OSG_DEBUG << "Inserting objects to main ObjectCache "
              << objectCache->_objectCache.size() << std::endl;

    _objectCache.insert( objectCache->_objectCache.begin(),
                         objectCache->_objectCache.end() );
}

void
ObjectCache::addEntryToObjectCache( const std::string& filename,
                                    osg::Object*       object,
                                    double             timestamp,
                                    const Options*     options )
{
    ...
    _objectCache[FileNameOptionsPair( filename, options ? osg::clone( options ) : 0 )] =
        ObjectTimeStampPair( object, timestamp );
    OSG_DEBUG << "Adding " << filename << " with options '"
              << ( options ? options->getOptionString() : "" ) << "' to ObjectCache "
              << this << std::endl;
}

ObjectCache::ObjectCacheMap::iterator
ObjectCache::find( const std::string&    fileName,
                   const osgDB::Options* options )
{
    for( ObjectCacheMap::iterator itr = _objectCache.begin(); itr != _objectCache.end();
         ++itr )
    {
        if( itr->first.first == fileName )
        {
            ...
            {
                if( options && *( itr->first.second ) == *options )
                {
                    return itr;
                }
            }
...
osg::Object*
ObjectCache::getFromObjectCache( const std::string& fileName,
                                 const Options*     options )
{
    std::lock_guard<std::mutex> lock( _objectCacheMutex );
    ...
    if( itr != _objectCache.end() )
    {
        ...
        if( o.valid() )
        {
            OSG_DEBUG << "Found " << fileName << " with options '"
                      << o->getOptionString() << "' in ObjectCache " << this
                      << std::endl;
        }
        else
        {
...
ObjectCache::getRefFromObjectCache( const std::string& fileName,
                                    const Options*     options )
{
    std::lock_guard<std::mutex> lock( _objectCacheMutex );
    ObjectCacheMap::iterator    itr = find( fileName, options );
...
void
ObjectCache::updateTimeStampOfObjectsInCacheWithExternalReferences(
...
void
ObjectCache::removeExpiredObjectsInCache( double expiryTime )
{
    std::lock_guard<std::mutex> lock( _objectCacheMutex );
...
void
ObjectCache::removeFromObjectCache( const std::string& fileName,
                                    const Options*     options )
{
    std::lock_guard<std::mutex> lock( _objectCacheMutex );
    ObjectCacheMap::iterator    itr = find( fileName, options );
...
namespace ObjectCacheUtils
{
    ...
    struct ContainsUnreffedTextures : public osg::DualModeVisitor
    {
            ...
            bool
            check( const osg::Texture* texture )
            {
                if( !texture )
                {
                    return false;
            ...
            bool
            check( const osg::StateSet* stateset )
            {
                for( unsigned int i = 0; i < stateset->getNumTextureAttributeLists();
                     ++i )
                {
                    const osg::StateAttribute* sa = stateset->getTextureAttribute(
                        i,
                        osg::StateAttribute::Type::TEXTURE
                    );
                    if( sa && check( sa->asTexture() ) )
                    {
                        return true;
                    }
                }
            ...
            bool
            check( osg::Object* object )
            {
                if( object->asStateAttribute() )
                {
                    return check( dynamic_cast<const osg::Texture*>( object ) );
                }
                if( object->asStateSet() )
                {
                    return check( object->asStateSet() );
                }
                if( !object->asNode() )
                {
                    return false;
...
void
ObjectCache::releaseGLObjects( osg::State* state )
{
    std::lock_guard<std::mutex>                lock( _objectCacheMutex );
