 ...
 * Background scene graph loader. Pages in subgraphs (ProxyNode,
 * PagedLOD) from disk using worker threads.
 */
...
static osg::ApplicationUsageProxy DatabasePager_e0(
...
static osg::ApplicationUsageProxy DatabasePager_e3(
    osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
    "OSG_DATABASE_PAGER_DRAWABLE <mode>",
    "Set the drawable policy for setting of loaded drawable to specified type.  mode "
    "can be one of DoNotModify, VBO or VertexArrays>."
);
static osg::ApplicationUsageProxy DatabasePager_e4(
    osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
...
static osg::ApplicationUsageProxy
    DatabasePager_e11( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                       "OSG_MAX_PAGEDLOD <num>",
                       "Set the target maximum number of PagedLOD to maintain." );
static osg::ApplicationUsageProxy
    DatabasePager_e12( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                       "OSG_ASSIGN_PBO_TO_IMAGES <ON/OFF>",
                       "Set whether PixelBufferObjects should be assigned to Images to "
                       "aid download to the GPU." );
    ...
    : public osgUtil::IncrementalCompileOperation::CompileCompletedCallback
{
        DatabasePagerCompileCompletedCallback(
            ...
            osgDB::DatabasePager::DatabaseRequest* databaseRequest
        ) :
            _pager( pager ),
        ...
        virtual bool
        compileCompleted( osgUtil::IncrementalCompileOperation::
...
void
DatabasePager::compileCompleted( DatabaseRequest* databaseRequest )
{
...
class DatabasePager::ExpirePagedLODsVisitor : public osg::DualModeVisitor
{
    public:
        ...
        {
            return "ExpirePagedLODsVisitor";
        }

        void
        apply( osg::PagedLOD& plod ) override
        {
        ...
        // subgraphs.
        bool
        removeExpiredChildrenAndFindPagedLODs( osg::PagedLOD* plod,
                                               double         expiryTime,
    ...
    private:
        ...
        void
        markRequestsExpired( osg::PagedLOD* plod )
        {
            unsigned numFiles = plod->getNumFileNames();
            for( unsigned i = 0; i < numFiles; ++i )
            {
                DatabasePager::DatabaseRequest* request =
                    dynamic_cast<DatabasePager::DatabaseRequest*>(
                        plod->getDatabaseRequest( i ).get()
                    );
                if( request )
                {
...
class SetBasedPagedLODList : public DatabasePager::PagedLODList
{
    public:
        ...
        size()
        {
            return static_cast<unsigned int>( _pagedLODs.size() );
        }

        virtual void
        removeExpiredChildren( int                        numberChildrenToRemove,
                               double                     expiryTime,
                               unsigned int               expiryFrame,
                               DatabasePager::ObjectList& childrenRemoved,
                               bool                       visitActive )
        {
                 ...
                 itr != _pagedLODs.end() && leftToRemove > 0; )
            {
                ...
                {
                    bool plodActive =
                        expiryFrame < plod->getFrameNumberOfLastTraversal();
                    if( visitActive ==
                        plodActive )    // true if (visitActive && plodActive) OR
                                        // (!visitActive &&!plodActive)
                    {
                        ...
                        std::copy( expiredChildren.begin(),
                                   expiredChildren.end(),
                                   std::back_inserter( childrenRemoved ) );
                    }
                ...
                else
                {
                    ...
                    // observer pointers.
                    leftToRemove--;
                    OSG_INFO << "DatabasePager::removeExpiredSubgraphs() "
                                "_inactivePagedLOD has been invalidated, but ignored"
                             << std::endl;
                }
            }
        }
        ...
        virtual void
        removeNodes( osg::NodeList& nodesToRemove )
        {
        ...
        virtual void
        insertPagedLOD( const osg::observer_ptr<osg::PagedLOD>& plod )
        {
            if( _pagedLODs.count( plod ) != 0 )
            {
                OSG_NOTICE << "Warning: SetBasedPagedLODList::insertPagedLOD("
                           << plod.get() << ") already inserted" << std::endl;
                // abort();
                return;
            }

            // OSG_NOTICE<<"OK: SetBasedPagedLODList::insertPagedLOD("<<plod<<")
            // inserting"<<std::endl;
        ...
        virtual bool
        containsPagedLOD( const osg::observer_ptr<osg::PagedLOD>& plod ) const
        {
...
class DatabasePager::FindCompileableGLObjectsVisitor : public osgUtil::StateToCompile
{
    public:
            ...
            switch( _pager->_drawablePolicy )
            {
                ...
                case DatabasePager::USE_VERTEX_ARRAYS :
                    ...
                    _mode =
                        _mode |
                        static_cast<unsigned int>(
                            osgUtil::GLObjectsVisitor::SWITCH_OFF_VERTEX_BUFFER_OBJECTS
                        );
                    break;
            }

            if( osgDB::Registry::instance()->getBuildKdTreesHint() ==
        ...
        className() const override
        {
            return "FindCompileableGLObjectsVisitor";
        }

        bool
        requiresCompilation() const
        {
            return !empty();
        }
        ...
        void
        apply( osg::Texture& texture ) override
        {
            // apply any changes if the texture is not static.
            if( texture.getDataVariance() !=
    ...
    protected:
        ...
        FindCompileableGLObjectsVisitor&
        operator=( const FindCompileableGLObjectsVisitor& )
        {
            return *this;
        }
};
...
{
        bool
        operator()( const osg::ref_ptr<DatabasePager::DatabaseRequest>& lhs,
...
{
    OSG_INFO << "DatabasePager::RequestQueue::~RequestQueue() Destructing queue."
             << std::endl;
    for( RequestList::iterator itr = _requestList.begin(); itr != _requestList.end();
         ++itr )
    {
        invalidate( itr->get() );
    }
...
void
DatabasePager::RequestQueue::invalidate( DatabaseRequest* dr )
{
    // OSG_NOTICE<<"DatabasePager::RequestQueue::invalidate(DatabaseRequest* dr)
    // dr->_compileSet="<<dr->_compileSet.get()<<std::endl;
    // XXX _dr_mutex?
...
bool
DatabasePager::RequestQueue::pruneOldRequestsAndCheckIfEmpty()
{
    ...
    if( _frameNumberLastPruned != frameNumber )
    {
        ...
        {
            std::lock_guard<std::mutex> drLock( _pager->_dr_mutex );
            if( ( *citr )->isRequestCurrent( static_cast<int>( frameNumber ) ) )
            {
                ++citr;
            }
            else
            {
                ...
                OSG_INFO << "DatabasePager::RequestQueue::"
                            "pruneOldRequestsAndCheckIfEmpty(): Pruning "
                         << citr->get() << std::endl;
                citr = _requestList.erase( citr );
            }
        }

        _frameNumberLastPruned = static_cast<unsigned int>( frameNumber );
...
bool
DatabasePager::RequestQueue::empty()
{
    std::lock_guard<std::mutex> lock( _requestMutex );
    return _requestList.empty();
}
...
unsigned int
DatabasePager::RequestQueue::size()
{
    std::lock_guard<std::mutex> lock( _requestMutex );
    return static_cast<unsigned int>( _requestList.size() );
}

void
DatabasePager::RequestQueue::clear()
{
    std::lock_guard<std::mutex> lock( _requestMutex );

    for( RequestList::iterator citr = _requestList.begin(); citr != _requestList.end();
         ++citr )
    {
        std::lock_guard<std::mutex> drLock( _pager->_dr_mutex );
        invalidate( citr->get() );
...
void
DatabasePager::RequestQueue::add( DatabasePager::DatabaseRequest* databaseRequest )
{
    std::lock_guard<std::mutex> lock( _requestMutex );
...
void
DatabasePager::RequestQueue::remove( DatabasePager::DatabaseRequest* databaseRequest )
{
    // OSG_NOTICE<<"DatabasePager::RequestQueue::remove(DatabaseRequest*
    // databaseRequest)"<<std::endl;
    std::lock_guard<std::mutex> lock( _requestMutex );
    for( RequestList::iterator citr = _requestList.begin(); citr != _requestList.end();
         ++citr )
    {
...
void
DatabasePager::RequestQueue::addNoLock( DatabasePager::DatabaseRequest* databaseRequest )
{
...
void
DatabasePager::RequestQueue::swap( RequestList& requestList )
{
    std::lock_guard<std::mutex> lock( _requestMutex );
...
void
DatabasePager::RequestQueue::takeFirst( osg::ref_ptr<DatabaseRequest>& databaseRequest )
{
    std::lock_guard<std::mutex> lock( _requestMutex );
    ...
    if( !_requestList.empty() )
    {
        ...
        int frameNumber = static_cast<int>( _pager->_frameNumber );
             ...
             citr != _requestList.end(); )
        {
            ...
            if( ( *citr )->isRequestCurrent( static_cast<int>( frameNumber ) ) )
            {
                if( selected_itr ==
                    _requestList.end() ||
                    highPriority( *citr, *selected_itr ) )
                {
                    selected_itr = citr;
                }
        ...
        if( selected_itr != _requestList.end() )
        {
            ...
            OSG_INFO << " DatabasePager::RequestQueue::takeFirst() Found "
                        "DatabaseRequest size()="
                     << _requestList.size() << std::endl;
        }
        else
        {
            OSG_INFO << " DatabasePager::RequestQueue::takeFirst() No suitable "
                        "DatabaseRequest found size()="
                     << _requestList.size() << std::endl;
        }
...
//  ReadQueue
//
DatabasePager::ReadQueue::ReadQueue( DatabasePager*     pager,
...
void
DatabasePager::ReadQueue::updateBlock()
{
...
DatabasePager::DatabaseThread::DatabaseThread( const DatabaseThread& dt,
                                               DatabasePager*        pager ) :
    _done( false ),
    _active( false ),
    _pager( pager ),
...
int
DatabasePager::DatabaseThread::cancel()
{
    int result = 0;
...
DatabasePager::DatabaseThread::run()
{
    ...
    do
    {
        ...
        OSG_INFO << _name << ": _pager->size()= " << read_queue->size()
                 << " to delete = " << read_queue->_childrenToDeleteList.size()
                 << std::endl;
        ...
        if(
            _pager
                ->_deleteRemovedSubgraphsInDatabaseThread /* &&
                                                             !(read_queue->_childrenToDeleteList.empty())*/
        )
        {
            ObjectList deleteList;
            {
                // Don't hold lock during destruction of deleteList
                std::lock_guard<std::mutex> lock( read_queue->_requestMutex );
        ...
        osg::ref_ptr<FileLocationCallback> fileLocationCallback =
            osgDB::Registry::instance()->getFileLocationCallback();
        ...
        {
            {
                ...
                frameNumberLastRequest =
                    static_cast<int>( databaseRequest->_frameNumberLastRequest );
            }
            ...
            // disable the FileCache if the fileLocationCallback tells us that it isn't
            ...
            cacheNodes = ( dr_loadOptions->getObjectCacheHint() &
                           osgDB::Options::CACHE_NODES ) != 0;
            if( cacheNodes )
            {
                // OSG_NOTICE<<"Checking main ObjectCache"<<std::endl;
                //  check the object cache to see if the file we want has already been
                //  loaded.
                osg::ref_ptr<osg::Object> objectFromCache =
                    osgDB::Registry::instance()->getRefFromObjectCache( fileName );

                // if no object with fileName in ObjectCache then try the filename
                // appropriate for fileCache
                if( !objectFromCache &&
                    ( fileCache.valid() &&
                      fileCache->isFileAppropriateForFileCache( fileName ) ) )
                {
                    ...
                    {
                        objectFromCache =
                            osgDB::Registry::instance()->getRefFromObjectCache(
                                fileCache->createCacheFileName( fileName )
                            );
                    }
                }

                osg::Node* modelFromCache =
                    dynamic_cast<osg::Node*>( objectFromCache.get() );
                if( modelFromCache )
                {
                    ...
                    // move the request to the dataToMerge list so it can be merged
                    // during the update phase of the frame.
                    {
                    ...
                    // skip the rest of the do/while loop as we have done all the
                ...
                // need to disable any attempt to use the cache when loading as we're
                // handle this ourselves to avoid threading conflicts
                {
                    std::lock_guard<std::mutex> drLock( _pager->_dr_mutex );
                    databaseRequest->_objectCache = new ObjectCache;
                    dr_loadOptions->setObjectCache(
                        databaseRequest->_objectCache.get()
                    );
                }
            }

            // check if databaseRequest is still relevant
            if( ( static_cast<int>( _pager->_frameNumber ) - frameNumberLastRequest ) <=
                1 )
            {
                ...
                switch( _mode )
                {
                    ...
                    case( HANDLE_NON_HTTP ) :
                        {
                            ...
                            {
                                isHighLatencyFileRequest =
                                    fileLocationCallback->fileLocation(
                                        fileName,
                                        dr_loadOptions.get()
                                    ) == FileLocationCallback::REMOTE_FILE;
                            }
                            else if( fileCache.valid() &&
                                     fileCache->isFileAppropriateForFileCache(
                                         fileName
                                     ) )
                            {
                            ...
                            {
                                if( fileCache.valid() &&
                                    fileCache->existsInCache( fileName ) )
                                {
                                    readFromFileCache = true;
                                }
                                else
                                {
                                    OSG_INFO << _name << ": Passing http requests over "
                                             << fileName << std::endl;
                    ...
                    case( HANDLE_ONLY_HTTP ) :
                        {
                            // accept all requests, as we'll assume only high latency
                            // requests will have got here.
                            break;
                        }
                }
            }
        ...
        if( databaseRequest.valid() )
        {
            ...
            // it is created this thread is the only one to write to the _loadedModel
            // pointer.
            // OSG_NOTICE<<"In DatabasePager thread
            // readNodeFile("<<databaseRequest->_fileName<<")"<<std::endl; osg::Timer_t
            // before = osg::Timer::instance()->tick();

            // assume that readNode is thread safe...
            ReaderWriter::ReadResult rr =
                readFromFileCache
                    ? fileCache->readNode( fileName, dr_loadOptions.get(), false )
                    : Registry::instance()->readNode( fileName,
                                                      dr_loadOptions.get(),
                                                      false );
            ...
            if( !rr.success() )
            {
                OSG_WARN << "Error in reading file " << fileName << " : "
...

            {
                ...
                if( ( _pager->_frameNumber - databaseRequest->_frameNumberLastRequest ) >
                    1 )
                {
                    OSG_INFO << _name << ": Warning DatabaseRquest no longer required."
                             << std::endl;
                    loadedModel = 0;
                }
            }

            // OSG_NOTICE<<"     node read in
            ...
            if( loadedModel.valid() )
            {
                ...
                if( !rr.loadedFromCache() )
                {
                    ...
                    DatabasePager::FindCompileableGLObjectsVisitor stateToCompile(
                        _pager,
                    ...
                    loadedObjectsNeedToBeCompiled =
                        _pager->_doPreCompile &&
                        _pager->_incrementalCompileOperation.valid() &&
                        _pager->_incrementalCompileOperation->requiresCompile(
                            stateToCompile
                        );

                    // move the databaseRequest from the front of the fileRequest to the
                    // end of dataToCompile or dataToMerge lists.
                    if( loadedObjectsNeedToBeCompiled )
                    {
                        ...
                        compileSet->buildCompileMap(
                            _pager->_incrementalCompileOperation->getContextSet(),
                            stateToCompile
                        );
                        compileSet->_compileCompletedCallback =
                            new DatabasePagerCompileCompletedCallback(
                                _pager,
                                databaseRequest.get()
                            );
                        _pager->_incrementalCompileOperation->add( compileSet.get(),
                                                                   false );
                    }
                }
                else
...
DatabasePager::DatabasePager()
{
    ...
    {
        _assignPBOToImages = strcmp( str, "yes" ) ==
                             0 ||
                             strcmp( str, "YES" ) ==
                             0 ||
                             strcmp( str, "on" ) ==
        ...
        OSG_NOTICE << "OSG_ASSIGN_PBO_TO_IMAGES set to " << _assignPBOToImages
                   << std::endl;
    }
    ...
    {
        _deleteRemovedSubgraphsInDatabaseThread = strcmp( str, "yes" ) ==
                                                  0 ||
                                                  strcmp( str, "YES" ) ==
                                                  0 ||
                                                  strcmp( str, "on" ) ==
    ...
    {
        _targetMaximumNumberOfPageLOD = static_cast<unsigned int>( atoi( str ) );
        OSG_NOTICE << "_targetMaximumNumberOfPageLOD = " << _targetMaximumNumberOfPageLOD
                   << std::endl;
    }
    ...
    // _affinity = osg::Thread::Affinity(4,4);
    ...
    if( str )
    {
        ...
        {
            setSchedulePriority( osg::Thread::THREAD_PRIORITY_DEFAULT );
        }
        else if( strcmp( str, "MIN" ) == 0 )
        {
            setSchedulePriority( osg::Thread::THREAD_PRIORITY_MIN );
        }
        else if( strcmp( str, "LOW" ) == 0 )
        {
            setSchedulePriority( osg::Thread::THREAD_PRIORITY_LOW );
        }
        else if( strcmp( str, "NOMINAL" ) == 0 )
        {
            setSchedulePriority( osg::Thread::THREAD_PRIORITY_NOMINAL );
        }
        else if( strcmp( str, "HIGH" ) == 0 )
        {
            setSchedulePriority( osg::Thread::THREAD_PRIORITY_HIGH );
        }
        else if( strcmp( str, "MAX" ) == 0 )
        {
            setSchedulePriority( osg::Thread::THREAD_PRIORITY_MAX );
        }
    }
...
DatabasePager::DatabasePager( const DatabasePager& rhs )
{
    ...
    _deleteRemovedSubgraphsInDatabaseThread =
        rhs._deleteRemovedSubgraphsInDatabaseThread;
...
void
DatabasePager::setIncrementalCompileOperation(
...
{
    return DatabasePager::prototype().valid() ? DatabasePager::prototype()->clone()
                                              : new DatabasePager;
}

void
DatabasePager::setProcessorAffinity( const osg::Thread::Affinity& affinity )
{
    _affinity = affinity;
...
void
DatabasePager::setUpThreads( unsigned int totalNumThreads,
                             unsigned int numHttpThreads )
{
    _databaseThreads.clear();

    unsigned int numGeneralThreads =
        numHttpThreads < totalNumThreads ? totalNumThreads - numHttpThreads : 1;

    if( numHttpThreads == 0 )
    {
        for( unsigned int i = 0; i < numGeneralThreads; ++i )
        {
            addDatabaseThread( DatabaseThread::HANDLE_ALL_REQUESTS,
                               "HANDLE_ALL_REQUESTS" );
        }
    }
    else
...
unsigned int
DatabasePager::addDatabaseThread( DatabaseThread::Mode mode,
                                  const std::string&   name )
{
...
int
DatabasePager::setSchedulePriority( osg::Thread::ThreadPriority priority )
{
    int result = 0;
...
bool
DatabasePager::getRequestsInProgress() const
{
...
void
DatabasePager::requestNodeFile( const std::string&             fileName,
...
{
    osgDB::Options* loadOptions =
        dynamic_cast<osgDB::Options*>( const_cast<osg::Referenced*>( options ) );
    if( !loadOptions )
    {
    ...
    if( nodePath.empty() )
    {
        OSG_NOTICE << "Warning: DatabasePager::requestNodeFile(..) passed empty "
                      "NodePath, so nowhere to attach new subgraph to."
                   << std::endl;
        return;
    }
    ...
    {
        OSG_NOTICE
            << "Warning: DatabasePager::requestNodeFile(..) passed NodePath without "
               "group as last node in path, so nowhere to attach new subgraph to."
            << std::endl;
        return;
    }

    osg::Node* terrain = 0;
    for( osg::NodePath::reverse_iterator itr = nodePath.rbegin(); itr != nodePath.rend();
         ++itr )
    {
        if( ( *itr )->asTerrain() )
        {
    ...
    unsigned int frameNumber = framestamp ? framestamp->getFrameNumber()
                                          : static_cast<unsigned int>( _frameNumber );
...
// #define WITH_REQUESTNODEFILE_TIMING
    ...
    {
        OSG_NOTICE << "requestNodeFiles for " << previousFrame << " time = " << totalTime
                   << std::endl;
    ...
    if( databaseRequestRef.valid() )
    {
        DatabaseRequest* databaseRequest =
            dynamic_cast<DatabaseRequest*>( databaseRequestRef.get() );
        bool requeue = false;
        if( databaseRequest )
        {
            std::lock_guard<std::mutex> drLock( _dr_mutex );
            if( !( databaseRequest->valid() ) )
            {
                OSG_INFO << "DatabaseRequest has been previously invalidated whilst "
                            "still attached to scene graph."
                         << std::endl;
                databaseRequest = 0;
            }
            else
    ...
    if( !foundEntry )
    {
        ...
        std::lock_guard<std::mutex> lock( _fileRequestQueue->_requestMutex );
...
#ifdef WITH_REQUESTNODEFILE_TIMING
    totalTime +=
        osg::Timer::instance()->delta_m( start_tick, osg::Timer::instance()->tick() );
#endif
}

void
DatabasePager::signalBeginFrame( const osg::FrameStamp* framestamp )
{
#if 0
    ...
    if( framestamp )
    {
        ...
        // OSG_INFO << "signalBeginFrame
        // "<<framestamp->getFrameNumber()<<">>>>>>>>>>>>>>>>"<<std::endl;
...
void
DatabasePager::setDatabasePagerThreadPause( bool pause )
{
    ...
    {
        std::lock_guard<std::mutex> lock( _httpRequestQueue->_requestMutex );
        _httpRequestQueue->updateBlock();
    }
}

bool
DatabasePager::requiresUpdateSceneGraph() const
{
    return ( getDataToMergeListSize() > 0 );
}

void
DatabasePager::updateSceneGraph( const osg::FrameStamp& frameStamp )
{
...
#define UPDATE_TIMING 0
...
#if UPDATE_TIMING
        timeFor_addLoadedDataToSceneGraph =
            timer.elapsedTime_m() - timeFor_removeExpiredSubgraphs;
#endif
    }
    ...
    if( elapsedTime > 0.4 )
    {
        OSG_NOTICE << "DatabasePager::updateSceneGraph() total time = " << elapsedTime
                   << "ms" << std::endl;
        OSG_NOTICE << "   timeFor_removeExpiredSubgraphs    = "
                   << timeFor_removeExpiredSubgraphs << "ms" << std::endl;
        OSG_NOTICE << "   timeFor_addLoadedDataToSceneGraph = "
                   << timeFor_addLoadedDataToSceneGraph << "ms" << std::endl;
        OSG_NOTICE << "   _activePagedLODList.size()        = "
                   << _activePagedLODList->size() << std::endl;
        OSG_NOTICE << "   _inactivePagedLODList.size()      = "
                   << _inactivePagedLODList->size() << std::endl;
        OSG_NOTICE << "   total                             = "
                   << _activePagedLODList->size() +
            _inactivePagedLODList->size()
                   << std::endl;
    }
#endif
}
...
bool
DatabasePager::requiresRedraw() const
{
    return ( getDataToCompileListSize() > 0 );
}

void
DatabasePager::addLoadedDataToSceneGraph( const osg::FrameStamp& frameStamp )
{
         ...
         ++itr )
    {
        ...
        if( !databaseRequest->_groupExpired && databaseRequest->_group.lock( group ) )
        {
            ...
            {
                osgDB::Registry::instance()->getSharedStateManager()->share(
                    databaseRequest->_loadedModel.get()
                );
            }
            ...
            // Check if parent plod was already registered if not start visitor from
            // parent
            ...
            // OSG_NOTICE<<"merged subgraph"<<databaseRequest->_fileName<<" after
            // "<<databaseRequest->_numOfRequests<<" requests and
            // time="<<(timeStamp-databaseRequest->_timestampFirstRequest)*1000.0<<std::endl;
        ...
        {
            OSG_INFO << "DatabasePager::addLoadedDataToSceneGraph() node in parental "
                        "chain deleted, discarding subgaph."
                     << std::endl;
        }
            ...
            osgDB::Registry::instance()->getObjectCache() )
        {
            ...
            osgDB::Registry::instance()->getObjectCache()->addObjectCache(
                databaseRequest->_objectCache.get()
            );
        ...
        // OSG_NOTICE<<"curr = "<<timeToMerge<<" min
        // "<<getMinimumTimeToMergeTile()*1000.0<<" max =
        // "<<getMaximumTimeToMergeTile()*1000.0<<" average =
        // "<<getAverageTimToMergeTiles()*1000.0<<std::endl;
    }
    ...
    if( !localFileLoadedList.empty() )
    {
        OSG_INFO << "Done DatabasePager::addLoadedDataToSceneGraph"
                 ...
                 << osg::Timer::instance()->delta_m( mid, last ) << "ms" << "  objects"
                 << localFileLoadedList.size() << std::endl
                 << std::endl;
    }
}

void
DatabasePager::removeExpiredSubgraphs( const osg::FrameStamp& frameStamp )
{
    ...
    int numToPrune = static_cast<int>( numPagedLODs - _targetMaximumNumberOfPageLOD );
    ...
    if( numToPrune > 0 )
    {
        _activePagedLODList->removeExpiredChildren( numToPrune,
                                                    expiryTime,
                                                    expiryFrame,
                                                    childrenRemoved,
                                                    false );
    ...
    numToPrune =
        static_cast<int>( _activePagedLODList->size() - _targetMaximumNumberOfPageLOD );
    if( numToPrune > 0 )
    {
        _activePagedLODList->removeExpiredChildren( numToPrune,
                                                    expiryTime,
    ...
    OSG_INFO << "active=" << _activePagedLODList->size()
             << " overall = " << osg::Timer::instance()->delta_m( startTick, end_c_Tick )
             << " A=" << time_a << " avg=" << s_total_time_stage_a /
        s_total_iter_stage_a
             << " max = " << s_total_max_stage_a << " B=" << time_b
             << " avg=" << s_total_time_stage_b /
        s_total_iter_stage_b
             << " max = " << s_total_max_stage_b << " C=" << time_c
             << " avg=" << s_total_time_stage_c /
        s_total_iter_stage_c
             << " max = " << s_total_max_stage_c << std::endl;
}

class DatabasePager::FindPagedLODsVisitor : public osg::DualModeVisitor
{
    public:
    ...
    protected:
        ...
        operator=( const FindPagedLODsVisitor& )
        {
            return *this;
        }
};
...
void
DatabasePager::registerPagedLODs( osg::Node*   subgraph,
                                  unsigned int frameNumber )
{
    if( !subgraph )
    {
