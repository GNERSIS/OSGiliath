...
void
SharedStateManager::setShareMode( unsigned int mode )
{
    _shareMode = mode;

    _shareTexture[static_cast<int>( osg::Object::DataVariance::DYNAMIC )] =
        ( _shareMode & SHARE_DYNAMIC_TEXTURES ) != 0;
    _shareTexture[static_cast<int>( osg::Object::DataVariance::STATIC )] =
        ( _shareMode & SHARE_STATIC_TEXTURES ) != 0;
    _shareTexture[static_cast<int>( osg::Object::DataVariance::UNSPECIFIED )] =
        ( _shareMode & SHARE_UNSPECIFIED_TEXTURES ) != 0;

    _shareStateSet[static_cast<int>( osg::Object::DataVariance::DYNAMIC )] =
        ( _shareMode & SHARE_DYNAMIC_STATESETS ) != 0;
    _shareStateSet[static_cast<int>( osg::Object::DataVariance::STATIC )] =
        ( _shareMode & SHARE_STATIC_STATESETS ) != 0;
    _shareStateSet[static_cast<int>( osg::Object::DataVariance::UNSPECIFIED )] =
        ( _shareMode & SHARE_UNSPECIFIED_STATESETS ) != 0;
}
...
//----------------------------------------------------------------
void
SharedStateManager::share( osg::Node*  node,
                           std::mutex* mt )
{
    ...
    //  std::cout << "SHARING TIME =
    //  "<<timer.delta_m(start_tick,end_tick)<<"ms"<<std::endl; std::cout << "
    //  _sharedStateSetList.size() = "<<_sharedStateSetList.size()<<std::endl; std::cout
    //  << "   _sharedTextureList.size() = "<<_sharedTextureList.size()<<std::endl;
}
...
//----------------------------------------------------------------
void
SharedStateManager::apply( osg::Node& node )
{
...
bool
SharedStateManager::isShared( osg::StateSet* ss )
{
...
bool
SharedStateManager::isShared( osg::Texture* texture )
{
...
osg::StateSet*
SharedStateManager::find( osg::StateSet* ss )
{
...
osg::StateAttribute*
SharedStateManager::find( osg::StateAttribute* sa )
{
...
//----------------------------------------------------------------
void
SharedStateManager::setStateSet( osg::StateSet* ss,
                                 osg::Object*   object )
{
...
//----------------------------------------------------------------
void
SharedStateManager::shareTextures( osg::StateSet* ss )
{
    const osg::StateSet::TextureAttributeList& texAttributes =
        ss->getTextureAttributeList();
    for( unsigned int unit = 0; unit < texAttributes.size(); ++unit )
    {
        osg::StateSet::RefAttributePair* pair =
            ss->getTextureAttributePair( unit, osg::StateAttribute::Type::TEXTURE );
        if( !pair )
        {
            continue;
        }
        ...
        {
            TextureTextureSharePairMap::iterator titr =
                tmpSharedTextureList.find( texture );
            if( titr == tmpSharedTextureList.end() )
            {
                // Texture is not in tmp list:
                // First time it appears in this file, search Texture in
                // sharedAttributeList
                ...
                if( textureFromSharedList )
                {
                    ...
                    tmpSharedTextureList[texture] =
                        TextureSharePair( textureFromSharedList, true );
                }
                else
                {
...
//----------------------------------------------------------------
void
SharedStateManager::process( osg::StateSet* ss,
                             osg::Object*   parent )
{
    ...
    else if( _shareMode & ( SHARE_DYNAMIC_TEXTURES |
                            SHARE_STATIC_TEXTURES |
                            SHARE_UNSPECIFIED_TEXTURES ) )
    {
        shareTextures( ss );
...
void
SharedStateManager::releaseGLObjects( osg::State* state ) const
{
    std::lock_guard<std::mutex> lock( _listMutex );
