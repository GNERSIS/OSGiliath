...
DynamicLibrary::loadLibrary( const std::string& libraryName )
{
    ...
    OSG_DEBUG << "DynamicLibrary::try to load library \"" << libraryName << "\""
              << std::endl;
    ...
    else
    {
        handle = getLibraryHandle(
            libraryName
        );    // haven't found a lib ourselves, see if the OS can find it simply from the
              // library name.
    }
...
DynamicLibrary::getLibraryHandle( const std::string& libraryName )
{
    ...
    if( NSCreateObjectFileImageFromFile( libraryName.c_str(), &image ) ==
    ...
    handle =
        shl_load( libraryName.c_str(), BIND_DEFERRED | BIND_FIRST | BIND_VERBOSE, 0 );
    return handle;
#else    // other unix
    ...
    if( handle == NULL )
    {
        ...
        {
            OSG_WARN << "Warning: dynamic library '" << libraryName
        ...
        {
            OSG_INFO << "Warning: dynamic library '" << libraryName
                     << "' does not exist (or isn't readable):" << std::endl;
...
DynamicLibrary::PROC_ADDRESS
DynamicLibrary::getProcAddress( const std::string& procName )
{
    if( _handle == NULL )
    {
        return NULL;
...
#if defined( _WIN32 ) && !defined( __CYGWIN__ )
    return osg::convertPointerType<DynamicLibrary::PROC_ADDRESS, FARPROC>(
        GetProcAddress( ( HMODULE )_handle, procName.c_str() )
    );
#elif defined( __APPLE__ ) && defined( APPLE_PRE_10_3 )
    std::string temp( "_" );
    NSSymbol    symbol;
...
#elif defined( __hpux )
    void* result = NULL;
    if( shl_findsym( reinterpret_cast<shl_t*>( &_handle ),
                     procName.c_str(),
                     TYPE_PROCEDURE,
                     result ) == 0 )
    {
