...
osgDB::listAllAvailablePlugins()
{
    ...
    if( !fullPath.empty() )
    {
        ...
        for( DirectoryContents::iterator itr = contents.begin(); itr != contents.end();
             ++itr )
        {
...
bool
osgDB::queryPlugin( const std::string&    fileName,
...
static std::string
padwithspaces( const std::string& str,
...
bool
osgDB::outputPluginDetails( std::ostream&      out,
                            const std::string& fileName )
{
    ...
    if( osgDB::queryPlugin( fileName, infoList ) )
    {
             ...
             ++rwi_itr )
        {
            ...
            out << "    {" << std::endl;
            out << "        features   : ";
            osgDB::ReaderWriter::FeatureList fl =
                ReaderWriter::featureAsString( info.features );
            osgDB::ReaderWriter::FeatureList::iterator fl_itr;
            for( fl_itr = fl.begin(); fl_itr != fl.end(); ++fl_itr )
            {
                out << *fl_itr << " ";
            }
            out << std::endl;
            ...
            for( fdm_itr = info.protocols.begin(); fdm_itr != info.protocols.end();
                 ++fdm_itr )
            {
                if( fdm_itr->first.length() > longestOptionLength )
                {
                    longestOptionLength =
                        static_cast<unsigned int>( fdm_itr->first.length() );
                }
            }

            for( fdm_itr = info.extensions.begin(); fdm_itr != info.extensions.end();
                 ++fdm_itr )
            {
                if( fdm_itr->first.length() > longestOptionLength )
                {
            ...
            for( fdm_itr = info.options.begin(); fdm_itr != info.options.end();
                 ++fdm_itr )
            {
                if( fdm_itr->first.length() > longestOptionLength )
                {
            ...
            for( fdm_itr = info.environment.begin(); fdm_itr != info.environment.end();
                 ++fdm_itr )
            {
                if( fdm_itr->first.length() > longestOptionLength )
                {
            ...
            {
                out << "        protocol   : "
                    << padwithspaces( fdm_itr->first, padLength ) << fdm_itr->second
                    << std::endl;
            }
            ...
            {
                out << "        extensions : ."
                    << padwithspaces( fdm_itr->first, padLength - 1 ) << fdm_itr->second
                    << std::endl;
            }
