...
static osg::ApplicationUsageProxy
    Registry_e0( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                 "OSG_FILE_PATH <path>[:path]..",
                 "Paths for locating datafiles" );
static osg::ApplicationUsageProxy
    Registry_e1( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
...
static osg::ApplicationUsageProxy Registry_e2(
...
class Registry::AvailableReaderWriterIterator
{
    public:
        ...
        AvailableReaderWriterIterator( Registry::ReaderWriterList& rwList,
                                       std::recursive_mutex&       pluginMutex ) :
            _rwList( rwList ),
    ...
    protected:
        ...
        operator=( const AvailableReaderWriterIterator& )
        {
            return *this;
        }
...
class Registry::AvailableArchiveIterator
{
    ...
    protected:
        ...
        operator=( const AvailableArchiveIterator& )
        {
            return *this;
        }
...
void
PrintFilePathList( std::ostream&       stream,
                   const FilePathList& filepath )
{
    for( FilePathList::const_iterator itr = filepath.begin(); itr != filepath.end();
         ++itr )
    {
        stream << "    " << *itr << std::endl;
    }
...
OSG_INIT_SINGLETON_PROXY( ProxyInitRegistry,
                          Registry::instance() )

// definition of the Registry
Registry::Registry()
{
    // comment out because it was causing problems under OSX - causing it to crash
    // osgconv when constructing ostream in osg::notify(). OSG_INFO << "Constructing
    // osg::Registry"<<std::endl;
    ...
    if( kdtree_str )
    {
        bool switchOff = ( strcmp( kdtree_str, "off" ) ==
                           0 ||
                           strcmp( kdtree_str, "OFF" ) ==
                           0 ||
                           strcmp( kdtree_str, "Off" ) == 0 );
        if( switchOff )
        {
    ...
    // won't add type1 and type2 until resolve extension collision with Performer binary
    // and ascii files. addFileExtensionAlias("pfb",   "freetype");  // type1 binary
...
void
Registry::setDataFilePathList( const std::string& paths )
{
    _dataFilePath.clear();
...
void
Registry::setLibraryFilePathList( const std::string& paths )
{
...
void
Registry::readCommandLine( osg::ArgumentParser& arguments )
{
    // report the usage options.
    if( arguments.getApplicationUsage() )
    {
        arguments.getApplicationUsage()->addCommandLineOption( "-l <library>",
                                                               "Load the plugin" );
...
void
Registry::addReaderWriter( ReaderWriter* rw )
{
    if( rw == 0L )
    {
        return;
    ...
    // OSG_NOTIFY(INFO) << "osg::Registry::addReaderWriter("<<rw->className()<<")"<<
    // std::endl;
...
void
Registry::removeReaderWriter( ReaderWriter* rw )
{
    if( rw == 0L )
    {
        return;
...
ImageProcessor*
Registry::getImageProcessorForExtension( const std::string& ext )
{
    {
        std::lock_guard<std::recursive_mutex> lock( _pluginMutex );
        if( !_ipList.empty() )
    ...
    if( loadLibrary( libraryName ) == LOADED )
    {
        ...
        if( !_ipList.empty() )
        {
            OSG_NOTICE << "Loaded plug-in " << libraryName
                       << " and located ImageProcessor" << std::endl;
            return _ipList.front().get();
        }
    }
    return 0;
...
void
Registry::addImageProcessor( ImageProcessor* ip )
{
    if( ip == 0L )
    {
        return;
    ...
    OSG_NOTIFY( NOTICE ) << "osg::Registry::addImageProcessor(" << ip->className() << ")"
                         << std::endl;
...
void
Registry::removeImageProcessor( ImageProcessor* ip )
{
    if( ip == 0L )
    {
        return;
...
void
Registry::addFileExtensionAlias( const std::string mapExt,
...
void
Registry::addMimeTypeExtensionMapping( const std::string fromMimeType,
                                       const std::string toExt )
{
...
bool
Registry::readPluginAliasConfigurationFile( const std::string& file )
{
    std::string fileName = osgDB::findDataFile( file );
    if( fileName.empty() )
    {
        OSG_NOTIFY( osg::WARN )
            << "Can't find plugin alias config file \"" << file << "\"." << std::endl;
        return false;
    }
    ...
    if( !ifs.good() )
    {
        OSG_NOTIFY( osg::WARN ) << "Can't open plugin alias config file \"" << fileName
                                << "\"." << std::endl;
        return false;
    }
    ...
    while( ifs.good() )
    {
        ...
        if( spIdx == ln.npos )
        {
            ...
            OSG_NOTIFY( osg::WARN )
                << file << ", line " << lineNum << ": Syntax error: missing space in \""
                << raw << "\"." << std::endl;
            continue;
        }
...
std::string
Registry::trim( const std::string& str )
{
    if( !str.size() )
    {
        return str;
...
std::string
Registry::createLibraryNameForFile( const std::string& fileName )
{
    return createLibraryNameForExtension( getFileExtension( fileName ) );
}

std::string
Registry::createLibraryNameForExtension( const std::string& ext )
{
    std::string lowercase_ext;
    for( std::string::const_iterator sitr = ext.begin(); sitr != ext.end(); ++sitr )
    {
        lowercase_ext.push_back( static_cast<char>( tolower( *sitr ) ) );
    }
...
std::string
Registry::createLibraryNameForNodeKit( const std::string& name )
{
#if defined( __CYGWIN__ )
...
Registry::LoadStatus
Registry::loadLibrary( const std::string& fileName )
{
    std::lock_guard<std::recursive_mutex> lock( _pluginMutex );
...
bool
Registry::closeLibrary( const std::string& fileName )
{
    std::lock_guard<std::recursive_mutex> lock( _pluginMutex );
    DynamicLibraryList::iterator          ditr = getLibraryItr( fileName );
    if( ditr != _dlList.end() )
...
Registry::DynamicLibraryList::iterator
Registry::getLibraryItr( const std::string& fileName )
{
...
DynamicLibrary*
Registry::getLibrary( const std::string& fileName )
{
    std::lock_guard<std::recursive_mutex> lock( _pluginMutex );
    DynamicLibraryList::iterator          ditr = getLibraryItr( fileName );
    if( ditr != _dlList.end() )
...
Registry::getReaderWriterForExtension( const std::string& ext )
{
    ...
    for( ReaderWriterList::iterator itr = _rwList.begin(); itr != _rwList.end(); ++itr )
    {
    ...
    if( loadLibrary( libraryName ) == LOADED )
    {
        for( ReaderWriterList::iterator itr = _rwList.begin(); itr != _rwList.end();
             ++itr )
        {
...
Registry::getReaderWriterForMimeType( const std::string& mimeType )
{
    ...
    return i != _mimeTypeExtMap.end() ? getReaderWriterForExtension( i->second ) : NULL;
}
...
{
        ReadObjectFunctor( const std::string& filename,
                           const Options*     options ) :
            ReadFunctor( filename,
                         options )
        {
        }

        virtual ReaderWriter::ReadResult
        doRead( ReaderWriter& rw ) const
        {
            return rw.readObject( _filename, _options );
        }

        virtual bool
        isValid( ReaderWriter::ReadResult& readResult ) const
        {
            return readResult.validObject();
        }

        virtual bool
        isValid( osg::Object* object ) const
        {
            return object != 0;
        }
...
{
        ReadImageFunctor( const std::string& filename,
                          const Options*     options ) :
            ReadFunctor( filename,
                         options )
        {
...
{
        ReadHeightFieldFunctor( const std::string& filename,
                                const Options*     options ) :
            ReadFunctor( filename,
                         options )
        {
...
{
        ReadNodeFunctor( const std::string& filename,
                         const Options*     options ) :
            ReadFunctor( filename,
                         options )
        {
...
struct Registry::ReadArchiveFunctor : public Registry::ReadFunctor
{
        ...
        cloneType( const std::string& filename,
                   const Options*     options ) const
        {
            return new ReadArchiveFunctor( filename,
                                           _status,
                                           _indexBlockSizeHint,
                                           options );
        }
...
{
        ReadShaderFunctor( const std::string& filename,
                           const Options*     options ) :
            ReadFunctor( filename,
                         options )
        {
...
{
        ReadScriptFunctor( const std::string& filename,
                           const Options*     options ) :
            ReadFunctor( filename,
                         options )
        {
...
void
Registry::addArchiveExtension( const std::string ext )
{
...
std::string
Registry::findDataFileImplementation( const std::string& filename,
                                      const Options*     options,
                                      CaseSensitivity    caseSensitivity )
{
    ...
    // if data file contains a server address then we can't find it in local directories
    ...
    {
        OSG_DEBUG << "FindFileInPath(" << filename << "): returning " << filename
                  << std::endl;
        return filename;
    }
    ...
    {
        fileFound =
            findFileInPath( filename, options->getDatabasePathList(), caseSensitivity );
        if( !fileFound.empty() )
        {
            return fileFound;
        }

        if( osgDB::containsCurrentWorkingDirectoryReference(
    ...
    if( !filepaths.empty() )
    {
        ...
        if( !pathsContainsCurrentWorkingDirectory &&
            osgDB::containsCurrentWorkingDirectoryReference( filepaths ) )
        {
            pathsContainsCurrentWorkingDirectory = true;
        }
    }
    ...
    // if a directory is included in the filename, get just the (simple) filename itself
    // and try that
    ...
    if( simpleFileName != filename )
    {
        ...
        if( fileExists( simpleFileName ) )
        {
            OSG_DEBUG << "FindFileInPath(" << filename << "): returning "
                      << simpleFileName << std::endl;
            return simpleFileName;
        }

        if( options && !options->getDatabasePathList().empty() )
        {
            fileFound = findFileInPath( simpleFileName,
                                        options->getDatabasePathList(),
                                        caseSensitivity );
            if( !fileFound.empty() )
            {
...
std::string
Registry::findLibraryFileImplementation( const std::string& filename,
                                         const Options* /*options*/,
                                         CaseSensitivity caseSensitivity )
{
    if( filename.empty() )
...
Registry::read( const ReadFunctor& readFunctor )
{
    for( ArchiveExtensionList::iterator aitr = _archiveExtList.begin();
         aitr != _archiveExtList.end();
         ++aitr )
    {
        ...
        std::string::size_type positionArchive =
            readFunctor._filename.find( archiveExtension + '/' );
        ...
        if( positionArchive != std::string::npos )
        {
            std::string::size_type endArchive =
                positionArchive + archiveExtension.length();
            std::string archiveName( readFunctor._filename.substr( 0, endArchive ) );
            std::string fileName( readFunctor._filename.substr( endArchive + 1,
                                                                std::string::npos ) );
            ...
            ReaderWriter::ReadResult result =
                openArchiveImplementation( archiveName,
                                           ReaderWriter::READ,
                                           4'096,
                                           readFunctor._options );
            ...
            osg::ref_ptr<osgDB::ReaderWriter::Options> options =
                readFunctor._options ? readFunctor._options->cloneOptions()
                                     : new osgDB::ReaderWriter::Options;

            options->setDatabasePath( archiveName );

            osg::ref_ptr<ReadFunctor> rf( readFunctor.cloneType( fileName,
                                                                 options.get() ) );
    ...
    for( ; aaitr.valid(); ++aaitr )
    {
        ...
        {
            // don't pass on FILE_NOT_FOUND results as we don't want to prevent non
            // archive plugins that haven't been loaded yet from getting a chance to test
    ...
    // If the filename contains a server address and wasn't loaded by any of the plugins,
    // try to find a plugin which supports the server protocol and supports wildcards. If
    // not successfully use curl as a last fallback
    if( containsServerAddress( readFunctor._filename ) )
    {
        ...
        {
            return ReaderWriter::ReadResult(
                "Could not find the .curl plugin to read from server."
            );
        }
    }
    ...
    if( results.empty() )
    {
        return ReaderWriter::ReadResult(
            ...
            readFunctor._filename +
            "\"."
        );
    }

    // sort the results so the most relevant (i.e. ERROR_IN_READING_FILE is more relevant
    // than FILE_NOT_FOUND) results get placed at the end of the results list.
...
ReaderWriter::ReadResult
Registry::readImplementation( const ReadFunctor&        readFunctor,
                              Options::CacheHintOptions cacheHint )
{
    ...
    // Note CACHE_ARCHIVES has a different object that it caches to so it will never be
    // used here
    if( ( optionsCache || _objectCache.valid() ) &&
        cacheHint != Options::CACHE_ARCHIVES )
    {
        useObjectCache =
            options ? ( options->getObjectCacheHint() & cacheHint ) != 0 : false;
    }
    ...
    if( useObjectCache )
    {
        ...
        osg::ref_ptr<osg::Object> object =
            optionsCache ? optionsCache->getRefFromObjectCache( file, options ) : 0;
        ...
        if( object.valid() )
        {
            ...
            {
                return ReaderWriter::ReadResult(
                    "Error file does not contain an osg::Object"
                );
            }
        }
        ...
        if( rr.validObject() )
        {
            ...
            if( optionsCache )
            {
                optionsCache->addEntryToObjectCache( file,
                                                     rr.getObject(),
                                                     0.0,
                                                     options );
            }
            ...
            {
                _objectCache->addEntryToObjectCache( file,
                                                     rr.getObject(),
                                                     0.0,
                                                     options );
            }
...
ReaderWriter::ReadResult
Registry::openArchiveImplementation( const std::string&          fileName,
                                     ReaderWriter::ArchiveStatus status,
                                     unsigned int                indexBlockSizeHint,
                                     const Options*              options )
{
    ...
    ReaderWriter::ReadResult result = readImplementation(
        ReadArchiveFunctor( fileName, status, indexBlockSizeHint, options ),
        Options::CACHE_ARCHIVES
    );

    // default to using caching archive if no options structure provided, but if options
    // are provided use archives only if supplied.
    if( result.validArchive() &&
...
ReaderWriter::ReadResult
Registry::readObjectImplementation( const std::string& fileName,
                                    const Options*     options )
{
    return readImplementation( ReadObjectFunctor( fileName, options ),
                               Options::CACHE_OBJECTS );
}

ReaderWriter::WriteResult
Registry::writeObjectImplementation( const Object&      obj,
                                     ...
                                     const Options*     options )
{
    ...
    if( results.empty() )
    {
        return ReaderWriter::WriteResult(
            "Could not find plugin to write objects to file \"" + fileName + "\"."
        );
    }

    // sort the results so the most relevant (i.e. ERROR_IN_WRITING_FILE is more relevant
    // than FILE_NOT_FOUND) results get placed at the end of the results list.
    std::sort( results.begin(), results.end() );
...
ReaderWriter::ReadResult
Registry::readImageImplementation( const std::string& fileName,
                                   const Options*     options )
{
    return readImplementation( ReadImageFunctor( fileName, options ),
                               Options::CACHE_IMAGES );
}

ReaderWriter::WriteResult
Registry::writeImageImplementation( const Image&       image,
                                    const std::string& fileName,
                                    const Options*     options )
{
    // record the errors reported by readerwriters.
    ...
    {
        return ReaderWriter::WriteResult(
            "Could not find plugin to write image to file \"" + fileName + "\"."
        );
    }
...
ReaderWriter::ReadResult
Registry::readHeightFieldImplementation( const std::string& fileName,
                                         const Options*     options )
{
    return readImplementation( ReadHeightFieldFunctor( fileName, options ),
                               Options::CACHE_HEIGHTFIELDS );
}

ReaderWriter::WriteResult
Registry::writeHeightFieldImplementation( const HeightField& HeightField,
                                          const std::string& fileName,
                                          const Options*     options )
{
    // record the errors reported by readerwriters.
    ...
    for( ; itr.valid(); ++itr )
    {
        ReaderWriter::WriteResult rr =
            itr->writeHeightField( HeightField, fileName, options );
        if( rr.success() )
        {
            return rr;
        }
    ...
    {
        return ReaderWriter::WriteResult(
            "Could not find plugin to write HeightField to file \"" + fileName + "\"."
        );
    }
...
ReaderWriter::ReadResult
Registry::readNodeImplementation( const std::string& fileName,
                                  const Options*     options )
{
#if 0
...
ReaderWriter::WriteResult
Registry::writeNodeImplementation( const Node&        node,
                                   const std::string& fileName,
                                   const Options*     options )
{
    // record the errors reported by readerwriters.
    ...
    {
        return ReaderWriter::WriteResult(
            "Could not find plugin to write nodes to file \"" + fileName + "\"."
        );
    }
...
ReaderWriter::ReadResult
Registry::readShaderImplementation( const std::string& fileName,
                                    const Options*     options )
{
...
ReaderWriter::WriteResult
Registry::writeShaderImplementation( const Shader&      shader,
                                     const std::string& fileName,
                                     const Options*     options )
{
    // record the errors reported by readerwriters.
    ...
    {
        return ReaderWriter::WriteResult(
            "Could not find plugin to write shader to file \"" + fileName + "\"."
        );
    }
...
ReaderWriter::ReadResult
Registry::readScriptImplementation( const std::string& fileName,
                                    const Options*     options )
{
    return readImplementation( ReadScriptFunctor( fileName, options ),
                               Options::CACHE_IMAGES );
}

ReaderWriter::WriteResult
Registry::writeScriptImplementation( const Script&      image,
                                     const std::string& fileName,
                                     const Options*     options )
{
    // record the errors reported by readerwriters.
...
void
Registry::addEntryToObjectCache( const std::string& filename,
                                 osg::Object*       object,
                                 double             timestamp,
                                 Options*           options )
{
...
osg::Object*
Registry::getFromObjectCache( const std::string& filename,
                              Options*           options )
{
    return _objectCache.valid() ? _objectCache->getFromObjectCache( filename, options )
                                : 0;
}

osg::ref_ptr<osg::Object>
Registry::getRefFromObjectCache( const std::string& filename,
                                 Options*           options )
{
    return _objectCache.valid()
             ? _objectCache->getRefFromObjectCache( filename, options )
             : 0;
}
...
)
{
    if( _objectCache.valid() )
    {
        _objectCache->updateTimeStampOfObjectsInCacheWithExternalReferences(
...
void
Registry::removeExpiredObjectsInCache( const osg::FrameStamp& frameStamp )
{
...
void
Registry::removeFromObjectCache( const std::string& filename,
                                 Options*           options )
{
    if( _objectCache.valid() )
    {
...
void
Registry::addToArchiveCache( const std::string& fileName,
                             osgDB::Archive*    archive )
{
    std::lock_guard<std::recursive_mutex> lock( _archiveCacheMutex );
    _archiveCache[fileName] = archive;
}

/** Remove archive from cache.*/
void
Registry::removeFromArchiveCache( const std::string& fileName )
{
    std::lock_guard<std::recursive_mutex> lock( _archiveCacheMutex );
...
osgDB::Archive*
Registry::getFromArchiveCache( const std::string& fileName )
{
    std::lock_guard<std::recursive_mutex> lock( _archiveCacheMutex );
    ArchiveCache::iterator                itr = _archiveCache.find( fileName );
    if( itr != _archiveCache.end() )
...
Registry::getRefFromArchiveCache( const std::string& fileName )
{
    std::lock_guard<std::recursive_mutex> lock( _archiveCacheMutex );
    ArchiveCache::iterator                itr = _archiveCache.find( fileName );
    if( itr != _archiveCache.end() )
...
void
Registry::releaseGLObjects( osg::State* state )
{
    if( _objectCache.valid() )
    {
...
bool
Registry::isProtocolRegistered( const std::string& protocol )
{
    return ( _registeredProtocols.find( convertToLowerCase( protocol ) ) !=
             _registeredProtocols.end() );
}

void
Registry::getReaderWriterListForProtocol( const std::string& protocol,
...
ReaderWriter*
Registry::getReaderWriterForProtocolAndExtension( const std::string& protocol,
