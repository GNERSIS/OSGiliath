 ...
 * File system utilities: path searching (findDataFile, findLibraryFile),
 * directory listing, file existence checks, and path manipulation.
 */
// handle TCHAR type on various platforms
// #ifndef is inspired by
// https://svn.apache.org/repos/asf/logging/log4cxx/tags/v0_9_4/include/log4cxx/helpers/tchar.h
...
// set up for windows so acts just like unix access().
        ...
        #include <sys/cdefs.h>
            ...
            // workaround a bug which appears when compiling for SDK < 4.0 and for the
            // simulator
        ...
        #if ( MAC_OS_X_VERSION_MAX_ALLOWED <= 1'040 )
            #define stat64 stat
        #endif
...
FILE*
osgDB::fopen( const char* filename,
              const char* mode )
{
#ifdef OSG_USE_UTF8_FILENAME
...
bool
osgDB::makeDirectory( const std::string& path )
{
    if( path.empty() )
    {
        OSG_DEBUG << "osgDB::makeDirectory(): cannot create an empty directory"
                  << std::endl;
        return false;
    }
...
#endif
    {
        ...
        {
            OSG_DEBUG << "osgDB::makeDirectory(): " << path
    ...
    for( std::string dir = path; !dir.empty(); dir = getFilePath( dir ) )
    {
#ifdef OSG_USE_UTF8_FILENAME
...
#endif
        {
            switch( errno )
            {
                ...
                default :
                    OSG_DEBUG << "osgDB::makeDirectory(): " << strerror( errno )
                              << std::endl;
                    return false;
            }
        }
...
#endif
        {
            // Only return an error if the directory actually doesn't exist.  It's
            // possible that the directory was created by another thread or process
...
bool
osgDB::makeDirectoryForFile( const std::string& path )
{
    return makeDirectory( getFilePath( path ) );
}

std::string
osgDB::getCurrentWorkingDirectory( void )
{
    // MAX_PATH/cwd inspired by unzip.cpp
#ifndef MAX_PATH
    #define MAX_PATH 1'024
#endif
...
bool
osgDB::setCurrentWorkingDirectory( const std::string& newCurrentWorkingDirectory )
{
    if( newCurrentWorkingDirectory.empty() )
    {
        OSG_DEBUG << "osgDB::setCurrentWorkingDirectory(): called with empty string."
                  << std::endl;
        return false;
    }

#ifdef OSG_USE_UTF8_FILENAME
    return _wchdir( OSGDB_STRING_TO_FILENAME( newCurrentWorkingDirectory ).c_str() ) ==
           0;
#else
...
void
osgDB::convertStringPathIntoFilePathList( const std::string& paths,
...
bool
osgDB::fileExists( const std::string& filename )
{
#ifdef OSG_USE_UTF8_FILENAME
...
osgDB::FileType
osgDB::fileType( const std::string& filename )
{
...
std::string
osgDB::findFileInPath( const std::string&  filename,
...
#ifdef _WIN32
        // if combined file path exceeds MAX_PATH then ignore as it's not a legal path
        // otherwise subsequent IO calls with this path may result in undefined behavior
...
std::string
osgDB::findDataFile( const std::string& filename,
                     CaseSensitivity    caseSensitivity )
{
    return findDataFile( filename, static_cast<Options*>( 0 ), caseSensitivity );
}

OSGDB_EXPORT std::string
osgDB::findDataFile( const std::string& filename,
...
std::string
osgDB::findLibraryFile( const std::string& filename,
                        CaseSensitivity    caseSensitivity )
{
    return Registry::instance()->findLibraryFile(
        filename,
...
std::string
osgDB::findFileInDirectory( const std::string& fileName,
...
// Skip case-insensitive recursion if on Windows
    ...
    if( realDirName.size() > 2 )
    {
        ...
        if( trimLastCharacter )
        {
            ...
            OSG_DEBUG << "findFileInDirectory() Trimming last character of filepath, "
                         "now realDirName="
                      << realDirName << std::endl;
        }
    }

    OSG_DEBUG << "findFileInDirectory() : looking for " << realFileName << " in "
              << realDirName << std::endl;
    ...
    else
    {
        ...
        if( ( caseSensitivity == CASE_INSENSITIVE ) && ( !win32 ) )
        {
            ...
            {
                std::string directoryStringToUse =
                    ( realDirName[0] == '/' || realDirName[0] == '\\' )
                        ? std::string( "/" )
                        : std::string( "." );
            ...
            else
            {
                ...
                // in the parent path
                realDirName =
                    findFileInDirectory( lastElement, parentPath, CASE_INSENSITIVE );
    ...
    for( osgDB::DirectoryContents::iterator itr = dc.begin(); itr != dc.end(); ++itr )
    {
        if( ( caseSensitivity ==
              CASE_INSENSITIVE &&
              osgDB::equalCaseInsensitive( realFileName, *itr ) ) ||
            ( realFileName == *itr ) )
        {
...
osgDB::getDirectoryContents( const std::string& dirName )
{
    ...
    HANDLE handle = OSGDB_WINDOWS_FUNCT( FindFirstFile )(
        ( OSGDB_STRING_TO_FILENAME( dirName ) + OSGDB_FILENAME_TEXT( "\\*" ) ).c_str(),
        &data
    );
...
osgDB::DirectoryContents
osgDB::getSortedDirectoryContents( const std::string& dirName )
{
...
osgDB::DirectoryContents
osgDB::expandWildcardsInFilename( const std::string& filename )
{
    osgDB::DirectoryContents contents;

    std::string              dir = osgDB::getFilePath( filename );
    std::string              filenameOnly =
        dir.empty() ? filename : filename.substr( dir.length() + 1, std::string::npos );
    std::string left = filenameOnly.substr( 0, filenameOnly.find( '*' ) );
    std::string right =
        filenameOnly.substr( filenameOnly.find( '*' ) + 1, std::string::npos );
    ...
    for( unsigned int i = 0; i < dirContents.size(); ++i )
    {
        ...
        if( ( filenameInDir.find( left ) == 0 || left.empty() ) &&
            ( filenameInDir.find( right ) ==
              filenameInDir.length() -
              right.length() ||
              right.empty() ) )
        {
...
osgDB::CopyFileResult
osgDB::copyFile( const std::string& source,
                 const std::string& destination )
{
    ...
    if( source ==
        destination ||
        osgDB::getRealPath( source ) == osgDB::getRealPath( destination ) )
    {
        OSG_INFO << "copyFile(): Source and destination point to the same file: source="
                 << source << ", destination=" << destination << std::endl;
    ...
    if( !fin )
    {
        ...
        return COPY_FILE_SOURCE_NOT_OPENED;    // Return success since it's not an output
                                               // error.
    }
    ...
    {
        OSG_INFO << "Can't create directory for file '" << destination
                 << "'. Copy may fail creating the file." << std::endl;
    }

    // Open destination file
    osgDB::ofstream fout( destination.c_str(),
                          std::ios::out | std::ios::binary | std::ios::trunc );
    if( !fout )
    {
        OSG_NOTICE << "copyFile(): Can't write destination file: " << destination
                   << std::endl;
        return COPY_FILE_DESTINATION_NOT_OPENED;
    }

    // Copy file
    const unsigned int         BUFFER_SIZE = 10'240;
    ...
    if( !fout.good() )
    {
        OSG_NOTICE << "copyFile(): Error writing destination file: " << destination
                   << std::endl;
...
bool
osgDB::containsCurrentWorkingDirectoryReference( const FilePathList& paths )
{
    const std::string cwd( "." );
    for( FilePathList::const_iterator itr = paths.begin(); itr != paths.end(); ++itr )
    {
        if( itr->empty() )
        {
            return true;
...
void
osgDB::appendPlatformSpecificLibraryFilePaths( FilePathList& filepath )
{
...
osgDB::appendPlatformSpecificLibraryFilePaths( FilePathList& filepath )
{
    ...
    if( retval != 0 && retval < size )
    {
        ...
        filenamestring executableDir(
            pathstr,
            0,
            pathstr.find_last_of( OSGDB_FILENAME_TEXT( "\\/" ) )
        );
        convertStringPathIntoFilePathList( OSGDB_FILENAME_TO_STRING( executableDir ),
                                           filepath );
    }
    else
    {
    ...
    #if defined( _MSC_VER )
    // Requires use of the GetModuleHandleEx() function which is available only on
    // Windows XP or higher. In order to allow execution on older versions, we load the
    // function dynamically from the library and use it only if it's available.
    OSGDB_WINDOWS_FUNCT( PGET_MODULE_HANDLE_EX )
    pGetModuleHandleEx = reinterpret_cast<OSGDB_WINDOWS_FUNCT( PGET_MODULE_HANDLE_EX )>(
        GetProcAddress( GetModuleHandleA( "kernel32.dll" ),
                        OSGDB_WINDOWS_FUNCT_STRING( GetModuleHandleEx ) )
    );
    if( pGetModuleHandleEx )
    {
        HMODULE             thisModule = 0;
        static filenamechar static_variable =
            0;    // Variable that is located in DLL address space.

        if( pGetModuleHandleEx( GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                                &static_variable,
                                &thisModule ) )
        {
            ...
            {
                filenamestring pathstr( path );
                filenamestring dllDir(
                    pathstr,
                    0,
                    pathstr.find_last_of( OSGDB_FILENAME_TEXT( "\\/" ) )
                );
                convertStringPathIntoFilePathList( OSGDB_FILENAME_TO_STRING( dllDir ),
                                                   filepath );
            }
            else
            {
        ...
        else
        {
            OSG_WARN << "Could not get dll module handle "
                        "using Win32 API. Dll directory will not be searched."
                     << std::endl;
        }
    }
    #endif
    ...
    else
    {
        OSG_WARN << "Could not get system directory using "
                    "Win32 API, using default directory."
                 << std::endl;
        convertStringPathIntoFilePathList( "C:\\Windows\\System32", filepath );
    }
    ...
    if( retval != 0 && retval < size )
    {
        convertStringPathIntoFilePathList(
            std::string( OSGDB_FILENAME_TO_STRING( windowsDir ) ) + "\\System",
            filepath
        );
    ...
    #if ( TARGET_OS_IPHONE ) || ( MAC_OS_X_VERSION_MIN_REQUIRED >= 1'080 )
...
std::string
GetShortenedPath( std::string path,
                  int         numToShorten )
{
...
std::string
GetPathFromCFURLRef( CFURLRef urlRef )
{
    char        buffer[1'024];
    std::string path;
    if( CFURLGetFileSystemRepresentation( urlRef, true, ( UInt8* )buffer, 1'024 ) )
    {
...
std::string
GetApplicationBundlePath( CFBundleRef mainBundle )
{
    std::string path;
...
std::string
GetApplicationParentPath( CFBundleRef mainBundle )
{
    return GetShortenedPath( GetApplicationBundlePath( mainBundle ), 1 );
}

std::string
GetApplicationPluginsPath( CFBundleRef mainBundle )
{
    std::string path;
...
std::string
GetApplicationResourcesPath( CFBundleRef mainBundle )
{
    std::string path;
...
osgDB::appendPlatformSpecificLibraryFilePaths( FilePathList& filepath )
{
    ...
    {
        NSString* systemPluginsPath =
            [systemPath stringByAppendingPathComponent:@"OpenSceneGraph/PlugIns"];
...
osgDB::appendPlatformSpecificLibraryFilePaths( FilePathList& filepath )
{
    ...
    else
    {
        OSG_NOTIFY( osg::DEBUG_INFO )
            << "Couldn't find the Application Bundle" << std::endl;
    }
    ...
    {
        errCode = FSFindFolder( domains[domain_ndx],
        ...
        if( noErr == errCode )
        {
            ...
            {
                OSG_NOTIFY( osg::DEBUG_INFO )
                    << "Couldn't create CFURLRef for application support Path at ndx "
                    << domain_ndx << std::endl;
            }
        ...
        {
            OSG_NOTIFY( osg::DEBUG_INFO )
                << "Couldn't find the User's Application Support Path at ndx "
                << domain_ndx << std::endl;
        }
    }
}
...
#ifdef __APPLE__
void
osgDB::appendPlatformSpecificResourceFilePaths( FilePathList& filepath )
{
