...
Image*
osgDB::readImageFile( const std::string& filename,
                      const Options*     options )
{
...
Shader*
osgDB::readShaderFile( const std::string& filename,
                       const Options*     options )
{
...
HeightField*
osgDB::readHeightFieldFile( const std::string& filename,
                            const Options*     options )
{
    ReaderWriter::ReadResult rr =
        Registry::instance()->readHeightField( filename, options );
...
Node*
osgDB::readNodeFile( const std::string& filename,
                     const Options*     options )
{
...
Node*
osgDB::readNodeFiles( std::vector<std::string>& fileList,
                      const Options*            options )
{
...
Node*
osgDB::readNodeFiles( osg::ArgumentParser& arguments,
                      const Options*       options )
{
...
Script*
osgDB::readScriptFile( const std::string& filename,
                       const Options*     options )
{
...
osgDB::readRefObjectFile( const std::string& filename,
                          const Options*     options )
{
    ReaderWriter::ReadResult rr = Registry::instance()->readObject( filename, options );
    if( rr.validObject() )
...
osgDB::readRefImageFile( const std::string& filename,
                         const Options*     options )
{
    ReaderWriter::ReadResult rr = Registry::instance()->readImage( filename, options );
    if( rr.validImage() )
...
osgDB::readRefShaderFile( const std::string& filename,
                          const Options*     options )
{
    ReaderWriter::ReadResult rr = Registry::instance()->readShader( filename, options );
    if( rr.validShader() )
...
osgDB::readRefHeightFieldFile( const std::string& filename,
                               const Options*     options )
{
    ReaderWriter::ReadResult rr =
        Registry::instance()->readHeightField( filename, options );
...
osgDB::readRefNodeFile( const std::string& filename,
                        const Options*     options )
{
    ReaderWriter::ReadResult rr = Registry::instance()->readNode( filename, options );
    if( rr.validNode() )
...
osgDB::readRefScriptFile( const std::string& filename,
                          const Options*     options )
{
    ReaderWriter::ReadResult rr = Registry::instance()->readScript( filename, options );
    if( rr.validScript() )
...
osg::ref_ptr<Node>
osgDB::readRefNodeFiles( std::vector<std::string>& fileList,
                         const Options*            options )
{
    typedef std::vector<osg::ref_ptr<osg::Node>> RefNodeList;
...
osg::ref_ptr<Node>
osgDB::readRefNodeFiles( osg::ArgumentParser& arguments,
                         const Options*       options )
{

    typedef std::vector<osg::ref_ptr<osg::Node>> RefNodeList2;
    RefNodeList2                                 nodeList;
        ...
        if( image.valid() )
        {
            ...
            if( image->isImageTranslucent() )
            {
                OSG_INFO << "Image " << image->getFileName()
                         << " is translucent; setting up blending." << std::endl;
    ...
    while( arguments.read( "--movie", filename ) )
    {
        ...
        if( imageStream.valid() )
        {
            ...
            osg::ref_ptr<osg::Geometry> pictureQuad = osg::createTexturedQuadGeometry(
                ...
                osg::vec3( static_cast<float>( image->s() ), 0.0F, 0.0F ),
                osg::vec3( 0.0F, 0.0F, static_cast<float>( image->t() ) ),
                0.0F,
                flip ? 1.0F : 0.0F,
                1.0F,
                flip ? 0.0F : 1.0F
    ...
    while( arguments.read( "--dem", filename ) )
    {
        osg::ref_ptr<osg::HeightField> hf =
            readRefHeightFieldFile( filename.c_str(), options );
        if( hf )
        {
            osg::ref_ptr<osg::Geode> geode = new osg::Geode;
    ...
    for( int pos = 1; pos < arguments.argc(); ++pos )
    {
        if( !arguments.isOption( pos ) )
        {
            ...
            osg::ref_ptr<osg::Node> node =
                osgDB::readRefNodeFile( arguments[pos], options );
                                      ...
                                      const char*        fallback )
{
    ...
    if( !rr.success() )
    {
        OSG_INFO << "Error reading file " << filename << ": " << rr.statusMessage()
                 << std::endl;
    }
...
struct LoadShaderShaderPipelineFilesProxy
{
        LoadShaderShaderPipelineFilesProxy()
        {
            ...
            OSG_INFO << "LoadShaderShaderPipelineFilesProxy() Pre-loading the "
                        "ShaderPipeline shaders"
                     << std::endl;
            for( osg::DisplaySettings::Filenames::const_iterator itr =
                     osg::DisplaySettings::instance()->getShaderPipelineFiles().begin();
