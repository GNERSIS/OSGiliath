...
XmlNode*
osgDB::readXmlFile( const std::string& filename,
...
std::string
osgDB::trimEnclosingSpaces( const std::string& str )
{
    if( str.empty() )
    {
        return str;
...
XmlNode*
osgDB::readXmlStream( std::istream& fin )
{
    XmlNode::Input input;
    input.attach( fin );
    input.readAllDataIntoBuffer();
...
void
XmlNode::ControlMap::addControlToCharacter( const std::string& control,
                                            int                c )
{
...
void
XmlNode::ControlMap::setUpControlMappings()
{
...
void
XmlNode::Input::open( const std::string& filename )
{
    _fin.open( filename.c_str() );
}

void
XmlNode::Input::attach( std::istream& fin )
{
    std::ios& fios = _fin;
...
void
XmlNode::Input::readAllDataIntoBuffer()
{
    while( _fin )
    {
        int c = _fin.get();
...
XmlNode::Input::skipWhiteSpace()
{
             ...
             _buffer[_currentPos] == '\r' ) )
    {
        ...
        // v="<<int(_buffer[_currentPos])<<std::endl;
        ++_currentPos;
    }
...
XmlNode::read( Input& input )
{
    ...
    while( input )
    {
        ...
        if( input.match( "<!--" ) )
        {
            ...
            if( end != std::string::npos )
            {
                OSG_INFO << "Valid Comment record [" << commentNode->contents << "]"
                         << std::endl;
                input += ( end + 3 );
            }
            else
            {
                OSG_NOTICE << "Error: Unclosed Comment record [" << commentNode->contents
                           << "]" << std::endl;
                input += end;
            }
        }
        ...
        else if( input.match( "<!DOCTYPE" ) )
        {
            ...
            if( end != std::string::npos )
            {
                OSG_INFO << "Valid information record [" << commentNode->contents << "]"
                         << std::endl;
                input += ( end + 2 );
            }
            else
            {
                OSG_NOTICE << "Error: Unclosed information record ["
        ...
        else if( input.match( "<?" ) )
        {
            ...
            if( end != std::string::npos )
            {
                if( commentNode->contents.find( "encoding=\"UTF-8\"" ) !=
                    std::string::npos )
                {
        ...
        else if( input.match( "<" ) )
        {
            ...
            while( ( c = input[0] ) >= 0 && c != '>' && c != '/' )
            {
                ...
                if( input[0] == '"' )
                {
                    option.push_back( static_cast<char>( input[0] ) );
                    ++input;
                    while( ( c = input[0] ) >= 0 && c != '"' )
                    {
                        if( c == '&' )
                ...
                {
                    OSG_NOTICE << "Error, parser iterator not advanced, position: "
                               << input.substr( 0, 50 ) << std::endl;
                    ++input;
                }

                if( !option.empty() )
                {
                    OSG_INFO << "Assigning option " << option << " with value " << value
                             << std::endl;
...
bool
XmlNode::write( std::ostream&      fout,
...
bool
XmlNode::write( const ControlMap&  controlMap,
                std::ostream&      fout,
                const std::string& indent ) const
{
    switch( type )
    {
        ...
        case( NODE ) :
            ...
            writeProperties( controlMap, fout );
            fout << ">";
            writeString( controlMap, fout, contents );
            fout << "</" << name << ">" << std::endl;
            return true;
        case( GROUP ) :
            {
...
bool
XmlNode::writeString( const ControlMap&  controlMap,
                      std::ostream&      fout,
                      const std::string& str ) const
{
    ...
    {
        int                                               c = *itr;
        ControlMap::CharacterToControlMap::const_iterator citr =
            controlMap._characterToControlMap.find( c );
...
bool
XmlNode::writeChildren( const ControlMap& /*controlMap*/,
                        std::ostream&      fout,
                        const std::string& indent ) const
{
    for( Children::const_iterator citr = children.begin(); citr != children.end();
         ++citr )
    {
...
bool
XmlNode::writeProperties( const ControlMap& controlMap,
                          std::ostream&     fout ) const
{
    for( Properties::const_iterator oitr = properties.begin(); oitr != properties.end();
         ++oitr )
    {
        fout << " " << oitr->first << "=\"";
...
bool
XmlNode::readAndReplaceControl( std::string&    in_contents,
                                XmlNode::Input& input ) const
{
    int         c = 0;
    std::string value;
    ...
    if( input._controlToCharacterMap.count( value ) != 0 )
    {
        ...
        OSG_INFO << "Read control character " << value << " converted to " << char( c )
                 << std::endl;
        in_contents.push_back( static_cast<char>( c ) );
        return true;
    }
    else
    {
        OSG_NOTICE << "Warning: read control character " << value
                   << ", but have no mapping to convert it to." << std::endl;
        return false;
    }
}
