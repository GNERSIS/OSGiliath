...
std::string
osgDB::getFilePath( std::string_view fileName )
{
    std::string_view::size_type slash = fileName.find_last_of( PATH_SEPARATORS );
    ...
    {
        return std::string( fileName.substr( 0, slash ) );
    }
}

std::string
osgDB::getSimpleFileName( std::string_view fileName )
{
    std::string_view::size_type slash = fileName.find_last_of( PATH_SEPARATORS );
    if( slash == std::string_view::npos )
    {
...
std::string
osgDB::getFileExtension( std::string_view fileName )
{
    std::string_view::size_type dot   = fileName.find_last_of( '.' );
    std::string_view::size_type slash = fileName.find_last_of( PATH_SEPARATORS );
    if( dot ==
...
std::string
osgDB::getFileExtensionIncludingDot( std::string_view fileName )
{
    std::string_view::size_type dot   = fileName.find_last_of( '.' );
    std::string_view::size_type slash = fileName.find_last_of( PATH_SEPARATORS );
    if( dot ==
...
std::string
osgDB::convertFileNameToWindowsStyle( std::string_view fileName )
{
...
std::string
osgDB::convertFileNameToUnixStyle( std::string_view fileName )
{
    std::string            new_fileName( fileName );
...
bool
osgDB::isFileNameNativeStyle( std::string_view fileName )
{
#if defined( _WIN32 ) && !defined( __CYGWIN__ )
    return fileName.find( UNIX_PATH_SEPARATOR ) ==
           std::string_view::npos;    // return true if no unix style slash exist
#else
    return fileName.find( WINDOWS_PATH_SEPARATOR ) ==
           std::string_view::npos;    // return true if no windows style backslash exist
#endif
}

std::string
osgDB::convertFileNameToNativeStyle( std::string_view fileName )
{
#if defined( _WIN32 ) && !defined( __CYGWIN__ )
...
std::string
osgDB::getLowerCaseFileExtension( std::string_view filename )
{
    return convertToLowerCase( osgDB::getFileExtension( filename ) );
}

std::string
osgDB::convertToLowerCase( std::string_view str )
{
    std::string lowcase_str( str );
    for( std::string::iterator itr = lowcase_str.begin(); itr != lowcase_str.end();
         ++itr )
    {
...
std::string
osgDB::getNameLessExtension( std::string_view fileName )
{
    std::string_view::size_type dot   = fileName.find_last_of( '.' );
    std::string_view::size_type slash = fileName.find_last_of(
        PATH_SEPARATORS
...
std::string
osgDB::getNameLessAllExtensions( std::string_view fileName )
{
    // Finds start search position: from last slash, or the beginning of the string if
    // none found
    std::string_view::size_type startPos = fileName.find_last_of(
        PATH_SEPARATORS
    );    // Finds forward slash *or* back slash
    ...
    std::string_view::size_type dot =
        fileName.find_first_of( '.', startPos );    // Finds *FIRST* dot from start pos
...
std::string
osgDB::getStrippedName( std::string_view fileName )
{
...
bool
osgDB::equalCaseInsensitive( std::string_view lhs,
                             std::string_view rhs )
{
...
bool
osgDB::containsServerAddress( std::string_view filename )
{
    // need to check for ://
    std::string_view::size_type pos( filename.find( "://" ) );
...
std::string
osgDB::getServerProtocol( std::string_view filename )
{
    std::string_view::size_type pos( filename.find( "://" ) );
    if( pos != std::string_view::npos )
    {
...
std::string
osgDB::getServerAddress( std::string_view filename )
{
    std::string_view::size_type pos( filename.find( "://" ) );

    if( pos != std::string_view::npos )
    {
        std::string_view::size_type pos_slash = filename.find_first_of( '/', pos + 3 );
        if( pos_slash != std::string_view::npos )
        {
            return std::string( filename.substr( pos + 3, pos_slash - pos - 3 ) );
        }
        else
        {
            return std::string( filename.substr( pos + 3 ) );
        }
    }
    return "";
}
...
std::string
osgDB::getServerFileName( std::string_view filename )
{
    std::string_view::size_type pos( filename.find( "://" ) );
    ...
    if( pos != std::string_view::npos )
    {
        ...
        if( pos_slash != std::string_view::npos )
        {
            return std::string( filename.substr( pos_slash + 1 ) );
        }
        else
        {
            return "";
...
std::string
osgDB::concatPaths( const std::string& left,
...
std::string
osgDB::getRealPath( const std::string& path )
{
#if defined( _WIN32 ) && !defined( __CYGWIN__ )
    ...
    else
    {
        ...
        if( 0 == GetShortPathNameW( convertUTF8toUTF16( FilePath ).c_str(),
                                    tempbuf1,
                                    _countof( tempbuf1 ) ) )
        {
            return retbuf8;
    ...
    class PathIterator
    {
        ...
        protected:
            ...
            std::string::const_iterator
                stop;    ///< Points to the separator after 'start', or ==end()

            /// Iterate until 'it' points to something different from a separator
            std::string::const_iterator
            skipSeparators( std::string::const_iterator it );
            std::string::const_iterator
            next( std::string::const_iterator it );
    };

}

osgDB::PathIterator::PathIterator( const std::string& v ) :
    end( v.end() ),
    start( v.begin() ),
    stop( v.begin() )
{
...
std::string::const_iterator
osgDB::PathIterator::skipSeparators( std::string::const_iterator it )
{
    for( ; it !=
           end &&
...
std::string::const_iterator
osgDB::PathIterator::next( std::string::const_iterator it )
{
    return std::find_first_of( it,
                               end,
                               PATH_SEPARATORS,
                               PATH_SEPARATORS + PATH_SEPARATORS_LEN );
}
...
void
osgDB::getPathElements( const std::string&        path,
...
std::string
osgDB::getPathRoot( std::string_view path )
{
    // Test for unix root
    if( path.empty() )
    {
    ...
    if( path[1] == ':' )
    {
        return std::string(
            path.substr( 0, 2 )
        );    // We should check that path[0] is a letter, but as ':' is invalid in paths
              // in other cases, that's not a problem.
    }
    return "";
}

bool
osgDB::isAbsolutePath( std::string_view path )
{
    // Test for unix root
    if( path.empty() )
    {
    ...
    return path[1] == ':';    // We should check that path[0] is a letter, but as ':' is
                              // invalid in paths in other cases, that's not a problem.
}

std::string
osgDB::getPathRelative( const std::string& from,
                        const std::string& to )
{
    // This implementation is not 100% robust, and should be replaced with C++0x
    // "std::path" as soon as possible.

    // Definition: an "element" is a part between slashes. Ex: "/a/b" has two elements
    // ("a" and "b"). Algorithm:
    // 1. If paths are neither both absolute nor both relative, then we cannot do
    // anything (we need to make them absolute, but need additional info on how to make
    // it). Return.
    // 2. If both paths are absolute and root isn't the same (for Windows only, as roots
    // are of the type "C:", "D:"), then the operation is impossible. Return.
    ...
    // Iterators may point to Windows roots. As we tested they are equal, there is no
    // need to ++itFrom and ++itTo. However, if we got an Unix root, we must add it to
    // the result.
    std::string  res( root == "/" ? "/" : "" );
...
void
osgDB::stringcopy( char*       dest,
                   const char* src,
                   size_t      length )
{
    strncpy( dest, src, length - 1 );
    dest[length - 1] = '\0';
    // OSG_NOTICE<<"osgDB::stringcopy(["<<dest<<"], src=["<<src<<"],
    // "<<length<<")"<<std::endl;
}
...
// std::string testF = getPathRelative("C:/a/b", "a/d");                 // Precondition
// fail. Returns d. std::string testG = getPathRelative("/a/b", "a/d"); // Precondition
// fail. Returns d. std::string testH = getPathRelative("a/b", "/a/d"); // Precondition
// fail. Returns d.
