...
bool
FileCache::isFileAppropriateForFileCache( const std::string& originalFileName ) const
{
    return osgDB::containsServerAddress( originalFileName );
}

std::string
FileCache::createCacheFileName( const std::string& originalFileName ) const
{
    ...
    OSG_DEBUG << "FileCache::createCacheFileName(" << originalFileName
...
bool
FileCache::existsInCache( const std::string& originalFileName ) const
{
...
FileCache::writeObject( const osg::Object&    object,
                        ...
                        const osgDB::Options* options ) const
{
    ...
    if( !cacheFileName.empty() )
    {
        ...
        OSG_INFO << "FileCache::writeObjectToCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
        ReaderWriter::WriteResult result =
            osgDB::Registry::instance()->writeObject( object, cacheFileName, options );
        if( result.success() )
        {
...
ReaderWriter::ReadResult
FileCache::readImage( const std::string&    originalFileName,
                      const osgDB::Options* options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() && osgDB::fileExists( cacheFileName ) )
    {
        OSG_INFO << "FileCache::readImageFromCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
...
ReaderWriter::WriteResult
FileCache::writeImage( const osg::Image&     image,
                       const std::string&    originalFileName,
                       const osgDB::Options* options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() )
    {
        ...
        OSG_INFO << "FileCache::writeImageToCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
        ReaderWriter::WriteResult result =
            osgDB::Registry::instance()->writeImage( image, cacheFileName, options );
        if( result.success() )
        {
            removeFileFromBlackListed( originalFileName );
        }
...
ReaderWriter::ReadResult
FileCache::readHeightField( const std::string&    originalFileName,
                            const osgDB::Options* options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() && osgDB::fileExists( cacheFileName ) )
    {
        OSG_INFO << "FileCache::readHeightFieldFromCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
...
ReaderWriter::WriteResult
FileCache::writeHeightField( const osg::HeightField& hf,
                             const std::string&      originalFileName,
                             const osgDB::Options*   options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() )
    {
        ...
        OSG_INFO << "FileCache::writeHeightFieldToCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
        ReaderWriter::WriteResult result =
            osgDB::Registry::instance()->writeHeightField( hf, cacheFileName, options );
        if( result.success() )
        {
            removeFileFromBlackListed( originalFileName );
        }
...
ReaderWriter::ReadResult
FileCache::readNode( const std::string&    originalFileName,
                     ...
                     bool                  buildKdTreeIfRequired ) const
{
    ...
    if( !cacheFileName.empty() && osgDB::fileExists( cacheFileName ) )
    {
        OSG_INFO << "FileCache::readNodeFromCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
        return osgDB::Registry::instance()->readNode( cacheFileName,
                                                      options,
                                                      buildKdTreeIfRequired );
    }
    else
    {
        return 0;
...
ReaderWriter::WriteResult
FileCache::writeNode( const osg::Node&      node,
                      const std::string&    originalFileName,
                      const osgDB::Options* options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() )
    {
        ...
        OSG_INFO << "FileCache::writeNodeToCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
        ReaderWriter::WriteResult result =
            osgDB::Registry::instance()->writeNode( node, cacheFileName, options );
        if( result.success() )
        {
            removeFileFromBlackListed( originalFileName );
        }
...
ReaderWriter::ReadResult
FileCache::readShader( const std::string&    originalFileName,
                       const osgDB::Options* options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() && osgDB::fileExists( cacheFileName ) )
    {
        OSG_INFO << "FileCache::readShaderFromCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
...
ReaderWriter::WriteResult
FileCache::writeShader( const osg::Shader&    shader,
                        const std::string&    originalFileName,
                        const osgDB::Options* options ) const
{
    std::string cacheFileName = createCacheFileName( originalFileName );
    if( !cacheFileName.empty() )
    {
        ...
        OSG_INFO << "FileCache::writeShaderToCache(" << originalFileName << ") as "
                 << cacheFileName << std::endl;
        ReaderWriter::WriteResult result =
            osgDB::Registry::instance()->writeShader( shader, cacheFileName, options );
        if( result.success() )
        {
            removeFileFromBlackListed( originalFileName );
        }
...
bool
FileCache::isCachedFileBlackListed( const std::string& originalFileName ) const
{
...
bool
FileCache::removeFileFromBlackListed( const std::string& originalFileName ) const
{
         ...
         ++dr_itr )
    {
        ...
        if( originalFileName.compare( 0,
                                      dr->getDatabasePath().length(),
                                      dr->getDatabasePath() ) != 0 )
        {
            continue;
        }

        std::string localPath( originalFileName,
                               dr->getDatabasePath().empty()
                                   ? 0
                                   : dr->getDatabasePath().length() + 1,
                               std::string::npos );

        for( DatabaseRevisions::DatabaseRevisionList::const_iterator itr =
                 dr->getDatabaseRevisionList().begin();
             ...
             ++itr )
        {
            ...
            if( revision->getFilesAdded() &&
                revision->getFilesAdded()->removeFile( localPath ) )
            {
            ...
            if( revision->getFilesRemoved() &&
                revision->getFilesRemoved()->removeFile( localPath ) )
            {
            ...
            if( revision->getFilesModified() &&
                revision->getFilesModified()->removeFile( localPath ) )
            {
...
{
    OSG_INFO << "FileCache::loadDatabaseRevisionsForFile(" << originalFileName << ")"
             << std::endl;
    ...
    for( ; ritr != _databaseRevisionsList.end() && !dr_local; ++ritr )
    {
        OSG_INFO << "   comparing " << ( *ritr )->getName() << " to "
                 << revisionsFileName << std::endl;
    ...
    if( !dr_local )
    {
        if( !cacheFileName.empty() && osgDB::fileExists( cacheFileName ) )
        {
            OSG_INFO << "   found revisions file in local cache, now loading it"
                     << std::endl;
            osg::ref_ptr<osg::Object> object = osgDB::readRefObjectFile( cacheFileName );
            dr_local = dynamic_cast<DatabaseRevisions*>( object.get() );
            if( dr_local )
            {
                OSG_INFO << "   loaded local revisions File(" << cacheFileName << ")"
                         << std::endl;
            }
        }
        else
        {
            OSG_INFO << "   could not load found revisions file from local cache."
                     << std::endl;
        }
    }
    ...
    osg::ref_ptr<osg::Object> object =
        osgDB::readRefObjectFile( revisionsFileName + ".curl" );
    osg::ref_ptr<DatabaseRevisions> dr_remote =
        dynamic_cast<DatabaseRevisions*>( object.get() );
    ...
    if( dr_remote.valid() )
    {
        ...
        if( dr_local.valid() )
        {
            if( dr_local->getDatabaseRevisionList().size() ==
                dr_remote->getDatabaseRevisionList().size() )
            {
                unsigned int i;
                for( i = 0; i < dr_local->getDatabaseRevisionList().size(); ++i )
                {
                    DatabaseRevision* revision_local =
                        dr_local->getDatabaseRevision( i );
                    DatabaseRevision* revision_remote =
                        dr_remote->getDatabaseRevision( i );
                    OSG_INFO << "   Comparing local " << revision_local->getName()
                             << " to remote " << revision_remote->getName() << std::endl;
                    if( revision_local->getName() != revision_remote->getName() )
                    {
                        break;
                    }
                }
                needToWriteRevisionsFileToDisk =
                    ( i != dr_local->getDatabaseRevisionList().size() );
                OSG_INFO << "Local and remote revisions are different "
                         << needToWriteRevisionsFileToDisk << std::endl;
            }
        }

        if( needToWriteRevisionsFileToDisk )
        {
            OSG_INFO << "Need to write DatabaseRevions " << revisionsFileName
                     << " to local FileCache" << std::endl;
            if( !cacheFileName.empty() )
            {
        ...
        {
            OSG_INFO << "No need to write DatabaseRevions " << revisionsFileName
                     << " to local FileCache" << std::endl;
        }
    }
    ...
    if( dr.valid() )
    {
        OSG_INFO << "   loaded remote revisions File(" << revisionsFileName << ")"
                 << std::endl;
        ...
        {
            OSG_INFO << "Added newly loaded DatabaseRevisions object " << dr->getName()
                     << std::endl;
            _databaseRevisionsList.push_back( dr );
        }

        // now need to load the individual FileLists
        for( DatabaseRevisions::DatabaseRevisionList::iterator itr =
                 dr->getDatabaseRevisionList().begin();
             itr != dr->getDatabaseRevisionList().end();
             ++itr )
        {
            ...
            OSG_INFO << "     now loaded DatabaseRevisions " << revision->getName()
                     << " FileList contents" << std::endl;
            if( revision->getFilesAdded() )
            {
                FileList* fileList = readFileList(
                    osgDB::concatPaths( revision->getDatabasePath(),
                                        revision->getFilesAdded()->getName() )
                );
                if( fileList )
                {
            ...
            if( revision->getFilesModified() )
            {
                FileList* fileList = readFileList(
                    osgDB::concatPaths( revision->getDatabasePath(),
                                        revision->getFilesModified()->getName() )
                );
                if( fileList )
                {
    ...
    {
        OSG_NOTICE << "   failed to read revisions File, object.get()=" << object.get()
                   << std::endl;
        return false;
    }
}
...
FileList*
FileCache::readFileList( const std::string& originalFileName ) const
{
    ...
    if( !cacheFileListName.empty() && osgDB::fileExists( cacheFileListName ) )
    {
        ...
        if( fileList )
        {
            OSG_INFO << "     loadeded FileList from local cache " << fileList->getName()
                     << std::endl;
        }
    }
    ...
    if( !fileList )
    {
        ...
        osg::ref_ptr<osg::Object> object =
            osgDB::readRefObjectFile( originalFileName + ".curl" );
        fileList = dynamic_cast<osgDB::FileList*>( object.get() );
        if( fileList )
        {
            OSG_INFO << "     loadeded FileList from remote system "
                     << fileList->getName() << std::endl;
            OSG_INFO << "     Need to write to local file cache " << fileList->getName()
                     << std::endl;
