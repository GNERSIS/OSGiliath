 ...
 * Utility functions for osgDB: strcmp, strncmp, getNoCharacters, strtol.
 */
...
Field::_copy( const Field& ic )
{
    ...
    if( ic._fieldCache )
    {
        ...
        _fieldCache         = new char[static_cast<std::size_t>( _fieldCacheCapacity )];
        strncpy( _fieldCache,
                 ic._fieldCache,
                 static_cast<std::size_t>( _fieldCacheCapacity ) );
    }
    else
    {
...
void
Field::setWithinQuotes( bool withinQuotes )
{
...
bool
Field::getWithinQuotes()
{
    return _withinQuotes;
}

void
Field::setNoNestedBrackets( int no )
{
    _noNestedBrackets = no;
}

int
Field::getNoNestedBrackets()
{
...
Field::addChar( char c )
{
    if( _fieldCache == NULL )
    {
        ...
        _fieldCache = new char[static_cast<std::size_t>( _fieldCacheCapacity )];
        memset( _fieldCache, 0, static_cast<std::size_t>( _fieldCacheCapacity ) );
        _fieldCacheSize = 0;
    }
    else if( _fieldCacheSize >= _fieldCacheCapacity - 1 )
    {
        ...
        _fieldCache   = new char[static_cast<std::size_t>( _fieldCacheCapacity )];
        memset( _fieldCache, 0, static_cast<std::size_t>( _fieldCacheCapacity ) );
        strncpy( _fieldCache, tmp_str, static_cast<std::size_t>( _fieldCacheSize ) );
        delete[] tmp_str;
    }
...
bool
Field::matchWord( const char* str ) const
{
    getFieldType();
...
bool
Field::matchWord( const char* str,
                  int         noCharacters ) const
{
    getFieldType();
    return _fieldType ==
           WORD &&
           strncmp( _fieldCache, str, static_cast<std::size_t>( noCharacters ) ) == 0;
}
...
bool
Field::matchString( const char* str ) const
{
    return strcmp( _fieldCache, str ) == 0;
}

bool
Field::matchString( const char* str,
                    int         noCharacters ) const
{
    return strncmp( _fieldCache, str, static_cast<std::size_t>( noCharacters ) ) == 0;
}

bool
Field::isQuotedString() const
{
    return _withinQuotes;
}
...
bool
Field::getInt( int& i ) const
{
    getFieldType();
    if( _fieldType == INTEGER )
    {
        i = static_cast<int>( strtol( _fieldCache, NULL, 0 ) );
        return true;
    }
    else
    {
...
bool
Field::matchUInt( unsigned int i ) const
{
    getFieldType();
    if( _fieldType == INTEGER )
    {
...
bool
Field::getUInt( unsigned int& i ) const
{
    getFieldType();
    if( _fieldType == INTEGER )
    {
        i = static_cast<unsigned int>( strtoul( _fieldCache, NULL, 0 ) );
        return true;
    }
    else
    {
...
bool
Field::matchFloat( float f ) const
{
    getFieldType();
...
Field::FieldType
Field::calculateFieldType( const char* str,
                           bool        withinQuotes )
{
    if( str == NULL )
    {
