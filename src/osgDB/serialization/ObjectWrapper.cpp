 ...
 * Serialization wrapper for osg::Object subclasses. Maps class
 * properties to read/write methods for binary and XML formats.
 */
...
void
osgDB::split( const std::string& src,
              StringList&        list,
              char               separator )
{
...
void
ObjectWrapper::splitAssociates( const std::string&                    src,
                                ...
                                char                                  separator )
{
    ...
    while( start != std::string::npos )
    {
        ...
        {
            list.push_back(
                ObjectWrapperAssociate( std::string( src, start, end - start ) )
            );
            start = src.find_first_not_of( separator, end );
        }
        else
        {
            list.push_back(
                ObjectWrapperAssociate( std::string( src, start, src.size() - start ) )
            );
            start = end;
        }
    }
...
// ObjectWrapper
//
ObjectWrapper::ObjectWrapper( CreateInstanceFunc* createInstanceFunc,
    ...
    _name( name ),
    _version( 0 ),
    _isAssociatesRevisionsInheritanceDone( false )
{
...
void
ObjectWrapper::setupAssociatesRevisionsInheritanceIfRequired()
{
    if( !_isAssociatesRevisionsInheritanceDone )
    {
             ...
             ++itr )
        {
            ...
            if( assocWrapper && assocWrapper != this )
            {
                ...
                for( ObjectWrapper::RevisionAssociateList::const_iterator itr2 =
                         assocWrapper->getAssociates().begin();
                     itr2 != assocWrapper->getAssociates().end();
                     ++itr2 )
                {
                    for( ObjectWrapper::RevisionAssociateList::iterator itr3 =
                             _associates.begin();
                         itr3 != _associates.end();
                         ++itr3 )
                    {
                        ...
                        {
                            itr3->_firstVersion =
                                itr3->_firstVersion > itr2->_firstVersion
                                    ? itr3->_firstVersion
                                    : itr2->_firstVersion;
                            itr3->_lastVersion = itr3->_lastVersion < itr2->_lastVersion
                                                   ? itr3->_lastVersion
                                                   : itr2->_lastVersion;
                        }
                    }
...
void
ObjectWrapper::markAssociateAsAdded( const std::string& name )
{
    for( ObjectWrapper::RevisionAssociateList::iterator itr = _associates.begin();
         itr != _associates.end();
         ++itr )
    {
        if( itr->_name == name )
    ...
    OSG_NOTIFY( osg::WARN ) << "ObjectWrapper::associateAddedAtVersion: Associate class "
                            << name << " not defined for wrapper " << _name << std::endl;
}

void
ObjectWrapper::markAssociateAsRemoved( const std::string& name )
{
    for( ObjectWrapper::RevisionAssociateList::iterator itr = _associates.begin();
         itr != _associates.end();
         ++itr )
    ...
    OSG_NOTIFY(
        osg::WARN
    ) << "ObjectWrapper::associateRemovedAtVersion: Associate class "
      << name << " not defined for wrapper " << _name << std::endl;
}

void
ObjectWrapper::addSerializer( BaseSerializer*      s,
                              BaseSerializer::Type t )
{
...
void
ObjectWrapper::markSerializerAsRemoved( const std::string& name )
{
    for( SerializerList::iterator itr = _serializers.begin(); itr != _serializers.end();
         ++itr )
    {
        // When a serializer is marked as 'removed', it means that this serializer won't
        // be used any more from specified OSG version (by macro UPDATE_TO_VERSION). The
        // read() functions of higher versions will thus ignore it according to the sign
        // and value of the _version variable.
        if( ( *itr )->getName() == name )
        {
            ( *itr )->_lastVersion = _version - 1;
        }
...
BaseSerializer*
ObjectWrapper::getSerializer( const std::string& name )
{
    for( SerializerList::iterator itr = _serializers.begin(); itr != _serializers.end();
         ++itr )
    {
    ...
    for( RevisionAssociateList::const_iterator itr = _associates.begin();
         itr != _associates.end();
         ++itr )
    {
        const std::string& assocName = itr->_name;
        ObjectWrapper*     assocWrapper =
            Registry::instance()->getObjectWrapperManager()->findWrapper( assocName );
        if( !assocWrapper )
        {
            osg::notify( osg::WARN )
                << "ObjectWrapper::getSerializer(): Unsupported associated class "
                << assocName << std::endl;
            continue;
        }
...
bool
ObjectWrapper::read( InputStream& is,
                     osg::Object& obj )
{
    bool readOK       = true;
         ...
         ++itr )
    {
        ...
        if( serializer->_firstVersion <=
            inputVersion &&
            inputVersion <=
            serializer->_lastVersion &&
            serializer->supportsReadWrite() )
        {
            if( !serializer->read( is, obj ) )
            {
                OSG_WARN << "ObjectWrapper::read(): Error reading property " << _name
    ...
    for( FinishedObjectReadCallbackList::iterator itr =
             _finishedObjectReadCallbacks.begin();
...
bool
ObjectWrapper::write( OutputStream&      os,
                      const osg::Object& obj )
{
         ...
         ++itr )
    {
            ...
            serializer->supportsReadWrite() )
        {
            ...
            {
                OSG_WARN << "ObjectWrapper::write(): Error writing property " << _name
                         << "::" << ( *itr )->getName() << std::endl;
                writeOK = false;
            }
        }
...
bool
ObjectWrapper::readSchema( const StringList& properties,
                           const TypeList& )
{
    // FIXME: At present, I didn't do anything to determine serializers from their
    // types...
    ...
    unsigned int size = static_cast<unsigned int>( properties.size() );
    unsigned int serializersSize =
        static_cast<unsigned int>( _backupSerializers.size() );
...
void
ObjectWrapper::writeSchema( StringList& properties,
                            TypeList&   types )
{
...
void
ObjectWrapper::addMethodObject( const std::string& methodName,
                                MethodObject*      mo )
{
...
ObjectWrapperManager::ObjectWrapperManager()
{
    ...
    glTable.add( "GL_TEXTURE_GEN_S", 0X0C'60 );
    glTable.add( "GL_TEXTURE_GEN_T", 0X0C'61 );
    glTable.add( "GL_TEXTURE_GEN_R", 0X0C'62 );
    glTable.add( "GL_TEXTURE_GEN_Q", 0X0C'63 );
    ...
    glTable.add( "GL_COMPRESSED_LUMINANCE_ALPHA_ARB",
    ...
    glTable.add( "GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG",
                 GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG );
    glTable.add( "GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG",
                 GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG );
    glTable.add( "GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG",
                 GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG );
    glTable.add( "GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG",
    ...
    glTable.add( "GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2",
                 GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2 );
    glTable.add( "GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2",
                 GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2 );
    glTable.add( "GL_COMPRESSED_RGBA8_ETC2_EAC", GL_COMPRESSED_RGBA8_ETC2_EAC );
    glTable.add( "GL_COMPRESSED_SRGB8_ALPHA8_ETC2_EAC",
    ...
    primitiveTable.add2( "GL_LINES_ADJACENCY_EXT",
                         "GL_LINES_ADJACENCY",
                         GL_LINES_ADJACENCY );
    primitiveTable.add2( "GL_LINE_STRIP_ADJACENCY_EXT",
                         "GL_LINE_STRIP_ADJACENCY",
                         GL_LINE_STRIP_ADJACENCY );
    primitiveTable.add2( "GL_TRIANGLES_ADJACENCY_EXT",
...
void
ObjectWrapperManager::addWrapper( ObjectWrapper* wrapper )
{
    if( !wrapper )
    {
        return;
    ...
    std::lock_guard<std::recursive_mutex> lock( _wrapperMutex );
...
void
ObjectWrapperManager::removeWrapper( ObjectWrapper* wrapper )
{
    if( !wrapper )
    {
        return;
...
ObjectWrapper*
ObjectWrapperManager::findWrapper( const std::string& name )
{
    std::lock_guard<std::recursive_mutex> lock( _wrapperMutex );
    ...
    if( posDoubleColon != std::string::npos )
    {
        ...
        std::string    nodeKitLib =
            osgDB::Registry::instance()->createLibraryNameForNodeKit( libName );
        if( osgDB::Registry::instance()->loadLibrary( nodeKitLib ) ==
            osgDB::Registry::LOADED )
        {
        ...
        if( osgDB::Registry::instance()->loadLibrary( pluginLib ) ==
            osgDB::Registry::LOADED )
        {
            found = findWrapper( name );
        }
        ...
        if( found )
        {
            found->setupAssociatesRevisionsInheritanceIfRequired();
        }
...
void
ObjectWrapperManager::addCompressor( BaseCompressor* compressor )
{
    if( !compressor )
    {
        return;
...
void
ObjectWrapperManager::removeCompressor( BaseCompressor* compressor )
{
    if( !compressor )
    {
        return;
...
BaseCompressor*
ObjectWrapperManager::findCompressor( const std::string& name )
{
    std::lock_guard<std::recursive_mutex> lock( _wrapperMutex );
    ...
    // Load external libraries
    std::string nodeKitLib =
        osgDB::Registry::instance()->createLibraryNameForNodeKit( name );
    if( osgDB::Registry::instance()->loadLibrary( nodeKitLib ) ==
        osgDB::Registry::LOADED )
    {
    ...
    std::string pluginLib = osgDB::Registry::instance()->createLibraryNameForExtension(
        std::string( "compressor_" ) + name
    );
    if( osgDB::Registry::instance()->loadLibrary( pluginLib ) ==
        osgDB::Registry::LOADED )
    {
...
//
RegisterCompressorProxy::RegisterCompressorProxy( const std::string& name,
    ...
    _compressor( compressor )
{
    ...
    if( Registry::instance() )
    {
        Registry::instance()->getObjectWrapperManager()->addCompressor(
            _compressor.get()
        );
    }
}
...
RegisterCompressorProxy::~RegisterCompressorProxy()
{
    if( Registry::instance() )
    {
        Registry::instance()->getObjectWrapperManager()->removeCompressor(
            _compressor.get()
        );
    }
}
