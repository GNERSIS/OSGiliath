 ...
 * Utility functions for osgDB: _init, _readField.
 */
#include <istream>
...
FieldReader&
FieldReader::operator=( const FieldReader& ic )
{
    if( this == &ic )
    {
        return *this;
...
void
FieldReader::_copy( const FieldReader& ic )
{
...
void
FieldReader::attach( std::istream* input )
{
    _fin = input;
...
bool
FieldReader::eof() const
{
    return _eof;
}

bool
FieldReader::findStartOfNextField()
{
    int ch = 0;
    while( true )
    {
...
bool
FieldReader::readField( Field& fieldPtr )
{
    return _readField( &fieldPtr );
}

void
FieldReader::ignoreField()
{
    _readField( NULL );
}

bool
FieldReader::_readField( Field* fieldPtr )
{
    if( fieldPtr )
    {
        fieldPtr->reset();
    ...
    if( !eof() && findStartOfNextField() )
    {
        ...
        else if( ch == '"' )
        {
            ...
            _fin->ignore( 1 );
            char c;
            bool escape = false;    // use the escape character sequence \" to allow " to
                                    // included in strings.
            while( true )
            {
                ch = _fin->peek();
...
int
FieldReader::getNoNestedBrackets() const
{
