 ...
 * Utility functions for osgDB: Field, field.
 */
...
void
FieldReaderIterator::_copy( const FieldReaderIterator& ic )
{
    _reader = ic._reader;
    ...
    {
        _fieldQueue = new Field*[static_cast<std::size_t>( ic._fieldQueueCapacity )];
...
void
FieldReaderIterator::attach( std::istream* input )
{
    _reader.attach( input );
}

void
FieldReaderIterator::detach()
{
    _reader.detach();
}

bool
FieldReaderIterator::eof() const
{
    return _fieldQueueSize == 0 && _reader.eof();
}

void
FieldReaderIterator::insert( int    pos,
                             Field* field )
{
    if( field == NULL )
    {
    ...
    if( _fieldQueueSize >= _fieldQueueCapacity )
    {
        ...
        Field** newFieldStack = new Field*[static_cast<std::size_t>( newCapacity )];
...
Field&
FieldReaderIterator::operator[]( int pos )
{
    return field( pos );
}

Field&
FieldReaderIterator::field( int pos )
{
    if( pos < 0 )
    {
...
FieldReaderIterator::operator+=( int no )
{
    ...
    {
        Field** tmpFields = new Field*[static_cast<std::size_t>( no )];
        int     i;
        for( i = 0; i < no; ++i )
        {
...
void
FieldReaderIterator::advanceOverCurrentFieldOrBlock()
{
...
void
FieldReaderIterator::advanceToEndOfCurrentBlock()
{
...
void
FieldReaderIterator::advanceToEndOfBlock( int noNestedBrackets )
{
...
bool
FieldReaderIterator::matchSequence( const char* str )
{
    if( str == NULL )
    {
        return false;
        ...
        else
        {
            if( start != end )
            {
                ...
                else
                {
                    ...
                    {
                        if( !field( fieldCount )
                                 .matchWord( start, static_cast<int>( end - start ) ) )
                        {
                            return false;
                        }
                    }
...
bool
FieldReaderIterator::readSequence( const char*  keyword,
                                   std::string& value )
{
...
FieldReaderIterator::readSequence( const char* keyword,
                                   float&      value )
{
    if( ( *this )[0].matchWord( keyword ) && ( *this )[1].getFloat( value ) )
    {
        ( *this ) += 2;
        return true;
    }
...
bool
FieldReaderIterator::readSequence( std::string& value )
{
    if( ( *this )[0].isString() )
    {
        value      = ( *this )[0].getStr();
...
bool
FieldReaderIterator::readSequence( unsigned int& value )
{
...
bool
FieldReaderIterator::readSequence( int& value )
{
...
bool
FieldReaderIterator::readSequence( float& value )
{
...
{
    if( ( *this )[0].getFloat( value[0] ) && ( *this )[1].getFloat( value[1] ) )
    {
        ( *this ) += 2;
        return true;
    }
...
bool
FieldReaderIterator::readSequence( osg::vec3& value )
{
...
bool
FieldReaderIterator::readSequence( osg::vec4& value )
{
    if( ( *this )[0].getFloat( value[0] ) &&
        ( *this )[1].getFloat( value[1] ) &&
        ( *this )[2].getFloat( value[2] ) &&
...
bool
FieldReaderIterator::readSequence( osg::dvec2& value )
{
    if( ( *this )[0].getFloat( value[0] ) && ( *this )[1].getFloat( value[1] ) )
    {
        ( *this ) += 2;
...
bool
FieldReaderIterator::readSequence( osg::dvec3& value )
{
    if( ( *this )[0].getFloat( value[0] ) &&
        ( *this )[1].getFloat( value[1] ) &&
        ( *this )[2].getFloat( value[2] ) )
...
bool
FieldReaderIterator::readSequence( osg::dvec4& value )
{
    if( ( *this )[0].getFloat( value[0] ) &&
        ( *this )[1].getFloat( value[1] ) &&
        ( *this )[2].getFloat( value[2] ) &&
