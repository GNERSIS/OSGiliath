 ...
 * NullCompressor, derived from BaseCompressor.
 * Provides: compress, decompress, REGISTER_COMPRESSOR, ZLibCompressor, compress,
 * decompress.
 */
...
class NullCompressor : public BaseCompressor
{
    public:
        ...
        virtual bool
        compress( std::ostream&      fout,
                  const std::string& src )
        {
            int size = static_cast<int>( src.size() );
            fout.write( ( char* )&size, INT_SIZE );
            fout.write( src.c_str(), static_cast<std::streamsize>( src.size() ) );
            return true;
        }

        virtual bool
        decompress( std::istream& fin,
                    std::string&  target )
        {
            int size = 0;
            fin.read( ( char* )&size, INT_SIZE );
            if( size )
            {
