                              ...
                              ReadWriteMode      readWriteMode )
{
    ...
    {
        std::string::size_type end_of_name =
            associates.find_first_of( ' ', start_of_name );
        if( end_of_name != std::string::npos )
        {
            _associates.push_back(
                std::string( associates, start_of_name, end_of_name - start_of_name )
            );
            start_of_name = associates.find_first_not_of( ' ', end_of_name );
        }
        else
        {
            _associates.push_back( std::string( associates,
                                                start_of_name,
                                                associates.size() - start_of_name ) );
...
)
{
    ...
    {
        _wrapper = new DotOsgWrapper( proto,
                                      name,
                                      associates,
                                      readFunc,
                                      writeFunc,
...
RegisterDotOsgWrapperProxy::~RegisterDotOsgWrapperProxy()
{
    if( Registry::instance() )
    {
        Registry::instance()
            ->getDeprecatedDotOsgObjectWrapperManager()
            ->removeDotOsgWrapper( _wrapper.get() );
    }
}

void
DeprecatedDotOsgWrapperManager::addDotOsgWrapper( DotOsgWrapper* wrapper )
{
    if( wrapper == 0L )
    {
        return;
    ...
    // OSG_INFO << "osg::Registry::addDotOsgWrapper("<<wrapper->getName()<<")"<<
    // std::endl;
...
void
DeprecatedDotOsgWrapperManager::eraseWrapper( DotOsgWrapperMap& wrappermap,
                                              DotOsgWrapper*    wrapper )
{
    typedef std::vector<DotOsgWrapperMap::iterator> EraseList;
    EraseList                                       eraseList;
    for( DotOsgWrapperMap::iterator witr = wrappermap.begin(); witr != wrappermap.end();
         ++witr )
    {
    ...
    for( EraseList::iterator eitr = eraseList.begin(); eitr != eraseList.end(); ++eitr )
    {
...
void
DeprecatedDotOsgWrapperManager::removeDotOsgWrapper( DotOsgWrapper* wrapper )
{
    if( wrapper == 0L )
    {
        return;
...
struct concrete_wrapper : basic_type_wrapper
{
        ...
        bool
        matches( const osg::Object* proto ) const
        {
...
osg::Object*
DeprecatedDotOsgWrapperManager::readObjectOfType( const osg::Object& compObj,
                                                  Input&             fr )
{
...
bool
DeprecatedDotOsgWrapperManager::getLibraryFileNamesToTry( const std::string& name,
                                                          FileNames&         fileNames )
{
    ...
    libraryName = osgDB::Registry::instance()->createLibraryNameForExtension(
        std::string( "deprecated_" ) + name
    );
    if( !libraryName.empty() )
    {
        fileNames.push_back( libraryName );
...
osg::Object*
DeprecatedDotOsgWrapperManager::readObjectOfType( const basic_type_wrapper& btw,
                                                  Input&                    fr )
{
    ...
    if( ow_itr == _objectWrapperMap.end() )
    {
        ...
        if( posDoubleColon != std::string::npos )
        {
            ...
            {
                for( FileNames::iterator itr = fileNames.begin(); itr != fileNames.end();
                     ++itr )
                {
                    if( osgDB::Registry::instance()->loadLibrary( *itr ) ==
    ...
    else if( fr[1].isOpenBracket() )
    {
        ...
        if( proto == NULL )
        {
            OSG_WARN << "Token " << fr[0].getStr()
                     << " read, but has no prototype, cannot load." << std::endl;
            return NULL;
        }
                 ...
                 ++aitr )
            {
                ...
                if( mitr == _objectWrapperMap.end() )
                {
                    ...
                    if( posDoubleColon != std::string::npos )
                    {
                        ...
                        // if we can recognize the objects.
                        std::string libraryName =
                            std::string( token, 0, posDoubleColon );
...
//
osg::Object*
DeprecatedDotOsgWrapperManager::readObject( DotOsgWrapperMap& dowMap,
                                            Input&            fr )
{
    const char* str = fr[0].getStr();
    if( str == NULL )
...
//
osg::Object*
DeprecatedDotOsgWrapperManager::readObject( Input& fr )
{
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
...
//
osg::Image*
DeprecatedDotOsgWrapperManager::readImage( Input& fr )
{
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
        {
            osg::Image* image =
                dynamic_cast<osg::Image*>( fr.getObjectForUniqueID( fr[1].getStr() ) );
            if( image )
            {
                fr += 2;
            }
...
//
osg::Drawable*
DeprecatedDotOsgWrapperManager::readDrawable( Input& fr )
{
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
        {
            osg::Drawable* drawable = dynamic_cast<osg::Drawable*>(
                fr.getObjectForUniqueID( fr[1].getStr() )
            );
            if( drawable )
            {
                fr += 2;
...
//
osg::StateAttribute*
DeprecatedDotOsgWrapperManager::readStateAttribute( Input& fr )
{
    ...
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
        {
            osg::StateAttribute* attribute = dynamic_cast<osg::StateAttribute*>(
                fr.getObjectForUniqueID( fr[1].getStr() )
            );
            if( attribute )
            {
...
//
osg::Uniform*
DeprecatedDotOsgWrapperManager::readUniform( Input& fr )
{
    ...
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
        {
            osg::Uniform* attribute =
                dynamic_cast<osg::Uniform*>( fr.getObjectForUniqueID( fr[1].getStr() ) );
            if( attribute )
            {
                fr += 2;
            }
...
//
osg::Node*
DeprecatedDotOsgWrapperManager::readNode( Input& fr )
{
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
        {
            osg::Node* node =
                dynamic_cast<osg::Node*>( fr.getObjectForUniqueID( fr[1].getStr() ) );
            if( node )
            {
                fr += 2;
            }
...
//
osg::Shader*
DeprecatedDotOsgWrapperManager::readShader( Input& fr )
{
    if( fr[0].matchWord( "Use" ) )
    {
        if( fr[1].isString() )
        {
            osg::Shader* shader =
                dynamic_cast<osg::Shader*>( fr.getObjectForUniqueID( fr[1].getStr() ) );
            if( shader )
            {
                fr += 2;
            }
...
//
bool
DeprecatedDotOsgWrapperManager::writeObject( const osg::Object& obj,
                                             Output&            fw )
{
