...
static osg::ApplicationUsageProxy
    Output_e0( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
...
void
Output::setOptions( const Options* options )
{
    _options = options;
}

void
Output::open( const char* name )
{
    init();
...
std::string
Output::wrapString( const char* str )
{
    if( !str )
    {
        return std::string( "\"\"" );
...
std::string
Output::wrapString( const std::string& str )
{
...
bool
Output::writeObject( const osg::Object& obj )
{
    return Registry::instance()->getDeprecatedDotOsgObjectWrapperManager()->writeObject(
        obj,
        *this
    );
}
...
void
Output::writeBeginObject( const std::string& name )
{
...
void
Output::writeUseID( const std::string& id )
{
    indent() << "Use " << id << std::endl;
}

void
Output::writeUniqueID( const std::string& id )
{
    indent() << "UniqueID " << id << std::endl;
}

bool
Output::getUniqueIDForObject( const osg::Object* obj,
                              std::string&       uniqueID )
{
...
bool
Output::createUniqueIDForObject( const osg::Object* obj,
                                 std::string&       uniqueID )
{
    char str[256];
    sprintf( str,
             "%s_%i",
             obj->className(),
             ( unsigned int )_objectToUniqueIDMap.size() );
    uniqueID = str;
    return true;
}

bool
Output::registerUniqueIDForObject( const osg::Object* obj,
                                   std::string&       uniqueID )
{
...
std::string
Output::getFileNameForOutput( const std::string& filename ) const
{
        ...
        case( FULL_PATH ) :
            {
                ...
                OSG_WARN << "Warning: Output::getFileNameForOutput() does not support "
                            "FULL_PATH yet."
                         << std::endl;
                return filename;
            }
...
std::string
Output::getTextureFileNameForOutput()
{
...
void
Output::setExternalFileWritten( const std::string& filename,
                                bool               hasBeenWritten )
{
...
bool
Output::getExternalFileWritten( const std::string& filename ) const
{
