...
namespace
{
    ...
    // basis vectors
    class TsgVisitor : public DualModeVisitor
    {
        public:

            TsgVisitor( BumpMapping* bm ) :
    ...
    class TexCoordGenerator : public osg::DualModeVisitor
    {
        public:

            TexCoordGenerator( int du,
                               int nu ) :
                DualModeVisitor( NodeVisitor::TRAVERSE_ALL_CHILDREN ),
                du_( du ),
                nu_( nu )
            ...
            apply( osg::Geode& geode ) override
            {
                const osg::sphere& bsphere = geode.getBound();
                float              scale   = 10;
                if( bsphere.radius != 0 )
                {
                ...
                for( unsigned i = 0; i < geode.getNumDrawables(); ++i )
                {
                    ...
                    if( geo )
                    {
                        osg::ref_ptr<osg::Vec2Array> tc =
                            generate_coords( geo->getVertexArray(),
                                             geo->getNormalArray(),
                                             scale );
                        geo->setTexCoordArray( static_cast<unsigned int>( du_ ),
                                               tc.get() );
                        geo->setTexCoordArray( static_cast<unsigned int>( nu_ ),
                                               tc.get() );
                    }
                }
                NodeVisitor::apply( geode );
...
BumpMapping::BumpMapping( const BumpMapping& copy,
...
void
BumpMapping::prepareGeometry( osg::Geometry* geo )
{
...
void
BumpMapping::prepareNode( osg::Node* node )
{
...
BumpMapping::setUpDemo()
{
    ...
    if( !_diffuse_tex.valid() )
    {
        ...
        _diffuse_tex->setImage(
            osgDB::readRefImageFile( "Images/whitemetal_diffuse.jpg" )
        );
        _diffuse_tex->setFilter( osg::Texture::MIN_FILTER,
    ...
    if( !_normal_tex.valid() )
    {
        ...
        _normal_tex->setImage(
            osgDB::readRefImageFile( "Images/whitemetal_normal.jpg" )
        );
        _normal_tex->setFilter( osg::Texture::MIN_FILTER,
                                osg::Texture::LINEAR_MIPMAP_LINEAR );
