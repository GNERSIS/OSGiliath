...
namespace
{
    ...
    create_default_image()
    {
        ...
        image->setImage( _texturesize,
                         _texturesize,
                         1,
                         3,
                         GL_RGB,
                         GL_UNSIGNED_BYTE,
                         new unsigned char[3 * _texturesize * _texturesize],
                         osg::Image::USE_NEW_DELETE );
        for( unsigned int i = 0; i < static_cast<unsigned int>( _texturesize ); ++i )
        {
            for( unsigned int j = 0; j < static_cast<unsigned int>( _texturesize ); ++j )
            {
...
AnisotropicLighting::AnisotropicLighting( const AnisotropicLighting& copy,
...
bool
AnisotropicLighting::define_techniques()
{
