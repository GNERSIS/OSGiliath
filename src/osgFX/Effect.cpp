...
Effect::~Effect()
{
    ...
    if( _dummy_for_validation.valid() )
    {
        ...
        if( ss )
        {
            Validator* validator = dynamic_cast<Validator*>(
                ss->getAttribute( Validator::Type::VALIDATOR )
            );
            if( validator )
            {
                validator->disable();
...
void
Effect::traverse( osg::NodeVisitor& nv )
{
    ...
    if( !_techs_defined )
    {
        ...
        if( !_techs_defined )
        {
            OSG_WARN << "Warning: osgFX::Effect: could not define techniques for effect "
                     << className() << std::endl;
            return;
        }
        ...
        if( _techs.empty() )
        {
            OSG_WARN << "Warning: osgFX::Effect: no techniques defined for effect "
                     << className() << std::endl;
            return;
        }
    }
    ...
    if( _global_sel_tech == AUTO_DETECT )
    {
        ...
        if( max_index >= 0 )
        {
            tech = _techs[static_cast<std::size_t>( max_index )].get();
        }
    }
    else
    {
        ...
        tech = _techs[static_cast<std::size_t>( _global_sel_tech )].get();
    }
...
Effect::resizeGLObjectBuffers( unsigned int maxSize )
{
    ...
    if( _dummy_for_validation )
    {
        _dummy_for_validation->resizeGLObjectBuffers( maxSize );
    }
}
...
Effect::releaseGLObjects( osg::State* state ) const
{
    ...
    if( _dummy_for_validation )
    {
        _dummy_for_validation->releaseGLObjects( state );
    }
}

void
Effect::build_dummy_node()
{
