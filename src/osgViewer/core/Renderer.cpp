...
void
EXTQuerySupport::checkQuery( osg::Stats* stats,
                             osg::State* /*state*/,
                             osg::Timer_t startTick )
{
         ...
         itr != _queryFrameNumberList.end(); )
    {
        ...
        if( available )
        {
            ...
            double currentTime =
                osg::Timer::instance()->delta_s( startTick,
    ...
    _previousQueryTime =
        osg::Timer::instance()->delta_s( startTick, osg::Timer::instance()->tick() );
}

GLuint
EXTQuerySupport::createQueryObject()
{
...
void
EXTQuerySupport::beginQuery( unsigned int frameNumber,
                             osg::State* /*state*/ )
{
...
void
EXTQuerySupport::endQuery( osg::State* /*state*/ )
{
    _extensions->glEndQuery( GL_TIME_ELAPSED );
}

void
OpenGLQuerySupport::initialize( osg::State* state,
                                osg::Timer_t /*startTick*/ )
{
...
void
EXTQuerySupport::initialize( osg::State*  state,
                             osg::Timer_t startTick )
{
...
class ARBQuerySupport : public OpenGLQuerySupport
{
    ...
    protected:
        ...
        struct ActiveQuery
        {
                ...
                ActiveQuery( GLuint start_,
                             GLuint end_,
                             int    frameNumber_ ) :
                    queries( start_,
                             end_ ),
                    frameNumber( static_cast<unsigned int>( frameNumber_ ) )
                {
                }

                ActiveQuery( const QueryPair& queries_,
...
void
ARBQuerySupport::initialize( osg::State*  state,
                             osg::Timer_t startTick )
{
    OpenGLQuerySupport::initialize( state, startTick );
}
...
void
ARBQuerySupport::beginQuery( unsigned int frameNumber,
                             osg::State* /*state*/ )
{
    QueryPair query;
    if( _availableQueryObjects.empty() )
...
void
ARBQuerySupport::endQuery( osg::State* /*state*/ )
{
    _extensions->glQueryCounter( _queryFrameList.back().queries.second, GL_TIMESTAMP );
}

void
ARBQuerySupport::checkQuery( osg::Stats* stats,
                             osg::State* state,
                             osg::Timer_t /*startTick*/ )
{
         ...
         itr != _queryFrameList.end(); )
    {
        ...
        if( available )
        {
            ...
            _extensions->glGetQueryObjectui64v( queries.first,
                                                GL_QUERY_RESULT,
                                                &beginTimestamp );
            _extensions->glGetQueryObjectui64v( queries.second,
                                                GL_QUERY_RESULT,
                                                &endTimestamp );
            ...
            if( tbits < 64 )
            {
                ...
                const GLuint64 sum     = ( beginTimestamp >> hiShift ) +
                                         ( endTimestamp >> hiShift ) +
                                         ( gpuTimestamp >> hiShift );
                if( sum == 1 || sum == 2 )
                {
                    ...
                    else if( gpuTimestamp <
                             beginTimestamp &&
                             beginTimestamp -
                             gpuTimestamp > ( hiMask >> 1 ) )
                    {
            ...
            stats->setAttribute( itr->frameNumber,
                                 "GPU draw time taken",
                                 timeElapsedSeconds );
...
void
Renderer::ThreadSafeQueue::release()
{
...
void
Renderer::ThreadSafeQueue::reset()
{
    std::lock_guard<std::mutex> lock( _mutex );
    _queue.clear();
    _isReleased = false;
...
osgUtil::SceneView*
Renderer::ThreadSafeQueue::takeFront()
{
...
void
Renderer::ThreadSafeQueue::add( osgUtil::SceneView* sv )
{
    std::lock_guard<std::mutex> lock( _mutex );
    _queue.push_back( sv );
    _isReleased = true;
...
static std::recursive_mutex s_drawSerializerMutex;
    ...
    _startTick( 0 )
{
    ...
    // each SceneView to have their own FrameStamp to avoid thread conflicts with the
    // Viewer's main FrameStamp
    _sceneView[0]->setFrameStamp( new osg::FrameStamp() );
    _sceneView[1]->setFrameStamp( new osg::FrameStamp() );

    osg::Camera* masterCamera =
        _camera->getView() ? _camera->getView()->getCamera() : camera;
    ...
    {
        // assign CullVisitor::Identifier so that the double buffering of SceneView
        // doesn't interfer with code that requires a consistent knowledge and which
        // effective cull traversal to taking place
        osg::ref_ptr<osgUtil::CullVisitor::Identifier> leftEyeIdentifier =
            new osgUtil::CullVisitor::Identifier();
        osg::ref_ptr<osgUtil::CullVisitor::Identifier> rightEyeIdentifier =
            new osgUtil::CullVisitor::Identifier();
    ...
    // prevent the draw traversal from reading from it before the cull traversal has been
    // completed.
    _availableQueue.add( _sceneView[0].get() );
    _availableQueue.add( _sceneView[1].get() );

    DEBUG_MESSAGE << "_availableQueue.size()=" << _availableQueue._queue.size()
                  << std::endl;
}
...
void
Renderer::initialize( osg::State* state )
{
    if( !_initialized )
    {
        _initialized           = true;
...
void
Renderer::setGraphicsThreadDoesCull( bool flag )
{
...
void
Renderer::updateSceneView( osgUtil::SceneView* sceneView )
{
    osg::Camera* masterCamera =
        _camera->getView() ? _camera->getView()->getCamera() : _camera.get();
...
static void
collectSceneViewStats( unsigned int        frameNumber,
                       ...
                       osg::Stats*         stats )
{
    ...
    stats->setAttribute( frameNumber,
                         ...
                         static_cast<double>( sceneStats._vertexCount ) );
    stats->setAttribute( frameNumber,
                         "Visible number of drawables",
                         static_cast<double>( sceneStats.numDrawables ) );
    stats->setAttribute( frameNumber,
                         "Visible number of fast drawables",
                         static_cast<double>( sceneStats.numFastDrawables ) );
    stats->setAttribute( frameNumber,
                         "Visible number of lights",
                         static_cast<double>( sceneStats.nlights ) );
    stats->setAttribute( frameNumber,
                         "Visible number of render bins",
                         static_cast<double>( sceneStats.nbins ) );
    stats->setAttribute( frameNumber,
                         "Visible depth",
                         static_cast<double>( sceneStats.depth ) );
    stats->setAttribute( frameNumber,
                         "Number of StateGraphs",
                         static_cast<double>( sceneStats.numStateGraphs ) );
    stats->setAttribute( frameNumber,
                         "Visible number of impostors",
                         static_cast<double>( sceneStats.nimpostor ) );
    stats->setAttribute( frameNumber,
                         "Number of ordered leaves",
                         static_cast<double>( sceneStats.numOrderedLeaves ) );

    unsigned int                                  totalNumPrimitiveSets = 0;
    const osgUtil::Statistics::PrimitiveValueMap& pvm =
        sceneStats.getPrimitiveValueMap();
    ...
    stats->setAttribute( frameNumber,
                         "Visible number of PrimitiveSets",
                         static_cast<double>( totalNumPrimitiveSets ) );

    osgUtil::Statistics::PrimitiveCountMap& pcm = sceneStats.getPrimitiveCountMap();
    stats->setAttribute( frameNumber,
                         "Visible number of GL_POINTS",
                         static_cast<double>( pcm[GL_POINTS] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_LINES",
                         static_cast<double>( pcm[GL_LINES] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_LINE_STRIP",
                         static_cast<double>( pcm[GL_LINE_STRIP] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_LINE_LOOP",
                         static_cast<double>( pcm[GL_LINE_LOOP] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_TRIANGLES",
                         static_cast<double>( pcm[GL_TRIANGLES] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_TRIANGLE_STRIP",
                         static_cast<double>( pcm[GL_TRIANGLE_STRIP] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_TRIANGLE_FAN",
                         static_cast<double>( pcm[GL_TRIANGLE_FAN] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_QUADS",
                         static_cast<double>( pcm[GL_QUADS] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_QUAD_STRIP",
                         static_cast<double>( pcm[GL_QUAD_STRIP] ) );
    stats->setAttribute( frameNumber,
                         "Visible number of GL_POLYGON",
                         static_cast<double>( pcm[GL_POLYGON] ) );
}
    ...
    if( sceneView )
    {
        ...
        if( stats && stats->collectStats( "rendering" ) )
        {
            ...
            stats->setAttribute( frameNumber,
                                 ...
                                 osg::Timer::instance()->delta_s( beforeCullTick,
                                                                  afterCullTick ) );
        }
...
Renderer::draw()
{
    ...
    if( sceneView && !_done )
    {
        // since we are running the draw thread in parallel with the main thread it's
        // possible to unreference Camera's that are still being used by this rendering
        // thread, so to prevent this we'll take references to all these Camera's and the
        // clear these references once we've completed the whole draw dispatch.
        ...
        {
            OSG_INFO << "Renderer::draw() completing early due to change in "
                        "_graphicsThreadDoesCull flag."
                     << std::endl;
            return;
        }
        ...
        if( sceneView->getDynamicObjectCount() ==
            0 &&
        ...
        if( _serializeDraw )
        {
            std::lock_guard<std::recursive_mutex> lock( s_drawSerializerMutex );
        ...
        // "<<osg::Timer::instance()->delta_m(startDrawTick, beforeDrawTick)<<std::endl;
        // OSG_NOTICE<<"     time for draw =
        // "<<osg::Timer::instance()->delta_m(beforeDrawTick, afterDrawTick)<<std::endl;
        ...
        if( stats && stats->collectStats( "rendering" ) )
        {
            ...
            stats->setAttribute( frameNumber,
                                 ...
                                 osg::Timer::instance()->delta_s( beforeDrawTick,
                                                                  afterDrawTick ) );
        }
...
void
Renderer::operator()( osg::Object* object )
{
...
void
Renderer::operator()( osg::GraphicsContext* /*context*/ )
{
    if( _graphicsThreadDoesCull )
    {
        cull_draw();
...
void
Renderer::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
Renderer::releaseGLObjects( osg::State* state ) const
{
...
void
Renderer::setCameraRequiresSetUp( bool flag )
{
    for( int i = 0; i < 2; ++i )
    {
        osgUtil::SceneView*   sv = getSceneView( static_cast<unsigned int>( i ) );
...
bool
Renderer::getCameraRequiresSetUp() const
{
    bool result = false;
    for( int i = 0; i < 2; ++i )
    {
        const osgUtil::SceneView*   sv = getSceneView( static_cast<unsigned int>( i ) );
