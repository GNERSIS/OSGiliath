...
static osg::ApplicationUsageProxy
    ViewerBase_e0( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_CONFIG_FILE <filename>",
                   "Specify a viewer configuration file to load by default." );
static osg::ApplicationUsageProxy
    ViewerBase_e1( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_THREADING <value>",
...
static osg::ApplicationUsageProxy
    ViewerBase_e2( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_SCREEN <value>",
                   "Set the default screen that windows should open up on." );
static osg::ApplicationUsageProxy
    ViewerBase_e3( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_WINDOW x y width height",
                   "Set the default window dimensions that windows should open up on." );
static osg::ApplicationUsageProxy
    ViewerBase_e4( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_RUN_FRAME_SCHEME",
                   "Frame rate manage scheme that viewer run should use,  ON_DEMAND or "
                   "CONTINUOUS (default)." );
static osg::ApplicationUsageProxy
    ViewerBase_e5( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_RUN_MAX_FRAME_RATE",
                   "Set the maximum number of frame as second that viewer run. 0.0 is "
                   "default and disables an frame rate capping." );
static osg::ApplicationUsageProxy
    ViewerBase_e6( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                   "OSG_RUN_FRAME_COUNT",
...
void
ViewerBase::configureAffinity()
{
    unsigned int numProcessors =
        static_cast<unsigned int>( osg::GetNumberOfProcessors() );

    OSG_INFO << "ViewerBase::configureAffinity() numProcessors=" << numProcessors
             << std::endl;
...
#if 1
    // for hyper-threaed processors we want to place the threads on preferentiallly on
    // 0,2,4,6
    ...
    _affinity = osg::Thread::Affinity( availableProcessors[availableProcessor] );

    // all threading models except DrawThreadPerContext can share the first cull or
    // culldraw threads with thread with the main thread, so only increment the
    // availableProcessor for DrawThreadPerContext to prevent draw threads sitting on the
    // same thread as main thread that does cull
    ...
    if( requiresCameraThreads )
    {
        ...
        {
            ( *itr )->setProcessorAffinity( osg::Thread::Affinity(
                availableProcessors[availableProcessor++ % availableProcessors.size()]
            ) );
        }
    }
        ...
        for( Contexts::iterator itr = contexts.begin(); itr != contexts.end(); ++itr )
        {
            if( ( *itr )->getTraits() )
            {
                osg::GraphicsContext::Traits* traits =
                    const_cast<osg::GraphicsContext::Traits*>( ( *itr )->getTraits() );
                traits->affinity = osg::Thread::Affinity(
    ...
    if( availableProcessor < numProcessors )
    {
        ...
        osg::Thread::Affinity databasePagerAffinity;
        for( DatabasePagers::iterator itr = databasePagers.begin();
             itr != databasePagers.end();
             ++itr )
        {
            ( *itr )->setProcessorAffinity(
                osg::Thread::Affinity( availableProcessor,
                                       numProcessors - availableProcessor )
            );
        }
    }
}
...
void
ViewerBase::setThreadingModel( ThreadingModel threadingModel )
{
...
ViewerBase::ThreadingModel
ViewerBase::suggestBestThreadingModel()
{
    std::string str;
    ...
    int numProcessors = osg::GetNumberOfProcessors();
...
ViewerBase::setUpThreading()
{
    ...
    osg::SetProcessorAffinityOfCurrentThread( _affinity );
    ...
    // set up the number of graphics contexts.
    {
        ...
        for( Scenes::iterator scitr = scenes.begin(); scitr != scenes.end(); ++scitr )
        {
            ...
            {
                // update the scene graph so that it has enough GL object buffer memory
                // for the graphics contexts that will be using it.
...
void
ViewerBase::setEndBarrierPosition( BarrierPosition bp )
{
...
void
ViewerBase::setEndBarrierOperation( osg::BarrierOperation::PreBlockOp op )
{
...
ViewerBase::startThreading()
{
    ...
    OSG_INFO << "Viewer::startThreading() - contexts.size()=" << contexts.size()
             << std::endl;
    ...
    switch( _threadingModel )
    {
        case( SingleThreaded ) :
            ...
            return;
        case( CullDrawThreadPerContext ) :
            numThreadsOnStartBarrier = static_cast<unsigned int>( contexts.size() ) + 1;
            numThreadsOnEndBarrier   = static_cast<unsigned int>( contexts.size() ) + 1;
            break;
        case( DrawThreadPerContext ) :
            numThreadsOnStartBarrier = 1;
            numThreadsOnEndBarrier   = 1;
            break;
        case( CullThreadPerCameraDrawThreadPerContext ) :
            numThreadsOnStartBarrier = static_cast<unsigned int>( cameras.size() ) + 1;
            numThreadsOnEndBarrier   = 1;
            break;
        default :
    ...
    for( Scenes::iterator scitr = scenes.begin(); scitr != scenes.end(); ++scitr )
    {
        if( ( *scitr )->getSceneData() )
        {
            ...
            // update the scene graph so that it has enough GL object buffer memory for
            // the graphics contexts that will be using it.
            ( *scitr )->getSceneData()->resizeGLObjectBuffers(
                osg::DisplaySettings::instance()->getMaxNumberOfGraphicsContexts()
            );
        }
    ...
    bool               graphicsThreadsDoesCull =
        _threadingModel == CullDrawThreadPerContext || _threadingModel == SingleThreaded;
             ...
             _threadingModel == CullThreadPerCameraDrawThreadPerContext )
    {
        ...
        _endDynamicDrawBlock =
            new osg::EndOfDynamicDrawBlock( numViewerDoubleBufferedRenderingOperation );
    ...
    if( numThreadsOnStartBarrier > 1 )
    {
        _startRenderingBarrier =
            new osg::BarrierOperation( static_cast<int>( numThreadsOnStartBarrier ),
    ...
    if( numThreadsOnEndBarrier > 1 )
    {
        _endRenderingDispatchBarrier =
            new osg::BarrierOperation( static_cast<int>( numThreadsOnEndBarrier ),
                                       _endBarrierOperation );
    }

    osg::ref_ptr<osg::BarrierOperation> swapReadyBarrier =
        contexts.empty()
            ? 0
            : new osg::BarrierOperation( static_cast<int>( contexts.size() ),
                                         osg::BarrierOperation::NO_OPERATION );
    ...
    for( citr = contexts.begin(); citr != contexts.end(); ++citr )
    {
        ...
        if( !gc->isRealized() )
        {
            OSG_INFO << "ViewerBase::startThreading() : Realizng window " << gc
                     << std::endl;
            gc->realize();
        }
        ...
        numThreadsOnStartBarrier > 1 )
    {
        ...
        for( camItr = cameras.begin(); camItr != cameras.end(); ++camItr )
        {
            ...
            {
                OSG_INFO << "  camera->getCameraThread()-> " << camera->getCameraThread()
                         << std::endl;
...
void
ViewerBase::getWindows( Windows& windows,
                        bool     onlyValid )
{
    windows.clear();
...
void
ViewerBase::checkWindowStatus( const Contexts& contexts )
{
    if( contexts.size() == 0 )
    {
        _done = true;
...
void
ViewerBase::addUpdateOperation( osg::Operation* operation )
{
    if( !operation )
    {
        return;
...
void
ViewerBase::removeUpdateOperation( osg::Operation* operation )
{
    if( !operation )
    {
        return;
...
void
ViewerBase::setIncrementalCompileOperation( osgUtil::IncrementalCompileOperation* ico )
{
...
ViewerBase::run()
{
    ...
    while( !done() && ( runTillFrameNumber ==
                        osg::UNINITIALIZED_FRAME_NUMBER ||
                        getViewerFrameStamp()->getFrameNumber() < runTillFrameNumber ) )
    {
        ...
        if( _runFrameScheme == ON_DEMAND )
        {
            ...
            {
                // we don't need to render a frame but we don't want to spin the run loop
                // so make sure the minimum loop time is 1/100th of second, if not
                // otherwise set, so enabling the frame microSleep below to avoid consume
...
void
ViewerBase::frame( double simulationTime )
{
    if( _done )
    {
        return;
...
ViewerBase::renderingTraversals()
{
    ...
    if( _endDynamicDrawBlock.valid() )
    {
        ...
        // OSG_NOTICE<<"Time waiting "<<osg::Timer::instance()->delta_m(startTick,
    ...
    if( getViewerStats() && getViewerStats()->collectStats( "update" ) )
    {
        ...
        getViewerStats()->setAttribute( frameNumber,
