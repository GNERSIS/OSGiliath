 ...
 * Multi-view scene viewer. Manages multiple View objects with
 * independent cameras, scenes, and event handlers in one loop.
 */
...
CompositeViewer::CompositeViewer( const CompositeViewer& cv,
...
CompositeViewer::CompositeViewer( osg::ArgumentParser& arguments )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--run-on-demand",
        "Set the run methods frame rate management to only rendering frames when "
        "required."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--run-continuous",
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "--run-max-frame-rate",
        "Set the run methods maximum permissible frame rate, 0.0 is default and "
        "switching off frame rate capping."
    );
...
void
CompositeViewer::constructorInit()
{
...
bool
CompositeViewer::readConfiguration( const std::string& filename )
{
...
void
CompositeViewer::addView( osgViewer::View* view )
{
    if( !view )
    {
        return;
    ...
    if( view->getSceneData() )
    {
        ...
        view->getSceneData()->resizeGLObjectBuffers(
...
void
CompositeViewer::removeView( osgViewer::View* view )
{
    for( RefViews::iterator itr = _views.begin(); itr != _views.end(); ++itr )
    {
        if( *itr == view )
        {
            ...
            if( view->getCamera()->getGraphicsContext() )
            {
                view->getCamera()->releaseGLObjects(
            ...
            for( unsigned int i = 0; i < view->getNumSlaves(); ++i )
            {
                if( view->getSlave( i )._camera->getGraphicsContext() )
                {
                    view->getSlave( i )._camera->releaseGLObjects(
            ...
            for( ContextMap::iterator citr = contexts.begin(); citr != contexts.end();
                 ++citr )
            {
...
bool
CompositeViewer::isRealized() const
{
    Contexts contexts;
...
CompositeViewer::checkNeedToDoFrame()
{
    ...
    for( RefViews::iterator itr = _views.begin(); itr != _views.end(); ++itr )
    {
        ...
        if( view && ( view->requiresUpdateSceneGraph() || view->requiresRedraw() ) )
        {
            return true;
        }
    }
...
void
CompositeViewer::setStartTick( osg::Timer_t tick )
{
    _startTick = tick;
...
void
CompositeViewer::setReferenceTime( double time )
{
...
void
CompositeViewer::getContexts( Contexts& contexts,
                              bool      onlyValid )
{
    ...
    for( RefViews::iterator vitr = _views.begin(); vitr != _views.end(); ++vitr )
    {
        ...
        osg::GraphicsContext* gc =
            view->getCamera() ? view->getCamera()->getGraphicsContext() : 0;
        ...
        for( unsigned int i = 0; i < view->getNumSlaves(); ++i )
        {
            ...
            osg::GraphicsContext* sgc =
                slave._camera.valid() ? slave._camera->getGraphicsContext() : 0;
...
void
CompositeViewer::getCameras( Cameras& cameras,
                             bool     onlyActive )
{
    cameras.clear();
...
void
CompositeViewer::getScenes( Scenes& scenes,
                            bool    onlyValid )
{
    scenes.clear();
...
void
CompositeViewer::getViews( Views& views,
                           bool /*onlyValid*/ )
{
    views.clear();
...
void
CompositeViewer::getAllThreads( Threads& threads,
                                bool     onlyActive )
{
    threads.clear();
...
void
CompositeViewer::getOperationThreads( OperationThreads& threads,
                                      bool              onlyActive )
{
    threads.clear();
...
CompositeViewer::realize()
{
    ...
    if( contexts.empty() )
    {
        OSG_INFO << "CompositeViewer::realize() - No valid contexts found, setting up "
                    "view across all screens."
                 << std::endl;
    ...
    if( contexts.empty() )
    {
        OSG_NOTICE << "CompositeViewer::realize() - failed to set up any windows"
                   << std::endl;
        _done = true;
        return;
    }
...
void
CompositeViewer::advance( double simulationTime )
{
    if( _done )
    {
        return;
    ...
    if( getViewerStats() && getViewerStats()->collectStats( "frame_rate" ) )
    {
        ...
        getViewerStats()->setAttribute( previousFrameNumber,
                                        "Frame duration",
                                        deltaFrameTime );
        getViewerStats()->setAttribute( previousFrameNumber,
                                        "Frame rate",
...
void
CompositeViewer::setCameraWithFocus( osg::Camera* camera )
{
...
void
CompositeViewer::generateSlavePointerData( osg::Camera*            camera,
                                           osgGA::GUIEventAdapter& event )
{
    osgViewer::GraphicsWindow* gw =
        dynamic_cast<osgViewer::GraphicsWindow*>( event.getGraphicsContext() );
    if( !gw )
    {
        return;
    }
    ...
    // 2) Slave Camera, Relative RF, Same scene graph as master : transform coords into
    // Master Camera and add to PointerData list 3) Slave Camera, Relative RF, Different
    // scene graph from master : do nothing extra? 4) Slave Camera, Absolute RF, Same
    // scene graph as master : do nothing extra? 5) Slave Camera, Absolute RF, Different
    // scene graph : do nothing extra? 6) Slave Camera, Absolute RF, Different scene
    // graph but a distortion correction subgraph depending upon RTT Camera (slave or
    // master)
    //                              : project ray into RTT Camera's clip space, and RTT
    //                              Camera's is Relative RF and sharing same scene graph
    //                              as master then transform coords.
    ...
    if( camera != view_masterCamera )
    {
        ...
        bool  invert_y = event.getMouseYOrientation() ==
                         osgGA::GUIEventAdapter::Y_INCREASING_DOWNWARDS;
        if( invert_y && gw->getTraits() )
        {
            y = static_cast<float>( gw->getTraits()->height ) - 1 - y;
        }
        ...
        osg::dmat4 masterCameraVPW = view_masterCamera->getProjectionMatrix() *
                                     view_masterCamera->getViewMatrix();
        if( view_masterCamera->getViewport() )
        {
            ...
            masterCameraVPW         = viewport->computeWindowMatrix() * masterCameraVPW;
        }
        ...
        if( slave )
        {
            if( camera->getReferenceFrame() ==
                osg::Camera::RELATIVE_RF &&
                slave->_useMastersSceneData )
            {
                ...
                osg::dmat4 matrix( masterCameraVPW * osg::inverse( localCameraVPW ) );
                osg::dvec3 new_coord = matrix * osg::dvec3( x, y, 0.0 );
                // OSG_NOTICE<<"    pointer event new_coord.x="<<new_coord.x<<"
                // new_coord.y="<<new_coord.y<<std::endl;
                event.addPointerData(
                    new osgGA::PointerData( view_masterCamera,
                                            static_cast<float>( new_coord.x ),
                                            static_cast<float>( master_min_x ),
                                            static_cast<float>( master_max_x ),
                                            static_cast<float>( new_coord.y ),
                                            static_cast<float>( master_min_y ),
                                            static_cast<float>( master_max_y ) )
                );
            }
            else if( !slave->_useMastersSceneData )
            {
                ...
                if( ray->containsIntersections() )
                {
                    osg::vec3     tc;
                    osg::Texture* texture =
                        ray->getFirstIntersection().getTextureLookUp( tc );
                    if( texture )
                    {
                        // look up Texture in RTT Camera's.
                        for( unsigned int i = 0; i < view->getNumSlaves(); ++i )
                        {
                            osg::Camera* slave_camera =
                                view->getSlave( i )._camera.get();
                            if( slave_camera )
                            {
                                osg::Camera::BufferAttachmentMap::const_iterator ba_itr =
                                    slave_camera->getBufferAttachmentMap().find(
                                        osg::Camera::COLOR_BUFFER
                                    );
                                if( ba_itr !=
                                    slave_camera->getBufferAttachmentMap().end() )
                                {
                                    if( ba_itr->second._texture == texture )
                                    {
                                        osg::TextureRectangle* tr =
                                            dynamic_cast<osg::TextureRectangle*>(
                                                ba_itr->second._texture.get()
                                            );
                                        osg::TextureCubeMap* tcm =
                                            dynamic_cast<osg::TextureCubeMap*>(
                                                ba_itr->second._texture.get()
                                            );
                                        if( tr )
                                        {
                                            event.addPointerData( new osgGA::PointerData(
                                                slave_camera,
                                                tc.x,
                                                0.0F,
                                                static_cast<float>(
                                        ...
                                        {
                                            OSG_INFO
                                                << "  Slave has matched texture cubemap"
                                                << ba_itr->second._texture.get() << ", "
                                                << ba_itr->second._face << std::endl;
                                        }
                                        else
                                        {
                                            event.addPointerData(
                                                new osgGA::PointerData( slave_camera,
                                                                        tc.x,
                                                                        0.0F,
                                                                        1.0F,
                                                                        tc.y,
...
void
CompositeViewer::generatePointerData( osgGA::GUIEventAdapter& event )
{
    osgViewer::GraphicsWindow* gw =
        dynamic_cast<osgViewer::GraphicsWindow*>( event.getGraphicsContext() );
    if( !gw )
    ...
    event.addPointerData(
        new osgGA::PointerData( gw,
                                x,
                                0,
                                static_cast<float>( gw->getTraits()->width - 1 ),
                                y,
                                0,
                                static_cast<float>( gw->getTraits()->height - 1 ) )
    );

    event.setMouseYOrientationAndUpdateCoords(
    ...
    if( camera )
    {
        ...
        event.addPointerData( new osgGA::PointerData(
            camera,
            static_cast<float>(
                ( x - viewport->x() ) / ( viewport->width() - 1 ) * 2.0 - 1.0
            ),
            -1.0F,
            1.0F,
            static_cast<float>(
                ( y - viewport->y() ) / ( viewport->height() - 1 ) * 2.0 - 1.0
            ),
            -1.0F,
            1.0F
        ) );
...
void
CompositeViewer::reprojectPointerData( osgGA::GUIEventAdapter& source_event,
                                       osgGA::GUIEventAdapter& dest_event )
{
    ...
    bool  invert_y = dest_event.getMouseYOrientation() ==
                     osgGA::GUIEventAdapter::Y_INCREASING_DOWNWARDS;
    if( invert_y && gw->getTraits() )
    {
        y = static_cast<float>( gw->getTraits()->height ) - 1 - y;
    ...
    dest_event.setMouseYOrientationAndUpdateCoords(
        osgGA::GUIEventAdapter::Y_INCREASING_UPWARDS
    );

    osg::Object*   object   = ( source_event.getNumPointerData() >= 2 )
                                ? source_event.getPointerData( 1 )->object.get()
                                : 0;
    ...
    dest_event.addPointerData( new osgGA::PointerData(
        camera,
        static_cast<float>(
            ( x - viewport->x() ) / ( viewport->width() - 1 ) * 2.0 - 1.0
        ),
...
{
        bool
        operator()( const osg::ref_ptr<osgGA::Event>& lhs,
                    const osg::ref_ptr<osgGA::Event>& rhs ) const
        {
...
CompositeViewer::eventTraversal()
{
    ...
    // sort all the events in time order so we can make sure we pass them all on in the
    // correct order.
    all_events.sort( SortEvents() );

    // pass on pointer data onto non mouse events to keep the position data usable by all
    // recipients of all events.
         ...
         ++itr )
    {
            ...
            case( osgGA::GUIEventAdapter::SCROLL ) :
                {
                    if( ( event->getEventType() !=
                        ...
                        !_previousEvent ||
                        _previousEvent->getGraphicsContext() !=
                        event->getGraphicsContext() ||
        ...
        if( !view )
        {
            ...
            {
                OSG_INFO << "Falling back to using first view as one with focus"
                         << std::endl;
                view = _views[0].get();
            }
        }
        ...
        switch( classification )
        {
            case( EVENT_FOR_VIEW_ASSOCIATED_WITH_FOCUS ) :
                {
                    if( view )
                    {
                        OSG_INFO << "Sending EVENT_FOR_VIEW_ASSOCIATED_WITH_FOCUS event "
                                 << event << " to view " << view << std::endl;
            ...
            case( EVENT_FOR_VIEWS_ASSOCIATED_WITH_WINDOW ) :
                {
                    ...
                    if( gc )
                    {
                        ...
                        for( osg::GraphicsContext::Cameras::iterator citr =
                             ...
                             ++citr )
                        {
                            osgViewer::View* camera_view =
                                dynamic_cast<osgViewer::View*>( ( *citr )->getView() );
                            if( camera_view )
                            {
                                views.insert( camera_view );
                            }
                        }
                    }
                    for( ViewSet::iterator vitr = views.begin(); vitr != views.end();
                         ++vitr )
                    {
                        OSG_INFO
                            << "Sending EVENT_FOR_VIEWS_ASSOCIATED_WITH_WINDOW event "
                            << event << " to view " << *vitr << std::endl;
            ...
            case( EVENT_FOR_ALL_VIEWS ) :
                {
                    for( RefViews::iterator vitr = _views.begin(); vitr != _views.end();
                         ++vitr )
                    {
                        OSG_INFO << "Sending EVENT_FOR_ALL_VIEWS event " << event
                                 << " to view " << vitr->get() << std::endl;
    ...
    for( RefViews::iterator vitr = _views.begin(); vitr != _views.end(); ++vitr )
    {
             ...
             ++eitr )
        {
            ...
            // open question, will we need to reproject mouse coordinates into current
            // view's coordinate frame as is down for GraphicsWindow provided events? for
            // now assume now and just get the events directly without any reprojection.
             ...
             ++veitr )
        {
            ...
            if( view && view->getSceneData() )
            {
                     ...
                     ++itr )
                {
                    ...
                    // call any camera event callbacks, but only traverse that callback,
                    // don't traverse its subgraph leave that to the scene update
                    // traversal.
...
CompositeViewer::updateTraversal()
{
    ...
    for( RefViews::iterator vitr = _views.begin(); vitr != _views.end(); ++vitr )
    {
...

        {
            ...
            // traverse its subgraph leave that to the scene update traversal.
...
double
CompositeViewer::elapsedTime()
{
    return osg::Timer::instance()->delta_s( _startTick, osg::Timer::instance()->tick() );
}

void
CompositeViewer::getUsage( osg::ApplicationUsage& usage ) const
{
    for( RefViews::const_iterator vitr = _views.begin(); vitr != _views.end(); ++vitr )
    {
