...
Viewer::Viewer( osg::ArgumentParser& arguments )
{
    ...
    {
        osg::ref_ptr<osgViewer::SingleWindow> sw =
            new osgViewer::SingleWindow( x,
                                         y,
                                         width,
                                         height,
                                         static_cast<unsigned int>( screenNum ) );
             ...
             arguments.read( "--wowvx" ) )
    {
        ...
        if( screenNum >= 0 )
        {
            wow->setScreenNum( static_cast<unsigned int>( screenNum ) );
        }
        if( wowvx20 )
        {
            wow->WoWVx20();
        ...
        {
            wow->setContent( static_cast<unsigned char>( c ) );
        }
        while( arguments.read( "--wow-factor", c ) )
        {
            wow->setFactor( static_cast<unsigned char>( c ) );
        }
        while( arguments.read( "--wow-offset", c ) )
        {
            wow->setOffset( static_cast<unsigned char>( c ) );
        }
    ...
    else if( ( ss3d = arguments.read( "--3d-sd" ) ) ||
             arguments.read( "--panoramic-sd" ) )
    {
        double radius = 1.0;
        ...
        {
            // setThreadingModel(SingleThreaded);
            setUpViewFor3DSphericalDisplay( radius,
                                            collar,
                                            static_cast<unsigned int>( screenNum ),
                                            intensityMap.get() );
        }
    ...
    {
        setUpViewOnSingleScreen( static_cast<unsigned int>( screenNum ) );
    }
}

Viewer::Viewer( const osgViewer::Viewer& viewer,
...
Viewer::~Viewer()
{
    ...
    OSG_INFO << "Viewer::~Viewer():: start destructor getThreads = " << threads.size()
             << std::endl;
    ...
    OSG_INFO << "Viewer::~Viewer() end destructor getThreads = " << threads.size()
             << std::endl;
}

void
Viewer::take( osg::View& rhs )
{
...
bool
Viewer::readConfiguration( const std::string& filename )
{
    ...
    if( !object )
    {
        // OSG_NOTICE<<"Error: Unable to load configuration file
        // \""<<filename<<"\""<<std::endl;
        return false;
    }
    ...
    if( compositeViewer )
    {
        OSG_NOTICE << "Error: Config file \"" << filename
                   << "\" containing CompositeViewer cannot be loaded by Viewer."
                   << std::endl;
        return false;
    }
    ...
    {
        OSG_NOTICE << "Error: Config file \"" << filename
                   << "\" does not contain a valid Viewer configuration." << std::endl;
        return false;
    }
}

bool
Viewer::isRealized() const
{
    Contexts contexts;
...
void
Viewer::setStartTick( osg::Timer_t tick )
{
...
void
Viewer::setReferenceTime( double time )
{
...
Viewer::setUpViewerAsEmbeddedInWindow( int x,
                                       int y,
                                       int width,
                                       int height )
{
...
Viewer::realize()
{
    ...
    if( contexts.empty() )
    {
        OSG_INFO << "Viewer::realize() - No valid contexts found, setting up view "
                    "across all screens."
                 << std::endl;
        ...
        else
        {
            ...
            {
                osg::ref_ptr<osgViewer::SingleWindow> sw = new osgViewer::SingleWindow(
                    x,
                    y,
                    width,
                    height,
...
void
Viewer::advance( double simulationTime )
{
    if( _done )
    {
        return;
    ...
    if( osg::Referenced::getDeleteHandler() )
    {
        ...
        osg::Referenced::getDeleteHandler()->setFrameNumber(
...
void
Viewer::generateSlavePointerData( osg::Camera*            camera,
        ...
        if( slave )
        {
                ...
                slave->_useMastersSceneData )
            {
                ...
                event.addPointerData(
                    new osgGA::PointerData( getCamera(),
            ...
            else if( !slave->_useMastersSceneData )
            {
                ...
                osg::ref_ptr<osgUtil::RayIntersector> ray =
                    new osgUtil::RayIntersector( osgUtil::Intersector::WINDOW, x, y );
...
void
Viewer::generatePointerData( osgGA::GUIEventAdapter& event )
{
    osgViewer::GraphicsWindow* gw =
        dynamic_cast<osgViewer::GraphicsWindow*>( event.getGraphicsContext() );
    if( !gw )
...
void
Viewer::reprojectPointerData( osgGA::GUIEventAdapter& source_event,
...
Viewer::eventTraversal()
{
         ...
         ++eitr )
    {
        ...
        // open question, will we need to reproject mouse coordinates into current view's
        // coordinate frame as is down for GraphicsWindow provided events? for now assume
        // now and just get the events directly without any reprojection.
    ...
    // Ymin="<<eventState->getYmin()<<" xMax="<<eventState->getXmax()<<"
    ...
    if( _eventVisitor.valid() && getSceneData() )
    {
             ...
             ++itr )
        {
            ...
            // call any camera event callbacks, but only traverse that callback, don't
    ...
    for( osgGA::EventQueue::Events::iterator itr = events.begin(); itr != events.end();
         ++itr )
    {
...
void
Viewer::getScenes( Scenes& scenes,
                   bool /*onlyValid*/ )
{
    scenes.clear();
...
void
Viewer::getAllThreads( Threads& threads,
                       bool     onlyActive )
{
    threads.clear();
...
void
Viewer::getOperationThreads( OperationThreads& threads,
                             bool              onlyActive )
{
    threads.clear();
...
void
Viewer::getContexts( Contexts& contexts,
                     bool      onlyValid )
{
...
void
Viewer::getCameras( Cameras& cameras,
                    bool     onlyActive )
{
    cameras.clear();
    ...
    if( _camera.valid() &&
        ( !onlyActive ||
          ( _camera->getGraphicsContext() && _camera->getGraphicsContext()->valid() ) ) )
    {
...
void
Viewer::getUsage( osg::ApplicationUsage& usage ) const
{
    if( _cameraManipulator.valid() )
    {
