 ...
 * Single view within a CompositeViewer. Holds camera, scene data,
 * event handlers, and camera manipulator for one viewport.
 */
...
osg::DisplaySettings*
ViewConfig::getActiveDisplaySetting( osgViewer::View& view ) const
{
    return view.getDisplaySettings() ? view.getDisplaySettings()
                                     : osg::DisplaySettings::instance().get();
}

class CollectedCoordinateSystemNodesVisitor : public osg::DualModeVisitor
{
    public:

        CollectedCoordinateSystemNodesVisitor() :
            DualModeVisitor( osg::NodeVisitor::TRAVERSE_ACTIVE_CHILDREN )
        {
        }
        ...
        {
            return "CollectedCoordinateSystemNodesVisitor";
        }
        ...
        void
        apply( osg::CoordinateSystemNode& node ) override
        {
            if( _pathToCoordinateSystemNode.empty() )
            {
                OSG_DEBUG << "Found CoordinateSystemNode node" << std::endl;
                OSG_DEBUG << "     CoordinateSystem = " << node.getCoordinateSystem()
                          << std::endl;
                _pathToCoordinateSystemNode = getNodePath();
            }
            else
            {
                OSG_DEBUG << "Found additional CoordinateSystemNode node, but ignoring"
                          << std::endl;
                OSG_DEBUG << "     CoordinateSystem = " << node.getCoordinateSystem()
                          << std::endl;
            }
...
/** callback class to use to allow matrix manipulators to query the application for the
 * local coordinate frame.*/
class ViewerCoordinateFrameCallback
    : public osgGA::CameraManipulator::CoordinateFrameCallback
{
    public:
        ...
        getCoordinateFrame( const osg::dvec3& position ) const
        {
            ...
            if( !tmpPath.empty() )
            {
                osg::dmat4                 coordinateFrame;

                osg::CoordinateSystemNode* csn =
                    dynamic_cast<osg::CoordinateSystemNode*>( tmpPath.back() );
                if( csn )
                {
                    osg::dvec3 local_position =
                        osg::computeWorldToLocal( tmpPath ) * position;

                    // get the coordinate frame in world coords.
                    coordinateFrame = osg::computeLocalToWorld( tmpPath ) *
                                      csn->computeLocalCoordinateFrame( local_position );

                    // keep the position of the coordinate frame to reapply after
                    // rescale.
                    osg::dvec3 pos = osg::getTrans( coordinateFrame );

                    // compensate for any scaling, so that the coordinate frame is a unit
                    // size
                    osg::dvec3 x( 1.0, 0.0, 0.0 );
                    osg::dvec3 y( 0.0, 1.0, 0.0 );
                    osg::dvec3 z( 0.0, 0.0, 1.0 );
                    x = osg::transform3x3( x, coordinateFrame );
                    y = osg::transform3x3( y, coordinateFrame );
                    z = osg::transform3x3( z, coordinateFrame );
                    osg::preMultScale( coordinateFrame,
                                       osg::dvec3( 1.0 / osg::length( x ),
                                                   1.0 / osg::length( y ),
                                                   1.0 / osg::length( z ) ) );

                    // reapply the position.
                    osg::setTrans( coordinateFrame, pos );

                    OSG_DEBUG << "csn->computeLocalCoordinateFrame(position)* "
                                 "osg::computeLocalToWorld(tmpPath)"
                              << coordinateFrame << std::endl;
                }
                else
                {
            ...
            {
                OSG_DEBUG << "   no coordinate system found, using default orientation"
                          << std::endl;
                return osg::translate( position );
            }
        }
...
View::View( const osgViewer::View& view,
...
View::take( osg::View& rhs )
{
    ...
    if( rhs_osgViewer )
    {
        ...
        _eventHandlers.insert( _eventHandlers.end(),
                               rhs_osgViewer->_eventHandlers.begin(),
                               rhs_osgViewer->_eventHandlers.end() );
...
View::createRenderer( osg::Camera* camera )
{
...
void
View::setStartTick( osg::Timer_t tick )
{
    _startTick = tick;

    for( Devices::iterator eitr = _eventSources.begin(); eitr != _eventSources.end();
         ++eitr )
    {
...
void
View::setSceneData( osg::Node* node )
{
    ...
    else
    {
        if( _scene->referenceCount() != 1 )
        {
            ...
            _scene = new Scene;
            OSG_INFO << "View::setSceneData() Allocating new scene" << _scene.get()
                     << std::endl;
        }
        else
        {
            OSG_INFO << "View::setSceneData() Reusing existing scene" << _scene.get()
                     << std::endl;
        }
    ...
    if( getSceneData() )
    {
        // now make sure the scene graph is set up with the correct DataVariance to
        // protect the dynamic elements of the scene graph from being run in parallel.
        osgUtil::Optimizer::StaticObjectDetectionVisitor sodv;
        getSceneData()->accept( sodv );

        // make sure that existing scene graph objects are allocated with thread safe
        // ref/unref
        if( getViewerBase() &&
        ...
        // update the scene graph so that it has enough GL object buffer memory for the
        // graphics contexts that will be using it.
        getSceneData()->resizeGLObjectBuffers(
...
void
View::setDatabasePager( osgDB::DatabasePager* dp )
{
...
View::getImagePager() const
{
    return _scene->getImagePager();
}

void
View::setCameraManipulator( osgGA::CameraManipulator* manipulator,
                            bool                      resetPosition )
{
    ...
    if( _cameraManipulator.valid() )
    {
        _cameraManipulator->setCoordinateFrameCallback(
            new ViewerCoordinateFrameCallback( this )
        );
...
void
View::addEventHandler( osgGA::EventHandler* eventHandler )
{
    EventHandlers::iterator itr =
...
void
View::removeEventHandler( osgGA::EventHandler* eventHandler )
{
    EventHandlers::iterator itr =
        std::find( _eventHandlers.begin(), _eventHandlers.end(), eventHandler );
    if( itr != _eventHandlers.end() )
...
void
View::setCoordinateSystemNodePath( const osg::NodePath& nodePath )
{
    _coordinateSystemNodePath.setNodePath( nodePath );
}

osg::NodePath
View::getCoordinateSystemNodePath() const
{
...
void
View::computeActiveCoordinateSystemNodePath()
{
...
void
View::apply( ViewConfig* config )
{
    if( config )
    {
        OSG_INFO << "Applying osgViewer::ViewConfig : " << config->className()
                 << std::endl;
...
void
View::setUpViewOnSingleScreen( unsigned int screenNum )
{
    apply( new osgViewer::SingleScreen( screenNum ) );
}

void
View::setUpViewFor3DSphericalDisplay( double            radius,
                                      double            collar,
                                      unsigned int      screenNum,
                                      osg::Image*       intensityMap,
                                      const osg::dmat4& projectorMatrix )
{
    apply( new osgViewer::SphericalDisplay( radius,
                                            collar,
                                            screenNum,
                                            intensityMap,
                                            projectorMatrix ) );
...
void
View::setUpViewForPanoramicSphericalDisplay( double            radius,
                                             double            collar,
                                             unsigned int      screenNum,
                                             osg::Image*       intensityMap,
                                             const osg::dmat4& projectorMatrix )
{
    apply( new osgViewer::PanoramicSphericalDisplay( radius,
                                                     collar,
                                                     screenNum,
                                                     intensityMap,
                                                     projectorMatrix ) );
...
void
View::setUpViewForWoWVxDisplay( unsigned int  screenNum,
...
{
    apply( new osgViewer::WoWVxDisplay( screenNum,
                                        wow_content,
                                        wow_factor,
                                        wow_offset,
                                        wow_disparity_Zd,
...
bool
DepthPartitionSettings::getDepthRange( osg::View&   view,
                                       ...
                                       double&      zFar )
{
    switch( _mode )
    {
        ...
        case( BOUNDING_VOLUME ) :
            {
                osgViewer::View* view_withSceneData =
                    dynamic_cast<osgViewer::View*>( &view );
                const osg::Node* node =
                    view_withSceneData ? view_withSceneData->getSceneData() : 0;
                if( !node )
                {
                    return false;
                }
                ...
                const osg::dmat4& viewMatrix = masterCamera->getViewMatrix();
                // osg::dmat4& projectionMatrix = masterCamera->getProjectionMatrix();

                osg::dvec3        lookVectorInWorldCoords =
                    osg::transform3x3( viewMatrix, osg::dvec3( 0.0, 0.0, -1.0 ) );
                lookVectorInWorldCoords = osg::normalize( lookVectorInWorldCoords );

                osg::dvec3 nearPointInWorldCoords =
                    bs.center - lookVectorInWorldCoords * bs.radius;
                osg::dvec3 farPointInWorldCoords =
                    bs.center + lookVectorInWorldCoords * bs.radius;

                osg::dvec3 nearPointInEyeCoords = nearPointInWorldCoords * viewMatrix;
                osg::dvec3 farPointInEyeCoords  = farPointInWorldCoords * viewMatrix;
...
namespace osgDepthPartition
{
    ...
    {
            MyUpdateSlaveCallback( DepthPartitionSettings* dps,
                                   unsigned int            partition ) :
                _dps( dps ),
                         ...
                         osg::View::Slave& slave )
            {
                ...
                {
                    camera->setNodeMask( 0XFF'FF'FF );
                }

                if( camera->getProjectionMatrix()( 0, 3 ) ==
                    0.0 &&
                    camera->getProjectionMatrix()( 1, 3 ) ==
                    0.0 &&
                    camera->getProjectionMatrix()( 2, 3 ) == 0.0 )
                {
                    double left, right, bottom, top, zNear, zFar;
                    camera->getProjectionMatrixAsOrtho( left,
                                                        right,
                                                        bottom,
                                                        top,
                                                        zNear,
                                                        zFar );
                    camera->setProjectionMatrixAsOrtho( left,
                                                        right,
                                                        bottom,
                                                        top,
                                                        computed_zNear,
                ...
                {
                    double left, right, bottom, top, zNear, zFar;
                    camera->getProjectionMatrixAsFrustum( left,
                                                          right,
                                                          bottom,
                                                          top,
                                                          zNear,
                    ...
                    camera->setProjectionMatrixAsFrustum( left * nr,
                                                          right * nr,
                                                          bottom * nr,
                                                          top * nr,
                                                          computed_zNear,
    ...
    Cameras
    getActiveCameras( osg::View& view )
    {
...
bool
View::setUpDepthPartitionForCamera( osg::Camera*            cameraToPartition,
                                    DepthPartitionSettings* incoming_dps )
{
    ...
    osg::dmat4 projectionOffset;
    osg::dmat4 viewOffset;

    if( getCamera() == cameraToPartition )
    {
        // replace main camera with depth partition cameras
        OSG_INFO << "View::setUpDepthPartitionForCamera(..) Replacing main Camera"
                 << std::endl;
    }
    else
    {
        ...
        OSG_NOTICE << "View::setUpDepthPartitionForCamera(..) Replacing slave Camera"
                   << i << std::endl;
        removeSlave( i );
    }
    ...
    // far camera
    {
        ...
        slave._updateSlaveCallback =
            new osgDepthPartition::MyUpdateSlaveCallback( dps.get(), 1 );
    }
    ...
    // near camera
    {
        ...
        slave._viewOffset          = viewOffset;
        slave._updateSlaveCallback =
            new osgDepthPartition::MyUpdateSlaveCallback( dps.get(), 0 );
    }
...
bool
View::setUpDepthPartition( DepthPartitionSettings* dsp )
{
    osgDepthPartition::Cameras originalCameras =
        osgDepthPartition::getActiveCameras( *this );
    if( originalCameras.empty() )
    {
        OSG_INFO << "osgView::View::setUpDepthPartition(,..), no windows assigned, "
                    "doing view.setUpViewAcrossAllScreens()"
                 << std::endl;
        ...
        if( originalCameras.empty() )
        {
            OSG_NOTICE << "osgView::View::setUpDepthPartition(View,..) Unable to set up "
                          "windows for viewer."
                       << std::endl;
            return false;
        }
...
View::assignSceneDataToCameras()
{
    ...
    {
        _scene->getDatabasePager()->setIncrementalCompileOperation(
            getViewerBase()->getIncrementalCompileOperation()
        );
    }
...
View::requestRedraw()
{
    ...
    {
        OSG_INFO << "View::requestRedraw(), No viewer base has been assigned yet."
                 << std::endl;
    }
}

void
View::requestContinuousUpdate( bool flag )
{
    if( getViewerBase() )
    {
    ...
    {
        OSG_INFO
            << "View::requestContinuousUpdate(), No viewer base has been assigned yet."
            << std::endl;
    }
}

void
View::requestWarpPointer( float x,
                          float y )
{
    ...
    if( camera )
    {
        const osgViewer::GraphicsWindow* gw =
            dynamic_cast<const osgViewer::GraphicsWindow*>(
        ...
        if( gw )
        {
            ...
            {
                local_y = static_cast<float>( gw->getTraits()->height ) - 1 - local_y;
            }
            const_cast<osgViewer::GraphicsWindow*>( gw )->getEventQueue()->mouseWarped(
                local_x,
                local_y
            );
            const_cast<osgViewer::GraphicsWindow*>( gw )->requestWarpPointer( local_x,
                                                                              local_y );
        }
    }
    else
    {
        OSG_INFO << "View::requestWarpPointer failed no camera containing pointer"
                 << std::endl;
    }
}

bool
View::containsCamera( const osg::Camera* camera ) const
{
    if( _camera == camera )
    {
        return true;
...
const osg::Camera*
View::getCameraContainingPosition( float  x,
                                   float  y,
                                   float& local_x,
                                   float& local_y ) const
{
    ...
    bool   view_invert_y = eventState->getMouseYOrientation() ==
                           osgGA::GUIEventAdapter::Y_INCREASING_DOWNWARDS;

    // OSG_NOTICE<<"getCameraContainingPosition("<<x<<", "<<y<<") view_invert_y =
    // "<<view_invert_y<<", Xmin() = "<<eventState->getXmin()<<", Xmax() =
    // "<<eventState->getXmax()<<", Ymin() = "<<eventState->getYmin()<<", Ymax() =
    // "<<eventState->getYmax()<<std::endl;

    double epsilon = 0.5;

    // if master camera has graphics context and eventState context matches then assume
    // coordinates refer to master camera
    bool masterActive = ( _camera->getGraphicsContext() != 0 && _camera->getViewport() );
    bool eventStateMatchesMaster =
        ( gc != 0 ) ? _camera->getGraphicsContext() == gc : false;
    ...
    if( masterActive && eventStateMatchesMaster )
    {
        ...
        double               new_x = ( x - eventState->getXmin() ) /
                                     ( eventState->getXmax() - eventState->getXmin() );
        double               new_y = ( y - eventState->getYmin() ) /
        ...
        // rescale mouse x, y to window dimensions so we can check against master
        // Camera's viewport
        new_x *=
            static_cast<double>( _camera->getGraphicsContext()->getTraits()->width );
        new_y *=
            static_cast<double>( _camera->getGraphicsContext()->getTraits()->height );
        ...
        if( new_x >=
            ...
            new_y < ( viewport->y() + viewport->height() + epsilon ) )
        {
            local_x = static_cast<float>( new_x );
            local_y = static_cast<float>( new_y );
    ...
    osg::dmat4 masterCameraVPW =
        getCamera()->getProjectionMatrix() * getCamera()->getViewMatrix();

    // convert to non dimensional
    x = static_cast<float>( ( x - eventState->getXmin() ) *
                            2.0 /
                            ( eventState->getXmax() - eventState->getXmin() ) -
                            1.0 );
    y = static_cast<float>( ( y - eventState->getYmin() ) *
                            2.0 /
                            ( eventState->getYmax() - eventState->getYmin() ) -
                            1.0 );
    ...
    for( int i = static_cast<int>( getNumSlaves() ) - 1; i >= 0; --i )
    {
        const Slave& slave = getSlave( static_cast<unsigned int>( i ) );
            ...
            slave._camera->getRenderTargetImplementation() == osg::Camera::FRAME_BUFFER )
        {
            ...
            osg::dmat4           localCameraVPW =
                camera->getProjectionMatrix() * camera->getViewMatrix();
            if( viewport )
            {
                localCameraVPW = viewport->computeWindowMatrix() * localCameraVPW;
            }

            osg::dmat4 matrix( localCameraVPW * osg::inverse( masterCameraVPW ) );

            osg::dvec3 new_coord = matrix * osg::dvec3( x, y, 0.0 );

            // OSG_NOTICE<<"  x="<<x<<" y="<<y<<std::endl;;
            // OSG_NOTICE<<"  eventState->getXmin()="<<eventState->getXmin()<<"
            // eventState->getXmax()="<<eventState->getXmax()<<std::endl;; OSG_NOTICE<<"
            // new_coord "<<new_coord<<std::endl;;
            ...
            if( viewport &&
                ...
                new_coord.x <
                ( viewport->x() + viewport->width() + epsilon ) &&
                new_coord.y < ( viewport->y() + viewport->height() + epsilon ) )
            {
                // OSG_NOTICE<<"  in viewport "<<std::endl;;

                local_x = static_cast<float>( new_coord.x );
                local_y = static_cast<float>( new_coord.y );

                return camera;
            }
            else
            {
                // OSG_NOTICE<<"  not in viewport "<<viewport->x()<<"
                // "<<(viewport->x()+viewport->width())<<std::endl;;
            }
        }
    }
...
)
{
    ...
    OSG_INFO << "computeIntersections(" << x << ", " << y << ") local_x=" << local_x
...
#endif
    return computeIntersections( ea.getX(),
                                 ea.getY(),
                                 nodePath,
                                 intersections,
                                 traversalMask );
...
)
{
    ...
    matrix       = camera->getViewMatrix() * matrix;
    matrix       = camera->getProjectionMatrix() * matrix;

    double zNear = -1.0;
    double zFar  = 1.0;
    if( cf == osgUtil::Intersector::WINDOW && camera->getViewport() )
    {
        matrix = camera->getViewport()->computeWindowMatrix() * matrix;
        zNear  = 0.0;
        zFar   = 1.0;
    }
    ...
    osg::dvec3 startVertex = inverse * osg::dvec3( x, y, zNear );
    osg::dvec3 endVertex   = inverse * osg::dvec3( x, y, zFar );
...
void
View::addDevice( osgGA::Device* eventSource )
{
    Devices::iterator itr =
        std::find( _eventSources.begin(), _eventSources.end(), eventSource );
...
void
View::removeDevice( osgGA::Device* eventSource )
{
    Devices::iterator itr =
        std::find( _eventSources.begin(), _eventSources.end(), eventSource );
    if( itr != _eventSources.end() )
...
bool
View::requiresUpdateSceneGraph() const
{
...
//
osg::Texture*
View::createDistortionTexture( int width,
                               int height )
{
...
osg::Camera*
View::assignRenderToTextureCamera( osg::GraphicsContext* gc,
                                   int                   width,
                                   int                   height,
                                   osg::Texture*         texture )
{
    ...
    addSlave( camera.get(), osg::dmat4(), osg::dmat4() );

    return camera.release();
}

osg::Camera*
View::assignKeystoneDistortionCamera( osg::DisplaySettings* ds,
                                      ...
                                      Keystone*             keystone )
{
    ...
    double      fovy = osg::degrees( 2.0 * atan2( screenHeight / 2.0, screenDistance ) );
    ...
    addSlave( camera.get(), osg::dmat4(), osg::dmat4(), false );

    return camera.release();
}

void
View::StereoSlaveCallback::updateSlave( osg::View&        view,
                                        osg::View::Slave& slave )
{
    osg::Camera*     camera      = slave._camera.get();
    ...
    if( _ds.valid() && camera && viewer_view )
    {
        ...
        camera->inheritCullSettings(
            *( view.getCamera() ),
            static_cast<unsigned int>( camera->getInheritanceMask() )
        );
        ...
        if( _eyeScale < 0.0 )
        {
            camera->setProjectionMatrix( _ds->computeLeftEyeProjectionImplementation(
                view.getCamera()->getProjectionMatrix()
            ) );
        }
        else
        {
            camera->setProjectionMatrix( _ds->computeRightEyeProjectionImplementation(
                view.getCamera()->getProjectionMatrix()
            ) );
        }
        ...
        if( _eyeScale < 0.0 )
        {
            camera->setViewMatrix(
                _ds->computeLeftEyeViewImplementation( view.getCamera()->getViewMatrix(),
                                                       eyeScale )
            );
        }
        else
        {
            camera->setViewMatrix( _ds->computeRightEyeViewImplementation(
...
osg::Camera*
View::assignStereoCamera( osg::DisplaySettings* ds,
                          osg::GraphicsContext* gc,
                          int                   x,
                          int                   y,
                          int                   width,
...
void
View::assignStereoOrKeystoneToCamera( osg::Camera*          camera,
                                      osg::DisplaySettings* ds )
{
    ...
    osg::ref_ptr<osg::GraphicsContext::Traits> traits =
        const_cast<osg::GraphicsContext::Traits*>(
    ...
    if( !ds->getStereo() )
    {
        ...
        {
            keystone =
                dynamic_cast<osgViewer::Keystone*>( ds->getKeystones().front().get() );
        }
        if( !keystone )
        {
        ...
        osg::ref_ptr<osg::Texture> texture =
            createDistortionTexture( traits->width, traits->height );

        // create RTT Camera
        assignRenderToTextureCamera( gc.get(),
                                     traits->width,
                                     traits->height,
                                     texture.get() );

        // create Keystone distortion camera
        osg::ref_ptr<osg::Camera> distortion_camera =
            assignKeystoneDistortionCamera( ds,
                                            gc.get(),
                                            0,
                                            0,
                                            traits->width,
        ...
        case( osg::DisplaySettings::QUAD_BUFFER ) :
            {
                ...
                osg::ref_ptr<osg::Camera> left_camera =
                    assignStereoCamera( ds,
                                        gc.get(),
                                        0,
                                        0,
                                        traits->width,
                                        traits->height,
                                        traits->doubleBuffer ? GL_BACK_LEFT
                                                             : GL_FRONT_LEFT,
                                        -1.0 );
                ...
                osg::ref_ptr<osg::Camera> right_camera =
                    assignStereoCamera( ds,
                                        ...
                                        traits->width,
                                        traits->height,
                                        traits->doubleBuffer ? GL_BACK_RIGHT
                                                             : GL_FRONT_RIGHT,
                                        1.0 );
                ...
                {
                    // for keystone:
                    // left camera to render to left texture using whole viewport of left
                    // texture right camera to render to right texture using whole
                    // viewport of right texture left keystone camera to render to left
                    // viewport/window right keystone camera to render to right
                    // viewport/window two keystone, one for each of the left and right
                    // viewports/windows

                    osg::ref_ptr<Keystone>     keystone = keystones.front();

                    // create distortion texture
                    osg::ref_ptr<osg::Texture> left_texture =
                        createDistortionTexture( traits->width, traits->height );
                    ...
                    left_camera->setRenderTargetImplementation(
                    ...
                    // create distortion texture
                    osg::ref_ptr<osg::Texture> right_texture =
                        createDistortionTexture( traits->width, traits->height );
                    ...
                    right_camera->setRenderTargetImplementation(
                        osg::Camera::FRAME_BUFFER_OBJECT
                    );
                    ...
                    osg::ref_ptr<osg::Camera> left_keystone_camera =
                        assignKeystoneDistortionCamera( ds,
                                                        gc.get(),
                                                        0,
                                                        0,
                    ...
                    left_keystone_camera->setRenderOrder( osg::Camera::NESTED_RENDER,
                                                          2 );

                    // attach Keystone editing event handler.
                    left_keystone_camera->addEventCallback(
                        new KeystoneHandler( keystone.get() )
                    );

                    // create Keystone right distortion camera
                    osg::ref_ptr<osg::Camera> right_keystone_camera =
                        assignKeystoneDistortionCamera( ds,
                                                        gc.get(),
                                                        0,
                                                        0,
                    ...
                    right_keystone_camera->setRenderOrder( osg::Camera::NESTED_RENDER,
                                                           3 );
        ...
        case( osg::DisplaySettings::ANAGLYPHIC ) :
            {
                ...
                osg::ref_ptr<osg::Camera> left_camera =
                    assignStereoCamera( ds,
                                        ...
                                        -1.0 );
                left_camera->setClearMask( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
                left_camera->getOrCreateStateSet()->setAttribute(
                    new osg::ColorMask( true, false, false, true )
                );
                left_camera->setRenderOrder( osg::Camera::NESTED_RENDER, 0 );
                ...
                right_camera->getOrCreateStateSet()->setAttribute(
                ...
                if( !keystones.empty() )
                {
                    ...
                    if( useTwoTexture )
                    {
                        ...
                        left_camera->getOrCreateStateSet()->removeAttribute(
                            osg::StateAttribute::Type::COLORMASK
                        );
                        left_camera->setClearMask( GL_COLOR_BUFFER_BIT |
                                                   GL_DEPTH_BUFFER_BIT );

                        // attach the texture and use it as the color buffer.
                        left_camera->attach( osg::Camera::COLOR_BUFFER,
                                             left_texture.get() );
                        ...
                        right_camera->getOrCreateStateSet()->removeAttribute(
                            osg::StateAttribute::Type::COLORMASK
                        );
                        right_camera->setClearMask( GL_COLOR_BUFFER_BIT |
                                                    GL_DEPTH_BUFFER_BIT );
                        ...
                        left_keystone_camera->setRenderOrder( osg::Camera::NESTED_RENDER,
                                                              2 );
                        left_keystone_camera->setClearMask( GL_COLOR_BUFFER_BIT |
                                                            GL_DEPTH_BUFFER_BIT );
                        left_keystone_camera->getOrCreateStateSet()->setAttribute(
                            new osg::ColorMask( true, false, false, true )
                        );
                        ...
                        right_keystone_camera->setRenderOrder(
                            osg::Camera::NESTED_RENDER,
                            3
                        );
                        right_keystone_camera->setClearMask( GL_DEPTH_BUFFER_BIT );
                        right_keystone_camera->getOrCreateStateSet()->setAttribute(
                            new osg::ColorMask( false, true, true, true )
                        );
                    ...
                    else
                    {
                        ...
                        distortion_camera->setRenderOrder( osg::Camera::NESTED_RENDER,
                                                           2 );

                        // attach Keystone editing event handler.
                        distortion_camera->addEventCallback(
                            new KeystoneHandler( keystone.get() )
                        );
        ...
        case( osg::DisplaySettings::HORIZONTAL_SPLIT ) :
            {
                ...
                if( !keystones.empty() )
                {
                    ...
                    // create distortion texture
                    osg::ref_ptr<osg::Texture> left_texture =
                        createDistortionTexture( traits->width / 2, traits->height );
                    ...
                    // attach Keystone editing event handler.
                    left_keystone_camera->addEventCallback(
                        new KeystoneHandler( left_keystone.get() )
                    );
                    ...
                    // attach Keystone editing event handler.
                    right_keystone_camera->addEventCallback(
                        new KeystoneHandler( right_keystone.get() )
                    );
        ...
        case( osg::DisplaySettings::VERTICAL_SPLIT ) :
            {
                ...
                bool left_eye_bottom_viewport =
                    ds->getSplitStereoVerticalEyeMapping() ==
                ...
                // for keystone:
                // left camera to render to left texture using whole viewport of left
                // texture right camera to render to right texture using whole viewport
                // of right texture left keystone camera to render to left
                // viewport/window right keystone camera to render to right
                // viewport/window two keystone, one for each of the left and right
                // viewports/windows
                ...
                if( !keystones.empty() )
                {
                    ...
                    // create distortion texture
                    osg::ref_ptr<osg::Texture> left_texture =
                        createDistortionTexture( traits->width, traits->height / 2 );
        ...
        case( osg::DisplaySettings::CHECKERBOARD ) :
            {
                ...
                // set up the stencil buffer
                {
                    ...
                    osg::ref_ptr<osg::Geometry> geometry =
                        osg::createTexturedQuadGeometry( osg::vec3( -1.0F, -1.0F, 0.0F ),
                    ...
                    stencil->setOperation( osg::Stencil::Operation::REPLACE,
                ...
                // left Camera
                {
                    ...
                    left_camera->setClearMask( GL_DEPTH_BUFFER_BIT |
                                               GL_COLOR_BUFFER_BIT );
                    left_camera->setRenderOrder( osg::Camera::NESTED_RENDER, 1 );

                    osg::ref_ptr<osg::Stencil> stencil = new osg::Stencil;
                    stencil->setFunction( osg::Stencil::Function::EQUAL, 0, ~0U );
                    stencil->setOperation( osg::Stencil::Operation::KEEP,
                                           osg::Stencil::Operation::KEEP,
                                           osg::Stencil::Operation::KEEP );
                    left_camera->getOrCreateStateSet()->setAttributeAndModes(
                        stencil.get(),
                        osg::StateAttribute::ON
                    );
                }
                ...
                // right Camera
                {
                    ...
                    osg::ref_ptr<osg::Stencil> stencil = new osg::Stencil;
                    stencil->setFunction( osg::Stencil::Function::NOTEQUAL, 0, ~0U );
                    stencil->setOperation( osg::Stencil::Operation::KEEP,
                                           osg::Stencil::Operation::KEEP,
                                           osg::Stencil::Operation::KEEP );
