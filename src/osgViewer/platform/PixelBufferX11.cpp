 ...
 * Utility functions for osgViewer: setBorder, Copyright, _valid, defined.
 */
...
bool
PixelBufferX11::createVisualInfo()
{
    ...
    _visualInfo =
        glXChooseVisual( _display, _traits->screenNum, &( attributes.front() ) );
...
PixelBufferX11::init()
{
    ...
    int screen = static_cast<int>( _traits->screenNum );
    ...
    {
        OSG_NOTICE << "Error: " << XDisplayName( _traits->displayName().c_str() )
                   << " GLX version " << major << "." << minor << " is too old."
                   << std::endl;
        XCloseDisplay( _display );
        _display = 0;
        _valid   = false;
    ...
    if( haveGLX1_3 )
    {
        ...
        {
            int visual_id;
            if( glXGetFBConfigAttrib( _display,
                                      fbconfigs[i],
                                      GLX_VISUAL_ID,
                                      &visual_id ) == 0 )
            {
                if( !_pbuffer && ( unsigned int )visual_id == _visualInfo->visualid )
                {
                    ...
                    _pbuffer =
                        glXCreatePbuffer( _display, fbconfigs[i], &attributes.front() );
                    _useGLX1_3 = true;
                }
            }
        }
        if( _pbuffer )
        {
            ...
            {
                OSG_NOTICE << "PixelBufferX11::init(), pbuffer created with different "
                              "size then requested"
                           << std::endl;
    ...
    #ifdef GLX_SGIX_pbuffer
    // If we still have no pbuffer but a capable display with the SGIX extension, try to
    // use that
    if( !_pbuffer && haveSGIX_pbuffer )
    {
        GLXFBConfigSGIX fbconfig =
            _glXGetFBConfigFromVisualSGIX( _display, _visualInfo );
        typedef std::vector<int> AttributeList;
        ...
        _pbuffer =
            _glXCreateGLXPbufferSGIX( _display,
                                      fbconfig,
        ...
        if( _pbuffer )
        {
            ...
            int iWidth  = 0;
            int iHeight = 0;
            _glXQueryGLXPbufferSGIX( _display,
                                     _pbuffer,
                                     GLX_WIDTH_SGIX,
                                     ( unsigned int* )&iWidth );
            _glXQueryGLXPbufferSGIX( _display,
            ...
            if( _traits->width != iWidth || _traits->height != iHeight )
            {
                OSG_NOTICE << "PixelBufferX11::init(), SGIX_pbuffer created with "
                              "different size then requested"
                           << std::endl;
                OSG_NOTICE << "\tRequested size (" << _traits->width << ","
                           << _traits->height << ")" << std::endl;
                OSG_NOTICE << "\tPbuffer size (" << iWidth << "," << iHeight << ")"
...
void
PixelBufferX11::closeImplementation()
{
...
bool
PixelBufferX11::realizeImplementation()
{
    if( _realized )
    {
        OSG_NOTICE << "PixelBufferX11::realizeImplementation() Already realized"
                   << std::endl;
        return true;
    }
...
bool
PixelBufferX11::makeContextCurrentImplementation( osg::GraphicsContext* /*readContext*/ )
{
    // OSG_NOTICE<<"PixelBufferX11::makeContextCurrentImplementation() not implementation
    // yet."<<std::endl;
    return makeCurrentImplementation();
}

bool
PixelBufferX11::releaseContextImplementation()
{
    if( !_realized )
    {
        OSG_NOTICE << "Warning: GraphicsWindow not realized, cannot do makeCurrent."
...
#ifdef OSG_USE_EGL
    return eglMakeCurrent( _display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT ) ==
           EGL_TRUE;
#else
...
void
PixelBufferX11::bindPBufferToTextureImplementation( GLenum /*buffer*/ )
{
    OSG_NOTICE
        << "PixelBufferX11::bindPBufferToTextureImplementation() not implementation yet."
        << std::endl;
}

void
PixelBufferX11::swapBuffersImplementation()
{
    if( !_realized )
    {
        return;
