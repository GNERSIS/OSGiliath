 ...
 * X11KeyboardMap class.
 * Provides: remapKey, remapExtendedKey, X11WindowingSystemInterface, XSetErrorHandler,
 * exit, XSetErrorHandler.
 */
/* Note, elements of GraphicsWindowX11 have used Prodcer/RenderSurface_X11.cpp as both
 * a guide to use of X11/GLX and copying directly in the case of setBorder().
 * These elements are licensed under OSGPL as above, with Copyright (C) 2001-2004  Don
 * Burns.
 */
...
#ifdef OSG_USE_EGL
bool
checkEGLError( const char* str )
{
    EGLint err = eglGetError();
    if( err != EGL_SUCCESS )
    {
        OSG_WARN << "Warning: " << str << " EGL error " << std::hex << err << std::dec
                 << std::endl;
        return true;
    }
    else
...
Display*
GraphicsWindowX11::getDisplayToUse() const
{
    ...
    if( osg::Thread::CurrentThreadId() == _threadOfLastMakeCurrent )
    {
        return _display;
    }
    else
...
bool
GraphicsWindowX11::createVisualInfo()
{
    if( _visualInfo )
    {
#ifdef OSG_USE_EGL
    ...
    else
    {
        ...
        attributes.push_back( static_cast<int>( _traits->red ) );
        attributes.push_back( GLX_GREEN_SIZE );
        attributes.push_back( static_cast<int>( _traits->green ) );
        attributes.push_back( GLX_BLUE_SIZE );
        attributes.push_back( static_cast<int>( _traits->blue ) );
        attributes.push_back( GLX_DEPTH_SIZE );
        attributes.push_back( static_cast<int>( _traits->depth ) );

        if( _traits->alpha )
        {
            attributes.push_back( GLX_ALPHA_SIZE );
            attributes.push_back( static_cast<int>( _traits->alpha ) );
        }
        if( _traits->stencil )
        {
            attributes.push_back( GLX_STENCIL_SIZE );
            attributes.push_back( static_cast<int>( _traits->stencil ) );
        }
        ...
        if( _traits->sampleBuffers )
        {
            ...
            attributes.push_back( static_cast<int>( _traits->sampleBuffers ) );
        }
        if( _traits->samples )
        {
            attributes.push_back( GLX_SAMPLES );
            attributes.push_back( static_cast<int>( _traits->samples ) );
        }
    #endif
        // TODO
        //  GLX_AUX_BUFFERS
        ...
        int          numFbConfigs = 0;
        GLXFBConfig* fbConfigs    = glXChooseFBConfig( _display,
                                                       _traits->screenNum,
                                                       attributes.data(),
                                                       &numFbConfigs );
        if( numFbConfigs > 0 )
...
bool
GraphicsWindowX11::checkAndSendEventFullScreenIfNeeded( Display* display,
                                                        int      x,
                                                        int      y,
                                                        int      width,
                                                        int      height,
                                                        bool     windowDecoration )
{
    ...
    if( wsi == NULL )
    {
        OSG_NOTICE << "Error, no WindowSystemInterface available, cannot toggle window "
                      "fullscreen."
                   << std::endl;
        return false;
    }
    ...
    if( netWMStateAtom != None && netWMStateFullscreenAtom != None )
    {
        // set up full screen
        {
            ...
            xev.xclient.data.l[1]    = static_cast<long>( netWMStateFullscreenAtom );
        ...
        if( isFullScreen && XineramaIsActive( display ) )
        {
            ...
            int32_t  minx    = 2'147'483'647;     // INT_MAX
            int32_t  maxx    = -2'147'483'648;    // INT_MIN
            int32_t  miny    = 2'147'483'647;     // INT_MAX
            int32_t  maxy    = -2'147'483'648;    // INT_MIN
...
bool
GraphicsWindowX11::setWindowDecorationImplementation( bool flag )
{
    ...
    checkAndSendEventFullScreenIfNeeded( display,
                                         _traits->x,
                                         _traits->y,
                                         _traits->width,
                                         _traits->height,
    ...
    if( ( atom = XInternAtom( display, "_MOTIF_WM_HINTS", 0 ) ) != None )
    {
        ...
        if( flag )
        {
            ...
            // if traits says not resize we want to set the functions to exlude
            // MWM_FUNC_RESIZE, but this bitmask needs to be set if the MWM_FUNC_ALL bit
            // is already set in order to toggle it off.
    ...
    {
        OSG_NOTICE << "Error: GraphicsWindowX11::setWindowDecorationImplementation("
                   << flag << ") - couldn't change decorations." << std::endl;
        result = false;
    }
...
bool
GraphicsWindowX11::setWindowRectangleImplementation( int x,
                                                     int y,
                                                     int width,
                                                     int height )
{
...
void
GraphicsWindowX11::setWindowName( const std::string& name )
{
    if( _window == 0 )
    {
        return;
...
void
GraphicsWindowX11::setCursor( MouseCursor mouseCursor )
{
...
Cursor
GraphicsWindowX11::getOrCreateCursor( MouseCursor mouseCursor )
{
    ...
    switch( mouseCursor )
    {
        case NoCursor :
            {
                // create an empty mouse cursor, note that it is safe to destroy the
                // Pixmap just past cursor creation since the resource in the x server is
                // reference counted.
                char   buff[2] = { 0, 0 };
                XColor ncol    = { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 };
                Pixmap pixmap  = XCreateBitmapFromData( display, _parent, buff, 1, 1 );
                _mouseCursorMap[mouseCursor] =
                    XCreatePixmapCursor( display, pixmap, pixmap, &ncol, &ncol, 0, 0 );
                XFreePixmap( display, pixmap );
                // Important to have the pixmap and the buffer still available when the
        ...
        case HelpCursor :
            _mouseCursorMap[mouseCursor] =
                XCreateFontCursor( display, XC_question_arrow );
            break;
        case CycleCursor :
...
GraphicsWindowX11::init()
{
    ...
    WindowData* inheritedWindowData =
    ...
    if( !_display )
    {
        OSG_NOTICE << "Error: Unable to open display \""
                   << XDisplayName( _traits->displayName().c_str() ) << "\"."
                   << std::endl;
        _valid = false;
        return;
    }
    ...
    OSG_NOTICE
        << "GraphicsWindowX11::init() - eglInitialize() succeeded eglMajorVersion="
        << eglMajorVersion << " iMinorVersion=" << eglMinorVersion << std::endl;
    ...
    attributes.push_back( _traits->red );
    ...
    int numConfigs;
    if( !eglChooseConfig( _eglDisplay,
                          &( attributes.front() ),
                          &eglConfig,
                          1,
                          &numConfigs ) ||
        ( numConfigs != 1 ) )
    {
        OSG_NOTICE << "GraphicsWindowX11::init() - eglChooseConfig() failed."
                   << std::endl;
        XCloseDisplay( _display );
        _valid   = false;
        _display = 0;
    ...
    {
        OSG_NOTICE << "GraphicsWindowX11::init() - eglCreateWindowSurface(..) failed."
                   << std::endl;
        XCloseDisplay( _display );
        _valid   = false;
        _display = 0;
    ...
    {
        OSG_NOTICE << "GraphicsWindowX11::init() - eglCreateContext(..) failed."
                   << std::endl;
        XCloseDisplay( _display );
        _valid   = false;
        _display = 0;
    ...
    bool supportsProfiles =
        glx_string.find( "GLX_ARB_create_context" ) != std::string::npos;
    ...
    if( glXCreateContextAttribsARB )
    {
        ...
        int              majorVersion =
            std::atoi( _traits->glContextVersion.substr( 0, pos ).c_str() );
        int minorVersion =
            pos == std::string::npos
                ? 0
                : std::atoi( _traits->glContextVersion.substr( pos + 1 ).c_str() );
        ...
        float glVersion = static_cast<float>( majorVersion ) +
        ...
        if( glVersion >= 3.2 && supportsProfiles && _traits->glContextProfileMask != 0 )
        {
            ...
            contextAttributes.push_back(
                static_cast<int>( _traits->glContextProfileMask )
            );
        }
        if( _traits->glContextFlags != 0 )
        {
            contextAttributes.push_back( GLX_CONTEXT_FLAGS_ARB );
            contextAttributes.push_back( static_cast<int>( _traits->glContextFlags ) );
        }
...
bool
GraphicsWindowX11::createWindow()
{
    unsigned int screen = static_cast<unsigned int>( _traits->screenNum );
    ...
    swatt.colormap =
        XCreateColormap( _display, _parent, _visualInfo->visual, AllocNone );
    ...
    {
        // follows is hack to get around problems with toggling off full screen with
        // modern X11 window managers that try to be too clever when toggling off full
        // screen and ignore the window size calls made by the OSG when the initial
        // window size is full screen.

        Atom netWMStateAtom = XInternAtom( _display, "_NET_WM_STATE", True );
        Atom netWMStateFullscreenAtom =
            XInternAtom( _display, "_NET_WM_STATE_FULLSCREEN", True );
    ...
    // Give window a class so that user preferences can be saved in the resource
    // database.
    XClassHint clH;
    ...
    // For headless mode: make the window completely invisible to the
    // desktop environment. Set override-redirect so the WM ignores it
    // entirely — no taskbar, no notifications, no focus stealing.
    if( _traits->headless )
    {
        XSetWindowAttributes overrideAttrs;
        overrideAttrs.override_redirect = True;
        XChangeWindowAttributes( _display, _window, CWOverrideRedirect, &overrideAttrs );
    }
    ...
    int    windowX, windowY;
    XGetWindowAttributes( _display, _window, &watt );
    XTranslateCoordinates( _display,
                           _window,
                           watt.root,
                           watt.x,
                           watt.y,
    ...
    // OSG_NOTICE<<"After sync apply.x = "<<watt.x<<" watt.y="<<watt.y<<"
    // width="<<watt.width<<" height="<<watt.height<<std::endl;

    XSelectInput( _eventDisplay,
                  _window,
                  ExposureMask |
                      StructureNotifyMask |
                      KeyPressMask |
...
bool
GraphicsWindowX11::setWindow( Window window )
{
    if( _initialized )
    {
        OSG_NOTICE << "GraphicsWindowX11::setWindow() - Window already created; it "
                      "cannot be changed";
        return false;
    }
    ...
    if( _window == 0 )
    {
        OSG_NOTICE << "GraphicsWindowX11::setWindow() - Unable to retrieve native "
                      "window handle";
        return false;
    }
...
bool
GraphicsWindowX11::realizeImplementation()
{
    if( _realized )
    {
        OSG_NOTICE << "GraphicsWindowX11::realizeImplementation() Already realized"
                   << std::endl;
        return true;
    }
    ...
    // Skip mapping (showing) the window for headless rendering
    if( !_traits->headless )
    {
...
bool
GraphicsWindowX11::makeCurrentImplementation()
{
    if( !_realized )
    {
        OSG_NOTICE << "Warning: GraphicsWindow not realized, cannot do makeCurrent."
                   << std::endl;
        return false;
    }
...
#ifdef OSG_USE_EGL
    bool result =
        eglMakeCurrent( _eglDisplay, _eglSurface, _eglSurface, _context ) == EGL_TRUE;
...
bool
GraphicsWindowX11::releaseContextImplementation()
{
    if( !_realized )
    {
        OSG_NOTICE << "Warning: GraphicsWindow not realized, cannot do release context."
                   << std::endl;
        return false;
    }
...
void
GraphicsWindowX11::swapBuffersImplementation()
{
    if( !_realized )
    {
        return;
    ...
    // OSG_NOTICE<<"swapBuffersImplementation "<<this<<"
    // "<<osg::Thread::CurrentThread()<<std::endl;
...
GraphicsWindowX11::checkEvents()
{
    ...
    // getEventQueue()->getCurrentEventState()->getGraphicsContext()="<<getEventQueue()->getCurrentEventState()->getGraphicsContext()<<std::endl;
    ...
    while( XPending( display ) )
    {
        ...
        switch( ev.type )
        {
            ...
            case Expose :
                OSG_INFO << "Expose x=" << ev.xexpose.x << " y=" << ev.xexpose.y
                         << " width=" << ev.xexpose.width
                         << ", height=" << ev.xexpose.height << std::endl;
                requestRedraw();
                break;
            ...
            case FocusOut :
                {
                    ...
                    for( unsigned int key = 8; key < 256; key++ )
                    {
                        ...
                        if( !isModifier )
                        {
                            forceKey( static_cast<int>( key ), eventTime, false );
                        }
                    }
                    needNewWindowSize = true;
            ...
            case EnterNotify :
                ...
                _modifierState = static_cast<int>( ev.xcrossing.state );
                syncLocks();
                break;
            ...
            case KeymapNotify :
                {
                    ...
                    for( unsigned int key = 8; key < 256; key++ )
                    {
                        ...
                        bool isPressed = keyMapGetKey( keyMap, key );
                        forceKey( static_cast<int>( key ), eventTime, isPressed );
                    }
                    ...
                    for( unsigned int key = 8; key < 256; key++ )
                    {
                        ...
                        if( isPressed )
                        {
                            forceKey( static_cast<int>( key ), eventTime, true );
                        }
                    }
                    break;
                }
            ...
            case MotionNotify :
                {
                    ...
                    else
                    {
                        ...
                        // coordinates of the mouse position relative to an absolute
                        // position then take away the position of the original
                        // window/screen to get the coordinates relative to the original
                        // position.
                        Window       root;
                        int          rx, ry;
                    ...
                    float mx = static_cast<float>( wx );
                    float my = static_cast<float>( wy );
                    transformMouseXY( mx, my );
                    getEventQueue()->mouseMotion( mx, my, eventTime );

                    // OSG_NOTICE<<"MotionNotify wx="<<wx<<" wy="<<wy<<" mx="<<mx<<"
                    // my="<<my<<std::endl;
            ...
            case ButtonPress :
                {
                    ...
                    else
                    {
                        ...
                        float my = static_cast<float>( ev.xmotion.y );
                        transformMouseXY( mx, my );
                        getEventQueue()->mouseButtonPress( mx,
                                                           my,
                                                           ev.xbutton.button,
                                                           eventTime );
                    }
            ...
            case ButtonRelease :
                {
                    ...
                    if( ev.xbutton.button == Button4 )
                    {
                        // ignore scroll release as X11 is generating button press and
                        // release pairs for scroll wheel motion and we only need only
                        // event for each scroll action
                        // getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_UP,
                        // eventTime);
                    }
                    else if( ev.xbutton.button == Button5 )
                    ...
                    else
                    {
                        ...
                        float my = static_cast<float>( ev.xmotion.y );
                        transformMouseXY( mx, my );
                        getEventQueue()->mouseButtonRelease( mx,
                                                             my,
                                                             ev.xbutton.button,
                                                             eventTime );
                    }
            ...
            case KeyPress :
                {
                    ...
                    _modifierState = static_cast<int>( ev.xkey.state );
...
void
GraphicsWindowX11::grabFocusIfPointerInWindow()
{
    Window       win, root;
    int          wx, wy, rx, ry;
    unsigned int buttons;
    ...
    if( XQueryPointer( display, _window, &root, &win, &rx, &ry, &wx, &wy, &buttons ) )
    {
#if 0
...
void
GraphicsWindowX11::transformMouseXY( float& x,
                                     float& y )
{
...
void
GraphicsWindowX11::adaptKey( XKeyEvent& keyevent,
                             int&       keySymbol,
                             int&       unmodifiedKeySymbol )
{
    ...
    int           numChars = XLookupString( &keyevent,
                                            reinterpret_cast<char*>( buffer_return ),
                                            bytes_buffer,
                                            &keysym_return,
                                            NULL );
    keySymbol              = static_cast<int>( keysym_return );
    ...
    unmodifiedKeySymbol =
        static_cast<int>( XkbKeycodeToKeysym( keyevent.display,
                                              static_cast<KeyCode>( keyevent.keycode ),
                                              0,
                                              0 ) );
}
...
void
GraphicsWindowX11::forceKey( int    key,
                             double time,
                             bool   state )
{
    if( !( state ^ static_cast<bool>(
                       keyMapGetKey( _keyMap, static_cast<unsigned int>( key ) )
                   ) ) )
    {
    ...
    event.y_root     = 0;
    event.state =
        static_cast<unsigned int>( getModifierMask() |
                                   ( _modifierState & ( LockMask | _numLockMask ) ) );
    ...
    if( state )
    {
        ...
        keyMapSetKey( _keyMap, static_cast<unsigned int>( key ) );
    }
    else
    {
        ...
        keyMapClearKey( _keyMap, static_cast<unsigned int>( key ) );
    }
}

void
GraphicsWindowX11::syncLocks()
{
    unsigned int mask = static_cast<unsigned int>(
        getEventQueue()->getCurrentEventState()->getModKeyMask()
    );
    ...
    {
        mask &= ~static_cast<unsigned int>( osgGA::GUIEventAdapter::MODKEY_CAPS_LOCK );
    }
    ...
    {
        mask &= ~static_cast<unsigned int>( osgGA::GUIEventAdapter::MODKEY_NUM_LOCK );
    }

    getEventQueue()->getCurrentEventState()->setModKeyMask( static_cast<int>( mask ) );
}

void
GraphicsWindowX11::rescanModifierMapping()
{
...
void
GraphicsWindowX11::flushKeyEvents()
{
    XEvent e;
...
void
GraphicsWindowX11::getModifierMap( char* keymap ) const
{
    memset( keymap, 0, 32 );
    XModifierKeymap* mkm = XGetModifierMapping( _eventDisplay );
    KeyCode*         m   = mkm->modifiermap;
...
int
GraphicsWindowX11::getModifierMask() const
{
    int              mask = 0;
    XModifierKeymap* mkm  = XGetModifierMapping( _eventDisplay );
...
void
GraphicsWindowX11::requestWarpPointer( float x,
                                       float y )
{
    if( !_realized )
    {
        OSG_INFO << "GraphicsWindowX11::requestWarpPointer() - Window not realized; "
                    "cannot warp pointer, screenNum="
...
extern "C"
{
    ...
    typedef int ( *X11ErrorHandler )( Display*,
                                      XErrorEvent* );

    int
    X11ErrorHandling( Display*     display,
                      XErrorEvent* event )
    {
        OSG_NOTICE << "Got an X11ErrorHandling call display=" << display
...
{
#ifdef OSGVIEWER_USE_XRANDR
        // TODO: Investigate whether or not Robert thinks we should store/restore the
        // original resolution in the destructor; I'm not sure the other ones do this,
        // and it may be the responsibility of the user.
        bool
        _setScreen( const osg::GraphicsContext::ScreenIdentifier& si,
                    unsigned int                                  width,
                    unsigned int                                  height,
                    ...
                    double                                        rate )
        {
            if( colorDepth > 0 )
            {
                OSG_NOTICE << "X11WindowingSystemInterface::_setScreen() is not fully "
                              "implemented (missing depth)."
                           << std::endl;
            }
            ...
            if( display )
            {
                XRRScreenConfiguration* sc =
                    XRRGetScreenInfo( display, RootWindow( display, si.screenNum ) );

                if( !sc )
                {
                    OSG_NOTICE << "Unable to create XRRScreenConfiguration on display \""
                               << XDisplayName( si.displayName().c_str() ) << "\"."
                               << std::endl;
                    return false;
                }
                ...
                // If this somehow fails, okay will still be false, no iteration will
                // take place below, and the sc pointer will still be freed later on.
                ...
                for( int i = 0; i < numScreens; i++ )
                {
                    if( ss[i].width ==
                        ...
                        ss[i].height == static_cast<int>( height ) )
                    {
                        short* rates     = XRRConfigRates( sc, i, &numRates );
                        bool   rateFound = false;

                        // Search for our rate in the list of acceptable rates given to
                        // us by Xrandr. If it's not found, rateFound will still be false
                        // and the call will never be made to XRRSetScreenConfigAndRate
                        // since the rate will be invalid.
                        ...
                        {
                            OSG_NOTICE << "Unable to find valid refresh rate " << rate
                                       << " on display \""
                                       << XDisplayName( si.displayName().c_str() )
                                       << "\"." << std::endl;
                        }
                        else if( XRRSetScreenConfigAndRate( display,
                                                            sc,
                                                            DefaultRootWindow( display ),
                                                            i,
                                                            currentRot,
                                                            static_cast<short>( rate ),
                                                            CurrentTime ) !=
                                 RRSetConfigSuccess )
                        {
                            OSG_NOTICE << "Unable to set resolution to " << width << "x"
                                       << height << " on display \""
                                       << XDisplayName( si.displayName().c_str() )
                                       << "\"." << std::endl;
                        }
            ...
            {
                OSG_NOTICE << "Unable to open display \""
                           << XDisplayName( si.displayName().c_str() ) << "\"."
                           << std::endl;
                return false;
            }
    ...
    public:
        ...
        ~X11WindowingSystemInterface()
        {
            ...
            // Unset our X11 error handler, providing the application has not replaced
            // it.
        ...
        getNumScreens( const osg::GraphicsContext::ScreenIdentifier& si )
        {
            ...
            if( display )
            {
                unsigned int numScreens =
                    static_cast<unsigned int>( ScreenCount( display ) );
            ...
            {
                OSG_NOTICE << "A Unable to open display \""
                           << XDisplayName( si.displayName().c_str() ) << "\""
                           << std::endl;
                return 0;
            }
        ...
        bool
        supportsRandr( Display* display ) const
        {
#ifdef OSGVIEWER_USE_XRANDR
            int  event_basep;
            int  error_basep;
            bool supports_randr =
                XRRQueryExtension( display, &event_basep, &error_basep );
            if( supports_randr )
            {
                int major, minor;
                           ...
                           osg::GraphicsContext::ScreenSettings&         resolution )
        {
            ...
            if( display )
            {
                ...
                resolution.colorDepth =
                    static_cast<unsigned int>( DefaultDepth( display, si.screenNum ) );
        ...
        )
        {
            ...
            if( display )
            {
                ...
                if( supportsRandr( display ) )
                {
                    ...
                    XRRScreenSize* screenSizes =
                        XRRSizes( display, si.screenNum, &nsizes );
                    if( screenSizes && nsizes > 0 )
                    {
                        for( int i = 0; i < nsizes; ++i )
                        {
                            OSG_INFO << "Screen size " << screenSizes[i].width << " "
                                     << screenSizes[i].height << " "
                                     << screenSizes[i].mwidth << " "
                                     << screenSizes[i].mheight << std::endl;
            ...
            {
                OSG_NOTICE << "X11WindowingSystemInterface::enumerateScreenSettings() "
                              "not supported."
                           << std::endl;
            }
        }
        ...
        createGraphicsContext( osg::GraphicsContext::Traits* traits )
        {
            ...
            {
#if 1
                osg::ref_ptr<osgViewer::PixelBufferX11> pbuffer =
                    new PixelBufferX11( traits );
                if( pbuffer->valid() )
                {
...
#else
                osg::ref_ptr<osgViewer::GraphicsWindowX11> window =
                    new GraphicsWindowX11( traits );
                if( window->valid() )
                {
...
REGISTER_WINDOWINGSYSTEMINTERFACE2( X11,
                                    X11WindowingSystemInterface,
                                    OSGVIEWER_EXPORT )

void
GraphicsWindowX11::raiseWindow()
{
    Display* display = getDisplayToUse();
    if( !display )
    {
    ...
    else
    {
        ...
        while( !xraise && i < nchildren )
        {
            ...
            if( ( windowName.nitems != 0 ) &&
                ( strcmp( _traits->windowName.c_str(),
                          ( const char* )windowName.value ) == 0 ) )
            {
                xraise = true;
            }
        }
