 ...
 * TemporaryWindow, derived from Referenced.
...
namespace
{
    ...
    class TemporaryWindow : public osg::Referenced
    {
        ...
        protected:
            ...
            TemporaryWindow&
            operator=( const TemporaryWindow& )
            {
                return *this;
            }
    ...
    TemporaryWindow::create()
    {
        ...
        {
            OSG_WARN << "PixelBufferWin32, could not get device context for temporary "
                        "window: "
                     << sysError() << std::endl;
            kill();
            return;
        ...
        {
            OSG_WARN
                << "PixelBufferWin32, could not set pixel format for temporary window: "
                << sysError() << std::endl;
            kill();
            return;
        }
        ...
        {
            OSG_WARN << "PixelBufferWin32, could not get graphics context for temporary "
                        "window: "
                     << sysError() << std::endl;
            kill();
            return;
    ...
    class WGLExtensions : public osg::Referenced
    {
        public:
            ...
            typedef int( WINAPI* WGLReleasePBufferDCProc )( HPBUFFERARB hPbuffer,
                                                            HDC         hDC );
            typedef bool( WINAPI* WGLDestroyPBufferProc )( HPBUFFERARB hPbuffer );
            typedef bool( WINAPI* WGLQueryPBufferProc )( HPBUFFERARB hPbuffer,
                                                         int         iAttribute,
                                                         int*        piValue );
            typedef bool( WINAPI* WGLBindTexImageProc )( HPBUFFERARB hPbuffer,
                                                         int         iBuffer );
            typedef bool( WINAPI* WGLReleaseTexImageProc )( HPBUFFERARB hPbuffer,
                                                            int         iBuffer );
            typedef bool( WINAPI* WGLSetPbufferAttribProc )( HPBUFFERARB hPbuffer,
                                                             const int*  piAttribList );
            typedef bool( WINAPI* WGLChoosePixelFormatProc )( HDC,
                                                              const int*,
                                                              const float*,
                                                              unsigned int,
                                                              int*,
                                                              unsigned int* );
            typedef bool( WINAPI* WGLMakeContextCurrentProc )( HDC,
                                                               HDC,
                                                               HGLRC );
    ...
    WGLExtensions::WGLExtensions()
    {
        ...
        {
            wglMakeContextCurrentARB = ( WGLMakeContextCurrentProc )wglGetProcAddress(
...
PixelBufferWin32::init()
{
    ...
    _hwnd = reinterpret_cast<HWND>( wgle->wglCreatePbufferARB( hdc,
                                                               format,
                                                               _traits->width,
                                                               _traits->height,
                                                               &bAttribList[0] ) );
    if( !_hwnd )
    {
        OSG_NOTICE << "PixelBufferWin32::init, wglCreatePbufferARB error: " << sysError()
                   << std::endl;
        return;
    }
    ...
    if( !_hdc )
    {
        OSG_NOTICE << "PixelBufferWin32::init, wglGetPbufferDCARB error: " << sysError()
                   << std::endl;
        return;
    }
    ...
    if( !_hglrc )
    {
        OSG_NOTICE << "PixelBufferWin32::init, wglCreateContext error: " << sysError()
                   << std::endl;
        return;
    }
    ...
    {
        OSG_NOTICE << "PixelBufferWin32::init(), pbuffer created with different size "
                      "then requested"
                   << std::endl;
        OSG_NOTICE << "\tRequested size (" << _traits->width << "," << _traits->height
                   << ")" << std::endl;
...
PixelBufferWin32::realizeImplementation()
{
    if( _realized )
    {
        OSG_NOTICE << "PixelBufferWin32::realizeImplementation() Already realized"
                   << std::endl;
        return true;
    }
    ...
    if( _traits->sharedContext.valid() )
    {
        ...
        if( graphicsHandleWin32 )
        {
            ...
            {
                OSG_NOTICE
                    << "PixelBufferWin32::realizeImplementation, wglShareLists error: "
                    << sysError() << std::endl;
            }
        }
    }
...
PixelBufferWin32::closeImplementation()
{
    if( _hwnd )
    {
        ...
        {
            OSG_NOTICE
                << "PixelBufferWin32::closeImplementation, wglDeleteContext error: "
                << sysError() << std::endl;
        }

        if( wgle && wgle->isValid() )
        {
            // Note that closeImplementation() should only be called from the same thread
            // as created the pbuffer, otherwise these routines will return an error.

            if( !wgle->wglReleasePbufferDCARB( reinterpret_cast<HPBUFFERARB>( _hwnd ),
                                               _hdc ) )
            {
                OSG_NOTICE << "PixelBufferWin32::closeImplementation, "
                              "wglReleasePbufferDCARB error: "
                           << sysError() << std::endl;
            }
            if( !wgle->wglDestroyPbufferARB( reinterpret_cast<HPBUFFERARB>( _hwnd ) ) )
            {
                OSG_NOTICE << "PixelBufferWin32::closeImplementation, "
                              "wglDestroyPbufferARB error: "
                           << sysError() << std::endl;
            }
        }
    }
...
PixelBufferWin32::makeCurrentImplementation()
{
    ...
    // If the pbuffer is bound to a texture then release it.  This operation requires a
    // current context, so do it after the MakeCurrent.
    ...
    if( _boundBuffer != 0 )
    {
        ...
        {
            if( !wgle->wglReleaseTexImageARB( reinterpret_cast<HPBUFFERARB>( _hwnd ),
                                              _boundBuffer ) )
            {
                OSG_NOTICE << "PixelBufferWin32::makeCurrentImplementation, "
                              "wglReleaseTexImageARB error: "
                           << sysError() << std::endl;
            }
            _boundBuffer = 0;
        }
...
bool
PixelBufferWin32::makeContextCurrentImplementation( GraphicsContext* readContext )
{
    WGLExtensions* wgle = WGLExtensions::instance();

    if( !wgle || !wgle->wglMakeContextCurrentARB )
    {
        OSG_NOTICE << "PixelBufferWin32, wglMakeContextCurrentARB not available"
                   << std::endl;
        return false;
    }
...
void
PixelBufferWin32::bindPBufferToTextureImplementation( GLenum buffer )
{
    WGLExtensions* wgle = WGLExtensions::instance();
    ...
    if( bindBuffer != _boundBuffer )
    {
        if( _boundBuffer !=
            ...
            !wgle->wglReleaseTexImageARB( reinterpret_cast<HPBUFFERARB>( _hwnd ),
                                          _boundBuffer ) )
        {
            OSG_NOTICE << "PixelBufferWin32::bindPBufferToTextureImplementation, "
                          "wglReleaseTexImageARB error: "
                       << sysError() << std::endl;
        }
...
void
PixelBufferWin32::swapBuffersImplementation()
{
    SwapBuffers( _hdc );
}
