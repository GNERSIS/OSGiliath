 ...
 * Win32WindowingSystem, derived from WindowingSystemInterface.
...
#define MOUSEEVENTF_FROMTOUCH 0XFF'51'57'00

// _MSC_VER 1500: VS 2008
#if ( WINVER < 0X06'01 || ( _MSC_VER <= 1'500 && !__MINGW32__ ) )
// Provide Declarations for Multitouch

    #define WM_TOUCH 0X02'40
...
typedef TOUCHINPUT const* PCTOUCHINPUT;
     ...
     */
    #define TOUCHEVENTF_MOVE          0X00'01
    #define TOUCHEVENTF_DOWN          0X00'02
    #define TOUCHEVENTF_UP            0X00'04
    #define TOUCHEVENTF_INRANGE       0X00'08
    #define TOUCHEVENTF_PRIMARY       0X00'10
    #define TOUCHEVENTF_NOCOALESCE    0X00'20
    #define TOUCHEVENTF_PEN           0X00'40
    #define TOUCHEVENTF_PALM          0X00'80
...
#if ( WINVER < 0X06'02 || ( _MSC_VER <= 1'600 && !__MINGW32__ ) )

    #define WM_POINTERUPDATE 0X02'45
    #define WM_POINTERDOWN   0X02'46
    #define WM_POINTERUP     0X02'47
...
typedef BOOL( WINAPI GetTouchInputInfoFunc )(
...
typedef BOOL(
    WINAPI CloseTouchInputHandleFunc( HTOUCHINPUT hTouchInput )
);    // input event handle; from touch message lParam

typedef BOOL( WINAPI RegisterTouchWindowFunc( HWND  hwnd,
                                              ULONG ulFlags ) );

// used together with WM_POINTER* events
typedef BOOL( WINAPI GetPointerTypeFunc( UINT32              pointerId,
...
typedef BOOL( WINAPI SetProcessDpiAwarenessFunc( PROCESS_DPI_AWARENESS dpi_awareness ) );
...
namespace osgViewer
{
    ...
    static osg::ApplicationUsageProxy GraphicsWindowWin32_e0(
        osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
        "OSG_WIN32_NV_MULTIMON_MULTITHREAD_WORKAROUND on/off",
        "Enable/disable duplicate makeCurrentContext call used as workaround for "
        "WinXP/NVidia/MultiView/MulitThread isues (pre 178.13 drivers)."
    );

    //
    // Utility class to specify the visual attributes for wglChoosePixelFormatARB()
    // function
    //
    ...
    //         setup, update & event processing. Rendering is done (optionally) by other
    //         threads.
    //
    // !@todo Have a dedicated thread managed by the Win32WindowingSystem class handle
    // the
    ...
    class Win32WindowingSystem : public osg::GraphicsContext::WindowingSystemInterface
    {
        public:
            ...
            static std::string
                osgGraphicsWindowWithCursorClass;       //!< Name of Win32 window class
                                                        //!< (with cursor) used by OSG
                                                        //!< graphics window instances
            static std::string
                osgGraphicsWindowWithoutCursorClass;    //!< Name of Win32 window class
                                                        //!< (without cursor) used by OSG
                                                        //!< graphics window instances

            Win32WindowingSystem();

            // Access the Win32 windowing system through this singleton class.
            static osg::observer_ptr<Win32WindowingSystem>&
            getInterface()
            {
            ...
            virtual void
            getScreenColorDepth( const osg::GraphicsContext::ScreenIdentifier& si,
            ...
            virtual void
            getScreenPosition( const osg::GraphicsContext::ScreenIdentifier& si,
                               int&                                          originX,
                               int&                                          originY,
                               unsigned int&                                 width,
                               unsigned int&                                 height );
            ...
            // Register a newly created native window along with its application
            // counterpart This is required to maintain a link between Windows messages
            // and the application window object at event processing time
            virtual void
            ...
            virtual void
            unregisterWindow( HWND hwnd );

            // Get the application window object associated with a native window
            virtual osgViewer::GraphicsWindowWin32*
            getGraphicsWindowFor( HWND hwnd );

            // Return a valid sample OpenGL Device Context and current rendering context
            // that can be used with wglXYZ extensions
            virtual bool
            getSampleOpenGLContext( OpenGLContext& context,
                                    HDC            windowHDC,
                                    int            windowOriginX,
                                    int            windowOriginY );
        ...
        protected:
            ...
            void
            enumerateDisplayDevices( DisplayDevices& displayDevices ) const;

            // Get the screen device current mode information
            bool
            getScreenInformation( const osg::GraphicsContext::ScreenIdentifier& si,
                                  DISPLAY_DEVICE& displayDevice,
                                  DEVMODE&        deviceMode );

            // Change the screen settings (resolution, refresh rate, etc.)
            bool
            changeScreenSettings( const osg::GraphicsContext::ScreenIdentifier& si,
                                  DISPLAY_DEVICE& displayDevice,
                                  DEVMODE&        deviceMode );
            ...
            bool _windowClassesRegistered;    //!< true after window classes have been
                                              //!< registered
        ...
        private:
            ...
            Win32WindowingSystem&
            operator=( const Win32WindowingSystem& );
    };
    ...
    //////////////////////////////////////////////////////////////////////////////
    static bool
    areWindowDimensionsChanged( HWND hwnd,
                                int  screenOriginX,
                                int  screenOriginY,
                                int& windowX,
                                int& windowY,
    ...
    {
        osgViewer::GraphicsWindowWin32* window =
            Win32WindowingSystem::getInterface()->getGraphicsWindowFor( hwnd );
        ...
        _windowClassesRegistered( false )
    {
        ...
        if( hModule )
        {
            registerTouchWindowFunc =
                ( RegisterTouchWindowFunc* )GetProcAddress( hModule,
                                                            "RegisterTouchWindow" );
            closeTouchInputHandleFunc =
                ( CloseTouchInputHandleFunc* )GetProcAddress( hModule,
                                                              "CloseTouchInputHandle" );
            getTouchInputInfoFunc =
                ( GetTouchInputInfoFunc* )GetProcAddress( hModule, "GetTouchInputInfo" );

            // check if Win8 and later API is available
            getPointerTypeFunc =
                ( GetPointerTypeFunc* )GetProcAddress( hModule, "GetPointerType" );
        ...
        // adjusts the scale factor whenever the DPI changes.These applications are not
    ...
    void
    Win32WindowingSystem::enumerateDisplayDevices( DisplayDevices& displayDevices ) const
    {
        for( unsigned int deviceNum = 0;; ++deviceNum )
        {
            ...
            // Do not track devices used for remote access (Terminal Services
            // pseudo-displays, etc.)
    ...
    void
    Win32WindowingSystem::registerWindowClasses()
    {
        ...
        if( ::RegisterClassEx( &wc ) == 0 )
        {
            ...
            {
                reportError( "Win32WindowingSystem::registerWindowClasses() - Unable to "
                             "register first window class",
                             lastError );
                return;
            }
        }
    ...
    void
    Win32WindowingSystem::unregisterWindowClasses()
    {
        if( _windowClassesRegistered )
        {
            ::UnregisterClass( osgGraphicsWindowWithCursorClass.c_str(),
                               ::GetModuleHandle( NULL ) );
            ::UnregisterClass( osgGraphicsWindowWithoutCursorClass.c_str(),
                               ::GetModuleHandle( NULL ) );
            _windowClassesRegistered = false;
        }
    }
    ...
    bool
    Win32WindowingSystem::getSampleOpenGLContext( OpenGLContext& context,
                                                  HDC            windowHDC,
                                                  int            windowOriginX,
                                                  int            windowOriginY )
    {
        ...
        if( hwnd == 0 )
        {
            reportError( "Win32WindowingSystem::getSampleOpenGLContext() - Unable to "
                         "create window",
                         ::GetLastError() );
            return false;
        }
    ...
    unsigned int
    Win32WindowingSystem::getNumScreens(
    ...
    bool
    Win32WindowingSystem::getScreenInformation(
        const osg::GraphicsContext::ScreenIdentifier& si,
        DISPLAY_DEVICE&                               displayDevice,
        DEVMODE&                                      deviceMode
    )
    {
        if( si.displayNum > 0 )
        {
            OSG_WARN << "Win32WindowingSystem::getScreenInformation() - The screen "
                        "identifier on the Win32 platform must always use display "
                        "number 0. Value received was "
                     << si.displayNum << std::endl;
            return false;
        }
        ...
        {
            OSG_WARN << "Win32WindowingSystem::getScreenInformation() - Cannot get "
                        "information for screen "
                     << si.screenNum << " because it does not exist." << std::endl;
            return false;
        }
        ...
        {
            std::ostringstream str;
            str << "Win32WindowingSystem::getScreenInformation() - Unable to query "
                   "information for screen number "
                << si.screenNum;
    ...
    void
    Win32WindowingSystem::getScreenSettings(
        const osg::GraphicsContext::ScreenIdentifier& si,
        osg::GraphicsContext::ScreenSettings&         resolution
    )
    {
        ...
        if( ( deviceMode.dmFields & DM_DISPLAYFREQUENCY ) != 0 )
        {
            ...
            {
                // Windows specific: 0 and 1 represent the hhardware's default refresh
                // rate. If someone knows how to get this refresh rate (in Hz)...
                OSG_NOTICE << "Win32WindowingSystem::getScreenSettings() is not fully "
                              "implemented (cannot retrieve the hardware's default "
                              "refresh rate)."
                           << std::endl;
    ...
    void
    Win32WindowingSystem::getScreenColorDepth(
        const osg::GraphicsContext::ScreenIdentifier& si,
        unsigned int&                                 dmBitsPerPel
    )
    {
    ...
    bool
    Win32WindowingSystem::changeScreenSettings(
        const osg::GraphicsContext::ScreenIdentifier& si,
        DISPLAY_DEVICE&                               displayDevice,
        DEVMODE&                                      deviceMode
    )
    {
        ...
        std::string msg = "Win32WindowingSystem::changeScreenSettings() - Unable to "
                          "change the screen settings.";

        switch( result )
        {
            case DISP_CHANGE_BADMODE :
                msg += " The specified graphics mode is not supported.";
                break;
    ...
    bool
    Win32WindowingSystem::setScreenSettings(
        const osg::GraphicsContext::ScreenIdentifier& si,
        const osg::GraphicsContext::ScreenSettings&   resolution
    )
    {
    ...
    void
    Win32WindowingSystem::enumerateScreenSettings(
        const osg::GraphicsContext::ScreenIdentifier& si,
    ...
    )
    {
        ...
        if( si.displayNum > 0 )
        {
            OSG_WARN << "Win32WindowingSystem::enumerateScreenSettings() - The screen "
                        "identifier on the Win32 platform must always use display "
                        "number 0. Value received was "
                     << si.displayNum << std::endl;
            return;
        ...
        if( si.screenNum >= static_cast<int>( displayDevices.size() ) )
        {
            OSG_WARN << "Win32WindowingSystem::enumerateScreenSettings() - Cannot get "
                        "information for screen "
                     << si.screenNum << " because it does not exist." << std::endl;
            return;
        }
        ...
        // Do the enumeration
        DEVMODE                   deviceMode;
        static const unsigned int MAX_RESOLUTIONS =
            4'046;    // Upper limit to avoid infinite (= very long) loop.
        for( unsigned int i = 0; i < MAX_RESOLUTIONS; ++i )
        {
            ...
            resolutionList.push_back(
                osg::GraphicsContext::ScreenSettings( deviceMode.dmPelsWidth,
    ...
    void
    Win32WindowingSystem::getScreenPosition(
        const osg::GraphicsContext::ScreenIdentifier& si,
        int&                                          originX,
        int&                                          originY,
        unsigned int&                                 width,
    ...
    osg::GraphicsContext*
    Win32WindowingSystem::createGraphicsContext( osg::GraphicsContext::Traits* traits )
    {
        if( traits->pbuffer )
        {
            osg::ref_ptr<osgViewer::PixelBufferWin32> pbuffer =
                new PixelBufferWin32( traits );
            if( pbuffer->valid() )
            {
        ...
        else
        {
            ...
            osg::ref_ptr<osgViewer::GraphicsWindowWin32> window =
                new GraphicsWindowWin32( traits );
            if( window->valid() )
            {
    ...
    void
    Win32WindowingSystem::registerWindow( HWND                            hwnd,
                                          osgViewer::GraphicsWindowWin32* window )
    {
        if( hwnd )
        {
    ...
    void
    Win32WindowingSystem::unregisterWindow( HWND hwnd )
    {
        if( hwnd )
        {
    ...
    Win32WindowingSystem::getGraphicsWindowFor( HWND hwnd )
    {
    ...
    GraphicsWindowWin32::init()
    {
        ...
        if( areWindowDimensionsChanged( _hwnd,
                                        _screenOriginX,
                                        _screenOriginY,
                                        windowX,
                                        windowY,
        ...
        // make sure the event queue has the correct window rectangle size and input
        // range
        getEventQueue()->syncWindowRectangleWithGraphicsContext();

        // 2008/10/03
        // Few days ago NVidia released WHQL certified drivers ver 178.13.
        // These drivers (as well as former beta ver 177.92) were free from the bug
        // described below. So it looks like its high time to make the workaround
        // inactive by default. If you happen to still use earlier drivers and have
        // problems consider changing to new ones or activate
        // OSG_MULTIMONITOR_MULTITHREAD_WIN32_NVIDIA_WORKAROUND macro def through CMake
        // advanced vars.
        ...
        // dualview / multicore CPU affects GeForce 6x00, 7x00, 8x00 boards (others were
        // not tested) driver versions 174.xx - 175.xx pre 174.xx had other issues so
        // reverting is not an option (statitistics, fbo) drivers release 175.16 is the
        // latest currently available
        //
        // When using OpenGL in threaded app ( main thread sets up context / renderer
        // thread draws using it ) first wglMakeCurrent seems to not work right and screw
        // OpenGL context driver data: 1: successive drawing shows a number of artifacts
        // in TriangleStrips and TriangleFans 2: weird behaviour of FramBufferObjects
        // (glGenFramebuffer generates already generated ids ...) Looks like repeating
        // wglMakeCurrent call fixes all these issues wglMakeCurrent call can impact
        // performance so I try to minimize number of wglMakeCurrent calls by checking
        // current HDC and GL context and repeat wglMakeCurrent only when they change for
        // current thread
        ...
        {
            _applyWorkaroundForMultimonitorMultithreadNVidiaWin32Issues =
    ...
    GraphicsWindowWin32::createWindow()
    {
        ...
        if( !determineWindowPositionAndStyle( _traits->screenNum,
                                              ...
                                              extendedStyle ) )
        {
            reportError( "GraphicsWindowWin32::createWindow() - Unable to determine the "
        ...
        _hwnd = ::CreateWindowEx(
            extendedStyle,
            _traits->useCursor
                ? Win32WindowingSystem::osgGraphicsWindowWithCursorClass.c_str()
        ...
        if( _hwnd == 0 )
        {
            reportErrorForScreen(
                "GraphicsWindowWin32::createWindow() - Unable to create window",
                _traits->screenNum,
                ::GetLastError()
            );
            return false;
        ...
        if( _hdc == 0 )
        {
            reportErrorForScreen( "GraphicsWindowWin32::createWindow() - Unable to get "
...
//
        ...
        if( _hglrc == 0 )
        {
            reportErrorForScreen( "GraphicsWindowWin32::createWindow() - Unable to "
    ...
    GraphicsWindowWin32::setWindow( HWND handle )
    {
        if( _initialized )
        {
            reportErrorForScreen( "GraphicsWindowWin32::setWindow() - Window already "
        ...
        {
            reportErrorForScreen(
                "GraphicsWindowWin32::setWindow() - Invalid window handle passed",
                _traits->screenNum,
                ::GetLastError()
            );
            return false;
        ...
        {
            reportErrorForScreen(
                "GraphicsWindowWin32::setWindow() - Unable to get window device context",
                _traits->screenNum,
                ::GetLastError()
            );
            _hwnd = 0;
        ...
        if( !setPixelFormat() )
        {
            reportErrorForScreen( "GraphicsWindowWin32::setWindow() - Unable to set the "
                                  "inherited window pixel format",
                                  _traits->screenNum,
                                  ::GetLastError() );
            ::ReleaseDC( _hwnd, _hdc );
            _hdc  = 0;
        ...
        if( _hglrc == 0 )
        {
            reportErrorForScreen( "GraphicsWindowWin32::setWindow() - Unable to create "
    ...
    void
    GraphicsWindowWin32::destroyWindow( bool deleteNativeWindow )
    {
        if( _destroying )
        {
            return;
        ...
        if( _graphicsThread && _graphicsThread->isRunning() )
        {
            ...
            for( osg::GraphicsContext::Cameras::iterator it = cameras.begin();
                 it != cameras.end();
                 ++it )
            {
                osgViewer::View* view =
                    dynamic_cast<osgViewer::View*>( ( *it )->getView() );
    ...
    void
    GraphicsWindowWin32::registerWindow()
    {
        Win32WindowingSystem::getInterface()->registerWindow( _hwnd, this );
    }

    void
    GraphicsWindowWin32::unregisterWindow()
    {
        Win32WindowingSystem::getInterface()->unregisterWindow( _hwnd );
    }

    bool
    GraphicsWindowWin32::registerWindowProcedure()
    {
        ::SetLastError( 0 );
        _windowProcedure =
            ( WNDPROC )::SetWindowLongPtr( _hwnd, GWLP_WNDPROC, LONG_PTR( WindowProc ) );
        unsigned int error = ::GetLastError();

        if( _windowProcedure == 0 && error )
        {
            reportErrorForScreen( "GraphicsWindowWin32::registerWindowProcedure() - "
                                  "Unable to register window procedure",
                                  _traits->screenNum,
                                  error );
            return false;
        }
    ...
    bool
    GraphicsWindowWin32::unregisterWindowProcedure()
    {
        ...
        ::SetLastError( 0 );
        WNDPROC wndProc = ( WNDPROC )::SetWindowLongPtr( _hwnd,
                                                         GWLP_WNDPROC,
                                                         LONG_PTR( _windowProcedure ) );
        unsigned int error = ::GetLastError();

        if( wndProc == 0 && error )
        {
            reportErrorForScreen( "GraphicsWindowWin32::unregisterWindowProcedure() - "
                                  "Unable to unregister window procedure",
                                  _traits->screenNum,
                                  error );
            return false;
        }
    ...
    bool
    GraphicsWindowWin32::determineWindowPositionAndStyle( unsigned int  screenNum,
                                                          int           clientAreaX,
                                                          int           clientAreaY,
                                                          ...
                                                          unsigned int& extendedStyle )
    {
        ...
        osg::ref_ptr<Win32WindowingSystem>     windowManager =
            Win32WindowingSystem::getInterface();

        windowManager->getScreenPosition( screenId,
                                          _screenOriginX,
                                          _screenOriginY,
                                          _screenWidth,
                                          _screenHeight );
        ...
        if( decorated )
        {
            ...
            {
                reportErrorForScreen(
                    "GraphicsWindowWin32::determineWindowPositionAndStyle() - Unable to "
    ...
    GraphicsWindowWin32::createContextImplementation()
    {
        ...
        OSG_NOTIFY( osg::INFO ) << "EGL: Attempting to create context." << std::endl;
        OSG_NOTIFY( osg::INFO )
            << "EGL: version: " << _traits->glContextVersion << std::endl;
        OSG_NOTIFY( osg::INFO )
            << "EGL: context flags: " << _traits->glContextFlags << std::endl;
        OpenGLContext openGLContext;
        if( !Win32WindowingSystem::getInterface()->getSampleOpenGLContext(
                openGLContext,
                _hdc,
                _screenOriginX,
                _screenOriginY
        ...
        {
            reportErrorForScreen( "EGL: Can't create context.",
                                  _traits->screenNum,
                                  ::GetLastError() );
        }
        else
        {
            ...
            context.eglContext = createContext( openGLContext.contextInfo().eglDisplay,
                                                openGLContext.getConfig() );

            EGLint surfaceAttributes[] = { EGL_NONE, EGL_NONE };
            context.eglSurface =
                eglCreateWindowSurface( openGLContext.contextInfo().eglDisplay,
                                        openGLContext.getConfig(),
                                        _hwnd,
                                        surfaceAttributes );

            OSG_NOTIFY( osg::INFO ) << "EGL: context created successfully." << std::endl;
        }
        return context;
    }
    ...
    GraphicsWindowWin32::setPixelFormat()
    {
        ...
        if( !eglChooseConfig( openGLContext.contextInfo().eglDisplay,
                              formatSpecs.get(),
                              NULL,
                              0,
                              &numConfigs ) )
        ...
        EGLBoolean err = eglChooseConfig( openGLContext.contextInfo().eglDisplay,
                                          formatSpecs.get(),
                                          &windowConfig,
                                          1,
                                          &numConfigs );
        ...
        {
            unsigned int bpp;
            Win32WindowingSystem::getInterface()->getScreenColorDepth( *_traits.get(),
                                                                       bpp );
            if( bpp < 32 )
            {
                OSG_INFO << "GraphicsWindowWin32::setPixelFormat() - Display setting is "
                            "not 32 bit colors, "
                         << bpp << " bits per pixel on screen #" << _traits->screenNum
                         << std::endl;
                ...
                _traits->blue  = bpp / 4;
                preparePixelFormatSpecifications( *_traits, formatSpecs, true );
                err = eglChooseConfig( openGLContext.contextInfo().eglDisplay,
                                       formatSpecs.get(),
                                       &windowConfig,
                                       1,
                                       &numConfigs );
        ...
        if( err == EGL_FALSE )
        {
            ...
            {
                reportErrorForScreen(
                    "GraphicsWindowWin32::setPixelFormat() - No matching pixel format "
                    "found based on traits specified",
                    _traits->screenNum,
                    0
                );
                return false;
            ...
            OSG_INFO << "GraphicsWindowWin32::setPixelFormat() - Found a matching pixel "
                        "format but without the WGL_SWAP_METHOD_ARB specification for "
                        "screen #"
                     << _traits->screenNum << std::endl;
        }
        ...
        if( traits.doubleBuffer )
        {
            ...
            switch( traits.swapMethod )
            {
                ...
                case osg::DisplaySettings::SWAP_DEFAULT :
                    ...
                    // WGL_SWAP_EXCHANGE_ARB is the GL default when no WGL_SWAP attrib is
                    // given. To be precise: At least on Windows 7 and Nvidia it seems to
                    // be a default.
    ...
    static int
    ChooseMatchingPixelFormat( HDC                           hdc,
                               int                           screenNum,
                               const XGLIntegerAttributes&   formatSpecifications,
                               osg::GraphicsContext::Traits* _traits )
    {
        //
        ...
        if( wglChoosePixelFormatARB == 0 )
        {
            ...
            PIXELFORMATDESCRIPTOR pixelFormat = {
                ...
                PFD_DRAW_TO_WINDOW |                        // support window
                    ...
                    DWORD( _traits->doubleBuffer ? PFD_DOUBLEBUFFER
                                                 : 0 ) |    // double buffered ?
            ...
            {
                reportErrorForScreen(
                    "ChooseMatchingPixelFormat() - GDI ChoosePixelFormat Failed.",
                    screenNum,
                    ::GetLastError()
                );
                return -1;
            ...
            ::DescribePixelFormat( hdc,
                                   ...
                                   sizeof( PIXELFORMATDESCRIPTOR ),
                                   &pixelFormat );
            if( ( ( pixelFormat.dwFlags & PFD_GENERIC_FORMAT ) != 0 ) &&
                ( ( pixelFormat.dwFlags & PFD_GENERIC_ACCELERATED ) == 0 ) )
            {
                OSG_WARN << "Rendering in software: pixelFormatIndex "
                         << pixelFormatIndex << std::endl;
            }
        ...
        {
            reportErrorForScreen( "ChooseMatchingPixelFormat() - Unable to choose the "
    ...
    GraphicsWindowWin32::setPixelFormat()
    {
        ...
        int pixelFormatIndex =
            ::ChooseMatchingPixelFormat( openGLContext.deviceContext(),
                                         _traits->screenNum,
                                         formatSpecs,
                                         _traits.get() );
        ...
        if( pixelFormatIndex < 0 )
        {
            ...
            if( bpp < 32 )
            {
                ...
                _traits->red = bpp / 4;    // integer divide, determine minimum number of
                                           // bits we will accept
                _traits->green = bpp / 4;
                _traits->blue  = bpp / 4;
                ::PreparePixelFormatSpecifications(
                    *_traits,
                    formatSpecs,
                    true
                );    // try again with WGL_SWAP_METHOD_ARB
                pixelFormatIndex =
                    ::ChooseMatchingPixelFormat( openGLContext.deviceContext(),
                                                 _traits->screenNum,
                                                 formatSpecs,
        ...
        {
            reportErrorForScreen(
                "GraphicsWindowWin32::setPixelFormat() - Unable to set pixel format",
                _traits->screenNum,
                ::GetLastError()
            );
            return false;
    ...
    GraphicsWindowWin32::createContextImplementation()
    {
        ...
        {
            OSG_NOTIFY( osg::INFO )
                << "GL4.6 Core: Attempting to create OpenGL 4.6 Core Profile context."
                << std::endl;
            OSG_NOTIFY( osg::INFO )
                << "GL4.6 Core: version: " << _traits->glContextVersion << std::endl;
            OSG_NOTIFY( osg::INFO )
                << "GL4.6 Core: context flags: " << _traits->glContextFlags << std::endl;
            OSG_NOTIFY( osg::INFO )
                << "GL4.6 Core: profile: " << _traits->glContextProfileMask << std::endl;
            ...
            {
                reportErrorForScreen( "GL4.6 Core: Can't create sample context.",
                                      _traits->screenNum,
                                      ::GetLastError() );
            }
            else
            {
                ...
                {
                    reportErrorForScreen(
                        "GL4.6 Core: wglCreateContextAttribsARB not available.",
                        _traits->screenNum,
                        ::GetLastError()
                    );
                }
                else
                {
                    ...
                    attribs[idx++] = WGL_CONTEXT_CORE_PROFILE_BIT_ARB;
                    ...
                    {
                        reportErrorForScreen(
                            "GL4.6 Core: wglCreateContextAttribsARB returned NULL.",
                            _traits->screenNum,
                            ::GetLastError()
                        );
                    }
                    ...
                    {
                        OSG_NOTIFY( osg::INFO )
                            << "GL4.6 Core: context created successfully." << std::endl;
                    }
                }
            }
        }

        // Fallback: if GL 4.6 Core context creation fails for any reason,
        // create a legacy context. This is unlikely to work properly since
        // OSG is now built for GL 4.6 Core features only.
        if( context == NULL )
        {
            OSG_WARN << "GL4.6 Core: Falling back to legacy wglCreateContext. "
                     << "This context may not support required GL 4.6 features."
                     << std::endl;
    ...
    bool
    GraphicsWindowWin32::setWindowDecorationImplementation( bool decorated )
    {
        unsigned int windowStyle;
        unsigned int extendedStyle;

        //
        // Determine position and size of window with/without decorations to retain the
        // size specified in traits
        //
        ...
        if( !determineWindowPositionAndStyle( _traits->screenNum,
                                              ...
                                              extendedStyle ) )
        {
            reportErrorForScreen( "GraphicsWindowWin32::setWindowDecoration() - Unable "
                                  "to determine the window position and style",
                                  _traits->screenNum,
                                  0 );
            return false;
        }
        ...
        {
            reportErrorForScreen( "GraphicsWindowWin32::setWindowDecoration() - Unable "
                                  "to set new window position and size",
                                  _traits->screenNum,
                                  ::GetLastError() );
            return false;
        }
    ...
    GraphicsWindowWin32::realizeImplementation()
    {
        ...
        if( _traits.valid() && ( _traits->sharedContext.valid() ||
                                 _traits->vsync ||
                                 _traits->swapGroupEnabled ) )
        {
#ifdef OSG_USE_EGL
#else
            // make context current so we can test capabilities and set up context
            // sharing
            ...
            GraphicsHandleWin32* graphicsHandleWin32 =
                dynamic_cast<GraphicsHandleWin32*>( _traits->sharedContext.get() );
            if( graphicsHandleWin32 )
            {
                if( !wglShareLists( graphicsHandleWin32->getWGLContext(),
                                    getWGLContext() ) )
                {
                    reportErrorForScreen( "GraphicsWindowWin32::realizeImplementation() "
            ...
            {
                setSwapGroup( _traits->swapGroupEnabled,
                              _traits->swapGroup,
                              _traits->swapBarrier );
            }
#endif
        ...
        {
            //
            // Bring the window on top of other ones (including the taskbar if it covers
            // it completely)
            //
            // NOTE: To cover the taskbar with a window that does not completely cover
            // it, the HWND_TOPMOST Z-order must be used in the code below instead of
            // HWND_TOP.
            // @todo: This should be controlled through a flag in the traits
            // (topMostWindow)
            //
    ...
    GraphicsWindowWin32::makeCurrentImplementation()
    {
        if( !_realized )
        {
            reportErrorForScreen( "GraphicsWindowWin32::makeCurrentImplementation() - "
                                  "Window not realized; cannot do makeCurrent.",
                                  _traits->screenNum,
                                  0 );
            return false;
        }
        ...
        OSG_NOTIFY( osg::INFO ) << "eglMakeCurrent: " << c.eglDisplay << " "
                                << c.eglSurface << " " << c.eglContext << std::endl;
#else
        if( _applyWorkaroundForMultimonitorMultithreadNVidiaWin32Issues )
        {
            if( ::wglGetCurrentDC() != _hdc || ::wglGetCurrentContext() != _hglrc )
            {
                if( !::wglMakeCurrent( _hdc, _hglrc ) )
                {
                    reportErrorForScreen(
                        "GraphicsWindowWin32::makeCurrentImplementation() - Unable to "
        ...
        // Initialize GLEW for this context (safe to call multiple times)
        glewExperimental = GL_TRUE;
        GLenum glewErr   = glewInit();
        if( glewErr != GLEW_OK )
        {
            OSG_WARN << "GLEW initialization failed: " << glewGetErrorString( glewErr )
                     << std::endl;
        }
        // Clear any GL error generated by GLEW init (common with core profile)
    ...
    bool
    GraphicsWindowWin32::releaseContextImplementation()
    {
#ifdef OSG_USE_EGL
        if( !::eglMakeCurrent( _eglContextInfo.eglDisplay,
                               _eglContextInfo.eglSurface,
                               EGL_NO_SURFACE,
                               EGL_NO_CONTEXT ) )
#else
...
#endif
        {
            reportErrorForScreen( "GraphicsWindowWin32::releaseContextImplementation() "
                                  "- Unable to release current OpenGL rendering context",
                                  _traits->screenNum,
                                  ::GetLastError() );
            return false;
        }
    ...
    void
    GraphicsWindowWin32::closeImplementation()
    {
        destroyWindow();
    ...
    void
    GraphicsWindowWin32::swapBuffersImplementation()
    {
        if( !_realized )
        {
            return;
        ...
        if( !res )
        {
            reportErrorForScreen( "GraphicsWindowWin32::swapBuffersImplementation() - "
                                  "Unable to swap display buffers",
                                  _traits->screenNum,
                                  res );
        }
#else
    ...
    bool
    GraphicsWindowWin32::checkEvents()
    {
        if( !_realized )
        {
            return false;
    ...
    void
    GraphicsWindowWin32::grabFocus()
    {
        if( !::SetForegroundWindow( _hwnd ) )
        {
            OSG_WARN << "Warning: GraphicsWindowWin32::grabFocus() - Failed grabbing "
                        "the focus"
                     << std::endl;
        }
    }
    ...
    void
    GraphicsWindowWin32::grabFocusIfPointerInWindow()
    {
        POINT mousePos;
        if( !::GetCursorPos( &mousePos ) )
        {
            reportErrorForScreen( "GraphicsWindowWin32::grabFocusIfPointerInWindow() - "
    ...
    void
    GraphicsWindowWin32::requestWarpPointer( float x,
                                             float y )
    {
        if( !_realized )
        {
            OSG_INFO << "GraphicsWindowWin32::requestWarpPointer() - Window not "
                        "realized; cannot warp pointer, screenNum="
                     << _traits->screenNum << std::endl;
            return;
        }
    ...
    if (!::SetCursorPos(windowRect.left + x, windowRect.top + y))
    {
        ...
        {
            reportErrorForScreen( "GraphicsWindowWin32::requestWarpPointer() - Unable "
                                  "to convert cursor position to screen coordinates",
                                  _traits->screenNum,
                                  ::GetLastError() );
        }
    ...
    bool
    GraphicsWindowWin32::setWindowRectangleImplementation( int x,
                                                           int y,
                                                           int width,
                                                           int height )
    {
        ...
        {
            reportErrorForScreen(
                "GraphicsWindowWin32::setWindowRectangleImplementation() - Unable to "
                "determine the window position and style",
                _traits->screenNum,
                0
            );
            return false;
    ...
    void
    GraphicsWindowWin32::setWindowName( const std::string& name )
    {
    ...
    void
    GraphicsWindowWin32::useCursor( bool cursorOn )
    {
        if( _traits.valid() )
        {
    ...
    void
    GraphicsWindowWin32::setCursorImpl( MouseCursor mouseCursor )
    {
        if( _mouseCursor != mouseCursor )
        {
            ...
            _traits->useCursor =
                ( _currentCursor != NULL ) && ( _mouseCursor != NoCursor );
    ...
    HCURSOR
    GraphicsWindowWin32::getOrCreateCursor( MouseCursor mouseCursor )
    {
    ...
    void
    GraphicsWindowWin32::setSwapGroup( bool   on,
                                       GLuint group,
                                       GLuint barrier )
    {
#if defined( OSG_USE_EGL )
        ...
        typedef BOOL( GL_APIENTRY * PFNWGLBINDSWAPBARRIERNVPROC )( GLuint group,
                                                                   GLuint barrier );
        ...
        {
            OSG_INFO << "GraphicsWindowWin32::wglJoinSwapGroupNV(bool, GLuint, GLuint) "
                        "not supported"
                     << std::endl;
            return;
        }
        ...
        OSG_INFO << "GraphicsWindowWin32::wglJoinSwapGroupNV (" << swapGroup
                 << ") returned " << resultJoin << std::endl;

        int  swapBarrier = ( on ? barrier : 0 );
        BOOL resultBind  = wglBindSwapBarrierNV( swapGroup, swapBarrier );
        OSG_INFO << "GraphicsWindowWin32::wglBindSwapBarrierNV (" << swapGroup << ", "
                 << swapBarrier << ") returned " << resultBind << std::endl;
#endif
    }

    void
    GraphicsWindowWin32::setSyncToVBlank( bool on )
    {
#if defined( OSG_USE_EGL )
#else
        if( _traits.valid() )
        ...
        // #if 0
        //  we ought to properly check if the extension is listed as supported rather
        //  than just if the function pointer resolves through wglGetProcAddress, but in
        //  practice everything supports this extension
        ...
        if( wglSwapIntervalEXT )
        {
            ...
            OSG_INFO << "GraphicsWindowWin32::setSyncToVBlank " << ( on ? "on" : "off" )
                     << std::endl;
        }
        else
        {
            OSG_INFO << "GraphicsWindowWin32::setSyncToVBlank(bool) not supported"
                     << std::endl;
        }
        // #else
    ...
    void
    GraphicsWindowWin32::adaptKey( WPARAM        wParam,
                                   LPARAM        lParam,
                                   int&          keySymbol,
                                   ...
                                   int&          unmodifiedKeySymbol )
    {
        ...
        bool rightSide = ( lParam & 0X01'00'00'00 ) != 0;
        int  virtualKey =
            ::MapVirtualKeyEx( ( lParam >> 16 ) & 0XFF, 3, ::GetKeyboardLayout( 0 ) );
        ...
        switch( virtualKey )
        {
            ...
            case VK_LCONTROL :
                ...
                modifierMask |= rightSide ? osgGA::GUIEventAdapter::MODKEY_RIGHT_CTRL
            ...
            case VK_LMENU :
                ...
                modifierMask |= rightSide ? osgGA::GUIEventAdapter::MODKEY_RIGHT_ALT
        ...
        if( ( keySymbol & 0XFF'00 ) == 0 )
        {
            char asciiKey[2];
            int  numChars = ::ToAscii( wParam,
                                       ( lParam >> 16 ) & 0XFF,
                                       keyState,
                                       reinterpret_cast<WORD*>( asciiKey ),
                                       0 );
            if( numChars > 0 )
            {
    ...
    void
    GraphicsWindowWin32::transformMouseXY( float& x,
                                           float& y )
    {
        if( getEventQueue()->getUseFixedMouseInputRange() )
        {
            ...
            y = eventState->getYmin() +
                ...
                y /
                float( _traits->height );
        }
    }

    LRESULT
    GraphicsWindowWin32::handleNativeWindowingEvent( HWND   hwnd,
                                                     UINT   uMsg,
                                                     WPARAM wParam,
                                                     LPARAM lParam )
    {
        ...
        switch( uMsg )
        {
            ...
            case WM_MOUSEWHEEL :
                ...
                getEventQueue()->mouseScroll( GET_WHEEL_DELTA_WPARAM( wParam ) < 0
            ...
            case WM_SIZE :
                ...
                {
                    int windowX = _traits->x, windowY = _traits->y,
                        windowWidth = _traits->width, windowHeight = _traits->height;
                    if( areWindowDimensionsChanged( hwnd,
                                                    _screenOriginX,
                                                    _screenOriginY,
                                                    windowX,
                                                    windowY,
                    ...
                    {
                        resized( windowX, windowY, windowWidth, windowHeight );
                        getEventQueue()->windowResize( windowX,
                                                       windowY,
                                                       windowWidth,
                                                       windowHeight,
                                                       eventTime );
            ...
            case WM_SETCURSOR :
                ///////////////////
                // The cursor is only modified in response to the WM_SETCURSOR message if
                // the mouse cursor isn't set to InheritCursor.  InheritCursor lets the
                // user manage the cursor externally.
            ...
            case WM_SETFOCUS :
                ...
                for( unsigned int i = 0X08; i < 0XFF; i++ )
                {
                    ...
                    // Skip CONTROL | MENU | SHIFT tests because we are polling exact
                    // left or right keys above return press for both right and left so
                    // we may end up with inconsistent modifier mask if we report left
                    // control & right control while only right was pressed
                    ...
                    switch( i )
                    {
                        ...
                        case VK_RSHIFT :
                        case VK_RMENU :
                            rightSideCode = 0X01'00'00'00;
                    }
                    if( ( ::GetAsyncKeyState( i ) & 0X80'00 ) != 0 )
                    {
                        // Compute lParam because subsequent adaptKey will rely on
                        // correct lParam
                        UINT scanCode =
                            ::MapVirtualKeyEx( i, 0, ::GetKeyboardLayout( 0 ) );
                        // Set Extended Key bit + Scan Code + 30 bit to indicate key was
                        // set before sending message See Windows SDK help on WM_KEYDOWN
                        // for explanation
                        LONG lParamKey =
                            rightSideCode | ( ( scanCode & 0XFF ) << 16 ) | ( 1 << 30 );
            ...
            case WM_KILLFOCUS :
                     ...
                     ++key )
                {
                    if( key->second )
                    {
                        getEventQueue()->keyRelease( key->first.first,
                                                     key->first.second );
                        key->second = false;
                    }
                }
            ...
            case WM_NCHITTEST :
                ///////////////////
                {
                    LONG_PTR result = _windowProcedure == 0
                                        ? ::DefWindowProc( hwnd, uMsg, wParam, lParam )
                                        : ::CallWindowProc( _windowProcedure,
                                                            hwnd,
                                                            uMsg,
                                                            wParam,
                                                            lParam );
            ...
            case WM_TOUCH :
                /////////////
                {
                    ...
                    if( getTouchInputInfoFunc &&
                        ( *getTouchInputInfoFunc )( ( HTOUCHINPUT )lParam,
                                                    numInputs,
                                                    ti,
                                                    sizeof( TOUCHINPUT ) ) )
                    {
            ...
            // at the moment if the input doesn't come from a PEN, than the
            // DefWindowProc() default implementation is invoked, which will generate the
            // WM_TOUCH messages.
...
extern "C" OSGVIEWER_EXPORT void
    graphicswindow_Win32( void )
    {
    }

    static osg::WindowingSystemInterfaceProxy<Win32WindowingSystem>
    s_proxy_Win32WindowingSystem( "Win32" );

void
GraphicsWindowWin32::raiseWindow()
{
