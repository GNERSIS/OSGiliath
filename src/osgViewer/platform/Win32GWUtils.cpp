...
namespace osgViewer
{
    ...
    namespace EGL
    {
        ...
        OpenGLContext::~OpenGLContext()
        {
            if( _eglCtx.eglContext )
            {
                ...
                OSG_NOTIFY( osg::INFO ) << "eglMakeCurrent: " << _eglCtx.eglDisplay
                                        << " " << EGL_NO_SURFACE << std::endl;
            }
        ...
        bool
        OpenGLContext::makeCurrent( HDC  restoreOnHdc,
                                    bool restorePreviousOnExit )
        {
            if( _hdc == 0 || _eglCtx.isEmpty() )
            {
                return false;
            }
            ...
            if( _eglCtx.eglContext == _previousContext )
            {
                return true;
            }

            if( !::eglMakeCurrent( _eglCtx.eglDisplay,
                                   _eglCtx.eglSurface,
                                   _eglCtx.eglSurface,
                                   _eglCtx.eglContext ) )
            {
            ...
            OSG_NOTIFY( osg::DEBUG_INFO )
                << "eglMakeCurrent: " << _eglCtx.eglDisplay << " " << _eglCtx.eglSurface
                << " " << _eglCtx.eglContext << std::endl;

            return true;
        }

        ContextInfo&
        OpenGLContext::contextInfo()
        {
            return _eglCtx;
        }
        ...
        ContextInfo::ContextInfo( EGLContext _eglContext,
        ...
        ContextInfo::ContextInfo( const ContextInfo& o ) :
            eglContext( o.eglContext ),
            eglDisplay( o.eglDisplay ),
            eglSurface( o.eglSurface )
        {
        }
        ...
        EGLDisplay
        createAndInitializeEGLDisplay( HDC hdc = 0L )
        {
            /* possible types:
            ...
            */
            ...
            OSG_NOTIFY( osg::INFO )
                << "EGL Version: \"" << eglQueryString( eglDisplay, EGL_VERSION ) << "\""
                << std::endl;
            OSG_NOTIFY( osg::INFO )
                << "EGL Vendor: \"" << eglQueryString( eglDisplay, EGL_VENDOR ) << "\""
                << std::endl;
            OSG_NOTIFY( osg::INFO )
                << "EGL Extensions: \"" << eglQueryString( eglDisplay, EGL_EXTENSIONS )
                << "\"" << std::endl;
                                        ...
                                        HDC                 hdc )
        {
            ...
            EGLint     numConfigs;
            EGLBoolean err = eglChooseConfig( eglDisplay,
                                              configAttribs.get(),
                                              &windowConfig,
                                              1,
                                              &numConfigs );
            ...
            // EGLint surfaceAttributes[] = { EGL_RENDER_BUFFER,
            // EGL_POST_SUB_BUFFER_SUPPORTED_NV, EGL_NONE };
                                                  ...
                                                  XGLAttributes<int>& configAttribs )
        {
            ...
            EGLSurface eglSurface =
                eglCreatePbufferSurface( eglDisplay, config, configAttribs.get() );
            eglBindAPI( eglApi );
                       ...
                       const EGLConfig& config )
        {
            EGLint contextAttributes[] =
                { EGL_CONTEXT_CLIENT_VERSION, eglContextClientVersion, EGL_NONE };
            return eglCreateContext( eglDisplay,
                                     config,
                                     EGL_NO_CONTEXT,
                                     contextAttributes );
        }
        ...
        {
            if( !::eglMakeCurrent( c.eglDisplay,
                                   EGL_NO_SURFACE,
                                   EGL_NO_SURFACE,
                                   EGL_NO_CONTEXT ) )
            {
                reportError( "Win32GWUtils.destroyContext() - Unable to set current EGL "
                             "rendering context",
                             ::eglGetError() );
            }
            if( !::eglDestroySurface( c.eglDisplay, c.eglSurface ) )
            {
                reportError( "Win32GWUtils.destroyContext() - Unable to destroy current "
                             "EGL Surface context",
                             ::eglGetError() );
            }
	...
	if (traits.doubleBuffer)
	{
		...
		case osg::DisplaySettings::SWAP_DEFAULT:
            ...
            if( traits.alpha )
            {
                attributes.set( EGL_ALPHA_SIZE, traits.alpha );
            }
            if( traits.stencil )
            {
                attributes.set( EGL_STENCIL_SIZE, traits.stencil );
            }
            if( traits.sampleBuffers )
            {
                attributes.set( EGL_SAMPLE_BUFFERS, traits.sampleBuffers );
            }
            if( traits.samples )
            {
                attributes.set( EGL_SAMPLES, traits.samples );
            }
    ...
    namespace WGL
    {
        ...
        OpenGLContext::~OpenGLContext()
        {
            if( _restorePreviousOnExit &&
                _previousHglrc !=
                _hglrc &&
                !::wglMakeCurrent( _previousHdc, _previousHglrc ) )
            {
                reportError( "Win32WindowingSystem::OpenGLContext() - Unable to restore "
                                    ...
                                    bool restorePreviousOnExit )
        {
            ...
            {
                reportError( "Win32WindowingSystem::OpenGLContext() - Unable to set "
                             "current OpenGL rendering context",
                             ::GetLastError() );
                return false;
            }
                                          ...
                                          bool allowSwapExchangeARB )
        {
            ...
            attributes.set( WGL_COLOR_BITS_ARB,
            ...
            if( traits.doubleBuffer )
            {
                ...
                switch( traits.swapMethod )
                {
                    ...
                    case osg::DisplaySettings::SWAP_DEFAULT :
                        // Wojtek Lewandowski 2010-09-28:
                        // Keep backward compatibility if no method is selected via
                        // traits and let wglSwapExchangeARB flag select swap method.
                        // However, I would rather remove this flag because its
                        // now redundant to Traits::swapMethod and it looks like
                        // WGL_SWAP_EXCHANGE_ARB is the GL default when no WGL_SWAP
                        // attrib is given. To be precise: At least on Windows 7 and
                        // Nvidia it seems to be a default.
                        if( allowSwapExchangeARB )
                        {
                            attributes.set( WGL_SWAP_METHOD_ARB, WGL_SWAP_EXCHANGE_ARB );
                        }
    ...
    {
        //
        // Some APIs are documented as returning the error in ::GetLastError but
        // apparently do not Skip "Reason" field if the errorCode is still success
        //
