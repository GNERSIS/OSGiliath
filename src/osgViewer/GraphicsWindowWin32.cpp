...
const ContextInfo GraphicsWindowWin32::createContextImplementation()
{
	...
	OSG_NOTIFY(osg::INFO) << "EGL: version: " << _traits->glContextVersion << std::endl;
	OSG_NOTIFY(osg::INFO) << "EGL: context flags: " << _traits->glContextFlags << std::endl;
...
HGLRC GraphicsWindowWin32::createContextImplementation()
{
    ...
    {
        OSG_NOTIFY( osg::INFO ) << "GL4.6 Core: Attempting to create OpenGL 4.6 Core Profile context." << std::endl;
        OSG_NOTIFY( osg::INFO ) << "GL4.6 Core: version: " << _traits->glContextVersion << std::endl;
        OSG_NOTIFY( osg::INFO ) << "GL4.6 Core: context flags: " << _traits->glContextFlags << std::endl;
        OSG_NOTIFY( osg::INFO ) << "GL4.6 Core: profile: " << _traits->glContextProfileMask << std::endl;
        ...
        else
        {
            ...
            {
                reportErrorForScreen( "GL4.6 Core: wglCreateContextAttribsARB not available.",
                    _traits->screenNum, ::GetLastError() );
            }
            else
            {
                ...
                if( context == NULL )
                {
                    reportErrorForScreen( "GL4.6 Core: wglCreateContextAttribsARB returned NULL.",
                        _traits->screenNum, ::GetLastError() );
                }
                else
                {
                    OSG_NOTIFY( osg::INFO ) << "GL4.6 Core: context created successfully." << std::endl;
                }
            }
        }
    }
    ...
    if( context == NULL )
    {
        OSG_WARN << "GL4.6 Core: Falling back to legacy wglCreateContext. "
                 << "This context may not support required GL 4.6 features." << std::endl;
...
bool GraphicsWindowWin32::makeCurrentImplementation()
{
    ...
    if (glewErr != GLEW_OK)
    {
        OSG_WARN << "GLEW initialization failed: " << glewGetErrorString(glewErr) << std::endl;
    }
