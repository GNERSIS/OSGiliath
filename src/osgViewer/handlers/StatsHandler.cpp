 ...
 * Utility functions for osgViewer: _keyEventTogglesOnScreenStats, Renderer,
...
namespace osgViewer
{
    ...
    static const char* statsVertexShader = {
        "#version 460 core\n"
    ...
    static const char* statsFragmentShader = { "#version 460 core\n"
                                               "in vec4 vertexColor;\n"
                                               "out vec4 fragColor;\n"
                                               "void main(void)\n"
                                               "{\n"
        ...
        _lineHeight( 1.5F )
    {
        ...
        if( shaderHint == osg::DisplaySettings::SHADER_GL46 )
        {
            OSG_INFO << "StatsHandler::StatsHandler() Setting up GL 4.6 shaders"
                     << std::endl;
    ...
    void
    StatsHandler::collectWhichCamerasToRenderStatsFor(
    ...
    bool
    StatsHandler::handle( const osgGA::GUIEventAdapter& ea,
                                ...
                                case( NO_STATS ) :
                                    {
                                        ...
                                        viewer->getViewerStats()->collectStats( "event",
                                                                                false );
                                        viewer->getViewerStats()->collectStats( "update",
                                                                                false );

                                        for( osgViewer::ViewerBase::Cameras::iterator
                                                 itr = cameras.begin();
                                             itr != cameras.end();
                                             ++itr )
                                        {
                                ...
                                case( VIEWER_STATS ) :
                                    {
                                        ...
                                        for( ViewerBase::Scenes::iterator itr =
                                                 scenes.begin();
                                             itr != scenes.end();
                                             ++itr )
                                        {
                    ...
                    if( ea.getKey() == _keyEventPrintsOutStats )
                    {
                        if( viewer && viewer->getViewerStats() )
                        {
                            ...
                            for( unsigned int i =
                                 ...
                                 i < viewer->getViewerStats()->getLatestFrameNumber();
                                 ++i )
                            {
                                     ...
                                     ++itr )
                                {
                                    ...
                                    {
                                        ( *itr )->report( osg::notify( osg::NOTICE ),
                                                          i );
                                    }
                                    else
                                    {
    ...
    void
    StatsHandler::updateThreadingModelText()
    {
        switch( _threadingModel )
        {
            ...
            case( osgViewer::Viewer::DrawThreadPerContext ) :
                _threadingModelText->setText( "ThreadingModel: DrawThreadPerContext" );
                break;
    ...
    void
    StatsHandler::addUserStatsLine( const std::string& label,
                                    ...
                                    float              maxValue )
    {
        _userStatsLines.push_back( UserStatsLine( label,
                                                  textColor,
                                                  barColor,
                                                  timeTakenName,
                                                  multiplier,
    ...
    void
    StatsHandler::removeUserStatsLine( const std::string& label )
    {
        ...
        for( unsigned int i = 0; i < _userStatsLines.size(); ++i )
        {
            if( _userStatsLines[i].label == label )
            {
                ...
                reset();    // Rebuild the stats display without the removed user stats
                            // line.
                break;
            }
        }
    ...
    void
    StatsHandler::setUpHUDCamera( osgViewer::ViewerBase* viewer )
    {
    ...
    void
    StatsHandler::setWindowSize( int width,
                                 int height )
    {
        if( width <= 0 || height <= 0 )
        {
        ...
        if( fabs( static_cast<float>( height ) * _statsWidth ) <=
            fabs( static_cast<float>( width ) * _statsHeight ) )
        {
            _camera->setProjectionMatrix(
                osg::ortho2D( 0.0,
                              static_cast<double>( static_cast<float>( width ) *
                                                   _statsHeight /
                                                   static_cast<float>( height ) ),
                              0.0,
                              static_cast<double>( _statsHeight ) )
            );
        }
        else
        {
            _camera->setProjectionMatrix(
                osg::ortho2D( 0.0,
                              static_cast<double>( _statsWidth ),
                              static_cast<double>( _statsHeight -
                                                   static_cast<float>( height ) *
                                                   _statsWidth /
    ...
    struct AveragedValueTextDrawCallback : public virtual osg::Drawable::DrawCallback
    {
                                ...
                                const osg::Drawable* drawable ) const
            {
                ...
                if( delta > 50 )    // update every 50ms
                {
                    ...
                    double value;
                    if( _stats->getAveragedAttribute( _attributeName,
                                                      value,
    ...
    {
            RawValueTextDrawCallback( osg::Stats*        stats,
                                      const std::string& name,
                                      int                frameDelta,
                                      double             multiplier ) :
                _stats( stats ),
    ...
    {
            CameraSceneStatsTextDrawCallback( osg::Camera* camera,
                                              int          cameraNumber ) :
                _camera( camera ),
                _tickLastUpdated( 0 ),
                                ...
                                const osg::Drawable* drawable ) const
            {
                ...
                if( delta > 100 )    // update every 100ms
                {
                    ...
                    if( stats && renderer )
                    {
                        ...
                        viewStr.width( 14 );
                        // Used fixed formatting, as scientific will switch to "...e+.."
                        // notation for large numbers of vertices/drawables/etc.
...
#define STATS_ATTRIBUTE( str )                           \
    ...
    {
            ViewSceneStatsTextDrawCallback( osgViewer::View* view,
                                            int              viewNumber ) :
                _view( view ),
                _tickLastUpdated( 0 ),
...
#define STATS_ATTRIBUTE_PAIR( str1, str2 )                \
    ...
    if( stats->getAttribute( frameNumber, str2, value ) ) \
        ...
        viewStr << std::setw( 9 ) << "." << std::endl;
                    ...
                    {
                        OSG_WARN << std::endl
                                 << "No valid view to collect scene stats from"
                                 << std::endl;
    ...
    {
            BlockDrawCallback( StatsHandler*      statsHandler,
                               float              xPos,
                                ...
                                const osg::Drawable* drawable ) const
            {
                ...
                if( !_viewerStats->getAttribute( static_cast<unsigned int>( startFrame ),
                                                 "Reference time",
                                                 referenceTime ) )
                {
                    return;
                ...
                {
                    if( _stats->getAttribute( static_cast<unsigned int>( i ),
                                              _beginName,
                                              beginValue ) &&
                        _stats->getAttribute( static_cast<unsigned int>( i ),
                                              _endName,
                                              endValue ) )
                    {
                        ( *vertices )[vi++].x =
                            static_cast<float>( _xPos +
    ...
    osg::Geometry*
    StatsHandler::createBackgroundRectangle( const osg::vec3& pos,
                                             const float      width,
                                             const float      height,
                                             osg::vec4&       color )
    {
    ...
    struct StatsGraph : public osg::MatrixTransform
    {
            ...
            {
                _pos -= osg::vec3( 0, height, 0.1F );
                setMatrix( osg::translate( osg::dvec3( _pos ) ) );
                addChild( _statsGraphGeode.get() );
            }

            void
            addStatGraph( osg::Stats*        viewerStats,
                          osg::Stats*        stats,
            ...
            {
                _statsGraphGeode->addDrawable( new Graph( _pos,
                                                          _width,
                                                          _height,
                                                          viewerStats,
                                                          stats,
        ...
        protected:
            ...
            struct Graph : public osg::Geometry
            {
                           ...
                           const std::string& nameEnd = "" )
                    {
                        ...
                        osg::ref_ptr<osg::BufferObject> vbo =
                            new osg::VertexBufferObject;
                        vbo->setUsage( GL_DYNAMIC_DRAW );
                        vbo->getProfile()._size =
                            static_cast<unsigned int>( width ) * 12;

                        osg::ref_ptr<osg::Vec3Array> vertices = new osg::Vec3Array;
                        vertices->setBufferObject( vbo.get() );
                        vertices->reserve( static_cast<std::size_t>( width ) );
                        ...
                        setDrawCallback( new GraphUpdateCallback( this,
                                                                  pos,
                                                                  width,
                                                                  height,
                                                                  viewerStats,
            ...
            {
                    GraphUpdateCallback( osg::Geometry*     geometry,
                                         const osg::vec3&   pos,
                                         float              width,
                                         float              height,
                                         osg::Stats*        viewerStats,
                        ...
                        _nameEnd( nameEnd )
                    {
                        ...
                        _drawArrays = dynamic_cast<osg::DrawArrays*>(
                                        ...
                                        const osg::Drawable* drawable ) const
                    {
                        ...
                        if( _nameEnd.empty() )
                        {
                            if( !_stats->getAveragedAttribute( _nameBegin,
                                                               value,
                                                               true ) )
                            {
                                value = 0.0;
                        ...
                        {
                            double beginValue, endValue;
                            if( _stats->getAttribute( frameNumber,
                                                      _nameBegin,
                                                      beginValue ) &&
                        ...
                        _vertices->push_back( osg::vec3(
                            float( _curX ),
                            static_cast<float>( float( _height ) / _max * value ),
                            0
                        ) );

                        // One vertex per pixel in X.
                        int excedent = static_cast<int>( _vertices->size() ) -
                                       static_cast<int>( _width );
                        if( excedent > 0 )
                        {
                            _vertices->erase( _vertices->begin(),
                                              _vertices->begin() + excedent );
                        }

                        // Update primitive set.
                        _drawArrays->setFirst( 0 );
                        _drawArrays->setCount(
                            static_cast<GLsizei>( _vertices->size() )
                        );
                        ...
                        if( GraphUpdateCallback::_frameNumber != frameNumber )
                        {
                            ...
                            osg::MatrixTransform* transform =
                                const_cast<osg::MatrixTransform*>(
                            ...
                            if( transform )
                            {
                                // osg::dmat4 matrix = transform->getMatrix();
                                // matrix.setTrans(-(*vertices)[0].x,
                                // matrix.getTrans().y, matrix.getTrans().z);
                                transform->setMatrix( osg::translate( osg::dvec3(
                                    _pos + osg::vec3( -( *_vertices )[0].x, 0, 0 )
                                ) ) );
                            }
                        }
    ...
    osg::Geometry*
    StatsHandler::createGeometry( const osg::vec3& pos,
                                  float            height,
                                  ...
                                  unsigned int     numBlocks )
    {
        ...
        geometry->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        {
            unsigned int vi = static_cast<unsigned int>( vertices->size() );
            vertices->push_back( pos + osg::vec3( static_cast<float>( i * 20 ),
                                                  static_cast<float>( height ),
                                                  0.0F ) );
            vertices->push_back( pos +
                                 osg::vec3( static_cast<float>( i * 20 ), 0.0F, 0.0F ) );
            vertices->push_back(
                pos + osg::vec3( static_cast<float>( i * 20 ) + 10.0F, 0.0F, 0.0F )
            );
            vertices->push_back( pos + osg::vec3( static_cast<float>( i * 20 ) + 10.0F,
                                                  static_cast<float>( height ),
                                                  0.0F ) );

            primitives->push_back( static_cast<unsigned short>( vi ) );
            primitives->push_back( static_cast<unsigned short>( vi + 1 ) );
            primitives->push_back( static_cast<unsigned short>( vi + 2 ) );
            primitives->push_back( static_cast<unsigned short>( vi ) );
            primitives->push_back( static_cast<unsigned short>( vi + 2 ) );
            primitives->push_back( static_cast<unsigned short>( vi + 3 ) );
        }
    ...
    {
            FrameMarkerDrawCallback( StatsHandler* statsHandler,
                                     float         xPos,
                                     osg::Stats*   viewerStats,
                                     int           frameDelta,
                                     int           numFrames ) :
                                ...
                                const osg::Drawable* drawable ) const
            {
                ...
                for( int i = startFrame; i <= endFrame; ++i )
                {
                    if( _viewerStats->getAttribute( static_cast<unsigned int>( i ),
                                                    "Reference time",
                                                    currentReferenceTime ) )
                    {
                        ( *vertices )[vi++].x = static_cast<float>(
                            _xPos +
    ...
    osg::Geometry*
    StatsHandler::createFrameMarkers( const osg::vec3& pos,
                                      float            height,
                                      const osg::vec4& colour,
                                      unsigned int     numBlocks )
    {
        ...
        for( unsigned int i = 0; i < numBlocks; ++i )
        {
            vertices->push_back(
                pos +
                osg::vec3( static_cast<float>( double( i ) * _blockMultiplier * 0.01 ),
                           height,
                           0.0F )
            );
            vertices->push_back(
        ...
        geometry->addPrimitiveSet(
            new osg::DrawArrays( GL_LINES, 0, static_cast<GLsizei>( numBlocks * 2 ) )
        );
    ...
    osg::Geometry*
    StatsHandler::createTick( const osg::vec3& pos,
                              float            height,
                              const osg::vec4& colour,
                              unsigned int     numTicks )
    {
        ...
        {
            float tickHeight = ( i % 10 ) ? height : height * 2.0F;
            vertices->push_back(
                pos +
                osg::vec3( static_cast<float>( double( i ) * _blockMultiplier * 0.001 ),
                           tickHeight,
                           0.0F )
            );
            vertices->push_back(
        ...
        geometry->addPrimitiveSet(
            new osg::DrawArrays( GL_LINES, 0, static_cast<GLsizei>( numTicks * 2 ) )
        );
    ...
    void
    StatsHandler::setUpScene( osgViewer::ViewerBase* viewer )
    {
        ...
        for( ViewerBase::Cameras::iterator citr = cameras.begin(); citr != cameras.end();
             ++citr )
        {
        ...
        // frame rate stats
        {
            ...
            frameRateValue->setDataVariance( osg::Object::DataVariance::DYNAMIC );

            frameRateValue->setDrawCallback(
        ...
        osg::vec4 backgroundColor( 0.0F, 0.0F, 0.0F, 0.3F );
        osg::vec4 staticTextColor( 1.0F, 1.0F, 0.0F, 1.0F );
        osg::vec4 dynamicTextColor( 1.0F, 1.0F, 1.0F, 1.0F );
        ...
        // viewer stats
        {
            ...
            double cameraSize =
                _lineHeight * 3.0 * static_cast<double>( cameras.size() );
            if( !acquireGPUStats )    // reduce size if GPU stats not needed
            {
                cameraSize -= _lineHeight * static_cast<double>( cameras.size() );
            }

            double userStatsLinesSize =
                _lineHeight * static_cast<double>( _userStatsLines.size() );

            _statsGeode->addDrawable( createBackgroundRectangle(
                pos +
                    osg::vec3( -backgroundMargin, _characterSize + backgroundMargin, 0 ),
                _statsWidth - 2 * backgroundMargin,
                static_cast<float>( ( 3 + cameraSize + userStatsLinesSize ) *
                                    _characterSize +
                                    2 *
                                    backgroundMargin ),
                backgroundColor
            ...
            for( unsigned int i = 0; i < _userStatsLines.size(); ++i )
            {
                ...
                createTimeStatsLine( line.label,
                                     pos,
                                     line.textColor,
                                     line.barColor,
            ...
            // add frame ticks
            {
                ...
                osg::Geometry* frameMarkers =
                    createFrameMarkers( pos, height, colourTicks, _numBlocks + 1 );
                frameMarkers->setDrawCallback(
                    new FrameMarkerDrawCallback( this,
                                                 _startBlocks,
                                                 viewer->getViewerStats(),
                                                 0,
                                                 static_cast<int>( _numBlocks + 1 ) )
                );
            ...
            ViewerBase::Scenes scenes;
            viewer->getScenes( scenes );
            for( ViewerBase::Scenes::iterator itr = scenes.begin(); itr != scenes.end();
                 ++itr )
            {
                Scene*                scene = *itr;
                ...
                if( dp && dp->isRunning() )
                {
                    ...
                    _statsGeode->addDrawable( createBackgroundRectangle(
                        pos + osg::vec3( -backgroundMargin,
                                         _characterSize + backgroundMargin,
                                         0 ),
                        _statsWidth - 2 * backgroundMargin,
                    ...
                    averageValue->setDataVariance( osg::Object::DataVariance::DYNAMIC );

                    pos.x =
                        averageValue->getBoundingBox().xMax() + 2.0F * _characterSize;
                    ...
                    minValue->setDataVariance( osg::Object::DataVariance::DYNAMIC );
                    ...
                    maxValue->setDataVariance( osg::Object::DataVariance::DYNAMIC );
                    ...
                    requestList->setDataVariance( osg::Object::DataVariance::DYNAMIC );
                    ...
                    compileList->setDataVariance( osg::Object::DataVariance::DYNAMIC );

                    pos.x = maxLabel->getBoundingBox().xMax();

                    _statsGeode->setCullCallback( new PagerCallback( dp,
                                                                     minValue.get(),
                                                                     maxValue.get(),
                                                                     averageValue.get(),
                                                                     requestList.get(),
        ...
        // Camera scene stats
        {
                 ...
                 ++citr )
            {
                ...
                camStatsText->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    void
    StatsHandler::createTimeStatsLine( const std::string& lineLabel,
                                       osg::vec3          pos,
                                       const osg::vec4&   textColor,
                                       const osg::vec4&   barColor,
                                       osg::Stats*        viewerStats,
                                       ...
                                       const std::string& endTimeName )
    {
        ...
        value->setText( "0.0" );
        value->setDataVariance( osg::Object::DataVariance::DYNAMIC );
        ...
        if( !timeTakenName.empty() )
        {
            if( average )
            {
                value->setDrawCallback(
                    new AveragedValueTextDrawCallback( stats,
                                                       timeTakenName,
                                                       -1,
                                                       averageInInverseSpace,
                                                       multiplier )
            ...
            {
                value->setDrawCallback(
                    new RawValueTextDrawCallback( stats, timeTakenName, -1, multiplier )
                );
            }
        }
        ...
        if( !beginTimeName.empty() && !endTimeName.empty() )
        {
            ...
            osg::Geometry* geometry =
                createGeometry( pos, _characterSize * 0.8F, barColor, _numBlocks );
            geometry->setDrawCallback(
                new BlockDrawCallback( this,
                                       _startBlocks,
                                       viewerStats,
                                       stats,
                                       beginTimeName,
    ...
    void
    StatsHandler::createCameraTimeStats( osg::vec3&   pos,
    ...
    void
    StatsHandler::getUsage( osg::ApplicationUsage& usage ) const
    {
        usage.addKeyboardMouseBinding( _keyEventTogglesOnScreenStats,
                                       "On screen stats." );
        usage.addKeyboardMouseBinding( _keyEventPrintsOutStats,
