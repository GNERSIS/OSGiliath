 ...
 * WindowCaptureCallback, derived from DrawCallback.
 * Provides: getFramePosition, setCaptureOperation, getCaptureOperation,
 * setFramesToCapture, getFramesToCapture, ContextData.
 */
...
namespace osgViewer
{
    ...
    class WindowCaptureCallback : public osg::Camera::DrawCallback
    {
        public:
            ...
            WindowCaptureCallback( int           numFrames,
                                   Mode          mode,
                                   FramePosition position,
                                   GLenum        readBuffer );

            FramePosition
            getFramePosition() const
            {
                return _position;
            }

            void
            setCaptureOperation( ScreenCaptureHandler::CaptureOperation* operation );
            ...
            struct OSGVIEWER_EXPORT ContextData : public osg::Referenced
            {
                    ...
                    void
                    getSize( osg::GraphicsContext* gc,
                             int&                  width,
                             int&                  height );

                    void
                    updateTimings( osg::Timer_t tick_start,
                    ...
                    readPixels();
                    void
                    singlePBO( osg::GLExtensions* ext );
                    void
                    multiPBO( osg::GLExtensions* ext );
            ...
            ContextData*
            createContextData( osg::GraphicsContext* gc ) const;
            ContextData*
                                   getContextData( osg::GraphicsContext* gc ) const;
        ...
        _previousFrameTick( 0 )
    {
        ...
        if( gc->getTraits() )
        {
            ...
            {
                OSG_NOTIFY( level )
                    << "ScreenCaptureHandler: Selected GL_RGBA read back format"
                    << std::endl;
                _pixelFormat = GL_RGBA;
            }
            else
            {
                OSG_NOTIFY( level )
                    << "ScreenCaptureHandler: Selected GL_RGB read back format"
                    << std::endl;
                _pixelFormat = GL_RGB;
            }
        }
        ...
        {
            case( READ_PIXELS ) :
                OSG_NOTIFY( level ) << "ScreenCaptureHandler: Reading window using "
                                       "glReadPixels, without PixelBufferObject."
                                    << std::endl;
                break;
            case( SINGLE_PBO ) :
                OSG_NOTIFY( level ) << "ScreenCaptureHandler: Reading window using "
                                       "glReadPixels, with a single PixelBufferObject."
                                    << std::endl;
                _pboBuffer.push_back( 0 );
                break;
            case( DOUBLE_PBO ) :
                OSG_NOTIFY( level )
                    << "ScreenCaptureHandler: Reading window using glReadPixels, with a "
                       "double buffer PixelBufferObject."
                    << std::endl;
                _pboBuffer.push_back( 0 );
                _pboBuffer.push_back( 0 );
                break;
            case( TRIPLE_PBO ) :
                OSG_NOTIFY( level )
                    << "ScreenCaptureHandler: Reading window using glReadPixels, with a "
                       "triple buffer PixelBufferObject."
                    << std::endl;
                _pboBuffer.push_back( 0 );
                _pboBuffer.push_back( 0 );
                _pboBuffer.push_back( 0 );
    ...
    void
    WindowCaptureCallback::ContextData::getSize( osg::GraphicsContext* gc,
                                                 int&                  width,
                                                 int&                  height )
    {
        if( gc->getTraits() )
    ...
    {
        _timeForReadPixels =
            osg::Timer::instance()->delta_s( tick_start, tick_afterReadPixels );
        _timeForMemCpy =
        ...
        _timeForFullCopy =
            osg::Timer::instance()->delta_s( tick_start, tick_afterMemCpy );
        _timeForFullCopyAndOperation =
            osg::Timer::instance()->delta_s( tick_start, tick_afterCaptureOperation );
    }

    void
    WindowCaptureCallback::ContextData::read()
    {
        osg::GLExtensions* ext =
            osg::GLExtensions::Get( _gc->getState()->getContextID(), true );
    ...
    WindowCaptureCallback::ContextData::readPixels()
    {
        ...
        unsigned int nextPboIndex =
            _pboBuffer.empty() ? 0 : ( _currentPboIndex + 1 ) % _pboBuffer.size();
    ...
    void
    WindowCaptureCallback::ContextData::singlePBO( osg::GLExtensions* ext )
    {
        unsigned int nextImageIndex = ( _currentImageIndex + 1 ) % _imageBuffer.size();
    ...
    void
    WindowCaptureCallback::ContextData::multiPBO( osg::GLExtensions* ext )
    {
        unsigned int nextImageIndex = ( _currentImageIndex + 1 ) % _imageBuffer.size();
        ...
        if( image->s() != _width || image->t() != _height )
        {
            ...
            if( read_pbo != 0 )
            {
                // OSG_NOTICE<<"ScreenCaptureHandler: deleting pbo
                // "<<read_pbo<<std::endl;
    ...
    WindowCaptureCallback::WindowCaptureCallback( int           numFrames,
                                                  Mode          mode,
                                                  FramePosition position,
                                                  GLenum        readBuffer ) :
        _mode( mode ),
    ...
    {
        WindowCaptureCallback::ContextData* cd =
            new WindowCaptureCallback::ContextData( gc, _mode, _readBuffer );
    ...
    void
    WindowCaptureCallback::setCaptureOperation(
        ScreenCaptureHandler::CaptureOperation* operation
    )
    {
        _defaultCaptureOperation = operation;

        // Set the capture operation for each ContextData.
        for( ContextDataMap::iterator it = _contextDataMap.begin();
             it != _contextDataMap.end();
             ++it )
        {
    ...
    void
    WindowCaptureCallback::operator()( osg::RenderInfo& renderInfo ) const
    {
    ...
    void
    ScreenCaptureHandler::WriteToFile::operator()( const osg::Image&  image,
        ...
        if( _savePolicy == SEQUENTIAL_NUMBER )
        {
            if( _contextSaveCounter.size() <= context_id )
            {
                unsigned int oldSize =
                    static_cast<unsigned int>( _contextSaveCounter.size() );
    ...
    void
    ScreenCaptureHandler::setCaptureOperation( CaptureOperation* operation )
    {
    ...
    void
    ScreenCaptureHandler::addCallbackToViewer( osgViewer::ViewerBase& viewer )
    {
        ...
        WindowCaptureCallback* callback =
            static_cast<WindowCaptureCallback*>( _callback.get() );
        if( callback &&
            callback->getFramePosition() == WindowCaptureCallback::START_FRAME )
        {
    ...
    void
    ScreenCaptureHandler::removeCallbackFromViewer( osgViewer::ViewerBase& viewer )
    {
        osg::Camera* camera = findAppropriateCameraForCallback( viewer );
        if( !camera )
        {
    ...
    osg::Camera*
    ScreenCaptureHandler::findAppropriateCameraForCallback(
    ...
    )
    {
        ...
        if( callback->getFramePosition() == WindowCaptureCallback::START_FRAME )
        {
                 ...
                 ++itr )
            {
                ...
                if( firstCamera )
                {
                    // OSG_NOTICE<<"ScreenCaptureHandler: First camera
        ...
        else
        {
                 ...
                 ++itr )
            {
                ...
                if( lastCamera )
                {
                    // OSG_NOTICE<<"ScreenCaptureHandler: Last camera
                    // "<<lastCamera<<std::endl;
    ...
    bool
    ScreenCaptureHandler::handle( const osgGA::GUIEventAdapter& ea,
    ...
    void
    ScreenCaptureHandler::captureNextFrame( osgViewer::ViewerBase& viewer )
    {
        addCallbackToViewer( viewer );
    }
    ...
    void
    ScreenCaptureHandler::setFramesToCapture( int numFrames )
    {
        WindowCaptureCallback* callback =
            static_cast<WindowCaptureCallback*>( _callback.get() );
    ...
    int
    ScreenCaptureHandler::getFramesToCapture() const
    {
        WindowCaptureCallback* callback =
            static_cast<WindowCaptureCallback*>( _callback.get() );
    ...
    void
    ScreenCaptureHandler::getUsage( osg::ApplicationUsage& usage ) const
    {
        usage.addKeyboardMouseBinding( _keyEventTakeScreenShot, "Take screenshot." );
        usage.addKeyboardMouseBinding( _keyEventToggleContinuousCapture,
