 ...
 * Built-in event handlers: StatsHandler (on-screen stats),
 * WindowSizeHandler, ScreenCaptureHandler, and HelpHandler.
 */
...
namespace osgViewer
{
    ...
    */
        ...
        _currentResolutionIndex( -1 )
    {
        ...
        _resolutionList.push_back( osg::vec2( 1'024, 768 ) );
        _resolutionList.push_back( osg::vec2( 1'152, 864 ) );
        _resolutionList.push_back( osg::vec2( 1'280, 720 ) );
        _resolutionList.push_back( osg::vec2( 1'280, 768 ) );
        _resolutionList.push_back( osg::vec2( 1'280, 1'024 ) );
        _resolutionList.push_back( osg::vec2( 1'440, 900 ) );
        _resolutionList.push_back( osg::vec2( 1'400, 1'050 ) );
        _resolutionList.push_back( osg::vec2( 1'600, 900 ) );
        _resolutionList.push_back( osg::vec2( 1'600, 1'024 ) );
        _resolutionList.push_back( osg::vec2( 1'600, 1'200 ) );
        _resolutionList.push_back( osg::vec2( 1'680, 1'050 ) );
        _resolutionList.push_back( osg::vec2( 1'920, 1'080 ) );
        _resolutionList.push_back( osg::vec2( 1'920, 1'200 ) );
        _resolutionList.push_back( osg::vec2( 2'048, 1'536 ) );
        _resolutionList.push_back( osg::vec2( 2'560, 2'048 ) );
        _resolutionList.push_back( osg::vec2( 3'200, 2'400 ) );
        _resolutionList.push_back( osg::vec2( 3'840, 2'400 ) );
    }

    void
    WindowSizeHandler::getUsage( osg::ApplicationUsage& usage ) const
    {
        usage.addKeyboardMouseBinding( _keyEventToggleFullscreen,
                                       "Toggle full screen." );
        usage.addKeyboardMouseBinding(
            _keyEventWindowedResolutionUp,
            "Increase the screen resolution (in windowed mode)."
        );
        usage.addKeyboardMouseBinding(
            _keyEventWindowedResolutionDown,
    ...
    bool
    WindowSizeHandler::handle( const osgGA::GUIEventAdapter& ea,
            ...
            case( osgGA::GUIEventAdapter::KEYUP ) :
                {
                    if( _toggleFullscreen ==
                        true &&
                        ea.getKey() == _keyEventToggleFullscreen )
                    {

                        // sleep to allow any viewer rendering threads to complete before
                    ...
                    else if( _changeWindowedResolution ==
                             true &&
                             ea.getKey() == _keyEventWindowedResolutionUp )
                    {
                        // sleep to allow any viewer rendering threads to complete before
                        // we resize the window
                        osg::Thread::microSleep( 100'000 );
                        ...
                        return true;
                             ...
                             true &&
                             ea.getKey() == _keyEventWindowedResolutionDown )
                    {
                        // sleep to allow any viewer rendering threads to complete before
                        // we resize the window
                        osg::Thread::microSleep( 100'000 );
    ...
    void
    WindowSizeHandler::toggleFullscreen( osgViewer::GraphicsWindow* window )
    {
        ...
        if( wsi == NULL )
        {
            OSG_NOTICE << "Error, no WindowSystemInterface available, cannot toggle "
                          "window fullscreen."
                       << std::endl;
            return;
        }
        ...
        if( isFullScreen )
        {
            ...
            {
                _currentResolutionIndex = static_cast<int>(
                    getNearestResolution( static_cast<int>( screenWidth ),
                                          static_cast<int>( screenHeight ),
                                          static_cast<int>( screenWidth ) / 2,
                                          static_cast<int>( screenHeight ) / 2 )
                );
            }
            resolution =
                _resolutionList[static_cast<std::size_t>( _currentResolutionIndex )];
            window->setWindowDecoration( true );
            window->setWindowRectangle(
                ( static_cast<int>( screenWidth ) - ( int )resolution.x ) / 2,
                ( static_cast<int>( screenHeight ) - ( int )resolution.y ) / 2,
                ( int )resolution.x,
                ( int )resolution.y
            );
            OSG_INFO << "Screen resolution = " << ( int )resolution.x << "x"
                     << ( int )resolution.y << std::endl;
        }
        else
        {
            window->setWindowDecoration( false );
            window->setWindowRectangle( 0,
                                        0,
    ...
    void
    WindowSizeHandler::changeWindowedResolution( osgViewer::GraphicsWindow* window,
                                                 bool                       increase )
    {
        osg::GraphicsContext::WindowingSystemInterface* wsi =
            osg::GraphicsContext::getWindowingSystemInterface();
        ...
        if( window->getWindowDecoration() == true || isFullScreen == false )
        {
            ...
            if( increase == true )
            {
                ...
                for( int i = _currentResolutionIndex + 1;
                     i < ( int )_resolutionList.size();
                     ++i )
                {
                    if( static_cast<unsigned int>(
                            _resolutionList[static_cast<std::size_t>( i )].x
                        ) <=
                        screenWidth &&
                        static_cast<unsigned int>(
                            _resolutionList[static_cast<std::size_t>( i )].y
                        ) <= screenHeight )
                    {
    ...
    unsigned int
    WindowSizeHandler::getNearestResolution( int screenWidth,
                                             int screenHeight,
                                             int width,
                                             int height ) const
    {
             ...
             ++it, ++position )
        {
            if( ( int )it->x <= screenWidth && ( int )it->y <= screenHeight )
            {
                int tmp = static_cast<int>(
                    std::abs( static_cast<float>( width * height ) - ( it->x * it->y ) )
                );
    ...
    */
    ...
    void
    ThreadingHandler::getUsage( osg::ApplicationUsage& usage ) const
    {
        usage.addKeyboardMouseBinding( _keyEventChangeThreadingModel,
                                       "Toggle threading model." );
        usage.addKeyboardMouseBinding(
            _keyEventChangeEndBarrierPosition,
            "Toggle the placement of the end of frame barrier."
        );
    }

    bool
    ThreadingHandler::handle( const osgGA::GUIEventAdapter& ea,
                              osgGA::GUIActionAdapter&      aa )
    {
        osgViewer::View* view = dynamic_cast<osgViewer::View*>( &aa );
        if( !view )
        ...
        switch( ea.getEventType() )
        {
            case( osgGA::GUIEventAdapter::KEYUP ) :
                {
                    double delta = osg::Timer::instance()->delta_s(
                        _tickOrLastKeyPress,
                        osg::Timer::instance()->tick()
                    );

                    if( _changeThreadingModel ==
                        true &&
                        ea.getKey() ==
                        ...
                        delta > 1.0 )
                    {
                        ...
                        switch( viewerBase->getThreadingModel() )
                        {
                            case( osgViewer::ViewerBase::SingleThreaded ) :
                                viewerBase->setThreadingModel(
                                    osgViewer::ViewerBase::CullDrawThreadPerContext
                                );
                                OSG_NOTICE << "Threading model "
                                              "'CullDrawThreadPerContext' selected."
                                           << std::endl;
                                break;
                            ...
                            case( osgViewer::ViewerBase::AutomaticSelection ) :
                                viewerBase->setThreadingModel(
                                    viewer->suggestBestThreadingModel()
                                );
                                OSG_NOTICE
                                    << "Threading model 'AutomaticSelection' selected."
                                    << std::endl;
#endif
                                break;
                        }
                    ...
                    if( viewer &&
                        ...
                        true &&
                        ea.getKey() == _keyEventChangeEndBarrierPosition )
                    {
                        switch( viewer->getEndBarrierPosition() )
                        {
                            case( osgViewer::Viewer::BeforeSwapBuffers ) :
                                viewer->setEndBarrierPosition(
                                    osgViewer::Viewer::AfterSwapBuffers
                                );
                                OSG_NOTICE << "Threading end of frame barrier position "
                                              "'AfterSwapBuffers' selected."
                                           << std::endl;
                                break;
    ...
    {
        usage.addKeyboardMouseBinding( _keyEventToggleRecord,
                                       "Toggle camera path recording." );
        usage.addKeyboardMouseBinding( _keyEventTogglePlayback,
                                       "Toggle camera path playback." );
    }

    bool
    RecordCameraPathHandler::handle( const osgGA::GUIEventAdapter& ea,
                                     osgGA::GUIActionAdapter&      aa )
    {
        osgViewer::View* view = dynamic_cast<osgViewer::View*>( &aa );
        ...
        {
            // Calculate our current delta (difference) in time between the last frame
            // and current frame, regardless of whether we actually store a
            // ControlPoint...
            ...
            // insertion, do so now. Be sure and reset the internal _delta, so we can
            // start calculating when the next insert should happen.
            if( _animPath.valid() && _currentlyRecording && _delta >= _interval )
            {
                const osg::dmat4& m = view->getCamera()->getInverseViewMatrix();
                double            animationPathTime =
                    osg::Timer::instance()->delta_s( _animStartTime, time );
                _animPath->insert(
                    animationPathTime,
                    osg::AnimationPath::ControlPoint( osg::getTrans( m ),
                                                      osg::quat( osg::getRotate( m ) ) )
                );
                _delta = 0.0F;
                ...
                {
                    _animPath->write(
                        _animPath->getTimeControlPointMap().find( animationPathTime ),
                        _fout
                    );
                    _fout.flush();
                }
            ...
            case( osgGA::GUIEventAdapter::KEYUP ) :
                {
                    ...
                    if( ea.getKey() == _keyEventToggleRecord )
                    {
                        ...
                        if( !_currentlyRecording )
                        {
                            ...
                            if( !_filename.empty() )
                            {
                                ...
                                OSG_NOTICE << "Recording camera path to file "
                                           << ss.str() << std::endl;
                                _fout.open( ss.str().c_str() );

                                // make sure doubles are not truncated by default stream
                                // precision = 6
                                _fout.precision( 15 );
                            }
                            else
                    ...
                    // code below that we take over the current manipulator; it was
                    // originally recommended that we check for a KeySwitchManipulator,
                    // create one if not present, and then add this to either the newly
                    // created one or the existing one. However, the code do that was
                    // EXTREMELY dirty, so I opted for a simpler solution. At a later
                    // date, someone may want to implement the original recommendation
                    // (which is in a mailing list reply from June 1st by Robert in a
                    // thread called "osgviewer Camera Animation (preliminary)".
                    else if( ea.getKey() == _keyEventTogglePlayback )
                    {
                        if( _currentlyRecording )
                        {
                            ...
                            {
                                // In the future this will need to be written
                                // continuously, rather than all at once.
                                osgDB::ofstream out( _filename.c_str() );
                                OSG_NOTICE << "Writing camera file: " << _filename
                                           << std::endl;
                                _animPath->write( out );
                                out.close();
                            }
                            ...
                            {
                                OSG_NOTICE << "No animation path to write out."
                                           << std::endl;
                            }
                        }
                        ...
                        if( !_currentlyPlaying )
                        {
                            if( _animPath.valid() && !_animPath->empty() )
                            {
                                ...
                                // If we successfully found our _filename file, set it
                                // and keep a copy around of the original
                                // CameraManipulator to restore later.
    ...
    bool
    LODScaleHandler::handle( const osgGA::GUIEventAdapter& ea,
                             osgGA::GUIActionAdapter&      aa )
    {
        osgViewer::View* view   = dynamic_cast<osgViewer::View*>( &aa );
            ...
            case( osgGA::GUIEventAdapter::KEYUP ) :
                {
                    ...
                    {
                        camera->setLODScale( camera->getLODScale() * 1.1F );
                        OSG_NOTICE << "LODScale = " << camera->getLODScale()
                                   << std::endl;
    ...
    void
    LODScaleHandler::getUsage( osg::ApplicationUsage& usage ) const
    {
    ...
    bool
    ToggleSyncToVBlankHandler::handle( const osgGA::GUIEventAdapter& ea,
                                       osgGA::GUIActionAdapter&      aa )
    {
        osgViewer::View* view = dynamic_cast<osgViewer::View*>( &aa );
        if( !view )
    ...
    void
    ToggleSyncToVBlankHandler::getUsage( osg::ApplicationUsage& usage ) const
    {
        usage.addKeyboardMouseBinding( _keyEventToggleSyncToVBlank,
                                       "Toggle SyncToVBlank." );
    }
                                            ...
                                            int&                          y ) const
    {
        ...
        if( foundIntersection )
        {
            ...
            osg::Vec3Array* vertices =
                geometry ? dynamic_cast<osg::Vec3Array*>( geometry->getVertexArray() )
                         : 0;
            if( vertices )
            {
                ...
                if( indices.size() == 3 && ratios.size() == 3 )
                {
                    ...
                    float           r1        = static_cast<float>( ratios[0] );
                    float           r2        = static_cast<float>( ratios[1] );
                    float           r3        = static_cast<float>( ratios[2] );
                ...
                if( drawable->getStateSet() )
                {
                    osg::Texture* texture = dynamic_cast<osg::Texture*>(
                        drawable->getStateSet()->getTextureAttribute(
                            0,
    ...
    bool
    InteractiveImageHandler::handle( const osgGA::GUIEventAdapter& ea,
                                     osgGA::GUIActionAdapter&      aa,
                                     osg::Object*,
                                     osg::NodeVisitor* nv )
    {
    ...
    bool
    InteractiveImageHandler::cull( osg::NodeVisitor* nv,
                                   osg::Drawable*,
    ...
    void
    InteractiveImageHandler::resize( int width,
                                     int height )
    {
        if( _image.valid() )
        {
