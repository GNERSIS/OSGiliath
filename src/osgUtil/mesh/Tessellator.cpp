 ...
 * Polygon tessellation utility using GLU-style algorithms.
 * Converts concave polygons and polygons with holes into triangles.
 */
    ...
    _extraPrimitives( 0 )
{
    ...
    if( _tobj )
    {
        gluTessCallback( _tobj,
                         ...
                         ( GLU_TESS_CALLBACK )vertexCallback );
        gluTessCallback( _tobj,
                         GLU_TESS_BEGIN_DATA,
                         ( GLU_TESS_CALLBACK )beginCallback );
        gluTessCallback( _tobj, GLU_TESS_END_DATA, ( GLU_TESS_CALLBACK )endCallback );
        gluTessCallback( _tobj,
                         GLU_TESS_COMBINE_DATA,
                         ( GLU_TESS_CALLBACK )combineCallback );
        gluTessCallback( _tobj,
                         GLU_TESS_ERROR_DATA,
                         ( GLU_TESS_CALLBACK )errorCallback );
    }
    _errorCode = 0;
    _index     = 0;
}
...
void
Tessellator::addVertex( osg::vec3* vertex )
{
    if( _tobj )
    {
        if( vertex &&
        ...
        else
        {
            if( vertex )
            {
                OSG_INFO << "Tessellator::addVertex(" << *vertex
                         << ") detected NaN, ignoring vertex." << std::endl;
            }
            else
            {
                OSG_INFO << "Tessellator::addVertex(NULL) detected Nullpointer, "
                            "ignoring vertex."
                         << std::endl;
            }
        }
...
Tessellator::reset()
{
    ...
    for( NewVertexList::iterator j = _newVertexList.begin(); j != _newVertexList.end();
         ++j )
    {
...
#pragma clang diagnostic pop
        ...
        virtual void
        apply( osg::ByteArray& ba )
        {
            apply_imp( ba, GLbyte( 0 ) );
        }

        virtual void
        apply( osg::ShortArray& ba )
        {
            apply_imp( ba, GLshort( 0 ) );
        }

        virtual void
        apply( osg::IntArray& ba )
        {
            apply_imp( ba, GLint( 0 ) );
        }

        virtual void
        apply( osg::UByteArray& ba )
        {
            apply_imp( ba, GLubyte( 0 ) );
        }

        virtual void
        apply( osg::UShortArray& ba )
        {
            apply_imp( ba, GLushort( 0 ) );
        }

        virtual void
        apply( osg::UIntArray& ba )
        {
            apply_imp( ba, GLuint( 0 ) );
        }

        virtual void
        apply( osg::Vec4ubArray& ba )
        {
            apply_imp( ba, ubvec4() );
        }

        virtual void
        apply( osg::FloatArray& ba )
        {
            apply_imp( ba, float( 0 ) );
        }

        virtual void
        apply( osg::Vec2Array& ba )
        {
            apply_imp( ba, vec2() );
        }

        virtual void
        apply( osg::Vec3Array& ba )
        {
            apply_imp( ba, vec3() );
        }

        virtual void
        apply( osg::Vec4Array& ba )
        {
            apply_imp( ba, vec4() );
        }
};
...
void
Tessellator::retessellatePolygons( osg::Geometry& geom )
{
    // turn the contour list into primitives, a little like Tessellator does but more
    // generally
    ...
    // the reset is needed by the flt loader which reuses a Tessellator for triangulating
    // polygons. as such it might be reset by other loaders/developers in future.
    ...
    // the main difference from osgUtil::Tessellator for Geometry sets of multiple
    // contours is that the begin/end tessellation occurs around the whole set of
    // contours.
    ...
    int noContours       = static_cast<int>( _Contours.size() );
    int currentPrimitive = 0;
    for( int primNo = 0; primNo < noContours; ++primNo )
    {
        osg::ref_ptr<osg::PrimitiveSet> primitive =
            _Contours[static_cast<std::size_t>( primNo )].get();
        if( _ttype == TESS_TYPE_POLYGONS || _ttype == TESS_TYPE_DRAWABLE )
        {    // this recovers the 'old' tessellation which just retessellates single
             // polygons.
            if( primitive->getMode() ==
                ...
                _ttype == TESS_TYPE_DRAWABLE )
            {
                    ...
                    osg::PrimitiveSet::DrawArrayLengthsPrimitiveType )
                {
                    ...
                    {
                        beginTessellation();
                        unsigned int last = first + static_cast<unsigned int>( *itr );
                        addContour( primitive->getMode(), first, last, vertices );
                        first = last;
                        endTessellation();
                        collectTessellation(
                            geom,
                            static_cast<unsigned int>( currentPrimitive )
                        );
                        currentPrimitive++;
                    }
                }
                else
                {
                    ...
                    {    // April 2005 gwm only retessellate "complex" polygons
                        beginTessellation();
                    ...
                    {    // April 2005 gwm triangles don't need to be retessellated
...
void
Tessellator::addContour( GLenum          mode,
                         unsigned int    first,
                         unsigned int    last,
        ...
        case osg::PrimitiveSet::TRIANGLE_STRIP :
            {
                ...
                for( i = ( ( last - first ) % 2 ) ? ( last - 2 ) : ( last - 1 );
                     i > first && i < last;
                     i -= 2 )
                {
                    addVertex( &( ( *vertices )[i] ) );
...
void
Tessellator::addContour( osg::PrimitiveSet* primitive,
                         osg::Vec3Array*    vertices )
{
    ...
    switch( primitive->getType() )
    {
        case( osg::PrimitiveSet::DrawArraysPrimitiveType ) :
            {
                ...
                unsigned int first = static_cast<unsigned int>( drawArray->getFirst() );
                unsigned int last =
                    first + static_cast<unsigned int>( drawArray->getCount() );
                addContour( primitive->getMode(), first, last, vertices );
                break;
            }
        ...
        default :
            OSG_NOTICE
                << "Tessellator::addContour(primitive, vertices) : Primitive type "
                << primitive->getType() << " not handled" << std::endl;
            break;
    }
}

void
Tessellator::handleNewVertices( osg::Geometry&       geom,
                                VertexPtrToIndexMap& vertexPtrToIndexMap )
{
    if( !_newVertexList.empty() )
    {
             ...
             ++itr )
        {
            ...
            // assign vertex.
            vertexPtrToIndexMap[vertex] = static_cast<unsigned int>( vertices->size() );
            ...
            if( !arrays.empty() )
            {
                InsertNewVertices inv( newVertex._f1,
                                       vertexPtrToIndexMap[newVertex._v1],
                                       newVertex._f2,
                                       vertexPtrToIndexMap[newVertex._v2],
                                       newVertex._f3,
                                       vertexPtrToIndexMap[newVertex._v3],
                                       newVertex._f4,
                                       vertexPtrToIndexMap[newVertex._v4] );

                // assign the rest of the attributes.
                for( ArrayList::iterator aItr = arrays.begin(); aItr != arrays.end();
                     ++aItr )
                {
                    ( *aItr )->accept( inv );
                }
...
void
Tessellator::begin( GLenum mode )
{
    _primList.push_back( new Prim( mode ) );
}

void
Tessellator::vertex( osg::vec3* vertex )
{
    if( !_primList.empty() )
    {
...
void
Tessellator::combine( osg::vec3* vertex,
                      void*      vertex_data[4],
                      GLfloat    weight[4] )
{
...
void
Tessellator::error( GLenum errorCode )
{
    _errorCode = errorCode;
}

void CALLBACK
Tessellator::beginCallback( GLenum which,
                            void*  userData )
{
...
void CALLBACK
Tessellator::endCallback( void* userData )
{
    ( ( Tessellator* )userData )->end();
}

void CALLBACK
Tessellator::vertexCallback( GLvoid* data,
                             void*   userData )
{
...
void CALLBACK
Tessellator::combineCallback( GLdouble coords[3],
                              void*    vertex_data[4],
                              GLfloat  weight[4],
                              void**   outData,
                              void*    userData )
{
    vec3* newData = new osg::vec3( static_cast<float>( coords[0] ),
...
void CALLBACK
Tessellator::errorCallback( GLenum errorCode,
                            void*  userData )
{
...
void
Tessellator::reduceArray( osg::Array*        cold,
    ...
    if( cold && cold->getNumElements() > nnu )
    {
        ...
        switch( cold->getType() )
        {
            ...
            case osg::Array::Vec4ArrayType :
                ...
                break;
            default :    // should also handle:ArrayType' ByteArrayType' ShortArrayType'
                         // IntArrayType'
...
void
Tessellator::collectTessellation( osg::Geometry& geom,
                                  unsigned int /*originalIndex*/ )
{
    ...
    // will need to address this soon. Robert Oct 2002.
    {
        ...
        {
            normals = dynamic_cast<osg::Vec3Array*>(
                geom.getNormalArray()
            );    // GWM Sep 2002
        }
        ...
        {
            Array* colours =
                geom.getColorArray();    // GWM Dec 2003 - need to duplicate face colours
        ...
        for( PrimList::iterator primItr = _primList.begin(); primItr != _primList.end();
             ++primItr, ++_index )
        {
            ...
            if( vertexPtrToIndexMap.size() <= 255 )
            {
                osg::DrawElementsUByte* elements =
                    new osg::DrawElementsUByte( prim->_mode );
                for( Prim::VecList::iterator vitr = prim->_vertices.begin();
                     vitr != prim->_vertices.end();
                     ++vitr )
                {
                    elements->push_back(
                        static_cast<unsigned char>( vertexPtrToIndexMap[*vitr] )
                    );
                }
            ...
            else if( vertexPtrToIndexMap.size() >
                     255 &&
                     vertexPtrToIndexMap.size() <= 65'535 )
            {
                osg::DrawElementsUShort* elements =
                    new osg::DrawElementsUShort( prim->_mode );
                for( Prim::VecList::iterator vitr = prim->_vertices.begin();
                     vitr != prim->_vertices.end();
                     ++vitr )
                {
                    elements->push_back(
                        static_cast<unsigned short>( vertexPtrToIndexMap[*vitr] )
                    );
                }
            ...
            else
            {
                osg::DrawElementsUInt* elements =
                    new osg::DrawElementsUInt( prim->_mode );
                for( Prim::VecList::iterator vitr = prim->_vertices.begin();
                     vitr != prim->_vertices.end();
                     ++vitr )
                {
            ...
            {    // first primitive so collect primitive normal & colour.
                if( normals )
                {
                    norm = ( *normals )[static_cast<std::size_t>(
                        iprim
                    )];    // GWM Sep 2002 the flat shaded normal
                }
                if( cols4 )
                {
                    primCol4 = ( *cols4 )[static_cast<std::size_t>(
                        iprim
                    )];    // GWM Dec 2003 the flat shaded rgba colour
                    if( _index >= cols4->size() )
                    {
                        cols4->push_back(
                            primCol4
                        );    // GWM Dec 2003 add flat shaded colour for new facet
                    }
                }
                if( cols3 )
                {
                    primCol3 = ( *cols3 )[static_cast<std::size_t>(
                        iprim
                    )];    // GWM Dec 2003 flat shaded rgb colour
            ...
            {    // later primitives use same colour
                if( normals )
                {
                    normals->push_back(
                        norm
                    );    // GWM Sep 2002 add flat shaded normal for new facet
                }
                    ...
                    {    // need one per triangle? Not one per set.
                        for( int ii = 1; ii < ntris; ii++ )
                        {
                            if( normals )
                            {
                                normals->push_back(
                                    norm
                                );    // GWM Sep 2002 add flat shaded normal for new
                                      // facet
                            }
                        }
                    }
