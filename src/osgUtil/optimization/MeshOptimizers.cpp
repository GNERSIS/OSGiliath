 ...
 * Mesh optimization visitors: vertex cache optimization, vertex
 * prefetch optimization, and vertex access order improvement.
 */
...
#include <osg/geometry/TriangleLinePointIndexFunctor.hpp>
...
namespace osgUtil
{
    ...
    void
    GeometryCollector::reset()
    {
        _geometryList.clear();
    }

    void
    GeometryCollector::apply( Geometry& geom )
    {
    ...
    namespace
    {
        ...
        struct GeometryArrayGatherer
        {
                ...
                void
                add( osg::Array*         array,
                     osg::Array::Binding overrideBinding = osg::Array::BIND_UNDEFINED )
                {
                    if( !array )
                    {
                        return;
                    ...
                    if( overrideBinding !=
                        ...
                        array->getBinding() != overrideBinding )
                    {
        ...
        struct VertexAttribComparitor : public GeometryArrayGatherer
        {
            ...
            protected:
                ...
                operator=( const VertexAttribComparitor& )
                {
                    return *this;
                }
        };
        ...
        class RemapArray : public osg::ArrayVisitor
        {
            public:
                ...
                RemapArray( const IndexList& remapping ) :
                ...
                remap( T& array )
                {
                    ...
                    array.erase( array.begin() +
                                     static_cast<typename T::difference_type>(
                                         _remapping.size()
                                     ),
                                 array.end() );
                }
                ...
                virtual void
                apply( osg::Vec2bArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec3bArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec4bArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec2sArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec3sArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec4sArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec2dArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec3dArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::Vec4dArray& array )
                {
                    remap( array );
                }

                virtual void
                apply( osg::MatrixfArray& array )
                {
                    remap( array );
                }
    ...
    void
    IndexMeshVisitor::makeMesh( Geometry& geom )
    {
        ...
        if( osg::getBinding( geom.getSecondaryColorArray() ) ==
            osg::Array::BIND_PER_PRIMITIVE_SET )
        {
            return;
        }

        if( osg::getBinding( geom.getFogCoordArray() ) ==
            osg::Array::BIND_PER_PRIMITIVE_SET )
        {
            return;
        }
             ...
             ++itr )
        {
            ...
            if( !( type ==
                   PrimitiveSet::DrawElementsUBytePrimitiveType ||
                   type ==
                   PrimitiveSet::DrawElementsUShortPrimitiveType ||
        ...
        if( !numSurfacePrimitives ||
            ( !_generateNewIndicesOnAllGeometries && !numNonIndexedPrimitives ) )
        {
            return;
        }

        // duplicate shared arrays as it isn't safe to rearrange vertices when arrays are
        // shared.
        ...
        for( i = 1; i < numVertices; ++i )
        {
            if( arrayComparitor.compare( indices[lastUnique], indices[i] ) != 0 )
            {
                ...
                {
                    min_index = std::min( min_index, indices[j] );
                }
        ...
        if( taf._in_indices.size() < 65'536 )
        {
        ...
        else
        {
            osg::DrawElementsUInt* elements =
                new DrawElementsUInt( GL_TRIANGLES,
                                      taf._in_indices.begin(),
                                      taf._in_indices.end() );
            new_primitives.push_back( elements );
        }
    ...
    namespace
    {
        ...
        struct LRUCache
        {
                            ...
                            unsigned* end )
                {
                    ...
                    size_t newEnts     = static_cast<size_t>( end - begin );
                    int    spaceNeeded = static_cast<int>( newEnts ) -
                                         static_cast<int>( entries.rend() - validEnd );
                    if( spaceNeeded > 0 )
                    {
                        if( maxSize -
                            entries.size() >= static_cast<size_t>( spaceNeeded ) )
                        {
                            entries.resize( entries.size() +
                                            static_cast<size_t>( spaceNeeded ) );
                        }
                        else if( entries.size() < maxSize )
                        {
                            entries.resize( maxSize );
                        }
                        std::copy_backward( entries.begin() +
                                                static_cast<ptrdiff_t>( newEnts ) -
                                                spaceNeeded,
                                            entries.end() -
                                                static_cast<ptrdiff_t>( spaceNeeded ),
                                            entries.end() );
                    }
        ...
        findVertexScore( Vertex& vert )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    const float scaler = 1.0F / static_cast<float>( maxCacheSize - 3 );
            ...
            float valenceBoost =
                powf( static_cast<float>( vert.numActiveTris ), -valenceBoostPower );
        ...
        inline float
        findTriangleScore( Triangle&         tri,
        ...
        TriangleScore
        computeTriScores( Vertex&                vert,
                          const VertexList&      vertices,
                          TriangleList&          triangles,
                          std::vector<unsigned>& triStore )
        {
            float    bestScore = 0.0;
            unsigned bestTri   = 0;
            for( size_t i = vert.triList;
                 i < vert.triList + static_cast<size_t>( vert.numActiveTris );
                 ++i )
            {
                unsigned tri   = triStore[i];
                float    score = triangles[tri].score =
                    findTriangleScore( triangles[tri], vertices );
                if( score > bestScore )
                {
                    bestScore = score;
                    bestTri   = tri;
        ...
        struct TriangleAddOperator
        {
                ...
                doVertex( unsigned p )
                {
                    ( *vertexTris )[( *vertices )[p].triList +
                                    static_cast<size_t>( ( *vertices )[p]
                                                             .numActiveTris++ )] =
                        static_cast<unsigned int>( triIdx );
                }
                            ...
                            unsigned int p3 )
                {
                    ...
                    doVertex( p2 );
                    doVertex( p3 );
                    ( *triangles )[static_cast<std::size_t>( triIdx )].verts[0] = p1;
                    ( *triangles )[static_cast<std::size_t>( triIdx )].verts[1] = p2;
                    ( *triangles )[static_cast<std::size_t>( triIdx )].verts[2] = p3;
                    triIdx++;
                }
        };
        ...
        {
                TriangleAdder( VertexList*            vertices_,
                               std::vector<unsigned>* vertexTris_,
    ...
    void
    VertexCacheVisitor::optimizeVertices( Geometry& geom )
    {
    ...
    missv.reset();
        ...
        {
            osg::DrawElementsUInt* elements = new DrawElementsUInt( GL_TRIANGLES,
                                                                    newVertList.begin(),
                                                                    newVertList.end() );
            if( geom.getUseVertexBufferObjects() )
            {
    ...
    void
    VertexCacheVisitor::doVertexOptimization( Geometry&              geom,
                                              std::vector<unsigned>& vertDrawList )
    {
        ...
        triangles.resize( static_cast<std::size_t>( triCounter.triangleCount ) );
             ...
             ++itr )
        {
            ...
            vertTrisSize += static_cast<std::size_t>( itr->trisUsing );
        }
        ...
        unsigned trisLeft = static_cast<unsigned int>( triangles.size() );
        ...
        while( trisLeft-- > 0 )
        {
            ...
            for( std::vector<unsigned>::const_iterator itr = cache.entries.begin(),
                 ...
                 ++itr )
            {
                TriangleScore tscore = computeTriScores( vertices[*itr],
                                                         vertices,
                                                         triangles,
                                                         vertTriListStore );
            ...
            if( !triToAdd )
            {
                ...
                TriangleList::iterator maxItr = std::max_element( triangles.begin(),
                                                                  triangles.end(),
                                                                  CompareTriangle() );
                triToAdd                      = &( *maxItr );
            }
            ...
            triToAdd->score      = -1.0F;
            unsigned triToAddIdx = static_cast<unsigned int>( triToAdd - &triangles[0] );
                 ...
                 ++itr )
            {
                ...
                vertices[vertIdx].cachePosition =
                    static_cast<int>( itr - cache.entries.begin() );
    ...
    void
    VertexCacheVisitor::optimizeVertices()
    {
        for( GeometryList::iterator itr = _geometryList.begin();
             itr != _geometryList.end();
             ++itr )
    ...
    VertexCacheMissVisitor::VertexCacheMissVisitor( unsigned cacheSize ) :
        osg::DualModeVisitor( DualModeVisitor::TRAVERSE_ALL_CHILDREN ),
        misses( 0 ),
        triangles( 0 ),
    ...
    void
    VertexCacheMissVisitor::apply( Geometry& geom )
    {
        doGeometry( geom );
    }
    ...
    namespace
    {
        ...
        struct FIFOCache
        {
                            ...
                            unsigned* end )
                {
                    ...
                    {
                        entries.resize( std::min( entries.size() + newEnts, maxSize ) );
                    }
                    std::vector<unsigned>::iterator copyEnd =
                        entries.end() - static_cast<std::ptrdiff_t>( newEnts );
        ...
        struct CacheRecordOperator
        {
                            ...
                            unsigned p3 )
                {
                    ...
                    for( int i = 0; i < 3; ++i )
                    {
                        if( std::find( cache->entries.begin(),
                                       cache->entries.end(),
                                       verts[i] ) == cache->entries.end() )
                        {
                            misses++;
                        }
                    }
    ...
    void
    VertexCacheMissVisitor::doGeometry( Geometry& geom )
    {
        Array* vertArray = geom.getVertexArray();
    ...
    namespace
    {
        ...
        class Remapper : public osg::ArrayVisitor
        {
            public:
                ...
                remap( T& array )
                {
                    ref_ptr<T> newarray = new T( static_cast<unsigned int>( _newsize ) );
        ...
        // Record the order in which vertices in a Geometry are used in triangle, line or
        // point primitives.
        ...
        struct VertexReorder
            : public TriangleLinePointIndexFunctor<osgUtil::VertexReorderOperator>
        {
    ...
    void
    VertexAccessOrderVisitor::optimizeOrder()
    {
        for( GeometryList::iterator itr = _geometryList.begin(),
                                    end = _geometryList.end();
             itr != end;
             ++itr )
        {
            optimizeOrder( *( *itr ) );
    ...
    inline void
    reorderDrawElements( DE&                          drawElements,
    ...
    void
    VertexAccessOrderVisitor::optimizeOrder( Geometry& geom )
    {
        Array* vertArray = geom.getVertexArray();
        if( !vertArray || vertArray->getNumElements() == 0 )
        {
             ...
             ++itr )
        {
            ...
            switch( ps->getType() )
            {
                case PrimitiveSet::DrawElementsUBytePrimitiveType :
                    reorderDrawElements( *static_cast<DrawElementsUByte*>( ps ),
                                         vr.remap );
                    break;
                case PrimitiveSet::DrawElementsUShortPrimitiveType :
                    reorderDrawElements( *static_cast<DrawElementsUShort*>( ps ),
                                         vr.remap );
                    break;
                case PrimitiveSet::DrawElementsUIntPrimitiveType :
                    reorderDrawElements( *static_cast<DrawElementsUInt*>( ps ),
                                         vr.remap );
                    break;
                default :
                    break;
    ...
    void
    SharedArrayOptimizer::findDuplicatedUVs( const osg::Geometry& geometry )
    {
        ...
        for( unsigned int id = 0; id != geometry.getNumTexCoordArrays(); ++id )
        {
            ...
            {
                std::map<const osg::Array*, unsigned int>::const_iterator reference =
                    references.find( array );
    ...
    void
    SharedArrayOptimizer::deduplicateUVs( osg::Geometry& geometry )
    {
        for( std::map<unsigned int, unsigned int>::const_iterator it_duplicate =
                 _deduplicateUvs.begin();
             it_duplicate != _deduplicateUvs.end();
             ++it_duplicate )
        {
            osg::Array* original = geometry.getTexCoordArray( it_duplicate->second );
            geometry.setTexCoordArray( it_duplicate->first,
                                       original,
                                       original ? original->getBinding()
                                                : osg::Array::BIND_UNDEFINED );
        }
    }
