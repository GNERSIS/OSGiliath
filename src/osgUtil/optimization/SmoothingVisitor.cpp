...
namespace Smoother
{
    ...
    {
            inline bool
            operator()( const osg::vec3* lhs,
                        const osg::vec3* rhs ) const
            {
                return *lhs < *rhs;
            }
    };
    ...
    struct SmoothTriangleFunctor
    {
            ...
            inline void
            updateNormal( const osg::vec3& normal,
                          const osg::vec3* vptr )
            {
    ...
    smooth_old( osg::Geometry& geom )
    {
        ...
        osg::Vec3Array* normals =
            new osg::Vec3Array( static_cast<unsigned int>( coords->size() ) );
    ...
    struct SmoothTriangleIndexFunctor
    {
                 ...
                 osg::Vec3Array* normals )
            {
                ...
                if( !_vertices )
                {
                    OSG_NOTICE << "Warning: SmoothTriangleIndexFunctor::set(..) "
                                  "requires a valid vertex arrays."
                               << std::endl;
                    return false;
                }
                ...
                {
                    OSG_NOTICE << "Warning: SmoothTriangleIndexFunctor::set(..) "
                                  "requires a valid normal arrays."
                               << std::endl;
                    return false;
                }
    ...
    struct FindSharpEdgesFunctor
    {
            FindSharpEdgesFunctor() :
                ...
                _vertices( 0 ),
                _normals( 0 ),
                _creaseAngle( static_cast<float>( osg::PI_2 ) ),
            ...
            {
                    Triangle( unsigned int primitiveSetIndex,
                              unsigned int p1,
                              unsigned int p2,
                              unsigned int p3 ) :
                        _primitiveSetIndex( primitiveSetIndex ),
                        _p1( p1 ),
                        _p2( p2 ),
                        _p3( p3 )
                    {
                 ...
                 float          creaseAngle )
            {
                ...
                _maxDeviationDotProduct =
                    static_cast<float>( cos( _creaseAngle * 0.5 ) );
                        ...
                        unsigned int p3 )
            {
                ...
                if( p1 == p2 || p2 == p3 || p1 == p3 )
                {
                    // OSG_NOTICE<<"NULL triangle ("<<p1<<", "<<p2<<",
                    // "<<p3<<")"<<std::endl;
                    return;
                }
            ...
            void
            markProblemVertex( unsigned int p )
            {
            ...
            void
            insertTriangleIfProblemVertex( unsigned int p,
                                           Triangle*    tri )
            {
                if( _problemVertexVector[p] )
                {
            ...
            bool
            checkDeviation( unsigned int p,
                            osg::vec3&   normal )
            {
                float deviation = osg::dot( normal, ( *_normals )[p] );
                return ( deviation < _maxDeviationDotProduct );
            }

            osg::vec3
            computeNormal( unsigned int p1,
                           unsigned int p2,
                           unsigned int p3 )
            {
                const osg::vec3& v1 = ( *_vertices )[p1];
            ...
            listProblemVertices()
            {
                OSG_NOTICE << "listProblemVertices() " << _problemVertexList.size()
                           << std::endl;
                     ...
                     ++itr )
                {
                    ...
                    OSG_NOTICE << "  pv._point = " << pv->_point << " triangles "
                               << pv->_triangles.size() << std::endl;
                    for( Triangles::iterator titr = pv->_triangles.begin();
                         titr != pv->_triangles.end();
                         ++titr )
                    {
                        Triangle* tri = titr->get();
                        OSG_NOTICE << "    triangle(" << tri->_p1 << ", " << tri->_p2
                                   << ", " << tri->_p3 << ")" << std::endl;
                        osg::vec3 normal(
                            computeNormal( tri->_p1, tri->_p2, tri->_p3 )
                        );
                        float deviation = osg::dot( normal, ( *_normals )[pv->_point] );
            ...
            class DuplicateVertex : public osg::ArrayVisitor
            {
                public:
                    ...
                    {
                        _end = static_cast<unsigned int>( array.size() );
            ...
            duplicateVertex( unsigned int i )
            {
                ...
                for( ArrayList::iterator aItr = _arrays.begin(); aItr != _arrays.end();
                     ++aItr )
                {
            ...
            void
            duplicateProblemVertexAll( ProblemVertex* pv )
            {
                unsigned int        p    = pv->_point;
                Triangles::iterator titr = pv->_triangles.begin();
                ++titr;
                for( ; titr != pv->_triangles.end(); ++titr )
                {
                    Triangle*    tri          = titr->get();
            ...
            void
            duplicateProblemVertex( ProblemVertex* pv )
            {
                ...
                {
                    // implement a form of greedy association based on similar
                    // orientation rather than iterating through all the various
                    // permutation of triangles that might provide the best fit.
                    unsigned int        p    = pv->_point;
                    Triangles::iterator titr = pv->_triangles.begin();
                    while( titr != pv->_triangles.end() )
                    {
                        ...
                        osg::vec3 normal =
                            computeNormal( pv_tri->_p1, pv_tri->_p2, pv_tri->_p3 );
                        ...
                        while( titr != pv->_triangles.end() )
                        {
                            ...
                            osg::vec3 normal2 =
                                computeNormal( tri2->_p1, tri2->_p2, tri2->_p3 );
                            float deviation = osg::dot( normal, normal2 );
                        ...
                        // now rest the index on the triangles of the point that was
                        // duplicated
            ...
            osg::PrimitiveSet*
            createPrimitiveSet( Triangles& triangles )
            {
                osg::ref_ptr<osg::DrawElements> elements =
                    ( _vertices->size() < 16'384 )
                ...
                elements->reserveElements( static_cast<unsigned int>( triangles.size() *
                                                                      3 ) );

                for( Triangles::iterator itr = triangles.begin(); itr != triangles.end();
                     ++itr )
                {
                    Triangle* tri = itr->get();
            ...
            updateGeometry()
            {
                ...
                for( TriangleMap::iterator itr = triangleMap.begin();
                     ...
                     ++itr )
                {
                    osg::PrimitiveSet* originalPrimitiveSet =
                        _geometry->getPrimitiveSet( itr->first );
                    osg::PrimitiveSet* newPrimitiveSet =
                        createPrimitiveSet( itr->second );
    ...
    static void
    smooth_new( osg::Geometry& geom,
                double         creaseAngle )
    {
        OSG_INFO << "smooth_new(" << &geom << ", " << osg::degrees( creaseAngle ) << ")"
                 << std::endl;
        ...
        {
            normals =
                new osg::Vec3Array( static_cast<unsigned int>( vertices->size() ) );
            geom.setNormalArray( normals, osg::Array::BIND_PER_VERTEX );
        }
        ...
        if( fsef.set( &geom, static_cast<float>( creaseAngle ) ) )
        {
...
void
SmoothingVisitor::smooth( osg::Geometry& geom,
                          double         creaseAngle )
{
...
void
SmoothingVisitor::apply( osg::Geometry& geom )
{
