 ...
 * Scene graph optimization passes: state sharing, geometry merging,
 * redundant node removal, and spatial sorting.
 */
...
#include <osgUtil/utils/Statistics.hpp>
#include <sstream>
#include <stdlib.h>
#include <string.h>
#include <typeinfo>
...
static osg::ApplicationUsageProxy Optimizer_e0(
    osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
    "OSG_OPTIMIZER \"<type> [<type>]\"",
    "OFF | DEFAULT | FLATTEN_STATIC_TRANSFORMS | "
    "FLATTEN_STATIC_TRANSFORMS_DUPLICATING_SHARED_SUBGRAPHS | REMOVE_REDUNDANT_NODES | "
    "COMBINE_ADJACENT_LODS | SHARE_DUPLICATE_STATE | MERGE_GEOMETRY | MERGE_GEODES | "
    "SPATIALIZE_GROUPS  | COPY_SHARED_NODES | OPTIMIZE_TEXTURE_SETTINGS | "
    "REMOVE_LOADED_PROXY_NODES | TESSELLATE_GEOMETRY | CHECK_GEOMETRY |  "
    "FLATTEN_BILLBOARDS | TEXTURE_ATLAS_BUILDER | STATIC_OBJECT_DETECTION | INDEX_MESH "
    "| VERTEX_POSTTRANSFORM | VERTEX_PRETRANSFORM | BUFFER_OBJECT_SETTINGS"
);

void
Optimizer::optimize( osg::Node* node )
{
    ...
    if( env )
    {
        ...
        if( str.find( "~FLATTEN_STATIC_TRANSFORMS_DUPLICATING_SHARED_SUBGRAPHS" ) !=
            std::string::npos )
        {
        ...
        if( str.find( "~BUFFER_OBJECT_SETTINGS" ) != std::string::npos )
        {
            options ^= BUFFER_OBJECT_SETTINGS;
        }
        else if( str.find( "BUFFER_OBJECT_SETTINGS" ) != std::string::npos )
        {
            options |= BUFFER_OBJECT_SETTINGS;
        }
    }
                     ...
                     unsigned int options )
{
    ...
    if( options & OPTIMIZE_TEXTURE_SETTINGS )
    {
        ...
        TextureVisitor tv( true,
                           true,     // unref image
                           false,
                           false,    // client storage
                           false,
                           1.0,      // anisotropic filtering
                           this );
        node->accept( tv );
    }
    ...
    if( options & SHARE_DUPLICATE_STATE )
    {
        ...
        StateVisitor osv( combineDynamicState,
                          combineStaticState,
                          combineUnspecifiedState,
                          this );
        node->accept( osv );
        osv.optimize();
    }
    ...
    if( options & TEXTURE_ATLAS_BUILDER )
    {
        ...
        // now merge duplicate state, that may have been introduced by merge textures
        // into texture atlas'
        bool         combineDynamicState     = false;
        bool         combineStaticState      = true;
        bool         combineUnspecifiedState = true;
    ...
    if( options & MERGE_GEOMETRY )
    {
        ...
        mgv.setTargetMaximumNumberOfVertices( 10'000 );
        node->accept( mgv );

        osg::Timer_t endTick = osg::Timer::instance()->tick();

        OSG_INFO << "MERGE_GEOMETRY took "
                 << osg::Timer::instance()->delta_s( startTick, endTick ) << std::endl;
    }
...
////////////////////////////////////////////////////////////////////////////
void
Optimizer::TessellateVisitor::apply( osg::Geometry& geom )
{
...
{
        bool
        operator()( const osg::StateSet* lhs,
                    const osg::StateSet* rhs ) const
        {
            return ( *lhs < *rhs );
        }
};
...
void
Optimizer::StateVisitor::reset()
{
    _statesets.clear();
}

void
Optimizer::StateVisitor::addStateSet( osg::StateSet* stateset,
                                      osg::Node*     node )
{
...
void
Optimizer::StateVisitor::apply( osg::Node& node )
{

    osg::StateSet* ss = node.getStateSet();
    if( ss && ss->getDataVariance() == osg::Object::DataVariance::STATIC )
    {
...
void
Optimizer::StateVisitor::optimize()
{
...

    {
        ...
        const unsigned int   NON_TEXTURE_ATTRIBUTE = 0XFF'FF'FF'FF;

        UniformToStateSetMap uniformToStateSetMap;

        // NOTE - TODO will need to track state attribute override value too.

        for( StateSetMap::iterator sitr = _statesets.begin(); sitr != _statesets.end();
             ++sitr )
        {
            const osg::StateSet::AttributeList& attributes =
                sitr->first->getAttributeList();
                 ...
                 ++aitr )
            {
                ...
                {
                    attributeToStateSetMap[aitr->second.first.get()].insert(
                        StateSetUnitPair( sitr->first, NON_TEXTURE_ATTRIBUTE )
                    );
                }
            }

            const osg::StateSet::TextureAttributeList& texAttributes =
                sitr->first->getTextureAttributeList();
            for( unsigned int unit = 0; unit < texAttributes.size(); ++unit )
            {
                const osg::StateSet::AttributeList& tex_attributes = texAttributes[unit];
                for( osg::StateSet::AttributeList::const_iterator aitr =
                         tex_attributes.begin();
                     ...
                     ++aitr )
                {
                    ...
                    {
                        attributeToStateSetMap[aitr->second.first.get()].insert(
                            StateSetUnitPair( sitr->first, unit )
                        );
                    }
                }
            }
        ...
        if( attributeToStateSetMap.size() >= 2 )
        {
            ...
            // other.
            std::sort( attributeList.begin(),
                       attributeList.end(),
                       LessDerefFunctor<osg::StateAttribute>() );
            ...
            for( AttributeList::iterator aaitr = attributeList.begin();
                 ...
                 ++aaitr )
            {
                OSG_INFO << "    " << *aaitr << "  " << ( *aaitr )->className()
                         << std::endl;
            }
            ...
            for( ; current != attributeList.end(); ++current )
            {
                ...
                {
                    OSG_INFO << "    found duplicate " << ( *current )->className()
                             << "  first=" << *first_unique << "  current=" << *current
                             << std::endl;
                    ...
                    for( StateSetList::iterator sitr = statesetlist.begin();
                         ...
                         ++sitr )
                    {
                        OSG_INFO << "       replace duplicate " << *current << " with "
                                 << *first_unique << std::endl;
        ...
        if( uniformToStateSetMap.size() >= 2 )
        {
            ...
            // other.
            std::sort( uniformList.begin(),
                       uniformList.end(),
                       LessDerefFunctor<osg::UniformBase>() );
            ...
            for( ; current_uniform != uniformList.end(); ++current_uniform )
            {
                ...
                {
                    OSG_INFO << "    found duplicate uniform "
                             << ( *current_uniform )->getName()
                             << "  first_unique_uniform=" << *first_unique_uniform
                             << "  current_uniform=" << *current_uniform << std::endl;
                    ...
                    for( StateSetSet::iterator sitr = statesetset.begin();
                         ...
                         ++sitr )
                    {
                        OSG_INFO << "       replace duplicate " << *current_uniform
                                 << " with " << *first_unique_uniform << std::endl;
    ...
    if( _statesets.size() >= 2 )
    {
        ...
        for( StateSetMap::iterator ssitr = _statesets.begin(); ssitr != _statesets.end();
             ++ssitr )
        {
        ...
        // other.
        std::sort( statesetSortList.begin(),
                   statesetSortList.end(),
                   LessDerefFunctor<osg::StateSet>() );

        OSG_INFO << "searching for duplicate attributes" << std::endl;
        // find the duplicates.
        StateSetSortList::iterator first_unique = statesetSortList.begin();
        StateSetSortList::iterator current      = first_unique;
        ++current;
        for( ; current != statesetSortList.end(); ++current )
        {
            if( **current == **first_unique )
            {
                ...
                for( NodeSet::iterator sitr = nodeSet.begin(); sitr != nodeSet.end();
                     ++sitr )
                {
                    OSG_INFO << "       replace duplicate " << *current << " with "
                             << *first_unique << std::endl;
...
class CollectLowestTransformsVisitor : public BaseOptimizerVisitor
{
    public:
        ...
        virtual void
        apply( osg::Billboard& geode )
        {
            traverse( geode );
        }

        void
        collectDataFor( osg::Node* node )
        {
        ...
        void
        collectDataFor( osg::Billboard* billboard )
        {
        ...
        setUpMaps();
        void
        disableTransform( osg::Transform* transform );
        bool
        removeTransforms( osg::Node* nodeWeCannotRemove );

        inline bool
        isOperationPermissibleForObject( const osg::Object* object ) const
        {
        ...
        inline bool
        isOperationPermissibleForObject( const osg::Drawable* drawable ) const
        {
            // disable if cannot apply transform functor.
            if( !drawable->supports( _transformFunctor ) )
            {
                return false;
            }
        ...
        inline bool
        isOperationPermissibleForObject( const osg::Node* node ) const
        {
    ...
    protected:
        ...
        struct ObjectStruct
        {
                ...
                void
                add( osg::Transform* transform )
                {
                    if( transform )
                    {
                        if( transform->getDataVariance() !=
                            osg::Object::DataVariance::STATIC )
                        {
                ...
                osg::dmat4   _firstMatrix;
                TransformSet _transformSet;
        };

        void
        registerWithCurrentObjects( osg::Transform* transform )
        {
        ...
        void
        disableObject( osg::Object* object )
        {
...
void
CollectLowestTransformsVisitor::doTransform( osg::Object* obj,
                                             osg::dmat4&  matrix )
{
    ...
    if( lod )
    {
        osg::dmat4 matrix_no_trans = matrix;
        osg::setTrans( matrix_no_trans, 0.0, 0.0, 0.0 );

        osg::vec3 v111( 1.0F, 1.0F, 1.0F );
        osg::vec3 new_v111 = osg::vec3( matrix_no_trans * osg::dvec3( v111 ) );
        float     ratio    = osg::length( new_v111 ) / osg::length( v111 );

        // move center point.
        lod->setCenter( osg::vec3( matrix * osg::dvec3( lod->getCenter() ) ) );

        // adjust ranges to new scale.
        for( unsigned int i = 0; i < lod->getNumRanges(); ++i )
        {
            lod->setRange( i,
                           lod->getMinRange( i ) * ratio,
                           lod->getMaxRange( i ) * ratio );
        }
    ...
    if( billboard )
    {
        ...
        osg::vec3 axis = osg::transform3x3( tf._im, billboard->getAxis() );
        axis           = osg::normalize( axis );
        billboard->setAxis( axis );

        osg::vec3 normal = osg::transform3x3( tf._im, billboard->getNormal() );
        ...
        for( unsigned int i = 0; i < billboard->getNumDrawables(); ++i )
        {
            billboard->setPosition(
                i,
                osg::vec3( matrix * osg::dvec3( billboard->getPosition( i ) ) )
            );
...
void
CollectLowestTransformsVisitor::disableObject( ObjectMap::iterator itr )
{
    ...
    if( itr->second._canBeApplied )
    {
        ...
        for( ObjectStruct::TransformSet::iterator titr =
                 itr->second._transformSet.begin();
...
void
CollectLowestTransformsVisitor::disableTransform( osg::Transform* transform )
{
...
void
CollectLowestTransformsVisitor::setUpMaps()
{
    // create the TransformMap from the ObjectMap
    ObjectMap::iterator oitr;
    for( oitr = _objectMap.begin(); oitr != _objectMap.end(); ++oitr )
    {
    ...
    for( oitr = _objectMap.begin(); oitr != _objectMap.end(); ++oitr )
    {
        ...
        if( os._canBeApplied )
        {
            if( os._moreThanOneMatrixRequired ||
                !isOperationPermissibleForObject( object ) )
            {
                disableObject( oitr );
            }
        }
...
bool
CollectLowestTransformsVisitor::removeTransforms( osg::Node* nodeWeCannotRemove )
{
    // transform the objects that can be applied.
    for( ObjectMap::iterator oitr = _objectMap.begin(); oitr != _objectMap.end();
         ++oitr )
    {
        osg::Object*  object = oitr->first;
        ObjectStruct& os     = oitr->second;
         ...
         ++titr )
    {
        if( titr->first != 0 && titr->second._canBeApplied )
        {
            if( titr->first != nodeWeCannotRemove )
            {
                ...
                for( int i2 = static_cast<int>( transform->getNumParents() ) - 1;
                     i2 >= 0;
                     --i2 )
                {
                    transform->getParent( static_cast<unsigned int>( i2 ) )
                        ->replaceChild( transform.get(), group.get() );
                }
            }
            else
            {
                ...
                if( mt )
                {
                    mt->setMatrix( osg::dmat4() );
                }
                else
                {
                    osg::PositionAttitudeTransform* pat =
                        titr->first->asPositionAttitudeTransform();
                    if( pat )
                    {
                        pat->setPosition( osg::dvec3( 0.0, 0.0, 0.0 ) );
                        pat->setAttitude( osg::quat() );
                        pat->setPivotPoint( osg::dvec3( 0.0, 0.0, 0.0 ) );
                    }
                    else
                    {
                        OSG_WARN << "Warning:: during "
                                    "Optimize::CollectLowestTransformsVisitor::"
                                    "removeTransforms(Node*)"
                                 << std::endl;
                        OSG_WARN
                            << "          unhandled of setting of indentity matrix on "
                            << titr->first->className() << std::endl;
                        OSG_WARN
                            << "          model will appear in the incorrect position."
                            << std::endl;
                    }
                }
            }
...
void
Optimizer::FlattenStaticTransformsVisitor::apply( osg::ProxyNode& node )
{
    _excludedNodeSet.insert( &node );
...
void
Optimizer::FlattenStaticTransformsVisitor::apply( osg::PagedLOD& node )
{
    _excludedNodeSet.insert( &node );
...
void
Optimizer::FlattenStaticTransformsVisitor::apply( osg::Drawable& drawable )
{
    ...
    if( ( geometry ) && ( isOperationPermissibleForObject( &drawable ) ) )
    {
        ...
        {
            geometry->setVertexArray( dynamic_cast<osg::Array*>(
                geometry->getVertexArray()->clone( osg::CopyOp::DEEP_COPY_ALL )
            ) );
        }
        if( geometry->getNormalArray() &&
            geometry->getNormalArray()->referenceCount() > 1 )
        {
            geometry->setNormalArray( dynamic_cast<osg::Array*>(
                geometry->getNormalArray()->clone( osg::CopyOp::DEEP_COPY_ALL )
            ) );
        }
    }
...
void
Optimizer::FlattenStaticTransformsVisitor::apply( osg::Billboard& billboard )
{
...
void
Optimizer::FlattenStaticTransformsVisitor::apply( osg::Transform& transform )
{
    if( !_transformStack.empty() )
    {
...
bool
Optimizer::FlattenStaticTransformsVisitor::removeTransforms(
...
)
{
    ...
    for( DrawableSet::iterator ditr = _drawableSet.begin(); ditr != _drawableSet.end();
         ++ditr )
    {
...
void
Optimizer::CombineStaticTransformsVisitor::apply( osg::MatrixTransform& transform )
{
    if( transform.getDataVariance() ==
        osg::Object::DataVariance::STATIC &&
        transform.getNumChildren() ==
        1 &&
        transform.getChild( 0 )->asTransform() !=
        0 &&
        transform.getChild( 0 )->asTransform()->asMatrixTransform() !=
        0 &&
        transform.getChild( 0 )->asTransform()->getDataVariance() ==
        osg::Object::DataVariance::STATIC &&
        isOperationPermissibleForObject( &transform ) &&
        isOperationPermissibleForObject( transform.getChild( 0 ) ) )
    {
        _transformSet.insert( &transform );
    }
...
bool
Optimizer::CombineStaticTransformsVisitor::removeTransforms(
    osg::Node* nodeWeCannotRemove
)
{
    if( nodeWeCannotRemove &&
        nodeWeCannotRemove->asTransform() !=
        0 &&
    ...
    while( !_transformSet.empty() )
    {
            ...
            osg::Object::DataVariance::STATIC )
        {
            ...
            osg::dmat4 newMatrix = child->getMatrix() * transform->getMatrix();
...
void
Optimizer::RemoveEmptyNodesVisitor::apply( osg::Group& group )
{
    if( group.getNumParents() > 0 )
    {
        ...
        if( group.getNumChildren() ==
            0 &&
            isOperationPermissibleForObject( &group ) &&
            ( typeid( group ) ==
              typeid( osg::Group ) ||
              ( group.asTransform() && !dynamic_cast<osg::CameraView*>( &group ) ) ) &&
            ( group.getNumChildrenRequiringUpdateTraversal() ==
              0 &&
              group.getNumChildrenRequiringEventTraversal() == 0 ) )
        {
...
void
Optimizer::RemoveEmptyNodesVisitor::removeEmptyNodes()
{
    ...
    while( !_redundantNodeList.empty() )
    {
             ...
             ++itr )
        {
            ...
            // take a copy of parents list since subsequent removes will modify the
            // original one.
                 ...
                 ++pitr )
            {
                    ...
                    strcmp( parent->className(), "MultiSwitch" ) != 0 )
                {
                    ...
                    if( parent->getNumChildren() ==
                        0 &&
                        isOperationPermissibleForObject( parent ) )
                    {
                        newEmptyGroups.insert( parent );
                    }
                }
            }
        }
...
bool
Optimizer::RemoveRedundantNodesVisitor::isOperationPermissible( osg::Node& node )
{
    return node.getNumParents() >
           0 &&
           !node.getStateSet() &&
           node.getName().empty() &&
...
void
Optimizer::RemoveRedundantNodesVisitor::apply( osg::Group& group )
{
    if( group.getNumChildren() ==
        1 &&
        typeid( group ) ==
...
void
Optimizer::RemoveRedundantNodesVisitor::apply( osg::Transform& transform )
{
    if( transform.getDataVariance() ==
        osg::Object::DataVariance::STATIC &&
...
void
Optimizer::RemoveRedundantNodesVisitor::removeRedundantNodes()
{
         ...
         ++itr )
    {
        ...
        {
            OSG_WARN << "Optimizer::RemoveRedundantNodesVisitor::removeRedundantNodes() "
                        "- failed dynamic_cast"
                     << std::endl;
        }
    }
...
void
Optimizer::RemoveLoadedProxyNodesVisitor::apply( osg::ProxyNode& proxyNode )
{
    if( proxyNode.getNumParents() >
        0 &&
...
void
Optimizer::RemoveLoadedProxyNodesVisitor::removeRedundantNodes()
{
         ...
         ++itr )
    {
        ...
        if( group.valid() )
        {
            ...
            // first check to see if data was attached to the ProxyNode that we need to
            // keep.
            bool keepData = false;
            ...
            if( keepData )
            {
                ...
                osg::ref_ptr<osg::Group> newGroup =
                    new osg::Group( *group, osg::CopyOp::SHALLOW_COPY );
        ...
        {
            OSG_WARN << "Optimizer::RemoveLoadedProxyNodesVisitor::removeRedundantNodes("
                        ") - failed dynamic_cast"
                     << std::endl;
        }
    }
...
////////////////////////////////////////////////////////////////////////////
void
Optimizer::CombineLODsVisitor::apply( osg::LOD& lod )
{
...
void
Optimizer::CombineLODsVisitor::combineLODs()
{
    for( GroupList::iterator itr = _groupList.begin(); itr != _groupList.end(); ++itr )
    {
        osg::Group*                 group = *itr;
        ...
        if( lodChildren.size() >= 2 )
        {
            ...
            for( lod_itr = lodChildren.begin(); lod_itr != lodChildren.end(); ++lod_itr )
            {
            ...
            if( bb.radius() < smallestRadius * 0.1F )
            {
                ...
                RangeMap                                     rangeMap;
                for( lod_itr = lodChildren.begin(); lod_itr != lodChildren.end();
                     ++lod_itr )
                {
                    ...
                    for( unsigned int i = 0; i < lod->getNumRanges(); ++i )
                    {
                        rangeMap.insert( RangeMap::value_type(
                            RangePair( lod->getMinRange( i ), lod->getMaxRange( i ) ),
                            lod->getChild( i )
                        ) );
                    }
                }
                ...
                for( RangeMap::iterator c_itr = rangeMap.begin();
                     ...
                     ++c_itr, ++i )
                {
                    newLOD->setRange( static_cast<unsigned int>( i ),
                                      c_itr->first.first,
                                      c_itr->first.second );
...
#define COMPARE_BINDING( lhs, rhs )                       \
    if( osg::getBinding( lhs ) < osg::getBinding( rhs ) ) \
        return true;                                      \
    if( osg::getBinding( rhs ) < osg::getBinding( lhs ) ) \
        return false;

struct LessGeometry
{
        bool
        operator()( const osg::ref_ptr<osg::Geometry>& lhs,
                    const osg::ref_ptr<osg::Geometry>& rhs ) const
        {
            ...
            COMPARE_BINDING( lhs->getSecondaryColorArray(),
            ...
            if( osg::getBinding( lhs->getNormalArray() ) == osg::Array::BIND_OVERALL )
            {
                ...
                // should be the case after running
                ...
                switch( lhs_normalArray->getType() )
                {
                    case( osg::Array::Vec3bArrayType ) :
                        if( (
                                *static_cast<const osg::Vec3bArray*>( lhs_normalArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3bArray*>(
                                rhs_normalArray
                            ) )[0] )
                        {
                            return true;
                        ...
                        if( (
                                *static_cast<const osg::Vec3bArray*>( rhs_normalArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3bArray*>(
                                lhs_normalArray
                            ) )[0] )
                    ...
                    case( osg::Array::Vec3sArrayType ) :
                        if( (
                                *static_cast<const osg::Vec3sArray*>( lhs_normalArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3sArray*>(
                                rhs_normalArray
                            ) )[0] )
                        {
                            return true;
                        ...
                        if( (
                                *static_cast<const osg::Vec3sArray*>( rhs_normalArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3sArray*>(
                                lhs_normalArray
                            ) )[0] )
                    ...
                    case( osg::Array::Vec3ArrayType ) :
                        if( (
                                *static_cast<const osg::Vec3Array*>( lhs_normalArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3Array*>(
                                rhs_normalArray
                            ) )[0] )
                        {
                            return true;
                        ...
                        if( (
                                *static_cast<const osg::Vec3Array*>( rhs_normalArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3Array*>(
                                lhs_normalArray
                            ) )[0] )
            ...
            if( osg::getBinding( lhs->getColorArray() ) == osg::Array::BIND_OVERALL )
            {
                ...
                switch( lhs_colorArray->getType() )
                {
                    case( osg::Array::Vec4ubArrayType ) :
                        if( (
                                *static_cast<const osg::Vec4ubArray*>( lhs_colorArray )
                            )[0] <
                            ( *static_cast<const osg::Vec4ubArray*>(
                                rhs_colorArray
                            ) )[0] )
                        {
                            return true;
                        ...
                        if( (
                                *static_cast<const osg::Vec4ubArray*>( rhs_colorArray )
                            )[0] <
                            ( *static_cast<const osg::Vec4ubArray*>(
                                lhs_colorArray
                            ) )[0] )
                    ...
                    case( osg::Array::Vec3ArrayType ) :
                        if( (
                                *static_cast<const osg::Vec3Array*>( lhs_colorArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3Array*>(
                                rhs_colorArray
                            ) )[0] )
                        ...
                        if( (
                                *static_cast<const osg::Vec3Array*>( rhs_colorArray )
                            )[0] <
                            ( *static_cast<const osg::Vec3Array*>(
                                lhs_colorArray
                            ) )[0] )
                    ...
                    case( osg::Array::Vec4ArrayType ) :
                        if( (
                                *static_cast<const osg::Vec4Array*>( lhs_colorArray )
                            )[0] <
                        ...
                        if( (
                                *static_cast<const osg::Vec4Array*>( rhs_colorArray )
                            )[0] <
                            ( *static_cast<const osg::Vec4Array*>(
                                lhs_colorArray
                            ) )[0] )
...
struct LessGeometryPrimitiveType
{
                    ...
                    const osg::ref_ptr<osg::Geometry>& rhs ) const
        {
            for( unsigned int i = 0;
                 ...
                 ++i )
            {
                if( lhs->getPrimitiveSet( i )->getType() <
                    rhs->getPrimitiveSet( i )->getType() )
                {
                    return true;
                }
                else if( rhs->getPrimitiveSet( i )->getType() <
                         lhs->getPrimitiveSet( i )->getType() )
                {
                    return false;
                }

                if( lhs->getPrimitiveSet( i )->getMode() <
                    rhs->getPrimitiveSet( i )->getMode() )
                {
                    return true;
                }
                else if( rhs->getPrimitiveSet( i )->getMode() <
                         lhs->getPrimitiveSet( i )->getMode() )
                {
                    return false;
                }
            }
...
void
Optimizer::MakeFastGeometryVisitor::apply( osg::Geometry& geom )
{
...
inline unsigned int
getSize( const osg::Array* a )
{
    return a ? a->getNumElements() : 0;
}

/// When merging geometries, tests if two arrays can be merged, regarding to their number
/// of components, and the number of vertices.
bool
isArrayCompatible( unsigned int      numVertice1,
                   unsigned int      numVertice2,
                   const osg::Array* compare1,
                   const osg::Array* compare2 )
{
    // Sumed up truth table:
    //  If array (1 or 2) not empty and vertices empty => error, should not happen
    //  (allows simplification in formulae below) If one side has both vertices and
    //  array, and the other side has only vertices => then arrays cannot be merged Else,
...
/// Return true only if both geometries have same array type and if arrays (such as
/// TexCoords) are compatible (i.e. both empty or both filled)
bool
isAbleToMerge( const osg::Geometry& g1,
               const osg::Geometry& g2 )
{
    ...
    if( !isArrayCompatible( numVertice1,
                            numVertice2,
                            g1.getNormalArray(),
                            g2.getNormalArray() ) ||
        !isArrayCompatible( numVertice1,
                            numVertice2,
                            g1.getColorArray(),
                            g2.getColorArray() ) ||
        !isArrayCompatible( numVertice1,
                            numVertice2,
                            g1.getSecondaryColorArray(),
                            g2.getSecondaryColorArray() ) ||
        !isArrayCompatible( numVertice1,
                            numVertice2,
                            g1.getFogCoordArray(),
                            g2.getFogCoordArray() ) ||
    ...
    for( unsigned int eachTexCoordArray = 0;
         eachTexCoordArray < g1.getNumTexCoordArrays();
         ++eachTexCoordArray )
    {
        if( !isArrayCompatible( numVertice1,
                                numVertice2,
                                g1.getTexCoordArray( eachTexCoordArray ),
                                g2.getTexCoordArray( eachTexCoordArray ) ) )
        {
            return false;
        }
    }
    ...
    if( g1.getVertexArray() &&
        g2.getVertexArray() &&
        g1.getVertexArray()->getDataType() != g2.getVertexArray()->getDataType() )
    {
        return false;
    }
    if( g1.getNormalArray() &&
        g2.getNormalArray() &&
        g1.getNormalArray()->getDataType() != g2.getNormalArray()->getDataType() )
    {
        return false;
    }
    if( g1.getColorArray() &&
        g2.getColorArray() &&
        g1.getColorArray()->getDataType() != g2.getColorArray()->getDataType() )
    {
        return false;
    }
    if( g1.getSecondaryColorArray() &&
        g2.getSecondaryColorArray() &&
        g1.getSecondaryColorArray()->getDataType() !=
        g2.getSecondaryColorArray()->getDataType() )
    {
        return false;
    }
    if( g1.getFogCoordArray() &&
        g2.getNormalArray() &&
        g1.getFogCoordArray()->getDataType() != g2.getFogCoordArray()->getDataType() )
    {
        return false;
    }
    return true;
...
bool
Optimizer::MergeGeometryVisitor::mergeGroup( osg::Group& group )
{
    ...
    if( group.getNumChildren() >= 2 )
    {
        ...
        typedef std::map<osg::ref_ptr<osg::Geometry>, DuplicateList, LessGeometry>
        ...
        MergeList mergeListChecked;    // List of drawables just before merging, grouped
                                       // by "compatibility" and vertex limit
        MergeList mergeList;           // Intermediate list of drawables, grouped ony by
                                       // "compatibility"
             ...
             ++itr )
        {
            ...
            for( MergeList::iterator eachMergeList = mergeListTmp.begin();
                 eachMergeList != mergeListTmp.end();
                 ++eachMergeList )
            {
        ...
        // dequeue each DuplicateList when vertices limit is reached or when all elements
        // has been checked
        for( MergeList::iterator itr = mergeListChecked.begin();
             itr != mergeListChecked.end();
             ++itr )
        {
                 ...
                 ++ditr )
            {
                ...
                unsigned int   numVertices =
                    ( geometry->getVertexArray()
                          ...
                          : 0 );
                if( ( totalNumberVertices + numVertices ) >
        ...
        if( needToDoMerge )
        {
            // to avoid performance issues associated with incrementally removing a large
            // number children, we remove them all and add back the ones we need.
                 ...
                 ++mitr )
            {
                ...
                if( !duplicateList.empty() )
                {
                    ...
                    for( ; ditr != duplicateList.end(); ++ditr )
                    {
    ...
    // convert all polygon primitives which has 3 indices into TRIANGLES, 4 indices into
    // QUADS.
    unsigned int i;
    for( i = 0; i < group.getNumChildren(); ++i )
    {
    ...
    for( i = 0; i < group.getNumChildren(); ++i )
    {
        ...
        if( geom )
        {
            if( geom->getNumPrimitiveSets() >
                0 &&
                osg::getBinding( geom->getNormalArray() ) !=
                osg::Array::BIND_PER_PRIMITIVE_SET &&
                osg::getBinding( geom->getColorArray() ) !=
                osg::Array::BIND_PER_PRIMITIVE_SET &&
                osg::getBinding( geom->getSecondaryColorArray() ) !=
                osg::Array::BIND_PER_PRIMITIVE_SET &&
                osg::getBinding( geom->getFogCoordArray() ) !=
                osg::Array::BIND_PER_PRIMITIVE_SET )
            {
                    ...
                    if( combine )
                    {
                        ...
                        switch( lhs->getType() )
                        {
                            case( osg::PrimitiveSet::DrawArraysPrimitiveType ) :
                                combine = mergePrimitive(
                                    *( static_cast<osg::DrawArrays*>( lhs ) ),
                                    *( static_cast<osg::DrawArrays*>( rhs ) )
                                );
                                break;
                            case( osg::PrimitiveSet::DrawArrayLengthsPrimitiveType ) :
                                combine = mergePrimitive(
                                    *( static_cast<osg::DrawArrayLengths*>( lhs ) ),
                                    *( static_cast<osg::DrawArrayLengths*>( rhs ) )
                                );
                                break;
                            case( osg::PrimitiveSet::DrawElementsUBytePrimitiveType ) :
                                combine = mergePrimitive(
                                    *( static_cast<osg::DrawElementsUByte*>( lhs ) ),
                                    *( static_cast<osg::DrawElementsUByte*>( rhs ) )
                                );
                                break;
                            case( osg::PrimitiveSet::DrawElementsUShortPrimitiveType ) :
                                combine = mergePrimitive(
                                    *( static_cast<osg::DrawElementsUShort*>( lhs ) ),
                                    *( static_cast<osg::DrawElementsUShort*>( rhs ) )
                                );
                                break;
                            case( osg::PrimitiveSet::DrawElementsUIntPrimitiveType ) :
                                combine = mergePrimitive(
                                    *( static_cast<osg::DrawElementsUInt*>( lhs ) ),
                                    *( static_cast<osg::DrawElementsUInt*>( rhs ) )
                                );
                                break;
                            default :
                                combine = false;
                ...
                if( doneCombine )
                {
                    // now need to clean up primitiveset so it no longer contains the rhs
                    // combined primitives.
                    ...
                    for( osg::Geometry::PrimitiveSetList::iterator pitr =
                             oldPrimitives.begin();
                         pitr != oldPrimitives.end();
                         ++pitr )
                    {
                        if( ( *pitr )->getMode() != 0XFF'FF'FF )
                        {
...
bool
Optimizer::MergeGeometryVisitor::geometryContainsSharedArrays( osg::Geometry& geom )
{
    ...
    if( geom.getSecondaryColorArray() &&
        geom.getSecondaryColorArray()->referenceCount() > 1 )
    {
        return true;
    }
    ...
    // shift the indices of the incoming primitives to account for the pre existing
    // geometry.
    for( osg::Geometry::PrimitiveSetList::iterator primItr =
             geom.getPrimitiveSetList().begin();
...
class MergeArrayVisitor : public osg::ArrayVisitor
{
    ...
    public:
        ...
        apply( osg::Array& )
        {
            OSG_WARN << "Warning: Optimizer's MergeArrayVisitor cannot merge Array type."
                     << std::endl;
        }

        virtual void
        apply( osg::ByteArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::ShortArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::IntArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::UByteArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::UShortArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::UIntArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec4ubArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec3ubArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec2ubArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec4usArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec3usArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec2usArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::FloatArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec2Array& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec3Array& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec4Array& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::DoubleArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec2dArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec3dArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec4dArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec2bArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec3bArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec4bArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec2sArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec3sArray& rhs )
        {
            _merge( rhs );
        }

        virtual void
        apply( osg::Vec4sArray& rhs )
        {
            _merge( rhs );
        }
};
...
bool
Optimizer::MergeGeometryVisitor::mergeGeometry( osg::Geometry& lhs,
                                                osg::Geometry& rhs )
{
    ...
    if( lhs.getVertexArray() && rhs.getVertexArray() )
    {
        ...
        {
            OSG_DEBUG << "MergeGeometry: vertex array not merged. Some data may be lost."
                      << std::endl;
        }
    }
    ...
    if( lhs.getNormalArray() &&
        rhs.getNormalArray() &&
        lhs.getNormalArray()->getBinding() != osg::Array::BIND_OVERALL )
    {
        if( !merger.merge( lhs.getNormalArray(), rhs.getNormalArray() ) )
        {
            OSG_DEBUG << "MergeGeometry: normal array not merged. Some data may be lost."
                      << std::endl;
        }
    }
    ...
    if( lhs.getColorArray() &&
        rhs.getColorArray() &&
        lhs.getColorArray()->getBinding() != osg::Array::BIND_OVERALL )
    {
        if( !merger.merge( lhs.getColorArray(), rhs.getColorArray() ) )
        {
            OSG_DEBUG << "MergeGeometry: color array not merged. Some data may be lost."
                      << std::endl;
        }
    }
    ...
    if( lhs.getSecondaryColorArray() &&
        rhs.getSecondaryColorArray() &&
        lhs.getSecondaryColorArray()->getBinding() != osg::Array::BIND_OVERALL )
    {
        if( !merger.merge( lhs.getSecondaryColorArray(), rhs.getSecondaryColorArray() ) )
        {
            OSG_DEBUG << "MergeGeometry: secondary color array not merged. Some data "
                         "may be lost."
                      << std::endl;
        }
    }
    ...
    if( lhs.getFogCoordArray() &&
        rhs.getFogCoordArray() &&
        lhs.getFogCoordArray()->getBinding() != osg::Array::BIND_OVERALL )
    {
        if( !merger.merge( lhs.getFogCoordArray(), rhs.getFogCoordArray() ) )
        {
            OSG_DEBUG
                << "MergeGeometry: fog coord array not merged. Some data may be lost."
                << std::endl;
        }
    }
    ...
    for( unit = 0; unit < lhs.getNumTexCoordArrays(); ++unit )
    {
        ...
        {
            OSG_DEBUG
                << "MergeGeometry: tex coord array not merged. Some data may be lost."
                << std::endl;
        }
    }
    ...
    {
        if( !merger.merge( lhs.getVertexAttribArray( unit ),
                           rhs.getVertexAttribArray( unit ) ) )
        {
            OSG_DEBUG << "MergeGeometry: vertex attrib array not merged. Some data may "
                         "be lost."
                      << std::endl;
        }
    }
         ...
         ++primItr )
    {
        ...
        switch( primitive->getType() )
        {
            case( osg::PrimitiveSet::DrawElementsUBytePrimitiveType ) :
                {
                    osg::DrawElementsUByte* primitiveUByte =
                    ...
                    {
                        currentMaximum =
                            std::max( currentMaximum, ( unsigned int )*eitr );
                    }
                    if( ( base + currentMaximum ) >= 65'536 )
                    {
                        // must promote to a DrawElementsUInt
                        osg::DrawElementsUInt* new_primitive =
                            new osg::DrawElementsUInt( primitive->getMode() );
                        std::copy( primitiveUByte->begin(),
                                   primitiveUByte->end(),
                                   std::back_inserter( *new_primitive ) );
                        new_primitive->offsetIndices( static_cast<int>( base ) );
                        ( *primItr ) = new_primitive;
                    }
                    else if( ( base + currentMaximum ) >= 256 )
                    {
                        // must promote to a DrawElementsUShort
                        osg::DrawElementsUShort* new_primitive =
                            new osg::DrawElementsUShort( primitive->getMode() );
                        std::copy( primitiveUByte->begin(),
                                   primitiveUByte->end(),
                                   std::back_inserter( *new_primitive ) );
                        new_primitive->offsetIndices( static_cast<int>( base ) );
                    ...
                    {
                        primitive->offsetIndices( static_cast<int>( base ) );
                    }
                }
                break;
            ...
            case( osg::PrimitiveSet::DrawElementsUShortPrimitiveType ) :
                {
                    osg::DrawElementsUShort* primitiveUShort =
                        static_cast<osg::DrawElementsUShort*>( primitive );
                    unsigned int currentMaximum = 0;
                    for( osg::DrawElementsUShort::iterator eitr =
                             primitiveUShort->begin();
                    ...
                    if( ( base + currentMaximum ) >= 65'536 )
                    {
                        ...
                        osg::DrawElementsUInt* new_primitive =
                            new osg::DrawElementsUInt( primitive->getMode() );
                        std::copy( primitiveUShort->begin(),
                                   primitiveUShort->end(),
                                   std::back_inserter( *new_primitive ) );
                        new_primitive->offsetIndices( static_cast<int>( base ) );
                        ( *primItr ) = new_primitive;
...
bool
Optimizer::MergeGeometryVisitor::mergePrimitive( osg::DrawArrays& lhs,
                                                 osg::DrawArrays& rhs )
{
...
bool
Optimizer::MergeGeometryVisitor::mergePrimitive( osg::DrawArrayLengths& lhs,
...
bool
Optimizer::MergeGeometryVisitor::mergePrimitive( osg::DrawElementsUByte& lhs,
...
bool
Optimizer::MergeGeometryVisitor::mergePrimitive( osg::DrawElementsUShort& lhs,
...
bool
Optimizer::MergeGeometryVisitor::mergePrimitive( osg::DrawElementsUInt& lhs,
...
void
Optimizer::SpatializeGroupsVisitor::apply( osg::Group& group )
{
...
void
Optimizer::SpatializeGroupsVisitor::apply( osg::Geode& geode )
{
...
bool
Optimizer::SpatializeGroupsVisitor::divide( unsigned int maxNumTreesPerCell )
{
    bool divided = false;
...
bool
Optimizer::SpatializeGroupsVisitor::divide( osg::Group*  group,
                                            unsigned int maxNumTreesPerCell )
{
    ...
    OSG_INFO << "Dividing " << group->className()
             << "  num children = " << group->getNumChildren() << "  xAxis=" << xAxis
             << "  yAxis=" << yAxis << "   zAxis=" << zAxis << std::endl;
    ...
    typedef std::pair<osg::box, osg::ref_ptr<osg::Group>> BoxGroupPair;
    ...
    if( xAxis )
    {
        unsigned int numCellsToDivide = static_cast<unsigned int>( boxes.size() );
        for( unsigned int i = 0; i < numCellsToDivide; ++i )
        {
            osg::box& orig_cell = boxes[i].first;
            osg::box  new_cell  = orig_cell;

            float     xCenter   = ( orig_cell.xMin() + orig_cell.xMax() ) * 0.5F;
            orig_cell.max.x     = xCenter;
            new_cell.min.x      = xCenter;
    ...
    if( yAxis )
    {
        ...
        for( unsigned int i = 0; i < numCellsToDivide; ++i )
        {
            ...
            orig_cell.max.y     = yCenter;
            new_cell.min.y      = yCenter;
    ...
    if( zAxis )
    {
        ...
        for( unsigned int i = 0; i < numCellsToDivide; ++i )
        {
            ...
            orig_cell.max.z     = zCenter;
            new_cell.min.z      = zCenter;
    ...
    for( unsigned int i = 0; i < group->getNumChildren(); ++i )
    {
        ...
        for( Boxes::iterator itr = boxes.begin(); itr != boxes.end() && !assigned;
             ++itr )
        {
    ...
    for( Boxes::iterator itr = boxes.begin(); itr != boxes.end(); ++itr )
    {
        // move child from main group into bb group.
    ...
    // add then the unassigned children.
    for( NodeList::iterator nitr = unassignedList.begin(); nitr != unassignedList.end();
         ++nitr )
    {
...
bool
Optimizer::SpatializeGroupsVisitor::divide( osg::Geode*  geode,
                                            unsigned int maxNumTreesPerCell )
{
    ...
    OSG_INFO << "INFO " << geode->className()
             << "  num drawables = " << geode->getNumDrawables() << "  xAxis=" << xAxis
             << "  yAxis=" << yAxis << "   zAxis=" << zAxis << std::endl;
    ...
    if( parents.empty() )
    {
        OSG_INFO << "  Cannot perform spatialize on root Geode, add a Group above it to "
                    "allow subdivision."
                 << std::endl;
        return false;
    }
...
void
Optimizer::CopySharedSubgraphsVisitor::apply( osg::Node& node )
{
...
void
Optimizer::CopySharedSubgraphsVisitor::copySharedNodes()
{
         ...
         ++itr )
    {
        ...
        {
            // create a clone.
            osg::ref_ptr<osg::Object> new_object =
                node->clone( osg::CopyOp::DEEP_COPY_NODES |
...
void
Optimizer::TextureVisitor::apply( osg::Node& node )
{
...
void
Optimizer::TextureVisitor::apply( osg::StateSet& stateset )
{
    for( unsigned int i = 0; i < stateset.getTextureAttributeList().size(); ++i )
    {
        osg::StateAttribute* sa =
            stateset.getTextureAttribute( i, osg::StateAttribute::Type::TEXTURE );
...
void
Optimizer::TextureVisitor::apply( osg::Texture& texture )
{
    if( _changeAutoUnRef )
    {
        ...
        {
            osg::ImageStream* is =
                dynamic_cast<osg::ImageStream*>( texture.getImage( i ) );
            if( is )
            {
                ++numImageStreams;
            }
...
void
Optimizer::MergeGeodesVisitor::apply( osg::Group& group )
{
...
{
        bool
        operator()( const osg::Geode* lhs,
                    const osg::Geode* rhs ) const
        {
...
bool
Optimizer::MergeGeodesVisitor::mergeGeodes( osg::Group& group )
{
    if( !isOperationPermissibleForObject( &group ) )
    {
        return false;
    ...
    for( i = 0; i < group.getNumChildren(); ++i )
    {
        // keep a reference to this child so we can safely clear the group of all
        // children this is done so we don't have to do a search and remove from the list
        // later on.
    ...
    OSG_INFO << "mergeGeodes in group '" << group.getName() << "' "
             << geodeDuplicateMap.size() << std::endl;
...
bool
Optimizer::MergeGeodesVisitor::mergeGeode( osg::Geode& lhs,
                                           osg::Geode& rhs )
{
...
////////////////////////////////////////////////////////////////////////////
void
Optimizer::FlattenBillboardVisitor::reset()
{
    _billboards.clear();
}

void
Optimizer::FlattenBillboardVisitor::apply( osg::Billboard& billboard )
{
    _billboards[&billboard].push_back( getNodePath() );
}

void
Optimizer::FlattenBillboardVisitor::process()
{
         ...
         ++itr )
    {
        ...
        if( npl.size() > 1 )
        {
            for( NodePathList::iterator nitr = npl.begin(); nitr != npl.end(); ++nitr )
            {
                ...
                if( np.size() >= 3 )
                {
                    ...
                    osg::MatrixTransform* mt =
                        dynamic_cast<osg::MatrixTransform*>( np[np.size() - 2] );
                    ...
                    if( group ==
                        ...
                        mt->getNumChildren() == 1 )
                    {
                        const osg::dmat4& m = mt->getMatrix();
                        mergeAcceptable     = ( m( 0, 0 ) ==
                                                1.0 &&
                                                m( 0, 1 ) ==
                                                0.0 &&
                                                m( 0, 2 ) ==
        ...
        if( mergeAcceptable && mainGroup )
        {
            ...
            for( MatrixTransformSet::iterator mitr = mts.begin(); mitr != mts.end();
                 ++mitr )
            {
                osg::MatrixTransform* mt = *mitr;
                for( unsigned int i = 0; i < billboard->getNumDrawables(); ++i )
                {
                    new_billboard->addDrawable(
                        billboard->getDrawable( i ),
                        osg::vec3( mt->getMatrix() *
                                   osg::dvec3( billboard->getPosition( i ) ) )
                    );
                }
...
Optimizer::TextureAtlasBuilder::TextureAtlasBuilder() :
    _maximumAtlasWidth( 2'048 ),
    _maximumAtlasHeight( 2'048 ),
    _margin( 8 )
{
}

void
Optimizer::TextureAtlasBuilder::reset()
{
    _sourceList.clear();
    _atlasList.clear();
}
...
void
Optimizer::TextureAtlasBuilder::setMaximumAtlasSize( int width,
                                                     int height )
{
...
void
Optimizer::TextureAtlasBuilder::setMargin( int margin )
{
    _margin = margin;
}

void
Optimizer::TextureAtlasBuilder::addSource( const osg::Image* image )
{
    if( !getSource( image ) )
    {
...
void
Optimizer::TextureAtlasBuilder::addSource( const osg::Texture2D* texture )
{
...
void
Optimizer::TextureAtlasBuilder::completeRow( unsigned int indexAtlas )
{
    ...
    if( atlas->_indexFirstOfRow < atlas->_sourceList.size() )
    {
        ...
        // Fill last Row
        for( SourceList::iterator sitr3 = _sourceList.begin();
             sitr3 != _sourceList.end();
             ++sitr3 )
        {
            ...
            if( x_min +
                ...
                x_max &&
                y_min +
                image_t <= y_max )    // Test if the image can fit in the empty space.
            {
                source->_x = x_min;
                source->_y = y_min;
                // assert(source->_x + source->_image->s()+_margin <=
                // atlas->_maximumAtlasWidth );        // "+_margin" and not "+2*_margin"
                // because _x already takes the margin into account assert(source->_y +
                // source->_image->t()+_margin <= atlas->_maximumAtlasHeight);
        ...
        for( SourceList::iterator sitr4 =
                 atlas->_sourceList.begin() + atlas->_indexFirstOfRow;
             sitr4 != atlas->_sourceList.end();
             ++sitr4 )
        {
            ...
            int     inner_x_max =
                x_min + srcAdded->_image->s();    // Hides upper block's x_max
            if( y_min >= y_max || x_min >= inner_x_max )
            {
                continue;
            }

            Source* maxWidthSource = NULL;
            for( SourceList::iterator sitr2 = _sourceList.begin();
                 sitr2 != _sourceList.end();
                 ++sitr2 )
            {
                ...
                if( x_min +
                    ...
                    y_min +
                    image_t <=
                    y_max )    // Test if the image can fit in the empty space.
                {
                    if( maxWidthSource ==
                        NULL ||
                        maxWidthSource->_image->s() < source->_image->s() )
                    {
            ...
            if( maxWidthSource )
            {
                ...
                srcListTmp.push_back(
                    maxWidthSource
                );    // Store the mawWidth source in the temporary vector.
            }
        }
        for( SourceList::iterator itTmp = srcListTmp.begin(); itTmp != srcListTmp.end();
             ++itTmp )
        {
            // Add the sources to the general list (wasn't possible in the loop using the
            // iterator on the same list)
            atlas->_sourceList.push_back( *itTmp );
        }
        atlas->_indexFirstOfRow = static_cast<unsigned int>( atlas->_sourceList.size() );
    }
}

void
Optimizer::TextureAtlasBuilder::buildAtlas()
{
    std::sort( _sourceList.begin(),
               _sourceList.end(),
               CompareSrc() );    // Sort using the height of images
    _atlasList.clear();
    for( SourceList::iterator sitr = _sourceList.begin(); sitr != _sourceList.end();
         ++sitr )
    {
        Source* source = sitr->get();
        if( !source->_atlas && source->suitableForAtlas( _maximumAtlasWidth,
                                                         _maximumAtlasHeight,
                                                         _margin ) )
        {
                 ...
                 ++aitr )
            {
                if( !( *aitr )->_image || ( ( *aitr )->_image->getPixelFormat() ==
                                            ...
                                            ( *aitr )->_image->getPacking() ==
                                            ( *sitr )->_image->getPacking() ) )
                {
                    OSG_INFO << "checking source " << source->_image->getFileName()
                             << " to see it it'll fit in atlas " << aitr->get()
                             << std::endl;
                    Optimizer::TextureAtlasBuilder::Atlas::FitsIn fitsIn =
                        ( *aitr )->doesSourceFit( source );
                    if( fitsIn ==
                        Optimizer::TextureAtlasBuilder::Atlas::FITS_IN_CURRENT_ROW )
                    {
                        addedSourceToAtlas = true;
                        ( *aitr )->addSource( source );    // Add in the currentRow.
                    }
                    else if( fitsIn ==
                             Optimizer::TextureAtlasBuilder::Atlas::IN_NEXT_ROW )
                    {
                        completeRow(
                            static_cast<unsigned int>( aitr - _atlasList.begin() )
                        );    // Fill Empty
                              // spaces.
                        addedSourceToAtlas = true;
                        ( *aitr )->addSource(
                            source
                        );    // Add the source in the new row.
                    }
                    else
                    {
                        completeRow(
                            static_cast<unsigned int>( aitr - _atlasList.begin() )
                        );    // Fill Empty spaces before creating a new atlas.
                    }
                }
            }
            ...
            {
                OSG_INFO << "creating new Atlas for " << source->_image->getFileName()
                         << std::endl;

                osg::ref_ptr<Atlas> atlas =
                    new Atlas( _maximumAtlasWidth, _maximumAtlasHeight, _margin );
    ...
    for( AtlasList::iterator aitr = _atlasList.begin(); aitr != _atlasList.end();
         ++aitr )
    {
...
osg::Image*
Optimizer::TextureAtlasBuilder::getImageAtlas( unsigned int i )
{
...
osg::Texture2D*
Optimizer::TextureAtlasBuilder::getTextureAtlas( unsigned int i )
{
    Source* source = _sourceList[i].get();
    Atlas*  atlas  = source ? source->_atlas : 0;
...
osg::dmat4
Optimizer::TextureAtlasBuilder::getTextureMatrix( unsigned int i )
{
    Source* source = _sourceList[i].get();
    return source ? source->computeTextureMatrix() : osg::dmat4();
}

osg::Image*
Optimizer::TextureAtlasBuilder::getImageAtlas( const osg::Image* image )
{
...
osg::Texture2D*
Optimizer::TextureAtlasBuilder::getTextureAtlas( const osg::Image* image )
{
    Source* source = getSource( image );
    Atlas*  atlas  = source ? source->_atlas : 0;
    return atlas ? atlas->_texture.get() : 0;
...
osg::dmat4
Optimizer::TextureAtlasBuilder::getTextureMatrix( const osg::Image* image )
{
    Source* source = getSource( image );
    return source ? source->computeTextureMatrix() : osg::dmat4();
}
...
osg::Image*
Optimizer::TextureAtlasBuilder::getImageAtlas( const osg::Texture2D* texture )
{
...
osg::Texture2D*
Optimizer::TextureAtlasBuilder::getTextureAtlas( const osg::Texture2D* texture )
{
    Source* source = getSource( texture );
    Atlas*  atlas  = source ? source->_atlas : 0;
    return atlas ? atlas->_texture.get() : 0;
...
osg::dmat4
Optimizer::TextureAtlasBuilder::getTextureMatrix( const osg::Texture2D* texture )
{
    Source* source = getSource( texture );
    return source ? source->computeTextureMatrix() : osg::dmat4();
}

Optimizer::TextureAtlasBuilder::Source*
Optimizer::TextureAtlasBuilder::getSource( const osg::Image* image )
{
    for( SourceList::iterator itr = _sourceList.begin(); itr != _sourceList.end();
         ++itr )
    {
        if( ( *itr )->_image == image )
        {
...
Optimizer::TextureAtlasBuilder::Source*
Optimizer::TextureAtlasBuilder::getSource( const osg::Texture2D* texture )
{
    for( SourceList::iterator itr = _sourceList.begin(); itr != _sourceList.end();
         ++itr )
    {
...
bool
Optimizer::TextureAtlasBuilder::Source::suitableForAtlas( int maximumAtlasWidth,
                                                          ...
                                                          int margin )
{
    ...
    {
        // pixel size not byte aligned so report as not suitable to prevent other atlas
        // code from having problems with byte boundaries.
        return false;
    }
    if( _texture.valid() )
    {

        if( _texture->getWrap( osg::Texture2D::WRAP_S ) ==
        ...
        if( _texture->getWrap( osg::Texture2D::WRAP_T ) ==
            osg::Texture2D::REPEAT ||
...
osg::dmat4
Optimizer::TextureAtlasBuilder::Source::computeTextureMatrix() const
{
    if( !_atlas )
    {
        return osg::dmat4();
    ...
    typedef osg::dmat4::value_type Float;
    return osg::scale( Float( _image->s() ) / Float( _atlas->_image->s() ),
                       Float( _image->t() ) / Float( _atlas->_image->t() ),
                       1.0 ) *
           osg::translate( Float( _x ) / Float( _atlas->_image->s() ),
                           Float( _y ) / Float( _atlas->_image->t() ),
                           0.0 );
}

Optimizer::TextureAtlasBuilder::Atlas::FitsIn
Optimizer::TextureAtlasBuilder::Atlas::doesSourceFit( Source* source )
{
    ...
    if( sourceTexture )
    {
        if( sourceTexture->getWrap( osg::Texture2D::WRAP_S ) ==
            osg::Texture2D::REPEAT ||
        ...
        if( sourceTexture->getWrap( osg::Texture2D::WRAP_T ) ==
            osg::Texture2D::REPEAT ||
        ...
        if( _texture.valid() )
        {
            ...
            bool sourceUsesBorder = sourceTexture->getWrap( osg::Texture2D::WRAP_S ) ==
                                    osg::Texture2D::CLAMP_TO_BORDER ||
                                    sourceTexture->getWrap( osg::Texture2D::WRAP_T ) ==
                                    osg::Texture2D::CLAMP_TO_BORDER;

            bool atlasUsesBorder  = sourceTexture->getWrap( osg::Texture2D::WRAP_S ) ==
                                    osg::Texture2D::CLAMP_TO_BORDER ||
                                    sourceTexture->getWrap( osg::Texture2D::WRAP_T ) ==
                                    osg::Texture2D::CLAMP_TO_BORDER;
            ...
            if( _texture->getFilter( osg::Texture2D::MIN_FILTER ) !=
                sourceTexture->getFilter( osg::Texture2D::MIN_FILTER ) )
            {
                // inconsistent min filters
                return DOES_NOT_FIT_IN_ANY_ROW;
            }

            if( _texture->getFilter( osg::Texture2D::MAG_FILTER ) !=
                sourceTexture->getFilter( osg::Texture2D::MAG_FILTER ) )
            {
            ...
            if( _texture->getShadowCompareFunc() !=
                sourceTexture->getShadowCompareFunc() )
            {
                // shadow functions inconsistent
                return DOES_NOT_FIT_IN_ANY_ROW;
            }

            if( _texture->getShadowTextureMode() !=
                sourceTexture->getShadowTextureMode() )
            {
...
bool
Optimizer::TextureAtlasBuilder::Atlas::addSource( Source* source )
{
    // double check source is compatible
    if( !doesSourceFit( source ) )
    {
        OSG_INFO << "source " << source->_image->getFileName()
                 << " does not fit in atlas " << this << std::endl;
        return false;
    }
    const osg::Image*     sourceImage   = source->_image.get();
    const osg::Texture2D* sourceTexture = source->_texture.get();
    ...
    if( !_texture && sourceTexture )
    {
        ...
        _texture->setWrap( osg::Texture2D::WRAP_S,
                           sourceTexture->getWrap( osg::Texture2D::WRAP_S ) );
        _texture->setWrap( osg::Texture2D::WRAP_T,
                           sourceTexture->getWrap( osg::Texture2D::WRAP_T ) );

        _texture->setBorderColor( sourceTexture->getBorderColor() );
        _texture->setBorderWidth( 0 );

        _texture->setFilter( osg::Texture2D::MIN_FILTER,
                             sourceTexture->getFilter( osg::Texture2D::MIN_FILTER ) );
        _texture->setFilter( osg::Texture2D::MAG_FILTER,
                             sourceTexture->getFilter( osg::Texture2D::MAG_FILTER ) );
    ...
    if( ( _x + sourceImage->s() + 2 * _margin ) <= _maximumAtlasWidth )
    {
        ...
        OSG_INFO << "current row insertion, source " << source->_image->getFileName()
                 << " " << _x << "," << _y << " fits in row of atlas " << this
                 << std::endl;
    ...
    if( ( _height + sourceImage->t() + 2 * _margin ) <= _maximumAtlasHeight )
    {
        ...
        OSG_INFO << "next row insertion, source " << source->_image->getFileName() << " "
                 << _x << "," << _y << " fits in row of atlas " << this << std::endl;
        ...
        OSG_INFO << "source " << source->_image->getFileName() << " " << _x << "," << _y
                 << " fits in row of atlas " << this << std::endl;

        return true;
    }

    OSG_INFO << "source " << source->_image->getFileName() << " does not fit in atlas "
             << this << std::endl;
...
void
Optimizer::TextureAtlasBuilder::Atlas::clampToNearestPowerOfTwoSize()
{
    int w = 1;
    while( w < _width )
    {
    ...
    OSG_INFO << "Clamping " << _width << ", " << _height << " to " << w << "," << h
             << std::endl;
...
void
Optimizer::TextureAtlasBuilder::Atlas::copySources()
{
         ...
         ++itr )
    {
        ...
        if( atlas == this )
        {
            OSG_INFO << "Copying image " << source->_image->getFileName() << " to "
                     << source->_x << " ," << source->_y << std::endl;
            OSG_INFO << "        image size " << source->_image->s() << ","
                     << source->_image->t() << std::endl;

            const osg::Image* sourceImage = source->_image.get();
            osg::Image*       atlasImage  = atlas->_image.get();
            // assert(sourceImage->getPacking() == atlasImage->getPacking()); //Test if
            // packings are equals.
            ...
            unsigned int      marginSizeInBytes =
                pixelSizeInBytes * static_cast<unsigned int>( _margin );

            // assert(atlas->_width  == static_cast<int>(atlasImage->s()));
            // assert(atlas->_height == static_cast<int>(atlasImage->t()));
            // assert(source->_x + static_cast<int>(source->_image->s())+_margin <=
            // static_cast<int>(atlas->_image->s()));        // "+_margin" and not
            // "+2*_margin" because _x already takes the margin into account
            // assert(source->_y + static_cast<int>(source->_image->t())+_margin <=
            // static_cast<int>(atlas->_image->t())); assert(source->_x >= _margin);
            // assert(source->_y >= _margin);
            int x = source->_x;
            int y = source->_y;
            ...
            for( t = 0; t < sourceImage->t(); ++t, ++y )
            {
                unsigned char* destPtr =
                    atlasImage->data( static_cast<unsigned int>( x ),
                                      static_cast<unsigned int>( y ) );
                const unsigned char* sourcePtr =
                    sourceImage->data( 0, static_cast<unsigned int>( t ) );
            ...
            for( m = 0; m < _margin; ++m, ++y )
            {
                unsigned char* destPtr =
                    atlasImage->data( static_cast<unsigned int>( x ),
                                      static_cast<unsigned int>( y ) );
                const unsigned char* sourcePtr = sourceImage->data(
                    0,
                    static_cast<unsigned int>( sourceImage->t() - 1 )
                );
                for( unsigned int i = 0; i < rowSize; i++ )
                {
                    *( destPtr++ ) = *( sourcePtr++ );
                ...
                for( m = 0; m < _margin; ++m, ++x )
                {
                    unsigned char* destPtr =
                        atlasImage->data( static_cast<unsigned int>( x ),
                                          static_cast<unsigned int>( y ) );
                    const unsigned char* sourcePtr = sourceImage->data(
                        static_cast<unsigned int>( sourceImage->s() - 1 ),
            ...
            {
                unsigned char* destPtr =
                    atlasImage->data( static_cast<unsigned int>( source->_x - _margin ),
                                      static_cast<unsigned int>( y ) );
                unsigned char* sourcePtr = atlasImage->data(
                    static_cast<unsigned int>( source->_x - _margin ),
            ...
            for( m = 0; m < _margin; ++m, ++y )
            {
                unsigned char* destPtr = atlasImage->data(
                    static_cast<unsigned int>( source->_x + sourceImage->s() ),
                    static_cast<unsigned int>( y )
                );
                unsigned char* sourcePtr = atlasImage->data(
                    static_cast<unsigned int>( source->_x + sourceImage->s() ),
...
void
Optimizer::TextureAtlasVisitor::reset()
{
    _statesetMap.clear();
...
bool
Optimizer::TextureAtlasVisitor::pushStateSet( osg::StateSet* stateset )
{
    ...
    else
    {
        ...
        for( unsigned int unit = 0; unit < tal.size(); ++unit )
        {
            osg::Texture2D* texture2D = dynamic_cast<osg::Texture2D*>(
                stateset->getTextureAttribute( unit, osg::StateAttribute::Type::TEXTURE )
            );
            if( texture2D )
            {
...
void
Optimizer::TextureAtlasVisitor::popStateSet()
{
    _statesetStack.pop_back();
}

void
Optimizer::TextureAtlasVisitor::apply( osg::Node& node )
{
...
void
Optimizer::TextureAtlasVisitor::apply( osg::Drawable& node )
{
    bool           pushedStateState = false;
...
void
Optimizer::TextureAtlasVisitor::optimize()
{
    _builder.reset();
    ...
    for( sitr = _statesetMap.begin(); sitr != _statesetMap.end(); ++sitr )
    {
        osg::StateSet*                             stateset  = sitr->first;
        Drawables&                                 drawables = sitr->second;

        const osg::StateSet::TextureAttributeList& tal =
        ...
        for( unsigned int unit = 0; unit < tal.size(); ++unit )
        {
            ...
            if( texture )
            {
                bool s_repeat = texture->getWrap( osg::Texture2D::WRAP_S ) ==
                                osg::Texture2D::REPEAT ||
                                texture->getWrap( osg::Texture2D::WRAP_S ) ==
                                osg::Texture2D::MIRROR;

                bool t_repeat = texture->getWrap( osg::Texture2D::WRAP_T ) ==
                                osg::Texture2D::REPEAT ||
                ...
                if( s_repeat || t_repeat )
                {
                         ...
                         ++ditr )
                    {
                        ...
                        osg::Vec2Array* texcoords =
                            geom ? dynamic_cast<osg::Vec2Array*>(
                                       geom->getTexCoordArray( unit )
                                   )
                                 : 0;
                        if( texcoords && !texcoords->empty() )
                        {
                            for( osg::Vec2Array::iterator titr = texcoords->begin();
                                 titr !=
                                 texcoords->end() /*&& !s_outOfRange && !t_outOfRange*/;
                                 ++titr )
                            {
                                osg::vec2 tc = *titr;
                                if( tc[0] < s_min )
                        ...
                        {
                            // if no texcoords then texgen must be being used, therefore
                            // must assume that texture is truly repeating
                            s_outOfRange = true;
                            t_outOfRange = true;
                        }
                    }
    ...
    for( Textures::iterator titr = _textures.begin(); titr != _textures.end(); ++titr )
    {
        osg::Texture2D* texture  = *titr;
    ...
    {
        Drawables& drawables = sitr->second;
        for( Drawables::iterator ditr = drawables.begin(); ditr != drawables.end();
             ++ditr )
        {
    ...
    for( DrawableStateSetMap::iterator ditr = dssm.begin(); ditr != dssm.end(); ++ditr )
    {
        ...
        if( ssm.size() > 1 )
        {
            ...
            for( StateSetSet::iterator ssm_itr = ssm.begin(); ssm_itr != ssm.end();
                 ++ssm_itr )
            {
                osg::StateSet* ss = *ssm_itr;
                unsigned int   numTextureUnits =
                    static_cast<unsigned int>( ss->getTextureAttributeList().size() );
                for( unsigned int unit = 0; unit < numTextureUnits; ++unit )
                {
                    osg::Texture2D* texture = dynamic_cast<osg::Texture2D*>(
                        ss->getTextureAttribute( unit,
        ...
        for( unsigned int unit = 0; unit < tal.size(); ++unit )
        {
            ...
            if( texture )
            {
                ...
                if( newTexture && newTexture != texture )
                {
                    ...
                    osg::dmat4 matrix    = _builder.getTextureMatrix( texture );

                    // first check to see if all drawables are ok for applying
                    // texturematrix to.
                         ...
                         ++ditr )
                    {
                        ...
                        if( drawablesThatHaveMultipleTexturesOnOneUnit.count( *ditr ) !=
                            0 )
                        {
                            canTexMatBeFlattenedToAllDrawables = false;
                        }
                    ...
                    {
                        // OSG_NOTICE<<"All drawables can be flattened
                        // "<<drawables.size()<<std::endl;
                        for( Drawables::iterator ditr = drawables.begin();
                             ditr != drawables.end();
                             ++ditr )
                        {
                            ...
                            if( texcoords )
                            {
                                ...
                                {
                                    osg::vec2 tc = *titr;
                                    ( *titr ).set( static_cast<float>( tc[0] *
                                                                       matrix( 0, 0 ) +
                                                                       tc[1] *
                                                                       matrix( 1, 0 ) +
                                                                       matrix( 3, 0 ) ),
                                                   static_cast<float>( tc[0] *
                                                                       matrix( 0, 1 ) +
                                                                       tc[1] *
                                                                       matrix( 1, 1 ) +
                                                                       matrix( 3,
...
void
Optimizer::StaticObjectDetectionVisitor::apply( osg::Node& node )
{
    if( node.getStateSet() )
    {
...
void
Optimizer::StaticObjectDetectionVisitor::apply( osg::Drawable& drawable )
{
...
void
Optimizer::StaticObjectDetectionVisitor::applyStateSet( osg::StateSet& stateset )
{
...
void
Optimizer::FlattenStaticTransformsDuplicatingSharedSubgraphsVisitor::reset()
{
    _matrixStack.clear();
}

void
Optimizer::FlattenStaticTransformsDuplicatingSharedSubgraphsVisitor::apply(
    osg::Group& group
)
{
    // only continue if there is a parent
    const unsigned int nodepathsize = static_cast<unsigned int>( _nodePath.size() );
    if( !_matrixStack.empty() && group.getNumParents() > 1 && nodepathsize > 1 )
    {
        // copy this Group
        osg::ref_ptr<osg::Group> new_group =
            osg::clone( &group,
        ...
        osg::Group* parent_group =
            dynamic_cast<osg::Group*>( _nodePath[nodepathsize - 2] );
...
)
{
        ...
        nodepathsize > 1 )
    {
        ...
        osg::ref_ptr<osg::Group> group =
            new osg::Group( dynamic_cast<osg::Group&>( transform ),
...
)
{
    ...
    if( !_matrixStack.empty() && lod.getNumParents() > 1 && nodepathsize > 1 )
    {
        ...
        if( parent_group )
        {
            ...
            if( !_matrixStack.empty() )
            {
                new_lod->setCenter( osg::vec3( _matrixStack.back() *
                                               osg::dvec3( new_lod->getCenter() ) ) );
            }
    ...
    else
    {
        ...
        if( !_matrixStack.empty() )
        {
            lod.setCenter( osg::vec3( _matrixStack.back() *
                                      osg::dvec3( lod.getCenter() ) ) );
        }
...
)
{
    if( !_matrixStack.empty() )
    {
        ...
        else
        {
            ...
            const unsigned int nodepathsize =
                static_cast<unsigned int>( _nodePath.size() );
            if( nodepathsize > 1 )
            {
                // convert this Transform to a Group
                osg::ref_ptr<osg::Geode> new_geode =
                    new osg::Geode( geode,
                                    osg::CopyOp::DEEP_COPY_NODES |
                                        osg::CopyOp::DEEP_COPY_DRAWABLES |
                                        osg::CopyOp::DEEP_COPY_ARRAYS );
...
)
{
    if( !_matrixStack.empty() )
    {
        ...
        else
        {
            ...
            {
                // convert this Transform to a Group
                osg::ref_ptr<osg::Billboard> new_billboard =
                    new osg::Billboard( billboard,
                                        osg::CopyOp::DEEP_COPY_NODES |
                                            osg::CopyOp::DEEP_COPY_DRAWABLES |
                                            osg::CopyOp::DEEP_COPY_ARRAYS );

                // New Billboard should only be added to parent through which this
                // Billboard was traversed, not to all parents of this Billboard.
                osg::Group* parent_group =
                    dynamic_cast<osg::Group*>( _nodePath[nodepathsize - 2] );
                if( parent_group )
                {
...
void
Optimizer::FlattenStaticTransformsDuplicatingSharedSubgraphsVisitor::transformGeode(
    osg::Geode& geode
)
{
...
void
Optimizer::FlattenStaticTransformsDuplicatingSharedSubgraphsVisitor::transformDrawable(
...
)
{
    ...
    if( geometry )
    {
        ...
        if( verts3 )
        {
            ...
            {
                ( *verts3 )[j] =
                    osg::vec3( _matrixStack.back() * osg::dvec3( ( *verts3 )[j] ) );
            }
        }
        else
        {
            osg::Vec4Array* verts =
                dynamic_cast<osg::Vec4Array*>( geometry->getVertexArray() );
            if( verts )
            {
        ...
        if( normals )
        {
            ...
            {
                ( *normals )[j] =
                    osg::transform3x3( ( *normals )[j], _matrixStack.back() );
            }
        }
...
void
Optimizer::FlattenStaticTransformsDuplicatingSharedSubgraphsVisitor::transformBillboard(
    osg::Billboard& billboard
)
{
    osg::vec3 axis = osg::transform3x3( billboard.getAxis(), _matrixStack.back() );
    axis           = osg::normalize( axis );
    billboard.setAxis( axis );

    osg::vec3 normal = osg::transform3x3( billboard.getNormal(), _matrixStack.back() );
    normal           = osg::normalize( normal );
    billboard.setNormal( normal );
    ...
    for( unsigned int i = 0; i < billboard.getNumDrawables(); i++ )
    {
        ...
        billboard.setPosition( i,
                               osg::vec3( _matrixStack.back() *
                                          osg::dvec3( originalBillboardPosition ) ) );

        osg::dmat4 matrixForDrawable = _matrixStack.back();
        matrixForDrawable = osg::translate( -osg::dvec3( billboard.getPosition( i ) ) ) *
                            matrixForDrawable *
                            osg::translate( osg::dvec3( originalBillboardPosition ) );
...
void
Optimizer::BufferObjectVisitor::apply( osg::Geometry& geometry )
{
    ...
    {
        OSG_NOTICE << "geometry.setUseVertexBufferObjects(" << _valueVertexBufferObject
                   << ")" << std::endl;
