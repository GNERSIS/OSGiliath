...
template<class T>
bool
dereference_check_less( const T& lhs,
                        const T& rhs )
{
    if( lhs == rhs )
    {
...
class EdgeCollapse
{
    public:
        ...
        void
        setGeometry( osg::Geometry*               geometry,
                     const Simplifier::IndexList& protectedPoints );
        ...
        void
        setComputeErrorMetricUsingLength( bool flag )
        {
            _computeErrorMetricUsingLength = flag;
        }

        bool
        getComputeErrorMetricUsingLength() const
        {
        ...
        getNumOfTriangles()
        {
            return static_cast<unsigned int>( _triangleSet.size() );
        }

        Point*
        computeInterpolatedPoint( Edge* edge,
                                  float r ) const
        {
            ...
            if( p1 == 0 || p2 == 0 )
            {
                OSG_NOTICE << "Error computeInterpolatedPoint(" << edge
                           << ",r) p1 and/or p2==0" << std::endl;
                return 0;
            }
            ...
            unsigned int s =
                static_cast<unsigned int>( std::min( p1->_attributes.size(),
                                                     p2->_attributes.size() ) );
            for( unsigned int i = 0; i < s; ++i )
            {
                point->_attributes.push_back(
                    p1->_attributes[i] * r1 + p2->_attributes[i] * r2
                );
            }
            return point;
        }
        ...
        Point*
        computeOptimalPoint( Edge* edge ) const
        {
            return computeInterpolatedPoint( edge, 0.5F );
        }

        error_type
        computeErrorMetric( Edge*  edge,
                            Point* point ) const
        {
            if( _computeErrorMetricUsingLength )
            {
                error_type dx = error_type( edge->_p1->_vertex.x ) -
                                error_type( edge->_p2->_vertex.x );
                error_type dy = error_type( edge->_p1->_vertex.y ) -
                                error_type( edge->_p2->_vertex.y );
                error_type dz = error_type( edge->_p1->_vertex.z ) -
                                error_type( edge->_p2->_vertex.z );
                return static_cast<error_type>( sqrt( dx * dx + dy * dy + dz * dz ) );
            }
            else if( point )
            {
                typedef std::set<osg::ref_ptr<Triangle>> LocalTriangleSet;
                LocalTriangleSet                         triangles;
                std::copy( edge->_p1->_triangles.begin(),
                           edge->_p1->_triangles.end(),
                           std::inserter( triangles, triangles.begin() ) );
                std::copy( edge->_p2->_triangles.begin(),
                           edge->_p2->_triangles.end(),
                           std::inserter( triangles, triangles.begin() ) );
                ...
                {
                    error +=
                        static_cast<error_type>( fabs( ( *itr )->distance( vertex ) ) );
                }
        ...
        void
        updateErrorMetricForEdge( Edge* edge )
        {
            if( !edge->_p1 || !edge->_p2 )
            {
                OSG_NOTICE << "Error updateErrorMetricForEdge(" << edge
                           << ") p1 and/or p2==0" << std::endl;
                return;
            }
            ...
            if( _computeErrorMetricUsingLength )
            {
                edge->setErrorMetric( computeErrorMetric( edge,
                                                          edge->_proposedPoint.get() ) );
            }
            else
            {
                edge->updateMaxNormalDeviationOnEdgeCollapse();

                if( edge->getMaxNormalDeviationOnEdgeCollapse() <=
                    1.0 &&
                    !edge->isAdjacentToBoundary() )
                {
                    edge->setErrorMetric(
                        computeErrorMetric( edge, edge->_proposedPoint.get() )
                    );
                }
                else
                {
        ...
        updateErrorMetricForAllEdges()
        {
            ...
            for( LocalEdgeList::iterator itr = edges.begin(); itr != edges.end(); ++itr )
            {
                Edge* edge = itr->get();
        ...
        collapseMinimumErrorEdge()
        {
            if( !_edgeSet.empty() )
            {
                ...
                {
                    OSG_INFO << "collapseMinimumErrorEdge() return false due to "
                                "edge->getErrorMetric()==FLT_MAX"
                             << std::endl;
                    return false;
                }
                ...
                osg::ref_ptr<Point> pNew = edge->_proposedPoint.valid()
                                             ? edge->_proposedPoint.get()
                                             : computeInterpolatedPoint( edge, 0.5F );
                return ( collapseEdge( edge, pNew.get() ) );
            }
            OSG_INFO << "collapseMinimumErrorEdge() return false due to _edgeSet.empty()"
                     << std::endl;
            return false;
        }
        ...
        divideLongestEdge()
        {
            if( !_edgeSet.empty() )
            {
                ...
                if( edge->getErrorMetric() == FLT_MAX )
                {
                    OSG_INFO << "divideLongestEdge() return false due to "
                                "edge->getErrorMetric()==FLT_MAX"
                             << std::endl;
                    return false;
                }
            ...
            OSG_INFO << "divideLongestEdge() return false due to _edgeSet.empty()"
                     << std::endl;
            return false;
        }
        ...
        struct Point : public osg::Referenced
        {
                ...
                isBoundaryPoint() const
                {
                         ...
                         ++itr )
                    {
                        ...
                        if( ( triangle->_e1->_p1 ==
                              this ||
                              triangle->_e1->_p2 == this ) &&
                            triangle->_e1->isBoundaryEdge() )
                        {
                            return true;
                        }
                        if( ( triangle->_e2->_p1 ==
                              this ||
                              triangle->_e2->_p2 == this ) &&
                            triangle->_e2->isBoundaryEdge() )
                        {
                            return true;
                        }
                        if( ( triangle->_e3->_p1 ==
                              this ||
                              triangle->_e3->_p2 == this ) &&
                            triangle->_e3->isBoundaryEdge() )
                        {
                            return true;
                        }
        ...
        struct Edge : public osg::Referenced
        {
                ...
                void
                setErrorMetric( error_type errorMetric )
                {
                    _errorMetric = errorMetric;
                }

                error_type
                getErrorMetric() const
                {
                    return _errorMetric;
                }
                ...
                addTriangle( Triangle* triangle )
                {
                    ...
                    // if (_triangles.size()>2) OSG_NOTICE<<"Warning too many triangles
                    // ("<<_triangles.size()<<") sharing edge "<<std::endl;
                }
                ...
                void
                updateMaxNormalDeviationOnEdgeCollapse()
                {
                         ...
                         ++itr1 )
                    {
                        ...
                        {
                            _maximumDeviation = std::max(
                                _maximumDeviation,
                                ( *itr1 )->computeNormalDeviationOnEdgeCollapse(
                                    this,
                                    _proposedPoint.get()
                                )
                            );
                         ...
                         ++itr2 )
                    {
                        if( _triangles.count( *itr2 ) == 0 )
                        {
                            _maximumDeviation = std::max(
                                _maximumDeviation,
                                ( *itr2 )->computeNormalDeviationOnEdgeCollapse(
                                    this,
                                    _proposedPoint.get()
                                )
                            );
                ...
                error_type
                getMaxNormalDeviationOnEdgeCollapse() const
                {
        ...
        struct Triangle : public osg::Referenced
        {
                ...
                operator<( const Triangle& rhs ) const
                {
                    ...
                    const Point* lhs_lower =
                        dereference_check_less( _p2, _p3 ) ? _p2.get() : _p3.get();
                    const Point* rhs_lower = dereference_check_less( rhs._p2, rhs._p3 )
                                               ? rhs._p2.get()
                                               : rhs._p3.get();
                    ...
                    const Point* lhs_upper =
                        dereference_check_less( _p2, _p3 ) ? _p3.get() : _p2.get();
                    const Point* rhs_upper = dereference_check_less( rhs._p2, rhs._p3 )
                                               ? rhs._p3.get()
                                               : rhs._p2.get();
                ...
                {
                    {
                        osg::vec3 __n = osg::cross( _p2->_vertex - _p1->_vertex,
                                                    _p3->_vertex - _p1->_vertex );
                        _plane.set( osg::dvec3( __n ), osg::dvec3( _p1->_vertex ) );
                    }
                }

                osg::Plane
                computeNewPlaneOnEdgeCollapse( Edge*  edge,
                                               Point* pNew ) const
                {
                    const Point* p1 =
                        ( _p1 == edge->_p1 || _p1 == edge->_p2 ) ? pNew : _p1.get();
                    const Point* p2 =
                        ( _p2 == edge->_p1 || _p2 == edge->_p2 ) ? pNew : _p2.get();
                    const Point* p3 =
                        ( _p3 == edge->_p1 || _p3 == edge->_p2 ) ? pNew : _p3.get();

                    return osg::Plane( osg::dvec3( osg::cross( p2->_vertex - p1->_vertex,
                                                               p3->_vertex -
                                                                   p1->_vertex ) ),
                                       osg::dvec3( p1->_vertex ) );
                }

                // note return 1 - dotproduct, so that deviation is in the range of 0.0
                // to 2.0, where 0 is coincident, 1.0 is 90 degrees, and 2.0 is 180
                // degrees.
                error_type
                computeNormalDeviationOnEdgeCollapse( Edge*  edge,
                                                      Point* pNew ) const
                {
                    const Point* p1 =
                        ( _p1 == edge->_p1 || _p1 == edge->_p2 ) ? pNew : _p1.get();
                    ...
                    osg::vec3 new_normal = osg::cross( p2->_vertex - p1->_vertex,
                                                       p3->_vertex - p2->_vertex );
                    new_normal           = osg::normalize( new_normal );

                    error_type result    = 1.0F - static_cast<float>( new_normal.x *
                                                                      _plane[0] +
                                                                      new_normal.y *
                                                                      _plane[1] +
                                                                      new_normal.z *
                ...
                error_type
                distance( const osg::vec3& vertex ) const
                {
                    return error_type( _plane[0] ) *
                           error_type( vertex.x ) +
                           error_type( _plane[1] ) *
                           error_type( vertex.y ) +
                           error_type( _plane[2] ) *
                ...
                bool
                isBoundaryTriangle() const
                {
                    return ( _e1->isBoundaryEdge() ||
                             _e2->isBoundaryEdge() ||
                             _e3->isBoundaryEdge() );
                }
                     ...
                     unsigned int p3 )
        {
            ...
            triangle->_e1 =
                addEdge( triangle, triangle->_p1.get(), triangle->_p2.get() );
            triangle->_e2 =
                addEdge( triangle, triangle->_p2.get(), triangle->_p3.get() );
            triangle->_e3 =
                addEdge( triangle, triangle->_p3.get(), triangle->_p1.get() );
                     ...
                     Point* p3 )
        {
            ...
            if( p1 == p2 || p2 == p3 || p1 == p3 )
            {
                // OSG_NOTICE<<"    **** addTriangle failed - p1==p2 || p2==p3 ||
                // p1==p3"<<std::endl;
                return 0;
            }
        ...
        void
        replaceTrianglePoint( Triangle* triangle,
                              Point*    pOriginal,
                              Point*    pNew )
        {
            if( triangle->_p1 ==
                ...
                triangle->_p3 == pOriginal )
            {
                ...
                // remove the triangle form the original point, and possibly the point if
                // its the last triangle to use it
        ...
        unsigned int
        testTriangle( Triangle* triangle )
        {
            ...
            {
                OSG_NOTICE << "testTriangle(" << triangle
                           << ") _p1->_triangles does not contain triangle" << std::endl;
                ++result;
            }
            ...
            {
                OSG_NOTICE << "testTriangle(" << triangle
                           << ") _p2->_triangles does not contain triangle" << std::endl;
                ++result;
            }
            ...
            {
                OSG_NOTICE << "testTriangle(" << triangle
                           << ") _p3->_triangles does not contain triangle" << std::endl;
                ++result;
            }
            ...
            {
                ++result;
                OSG_NOTICE << "testTriangle(" << triangle << ") _e1 test failed"
                           << std::endl;
            }
            ...
            {
                ++result;
                OSG_NOTICE << "testTriangle(" << triangle << ") _e2 test failed"
                           << std::endl;
            }

            if( testEdge( triangle->_e3.get() ) )
            {
                OSG_NOTICE << "testTriangle(" << triangle << ") _e3 test failed"
                           << std::endl;
                ++result;
            }
                 ...
                 Point*    p2 )
        {
            ...
            if( itr == _edgeSet.end() )
            {
                // OSG_NOTICE<<"          addEdge("<<edge.get()<<")
                // edge->_p1="<<edge->_p1.get()<<" _p2="<<edge->_p2.get()<<std::endl;
                _edgeSet.insert( edge );
            }
            else
            {
                // OSG_NOTICE<<"          reuseEdge("<<edge.get()<<")
                // edge->_p1="<<edge->_p1.get()<<" _p2="<<edge->_p2.get()<<std::endl;
                edge = *itr;
            }
                      ...
                      Point* pNew )
        {
            ...
            if( edge->getMaxNormalDeviationOnEdgeCollapse() > 1.0 )
            {
                OSG_NOTICE
                    << "collapseEdge(" << edge
                    << ") refused due to edge->getMaxNormalDeviationOnEdgeCollapse() = "
                    << edge->getMaxNormalDeviationOnEdgeCollapse() << std::endl;
                return false;
            }
            else
            {
                // OSG_NOTICE<<"collapseEdge("<<edge<<")
                // edge->getMaxNormalDeviationOnEdgeCollapse() =
                // "<<edge->getMaxNormalDeviationOnEdgeCollapse()<<std::endl;
            }
#endif
            ...
            osg::ref_ptr<Edge>
                keep_edge_locally_referenced_to_prevent_premature_deletion = edge;
            osg::ref_ptr<Point>
                keep_point_locally_referenced_to_prevent_premature_deletion = pNew;
            ...
            for( titr_p1 = triangles_p1.begin(); titr_p1 != triangles_p1.end();
                 ++titr_p1 )
            {
                removeTriangle( const_cast<Triangle*>( titr_p1->get() ) );
            }

            for( titr_p2 = triangles_p2.begin(); titr_p2 != triangles_p2.end();
                 ++titr_p2 )
            {
            ...
            // OSG_NOTICE<<"
            // pNew="<<pNew<<"\tedge_p1"<<edge_p1.get()<<"\tedge_p2"<<edge_p2.get()<<std::endl;

            // we copy the edge's _triangles and iterate the copy of the triangle set to
                 ...
                 ++titr_p1 )
            {
                ...
                Point*    p1 = ( triangle->_p1 == edge_p1 || triangle->_p1 == edge_p2 )
                                 ? pNew
                                 : triangle->_p1.get();
                Point*    p2 = ( triangle->_p2 == edge_p1 || triangle->_p2 == edge_p2 )
                                 ? pNew
                                 : triangle->_p2.get();
                Point*    p3 = ( triangle->_p3 == edge_p1 || triangle->_p3 == edge_p2 )
                                 ? pNew
                                 : triangle->_p3.get();
            ...
            // OSG_NOTICE<<"Edges to recalibarate
            // "<<edges2UpdateErrorMetric.size()<<std::endl;
        ...
        bool
        divideEdge( Edge*  edge,
                    Point* pNew )
        {
            // OSG_NOTICE<<"divideEdge("<<edge<<") before
            // _edgeSet.size()="<<_edgeSet.size()<<"
            // _triangleSet.size()="<<_triangleSet.size()<<std::endl;
            ...
            for( titr = triangles.begin(); titr != triangles.end(); ++titr )
            {
                ...
                switch( edgeToReplace )
                {
                    case( 0 ) :    // error, shouldn't get here.
                        OSG_NOTICE << "Error EdgeCollapse::divideEdge(Edge*,Point*) "
                                      "passed invalid edge."
                                   << std::endl;
                        return false;
                    case( 1 ) :    // p1, p2
                        // OSG_NOTICE<<"   // p1, p2 "<<std::endl;
                        // OSG_NOTICE<<"   newTri1 = addTriangle(tri->_p1.get(), pNew,
                        // tri->_p3.get());"<<std::endl;
                        newTri1 = addTriangle( tri->_p1.get(), pNew, tri->_p3.get() );
                        // OSG_NOTICE<<"   newTri2 = addTriangle(pNew, tri->_p2.get(),
                        // tri->_p3.get());"<<std::endl;
                        newTri2 = addTriangle( pNew, tri->_p2.get(), tri->_p3.get() );
                        break;
                    case( 2 ) :    // p2, p3
                        // OSG_NOTICE<<"   // p2, p3"<<std::endl;
                        // OSG_NOTICE<<"   newTri1 = addTriangle(tri->_p1.get(),
                        // tri->_p2.get(), pNew);"<<std::endl;
                        newTri1 = addTriangle( tri->_p1.get(), tri->_p2.get(), pNew );
                        // OSG_NOTICE<<"   newTri2 = addTriangle(tri->_p1.get(), pNew,
                        // tri->_p3.get());"<<std::endl;
            ...
            // OSG_NOTICE<<"   numTriangles1="<<numTriangles1<<"
            // numTriangles2="<<numTriangles2<<"
            // numTriangles3="<<numTriangles3<<std::endl; OSG_NOTICE<<"
            // numEdges1="<<numEdges1<<"   numEdges2="<<numEdges2<<"
            // numEdges3="<<numEdges3<<std::endl; OSG_NOTICE<<"
            // numBoundaryEdges1="<<numBoundaryEdges1<<"
            // numBoundaryEdges2="<<numBoundaryEdges2<<"
            // numBoundaryEdges3="<<numBoundaryEdges3<<std::endl;
            // OSG_NOTICE<<"divideEdge("<<edge<<") after
            // _edgeSet.size()="<<_edgeSet.size()<<"
            // _triangleSet.size()="<<_triangleSet.size()<<std::endl;
        ...
        testEdge( Edge* edge )
        {
                 ...
                 ++teitr )
            {
                ...
                if( !( triangle->_e1 ==
                       ...
                       triangle->_e3 == edge ) )
                {
                    OSG_NOTICE << "testEdge(" << edge
                               << "). triangle != point back to this edge" << std::endl;
                    OSG_NOTICE
                        << "                     triangle->_e1==" << triangle->_e1.get()
                        << std::endl;
                    OSG_NOTICE
                        << "                     triangle->_e2==" << triangle->_e2.get()
                        << std::endl;
                    OSG_NOTICE
                        << "                     triangle->_e3==" << triangle->_e3.get()
                        << std::endl;
                    ++numErrors;
                }
            }
            ...
            if( edge->_triangles.empty() )
            {
                OSG_NOTICE << "testEdge(" << edge << ")._triangles is empty"
                           << std::endl;
                ++numErrors;
            }
            return numErrors;
        ...
        {
            unsigned int numErrors = 0;
            for( EdgeSet::iterator itr = _edgeSet.begin(); itr != _edgeSet.end(); ++itr )
            {
        ...
        testPoint( Point* point )
        {
                 ...
                 ++itr )
            {
                ...
                if( !( triangle->_p1 ==
                       ...
                       triangle->_p3 == point ) )
                {
                    OSG_NOTICE << "testPoint(" << point << ") error, triangle "
                               << triangle << " does not point back to this point"
                               << std::endl;
                    OSG_NOTICE << "             triangle->_p1 " << triangle->_p1.get()
                               << std::endl;
                    OSG_NOTICE << "             triangle->_p2 " << triangle->_p2.get()
                               << std::endl;
                    OSG_NOTICE << "             triangle->_p3 " << triangle->_p3.get()
                               << std::endl;
                    ++numErrors;
                }
            }
        ...
        {
            unsigned int numErrors = 0;
            for( PointSet::iterator itr = _pointSet.begin(); itr != _pointSet.end();
                 ++itr )
            {
...
struct CollectTriangleOperator
{
        ...
        void
        setEdgeCollapse( EdgeCollapse* ec )
        {
            _ec = ec;
        }
...
EdgeCollapse::~EdgeCollapse()
{
    ...
    std::for_each( _originalPointList.begin(),
                   _originalPointList.end(),
                   dereference_clear() );
}
...
class CopyArrayToPointsVisitor : public osg::ArrayVisitor
{
    public:
        ...
        virtual void
        apply( osg::ByteArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::ShortArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::IntArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::UByteArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::UShortArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::UIntArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::FloatArray& array )
        {
            copy( array );
        }

        virtual void
        apply( osg::Vec4ubArray& array )
        {
            if( _pointList.size() != array.size() )
            {
                return;
    ...
    protected:
        ...
        CopyArrayToPointsVisitor&
        operator=( const CopyArrayToPointsVisitor& )
        {
            return *this;
        }
};
...
class CopyVertexArrayToPointsVisitor : public osg::ArrayVisitor
{
    ...
    protected:
        ...
        CopyVertexArrayToPointsVisitor&
        operator=( const CopyVertexArrayToPointsVisitor& )
        {
            return *this;
        }
};
...
void
EdgeCollapse::setGeometry( osg::Geometry*               geometry,
                           const Simplifier::IndexList& protectedPoints )
{
    _geometry = geometry;
    ...
    if( _geometry->containsSharedArrays() )
    {
        ...
        OSG_INFO << "EdgeCollapse::setGeometry(..): Duplicate shared arrays"
                 << std::endl;
    ...
    if( _geometry->getNormalArray() &&
        _geometry->getNormalArray()->getBinding() == osg::Array::BIND_PER_VERTEX )
    {
        geometry->getNormalArray()->accept( copyArrayToPoints );
    }

    if( _geometry->getColorArray() &&
        _geometry->getColorArray()->getBinding() == osg::Array::BIND_PER_VERTEX )
    {
        geometry->getColorArray()->accept( copyArrayToPoints );
    }

    if( _geometry->getSecondaryColorArray() &&
        _geometry->getSecondaryColorArray()->getBinding() ==
        osg::Array::BIND_PER_VERTEX )
    {
        geometry->getSecondaryColorArray()->accept( copyArrayToPoints );
    }

    if( _geometry->getFogCoordArray() &&
        _geometry->getFogCoordArray()->getBinding() == osg::Array::BIND_PER_VERTEX )
    {
    ...
    {
        if( _geometry->getVertexAttribArray( vi ) &&
            _geometry->getVertexAttribArray( vi )->getBinding() ==
            osg::Array::BIND_PER_VERTEX )
        {
...
class CopyPointsToArrayVisitor : public osg::ArrayVisitor
{
    public:
        ...
        // use local typedefs if usinged char,short and int to get round gcc 3.3.1
        // problem with defining unsigned short()
    ...
    protected:
        ...
        operator=( CopyPointsToArrayVisitor& )
        {
            return *this;
        }
};
...
class CopyPointsToVertexArrayVisitor : public osg::ArrayVisitor
{
    ...
    protected:
        ...
        CopyPointsToVertexArrayVisitor&
        operator=( const CopyPointsToVertexArrayVisitor& )
        {
            return *this;
        }
};
...
void
EdgeCollapse::copyBackToGeometry()
{

    // rebuild the _pointList from the _pointSet
    _originalPointList.clear();
    std::copy( _pointSet.begin(),
               _pointSet.end(),
               std::back_inserter( _originalPointList ) );
    ...
    for( TriangleSet::iterator itr = _triangleSet.begin(); itr != _triangleSet.end();
         ++itr )
    {
        trianglesSorted.insert( *itr );
    }

    osg::DrawElementsUInt* primitives = new osg::DrawElementsUInt(
        GL_TRIANGLES,
        static_cast<unsigned int>( trianglesSorted.size() * 3 )
    );
    unsigned int pos = 0;
...
Simplifier::Simplifier( double sampleRatio,
                        double maximumError,
...
void
Simplifier::simplify( osg::Geometry& geometry )
{
                      ...
                      const IndexList& protectedPoints )
{
    ...
    unsigned int numOriginalPrimitives =
        static_cast<unsigned int>( static_cast<unsigned int>( ec._triangleSet.size() ) );

    if( downSample )
    {
        while( !ec._edgeSet.empty() &&
               continueSimplification(
                   ( *ec._edgeSet.begin() )->getErrorMetric(),
                   numOriginalPrimitives,
                   static_cast<unsigned int>( ec._triangleSet.size() )
               ) &&
               ec.collapseMinimumErrorEdge() )
        {
            // OSG_INFO<<"   Collapsed edge
            // ec._triangleSet.size()="<<ec._triangleSet.size()<<"
            // error="<<(*ec._edgeSet.begin())->getErrorMetric()<<" vs
            // "<<getMaximumError()<<std::endl;
        }

        OSG_INFO << "******* AFTER EDGE COLLAPSE *********" << ec._triangleSet.size()
                 << std::endl;
    }
    else
    {
        ...
        while( !ec._edgeSet.empty() &&
               continueSimplification(
                   ( *ec._edgeSet.rbegin() )->getErrorMetric(),
                   numOriginalPrimitives,
                   static_cast<unsigned int>( ec._triangleSet.size() )
               ) &&
        ...
        {
            // OSG_INFO<<"   Edge divided
            // ec._triangleSet.size()="<<ec._triangleSet.size()<<"
            // error="<<(*ec._edgeSet.rbegin())->getErrorMetric()<<" vs
            // "<<getMaximumError()<<std::endl;
        }
        OSG_INFO << "******* AFTER EDGE DIVIDE *********" << ec._triangleSet.size()
                 << std::endl;
    }

    OSG_INFO << "Number of triangle errors after edge collapse= "
             << ec.testAllTriangles() << std::endl;
    OSG_INFO << "Number of edge errors before edge collapse= " << ec.testAllEdges()
             << std::endl;
    OSG_INFO << "Number of point errors after edge collapse= " << ec.testAllPoints()
             << std::endl;
    ...
    OSG_INFO << "Number of boundary edges= " << ec.computeNumBoundaryEdges()
             << std::endl;

    if( !ec._edgeSet.empty() )
    {
        OSG_INFO << std::endl
                 << "Simplifier, in = " << numOriginalPrimitives
                 << "\tout = " << ec._triangleSet.size()
                 << "\terror=" << ( *ec._edgeSet.begin() )->getErrorMetric() << "\tvs "
                 << getMaximumError() << std::endl
                 << std::endl;
        OSG_INFO << "        !ec._edgeSet.empty()  = " << !ec._edgeSet.empty()
                 << std::endl;
        OSG_INFO << "        continueSimplification(,,)  = "
                 << continueSimplification(
                        ( *ec._edgeSet.begin() )->getErrorMetric(),
                        numOriginalPrimitives,
                        static_cast<unsigned int>( ec._triangleSet.size() )
