 ...
 * Collects edge connectivity from triangle meshes.
 * Used by simplification and adjacency analysis algorithms.
 */
#include <osgUtil/optimization/EdgeCollector.hpp>

#include <osg/geometry/TriangleIndexFunctor.hpp>
#include <osg/maths/common.hpp>
#include <osgUtil/utils/ConvertVec.hpp>

namespace osgUtil
{

    bool
    EdgeCollector::Point::isBoundaryPoint() const
    {
        if( _protected )
        {
            return true;
             ...
             ++itr )
        {
            ...
            if( ( triangle->_e1->_p1 == this || triangle->_e1->_p2 == this ) &&
                triangle->_e1->isBoundaryEdge() )
            {
                return true;
            }
            if( ( triangle->_e2->_p1 == this || triangle->_e2->_p2 == this ) &&
                triangle->_e2->isBoundaryEdge() )
            {
                return true;
            }
            if( ( triangle->_e3->_p1 == this || triangle->_e3->_p2 == this ) &&
    ...
    bool
    EdgeCollector::Edge::operator<( const Edge& rhs ) const
    {
    ...
    void
    EdgeCollector::Edge::setOrderedPoints( Point* p1,
                                           Point* p2 )
    {
    ...
    bool
    EdgeCollector::Triangle::operator<( const Triangle& rhs ) const
    {
        if( dereference_check_less( _p1, rhs._p1 ) )
        {
            return true;
        ...
        const Point* rhs_lower =
            dereference_check_less( rhs._p2, rhs._p3 ) ? rhs._p2.get() : rhs._p3.get();
        ...
        const Point* rhs_upper =
            dereference_check_less( rhs._p2, rhs._p3 ) ? rhs._p3.get() : rhs._p2.get();

        return dereference_check_less( lhs_upper, rhs_upper );
    }

    void
    EdgeCollector::Triangle::setOrderedPoints( Point* p1,
                                               Point* p2,
                                               Point* p3 )
    {
        Point* points[3];
    ...
    osg::UIntArray*
    EdgeCollector::Edgeloop::toIndexArray() const
    {
    ...
    EdgeCollector::addTriangle( unsigned int p1,
                                unsigned int p2,
                                unsigned int p3 )
    {
        ...
        triangle->setOrderedPoints( addPoint( triangle, p1 ),
    ...
    EdgeCollector::Triangle*
    EdgeCollector::addTriangle( Point* p1,
                                Point* p2,
                                Point* p3 )
    {
        // OSG_NOTICE<<"      addTriangle("<<p1<<","<<p2<<","<<p3<<")"<<std::endl;
    ...
    EdgeCollector::Edge*
    EdgeCollector::addEdge( Triangle* triangle,
                            Point*    p1,
                            Point*    p2 )
    {
    ...
    EdgeCollector::Point*
    EdgeCollector::addPoint( Triangle* triangle,
                             Point*    point )
    {
    ...
    void
    EdgeCollector::getBoundaryEdgeList( EdgeList& el )
    {
        for( EdgeSet::iterator it = _edgeSet.begin(), end = _edgeSet.end(); it != end;
             ++it )
        {
    ...
    bool
    EdgeCollector::extractBoundaryEdgeloop( EdgeList& el,
                                            Edgeloop& edgeloop )
    {
        if( el.empty() )
        {
        ...
        while( !done )
        {
            ...
            if( !found )
            {
                OSG_WARN << "extractBoundaryEdgeloop : unable to close edge loop"
                         << std::endl;
                return false;
            }
            else
    ...
    bool
    EdgeCollector::extractBoundaryEdgeloopList( EdgeList&     el,
    ...
    struct CollectTriangleOperator
    {
            ...
            void
            setEdgeCollector( EdgeCollector* ec )
            {
                _ec = ec;
            }
    ...
    typedef osg::TriangleIndexFunctor<CollectTriangleOperator>
        CollectTriangleIndexFunctor;
        ...
        public:
            ...
            apply( osg::Vec2Array& array )
            {
                ...
                for( unsigned int i = 0; i < _pointList.size(); ++i )
                {
                    ...
                    osgUtil::ConvertVec<osg::vec2, osg::dvec3>::convert(
                        array[i],
                        _pointList[i]->_vertex
                    );
                }
            }
            ...
            apply( osg::Vec4Array& array )
            {
                ...
                for( unsigned int i = 0; i < _pointList.size(); ++i )
                {
                    ...
                    osgUtil::ConvertVec<osg::vec4, osg::dvec3>::convert(
                        array[i],
                        _pointList[i]->_vertex
                    );
                }
            ...
            apply( osg::Vec2dArray& array )
            {
                ...
                for( unsigned int i = 0; i < _pointList.size(); ++i )
                {
                    ...
                    osgUtil::ConvertVec<osg::dvec2, osg::dvec3>::convert(
                        array[i],
                        _pointList[i]->_vertex
                    );
                }
            ...
            apply( osg::Vec4dArray& array )
            {
                ...
                for( unsigned int i = 0; i < _pointList.size(); ++i )
                {
                    ...
                    osgUtil::ConvertVec<osg::dvec4, osg::dvec3>::convert(
                        array[i],
                        _pointList[i]->_vertex
                    );
                }
    ...
    void
    EdgeCollector::setGeometry( osg::Geometry* geometry )
    {
        _geometry                = geometry;
    ...
    void
    EdgeCollector::getEdgeloopIndexList( IndexArrayList& ial )
    {
