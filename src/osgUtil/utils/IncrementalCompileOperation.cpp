...
namespace osgUtil
{
    ...
    static osg::ApplicationUsageProxy
        ICO_e1( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                "OSG_MINIMUM_COMPILE_TIME_PER_FRAME <float>",
                "minimum compile time allotted to compiling OpenGL objects per frame in "
                "database pager." );
    static osg::ApplicationUsageProxy UCO_e2(
    ...
    static osg::ApplicationUsageProxy UCO_e3(
        osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
    ...
    void
    StateToCompile::apply( osg::Node& node )
    {
        if( node.getStateSet() )
        {
    ...
    void
    StateToCompile::apply( osg::Drawable& drawable )
    {
        ...
        if( !_markerObject || _markerObject.get() != drawable.getUserData() )
        {
            ...
            if( _markerObject.valid() && drawable.getUserData() == 0 )
            {
    ...
    void
    StateToCompile::apply( osg::StateSet& stateset )
    {
            ...
            ( !_markerObject || _markerObject.get() != stateset.getUserData() ) )
        {
            osg::Program* program = dynamic_cast<osg::Program*>(
                stateset.getAttribute( osg::StateAttribute::Type::PROGRAM )
            );
            if( program &&
                ( !_markerObject || _markerObject.get() != program->getUserData() ) )
            {
                _programs.insert( program );

                // mark the stateset as visited
                if( _markerObject.valid() && program->getUserData() == 0 )
                {
                 ...
                 ++itr )
            {
                ...
                osg::StateAttribute::TypeMemberPair tmp(
            ...
            // mark the stateset as visited
            if( _markerObject.valid() && stateset.getUserData() == 0 )
            {
    ...
    void
    StateToCompile::apply( osg::Texture& texture )
    {
        // don't make any changes if Texture already processed
        if( _markerObject.valid() && _markerObject.get() == texture.getUserData() )
        {
            return;
        }
        ...
        if( _markerObject.valid() && texture.getUserData() == 0 )
        {
    ...
    // CompileOps
    //
    IncrementalCompileOperation::CompileDrawableOp::CompileDrawableOp(
    ...
    double
    IncrementalCompileOperation::CompileDrawableOp::estimatedTimeForCompile(
    ...
    bool
    IncrementalCompileOperation::CompileDrawableOp::compile( CompileInfo& compileInfo )
    {
    ...
    IncrementalCompileOperation::CompileTextureOp::CompileTextureOp(
        osg::Texture* texture
    ) :
        _texture( texture )
    {
    ...
    double
    IncrementalCompileOperation::CompileTextureOp::estimatedTimeForCompile(
        CompileInfo& compileInfo
    ) const
    {
        osg::GraphicsCostEstimator* gce =
    ...
    bool
    IncrementalCompileOperation::CompileTextureOp::compile( CompileInfo& compileInfo )
    {
    ...
    IncrementalCompileOperation::CompileProgramOp::CompileProgramOp(
        osg::Program* program
    ) :
        _program( program )
    {
    ...
    double
    IncrementalCompileOperation::CompileProgramOp::estimatedTimeForCompile(
        CompileInfo& compileInfo
    ) const
    {
        osg::GraphicsCostEstimator* gce =
    ...
    IncrementalCompileOperation::CompileInfo::CompileInfo(
    ...
    void
    IncrementalCompileOperation::CompileList::add( CompileOp* compileOp )
    {
        _compileOps.push_back( compileOp );
    }

    double
    IncrementalCompileOperation::CompileList::estimatedTimeForCompile(
        CompileInfo& compileInfo
    ) const
    {
    ...
    bool
    IncrementalCompileOperation::CompileList::compile( CompileInfo& compileInfo )
    {
             ...
             itr != _compileOps.end() && compileInfo.okToCompile(); )
        {
            ...
            OSG_NOTICE << "IncrementalCompileOperation::CompileList::compile() "
                          "estimatedTimForCompile = "
                       << estimatedCompileCost *
                1000.0
                       << "ms, actual = " << actualCompileCost * 1000.0 << "ms";
    ...
    //
    void
    IncrementalCompileOperation::CompileSet::buildCompileMap( ContextSet&     contexts,
                                                              StateToCompile& stc )
    {
        ...
        for( ContextSet::iterator itr = contexts.begin(); itr != contexts.end(); ++itr )
        {
    ...
    void
    IncrementalCompileOperation::CompileSet::buildCompileMap(
        ContextSet&            contexts,
    ...
    IncrementalCompileOperation::IncrementalCompileOperation() :
        osg::Referenced( true ),
        osg::GraphicsOperation( "IncrementalCompileOperation",
                                true ),
        _flushTimeRatio( 0.5 ),
        ...
        _compileAllTillFrameNumber( 0 )
    {
        ...
        if( ( ptr = getenv( "OSG_FORCE_TEXTURE_DOWNLOAD" ) ) != 0 )
        {
            ...
            OSG_NOTICE << "OSG_FORCE_TEXTURE_DOWNLOAD set to " << useForceTextureDownload
                       << std::endl;
        }
    ...
    void
    IncrementalCompileOperation::assignContexts( Contexts& contexts )
    {
    ...
    void
    IncrementalCompileOperation::removeContexts( Contexts& contexts )
    {
        for( Contexts::iterator itr = contexts.begin(); itr != contexts.end(); ++itr )
        {
            osg::GraphicsContext* gc = *itr;
    ...
    void
    IncrementalCompileOperation::addGraphicsContext( osg::GraphicsContext* gc )
    {
    ...
    void
    IncrementalCompileOperation::removeGraphicsContext( osg::GraphicsContext* gc )
    {
        if( _contexts.count( gc ) != 0 )
        {
            gc->remove( this );
    ...
    bool
    IncrementalCompileOperation::requiresCompile( StateToCompile& stateToCompile )
    {
        return isActive() && !stateToCompile.empty();
    }

    void
    IncrementalCompileOperation::add( osg::Node* subgraphToCompile )
    {
        OSG_INFO << "IncrementalCompileOperation::add(" << subgraphToCompile << ")"
                 << std::endl;
        add( new CompileSet( subgraphToCompile ) );
    }

    void
    IncrementalCompileOperation::add( osg::Group* attachmentPoint,
    ...
    void
    IncrementalCompileOperation::add( CompileSet* compileSet,
                                      bool        callBuildCompileMap )
    {
        ...
        {
            // force a compute of the bound of the subgraph to avoid the update traversal
            // from having to do this work and reducing the change of frame drop.
        ...
        std::lock_guard<std::mutex> lock( _toCompileMutex );
        _toCompile.push_back( compileSet );
    }

    void
    IncrementalCompileOperation::remove( CompileSet* compileSet )
    {
        ...
        {
            std::lock_guard<std::mutex> lock( _toCompileMutex );
            for( CompileSets::iterator itr = _toCompile.begin(); itr != _toCompile.end();
                 ++itr )
            {
                if( *itr == compileSet )
                {
        ...
        {
            std::lock_guard<std::mutex> lock( _compiledMutex );
            for( CompileSets::iterator itr = _compiled.begin(); itr != _compiled.end();
                 ++itr )
            {
                if( *itr == compileSet )
                {
    ...
    void
    IncrementalCompileOperation::mergeCompiledSubgraphs(
        const osg::FrameStamp* frameStamp
    )
    {
        // OSG_INFO<<"IncrementalCompileOperation::mergeCompiledSubgraphs()"<<std::endl;

        std::lock_guard<std::mutex> compilded_lock( _compiledMutex );
    ...
    void
    IncrementalCompileOperation::operator()( osg::GraphicsContext* context )
    {
        ...
        double availableTime =
            std::max( ( targetFrameTime - currentElapsedFrameTime ) *
...
#if 1
        OSG_NOTIFY( level ) << "    availableTime = " << availableTime *
            1000.0 << std::endl;
        OSG_NOTIFY( level ) << "    flushTime     = " << flushTime * 1000.0 << std::endl;
        OSG_NOTIFY( level ) << "    compileTime   = " << compileTime *
            1000.0 << std::endl;
#endif
        ...
        {
            std::lock_guard<std::mutex> toCompile_lock( _toCompileMutex );
            std::copy( _toCompile.begin(),
                       _toCompile.end(),
                       std::back_inserter<CompileSets>( toCompileCopy ) );
        }
        ...
        if( !toCompileCopy.empty() && compileInfo.maxNumObjectsToCompile > 0 )
        {
            ...
            // if any time left over from flush add on this remaining time to a second
            // pass of compiling.
            if( compileInfo.okToCompile() )
            {
                OSG_NOTIFY( level )
                    << "    Passing on " << flushTime
                    << " to second round of compileSets(..)" << std::endl;
                compileSets( toCompileCopy, compileInfo );
            }
        }
    ...
    void
    IncrementalCompileOperation::compileSets( CompileSets& toCompile,
                                              CompileInfo& compileInfo )
    {
        osg::NotifySeverity level = osg::INFO;
            ...
            {
                {
                    ...
                    CompileSets::iterator       cs_itr =
                        std::find( _toCompile.begin(), _toCompile.end(), *itr );
                    if( cs_itr != _toCompile.end() )
                    {
                        OSG_NOTIFY( level ) << "    Erasing from list" << std::endl;

                        // remove from the _toCompile list, note cs won't be deleted here
                        // as the tempoary toCompile_Copy list will retain a reference.
    ...
    void
    IncrementalCompileOperation::compileAllForNextFrame(
        unsigned int numFramesToDoCompileAll
    )
    {
