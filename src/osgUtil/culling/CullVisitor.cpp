...
CullVisitor::CullVisitor() :
    osg::DualModeVisitor( CULL_VISITOR,
...
osg::ref_ptr<CullVisitor>&
CullVisitor::prototype()
{
...
CullVisitor::create()
{
    return CullVisitor::prototype().valid() ? CullVisitor::prototype()->clone()
                                            : new CullVisitor;
}
...
CullVisitor::reset()
{
    ...
    for( RenderLeafList::iterator
             itr      = _reuseRenderLeafList.begin(),
...
float
CullVisitor::getDistanceToEyePoint( const vec3& pos,
...
float
CullVisitor::getDistanceToViewPoint( const vec3& pos,
                                     bool        withLODScale ) const
{
    if( withLODScale )
    {
...
inline CullVisitor::value_type
distance( const osg::vec3&  coord,
          const osg::dmat4& matrix )
{
    // Row 2 of MV gives eye-space Z: z_eye = M(2,0)*x + M(2,1)*y + M(2,2)*z + M(2,3)
    return -( ( CullVisitor::value_type )coord[0] *
              ( CullVisitor::value_type )matrix( 2, 0 ) +
              ( CullVisitor::value_type )coord[1] *
              ( CullVisitor::value_type )matrix( 2, 1 ) +
              ( CullVisitor::value_type )coord[2] *
              ( CullVisitor::value_type )matrix( 2, 2 ) +
              matrix( 2, 3 ) );
}

float
CullVisitor::getDistanceFromEyePoint( const osg::vec3& pos,
                                      bool             withLODScale ) const
{
    const dmat4& matrix = *_modelviewStack.back();
    float        dist   = static_cast<float>( distance( pos, matrix ) );
...
{
    // OSG_NOTICE<<"CullVisitor::computeNearPlane()
    // _computed_znear="<<_computed_znear<<",
    // _computed_zfar="<<_computed_zfar<<std::endl;
    if( !_nearPlaneCandidateMap.empty() )
    {
             ...
             ++itr )
        {
            ++numTests;
            value_type d_near =
                computeNearestPointInFrustum( itr->second._matrix,
                                              itr->second._planes,
                                              *( itr->second._drawable ) );
            // OSG_NOTICE<<"  testing computeNearestPointInFrustum with,
            // drawable"<<itr->second._drawable<<", "<<itr->first<<", d_near =
            // "<<d_near<<std::endl;
    ...
    if( !_farPlaneCandidateMap.empty() )
    {
        ...
        unsigned int numTests = 0;
        ( void )numTests;
        for( DistanceMatrixDrawableMap::reverse_iterator itr =
                 _farPlaneCandidateMap.rbegin();
             itr != _farPlaneCandidateMap.rend() && itr->first > _computed_zfar;
             ++itr )
        {
            ++numTests;
            // OSG_WARN<<"testing computeFurthestPointInFrustum with d_far =
            // "<<itr->first<<std::endl;
            value_type d_far =
                computeFurthestPointInFrustum( itr->second._matrix,
                                               itr->second._planes,
                                               *( itr->second._drawable ) );
        ...
        // osg::Timer_t end_t = osg::Timer::instance()->tick();
        // OSG_NOTICE<<"Took "<<osg::Timer::instance()->delta_m(start_t,end_t)<<"ms to
        // test "<<numTests<<" out of "<<_farPlaneCandidateMap.size()<<std::endl;
...
CullVisitor::popProjectionMatrix()
{
    ...
    if( _computeNearFar && _computed_zfar >= _computed_znear )
    {
        ...
        // OSG_INFO<<"clamping "<< "znear="<<_computed_znear << "
        // zfar="<<_computed_zfar<<std::endl;

        // adjust the projection matrix so that it encompases the local coords.
        // so it doesn't cull them out.
        osg::dmat4& projection = *_projectionStack.back();
    ...
    {
        // OSG_INFO<<"Not clamping "<< "znear="<<_computed_znear << "
        // zfar="<<_computed_zfar<<std::endl;
    }
...
bool
CullVisitor::clampProjectionMatrixImplementation( osg::mat4& projection,
                                                  double&    znear,
                                                  double&    zfar ) const
{
...
bool
CullVisitor::clampProjectionMatrixImplementation( osg::dmat4& projection,
                                                  double&     znear,
                                                  double&     zfar ) const
{
    return osg::clampProjectionMatrix( projection, znear, zfar, _nearFarRatio );
...
struct ComputeNearFarFunctor
{
        ...
        void
        set( CullVisitor::value_type         znear,
             const osg::dmat4&               matrix,
                    ...
                    bool )
        {
            ...
            for( pitr = _planes->begin(); pitr != _planes->end(); ++pitr )
            {
                    ...
                    bool )
        {
            ...
            for( pitr = _planes->begin(); pitr != _planes->end(); ++pitr )
            {
                ...
                unsigned int      numOutside =
                    ( ( d1 < 0.0 ) ? 1 : 0 ) + ( ( d2 < 0.0 ) ? 1 : 0 );
                if( numOutside == 2 )
                {
                    // OSG_NOTICE<<"Line totally outside frustum
                    // "<<d1<<"\t"<<d2<<std::endl;
                    return;
                }
            ...
            if( active_mask == 0 )
            {
                ...
                // OSG_NOTICE<<"Line all inside frustum "<<n1<<"\t"<<n2<<" number of
                // plane="<<_planes->size()<<std::endl;
                return;
            }

            // OSG_NOTICE<<"Using brute force method of line cutting frustum
            // walls"<<std::endl;
                    ...
                    bool )
        {
            ...
            for( pitr = _planes->begin(); pitr != _planes->end(); ++pitr )
            {
                ...
                unsigned int      numOutside = ( ( d1 < 0.0 ) ? 1 : 0 ) +
                                               ( ( d2 < 0.0 ) ? 1 : 0 ) +
                                               ( ( d3 < 0.0 ) ? 1 : 0 );
                if( numOutside == 3 )
                {
                    // OSG_NOTICE<<"Triangle totally outside frustum
                    // "<<d1<<"\t"<<d2<<"\t"<<d3<<std::endl;
                    return;
                }
                unsigned int numInside = ( ( d1 >= 0.0 ) ? 1 : 0 ) +
                                         ( ( d2 >= 0.0 ) ? 1 : 0 ) +
                                         ( ( d3 >= 0.0 ) ? 1 : 0 );
                if( numInside < 3 )
                {
                ...
                // OSG_NOTICE<<"Triangle ok w.r.t plane
                // "<<d1<<"\t"<<d2<<"\t"<<d3<<std::endl;
            ...
            if( active_mask == 0 )
            {
                ...
                // OSG_NOTICE<<"Triangle all inside frustum "<<n1<<"\t"<<n2<<"\t"<<n3<<"
                // number of plane="<<_planes->size()<<std::endl;
                return;
            }
            ...
            // OSG_NOTICE<<"Using brute force method of triangle cutting frustum
            // walls"<<std::endl;
                 ...
                 ++pitr )
            {
                if( active_mask & selector_mask )
                {
                    ...
                    unsigned int psize =
                        static_cast<unsigned int>( _polygonOriginal.size() );
                    ...
                    for( unsigned int ci = 0; ci < psize; ++ci )
                    {
                        ...
                        if( computeIntersection )
                        {
                            ...
                            float r = _polygonOriginal[ci].first /
                                      ( _polygonOriginal[ci].first -
                                        _polygonOriginal[ni].first );
                            _polygonNew.push_back(
                                DistancePoint( 0.0F,
                                               _polygonOriginal[ci].second *
                                                   ( 1.0F - r ) +
                                                   _polygonOriginal[ni].second *
                                                   r )
                            );
                        }
                    }
...
{
        inline bool
        less( CullVisitor::value_type lhs,
              CullVisitor::value_type rhs ) const
        {
            return lhs < rhs;
        }

        inline bool
        lessEqual( CullVisitor::value_type lhs,
                   CullVisitor::value_type rhs ) const
        {
            return lhs <= rhs;
        }
        ...
        inline bool
        greaterEqual( CullVisitor::value_type lhs,
                      CullVisitor::value_type rhs ) const
        {
            return lhs >= rhs;
        }
        ...
        inline CullVisitor::value_type
        minimum( CullVisitor::value_type lhs,
                 CullVisitor::value_type rhs ) const
        {
...
CullVisitor::value_type
CullVisitor::computeNearestPointInFrustum( const osg::dmat4&               matrix,
                                           const osg::Polytope::PlaneList& planes,
...
CullVisitor::value_type
CullVisitor::computeFurthestPointInFrustum( const osg::dmat4&               matrix,
                                            const osg::Polytope::PlaneList& planes,
                                            const osg::Drawable&            drawable )
{
...
bool
CullVisitor::updateCalculatedNearFar( const osg::dmat4& matrix,
                                      const osg::box&   bb )
{
    // efficient computation of near and far, only taking into account the nearest and
    // furthest corners of the bounding box.
    ...
    if( d_near > d_far )
    {
        ...
        if( !EQUAL_F( static_cast<float>( d_near ), static_cast<float>( d_far ) ) )
        {
            OSG_WARN << "Warning: CullVisitor::updateCalculatedNearFar(.) near>far in "
                        "range calculation,"
                     << std::endl;
            OSG_WARN << "         correcting by swapping values d_near=" << d_near
                     << " dfar=" << d_far << std::endl;
        }
    }
...
CullVisitor::updateCalculatedNearFar( const osg::dmat4&    matrix,
                                      ...
                                      bool                 isBillboard )
{
    const osg::box& bb = drawable.getBoundingBox();
    ...
    if( isBillboard )
    {
        ...
        {
            OSG_NOTICE << "Took " << elapsed_time << "ms to test " << numBillboards
                       << " billboards" << std::endl;
            numBillboards   = 0;
            elapsed_time    = 0.0;
        ...
        // Camera look direction in model space = -row_2 of MV rotation
        osg::vec3    lookVector( static_cast<float>( -matrix( 2, 0 ) ),
    ...
    {
        // efficient computation of near and far, only taking into account the nearest
        // and furthest corners of the bounding box.
    ...
    if( _computeNearFar ==
        ...
        _computeNearFar == COMPUTE_NEAR_USING_PRIMITIVES )
    {
        if( d_near < _computed_znear || d_far > _computed_zfar )
        {
            ...
            if( frustum.getResultMask() )
            {
                ...
                if( isBillboard )
                {
                    ...
                    transformed_frustum.setAndTransformProvidingInverse(
                        getProjectionCullingStack().back().getFrustum(),
                        matrix
                    );
                ...
                if( d_near < _computed_znear )
                {
                    _nearPlaneCandidateMap.insert(
                        DistanceMatrixDrawableMap::value_type( d_near, mpd )
                    );
                }
                ...
                if( _computeNearFar == COMPUTE_NEAR_FAR_USING_PRIMITIVES )
                {
                    if( d_far > _computed_zfar )
                    {
                        _farPlaneCandidateMap.insert(
                            DistanceMatrixDrawableMap::value_type( d_far, mpd )
                        );
                    }
                }

                // use the far point if its nearer than current znear as this is a
                // conservative estimate of the znear while the final computation for
                // this drawable is deferred.
                ...
                if( _computeNearFar == COMPUTE_NEAR_FAR_USING_PRIMITIVES )
                {
                    // use the near point if its further than current zfar as this is a
                    // conservative estimate of the zfar while the final computation for
                    // this drawable is deferred.
...
void
CullVisitor::updateCalculatedNearFar( const osg::vec3& pos )
{
    float d;
    if( !_modelviewStack.empty() )
    {
        const osg::dmat4& matrix = *( _modelviewStack.back() );
        d                        = static_cast<float>( distance( pos, matrix ) );
    }
    else
    {
        d = -pos.z;
...
void
CullVisitor::apply( osg::Drawable& drawable )
{
    RefMatrix& matrix = *getModelViewMatrix();

    const box& bb     = drawable.getBoundingBox();

    if( drawable.getCullCallback() )
    {
        osg::DrawableCullCallback* dcb =
            drawable.getCullCallback()->asDrawableCullCallback();
        if( dcb )
        {
    ...
    float depth =
        bb.valid() ? static_cast<float>( distance( bb.center(), matrix ) ) : 0.0F;

    if( osg::isNaN( depth ) )
    {
        OSG_NOTICE << "CullVisitor::apply(Geode&) detected NaN," << std::endl
                   << "    depth=" << depth << ", center=(" << bb.center() << "),"
                   << std::endl
                   << "    matrix=" << matrix << std::endl;
        OSG_DEBUG << "    NodePath:" << std::endl;
        for( NodePath::const_iterator i = getNodePath().begin();
             i != getNodePath().end();
             ++i )
        {
...
void
CullVisitor::apply( Billboard& node )
{
    if( isCulled( node ) )
    {
        return;
    ...
    for( unsigned int i = 0; i < node.getNumDrawables(); ++i )
    {
        ...
        float     depth = static_cast<float>( distance( pos, modelview ) );
        /*
                if (_computeNearFar)
                {
                    if (d<_computed_znear)
                    {
                        if (d<0.0) OSG_WARN<<"Alerting billboard handling ="<<d<<
           std::endl; _computed_znear = d;
                    }
        ...
        */
        ...
        if( osg::isNaN( depth ) )
        {
            ...
            OSG_DEBUG << "    NodePath:" << std::endl;
            for( NodePath::const_iterator itr = getNodePath().begin();
                 itr != getNodePath().end();
                 ++itr )
            {
...
void
CullVisitor::apply( LightSource& node )
{
    // push the node's state.
    StateSet* node_state = node.getStateSet();
    if( node_state )
...
void
CullVisitor::apply( Transform& node )
{
    if( isCulled( node ) )
    {
        return;
...
void
CullVisitor::apply( Projection& node )
{
    ...
    float                     previous_znear = static_cast<float>( _computed_znear );
    float                     previous_zfar  = static_cast<float>( _computed_zfar );
...
void
CullVisitor::apply( Switch& node )
{
    apply( ( Group& )node );
}

void
CullVisitor::apply( LOD& node )
{
    if( isCulled( node ) )
    {
        return;
...
void
CullVisitor::apply( osg::ClearNode& node )
{
...
namespace osgUtil
{
    ...
    class RenderStageCache : public osg::Inherit<osg::Object, RenderStageCache>,
                             public osg::Observer
    {
        public:
            ...
            RenderStageCache( const RenderStageCache& rhs,
            ...
            void
            setRenderStage( osg::Referenced* cv,
                            RenderStage*     rs )
            {
                std::lock_guard<std::mutex> lock( _mutex );
            ...
            RenderStage*
            getRenderStage( osg::Referenced* cv )
            {
                std::lock_guard<std::mutex> lock( _mutex );
                RenderStageMap::iterator    itr = _renderStageMap.find( cv );
                if( itr != _renderStageMap.end() )
...
void
CullVisitor::apply( osg::Camera& camera )
{
    // push the node's state.
    ...
    inheritCullSettings( saved_cull_settings,
    ...
    else
    {
        ...
        if( !rtts )
        {
            std::lock_guard<std::mutex> lock( *( camera.getDataChangeMutex() ) );

            rtts = _rootRenderStage.valid() ? osg::cloneType( _rootRenderStage.get() )
        ...
        {
            // reusing render to texture stage, so need to reset it to empty it from
            // previous frames contents.
            rtts->reset();
        }
        ...
        // replicate the StageGraph parental chain so that state graph and render leaves
        // are kept local to the Camera's RenderStage.
        {
            ...
            else
            {
                ...
                // assign the state graph to the RenderStage to ensure it remains in
                // memory for the draw traversal.
            ...
            if( ritr != stateGraphParentalChain.rend() )
            {
                ...
                {
                    _currentStateGraph =
                        _currentStateGraph->find_or_insert( ( *ritr++ )->getStateSet() );
                }
            }
        }
        ...
        rtts->setClearMask( ( camera.getInheritanceMask() & CLEAR_MASK )
                                ? previous_stage->getClearMask()
                                : camera.getClearMask() );
        rtts->setClearColor( ( camera.getInheritanceMask() & CLEAR_COLOR )
                                 ? previous_stage->getClearColor()
                                 : camera.getClearColor() );

        // set the color mask.
        osg::ColorMask* colorMask = camera.getColorMask() != 0
                                      ? camera.getColorMask()
                                      : previous_stage->getColorMask();
        rtts->setColorMask( colorMask );

        // set up the viewport.
        osg::Viewport* viewport = camera.getViewport() != 0
                                    ? camera.getViewport()
                                    : previous_stage->getViewport();
        rtts->setViewport( viewport );

        // set initial view matrix
        rtts->setInitialViewMatrix( modelview );

        // set up to charge the same PositionalStateContainer is the parent previous
        // stage.
        osg::dmat4 inheritedMVtolocalMV;
        inheritedMVtolocalMV = osg::inverse( originalModelView );
        inheritedMVtolocalMV = inheritedMVtolocalMV * *getModelViewMatrix();
        ...
        if( rtts->getStateGraphList().size() ==
            0 &&
        ...
        switch( camera.getRenderOrder() )
        {
            case osg::Camera::PRE_RENDER :
                getCurrentRenderBin()->getStage()->addPreRenderStage(
                    rtts.get(),
...
void
CullVisitor::apply( osg::OccluderNode& node )
{
...
void
CullVisitor::apply( osg::OcclusionQueryNode& node )
{
    if( isCulled( node ) )
    {
        return;
