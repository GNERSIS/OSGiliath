 ...
 * Sorted collection of render leaves. Orders geometry by state,
 * depth, or bin number for correct multi-pass rendering.
 */
...
class RenderBinPrototypeList
    : osg::depends_on<std::mutex*, osg::Referenced::getGlobalReferencedMutex>,
      public osg::Referenced,
      public std::map<std::string, osg::ref_ptr<RenderBin>>
{
    public:
        ...
        {
            add( "RenderBin",
                 new RenderBin( RenderBin::getDefaultRenderBinSortMode() ) );
...
static RenderBinPrototypeList*
renderBinPrototypeList()
{
    static osg::ref_ptr<RenderBinPrototypeList> s_renderBinPrototypeList =
        new RenderBinPrototypeList;
    return s_renderBinPrototypeList.get();
}

// Use a proxy to force the initialization of the RenderBinPrototypeListSingleton during
// static initialization
OSG_INIT_SINGLETON_PROXY( RenderBinSingletonProxy,
                          renderBinPrototypeList() )
...
RenderBin::createRenderBin( const std::string& binName )
{
    ...
    if( list )
    {
        ...
        if( prototype )
        {
            return dynamic_cast<RenderBin*>(
                prototype->clone( osg::CopyOp::DEEP_COPY_ALL )
            );
        }
    }

    OSG_WARN << "Warning: RenderBin \"" << binName
...
void
RenderBin::addRenderBinPrototype( const std::string& binName,
                                  RenderBin*         proto )
{
    RenderBinPrototypeList* list = renderBinPrototypeList();
    if( list && proto )
...
void
RenderBin::removeRenderBinPrototype( RenderBin* proto )
{
    RenderBinPrototypeList* list = renderBinPrototypeList();
    if( list && proto )
    {
        for( RenderBinPrototypeList::iterator itr = list->begin(); itr != list->end();
             ++itr )
        {
            if( itr->second == proto )
            {
...
static osg::ApplicationUsageProxy
    RenderBin_e0( osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE,
                  "OSG_DEFAULT_BIN_SORT_MODE <type>",
                  "SORT_BY_STATE | SORT_BY_STATE_THEN_FRONT_TO_BACK | "
                  "SORT_FRONT_TO_BACK | SORT_BACK_TO_FRONT" );

void
RenderBin::setDefaultRenderBinSortMode( RenderBin::SortMode mode )
{
...
RenderBin::RenderBin( const RenderBin& rhs,
                      const CopyOp&    copyop ) :
    Object( rhs,
            copyop ),
    _binNum( rhs._binNum ),
...
RenderBin::sort()
{
    ...
    for( RenderBinList::iterator itr = _bins.begin(); itr != _bins.end(); ++itr )
    {
        itr->second->sort();
    }
...
void
RenderBin::setSortMode( SortMode mode )
{
    _sortMode = mode;
}

void
RenderBin::sortImplementation()
{
    switch( _sortMode )
    {
        case( SORT_BY_STATE ) :
...
{
        bool
        operator()( const StateGraph* lhs,
                    const StateGraph* rhs ) const
        {
...
void
RenderBin::sortByStateThenFrontToBack()
{
    ...
    std::sort( _stateGraphList.begin(),
               _stateGraphList.end(),
               StateGraphFrontToBackSortFunctor() );
}

struct FrontToBackSortFunctor
{
        bool
        operator()( const RenderLeaf* lhs,
                    const RenderLeaf* rhs ) const
        {
...
void
RenderBin::sortFrontToBack()
{
    copyLeavesFromStateGraphListToRenderLeafList();

    // now sort the list into ascending depth order.
    std::sort( _renderLeafList.begin(),
               _renderLeafList.end(),
               FrontToBackSortFunctor() );
...
RenderBin::copyLeavesFromStateGraphListToRenderLeafList()
{
    ...
    for( itr = _stateGraphList.begin(); itr != _stateGraphList.end(); ++itr )
    {
        totalsize += ( *itr )->_leaves.size();
    }

    _renderLeafList.reserve( static_cast<std::size_t>( totalsize ) );
    ...
    // empty the render graph list to prevent it being drawn along side the render leaf
    // list (see drawImplementation.)
    _stateGraphList.clear();
}

RenderBin*
RenderBin::find_or_insert( int                binNum,
...
void
RenderBin::draw( osg::RenderInfo& renderInfo,
                 RenderLeaf*&     previous )
{
...
void
RenderBin::drawImplementation( osg::RenderInfo& renderInfo,
                               RenderLeaf*&     previous )
{
    osg::State&  state = *renderInfo.getState();

    // OSG_NOTICE<<"begin RenderBin::drawImplementation "<<className()<<" sortMode
    // "<<getSortMode()<<std::endl;

    unsigned int numToPop =
        static_cast<unsigned int>( previous ? StateGraph::numToPop( previous->_parent )
                                            : 0 );
    if( numToPop > 1 )
    {
        --numToPop;
    ...
    for( rbitr = _bins.begin(); rbitr != _bins.end() && rbitr->first < 0; ++rbitr )
    {
    ...
    bool draw_forward = true;    //(_sortMode!=SORT_BY_STATE) ||
                                 //(state.getFrameStamp()->getFrameNumber() % 2)==0;
...
// stats
bool
RenderBin::getStats( Statistics& stats ) const
{
    stats.addBins( 1 );

    // different by return type - collects the stats in this renderrBin
    bool statsCollected = false;
    stats.addOrderedLeaves( static_cast<int>( _renderLeafList.size() ) );
    // draw fine grained ordering.
    ...
    stats.addStateGraphs( static_cast<int>( _stateGraphList.size() ) );
    ...
    for( RenderBinList::const_iterator itr = _bins.begin(); itr != _bins.end(); ++itr )
    {
...
unsigned int
RenderBin::computeNumberOfDynamicRenderLeaves() const
{
...
void
RenderBin::releaseGLObjects( osg::State* state ) const
{
    if( _stateset )
    {
