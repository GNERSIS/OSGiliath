 ...
 * High-level single-camera scene renderer. Drives CullVisitor and
 * RenderStage to produce a complete frame for one viewport.
 */
...
SceneView::SceneView( const SceneView&   rhs,
...
void
SceneView::setDefaults( unsigned int options )
{
    osg::CullSettings::setDefaults();

    _camera->setProjectionMatrix( osg::perspective( 50.0, 1.4, 1.0, 10000.0 ) );
    ...
    if( ( options & HEADLIGHT ) || ( options & SKY_LIGHT ) )
    {
        ...
        _globalStateSet->setAttributeAndModes( _light.get(), osg::StateAttribute::ON );

        // GL_LIGHTING mode tracked (not glEnabled) for shader pipeline defines
    ...
    if( options & COMPILE_GLOBJECTS_AT_INIT )
    {
        ...
        dlv->setNodeMaskOverride( 0XFF'FF'FF'FF );
        _initVisitor = dlv;
    }
...
void
SceneView::setCamera( osg::Camera* camera,
                      bool         assumeOwnershipOfCamera )
{
    ...
    {
        OSG_NOTICE
            << "Warning: attempt to assign a NULL camera to SceneView not permitted."
            << std::endl;
    }
...
void
SceneView::setSceneData( osg::Node* node )
{
    ...
    if( _activeUniforms & VIEW_MATRIX_INVERSE_UNIFORM )
    {
        osg::Uniform* uniform =
            _localStateSet->getOrCreateUniform( "osg_ViewMatrixInverse",
                                                osg::Uniform::FLOAT_MAT4 );
        uniform->set( osg::inverse( getViewMatrix() ) );
    }
}

osg::dmat4
SceneView::computeLeftEyeProjectionImplementation( const osg::dmat4& projection ) const
{
    return _displaySettings.valid()
             ? _displaySettings->computeLeftEyeProjectionImplementation( projection )
             : projection;
}

osg::dmat4
SceneView::computeLeftEyeViewImplementation( const osg::dmat4& view ) const
{
    if( !_displaySettings )
    {
        return view;
...
osg::dmat4
SceneView::computeRightEyeProjectionImplementation( const osg::dmat4& projection ) const
{
    return _displaySettings.valid()
             ? _displaySettings->computeRightEyeProjectionImplementation( projection )
             : projection;
}

osg::dmat4
SceneView::computeRightEyeViewImplementation( const osg::dmat4& view ) const
{
    if( !_displaySettings )
    {
        return view;
...
void
SceneView::computeLeftEyeViewport( const osg::Viewport* viewport )
{
    if( !viewport )
    {
        return;
    ...
    switch( _displaySettings->getStereoMode() )
    {
        ...
        case( osg::DisplaySettings::HORIZONTAL_SPLIT ) :
            {
                Viewport::value_type separation =
                    _displaySettings->getSplitStereoHorizontalSeparation();

                if( _displaySettings->getSplitStereoHorizontalEyeMapping() ==
                    osg::DisplaySettings::LEFT_EYE_LEFT_VIEWPORT )
                {
                    Viewport::value_type left_half_width =
                        ( viewport->width() - separation ) / 2;
                    _viewportLeft->setViewport( viewport->x(),
                                                viewport->y(),
                                                left_half_width,
                                                viewport->height() );
                }
                ...
                {
                    Viewport::value_type right_half_begin =
                        ( viewport->width() + separation ) / 2;
                    Viewport::value_type right_half_width =
                        viewport->width() - right_half_begin;
                    _viewportLeft->setViewport( viewport->x() + right_half_begin,
                                                viewport->y(),
                                                right_half_width,
                                                viewport->height() );
                }
        ...
        case( osg::DisplaySettings::VERTICAL_SPLIT ) :
            {
                Viewport::value_type separation =
                    _displaySettings->getSplitStereoVerticalSeparation();

                if( _displaySettings->getSplitStereoVerticalEyeMapping() ==
                    osg::DisplaySettings::LEFT_EYE_TOP_VIEWPORT )
                {
                    Viewport::value_type top_half_begin =
                        ( viewport->height() + separation ) / 2;
                    Viewport::value_type top_half_height =
                        viewport->height() - top_half_begin;
                    _viewportLeft->setViewport( viewport->x(),
                                                viewport->y() + top_half_begin,
                                                viewport->width(),
                                                top_half_height );
                }
                else
                {
                    Viewport::value_type bottom_half_height =
                        ( viewport->height() - separation ) / 2;
                    _viewportLeft->setViewport( viewport->x(),
                                                viewport->y(),
                                                viewport->width(),
...
void
SceneView::computeRightEyeViewport( const osg::Viewport* viewport )
{
    if( !viewport )
    {
        return;
        ...
        case( osg::DisplaySettings::HORIZONTAL_SPLIT ) :
            {
                    ...
                    osg::DisplaySettings::LEFT_EYE_LEFT_VIEWPORT )
                {
                    Viewport::value_type left_half_width =
                        ( viewport->width() - separation ) / 2;
                    _viewportRight->setViewport( viewport->x(),
                                                 viewport->y(),
                                                 left_half_width,
                                                 viewport->height() );
                }
                else
                {
                    ...
                    Viewport::value_type right_half_width =
                        viewport->width() - right_half_begin;
                    _viewportRight->setViewport( viewport->x() + right_half_begin,
                                                 viewport->y(),
                                                 right_half_width,
                                                 viewport->height() );
                }
...
void
SceneView::setLightingMode( LightingMode mode )
{
    ...
    osg::StateSet* stateSetToModify =
        _secondaryStateSet.valid() ? _secondaryStateSet.get() : _globalStateSet.get();
    ...
    if( _lightingMode != NO_SCENEVIEW_LIGHT )
    {
        ...
        if( _light.valid() )
        {
            ...
            stateSetToModify->removeAttribute( _light.get() );
        }
    }
    ...
    if( _lightingMode != NO_SCENEVIEW_LIGHT )
    {
        // Set GL_LIGHTING mode so shader pipeline generates OSG_LIGHTING define.
        // isModeApplicable() prevents actual glEnable call — mode is tracked only.
        stateSetToModify->setMode( GL_LIGHTING, osg::StateAttribute::ON );
        if( _light.valid() )
        {
            // Add Light as both modes and attribute so Light::apply() is called
            // during state application, delivering actual light parameters to uniforms.
            stateSetToModify->setAttributeAndModes( _light.get(),
...
void
SceneView::inheritCullSettings( const osg::CullSettings& settings,
...
SceneView::cull()
{
    ...
    {
        OSG_INFO << "Warning: no valid osgUtil::SceneView::_state attached, creating a "
                    "default state automatically."
                 << std::endl;

        // note the constructor for osg::State will set ContextID to 0 which will be fine
        // to single context graphics applications which is ok for most apps, but not
        // multiple context/pipe applications.
    ...
    if( !_cullVisitor )
    {
        OSG_INFO << "Warning: no valid osgUtil::SceneView:: attached, creating a "
    ...
    if( !_stateGraph )
    {
        OSG_INFO << "Warning: no valid osgUtil::SceneView:: attached, creating a global "
                    "default StateGraph automatically."
                 << std::endl;
        _stateGraph = new StateGraph;
    }
    if( !_renderStage )
    {
        OSG_INFO << "Warning: no valid osgUtil::SceneView::_renderStage attached, "
    ...
    if( _displaySettings.valid() && _displaySettings->getStereo() )
    {
        ...
        if( _displaySettings->getStereoMode() == osg::DisplaySettings::LEFT_EYE )
        {
            ...
            bool computeNearFar =
                cullStage( computeLeftEyeProjection( getProjectionMatrix() ),
                           computeLeftEyeView( getViewMatrix() ),
                           _cullVisitor.get(),
                           _stateGraph.get(),
                           _renderStage.get(),
                           _viewportLeft.get() );
            ...
            if( computeNearFar )
            {
                ...
                _cullVisitor->clampProjectionMatrix( getProjectionMatrix(),
                                                     zNear,
                                                     zFar );
            }
        }
        else if( _displaySettings->getStereoMode() == osg::DisplaySettings::RIGHT_EYE )
        {
            ...
            bool computeNearFar =
                cullStage( computeRightEyeProjection( getProjectionMatrix() ),
                           computeRightEyeView( getViewMatrix() ),
                           _cullVisitor.get(),
                           _stateGraph.get(),
                           _renderStage.get(),
                           _viewportRight.get() );
        ...
        else
        {
            ...
            {
                _cullVisitorLeft = _cullVisitor->clone();
            }
            if( !_stateGraphLeft.valid() )
            {
                _stateGraphLeft = _stateGraph->cloneStateGraph();
            }
            if( !_renderStageLeft.valid() )
            {
                _renderStageLeft =
                    osg::clone( _renderStage.get(), osg::CopyOp::DEEP_COPY_ALL );
            }

            if( !_cullVisitorRight.valid() )
            {
                _cullVisitorRight = _cullVisitor->clone();
            }
            if( !_stateGraphRight.valid() )
            {
                _stateGraphRight = _stateGraph->cloneStateGraph();
            }
            ...
            _cullVisitorLeft->setDatabaseRequestHandler(
                _cullVisitor->getDatabaseRequestHandler()
            );
            _cullVisitorLeft->setClampProjectionMatrixCallback(
                _cullVisitor->getClampProjectionMatrixCallback()
            );
            ...
            bool computeNearFar =
                cullStage( computeLeftEyeProjection( getProjectionMatrix() ),
                           computeLeftEyeView( getViewMatrix() ),
                           _cullVisitorLeft.get(),
                           _stateGraphLeft.get(),
                           _renderStageLeft.get(),
                           _viewportLeft.get() );

            // set up the right eye.
            _cullVisitorRight->setDatabaseRequestHandler(
                _cullVisitor->getDatabaseRequestHandler()
            );
            _cullVisitorRight->setClampProjectionMatrixCallback(
                _cullVisitor->getClampProjectionMatrixCallback()
            );
            ...
            computeNearFar =
                cullStage( computeRightEyeProjection( getProjectionMatrix() ),
                           computeRightEyeView( getViewMatrix() ),
                           _cullVisitorRight.get(),
                           _stateGraphRight.get(),
                           _renderStageRight.get(),
                           _viewportRight.get() ) |
                computeNearFar;
            ...
            if( computeNearFar )
            {
                CullVisitor::value_type zNear =
                    std::min( _cullVisitorLeft->getCalculatedNearPlane(),
                              _cullVisitorRight->getCalculatedNearPlane() );
                CullVisitor::value_type zFar =
                    std::max( _cullVisitorLeft->getCalculatedFarPlane(),
                              _cullVisitorRight->getCalculatedFarPlane() );
                _cullVisitor->clampProjectionMatrix( getProjectionMatrix(),
                                                     zNear,
                                                     zFar );
            }
    ...
    else
    {
        ...
        // Save and restore original projection each frame.
        // clampProjectionMatrix modifies the projection in-place, and without
        // restoring, the clamped (potentially corrupted) values persist to the
        // next frame's frustum computation.
        osg::dmat4 savedProjection = getProjectionMatrix();

        bool       computeNearFar  = cullStage( getProjectionMatrix(),
                                                getViewMatrix(),
                                                _cullVisitor.get(),
                                                _stateGraph.get(),
                                                _renderStage.get(),
        ...
        // Restore original projection for next frame's frustum computation
        _camera->setProjectionMatrix( savedProjection );
    }
}

bool
SceneView::cullStage( const osg::dmat4&     projection,
                      const osg::dmat4&     modelview,
                      ...
                      osg::Viewport*        viewport )
{
    ...
    if( _camera->containsOccluderNodes() )
    {
        ...
        _collectOccludersVisitor->pushModelViewMatrix( mv.get(),
        ...
        OSG_DEBUG << "finished searching for occluder - found "
...
void
SceneView::resizeGLObjectBuffers( unsigned int maxSize )
{
    struct Resize
    {
            unsigned int maxSize;

            Resize( unsigned int ms ) :
                maxSize( ms )
            {
            }

            void
            operator()( osg::Referenced* object )
            {
            ...
            operator()( osg::Object* object )
            {
                if( object )
                {
                    object->resizeGLObjectBuffers( maxSize );
                }
            }
    } operation( maxSize );
...
void
SceneView::releaseGLObjects( osg::State* state ) const
{
    if( state && state != _renderInfo.getState() )
    {
        return;
    }
    ...
    struct Release
    {
            ...
            operator()( osg::Object* object )
            {
                if( object )
                {
                    object->releaseGLObjects( _state );
                }
            }
    } operation( state );
...
SceneView::draw()
{
    ...
    // we in theory should be able to be able to bypass reset, but we'll call it just in
    // case.
    //_state->reset();
    ...
    // assume the draw which is about to happen could generate GL objects that need
    // flushing in the next frame.
    ...
    if( _displaySettings.valid() && _displaySettings->getStereo() )
    {
        ...
        switch( _displaySettings->getStereoMode() )
        {
            case( osg::DisplaySettings::QUAD_BUFFER ) :
                {
                    ...
                    osg::ColorMask* cmask =
                        static_cast<osg::ColorMask*>( _localStateSet->getAttribute(
            ...
            case( osg::DisplaySettings::VERTICAL_SPLIT ) :
                {
                    ...
                    if( separation > 0.0 )
                    {
                        double bottom_half_height =
                            ( getViewport()->height() - separation ) / 2.0;
            ...
            case( osg::DisplaySettings::CHECKERBOARD ) :
                {
                    OSG_NOTICE
                        << "Warning: SceneView::draw() - VERTICAL_INTERLACE, "
                           "HORIZONTAL_INTERLACE, and CHECKERBOARD stereo not supported."
                        << std::endl;
                }
                break;
            default :
                {
                    OSG_NOTICE << "Warning: stereo mode not implemented yet."
                               << std::endl;
                }
                break;
        }
    ...
    else
    {
        ...
        {
            // ensure that all color planes are active.
            osg::ColorMask* cmask = static_cast<osg::ColorMask*>(
                _localStateSet->getAttribute( osg::StateAttribute::Type::COLORMASK )
            );
            if( cmask )
            {
                cmask->setMask( true, true, true, true );
...
// #define REPORT_TEXTURE_MANAGER_STATS
    ...
    // OSG_NOTICE<<"SceneView  draw()
    // DynamicObjectCount"<<getState()->getDynamicObjectCount()<<std::endl;
}

/** Calculate, via glUnProject, the object coordinates of a window point.
    Note, current implementation requires that SceneView::draw() has been previously
   called for projectWindowIntoObject to produce valid values.  As per OpenGL windows
   coordinates are calculated relative to the bottom left of the window.*/
bool
SceneView::projectWindowIntoObject( const osg::vec3& window,
                                    osg::vec3&       object ) const
{
    osg::dmat4 inverseMVPW;
    inverseMVPW = osg::inverse( computeMVPW() );

    object      = osg::vec3( osg::dvec3( window ) * inverseMVPW );
...
/** Calculate, via glUnProject, the object coordinates of a window x,y
   ...
   coordinates are calculated relative to the bottom left of the window.*/
bool
SceneView::projectWindowXYIntoObject( int        x,
                                      int        y,
                                      osg::vec3& near_point,
                                      osg::vec3& far_point ) const
{
    ...
    near_point  = osg::vec3( osg::dvec3( x, y, 0.0 ) * inverseMVPW );
    far_point   = osg::vec3( osg::dvec3( x, y, 1.0 ) * inverseMVPW );
...
/** Calculate, via glProject, the object coordinates of a window.
   ...
   coordinates are calculated relative to the bottom left of the window.*/
bool
SceneView::projectObjectIntoWindow( const osg::vec3& object,
                                    osg::vec3&       window ) const
{
    window = osg::vec3( osg::dvec3( object ) * computeMVPW() );
    return true;
}

const osg::dmat4
SceneView::computeMVPW() const
{
    osg::dmat4 matrix( getProjectionMatrix() * getViewMatrix() );

    if( getViewport() )
    {
        matrix = getViewport()->computeWindowMatrix() * matrix;
    }
    else
    {
        OSG_WARN << "osg::dmat4 SceneView::computeMVPW() - error no viewport attached "
                    "to SceneView, coords will be computed incorrectly."
                 << std::endl;
    }
...
void
SceneView::setProjectionMatrixAsOrtho( double left,
                                       double right,
                                       double bottom,
                                       double top,
                                       double zNear,
...
void
SceneView::setProjectionMatrixAsOrtho2D( double left,
                                         double right,
                                         double bottom,
                                         double top )
{
...
void
SceneView::setProjectionMatrixAsFrustum( double left,
                                         double right,
                                         double bottom,
                                         double top,
                                         double zNear,
...
void
SceneView::setProjectionMatrixAsPerspective( double fovy,
                                             double aspectRatio,
                                             double zNear,
                                             double zFar )
{
...
bool
SceneView::getProjectionMatrixAsOrtho( double& left,
                                       double& right,
                                       double& bottom,
                                       double& top,
                                       double& zNear,
                                       double& zFar ) const
{
    return osg::getOrtho( getProjectionMatrix(), left, right, bottom, top, zNear, zFar );
}

bool
SceneView::getProjectionMatrixAsFrustum( double& left,
                                         double& right,
                                         double& bottom,
                                         double& top,
                                         double& zNear,
                                         double& zFar ) const
{
    return osg::getFrustum( getProjectionMatrix(),
                            left,
                            right,
                            bottom,
                            top,
...
bool
SceneView::getProjectionMatrixAsPerspective( double& fovy,
                                             double& aspectRatio,
                                             double& zNear,
                                             double& zFar ) const
{
    return osg::getPerspective( getProjectionMatrix(), fovy, aspectRatio, zNear, zFar );
}

void
SceneView::setViewMatrixAsLookAt( const vec3& eye,
                                  const vec3& center,
                                  const vec3& up )
{
    setViewMatrix(
        osg::lookAt( osg::dvec3( eye ), osg::dvec3( center ), osg::dvec3( up ) )
    );
}

void
SceneView::getViewMatrixAsLookAt( vec3& eye,
                                  vec3& center,
                                  vec3& up,
                                  float lookDistance ) const
{
    {
        osg::dvec3 deye, dcenter, dup;
        osg::getLookAt( getViewMatrix(),
                        deye,
                        dcenter,
                        dup,
                        static_cast<double>( lookDistance ) );
        eye    = osg::vec3( deye );
        center = osg::vec3( dcenter );
        up     = osg::vec3( dup );
    }
}

bool
SceneView::getStats( Statistics& stats )
{
    if( _displaySettings.valid() && _displaySettings->getStereo() )
    {
        switch( _displaySettings->getStereoMode() )
