 ...
 * Top-level render bin that manages FBO setup, clear operations,
 * and post-draw callbacks for a single rendering pass.
 */
...
void
RenderStage::addPreRenderStage( RenderStage* rs,
                                int          order )
{
    if( rs )
    {
...
void
RenderStage::addPostRenderStage( RenderStage* rs,
                                 int          order )
{
    if( rs )
    {
...
void
RenderStage::drawPreRenderStages( osg::RenderInfo& renderInfo,
                                  RenderLeaf*&     previous )
{
    ...
    // cout << "Drawing prerendering stages "<<this<< "  "<<_viewport->x()<<","<<
    ...
    // cout << "Done Drawing prerendering stages "<<this<< "  "<<_viewport->x()<<","<<
    // _viewport->y()<<","<< _viewport->width()<<","<< _viewport->height()<<std::endl;
}
...
RenderStage::runCameraSetUp( osg::RenderInfo& renderInfo )
{
    ...
    OSG_INFO << "RenderStage::runCameraSetUp(osg::RenderInfo& renderInfo) " << this
             << std::endl;
    ...
    for( osg::Camera::BufferAttachmentMap::iterator itr = bufferAttachments.begin();
         ...
         ++itr )
    {
        width  = std::max( width, itr->second.width() );
        height = std::max( height, itr->second.height() );
        depth  = std::max( depth, itr->second.depth() );
    }

    // OSG_NOTICE<<"RenderStage::runCameraSetUp viewport "<<_viewport->x()<<"
    // "<<_viewport->y()<<" "<<_viewport->width()<<" "<<_viewport->height()<<std::endl;
    // OSG_NOTICE<<"RenderStage::runCameraSetUp computed "<<width<<" "<<height<<"
    // "<<depth<<std::endl;
         ...
         ++itr )
    {
        ...
        if( itr->second._texture.valid() )
        {
            ...
            else if( 0 != ( texture2DMS =
                                dynamic_cast<osg::Texture2DMultisample*>( texture ) ) )
            {
                if( texture2DMS->getTextureWidth() ==
                    0 ||
                    texture2DMS->getTextureHeight() == 0 )
                {
            ...
            {
                if( texture3D->getTextureWidth() ==
                    0 ||
                    texture3D->getTextureHeight() ==
                    0 ||
                    texture3D->getTextureDepth() == 0 )
                {
                    // note we dont' have the depth here, so we'll heave to assume that
                    // height and depth are the same..
                    texture3D->setTextureSize( width, height, height );
                }
            }
            else if( 0 !=
                     ( textureCubeMap = dynamic_cast<osg::TextureCubeMap*>( texture ) ) )
            {
                if( textureCubeMap->getTextureWidth() ==
                    0 ||
            ...
            {
                if( textureRectangle->getTextureWidth() ==
                    0 ||
    ...
    if( renderTargetImplementation == osg::Camera::FRAME_BUFFER_OBJECT )
    {
        ...
        if( fbo_supported )
        {
            ...
            std::lock_guard<std::mutex> lock( *( _camera->getDataChangeMutex() ) );
            ...
            unsigned int renderBuffersMask = static_cast<unsigned int>(
                _camera->getImplicitBufferAttachmentRenderMask( true )
            );
            unsigned int resolveBuffersMask = static_cast<unsigned int>(
                _camera->getImplicitBufferAttachmentRenderMask( true )
            );
            ...
            if( ext->isRenderbufferMultisampleSupported() )
            {
                     ...
                     ++itr )
                {
                    ...
                    colorSamples =
                        maximum( colorSamples, attachment._multisampleColorSamples );
                }

                if( colorSamples > samples )
                {
                    OSG_NOTIFY( WARN )
                        << "Multisample color samples must be less than or "
                           "equal to samples. Setting color samples equal to samples."
                        << std::endl;
                ...
                if( samples )
                {
                    ...
                    // resolve mask.
                    resolveBuffersMask = static_cast<unsigned int>(
                ...
                if( fbo_multisample.valid() )
                {
                    ...
                    if( !internalFormat )
                    {
                        switch( buffer )
                        {
                            ...
                            default :
                                // setup the internal format based on attached texture if
                                // such exists, otherwise just default format
                                if( attachment._texture )
                                {
                                    internalFormat = static_cast<GLenum>(
            ...
            if( !depthAttached )
            {
                // If doing MSFBO (and therefore need two FBOs, one for multisampled
                // rendering and one for final resolve), then configure "fbo" as the
                // resolve FBO, and When done configuring, swap it into "_resolveFbo"
                // (see line 554). But, if not using MSFBO, then "fbo" is just the render
                // fbo. If using MSFBO, then resolveBuffersMask is the value set by the
                // app for the resolve buffers. But if not using MSFBO, then
                // resolveBuffersMask is the value set by the app for render buffers. In
            ...
            if( !stencilAttached )
            {
                    ...
                    osg::Camera::IMPLICIT_STENCIL_BUFFER_ATTACHMENT )
                {
                    fbo->setAttachment(
                        ...
                        osg::FrameBufferAttachment(
                            new osg::RenderBuffer( width, height, GL_STENCIL_INDEX8_EXT )
                        )
                    );
                    stencilAttached = true;
                }
            ...
            if( !colorAttached )
            {
                if( resolveBuffersMask & osg::Camera::IMPLICIT_COLOR_BUFFER_ATTACHMENT )
                {
                    fbo->setAttachment(
                        ...
                        osg::FrameBufferAttachment(
                            new osg::RenderBuffer( width, height, GL_RGB )
                        )
                    );
                    colorAttached = true;
                }
            ...
            {
                OSG_NOTICE
                    << "RenderStage::runCameraSetUp(), FBO setup failed, FBO status= 0x"
                    << std::hex << status << std::dec << std::endl;

                fbo_supported = false;
                GLuint fboId  = state.getGraphicsContext()
                                  ? state.getGraphicsContext()->getDefaultFboId()
                                  : 0;
                ext->glBindFramebuffer( GL_FRAMEBUFFER_EXT, fboId );
                fbo = 0;

                // clean up.
                osg::get<osg::GLRenderBufferManager>( state.getContextID() )
                    ->flushAllDeletedGLObjects();
                osg::get<osg::GLFrameBufferObjectManager>( state.getContextID() )
                    ->flushAllDeletedGLObjects();
            }
            else
            {
    ...
    // if any of the renderTargetImplementations require a separate graphics context such
    // as with pbuffer try in turn to set up, but if each level fails then resort to the
    // next level down.
             ...
             renderTargetImplementation == osg::Camera::SEPARATE_WINDOW ) )
    {
        ...
        if( !context )
        {
                    ...
                    case( osg::Camera::COLOR_BUFFER ) :
                        {
                            ...
                            else
                            {
                                if( attachment._texture.valid() )
                                {
                                    ...
                                    bufferFormat   = static_cast<GLenum>(
                                        attachment._texture->getInternalFormat()
                                    );
                                }
                            ...
                            {
                                traits->target =
                                    attachment._texture.valid()
                                        ? attachment._texture->getTextureTarget()
                                        : 0;
                    ...
                    default :
                        {
                            ...
                            OSG_NOTICE << "does not support multiple color outputs."
                                       << std::endl;
                            break;
                        }
                }
            ...
            {
                OSG_INFO
                    << "RenderStage::runCameraSetUp(State&) Context has been realized "
                    << std::endl;
                ...
                if( pBufferTexture &&
                    renderTargetImplementation == osg::Camera::PIXEL_BUFFER_RTT )
                {
                    OSG_INFO << "RenderStage::runCameraSetUp(State&) Assign graphics "
                                "context to Texture"
                             << std::endl;
                ...
                {
                    OSG_INFO << "RenderStage::runCameraSetUp(State&) Assigning texture "
                                "to RenderStage so that it does the copy"
                             << std::endl;
            ...
            else
            {
                ...
                {
                    // fallback to using standard PBuffer, this will allow this while
                    // loop to continue
                    if( renderTargetImplementation < renderTargetFallback )
                    {
                        renderTargetImplementation = renderTargetFallback;
    ...
    // finally if all else has failed, then the frame buffer fallback will come in to
    // play.
    if( renderTargetImplementation == osg::Camera::FRAME_BUFFER )
    {
             ...
             ++itr )
        {
            ...
            if( itr->second._texture.valid() )
            {
                setTexture( itr->second._texture.get(),
                            itr->second._level,
                            itr->second._face );
            }
        }
...
void
RenderStage::copyTexture( osg::RenderInfo& renderInfo )
{
    osg::State& state = *renderInfo.getState();
    ...
    else if( ( textureRec = dynamic_cast<osg::TextureRectangle*>( _texture.get() ) ) !=
             0 )
    {
    ...
    else if( ( texture3D = dynamic_cast<osg::Texture3D*>( _texture.get() ) ) != 0 )
    {
        ...
        texture3D->copyTexSubImage3D( state,
                                      static_cast<int>( _viewport->x() ),
                                      static_cast<int>( _viewport->y() ),
                                      static_cast<int>( _face ),
                                      static_cast<int>( _viewport->x() ),
                                      static_cast<int>( _viewport->y() ),
                                      static_cast<int>( _viewport->width() ),
                                      static_cast<int>( _viewport->height() ) );
    }
    else if( ( textureCubeMap = dynamic_cast<osg::TextureCubeMap*>( _texture.get() ) ) !=
             0 )
    {
        // need to implement
        textureCubeMap->copyTexSubImageCubeMap(
            state,
            static_cast<int>( _face ),
            static_cast<int>( _viewport->x() ),
            static_cast<int>( _viewport->y() ),
...
void
RenderStage::drawInner( osg::RenderInfo& renderInfo,
                        RenderLeaf*&     previous,
                        bool&            doCopyTexture )
{
    struct SubFunc
    {
            static void
            applyReadFBO( bool&                    apply_read_fbo,
                          const FrameBufferObject* read_fbo,
                          osg::State&              state )
            {
                if( read_fbo->isMultisample() )
                {
                    OSG_WARN << "Attempting to read from a"
                                ...
                                " framebuffer on the RenderStage to fix this."
                             << std::endl;
                }
    ...
    bool               using_multiple_render_targets =
        fbo_supported && _fbo->hasMultipleRenderingTargets();
    ...
    if( fbo_supported && _resolveFbo.valid() && ext->glBlitFramebuffer )
    {
        ...
        for( FrameBufferObject::AttachmentMap::const_iterator
                 ...
                 end = _resolveFbo->getAttachmentMap().end();
             it != end;
             ++it )
        {
            switch( it->first )
        ...
        if( needToBlitColorBuffers )
        {
                 ...
                 ++it )
            {
                ...
                {
                    state.glReadBuffer( GL_COLOR_ATTACHMENT0_EXT +
                                        ( attachment - osg::Camera::COLOR_BUFFER0 ) );
                    state.glDrawBuffer( GL_COLOR_ATTACHMENT0_EXT +
                                        ( attachment - osg::Camera::COLOR_BUFFER0 ) );
            ...
            // reset the read and draw buffers?  will comment out for now with the
            // assumption that the buffers will be set explicitly when needed elsewhere.
    ...
    for( std::map<osg::Camera::BufferComponent, Attachment>::const_iterator itr =
             _bufferAttachmentMap.begin();
         ...
         ++itr )
    {
        if( itr->second._image.valid() )
        {
            ...
            if( using_multiple_render_targets )
            {
                ...
                {
                    glReadBuffer( static_cast<GLenum>(
...
void
RenderStage::draw( osg::RenderInfo& renderInfo,
                   RenderLeaf*&     previous )
{
    ...
    if( _cameraRequiresSetUp || ( _camera.valid() &&
                                  ...
                                  _camera->getAttachmentMapModifiedCount() ) )
    {
    ...
    if( _graphicsContext.valid() && _graphicsContext != callingContext )
    {
        ...
        // OSG_NOTICE<<"  enclosing state before -
        ...
        useState->setDynamicObjectRenderingCompletedCallback(
        ...
        if( !useThread )
        {
            ...
            // OSG_NOTICE<<"  nested state before -
    ...
    bool doCopyTexture = _texture.valid() ? ( callingContext != useContext ) : false;
    ...
    if( callingContext && useContext != callingContext )
    {
        ...
        // OSG_NOTICE<<"  nested state after -
        // "<<useState->getStateSetStackSize()<<std::endl; OSG_NOTICE<<"  enclosing state
        // after - "<<state.getStateSetStackSize()<<std::endl;
...
void
RenderStage::drawImplementation( osg::RenderInfo& renderInfo,
                                 RenderLeaf*&     previous )
{
    osg::State& state = *renderInfo.getState();
    ...
    glScissor( static_cast<int>( _viewport->x() ),
               ...
               static_cast<int>( _viewport->width() ),
               static_cast<int>( _viewport->height() ) );
    // cout << "    clearing "<<this<< "  "<<_viewport->x()<<","<< _viewport->y()<<","<<
    // _viewport->width()<<","<< _viewport->height()<<std::endl;
    ...
    if( _clearMask != 0 )
    {
        ...
        if( _clearMask & GL_STENCIL_BUFFER_BIT )
        {
            ...
            glStencilMask( ~0U );
            state.haveAppliedAttribute( osg::StateAttribute::Type::STENCIL );
        }
    ...
    if( _inheritedPositionalStateContainer.valid() )
    {
        _inheritedPositionalStateContainer
            ->draw( state, previous, &_inheritedPositionalStateContainerMatrix );
    }
...
void
RenderStage::drawPostRenderStages( osg::RenderInfo& renderInfo,
                                   RenderLeaf*&     previous )
{
...
// Statistics features
bool
RenderStage::getStats( Statistics& stats ) const
{
...
void
RenderStage::attach( osg::Camera::BufferComponent buffer,
                     osg::Image*                  image )
{
...
unsigned int
RenderStage::computeNumberOfDynamicRenderLeaves() const
{
...
void
RenderStage::setMultisampleResolveFramebufferObject( osg::FrameBufferObject* fbo )
{
...
void
RenderStage::collateReferencesToDependentCameras()
{
...
void
RenderStage::clearReferencesToDependentCameras()
{
    for( RenderStageList::iterator itr = _preRenderList.begin();
         itr != _preRenderList.end();
         ++itr )
...
void
RenderStage::releaseGLObjects( osg::State* state ) const
{
