 ...
 * Polytope (frustum) intersection tester. Finds all geometry
 * inside a convex volume for area-based selection.
 */
...
namespace PolytopeIntersectorUtils
{
    ...
    struct IntersectFunctor
    {
            ...
            addIntersection()
            {
                ...
                const osg::Plane& referencePlane =
                    _settings->_polytopeIntersector->getReferencePlane();
                for( typename Vertices::iterator itr = src.begin(); itr != src.end();
                     ++itr )
                {
                    center   += *itr;
                ...
                if( src.size() <
                    PolytopeIntersector::Intersection::MaxNumIntesectionPoints )
                {
                ...
                // OSG_NOTICE<<"intersection "<<src.size()<<"
            ...
            contains()
            {
                const osg::Polytope& polytope =
                    _settings->_polytopeIntersector->getPolytope();
                    ...
                    if( resultMask & selector_mask )
                    {
                        ...
                        for( ; vitr != src.end(); ++vitr )
                        {
                            ...
                            if( d_previous * d_current < 0.0 )
                            {
                                ...
                                vec_type v_new = ( *v_previous ) *
                                                 static_cast<float>( 1.0 - r_current ) +
                                                 ( *v_current ) *
                                                 r_current;
                        ...
                        if( dest.size() <= 1 )
                        {
                            // OSG_NOTICE<<"Polytope::contains() All points on triangle
                            // culled, dest.size()="<<dest.size()<<std::endl;
                            return false;
                        }
            ...
            contains( const osg::vec3& v0 )
            {
                if( _settings->_polytopeIntersector->getPolytope().contains( v0 ) )
                {
                    ...
                    src.clear();
                    src.push_back( vec_type( v0.x, v0.y, v0.z ) );
                    return true;
                }
                else
                {
                      ...
                      const osg::vec3& v1 )
            {
                ...
                src.clear();
                src.push_back( vec_type( v0.x, v0.y, v0.z ) );
                src.push_back( vec_type( v1.x, v1.y, v1.z ) );
                      ...
                      const osg::vec3& v2 )
            {
                ...
                src.push_back( vec_type( v0.x, v0.y, v0.z ) );
                src.push_back( vec_type( v1.x, v1.y, v1.z ) );
                src.push_back( vec_type( v2.x, v2.y, v2.z ) );
                src.push_back( vec_type( v0.x, v0.y, v0.z ) );
                      ...
                      const osg::vec3& v3 )
            {
                ...
                src.push_back( vec_type( v1.x, v1.y, v1.z ) );
                src.push_back( vec_type( v2.x, v2.y, v2.z ) );
                src.push_back( vec_type( v3.x, v3.y, v3.z ) );
                src.push_back( vec_type( v0.x, v0.y, v0.z ) );
                        ...
                        bool /*treatVertexDataAsTemporary*/ )
            {
                ...
                if( ( _settings->_primitiveMask &
                      PolytopeIntersector::POINT_PRIMITIVES ) == 0 )
                {
                    ++_primitiveIndex;
                    return;
                }
                ...
                if( resultMask )
                {
                    ...
                    const osg::Polytope::PlaneList& planeList = polytope.getPlaneList();
                    for( osg::Polytope::PlaneList::const_iterator pitr =
                             planeList.begin();
                         pitr != planeList.end();
                         ++pitr )
                    {
                        ...
                        bool /*treatVertexDataAsTemporary*/ )
            {
                ...
                if( ( _settings->_primitiveMask &
                      PolytopeIntersector::LINE_PRIMITIVES ) == 0 )
                {
                    ++_primitiveIndex;
                    return;
                }
                        ...
                        bool /*treatVertexDataAsTemporary*/ )
            {
                ...
                if( ( _settings->_primitiveMask &
                      PolytopeIntersector::TRIANGLE_PRIMITIVES ) == 0 )
                {
                    ++_primitiveIndex;
                    return;
                }
...
PolytopeIntersector::PolytopeIntersector( CoordinateFrame      cf,
                                          const osg::Polytope& polytope ) :
    Intersector( cf ),
    _parent( 0 ),
    _polytope( polytope ),
...
Intersector*
PolytopeIntersector::clone( osgUtil::IntersectionVisitor& iv )
{
    ...
    osg::ref_ptr<PolytopeIntersector> pi =
        new PolytopeIntersector( transformedPolytope );
    pi->_parent            = this;
    pi->_intersectionLimit = this->_intersectionLimit;
    pi->_primitiveMask     = this->_primitiveMask;
    pi->_referencePlane    = this->_referencePlane;
...
bool
PolytopeIntersector::enter( const osg::Node& node )
{
    if( reachedLimit() )
    {
        return false;
...
void
PolytopeIntersector::leave()
{
    // do nothing.
}

void
PolytopeIntersector::intersect( osgUtil::IntersectionVisitor& iv,
                                osg::Drawable*                drawable )
{
    ...
    osg::ref_ptr<PolytopeIntersectorUtils::Settings> settings =
        new PolytopeIntersectorUtils::Settings;
    ...
    if( getPrecisionHint() == USE_DOUBLE_CALCULATIONS )
    {
        osg::TemplatePrimitiveFunctor<
            PolytopeIntersectorUtils::IntersectFunctor<osg::dvec3>>
            intersector;
    ...
    {
        osg::TemplatePrimitiveFunctor<
            PolytopeIntersectorUtils::IntersectFunctor<osg::vec3>>
            intersector;
        intersector._settings = settings;
