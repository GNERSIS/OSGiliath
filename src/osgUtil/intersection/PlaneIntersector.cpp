...
namespace PlaneIntersectorUtils
{
    ...
    {
            typedef std::vector<osg::dvec4> Polyline;
            Polyline                        _polyline;
            ...
            {
                unsigned int s = 0;
                unsigned int e = static_cast<unsigned int>( _polyline.size() ) - 1;
                for( ; s < e; ++s, --e )
                {
    ...
    class PolylineConnector
    {
        public:
            ...
            typedef std::map<osg::dvec4, osg::ref_ptr<RefPolyline>> PolylineMap;
            ...
            void
            add( const osg::dvec3& v1,
                 const osg::dvec3& v2 )
            {
                add( osg::dvec4( v1, 0.0 ), osg::dvec4( v2, 0.0 ) );
            }

            void
            add( const osg::dvec4& v1,
                 const osg::dvec4& v2 )
            {
                if( v1 == v2 )
                {
                ...
                if( v1_connections == 0 )    // v1 is no connected to anything.
                {
                    ...
                    {
                        // v2 connects to a start and an end - must have a loop in the
                        // list!
                        OSG_NOTICE << "v2=" << v2
                                   << " must connect to a start and an end - must have "
                                      "a loop!!!!!."
                                   << std::endl;
                    }
                }
                else if( v2_connections == 0 )    // v1 is no connected to anything.
                {
                    ...
                    {
                        // v1 connects to a start and an end - must have a loop in the
                        // list!
                        OSG_NOTICE << "v1=" << v1
                                   << " must connect to a start and an end - must have "
                                      "a loop!!!!!."
                ...
                {
                    // v1 and v2 connect to existing lines, now need to fuse them
                    // together.
            ...
            void
            insertAtStart( const osg::dvec4&     v,
                           PolylineMap::iterator v_start_itr )
            {
            ...
            void
            insertAtEnd( const osg::dvec4&     v,
                         PolylineMap::iterator v_end_itr )
            {
            ...
            void
            fuse_start_to_start( PolylineMap::iterator start1_itr,
                                 PolylineMap::iterator start2_itr )
            {
                osg::ref_ptr<RefPolyline> poly1 = start1_itr->second;
                osg::ref_ptr<RefPolyline> poly2 = start2_itr->second;

                PolylineMap::iterator     end1_itr =
                    _endPolylineMap.find( poly1->_polyline.back() );
                PolylineMap::iterator end2_itr =
                    _endPolylineMap.find( poly2->_polyline.back() );
            ...
            void
            fuse_start_to_end( PolylineMap::iterator start_itr,
                               PolylineMap::iterator end_itr )
            {
                osg::ref_ptr<RefPolyline> end_poly   = end_itr->second;
                osg::ref_ptr<RefPolyline> start_poly = start_itr->second;

                PolylineMap::iterator     end_start_poly_itr =
                    _endPolylineMap.find( start_poly->_polyline.back() );

                // add start_poly to end of end_poly
                end_poly->_polyline.insert( end_poly->_polyline.end(),
                                            start_poly->_polyline.begin(),
                                            start_poly->_polyline.end() );

                // reassign the end of the start poly so that it now points to the merged
                // end_poly
                end_start_poly_itr->second = end_poly;

                // remove entries for the end of the end_poly and the start of the
                // start_poly
            ...
            void
            fuse_end_to_end( PolylineMap::iterator end1_itr,
                             PolylineMap::iterator end2_itr )
            {
                ...
                PolylineMap::iterator     start1_itr =
                    _startPolylineMap.find( poly1->_polyline.front() );
                PolylineMap::iterator start2_itr =
                    _startPolylineMap.find( poly2->_polyline.front() );
            ...
            report()
            {
                ...
                for( PolylineMap::iterator sitr = _startPolylineMap.begin();
                     ...
                     ++sitr )
                {
                    OSG_NOTICE << "  line - start = " << sitr->first
                               << " polyline size = " << sitr->second->_polyline.size()
                               << std::endl;
                }
                ...
                for( PolylineMap::iterator eitr = _endPolylineMap.begin();
                     ...
                     ++eitr )
                {
                    OSG_NOTICE << "  line - end = " << eitr->first
                               << " polyline size = " << eitr->second->_polyline.size()
                               << std::endl;
                }
    ...
    struct TriangleIntersector
    {
            ...
            void
            set( const osg::Plane&    plane,
                 const osg::Polytope& polytope,
                 osg::RefMatrix*      matrix,
                 bool                 recordHeightsAsAttributes,
            ...
            inline double
            distance( const osg::Plane& plane,
                      const osg::dvec4& v ) const
            {
                 ...
                 osg::dvec4& ve )
            {
                ...
                {
                    for( osg::Polytope::PlaneList::iterator itr =
                             _polytope.getPlaneList().begin();
                        ...
                        const osg::vec3& v3 )
            {
                ...
                osg::dvec4   p1( v1, v1.z );
                osg::dvec4   p2( v2, v2.z );
                osg::dvec4   p3( v3, v3.z );
                ...
                if( _em.valid() )
                {
                    ...
                    if( _matrix.valid() )
                    {
                        osg::dvec3 tp = ( *_matrix ) * v1;
                        _em->convertXYZToLatLongHeight( tp.x,
                                                        tp.y,
                                                        tp.z,
                                                        latitude,
                                                        longitude,
                    ...
                    {
                        _em->convertXYZToLatLongHeight( v1.x,
                                                        v1.y,
                                                        v1.z,
                                                        latitude,
                                                        longitude,
                        ...
                        _em->convertXYZToLatLongHeight( v2.x,
                                                        v2.y,
                                                        v2.z,
                                                        latitude,
                                                        longitude,
                        ...
                        _em->convertXYZToLatLongHeight( v3.x,
                                                        v3.y,
                                                        v3.z,
                                                        latitude,
                                                        longitude,
                ...
                if( d1 * d3 < 0.0 )
                {
                    ...
                    {
                        OSG_NOTICE << "!!! too many intersecting edges found !!!"
                                   << std::endl;
                    }
                }
...
//  PlaneIntersector
//
PlaneIntersector::PlaneIntersector( const osg::Plane&    plane,
                                    const osg::Polytope& boundingPolytope ) :
    _parent( 0 ),
    _recordHeightsAsAttributes( false ),
    _plane( plane ),
...
PlaneIntersector::PlaneIntersector( CoordinateFrame      cf,
...
Intersector*
PlaneIntersector::clone( osgUtil::IntersectionVisitor& iv )
{
    ...
    osg::ref_ptr<PlaneIntersector> pi =
        new PlaneIntersector( plane, transformedPolytope );
    pi->_parent                    = this;
    pi->_intersectionLimit         = this->_intersectionLimit;
    pi->_recordHeightsAsAttributes = _recordHeightsAsAttributes;
    pi->_em                        = _em;
...
bool
PlaneIntersector::enter( const osg::Node& node )
{
    if( reachedLimit() )
    {
        return false;
    }
    return !node.isCullingActive() || ( _plane.intersect( node.getBound() ) ==
                                        0 &&
                                        _polytope.contains( node.getBound() ) );
}

void
PlaneIntersector::leave()
{
    // do nothing.
}

void
PlaneIntersector::intersect( osgUtil::IntersectionVisitor& iv,
                             osg::Drawable*                drawable )
{
    // OSG_NOTICE<<"PlaneIntersector::intersect(osgUtil::IntersectionVisitor& iv,
    // osg::Drawable* drawable)"<<std::endl;
    ...
    // OSG_NOTICE<<"Succed PlaneIntersector::intersect(osgUtil::IntersectionVisitor& iv,
    // osg::Drawable* drawable)"<<std::endl;
    ...
    ti.set( _plane,
            _polytope,
            iv.getModelMatrix(),
            _recordHeightsAsAttributes,
            _em.get() );
    ti._limitOneIntersection = ( _intersectionLimit ==
    ...
    if( ti._hit )
    {
        ...
        for( PlaneIntersectorUtils::PolylineConnector::PolylineList::iterator pitr =
                 ti._polylineConnector._polylines.begin();
             pitr != ti._polylineConnector._polylines.end();
             ++pitr )
        {

            unsigned int pos = static_cast<unsigned int>( intersections.size() );
            ...
            for( PlaneIntersectorUtils::RefPolyline::Polyline::iterator vitr =
                     ( *pitr )->_polyline.begin();
                 vitr != ( *pitr )->_polyline.end();
                 ++vitr )
            {
                const osg::dvec4& v = *vitr;
                new_intersection.polyline.push_back( osg::dvec3( v.x, v.y, v.z ) );
                if( _recordHeightsAsAttributes )
                {
