char volume_compute_ray_color_frag[] =
    "#version 460 core\n"
    "\n"
    ...
    "\n"
    "in mat4 texgen_eyeToTile;\n"
    "\n"
    "// forward declare, probided by volume_accumulateSamples*.frag shaders\n"
    "vec4 accumulateSamples(vec4 fragColor, vec3 ts, vec3 te, vec3 dt, float scale, "
    "float cutoff, int num_iterations);\n"
    "\n"
    ...
    "\n"
    "    fragColor = accumulateSamples(fragColor, ts, te, deltaTexCoord, scale, cutoff, "
    "num_iterations);\n"
    "\n"
    "    fragColor *= baseColor;\n"
    "\n"
    ...
    "    vec4 start_clip = vec4((px/viewportWidth)*2.0-1.0, "
    "(py/viewportHeight)*2.0-1.0, (depth_start)*2.0-1.0, 1.0);\n"
    "    vec4 end_clip = vec4((px/viewportWidth)*2.0-1.0, (py/viewportHeight)*2.0-1.0, "
    "(depth_end)*2.0-1.0, 1.0);\n"
    "\n"
    ...
    "    end_tile.w = 1.0;\n"
    "\n"
    "    vec4 clamped_start_tile = vec4(clampToUnitCube(end_tile.xyz, start_tile.xyz), "
    "1.0);\n"
    "    vec4 clamped_end_tile = vec4(clampToUnitCube(start_tile.xyz, end_tile.xyz), "
    "1.0);\n"
    "\n"
    ...
    "    end_texcoord.w = 1.0;\n"
    "\n"
    "    vec3 clamped_start_texcoord = clampToUnitCube(end_texcoord.xyz, "
    "start_texcoord.xyz);\n"
    "    vec3 clamped_end_texcoord = clampToUnitCube(start_texcoord.xyz, "
    "end_texcoord.xyz);\n"
    "\n"
    "    return accumulateSegment(fragColor, clamped_start_texcoord, "
    "clamped_end_texcoord);\n"
    "}\n"
    "\n";
