...
namespace osgVolume
{
    ...
    osg::Image*
    createDownsampledImage( osg::Image* sourceImage )
    {
        ...
        OSG_NOTICE << "createDownsampledImage(" << sourceImage->s() << ", "
                   << sourceImage->t() << ", " << sourceImage->r() << ")" << std::endl;
        OSG_NOTICE << "  s_base = " << s_base << ", s_odd = " << s_odd << ", s=" << s
                   << std::endl;
        OSG_NOTICE << "  t_base = " << t_base << ", t_odd = " << t_odd << ", t=" << t
                   << std::endl;
        OSG_NOTICE << "  r_base = " << r_base << ", r_odd = " << r_odd << ", r=" << r
                   << std::endl;

        targetImage->allocateImage( s,
                                    t,
                                    r,
        ...
        for( unsigned int plane = 0; plane < static_cast<unsigned int>( r_base );
             ++plane )
        {
            for( unsigned int row = 0; row < static_cast<unsigned int>( t_base ); ++row )
            {
#if 0
...
#endif
                T* ptr_source_1 =
                    reinterpret_cast<T*>( sourceImage->data( 0, row * 2, plane * 2 ) );
                T* ptr_source_2 =
                    reinterpret_cast<T*>( sourceImage->data( 1, row * 2, plane * 2 ) );
                T* ptr_source_3 = reinterpret_cast<T*>(
                    sourceImage->data( 0, row * 2 + 1, plane * 2 )
                );
                T* ptr_source_4 = reinterpret_cast<T*>(
                    sourceImage->data( 1, row * 2 + 1, plane * 2 )
                );
                T* ptr_source_5 = reinterpret_cast<T*>(
                    sourceImage->data( 0, row * 2, plane * 2 + 1 )
                );
                T* ptr_source_6 = reinterpret_cast<T*>(
                    sourceImage->data( 1, row * 2, plane * 2 + 1 )
                );
                T* ptr_source_7 = reinterpret_cast<T*>(
                    sourceImage->data( 0, row * 2 + 1, plane * 2 + 1 )
                );
                T* ptr_source_8 = reinterpret_cast<T*>(
                    sourceImage->data( 1, row * 2 + 1, plane * 2 + 1 )
                );
                T* ptr_target =
                    reinterpret_cast<T*>( targetImage->data( 0, row, plane ) );
...
#else
                        TL value = 0;
                        value    = std::max( static_cast<TL>( *ptr_source_1 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_2 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_3 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_4 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_5 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_6 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_7 ), value );
                        value    = std::max( static_cast<TL>( *ptr_source_8 ), value );
#endif
    ...
    class RTTCameraCullCallback : public osg::NodeCallback
    {
        public:
            ...
            RTTCameraCullCallback( MultipassTechnique::MultipassTileData* tileData,
                                   MultipassTechnique*                    mt ) :
                _tileData( tileData ),
                _mt( mt )
            {
            }
                        ...
                        osg::NodeVisitor* nv )
            {
                ...
                // OSG_NOTICE<<"Traversing tile subgraph for pre-render
                // "<<_mt->getVolumeTile()->getNumChildren()<<std::endl;
        ...
        protected:
            ...
            osg::observer_ptr<osgVolume::MultipassTechnique::MultipassTileData>
                                                             _tileData;
            osg::observer_ptr<osgVolume::MultipassTechnique> _mt;
    };

    MultipassTechnique::MultipassTileData::MultipassTileData( osgUtil::CullVisitor* cv,
        ...
        multipassTechnique( mpt )
    {
        ...
        eyeToTileUniform = new osg::Uniform( "eyeToTile", osg::mat4() );
        stateset->addUniform( eyeToTileUniform.get() );

        tileToImageUniform = new osg::Uniform( "tileToImage", osg::mat4() );
        ...
        switch( currentRenderingMode )
        {
            case( MultipassTechnique::CUBE ) :
                {
                    ...
                    OSG_NOTICE << "Setting up MultipassTileData for CUBE rendering"
                               << std::endl;
            ...
            case( MultipassTechnique::HULL ) :
                {
                    OSG_NOTICE << "Setting up MultipassTileData for HULL rendering"
                               << std::endl;
                    setUp( frontFaceRttCamera, frontFaceDepthTexture, width, height );
                    frontFaceRttCamera->setName( "frontFaceRttCamera" );
                    frontFaceRttCamera->setCullCallback(
                        new RTTCameraCullCallback( this, mpt )
                    );
                    frontFaceRttCamera->getOrCreateStateSet()->setAttributeAndModes(
                        new osg::CullFace( osg::CullFace::Mode::BACK ),
                        osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE
                    );

                    stateset->setTextureAttribute( 2,
                                                   frontFaceDepthTexture.get(),
            ...
            case( MultipassTechnique::CUBE_AND_HULL ) :
                {
                    OSG_NOTICE
                        << "Setting up MultipassTileData for CUBE_AND_HULL rendering"
                        << std::endl;
                    setUp( frontFaceRttCamera, frontFaceDepthTexture, width, height );
                    frontFaceRttCamera->setName( "frontFaceRttCamera" );
                    frontFaceRttCamera->setCullCallback(
                    ...
                    backFaceRttCamera->setCullCallback(
                        new RTTCameraCullCallback( this, mpt )
                    );
                    backFaceRttCamera->getOrCreateStateSet()->setAttributeAndModes(
                    ...
                    stateset->setTextureAttribute( 2,
                                                   ...
                                                   osg::StateAttribute::ON |
                                                       osg::StateAttribute::OVERRIDE );
                    stateset->setTextureAttribute( 3,
                                                   backFaceDepthTexture.get(),
                                                   osg::StateAttribute::ON |
                                                       osg::StateAttribute::OVERRIDE );
                    break;
                }
    ...
    void
    MultipassTechnique::MultipassTileData::setUp(
    ...
    )
    {
        ...
        depthTexture->setBorderColor( osg::dvec4( 1.0F, 1.0F, 1.0F, 1.0F ) );
        ...
        camera->setProjectionMatrix( osg::dmat4() );
        camera->setViewMatrix( osg::dmat4() );
    }

    void
    MultipassTechnique::MultipassTileData::update( osgUtil::CullVisitor* cv )
    {
        ...
        {
            if( frontFaceDepthTexture->getTextureWidth() !=
                width ||
                frontFaceDepthTexture->getTextureHeight() != height )
            {
                OSG_NOTICE << "Need to change texture size to " << width << ", "
                           << height << std::endl;
        ...
        {
            if( backFaceDepthTexture->getTextureWidth() !=
                width ||
                backFaceDepthTexture->getTextureHeight() != height )
            {
                OSG_NOTICE << "Need to change texture size to " << width << ", "
                           << height << std::endl;
    ...
    MultipassTechnique::MultipassTechnique( const MultipassTechnique& fft,
    ...
    osg::StateSet*
    MultipassTechnique::createStateSet( osg::StateSet* statesetPrototype,
                                        osg::Program*  programPrototype,
                                        osg::Shader*   shaderToAdd1,
                                        osg::Shader*   shaderToAdd2 )
    {
        osg::ref_ptr<osg::StateSet> stateset =
            osg::clone( statesetPrototype, osg::CopyOp::SHALLOW_COPY );
        osg::ref_ptr<osg::Program> program =
            osg::clone( programPrototype, osg::CopyOp::SHALLOW_COPY );
    ...
    MultipassTechnique::RenderingMode
    MultipassTechnique::computeRenderingMode()
    {
        ...
        double etfValue = cpv._exteriorTransparencyFactorProperty.valid()
    ...
    void
    MultipassTechnique::init()
    {
        OSG_INFO << "MultipassTechnique::init()" << std::endl;

        if( !_volumeTile )
        {
            OSG_NOTICE << "MultipassTechnique::init(), error no volume tile assigned."
                       << std::endl;
            return;
        }
        ...
        {
            OSG_NOTICE
                << "MultipassTechnique::init(), error no layer assigned to volume tile."
                << std::endl;
            return;
        }
        ...
        {
            OSG_NOTICE << "MultipassTechnique::init(), error no image assigned to layer."
                       << std::endl;
            return;
        }
...

        {
            ...
            // triangle strip of cube based on :
            // http://www.cs.umd.edu/gvil/papers/av_ts.pdf
            osg::Vec3Array* coords = new osg::Vec3Array( 8 );
            ( *coords )[0]         = osg::dvec3( 0.0, 1.0, 1.0 );    // Back-top-left
            ( *coords )[1]         = osg::dvec3( 1.0, 1.0, 1.0 );    // Back-top-right
            ( *coords )[2]         = osg::dvec3( 0.0, 0.0, 1.0 );    // Front-top-left
            ( *coords )[3]         = osg::dvec3( 1.0, 0.0, 1.0 );    // Front-top-right
            ( *coords )[4]         = osg::dvec3( 0.0, 1.0, 0.0 );    // Back-bottom-left
            ( *coords )[5]         = osg::dvec3( 1.0, 1.0, 0.0 );    // Back-bottom-right
            ( *coords )[6] = osg::dvec3( 1.0, 0.0, 0.0 );    // Front-bottom-right
            ( *coords )[7] = osg::dvec3( 0.0, 0.0, 0.0 );    // Front-bottom-left
        ...
        osg::dmat4 geometryMatrix;
        if( masterLocator )
        {
            geometryMatrix = masterLocator->getTransform();
            _transform->setMatrix( geometryMatrix );
            masterLocator->applyAppropriateFrontFace(
                _transform->getOrCreateStateSet()
            );
            masterLocator->addCallback(
                new TransformLocatorCallback( _transform.get() )
            );
        }

        osg::dmat4 imageMatrix;
        if( layerLocator )
        {
            imageMatrix = layerLocator->getTransform();
        }

        OSG_NOTICE << "MultipassTechnique::init() : geometryMatrix = " << geometryMatrix
                   << std::endl;
        OSG_NOTICE << "MultipassTechnique::init() : imageMatrix = " << imageMatrix
                   << std::endl;
        ...
        // set up uniforms
        {
            ...
            {
                stateset->addUniform(
                    cpv._exteriorTransparencyFactorProperty->getUniform()
                );
            }
            else
            {
                stateset->addUniform(
                    new osg::Uniform( "ExteriorTransparencyFactorValue", 0.0F )
                );
            }
            ...
            {
                stateset->addUniform( new osg::Uniform( "AlphaFuncValue",
                                                        alphaFuncValue ) );
            }
            ...
            {
                stateset->addUniform( new osg::Uniform( "IsoSurfaceValue",
                                                        alphaFuncValue ) );
            }

            if( cpv._tfProperty.valid() )
            {
                tf = dynamic_cast<osg::TransferFunction1D*>(
                    cpv._tfProperty->getTransferFunction()
                );
            }
        }
        ...
        // osg::ref_ptr<osg::Image> downsampled_image_3d =
        // createDownsampledImage(image_3d.get());
...
#define VOLUME_TYPE 2
            ...
            texture3D->setWrap( osg::Texture3D::WRAP_R,
                                osg::Texture3D::CLAMP_TO_BORDER );
            texture3D->setWrap( osg::Texture3D::WRAP_S,
                                osg::Texture3D::CLAMP_TO_BORDER );
            texture3D->setWrap( osg::Texture3D::WRAP_T,
                                osg::Texture3D::CLAMP_TO_BORDER );
            // texture3D->setMaxAnisotropy(16.0f);
            texture3D->setBorderColor( osg::dvec4( 0.0, 0.0, 0.0, 0.0 ) );
            if( image_3d->getPixelFormat() == GL_RED )
            {
                texture3D->setInternalFormatMode(
                    osg::Texture3D::USE_USER_DEFINED_FORMAT
                );
                texture3D->setInternalFormat( GL_RED );
            }
            else
            {
            ...
            stateset->setTextureAttributeAndModes( volumeTextureUnit,
                                                   texture3D.get(),
                                                   osg::StateAttribute::ON |
                                                       osg::StateAttribute::OVERRIDE );

            osg::ref_ptr<osg::Uniform> baseTextureSampler =
                new osg::Uniform( "volumeTexture", int( volumeTextureUnit ) );
            stateset->addUniform( baseTextureSampler.get() );

            osg::ref_ptr<osg::Uniform> volumeCellSize = new osg::Uniform(
                "volumeCellSize",
                osg::vec3( 1.0F / static_cast<float>( image_3d->s() ),
                           1.0F / static_cast<float>( image_3d->t() ),
                           1.0F / static_cast<float>( image_3d->r() ) )
            );
            stateset->addUniform( volumeCellSize.get() );

            OSG_NOTICE << "Texture Dimensions " << image_3d->s() << ", " << image_3d->t()
                       << ", " << image_3d->r() << std::endl;
        }
        ...
        if( tf )
        {
            ...
            ImageLayer* imageLayer =
                dynamic_cast<ImageLayer*>( _volumeTile->getLayer() );
            if( imageLayer )
            {
                tfOffset = ( imageLayer->getTexelOffset()[3] - tf->getMinimum() ) /
                           ( tf->getMaximum() - tf->getMinimum() );
                tfScale  = imageLayer->getTexelScale()[3] /
                           ( tf->getMaximum() - tf->getMinimum() );
            }
            else
            {
            ...
            stateset->setTextureAttributeAndModes( transferFunctionTextureUnit,
                                                   tf_texture.get(),
                                                   osg::StateAttribute::ON |
                                                       osg::StateAttribute::OVERRIDE );
            stateset->addUniform(
                new osg::Uniform( "tfTexture", int( transferFunctionTextureUnit ) )
            );
        ...
        osg::ref_ptr<osg::CullFace> front_CullFace =
            new osg::CullFace( osg::CullFace::Mode::BACK );
        osg::ref_ptr<osg::CullFace> back_CullFace =
            new osg::CullFace( osg::CullFace::Mode::FRONT );

        osg::ref_ptr<osg::Shader> main_vertexShader =
            osgDB::readRefShaderFile( osg::Shader::VERTEX,
                                      "shaders/volume_multipass.vert" );
        if( !main_vertexShader )
        {
#include "shaders/volume_multipass_vert.cpp"
            main_vertexShader =
                new osg::Shader( osg::Shader::VERTEX, volume_multipass_vert );
        }

        osg::ref_ptr<osg::Shader> computeRayColorShader =
            osgDB::readRefShaderFile( osg::Shader::FRAGMENT,
                                      "shaders/volume_compute_ray_color.frag" );
        if( !computeRayColorShader )
        {
#include "shaders/volume_compute_ray_color_frag.cpp"
            computeRayColorShader =
                new osg::Shader( osg::Shader::FRAGMENT, volume_compute_ray_color_frag );
        }
        ...
        osg::ref_ptr<osg::Shader> cube_main_fragmentShader =
            osgDB::readRefShaderFile( osg::Shader::FRAGMENT,
                                      "shaders/volume_multipass_cube.frag" );
        ;
        if( !cube_main_fragmentShader )
        {
#include "shaders/volume_multipass_cube_frag.cpp"
            cube_main_fragmentShader =
                new osg::Shader( osg::Shader::FRAGMENT, volume_multipass_cube_frag );
        }
        ...
        {
            cube_stateset_prototype->setAttributeAndModes(
                back_CullFace.get(),
                osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE
            );
        ...
        osg::ref_ptr<osg::Shader> hull_main_fragmentShader =
            osgDB::readRefShaderFile( osg::Shader::FRAGMENT,
                                      "shaders/volume_multipass_hull.frag" );
        ;
        if( !hull_main_fragmentShader )
        {
#include "shaders/volume_multipass_hull_frag.cpp"
            hull_main_fragmentShader =
                new osg::Shader( osg::Shader::FRAGMENT, volume_multipass_hull_frag );
        }
        ...
        {
            hull_stateset_prototype->setAttributeAndModes(
                back_CullFace.get(),
                osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE
            );
            hull_stateset_prototype->addUniform(
                new osg::Uniform( "frontFaceDepthTexture", 2 )
            );
        ...
        // set up the program template for rendering just the cube
        osg::ref_ptr<osg::Shader> cube_and_hull_main_fragmentShader =
            osgDB::readRefShaderFile( osg::Shader::FRAGMENT,
                                      "shaders/volume_multipass_cube_and_hull.frag" );
        ;
        if( !cube_and_hull_main_fragmentShader )
        {
#include "shaders/volume_multipass_cube_and_hull_frag.cpp"
            cube_and_hull_main_fragmentShader =
                new osg::Shader( osg::Shader::FRAGMENT,
                                 volume_multipass_cube_and_hull_frag );
        }
        ...
        {
            cube_and_hull_stateset_prototype->setAttributeAndModes(
                back_CullFace.get(),
                osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE
            );
            cube_and_hull_stateset_prototype->addUniform(
                new osg::Uniform( "frontFaceDepthTexture", 2 )
            );
            cube_and_hull_stateset_prototype->addUniform(
                new osg::Uniform( "backFaceDepthTexture", 3 )
            );

            cube_and_hull_program_prototype->addShader( main_vertexShader.get() );
            cube_and_hull_program_prototype->addShader(
                cube_and_hull_main_fragmentShader.get()
            );
        ...
        // set up the rendering of the front face
        {
            ...
            _frontFaceStateSet->setAttributeAndModes(
                front_CullFace.get(),
                osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE
            );
            ...
            _frontFaceStateSet->setAttribute( program.get(),
                                              osg::StateAttribute::ON |
                                                  osg::StateAttribute::OVERRIDE );
        }
        ...
        // STANDARD_SHADERS
        {
            ...
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_standard.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_standard_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_standard_frag );
                }

                _stateSetMap[STANDARD_SHADERS | CUBE_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[STANDARD_SHADERS | HULL_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[STANDARD_SHADERS | CUBE_AND_HULL_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
            ...
            if( tf )
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_standard_tf.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_standard_tf_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_standard_tf_frag );
                }

                _stateSetMap[STANDARD_SHADERS | CUBE_SHADERS | TF_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[STANDARD_SHADERS | HULL_SHADERS | TF_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[STANDARD_SHADERS | CUBE_AND_HULL_SHADERS | TF_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
        ...
        if( cpv._isoProperty.valid() )
        {
            // ISO_SHADERS without TransferFunction
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_iso.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_iso_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_iso_frag );
                }

                _stateSetMap[ISO_SHADERS | CUBE_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[ISO_SHADERS | HULL_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[ISO_SHADERS | CUBE_AND_HULL_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
            ...
            if( tf )
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_iso_tf.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_iso_tf_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_iso_tf_frag );
                }

                _stateSetMap[ISO_SHADERS | CUBE_SHADERS | TF_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[ISO_SHADERS | HULL_SHADERS | TF_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[ISO_SHADERS | CUBE_AND_HULL_SHADERS | TF_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
        ...
        if( cpv._mipProperty.valid() )
        {
            // MIP_SHADERS without TransferFunction
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_mip.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_mip_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_mip_frag );
                }

                _stateSetMap[MIP_SHADERS | CUBE_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[MIP_SHADERS | HULL_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[MIP_SHADERS | CUBE_AND_HULL_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
            ...
            if( tf )
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_mip_tf.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_mip_tf_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_mip_tf_frag );
                }

                _stateSetMap[MIP_SHADERS | CUBE_SHADERS | TF_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[MIP_SHADERS | HULL_SHADERS | TF_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[MIP_SHADERS | CUBE_AND_HULL_SHADERS | TF_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
        ...
        if( cpv._lightingProperty.valid() )
        {
            // LIT_SHADERS without TransferFunction
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_lit.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_lit_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_lit_frag );
                }

                _stateSetMap[LIT_SHADERS | CUBE_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[LIT_SHADERS | HULL_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[LIT_SHADERS | CUBE_AND_HULL_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
            ...
            if( tf )
            {
                osg::ref_ptr<osg::Shader> accumulateSamplesShader =
                    osgDB::readRefShaderFile(
                        osg::Shader::FRAGMENT,
                        "shaders/volume_accumulateSamples_lit_tf.frag"
                    );
                if( !accumulateSamplesShader )
                {
#include "shaders/volume_accumulateSamples_lit_tf_frag.cpp"
                    accumulateSamplesShader =
                        new osg::Shader( osg::Shader::FRAGMENT,
                                         volume_accumulateSamples_lit_tf_frag );
                }

                _stateSetMap[LIT_SHADERS | CUBE_SHADERS | TF_SHADERS] =
                    createStateSet( cube_stateset_prototype.get(),
                                    cube_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[LIT_SHADERS | HULL_SHADERS | TF_SHADERS] =
                    createStateSet( hull_stateset_prototype.get(),
                                    hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
                _stateSetMap[LIT_SHADERS | CUBE_AND_HULL_SHADERS | TF_SHADERS] =
                    createStateSet( cube_and_hull_stateset_prototype.get(),
                                    cube_and_hull_program_prototype.get(),
                                    accumulateSamplesShader.get() );
            }
        ...
        if( cpv._sampleRatioWhenMovingProperty.valid() )
        {
            ...
            _whenMovingStateSet->addUniform(
                cpv._sampleRatioWhenMovingProperty->getUniform(),
    ...
    void
    MultipassTechnique::update( osgUtil::UpdateVisitor* uv )
    {
        // OSG_NOTICE<<"MultipassTechnique:update(osgUtil::UpdateVisitor*
        // uv):"<<std::endl;
    ...
    void
    MultipassTechnique::backfaceSubgraphCullTraversal( osgUtil::CullVisitor* cv )
    {
        if( !cv )
        {
            return;
    ...
    class ShadingModelVisitor : public osgVolume::PropertyVisitor
    {
        public:
            ...
            virtual void
            apply( TransferFunctionProperty& /*tf*/ )
            {
    ...
    void
    MultipassTechnique::cull( osgUtil::CullVisitor* cv )
    {
        std::string traversalPass;
        bool rttTraversal = cv->getUserValue( "VolumeSceneTraversal", traversalPass ) &&
                            traversalPass == "RenderToTexture";

        // OSG_NOTICE<<"MultipassTechnique::cull()
        // traversalPass="<<traversalPass<<std::endl;
        ...
        if( rttTraversal )
        {
            if( vs )
            {
                MultipassTileData* tileData = dynamic_cast<MultipassTileData*>(
        ...
        else
        {
            ...
            if( _volumeTile->getLayer()->getProperty() )
            {
                ...
                {
                    case( VolumeSettings::Isosurface ) :
                        shaderMask |= ISO_SHADERS;
                        break;
                    case( VolumeSettings::MaximumIntensityProjection ) :
                        shaderMask |= MIP_SHADERS;
                        break;
                    case( VolumeSettings::Light ) :
            ...
            {
                case( CUBE ) :
                    shaderMask |= CUBE_SHADERS;
                    break;
                case( HULL ) :
                    shaderMask |= HULL_SHADERS;
                    break;
                case( CUBE_AND_HULL ) :
                    shaderMask |= CUBE_AND_HULL_SHADERS;
                    break;
            }
            ...
            {
                MultipassTileData* tileData = dynamic_cast<MultipassTileData*>(
                    vs->getTileData( cv, getVolumeTile() )
                );
                if( tileData )
                {
                    Locator*   tileLocator = _volumeTile->getLocator();
                    osg::dmat4 tileToEye =
                        tileLocator->getTransform() * ( *( cv->getModelViewMatrix() ) );
                    osg::dmat4 eyeToTile;
                    eyeToTile = osg::inverse( tileToEye );

                    tileData->eyeToTileUniform->set( osg::mat4( eyeToTile ) );

                    Locator* layerLocator = _volumeTile->getLayer()->getLocator();
                    if( tileLocator == layerLocator )
                    {
                        tileData->tileToImageUniform->set( osg::mat4() );
                    }
                    else
                    {
                        osg::dmat4 tileToImage(
                            tileLocator->getTransform() *
                            osg::inverse( layerLocator->getTransform() )
                        );
                        tileData->tileToImageUniform->set( osg::mat4( tileToImage ) );
                    }
            ...
            osg::ref_ptr<osg::StateSet> moving_stateset =
                ( _whenMovingStateSet.valid() && isMoving( cv ) ) ? _whenMovingStateSet
                                                                  : 0;
            ...
            if( program_stateset.valid() )
            {
                ...
                {
                    case( CUBE ) :
                        // OSG_NOTICE<<"Traversing Transform for CUBE
                        // rendering"<<std::endl;
                        _transform->accept( *cv );
                        break;
                    case( HULL ) :
                        // OSG_NOTICE<<"Traversing children for HULL
                        // rendering"<<std::endl;
                        getVolumeTile()->osg::Group::traverse( *cv );
                        break;
                    case( CUBE_AND_HULL ) :
                        // OSG_NOTICE<<"Traversing Transform for CUBE_AND_HULL
                        // rendering"<<std::endl;
                        _transform->accept( *cv );
                        // getVolumeTile()->osg::Group::traverse(*cv);
                        break;
            ...
            {
                OSG_NOTICE << "Warning: No program available for required shader mask "
                           << shaderMask << std::endl;
            }
    ...
    void
    MultipassTechnique::cleanSceneGraph()
    {
        OSG_NOTICE << "MultipassTechnique::cleanSceneGraph()" << std::endl;
    }

    void
    MultipassTechnique::traverse( osg::NodeVisitor& nv )
    {
