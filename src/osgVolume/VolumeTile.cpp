...
VolumeTile::VolumeTile( const VolumeTile&  volumeTile,
...
void
VolumeTile::setVolume( Volume* volume )
{
    if( _volume == volume )
    {
        return;
...
void
VolumeTile::setTileID( const TileID& tileID )
{
    if( _tileID == tileID )
    {
        return;
...
void
VolumeTile::traverse( osg::NodeVisitor& nv )
{
...
void
VolumeTile::setLayer( Layer* layer )
{
    _layer = layer;
}

void
VolumeTile::setVolumeTechnique( VolumeTechnique* volumeTechnique )
{
...
void
VolumeTile::setDirty( bool dirty )
{
    if( _dirty == dirty )
    {
        return;
...
VolumeTile::computeBound() const
{
    ...
    else if( _layer.valid() )
    {
        ...
        return osg::sphere( osg::vec3( 0.5F, 0.5F, 0.5F ), 0.867F );
    }
    else
    {
        return osg::sphere();
