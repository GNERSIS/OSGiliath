...
void
AnimationManagerBase::clearTargets()
{
...
void
AnimationManagerBase::dirty()
{
    _needToLink = true;
}

void
AnimationManagerBase::setAutomaticLink( bool state )
{
    _automaticLink = state;
}

bool
AnimationManagerBase::getAutomaticLink() const
{
    return _automaticLink;
}

void
AnimationManagerBase::operator()( osg::Node*        node,
                                  osg::NodeVisitor* nv )
{
...
AnimationManagerBase::AnimationManagerBase( const AnimationManagerBase& b,
...
void
AnimationManagerBase::buildTargetReference()
{
    _targets.clear();
    for( AnimationList::iterator iterAnim = _animations.begin();
...
void
AnimationManagerBase::registerAnimation( Animation* animation )
{
    _needToLink = true;
    _animations.push_back( animation );
    buildTargetReference();
...
void
AnimationManagerBase::removeRegisteredAnimation( Animation* animation )
{
    unregisterAnimation( animation );
}

void
AnimationManagerBase::unregisterAnimation( Animation* animation )
{
    AnimationList::iterator it =
        std::find( _animations.begin(), _animations.end(), animation );
...
bool
AnimationManagerBase::needToLink() const
{
    return _needToLink && getAutomaticLink();
}

void
AnimationManagerBase::setLinkVisitor( LinkVisitor* visitor )
{
    _linker = visitor;
}

LinkVisitor*
AnimationManagerBase::getOrCreateLinkVisitor()
{
    if( !_linker.valid() )
    {
...
void
AnimationManagerBase::link( osg::Node* subgraph )
{
