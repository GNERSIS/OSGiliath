...
Timeline::Timeline( const Timeline&    nc,
                    const osg::CopyOp& op ) :
    Action( nc,
            op ),
...
void
Timeline::setAnimationManager( AnimationManagerBase* manager )
{
    _animationManager = manager;
}

void
Timeline::traverse( ActionVisitor& visitor )
{
...
void
Timeline::setStats( osg::Stats* stats )
{
    _stats = stats;
}
...
void
Timeline::collectStats( bool state )
{
    _collectStats = state;
}

StatsActionVisitor*
Timeline::getStatsVisitor()
{
...
void
Timeline::update( double simulationTime )
{
...
void
Timeline::removeAction( Action* action )
{
    if( getEvaluating() )
    {
...
void
Timeline::addActionAt( unsigned int frame,
                       Action*      action,
                       int          priority )
{
    // skip if this action has already been added this frame
    for( CommandList::iterator it = _addActionOperations.begin();
         it != _addActionOperations.end();
         ++it )
    {
        Command& command = *it;
        if( command._action.second.get() == action )
        {
            OSG_INFO << "Timeline::addActionAt command " << action->getName()
                     << " already added this frame, declined" << std::endl;
            return;
        }
    }
    ...
    {
        OSG_INFO << "Timeline::addActionAt command " << action->getName()
                 << " already active, remove the old" << std::endl;
        removeAction( action );
    }

    if( getEvaluating() )
    {
        _addActionOperations.push_back( Command( priority,
...
void
Timeline::addActionNow( Action* action,
                        int     priority )
{
...
void
Timeline::processPendingOperation()
{
...
void
Timeline::internalRemoveAction( Action* action )
{
...
void
Timeline::internalAddAction( int                priority,
...
bool
Timeline::isActive( Action* activeAction )
{
    // update from high priority to low priority
    for( ActionLayers::iterator iterAnim = _actions.begin(); iterAnim != _actions.end();
         ++iterAnim )
    {
