...
void
ActionVisitor::pushFrameActionOnStack( const FrameAction& fa )
{
    _stackFrameAction.push_back( fa );
}

void
ActionVisitor::popFrameAction()
{
    _stackFrameAction.pop_back();
}

void
ActionVisitor::pushTimelineOnStack( Timeline* tm )
{
    _stackTimeline.push_back( tm );
}

void
ActionVisitor::popTimeline()
{
    _stackTimeline.pop_back();
}

void
ActionVisitor::apply( Action& action )
{
    traverse( action );
}

void
ActionVisitor::apply( Timeline& tm )
{
    tm.traverse( *this );
}

void
ActionVisitor::apply( ActionBlendIn& action )
{
    apply( static_cast<Action&>( action ) );
}

void
ActionVisitor::apply( ActionBlendOut& action )
{
    apply( static_cast<Action&>( action ) );
}

void
ActionVisitor::apply( ActionAnimation& action )
{
    apply( static_cast<Action&>( action ) );
}

void
ActionVisitor::apply( ActionStripAnimation& action )
{
    apply( static_cast<Action&>( action ) );
}

void
ActionVisitor::traverse( Action& action )
{
    action.traverse( *this );
}

Timeline*
ActionVisitor::getCurrentTimeline()
{
...
void
UpdateActionVisitor::apply( Timeline& tm )
{
    _currentAnimationPriority = 0;
...
bool
UpdateActionVisitor::isActive( Action& action ) const
{
...
unsigned int
UpdateActionVisitor::getLocalFrame() const
{
    return _frame - _stackFrameAction.back().first;
}

void
UpdateActionVisitor::apply( Action& action )
{
    if( isActive( action ) )
    {
        ...
        if( !result )
        {
            OSG_DEBUG << action.getName() << " Action frame " << frameInAction
                      << " finished" << std::endl;
            return;
        }
        OSG_DEBUG << action.getName() << " Action frame " << frame
                  << " relative to loop " << frameInAction << " no loop " << loopDone
                  << std::endl;
        ...
        while( cb )
        {
            OSG_DEBUG << action.getName() << " evaluate callback " << cb->getName()
                      << " at " << frame << std::endl;
            ( *cb )( &action, this );
...
void
UpdateActionVisitor::apply( ActionBlendIn& action )
{
    if( isActive( action ) )
    {
        unsigned int frame = getLocalFrame();
...
void
UpdateActionVisitor::apply( ActionBlendOut& action )
{
    if( isActive( action ) )
    {
        unsigned int frame = getLocalFrame();
...
void
UpdateActionVisitor::apply( ActionAnimation& action )
{
    if( isActive( action ) )
    {
        unsigned int frame = getLocalFrame();
...
void
UpdateActionVisitor::apply( ActionStripAnimation& action )
{
    if( isActive( action ) )
    {
        apply( static_cast<Action&>( action ) );
...
ClearActionVisitor::ClearActionVisitor( ClearType type ) :
    _clearType( type )
{
    _frame = 0;
}
...
void
ClearActionVisitor::apply( Timeline& tm )
{
    _remove.clear();
    tm.traverse( *this );
    for( int i = 0; i < ( int )_remove.size(); i++ )
    {
        tm.removeAction( _remove[static_cast<std::size_t>( i )].get() );
    }
}

void
ClearActionVisitor::apply( Action& action )
{
    FrameAction fa = _stackFrameAction.back();
    switch( _clearType )
    {
