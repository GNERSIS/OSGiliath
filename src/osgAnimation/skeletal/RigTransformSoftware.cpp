...
RigTransformSoftware::RigTransformSoftware( const RigTransformSoftware& rts,
...
void
RigTransformSoftware::buildMinimumUpdateSet( const RigGeometry& rig )
{
    /// 1 Create Index2Vec<BoneWeight>
    unsigned int nbVertices =
        rig.getSourceGeometry()->getVertexArray()->getNumElements();
         ...
         ++perBoneinfit, ++vimapBoneID )
    {
        ...
        if( bonename.empty() )
        {
            OSG_WARN << "RigTransformSoftware::VertexInfluenceMap contains unnamed bone "
                        "IndexWeightList"
                     << std::endl;
        }
    ...
    /// 2 Create inverse mapping Vec<BoneWeight>2Vec<Index> from previous built
    /// Index2Vec<BoneWeight> in order to minimize weighted matrices computation on
    /// update
    ...
    for( std::vector<BonePtrWeightList>::iterator perVertinfit =
             perVertexInfluences.begin();
    ...
    for( UnifyBoneGroup::const_iterator it = unifyBuffer.begin();
         it != unifyBuffer.end();
         ++it )
    {
        _uniqVertexGroupList.push_back( it->second );
    }
    OSG_INFO << "uniq groups " << _uniqVertexGroupList.size() << " for " << rig.getName()
             << std::endl;
}

bool
RigTransformSoftware::prepareData( RigGeometry& rig )
{
    ...
    osg::Vec3Array* normalSrc =
        dynamic_cast<osg::Vec3Array*>( rig.getSourceGeometry()->getNormalArray() );
    osg::Vec3Array* positionSrc =
        dynamic_cast<osg::Vec3Array*>( rig.getSourceGeometry()->getVertexArray() );
    ...
    *positionDst = *positionSrc;
    positionDst->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    ...
    if( normalSrc )
    {
        ...
        *normalDst                = *normalSrc;
        ...
        normalDst->setDataVariance( osg::Object::DataVariance::DYNAMIC );
    }
...
bool
RigTransformSoftware::init( RigGeometry& rig )
{
         ...
         ++perBoneinfit )
    {
        ...
        if( bmit == boneMap.end() )
        {
            if( _invalidInfluence.find( bonename ) == _invalidInfluence.end() )
            {
                ...
                OSG_WARN << "RigTransformSoftware Bone " << bonename
         ...
         ++itvg )
    {
        ...
        for( BonePtrWeightList::iterator bwit = uniq.getBoneWeights().begin();
             bwit != uniq.getBoneWeights().end(); )
        {
...
void
RigTransformSoftware::VertexGroup::normalize()
{
    osg::dmat4::value_type sum = 0;
    ...
    if( sum < 1E-4 )
    {
        OSG_WARN << "RigTransformSoftware::VertexGroup: warning try to normalize a zero "
                    "sum vertexgroup"
                 << std::endl;
    }
    else
    {
        for( BonePtrWeightList::iterator bwit = _boneweights.begin();
             ...
             ++bwit )
        {
            bwit->setWeight( static_cast<float>( bwit->getWeight() / sum ) );
        }
    }
}

void
RigTransformSoftware::operator()( RigGeometry& geom )
{
    ...
    {
        OSG_WARN << this
                 << " RigTransformSoftware no source geometry found on RigGeometry"
                 << std::endl;
        return;
    }
    ...
    osg::Vec3Array* positionDst =
        static_cast<osg::Vec3Array*>( destination.getVertexArray() );
    osg::Vec3Array* normalSrc = dynamic_cast<osg::Vec3Array*>( source.getNormalArray() );
    osg::Vec3Array* normalDst =
        static_cast<osg::Vec3Array*>( destination.getNormalArray() );
