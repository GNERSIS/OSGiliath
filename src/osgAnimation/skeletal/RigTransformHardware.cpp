...
RigTransformHardware::RigTransformHardware( const RigTransformHardware& rth,
...
osg::Vec4Array*
RigTransformHardware::getVertexAttrib( unsigned int index )
{
...
void
RigTransformHardware::computeMatrixPaletteUniform(
...
)
{
    for( unsigned int i = 0; i < _bonePalette.size(); ++i )
    {
        ...
        const osg::dmat4&  invBindMatrix    = bone->getInvBindMatrixInSkeletonSpace();
        const osg::dmat4&  boneMatrix       = bone->getMatrixInSkeletonSpace();
        osg::dmat4         resultBoneMatrix = invBindMatrix * boneMatrix;
        osg::mat4          result = osg::mat4( transformFromSkeletonToGeometry *
                                               resultBoneMatrix *
                                               invTransformFromSkeletonToGeometry );
        if( !_uniformMatrixPalette->setElement( i, result ) )
        {
            OSG_WARN << "RigTransformHardware::computeUniformMatrixPalette can't set "
                        "uniform at "
                     << i << " elements" << std::endl;
        }
    }
...
/// create normalized a set of Vertex Attribs given a PerVertexInfList and return the max
/// num bone per vertex
unsigned int
createVertexAttribList(
    const PerVertexInfList&                     perVertexInfluences,
    RigTransformHardware::BoneWeightAttribList& boneWeightAttribArrays
)
{
    unsigned int               boneIndexInVec4;
    unsigned int               vertid           = 0, boneIndexInList;
    ...
    for( PerVertexInfList::const_iterator vertinfit = perVertexInfluences.begin();
         ...
         ++vertinfit )
    {
        maxBonePerVertex = std::max( maxBonePerVertex, vertinfit->size() );
    }

    OSG_INFO << "RigTransformHardware::createVertexAttribList maximum number of bone "
                "per vertex is "
             << maxBonePerVertex << std::endl;

    unsigned int nbArray =
        static_cast<unsigned int>( ceilf( ( ( float )maxBonePerVertex ) * 0.5F ) );
         ...
         ++vertinfit, ++vertid )
    {
        ...
        float sum = 0;
        for( IndexWeightList::const_iterator iwit = vertinfit->begin();
             iwit != vertinfit->end();
             ++iwit )
        {
            sum += iwit->second;
        ...
        if( sum < 1E-4 )
        {
            OSG_WARN << "RigTransformHardware::createVertexAttribList Warning: vertex "
                        "with zero sum weights: "
                     << vertid << std::endl;
            return false;
        }
        else
        {
            ...
            for( unsigned int j = 0; j < nbArray; ++j )
            {
                ...
                for( unsigned int b = 0; b < 2; ++b )
                {
                    ...
                    {
                        float boneIndex =
                            static_cast<float>( ( *vertinfit )[boneIndexInList].first );
                        /// normalization here
    ...
    return static_cast<unsigned int>( maxBonePerVertex );
}

bool
RigTransformHardware::prepareData( RigGeometry& rig )
{
         ...
         ++boneinflistit, ++localboneid )
    {
        ...
        for( IndexWeightList::const_iterator infit = boneinflist.begin();
             infit != boneinflist.end();
             ++infit )
        {
            ...
            {
                OSG_WARN << "RigTransformHardware::prepareData Bone " << bonename
                         << " has a weight " << weight << " for vertex " << index
                         << " this bone will not be in the palette" << std::endl;
            }
        }
    }
    return true;
...
bool
RigTransformHardware::buildPalette( const BoneMap&     boneMap,
                                    const RigGeometry& rig )
{
    ...
    for( osgAnimation::VertexInfluenceMap::const_iterator perBoneinfit =
             vertexInfluenceMap.begin();
         ...
         ++perBoneinfit )
    {
        ...
        if( bonename.empty() )
        {
            OSG_WARN << "RigTransformHardware::VertexInfluenceMap contains unnamed bone "
                        "IndexWeightList"
                     << std::endl;
        }
        ...
        if( bmit == boneMap.end() )
        {
            OSG_WARN << "RigTransformHardware Bone " << bonename
                     << " not found, skip the influence group " << std::endl;
            localid2bone.push_back( -1 );
            continue;
        }
        if( ( boneName2PaletteIndex = _boneNameToPalette.find( bonename ) ) !=
            _boneNameToPalette.end() )
        {
        ...
        else
        {
            ...
            _boneNameToPalette[bonename] =
                static_cast<unsigned int>( _bonePalette.size() );
            paletteindex = static_cast<unsigned int>( _bonePalette.size() );
            _bonePalette.push_back( bmit->second );
        }
        localid2bone.push_back( static_cast<int>( paletteindex ) );
    }
    OSG_INFO << "RigTransformHardware::buildPalette matrix palette has "
             << boneNameCountMap.size() << " entries" << std::endl;
    for( BoneNameCountMap::iterator it = boneNameCountMap.begin();
         it != boneNameCountMap.end();
         ++it )
    {
        OSG_INFO << "RigTransformHardware::buildPalette Bone " << it->first
                 << " is used " << it->second << " times" << std::endl;
    }
    OSG_INFO << "RigTransformHardware::buildPalette will use "
             << boneNameCountMap.size() * 4 << " uniforms" << std::endl;

    /// set paletteindices
    for( std::vector<IndexWeightList>::iterator idwlistit = _perVertexInfluences.begin();
         idwlistit != _perVertexInfluences.end();
         ++idwlistit )
    {
        for( IndexWeightList::iterator idwit = idwlistit->begin();
             idwit != idwlistit->end(); )
        {
            ...
            {
                idwit->first = static_cast<unsigned int>( localid2bone[idwit->first] );
                ++idwit;
            }
        }
    }
    if( ( _bonesPerVertex = createVertexAttribList( _perVertexInfluences,
                                                    _boneWeightAttribArrays ) ) < 1 )
    {
        return false;
    }
    ...
    _uniformMatrixPalette = new osg::Uniform( osg::Uniform::FLOAT_MAT4,
                                              "matrixPalette",
                                              static_cast<int>( _bonePalette.size() ) );
...
bool
RigTransformHardware::init( RigGeometry& rig )
{
    ...
    if( !_shader.valid() && ( program = ( osg::Program* )stateset->getAttribute(
...
void
RigTransformHardware::operator()( RigGeometry& geom )
{
    if( _needInit )
    {
        if( !init( geom ) )
