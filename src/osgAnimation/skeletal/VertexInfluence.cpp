...
{
        inline bool
        operator()( const BoneWeight& bw1,
                    const BoneWeight& bw2 ) const
        {
            if( bw1.second > bw2.second )
            {
                return true;
            }
            if( bw1.second < bw2.second )
...
void
VertexInfluenceMap::normalize( unsigned int numvert )
{

    typedef std::pair<float, std::vector<float*>> PerVertWeights;
    std::vector<PerVertWeights>                   localstore;
    localstore.resize( numvert );
    for( VertexInfluenceMap::iterator mapit = this->begin(); mapit != this->end();
         ++mapit )
    {
        IndexWeightList& curvecinf = mapit->second;
        for( IndexWeightList::iterator curinf = curvecinf.begin();
             curinf != curvecinf.end();
             ++curinf )
        {
         ...
         ++itvert, ++vertid )
    {
        ...
        {
            OSG_WARN << "VertexInfluenceMap::normalize warning the vertex " << vertid
                     << " seems to have 0 weight, skip normalize for this vertex"
                     << std::endl;
        }
        else
        {
            float mult = static_cast<float>( 1.0 / weights.first );
            for( std::vector<float*>::iterator itf = weights.second.begin();
                 itf != weights.second.end();
                 ++itf )
            {
                **itf *= mult;
            }
...
void
VertexInfluenceMap::cullInfluenceCountPerVertex( unsigned int numbonepervertex,
                                                 float        minweight,
                                                 bool         renormalize )
{
         ...
         ++mapit )
    {
             ...
             ++curinf )
        {
            ...
            {
                OSG_WARN
                    << "VertexInfluenceSet::cullInfluenceCountPerVertex warning vertex "
                    << inf.first << " is not assigned to a bone" << std::endl;
            }
            else if( inf.second > minweight )
            {
                tempVec2Bones[static_cast<int>( inf.first )].insert(
                    BoneWeight( bonename, inf.second )
                );
            }
        }
    }
    this->clear();
    for( std::map<int, BoneWeightOrdered>::iterator mapit = tempVec2Bones.begin();
         mapit != tempVec2Bones.end();
         ++mapit )
    {
        BoneWeightOrdered& bwset = mapit->second;
        unsigned int       newsize =
            numbonepervertex < static_cast<unsigned int>( bwset.size() )
                ? numbonepervertex
                : static_cast<unsigned int>( bwset.size() );
        float sum = 0.0F;
        while( bwset.size() > newsize )
        {
            bwset.erase( *bwset.rbegin() );
        }
        if( renormalize )
        {
            for( BoneWeightOrdered::iterator bwit = bwset.begin(); bwit != bwset.end();
                 ++bwit )
            {
                sum += bwit->second;
            }
            ...
            {
                sum = 1.0F / sum;
                for( BoneWeightOrdered::iterator bwit = bwset.begin();
                     bwit != bwset.end();
                     ++bwit )
                {
...
void
VertexInfluenceMap::computePerVertexInfluenceList(
    std::vector<BoneWeightList>& vertex2Bones,
    unsigned int                 numvert
) const
{
    vertex2Bones.resize( numvert );
    for( osgAnimation::VertexInfluenceMap::const_iterator it = begin(); it != end();
         ++it )
    {
        const IndexWeightList& inflist = it->second;
        if( it->first.empty() )
        {
            OSG_WARN << "VertexInfluenceMap::computePerVertexInfluenceList contains "
                        "unnamed bone IndexWeightList"
                     << std::endl;
        }

        for( IndexWeightList::const_iterator infit = inflist.begin();
             infit != inflist.end();
             ++infit )
        {
...
{
        bool
        operator()( const BoneWeightList& b0,
...
void
VertexInfluenceMap::computeMinimalVertexGroupList(
    std::vector<VertexGroup>& uniqVertexGroupList,
    unsigned int              numvert
) const
{
    ...
    for( std::vector<BoneWeightList>::iterator it = vertex2Bones.begin();
         it != vertex2Bones.end();
         ++it, ++vertexID )
    {
    ...
    {
        OSG_WARN << "VertexInfluenceMap::computeMinimalVertexGroupList is useless no "
                    "duplicate VertexGroup"
                 << std::endl;
    }

    uniqVertexGroupList.reserve( unifyBuffer.size() );
    for( UnifyBoneGroup::iterator it = unifyBuffer.begin(); it != unifyBuffer.end();
         ++it )
    {
...
class CollectRigVisitor : public osg::DualModeVisitor
{
    public:
        ...
        void
        apply( osg::Geometry& node ) override;

        inline const RigList&
        getRigList() const
        {
            return _map;
        }
...
void
CollectRigVisitor::apply( osg::Geometry& node )
{
...
void
VertexInfluenceMap::removeUnexpressedBones( Skeleton& skel ) const
{
