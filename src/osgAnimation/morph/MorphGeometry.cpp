...
MorphGeometry::MorphGeometry( const MorphGeometry& b,
                              const osg::CopyOp&   copyop ) :
    osg::Geometry( b,
                   copyop ),
    _morphTransformImplementation( osg::clone( b._morphTransformImplementation.get(),
                                               copyop ) ),
    _dirty( b._dirty ),
    _method( b._method ),
...
UpdateMorph::UpdateMorph( const UpdateMorph& apc,
                          const osg::CopyOp& copyop ) :
    Inherit( apc,
             copyop )
{
...
UpdateMorph::UpdateMorph( const std::string& name ) :
    Inherit( name )
{
}
...
void
UpdateMorph::operator()( osg::Node*        node,
                         osg::NodeVisitor* nv )
{
        ...
        if( geode )
        {
            ...
            for( unsigned int i = 0; i != numDrawables; ++i )
            {
                ...
                osgAnimation::RigGeometry* rig =
                    dynamic_cast<osgAnimation::RigGeometry*>( drw );
                ...
                osgAnimation::MorphGeometry* morph =
                    dynamic_cast<osgAnimation::MorphGeometry*>( drw );
                if( morph )
                {
                    // Update morph weights
                    std::map<int, osg::ref_ptr<osgAnimation::FloatTarget>>::iterator
                        iter = _weightTargets.begin();
                    while( iter != _weightTargets.end() )
                    {
                        if( iter->second->getValue() >= 0 )
                        {
                            morph->setWeight( static_cast<unsigned int>( iter->first ),
...
bool
UpdateMorph::link( osgAnimation::Channel* channel )
{
...
int
UpdateMorph::link( Animation* animation )
{
    if( getNumTarget() == 0 )
    {
        osg::notify( osg::WARN )
            << "An update callback has no name, it means it could link only with \"\" "
               "named Target, often an error, discard"
            << std::endl;
        return 0;
    }
         ...
         ++channel )
    {
        ...
        for( unsigned int i = 0, num = getNumTarget(); i < num; ++i )
        {
