...
MorphTransformHardware::MorphTransformHardware( const MorphTransformHardware& rth,
...
bool
MorphTransformHardware::init( MorphGeometry& morphGeometry )
{
    osg::Vec3Array* pos =
    ...
    morphGeometry.setDataVariance( osg::Object::DataVariance::STATIC );
    /// check for correct morph configuration
    ///(blender osgexport doesn't set sources so assume morphgeom arrays are sources)
    if( pos )
    {
        pos->setDataVariance( osg::Object::DataVariance::STATIC );
        /// check if source is set correctly
        if( !vertexSource || vertexSource->size() != pos->size() )
        {
            vertexSource = ( static_cast<osg::Vec3Array*>(
                pos->clone( osg::CopyOp::DEEP_COPY_ARRAYS )
            ) );
        }
        osg::Vec3Array* normal =
            dynamic_cast<osg::Vec3Array*>( morphGeometry.getNormalArray() );
        bool normalmorphable = morphGeometry.getMorphNormals() &&
                               normal &&
                               ( normal->getBinding() == osg::Array::BIND_PER_VERTEX );
        if( !normalmorphable )
        {
            OSG_WARN << "MorphTransformHardware::morph geometry "
                     << morphGeometry.getName()
                     << " without per vertex normal : HWmorphing not supported! "
                     << std::endl;
            return false;
        }
        normal->setDataVariance( osg::Object::DataVariance::STATIC );
        if( normalmorphable &&
            ( !normalSource || normalSource->size() != normal->size() ) )
        {
            normalSource = ( static_cast<osg::Vec3Array*>(
                normal->clone( osg::CopyOp::DEEP_COPY_ARRAYS )
            ) );
        }
    }
    ...
    morphGeometry.setNormalArray( morphGeometry.getNormalSource(),
    ...
    for( MorphGeometry::MorphTargetList::const_iterator curmorph = morphlist.begin();
         curmorph != morphlist.end();
         ++curmorph )
    {
        const osg::Geometry*  morphtargetgeom = curmorph->getGeometry();
        const osg::Vec3Array* varray =
            ( osg::Vec3Array* )morphtargetgeom->getVertexArray();
        const osg::Vec3Array* narray =
            ( osg::Vec3Array* )morphtargetgeom->getNormalArray();
        if( morphGeometry.getMethod() == MorphGeometry::RELATIVE )
        {
            for( unsigned int i = 0;
                 i < morphGeometry.getVertexArray()->getNumElements();
                 ++i )
            {
        ...
        else
        {
            ...
            const osg::Vec3Array* ovarray =
                ( osg::Vec3Array* )morphGeometry.getVertexArray();
            const osg::Vec3Array* onarray =
                ( osg::Vec3Array* )morphGeometry.getNormalArray();
            for( unsigned int i = 0;
                 i < morphGeometry.getVertexArray()->getNumElements();
                 ++i )
            {
    ...
    osg::ref_ptr<osg::Uniform> morphTBOHandle =
        new osg::Uniform( osg::Uniform::SAMPLER_BUFFER, "morphTargets" );
    morphTBOHandle->set( ( int )_reservedTextureUnit );

    // create dynamic uniform for morphtargets animation weights
    _uniformTargetsWeight = new osg::Uniform( osg::Uniform::FLOAT,
                                              "morphWeights",
                                              static_cast<int>( morphlist.size() ) );
    ...
    if( !vertexshader.valid() )
    {
        ...
        {
            vertexshader =
                osgDB::readRefShaderFile( osg::Shader::VERTEX, "morphing.vert" );
        }
        else
        {
    ...
    // replace max matrix by the value from uniform
    {
        ...
        {
            // perhaps remanance from previous init (if saved after init) so reload
            // shader
            vertexshader =
                osgDB::readRefShaderFile( osg::Shader::VERTEX, "morphing.vert" );
            if( !vertexshader.valid() )
    ...
    // morphGeometry.setStateSet((osg::StateSet *)
    // osg::CopyOp()(source.getOrCreateStateSet()));
    ...
    ss->addUniform(
        new osg::Uniform( "nbMorphVertex",
                          morphGeometry.getVertexArray()->getNumElements() )
    );
...
void
MorphTransformHardware::operator()( MorphGeometry& geom )
{
