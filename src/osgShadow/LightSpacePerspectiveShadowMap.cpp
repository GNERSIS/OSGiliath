...
void
LightSpacePerspectiveShadowMapAlgorithm::operator()(
    const osgShadow::ConvexPolyhedron* hullShadowedView,
    const osg::Camera*                 cameraMain,
    osg::Camera*                       cameraShadow
) const
{
    osg::box bb = hullShadowedView->computeBoundingBox( cameraMain->getViewMatrix() );
    double   nearDist                  = -bb.max[2];

    const osg::dmat4& eyeViewToWorld   = cameraMain->getInverseViewMatrix();

    osg::dmat4        lightViewToWorld = cameraShadow->getInverseViewMatrix();

    osg::dvec3        eyePos           = osg::dvec3( 0, 0, 0 ) * eyeViewToWorld;

    osg::dvec3 viewDir( osg::transform3x3( osg::dvec3( 0, 0, -1 ), eyeViewToWorld ) );

    osg::dvec3 lightDir( osg::transform3x3( osg::dvec3( 0, 0, -1 ), lightViewToWorld ) );
    osg::dvec3 up( osg::transform3x3( osg::dvec3( 0, 1, 0 ), lightViewToWorld ) );

    osg::dmat4 lightView;    // compute coarse light view matrix
    lightView             = osg::lookAt( eyePos, eyePos + lightDir, up );
    bb                    = hullShadowedView->computeBoundingBox( lightView );

    const double dotProd  = osg::dot( viewDir, lightDir );
    ...
    const double d =
        fabs( bb.max[1] -
              bb.min[1] );    // perspective transform depth //light space y extents
    const double z_f = z_n + d * sinGamma;
    const double n   = ( z_n + sqrt( z_f * z_n ) ) / sinGamma;
    const double f   = n + d;
    osg::dvec3   pos = eyePos - up * ( n - nearDist );

    #if PRINT_COMPUTED_N_OPT
    std::cout
        << " N=" << std::setw( 8 ) << n << " n=" << std::setw( 8 ) << z_n
        << " f=" << std::setw( 8 ) << z_f
        << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
        << std::flush;
    #endif

    lightView = osg::lookAt( pos, pos + lightDir, up );
    ...
    osg::dmat4 lispProjection( 1, 0, 0, 0, 0, a, 0, 1, 0, 0, -1, 0, 0, b, 0, 0 );

    // lispProjection.makeIdentity( );
    #if 0
    {
        osg::dmat4 mvp = _camera->getViewMatrix() *
                      _camera->getProjectionMatrix();

        extendScenePolytope( mvp, osg::inverse( mvp ) );
    }
    #endif

    bb = hullShadowedView->computeBoundingBox( lightView * lispProjection );

    osg::dmat4 fitToUnitFrustum;
    fitToUnitFrustum = osg::orthographic( bb.min[0],
                                          bb.max[0],
                                          bb.min[1],
                                          bb.max[1],
                                          -( bb.min[2] - 1 ),
    ...
    if( !osg::equivalent( 0.F, (bb.min - osg::dvec3(-1,-1,-1)).length2() ) ||
        !osg::equivalent( 0.F, (bb.max - osg::dvec3( 1, 1, 1)).length2() ) )
    {
        bb = computeScenePolytopeBounds
            ( cameraShadow->getProjectionMatrix() * cameraShadow->getViewMatrix() );
    }
...
) const
{
    ...
    osg::dmat4 m      = cameraMain->getInverseViewMatrix() * mvpLight;
    osg::dvec3 eye    = osg::dvec3( 0, 0, 0 ) * m;
    osg::dvec3 center = osg::dvec3( 0, 0, -1 ) * m;
    osg::dvec3 up( 0, 1, 0 );
    osg::dvec3 viewDir( center - eye );
    viewDir     = osg::normalize( viewDir );

    m           = osg::lookAt( eye, center, up );

    osg::box bb = hullShadowedView->computeBoundingBox( mvpLight * m );
    if( !bb.valid() )
    {
    #if ROBERTS_TEST_CHANGES
            // OSG_NOTICE<<"LightSpacePerspectiveShadowMapAlgorithm::operator() invalid
            // bb A"<<std::endl;
    #endif
        return;
    }
    ...
    // in non directional lights post projective space. But since I can't prove it
    // doesn't, I assume it does ;-). So I made an effort to modify their original
    // directional algo to work in true light post perspective space and compute all
    // params in this space. And here is a snag. Although shadowed hull fits completely
    // into light space, camera position may not, and after projective transform it may
    // land outside light frustum and even on/or below infinity plane. I need camera pos
    // to compute minimal distance to shadowed hull. If its not right rest of the
    // computation may be completely off. So in the end this approach is not singulartity
    // free. I guess this problem is solvable in other way but "this other way" looks
    // like a topic for other scientific paper and I am definitely not that ambitious
    // ;-). So for the time being I simply try to discover when this happens and apply
    // workaround, I found works. This workaround may mean that adjusted projection may
    // not be optimal in original LisPSM Lmax norm sense. But as I wrote above, I doubt
    // they are optimal when Light is not directional, anyway.
    ...
    {
        float clampedNearDist  = 0.0001F;
        eye                   += viewDir * ( clampedNearDist - nearDist );
        nearDist               = clampedNearDist;
        #if ROBERTS_TEST_CHANGES
                // OSG_NOTICE<<"LightSpacePerspectiveShadowMapAlgorithm::operator()
                // adjusting eye"<<std::endl;
        #endif
    }
    #endif
    ...
    {
        nearDist = 0.0;
        // OSG_NOTICE<<"LightSpacePerspectiveShadowMapAlgorithm::operator() nearDist<0.0,
    ...
    osg::dvec3 lightDir( 0, 0, -1 );
    osg::dmat4 lightView;    // compute coarse light view matrix
    lightView = osg::lookAt( eye, eye + lightDir, up );
    ...
    const double d   = fabs( bb.max[1] - bb.min[1] );
    const double z_f = z_n + d * sinGamma;
    ...
    {
        n = d * minRatio;
        // OSG_NOTICE<<"LightSpacePerspectiveShadowMapAlgorithm::operator() n too small,
        // clamping n to "<<minRatio<<"*d."<<std::endl;
    }
    #endif
    const double f   = n + d;
    osg::dvec3   pos = eye - up * ( n - nearDist );
    // pos = eye;
    lightView = osg::lookAt( pos, pos + lightDir, up );
    ...
    // OSG_NOTICE<<"LightSpacePerspectiveShadowMapAlgorithm::operator() normal case
    // dotProd="<<dotProd<<", sinGamma="<<sinGamma<<" d="<<d<<", pos=("<<pos<<"),
    // (n-nearDist)="<<(n-nearDist)<<std::endl; OSG_NOTICE<<"    eye=("<<eye<<"),
    // up=("<<up<<"), n="<<n<<", nearDist="<<nearDist<<", z_n="<<z_n<<",
    // z_f="<<z_f<<std::endl;
}
...
namespace osgShadow
{
    ...
    class LispSM
    {
        public:
            ...
            void
            setProjectionMatrix( const osg::dmat4& projectionMatrix )
            {
                _projectionMatrix = projectionMatrix;
            }

            void
            setViewMatrix( const osg::dmat4& viewMatrix )
            {
                _viewMatrix = viewMatrix;
            }

            void
            setHull( const ConvexPolyhedron& hull )
            {
                _hull = hull;
            }
            ...
            void
            setUseFormula( bool use )
            {
                _useFormula = use;
            }

            bool
            getUseOldFormula() const
            {
                return _useOldFormula;
            }

            void
            setUseOldFormula( bool use )
            {
                _useOldFormula = use;
            }
            ...
            void
            setNearDist( const double& nearDist )
            {
                _nearDist = nearDist;
            }
            ...
            void
            setFarDist( const double& farDist )
            {
                _farDist = farDist;
            }
            ...
            void
            setEyeDir( const osg::dvec3 eyeDir )
            {
                _eyeDir = eyeDir;
            }

            void
            setLightDir( const osg::dvec3 lightDir )
            {
                _lightDir = lightDir;
            }
        ...
        protected:
            ...
            osg::dmat4         _projectionMatrix;

            double
            getN( const osg::dmat4 lightSpace,
                  const osg::box&  B_ls ) const;

            osg::dvec3
            getNearCameraPointE() const;

            osg::dvec3
            getZ0_ls( const osg::dmat4& lightSpace,
                      const osg::dvec3& e,
                      const double&     b_lsZmax,
                      const osg::dvec3& eyeDir ) const;

            double
            calcNoptGeneral( const osg::dmat4 lightSpace,
                             const osg::box&  B_ls ) const;

            double
            calcNoptOld( const double gamma_ = 999 ) const;

            osg::dmat4
            getLispSmMtx( const osg::dmat4& lightSpace ) const;

            osg::dvec3
            getProjViewDir_ls( const osg::dmat4& lightSpace ) const;

            void
            updateLightMtx( osg::dmat4&                    lightView,
                            osg::dmat4&                    lightProj,
                            const std::vector<osg::dvec3>& B ) const;
        ...
        public:
            ...
            virtual void
            updateLightMtx( osg::dmat4& lightView,
                            osg::dmat4& lightProj ) const;
    };

}

osg::dvec3
LispSM::getNearCameraPointE() const
{
    const osg::dmat4&          eyeView = getViewMatrix();
    ...
    return LVS[max] * osg::inverse( eyeView );
}
...
osg::dvec3
LispSM::getZ0_ls( const osg::dmat4& lightSpace,
                  const osg::dvec3& e,
                  const double&     b_lsZmax,
                  const osg::dvec3& eyeDir ) const
{
    ...
    const osg::dvec3 n = A.getNormal();
    osg::dvec3       v( e_ls.x, ( -d - n.z * b_lsZmax - n.x * e_ls.x ) / n.y, b_lsZmax );
    #endif
...
double
LispSM::calcNoptGeneral( const osg::dmat4 lightSpace,
                         const osg::box&  B_ls ) const
{
    const osg::dmat4& eyeView       = getViewMatrix();
    const osg::dmat4  invLightSpace = osg::inverse( lightSpace );

    const osg::dvec3  z0_ls = getZ0_ls( lightSpace, _E, B_ls.zMax(), getEyeDir() );
    const osg::dvec3  z1_ls = osg::dvec3( z0_ls.x, z0_ls.y, B_ls.zMin() );

    // to world
    const osg::dvec4  z0_ws = osg::dvec4( z0_ls, 1 ) * invLightSpace;
    const osg::dvec4  z1_ws = osg::dvec4( z1_ls, 1 ) * invLightSpace;

    // to eye
    const osg::dvec4  z0_cs = z0_ws * eyeView;
    const osg::dvec4  z1_cs = z1_ws * eyeView;
    ...
    if( z1 / z0 <= 1.0 )
    {
        ...
        osg::box bb = _hull.computeBoundingBox( eyeView );
        z0          = -bb.zMax();
        if( z0 <= 0 )
        {
            z0 = 0.1;
    ...
    #if PRINT_COMPUTED_N_OPT
    std::cout
        << " N=" << std::setw( 8 ) << N << " n=" << std::setw( 8 ) << z0
        << " f=" << std::setw( 8 ) << z1
        << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
        << std::flush;
    #endif
...
double
LispSM::calcNoptOld( const double gamma_ ) const
{
...
double
LispSM::getN( const osg::dmat4 lightSpace,
              const osg::box&  B_ls ) const
{
    if( getUseFormula() )
    {
...
osg::dmat4
LispSM::getLispSmMtx( const osg::dmat4& lightSpace ) const
{
    const osg::box   B_ls = _hull.computeBoundingBox( lightSpace );

    const double     n    = getN( lightSpace, B_ls );

    // get the coordinates of the near camera point in light space
    const osg::dvec3 e_ls = _E * lightSpace;
    // c start has the x and y coordinate of e, the z coord of B.min()
    const osg::dvec3 Cstart_lp( e_ls.x, e_ls.y, B_ls.zMax() );
    ...
    // we work along a negative axis so we transform +n*<the positive axis> == -n*<neg
    // axis>
    const osg::dvec3 C( Cstart_lp + osg::dvec3( 0, 0, 1 ) * n );
    // construct a translation that moves to the projection center
    const osg::dmat4 projectionCenter = osg::translate( -C );
    ...
    // in directX you can use the same mapping and do a mapping to the directX
    // post-perspective cube with corner points [-1,-1,0] and [1,1,1] as the final step
    // after all the shadow mapping.
    osg::dmat4       P = osg::dmat4::frustum( -1.0, 1.0, -1.0, 1.0, n, n + d );

    // invert the transform from right handed into left handed coordinate system for the
    // ndc done by the openGL style frustumGL call so we stay in a right handed system
    P = P * osg::scale( 1.0, 1.0, -1.0 );
    // return the lispsm frustum with the projection center
    return projectionCenter * P;
}

osg::dvec3
LispSM::getProjViewDir_ls( const osg::dmat4& lightSpace ) const
{
    // get the point in the LVS volume that is nearest to the camera
    const osg::dvec3 e = _E;
    // construct edge to transform into light-space
    const osg::dvec3 b = e + getEyeDir();
    // transform to light-space
    osg::dvec4       e_lp = osg::dvec4( e, 1.0 ) * lightSpace;
    osg::dvec4       b_lp = osg::dvec4( b, 1.0 ) * lightSpace;
    ...
    if( b_lp[3] <= 0 )
    {
        osg::dvec4 v = ( e_lp - b_lp ) / ( e_lp[3] - b_lp[3] );
    ...
    osg::dvec3 projDir( osg::vec3( b_lp[0], b_lp[1], b_lp[2] ) /
                        b_lp[3] -
                        osg::vec3( e_lp[0], e_lp[1], e_lp[2] ) /
                        e_lp[3] );

    projDir = osg::normalize( projDir );
...
void
LispSM::updateLightMtx( osg::dmat4& lightView,
                        osg::dmat4& lightProj ) const
{
    ...
    // lightproj contains in the case of a spot light the spot light perspective
    // transformation in the case of a directional light a identity matrix
    ...
    lightProj = lightProj * osg::scale( 1, 1, -1 );

    // coordinate system change for calculations in the article
    osg::dmat4 switchToArticle = osg::dmat4();
    ...
    osg::dmat4 L           = lightView * lightProj;

    osg::dvec3 projViewDir = getProjViewDir_ls( L );
    ...
    if( getUseLiSPSM() /* && projViewDir.z < 0*/ )
    {
        ...
        // look(from position, into the direction of the projected direction, with
        // unchanged up-vector)
        lightProj =
            lightProj *
            osg::lookAt( osg::dvec3( 0, 0, 0 ), projViewDir, osg::dvec3( 0, 1, 0 ) );

        osg::dmat4 lispsm = getLispSmMtx( lightView * lightProj );
        lightProj         = lightProj * lispsm;
    }

    const osg::dmat4 PL = lightView * lightProj;

    osg::box         bb = _hull.computeBoundingBox( PL );
    ...
    // coordinate system change for calculations in the article
    osg::dmat4 switchToGL = osg::dmat4();
    switchToGL( 1, 1 )    = 0.0;
    ...
    lightProj = lightProj * osg::scale( 1.0, 1.0, -1.0 );
}
...
) const
{
    ...
    #if 1
    osg::dvec3 lightDir =
        osg::transform3x3( osg::dvec3( 0, 0, -1 ),
                           osg::inverse( cameraShadow->getViewMatrix() ) );
    osg::dvec3 eyeDir = osg::transform3x3( osg::dvec3( 0, 0, -1 ),
                                           osg::inverse( cameraMain->getViewMatrix() ) );

    #else

    osg::dvec3 lightDir =
        osg::transform3x3( cameraShadow->getViewMatrix(), osg::dvec3( 0.0, 0.0, -1.0 ) );
    osg::dvec3 eyeDir =
        osg::transform3x3( cameraMain->getViewMatrix(), osg::dvec3( 0.0, 0.0, -1.0 ) );
    ...
    osg::dmat4& proj = cameraShadow->getProjectionMatrix();
    double      l, r, b, t, n, f;
    if( osg::getOrtho( proj, l, r, b, t, n, f ) )
    {
        osg::dvec3 camPosInLightSpace = osg::dvec3( 0, 0, 0 ) *
                                        osg::inverse( cameraMain->getViewMatrix() ) *
    ...
    lispsm->updateLightMtx( cameraShadow->getViewMatrix(),
                            cameraShadow->getProjectionMatrix() );
}
