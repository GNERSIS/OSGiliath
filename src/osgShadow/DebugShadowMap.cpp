...
DebugShadowMap::DebugShadowMap() :
    ...
    _hudOrigin( -1,
                -1 ),
    _viewportSize( DEFAULT_DEBUG_HUD_SIZE_X,
                   DEFAULT_DEBUG_HUD_SIZE_Y ),
    _viewportOrigin( DEFAULT_DEBUG_HUD_ORIGIN_X,
                     DEFAULT_DEBUG_HUD_ORIGIN_Y ),
    _orthoSize( 2,
                2 ),
    _orthoOrigin( -1,
                  -1 ),
    _doDebugDraw( false )
{
    ...
    _depthColorFragmentShader = new osg::Shader(
        osg::Shader::FRAGMENT,
#if 0
        "#version 460 core\n"
        "in vec4 texCoord_0;                                                     \n"
        ...
        "void main(void)                                                         \n"
        "{                                                                       \n"
        "    float f = texture( texture, vec3( texCoord_0.xy, 1.0).xy ).r;       \n"
        "    fragColor =  vec4( 0.0, 1.0 - f,  0.5 - f, 0.5 );                  \n"
        "}                                                                       \n"
#else
        "#version 460 core\n"
        "in vec4 vertexColor;                                                    \n"
        ...
        "    vec3 color = fMax * rgb;                                            \n"
        "                                                                        \n"
        "    fragColor =  vec4( fS + fH * color, 1 ) * vertexColor;              \n"
        "}                                                                       \n"
#endif
    );    // end _depthColorFragmentShader
}

DebugShadowMap::DebugShadowMap( const DebugShadowMap& copy,
    ...
    _doDebugDraw( copy._doDebugDraw )
{
    ...
    {
        _depthColorFragmentShader = dynamic_cast<osg::Shader*>(
...
void
DebugShadowMap::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
DebugShadowMap::releaseGLObjects( osg::State* state ) const
{
...
void
DebugShadowMap::ViewData::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
DebugShadowMap::ViewData::releaseGLObjects( osg::State* state ) const
{
...
void
DebugShadowMap::ViewData::cull( void )
{
...
bool
DebugShadowMap::ViewData::DebugBoundingBox( const osg::box& bb,
                                            const char*     name )
{
    bool result = false;
    ...
    osg::box& bb_prev = _boundingBoxMap[std::string( name )];

    result            = bb.center() != bb_prev.center || bb.radius() != bb_prev.radius();
    if( result )
    {
        std::cout << "Box<" << name << "> (" << ( bb.max._v[0] + bb.min._v[0] ) *
            0.5 << " "
                  << ( bb.max._v[1] + bb.min._v[1] ) *
            0.5 << " "
                  << ( bb.max._v[2] + bb.min._v[2] ) *
            0.5 << ") ["
                  << ( bb.max._v[0] - bb.min._v[0] ) << " "
                  << ( bb.max._v[1] - bb.min._v[1] ) << " "
                  << ( bb.max._v[2] - bb.min._v[2] ) << "] " << std::endl;
    }
...
bool
DebugShadowMap::ViewData::DebugPolytope( const osg::Polytope& p,
                                         const char*          name )
{
    bool result = false;
#if !defined( _DEBUG ) && !defined( DEBUG )
    ...
    if( result )
    {
        std::cout << "Polytope<" << name << "> size(" << p.getPlaneList().size() << ")"
                  << std::endl;
            ...
            for( unsigned i = 0; i < p.getPlaneList().size(); ++i )
            {
                if( p.getPlaneList()[i] != p_prev.getPlaneList()[i] )
                {
                    std::cout << "Plane<" << i << "> ("
                              ...
                              << p.getPlaneList()[i].asVec4()[3] << ")" << std::endl;
                }
            }
        }
    }
...
bool
DebugShadowMap::ViewData::DebugMatrix( const osg::dmat4& m,
                                       const char*       name )
{
    bool result = false;
#if !defined( _DEBUG ) && !defined( DEBUG )
    ...
    osg::dmat4& m_prev = _matrixMap[std::string( name )];

    result             = ( m != m_prev );

    if( result )
    {
        std::cout << "dmat4<" << name << "> " << std::endl
                  << "[ " << m( 0, 0 ) << " " << m( 0, 1 ) << " " << m( 0, 2 ) << " "
                  << m( 0, 3 ) << " ]  " << std::endl
                  << "[ " << m( 1, 0 ) << " " << m( 1, 1 ) << " " << m( 1, 2 ) << " "
                  << m( 1, 3 ) << " ]  " << std::endl
...
void
DebugShadowMap::ViewData::setDebugPolytope( const char*             name,
                                            const ConvexPolyhedron& polytope,
                                            osg::vec4               colorOutline,
                                            osg::vec4               colorInside )
{
    if( !getDebugDraw() )
    {
        return;
    ...
    {    // update
        ...
        for( unsigned int i = 0; i < VECTOR_LENGTH( pg._geometry ); i++ )
        {
            if( !pg._geometry[i].valid() )
            {
                ...
                pg._geometry[i]->setDataVariance( osg::Object::DataVariance::DYNAMIC );
            }
...
void
DebugShadowMap::ViewData::updateDebugGeometry( const osg::Camera* viewCam,
                                               const osg::Camera* shadowCam )
{
    ...
    osg::dmat4 transform[num] =
        { viewCam->getViewMatrix() *
              // use near far clamped projection ( precomputed in cullDebugGeometry )
              ( _viewCamera == viewCam ? _viewProjection
                                       : viewCam->getProjectionMatrix() ),
          shadowCam->getProjectionMatrix() * shadowCam->getViewMatrix() },
               inverse[num] = {
                   osg::inverse( transform[0] ),
                   osg::inverse( transform[1] )
               };
    ...
    #else
        frustum[i].transform
            ( osg::inverse( camera[i]->getProjectionMatrix() ),
              camera[i]->getProjectionMatrix() );
        frustum[i].transform
            ( osg::inverse( camera[i]->getViewMatrix() ),
         ...
         ++itr )
    {
        ...
        for( int i = 0; i < num; i++ )
        {
            ...
            pg._geometry[i] = cp.buildGeometry( pg._colorOutline,
                                                pg._colorInside,
                                                pg._geometry[i].get() );
        }
    }
...
void
DebugShadowMap::ViewData::cullDebugGeometry()
{
    if( !getDebugDraw() )
    {
        return;
...
void
DebugShadowMap::ViewData::init( ThisClass*            st,
                                osgUtil::CullVisitor* cv )
{
    ...
    if( vp )
    {
        ...
        if( _viewportSize[0] >
            ( static_cast<unsigned short>( vp->width() ) - _viewportOrigin[0] ) )
        {
            _viewportSize[0] =
                static_cast<short>( static_cast<unsigned short>( vp->width() ) -
                                    _viewportOrigin[0] );
        }

        if( _viewportSize[1] >
            ( static_cast<unsigned short>( vp->height() ) - _viewportOrigin[1] ) )
        {
            _viewportSize[1] =
                static_cast<short>( static_cast<unsigned short>( vp->height() ) -
                                    _viewportOrigin[1] );
        }
    }
    ...
    : public osg::Drawable::DrawCallback
{
    public:
                            ...
                            const osg::Drawable* drawable ) const
        {
            ...
            glTexParameteri( _pTexture->getTextureTarget(),
...
void
DebugShadowMap::ViewData::dump( const std::string& filename )
{
...
void
DebugShadowMap::ViewData::createDebugHUD()
{
    ...
    {    // Initialize hud camera
        ...
        camera->setViewport( _viewportOrigin[0],
                             _viewportOrigin[1],
                             _viewportSize[0],
                             _viewportSize[1] );

        camera->setProjectionMatrixAsOrtho( _orthoOrigin[0],
                                            _orthoOrigin[0] + _orthoSize[0],
                                            _orthoOrigin[1],
                                            _orthoOrigin[1] + _orthoSize[1],
                                            -10,
                                            10 );
    ...
    {    // Add geode and drawable with BaseClass display
        ...
        stateset->setAttributeAndModes(
            new osg::Depth( osg::Depth::Function::ALWAYS, 0, 1, false )
        );
    ...
    {    // Create transforms and geodes to manage polytope drawing
        ...
        for( int i = 0; i < 2; i++ )
        {
            ...
            _transform[i]->setMatrix( osg::dmat4() );
            _transform[i]->setReferenceFrame( osg::MatrixTransform::ABSOLUTE_RF );
        }

        _transform[1]->setMatrix( osg::translate( 1.0, 1.0, 0.0 ) *
                                  osg::scale( 0.5, 0.5, 1.0 ) *
                                  osg::scale( static_cast<double>( _hudSize[0] ),
                                              static_cast<double>( _hudSize[1] ),
                                              1.0 ) *
                                  osg::translate( static_cast<double>( _hudOrigin[0] ),
                                                  static_cast<double>( _hudOrigin[1] ),
                                                  0.0 ) );
...
osg::dvec3
DebugShadowMap::ViewData::computeShadowTexelToPixelError( const osg::dmat4& mvpwView,
                                                          const osg::dmat4& mvpwShadow,
                                                          const osg::dvec3& vWorld,
                                                          const osg::dvec3& vDelta )
{
    osg::dvec3 vS0 = mvpwShadow * vWorld;
    osg::dvec3 vS1 = mvpwShadow * ( vWorld + vDelta );

    osg::dvec3 vV0 = mvpwView * vWorld;
    osg::dvec3 vV1 = mvpwView * ( vWorld + vDelta );

    osg::dvec3 dV  = vV1 - vV0;
    osg::dvec3 dS  = vS1 - vS0;

    return osg::dvec3( dS[0] / dV[0], dS[1] / dV[1], dS[2] / dV[2] );
}

void
DebugShadowMap::ViewData::displayShadowTexelToPixelErrors(
    const osg::Camera*      viewCamera,
    const osg::Camera*      shadowCamera,
    const ConvexPolyhedron* hull
)
{
    osg::dmat4         mvpwMain   = viewCamera->getViewMatrix() *
                                    viewCamera->getProjectionMatrix() *
                                    viewCamera->getViewport()->computeWindowMatrix();

    osg::dmat4         mvpwShadow = shadowCamera->getViewMatrix() *
                                    shadowCamera->getProjectionMatrix() *
                                    shadowCamera->getViewport()->computeWindowMatrix();

    osg::box           bb  = hull->computeBoundingBox( viewCamera->getViewMatrix() );

    osg::dmat4         m   = viewCamera->getInverseViewMatrix();

    osg::dvec3         vn  = osg::dvec3( 0, 0, bb.max[2] ) * m;
    osg::dvec3         vf  = osg::dvec3( 0, 0, bb.min[2] ) * m;
    osg::dvec3         vm  = osg::dvec3( 0, 0, ( bb.max[2] + bb.min[2] ) * 0.5 ) * m;

    osg::dvec3         vne = computeShadowTexelToPixelError( mvpwMain, mvpwShadow, vn );
    osg::dvec3         vfe = computeShadowTexelToPixelError( mvpwMain, mvpwShadow, vf );
    osg::dvec3         vme = computeShadowTexelToPixelError( mvpwMain, mvpwShadow, vm );
