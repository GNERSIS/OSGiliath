...
{
    _mainFragmentShader = new osg::Shader(
        osg::Shader::FRAGMENT,
        "#version 460 core\n"
        "                                                                       \n"
        "out vec4 fragColor;                                                    \n"
    ...
    _shadowFragmentShader = new osg::Shader(
        osg::Shader::FRAGMENT,
        "#version 460 core\n"
        "                                                                       \n"
    ...
    _shadowVertexShader = new osg::Shader(
        osg::Shader::VERTEX,
        "#version 460 core\n"
        "                                                                       \n"
    ...
    _mainVertexShader = new osg::Shader(
        osg::Shader::VERTEX,
        "#version 460 core\n"
        ...
        "                                                                       \n"
        "    vec4 sceneColor = osg_FrontMaterial.emission + osg_FrontMaterial.ambient * "
        "osg_LightSource.ambient; \n"
...
StandardShadowMap::StandardShadowMap( const StandardShadowMap& copy,
    ...
    _shadowTextureCoordIndex( copy._shadowTextureCoordIndex )
{
    ...
    {
        _mainVertexShader =
            dynamic_cast<osg::Shader*>( copy._mainVertexShader->clone( copyop ) );
    }

    if( copy._mainFragmentShader.valid() )
    {
        _mainFragmentShader =
            dynamic_cast<osg::Shader*>( copy._mainFragmentShader->clone( copyop ) );
    }

    if( copy._shadowVertexShader.valid() )
    {
        _shadowVertexShader =
            dynamic_cast<osg::Shader*>( copy._shadowVertexShader->clone( copyop ) );
    }

    if( copy._shadowFragmentShader.valid() )
    {
        _shadowFragmentShader =
            dynamic_cast<osg::Shader*>( copy._shadowFragmentShader->clone( copyop ) );
    }
}
...
void
StandardShadowMap::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
StandardShadowMap::releaseGLObjects( osg::State* state ) const
{
...
void
StandardShadowMap::ViewData::resizeGLObjectBuffers( unsigned int maxSize )
{
    osg::resizeGLObjectBuffers( _stateset, maxSize );
}

void
StandardShadowMap::ViewData::releaseGLObjects( osg::State* state ) const
{
    osg::releaseGLObjects( _stateset, state );
}

void
StandardShadowMap::updateTextureCoordIndices( unsigned int fromTextureCoordIndex,
                                              unsigned int toTextureCoordIndex )
{
    ...
    for( unsigned int i  = 0; i < sizeof( expressions ) / sizeof( expressions[0] );
         i              += 2 )
    {
...
void
StandardShadowMap::searchAndReplaceShaderSource( osg::Shader* shader,
...
void
StandardShadowMap::ViewData::cull()
{
    // step 1:
    ...
    {
        osg::dmat4 inverseCameraView = osg::inverse( *_cv->getModelViewMatrix() );
        // Note: getModelViewMatrix at this point returns just the camera view (no model
        // transform) since we haven't pushed any model matrices yet

        osg::dmat4 shadowView = _camera->getViewMatrix();
        osg::dmat4 shadowProj = _camera->getProjectionMatrix();
        ...
        osg::dmat4 shadowTextureMatrix =
            bias * shadowProj * shadowView * inverseCameraView;
...
void
StandardShadowMap::ViewData::init( ThisClass*            st,
                                   osgUtil::CullVisitor* cv )
{
    ...
    {    // Create and add fake texture for use with nodes without any texture
        ...
        *( osg::ubvec4* )image->data() = osg::ubvec4( 0XFF, 0XFF, 0XFF, 0XFF );
        ...
        _stateset->setTextureAttribute( st->_baseTextureUnit,
                                        fakeTex,
                                        osg::StateAttribute::ON );
    }

    {    // Add shadow texture
        _stateset->setTextureAttributeAndModes( st->_shadowTextureUnit,
                                                _texture.get(),
                                                osg::StateAttribute::ON );
    }
    ...
    {    // Setup shaders used in shadow casting
        ...
        _stateset->addUniform( new osg::Uniform( "baseTexture",
                                                 int( st->_baseTextureUnit ) ) );
        _stateset->addUniform( new osg::Uniform( "shadowTexture",
                                                 int( st->_shadowTextureUnit ) ) );

        // Shadow texture matrix uniform (updated each frame in cull())
        _shadowTextureMatrixUniform =
            new osg::Uniform( osg::Uniform::FLOAT_MAT4, "shadowTextureMatrix" );
    ...
    {    // Setup states used for shadow map generation
        ...
        stateset->setAttribute( new osg::CullFace( osg::CullFace::Mode::FRONT ),
                                osg::StateAttribute::ON |
                                    osg::StateAttribute::OVERRIDE );
...
const osg::Light*
StandardShadowMap::ViewData::selectLight( osg::vec4& lightPos,
                                          osg::vec3& lightDir )
{
    ...
    {    // transform light to world space
        ...
        osg::dmat4 localToWorld = osg::inverse( *_cv->getModelViewMatrix() );
        if( matrix )
        {
            localToWorld** matrix;
        }
        ...
        lightDir = osg::transform3x3( lightDir, localToWorld );
                            ...
                            /* by default = osg::vec3( 0, 1 0 )*/ )
{
    ...
    osg::sphere bs( cbbv.getBoundingBox().center(), cbbv.getBoundingBox().radius() );
#endif

    aimShadowCastingCamera( bs, light, lightPos, lightDir, lightUp );
}
...
StandardShadowMap::ViewData::
    aimShadowCastingCamera( const osg::sphere& bs,
                            ...
                            /* by default = osg::vec3( 0, 1 0 )*/ )
{
    osg::dmat4& view       = _camera->getViewMatrix();
    osg::dmat4& projection = _camera->getProjectionMatrix();
    ...
    osg::dvec3 position( lightPos.x, lightPos.y, lightPos.z );
    if( lightPos[3] == 0.0 )    // infinite directional light
    {
        // make an orthographic projection
        // set the position far away along the light direction
        position = osg::dvec3( bs.center ) -
                   osg::dvec3( lightDir ) *
                   static_cast<double>( bs.radius ) *
                   2.0;
    }

    double centerDistance = osg::length( position - osg::dvec3( bs.center ) );
    ...
    {                                            // positional light
        ...
        {
            double spotAngle = light->getSpotCutoff();
            projection       = osg::perspective( spotAngle * 2.0, 1.0, znear, zfar );
            view             = osg::lookAt( position,
                                            position + osg::dvec3( lightDir ),
                                            osg::dvec3( up ) );
        }
        else
        {    // standard omnidirectional positional light
            double top   = ( bs.radius / centerDistance ) * znear;
            double right = top;

            projection   = osg::perspective( -right, right, -top, top, znear, zfar );
            view = osg::lookAt( position, osg::dvec3( bs.center ), osg::dvec3( up ) );
        }
    }
    else    // directional light
    {
        ...
        double right = top;
        projection   = osg::orthographic( -right, right, -top, top, znear, zfar );
        view = osg::lookAt( position, osg::dvec3( bs.center ), osg::dvec3( up ) );
    }
}

void
StandardShadowMap::ViewData::cullShadowReceivingScene()
{
...
void
StandardShadowMap::ViewData::cullShadowCastingScene()
{
