...
// skip all the expensive shadow sampling if not needed
    ...
    "      smCoord.xy = sceneShadowProj.xy  + offset.xy * softFactor; \n"
    "      shadow +=  textureProj( osgShadow_shadowTexture, smCoord ).x * INV_SAMPLECOUNT; \n"
...
// apply shadow, modulo the ambient bias
    "  fragColor = vertexColor * (osgShadow_ambientBias.x + shadow * osgShadow_ambientBias.y); \n"
    "} \n";
...
// apply shadow, modulo the ambient bias
    "  fragColor = color * (osgShadow_ambientBias.x + shadow * osgShadow_ambientBias.y); \n"
    "} \n";
