...
std::string ParallelSplitShadowMap::FragmentShaderGenerator::generateGLSL_FragmentShader_BaseTex(
    ...
    unsigned int textureOffset
        ...
        for (unsigned int i=0;i<nbrSplits;i++)    {
            if (!filtered) {
                sstr << "    float shadow"    <<    i    <<" = texture( shadowTexture"    <<    i    <<",shadowTexCoord_"    <<    i    <<".xyz).r;"   << std::endl;
                ...
                sstr << "    float shadowOrg"    <<    i    <<" = texture( shadowTexture"  <<    i    <<",shadowTexCoord_"    <<    i    <<".xyz+vec3(0.0,0.0,fZOffSet) ).r;"   << std::endl;
                sstr << "    float shadow0"    <<    i    <<" = texture( shadowTexture"    <<    i    <<",shadowTexCoord_"    <<    i    <<".xyz+vec3(-fTexelSize,-fTexelSize,fZOffSet) ).r;"   << std::endl;
                sstr << "    float shadow1"    <<    i    <<" = texture( shadowTexture"    <<    i    <<",shadowTexCoord_"    <<    i    <<".xyz+vec3( fTexelSize,-fTexelSize,fZOffSet) ).r;"   << std::endl;
                sstr << "    float shadow2"    <<    i    <<" = texture( shadowTexture"    <<    i    <<",shadowTexCoord_"    <<    i    <<".xyz+vec3( fTexelSize, fTexelSize,fZOffSet) ).r;"   << std::endl;
                sstr << "    float shadow3"    <<    i    <<" = texture( shadowTexture"    <<    i    <<",shadowTexCoord_"    <<    i    <<".xyz+vec3(-fTexelSize, fTexelSize,fZOffSet) ).r;"   << std::endl;
        ...
        if ( debug ) {
            ...
            sstr << "    vec4 color    = vertexColor*( 1.0 - sumTerm ) + (sumTerm)* vertexColor*vec4(c0,(1.0-c0)*c1,(1.0-c0)*(1.0-c1)*c2,1.0); "    << std::endl;
        ...
        sstr << "    fragColor.rgb = (((color*(ambientBias.x+1.0)*(1.0-enableBaseTextureFilter)) + colorTex*(1.0+ambientBias.x)*enableBaseTextureFilter)*(1.0-ambientBias.y*v)).rgb; "<< std::endl;
        sstr << "    fragColor.a = (color*(1.0-enableBaseTextureFilter) + colorTex*enableBaseTextureFilter).a; "<< std::endl;
