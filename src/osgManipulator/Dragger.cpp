...
//
void
osgManipulator::computeNodePathToRoot( osg::Node&     node,
                                       osg::NodePath& np )
{
    np.clear();
    ...
    if( !nodePaths.empty() )
    {
        ...
        if( nodePaths.size() > 1 )
        {
            OSG_NOTICE << "osgManipulator::computeNodePathToRoot(,) taking first parent "
                          "path, ignoring others."
                       << std::endl;
        }
    }
}
...
DraggerTransformCallback::DraggerTransformCallback( osg::MatrixTransform* transform,
    ...
    _handleCommandMask( static_cast<unsigned int>( handleCommandMask ) ),
    _transform( transform )
{
}

bool
DraggerTransformCallback::receive( const MotionCommand& command )
{
    if( !_transform )
    {
        return false;
...
bool
DraggerTransformCallback::receive( const TranslateInLineCommand& command )
{
...
bool
DraggerTransformCallback::receive( const TranslateInPlaneCommand& command )
{
...
bool
DraggerTransformCallback::receive( const Scale1DCommand& command )
{
...
bool
DraggerTransformCallback::receive( const Scale2DCommand& command )
{
...
bool
DraggerTransformCallback::receive( const ScaleUniformCommand& command )
{
...
bool
DraggerTransformCallback::receive( const Rotate3DCommand& command )
{
...
bool
PointerInfo::contains( const osg::Node* node ) const
{
    if( node && _hitIter != _hitList.end() )
    {
        return std::find( ( *_hitIter ).first.begin(),
                          ( *_hitIter ).first.end(),
                          node ) != ( *_hitIter ).first.end();
    }
    else
    {
        return false;
...
bool
PointerInfo::projectWindowXYIntoObject( const osg::dvec2& windowCoord,
                                        osg::dvec3&       nearPoint,
                                        osg::dvec3&       farPoint ) const
{
    nearPoint = osg::dvec3( windowCoord.x, windowCoord.y, 0.0 ) * _inverseMVPW;
    farPoint  = osg::dvec3( windowCoord.x, windowCoord.y, 1.0 ) * _inverseMVPW;
...
Dragger::Dragger() :
    ...
    _parentDragger( 0 ),
    _intersectionMask( 0XFF'FF'FF'FF )
{
    _parentDragger = this;
    getOrCreateStateSet()->setDataVariance( osg::Object::DataVariance::DYNAMIC );

    _selfUpdater = new DraggerTransformCallback( this );
}

Dragger::Dragger( const Dragger&     rhs,
...
bool
Dragger::inverted() const
{
    osg::dvec3 xAxis( _matrix( 0, 0 ), _matrix( 1, 0 ), _matrix( 2, 0 ) );
    osg::dvec3 yAxis( _matrix( 0, 1 ), _matrix( 1, 1 ), _matrix( 2, 1 ) );
    osg::dvec3 zAxis( _matrix( 0, 2 ), _matrix( 1, 2 ), _matrix( 2, 2 ) );
    double     volume = osg::dot( osg::cross( xAxis, yAxis ), zAxis );
    return volume < 0.0;
}

void
Dragger::applyAppropriateFrontFace( osg::StateSet* ss ) const
{
...
void
Dragger::setHandleEvents( bool flag )
{
    ...
    // update the number of children that require an event traversal to make sure this
    // dragger receives events.
    if( _handleEvents )
    {
        setNumChildrenRequiringEventTraversal( getNumChildrenRequiringEventTraversal() +
                                               1 );
    }
...
void
Dragger::addConstraint( Constraint* constraint )
{
    // check to make sure constaint hasn't already been attached.
    for( Constraints::iterator itr = _constraints.begin(); itr != _constraints.end();
         ++itr )
    {
        if( *itr == constraint )
        {
...
void
Dragger::removeConstraint( Constraint* constraint )
{
    for( Constraints::iterator itr = _constraints.begin(); itr != _constraints.end(); )
    {
        if( *itr == constraint )
...
void
Dragger::addTransformUpdating( osg::MatrixTransform* transform,
                               int                   handleCommandMask )
{
...
void
Dragger::removeTransformUpdating( osg::MatrixTransform* transform )
{
...
void
Dragger::addDraggerCallback( DraggerCallback* dc )
{
...
void
Dragger::traverse( osg::NodeVisitor& nv )
{
...
bool
Dragger::handle( const osgGA::GUIEventAdapter& ea,
                 osgGA::GUIActionAdapter&      aa )
{
    ...
    if( _activationModKeyMask !=
        0 ||
        _activationMouseButtonMask !=
        0 ||
        _activationKeyEvent != 0 )
    {
        _activationPermittedByModKeyMask =
            ( _activationModKeyMask != 0 )
                ? ( ( static_cast<unsigned int>( ea.getModKeyMask() ) &
                      _activationModKeyMask ) != 0 )
                : false;

        _activationPermittedByMouseButtonMask =
            ( _activationMouseButtonMask != 0 )
                ? ( ( static_cast<unsigned int>( ea.getButtonMask() ) &
                      _activationMouseButtonMask ) != 0 )
                : false;
            ...
            case osgGA::GUIEventAdapter::PUSH :
                {
                    ...
                    if( view->computeIntersections( ea,
                                                    intersections,
                                                    _intersectionMask ) )
                    {
...
bool
Dragger::receive( const MotionCommand& command )
{
...
void
Dragger::dispatch( MotionCommand& command )
{
...
// CompositeDragger
//
CompositeDragger::CompositeDragger( const CompositeDragger& rhs,
                                    const osg::CopyOp&      copyop ) :
    Inherit( rhs,
             copyop )
{
    OSG_NOTICE << "CompositeDragger::CompositeDragger(const CompositeDragger& rhs, "
                  "const osg::CopyOp& copyop) not Implemented yet."
               << std::endl;
}

bool
CompositeDragger::handle( const PointerInfo&            pi,
                          ...
                          osgGA::GUIActionAdapter&      aa )
{
    ...
    for( DraggerList::iterator itr = _draggerList.begin(); itr != _draggerList.end();
         ++itr )
    {
...
bool
CompositeDragger::containsDragger( const Dragger* dragger ) const
{
    for( DraggerList::const_iterator itr = _draggerList.begin();
         itr != _draggerList.end();
         ++itr )
    {
        if( itr->get() == dragger )
        {
...
CompositeDragger::DraggerList::iterator
CompositeDragger::findDragger( const Dragger* dragger )
{
    for( DraggerList::iterator itr = _draggerList.begin(); itr != _draggerList.end();
         ++itr )
    {
...
bool
CompositeDragger::addDragger( Dragger* dragger )
{
...
bool
CompositeDragger::removeDragger( Dragger* dragger )
{
...
void
CompositeDragger::setParentDragger( Dragger* dragger )
{
    for( DraggerList::iterator itr = _draggerList.begin(); itr != _draggerList.end();
         ++itr )
    {
...
void
CompositeDragger::setIntersectionMask( osg::Node::NodeMask intersectionMask )
{
...
void
osgManipulator::setDrawableToAlwaysCull( osg::Drawable& drawable )
{
...
void
osgManipulator::setMaterialColor( const osg::vec4& color,
                                  osg::Node&       node )
{
    osg::Material* mat = dynamic_cast<osg::Material*>(
        node.getOrCreateStateSet()->getAttribute( osg::StateAttribute::Type::MATERIAL )
    );
    if( !mat )
    {
        mat = new osg::Material;
        mat->setDataVariance( osg::Object::DataVariance::DYNAMIC );
