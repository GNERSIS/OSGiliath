...
namespace
{
    ...
    //------------------------------------------------------------------------------
    osg::Geometry*
    createDiskGeometry( float        radius,
                        float        offset,
                        float        z,
                        unsigned int numSegments )
    {
        ...
        for( unsigned int i = 0; i < numSegments; ++i, angle += angleDelta )
        {
            ...
            ++p;
            // Inner point
            ( *vertexArray )[p].set( ( radius - offset ) * c,
                                     ( radius - offset ) * s,
                                     z );
            ( *normalArray )[p].set( 0.0, 0.0, -1.0 );
            ++p;
...
bool
RotateCylinderDragger::handle( const PointerInfo&            pointer,
        ...
        case( osgGA::GUIEventAdapter::DRAG ) :
            {
                ...
                if( _projector->project( pointer, projectedPoint ) )
                {
                    ...
                    osg::quat deltaRotation =
                        _projector->getRotation( prevProjectedPoint, projectedPoint );
...
RotateCylinderDragger::setupDefaultGeometry()
{
    osg::Geode* geode = new osg::Geode;
    {
        ...
        // inner cylinder
        osg::Cylinder* cylinder1 =
            const_cast<osg::Cylinder*>( _projector->getCylinder() );
        cylinder1->setHeight( height );
        cylinder1->setRadius( radius - thickness );
        osg::ShapeDrawable* cylinderDrawable1 =
            new osg::ShapeDrawable( cylinder1, hints );
