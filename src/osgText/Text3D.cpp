...
namespace osgText
{
    ...
    Text3D::Text3D( const Text3D&      text3D,
    ...
    float
    Text3D::getCharacterDepth() const
    {
        if( !_style )
        {
    ...
    void
    Text3D::setCharacterDepth( float characterDepth )
    {
    ...
    void
    Text3D::accept( osg::Drawable::ConstAttributeFunctor& af ) const
    {
    ...
    void
    Text3D::accept( osg::PrimitiveFunctor& pf ) const
    {
        if( !_coords || _coords->empty() )
        {
            return;
        ...
        // short term fix/workaround for _coords being transformed by a local matrix
        // before rendering, so we need to replicate this was doing tasks like
        ...
        if( !osg::isIdentity( _matrix ) )
        {
            ...
            for( osg::Vec3Array::iterator sitr = _coords->begin(),
                                          ditr = vertices->begin();
                 sitr != _coords->end();
                 ++sitr, ++ditr )
            {
    ...
    String::iterator
    Text3D::computeLastCharacterOnLine( osg::vec2&       cursor,
                                        ...
                                        String::iterator last )
    {
        ...
        for( bool outOfSpace = false; lastChar != last; ++lastChar )
        {
            ...
            if( glyph )
            {
                const osg::box& bb = glyph->getBoundingBox();
                ...
                if( kerning && previous_charcode )
                {
                    switch( _layout )
                    {
                        case LEFT_TO_RIGHT :
                            {
                                osg::vec2 delta( _font->getKerning( _fontSize,
                                                                    previous_charcode,
                                                                    charcode,
                                                                    _kerningType ) );
                                cursor.x += delta.x * wr;
        ...
        if( lastChar != last )
        {
            ...
            while( lastValidChar != first )
            {
                ...
                // Subtract off glyphs from the cursor position (to correctly center
                // text)
                if( *prevChar != '-' )
                {
                    ...
                    if( glyph )
                    {
                        switch( _layout )
                        {
                            ...
                            case RIGHT_TO_LEFT :
                                ...
                                break;
                            case VERTICAL :
                                cursor.y += glyph->getVerticalAdvance() * wr;
                                break;
                        }
                    }
                }
    ...
    void
    Text3D::copyAndOffsetPrimitiveSets(
        osg::Geometry::PrimitiveSetList& dest_PrimitiveSetList,
        osg::Geometry::PrimitiveSetList& src_PrimitiveSetList,
        unsigned int                     offset
    )
    {
        for( osg::Geometry::PrimitiveSetList::const_iterator pitr =
                 src_PrimitiveSetList.begin();
             pitr != src_PrimitiveSetList.end();
             ++pitr )
        {
            const osg::PrimitiveSet* src_primset = pitr->get();
            osg::PrimitiveSet*       dst_primset =
                osg::clone( src_primset, osg::CopyOp::DEEP_COPY_ALL );
            dst_primset->offsetIndices( static_cast<int>( offset ) );
    ...
    void
    Text3D::computeGlyphRepresentation()
    {
        if( _font.valid() == false )
        {
            return;
        ...
        for( String::iterator itr = _text.begin(); itr != _text.end(); )
        {
            ...
            TextBase::positionCursor( endOfLine_coords,
                                      cursor,
                                      ( unsigned int )( endOfLine_itr -
                                                        startOfLine_itr ) );
                ...
                for( ; itr != endOfLine_itr; ++itr )
                {
                    ...
                    if( glyph )
                    {
                        ...
                        switch( _layout )
                        {
                            case LEFT_TO_RIGHT :
                                _textBB.expandBy(
                                    osg::vec3( cursor.x + bb.xMin() * wr,
                                               cursor.y + bb.yMin() * hr,
                                               0.0F )
                                );    // lower left corner
                                _textBB.expandBy(
                                    osg::vec3( cursor.x + bb.xMax() * wr,
                                               cursor.y + bb.yMax() * hr,
                                               0.0F )
                                );    // upper right corner
                                cursor.x += glyph->getHorizontalAdvance() * wr;
                            ...
                            case VERTICAL :
                                _textBB.expandBy(
                                    osg::vec3( cursor.x, cursor.y, 0.0F )
                                );    // upper left corner
                                _textBB.expandBy(
                                    osg::vec3( cursor.x + glyph->getWidth() * wr,
                                               cursor.y - glyph->getHeight() * hr,
                                               0.0F )
                                );    // lower right corner
                                cursor.y -= glyph->getVerticalAdvance() * hr;
                                break;
                        ...
                        GlyphGeometry* glyphGeometry =
                            glyph->getGlyphGeometry( _style.get() );
                        currentLineRenderInfo.push_back(
                            Text3D::GlyphRenderInfo( glyphGeometry, pos )
                        );
                ...
                case RIGHT_TO_LEFT :
                    {
                        startOfLine_coords.y -=
                            static_cast<float>( ( 1.0 + _lineSpacing ) * hr );
                        ++_lineCount;
                        break;
                    }
                case VERTICAL :
                    {
                        startOfLine_coords.x += static_cast<float>(
                            _characterHeight * ( 1.0 + _lineSpacing ) * wr
                        );
                        // because _lineCount is the max vertical no. of characters....
                        _lineCount =
                            ( _lineCount > linelength ) ? _lineCount : linelength;
                        break;
                    }
                default :
                    break;
        ...
        _textBB.min.z   = -thickness;

        computePositions();

        if( !_coords )
        {
            OSG_NOTICE << "void Text3D::computeGlyphRepresentation() _coords = new "
                          "osg::Vec3Array;"
                       << std::endl;
        ...
        if( !_normals )
        {
            OSG_NOTICE << "void Text3D::computeGlyphRepresentation() _normals = new "
                          "osg::Vec3Array;"
                       << std::endl;
        ...
        for( itLine = _textRenderInfo.begin(); itLine != endText; ++itLine )
        {
            ...
            {
                osg::Vec3Array* src_vertices = dynamic_cast<osg::Vec3Array*>(
                    it->_glyphGeometry->getVertexArray()
                );
                osg::Vec3Array* src_normals = dynamic_cast<osg::Vec3Array*>(
                    it->_glyphGeometry->getNormalArray()
                );
                ...
                copyAndOffsetPrimitiveSets(
                    _frontPrimitiveSetList,
                    it->_glyphGeometry->getFrontPrimitiveSetList(),
                    base
                );
                copyAndOffsetPrimitiveSets(
                    _wallPrimitiveSetList,
                    it->_glyphGeometry->getWallPrimitiveSetList(),
                    base
                );
                copyAndOffsetPrimitiveSets(
                    _backPrimitiveSetList,
                    it->_glyphGeometry->getBackPrimitiveSetList(),
                    base
                );
            }
        }
    ...
    osg::box
    Text3D::computeBoundingBox() const
    {
        osg::box bbox;
    ...
    void
    Text3D::drawImplementation( osg::RenderInfo& renderInfo ) const
    {
        ...
        if( ( _drawMode & ( ~static_cast<unsigned int>( TEXT ) ) ) != 0 )
        {
        ...
        if( _drawMode & TEXT )
        {
            ...
            for( osg::Geometry::PrimitiveSetList::const_iterator
                     itr = _frontPrimitiveSetList.begin(),
                     end = _frontPrimitiveSetList.end();
                 itr != end;
                 ++itr )
            {
                ( *itr )->draw( state, usingVertexBufferObjects );
            ...
            for( osg::Geometry::PrimitiveSetList::const_iterator
                     itr = _wallPrimitiveSetList.begin(),
                     end = _wallPrimitiveSetList.end();
                 itr != end;
                 ++itr )
            {
                ( *itr )->draw( state, usingVertexBufferObjects );
            ...
            for( osg::Geometry::PrimitiveSetList::const_iterator
                     itr = _backPrimitiveSetList.begin(),
                     end = _backPrimitiveSetList.end();
                 itr != end;
                 ++itr )
            {
                ( *itr )->draw( state, usingVertexBufferObjects );
    ...
    void
    Text3D::resizeGLObjectBuffers( unsigned int maxSize )
    {
    ...
    void
    Text3D::releaseGLObjects( osg::State* state ) const
    {
