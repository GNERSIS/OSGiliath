...
void TextBase::compileGLObjects(osg::RenderInfo& renderInfo) const
{
    ...
    if (renderInfo.getState()->useVertexBufferObject(_supportsVertexBufferObjects && _useVertexBufferObjects))
    {
        ...
        if (_vbo.valid())
        {
            ...
            if (glBuf && glBuf->isDirty()) glBuf->compileBuffer();
        }
        ...
        if (state.useVertexArrayObject(_useVertexArrayObject))
        {
            ...
            if (_coords.valid()) vas->setVertexArray(state, _coords.get());
            if (_normals.valid()) vas->setNormalArray(state, _normals.get());
            if (_colorCoords.valid()) vas->setColorArray(state, _colorCoords.get());
            if (_texcoords.valid()) vas->setTexCoordArray(state, 0, _texcoords.get());
