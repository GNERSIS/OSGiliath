...
Text::Text() :
    ...
    _colorGradientTopLeft( 1.0F,
                           0.0F,
                           0.0F,
                           1.0F ),
    _colorGradientBottomLeft( 0.0F,
                              1.0F,
                              0.0F,
                              1.0F ),
    _colorGradientBottomRight( 0.0F,
                               0.0F,
                               1.0F,
                               1.0F ),
    _colorGradientTopRight( 1.0F,
                            1.0F,
                            1.0F,
                            1.0F )
{
    ...
    if( !str.empty() )
    {
        if( str == "ALL_FEATURES" || str == "ALL" )
        {
            _shaderTechnique = ALL_FEATURES;
        }
        else if( str == "GREYSCALE" )
        {
            _shaderTechnique = GREYSCALE;
        }
        else if( str == "SIGNED_DISTANCE_FIELD" || str == "SDF" )
        {
            _shaderTechnique = SIGNED_DISTANCE_FIELD;
        }
        else if( str == "NO_TEXT_SHADER" || str == "NONE" )
        {
...
void
Text::setShaderTechnique( ShaderTechnique technique )
{
    if( _shaderTechnique == technique )
    {
        return;
    }
...
Text::createStateSet()
{
    ...
    {
        ss.str( "" );
        ss << "vec4(" << _backdropColor.r << ", " << _backdropColor.g << ", "
           << _backdropColor.b << ", " << _backdropColor.a << ")";

        defineList["BACKDROP_COLOR"] =
        ...
        else
        {
            ...
            switch( _backdropType )
            {
                case( DROP_SHADOW_BOTTOM_RIGHT ) :
                    offset.set( _backdropHorizontalOffset, -_backdropVerticalOffset );
                    break;
                case( DROP_SHADOW_CENTER_RIGHT ) :
                    offset.set( _backdropHorizontalOffset, 0.0F );
                    break;
    ...
    {
        defineList["SIGNED_DISTANCE_FIELD"] =
    ...
    if( !statesets.empty() )
    {
        for( Font::StateSets::iterator itr = statesets.begin(); itr != statesets.end();
             ++itr )
        {
            if( ( *itr )->getDefineList() == defineList )
            {
                // OSG_NOTICE<<"Text::createStateSet() : Matched DefineList, return
                // StateSet "<<itr->get()<<std::endl;
                return itr->get();
            }
            else
            {
    ...
    if( osg::isNotifyEnabled( DEBUG_MESSAGE_LEVEL ) )
    {
        ...
        {
            case( NO_TEXT_SHADER ) :
                DEBUG_MESSAGE << "NO_TEXT_SHADER" << std::endl;
                break;
            case( GREYSCALE ) :
                DEBUG_MESSAGE << "GREYSCALE" << std::endl;
                break;
            case( SIGNED_DISTANCE_FIELD ) :
                DEBUG_MESSAGE << "SIGNED_DISTANCE_FIELD" << std::endl;
                break;
            case( ALL_FEATURES ) :
                DEBUG_MESSAGE << "ALL_FEATURES" << std::endl;
                break;
        }
    }
...
#include "shaders/osgText_Text_frag.cpp"
        program->addShader(
            osgDB::readRefShaderFileWithFallback( osg::Shader::FRAGMENT,
                                                  "shaders/osgText_Text.frag",
                                                  osgText_Text_frag )
        );
    }
...
Text::getActiveFont()
{
    ...
    if( !_fontFallback )
    {
        _fontFallback = Font::getDefaultFont();
    }
...
String::iterator
Text::computeLastCharacterOnLine( osg::vec2&       cursor,
    ...
    for( bool outOfSpace = false; lastChar != last; ++lastChar )
    {
        ...
        if( glyph )
        {
            ...
            if( kerning && previous_charcode )
            {
                ...
                }    // check to see if we are still within line if not move to next
                     // line.
            }
                    ...
                    const osg::vec2& maxtc )
{
    ...
    if( !primitives )
    {
        unsigned int maxIndices = static_cast<unsigned int>( _text.size() ) * 4;
        if( maxIndices >= 16'384 )
        {
...
void
Text::computeGlyphRepresentation()
{
    Font* activefont = getActiveFont();
    if( !activefont )
    {
    ...
    for( String::iterator itr = _text.begin(); itr != _text.end(); )
    {
        ...
        linelength = static_cast<unsigned int>( endOfLine_itr - startOfLine_itr );
            ...
            case VERTICAL :
                {
                    switch( _alignment )
                    {
                        ...
                        // need to implement
                        //       top of characters alignment - Question is this
                        //       necessary?
                        ...
                        case RIGHT_CENTER :
                            cursor.y =
                                cursor.y + ( cursor.y - endOfLine_coords.y ) * 0.5F;
                            break;
                        case LEFT_BOTTOM_BASE_LINE :
                        case CENTER_BOTTOM_BASE_LINE :
                        case RIGHT_BOTTOM_BASE_LINE :
                        ...
                        case CENTER_BOTTOM :
                        case RIGHT_BOTTOM :
                            cursor.y = 2.0F * cursor.y - endOfLine_coords.y;
                            break;
                        default :
                            break;
                    }
        ...
        if( itr != endOfLine_itr )
        {
            ...
            for( ; itr != endOfLine_itr; ++itr )
            {
                ...
                if( glyph )
                {
                    ...
                    if( kerning && previous_charcode )
                    {
                        switch( _layout )
                        {
                            case LEFT_TO_RIGHT :
                                {
                                    osg::vec2 delta(
                                        activefont->getKerning( _fontSize,
                                                                previous_charcode,
                                                                charcode,
                                                                _kerningType )
                                    );
                    ...
                    const Glyph::TextureInfo* info =
                        glyph->getOrCreateTextureInfo( _shaderTechnique );
                    if( info )
                    {
                        ...
                        float     fHorizTCMargin =
                            texelMargin /
                            static_cast<float>( info->texture->getTextureWidth() );
                        float fVertTCMargin =
                            texelMargin /
                            static_cast<float>( info->texture->getTextureHeight() );
                        float fHorizQuadMargin =
                            vDiff.x == 0.0F ? 0.0F : width * fHorizTCMargin / vDiff.x;
                        float fVertQuadMargin =
                        ...
                        osg::vec2 minc  = local + osg::vec2( 0.0F - fHorizQuadMargin,
                                                             0.0F - fVertQuadMargin );
                        osg::vec2 maxc  = local + osg::vec2( width + fHorizQuadMargin,
                                                             height + fVertQuadMargin );
                        ...
                        switch( _layout )
                        {
                            case LEFT_TO_RIGHT :
                                cursor.x += glyph->getHorizontalAdvance() * wr;
                                _textBB.expandBy(
                                    osg::vec3( local.x, local.y, 0.0F )
                                );    // lower left corner
                                _textBB.expandBy(
                                    osg::vec3( local.x + width, local.y + height, 0.0F )
                                );    // upper right corner
                                break;
                            case VERTICAL :
                                cursor.y -= glyph->getVerticalAdvance() * hr;
                                _textBB.expandBy(
                                    osg::vec3( local.x, local.y + height, 0.0F )
                                );    // upper left corner
                                _textBB.expandBy(
                                    osg::vec3( local.x + width, local.y, 0.0F )
                                );    // lower right corner
                                break;
                            case RIGHT_TO_LEFT :
                                _textBB.expandBy(
        ...
        switch( _layout )
        {
            case LEFT_TO_RIGHT :
                {
                    startOfLine_coords.y -=
                        static_cast<float>( _characterHeight * ( 1.0 + _lineSpacing ) );
...
// This method is used several times in a loop for the same object which will produce the
// same values. Further optimization may try saving these values instead of recomputing
// them.
bool
Text::computeAverageGlyphWidthAndHeight( float& avg_width,
                                         float& avg_height ) const
{
    float width          = 0.0F;
    float height         = 0.0F;
    ...
    {
        avg_width  = running_width / static_cast<float>( counter );
        avg_height = running_height / static_cast<float>( counter );
    }
    return is_valid_size;
}

void
Text::computePositionsImplementation()
{
    // TextBase::computePositionsImplementation(); computes basic positions and maps the
    // _textBB to _textBBWithMargin
    ...
    {
        _textBBWithMargin.set( _textBBWithMargin.xMin() - _textBBMargin,
...
void
Text::computeColorGradientsOverall()
{
    ...
    unsigned int       numCoords   = static_cast<unsigned int>( coords->size() );
    ...
    {
        float red   = bilinearInterpolate( min_x,
                                           max_x,
                                           min_y,
                                           max_y,
                                           ( *coords )[i].x,
        ...
        float green = bilinearInterpolate( min_x,
                                           max_x,
                                           min_y,
                                           max_y,
                                           ( *coords )[i].x,
        ...
        float blue  = bilinearInterpolate( min_x,
                                           max_x,
                                           min_y,
                                           max_y,
                                           ( *coords )[i].x,
        ...
        float alpha         = bilinearInterpolate( min_x,
                                                   max_x,
                                                   min_y,
                                                   max_y,
                                                   ( *coords )[i].x,
...
void
Text::computeColorGradientsPerCharacter()
{
...
void
Text::drawImplementation( osg::RenderInfo& renderInfo ) const
{
    drawImplementation( *renderInfo.getState(), osg::vec4( 1.0F, 1.0F, 1.0F, 1.0F ) );
}

void
Text::drawImplementationSinglePass( osg::State&      state,
                                    const osg::vec4& colorMultiplier ) const
{
    if( colorMultiplier.a == 0.0F || _color.a == 0.0F )
    {
        return;
    }
    ...
    if( ( _drawMode & ( ~static_cast<unsigned int>( TEXT ) ) ) !=
        0 &&
        !_decorationPrimitives.empty() )
    {
    ...
    if( _drawMode & TEXT )
    {
             ...
             ++titr )
        {
            ...
            {
                vas->disableColorArray( state );
                state.Color( colorMultiplier.r * _color.r,
                             colorMultiplier.g * _color.g,
                             colorMultiplier.b * _color.b,
                             colorMultiplier.a * _color.a );
            }
            else
            {
...
void
Text::drawImplementation( osg::State&      state,
                          const osg::vec4& colorMultiplier ) const
{
    // save the previous modelview matrix
    osg::dmat4 previous_modelview = state.getModelViewMatrix();
    ...
    if( needToApplyMatrix )
    {
        ...
        modelview = previous_modelview * modelview;
    ...
    state.haveAppliedAttribute( osg::StateAttribute::Type::DEPTH );
...
void
Text::accept( osg::Drawable::ConstAttributeFunctor& af ) const
{
    if( _coords.valid() && !_coords->empty() )
    {
        af.apply( osg::Drawable::VERTICES,
                  static_cast<unsigned int>( _coords->size() ),
                  &( _coords->front() ) );
    }

    if( _texcoords.valid() && !_texcoords->empty() )
    {
        af.apply( osg::Drawable::TEXTURE_COORDS_0,
                  static_cast<unsigned int>( _texcoords->size() ),
                  &( _texcoords->front() ) );
    }
}

void
Text::accept( osg::PrimitiveFunctor& pf ) const
{
    if( !_coords || _coords->empty() )
    {
        return;
    }

    // short term fix/workaround for _coords being transformed by a local matrix before
    // rendering, so we need to replicate this was doing tasks like intersection testing.
    osg::ref_ptr<osg::Vec3Array> vertices = _coords;
    if( !osg::isIdentity( _matrix ) )
    {
        ...
        for( Vec3Array::iterator sitr = _coords->begin(), ditr = vertices->begin();
             ...
             ++sitr, ++ditr )
        {
            *ditr = osg::vec3( _matrix * osg::dvec3( *sitr ) );
        }
    }

    pf.setVertexArray( static_cast<unsigned int>( vertices->size() ),
                       &( vertices->front() ) );
         ...
         ++titr )
    {
        ...
        {
            const osg::DrawElementsUShort* drawElementsUShort =
                dynamic_cast<const osg::DrawElementsUShort*>(
                    glyphquad._primitives.get()
                );
            if( drawElementsUShort && drawElementsUShort->size() > 0 )
            {
                pf.drawElements( GL_TRIANGLES,
                                 static_cast<GLsizei>( drawElementsUShort->size() ),
                                 &( drawElementsUShort->front() ) );
            }
            else
            {
                const osg::DrawElementsUInt* drawElementsUInt =
                ...
                {
                    pf.drawElements( GL_TRIANGLES,
                                     static_cast<GLsizei>( drawElementsUInt->size() ),
                                     &( drawElementsUInt->front() ) );
                }
            }
        }
    }
...
bool
Text::getCharacterCorners( unsigned int index,
                           osg::vec3&   bottomLeft,
                           ...
                           osg::vec3&   topRight ) const
{
    ...
    if( ( index * 4 + 4 ) > static_cast<unsigned int>( _coords->size() ) )
    {
        return false;
    }
...
void
Text::resizeGLObjectBuffers( unsigned int maxSize )
{
...
void
Text::releaseGLObjects( osg::State* state ) const
{
...
void
Text::setBackdropType( BackdropType type )
{
...
void
Text::setBackdropOffset( float offset )
{
    if( _backdropHorizontalOffset == offset && _backdropVerticalOffset == offset )
    {
        return;
    }
...
void
Text::setBackdropOffset( float horizontal,
                         float vertical )
{
    if( _backdropHorizontalOffset == horizontal && _backdropVerticalOffset == vertical )
    {
        return;
    }
...
void
Text::setBackdropColor( const osg::vec4& color )
{
    if( _backdropColor == color )
    {
        return;
    }
...
void
Text::setColorGradientMode( ColorGradientMode mode )
{
...
void
Text::setColorGradientCorners( const osg::vec4& topLeft,
...
float
Text::bilinearInterpolate( float x1,
                           float x2,
                           float y1,
                           float y2,
                           float x,
...
void
Text::GlyphQuads::resizeGLObjectBuffers( unsigned int maxSize )
{
    if( _primitives.valid() )
    {
        _primitives->resizeGLObjectBuffers( maxSize );
    }
}

void
Text::GlyphQuads::releaseGLObjects( osg::State* state ) const
{
    if( _primitives.valid() )
    {
        _primitives->releaseGLObjects( state );
    }
}
