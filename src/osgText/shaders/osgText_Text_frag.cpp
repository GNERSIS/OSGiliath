char osgText_Text_frag[] =
    ...
    "    {\n"
    "        float f = smoothstep(0.0, 1.0, "
    "(blend_half_width-edge_distance)/(blend_width));\n"
    "        return vec2(1.0 - f, f);\n"
    "    }\n"
    ...
    "    {\n"
    "        return vec2(0.0, smoothstep(0.0, 1.0, "
    "(blend_half_width+outline_width+edge_distance)/blend_width));\n"
    "    }\n"
    "    else\n"
    "    {\n"
    "        return vec2(0.0, 0.0);\n"
    ...
    "\n"
    "\n"
    "    float distance_across_pixel = "
    "length(dx+dy)*(TEXTURE_DIMENSION/GLYPH_DIMENSION);\n"
    "\n"
    ...
    "    if (numSamplesX>maxNumSamplesAcrossSide) numSamplesX = "
    "maxNumSamplesAcrossSide;\n"
    "    if (numSamplesY>maxNumSamplesAcrossSide) numSamplesY = "
    "maxNumSamplesAcrossSide;\n"
    "\n"
    "\n"
    ...
    "    float cd = distanceFromEdge(src_texCoord); // central distance (distance from "
    "center to edge)\n"
    "    if (cd-blend_half_width>distance_across_pixel) return vertexColor; // pixel "
    "fully within glyph body\n"
    "\n"
    "    #ifdef OUTLINE\n"
    "    float outline_width = OUTLINE*0.5;\n"
    "    if ((-cd-outline_width-blend_half_width)>distance_across_pixel) return "
    "vec4(0.0, 0.0, 0.0, 0.0); // pixel fully outside outline+glyph body\n"
    "    vec2 color_coeff = vec2(0.0, 0.0);\n"
    "    #else\n"
    "    if (-cd-blend_half_width>distance_across_pixel) return vec4(0.0, 0.0, 0.0, "
    "0.0); // pixel fully outside glyph body\n"
    ...
    "        {\n"
    "        #ifdef OUTLINE\n"
    "            color_coeff += colorCoeff(distanceFromEdge(pos), blend_width, "
    "blend_half_width);\n"
    "        #else\n"
    "            color_coeff += colorCoeff(distanceFromEdge(pos), blend_width, "
    "blend_half_width);\n"
    ...
    "\n"
    "        return vec4(vertexColor.rgb * (vertex_alpha / total_alpha) + "
    ...
    "        {\n"
    "            float local_alpha = TEXTURE(glyphTexture, origin + vec2(i*delta_tc, "
    "j*delta_tc)).ALPHA;\n"
    ...
    "        if (mipmapLevel<1.0)\n"
    "        {\n"
    "            outline_alpha = mix(1.0-background_alpha, outline_alpha, "
    "mipmapLevel/1.0);\n"
    "        }\n"
    "    #endif\n"
    "\n"
    ...
    "\n"
    "    if (outline_alpha==0.0) return vec4(0.0, 0.0, 0.0, 0.0); // outside glyph and "
    "outline\n"
    "\n"
    ...
    "\n"
    "    // lower the alpha_power value the greater the staturation, no need to be so "
    "aggressive with SDF than GREYSCALE\n"
    ...
    "    #endif\n"
    "\n"
    "    // over saturate the alpha values to make sure the font and it's shadow are "
    "clear\n"
