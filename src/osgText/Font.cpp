...
Font::getDefaultFont()
{
    ...
    std::lock_guard<std::mutex> lock( s_DefaultFontMutex );

    osg::ref_ptr<osg::Object>   object =
        osgDB::Registry::instance()->getObjectCache()->getFromObjectCache(
            "DefaultFont"
        );
    ...
    if( !font )
    {
        ...
        osgDB::Registry::instance()->getObjectCache()->addEntryToObjectCache(
            "DefaultFont",
            font.get()
        );
        return font;
...
getFontFileMutex()
{
    static std::recursive_mutex s_FontFileMutex;
...
static bool
getFilePathFromFontconfig( const std::string& fontName,
                           std::string&       fontPath )
{
...
std::string
osgText::findFontFile( const std::string& str )
{
    ...
    std::lock_guard<std::recursive_mutex> lock( getFontFileMutex() );
    ...
    if( !initialized )
    {
        initialized = true;
#if defined( _WIN32 )
        osgDB::convertStringPathIntoFilePathList( ".;C:/winnt/fonts;C:/windows/fonts",
                                                  s_FontFilePath );
        ...
        osgDB::convertStringPathIntoFilePathList(
            ".:/usr/share/fonts/ttf:/usr/share/fonts/ttf/western:/usr/share/fonts/ttf/"
            "decoratives:/Library/Fonts:/System/Library/Fonts",
            s_FontFilePath
        );
#else
        osgDB::convertStringPathIntoFilePathList(
            ".:/usr/share/fonts/ttf:/usr/share/fonts/truetype:/usr/share/fonts/ttf/"
            "western:/usr/share/fonts/ttf/decoratives",
            s_FontFilePath
        );
#endif
    }
...
osgText::Font*
osgText::readFontFile( const std::string&                  filename,
                       const osgDB::ReaderWriter::Options* userOptions )
{
    if( filename.empty() )
    {
        return 0;
    ...
    osg::Object* object =
        osgDB::readObjectFile( foundFile,
    ...
    // otherwise if the object has zero references then delete it by doing another
    // unref().
...
osgText::Font*
osgText::readFontStream( std::istream&                       stream,
                         const osgDB::ReaderWriter::Options* userOptions )
{
    std::lock_guard<std::recursive_mutex>      lock( getFontFileMutex() );
    ...
    osgDB::ReaderWriter* reader =
        osgDB::Registry::instance()->getReaderWriterForExtension( "ttf" );
    if( reader == 0 )
    {
        return 0;
    }
    osgDB::ReaderWriter::ReadResult rr =
        reader->readObject( stream, userOptions ? userOptions : localOptions.get() );
    if( !rr.success() )
    {
...
osg::ref_ptr<Font>
osgText::readRefFontFile( const std::string&                  filename,
                          const osgDB::ReaderWriter::Options* userOptions )
{
    if( filename.empty() )
    {
    ...
    osg::ref_ptr<osg::Object> object =
        osgDB::readRefObjectFile( foundFile,
                                  userOptions ? userOptions : localOptions.get() );
...
Font::Font( FontImplementation* implementation ) :
    osg::Object( true ),
    _textureWidthHint( 1'024 ),
    _textureHeightHint( 1'024 ),
    ...
    _numCurveSamples( 10 )
{
    ...
    {
        unsigned int osg_max_size = static_cast<unsigned int>( atoi( ptr ) );
...
void
Font::setImplementation( FontImplementation* implementation )
{
    if( _implementation.valid() )
    {
        _implementation->_facade = 0;
...
Font::getImplementation()
{
    return _implementation.get();
}

const Font::FontImplementation*
Font::getImplementation() const
{
    return _implementation.get();
}
...
void
Font::setTextureSizeHint( unsigned int width,
                          unsigned int height )
{
...
unsigned int
Font::getTextureWidthHint() const
{
    return _textureWidthHint;
}

unsigned int
Font::getTextureHeightHint() const
{
    return _textureHeightHint;
}

void
Font::setMinFilterHint( osg::Texture::FilterMode mode )
{
    _minFilterHint = mode;
}

osg::Texture::FilterMode
Font::getMinFilterHint() const
{
    return _minFilterHint;
}

/** Set the magnification texture filter to use when creating the texture to store the
 * glyph images when rendering. Note, this doesn't affect already created Texture
 * Glhph's.*/
void
Font::setMagFilterHint( osg::Texture::FilterMode mode )
{
    _magFilterHint = mode;
}

osg::Texture::FilterMode
Font::getMagFilterHint() const
{
    return _magFilterHint;
}

Glyph*
Font::getGlyph( const FontResolution& fontRes,
                unsigned int          charcode )
{
    ...
    std::lock_guard<std::mutex> lock( _glyphMapMutex );
...
Glyph3D*
Font::getGlyph3D( const FontResolution& fontRes,
                  unsigned int          charcode )
{
    if( !_implementation )
    {
...
void
Font::setThreadSafeRefUnref( bool threadSafe )
{
...
void
Font::resizeGLObjectBuffers( unsigned int maxSize )
{
    for( StateSets::iterator itr = _statesets.begin(); itr != _statesets.end(); ++itr )
    {
...
void
Font::releaseGLObjects( osg::State* state ) const
{
    for( StateSets::const_iterator itr = _statesets.begin(); itr != _statesets.end();
         ++itr )
    {
...
osg::vec2
Font::getKerning( const FontResolution& fontRes,
                  ...
                  KerningType           kerningType )
{
    if( _implementation.valid() )
    {
        return _implementation->getKerning( fontRes,
                                            leftcharcode,
                                            rightcharcode,
                                            kerningType );
    }
...
void
Font::addGlyph( const FontResolution& fontRes,
                unsigned int          charcode,
                Glyph*                glyph )
{
    std::lock_guard<std::mutex> lock( _glyphMapMutex );
...
void
Font::assignGlyphToGlyphTexture( Glyph*          glyph,
                                 ShaderTechnique shaderTechnique )
{
    ...
    for( GlyphTextureList::iterator itr = _glyphTextureList.begin();
         ...
         ++itr )
    {
        if( ( *itr )->getShaderTechnique() ==
            shaderTechnique &&
            ( *itr )->getSpaceForGlyph( glyph, posX, posY ) )
        {
    ...
    if( glyphTexture )
    {
        // cout << "    Font::assignGlyphToGlyphTexture() found space for texture
        // "<<glyphTexture<<" posX="<<posX<<" posY="<<posY<<endl;
    }
    ...
    if( !glyphTexture )
    {
        ...
        OSG_INFO << "   Font " << this << ", numberOfTexturesAllocated "
                 << numberOfTexturesAllocated << std::endl;

        // reserve enough space for the glyphs.
        glyphTexture->setShaderTechnique( shaderTechnique );
        glyphTexture->setTextureSize( static_cast<int>( _textureWidthHint ),
                                      static_cast<int>( _textureHeightHint ) );
        ...
        {
            OSG_WARN << "Warning: unable to allocate texture big enough for glyph"
                     << std::endl;
            return;
        }
    }
