...
#ifndef CALLBACK
    /* Win32 calling conventions. (or a least that's what the GLUT example tess.c
     * uses.)*/
    #define CALLBACK
#endif
...
namespace osgUtil
{
    ...
    class OSGUTIL_EXPORT Tessellator : public osg::Referenced
    {
        public:
             ...
             * tessellated or each separate drawable's contours needs to be tessellated.
             */
            ...
            getBoundaryOnly()
            {
                return _boundaryOnly;
            }

            /** Set and get tessellation windong rule */
            void
            setWindingType( const WindingType wt )
            {
                _wtype = wt;
            }
            ...
            void
            setTessellationType( const TessellationType tt )
            {
                _ttype = tt;
            }

            inline TessellationType
            getTessellationType()
            {
                return _ttype;
            }

            /** Change the contours lists of the geometry into tessellated primitives
             * (the list of primitives in the original geometry is stored in the
             * Tessellator for possible re-use. The name remains retessellatePolygons
             * although it now handles trifans, strips, quads etc. as well as Polygons so
             * as to not break old codes relying on this function name. */
            void
            retessellatePolygons( osg::Geometry& cxgeom );

            /** Define the normal to the tessellated polygon - this provides a hint how
             * to tessellate the contours; see gluTessNormal in red book or man pages.
             ...
             */
            void
            setTessellationNormal( const osg::vec3 norm )
            {
                tessNormal = norm;
            }
        ...
        protected:
             ...
             * boundaries, these will leak memory if not removed when retessellating. */
            void
            reduceArray( osg::Array*        cold,
                         const unsigned int nnu );

            void
            collectTessellation( osg::Geometry& cxgeom,
                                 unsigned int   originalIndex );

            typedef std::map<osg::vec3*, unsigned int> VertexPtrToIndexMap;
            void
            addContour( GLenum          mode,
                        unsigned int    first,
                        unsigned int    last,
                        osg::Vec3Array* vertices );
            void
            addContour( osg::PrimitiveSet* primitive,
                        osg::Vec3Array*    vertices );
            void
            handleNewVertices( osg::Geometry&       geom,
                               VertexPtrToIndexMap& vertexPtrToIndexMap );

            void
            begin( GLenum mode );
            void
            vertex( osg::vec3* vertex );
            void
            combine( osg::vec3* vertex,
                     void*      vertex_data[4],
                     GLfloat    weight[4] );
            void
            end();
            ...
            typedef std::vector<dvec3*>    Vec3dList;
            ...
            /** count number of primitives in a geometry to get right no. of norms/colurs
             * etc for per_primitive attributes. */
            unsigned int                    _index;
