...
#include <osgUtil/culling/RenderBin.hpp>
...
namespace osgUtil
{
    ...
    class OSGUTIL_EXPORT RenderStage : public RenderBin
    {
        public:
            ...
            RenderStage( const RenderStage& rhs,
            ...
            {
                return new RenderStage( *this, copyop );
            }    // note only implements a clone of type.

            virtual bool
            isSameKindAs( const osg::Object* obj ) const
            {
                return dynamic_cast<const RenderStage*>( obj ) != 0L;
            }
            ...
            void
            setDrawBuffer( GLenum buffer,
                           bool   applyMask = true )
            {
                _drawBuffer = buffer;
                setDrawBufferApplyMask( applyMask );
            }
            ...
            /** Get the apply mask defining whether glDrawBuffer is called at each frame
             * draw. */
            bool
            getDrawBufferApplyMask() const
            {
                return _drawBufferApplyMask;
            }

            /** Set the apply mask defining whether glDrawBuffer is called at each frame
             * draw. */
            void
            setDrawBufferApplyMask( bool applyMask )
            {
                _drawBufferApplyMask = applyMask;
            }

            /** Set the read buffer for any required copy operations to use. */
            void
            setReadBuffer( GLenum buffer,
                           bool   applyMask = true )
            {
                _readBuffer = buffer;
                setReadBufferApplyMask( applyMask );
            }
            ...
            /** Get the apply mask defining whether glReadBuffer is called at each frame
             * draw. */
            bool
            getReadBufferApplyMask() const
            {
                return _readBufferApplyMask;
            }

            /** Set the apply mask defining whether glReadBuffer is called at each frame
             * draw. */
            void
            setReadBufferApplyMask( bool applyMask )
            {
                _readBufferApplyMask = applyMask;
            }
            ...
            void
            setClearAccum( const osg::vec4& color )
            {
                _clearAccum = color;
            }
            ...
            {
                _cameraRequiresSetUp = flag;
            }

            bool
            getCameraRequiresSetUp() const
            {
                return _cameraRequiresSetUp;
            }

            void
            setCameraAttachmentMapCount( unsigned int v )
            {
                _cameraAttachmentMapModifiedCount = v;
            }

            unsigned int
            getCameraAttachmentMapCount()
            {
                return _cameraAttachmentMapModifiedCount;
            }

            /** Attempt the set the RenderStage from the Camera settings.*/
            void
            runCameraSetUp( osg::RenderInfo& renderInfo );
            ...
            void
            setImageReadPixelFormat( GLenum format )
            {
                _imageReadPixelFormat = format;
            }

            GLenum
            getImageReadPixelFormat() const
            {
                return _imageReadPixelFormat;
            }

            void
            setImageReadPixelDataType( GLenum type )
            {
                _imageReadPixelDataType = type;
            }

            GLenum
            getImageReadPixelDataType() const
            {
                return _imageReadPixelDataType;
            }
            ...
            void
            setFrameBufferObject( osg::FrameBufferObject* fbo )
            {
                _fbo = fbo;
            }
            ...
            void
            setMultisampleResolveFramebufferObject( osg::FrameBufferObject* fbo );

            osg::FrameBufferObject*
            getMultisampleResolveFramebufferObject()
            {
                return _resolveFbo.get();
            }

            const osg::FrameBufferObject*
            getMultisampleResolveFramebufferObject() const
            {
                return _resolveFbo.get();
            }
            ...
            void
            setDisableFboAfterRender( bool disable )
            {
                _disableFboAfterRender = disable;
            }

            bool
            getDisableFboAfterRender() const
            {
                return _disableFboAfterRender;
            }
            ...
            void
            setInheritedPositionalStateContainerMatrix( const osg::dmat4& matrix )
            {
                _inheritedPositionalStateContainerMatrix = matrix;
            }

            const osg::dmat4&
            getInheritedPositionalStateContainerMatrix() const
            {
                return _inheritedPositionalStateContainerMatrix;
            }

            void
            setInheritedPositionalStateContainer( PositionalStateContainer* rsl )
            {
                _inheritedPositionalStateContainer = rsl;
            }

            PositionalStateContainer*
            getInheritedPositionalStateContainer()
            {
                return _inheritedPositionalStateContainer.get();
            }

            void
            setPositionalStateContainer( PositionalStateContainer* rsl )
            {
                _renderStageLighting = rsl;
            }

            PositionalStateContainer*
            getPositionalStateContainer() const
            {
                if( !_renderStageLighting.valid() )
                {
                    _renderStageLighting = new PositionalStateContainer;
                }
                                           ...
                                           const osg::StateAttribute* attr )
            {
                getPositionalStateContainer()
                    ->addPositionedTextureAttribute( textureUnit, matrix, attr );
            }

            void
            copyTexture( osg::RenderInfo& renderInfo );

            virtual void
            sort();

            virtual void
            drawPreRenderStages( osg::RenderInfo& renderInfo,
                                 RenderLeaf*&     previous );
            ...
            virtual void
            drawInner( osg::RenderInfo& renderInfo,
                       RenderLeaf*&     previous,
                       bool&            doCopyTexture );

            virtual void
            drawPostRenderStages( osg::RenderInfo& renderInfo,
                                  RenderLeaf*&     previous );
            ...
            void
            addToDependencyList( RenderStage* rs )
            {
                addPreRenderStage( rs );
            }

            void
            addPreRenderStage( RenderStage* rs,
                               int          order = 0 );

            void
            addPostRenderStage( RenderStage* rs,
                                int          order = 0 );

            const RenderStageList&
            getPreRenderList() const
            {
                return _preRenderList;
            }
            ...
            bool
            getStats( Statistics& stats ) const;

            /** Compute the number of dynamic RenderLeaves.*/
            virtual unsigned int
            computeNumberOfDynamicRenderLeaves() const;
            ...
            void
            attach( osg::Camera::BufferComponent buffer,
                    osg::Image*                  image );
        ...
        protected:
            ...
            mutable osg::dmat4 _inheritedPositionalStateContainerMatrix;
            mutable osg::ref_ptr<PositionalStateContainer>
