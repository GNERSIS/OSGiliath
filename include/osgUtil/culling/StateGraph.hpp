...
namespace osgUtil
{
    ...
    {
            bool
            operator()( const osg::ref_ptr<RenderLeaf>& lhs,
                        const osg::ref_ptr<RenderLeaf>& rhs )
            {
    ...
    class OSGUTIL_EXPORT StateGraph : public osg::Object
    {
        public:
                ...
                _dynamic( false )
            {
                ...
                if( _parent && _parent->_dynamic )
                {
                    _dynamic = true;
                }
                else
                {
                    _dynamic = stateset->getDataVariance() ==
            ...
            {
                return dynamic_cast<const StateGraph*>( obj ) != 0L;
            }
            ...
            void
            setStateSet( const osg::StateSet* stateset )
            {
                _stateset = stateset;
            }
            ...
            getMinimumDistance() const
            {
                if( _minimumDistance == FLT_MAX && !_leaves.empty() )
                {
                    ...
                    {
                        if( ( *itr )->_depth < _minimumDistance )
                        {
                            _minimumDistance = ( *itr )->_depth;
                        }
                    }
            ...
            /** Reset the internal contents of a StateGraph, including deleting all
             * children.*/
            void
            reset();
            ...
            {
                for( ChildList::iterator itr = _children.begin(); itr != _children.end();
                     ++itr )
                {
                    ( itr->second )->resizeGLObjectBuffers( maxSize );
                }

                for( LeafList::iterator itr = _leaves.begin(); itr != _leaves.end();
                     ++itr )
                {
            ...
            inline StateGraph*
            find_or_insert( const osg::StateSet* stateset )
            {
            ...
            static inline void
            moveStateGraph( osg::State& state,
                            StateGraph* sg_curr,
                            StateGraph* sg_new )
            {
                if( sg_new == sg_curr || sg_new == NULL )
                {
                    return;
                }
                ...
                if( sg_curr == NULL )
                {
                    ...
                    return_path.reserve( static_cast<std::size_t>( sg_new->_depth +
                                                                   1 ) );
                    ...
                    for( std::vector<StateGraph*>::reverse_iterator itr =
                             return_path.rbegin();
                         ...
                         ++itr )
                    {
                        ...
                        if( rg->getStateSet() )
                        {
                            state.pushStateSet( rg->getStateSet() );
                        }
                    }
                    return;
                }
                ...
                if( sg_curr->_parent == sg_new->_parent )
                {
                    ...
                    if( sg_curr->getStateSet() )
                    {
                        state.popStateSet();
                    }
                    // and push new state.
                    if( sg_new->getStateSet() )
                    {
                        state.pushStateSet( sg_new->getStateSet() );
                    }
                    return;
                }
                ...
                // use return path to trace back steps to sg_new.
                std::vector<StateGraph*> return_path;
                return_path.reserve( static_cast<std::size_t>( sg_new->_depth + 1 ) );
            ...
            inline static void
            moveToRootStateGraph( osg::State& state,
                                  StateGraph* sg_curr )
            {
