...
namespace osgUtil
{
     ...
     * A renderBin can contain further renderBins producing a tree hierarchy of
     * renderBins.
     */
    class OSGUTIL_EXPORT RenderBin : public osg::Object
    {
        public:
            ...
            // static methods.
            static RenderBin*
            createRenderBin( const std::string& binName );
            static RenderBin*
            getRenderBinPrototype( const std::string& binName );
            static void
            addRenderBinPrototype( const std::string& binName,
                                   RenderBin*         proto );
            static void
            ...
            getStateGraphList()
            {
                return _stateGraphList;
            }

            const StateGraphList&
            getStateGraphList() const
            {
                return _stateGraphList;
            }
            ...
            getRenderLeafList()
            {
                return _renderLeafList;
            }

            const RenderLeafList&
            getRenderLeafList() const
            {
                return _renderLeafList;
            }
            ...
            {
                    virtual void
                    sortImplementation( RenderBin* ) = 0;
            };

            void
            setSortCallback( SortCallback* sortCallback )
            {
            ...
            getSortCallback()
            {
                return _sortCallback.get();
            }

            const SortCallback*
            getSortCallback() const
            {
                return _sortCallback.get();
            }
            ...
            {
                    virtual void
                    drawImplementation( RenderBin*       bin,
                                        osg::RenderInfo& renderInfo,
                                        RenderLeaf*&     previous ) = 0;
            };

            void
            setDrawCallback( DrawCallback* drawCallback )
            {
            ...
            bool
            getStats( Statistics& primStats ) const;
