...
namespace osgUtil
{
    ...
    class Intersector : public osg::Referenced
    {
        public:
            ...
            void
            setCoordinateFrame( CoordinateFrame cf )
            {
                _coordinateFrame = cf;
            }

            CoordinateFrame
            getCoordinateFrame() const
            {
                return _coordinateFrame;
            }

            void
            setIntersectionLimit( IntersectionLimit limit )
            {
                _intersectionLimit = limit;
            }

            IntersectionLimit
            getIntersectionLimit() const
            {
                return _intersectionLimit;
            }

            virtual Intersector*
            clone( osgUtil::IntersectionVisitor& iv ) = 0;

            virtual bool
            enter( const osg::Node& node ) = 0;
            ...
            reachedLimit()
            {
                return _intersectionLimit == LIMIT_ONE && containsIntersections();
            }
            ...
            /** Set the hint to what precision to use in the intersections
             * calculations.*/
            void
            ...
            /** Get the hint to what precision should be used in the intersections
             * calculations.*/
            PrecisionHint
    ...
    class OSGUTIL_EXPORT IntersectorGroup : public Intersector
    {
        public:
            ...
            /** Add an Intersector. */
            void
            addIntersector( Intersector* intersector );
    ...
    /** IntersectionVisitor is used to testing for intersections with the scene,
     * traversing the scene using generic osgUtil::Intersector's to test against the
     * scene. To implement different types of intersection techniques, one implements
     * custom versions of the osgUtil::Intersector, and then pass the constructed
     * intersector to the IntersectionVisitor.*/
    class OSGUTIL_EXPORT IntersectionVisitor : public osg::DualModeVisitor
    {
        public:

            /** Callback used to implement the reading of external files, allowing
             * support for paged databases to be integrated with IntersectionVisitor.  A
             * concrete implementation can be found in osgDB. Note, this loose coupling
             * approach is required as osgUtil is independent from osgDB where the file
             * reading is implemented, and osgDB itself is dependent upon osgUtil so a
             * circular dependency would result from tighter integration.*/
            struct ReadCallback : public osg::Referenced
            {
                    virtual osg::ref_ptr<osg::Node>
                    readNodeFile( const std::string& filename ) = 0;
            };

            IntersectionVisitor( Intersector*  intersector  = 0,
            ...
            /** Set the intersector that will be used to intersect with the scene, and to
             * store any hits that occur.*/
            void
            setIntersector( Intersector* intersector );

            /** Get the intersector that will be used to intersect with the scene, and to
             * store any hits that occur.*/
            Intersector*
            getIntersector()
            {
            ...
            /** Get the const intersector that will be used to intersect with the scene,
             * and to store any hits that occur.*/
            const Intersector*
            getIntersector() const
            {
                return _intersectorStack.empty() ? 0 : _intersectorStack.front().get();
            }

            /** Set whether the intersectors should use KdTrees when they are found on
             * the scene graph.*/
            void
            ...
            bool
            getUseKdTreeWhenAvailable() const
            {
                return _useKdTreesWhenAvailable;
            }

            void
            setDoDummyTraversal( bool dummy )
            {
                _dummyTraversal = dummy;
            }

            bool
            getDoDummyTraversal() const
            {
                return _dummyTraversal;
            }

            /** Set the read callback.*/
            void
            setReadCallback( ReadCallback* rc )
            {
                _readCallback = rc;
            }
            ...
            getReadCallback()
            {
                return _readCallback.get();
            }

            /** Get the const read callback.*/
            const ReadCallback*
            getReadCallback() const
            {
                return _readCallback.get();
            }
            ...
            {
                _windowStack.push_back(
                    new osg::RefMatrix( viewport->computeWindowMatrix() )
                );
                _eyePointDirty = true;
            }
            ...
            getWindowMatrix()
            {
                return _windowStack.empty() ? 0 : _windowStack.back().get();
            }
            ...
            getProjectionMatrix()
            {
                return _projectionStack.empty() ? 0 : _projectionStack.back().get();
            }
            ...
            getViewMatrix()
            {
                return _viewStack.empty() ? 0 : _viewStack.back().get();
            }
            ...
            getModelMatrix()
            {
                return _modelStack.empty() ? 0 : _modelStack.back().get();
            }
            ...
            /** Set the reference eye point that is used for nodes that require an eye
             * point to position themselves, such as billboards.*/
            void
            setReferenceEyePoint( const osg::vec3& ep )
            {
                _referenceEyePoint = ep;
                _eyePointDirty     = true;
            }

            /** Get the reference eye point.*/
            const osg::vec3&
            getReferenceEyePoint() const
            {
                return _referenceEyePoint;
            }

            /** Set the coordinate frame of the reference eye point.*/
            void
            setReferenceEyePointCoordinateFrame( Intersector::CoordinateFrame cf )
            {
                _referenceEyePointCoordinateFrame = cf;
            }

            /** Get the coordinate frame of the reference eye point.*/
            Intersector::CoordinateFrame
            getReferenceEyePointCoordinateFrame() const
            {
                return _referenceEyePointCoordinateFrame;
            }

            /** Get the eye point in the local coordinate frame a given traversal
             * point.*/
            virtual osg::vec3
            getEyePoint() const;
            ...
            void
            setLODSelectionMode( LODSelectionMode mode )
            {
            ...
            LODSelectionMode
            getLODSelectionMode() const
            {
                return _lodSelectionMode;
            }

            /** Get the distance from a point to the eye point, distance value in local
             * coordinate system. This is calculated using the pseudo-EyePoint (above)
             * when doing LOD calculcations. */
            virtual float
        ...
        public:
            ...
            apply( osg::LOD& lod );
            virtual void
            apply( osg::PagedLOD& lod );
            virtual void
            apply( osg::Transform& transform );
            virtual void
            apply( osg::Projection& projection );
            virtual void
            apply( osg::Camera& camera );
        ...
        protected:
            ...
            enter( const osg::Node& node )
            {
                return _intersectorStack.empty()
                         ? false
                         : _intersectorStack.back()->enter( node );
            }

            inline void
            leave()
            {
                _intersectorStack.back()->leave();
            }

            inline void
            intersect( osg::Drawable* drawable )
            {
            ...
            push_clone()
            {
                _intersectorStack.push_back( _intersectorStack.front()->clone( *this ) );
            }

            inline void
            pop_clone()
            {
                if( _intersectorStack.size() >= 2 )
                {
                    _intersectorStack.pop_back();
                }
            }
