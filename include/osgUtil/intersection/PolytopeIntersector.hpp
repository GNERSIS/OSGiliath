...
namespace osgUtil
{
    ...
    class OSGUTIL_EXPORT PolytopeIntersector : public Intersector
    {
        public:
            ...
            /** Construct a PolytopeIntersector using specified polytope in MODEL
             * coordinates.*/
            PolytopeIntersector( const osg::Polytope& polytope );

            /** Construct a PolytopeIntersector using specified polytope in specified
             * coordinate frame.*/
             ...
             * coordinates In WINDOW coordinates (clip space cube) creates a five sided
             * polytope box that has a front face at 0.0 and sides around box xMin, yMin,
             * xMax, yMax. In PROJECTION coordinates (clip space cube) creates a five
             * sided polytope box that has a front face at -1 and sides around box xMin,
             * yMin, xMax, yMax. In VIEW and MODEL coordinates (clip space cube) creates
             * a five sided polytope box that has a front face at 0.0 and sides around
            ...
            struct Intersection
            {
                    ...
                    operator<( const Intersection& rhs ) const
                    {
                        ...
                        if( primitiveIndex < rhs.primitiveIndex )
                        {
                            return true;
                        }
                        if( rhs.primitiveIndex < primitiveIndex )
                        {
                            return false;
                        }
                    ...
                    enum
                    {
                        MaxNumIntesectionPoints = 6,
                    };

                    double distance;       ///< distance from reference plane
                    double maxDistance;    ///< maximum distance of intersection points
                                           ///< from reference plane
                    osg::NodePath                nodePath;
                    osg::ref_ptr<osg::Drawable>  drawable;
                    osg::ref_ptr<osg::RefMatrix> matrix;
                    Vec3_type
                        localIntersectionPoint;    ///< center of all intersection points
            ...
            void
            setPrimitiveMask( unsigned int mask )
            {
                _primitiveMask = mask;
            }
            ...
            unsigned int
            getPrimitiveMask() const
            {
                return _primitiveMask;
            }
             ...
             */
            inline void
            setReferencePlane( const osg::Plane& plane )
            {
            ...
            inline void
            setDimensionMask( unsigned int mask )
            {
