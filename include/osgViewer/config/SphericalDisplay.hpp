 ...
 * Viewer configuration for spherical display projection.
 * Projects onto a curved screen with distortion correction.
 */
#pragma once
...
namespace osgViewer
{
    ...
    /** spherical display using 6 slave cameras rendering the 6 sides of a cube map, and
     * 7th camera doing distortion correction to present on a spherical display.*/
    class OSGVIEWER_EXPORT SphericalDisplay
        : public osg::Inherit<ViewConfig, SphericalDisplay>
    {
        public:

            SphericalDisplay( double            radius          = 1.0,
                              double            collar          = 0.45,
                              unsigned int      screenNum       = 0,
                              osg::Image*       intensityMap    = 0,
                              const osg::dmat4& projectorMatrix = osg::dmat4() ) :
                _radius( radius ),
                _collar( collar ),
            ...
            SphericalDisplay( const SphericalDisplay& rhs,
            ...
            void
            setIntensityMap( osg::Image* im )
            {
                _intensityMap = im;
            }

            const osg::Image*
            getIntensityMap() const
            {
                return _intensityMap.get();
            }

            void
            setProjectionMatrix( const osg::dmat4& m )
            {
                _projectorMatrix = m;
            }
...
class OSGVIEWER_EXPORT ViewForPanoramicSphericalDisplay : public Config
{
    public:
        ...
        ViewForPanoramicSphericalDisplay(double radius=1.0, double collar=0.45, unsigned int screenNum=0, osg::Image* intensityMap=0, const osg::dmat4& projectorMatrix = osg::dmat4());
        ...
        OSG_REGISTER_TYPE(osgViewer, ViewOnSingleScreen)
...
class OSGVIEWER_EXPORT ViewForWoWVxDisplay : public osg::Inherit<Config, ViewForWoWVxDisplay>
{
    public:
        ...
        OSG_REGISTER_TYPE(osgViewer, ViewForWoWVxDisplay)
...
class OSGVIEWER_EXPORT DepthPartition : public osg::Inherit<Config, DepthPartition>
{
    public:
        ...
        OSG_REGISTER_TYPE(osgViewer, DepthPartition)
