 ...
 * Morph target (blend shape) geometry. Interpolates between
 * base and target shapes using per-target weights.
 */
#pragma once
...
namespace osgAnimation
{
    ...
    class OSGANIMATION_EXPORT MorphGeometry : public osg::Geometry
    {
        ...
        public:
            ...
            MorphGeometry( const MorphGeometry& b,
                           const osg::CopyOp&   copyop = osg::CopyOp::SHALLOW_COPY );

            virtual osg::Object*
            cloneType() const
            {
                return new MorphGeometry();
            }

            virtual osg::Object*
            clone( const osg::CopyOp& copyop ) const
            {
                return new MorphGeometry( *this, copyop );
            }

            virtual bool
            isSameKindAs( const osg::Object* obj ) const
            {
                return dynamic_cast<const MorphGeometry*>( obj ) != NULL;
            }
            ...
            inline void
            setMorphTransformImplementation( MorphTransform* mt )
            {
                _morphTransformImplementation = mt;
            }

            inline MorphTransform*
            getMorphTransformImplementation()
            {
                return _morphTransformImplementation.get();
            }

            inline const MorphTransform*
            getMorphTransformImplementation() const
            {
                return _morphTransformImplementation.get();
            }
            ...
            inline void
            setMorphNormals( bool morphNormals )
            {
            ...
            inline bool
            getMorphNormals() const
            {
                return _morphNormals;
            }

            /** Get the list of MorphTargets.*/
            inline const MorphTargetList&
            getMorphTargetList() const
            {
                return _morphTargets;
            }

            /** Get the list of MorphTargets. Warning if you modify this array you will
             * have to call dirty() */
            inline MorphTargetList&
            getMorphTargetList()
            {
                return _morphTargets;
            }
            ...
            inline const MorphTarget&
            getMorphTarget( unsigned int i ) const
            {
                return _morphTargets[i];
            }

            /** Return the \c MorphTarget at position \c i.*/
            inline MorphTarget&
            getMorphTarget( unsigned int i )
            {
                return _morphTargets[i];
            }
            ...
            inline void
            setVertexSource( osg::Vec3Array* v )
            {
                _positionSource = v;
            }
            ...
            inline void
            setNormalSource( osg::Vec3Array* n )
            {
                _normalSource = n;
            }
            ...
            inline osg::Vec3Array*
            getNormalSource() const
            {
                return _normalSource.get();
            }

            /** Add a \c MorphTarget to the \c MorphGeometry.
             * If \c MorphTarget is not \c NULL and is not contained in the \c
             * MorphGeometry then increment its reference count, add it to the
             * MorphTargets list and dirty the bounding sphere to force it to be
             * recomputed on the next call to \c getBound().
             ...
             */
            virtual void
            addMorphTarget( osg::Geometry* morphTarget,
                            float          weight = 1.0 )
            {
            ...
            virtual void
            removeMorphTarget( osg::Geometry* morphTarget )
            {
                for( MorphTargetList::iterator iterator = _morphTargets.begin();
                     iterator != _morphTargets.end();
                     ++iterator )
                {
            ...
            virtual void
            removeMorphTarget( const std::string& name )
            {
                for( MorphTargetList::iterator iterator = _morphTargets.begin();
                     iterator != _morphTargets.end();
                     ++iterator )
                {
                    if( iterator->getGeometry() &&
                        iterator->getGeometry()->getName() == name )
                    {
                        _morphTargets.erase( iterator );
                        break;
                    }
            ...
            {
                ( *_morphTransformImplementation.get() )( *this );
            }
    ...
    class OSGANIMATION_EXPORT UpdateMorph
        : public osg::Inherit<AnimationUpdateCallback<osg::NodeCallback>, UpdateMorph>
    {
        public:
            ...
            UpdateMorph( const UpdateMorph& apc,
                         const osg::CopyOp& copyop );

            void
            addTarget( const std::string& name )
            {
                _targetNames.push_back( name );
            }

            unsigned int
            getNumTarget() const
            {
                return static_cast<unsigned int>( _targetNames.size() );
            }

            const std::string&
            getTargetName( unsigned int index )
            {
            ...
            {
                TargetNames::iterator found =
                    std::find( _targetNames.begin(), _targetNames.end(), name );
            ...
            void
            setTargetNames( const TargetNames& targetNames )
            {
    ...
    struct UpdateMorphGeometry : public osg::DrawableUpdateCallback
    {
            ...
            UpdateMorphGeometry( const UpdateMorphGeometry& org,
                                 const osg::CopyOp&         copyop ) :
                osg::Object( org,
                             copyop ),
                osg::Callback( org,
                    ...
                    osg::Drawable* drw )
            {
                ...
                MorphTransform& implementation =
                    *geom->getMorphTransformImplementation();
                ( implementation )( *geom );
            }
    };
