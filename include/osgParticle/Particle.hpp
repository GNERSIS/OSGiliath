 ...
 * Single particle with lifecycle state: position, velocity, age,
 * color range, size range, and texture tile index.
 */
...
#include <osgParticle/Interpolator.hpp>
    ...
    */
    class OSGPARTICLE_EXPORT Particle
    {
        ...
        public:
            ...
            /**
             Shape of particles.
             NOTE: the LINE shape should be used in conjunction with FIXED alignment mode
             (see ParticleSystem).
            */
            enum Shape
            {
                ...
                LINE,    // uses GL_LINES to draw line segments that point to the
                         // direction of motion
            ...
            */
            ...
            /** Get the mass of the particle.
                For built-in operators to work correctly, remember that the mass is
               expressed in kg.
            */
            inline float
            getMass() const;
            ...
            /**    Get the velocity vector.
                For built-in operators to work correctly, remember that velocity
               components are expressed in meters per second.
            */
            inline const osg::vec3&
            getVelocity() const;
            ...
            /** Kill the particle on next update
                NOTE: after calling this function, the <CODE>isAlive()</CODE> method will
               still return true until the particle is updated again.
            */
            inline void
            kill();
            ...
            inline void
            setSizeRange( const rangef& r );

            /// Set the minimum and maximum values for alpha.
            inline void
            setAlphaRange( const rangef& r );

            /// Set the minimum and maximum values for color.
            inline void
            setColorRange( const rangev4& r );

            /// Set the interpolator for computing size values.
            inline void
            setSizeInterpolator( Interpolator* ri );

            template<class T>
            void
            setSizeInterpolator( const osg::ref_ptr<T>& ri )
            {
                setSizeInterpolator( ri.get() );
            }

            /// Set the interpolator for computing alpha values.
            inline void
            setAlphaInterpolator( Interpolator* ai );

            template<class T>
            void
            setAlphaInterpolator( const osg::ref_ptr<T>& ri )
            {
                setAlphaInterpolator( ri.get() );
            }

            /// Set the interpolator for computing color values.
            inline void
            setColorInterpolator( Interpolator* ci );

            template<class T>
            void
            setColorInterpolator( const osg::ref_ptr<T>& ri )
            {
                setColorInterpolator( ri.get() );
            }
            ...
            */
            inline void
            setVelocity( const osg::vec3& v );

            /// Add a vector to the velocity vector.
            inline void
            addVelocity( const osg::vec3& dv );

            /// Transform position and velocity vectors by a matrix.
            inline void
            transformPositionVelocity( const osg::dmat4& xform );

            /// Transform position and velocity vectors by a combination of two matrices
            void
            transformPositionVelocity( const osg::dmat4& xform1,
                                       const osg::dmat4& xform2,
                                       float             r );

            /// Set the angle vector.
            inline void
            setAngle( const osg::vec3& a );

            /**
              Set the angular velocity vector.
              Components x, y and z are angles of rotation around the respective axis (in
              radians).
            */
            inline void
            setAngularVelocity( const osg::vec3& v );

            /// Add a vector to the angular velocity vector.
            inline void
            addAngularVelocity( const osg::vec3& dv );

            /// Transform angle and angularVelocity vectors by a matrix.
            inline void
            transformAngleVelocity( const osg::dmat4& xform );

            /** Update the particle (don't call this method manually).
                This method is called automatically by
               <CODE>ParticleSystem::update()</CODE>; it updates the graphical properties
               of the particle for the current time, checks whether the particle is still
               alive, and then updates its position by computing <I>P = P + V * dt</I>
               (where <I>P</I> is the position and <I>V</I> is the velocity).
            */
            bool
            update( double dt,
                    bool   onlyTimeStamp );

            /// Get the current (interpolated) polygon size. Valid only after the first
            /// call to update().
            inline float
            ...
            /// lifetime of the particle. When no range is given, all tiles are displayed
            /// during the lifetime.
            inline void
            setTextureTileRange( int sTile,
                                 int tTile,
                                 int startTile,
                                 int endTile );
            ...
            {
                return _depth < P._depth;
            }

            /// Method for initializing a particles texture coords as part of a connected
            /// particle system.
            void
            setUpTexCoordsAsPartOfConnectedParticleSystem( ParticleSystem* ps );
    ...
    inline Particle::Shape
    Particle::getShape() const
    {
        return _shape;
    }

    inline void
    Particle::setShape( Shape s )
    {
        _shape = s;
    }
    ...
    inline double
    Particle::getLifeTime() const
    {
        return _lifeTime;
    }

    inline double
    Particle::getAge() const
    {
        return _t0;
    }

    inline float
    Particle::getRadius() const
    {
        return _radius;
    }

    inline void
    Particle::setRadius( float r )
    {
        _radius = r;
    }

    inline const rangef&
    Particle::getSizeRange() const
    {
        return _sr;
    }

    inline const rangef&
    Particle::getAlphaRange() const
    {
        return _ar;
    }

    inline const rangev4&
    Particle::getColorRange() const
    {
        return _cr;
    }

    inline const Interpolator*
    Particle::getSizeInterpolator() const
    {
        return _si.get();
    }

    inline const Interpolator*
    Particle::getAlphaInterpolator() const
    {
        return _ai.get();
    }

    inline const Interpolator*
    Particle::getColorInterpolator() const
    {
        return _ci.get();
    }
    ...
    inline const osg::vec3&
    Particle::getPreviousPosition() const
    {
        return _prev_pos;
    }

    inline const osg::vec3&
    Particle::getAngle() const
    {
        return _angle;
    }

    inline const osg::vec3&
    Particle::getAngularVelocity() const
    {
        return _angul_arvel;
    }

    inline const osg::vec3&
    Particle::getPreviousAngle() const
    {
        return _prev_angle;
    }

    inline int
    Particle::getTileS() const
    {
        return ( _s_tile > 0.0F ) ? static_cast<int>( 1.0F / _s_tile ) : 1;
    }

    inline int
    Particle::getTileT() const
    {
    ...
    inline void
    Particle::setLifeTime( double t )
    {
        _lifeTime = t;
    }

    inline void
    Particle::setSizeRange( const rangef& r )
    {
        _sr = r;
    }

    inline void
    Particle::setAlphaRange( const rangef& r )
    {
        _ar = r;
    }

    inline void
    Particle::setColorRange( const rangev4& r )
    {
        _cr = r;
    }

    inline void
    Particle::setSizeInterpolator( Interpolator* ri )
    {
        _si = ri;
    }

    inline void
    Particle::setAlphaInterpolator( Interpolator* ai )
    {
        _ai = ai;
    }

    inline void
    Particle::setColorInterpolator( Interpolator* ci )
    {
        _ci = ci;
    }

    inline void
    Particle::setPosition( const osg::vec3& p )
    {
        _position = p;
    }

    inline void
    Particle::setVelocity( const osg::vec3& v )
    {
        _velocity = v;
    }

    inline void
    Particle::addVelocity( const osg::vec3& dv )
    {
        _velocity += dv;
    }

    inline void
    Particle::transformPositionVelocity( const osg::dmat4& xform )
    {
        _position = osg::vec3( xform * _position );
        _velocity = osg::transform3x3( _velocity, xform );
    }

    inline void
    Particle::transformPositionVelocity( const osg::dmat4& xform1,
                                         const osg::dmat4& xform2,
                                         float             r )
    {
        osg::vec3 position1   = osg::vec3( xform1 * _position );
        osg::vec3 velocity1   = osg::transform3x3( _velocity, xform1 );
        osg::vec3 position2   = osg::vec3( xform2 * _position );
        osg::vec3 velocity2   = osg::transform3x3( _velocity, xform2 );
    ...
    inline void
    Particle::setAngle( const osg::vec3& a )
    {
        _angle = a;
    }

    inline void
    Particle::setAngularVelocity( const osg::vec3& v )
    {
        _angul_arvel = v;
    }

    inline void
    Particle::addAngularVelocity( const osg::vec3& dv )
    {
        _angul_arvel += dv;
    }

    inline void
    Particle::transformAngleVelocity( const osg::dmat4& xform )
    {
        // this should be optimized!

        osg::vec3 a1 = _angle + _angul_arvel;

        _angle       = osg::vec3( xform * _angle );
        a1           = osg::vec3( xform * a1 );
    ...
    inline float
    Particle::getCurrentSize() const
    {
        return _current_size;
    }

    inline void
    Particle::setTextureTile( int sTile,
                              int tTile,
                              int end )
    {
    ...
    inline void
    Particle::setTextureTileRange( int sTile,
                                   int tTile,
                                   int startTile,
                                   int endTile )
    {
    ...
    inline int
    Particle::getStartTile() const
    {
        return _start_tile;
    }

    inline int
    Particle::getEndTile() const
    {
        return _end_tile;
    }
