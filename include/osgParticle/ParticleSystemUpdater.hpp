...
namespace osgParticle
{
    ...
    */
    class OSGPARTICLE_EXPORT ParticleSystemUpdater
        : public osg::Inherit<osg::Node, ParticleSystemUpdater>
    {
        public:

            ParticleSystemUpdater();
            ParticleSystemUpdater( const ParticleSystemUpdater& copy,
            ...
            virtual bool
            addParticleSystem( ParticleSystem* ps );
        ...
        protected:
            ...
            ParticleSystemUpdater&
            operator=( const ParticleSystemUpdater& )
            {
                return *this;
            }
    ...
    inline unsigned int
    ParticleSystemUpdater::getNumParticleSystems() const
    {
        return static_cast<unsigned int>( _psv.size() );
    }

    inline ParticleSystem*
    ParticleSystemUpdater::getParticleSystem( unsigned int i )
    {
        return _psv[i].get();
    }

    inline const ParticleSystem*
    ParticleSystemUpdater::getParticleSystem( unsigned int i ) const
    {
        return _psv[i].get();
    }

    inline bool
    ParticleSystemUpdater::containsParticleSystem( const ParticleSystem* ps ) const
    {
        for( ParticleSystem_Vector::const_iterator itr = _psv.begin(); itr != _psv.end();
             ++itr )
        {
            if( itr->get() == ps )
            {
    ...
    inline unsigned int
    ParticleSystemUpdater::getParticleSystemIndex( const ParticleSystem* ps ) const
    {
        for( unsigned int particleSystemNum = 0; particleSystemNum < _psv.size();
             ++particleSystemNum )
        {
            if( _psv[particleSystemNum] == ps )
            {
