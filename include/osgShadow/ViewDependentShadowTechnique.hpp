...
#include <map>
#include <osg/core/Identifier.hpp>
...
#define META_ViewDependentShadowTechniqueData( ShadowTechnique, TechniqueData ) \
    virtual ViewDependentShadowTechnique::ViewData* initViewDependentData(      \
        ...
        ViewDependentShadowTechnique::ViewData* vd                              \
    )                                                                           \
    {                                                                           \
        TechniqueData* td = dynamic_cast<TechniqueData*>( vd );                 \
        if( !td )                                                               \
            td = new TechniqueData;                                             \
        td->init( this, cv );                                                   \
        return td;                                                              \
    }
    ...
    */
    class OSGSHADOW_EXPORT ViewDependentShadowTechnique
        : public osg::Inherit<osgShadow::ShadowTechnique, ViewDependentShadowTechnique>
    {
        public:
            ...
            */
            ...
            ViewDependentShadowTechnique( const ViewDependentShadowTechnique& vdst,
        ...
        protected:
            ...
            similar struct and derive it from ViewData to contain their specufic
            resources.
            */
            struct OSGSHADOW_EXPORT ViewData : public osg::Referenced
            {
                    ...
                    */
                    virtual void
                    init( ViewDependentShadowTechnique* st,
                          osgUtil::CullVisitor*         cv );
                    ...
                    */
                    ...
                    update of resources after some of them were modified in parent
                    technique
                    */
                    virtual void
                    dirty( bool flag );
                    ...
                    */
                    ...
                    /**
                    Mutex used to guard _dirty flag from override in case when parent
                    technique calls dirty() simultaneously with ViewData while it is
                    updating resources inside init method.
                    */
                    std::mutex                                      _mutex;
                    ...
                    */
                     ...
                     * OpenGL objects for the specified graphics context. Otherwise,
                     * releases OpenGL objects for all graphics contexts. */
                    virtual void
                    releaseGLObjects( osg::State* = 0 ) const
                    {
                    }
            ...
            ViewDependentShadowTechnique uses this map to find VieData for each cull
            vitior
            */

            typedef std::map<osg::ref_ptr<osg::Identifier>, osg::ref_ptr<ViewData>>
                        ViewDataMap;
