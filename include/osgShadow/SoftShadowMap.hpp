...
namespace osgShadow
{
    ...
    class OSGSHADOW_EXPORT SoftShadowMap : public osg::Inherit<ShadowMap, SoftShadowMap>
    {
        public:

            SoftShadowMap();

            SoftShadowMap( const SoftShadowMap& es,
             ...
             *  Zero is for hard shadow (no penumbra). 0.01 is already very soft
             * penumbra. Default is 0.005.*/
            void
            setSoftnessWidth( float softnessWidth );

            /** Get the value used for width of the soft penumbra in the shader.*/
            float
            getSoftnessWidth() const
            {
             ...
             *  Default is 32. */
            void
            setJitteringScale( float jitteringScale );

            /** Get the value used for jittering scale in the shader.*/
            float
            getJitteringScale() const
            {
            ...
            void
            setJitterTextureUnit( unsigned int jitterTextureUnit );

            /** Get the texture unit that the jitter texture will be applied on.*/
            unsigned int
            getJitterTextureUnit() const
            {
