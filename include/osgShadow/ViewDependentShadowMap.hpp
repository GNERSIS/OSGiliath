...
namespace osgShadow
{
    ...
    /** ViewDependentShadowMap provides an base implementation of view dependent shadow
     * mapping techniques.*/
    class OSGSHADOW_EXPORT ViewDependentShadowMap
        : public osg::Inherit<ShadowTechnique, ViewDependentShadowMap>
    {
        public:

            ViewDependentShadowMap();

            ViewDependentShadowMap( const ViewDependentShadowMap& vdsm,
            ...
            struct OSGSHADOW_EXPORT Frustum
            {
                    ...
                    osg::dvec3                        eye;
                    osg::dvec3                        centerNearPlane;
                    osg::dvec3                        centerFarPlane;
                    osg::dvec3                        center;
                    osg::dvec3                        frustumCenterLine;
            };
            ...
            class OSGSHADOW_EXPORT ViewDependentData : public osg::Referenced
            {
                public:
                    ...
                    const ViewDependentShadowMap*
                    getViewDependentShadowMap() const
                    {
                        return _viewDependentShadowMap;
                    }

                    LightDataList&
                    getLightDataList()
                    {
                        return _lightDataList;
                    }

                    ShadowDataList&
                    getShadowDataList()
                    {
                        return _shadowDataList;
                    }
            ...
            virtual ViewDependentData*
            createViewDependentData( osgUtil::CullVisitor* cv );

            ViewDependentData*
            getViewDependentData( osgUtil::CullVisitor* cv );

            virtual void
            createShaders();

            virtual bool
            selectActiveLights( osgUtil::CullVisitor* cv,
                                ViewDependentData*    vdd ) const;

            virtual osg::Polytope
            computeLightViewFrustumPolytope( Frustum&   frustum,
                                             LightData& positionedLight );

            virtual bool
            computeShadowCameraSettings( Frustum&    frustum,
                                         LightData&  positionedLight,
                                         osg::dmat4& projectionMatrix,
                                         osg::dmat4& viewMatrix );

            virtual bool
            adjustPerspectiveShadowMapCameraSettings( osgUtil::RenderStage* renderStage,
                                                      Frustum&              frustum,
                                                      LightData&   positionedLight,
                                                      osg::Camera* camera );

            virtual void
            cullShadowReceivingScene( osgUtil::CullVisitor* cv ) const;

            virtual void
            cullShadowCastingScene( osgUtil::CullVisitor* cv,
                                    osg::Camera*          camera ) const;

            virtual osg::StateSet*
            selectStateSetForRenderingShadow( ViewDependentData& vdd ) const;

        protected:

            virtual ~ViewDependentShadowMap();

            typedef std::map<osgUtil::CullVisitor*, osg::ref_ptr<ViewDependentData>>
