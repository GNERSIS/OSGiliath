 ...
 * Cel-shading (toon) effect. Quantizes lighting into discrete
 * bands with ink-style outlines for cartoon rendering.
 */
...
namespace osgFX
{
    ...
    */
    class OSGFX_EXPORT Cartoon : public osg::Inherit<Effect, Cartoon>
    {
        public:

            Cartoon();
            Cartoon( const Cartoon&     copy,
            ...
            META_Effect(
                ...
                "cartoon-style (non photorealistic) rendering. Two passes are required: "
                "the first one draws solid surfaces, the second one draws the outlines. "
                "A vertex program is used to setup texture coordinates for a sharp "
                "lighting "
                "texture on unit 0 which is generated on-the-fly.\n"
                "This effect requires the ARB_vertex_program extension "
    ...
    inline int
    Cartoon::getLightNumber() const
    {
        return _lightnum;
    }

    inline void
    Cartoon::setLightNumber( int n )
    {
        _lightnum = n;
        dirtyTechniques();
    }
