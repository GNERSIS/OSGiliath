 ...
 * Abstract base for visual effects. Manages technique selection
 * and applies multi-pass rendering to child geometry.
 */
...
#define META_Effect( library, classname, effectname, effectdescription, effectauthor ) \
    OSG_REGISTER_TYPE( library, classname )                                            \
...
namespace osgFX
{
    ...
    class OSGFX_EXPORT Effect : public osg::Group
    {
        public:
            ...
            // Abstract class: no cloneType/clone. Concrete subclasses use
            // Inherit<Effect,Sub>.
            const char*
            ...
            {
                return dynamic_cast<const Effect*>( obj ) != NULL;
            }

            bool
            is_compatible( const std::type_info& type ) const noexcept override
            {
                return typeid( Effect ) == type || osg::Group::is_compatible( type );
            }
            ...
            virtual const char*
            effectDescription() const = 0;

            /** get the effect author's name */
            virtual const char*
            effectAuthor() const = 0;
            ...
            */
            ...
            inline const Technique*
            getTechnique( int i ) const;

            /** get the index of the currently selected Technique */
            inline int
            getSelectedTechnique() const;
            ...
            inline void
            selectTechnique( int i = AUTO_DETECT );

            /** custom traversal */
            void
            traverse( osg::NodeVisitor& nv ) override;

            /** default traversal */
            inline void
            inherited_traverse( osg::NodeVisitor& nv );
    ...
    inline bool
    Effect::getEnabled() const
    {
        return _enabled;
    }

    inline void
    Effect::setEnabled( bool v )
    {
        _enabled = v;
    }

    inline int
    Effect::getNumTechniques() const
    {
        return static_cast<int>( _techs.size() );
    }

    inline Technique*
    Effect::getTechnique( int i )
    {
        return _techs[static_cast<std::size_t>( i )].get();
    }

    inline const Technique*
    Effect::getTechnique( int i ) const
    {
        return _techs[static_cast<std::size_t>( i )].get();
    }

    inline int
    Effect::getSelectedTechnique() const
    {
        return _global_sel_tech;
    }

    inline void
    Effect::selectTechnique( int i )
    {
        _global_sel_tech = i;
    }

    inline void
    Effect::addTechnique( Technique* tech )
    {
        _techs.push_back( tech );
    }

    inline void
    Effect::dirtyTechniques()
    {
        _techs_defined = false;
    }

    inline void
    Effect::inherited_traverse( osg::NodeVisitor& nv )
    {
