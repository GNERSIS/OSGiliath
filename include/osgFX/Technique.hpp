 ...
 * Single rendering technique for an Effect. Defines one or more
 * rendering passes with associated state sets.
 */
...
#define META_Technique( name, description )           \
    inline virtual const char* techniqueName()        \
    {                                                 \
        return name;                                  \
    }                                                 \
    inline virtual const char* techniqueDescription() \
    {                                                 \
        return description;                           \
    }
...
namespace osgFX
{
    ...
    class OSGFX_EXPORT Technique : public osg::Referenced
    {
        public:
            ...
            {
                return "This is the default technique";
            }
        ...
        protected:
             ...
             */
            void
            traverse_implementation( osg::NodeVisitor& nv,
                                     Effect*           fx );
        ...
        private:
            ...
            std::mutex                                       _mutex;
            std::atomic<unsigned>                            _passesDefined;
            Pass_list                                        _passes;
    };
    ...
    inline osg::StateSet*
    Technique::getPassStateSet( int i )
    {
        return _passesDefined != 0 ? _passes[static_cast<std::size_t>( i )].get() : 0;
    }

    inline const osg::StateSet*
    Technique::getPassStateSet( int i ) const
    {
        return _passesDefined != 0 ? _passes[static_cast<std::size_t>( i )].get() : 0;
    }

    inline void
    Technique::dirtyPasses()
    {
    ...
    inline void
    Technique::traverse( osg::NodeVisitor& nv,
                         Effect*           fx )
    {
