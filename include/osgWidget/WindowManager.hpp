 ...
 * Top-level manager for osgWidget Windows. Handles focus,
 * event dispatch, pick testing, and Z-order management.
 */
...
#include <osgWidget/ScriptEngine.hpp>
#include <osgWidget/StyleManager.hpp>
...
namespace osgWidget
{
    ...
    // osgWidget/ViewerEventHandlers to handle all of these events. In fact, I need to
    // create an SDL example showing this.
    ...
    class OSGWIDGET_EXPORT WindowManager
        : public osg::Inherit<osg::Switch, WindowManager>,
          public UIObjectParent<Window>
    {
        public:

            enum WmFlags
            {
                WM_USE_LUA        = 0X00'00'00'01,
                WM_USE_PYTHON     = 0X00'00'00'02,
                WM_USE_RENDERBINS = 0X00'00'00'04,
                WM_PICK_DEBUG     = 0X00'00'00'08
            };

            enum PointerDirection
            {
                PD_NONE  = 0X00'00'00'00,
                PD_LEFT  = 0X00'00'00'01,
                PD_RIGHT = 0X00'00'00'02,
                PD_UP    = 0X00'00'00'04,
                PD_DOWN  = 0X00'00'00'08
            };

            enum PointerFocusMode
            {
                PFM_FOCUS   = 0X00'00'00'00,
                PFM_UNFOCUS = 0X00'00'00'01,
                PFM_SLOPPY  = 0X00'00'00'02
            };
        ...
        public:
            ...
            WindowManager( osgViewer::View* = 0,
                           point_type       = 0.0F,
                           point_type       = 0.0F,
                           unsigned int     = 0,
                           unsigned int     = 0 );
            ...
            // A static method that will set both the _widget and _window data of an
            // Event reference from a passed-in Interface.
            static void
            setEventFromInterface( Event&,
                                   EventInterface* );
            ...
            bool
            isUsingRenderBins() const
            {
                return ( _flags & WM_USE_RENDERBINS ) != 0;
            }

            int
            getMouseKeysDown() const
            {
                int flag  = 0;
            ...
            PointerDirection
            getPointerVerticalDirection() const
            {
                return _lastVertical;
            }

            PointerDirection
            getPointerHorizontalDirection() const
            {
                return _lastHorizontal;
            }

            PointerFocusMode
            getPointerFocusMode() const
            {
                return _focusMode;
            }

            int
            getPointerDirectionVector() const
            {
                return _lastVertical | _lastHorizontal;
            }

            bool
            isPointerMovingUp() const
            {
                return _lastVertical == PD_UP;
            }

            bool
            isPointerMovingDown() const
            {
                return _lastVertical == PD_DOWN;
            }

            bool
            isPointerMovingLeft() const
            {
                return _lastHorizontal == PD_LEFT;
            }

            bool
            isPointerMovingRight() const
            {
                return _lastHorizontal == PD_RIGHT;
            }

            bool
            isPointerMovingVertically() const
            {
                return _lastVertical != PD_NONE;
            }

            bool
            isPointerMovingHorizontally() const
            {
                return _lastHorizontal != PD_NONE;
            }

            bool
            isLeftMouseButtonDown() const
            {
                return _leftDown;
            }

            bool
            isMiddleMouseButtonDown() const
            {
                return _middleDown;
            }

            bool
            isRightMouseButtonDown() const
            {
                return _rightDown;
            }

            bool
            isMouseScrollingUp() const
            {
                return _scrolling == osgGA::GUIEventAdapter::SCROLL_UP;
            }

            bool
            isMouseScrollingDown() const
            {
                return _scrolling == osgGA::GUIEventAdapter::SCROLL_DOWN;
            }

            bool
            setFocusedByName( const std::string& name )
            {
                return setFocused( getByName( name ) );
            }

            void
            setScrollingMotion( osgGA::GUIEventAdapter::ScrollingMotion sm )
            {
                _scrolling = sm;
            }

            void
            setPointerFocusMode( PointerFocusMode pfm )
            {
                _focusMode = pfm;
            }
        ...
        private:
            ...
            // A functor used to sort the Windows by their Z component in descending
            // order.
            struct WindowZCompare
            {
                    bool
                    operator()( const ptr_type& x,
                                const ptr_type& y )
                    {
            ...
            // A functor used to sort the Windows by their BinNum component in descending
            // order.
            struct WindowBinNumberCompare
            {
                    ...
                    operator()( const ptr_type& x,
                                const ptr_type& y )
                    {
                        return x.get()->getOrCreateStateSet()->getBinNumber() >
            ...
            void
            childInserted( unsigned int );
            void
            childRemoved( unsigned int,
                          unsigned int );

            bool
            _handleMousePushed( float,
                                float,
                                bool& );
            bool
            _handleMouseReleased( float,
                                  float,
                                  bool& );
            bool
            _handleMouseScrolled( float,
                                  float,
                                  bool = false );
            void
            _getPointerXYDiff( float&,
                               float& );
            void
            _updatePickWindow( const WidgetList*,
                               point_type,
                               point_type );
    };

    // We use a template here because the container could be a list or a vector; or
    // something else that supports iteration!
    template<typename T>
    EventInterface*
    WindowManager::getFirstEventInterface( T&     container,
                                           Event& ev )
    {
        if( !container.size() )
        {
