 ...
 * Event dispatch interface for widgets. Defines callbacks
 * for mouse, keyboard, focus, and widget lifecycle events.
 */
...
namespace osgWidget
{
    ...
    enum EventType
    {
        EVENT_NONE          = 0X00'00,
        EVENT_FOCUS         = 0X00'01,
        EVENT_UNFOCUS       = 0X00'02,
        EVENT_MOUSE_ENTER   = 0X00'04,
        EVENT_MOUSE_OVER    = 0X00'08,
        EVENT_MOUSE_LEAVE   = 0X00'10,
        EVENT_MOUSE_DRAG    = 0X00'20,
        EVENT_MOUSE_PUSH    = 0X00'40,
        EVENT_MOUSE_RELEASE = 0X00'80,
        EVENT_MOUSE_SCROLL  = 0X01'00,
        EVENT_KEY_DOWN      = 0X02'00,
        EVENT_KEY_UP        = 0X04'00,
        EVENT_ALL           = 0XFF'FF
    };
    ...
    enum EventMask
    {
        ...
        EVENT_MASK_MOUSE_DRAG  = EVENT_MASK_MOUSE_MOVE |
    ...
    class ObjectCallback : public CallbackInterface
    {
        public:
            ...
            ObjectCallback( ObjectCallbackType callback,
                            T*                 obj ) :
                _object( obj ),
                _callback( callback )
            {
    ...
    class OSGWIDGET_EXPORT Callback : public osg::Referenced
    {
        public:
            ...
            template<typename T>
            Callback( bool      ( T::*function )( Event& ),
                      T*        obj,
                      EventType type,
                      void*     data = 0 ) :
                _type( type ),
                _data( data ),
                _callback( new ObjectCallback<T>( function,
                                                  obj ) )
            {
            }
    ...
    class OSGWIDGET_EXPORT EventInterface
    {
        public:
            ...
            // These functions take as their final argument the WindowManager which
            // issued the request. This is sometimes useful to get information about key
            // state, etc.

            // Notify the EventInterface object that is has been focused or unfocused;
            // since this isn't always bound to a mouse event (i.e., if you want to be
            // able to use the TAB key to focus), we need separate events here.
            virtual bool
            ...
            // These functions pass the osgGA::GUIEventAdapter::KeySymbol and KeyModMask
            // and, as above, the WindowManager.
            virtual bool
            keyDown( int,
                     int,
                     const WindowManager* )
            ...
            void
            addEventMask( unsigned int mask )
            {
                _eventMask |= mask;
            }

            void
            removeEventMask( unsigned int mask )
            {
                _eventMask ^= mask;
            }
            ...
            bool
            callCallbacks( Event& ev )
            {
                if( ev.type == EVENT_NONE || !( _eventMask & ev.type ) )
                {
                    return false;
                }
            ...
            bool
            callMethodAndCallbacks( Event& ev )
            {
                if( ev.type == EVENT_NONE || !( _eventMask & ev.type ) )
                {
                    return false;
                ...
                if( ev.type == EVENT_FOCUS )
                {
                    handled = focus( ev.getWindowManager() );
                }

                else if( ev.type == EVENT_UNFOCUS )
                {
                    handled = unfocus( ev.getWindowManager() );
                }
            ...
            canFocus() const
            {
                return ( _eventMask & EVENT_FOCUS ) != 0;
            }

            bool
            canUnfocus() const
            {
                return ( _eventMask & EVENT_UNFOCUS ) != 0;
            }

            bool
            canMouseEnter() const
            {
                return ( _eventMask & EVENT_MOUSE_ENTER ) != 0;
            }

            bool
            canMouseOver() const
            {
                return ( _eventMask & EVENT_MOUSE_OVER ) != 0;
            }

            bool
            canMouseLeave() const
            {
                return ( _eventMask & EVENT_MOUSE_LEAVE ) != 0;
            }

            bool
            canMouseDrag() const
            {
                return ( _eventMask & EVENT_MOUSE_DRAG ) != 0;
            }

            bool
            canMousePush() const
            {
                return ( _eventMask & EVENT_MOUSE_PUSH ) != 0;
            }

            bool
            canMouseRelease() const
            {
                return ( _eventMask & EVENT_MOUSE_RELEASE ) != 0;
            }

            bool
            canMouseScroll() const
            {
                return ( _eventMask & EVENT_MOUSE_SCROLL ) != 0;
            }

            bool
            canKeyDown() const
            {
                return ( _eventMask & EVENT_KEY_DOWN ) != 0;
            }

            bool
            canKeyUp() const
            {
                return ( _eventMask & EVENT_KEY_UP ) != 0;
            }
