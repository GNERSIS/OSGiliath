...
namespace osg
{
    ...
    /** A Polytope class for representing convex clipping volumes made up of a set of
     * planes. When adding planes, their normals should point inwards (into the volume)
     */
    class OSG_EXPORT Polytope
    {
        ...
        public:
                              ...
                              bool withFar  = true )
            {
                ...
                if( withNear )
                {
                    _planeList.push_back( Plane( 0.0, 0.0, 1.0, 1.0 ) );    // near plane
                }
                if( withFar )
                {
                    _planeList.push_back( Plane( 0.0, 0.0, -1.0, 1.0 ) );    // far plane
                }
                setupMask();
            }

            /** Create a Polytope which is a equivalent to box.*/
            void
            setToBoundingBox( const box& bb )
            {
                _planeList.clear();
                _planeList.push_back(
                    Plane( 1.0, 0.0, 0.0, -bb.xMin() )
                );    // left plane.
                _planeList.push_back(
                    Plane( -1.0, 0.0, 0.0, bb.xMax() )
                );    // right plane.
                _planeList.push_back(
                    Plane( 0.0, 1.0, 0.0, -bb.yMin() )
                );    // bottom plane.
                _planeList.push_back(
                    Plane( 0.0, -1.0, 0.0, bb.yMax() )
                );    // top plane.
                _planeList.push_back(
                    Plane( 0.0, 0.0, 1.0, -bb.zMin() )
                );    // near plane
                _planeList.push_back(
                    Plane( 0.0, 0.0, -1.0, bb.zMax() )
                );    // far plane
                setupMask();
            }

            inline void
            setAndTransformProvidingInverse( const Polytope&   pt,
            ...
            inline void
            setReferenceVertexList( VertexList& vertices )
            {
                _referenceVertexList = vertices;
            }

            inline VertexList&
            getReferenceVertexList()
            {
                return _referenceVertexList;
            }

            inline const VertexList&
            getReferenceVertexList() const
            {
                return _referenceVertexList;
            }
            ...
            getCurrentMask()
            {
                return _maskStack.back();
            }

            inline ClippingMask
            getCurrentMask() const
            {
                return _maskStack.back();
            }

            inline void
            setResultMask( ClippingMask mask )
            {
                _resultMask = mask;
            }
            ...
            inline bool
            contains( const osg::vec3& v ) const
            {
                if( !_maskStack.back() )
                {
                    return true;
                ...
                for( PlaneList::const_iterator itr = _planeList.begin();
                     ...
                     ++itr )
                {
                    if( ( _maskStack.back() & selector_mask ) &&
                        ( itr->distance( v ) < 0.0F ) )
                    {
                        return false;
                    }
            ...
            /** Check whether any part of a bounding sphere is contained within clipping
               set. Using a mask to determine which planes should be used for the check,
               and modifying the mask to turn off planes which wouldn't contribute to
               clipping of any internal objects.  This feature is used in
               osgUtil::CullVisitor to prevent redundant plane checking.*/
            inline bool
            contains( const osg::sphere& bs )
            {
                if( !_maskStack.back() )
                {
                    return true;
                     ...
                     ++itr )
                {
                    if( _resultMask & selector_mask )
                    {
                        ...
                        {
                            _resultMask ^= selector_mask;    // subsequent checks against
                                                             // this plane not required.
                        }
                    }
                    selector_mask <<= 1;
                }
            ...
            inline bool
            containsAllOf( const std::vector<vec3>& vertices )
            {
                if( !_maskStack.back() )
                {
                    return false;
             ...
             * set.*/
            inline bool
            containsAllOf( const osg::sphere& bs )
            {
                if( !_maskStack.back() )
                {
                    return false;
            ...
            inline bool
            containsAllOf( const osg::box& bb )
            {
                if( !_maskStack.back() )
                {
                    return false;
