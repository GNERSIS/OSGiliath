 ...
 * GLSL shader program attribute. Links vertex, fragment, geometry, and
 * compute shaders. Manages per-context compilation and uniform locations.
 */
...
namespace osg
{
    ...
    class OSG_EXPORT Program : public osg::Inherit<osg::StateAttribute, Program>
    {
        public:
            ...
            template<class T>
            bool
            addShader( const ref_ptr<T>& shader )
            {
                return addShader( shader.get() );
            }
            ...
            template<class T>
            bool
            removeShader( const ref_ptr<T>& shader )
            {
                return removeShader( shader.get() );
            }

            /** Set/get GL program parameters */
            void
            setParameter( GLenum pname,
                          GLint  value );
            GLint
            getParameter( GLenum pname ) const;

            /** Add an attribute location binding. */
            void
            addBindAttribLocation( const std::string& name,
                                   GLuint             index );

            /** Remove an attribute location binding. */
            void
            removeBindAttribLocation( const std::string& name );

            /** Add an frag data location binding. See EXT_gpu_shader4 for
             * BindFragDataLocationEXT */
            void
            addBindFragDataLocation( const std::string& name,
                                     GLuint             index );

            /** Remove an frag data location binding. */
            void
            removeBindFragDataLocation( const std::string& name );
             ...
             * it is cheap to set. */
            void
            addBindUniformBlock( const std::string& name,
                                 GLuint             index );

            /** Remove a uniform block binding. */
            void
            removeBindUniformBlock( const std::string& name );

            /** Remove a TransformFeedBackVarying. */
            void
            removeTransformFeedBackVarying( const std::string& name )
            {
                for( std::vector<std::string>::iterator i = _feedbackout.begin();
                     i != _feedbackout.end();
                     i++ )
                {
                    if( *i == name )
                    {
                        _feedbackout.erase( i );
                        break;
                    }
                }
            }
            ...
            void
            addTransformFeedBackVarying( const std::string& outname )
            {
            ...
            inline unsigned int
            getNumTransformFeedBackVaryings() const
            {
                return static_cast<unsigned int>( _feedbackout.size() );
            }

            /** Get const TransformFeedBack Varying at index i. */
            inline const std::string&
            getTransformFeedBackVarying( unsigned int i ) const
            {
                return _feedbackout[i];
            }

            /** Set TransformFeedBack Mode. */
            void
            setTransformFeedBackMode( GLenum e )
            {
                _feedbackmode = e;
            }
            ...
            GLenum
            getTransformFeedBackMode() const
            {
            ...
            /** Experimental. */
            void
            setShaderPragmas( const ShaderPragmas& shaderPragmas )
            {
                _shaderPragmas = shaderPragmas;
            }

            ShaderPragmas&
            getShaderPragmas()
            {
                return _shaderPragmas;
            }

            const ShaderPragmas&
            getShaderPragmas() const
            {
                return _shaderPragmas;
            }

            /** Simple class for wrapping up the data used in glProgramBinary and
             * glGetProgramBinary. On the first run of your application Programs should
             * be assigned an empty ProgramBinary. Before your application exits it
             * should retrieve the program binary via
             * Program::PerContextProgram::compileProgramBinary and save it to disk.
             * When your application is run subsequently, load your binary from disk and
             * use it to set the data of a ProgramBinary, and set the ProgramBinary on
             * the associated Program. This will typically result in
             * Program::compileGLObjects executing much faster.*/
            class OSG_EXPORT ProgramBinary
                : public osg::Inherit<osg::Object, ProgramBinary>
            {
                public:
                    ...
                    /** Copy constructor using CopyOp to manage deep vs shallow copy.*/
                    ProgramBinary( const ProgramBinary& rhs,
                     ...
                     * locally stored data buffer, the original data can then be
                     * deleted.*/
                    void
                    assign( unsigned int         size,
            ...
            void
            setProgramBinary( ProgramBinary* programBinary )
            {
            ...
            const AttribBindingList&
            getAttribBindingList() const
            {
                return _attribBindingList;
            }

            const FragDataBindingList&
            getFragDataBindingList() const
            {
                return _fragDataBindingList;
            }

            const UniformBlockBindingList&
            getUniformBlockBindingList() const
            {
            ...
            struct ActiveVarInfo
            {
                    ActiveVarInfo() :
                        _location( -1 ),
                        _type( Uniform::UNDEFINED ),
                        _size( -1 )
                    {
                    }

                    ActiveVarInfo( GLint  loc,
                                   GLenum type,
                                   GLint  size ) :
                        _location( loc ),
                        _type( type ),
        ...
        public:
            ...
            class OSG_EXPORT PerContextProgram : public osg::Referenced
            {
                public:
                    ...
                    PerContextProgram( const Program* program,
                    ...
                    void
                    setDefineString( const std::string& defStr )
                    {
                        _defineStr = defStr;
                    }

                    const std::string&
                    getDefineString() const
                    {
                        return _defineStr;
                    }
                    ...
                    virtual bool
                    getInfoLog( std::string& infoLog ) const;

                    /** Was glProgramBinary called successfully? */
                    bool
                    loadedBinary() const
                    {
                        return _loadedBinary;
                    }

                    /** Compile a program binary. For this to work setProgramBinary must
                     * have been called on the osg::Program with an empty ProgramBinary
                     * prior to compileGLObjects being called. compileProgramBinary
                     * should be called after the program has been "exercised" by
                     * rendering with it. The ProgramBinary can then be saved to disk for
                     * faster subsequent compiling. */
                    virtual ProgramBinary*
                    compileProgramBinary( osg::State& state );

                    virtual void
                    useProgram() const;

                    void
                    resetAppliedUniforms() const
                    {
                        _lastAppliedUniformList.clear();
                    }

                    inline void
                    apply( const UniformBase& uniform ) const
                    {
                        GLint location = getUniformLocation( uniform.getNameID() );
                        if( location >= 0 )
                        {
                            unsigned int loc = static_cast<unsigned int>( location );
                            const UniformBase* lastAppliedUniform =
                                _lastAppliedUniformList[loc].first.get();
                            if( lastAppliedUniform != &uniform )
                            {
                                // new attribute
                                uniform.apply( _extensions.get(), location );
                                _lastAppliedUniformList[loc].first = &uniform;
                                _lastAppliedUniformList[loc].second =
                                    uniform.getModifiedCount();
                            }
                            else if( _lastAppliedUniformList[loc].second !=
                                     uniform.getModifiedCount() )
                            {
                    ...
                    const ActiveUniformMap&
                    getActiveUniforms() const
                    {
                        return _uniformInfoMap;
                    }

                    const ActiveVarInfoMap&
                    getActiveAttribs() const
                    {
                        return _attribInfoMap;
                    }

                    const UniformBlockMap&
                    getUniformBlocks() const
                    {
                        return _uniformBlockMap;
                    }

                    inline GLint
                    getUniformLocation( unsigned int uniformNameID ) const
                    {
                        ActiveUniformMap::const_iterator itr =
                            _uniformInfoMap.find( uniformNameID );
                        return ( itr != _uniformInfoMap.end() ) ? itr->second._location
                                                                : -1;
                    }

                    /**
                     * Alternative version of getUniformLocation( unsigned int
                     * uniformNameID ) retrofited into OSG for backward compatibility
                     * with osgCal, after uniform ids were refactored from std::strings
                     * to GLints in OSG version 2.9.10.
                     *
                     * Drawbacks: This method is not particularly fast. It has to access
                     * mutexed static map of uniform ids. So don't overuse it or your app
                     * performance will suffer.
                     */
                    inline GLint
                    getUniformLocation( const std::string& uniformName ) const
                    {
                        return getUniformLocation( Uniform::getNameID( uniformName ) );
                    }

                    inline GLint
                    getAttribLocation( const std::string& name ) const
                    {
                        ActiveVarInfoMap::const_iterator itr =
                            _attribInfoMap.find( name );
                        return ( itr != _attribInfoMap.end() ) ? itr->second._location
                                                               : -1;
                    }

                    inline void
                    addShaderToAttach( Shader* shader )
                    {
                        _shadersToAttach.push_back( shader );
                    }

                    inline void
                    addShaderToDetach( Shader* shader )
                    {
                ...
                protected: /*data*/
                    ...
                    typedef std::pair<osg::ref_ptr<const osg::UniformBase>, unsigned int>
                ...
                private:
                    ...
                    PerContextProgram&
                    operator=( const PerContextProgram& );            // disallowed
            };

            struct OSG_EXPORT ProgramObjects : public osg::GraphicsObject
            {
                    typedef std::vector<osg::ref_ptr<PerContextProgram>>
                        PerContextPrograms;

                    ProgramObjects( const Program* program,
                                    unsigned int   contextID );
                    ...
                    PerContextProgram*
                    getPCP( const std::string& defineStr ) const;
                    PerContextProgram*
                    ...
                    addShaderToDetach( Shader* shader );
                    bool
                    getGlProgramInfoLog( std::string& log ) const;
            };

            /** Get the PCP for a particular GL context */
            PerContextProgram*
            getPCP( State& state ) const;
        ...
        private:
            ...
            Program&
            operator=( const Program& );    // disallowed
    };
