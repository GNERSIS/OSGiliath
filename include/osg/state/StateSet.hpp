...
namespace osg
{
    ...
    class OSG_EXPORT StateSet : public Object
    {
        public:
            ...
            {
                return new StateSet( *this, copyop );
            }

            virtual bool
            isSameKindAs( const Object* obj ) const
            {
                return dynamic_cast<const StateSet*>( obj ) != NULL;
            }
            ...
            int
            compare( const StateSet& rhs,
                     bool            compareAttributeContents = false ) const;

            bool
            operator<( const StateSet& rhs ) const
            {
            ...
            /** Convert 'this' into a StateSet pointer if Object is a StateSet, otherwise
             * return 0. Equivalent to dynamic_cast<StateSet*>(this).*/
            virtual StateSet*
            asStateSet()
            {
                return this;
             ...
             * StateSet*>(this).*/
            virtual const StateSet*
            asStateSet() const
            {
                return this;
            }

            /** A vector of osg::Object pointers which is used to store the parent(s) of
             * this Stateset, the parents could be osg::Node or osg::Drawable.*/
             ...
             */
            void
            setMode( StateAttribute::GLMode      mode,
             ...
             */
            void
            removeMode( StateAttribute::GLMode mode );
            ...
            getMode( StateAttribute::GLMode mode ) const;

            /** Set the list of all <tt>GLMode</tt>s contained in this \c StateSet.*/
            inline void
            setModeList( ModeList& ml )
            {
                _modeList = ml;
            }
            ...
            getModeList() const noexcept
            {
                return _modeList;
            }

            /** Simple pairing between an attribute and its override flag.*/
            typedef std::pair<ref_ptr<StateAttribute>, StateAttribute::OverrideValue>
                RefAttributePair;

            /** a container to map <StateAttribyte::Types,Member> to their respective
             * RefAttributePair.*/
            typedef std::map<StateAttribute::TypeMemberPair, RefAttributePair>
                AttributeList;

            /** Set this StateSet to contain specified attribute and override flag.*/
            void
            setAttribute( StateAttribute*               attribute,
                          StateAttribute::OverrideValue value = StateAttribute::OFF );

            template<class T>
            void
            setAttribute( const ref_ptr<T>&             attribute,
                          StateAttribute::OverrideValue value = StateAttribute::OFF )
            {
                setAttribute( attribute.get(), value );
            }

            /** Set this StateSet to contain specified attribute and set the associated
             * GLMode's to specified value.*/
            void
            setAttributeAndModes( StateAttribute*             attribute,
                                  StateAttribute::GLModeValue value =
                                      StateAttribute::ON );

            template<class T>
            void
            setAttributeAndModes( const ref_ptr<T>&           attribute,
                                  StateAttribute::GLModeValue value =
                                      StateAttribute::ON )
            {
                setAttributeAndModes( attribute.get(), value );
            }

            /** remove attribute of specified type from StateSet.*/
            void
            removeAttribute( StateAttribute::Type type,
                             unsigned int         member = 0 );

            /** remove attribute from StateSet.*/
            void
            removeAttribute( StateAttribute* attribute );

            template<class T>
            void
            removeAttribute( const ref_ptr<T>& attribute )
            {
            ...
            StateAttribute*
            getAttribute( StateAttribute::Type type,
                          unsigned int         member = 0 );
             ...
             * Returns NULL if no type is contained within StateSet.*/
            RefAttributePair*
            getAttributePair( StateAttribute::Type type,
                              unsigned int         member = 0 );

            /** Get specified RefAttributePair for specified type.
             * Returns NULL if no type is contained within StateSet.*/
            const RefAttributePair*
            getAttributePair( StateAttribute::Type type,
                              unsigned int         member = 0 ) const;

            /** set the list of all StateAttributes contained in this StateSet.*/
            inline void
            setAttributeList( AttributeList& al )
            {
                _attributeList = al;
            }
             ...
             * StateSet.*/
            inline const AttributeList&
            getAttributeList() const noexcept
            {
                return _attributeList;
            }
            ...
            inline unsigned int
            getNumTextureModeLists() const noexcept
            {
                return static_cast<unsigned int>( _textureModeList.size() );
            }
            ...
            /** Set this StateSet to contain specified attribute and override flag.*/
            void
            setTextureAttribute( unsigned int                  unit,
            ...
            setTextureAttribute( unsigned int                  unit,
                                 ...
                                 StateAttribute::OverrideValue value =
                                     StateAttribute::OFF )
            {
                setTextureAttribute( unit, attribute.get(), value );
            }

            /** Set this StateSet to contain specified attribute and set the associated
             * GLMode's to specified value.*/
            void
            setTextureAttributeAndModes( unsigned int                unit,
                                         StateAttribute*             attribute,
                                         StateAttribute::GLModeValue value =
                                             StateAttribute::ON );
            ...
            setTextureAttributeAndModes( unsigned int                  unit,
                                         ...
                                         StateAttribute::OverrideValue value =
                                             StateAttribute::ON )
            {
                setTextureAttributeAndModes( unit, attribute.get(), value );
            }

            /** remove texture attribute of specified type from StateSet.*/
            void
            removeTextureAttribute( unsigned int         unit,
            ...
            removeTextureAttribute( unsigned int      unit,
                                    const ref_ptr<T>& attribute )
            {
                removeTextureAttribute( unit, attribute.get() );
            }

            /** Get specified Texture related StateAttribute for specified type.
             * Returns NULL if no type is contained within StateSet.*/
            StateAttribute*
            getTextureAttribute( unsigned int         unit,
                                 StateAttribute::Type type );
             ...
             * Returns NULL if no type is contained within StateSet.*/
            RefAttributePair*
            getTextureAttributePair( unsigned int         unit,
                                     StateAttribute::Type type );
             ...
             * StateSet.*/
            inline void
            setTextureAttributeList( TextureAttributeList& tal )
            {
                _textureAttributeList = tal;
            }

            /** Return the list of all Texture related StateAttributes contained in this
             * StateSet.*/
            inline TextureAttributeList&
            getTextureAttributeList() noexcept
            {
                return _textureAttributeList;
            }

            /** Return the const list of all Texture related StateAttributes contained in
             * this const StateSet.*/
            inline const TextureAttributeList&
            getTextureAttributeList() const noexcept
            {
                return _textureAttributeList;
            }
            ...
            inline unsigned int
            getNumTextureAttributeLists() const noexcept
            {
                return static_cast<unsigned int>( _textureAttributeList.size() );
            }

            void
            setAssociatedModes( const StateAttribute*       attribute,
                                StateAttribute::GLModeValue value );
            void
            removeAssociatedModes( const StateAttribute* attribute );

            void
            setAssociatedTextureModes( unsigned int                unit,
                                       const StateAttribute*       attribute,
                                       StateAttribute::GLModeValue value );
            void
            removeAssociatedTextureModes( unsigned int          unit,
                                          const StateAttribute* attribute );

            /** Simple pairing between a Uniform and its override flag.*/
            typedef std::pair<ref_ptr<UniformBase>, StateAttribute::OverrideValue>
                                                          RefUniformPair;
            ...
            void
            addUniform( UniformBase*                  uniform,
                        StateAttribute::OverrideValue value = StateAttribute::ON );

            template<class T>
            void
            addUniform( const ref_ptr<T>&             uniform,
                        StateAttribute::OverrideValue value = StateAttribute::ON )
            {
                addUniform( uniform.get(), value );
            }

            /** remove uniform of specified name from StateSet.*/
            void
            removeUniform( const std::string& name );

            /** remove Uniform from StateSet.*/
            void
            removeUniform( UniformBase* uniform );

            template<class T>
            void
            removeUniform( const ref_ptr<T>& uniform )
            {
            ...
            UniformBase*
            getUniformBase( const std::string& name );
            ...
            {
                return dynamic_cast<Uniform*>( getUniformBase( name ) );
            }

            /** Get Uniform for specified name, if one is not available create it, add it
             * to this StateSet and return a pointer to it.*/
            Uniform*
            getOrCreateUniform( const std::string& name,
                                Uniform::Type      type,
             ...
             * Returns NULL if no matching Uniform is contained within StateSet.*/
            const UniformBase*
            getUniformBase( const std::string& name ) const;
            ...
            inline UniformList&
            getUniformList() noexcept
            {
                return _uniformList;
            }

            /** return the const list of all Uniforms contained in this const StateSet.*/
            inline const UniformList&
            getUniformList() const noexcept
            {
                return _uniformList;
            }
             ...
             * requires(..). */
            void
            setDefine( const std::string&            defineName,
                       StateAttribute::OverrideValue value = StateAttribute::ON );

            /** Added define with value to pass on to shaders that use utilize that
             * define, as specified by the GLSL \#pragma import_defines(..) and \#pragma
             * requires(..). */
            void
            setDefine( const std::string&            defineName,
            ...
            DefinePair*
            getDefinePair( const std::string& defineName )
            {
            ...
            {
                DefineList::const_iterator itr = _defineList.find( defineName );
                return ( itr != _defineList.end() ) ? &( itr->second ) : 0;
            }

            /** Remove define*/
            void
            removeDefine( const std::string& defineName );

            /** Set the list of defines to pass on to shaders.*/
            void
            setDefineList( const DefineList& dl )
            {
                _defineList = dl;
            }
             ...
             */
            void
            setRenderingHint( int hint );

            /** Get the \c RenderingHint of this \c StateSet.*/
            inline int
            getRenderingHint() const noexcept
            {
            ...
            void
            setRenderBinDetails( int                binNum,
            ...
            inline bool
            useRenderBinDetails() const noexcept
            {
                return _binMode != INHERIT_RENDERBIN_DETAILS;
            }

            /** Set the render bin mode.*/
            inline void
            setRenderBinMode( RenderBinMode mode ) noexcept
            {
                _binMode = mode;
            }

            /** Get the render bin mode.*/
            inline RenderBinMode
            getRenderBinMode() const noexcept
            {
                return _binMode;
            }
            ...
            inline void
            setBinName( const std::string& name )
            {
                _binName = name;
            }
            ...
            inline void
            setNestRenderBins( bool val ) noexcept
            {
                _nestRenderBins = val;
            }
             ...
             * RenderBin.*/
            inline bool
            getNestRenderBins() const noexcept
            {
                return _nestRenderBins;
            }
            ...
            struct OSG_EXPORT Callback : public virtual osg::Callback
            {
                    ...
                    {
                        return new Callback( *this, copyop );
                    }

                    virtual bool
                    isSameKindAs( const osg::Object* obj ) const
                    {
                        return dynamic_cast<const Callback*>( obj ) != NULL;
                    }
            ...
            void
            setUpdateCallback( Callback* ac );

            template<class T>
            void
            setUpdateCallback( const ref_ptr<T>& ac )
            {
                setUpdateCallback( ac.get() );
            }
            ...
            /** Return whether this StateSet has update callbacks associated with it, and
             * therefore must be traversed.*/
            bool
            requiresUpdateTraversal() const noexcept
            {
             ...
             * traversal, since they have an Update Callback attached to them or their
             * children.*/
            inline unsigned int
             ...
             * children.*/
            void
            runUpdateCallbacks( osg::NodeVisitor* nv );

            /** Set the Event Callback which allows users to attach customize the
             * updating of an object during the event traversal.*/
            void
            setEventCallback( Callback* ac );

            template<class T>
            void
            setEventCallback( const ref_ptr<T>& ec )
            {
                setEventCallback( ec.get() );
            }
            ...
            /** Return whether this StateSet has event callbacks associated with it, and
             * therefore must be traversed.*/
            bool
            requiresEventTraversal() const noexcept
            {
                return _eventCallback.valid() ||
             ...
             * children.*/
            void
            runEventCallbacks( osg::NodeVisitor* nv );
            ...
            bool
            checkValidityOfAssociatedModes( State& state ) const;
        ...
        protected:
            ...
            void
            addParent( osg::Node* object );
            void
                       removeParent( osg::Node* object );
            ...
            inline ModeList&
            getOrCreateTextureModeList( unsigned int unit )
            {
                if( unit >= _textureModeList.size() )
                {
                    _textureModeList.resize( unit + 1 );
                }
                return _textureModeList[unit];
            }

            inline AttributeList&
            getOrCreateTextureAttributeList( unsigned int unit )
            {
                if( unit >= _textureAttributeList.size() )
                {
                    _textureAttributeList.resize( unit + 1 );
                }
                return _textureAttributeList[unit];
            }

            int
            compareModes( const ModeList& lhs,
                          const ModeList& rhs );
            int
            compareAttributePtrs( const AttributeList& lhs,
                                  const AttributeList& rhs );
            int
            compareAttributeContents( const AttributeList& lhs,
                                      const AttributeList& rhs );

            void
            setMode( ModeList&                   modeList,
                     StateAttribute::GLMode      mode,
                     StateAttribute::GLModeValue value );
            void
            setModeToInherit( ModeList&              modeList,
                              StateAttribute::GLMode mode );
            StateAttribute::GLModeValue
            getMode( const ModeList&        modeList,
                     StateAttribute::GLMode mode ) const;

            void
            setAttribute( AttributeList&                attributeList,
                          StateAttribute*               attribute,
                          StateAttribute::OverrideValue value = StateAttribute::OFF );

            StateAttribute*
            getAttribute( AttributeList&       attributeList,
                          StateAttribute::Type type );
            const StateAttribute*
            getAttribute( const AttributeList& attributeList,
                          StateAttribute::Type type ) const;
            ...
            RefAttributePair*
            getAttributePair( AttributeList&       attributeList,
                              StateAttribute::Type type );
            const RefAttributePair*
            getAttributePair( const AttributeList& attributeList,
                              StateAttribute::Type type ) const;
    ...
    isTextureMode( StateAttribute::GLMode mode );
