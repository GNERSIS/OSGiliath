...
namespace osg
{
    ...
    typedef std::vector<ShaderDefines> ShaderDefinesVector;

    struct ShaderPragmas
    {
            ShaderDefines       defines;
            ShaderDefines       requirements;
            ShaderDefines       modes;
            ShaderDefinesVector textureModes;
            ...
            void
            merge( const ShaderPragmas& rhs )
            {
                defines.insert( rhs.defines.begin(), rhs.defines.end() );
                requirements.insert( rhs.requirements.begin(), rhs.requirements.end() );
                modes.insert( rhs.modes.begin(), rhs.modes.end() );

                // rseize textureModes vector if required
                unsigned int rhs_rds_size =
                    static_cast<unsigned int>( rhs.textureModes.size() );
                if( rhs_rds_size > textureModes.size() )
                {
                    textureModes.resize( rhs_rds_size );
                }

                for( unsigned int i = 0; i < rhs_rds_size; ++i )
                {
                    const ShaderDefines& rhs_tsd = rhs.textureModes[i];
                    textureModes[i].insert( rhs_tsd.begin(), rhs_tsd.end() );
                }
            }
    };

    /** Simple class for wrapping up the data used in OpenGL ES 2's glShaderBinary calls.
     * ShaderBinary is set up with the binary data then assigned to one or more
     * osg::Shader. */
    class OSG_EXPORT ShaderBinary : public osg::Inherit<osg::Object, ShaderBinary>
    {
        public:
            ...
            ShaderBinary( const ShaderBinary& rhs,
            ...
            /** Assign shader binary data, copying the specified data into locally stored
             * data buffer, the original data can then be deleted.*/
            void
             ...
             * Return the resulting Shader or 0 if no valid shader binary could be
             * read.*/
            static ShaderBinary*
            readShaderBinaryFile( const std::string& fileName );
    ...
    class OSG_EXPORT Shader : public osg::Inherit<osg::Object, Shader>
    {
        public:
            ...
            int
            compare( const Shader& rhs ) const;
            ...
            void
            setShaderSource( const std::string& sourceText );

            /** Query the shader's source code text */
            inline const std::string&
            getShaderSource() const
            {
                return _shaderSource;
            }

            enum ShaderPragmaMode
            {
                USE_SHADER_PRAGMA,
                USE_MANUAL_SETTINGS,
            };
            ...
            void
            setShaderPragmaMode( ShaderPragmaMode spm )
            {
                _shaderPragmaMode = spm;
            }

            ShaderPragmaMode
            getShaderPragmaMode() const
            {
                return _shaderPragmaMode;
            }

            void
            setShaderDefines( const ShaderDefines& shaderDefs )
            {
                _shaderPragmas.defines = shaderDefs;
            }

            ShaderDefines&
            getShaderDefines()
            {
                return _shaderPragmas.defines;
            }
            ...
            void
            setShaderRequirements( const ShaderDefines& shaderDefs )
            {
                _shaderPragmas.requirements = shaderDefs;
            }

            ShaderDefines&
            getShaderRequirements()
            {
                return _shaderPragmas.requirements;
            }

            const ShaderDefines&
            getShaderRequirements() const
            {
                return _shaderPragmas.requirements;
            }

            void
            setShaderModes( const ShaderDefines& shaderDefs )
            {
                _shaderPragmas.modes = shaderDefs;
            }

            ShaderDefines&
            getShaderModes()
            {
                return _shaderPragmas.modes;
            }

            const ShaderDefines&
            getShaderModes() const
            {
                return _shaderPragmas.modes;
            }
            ...
            void
            setShaderBinary( ShaderBinary* shaderBinary )
            {
            ...
            static Shader*
            readShaderFile( Type               type,
                            const std::string& fileName );

            /** Deorecated use osgDB::readRefShaderFile(). */
            bool
            loadShaderSourceFromFile( const std::string& fileName );
#endif

            /** The code injection map used when generating the main shader during main
             * shader composition.*/
             ...
             * inserted before the main() will have a negative value, a position between
             * 0 and 1.0 will be inserted in main() and a position greater than 1.0 will
             * be placed after the main().
             * During shader composition all the code injections are sorted in ascending
             * order and then placed in the appropriate section of the main shader. */
            void
            addCodeInjection( float              position,
                              const std::string& code )
            {
                _codeInjectionMap.insert( CodeInjectionMap::value_type( position,
                                                                        code ) );
            }
            ...
            getCodeInjectionMap()
            {
                return _codeInjectionMap;
            }

            /** Get the const code injection map.*/
            const CodeInjectionMap&
            getCodeInjectionMap() const
            {
                return _codeInjectionMap;
            }
            ...
            void
            compileShader( osg::State& state ) const;
        ...
        public:
            ...
            class OSG_EXPORT PerContextShader : public osg::Referenced
            {
                public:
                    ...
                    PerContextShader( const Shader* shader,
                    ...
                    void
                    attachShader( GLuint program ) const;

                    /** Detach our glShader from a glProgram */
                    void
                    detachShader( GLuint program ) const;
                ...
                private:
                    ...
                    PerContextShader&
                    operator=( const PerContextShader& );           // disallowed
            };

            struct OSG_EXPORT ShaderObjects : public osg::Referenced
            {
                    typedef std::vector<osg::ref_ptr<PerContextShader>>
                        PerContextShaders;

                    ShaderObjects( const Shader* shader,
                                   unsigned int  contextID );
                    ...
                    PerContextShader*
                    getPCS( const std::string& defineStr ) const;
                    PerContextShader*
                    createPerContextShader( const std::string& defineStr );
                    void
                    requestCompile();
            };

            PerContextShader*
            getPCS( osg::State& state ) const;
        ...
        protected: /*methods*/
            ...
            bool
            addProgramRef( osg::Program* program );
            bool
            removeProgramRef( osg::Program* program );

            void
            _computeShaderDefines();
            bool
            _parseShaderDefines( const std::string& str,
                                 ShaderDefines&     defines,
                                 unsigned int&      index );
        ...
        protected: /*data*/
            ...
            // ShaderPragmas variables
            ShaderPragmaMode                                         _shaderPragmaMode;
            ShaderPragmas                                            _shaderPragmas;

            /** osg::Programs that this osg::Shader is attached to */
            typedef std::set<osg::Program*>                          ProgramSet;
            ProgramSet                                               _programSet;
            std::mutex                                               _programSetMutex;
            mutable osg::buffered_value<osg::ref_ptr<ShaderObjects>> _pcsList;

        private:

            Shader&
            operator=( const Shader& );    // disallowed
    };

    class OSG_EXPORT ShaderComponent : public osg::Inherit<osg::Object, ShaderComponent>
    {
        public:

            ShaderComponent();
            ShaderComponent( const ShaderComponent& sc,
