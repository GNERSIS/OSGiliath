...
/** COMPARE_StateAttribute_Types macro is a helper for implementing the
 * StateAtribute::compare(..) method.*/
#define COMPARE_StateAttribute_Types( TYPE, rhs_attribute )      \
    if( this == &rhs_attribute )                                 \
        return 0;                                                \
    const std::type_info* type_lhs = &typeid( *this );           \
    const std::type_info* type_rhs = &typeid( rhs_attribute );   \
    if( type_lhs->before( *type_rhs ) )                          \
        return -1;                                               \
    if( *type_lhs != *type_rhs )                                 \
        return 1;                                                \
    const TYPE& rhs = static_cast<const TYPE&>( rhs_attribute );

/** COMPARE_StateAttribute_Parameter macro is a helper for implementing the
 * StatateAtribute::compare(..) method. Macro assumes that variable rhs has been
 * correctly defined by preceding code macro.*/
#define COMPARE_StateAttribute_Parameter( parameter ) \
    if( parameter < rhs.parameter )                   \
        return -1;                                    \
    if( rhs.parameter < parameter )                   \
        return 1;
    ...
    class OSG_EXPORT StateAttribute : public Object
    {
        public:
            ...
            /** GLModeValue is used to specify whether a mode is enabled (ON) or disabled
             * (OFF). GLMoveValue is also used to specify the override behavior of modes
             * from parent to children. See enum Value description for more details.*/
             ...
             * OverrideValues. When using in conjunction with GLModeValues, all Values
             * have meaning. When using in conjunction with StateAttribute OverrideValue
             * only OFF,OVERRIDE and INHERIT are meaningful. However, they are useful
             * when using GLModeValue and OverrideValue in conjunction with each other as
             * when using StateSet::setAttributeAndModes(..).*/
            enum Values
            {
            ...
            /** Arithmetic support for Type enum class (used by Capability). */
            ...
            /** Simple pairing between an attribute type and the member within that
             * attribute type group.*/
            typedef std::pair<Type, unsigned int> TypeMemberPair;

            StateAttribute();

            StateAttribute( const StateAttribute& sa,
            ...
            {
                return dynamic_cast<const StateAttribute*>( obj ) != NULL;
            }
            ...
            inline TypeMemberPair
            getTypeMemberPair() const
            {
            ...
            virtual int
            compare( const StateAttribute& sa ) const = 0;

            bool
            operator<( const StateAttribute& rhs ) const
            {
            ...
            void
            setShaderComponent( ShaderComponent* sc ) noexcept
            {
                _shaderComponent = sc;
            }

            ShaderComponent*
            getShaderComponent() noexcept
            {
                return _shaderComponent.get();
            }

            const ShaderComponent*
            getShaderComponent() const noexcept
            {
                return _shaderComponent.get();
            }
            ...
            struct ModeUsage
            {
                    ...
                    virtual void
                    usesMode( GLMode mode ) = 0;
                    virtual void
                    usesTextureMode( GLMode mode ) = 0;
            };

            /** Return the modes associated with this StateAttribute.*/
            virtual bool
            getModeUsage( ModeUsage& ) const
            {
             ...
             * current OpenGL drivers, and if not set the associated mode in osg::State
             * to be black listed/invalid. Return true if all associated modes are
             * valid.*/
            virtual bool
            checkValidityOfAssociatedModes( osg::State& ) const
            {
                // default to no black listed GLMode's associated with use of the
                // StateAttribute.
                return true;
            }
            ...
            void
            setUpdateCallback( StateAttributeCallback* uc );
            ...
            void
            setEventCallback( StateAttributeCallback* ec );
            ...
            virtual void
            releaseGLObjects( State* = 0 ) const
            {
            }
        ...
        protected:
            ...
            {
                    /** Constructor caches and then removes attribute for all of it's
                     * parents.*/
