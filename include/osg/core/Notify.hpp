...
#include <osg/core/Referenced.hpp>    // for NotifyHandler
#include <ostream>
...
namespace osg
{
    ...
    extern OSG_EXPORT NotifySeverity
    getNotifyLevel();
    ...
    inline bool
    isNotifyEnabled( NotifySeverity )
    {
        return false;
    }
#else
    ...
    extern OSG_EXPORT bool
    isNotifyEnabled( NotifySeverity severity );
#endif
     ...
     */
    extern OSG_EXPORT std::ostream&
                      notify( const NotifySeverity severity );

    inline std::ostream&
    notify( void )
    {
        return notify( osg::INFO );
    }

#define OSG_NOTIFY( level )             \
...
#define OSG_DEBUG_FP OSG_NOTIFY( osg::DEBUG_FP )
    ...
    extern OSG_EXPORT NotifyHandler*
    getNotifyHandler();

    /** Redirects notification stream to stderr (severity <= WARN) or stdout (severity >
     * WARN). The fputs() function is used to write messages to standard files. Note that
