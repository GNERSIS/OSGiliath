...
/** Helper macro that creates a static proxy object to call singleton function on it's
 * construction, ensuring that the singleton gets initialized at startup.*/
    ...
    */
    class OSG_EXPORT Object : public Referenced
    {
        public:
            ...
            inline Object() :
                Referenced(),
                _dataVariance( DataVariance::UNSPECIFIED ),
                _userDataContainer( 0 )
            {
            }

            inline explicit Object( bool threadSafeRefUnref ) :
                Referenced( threadSafeRefUnref ),
                _dataVariance( DataVariance::UNSPECIFIED ),
                _userDataContainer( 0 )
            {
            }
            ...
            virtual bool
            isSameKindAs( const Object* ) const
            {
                return true;
            }

            /** Check whether this object's type is compatible with the given type_info.
             * Walks up the inheritance chain via Inherit<>. Enables cast<T>() without
             * dynamic_cast. */
            virtual bool
            is_compatible( const std::type_info& type ) const noexcept
            {
                return typeid( Object ) == type;
            }

            /** Return the type_info of the most-derived class. */
            ...
            /** Return sizeof the most-derived class. Useful for memory profiling. */
            virtual std::size_t
            sizeofObject() const noexcept
            {
                return sizeof( Object );
            }

            /** Safe downcast using the is_compatible() chain — avoids dynamic_cast. */
            template<class T>
            T*
            cast()
            {
                return is_compatible( typeid( T ) ) ? static_cast<T*>( this ) : nullptr;
            }
            ...
            cast() const
            {
                return is_compatible( typeid( T ) ) ? static_cast<const T*>( this )
                                                    : nullptr;
            }
            ...
            /** return the compound class name that combines the library name and class
             * name.*/
            std::string
            getCompoundClassName() const
            {
            ...
            /** convert 'const this' into a const StateAttribute pointer if Object is a
             * StateAttribute, otherwise return 0. Equivalent to dynamic_cast<const
             * StateAttribute*>(this).*/
            virtual const StateAttribute*
            asStateAttribute() const
            {
                return 0;
            }
             ...
             * otherwise return 0. Equivalent to dynamic_cast<const
             * UniformBase*>(this).*/
            ...
            /** Convert 'this' into a Uniform pointer if Object is a Uniform, otherwise
             * return 0. Equivalent to dynamic_cast<UniformBase*>(this).*/
            virtual UniformBase*
            asUniformBase()
            {
                return 0;
            ...
            /** Convert 'this' into a Callback pointer if Object is a Callback, otherwise
             * return 0. Equivalent to dynamic_cast<Callback*>(this).*/
            virtual Callback*
            asCallback()
            {
                return 0;
            ...
            /** convert 'const this' into a const CallbackObject pointer if Object is a
             * CallbackObject, otherwise return 0. Equivalent to dynamic_cast<const
             * CallbackObject*>(this).*/
            virtual const CallbackObject*
            ...
            /** convert 'const this' into a const UserDataContainer pointer if Object is
             * a UserDataContainer, otherwise return 0. Equivalent to dynamic_cast<const
             * UserDataContainer*>(this).*/
            virtual const UserDataContainer*
            asUserDataContainer() const
            {
                return 0;
            }
             ...
             * Can be set to either STATIC for values that do not change over the
             * lifetime of the object, or DYNAMIC for values that vary over the lifetime
             * of the object. The DataVariance value can be used by routines such as
             * optimization codes that wish to share static data. UNSPECIFIED is used to
             * specify that the DataVariance hasn't been set yet. */
            inline void
            setDataVariance( DataVariance dv ) noexcept
            {
                _dataVariance = dv;
            }
            ...
            inline DataVariance
            getDataVariance() const noexcept
            {
            ...
            template<class T>
            void
            setUserDataContainer( const ref_ptr<T>& udc )
            {
            ...
            getUserDataContainer() noexcept
            {
                return _userDataContainer;
            }

            /** get the const UserDataContainer attached to this object.*/
            const osg::UserDataContainer*
            getUserDataContainer() const noexcept
            {
                return _userDataContainer;
            }

            /** Convenience method that returns the UserDataContainer, and if one doesn't
             * already exist creates and assigns a DefaultUserDataContainer to the Object
             * and then return this new UserDataContainer.*/
            osg::UserDataContainer*
             ...
             */
            virtual void
            setUserData( Referenced* obj );

            template<class T>
            void
            setUserData( const ref_ptr<T>& ud )
            {
                setUserData( ud.get() );
            }
            ...
            template<typename T>
            bool
            getUserValue( const std::string& name,
                          T&                 value ) const;

            /** Convenience method that creates the osg::TemplateValueObject<T> to store
             * the specified value and adds it as a named UserObject. To use this
             * template method you need to include the osg/ValueObject header. */
            template<typename T>
            void
            setUserValue( const std::string& name,
                          const T&           value );
           ...
           const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY )
    {
        if( t )
        {
            ...
            {
                OSG_WARN << "Warning: osg::clone(const T*, osg::CopyOp&) cloned object "
                            "not of type T, returning NULL."
                         << std::endl;
                return 0;
            }
        }
        ...
        {
            OSG_WARN << "Warning: osg::clone(const T*, osg::CopyOp&) passed null object "
                        "to clone, returning NULL."
                     << std::endl;
            return 0;
        }
    }
           ...
           const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY )
    {
        ...
        {
            OSG_WARN << "Warning: osg::clone(const T*, const std::string&, const "
                        "osg::CopyOp) passed null object to clone, returning NULL."
                     << std::endl;
            return 0;
        }
    }
    ...
    cloneType( const T* t )
    {
        if( t )
        {
            ...
            {
                OSG_WARN << "Warning: osg::cloneType(const T*) cloned object not of "
                            "type T, returning NULL."
                         << std::endl;
                return 0;
            }
        }
        ...
        {
            OSG_WARN << "Warning: osg::cloneType(const T*) passed null object to clone, "
                        "returning NULL."
                     << std::endl;
            return 0;
        }
    ...
    class DummyObject : public osg::Object
    {
        public:
            ...
            DummyObject( const DummyObject& org,
                         const CopyOp&      copyop ) :
                Object( org,
                        copyop )
            {
            ...
            {
                return new DummyObject( *this, copyop );
            }

            virtual bool
            isSameKindAs( const osg::Object* obj ) const override
            {
                return dynamic_cast<const DummyObject*>( obj ) != nullptr;
            }
    ...
    inline void
    resizeGLObjectBuffers( osg::Object* object,
    ...
    template<class T>
    void
    resizeGLObjectBuffers( const osg::ref_ptr<T>& object,
                           unsigned int           maxSize )
    {
        if( object.valid() )
        {
    ...
    inline void
    releaseGLObjects( osg::Object* object,
                      osg::State*  state = 0 )
    {
        if( object )
        {
    ...
    template<class T>
    void
    releaseGLObjects( const osg::ref_ptr<T>& object,
                      osg::State*            state = 0 )
    {
        if( object.valid() )
        {
