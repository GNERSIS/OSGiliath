...
namespace osg
{
            ...
            case( GL_INTENSITY ) :
                {
                    ...
                    {
                        T v = *data++;
                        operation.rgba( operation.cast( v ),
                                        operation.cast( v ),
                                        operation.cast( v ),
                                        operation.cast( v ) );
                    }
            ...
            case( GL_LUMINANCE ) :
                {
                    for( unsigned int i = 0; i < num; ++i )
                    {
                        operation.luminance( operation.cast( *data++ ) );
                    }
                }
                break;
            case( GL_ALPHA ) :
                {
                    for( unsigned int i = 0; i < num; ++i )
                    {
                        operation.alpha( operation.cast( *data++ ) );
                    }
                }
                break;
            case( GL_LUMINANCE_ALPHA ) :
                {
                    for( unsigned int i = 0; i < num; ++i )
                    {
                        T l = *data++;
                        T a = *data++;
                        operation.luminance_alpha( operation.cast( l ),
                                                   operation.cast( a ) );
                    }
                }
                break;
            case( GL_RGB ) :
                {
                    for( unsigned int i = 0; i < num; ++i )
                    {
                        ...
                        T g = *data++;
                        T b = *data++;
                        operation.rgb( operation.cast( r ),
                                       operation.cast( g ),
                                       operation.cast( b ) );
                    }
                }
            ...
            case( GL_RGBA ) :
                {
                    for( unsigned int i = 0; i < num; ++i )
                    {
                        ...
                        T b = *data++;
                        T a = *data++;
                        operation.rgba( operation.cast( r ),
                                        operation.cast( g ),
                                        operation.cast( b ),
                                        operation.cast( a ) );
                    }
             ...
             O&                   operation )
    {
        ...
        {
            case( GL_BYTE ) :
                _readRow( num, pixelFormat, ( const char* )data, operation );
                break;
            case( GL_UNSIGNED_BYTE ) :
                _readRow( num, pixelFormat, ( const unsigned char* )data, operation );
                break;
            case( GL_SHORT ) :
                _readRow( num, pixelFormat, ( const short* )data, operation );
                break;
            case( GL_UNSIGNED_SHORT ) :
                _readRow( num, pixelFormat, ( const unsigned short* )data, operation );
                break;
            case( GL_INT ) :
                _readRow( num, pixelFormat, ( const int* )data, operation );
                break;
            case( GL_UNSIGNED_INT ) :
                _readRow( num, pixelFormat, ( const unsigned int* )data, operation );
                break;
            case( GL_FLOAT ) :
                _readRow( num, pixelFormat, ( const float* )data, operation );
                break;
            case( GL_DOUBLE ) :
                _readRow( num, pixelFormat, ( const double* )data, operation );
                break;
        }
    }
    ...
    template<class O>
    void
    readImage( const osg::Image* image,
               O&                operation )
    {
        if( !image )
        {
        ...
        for( int r = 0; r < image->r(); ++r )
        {
            ...
            {
                readRow( static_cast<unsigned int>( image->s() ),
    ...
    /** Convenience method for making it easy to cast all pixel channels types to a unit
     * float RGBA form.*/
...
void readImage(const osg::Image* image, O& operation)
{
            ...
            case( GL_LUMINANCE_ALPHA ) :
                {
                    ...
                    {
                        float l = float( *data ) * scale;
                        float a = float( *( data + 1 ) ) * scale;
                        operation.luminance_alpha( l, a );
                        *data++ = T( l * inv_scale );
                        *data++ = T( a * inv_scale );
                    }
                }
            ...
            case( GL_RGBA ) :
                {
                    for( unsigned int i = 0; i < num; ++i )
                    {
                        ...
                        float g = float( *( data + 1 ) ) * scale;
                        float b = float( *( data + 2 ) ) * scale;
                        float a = float( *( data + 3 ) ) * scale;
                        operation.rgba( r, g, b, a );
                        *data++ = T( r * inv_scale );
                        *data++ = T( g * inv_scale );
                        *data++ = T( b * inv_scale );
                        *data++ = T( a * inv_scale );
               ...
               const M&       operation )
    {
        ...
        {
            case( GL_BYTE ) :
                _modifyRow( num, pixelFormat, ( char* )data, 1.0F / 128.0F, operation );
                break;
            case( GL_UNSIGNED_BYTE ) :
                _modifyRow( num,
                            pixelFormat,
            ...
            case( GL_UNSIGNED_INT ) :
                ...
                break;
            case( GL_FLOAT ) :
                _modifyRow( num, pixelFormat, ( float* )data, 1.0F, operation );
                break;
        }
    }
                 ...
                 const M&    operation )
    {
        ...
        for( int r = 0; r < image->r(); ++r )
        {
            for( int t = 0; t < image->t(); ++t )
            {
                modifyRow( static_cast<unsigned int>( image->s() ),
                           image->getPixelFormat(),
                           image->getDataType(),
                           image->data( 0,
                                        static_cast<unsigned int>( t ),
    ...
    extern OSG_EXPORT osg::Image*
                      createImage3D( const ImageList& imageList,
                                     GLenum           desiredPixelFormat,
                                     int              s_maximumImageSize = 1'024,
                                     int              t_maximumImageSize = 1'024,
                                     int              r_maximumImageSize = 1'024,
     ...
     *  The colour across th image is computed from a balance between the center color
     * and the background color controlled by the power of the radius from the center.*/
    extern OSG_EXPORT osg::Image*
    ...
    /** Convert the RGBA values in a Image based on a ColorSpaceOperation defined
     * scheme.*/
    extern OSG_EXPORT osg::Image*
    ...
    /** Create a copy of an osg::Image. converting the origin and orientation to standard
     * lower left OpenGL style origin .*/
    extern OSG_EXPORT osg::Image*
