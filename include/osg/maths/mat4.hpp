...
namespace osg
{
    ...
    struct t_mat4
    {
        public:
            ...
            constexpr t_mat4( value_type v0,
                              value_type v1,
                              value_type v2,
                              value_type v3, /* column 0 */
                              value_type v4,
                              value_type v5,
                              value_type v6,
                              value_type v7, /* column 1 */
                              value_type v8,
                              value_type v9,
                              value_type v10,
                              value_type v11, /* column 2 */
                              value_type v12,
                              value_type v13,
                              value_type v14,
                              value_type v15 ) /* column 3 */ :
                value{
                    { v0,
                     v1,  v2,
                     v3 },
            ...
            constexpr explicit t_mat4( const value_type v[16] ) :
                value{
                    { v[0],
                     v[1],
                     v[2],
    ...
    using mat4   = t_mat4<float>;          /// float (32-bit) 4x4 matrix
    using dmat4  = t_mat4<double>;         /// double (64-bit) 4x4 matrix
    using ldmat4 = t_mat4<long double>;    /// long double 4x4 matrix

    OSG_type_name( osg::mat4 );
    OSG_type_name( osg::dmat4 );
    OSG_type_name( osg::ldmat4 );

    template<typename T>
    bool
    operator==( const t_mat4<T>& lhs,
                const t_mat4<T>& rhs )
    {
        return lhs.value[0] ==
               rhs.value[0] &&
    ...
    operator*( const t_mat4<T>& lhs,
               const t_mat4<T>& rhs )
    {
        return t_mat4<T>( dot( lhs, rhs, 0, 0 ),
                          dot( lhs, rhs, 0, 1 ),
                          dot( lhs, rhs, 0, 2 ),
                          dot( lhs, rhs, 0, 3 ),
                          dot( lhs, rhs, 1, 0 ),
    ...
    /* Right multiplication of a matrix and a plane. This can't be used directly to
       transform a plane from one coordinate system to another using the coordinate
       system's matrix; the inverse transpose of the matrix should be used. */
    template<typename T,
             typename R>
    t_plane<R>
    operator*( const t_mat4<T>&  lhs,
               const t_plane<R>& rhs )
    {
        ...
        return t_plane<T>( transformed[0] * inv,
                           transformed[1] * inv,
                           transformed[2] * inv,
                           transformed[3] * inv );
    }

    /* Left multiplication of a row vector. Equivalent to multiplying the column vector
       by the matrix transpose. */
    template<typename T>
    t_vec4<T>
    operator*( const t_vec4<T>& lhs,
               const t_mat4<T>& rhs )
    {
        return t_vec4<T>( lhs[0] *
                              rhs[0][0] +
                              lhs[1] *
                              rhs[0][1] +
                              lhs[2] *
    ...
    /* Left multiplication of a plane and a matrix. If the matrix is the inverse of the
       local-to-world transform i.e., the world-to-local transform, then this can be used
       directly to transform a plane from a coordinate system's local coordinates to
       world coordinates. */
    template<typename T,
             typename R>
    t_plane<T>
    operator*( const t_plane<T>& lhs,
               const t_mat4<R>&  rhs )
    {
        t_plane<T> transformed( lhs[0] *
                                    rhs[0][0] +
                                    lhs[1] *
                                    rhs[0][1] +
                                    lhs[2] *
    ...
    // Fixed: parenthesization bug from VSG — inv must multiply the entire sum, not just
    // the last term
    template<typename T>
    t_vec3<T>
