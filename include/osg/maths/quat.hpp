...
namespace osg
{
    ...
    struct t_quat
    {
            ...
            constexpr t_quat( value_type in_x,
                              value_type in_y,
                              value_type in_z,
                              value_type in_w ) :
                value{
            ...
            constexpr t_quat( value_type                angle_radians,
                              const t_vec3<value_type>& axis )
            {
                set( angle_radians, axis );
            }

            constexpr t_quat( const t_vec3<value_type>& from,
                              const t_vec3<value_type>& to )
            {
                set( from, to );
            }
                 ...
                 const t_vec3<value_type>& axis )
            {
                ...
                value_type coshalfangle = static_cast<value_type>(
                    cos( numbers<value_type>::half() * angle_radians )
                );
                value_type sinhalfangle = static_cast<value_type>(
                    sin( numbers<value_type>::half() * angle_radians )
                );

                x = static_cast<value_type>( axis.x * sinhalfangle * inversenorm );
                y = static_cast<value_type>( axis.y * sinhalfangle * inversenorm );
                z = static_cast<value_type>( axis.z * sinhalfangle * inversenorm );
                w = coshalfangle;
            }

            void
            set( const t_vec3<value_type>& from,
                 const t_vec3<value_type>& to )
            {
                value_type dot_pd = osg::dot( from, to );
                ...
                value_type len           = static_cast<value_type>( length( axis ) );
            ...
            operator bool() const noexcept
            {
                return value[0] !=
                       ...
                       value[2] !=
                       numbers<value_type>::zero() ||
                       value[3] != numbers<value_type>::zero();
            }
    };

    using quat   = t_quat<float>;          /// float (32-bit) quaternion
    using dquat  = t_quat<double>;         /// double (64-bit) quaternion
    using ldquat = t_quat<long double>;    /// long double quaternion

    OSG_type_name( osg::quat );
    OSG_type_name( osg::dquat );
    OSG_type_name( osg::ldquat );

    template<typename T>
    constexpr bool
    operator==( const t_quat<T>& lhs,
                const t_quat<T>& rhs )
    {
        return lhs[0] ==
               rhs[0] &&
    ...
    template<typename T>
    // Standard Hamilton product: (lhs * rhs) applies rhs first, then lhs
    // Matches column-vector convention where M1 * M2 applies M2 first
    constexpr t_quat<T>
    operator*( const t_quat<T>& lhs,
               const t_quat<T>& rhs )
    {
    ...
    template<typename T>
    constexpr t_quat<T>
    normalize( const t_quat<T>& v )
    {
        T inverse_len = numbers<T>::one() / length( v );
        return t_quat<T>( v[0] * inverse_len,
                          v[1] * inverse_len,
                          v[2] * inverse_len,
                          v[3] * inverse_len );
    }
    ...
    template<typename T>
    constexpr t_quat<T>
    inverse( const t_quat<T>& v )
    {
        t_quat<T> c           = conjugate( v );
        T         inverse_len = numbers<T>::one() / length( v );
        return t_quat<T>( c[0] * inverse_len,
                          c[1] * inverse_len,
                          c[2] * inverse_len,
                          c[3] * inverse_len );
    }
         ...
         T                r )
    {
        ...
        if( ( numbers<T>::one() - cosomega ) > numbers<T>::epsilon() )
        {
            ...
            T sinomega = static_cast<T>( sin( omega ) );
            T scale_from =
                static_cast<T>( sin( ( numbers<T>::one() - r ) * omega ) ) / sinomega;
            T scale_to = static_cast<T>( sin( r * omega ) ) / sinomega;
