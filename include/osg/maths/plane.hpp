...
namespace osg
{
    ...
    struct t_plane
    {
            ...
            constexpr t_plane&
            operator=( const t_plane& ) = default;
            ...
            constexpr t_plane( value_type nx,
                               value_type ny,
                               value_type nz,
                               value_type in_p ) :
                value{
            ...
            constexpr t_plane( const normal_type& normal,
                               value_type         in_p ) :
                value{
                    normal.x,
                    normal.y,
            ...
            constexpr t_plane( const normal_type& position,
    ...
    OSG_type_name( osg::plane );
    OSG_type_name( osg::dplane );
    ...
    /** return true if bounding sphere wholly or partially intersects with convex
     * polytope defined by a list of planes with normals pointing inwards towards center
     * of the polytope. */
    template<class PlaneItr,
             typename T>
    constexpr bool
    intersect( PlaneItr           first,
               PlaneItr           last,
               const t_sphere<T>& s )
    {
        ...
        {
            if( distance( *itr, s.center ) < negative_radius )
            {
                return false;
            }
        }
    ...
    template<class Polytope,
             typename T>
    constexpr bool
    intersect( const Polytope&    polytope,
               const t_sphere<T>& s )
    {
            ...
            T                epsilon = 1E-10 )
    {
        ...
        for( auto itr = first; itr != last; ++itr )
        {
            if( distance( *itr, v ) < negative_epsilon )
            {
                return false;
            }
        }
    ...
    template<class Polytope,
             typename T>
    constexpr bool
    inside( const Polytope&  polytope,
            const t_vec3<T>& v,
            T                epsilon = 1E-10 )
    {
