...
namespace osg
{
    ...
    struct t_vec3
    {
            ...
            constexpr t_vec3&
            operator=( const t_vec3& ) = default;

            constexpr t_vec3( value_type in_x,
                              value_type in_y,
                              value_type in_z ) :
                value{
                    in_x,
            ...
            template<typename R>
            constexpr t_vec3( const t_vec2<R>& v,
                              value_type       in_z ) :
                value{
                    v.x,
                    v.y,
            ...
            operator bool() const noexcept
            {
                return value[0] !=
                       ...
                       value[1] !=
                       numbers<value_type>::zero() ||
                       value[2] != numbers<value_type>::zero();
            }
    };

    using vec3   = t_vec3<float>;          // float (32-bit) 3D vector
    using dvec3  = t_vec3<double>;         // double (64-bit) 3D vector
    ...
    OSG_type_name( osg::vec3 );
    OSG_type_name( osg::dvec3 );
    OSG_type_name( osg::ldvec3 );
    OSG_type_name( osg::bvec3 );
    OSG_type_name( osg::svec3 );
    OSG_type_name( osg::ivec3 );
    OSG_type_name( osg::ubvec3 );
    OSG_type_name( osg::usvec3 );
    OSG_type_name( osg::uivec3 );
    ...
    /// Legacy cross product operator — prefer osg::cross(a, b)
    template<typename T>
    constexpr t_vec3<T>
    operator^( const t_vec3<T>& lhs,
               const t_vec3<T>& rhs )
    ...
    template<typename T>
    constexpr t_vec3<T>
    orthogonal( const t_vec3<T>& v )
    {
        // use the cross product against the axis which is the most orthogonal to the
        // input vector.
        auto abs_x = fabs( v.x );
        auto abs_y = fabs( v.y );
        auto abs_z = fabs( v.z );
        if( abs_x < abs_y )
        {
            if( abs_x < abs_z )
            {
                return {
                    ...
                    v.z,
                    -v.y
                };    // v.x shortest, use cross with x axis
            }
        }
        else if( abs_y < abs_z )
        {
            return {
                ...
                numbers<T>::zero(),
                v.x
            };    // v.y shortest, use cross with y axis
        }
        return {
            v.y,
            -v.x,
            numbers<T>::zero()
        };    // v.z shortest, use cross with z axis
    }
