...
namespace osg
{
    ...
    struct t_vec4
    {
            ...
            constexpr t_vec4&
            operator=( const t_vec4& ) = default;

            constexpr t_vec4( value_type in_x,
                              value_type in_y,
                              value_type in_z,
                              value_type in_w ) :
                value{
            ...
            template<typename R>
            constexpr t_vec4( const t_vec2<R>& v,
                              value_type       in_z,
                              value_type       in_w ) :
                value{
                    static_cast<T>( v.x ),
            ...
            template<typename R>
            constexpr t_vec4( const t_vec3<R>& v,
                              value_type       in_w ) :
                value{
                    static_cast<T>( v.x ),
                    static_cast<T>( v.y ),
            ...
            friend constexpr t_vec4<T>
            operator*( const t_vec4<T>& lhs,
                       T                rhs )
            {
                return t_vec4<T>( lhs[0] * rhs,
                                  lhs[1] * rhs,
                                  lhs[2] * rhs,
                                  lhs[3] * rhs );
            }
    ...
    using vec4   = t_vec4<float>;          // float (32-bit) 4D vector
    using dvec4  = t_vec4<double>;         // double (64-bit) 4D vector
    ...
    OSG_type_name( osg::vec4 );
    OSG_type_name( osg::dvec4 );
    OSG_type_name( osg::ldvec4 );
    OSG_type_name( osg::bvec4 );
    OSG_type_name( osg::svec4 );
    OSG_type_name( osg::ivec4 );
    OSG_type_name( osg::ubvec4 );
    OSG_type_name( osg::usvec4 );
    OSG_type_name( osg::uivec4 );
