...
namespace osg
{
    ...
    class OSG_EXPORT ContextData : public GraphicsObjectManager
    {
        public:
            ...
            void
            setNumContexts( unsigned int numContexts )
            {
                _numContexts = numContexts;
            }

            unsigned int
            getNumContexts() const
            {
                return _numContexts;
            }

            void
            setCompileContext( osg::GraphicsContext* gc )
            {
                _compileContext = gc;
            }
        ...
        protected:
            ...
            // ManagerMap contains GL Extentsions objects used by StateAttribue to call
    ...
    extern OSG_EXPORT ContextData*
    getContextData( unsigned int contextID );

    /** Get or create the ContextData for a specific contextID.*/
    extern OSG_EXPORT ContextData*
    getOrCreateContextData( unsigned int contextID );

    template<typename T>
    inline T*
    get( unsigned int contextID )
    {
    ...
    template<>
    inline ContextData*
    get<ContextData>( unsigned int contextID )
    {
        return getOrCreateContextData( contextID );
    }
