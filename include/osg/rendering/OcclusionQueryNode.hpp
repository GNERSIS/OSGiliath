...
namespace osg
{
    ...
    class OSG_EXPORT QueryGeometry : public osg::Geometry
    {
        public:
            ...
            /** return a QueryResult for specified Camera, where the QueryResult.valid is
             * true when query results are available, and in which case the
             * QueryResult.numPixels provides the num of pixels in the query result.*/
            QueryResult
            getQueryResult( const osg::Camera* cam ) const;

            unsigned int
            getNumPixels( const osg::Camera* cam ) const;

            virtual void
            releaseGLObjects( osg::State* state = 0 ) const;

            static void
            deleteQueryObject( unsigned int contextID,
                               GLuint       handle );
            static void
            ...
            static void
            discardDeletedQueryObjects( unsigned int contextID );

        protected:

            typedef std::map<const osg::Camera*, osg::ref_ptr<osg::TestResult>>
                               ResultMap;
    ...
    class OSG_EXPORT OcclusionQueryNode
        : public osg::Inherit<osg::Group, OcclusionQueryNode>
    {
        public:
            ...
            OcclusionQueryNode( const OcclusionQueryNode& oqn,
            ...
            void
            setQueriesEnabled( bool enable = true );

            bool
            getQueriesEnabled() const
            {
                return _enabled;
            }
            ...
            void
            setVisibilityThreshold( unsigned int pixels )
            {
                _visThreshold = pixels;
            }

            unsigned int
            getVisibilityThreshold() const
            {
                return _visThreshold;
            }

            // Specifies how many frames to wait before issuing another query.
            void
            setQueryFrameCount( unsigned int frames )
            {
                _queryFrameCount = frames;
            }

            unsigned int
            getQueryFrameCount() const
            {
            ...
            void
            setDebugDisplay( bool enable );
            bool
            getDebugDisplay() const;
            ...
            void
            setQueryStateSet( osg::StateSet* ss );
            osg::StateSet*
            getQueryStateSet();
            const osg::StateSet*
            getQueryStateSet() const;
            ...
            void
            setQueryGeometry( osg::QueryGeometry* geom );
            const osg::QueryGeometry*
            getQueryGeometry() const;

            // Set and get the StateSet used by the OcclusionQueryNode
            //   when rendering the debug query geometry (see setDebugDisplay).
            void
            setDebugStateSet( osg::StateSet* ss );
            osg::StateSet*
            getDebugStateSet();
            const osg::StateSet*
            getDebugStateSet() const;
            ...
            virtual bool
            getPassed( const osg::Camera* camera,
                       osg::NodeVisitor&  nv );
            void
            traverseQuery( const osg::Camera* camera,
                           osg::NodeVisitor&  nv );
            void
            traverseDebug( osg::NodeVisitor& nv );
            ...
            // Note, unlike flush no OpenGL calls are made, instead the handles are all
            // removed. this call is useful for when an OpenGL context has been
            // destroyed.
            static void
            discardDeletedQueryObjects( unsigned int contextID );
        ...
        protected:
            ...
            bool
            isQueryGeometryValid() const
            {
                return _queryGeometryState != INVALID;
            }

            void
            setQueryGeometryInternal( osg::QueryGeometry* queryGeom,
            ...
            mutable std::mutex                                 _frameCountMutex;
            ...
            mutable std::mutex                                 _computeBoundMutex;
    };
