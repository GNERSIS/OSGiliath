...
namespace osg
{
    ...
    class EllipsoidModel : public osg::Inherit<Object, EllipsoidModel>
    {
        public:
            ...
            void
            setRadiusEquator( double radius )
            {
                _radiusEquator = radius;
                computeCoefficients();
            }

            double
            getRadiusEquator() const
            {
                return _radiusEquator;
            }

            void
            setRadiusPolar( double radius )
            {
                _radiusPolar = radius;
                computeCoefficients();
            }
            ...
            double
            getRadiusPolar() const
            {
                return _radiusPolar;
            }

            inline void
            convertLatLongHeightToXYZ( double  latitude,
                                       double  longitude,
                                       double  height,
                                       double& X,
                                       double& Y,
            ...
            inline void
            convertXYZToLatLongHeight( double  X,
                                       double  Y,
                                       double  Z,
                                       double& latitude,
                                       double& longitude,
            ...
            inline void
            computeLocalToWorldTransformFromXYZ( double      X,
                                                 double      Y,
                                                 double      Z,
                                                 osg::dmat4& localToWorld ) const;

            inline void
            computeCoordinateFrame( double      latitude,
                                    double      longitude,
                                    osg::dmat4& localToWorld ) const;

            inline osg::dvec3
            computeLocalUpVector( double X,
                                  double Y,
                                  double Z ) const;

            // Convenience method for determining if EllipsoidModel is a stock WGS84
            // ellipsoid
            inline bool
            isWGS84() const
            {
            ...
            friend bool
            operator==( const EllipsoidModel& e1,
                        const EllipsoidModel& e2 )
            {
                return ( e1._radiusEquator ==
                         e2._radiusEquator &&
                         e1._radiusPolar == e2._radiusPolar );
            }
    ...
    typedef dmat4 CoordinateFrame;

    /** CoordinateSystem encapsulate the coordinate system that is associated with
       objects in a scene. For an overview of common earth bases coordinate systems see
       http://www.colorado.edu/geography/gcraft/notes/coordsys/coordsys_f.html */
    class OSG_EXPORT CoordinateSystemNode
        : public osg::Inherit<Group, CoordinateSystemNode>
    {
        public:
            ...
            CoordinateSystemNode( const std::string& format,
                                  const std::string& cs );

            /** Copy constructor using CopyOp to manage deep vs shallow copy.*/
            CoordinateSystemNode( const CoordinateSystemNode&,
            ...
            void
            set( const CoordinateSystemNode& csn );
            ...
            void
            setEllipsoidModel( EllipsoidModel* ellipsode )
            {
            ...
            CoordinateFrame
            computeLocalCoordinateFrame( const dvec3& position ) const;

            /** Compute the local up-vector for specified point.*/
            osg::dvec3
            computeLocalUpVector( const dvec3& position ) const;
    ...
    //
    inline void
    EllipsoidModel::convertLatLongHeightToXYZ( double  latitude,
                                               double  longitude,
                                               double  height,
                                               double& X,
                                               double& Y,
    ...
    inline void
    EllipsoidModel::convertXYZToLatLongHeight( double  X,
                                               double  Y,
                                               double  Z,
                                               double& latitude,
                                               double& longitude,
    ...
    inline void
    EllipsoidModel::computeLocalToWorldTransformFromLatLongHeight(
        double      latitude,
        double      longitude,
        double      height,
        osg::dmat4& localToWorld
    ) const
    {
        ...
        localToWorld = osg::translate( X, Y, Z );
        computeCoordinateFrame( latitude, longitude, localToWorld );
    }

    inline void
    EllipsoidModel::computeLocalToWorldTransformFromXYZ( double      X,
                                                         double      Y,
                                                         double      Z,
                                                         osg::dmat4& localToWorld ) const
    {
    ...
    inline void
    EllipsoidModel::computeCoordinateFrame( double      latitude,
                                            double      longitude,
                                            osg::dmat4& localToWorld ) const
    {
        // Compute up vector
        osg::dvec3 up( cos( longitude ) * cos( latitude ),
                       sin( longitude ) * cos( latitude ),
                       sin( latitude ) );

        // Compute east vector
        osg::dvec3 east( -sin( longitude ), cos( longitude ), 0 );

        // Compute north vector = outer product up x east
        osg::dvec3 north = osg::cross( up, east );
    ...
    inline osg::dvec3
    EllipsoidModel::computeLocalUpVector( double X,
                                          double Y,
                                          double Z ) const
    {
        ...
        // Compute up vector
        return osg::dvec3( cos( longitude ) * cos( latitude ),
                           sin( longitude ) * cos( latitude ),
                           sin( latitude ) );
    }
