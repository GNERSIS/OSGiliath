...
namespace osg
{
    ...
    /** Camera - is a subclass of Transform which represents encapsulates the settings of
     * a Camera.
     */
    class OSG_EXPORT Camera : public osg::Inherit<Transform, Camera>,
                              public CullSettings
    {
        public:
            ...
            void
            setAllowEventFocus( bool focus )
            {
                _allowEventFocus = focus;
            }

            /** Get whether this camera allows events to be generated by the
             * associated graphics window to be associated with this camera. */
            bool
            getAllowEventFocus() const
            {
                return _allowEventFocus;
            }
            ...
            void
            setColorMask( osg::ColorMask* colorMask );

            /** Set the color mask of the camera to specified values. */
            void
            setColorMask( bool red,
                          bool green,
                          bool blue,
                          bool alpha );
             ...
             * transformation.*/
            void
            setTransformOrder( TransformOrder order )
            {
            ...
            /** Set the policy used to determine if and how the projection matrix should
             * be adjusted on window resizes. */
            inline void
            setProjectionResizePolicy( ProjectionResizePolicy policy )
            {
                _projectionResizePolicy = policy;
            }

            /** Get the policy used to determine if and how the projection matrix should
             * be adjusted on window resizes. */
            inline ProjectionResizePolicy
            getProjectionResizePolicy() const
            {
                return _projectionResizePolicy;
            }
            ...
            void
            setViewMatrixAsLookAt( const osg::dvec3& eye,
                                   const osg::dvec3& center,
                                   const osg::dvec3& up );

            /** Get to the position and orientation of a modelview matrix, using the same
             * convention as gluLookAt. */
            void
            getViewMatrixAsLookAt( osg::dvec3& eye,
                                   osg::dvec3& center,
                                   osg::dvec3& up,
            ...
            /** Set the rendering order of this camera's subgraph relative to any camera
             * that this subgraph is nested within. For rendering to a texture, one
             * typically uses PRE_RENDER. For Head Up Displays, one would typically use
             * POST_RENDER.*/
            void
            setRenderOrder( RenderOrder order,
                            int         orderNum = 0 )
            {
                _renderOrder    = order;
                _renderOrderNum = orderNum;
            }

            /** Get the rendering order of this camera's subgraph relative to any camera
             * that this subgraph is nested within.*/
            RenderOrder
            getRenderOrder() const
            {
                return _renderOrder;
            }
            ...
            void
            setRenderTargetImplementation( RenderTargetImplementation impl );
            ...
            getRenderTargetImplementation() const
            {
                return _renderTargetImplementation;
            }

            /** Get the render target fallback.*/
            RenderTargetImplementation
            getRenderTargetFallback() const
            {
                return _renderTargetFallback;
            }

            /** Set the draw buffer used at the start of each frame draw.
             * Note, a buffer value of GL_NONE is used to specify that the rendering
             * back-end should choose the most appropriate buffer.*/
            void
            setDrawBuffer( GLenum buffer )
            {
                _drawBuffer = buffer;
                applyMaskAction( DRAW_BUFFER );
            }
             ...
             * attached. The face parameter controls the face of texture cube map or z
             * level of 3d texture. The mipMapGeneration flag controls whether mipmap
            ...
            const BufferAttachmentMap&
            getBufferAttachmentMap() const
            {
                return _bufferAttachmentMap;
            }

            /** Increment the _attachementMapModifiedCount so that the rendering backend
             * will know that it needs to be updated to handle any new settings (such as
             * format change/resizes.).*/
            void
            dirtyAttachmentMap()
            {
            ...
            void
            setAttachmentMapModifiedCount( unsigned int v )
            {
                _attachmentMapModifiedCount = v;
            }

            /** Get the AttachmentMapModifiedCount. */
            unsigned int
            getAttachmentMapModifiedCount() const
            {
                return _attachmentMapModifiedCount;
            }

            /** Resize the image and textures in the AttachementMap.*/
            void
            resizeAttachments( int width,
                               int height );
            ...
            /** Resize, to the specified width and height, the viewport, attachments and
             * projection matrix according to the resizeMask provided. Note, the
             * adjustment of the projection matrix is done if the RESIZE_PROJECTIONMATRIX
             * mask to set and according to the rules specified in the
            ...
            /** Explicit control over implicit allocation of buffers when using FBO.
             Implicit buffers are automatically substituted when user have not attached
             such buffer.

             Camera may set up two FBOs: primary Render FBO and secondary Resolve FBO for
             multisample usage. So in practice we have two masks defined for the Camera:
                implicitBufferAttachmentRenderMask
                implicitBufferAttachmentResolveMask
             They can be set together by setImplicitBufferAttachmentMask method, or
             separately by setImplicitBufferAttachmentRenderMask and
             setImplicitBufferAttachmentResolveMask.

             Camera defaults are USE_DISPLAY_SETTINGS_MASK which means that by default
             Camera chooses to substitute buffer attachments as defined by
             DisplaySettings.

             Usually DisplaySettings implicit buffer attachment selection defaults to:
             DEPTH and COLOR for both primary (Render) FBO and seconday Multisample
             (Resolve) FBO ie: IMPLICIT_DEPTH_BUFFER_ATTACHMENT |
             IMPLICIT_COLOR_BUFFER_ATTACHMENT

             If these masks are not changed and user did not attach depth buffer and/or
             color buffer to Camera, then OSG implicitly substitutes these buffers. By
             default it does not implicitly allocate a stencil buffer. Use implicit
             buffer attachment  masks to override default behavior: to turn off DEPTH or
             COLOR buffer substitution or to enforce STENCIL buffer substitution.
            ...
            */
            ...
            if effectiveMask parameter is set, method follows USE_DISPLAY_SETTINGS_MASK
            dependence and returns effective mask if effectiveMask parameter is reset,
            method returns nominal mask set by the Camera
            */
            ImplicitBufferAttachmentMask
            getImplicitBufferAttachmentRenderMask( bool effectiveMask ) const
            {
                if( effectiveMask &&
                    _implicitBufferAttachmentRenderMask == USE_DISPLAY_SETTINGS_MASK )
                {
                    const DisplaySettings* ds = _displaySettings.valid()
            ...
            effectiveMask parameter is reset, method returns nominal mask set by the
            Camera
            */
            ImplicitBufferAttachmentMask
            getImplicitBufferAttachmentResolveMask( bool effectiveMask ) const
            {
                if( effectiveMask &&
            ...
            /** Get the const Rendering object that is used to implement rendering of the
             * subgraph.*/
            const osg::GraphicsOperation*
            getRenderer() const
            {
                return _renderer.get();
            }

            /** Set the Rendering cache that is used for cached objects associated with
             * rendering of subgraphs.*/
            void
            ...
            /** Get the Rendering cache that is used for cached objects associated with
             * rendering of subgraphs.*/
            osg::Object*
            getRenderingCache()
            {
                return _renderingCache.get();
            }

            /** Get the const Rendering cache that is used for cached objects associated
             * with rendering of subgraphs.*/
            const osg::Object*
            getRenderingCache() const
            {
                return _renderingCache.get();
            }

            /** Draw callback for custom operations.*/
            struct OSG_EXPORT DrawCallback : osg::Inherit<Callback, DrawCallback>
            {
                    DrawCallback()
                    {
                    }
                    ...
                    inline void
                    run( osg::RenderInfo& renderInfo ) const
                    {
                        operator()( renderInfo );

                        if( _nestedCallback.valid() )
                        {
                            ( ( const DrawCallback* )_nestedCallback.get() )
                                ->run( renderInfo );
                        }
                    }
                     ...
                     * releases OpenGL objexts for all graphics contexts. */
                    virtual void
            ...
            /** Set the initial draw callback for custom operations to be done before the
             * drawing of the camera's subgraph and pre render stages.*/
            void
            setInitialDrawCallback( DrawCallback* cb )
            {
                _initialDrawCallback = cb;
            }
            ...
            {
                return _initialDrawCallback.get();
            }

            /** Get the const initial draw callback.*/
            const DrawCallback*
            getInitialDrawCallback() const
            {
                return _initialDrawCallback.get();
            }
            ...
            {
                return osg::Callback::findNestedCallback<T>(
                    _initialDrawCallback.get()
                );
            }
            ...
            /** Convenience method that sets DrawCallback Initial callback of the node if
             * it doesn't exist, or nest it into the existing one. */
            inline void
            addInitialDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL )
                {
                    if( _initialDrawCallback.valid() )
                    {
                        _initialDrawCallback->addNestedCallback( nc );
                    }
                    else
                    {
                        setInitialDrawCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            addInitialDrawCallback( const ref_ptr<T>& nc )
            {
                addInitialDrawCallback( nc.get() );
            }
            ...
            inline void
            removeInitialDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL && _initialDrawCallback.valid() )
                {
                    if( _initialDrawCallback == nc )
                    {
                        ref_ptr<DrawCallback> new_nested_callback =
                            ( DrawCallback* )nc->getNestedCallback();
                        nc->setNestedCallback( 0 );
                        setInitialDrawCallback( new_nested_callback.get() );
                    }
                    else
                    {
                        _initialDrawCallback->removeNestedCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            removeInitialDrawCallback( const ref_ptr<T>& nc )
            {
                removeInitialDrawCallback( nc.get() );
            }

            /** Set the pre draw callback for custom operations to be done before the
             * drawing of the camera's subgraph but after any pre render stages have been
             * completed.*/
            void
            ...
            getPreDrawCallback()
            {
                return _preDrawCallback.get();
            }

            /** Get the const pre draw callback.*/
            const DrawCallback*
            getPreDrawCallback() const
            {
                return _preDrawCallback.get();
            }
            ...
            findPreDrawCallback()
            {
                return osg::Callback::findNestedCallback<T>( _preDrawCallback.get() );
            }

            /** Convenience method to find a nested pre draw callback by type. */
            template<typename T>
            const T*
            findPreDrawCallback() const
            {
                return osg::Callback::findNestedCallback<T>( _preDrawCallback.get() );
            }
             ...
             * it doesn't exist, or nest it into the existing one. */
            inline void
            addPreDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL )
                {
                    if( _preDrawCallback.valid() )
                    {
                        _preDrawCallback->addNestedCallback( nc );
                    }
                    else
                    {
                        setPreDrawCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            addPreDrawCallback( const ref_ptr<T>& nc )
            {
                addPreDrawCallback( nc.get() );
            }
             ...
             * callback is not found. */
            inline void
            removePreDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL && _preDrawCallback.valid() )
                {
                    ...
                    {
                        _preDrawCallback->removeNestedCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            removePreDrawCallback( const ref_ptr<T>& nc )
            {
                removePreDrawCallback( nc.get() );
            }
             ...
             * have been completed.*/
            void
            setPostDrawCallback( DrawCallback* cb )
            {
            ...
            getPostDrawCallback()
            {
                return _postDrawCallback.get();
            }

            /** Get the const post draw callback.*/
            const DrawCallback*
            getPostDrawCallback() const
            {
                return _postDrawCallback.get();
            }
            ...
            {
                return osg::Callback::findNestedCallback<T>( _postDrawCallback.get() );
            }

            /** Convenience method to find a nested post draw callback by type. */
            template<typename T>
            const T*
            findPostDrawCallback() const
            {
                return osg::Callback::findNestedCallback<T>( _postDrawCallback.get() );
            }
             ...
             * it doesn't exist, or nest it into the existing one. */
            inline void
            addPostDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL )
                {
                    if( _postDrawCallback.valid() )
                    {
                        _postDrawCallback->addNestedCallback( nc );
                    }
                    else
                    {
                        setPostDrawCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            addPostDrawCallback( const ref_ptr<T>& nc )
            {
                addPostDrawCallback( nc.get() );
            }
             ...
             * callback is not found. */
            inline void
            removePostDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL && _postDrawCallback.valid() )
                {
                    ...
                    {
                        _postDrawCallback->removeNestedCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            removePostDrawCallback( const ref_ptr<T>& nc )
            {
                removePostDrawCallback( nc.get() );
            }
             ...
             * been completed.*/
            void
            setFinalDrawCallback( DrawCallback* cb )
            {
                _finalDrawCallback = cb;
            }
            ...
            {
                return _finalDrawCallback.get();
            }

            /** Get the const final draw callback.*/
            const DrawCallback*
            getFinalDrawCallback() const
            {
                return _finalDrawCallback.get();
            }
            ...
            {
                return osg::Callback::findNestedCallback<T>( _finalDrawCallback.get() );
            }

            /** Convenience method to find a nested final draw callback by type. */
            template<typename T>
            const T*
            findFinalDrawCallback() const
            {
                return osg::Callback::findNestedCallback<T>( _finalDrawCallback.get() );
            }
             ...
             * it doesn't exist, or nest it into the existing one. */
            inline void
            addFinalDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL )
                {
                    if( _finalDrawCallback.valid() )
                    {
                        _finalDrawCallback->addNestedCallback( nc );
                    }
                    else
                    {
                        setFinalDrawCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            addFinalDrawCallback( const ref_ptr<T>& nc )
            {
                addFinalDrawCallback( nc.get() );
            }
             ...
             * callback is not found. */
            inline void
            removeFinalDrawCallback( DrawCallback* nc )
            {
                if( nc != NULL && _finalDrawCallback.valid() )
                {
                    ...
                    {
                        _finalDrawCallback->removeNestedCallback( nc );
                    }
                }
            }
            ...
            template<class T>
            void
            removeFinalDrawCallback( const ref_ptr<T>& nc )
            {
                removeFinalDrawCallback( nc.get() );
            }

            std::mutex*
            getDataChangeMutex() const
            {
                return &_dataChangeMutex;
            }
        ...
        protected:
            ...
            mutable std::mutex                 _dataChangeMutex;
    ...
    /** Functor to assist with ordering cameras in the order they should be rendered
     * in.*/
    struct CameraRenderOrderSortOp
    {
            inline bool
            operator()( const Camera* lhs,
                        const Camera* rhs ) const
            {
                if( lhs->getRenderOrder() < rhs->getRenderOrder() )
                {
                    return true;
                }
                if( rhs->getRenderOrder() < lhs->getRenderOrder() )
                {
                    return false;
                }
