...
namespace osg
{
    ...
    /** PagedLOD.
     */
    class OSG_EXPORT PagedLOD : public osg::Inherit<LOD, PagedLOD>
    {
        public:
             ...
             *          This is a result of the differences in determining the node's
             * center.
             * @note Though the bounding spheres of both nodes with their default
             * settings will not differ if they have same subgraph.
             * @sa LOD::setCenter, LOD::setRadius */
            PagedLOD();

            /** Copy constructor using CopyOp to manage deep vs shallow copy.*/
            PagedLOD( const PagedLOD&,
            ...
            addChild( const ref_ptr<T>& child,
                      ...
                      float             rmax )
            {
                return addChild( child.get(), rmin, rmax );
            }
            ...
            removeChildren( unsigned int pos,
                            unsigned int numChildrenToRemove = 1 ) override;
            ...
            struct OSG_EXPORT PerRangeData
            {
                    ...
                    PerRangeData&
                                                  operator=( const PerRangeData& prd );
            ...
            void
            setPriorityOffset( unsigned int childNo,
            ...
            float
            getPriorityOffset( unsigned int childNo ) const
            {
                return _perRangeDataList[childNo]._priorityOffset;
            }

            unsigned int
            getNumPriorityOffsets() const
            {
                return static_cast<unsigned int>( _perRangeDataList.size() );
            }

            void
            setPriorityScale( unsigned int childNo,
                              float        priorityScale )
            {
                expandPerRangeDataTo( childNo );
            ...
            float
            getPriorityScale( unsigned int childNo ) const
            {
                return _perRangeDataList[childNo]._priorityScale;
            }

            unsigned int
            getNumPriorityScales() const
            {
                return static_cast<unsigned int>( _perRangeDataList.size() );
            }
            ...
            void
            setMinimumExpiryTime( unsigned int childNo,
                                  double       minTime )
            {
                expandPerRangeDataTo( childNo );
                _perRangeDataList[childNo]._minExpiryTime = minTime;
            }

            double
            getMinimumExpiryTime( unsigned int childNo ) const
            {
                return _perRangeDataList[childNo]._minExpiryTime;
            }

            unsigned int
            getNumMinimumExpiryTimes() const
            {
                return static_cast<unsigned int>( _perRangeDataList.size() );
            }

            /** Sets the minimum number of frames that must be rendered without a child
             * being traversed before it can be expired. */
            void
            setMinimumExpiryFrames( unsigned int childNo,
                                    unsigned int minFrames )
            {
                expandPerRangeDataTo( childNo );
                _perRangeDataList[childNo]._minExpiryFrames = minFrames;
            }

            unsigned int
            getMinimumExpiryFrames( unsigned int childNo ) const
            {
                return _perRangeDataList[childNo]._minExpiryFrames;
            }

            unsigned int
            getNumMinimumExpiryFrames() const
            {
                return static_cast<unsigned int>( _perRangeDataList.size() );
            }

            void
            setTimeStamp( unsigned int childNo,
                          double       timeStamp )
            {
                expandPerRangeDataTo( childNo );
                _perRangeDataList[childNo]._timeStamp = timeStamp;
            }

            double
            getTimeStamp( unsigned int childNo ) const
            {
                return _perRangeDataList[childNo]._timeStamp;
            }

            unsigned int
            getNumTimeStamps() const
            {
                return static_cast<unsigned int>( _perRangeDataList.size() );
            }

            void
            setFrameNumber( unsigned int childNo,
                            unsigned int frameNumber )
            {
                expandPerRangeDataTo( childNo );
                _perRangeDataList[childNo]._frameNumber = frameNumber;
            }

            unsigned
            getFrameNumber( unsigned int childNo ) const
            {
                return _perRangeDataList[childNo]._frameNumber;
            }

            unsigned int
            getNumFrameNumbers() const
            {
                return static_cast<unsigned int>( _perRangeDataList.size() );
            }

            /** Return the DatabaseRequest object used by the DatabasePager to keep track
             * of file load requests being carried on behalf of the DatabasePager. Note,
             * in normal OSG usage you should not set this value yourself, as this will
             * be managed by the osgDB::DatabasePager.*/
            osg::ref_ptr<osg::Referenced>&
            getDatabaseRequest( unsigned int childNo )
            {
                return _perRangeDataList[childNo]._databaseRequest;
            }
             ...
             * traversed. Note, this frame number is automatically set by the traverse()
             * method for all traversals (update, cull etc.).
             */
            inline void
            setFrameNumberOfLastTraversal( unsigned int frameNumber )
            {
             ...
             * traversed.*/
            inline unsigned int
            getFrameNumberOfLastTraversal() const
            {
                return _frameNumberOfLastTraversal;
            }

            /** Set the number of children that the PagedLOD must keep around, even if
             * they are older than their expiry time.*/
            inline void
            setNumChildrenThatCannotBeExpired( unsigned int num )
            {
             ...
             * they are older than their expiry time.*/
            unsigned int
            getNumChildrenThatCannotBeExpired() const
            {
                return _numChildrenThatCannotBeExpired;
            }

            /** Set whether you want to disable the paging in of external nodes.*/
            void
            setDisableExternalChildrenPaging( bool flag )
            {
                _disableExternalChildrenPaging = flag;
            }

            bool
            getDisableExternalChildrenPaging() const
            {
                return _disableExternalChildrenPaging;
            }

            /** Remove the children from the PagedLOD which haven't been visited since
             * specified expiry time and expiry frame number. The removed children are
             * added to the removeChildren list passed into the method, this allows the
             * children to be deleted later at the caller's discretion. Return true if
             * children are removed, false otherwise. */
            virtual bool
            removeExpiredChildren( double       expiryTime,
        ...
        protected:
            ...
            void
                                expandPerRangeDataTo( unsigned int pos );
