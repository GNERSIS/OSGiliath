...
namespace osg
{
    ...
    */
    class OSG_EXPORT Sequence : public osg::Inherit<Group, Sequence>
    {
        public:
            ...
            insertChild( unsigned int      index,
                         ...
                         double            t )
            {
                return insertChild( index, child.get(), t );
            }

            virtual bool
            removeChild( Node* child ) override;
            ...
            void
            setTime( unsigned int frame,
                     double       t );

            /** Get time for child. */
            double
            getTime( unsigned int frame ) const;

            /** Set the time list for children. */
            void
            setTimeList( const std::vector<double>& timeList )
            {
                _frameTime = timeList;
            }
            ...
            /** Set default time in seconds for new child.
                if t<0, t=0 */
            void
            setDefaultTime( double t )
            {
                _defaultTime = ( t < 0. ? 0. : t );
            }

            /** Get default time in seconds for new child. */
            double
            getDefaultTime( void ) const
            {
                return _defaultTime;
            };
            ...
            /** Set time of last frame of last loop, in seconds.
                if t<= 0, then ignored */
            void
            setLastFrameTime( double t )
            {
                _lastFrameTime = ( t < 0. ? 0. : t );
            }

            /** Get last frame time in seconds */
            double
            getLastFrameTime( void ) const
            {
            ...
            getNumFrames() const
            {
                return static_cast<unsigned int>( _frameTime.size() );
            }
            ...
            void
            setInterval( LoopMode mode,
                         int      begin,
                         int      end );

            /** Get sequence mode & interval. */
            inline void
            getInterval( LoopMode& mode,
                         int&      begin,
                         int&      end ) const
            {
                mode  = _loopMode;
            ...
            void
            setDuration( float speed,
                         int   nreps = -1 );

            /** Get duration & number of repeats. */
            inline void
            getDuration( float& speed,
                         int&   nreps ) const
            {
                speed = _speed;
                nreps = _nreps;
            ...
            void
            setClearOnStop( bool clearOnStop )
            {
                _clearOnStop = clearOnStop;
            }

            /** Get whether to show no child nodes after stopping */
            bool
            getClearOnStop() const
            {
                return _clearOnStop;
            }
