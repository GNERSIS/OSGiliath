...
namespace osgSim
{
    ...
    /** OverlayNode is for creating texture overlays on scenes, with the overlay texture
     * being generated by pre rendering an Overlay Subgraph to a texture, then projecting
     * this resulting texture on the scene.*/
    class OSGSIM_EXPORT OverlayNode : public osg::Inherit<osg::Group, OverlayNode>
    {
        public:
            ...
            OverlayNode( OverlayTechnique technique =
                             OBJECT_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY );

            OverlayNode( const OverlayNode& es,
            ...
            void
            setOverlayTechnique( OverlayTechnique technique );

            OverlayTechnique
            getOverlayTechnique() const
            {
                return _overlayTechnique;
            }
            ...
            void
            setOverlaySubgraph( osg::Node* node );

            template<class T>
            void
            setOverlaySubgraph( const osg::ref_ptr<T>& node )
            {
                setOverlaySubgraph( node.get() );
            }
            ...
            /** Set whether the OverlayNode should update the overlay texture on every
             * frame.*/
            void
            setContinuousUpdate( bool update )
            {
                _continuousUpdate = update;
            }

            /** Get whether the OverlayNode should update the overlay texture on every
             * frame.*/
            bool
            getContinuousUpdate() const
            {
                return _continuousUpdate;
            }

            /** Set the base height that the overlay subgraph will be projected down to.
             * Normally you'll set this to just below ground level, if you set it too
             * high then the overlay texture can end up being clipped in certain viewing
             * directions, while if its too low then there will be a limit to how close
             * you can get to the terrain before pixaltion becomes an issue.*/
            void
            setOverlayBaseHeight( double baseHeight )
            {
                _overlayBaseHeight = baseHeight;
            }

            /** Get the base height that the overlay subgraph will be projected down
             * to.*/
            double
            getOverlayBaseHeight() const
            {
                return _overlayBaseHeight;
            }

            /** Set the clear color to use when rendering the overlay subgraph.*/
            void
            setOverlayClearColor( const osg::vec4& color )
            {
                _overlayClearColor = color;
            }

            /** Get the clear color to use when rendering the overlay subgraph.*/
            const osg::vec4&
            getOverlayClearColor() const
            {
                return _overlayClearColor;
            }

            /** Set the texture unit that the texture should be assigned to.*/
            void
            setOverlayTextureUnit( unsigned int unit );

            /** Get the texture unit that the texture should be assigned to.*/
            unsigned int
            getOverlayTextureUnit() const
            {
                return _textureUnit;
            }

            /** Set the texture size hint. The size hint is used to request a texture of
             * specified size.*/
            void
            setOverlayTextureSizeHint( unsigned int size );

            /** Get the texture size hint.*/
            unsigned int
            getOverlayTextureSizeHint() const
            {
                return _textureSizeHint;
            }
        ...
        protected:
            ...
            void
            traverse_OBJECT_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY( osg::NodeVisitor& nv );
            void
            traverse_VIEW_DEPENDENT_WITH_ORTHOGRAPHIC_OVERLAY( osg::NodeVisitor& nv );
            void
            traverse_VIEW_DEPENDENT_WITH_PERSPECTIVE_OVERLAY( osg::NodeVisitor& nv );
            ...
            typedef std::map<osgUtil::CullVisitor*, osg::ref_ptr<OverlayData>>
                           OverlayDataMap;

            std::mutex     _overlayDataMapMutex;
            OverlayDataMap _overlayDataMap;

            OverlayNode::OverlayData*
            getOverlayData( osgUtil::CullVisitor* cv );
    };
