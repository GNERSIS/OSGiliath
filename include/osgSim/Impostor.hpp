...
namespace osgSim
{
     ...
     *
     * The principle behind Imposters is that they cache an image of real geometry and
     * then the image is drawn in subsequent frames instead of the real geometry. It's a
     * bit like a Billboard *but* is updated at runtime and w.r.t view point. By drawing
     ...
     * geometry.
     */
    class OSGSIM_EXPORT Impostor : public osg::Inherit<osg::LOD, Impostor>
    {
        public:

            Impostor();

            Impostor( const Impostor&    es,
             ...
             * For eye points further than this threshold the Imposter is used if
             * appropriate, otherwise the LOD children as chosen as per a standard LOD
             * node.
             */
            inline void
            setImpostorThreshold( float distance )
            {
                _impostorThreshold = distance;
            }

            /* Get the Impostor threshold distance. */
            inline float
            getImpostorThreshold() const
            {
                return _impostorThreshold;
            }
             ...
             */
            inline void
            setImpostorThresholdToBound( float ratio = 1.0F )
            {
                _impostorThreshold = getBound().radius * ratio;
            }
            ...
            void
            addImpostorSprite( unsigned int    contextID,
                               ImpostorSprite* is );

            /** Get the list of ImpostorSprites attached to this Impostor. */
            inline ImpostorSpriteList&
            getImpostorSpriteList( unsigned int contexID )
            {
                return _impostorSpriteListBuffer[contexID];
            }

            /** Get a const list of ImpostorSprites attached to this const Impostor. */
            inline const ImpostorSpriteList&
            getImpostorSpriteList( unsigned int contexID ) const
            {
                return _impostorSpriteListBuffer[contexID];
            }
        ...
        protected:
            ...
            ImpostorSprite*
                  createImpostorSprite( osgUtil::CullVisitor* cv );
