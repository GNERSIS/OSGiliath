...
namespace osgSim
{
    ...
    class OSGSIM_EXPORT ImpostorSprite : public osg::Geometry
    {
        public:
            ...
            {
                return dynamic_cast<const ImpostorSprite*>( obj ) != NULL;
            }
            ...
            inline void
            setStoredLocalEyePoint( const osg::vec3& v )
            {
                _storedLocalEyePoint = v;
            }

            /** Get the eye point for when the ImpostorSprite was snapped. */
            inline const osg::vec3&
            getStoredLocalEyePoint() const
            {
                return _storedLocalEyePoint;
            }

            /** Set the frame number for when the ImpostorSprite was last used in
             * rendering. */
            inline void
            setLastFrameUsed( unsigned int frameNumber )
            {
                _lastFrameUsed = frameNumber;
            }

            /** Get the frame number for when the ImpostorSprite was last used in
             * rendering. */
            inline unsigned int
            getLastFrameUsed() const
            {
             ...
             * Stored in the order, [0] - top_left, [1] - bottom_left, [2] -
             * bottom_right, [3] - top_left.
             */
            inline osg::vec3*
            getCoords()
            {
             ...
             * coords will lie on top of the corners of the quad in screen space - with a
             * pixel error of zero. Once the camera moves relative to the impostor sprite
             * the control coords will no longer lie on top of the corners of the quad in
            ...
            getControlCoords()
            {
                return _controlcoords;
            }

            /** Get the const control coordinates of the corners of the quad. */
            inline const osg::vec3*
            getControlCoords() const
            {
                return _controlcoords;
            }

            /** Calculate the pixel error value for passing in the
             * ModelViewProjectionWindow transform, which transform local coords into
             * screen space.
             */
            float
            calcPixelError( const osg::dmat4& MVPW ) const;

            void
            setTexture( osg::Texture2D* tex,
                        int             s,
                        int             t );
            ...
            /** Set the camera node to use for pre rendering the impostor sprite's
             * texture.*/
            void
            ...
            /** Get the camera node to use for pre rendering the impostor sprite's
             * texture.*/
            osg::Camera*
            getCamera()
            {
            ...
            /** Get the const camera node to use for pre rendering the impostor sprite's
             * texture.*/
            const osg::Camera*
            getCamera() const
            {
    ...
    class OSGSIM_EXPORT ImpostorSpriteManager : public osg::Referenced
    {
        public:
            ...
            void
            push_back( ImpostorSprite* is );

            void
            remove( ImpostorSprite* is );

            ImpostorSprite*
            createOrReuseImpostorSprite( int          s,
                                         int          t,
