...
namespace osgSim
{
    ...
    /** MultiSwitch is a Group node which allows switching between sets of selected
       children. MultiSwitch is based on the OpenFlight switch behaviour.
    */
    class OSGSIM_EXPORT MultiSwitch : public osg::Inherit<osg::Group, MultiSwitch>
    {
        public:
            ...
            void
            setValue( unsigned int switchSet,
                      unsigned int pos,
                      bool         value );

            bool
            getValue( unsigned int switchSet,
                      unsigned int pos ) const;

            void
            setChildValue( const osg::Node* child,
                           unsigned int     switchSet,
                           bool             value );

            bool
            getChildValue( const osg::Node* child,
                           unsigned int     switchSet ) const;

            /** Set all the children off (false), and set the new default child value to
             * off (false).*/
            bool
            setAllChildrenOff( unsigned int switchSet );

            /** Set all the children on (true), and set the new default child value to on
             * (true).*/
            bool
            setAllChildrenOn( unsigned int switchSet );

            /** Set a single child to be on, MultiSwitch off all other children.*/
            bool
            setSingleChildOn( unsigned int switchSet,
                              unsigned int pos );
            ...
            {
                _activeSwitchSet = switchSet;
            }

            /** Get which of the available switch set lists to use.*/
            unsigned int
            getActiveSwitchSet() const
            {
                return _activeSwitchSet;
            }
            ...
            void
            setSwitchSetList( const SwitchSetList& switchSetList );

            /** Get the compile set of different values.*/
            const SwitchSetList&
            getSwitchSetList() const
            {
                return _values;
            }
            ...
            void
            setValueList( unsigned int     switchSet,
                          const ValueList& values );

            /** Get the a single set of different values for a particular switch set.*/
            const ValueList&
            getValueList( unsigned int switchSet ) const
            {
                return _values[switchSet];
            }

            void
            setValueName( unsigned int       switchSet,
                          const std::string& name );

            const std::string&
            getValueName( unsigned int switchSet ) const
            {
        ...
        protected:
            ...
            void
                              expandToEncompassSwitchSet( unsigned int switchSet );
