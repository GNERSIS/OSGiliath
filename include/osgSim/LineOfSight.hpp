...
namespace osgSim
{
    ...
    class OSGSIM_EXPORT DatabaseCacheReadCallback
        : public osgUtil::IntersectionVisitor::ReadCallback
    {
        public:
            ...
            void
            setMaximumNumOfFilesToCache( unsigned int maxNumFilesToCache )
            {
                _maxNumFilesToCache = maxNumFilesToCache;
            }

            unsigned int
            getMaximumNumOfFilesToCache() const
            {
                return _maxNumFilesToCache;
            }
    ...
    /** Helper class for setting up and acquiring line of sight intersections with
    ...
    class OSGSIM_EXPORT LineOfSight
    {
        public:
            ...
            /** Add a line of sight test, consisting of start and end point. Returns the
             * index number of the newly adding LOS test.*/
            unsigned int
            addLOS( const osg::dvec3& start,
                    const osg::dvec3& end );
            ...
            getNumLOS() const
            {
                return static_cast<unsigned int>( _LOSList.size() );
            }

            /** Set the start point of single line of sight test.*/
            void
            setStartPoint( unsigned int      i,
                           const osg::dvec3& start )
            {
                _LOSList[i]._start = start;
            }

            /** Get the start point of single line of sight test.*/
            const osg::dvec3&
            getStartPoint( unsigned int i ) const
            {
                return _LOSList[i]._start;
            }

            /** Set the end point of single line of sight test.*/
            void
            setEndPoint( unsigned int      i,
                         const osg::dvec3& end )
            {
                _LOSList[i]._end = end;
            }
            ...
            const osg::dvec3&
            getEndPoint( unsigned int i ) const
            {
                return _LOSList[i]._end;
            }

            typedef std::vector<osg::dvec3> Intersections;

            /** Get the intersection points for a single line of sight test.*/
            const Intersections&
            getIntersections( unsigned int i ) const
            {
                return _LOSList[i]._intersections;
            }

            /** Compute the LOS intersections with the specified scene graph.
             * The results are all stored in the form of Intersections list, one per LOS
             * test.*/
            void
            computeIntersections( osg::Node*          scene,
                                  osg::Node::NodeMask traversalMask = 0XFF'FF'FF'FF );

            /** Compute the intersection between the specified scene graph and a single
             * LOS start,end pair. Returns an IntersectionList, of all the points
             * intersected.*/
             ...
             * HeightAboveTerrain objects in use at one time then you should share a
             * single DatabaseCacheReadCallback between all of them. */
            void
