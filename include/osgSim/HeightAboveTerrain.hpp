...
namespace osgSim
{
    ...
    class OSGSIM_EXPORT HeightAboveTerrain
    {
        public:
            ...
            /** Clear the internal HAT List so it contains no height above terrain
             * tests.*/
            void
            clear();
            ...
            unsigned int
            addPoint( const osg::dvec3& point );

            /** Get the number of height above terrain tests.*/
            unsigned int
            getNumPoints() const
            {
                return static_cast<unsigned int>( _HATList.size() );
            }

            /** Set the source point of single height above terrain test.*/
            void
            setPoint( unsigned int      i,
                      const osg::dvec3& point )
            {
                _HATList[i]._point = point;
            }

            /** Get the source point of single height above terrain test.*/
            const osg::dvec3&
            getPoint( unsigned int i ) const
            {
                return _HATList[i]._point;
            }

            /** Get the intersection height for a single height above terrain test.
             * Note, you must call computeIntersections(..) before you can query the
             * HeightAboveTerrain. If no intersections are found then height returned
             * will be the height above mean sea level. */
            double
            getHeightAboveTerrain( unsigned int i ) const
            {
                return _HATList[i]._hat;
            }

            /** Set the lowest height that the should be tested for.
             * Defaults to -1000, i.e. 1000m below mean sea level. */
            void
            setLowestHeight( double lowestHeight )
            {
            ...
            double
            getLowestHeight() const
            {
                return _lowestHeight;
            }

            /** Compute the HAT intersections with the specified scene graph.
             * The results are all stored in the form of a single height above terrain
             * value per HAT test. Note, if the topmost node is a CoordinateSystemNode
             * then the input points are assumed to be geocentric, with the up vector
             * defined by the EllipsoidModel attached to the CoordinateSystemNode. If the
             * topmost node is not a CoordinateSystemNode then a local coordinates frame
             * is assumed, with a local up vector. */
            void
             ...
             * single HAT point. */
            static double
            computeHeightAboveTerrain( osg::Node*          scene,
                                       const osg::dvec3&   point,
