...
namespace osgUI
{
    ...
    class OSGUI_EXPORT Widget : public osg::Inherit<osg::Group, Widget>
    {
        public:
            ...
            typedef std::vector<osgUtil::LineSegmentIntersector::Intersection>
                Intersections;
            virtual bool
            computeIntersections( osgGA::EventVisitor*    ev,
            ...
            computePositionInLocalCoordinates( osgGA::EventVisitor*    ev,
                                               osgGA::GUIEventAdapter* event,
                                               osg::dvec3& localPosition ) const;
            virtual bool
            computeExtentsPositionInLocalCoordinates( osgGA::EventVisitor*    ev,
                                                      osgGA::GUIEventAdapter* event,
                                                      osg::dvec3& localPosition,
                                                      bool withinExtents = true ) const;
            ...
            void
            setGraphicsSubgraph( int        orderNum,
                                 osg::Node* node )
            {
                _graphicsSubgraphMap[orderNum] = node;
                _graphicsInitialized           = true;
            }

            /** Get the subgraph to be used to render the widget.*/
            osg::Node*
            getGraphicsSubgraph( int orderNum )
            {
                GraphicsSubgraphMap::iterator itr =
                    _graphicsSubgraphMap.find( orderNum );
                return ( itr != _graphicsSubgraphMap.end() ) ? itr->second.get() : 0;
            }

            /** Get the const subgraph to be used to render the widget.*/
            const osg::Node*
            getGraphicsSubgraph( int orderNum ) const
            {
                GraphicsSubgraphMap::const_iterator itr =
                    _graphicsSubgraphMap.find( orderNum );
                return ( itr != _graphicsSubgraphMap.end() ) ? itr->second.get() : 0;
            }

            void
            setGraphicsSubgraphMap( const GraphicsSubgraphMap& gsm )
            {
                _graphicsSubgraphMap = gsm;
                _graphicsInitialized = true;
            }

            GraphicsSubgraphMap&
            getGraphicsSubgraphMap()
            {
                return _graphicsSubgraphMap;
            }

            const GraphicsSubgraphMap&
            getGraphicsSubgraphMap() const
            {
                return _graphicsSubgraphMap;
            }

            /** Set the WidgetStateSet is used internally by Widgets to manage state that
             * decorates the subgraph. WidgetStateSet is not serialized and is typically
             * populated by the Widget::createGraphics() implementation, end users will
             * not normally touoch the WidgetStateSet, use the normal Node::setStateSet()
             * if you want to apply your own state to the Widget and it's subgraphs.*/
            void
            setWidgetStateSet( osg::StateSet* stateset )
            {
                _widgetStateSet = stateset;
            }

            osg::StateSet*
            getWidgetStateSet()
            {
                return _widgetStateSet.get();
            }

            const osg::StateSet*
            getWidgetStateSet() const
            {
                return _widgetStateSet.get();
            }

            osg::StateSet*
            getOrCreateWidgetStateSet()
            {
                if( !_widgetStateSet )
                {
                    _widgetStateSet = new osg::StateSet;
                }
                return _widgetStateSet.get();
            }

            /** createGraphics entry method, calls either callback object named
             * "createGraphics" or the createGraphicsImplementation() method.*/
            virtual void
            createGraphics();

            /** createGraphicsImplementation method that creates the subgraph that will
             * render the widget and assigns it to the Widget via the
             * Widet::setGraphicsSubgraph() method.*/
            virtual void
            ...
            void
            setAlignmentSettings( AlignmentSettings* alignmentSettings )
            {
                _alignmentSettings = alignmentSettings;
            }

            AlignmentSettings*
            getAlignmentSettings()
            {
                return _alignmentSettings.get();
            }

            const AlignmentSettings*
            getAlignmentSettings() const
            {
                return _alignmentSettings.get();
            }

            void
            setFrameSettings( FrameSettings* textSettings )
            {
                _frameSettings = textSettings;
            }

            FrameSettings*
            getFrameSettings()
            {
                return _frameSettings.get();
            }

            const FrameSettings*
            getFrameSettings() const
            {
                return _frameSettings.get();
            }

            void
            setTextSettings( TextSettings* textSettings )
            {
            ...
            getTextSettings()
            {
                return _textSettings.get();
            }

            const TextSettings*
            getTextSettings() const
            {
                return _textSettings.get();
            }
            ...
            {
                _autoFillBackground = enabled;
            }

            /** get whether the widget should fill the extents of its background.*/
            virtual bool
            getAutoFillBackground() const
            {
                return _autoFillBackground;
            }
            ...
            /** invoke all callbacks with specified names providing input and output
             * parameters.*/
            bool
            runCallbacks( const std::string& name,
                          osg::Parameters&   inputParameters,
                          osg::Parameters&   outputParameters )
            {
                return osg::runNamedCallbackObjects( this,
                                                     name,
                                                     inputParameters,
                                                     outputParameters );
            }

            /** invoke all callbacks with specified names without any specified input or
             * output parameters.*/
            bool
            runCallbacks( const std::string& name )
            {
