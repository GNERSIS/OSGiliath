...
namespace osgUI
{
    ...
    class OSGUI_EXPORT LineEdit : public osg::Inherit<osgUI::Widget, LineEdit>
    {
        public:
            ...
            virtual void
            textChangedImplementation( const std::string& text );

            virtual void
            returnPressed()
            {
                if( !runCallbacks( "returnPressed" ) )
                {
                    returnPressedImplementation();
                }
            }
