...
#include <osgUI/AlignmentSettings.hpp>
...
namespace osgUI
{
    ...
    class OSGUI_EXPORT Style : public osg::Inherit<osg::Object, Style>
    {
        public:
            ...
            void
            setDisabledTextColor( const osg::vec4& color )
            {
                _disabledTextColor = color;
            }

            const osg::vec4&
            getDisabledTextColor() const
            {
                return _disabledTextColor;
            }

            virtual osg::Node*
            createDepthSetPanel( const osg::box& extents );
            virtual osg::Node*
            createPanel( const osg::box&  extents,
                         const osg::vec4& colour );
            virtual osg::Node*
            createFrame( const osg::box&      extents,
                         const FrameSettings* frameSettings,
                         const osg::vec4&     colour );
            virtual osg::Node*
            createText( const osg::box&          extents,
                        const AlignmentSettings* as,
                        const TextSettings*      textSettings,
                        const std::string&       text );
            virtual osg::Node*
            createIcon( const osg::box&    extents,
                        const std::string& filename,
                        const osg::vec4&   colour );
            virtual void
            setupDialogStateSet( osg::StateSet* stateset,
                                 int            binNum );
            virtual void
            setupPopupStateSet( osg::StateSet* stateset,
                                int            binNum );
            virtual void
            setupClipStateSet( const osg::box& extents,
