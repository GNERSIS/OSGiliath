...
namespace osgText
{
     ...
     */
    extern OSGTEXT_EXPORT Font*
    readFontFile( const std::string&    filename,
                  const osgDB::Options* userOptions = 0 );

    /** read a font from specified stream.*/
    extern OSGTEXT_EXPORT Font*
    readFontStream( std::istream&         stream,
                    const osgDB::Options* userOptions = 0 );
#endif

    extern OSGTEXT_EXPORT osg::ref_ptr<Font>
                          readRefFontFile( const std::string&    filename,
                                           const osgDB::Options* userOptions = 0 );

    extern OSGTEXT_EXPORT osg::ref_ptr<Font>
                          readRefFontStream( std::istream&         stream,
                                             const osgDB::Options* userOptions = 0 );

    extern OSGTEXT_EXPORT std::string
                          findFontFile( const std::string& str );
    ...
    class OSGTEXT_EXPORT Font : public osg::Object
    {
        ...
        public:
            ...
            {
                return 0;
            }    // cloneType() not appropriate

            virtual osg::Object*
            clone( const osg::CopyOp& ) const
            {
                return 0;
            }    // clone() not appropriate

            virtual bool
            isSameKindAs( const osg::Object* obj ) const
            {
                return dynamic_cast<const Font*>( obj ) != NULL;
            }
            ...
            const StateSets&
            getCachedStateSets() const
            {
                return _statesets;
            }

            /** Get a kerning (adjustment of spacing of two adjacent character) for
             * specified charcodes and a font resolution.*/
            virtual osg::vec2
            getKerning( const FontResolution& fontRes,
                        unsigned int          leftcharcode,
                        unsigned int          rightcharcode,
                        KerningType           kerningType );

            /** Get a Glyph for specified charcode, and the font size nearest to the
             * current font size hint.*/
            virtual Glyph*
            getGlyph( const FontResolution& fontSize,
                      unsigned int          charcode );

            /** Get a Glyph3D for specified charcode and a font size.*/
            virtual Glyph3D*
            getGlyph3D( const FontResolution& fontSize,
                        unsigned int          charcode );

            /** Return true if this font provides vertical alignments and spacing or
             * glyphs.*/
            virtual bool
            hasVertical() const;

            /** Get the ascender and descender sizes of the font where supported by the
             * FontImplementation, return true on success, return false when not
             * supported.*/
            virtual bool
                             ...
                             float& descender ) const
            {
                return _implementation
                         ? _implementation->getVerticalSize( ascender, descender )
                         : false;
            }

            /** Set the size of texture to create to store the glyph images when
             * rendering. Note, this doesn't affect already created Texture Glhph's.*/
            void
            setTextureSizeHint( unsigned int width,
                                unsigned int height );

            unsigned int
            getTextureWidthHint() const;
            unsigned int
            getTextureHeightHint() const;

            /** Set the minification texture filter to use when creating the texture to
             * store the glyph images when rendering. Note, this doesn't affect already
             * created Texture Glhph's.*/
            void
            setMinFilterHint( osg::Texture::FilterMode mode );
            osg::Texture::FilterMode
            getMinFilterHint() const;

            /** Set the magnification texture filter to use when creating the texture to
             * store the glyph images when rendering. Note, this doesn't affect already
             * created Texture Glhph's.*/
            void
            setMagFilterHint( osg::Texture::FilterMode mode );
            osg::Texture::FilterMode
            getMagFilterHint() const;
            ...
            void
            setNumberCurveSamples( unsigned int numSamples )
            {
                _numCurveSamples = numSamples;
            }

            unsigned int
            getNumberCurveSamples() const
            {
                return _numCurveSamples;
            }

            // make Text a friend to allow it add and remove its entry in the Font's
            // _textList.
            friend class FontImplementation;

            void
            setImplementation( FontImplementation* implementation );

            FontImplementation*
            getImplementation();
            const FontImplementation*
            ...
            getGlyphTextureList()
            {
                return _glyphTextureList;
            }

            void
            assignGlyphToGlyphTexture( Glyph*          glyph,
        ...
        protected:
            ...
            void
            addGlyph( const FontResolution& fontRes,
                      unsigned int          charcode,
                      Glyph*                glyph );
            ...
            mutable std::mutex                                    _glyphMapMutex;
        ...
        public:
            ...
            class FontImplementation : public osg::Referenced
            {
                public:
                    ...
                    virtual bool
                    supportsMultipleFontResolutions() const = 0;

                    /** Get a Glyph for specified charcode, and the font size nearest to
                     * the current font size hint.*/
                    virtual Glyph*
                    ...
                    virtual Glyph3D*
                    getGlyph3D( const FontResolution& fontRes,
                                unsigned int          charcode ) = 0;

                    /** Get a kerning (adjustment of spacing of two adjacent character)
                     * for specified charcodes, w.r.t the current font size hint.*/
                    virtual osg::vec2
                    getKerning( const FontResolution& fontRes,
                                unsigned int          leftcharcode,
                                unsigned int          rightcharcode,
                    ...
                    /** Return true if this font provides vertical alignments and spacing
                     * or glyphs.*/
                    virtual bool
                    hasVertical() const = 0;
