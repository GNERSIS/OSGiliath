...
namespace osgText
{
    ...
    class OSGTEXT_EXPORT Text : public osgText::TextBase
    {
        public:
            ...
            {
                return dynamic_cast<const Text*>( obj ) != NULL;
            }
            ...
            /** Set the ShaderTechnique hint to specify what fatures in the text shaders
             * to enable.*/
            void
            ...
            /**
             * Turns off writing to the depth buffer when rendering text. This only
             * affects text with no backdrop or text using the DELAYED_DEPTH_WRITES
             * implementation, since the other backdrop implementations are really only
             * useful for backwards compatibility and are not worth updating to utilize
             * this flag.
             */
            void
            setEnableDepthWrites( bool enable )
            {
                _enableDepthWrites = enable;
            }

            bool
            getEnableDepthWrites() const
            {
                return _enableDepthWrites;
            }
             ...
             * The color of the background shadow text is specified by
             * setBackdropColor(). DROP_SHADOW_BOTTOM_RIGHT will draw backdrop text to
             * the right and down of the normal text. Other DROP_SHADOW_* modes do the
             * same for their respective directions. OUTLINE will draw backdrop text so
             * that it appears the text has an outline or border around the normal text.
             * This mode is particularly useful against really noisy backgrounds that may
             * put text on top of things that have all types of colors which you don't
             * have control over. Some real world examples of this general technique in
             * use that I know of are Google Earth, Sid Meier's Pirates (2004 Remake),
             * and Star Control 2 (PC 1993). The default is NONE.
             */
            void
            setBackdropType( BackdropType type );

            BackdropType
            getBackdropType() const
            {
             ...
             * Set the value too high and for example, in OUTLINE mode you will get a
             * "Brady Bunch" effect where you see duplicates of the text in a 3x3 grid.
             ...
             * However, other metric information such as getCharacterHeight() are
             * unaffected by this. This means that individual glyph spacing (kerning?)
             * are unchanged even when this mode is used. The default is 0.07 (7%
             * offset).
             */
            void
            setBackdropOffset( float offset = 0.07F );
            /**
             * This overloaded version lets you specify the offset for the horizontal
             * and vertical components separately.
             */
            void
            setBackdropOffset( float horizontal,
                               float vertical );

            float
            getBackdropHorizontalOffset() const
            {
                return _backdropHorizontalOffset;
            }

            float
            getBackdropVerticalOffset() const
            {
                return _backdropVerticalOffset;
            }
             ...
             */
            void
            setBackdropColor( const osg::vec4& color );

            const osg::vec4&
            getBackdropColor() const
            {
             ...
             */
            void
            setColorGradientMode( ColorGradientMode mode );

            ColorGradientMode
            getColorGradientMode() const
            {
                return _colorGradientMode;
            }

            /**
             * Used only for gradient mode, let's you specify the colors of the 4
             * corners. If ColorGradients are off, these values are ignored (and the
             * value from setColor() is the only one that is relevant.
             */
            void
            setColorGradientCorners( const osg::vec4& topLeft,
                                     const osg::vec4& bottomLeft,
                                     const osg::vec4& bottomRight,
                                     const osg::vec4& topRight );

            const osg::vec4&
            getColorGradientTopLeft() const
            {
                return _colorGradientTopLeft;
            }

            const osg::vec4&
            getColorGradientBottomLeft() const
            {
                return _colorGradientBottomLeft;
            }

            const osg::vec4&
            getColorGradientBottomRight() const
            {
                return _colorGradientBottomRight;
            }

            const osg::vec4&
            getColorGradientTopRight() const
            {
                return _colorGradientTopRight;
            }
            ...
            virtual bool
            supports( const osg::Drawable::AttributeFunctor& ) const
            {
                return false;
            }
            ...
            virtual bool
            supports( const osg::Drawable::ConstAttributeFunctor& ) const
            {
                return true;
            }

            /** accept an ConstAttributeFunctor and call its methods to tell it about the
             * internal attributes that this Drawable has.*/
            virtual void
            accept( osg::Drawable::ConstAttributeFunctor& af ) const;
            ...
            /** accept a PrimtiveFunctor and call its methods to tell it about the
             * internal primitives that this Drawable has.*/
            virtual void
            accept( osg::PrimitiveFunctor& pf ) const;

            /** Get the coordinates of the character corners in local coordinates. Use
             * Text::getMatrix() or Text::computeMatrix(..) to get the transform into
             * model coordinates (see TextBase header.) */
            bool
            getCharacterCorners( unsigned int index,
                                 osg::vec3&   bottomLeft,
        ...
        public:
            ...
            /** deprecated, value ignored.*/
            void
            setBackdropImplementation( BackdropImplementation )
            {
            }

            /** deprecated, value should be ignored.*/
            BackdropImplementation
            getBackdropImplementation() const
            {
                return DELAYED_DEPTH_WRITES;
            }

            // internal structures, variable and methods used for rendering of
            // characters.
            struct OSGTEXT_EXPORT GlyphQuads
            {
                    ...
                    void
                    setupPrimitives( Text::BackdropType backdropType );
            ...
            const GlyphQuads*
            getGlyphQuads( GlyphTexture* texture ) const
            {
                TextureGlyphQuadMap::const_iterator itGlyphQuad =
                    _textureGlyphQuadMap.find( texture );
                if( itGlyphQuad == _textureGlyphQuadMap.end() )
                {
                    return NULL;
                }
            ...
            const TextureGlyphQuadMap&
            getTextureGlyphQuadMap() const
            {
                return _textureGlyphQuadMap;
            }

            void
            addGlyphQuad( Glyph*           glyph,
                          const osg::vec2& minc,
                          const osg::vec2& maxc,
        ...
        protected:
            ...
            String::iterator
                                computeLastCharacterOnLine( osg::vec2&       cursor,
                                                            String::iterator first,
                                                            String::iterator last );

            // members which have public access.

            // iternal map used for rendering. Set up by the computeGlyphRepresentation()
            // method.
            ...
            bool
            computeAverageGlyphWidthAndHeight( float& avg_width,
            ...
            void
            drawImplementation( osg::State&      state,
                                const osg::vec4& colorMultiplier ) const;

            void
            drawImplementationSinglePass( osg::State&      state,
                                          const osg::vec4& colorMultiplier ) const;
