...
namespace osgText
{
    ...
    class VectorUInt : public std::vector<unsigned int>
    {
        ...
        public:
            ...
            explicit VectorUInt( unsigned int n ) :
                vector_type( n )
            {
            }
    };
    ...
    class OSGTEXT_EXPORT String : public VectorUInt
    {
        public:
            ...
            void
            set( const wchar_t* text );

            /** Set the text using a Unicode encoded std::string, which is converted to
             * an internal TextString. The encoding parameter specifies which Unicode
             * encoding is used in the std::string. */
            void
