...
#define NEW_APPROACH
    ...
    class OSGTEXT_EXPORT TextBase : public osg::Drawable
    {
        public:
            ...
            {
                return dynamic_cast<const TextBase*>( obj ) != NULL;
            }
            ...
            /** Set the font, loaded from the specified front file, to use to render the
             * text, setFont("") sets the use of the default font. See the
             * osgText::readFontFile function for how the font file will be located. */
            virtual void
            ...
            void
            setFontResolution( unsigned int width,
                               unsigned int height );

            unsigned int
            getFontWidth() const
            {
                return _fontSize.first;
            }

            unsigned int
            getFontHeight() const
            {
                return _fontSize.second;
            }
            ...
            /** Set the text using a Unicode encoded std::string, which is converted to
             * an internal TextString. The encoding parameter specificies which Unicode
             * encodeding is used in the std::string. */
            void
            setText( const std::string& text,
            ...
            float
            getCharacterHeight() const
            {
                return _characterHeight;
            }

            float
            getCharacterAspectRatio() const
            {
                return _style.valid() ? _style->getWidthRatio() : 1.0F;
            }

            enum CharacterSizeMode
            {
                OBJECT_COORDS,    /// default
                SCREEN_COORDS,    /// internally scale the characters to be constant
                                  /// screen size.
                OBJECT_COORDS_WITH_MAXIMUM_SCREEN_SIZE_CAPPED_BY_FONT_HEIGHT    /// text
                                                                                /// that
                                                                                /// behavaves
                                                                                /// like
                                                                                /// OBJECT_COORDS
             ...
             * character.*/
            void
            setCharacterSizeMode( CharacterSizeMode mode )
            {
            ...
            CharacterSizeMode
            getCharacterSizeMode() const
            {
                return _characterSizeMode;
            }

            /** Set the maximum width of the text box.
             * With horizontal layouts any characters which do not fit are wrapped
             * around. 0 or negative values indicate that no maximum width is set, lines
             * can be as long as they need be to fit thre required text*/
            void
            setMaximumWidth( float maximumWidth );

            /** Get the maximim width of the text box.*/
            float
            getMaximumWidth() const
            {
             ...
             * With horizontal layouts any characters which do not fit are wrapped
             * around. 0 or negative values indicate that no maximum height is set, lines
             * can be as long as they need be to fit the required text*/
            void
            setMaximumHeight( float maximumHeight );

            /** Get the maximum height of the text box.*/
            float
            getMaximumHeight() const
            {
            ...
            void
            setAxisAlignment( AxisAlignment axis );

            AxisAlignment
            getAxisAlignment() const
            {
                return _axisAlignment;
            }

            void
            setRotation( const osg::quat& quat );
            ...
            bool
            getAutoRotateToScreen() const
            {
                return _autoRotateToScreen;
            }
            ...
            void
            setBoundingBoxMargin( float margin );

            float
            getBoundingBoxMargin() const
            {
                return _textBBMargin;
            }

            void
            setBoundingBoxColor( const osg::vec4& color )
            {
                _textBBColor = color;
            }

            const osg::vec4&
            getBoundingBoxColor() const
            {
                return _textBBColor;
            }

            void
            setKerningType( KerningType kerningType )
            {
                _kerningType = kerningType;
            }

            KerningType
            getKerningType() const
            {
                return _kerningType;
            }

            /** Get the number of wrapped lines - only valid after
             * computeGlyphRepresentation() has been called, returns 0 otherwise */
            unsigned int
            getLineCount() const
            {
                return _lineCount;
            ...
            void
            getCoord( unsigned int i,
                      osg::vec2&   c ) const
            {
                c.set( ( *_coords )[i].x, ( *_coords )[i].y );
            }
            ...
            /** Get the cached internal matrix used to provide positioning of text.  The
             * cached matrix is originally computed by computeMatrix(..). */
            const osg::dmat4&
            getMatrix() const
            {
                return _matrix;
            ...
            /** compute the matrix that positions the text in model space for the given
             * viewpoint.*/
            bool
            computeMatrix( osg::dmat4& matrix,
        ...
        protected:
            ...
            void
            positionCursor( const osg::vec2& endOfLine_coords,
                            osg::vec2&       cursor,
            ...
            unsigned int
            addCoord( const osg::vec2& c )
            {
                unsigned int s = static_cast<unsigned int>( _coords->size() );
                _coords->push_back( osg::vec3( c.x, c.y, 0.0F ) );
                _coords->dirty();
                return s;
            }

            unsigned int
            addCoord( const osg::vec3& c )
            {
                unsigned int s = static_cast<unsigned int>( _coords->size() );
                _coords->push_back( c );
                _coords->dirty();
