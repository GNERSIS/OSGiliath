...
namespace osgDB
{
    ...
    class OSGDB_EXPORT DatabasePager : public osg::NodeVisitor::DatabaseRequestHandler
    {
        public:
            ...
            typedef osg::Thread::ThreadPriority ThreadPriority;
            ...
            int
            setSchedulePriority( osg::Thread::ThreadPriority priority );
            ...
            class OSGDB_EXPORT DatabaseThread : public osg::Referenced,
                                                public osg::Thread
            {
                public:
                    ...
                    DatabaseThread( const DatabaseThread& dt,
                                    DatabasePager*        pager );
            ...
            virtual void
            setProcessorAffinity( const osg::Thread::Affinity& affinity );
            ...
            const osg::Thread::Affinity&
            getProcessorAffinity() const
            {
                return _affinity;
            }

            void
            setUpThreads( unsigned int totalNumThreads = 2,
                          unsigned int numHttpThreads  = 1 );

            virtual unsigned int
            addDatabaseThread( DatabaseThread::Mode mode,
                               const std::string&   name );

            DatabaseThread*
            getDatabaseThread( unsigned int i )
            {
                return _databaseThreads[i].get();
            }

            const DatabaseThread*
            getDatabaseThread( unsigned int i ) const
            {
                return _databaseThreads[i].get();
            }

            unsigned int
            getNumDatabaseThreads() const
            {
                return static_cast<unsigned int>( _databaseThreads.size() );
            }

            /** Set whether the database pager thread should be paused or not.*/
            void
            setDatabasePagerThreadPause( bool pause );

            /** Get whether the database pager thread should is paused or not.*/
            bool
            getDatabasePagerThreadPause() const
            {
                return _databasePagerThreadPaused;
            }

            /** Set whether new database request calls are accepted or ignored.*/
            void
            setAcceptNewDatabaseRequests( bool acceptNewRequests )
            {
                _acceptNewRequests = acceptNewRequests;
            }

            /** Get whether new database request calls are accepted or ignored.*/
            bool
            getAcceptNewDatabaseRequests() const
            {
                return _acceptNewRequests;
            }

            /** Get the number of frames that are currently active.*/
            int
            getNumFramesActive() const
            {
                return _numFramesActive;
            }

            /** Signal the database thread that the update, cull and draw has begun for a
             * new frame. Note, this is called by the application so that the database
             * pager can go to sleep while the CPU is busy on the main rendering threads.
             */
            virtual void
            signalBeginFrame( const osg::FrameStamp* framestamp );

            /** Signal the database thread that the update, cull and draw dispatch has
             * completed. Note, this is called by the application so that the database
             * pager can go to wake back up now the main rendering threads are iddle
            ...
            virtual void
            registerPagedLODs( osg::Node*   subgraph,
                               unsigned int frameNumber = 0 );

            /** Set the incremental compile operation.
             * Used to manage the OpenGL object compilation and merging of subgraphs in a
             * way that avoids overloading the rendering of frame with too many new
             ...
             * allowing them to be merged into the scene graph. Pre compilation helps
             * reduce the chances of frame drops, but also slows the speed at which tiles
             * are merged as they have to be compiled first.*/
            void
            setDoPreCompile( bool flag )
            {
                _doPreCompile = flag;
            }
            ...
            bool
            getDoPreCompile() const
            {
                return _doPreCompile;
            }

            /** Set the target maximum number of PagedLOD to maintain in memory.
             * Note, if more than the target number are required for rendering of a frame
             * then these active PagedLOD are excempt from being expiried. But once the
             * number of active drops back below the target the inactive PagedLOD will be
             * trimmed back to the target number.*/
            void
            ...
            unsigned int
            getTargetMaximumNumberOfPageLOD() const
            {
                return _targetMaximumNumberOfPageLOD;
            }

            /** Set whether the removed subgraphs should be deleted in the database
             * thread or not.*/
            void
            setDeleteRemovedSubgraphsInDatabaseThread( bool flag )
            {
                _deleteRemovedSubgraphsInDatabaseThread = flag;
            }

            /** Get whether the removed subgraphs should be deleted in the database
             * thread or not.*/
            bool
            getDeleteRemovedSubgraphsInDatabaseThread() const
            {
                return _deleteRemovedSubgraphsInDatabaseThread;
            }
            ...
            /** Set how loaded drawables should be handled w.r.t their vertex buffer
             * object/vertex array settings.*/
            void
            setDrawablePolicy( DrawablePolicy policy )
            {
                _drawablePolicy = policy;
            }

            /** Get how loaded drawables should be handled w.r.t their vertex buffer
             * object/vertex array settings.*/
            DrawablePolicy
            getDrawablePolicy() const
            {
                return _drawablePolicy;
            }

            /** Set whether newly loaded textures should have a PixelBufferObject
             * assigned to them to aid download to the GPU.*/
            void
            setApplyPBOToImages( bool assignPBOToImages )
            {
                _assignPBOToImages = assignPBOToImages;
            }

            /** Get whether newly loaded textures should have a PixelBufferObject
             * assigned to them.*/
            bool
            getApplyPBOToImages() const
            {
                return _assignPBOToImages;
            }

            /** Set whether newly loaded textures should have their
             * UnrefImageDataAfterApply set to a specified value.*/
            void
            setUnrefImageDataAfterApplyPolicy( bool changeAutoUnRef,
                                               bool valueAutoUnRef )
            {
             ...
             * UnrefImageDataAfterApply set to a specified value.*/
            void
            getUnrefImageDataAfterApplyPolicy( bool& changeAutoUnRef,
                                               bool& valueAutoUnRef ) const
            {
                changeAutoUnRef = _changeAutoUnRef;
                valueAutoUnRef  = _valueAutoUnRef;
            }

            /** Set whether newly loaded textures should have their MaxAnisotopy set to a
             * specified value.*/
            void
            setMaxAnisotropyPolicy( bool  changeAnisotropy,
             ...
             * specified value.*/
            void
            getMaxAnisotropyPolicy( bool&  changeAnisotropy,
                                    float& valueAnisotropy ) const
            {
                changeAnisotropy = _changeAnisotropy;
             ...
             * removeExpiredSubgraphs then addLoadedDataToSceneGraph. Note, must only be
             * called from single thread update phase. */
            virtual void
            updateSceneGraph( const osg::FrameStamp& frameStamp );

            /** Return true if there are GL objects that need to be compiled by a draw
             * traversal. */
            bool
            ...
            unsigned int
            getFileRequestListSize() const
            {
                return static_cast<unsigned int>( _fileRequestQueue->size() +
                                                  _httpRequestQueue->size() );
            }

            /** Report how many items are in the _dataToCompileList queue */
            unsigned int
            getDataToCompileListSize() const
            {
                return static_cast<unsigned int>( _dataToCompileList->size() );
            }

            /** Report how many items are in the _dataToMergeList queue */
            unsigned int
            getDataToMergeListSize() const
            {
                return static_cast<unsigned int>( _dataToMergeList->size() );
            }

            /** Report whether any requests are in the pager.*/
            bool
            getRequestsInProgress() const;

            /** Get the minimum time between the first request for a tile to be loaded
             * and the time of its merge into the main scene graph.*/
            double
            getMinimumTimeToMergeTile() const
            {
                return _minimumTimeToMergeTile;
            }

            /** Get the maximum time between the first request for a tile to be loaded
             * and the time of its merge into the main scene graph.*/
            double
            getMaximumTimeToMergeTile() const
            {
                return _maximumTimeToMergeTile;
            }

            /** Get the average time between the first request for a tile to be loaded
             * and the time of its merge into the main scene graph.*/
            double
            getAverageTimeToMergeTiles() const
            {
                return ( _numTilesMerges > 0 )
                         ? _totalTimeToMergeTiles /
                               static_cast<double>( _numTilesMerges )
                         : 0;
            }
            ...
            struct PagedLODList : public osg::Referenced
            {
                    ...
                    removeExpiredChildren( int          numberChildrenToRemove,
                                           ...
                                           bool         visitActive ) = 0;
                    virtual void
                    removeNodes( osg::NodeList& nodesToRemove ) = 0;
                    virtual void
                    insertPagedLOD( const osg::observer_ptr<osg::PagedLOD>& plod ) = 0;
                    virtual bool
                    containsPagedLOD(
                        const osg::observer_ptr<osg::PagedLOD>& plod
                    ) const = 0;
            };
        ...
        protected:
            ...
            struct OSGDB_EXPORT DatabaseRequest : public osg::Referenced
            {
                    ...
                    bool
                    isRequestCurrent( int frameNumber ) const
                    {
                        return _valid && ( static_cast<unsigned int>( frameNumber ) -
                                           _frameNumberLastRequest <= 1 );
                    }
            ...
            struct OSGDB_EXPORT RequestQueue : public osg::Referenced
            {
                public:
                    ...
                    void
                    add( DatabaseRequest* databaseRequest );
                    void
                    remove( DatabaseRequest* databaseRequest );

                    void
                    addNoLock( DatabaseRequest* databaseRequest );

                    void
                    takeFirst( osg::ref_ptr<DatabaseRequest>& databaseRequest );

                    /// prune all the old requests and then return true if requestList
                    /// left empty
                    bool
            ...
            struct OSGDB_EXPORT ReadQueue : public RequestQueue
            {
                    ...
                    std::mutex                  _childrenToDeleteListMutex;
            ...
            void
            compileCompleted( DatabaseRequest* databaseRequest );

            /** Iterate through the active PagedLOD nodes children removing
             * children which haven't been visited since specified expiryTime.
             * note, should be only be called from the update thread. */
            virtual void
            removeExpiredSubgraphs( const osg::FrameStamp& frameStamp );

            /** Add the loaded data to the scene graph.*/
            void
            addLoadedDataToSceneGraph( const osg::FrameStamp& frameStamp );

            osg::Thread::Affinity      _affinity;
            ...
            mutable std::mutex         _numFramesActiveMutex;
