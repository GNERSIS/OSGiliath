...
#include <osgDB/serialization/Serializer.hpp>
...
namespace osgDB
{
    ...
    struct MethodObject : public osg::Referenced
    {
            ...
            run( void*            objectPtr,
                 ...
                 osg::Parameters& outputParameters ) const = 0;
    ...
    {
            virtual void
            objectRead( osgDB::InputStream& is,
                        osg::Object&        obj ) = 0;
    };
    ...
    class OSGDB_EXPORT ObjectWrapper : public osg::Referenced
    {
        public:
            ...
            typedef std::vector<osg::ref_ptr<FinishedObjectReadCallback>>
            ...
            void
            setUpdatedVersion( int ver )
            {
                _version = ver;
            }

            int
            getUpdatedVersion() const
            {
                return _version;
            }
            ...
            void
            addSerializer( BaseSerializer*      s,
                           BaseSerializer::Type t = BaseSerializer::RW_UNDEFINED );
            void
            markSerializerAsRemoved( const std::string& name );

            void
            markAssociateAsRemoved( const std::string& name );
            void
            markAssociateAsAdded( const std::string& name );

            BaseSerializer*
            getLastSerializer()
            {
                return _serializers.empty() ? 0 : _serializers.back().get();
            }
            ...
            void
            addFinishedObjectReadCallback( FinishedObjectReadCallback* forc )
            {
                _finishedObjectReadCallbacks.push_back( forc );
            }
            ...
            bool
            readSchema( const StringList& properties,
                        const TypeList&   types );
            void
            writeSchema( StringList& properties,
                         TypeList&   types );

            void
            resetSchema()
            {
                if( _backupSerializers.size() > 0 )
                {
                    _serializers = _backupSerializers;
                }
            }

            void
            addMethodObject( const std::string& methodName,
                             MethodObject*      mo );

            typedef std::multimap<std::string, osg::ref_ptr<MethodObject>>
                MethodObjectMap;

            MethodObjectMap&
            getMethodObjectMap()
            {
                return _methodObjectMap;
            }

            const MethodObjectMap&
            getMethodObjectMap() const
            {
                return _methodObjectMap;
            }
        ...
        protected:
            ...
            static void
            splitAssociates( const std::string&                    src,
    ...
    {
            UpdateWrapperVersionProxy( ObjectWrapper* w,
                                       int            v ) :
                _wrapper( w )
            {
    ...
    class OSGDB_EXPORT ObjectWrapperManager : public osg::Referenced
    {
        public:
            ...
            // Wrapper handlers
            void
            addWrapper( ObjectWrapper* wrapper );
            void
            removeWrapper( ObjectWrapper* wrapper );
            ObjectWrapper*
            findWrapper( const std::string& name );
            ...
            // Compressor handlers
            void
            addCompressor( BaseCompressor* compressor );
            void
            removeCompressor( BaseCompressor* compressor );
            BaseCompressor*
            findCompressor( const std::string& name );
            ...
            IntLookup::Value
            getValue( const std::string& group,
            ...
            const std::string&
            getString( const std::string& group,
        ...
        protected:
            ...
            std::recursive_mutex _wrapperMutex;

            WrapperMap           _wrappers;
            CompressorMap        _compressors;

            IntLookup&
            findLookup( const std::string& group )
            {
                IntLookupMap::iterator itr = _globalMap.find( group );
                if( itr != _globalMap.end() )
                {
                    return itr->second;
                }
                else
                {
                    return _globalMap["GL"];
                }
            }
    ...
    class OSGDB_EXPORT RegisterWrapperProxy
    {
        public:
            ...
            RegisterWrapperProxy( ObjectWrapper::CreateInstanceFunc* createInstanceFunc,
                                  const std::string&                 name,
                                  const std::string&                 associates,
                                  AddPropFunc                        func );
    ...
    class OSGDB_EXPORT RegisterCustomWrapperProxy
    {
        public:
            ...
            typedef void ( *AddPropFunc )( const char*,
                                           ObjectWrapper* );
...
#define REGISTER_OBJECT_WRAPPER( NAME, CREATEINSTANCE, CLASS, ASSOCIATES ) \
    ...
    static osgDB::RegisterWrapperProxy wrapper_proxy_##NAME(               \
        wrapper_createinstancefunc##NAME,                                  \
        #CLASS,                                                            \
        ASSOCIATES,                                                        \
        &wrapper_propfunc_##NAME                                           \
    );                                                                     \
