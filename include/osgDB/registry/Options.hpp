...
namespace osgDB
{
    ...
    /** Options base class used for passing options into plugins to control their
     * operation.*/
    class OSGDB_EXPORT Options : public osg::Inherit<osg::Object, Options>
    {
        public:

            /// bit mask for setting up which object types get cached by
            /// readObject/Image/HeightField/Node(filename) calls
            ...
            /// precision where source data is held in double precision This is useful
            /// for data that will be pre-processed before rendering. In general the
            /// geometry should be converted to floating point before rendering to ensure
            /// good performance.
            enum PrecisionHint
            {
            ...
            Options*
            cloneOptions( const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY ) const
            {
                return static_cast<Options*>( clone( copyop ) );
            }

            /** Set the general Options string.*/
            void
            setOptionString( const std::string& str )
            {
                _str = str;
            ...
            const std::string&
            getOptionString() const
            {
                return _str;
            }

            /** Set the database path to use a hint of where to look when loading
             * models.*/
            void
            setDatabasePath( const std::string& str )
            {
                _databasePaths.clear();
                _databasePaths.push_back( str );
            }

            /** Get the database path which is used a hint of where to look when loading
             * models.*/
            FilePathList&
            getDatabasePathList()
            {
            ...
            /** Get the const database path which is used a hint of where to look when
             * loading models.*/
            const FilePathList&
            getDatabasePathList() const
            {
                return _databasePaths;
            }
            ...
            void
            setObjectCacheHint( CacheHintOptions useObjectCache )
            {
            ...
            CacheHintOptions
            getObjectCacheHint() const
            {
                return _objectCacheHint;
            }
             ...
             */
            void
            setPrecisionHint( PrecisionHint hint )
            {
                _precisionHint = hint;
            }
             ...
             */
            void
            setBuildKdTreesHint( BuildKdTreesHint hint )
            {
            ...
            void
            setPluginData( const std::string& s,
                           void*              v ) const
            {
                _pluginData[s] = v;
            }
            ...
            void*
            getPluginData( const std::string& s )
            {
                return _pluginData[s];
            }

            /** Get a value from the PluginData */
            const void*
            getPluginData( const std::string& s ) const
            {
            ...
            void
            removePluginData( const std::string& s ) const
            {
                _pluginData.erase( s );
            }
            ...
            /** Sets a plugindata value PluginData with a string */
            void
            setPluginStringData( const std::string& s,
                                 const std::string& v ) const
            {
                _pluginStringData[s] = v;
            }

            /** Get a string from the PluginStrData */
            std::string&
            getPluginStringData( const std::string& s )
            {
                return _pluginStringData[s];
            }

            /** Get a value from the PluginData */
            const std::string
            getPluginStringData( const std::string& s ) const
            {
                PluginStringDataMap::const_iterator itr = _pluginStringData.find( s );
                return ( itr == _pluginStringData.end() ) ? std::string( "" )
                                                          : itr->second;
            }

            /** Remove a value from the PluginData */
            void
            removePluginStringData( const std::string& s ) const
            {
                _pluginStringData.erase( s );
            }

            /** Get number of PluginStrData values */
            unsigned int
            getNumPluginStringData() const
            {
                return static_cast<unsigned int>( _pluginStringData.size() );
            }

            /** Parse string into plugin string data. This will be automatically done in
             * Options(const std::string&) */
            void
            parsePluginStringData( const std::string& str,
                                   char               separator1 = ' ',
                                   char               separator2 = '=' );
            ...
            /** Set the callback to use inform the DatabasePager whether a file is
            ...
            /** Get the callback to use inform the DatabasePager whether a file is
             * located on local or remote file system.*/
            ...
            void
            setTerrain( osg::observer_ptr<osg::Node>& terrain )
            {
                _terrain = terrain;
            }

            /** Get the terrain observer_ptr, use to decorate any osgTerrain subgraphs.*/
            const osg::observer_ptr<osg::Node>&
            getTerrain() const
            {
                return _terrain;
            }

            /** Set the parentGroup observer_ptr, where the loaded model is intended to
             * be added */
            void
            setParentGroup( osg::observer_ptr<osg::Group>& parentGroup )
            {
                _parentGroup = parentGroup;
            }

            /** Get the parentGroup observer_ptr, where the loaded model is intended to
             * be added */
            const osg::observer_ptr<osg::Group>&
            ...
            bool
            operator<( const Options& rhs ) const;
            bool
            operator==( const Options& rhs ) const;
        ...
        protected:
            ...
            osg::observer_ptr<osg::Group>
                _parentGroup;    // Set by the DatabasePager to the node where the
                                 // requested file will be inserted. NOTE: observer since
                                 // prent can be dettached whilst DB thread is loading
                                 // the object
    };
