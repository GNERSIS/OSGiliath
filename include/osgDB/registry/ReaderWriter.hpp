 ...
 * Abstract base for file format plugins. Defines the interface for
 * reading and writing nodes, images, objects, and shaders.
 */
#pragma once
...
namespace osgDB
{
    ...
    class OSGDB_EXPORT ReaderWriter : public osg::Inherit<osg::Object, ReaderWriter>
    {
        public:
            ...
            ReaderWriter( const ReaderWriter& rw,
            ...
            /** Return which list of file environment variables supported by
             * ReaderWriter. */
            virtual const FormatDescriptionMap&
            supportedEnvironment() const
            {
                return _supportedEnvironment;
            }
            ...
            virtual bool
            acceptsProtocol( const std::string& protocol ) const;

            /// Bit mask for setting up which feature types are available for read and/or
            /// write
            enum Features
            {
                FEATURE_NONE               = 0,
            ...
            static FeatureList
            featureAsString( Features feature );
            ...
            class OSGDB_EXPORT ReadResult
            {
                public:
                    ...
                    enum ReadStatus
                    {
                        NOT_IMPLEMENTED,     //!< read*() method not implemented in
                                             //!< concrete ReaderWriter.
                        FILE_NOT_HANDLED,    //!< File is not appropriate for this file
                                             //!< reader, due to some incompatibility,
                                             //!< but *not* a read error.
                        FILE_NOT_FOUND,      //!< File could not be found or could not be
                                             //!< read.
                        ERROR_IN_READING_FILE,    //!< File found, loaded, but an error
                        ...
                        FILE_REQUESTED,            //!< Asynchronous file read has been
                                           //!< requested, but returning immediately,
                                           //!< keep polling plugin until file read has
                                           //!< been completed.
                        INSUFFICIENT_MEMORY_TO_LOAD,    //!< File found but not loaded
                                                        //!< because estimated required
                                                        //!< memory surpasses available
                                                        //!< memory.
                    };
                    ...
                    template<class T>
                    ReadResult( const osg::ref_ptr<T>& obj,
                                ReadStatus             status = FILE_LOADED ) :
                        _status( status ),
                        _object( obj.get() )
                    {
                    ...
                    ReadResult( const ReadResult& rr ) :
                        _status( rr._status ),
                        _message( rr._message ),
                        _object( rr._object )
                    {
                    }

                    ReadResult&
                    operator=( const ReadResult& rr )
                    {
                        if( this == &rr )
                        {
                            return *this;
                    ...
                    bool
                    operator<( const ReadResult& rhs ) const
                    {
                    ...
                    /// report the ReadResult's status, and message (if any). Useful for
                    ...
                    bool
                    loadedFromCache() const
                    {
                        return _status == FILE_LOADED_FROM_CACHE;
                    }
                    ...
                    notFound() const
                    {
                        return _status == FILE_NOT_FOUND;
                    }

                    bool
                    notEnoughMemory() const
                    {
                        return _status == INSUFFICIENT_MEMORY_TO_LOAD;
                    }
            ...
            class WriteResult
            {
                public:
                    ...
                    enum WriteStatus
                    {
                        NOT_IMPLEMENTED,    //!< write*() method not implemented in
                                            //!< concrete ReaderWriter.
                        FILE_NOT_HANDLED,
                    ...
                    bool
                    operator<( const WriteResult& rhs ) const
                    {
                        return _status < rhs._status;
                    }
                    ...
                    /// Report the WriteResult's status, and message (if any). Useful for
                    /// reporting of errors to users.
                    std::string
                    statusMessage() const;
            ...
            /** Determine if a file exists, normally the default implementation will be
             * appropriate for local file access but with plugins like the libcurl based
             * one it will return true if the file is accessible at the server. */
            virtual bool
            fileExists( const std::string& filename,
                        const Options*     options ) const;

            /** Open an archive for reading, writing, or to create an empty archive for
             * writing to.*/
            virtual ReadResult
            ...
            virtual WriteResult
            writeScript( const osg::Script& /*script*/,
                         const std::string& /*fileName*/,
                         const Options* = NULL ) const
            {
                return WriteResult( WriteResult::NOT_IMPLEMENTED );
            ...
            virtual ReadResult
            readHeightField( std::istream& /*fin*/,
                             const Options* = NULL ) const
            {
                return ReadResult( ReadResult::NOT_IMPLEMENTED );
            }
             ...
             * Please note, this method should usually only be used internally by
             * subclasses of ReaderWriter, Only in special cases will a ReaderWriter
             * implementation be able to handle a protocol format that it wasn't
             * originally designed for. To know whether it's safe to inject a new
             * protocol format into an existing ReaderWriter you will need to review the
             * source code and dependencies of that ReaderWriter. */
            void
            supportsProtocol( const std::string& fmt,
                              const std::string& description );

            /** Specify ext string as a supported file extension.
             * Please note, this method should usually only be used internally by
             * subclasses of ReaderWriter. Only in special cases will a ReaderWriter
             * implementation be able to handle a file extension that it wasn't
             * originally designed for. To know whether it's safe to inject a new file
             * extension into an existing ReaderWriter you will need to review the the
             * source code and dependencies of that ReaderWriter. */
            void
            supportsExtension( const std::string& ext,
                               const std::string& description );

            /** Specify option string as a supported option string.
             * Please note, this should usually only be used internally by subclasses of
             * ReaderWriter. */
            void
            supportsOption( const std::string& opt,
                            const std::string& description );

            /** Specify env string as a supported environment string. */
            void
            supportsEnvironment( const std::string& opt,
                                 const std::string& description );
