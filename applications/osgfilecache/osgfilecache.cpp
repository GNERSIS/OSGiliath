...
static std::atomic<unsigned> s_ExitApplication;
static std::atomic<unsigned> s_SigValue;
...
struct Extents
{
        ...
        bool
        intersects( unsigned    level,
                    osg::dvec2& in_min,
                    osg::dvec2& in_max )
        {
            osg::notify( osg::INFO ) << "intersects(" << level << ", min=" << in_min
                                     << " max=" << in_max << ")" << std::endl;
            osg::notify( osg::INFO ) << "  _maxLevel=" << _maxLevel << ", _min=" << _min
                                     << " _max=" << _max << std::endl;
...
class LoadDataVisitor : public osg::NodeVisitor
{
    public:
        ...
        void
        setFileCache( osgDB::FileCache* fileCache )
        {
            _fileCache = fileCache;
        }

        void
        addExtents( unsigned int maxLevel,
                    double       minX,
                    double       minY,
                    double       maxX,
                    double       maxY )
        ...
        addExtents( unsigned int maxLevel )
        {
            _extentsList.push_back(
                Extents( maxLevel, DBL_MAX, DBL_MAX, -DBL_MAX, -DBL_MAX )
            );
        }

        void
        apply( osg::CoordinateSystemNode& cs )
        {
        ...
        apply( osg::Group& group )
        {
            ...
            if( locator )
            {
                osg::dvec3 l00( 0.0, 0.0, 0.0 );
                osg::dvec3 l10( 1.0, 0.0, 0.0 );
                osg::dvec3 l11( 1.0, 1.0, 0.0 );
                osg::dvec3 l01( 0.0, 1.0, 0.0 );

                osg::dvec3 w00, w10, w11, w01;
                ...
                if( locator->getEllipsoidModel() &&
                    locator->getCoordinateSystemType() ==
                    osgTerrain::Locator::GEOCENTRIC )
                {
        ...
        apply( osg::PagedLOD& plod )
        {
            ...
            if( intersects() )
            {
                for( unsigned int i = 0; i < plod.getNumFileNames(); ++i )
                {
                    osg::notify( osg::INFO ) << "   filename[" << i << "] "
                                             << plod.getFileName( i ) << std::endl;
                    if( !plod.getFileName( i ).empty() )
                    {
                        std::string filename;
                        ...
                        osg::ref_ptr<osg::Node> node =
                            readNodeFileAndWriteToCache( filename );
        ...
        readNodeFileAndWriteToCache( const std::string& filename )
        {
            ...
            if( _fileCache.valid() )
            {
                if( _fileCache->existsInCache( filename ) )
                {
                    osg::notify( osg::NOTICE )
                        << "reading from FileCache: " << filename << std::endl;
                    node = _fileCache
                               ->readNode( filename,
                ...
                else
                {
                    ...
                    {
                        osg::notify( osg::NOTICE )
                            << "write to FileCache : " << filename << std::endl;

                        _fileCache->writeNode(
                            *node,
                            filename,
                            osgDB::Registry::instance()->getOptions()
                        );
    ...
    protected:
        ...
        void
        convertXYZToLatLongHeight( osg::EllipsoidModel* em,
                                   osg::dvec3&          v )
        {
            em->convertXYZToLatLongHeight( v.x, v.y, v.z, v.y, v.x, v.z );
        ...
        void
        updateBound( osg::Vec3Array& vertices )
        {
            // set up matrix
            osg::dmat4 matrix;
            if( !_matrixStack.empty() )
            ...
            osg::EllipsoidModel* em =
                !_csnStack.empty() ? _csnStack.back()->getEllipsoidModel() : 0;

            for( osg::Vec3Array::iterator itr = vertices.begin(); itr != vertices.end();
                 ++itr )
            {
                osg::dvec3 v = osg::dvec3( *itr ) * matrix;
                if( em )
                {
        ...
        {
            osg::notify( osg::INFO )
                << "intersects() _min = " << _min << " _max = " << _max << std::endl;
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        arguments.getApplicationName() +
        " is an application for collecting a set of separate files into a single "
        "archive file that can be later read in OSG applications.."
    );
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "-l level",
        "Read down to level across the whole database."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-e level minX minY maxX maxY",
        "Read down to <level> across the extents minX, minY to maxY, maxY.  Note, for "
        "geocentric datase X and Y are longitude and latitude respectively."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-c directory",
        "Shorthand for --file-cache directory."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--file-cache directory",
        "Set directory as to place cache download files."
    );
    ...
    while( arguments.read( "--file-cache", fileCachePath ) ||
           arguments.read( "-c", fileCachePath ) )
    {
    }
    ...
    if( fileCachePath.empty() )
    {
        std::cout << "No path to the file cache defined, please set OSG_FILE_PATH env "
                     "var, or use --file-cache <directory> to set a suitable directory "
                     "for the file cache."
                  << std::endl;
        return 1;
    }
    ...
    if( s_ExitApplication )
    {
        std::cout << "osgfilecache exited in response to signal : " << s_SigValue
                  << std::endl;
    }
