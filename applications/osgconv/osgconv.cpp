...
#include <osgDB/registry/PluginQuery.hpp>
    ...
    public:
        ...
        MyGraphicsContext()
        {
            ...
            if( _gc.valid() )
            {
                ...
                {
                    std::cout << "Realized graphics window for OpenGL operations."
                              << std::endl;
                }
                else
                {
...
class DefaultNormalsGeometryVisitor : public osg::NodeVisitor
{
    public:
        ...
        apply( osg::Geode& geode )
        {
            for( unsigned int ii = 0; ii < geode.getNumDrawables(); ++ii )
            {
                ...
                if( geometry.valid() )
                {
                    ...
                    newnormals->push_back( osg::vec3( 0.0F, 0.0F, 1.0F ) );
        ...
        compress()
        {
            ...
            if( !context.valid() )
            {
                osg::notify( osg::NOTICE )
                    << "Error: Unable to create graphis context, problem with running "
                       "osgViewer-"
                    << osgViewerGetVersion() << ", cannot run compression." << std::endl;
                return;
            }
                 ...
                 ++itr )
            {
                    ...
                    ( image->s() >= 32 && image->t() >= 32 ) )
                {
                    ...
                    // need to disable the unref after apply, otherwise the image could
                    // go out of scope.
        ...
        write( const std::string& dir )
        {
                 ...
                 ++itr )
            {
                ...
                if( image.valid() )
                {
                    ...
                    std::string path =
                        dir.empty() ? name : osgDB::concatPaths( dir, name );
                    osgDB::writeImageFile( *image, path );
                    osg::notify( osg::NOTICE )
                        << "Image written to '" << path << "'." << std::endl;
                }
            }
        }
...
class FixTransparencyVisitor : public osg::NodeVisitor
{
    public:
        ...
        FixTransparencyVisitor( FixTransparencyMode mode =
                                    MAKE_OPAQUE_TEXTURE_STATESET_OPAQUE ) :
            osg::NodeVisitor( osg::NodeVisitor::TRAVERSE_ALL_CHILDREN ),
            _numTransparent( 0 ),
            _numOpaque( 0 ),
        ...
        {
            std::cout << "  Number of Transparent StateSet " << _numTransparent
                      << std::endl;
        ...
        virtual bool
        isTransparent( osg::StateSet& stateset )
        {
            bool hasTranslucentTexture = false;
            bool hasBlendFunc =
                dynamic_cast<osg::BlendFunc*>(
                    stateset.getAttribute( osg::StateAttribute::Type::BLENDFUNC )
                ) != 0;
            bool hasTransparentRenderingHint =
                stateset.getRenderingHint() == osg::StateSet::TRANSPARENT_BIN;
            bool hasDepthSortBin =
                ( stateset.getRenderBinMode() == osg::StateSet::USE_RENDERBIN_DETAILS )
                    ? ( stateset.getBinName() == "DepthSortedBin" )
                    : false;
            ...
            if( hasTranslucentTexture ||
                hasBlendFunc ||
                hasTransparentRenderingHint ||
                hasDepthSortBin )
            {
                ++_numTransparent;
                ...
                {
                    stateset.removeAttribute( osg::StateAttribute::Type::BLENDFUNC );
...
class PruneStateSetVisitor : public osg::NodeVisitor
{
    public:
        ...
        ~PruneStateSetVisitor()
        {
            std::cout << "  Number of StateState removed " << _numStateSetRemoved
                      << std::endl;
        }
...
class AddMissingColoursToGeometryVisitor : public osg::NodeVisitor
{
    public:
        ...
        AddMissingColoursToGeometryVisitor() :
            osg::NodeVisitor( osg::NodeVisitor::TRAVERSE_ALL_CHILDREN )
        {
        }
       ...
       const char* msg )
{
    ...
    osg::notify( osg::NOTICE )
        << "    " << prog << " [options] infile1 [infile2 ...] outfile" << std::endl;
    osg::notify( osg::NOTICE ) << std::endl;
    ...
    osg::notify( osg::NOTICE )
        << "    -O option          - ReaderWriter option" << std::endl;
    osg::notify( osg::NOTICE ) << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify( osg::NOTICE ) << std::endl;
    osg::notify( osg::NOTICE )
        << "    --fix-transparency - fix statesets which are currently" << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify(
        osg::NOTICE
    ) << "    --fix-transparency-mode <mode_string>  - fix statesets which are currently"
      << std::endl;
    osg::notify( osg::NOTICE )
        << "                         declared as transparent but should be opaque."
        << std::endl;
    osg::notify( osg::NOTICE )
        ...
        << std::endl;
    osg::notify( osg::NOTICE )
        << "                         to fix the transparency, options are:" << std::endl;
    osg::notify( osg::NOTICE )
        << "                                 MAKE_OPAQUE_TEXTURE_STATESET_OPAQUE,"
        << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify( osg::NOTICE ) << std::endl;
    osg::notify( osg::NOTICE )
        << "    -l libraryName     - load plugin of name libraryName" << std::endl;
    osg::notify( osg::NOTICE )
        << "                         i.e. -l osgdb_pfb" << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify(
      ...
      << std::endl;
    osg::notify( osg::NOTICE ) << "                         done with -l above, as it "
                                  "automatically expands to the"
                               << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify(
        osg::NOTICE
    ) << "                         Format of orientation argument must be the "
         "following:\n"
         "\n"
         "                             X1,Y1,Z1-X2,Y2,Z2\n"
         "                         or\n"
         ...
         "                         convert a model built in a Y-Up coordinate system to "
         "a\n"
         "                         model with a Z-up coordinate system, the argument "
         "may\n"
         "                         look like\n"
         "\n"
         "                             0,1,0-0,0,1"
    ...
    osg::notify(
        osg::NOTICE
    ) << "    -t translation     - Convert spatial position of output files.  Format "
         "of\n"
    ...
    osg::notify(
        osg::NOTICE
    ) << "    --use-world-frame  - Perform transformations in the world frame, rather\n"
         "                         than relative to the center of the bounding sphere.\n"
      << std::endl;
    osg::notify( osg::NOTICE )
        << "    --simplify n       - Run simplifier prior to output. Argument must be a"
        << std::endl
    ...
    osg::notify( osg::NOTICE ) << "    --smooth           - Smooth the surface by "
                               ...
                               << std::endl;
    osg::notify( osg::NOTICE )
        << "    --addMissingColors - Add a white color value to all geometry nodes\n"
           "                         that don't have their own color values\n"
           "                         (--addMissingColours also accepted)."
        << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify( osg::NOTICE ) << std::endl;
    osg::notify( osg::NOTICE ) << "    --formats          - List all supported formats "
                                  "and their supported options."
                               << std::endl;
    osg::notify( osg::NOTICE )
        << "    --format <format>  - Display information about the specified <format>,\n"
           "                         where <format> is the file extension, such as "
           "\"flt\"."
        << std::endl;
    osg::notify( osg::NOTICE )
    ...
    osg::notify( osg::NOTICE )
        << "    --plugin <plugin>  - Display information about the specified <plugin>,\n"
           "                         where <plugin> is the plugin's full path and file "
           "name."
        << std::endl;
}
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->setDescription(
        arguments.getApplicationName() + " is a utility for converting between various "
                                         "input and output databases formats."
    );
    ...
    // arguments.getApplicationUsage()->addCommandLineOption("--formats","List supported
    // file formats");
    // arguments.getApplicationUsage()->addCommandLineOption("--plugins","List database
    // olugins");
    ...
    if( arguments.read( "--plugins" ) )
    {
        ...
        for( osgDB::FileNameList::iterator itr = plugins.begin(); itr != plugins.end();
             ++itr )
        {
    ...
    while( arguments.read( "-s", str ) )
    {
        ...
        if( sscanf( str.c_str(), "%f,%f,%f", &scale[0], &scale[1], &scale[2] ) != 3 )
        {
    ...
    while( arguments.read( "--simplify", str ) )
    {
        ...
        osg::notify( osg::INFO )
            << "Simplifying with percentage: " << simplifyPercent << std::endl;
        do_simplify = true;
    }
    ...
    while( arguments.read( "-t", str ) )
    {
        ...
        if( sscanf( str.c_str(), "%f,%f,%f", &trans[0], &trans[1], &trans[2] ) != 3 )
        {
    ...
    while( arguments.read( "--addMissingColours" ) ||
           arguments.read( "--addMissingColors" ) )
    {
    ...
    if( root.valid() )
    {
        ...
        osg::notify( osg::INFO ) << "Time to load files "
    ...
    if( root.valid() )
    {
        ...
        if( internalFormatMode != osg::Texture::USE_IMAGE_DATA_FORMAT )
        {
            ...
            if( ext !=
                "ive" ||
                ( options &&
                  options->getOptionString().find( "noTexturesInIVEFile" ) !=
                  std::string::npos ) )
            {
        ...
        osgDB::ReaderWriter::WriteResult result = osgDB::Registry::instance()->writeNode(
            *root,
            fileNameOut,
            osgDB::Registry::instance()->getOptions()
        );
        ...
        {
            osg::notify( osg::NOTICE )
                << "Data written to '" << fileNameOut << "'." << std::endl;
        }
        else if( result.message().empty() )
        {
            osg::notify( osg::NOTICE ) << "Warning: file write to '" << fileNameOut
