...
void
SpellChecker::checkP3dXml( const std::string& filename ) const
{
...
void
SpellChecker::checkXml( osgDB::XmlNode* node ) const
{
    if( node->name == "page" )
    {
...
void
SpellChecker::checkWords( const std::string& words ) const
{
...
bool
SpellChecker::isCorrect( const std::string& word ) const
{
...
SpellChecker::suggest( const std::string& /*word*/ ) const
{
    return WordList();
}
...
void
XmlPatcher::stripP3dXml( const std::string& filename,
...
void
XmlPatcher::stripXml( osgDB::XmlNode* node,
                      std::ostream&   fout ) const
{
    if( node->name ==
        "presentation" ||
        ...
        node->name == "bullet" )
    {
        ...
        {
            fout << "<" << node->name << ">" << node->contents << "</" << node->name
                 << ">" << std::endl;
        }
    }
    else
...
osgDB::XmlNode*
XmlPatcher::simplifyP3dXml( const std::string& filename ) const
{
    std::string foundFileName = osgDB::findDataFile( filename );
    if( foundFileName.empty() )
    {
...
osgDB::XmlNode*
XmlPatcher::simplifyXml( osgDB::XmlNode* node ) const
{
    if( node->name.empty() ||
        node->name ==
        "presentation" ||
...
osgDB::XmlNode*
XmlPatcher::mergeP3dXml( const std::string& lhs_filename,
                         const std::string& rhs_filename ) const
{
...
osgDB::XmlNode*
XmlPatcher::mergeXml( osgDB::XmlNode* lhs_node,
                      osgDB::XmlNode* rhs_node ) const
{
