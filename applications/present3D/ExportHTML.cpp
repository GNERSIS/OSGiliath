...
class SnapImageDrawCallback : public osg::Camera::DrawCallback
{
    public:
        ...
        void
        setSnapImageOnNextFrame( bool flag )
        {
            _snapImageOnNextFrame = flag;
        }

        bool
        getSnapImageOnNextFrame() const
        {
        ...
        operator()( const osg::Camera& camera ) const
        {
            ...
            unsigned int width =
                static_cast<unsigned int>( camera.getViewport()->width() );
            unsigned int height =
                static_cast<unsigned int>( camera.getViewport()->height() );
            ...
            if( osgDB::writeImageFile( *image, _filename ) )
            {
                osg::notify( osg::NOTICE )
                    << "Saved screen image to `" << _filename << "`" << std::endl;
            }
...
std::string
ExportHTML::createFileName( const std::string& basename,
                            unsigned int       page,
...
bool
ExportHTML::write( osgPresentation::SlideEventHandler* seh,
                   ...
                   const std::string&                  filename )
{
    ...
    std::cout << "Writing slides to " << image_basename << "_[slidenumber]" << image_ext
              << std::endl;
        ...
        if( !html_basename.empty() )
        {
            ...
            if( fout )
            {
                std::string previous_file =
                    i > 0 ? createFileName( html_basename, i - 1, html_ext ) : "";
                std::string next_file =
                    i < seh->getNumSlides() - 1
                        ? createFileName( html_basename, i + 1, html_ext )
                        : "";
                ...
                {
                    fout << "<td align=\"left\" width=\"33%\"><a href=\""
                         << osgDB::getSimpleFileName( previous_file )
                         << "\"> Previous </a></td>" << std::endl;
                }
                else
                {
                ...
                if( i != 0 )
                {
                    fout << "<td align=\"center\" width=\"33%\"><a href=\""
                         << osgDB::getSimpleFileName( home_file ) << "\"> Home </a></td>"
                         << std::endl;
                }
                else
                {
                ...
                if( !next_file.empty() )
                {
                    fout << "<td align=\"right\" width=\"33%\"><a href=\""
                         << osgDB::getSimpleFileName( next_file ) << "\"> Next </a></td>"
                         << std::endl;
                }
                else
                {
                ...
                fout << "</tr>" << std::endl;
                fout << "</table>" << std::endl;
                fout << "<img src=\"" << osgDB::getSimpleFileName( os.str() ) << "\">"
                     << std::endl;
                fout << "</html>" << std::endl;
            }
            else
