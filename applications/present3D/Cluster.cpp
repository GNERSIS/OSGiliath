...
const unsigned int CameraPacket::SWAP_BYTES_COMPARE = 0X12'34'56'78;

void
DataConverter::write( const osg::FrameStamp& fs )
{
...
void
DataConverter::read( osg::FrameStamp& fs )
{
...
void
DataConverter::write( const osg::dmat4& matrix )
{
...
void
DataConverter::read( osg::dmat4& matrix )
{
...
void
DataConverter::write( const osgGA::GUIEventAdapter& event )
{
...
void
DataConverter::read( osgGA::GUIEventAdapter& event )
{
...
void
DataConverter::write( CameraPacket& cameraPacket )
{
...
void
DataConverter::read( CameraPacket& cameraPacket )
{
...
// setsockopt( _so, SOL_SOCKET, SO_REUSEADDR, (const char*) &on, sizeof(int));
    ...
    if( result )
    {
        OSG_NOTICE << "Warning: Receiver::init() setsockopt(..) failed, errno=" << errno
                   << std::endl;
        return false;
    }
...
void
Receiver::setPort( const short port )
{
    _port = port;
}

void
Receiver::setBuffer( void*              buffer,
...
Receiver::sync( void )
{
    ...
    if( result < 0 )
    {
        OSG_NOTICE << "Warning: Receiver::sync() recvfrom(..) failed, errno=" << errno
                   << std::endl;
        return;
    }
...
Broadcaster::setHost( const char* hostname )
{
    ...
    {
        fprintf( stderr,
                 "Broadcaster::setHost() - Cannot resolv an address for \"%s\".\n",
                 hostname );
        _address = 0;
    }
    else
