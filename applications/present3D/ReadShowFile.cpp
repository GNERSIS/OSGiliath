    ...
    public:
        ...
        apply( osg::Group& group )
        {
            osgVolume::VolumeTile* volumeTile =
                dynamic_cast<osgVolume::VolumeTile*>( &group );
            if( volumeTile )
            {
                if( dynamic_cast<osgVolume::PropertyAdjustmentCallback*>(
                        volumeTile->getEventCallback()
                    ) == 0 )
                {
                    volumeTile->addEventCallback(
...
bool
p3d::getFileNames( osg::ArgumentParser& arguments,
                   FileNameList&        xmlFiles,
                   FileNameList&        normalFiles )
{
...
bool
p3d::readEnvVars( osg::ArgumentParser& arguments )
{
    bool readVars = false;
...
bool
p3d::readEnvVars( const std::string& fileName )
{
...
osgDB::Options*
createOptions( const osgDB::ReaderWriter::Options* options )
{
...
p3d::readHoldingSlide( const std::string& filename )
{
    std::string ext = osgDB::getFileExtension( filename );
    if( !osgDB::equalCaseInsensitive( ext, "xml" ) &&
        !osgDB::equalCaseInsensitive( ext, "p3d" ) )
...
osg::ref_ptr<osg::Node>
p3d::readPresentation( const std::string&                  filename,
...
osg::ref_ptr<osg::Node>
p3d::readShowFiles( osg::ArgumentParser&                arguments,
                    const osgDB::ReaderWriter::Options* options )
{
    osg::ref_ptr<osgDB::Options> local_options = createOptions( options );
    local_options->setOptionString( "main" );
    ...
    while( arguments.read( "--image", filename ) )
    {
        osg::ref_ptr<osg::Image> image =
            readRefImageFile( filename.c_str(), local_options.get() );
        if( image.valid() )
        {
            nodeList.push_back( osg::createGeodeForImage( image ) );
        }
    }
    ...
    while( arguments.read( "--dem", filename ) )
    {
        osg::ref_ptr<osg::HeightField> hf =
            readRefHeightFieldFile( filename.c_str(), local_options.get() );
        if( hf )
        {
    ...
    for( int pos = 1; pos < arguments.argc(); ++pos )
    {
        if( !arguments.isOption( pos ) )
        {
            ...
            osg::ref_ptr<osg::Node> node =
                osgDB::readRefNodeFile( arguments[pos], local_options.get() );
