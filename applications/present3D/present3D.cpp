...
#include "PointsEventHandler.hpp"
#include "ReadShowFile.hpp"
...
void
setViewer( osgViewer::Viewer& viewer,
           float              width,
           float              height,
           float              distance )
{
    ...
    OSG_INFO << "setProjectionMatrixAsPerspective( " << vfov << ", "
             << width / height << ", " << 0.1 << ", " << 1000.0 << ");" << std::endl;
}
...
class ForwardToDeviceEventHandler : public osgGA::GUIEventHandler
{
    public:
        ...
        ForwardToDeviceEventHandler( osgGA::Device* device,
                                     bool           format_mouse_events ) :
            osgGA::GUIEventHandler(),
            _device( device ),
            _forwardMouseEvents( format_mouse_events )
        {
        }
...
class DumpEventHandler : public osgGA::GUIEventHandler
{
    public:
                ...
                osg::NodeVisitor* )
        {
            ...
            std::cout << ea.getX() << "/" << ea.getY() << " " << ea.isMultiTouchEvent()
                      << std::endl;
            return false;
        }
...
void
processLoadedModel( osg::ref_ptr<osg::Node>& loadedModel,
...
void
addDeviceTo( osgViewer::Viewer& viewer,
             ...
             bool               forward_mouse_events )
{
    ...
    if( dev.valid() )
    {
        ...
        {
            viewer.getEventHandlers().push_front(
                new ForwardToDeviceEventHandler( dev.get(), forward_mouse_events )
            );
        }
    }
    else
      ...
      char** argv )
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption(
        "-a",
        "Turn auto stepping on by default"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-d <float>",
        "Time duration in seconds between layers/slides"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "-s <float> <float> <float>",
        "width, height, distance and of the screen away from the viewer"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--viewer",
        "Start Present3D as the viewer version."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--authoring",
        "Start Present3D as the authoring version, license required."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--master",
        "Start Present3D as the master version, license required."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--slave",
        "Start Present3D as the slave version, license required."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--publishing",
        "Start Present3D as the publishing version, license required."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--timeDelayOnNewSlideWithMovies",
        "Set the time delay on new slide with movies, done to allow movie threads to "
        "get in sync with rendering thread."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--targetFrameRate",
        "Set the target frame rate, defaults to 80Hz."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--version",
        "Report the Present3D version."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--print <filename>",
        "Print out slides to a series of image files."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--html <filename>",
        "Print out slides to a series of html & image files."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--loop",
        "Switch on looping of presentation."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--devices",
        "Print the Video input capability via QuickTime and exit."
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--forwardMouseEvents",
        "forward also mouse/touch-events to the devices"
    );
    arguments.getApplicationUsage()->addCommandLineOption(
        "--suppressEnvTags",
        "suppresses all found ENV-tags in the presentation"
    );
    ...
    // read any env vars from presentations before we create viewer to make sure the
    // viewer utilises these env vars
    ...
    if( arguments.read( "--http-control" ) )
    {
        ...
        osg::ref_ptr<osgDB::Options> device_options =
            new osgDB::Options( "documentRegisteredHandlers" );

        osg::ref_ptr<osgGA::Device> rest_http_device = osgDB::readRefFile<osgGA::Device>(
            server_address + ":" + server_port + "/" + document_root + ".resthttp",
            device_options.get()
        );
    ...
    osg::ref_ptr<osgGA::StateSetManipulator> ssManipulator =
...

    {
        ...
        osg::NodeVisitor::TraversalMode updateTraversalMode = osg::NodeVisitor::
    ...
    // register the slide event handler - which moves the presentation from slide to
    // slide, layer to layer.
    osg::ref_ptr<osgPresentation::SlideEventHandler> seh =
        new osgPresentation::SlideEventHandler( &viewer );
    ...
    while( arguments.read( "--timeDelayOnNewSlideWithMovies",
                           timeDelayOnNewSlideWithMovies ) )
    {
    }
    ...
    osg::ref_ptr<osgViewer::ScreenCaptureHandler::WriteToFile> writeFile =
    ...
    screenCaptureHandler->setKeyEventTakeScreenShot(
        'm'
    ...
    while( arguments.read( "--version" ) )
    {
        ...
        osg::notify( osg::NOTICE )
            << "Present3D " << appTypeName << " version : " << s_version << std::endl;
        osg::notify( osg::NOTICE ) << std::endl;
    ...
    if( !p3d::getFileNames( arguments, xmlFiles, normalFiles ) )
    {
        osg::notify( osg::NOTICE ) << std::endl;
        osg::notify( osg::NOTICE )
            << "No file specified, please specify and file to load." << std::endl;
        osg::notify( osg::NOTICE ) << std::endl;
        return 1;
    }
    ...
    osg::ref_ptr<osgDB::ReaderWriter::Options> cacheAllOption =
    ...
    osg::ref_ptr<osg::Node> loadedModel = p3d::readShowFiles(
        arguments,
        cacheAllOption.get()
    ...
    {
        // have to add a frame in here to avoid problems with X11 threading issue on
        // switching off the cursor not yet sure why it makes a difference, but it at
        // least fixes the crash that would otherwise occur under X11.
        viewer.frame();
    ...
    if( exportName.empty() )
    {
        ...
        DataConverter scratchPad( 1'024 );

        while( !viewer.done() && !masterKilled )
        {
            osg::Timer_t startFrameTick = osg::Timer::instance()->tick();

            if( viewer.getRunFrameScheme() !=
                osgViewer::ViewerBase::ON_DEMAND ||
                seh->checkNeedToDoFrame() )
            {
                // do the normal frame.
                ...
                if( P3DApplicationType == MASTER )
                {
                    ...
                    osg::dmat4 modelview( viewer.getCamera()->getViewMatrix() );
                ...
                else if( P3DApplicationType == SLAVE )
                {
                    ...
                    if( cp.getMasterKilled() )
                    {
                        ...
                        // break out of while (!done) loop since we've now want to shut
                        // down.
                        masterKilled = true;
                    }
                }
            ...
            {
                osg::Thread::microSleep( static_cast<unsigned int>(
                    1000000.0 * ( targetFrameTime - frameTime )
                ) );
            }
        }
    }
