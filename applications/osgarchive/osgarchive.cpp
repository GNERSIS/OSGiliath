      ...
      char** argv )
{
    ...
    while( arguments.read( "-a", archiveFilename ) ||
           arguments.read( "--archive", archiveFilename ) )
    {
    }
    ...
    {
        std::cout << "Please specify an archive name using --archive filename"
                  << std::endl;
        return 1;
    }
    ...
    {
        std::cout << "Please specify an operation on the archive, either --insert, "
                     "--extract or --list"
                  << std::endl;
        return 1;
    }
    ...
    if( insert && extract )
    {
        std::cout << "Cannot insert and extract files from the archive at one time, "
                     "please use either --insert or --extract."
                  << std::endl;
        return 1;
    }
    ...
    if( insert )
    {
        ...
        if( archive.valid() )
        {
            for( FileNameList::iterator itr = files.begin(); itr != files.end(); ++itr )
            {
